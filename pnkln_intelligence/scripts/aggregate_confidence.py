...
class ConfidenceAggregator:
  ...
  def extract_recommendations(self) -> list[str]:
    ...
    rec_section = re.search(
      r"## (?:10\.|Recommended Actions|Recommendations)(.*?)(?=##|$)",
      self.report_text,
    ...
    self.recommendations = [
      f"[{priority}] {desc.strip()}" for priority, desc in matches
    ]
  ...
  def generate_summary(self) -> str:
    ...
    status_emoji = (
      "✅" if self.overall_confidence >= self.CONFIDENCE_THRESHOLD_APPROVED else "❌"
    )
    status_text = (
      "APPROVED"
...
"""
    ...
    if self.overall_confidence < self.CONFIDENCE_THRESHOLD_APPROVED:
      ...
      low_scores = [
        (dim, score)
        for dim, score in self.dimension_scores.items()
        if score < self.CONFIDENCE_THRESHOLD_APPROVED
      ]
  ...
  def export_metrics(self) -> dict:
    ...
    return {
      ...
      "thresholds": {
        "approved": self.CONFIDENCE_THRESHOLD_APPROVED,
        "warning": self.CONFIDENCE_THRESHOLD_WARNING,
      },
    }
...
def main():
  ...
  try:
    ...
    sys.exit(
      0
      if aggregator.overall_confidence >= aggregator.CONFIDENCE_THRESHOLD_APPROVED
      else 1
    )
