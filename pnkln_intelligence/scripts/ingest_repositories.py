...
class RepositoryIngestionPipeline:
  ...
  async def store_repository_metadata(
    self, repo_config: dict[str, Any], flattened, gcs_path: str
  ):
  ...
  async def ingest_all(
    self, filter_priority: str = None, filter_category: str = None, limit: int = None
  ):
    """
    ...
    if filter_priority:
      ...
      logger.info(
        f"Filtered to {len(repos)} repositories with priority={filter_priority}"
      )

    if filter_category:
      repos = [r for r in repos if r.get("category") == filter_category]
      logger.info(
        f"Filtered to {len(repos)} repositories with category={filter_category}"
      )
...
async def main():
  ...
  parser = argparse.ArgumentParser(
    description="Ingest repositories for PNKLN Intelligence Pipeline"
  )
  parser.add_argument(
    "--priority",
  ...
  try:
    await pipeline.ingest_all(
      filter_priority=args.priority, filter_category=args.category, limit=args.limit
    )
  finally:
