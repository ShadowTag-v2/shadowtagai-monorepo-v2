...
class InfrastructureInitializer:
    ...
    def init_bigquery(self):
        ...
        schema_path = Path(__file__).parent.parent / "infrastructure" / "bigquery_schemas.sql"
    ...
    def init_gcs(self):
        ...
        try:
            ...
            if not bucket.exists():
                bucket = self.gcs_client.create_bucket(bucket_name, location=self.settings.gcp.location)
            ...
            lifecycle_path = Path(__file__).parent.parent / "infrastructure" / "gcs_lifecycle_policy.json"
    ...
    async def init_vertex_ai(self):
        ...
        try:
            aiplatform.init(project=self.settings.gcp.project_id, location=self.settings.gcp.location)

            logger.info("Vertex AI initialized successfully")
            logger.info("Note: Index and endpoint creation should be done after embeddings are ready")
            logger.info("Use VectorSearchManager.create_index() and create_index_endpoint() when ready")
    ...
    async def run_all(self):
        ...
        logger.info("=" * 70)
        logger.info("\nNext steps:")
        logger.info("1. Start ingesting repositories: python -m pnkln_intelligence.scripts.ingest_repos")
        logger.info("2. Discover arXiv papers: python -m pnkln_intelligence.scripts.discover_papers")
        logger.info("3. Generate embeddings: python -m pnkln_intelligence.scripts.generate_embeddings")
        logger.info("4. Create vector search index: python -m pnkln_intelligence.scripts.create_index")
