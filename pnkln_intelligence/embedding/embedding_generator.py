...
class EmbeddingGenerator:
  ...
  async def generate_embedding(
    self, text: str, model: str | None = None, metadata: dict[str, Any] | None = None
  ) -> Embedding:
    """
    ...
    for i in range(0, len(texts), batch_size):
      ...
      if self.settings.provider == "openai":
        batch_embeddings = await self._generate_openai_embeddings_batch(
          batch_texts, model, batch_metadata
        )
      elif self.settings.provider == "voyage":
        batch_embeddings = await self._generate_voyage_embeddings_batch(
          batch_texts, model, batch_metadata
        )
      elif self.settings.provider == "codebert":
        batch_embeddings = await self._generate_codebert_embeddings_batch(
          batch_texts, model, batch_metadata
        )
      else:
        raise ValueError(f"Unsupported provider: {self.settings.provider}")
  ...
  async def _generate_openai_embedding(
    self, text: str, model: str, metadata: dict[str, Any]
  ) -> Embedding:
  ...
  async def _generate_openai_embeddings_batch(
    self, texts: list[str], model: str, metadata_list: list[dict[str, Any]]
  ) -> list[Embedding]:
  ...
  async def _generate_voyage_embedding(
    self, text: str, model: str, metadata: dict[str, Any]
  ) -> Embedding:
  ...
  async def _generate_voyage_embeddings_batch(
    self, texts: list[str], model: str, metadata_list: list[dict[str, Any]]
  ) -> list[Embedding]:
  ...
  async def _generate_codebert_embedding(
    self, text: str, model: str, metadata: dict[str, Any]
  ) -> Embedding:
    ...
    return Embedding(
      ...
      model=model,
      dimensions=len(embedding_vector),
      metadata={**metadata, "provider": "codebert"},
    )

  async def _generate_codebert_embeddings_batch(
    self, texts: list[str], model: str, metadata_list: list[dict[str, Any]]
  ) -> list[Embedding]:
  ...
  async def generate_code_embeddings(
    self, code_chunks: list[str], language: str, repo_name: str
  ) -> list[Embedding]:
    """
    ...
    metadata_list = [
      {"type": "code", "language": language, "repo": repo_name, "chunk_index": i}
      for i in range(len(code_chunks))
    ]

    return await self.generate_embeddings_batch(
      code_chunks, metadata_list=metadata_list
    )
...
if __name__ == "__main__":
  ...
  async def main():
    ...
    embedding = await generator.generate_embedding(
      code, metadata={"type": "code", "language": "python"}
    )
