...
class CodeChunker:
  ...
  def __init__(
    self, max_tokens: int = 1500, overlap_tokens: int = 300, min_chunk_tokens: int = 50
  ):
  ...
  def chunk_code(
    self, code: str, language: str, file_path: str | None = None
  ) -> list[CodeChunk]:
    """
    ...
    if chunks:
      logger.debug(
        f"Chunked {file_path or 'code'} into {len(chunks)} function-level chunks"
      )
      return chunks
  ...
  def _chunk_fixed_size(self, code: str, language: str) -> list[CodeChunk]:
    ...
    for line_num, line in enumerate(lines, start=1):
      ...
      if current_tokens + line_tokens > self.max_tokens and current_chunk_lines:
        ...
        current_chunk_lines = (
          current_chunk_lines[-overlap_line_count:] if overlap_line_count > 0 else []
        )
