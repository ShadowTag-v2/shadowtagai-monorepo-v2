...
class RepositoryFlattener:
  ...
  async def flatten_with_gitingest(
    self, repo_url: str, access_token: str | None = None
  ) -> FlattenedRepository:
    """
  ...
  async def flatten_repository(
    self, repo_url: str, tool: str | None = None, **kwargs
  ) -> FlattenedRepository:
    """
    ...
    if tool == "repomix":
      return await self.flatten_with_repomix(
        ...
        enable_compression=kwargs.get(
          "enable_compression", self.settings.repomix_compression
        ),
        run_security_scan=kwargs.get(
          "run_security_scan", self.settings.include_security_scan
        ),
      )
    elif tool == "gitingest":
      return await self.flatten_with_gitingest(
        repo_url, access_token=kwargs.get("access_token")
      )
    else:
      raise ValueError(f"Unknown flattening tool: {tool}")

  async def flatten_multiple_repositories(
    self, repo_urls: list[str], concurrency: int = 3
  ) -> list[FlattenedRepository]:
    """
    ...
    logger.info(
      f"Successfully flattened {len(successful_results)}/{len(repo_urls)} repositories"
    )
  ...
  async def main():
    ...
    try:
      ...
      repo = await flattener.flatten_repository(
        "https://github.com/anthropics/anthropic-sdk-python", tool="repomix"
      )
