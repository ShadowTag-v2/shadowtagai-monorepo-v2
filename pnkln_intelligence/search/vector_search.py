...
class VectorSearchManager:
  ...
  async def create_index(
    ...
    contents_delta_uri = (
      contents_delta_uri or f"gs://{self.settings.gcs_bucket_processed}/embeddings/"
    )
  ...
  async def create_index_endpoint(
    ...
    description: str = "Code search vector endpoint",
  ...
  async def deploy_index(
    ...
    logger.info(
      f"Deploying index {index.display_name} to endpoint {index_endpoint.display_name}"
    )
  ...
  async def batch_search(
    ...
    for neighbor_list in response:
      ...
      for neighbor in neighbor_list:
        results.append(
          SearchResult(id=neighbor.id, distance=neighbor.distance, metadata={})
        )
  ...
  async def update_index(
    self, contents_delta_uri: str, is_complete_overwrite: bool = False
  ) -> None:
    """
    ...
    self.index.update_embeddings(
      contents_delta_uri=contents_delta_uri, is_complete_overwrite=is_complete_overwrite
    )
