...
2.  **The Swarm (Flying Monkeys)**:
    *   Monkeys vote/discuss the prompt on the "Whiteboard".
    *   Monkeys use **Jetski** to research (Internet/Docs).
...
| **Flying Monkeys** | ✅ Active | `apps/flyingmonkeys-server` |
| **Jetski** | ✅ Active | `tools/jetski`, `jetski-sidecar` |
| **Aerial Scaffolding** | ⚠️ Partial | `libs/aiyou/bridge_client.py` exists but logic is manual. |
| **Cor.Self Loop** | ❌ Missing | No automated "4-loop" orchestrator exists in `libs/aiyou`. |

## 3. Implementation Plan
We need to create `libs/aiyou/recursive_intelligence.py` to strictly enforce this loop:
...
class CorSelfLoop:
    def __init__(self, iterations=4):
        ...
        self.swarm = FlyingMonkeysClient()
