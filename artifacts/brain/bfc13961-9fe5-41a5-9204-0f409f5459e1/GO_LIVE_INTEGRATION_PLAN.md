...
### 1. Stripe (Payments)
*   **Service:** `src/libs/aiyou/financial/stripe_wrapper.py`
*   **Features:** Subscription management for "God Mode Cycles".
*   **Webhook Handler:** `bin/flyingmonkeys-server.py` endpoint `/webhooks/stripe`.

### 2. Quicken Pro (Ledger)
*   **Export:** QIF/OFX generator for "Whale Data" ingestion.
*   **Location:** `src/libs/aiyou/financial/quicken_exporter.py`
