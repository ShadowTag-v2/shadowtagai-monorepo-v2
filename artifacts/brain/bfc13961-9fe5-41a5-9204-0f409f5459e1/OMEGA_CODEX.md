...
```dockerfile
# flyingmonkeys-server.Dockerfile
...
# 6. Ignite
# Runs the FlyingMonkeys Server (Tiered Gemini Logic)
CMD ["python", "bin/flyingmonkeys-server.py"]
```

---

## 2. The Brain (FlyingMonkeys Server)
...
app = FastAPI(title="FlyingMonkeys Sovereign Server")
...
```python
# src/libs/aiyou/agents/beads_agent.py
...
```python
# src/aiyou/services/gemini_failover.py
# The Safety Net.
...
3.  **Build**: `gcloud builds submit --tag gcr.io/shadowtag-omega-v2/flyingmonkeys`
4.  **Deploy**:
    ```bash
    gcloud run deploy antigravity-agent \
      --image gcr.io/shadowtag-omega-v2/flyingmonkeys \
      --platform managed \
      --region us-central1 \
