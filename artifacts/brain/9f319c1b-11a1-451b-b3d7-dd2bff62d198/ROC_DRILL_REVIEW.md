...
1. The `MonetizationMiddleware` enforces strict API Key validation (`X-AIYOU-API-KEY`) and token-burning tracking *only* if the incoming request path begins with the static `settings.api_prefix` (e.g. `/api/v1`).
...
2. **However, this is architecturally deliberate and secure.** Stripe webhooks inherently must hit public, unauthenticated ingress ports because they originate from Stripe servers which cannot attach custom `X-AIYOU-API-KEY` parameters dynamically.
