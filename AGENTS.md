...
`ShadowTag-v2/Monorepo-Uphillsnowball` is the one canonical root and the single source of truth.

The following are **folded-in components** — integrated sub-domains, not root truth:
...
These four live under `apps/pnkln-stack_stack/` as first-class citizens of the monorepo.
They are not root truth. They are not peers of the monorepo. The monorepo owns them.
