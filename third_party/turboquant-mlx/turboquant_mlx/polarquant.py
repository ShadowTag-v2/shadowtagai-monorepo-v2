...
class PolarQuantizer:
    ...
    def quantize(self, keys: mx.array) -> PolarQuantizedKV:
        ...
        keys_grouped = keys_rotated.reshape(batch, num_heads, num_blocks, self.group_size, head_dim)
        ...
        # Quantize
        r_quant = mx.clip(mx.floor((radius - r_min) / (r_scale + 1e-10)), 0, self.r_levels - 1).astype(mx.uint8)

        theta_quant = mx.clip(mx.floor((theta - theta_min) / (theta_scale + 1e-10)), 0, self.theta_levels - 1).astype(mx.uint8)
