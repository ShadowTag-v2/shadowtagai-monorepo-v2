...
def _check_dependencies():
    ...
    if not HAS_TRANSFORMERS:
        raise ImportError("transformers package is required for HuggingFace integration. Install it with: pip install transformers")
    if not HAS_TORCH:
        raise ImportError("torch package is required for HuggingFace integration. Install it with: pip install torch")
...
class TurboQuantHFCache(DynamicCache if HAS_TRANSFORMERS else object):
    ...
    def update(
        ...
        if current_len >= self.compress_after + self.group_size:
            ...
            compressed_keys, compressed_values = self._compress_layer(layer.keys, layer.values, layer_idx)
    ...
    def _polar_quantize_dequantize(self, x: "torch.Tensor") -> "torch.Tensor":
        ...
        r_quant = torch.clamp(torch.floor((radius - r_min) / (r_scale + 1e-10)), 0, r_levels - 1)

        theta_quant = torch.clamp(torch.floor((theta - theta_min) / (theta_scale + 1e-10)), 0, theta_levels - 1)
