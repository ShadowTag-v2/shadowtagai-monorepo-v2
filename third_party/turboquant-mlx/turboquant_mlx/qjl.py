...
class QJLSketch:
    ...
    def __init__(
        ...
        self.projection = mx.where(
            mx.random.uniform(shape=(sketch_dim, input_dim)) > 0.5, mx.array(1.0, dtype=mx.float32), mx.array(-1.0, dtype=mx.float32)
...
def qjl_decompress(
    ...
    projection = mx.where(
        mx.random.uniform(shape=(sketch_dim, original_dim)) > 0.5, mx.array(1.0, dtype=mx.float32), mx.array(-1.0, dtype=mx.float32)
    ) / math.sqrt(sketch_dim)
