...
describe("fixtures", () => {
  ...
  const binPath = path.resolve(__dirname, "../node_modules/.bin/prettier");

  for (const { ext, dir, name } of fixtures) {
    const fixturePath = path.resolve(__dirname, `fixtures/${dir}`);
    const inputPath = path.resolve(fixturePath, `index.${ext}`);
    const outputPath = path.resolve(fixturePath, `output.${ext}`);
    const cmd = `${binPath} ${inputPath} --plugin ${pluginPath}`;

    test.concurrent(name, async ({ expect }) => {
      const results = await execAsync(cmd);
      const formatted = results.stdout.replace(/\r\n/g, "\n");
      const expected = await fs.readFile(outputPath, "utf-8").then((c) => c.replace(/\r\n/g, "\n"));
