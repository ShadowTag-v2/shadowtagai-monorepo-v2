...
describe("parsers", async () => {
  for (const parser in tests) {
    test(parser, async ({ expect }) => {
      for (const [input, expected, options] of tests[parser]) {
...
describe("other", () => {
  test("non-tailwind classes are sorted to the front", async ({ expect }) => {
    const result = await format('<div class="sm:lowercase uppercase potato text-sm"></div>');

    expect(result).toEqual('<div class="potato text-sm uppercase sm:lowercase"></div>');
  });

  test("parasite utilities (v3)", async ({ expect }) => {
    const result = await format('<div class="group peer unknown-class p-0 container"></div>', {
...
describe("whitespace", () => {
  test("class lists containing interpolation are ignored", async ({ expect }) => {
    const result = await format('<div class="{{ this is ignored }}"></div>');

    expect(result).toEqual('<div class="{{ this is ignored }}"></div>');
  });

  test("whitespace can be preserved around classes", async ({ expect }) => {
    const result = await format(`;<div className={' underline text-red-500  flex '}></div>`, {
      parser: "babel",
  ...
  test("whitespace can be collapsed around classes", async ({ expect }) => {
    const result = await format('<div class=" underline text-red-500  flex "></div>');
  ...
  test("whitespace is not trimmed inside concat expressions", async ({ expect }) => {
    const result = await format(";<div className={a + ' p-4 ' + b}></div>", {
      parser: "babel",
    });
  ...
  test("whitespace is not trimmed before template literal quasis without leading space", async ({
    ...
    const result = await format(";<div className={`${foo ? 'sm:p-0 p-0 ' : ''}header`}></div>", {
      parser: "babel",
    });
  ...
  test("duplicate classes are dropped", async ({ expect }) => {
    const result = await format('<div class="underline line-through underline flex"></div>');
...
describe("regex matching", () => {
  test("attribute name exact matches", async ({ expect }) => {
    const result = await format('<div myClass="sm:p-0 p-0"></div>', {
  ...
  test("function name exact matches", async ({ expect }) => {
    const result = await format("let classList = tw`sm:p-0 p-0`", {
      parser: "babel",
  ...
  test("regex flags are supported", async ({ expect }) => {
    const result = await format(`;<div MyClass="sm:p-0 p-0" data-other={MyFn('sm:p-0 p-0')} />`, {
      parser: "babel",
  ...
  test("works with Vue dynamic bindings", async ({ expect }) => {
    const result = await format('<div :data-classes="`sm:p-0 p-0`"></div>', {
      parser: "vue",
      tailwindAttributes: ["/data-.*/"],
    });
  ...
  test("works with Angular property bindings", async ({ expect }) => {
    const result = await format('<div [dataClasses]="`sm:p-0 p-0`"></div>', {
      parser: "angular",
  ...
  test("invalid regex patterns do nothing", async ({ expect }) => {
    const result = await format('<div data-test="sm:p-0 p-0"></div>', {
      tailwindAttributes: ["/data-[/"],
    });
  ...
  test("dynamic attributes are not matched as static attributes", async ({ expect }) => {
    const result = await format(`<div :custom-class="['sm:p-0 flex underline p-0']"></div>`, {
      parser: "vue",
