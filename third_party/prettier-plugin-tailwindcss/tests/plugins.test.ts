...
for (const group of tests) {
  const name = group.plugins.join(", ");

  for (const parser in group.tests) {
    test(`parsing ${parser} works with: ${name}`, async ({ expect, skip }) => {
      ...
      if (parser === "pug") {
        const pug = await import("@prettier/plugin-pug");
        // @ts-expect-error
        pug.logger.level = "off";
      }

      const plugins = [...group.plugins.map((name) => require.resolve(name)), pluginPath];

      for (const [input, expected] of group.tests[parser]) {
        const output = await format(input, { parser, plugins, ...group.options });
