...
import { type Path, spliceChangesIntoString, visit } from "./utils.js";
...
function transformDynamicAngularAttribute(attr: any, env: TransformerEnv) {
  ...
  visit(directiveAst, {
    StringLiteral(node, path) {
      ...
      const collapseWhitespace = canCollapseWhitespaceIn(path, env);
...
function transformDynamicJsAttribute(attr: any, env: TransformerEnv) {
  ...
  const expressionPrefix = "let __prettier_temp__ = ";
  const source = `${expressionPrefix}${attr.value}`;
  const ast = prettierParserBabel.parsers["babel-ts"].parse(source, env.options);
  ...
  function addChange(
    ...
    const offsetStart = start - expressionPrefix.length;
    const offsetEnd = end - expressionPrefix.length;
  ...
  visit(ast, {
    StringLiteral(node, path) {
      const concat = findConcatEntry(path);
    ...
    TemplateLiteral(node, path) {
      const concat = findConcatEntry(path);
      const originalQuasis = node.quasis.map((quasi) => quasi.value.raw);
    ...
    TaggedTemplateExpression(node, path) {
      ...
      const concat = findConcatEntry(path);
      const originalQuasis = node.quasi.quasis.map((quasi) => quasi.value.raw);
      const sorted = sortTemplateLiteral(node.quasi, {
        env,
        collapseWhitespace: {
          start: concat?.key !== "right",
          end: concat?.key !== "left",
  ...
  function sortAttribute(attr: Liquid.AttrSingleQuoted | Liquid.AttrDoubleQuoted) {
    for (let i = 0; i < attr.value.length; i++) {
      ...
      if (node.type === "TextNode") {
        ...
        // @ts-expect-error: `LiquidDrop` is for older versions of the liquid plugin (1.2.x)
...
function sortStringLiteral(
  ...
  const raw = node.extra?.raw ?? node.raw;
  const quote = raw[0];
  const originalRawContent = raw.slice(1, -1);
  const originalValue = node.extra?.rawValue ?? node.value;
...
function canCollapseWhitespaceIn(
  ...
  for (const entry of path) {
    ...
    if (entry.parent.type === "TemplateLiteral") {
      const nodeStart = entry.node.start ?? null;
      const nodeEnd = entry.node.end ?? null;

      for (const quasi of entry.parent.quasis) {
        const quasiStart = quasi.start ?? null;
        const quasiEnd = quasi.end ?? null;
...
function transformCss(ast: any, env: TransformerEnv) {
  ...
  function tryParseAtRuleParams(name: string, params: any) {
    ...
    try {
      const parser = prettierParserCss.parsers.css;

      const root = parser.parse(`@import ${params};`, {
...
const html = defineTransform<HtmlNode>({
  staticAttrs: ["class"],
...
const glimmer = defineTransform<GlimmerNode>({
  staticAttrs: ["class"],
...
const css = defineTransform<CssNode>({
...
const js = defineTransform<import("@babel/types").Node>({
...
const svelte = defineTransform<SvelteNode>({
  staticAttrs: ["class"],
  ...
  reprint(path, { options, changes }) {
    ...
    const stringChanges: StringChange[] = changes.map((change) => ({
      ...change,
...
const astro = defineTransform<AstroNode>({
  staticAttrs: ["class", "className"],
...
const marko = defineTransform<MarkoNode>({
  staticAttrs: ["class"],
...
const twig = defineTransform<TwigNode>({
  staticAttrs: ["class"],
  load: [
    {
...
const pug = defineTransform<PugNode>({
  staticAttrs: ["class"],
...
const liquid = defineTransform<LiquidNode>({
  staticAttrs: ["class"],
  load: [
    {
