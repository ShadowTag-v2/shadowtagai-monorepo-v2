...
const pathToApiMap = expiringMap<string | null, Promise<UnifiedApi>>(10_000);
...
export async function getTailwindConfig(options: TailwindConfigOptions): Promise<UnifiedApi> {
  ...
  const inputDir = options.filepath ? path.dirname(options.filepath) : base;

  const configPath = resolveIfRelative(base, options.configPath);
  const stylesheetPath = resolveIfRelative(base, options.stylesheetPath);
...
const resolvedModCache = expiringMap<string, [any, string | null]>(10_000);

async function resolveTailwindPath(
  options: { packageName?: string },
  baseDir: string,
): Promise<[any, string | null]> {
  const pkgName = options.packageName ?? "tailwindcss";
  const makeKey = (dir: string) => `${pkgName}:${dir}`;

  // Check cache for this directory
  const cached = resolvedModCache.get(makeKey(baseDir));
  if (cached !== undefined) {
    return cached;
  }
...
function findClosestJsConfig(inputDir: string): string | null {
  // Check cache for this directory
  const cached = configPathCache.get(inputDir);
  if (cached !== undefined) {
    return cached;
  }
  ...
  try {
    const foundPath = escalade(inputDir, (_, names) => {
...
export async function createSorter(opts: SorterOptions): Promise<Sorter> {
  const preserveDuplicates = opts.preserveDuplicates ?? false;
  const preserveWhitespace = opts.preserveWhitespace ?? false;

  const api = await getTailwindConfig({
    base: opts.base,
