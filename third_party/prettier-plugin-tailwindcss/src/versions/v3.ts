...
type GenerateRules = (classes: Iterable<string>, context: LegacyTailwindContext) => [bigint][];
...
export async function loadV3(pkgDir: string | null, jsConfig: string | null): Promise<UnifiedApi> {
  ...
  try {
    if (jsConfig) {
      ...
      const url = pathToFileURL(jsConfig);
  ...
  // Create the context
  const context: LegacyTailwindContext = createContext(resolveConfig(tailwindConfig));
