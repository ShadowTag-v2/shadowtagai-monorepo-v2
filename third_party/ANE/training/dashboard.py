...
def load_weights_from_ckpt(path):
    try:
        with open(path, "rb") as f:
            ...
            w2_sz = DIM * HIDDEN
            w3_sz = HIDDEN * DIM
            adam_per_layer = wq_sz * 2 + wk_sz * 2 + wv_sz * 2 + wo_sz * 2 + w1_sz * 2 + w2_sz * 2 + w3_sz * 2 + DIM * 2 + DIM * 2
            W = {}
...
RE_TIMING_DYN = re.compile(
    r"ane_fwd=([\d.]+) io_fwd=([\d.]+) rms=([\d.]+) ane_bwd=([\d.]+) io_bwd=([\d.]+) silu=([\d.]+) rms_bwd=([\d.]+) cls=([\d.]+) cblas_wait=([\d.]+) dw_copy=([\d.]+)"
)
...
RE_EFFICIENCY = re.compile(
    r"(Total steps|Wall time|Compile time|Compile|Train time|Avg compile|Avg train|ANE TFLOPS|Total TFLOPS|ANE utilization):?\s+(.+)"
)
...
def parse_line(line):
    ...
    if m:
        ...
        S.model_config.update(dim=d, q_dim=qd, kv_dim=kvd, hd=hd, hidden=hid, seq=seq, vocab=voc, heads=qd // hd, kv_heads=kvd // hd)
    ...
    if m:
        S.flops.update(fwd=float(m[1]), bwd_dx=float(m[2]), bwd_dw=float(m[3]), sdpa_bwd=float(m[4]), total=float(m[5]))
        return
...
def draw(term):
    ...
    if cfg:
        ...
        row += 1
        line3 = (
            f"{k.get('total', '?')} kernels ({k.get('weight_bearing', '?')} wt-bearing) | Accum {t.get('accum', '?')} | Adam LR={t.get('lr', '?')}"
        )
        put(row, 0, "\u2502", term.cyan)
        put(row, 2, line3)
        put(row, w - 1, "\u2502", term.cyan)
    ...
    put(
        row,
        0,
        "\u251c\u2500 Loss Curve " + "\u2500" * max(0, left_w - 13) + "\u252c\u2500 Training Stats " + "\u2500" * max(0, right_w - 17) + "\u2524",
        term.cyan,
    )
    row += 1
    ...
    if ct:
        if ct.get("_dynamic"):
            put(
                sr,
                mid_x + 1,
                f" fwd={ct.get('ane_fwd', 0):.1f} bwd={ct.get('ane_bwd', 0):.1f} io={ct.get('io_fwd', 0) + ct.get('io_bwd', 0):.1f} silu={ct.get('silu', 0):.1f}",
            )
            sr += 1
    ...
    if has_sysmetrics:
        put(
            row,
            0,
            "\u251c\u2500 CPU % " + "\u2500" * max(0, left_w - 8) + "\u252c\u2500 Memory (MB) " + "\u2500" * max(0, right_w - 15) + "\u2524",
            term.cyan,
        )
        row += 1
...
def spawn_training(
    resume=False, steps=10000, dynamic=False, ane=False, scratch=False, lr=None, accum=None, no_ane_extras=False, data=None, model=None
):
    if dynamic:
    ...
    proc = subprocess.Popen(
        ["bash", "-c", cmd], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=os.path.dirname(os.path.abspath(__file__)) or "."
    )
...
def spawn_powermetrics():
    ...
    try:
        proc = subprocess.Popen(
            ["sudo", "powermetrics", "--samplers", "cpu_power,gpu_power,ane_power", "-i", "1000"], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL
        )
        set_nonblock(proc.stdout.fileno())
        return proc
...
def main():
    ...
    train_proc = spawn_training(
        ...
        accum=args.accum,
        ane=args.ane,
        no_ane_extras=args.no_ane_extras,
        data=args.data,
        model=args.model,
    )
