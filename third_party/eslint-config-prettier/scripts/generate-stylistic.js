...
const getRules = (postfix) => Object.keys(require(`@stylistic/eslint-plugin${postfix}`).rules);
...
require("assert/strict").strictEqual(
  ...[[getRules("")], plugins.map(({ rules }) => rules)].map((group) => new Set(group.flat()).size),
);
    ...
    for (const rule of rules) {
      const severity =
        stylisticPlugin == "plus"
          ? "off"
          : require("..").rules[
              externalPlugin + rule.replace(/^(func)tion(-call-spacing$)/, "$1$2")
            ];
...
const rulesForREADME = [
  ...`${require("fs").readFileSync(`${__dirname}/../README.md`)}`.matchAll(/### \[(.+)]/g),
]
  .map(([, base]) => Object.keys(rulesBySeverity[0]).filter((rule) => rule.endsWith(`/${base}`)))
