...
function createTestConfigDir() {
  ...
  for (const fileToCopy of filesToCopy) {
    fs.copyFileSync(path.join(ROOT, fileToCopy), path.join(TEST_CONFIG_DIR, fileToCopy));
  }
}
      ...
      const testFileName =
        plugin === "vue"
          ...
          : plugin === "@typescript-eslint"
            ? `${plugin}.ts`
            : `${plugin}.js`;
      expect(fs.existsSync(path.join(ROOT, "test-lint", testFileName))).toBe(true);
    });
  });
});
...
describe("all plugins are mentioned in the README", () => {
  ...
  plugins
    ...
    .forEach((plugin) => {
      test(plugin, () => {
        expect(readme).toMatch(plugin.startsWith("@") ? plugin : `eslint-plugin-${plugin}`);
      });
    });
});
...
describe("all special rules are mentioned in the README", () => {
  const readme = fs.readFileSync(path.join(ROOT, "README.md"), "utf8");
  const specialRuleNames = Object.keys(config.rules).filter((name) => config.rules[name] === 0);
