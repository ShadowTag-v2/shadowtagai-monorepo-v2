...
const testLintFiles = fs.readdirSync(path.join(__dirname, "..", "test-lint")).filter(
  (name) =>
    !name.startsWith(".") &&
...
describe("test-lint/ causes errors without eslint-config-prettier", () => {
  const result = childProcess.spawnSync(
    "npm",
    ["run", useEslintrc ? "test:lint-verify-fail" : "test:lint-verify-fail:flat", "--silent"],
  ...
  output.forEach((data) => {
    const name = path.basename(data.filePath).replace(/\.(?:js|jsx|ts)$|-file\.vue$/, "");
    ...
    if (
      useEslintrc &&
      ["@stylistic", "@stylistic__js", "@stylistic__jsx", "@stylistic__ts", "unicorn"].includes(
        name,
      )
    ) {
      return;
    ...
    describe(name, () => {
      test("has errors", () => {
        ...
        if (["@typescript-eslint"].concat(useEslintrc ? [] : "core").includes(name)) {
