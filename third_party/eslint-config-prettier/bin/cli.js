...
const { ESLint, FlatESLint = experimentalApi.FlatESLint } = localRequire("eslint");

const SPECIAL_RULES_URL = "https://github.com/prettier/eslint-config-prettier#special-rules";
...
function processRules(configRules) {
  ...
  const flaggedRules = enabledRules.filter(({ ruleName }) => ruleName in config.rules);
  ...
  const optionsFlaggedRuleNames = filterRuleNames(
    flaggedRules,
    ({ ruleName, ...rule }) => ruleName in optionsRules && !validators[ruleName](rule),
  );
  ...
  const prettierFlaggedRuleNames = filterRuleNames(
    ...
    ({ ruleName, source }) =>
      ...
      enabledRules.some((rule) => rule.ruleName === "prettier/prettier" && rule.source === source),
  );
  ...
  if (regularFlaggedRuleNames.length === 0 && optionsFlaggedRuleNames.length === 0) {
    const message =
      specialFlaggedRuleNames.length === 0 && prettierFlaggedRuleNames.length === 0
...
function filterRuleNames(rules, fn) {
  return [...new Set(rules.filter((rule) => fn(rule)).map((rule) => rule.ruleName))];
}
