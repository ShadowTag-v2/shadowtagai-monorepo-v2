...
async function main() {
  ...
  const importsCode = BROWSER_AVAILABLE_PLUGINS.map(
    (plugin) => `import * as ${plugin}Plugin from "prettier/plugins/${plugin}";`,
  ).join("\n");

  // Generate the plugins array code
  const pluginsArrayCode = BROWSER_AVAILABLE_PLUGINS.map((plugin) => `  ${plugin}Plugin,`).join(
    "\n",
  );
...
function generateNewContent(currentContent, languagesCode, _importsCode, _pluginsArrayCode) {
  let content = currentContent;

  // Update the languages array in getSupportInfo
  const languagesRegex = /(languages:\s*\[\s*\n)([\s\S]*?)(\s*\],\s*\};[\s\S]*?getFileInfo)/;
  const languagesMatch = content.match(languagesRegex);
  if (languagesMatch) {
    content = content.replace(languagesRegex, `$1${languagesCode}\n          $3`);
  } else {
    console.warn("Warning: Could not find languages array pattern to replace.");
    console.log("Please manually update the languages array in getSupportInfo.");
  }
