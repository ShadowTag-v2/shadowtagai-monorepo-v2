...
async function main() {
  ...
  if (!releaseType || !RELEASE_TYPES.includes(releaseType)) {
    console.error("Usage: npm run release <major|minor|patch|version> [version] [--pre]");
    console.error("");
    ...
    console.error("  npm run release patch -- --pre       - 11.0.1 -> 11.0.2 (tag: v11.0.2-pre)");
    console.error("  npm run release version 12.0.1       - Use specific version");
    console.error("  npm run release version 12.0.1 --pre - Specific version as prerelease");
    process.exit(1);
  }
  ...
  if (!isPrerelease && currentBranch !== "main") {
    ...
    console.error("  npm run release <major|minor|patch|version> [version] -- --pre");
    process.exit(1);
  }
  ...
  const newVersion = manualVersion || calculateNewVersion(currentVersion, releaseType);
