...
import { commands, type TextDocument, Uri, window, workspace } from "vscode";
...
import { FIND_UP_STOP, findUp, pathExists } from "./utils/find-up.js";
import {
  resolveGlobalNodePath,
  resolveGlobalPnpmPath,
  resolveGlobalYarnPath,
} from "./utils/global-node-paths.js";
import { getWorkspaceConfig, getWorkspaceRelativePath } from "./utils/workspace.js";
...
async function globalPathGet(packageManager: PackageManagers): Promise<string | undefined> {
...
export class ModuleResolver implements ModuleResolverInterface {
  ...
  public async getPrettierInstance(
    ...
    const { prettierPath, resolveGlobalModules } = getWorkspaceConfig(Uri.file(fileName));
    ...
    try {
      ...
      this.loggingService.logError(`${FAILED_TO_LOAD_MODULE_MESSAGE}: ${modulePath}`, error);
      return undefined;
    }
  ...
  private async getModuleFromPrettierPath(
    ...
    const absolutePrettierPath = path.isAbsolute(prettierPath)
      ? prettierPath
      : path.join(workspace.getWorkspaceFolder(Uri.file(fileName))?.uri.fsPath ?? "", prettierPath);
  ...
  public async getResolvedIgnorePath(
    ...
    const foundPath = await findUp(
      async (dir: string) => {
        ...
        // Stop at marker file
        if (await pathExists(path.join(dir, ".do-not-use-prettier-vscode-root"))) {
          return FIND_UP_STOP;
        }
        return undefined;
      },
    ...
    this.loggingService.logWarning(`Unable to resolve ignore path: ${ignorePath} for ${fileName}`);
    return;
  }
  ...
  public async clearModuleCache(filePath: string): Promise<void> {
    ...
    try {
      ...
      this.loggingService.logError(`Failed to clear config cache for ${filePath}`, error);
    }
  }

  private async findPrettierModule(fileName: string): Promise<string | undefined> {
    ...
    const foundPath = await findUp(
      async (d: string) => {
        ...
        if (await pathExists(path.join(d, ".do-not-use-prettier-vscode-root"))) {
          return FIND_UP_STOP;
        }
  ...
  private async findGlobalModule(moduleName: string): Promise<string | undefined> {
  ...
  private async isValidVersion(modulePath: string): Promise<boolean> {
    ...
    try {
      ...
      if (!semver.gte(version, minPrettierVersion)) {
        ...
        this.loggingService.logInfo(`Found version ${version}, requires ${minPrettierVersion}+`);
        ...
        await commands.executeCommand("setContext", "prettier.outdatedError", true);
        return false;
      }

      await commands.executeCommand("setContext", "prettier.outdatedError", false);
      return true;
    } catch {
      this.loggingService.logError(`${FAILED_TO_LOAD_MODULE_MESSAGE} ${modulePackageJsonPath}`);
      return false;
    }
  }
  ...
  public async resolveConfig(
    ...
    try {
      ...
      this.loggingService.logError(`Failed to resolve config file for ${fileName}`, error);
      return "error";
    }
    ...
    try {
      ...
      if (customConfigPath) {
        this.loggingService.logInfo(`Using custom config path from settings: ${customConfigPath}`);
      }
      ...
      resolvedConfig = await prettierInstance.resolveConfig(fileName, resolveConfigOptions);
    } catch (error) {
