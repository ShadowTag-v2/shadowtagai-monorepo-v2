 ...
 */
export const PrettierDynamicInstance: PrettierInstanceConstructor = class PrettierDynamicInstance
  implements PrettierInstance
{
  ...
  public async import(): Promise</* version of imported prettier */ string> {
    ...
    if (ext === ".js" || ext === ".cjs" || ext === ".mjs") {
      ...
      entryPath = resolveModuleEntry(this.modulePath, path.basename(this.modulePath));
    }
  ...
  public async format(source: string, options?: Options | undefined): Promise<string> {
  ...
  public async getSupportInfo({ plugins }: { plugins: (string | PrettierPlugin)[] }): Promise<{
  ...
  public async resolveConfigFile(filePath?: string | undefined): Promise<string | null> {
    if (!this.prettierModule) {
      await this.import();
    }
