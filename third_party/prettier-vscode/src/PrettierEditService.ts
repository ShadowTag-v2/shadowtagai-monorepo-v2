...
import { FormatterStatus, type StatusBar } from "./StatusBar.js";
import type {
...
export default class PrettierEditService implements Disposable {
  ...
  private handleActiveTextEditorChangedSync = (textEditor: TextEditor | undefined) => {
  ...
  private handleActiveTextEditorChanged = async (textEditor: TextEditor | undefined) => {
    ...
    const selectors = await this.getSelectors(prettierInstance, document.uri, workspaceFolder.uri);
  ...
  private registerDocumentFormatEditorProviders({
    ...
    const codeActionProvider = new PrettierCodeActionProvider(this.provideEdits);
