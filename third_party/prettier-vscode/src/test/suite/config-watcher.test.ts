...
describe("Test config file watcher", () => {
  ...
  it("detects config file changes and applies new formatting", async function () {
    ...
    const { actual: initialFormat } = await format("config", `${configDir}/test.js`);
    ...
    const { actual: newFormat } = await format("config", `${configDir}/test.js`);
    ...
    assert.ok(
      !newFormat.includes("  console.log(") || newFormat.includes("    console.log"),
