...
import { ModuleResolver, type PrettierNodeModule } from "../../ModuleResolverNode.js";
import { OUTDATED_PRETTIER_VERSION_MESSAGE, USING_BUNDLED_PRETTIER } from "../../message.js";
...
describe("Test ModuleResolver", () => {
  ...
  describe("getPrettierInstance", () => {
    it("it returns the bundled version of Prettier if local isn't found", async () => {
      const fileName = path.join(getWorkspaceFolderUri("no-dep").fsPath, "index.js");
      const prettierInstance = await moduleResolver.getPrettierInstance(fileName);
      ...
      assert.strictEqual((prettierInstance as PrettierNodeModule).version, prettier.version);
      assert.ok(logDebugMock.calls.some((args) => args[0] === USING_BUNDLED_PRETTIER));
    });

    it("it returns the bundled version of Prettier if local is outdated", async () => {
      const fileName = path.join(getWorkspaceFolderUri("outdated").fsPath, "ugly.js");
      const prettierInstance = await moduleResolver.getPrettierInstance(fileName);

      assert.strictEqual(prettierInstance, undefined);
      assert.ok(logErrorMock.calls.some((args) => args[0] === OUTDATED_PRETTIER_VERSION_MESSAGE));
    });

    it("it returns prettier version from package.json", async () => {
      const fileName = path.join(getWorkspaceFolderUri("specific-version").fsPath, "ugly.js");
    ...
    it("it returns prettier version from module dep", async () => {
      const fileName = path.join(getWorkspaceFolderUri("module").fsPath, "index.js");
      const prettierInstance = await moduleResolver.getPrettierInstance(fileName);
