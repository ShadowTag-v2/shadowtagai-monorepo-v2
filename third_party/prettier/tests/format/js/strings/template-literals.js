...
const x = `a long string ${1 + 2 + 3 + 2 + 3 + 2 + 3 + 2 + 3 + 2 + 3 + 2 + (() => 3)() + 3 + 2 + 3 + 2 + 3} with expr`;
...
const makeBody = (store, assets, html) =>
  `<!doctype html>${ReactDOMServer.renderToStaticMarkup(
    <Html
      ...
      bodyStyles={[]}
      stringScripts={[`window.__INITIAL_STATE__ = ${JSON.stringify(store.getState(), null, 2)};`]}
      content={[{ id: "app-container", dangerouslySetInnerHTML: { __html: html } }]}
    />,
  )}`;
...
const otherMessage = `this template contains two interpolations: ${this(one)}, which should be kept on its line,
