...
runFormatTest(
  {
    ...
    snippets: [
      ...
      ...POSSIBLE_MODIFIERS.filter((modifier) => modifier !== "readonly").flatMap((modifier) => [
        outdent`
          interface Foo {
            ${modifier} method();
          }
      ...
      ...POSSIBLE_MODIFIERS.filter(
        (modifier) => modifier !== "declare" && modifier !== "export",
      ).flatMap((modifier) => [`${modifier} module Foo {}`, `${modifier} namespace Foo {}`]),

      // `TSEnumDeclaration`
      ...POSSIBLE_MODIFIERS.filter(
        (modifier) => modifier !== "declare" && modifier !== "const" && modifier !== "export",
      ...
      ...POSSIBLE_MODIFIERS.filter(
        (modifier) =>
          ...
          modifier !== "public" &&
          modifier !== "readonly",
      ).map((modifier) => `class Foo { constructor(${modifier} parameter) {} }`),
