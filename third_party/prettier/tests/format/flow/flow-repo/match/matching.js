...
}
{
  ...
  => (a as 'foo', value as 1), // OK
  {
    type: 'bar' as a,
    const value;
  }
  => (a as 'bar', value as 2), // OK
}
...
// Disjoint object union with multiple pivot props
{
  ...
  const e1 = match (x) {
    {type: 'foo', val: const a} => a as number, // OKtype: 'bar', val: const a, ...} => a as string | boolean, // OK;
...
// Or pattern: objects
{
  ...
  const e1 = match (x) type: 'foo', ...| type: 'bar', ...| type: 'baz', ...=> 0,;

  const e2 = match (x) type: 'foo', ...| type: 'baz', ...=> 0,;
}
