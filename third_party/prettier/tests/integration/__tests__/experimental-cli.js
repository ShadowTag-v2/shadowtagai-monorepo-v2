...
describe("experimental cli", () => {
  const expectedVersion =
    ...
    (process.env.PRETTIER_INSTALLED_DIR && process.env.PRETTIER_INSTALL_NPM_CLIENT !== "npm")
      ? require(path.join(process.env.PRETTIER_DIR, "package.json")).version
      : createRequire(require.resolve("@prettier/cli/package.json"))("prettier/package.json")
          .version;
  ...
  for (const [index, args] of [
    ...
    test(args.join(" "), async () => {
      ...
      const directory = new URL(`../cli/experimental-cli/test-${index}/`, import.meta.url);
