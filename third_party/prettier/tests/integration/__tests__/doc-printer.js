...
const docToString = (doc, options) => printDocToString(doc, { tabSize: 2, ...options }).formatted;
...
test("`ifBreak` inside `group`", () => {
  ...
  expect(docs.map((doc) => docToString(doc, { printWidth: FLAT_TEXT.length }))).toStrictEqual(
    Array.from({ length: docs.length }, () => FLAT_TEXT),
  );
});
