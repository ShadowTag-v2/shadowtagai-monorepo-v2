describe("ignore-unknown dir", () => {
  runCli("cli/ignore-unknown", [".", "--ignore-unknown", "--list-different"]).test({
    status: "non-zero",
    stderr: "",
    write: [],
  });
...
describe("ignore-unknown write", () => {
  runCli("cli/ignore-unknown", [".", "--ignore-unknown", "--write", "--list-different"]).test({
    status: 0,
    stderr: "",
  });
});
...
describe("Not matching pattern", () => {
  runCli("cli/ignore-unknown", ["*.non-exist-pattern", "--ignore-unknown"]).test({
    status: 2,
  });
});
