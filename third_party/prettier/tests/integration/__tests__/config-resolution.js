...
test("resolves configuration from external files and overrides by extname", async () => {
  await expect(
    prettier.resolveConfig(new URL("../cli/config/external-overrides/file.js", import.meta.url)),
  ).resolves.toStrictEqual({ tabWidth: 3, semi: false });
  await expect(
    prettier.resolveConfig(new URL("../cli/config/external-overrides/file.ts", import.meta.url)),
...
test("API resolveConfig with file arg and .editorconfig (key = unset)", async () => {
  const file = new URL("../cli/config/editorconfig/tab_width=unset.js", import.meta.url);
  await expect(prettier.resolveConfig(file, { editorconfig: true })).resolves.not.toMatchObject({
    tabWidth: "unset",
  });
});

test("API resolveConfig with nested file arg and .editorconfig", async () => {
  const file = new URL("../cli/config/editorconfig/lib/file.js", import.meta.url);
...
test("API resolveConfig with nested file arg and .editorconfig and indent_size = tab", async () => {
  const file = new URL("../cli/config/editorconfig/lib/indent_size=tab.js", import.meta.url);
...
test("API resolveConfig overrides work with dotfiles", async () => {
  const file = new URL("../cli/config/dot-overrides/.foo.json", import.meta.url);
  await expect(prettier.resolveConfig(file)).resolves.toMatchObject({
    tabWidth: 4,
  });
});
...
test("API resolveConfig overrides excludeFiles", async () => {
  const notOverride = new URL("../cli/config/overrides-exclude-files/foo", import.meta.url);
...
test("API resolveConfig resolves relative path values based on config filepath", async () => {
  const currentDir = new URL("../cli/config/resolve-relative/", import.meta.url);
  const parentDir = path.resolve(url.fileURLToPath(currentDir), "..");
  await expect(prettier.resolveConfig(new URL("index.js", currentDir))).resolves.toMatchObject({
...
test("API resolveConfig de-references to an external module", async () => {
  ...
  await expect(
    prettier.resolveConfig(
      new URL("../cli/config/external-config/cjs-package/index.js", import.meta.url),
    ),
  ).resolves.toStrictEqual(config);
  await expect(
    prettier.resolveConfig(
      new URL("../cli/config/external-config/esm-package/index.js", import.meta.url),
    ),
  ).resolves.toStrictEqual(config);
  await expect(
    prettier.resolveConfig(
      new URL("../cli/config/external-config/esm-file/index.js", import.meta.url),
    ),
  ).resolves.toStrictEqual(config);
});
...
test("Search from directory, not treat file as directory", async () => {
  ...
  const getConfigFileByApi = async (file) => {
    const configFile = await prettier.resolveConfigFile(new URL(file, directory));
...
test("package.json/package.yaml", async () => {
  await expect(
    prettier.resolveConfig(new URL("../cli/config/package/file.js", import.meta.url)),
  ...
  `);
  await expect(
    prettier.resolveConfig(new URL("../cli/config/package/file.ts", import.meta.url)),
  ).resolves.toMatchInlineSnapshot(`
    {
      "tabWidth": 5,
    }
  `);
  await expect(
    prettier.resolveConfig(new URL("../cli/config/package-yaml/file.ts", import.meta.url)),
  ).resolves.toMatchInlineSnapshot(`
    {
      "printWidth": 101,
    }
...
test("'config' option should accept `URL` and `string`", async () => {
  ...
  for (const configFile of [configFileUrl, configFileUrl.href, url.fileURLToPath(configFileUrl)]) {
    await expect(prettier.resolveConfig(file, { config: configFile })).resolves.toStrictEqual({
      tabWidth: 7,
    });
  }
});
