...
test("API resolveConfigFile", async () => {
  ...
  expect(result).toBe(url.fileURLToPath(new URL("../../../prettier.config.js", import.meta.url)));
});

test("API resolveConfigFile accepts path or URL", async () => {
  const fileUrl = new URL("../cli/config/filepath/foo.js", import.meta.url);
  const expectedConfigFilePath = url.fileURLToPath(new URL("./.prettierrc", fileUrl));
  ...
  const resultByPath = await prettier.resolveConfigFile(url.fileURLToPath(fileUrl));
