...
describe("standalone", () => {
  ...
  let esmPlugins;
  beforeAll(async () => {
    esmStandalone = await importModule(path.join(distDirectory, "standalone.mjs"));
    esmPlugins = await Promise.all(
      fastGlob.sync(["plugins/*.mjs"], { cwd: distDirectory, absolute: true }).map(async (file) => {
  ...
  const printerVisitorKeysSettings = new Map([
    [
      "estree",
      {
        ...
        nodes: [{ type: "FunctionDeclaration" }, { type: "FunctionExpression" }],
      },
    ],
    [
      "estree-json",
    ...
    [
      ...
      {
        sharedVisitorKeys: false,
        nodes: [{ type: "MustacheStatement" }, { type: "ElementModifierStatement" }],
      },
    ],
    [
      "graphql",
      {
        sharedVisitorKeys: false,
        nodes: [{ kind: "ObjectTypeExtension" }, { kind: "InterfaceTypeExtension" }],
      },
    ],
    [
      "html",
  ...
  test("visitor keys with shared reference", () => {
    for (const [name, printer] of esmPlugins.flatMap((plugin) =>
      ...
      test(name, () => {
        ...
        expect(
          typeof sharedVisitorKeys === "boolean" && Array.isArray(nodes) && nodes.length > 1,
        ).toBe(true);
        ...
        try {
          ...
          throw new Error(`Missing visitor keys for '${name}' nodes: ${inspect(nodes)}.`);
        }
        ...
        if (sharedVisitorKeys) {
          ...
          expect(
            restNodeKeys.every(
              (keys) =>
                keys !== firstNodeKeys && JSON.stringify(keys) === JSON.stringify(firstNodeKeys),
            ),
          ).toBe(true);
        }
      });
...
test("global objects", async () => {
  ...
  const getGlobalObjects = (file) => {
    ...
    return Object.fromEntries(
      Object.entries(sandbox).filter(([property]) => !allowedGlobalObjects.has(property)),
    );
  };
...
test("Commonjs version", () => {
  const prettierCommonjsVersion = require(path.join(distDirectory, "index.cjs"));
  ...
  expect(Object.keys(prettierCommonjsVersion.util)).toStrictEqual(Object.keys(prettier.util));
