...
test("yaml parser should handle CRLF correctly", async () => {
  ...
  expect(
    ...
    JSON.stringify(await prettier.format(input, { parser: "yaml", endOfLine: "auto" })),
  ).toMatchSnapshot();
});
...
test("html parser should handle CRLF correctly", async () => {
  ...
  expect(
    // use JSON.stringify to observe CRLF
    JSON.stringify(await prettier.format(input, { parser: "html", endOfLine: "auto" })),
  ).toMatchSnapshot();
});
...
test("markdown parser should handle CRLF correctly", async () => {
  ...
  expect(
    // use JSON.stringify to observe CRLF
    JSON.stringify(await prettier.format(input, { parser: "markdown", endOfLine: "auto" })),
  ).toMatchSnapshot();
});
...
test("should work with foo plugin instance", async () => {
  ...
  ).toMatchInlineSnapshot(String.raw`""{\"tabWidth\":8,\"bracketSpacing\":false}""`);
});

test("'Adjacent JSX' error should not be swallowed by Babel's error recovery", async () => {
  const input = "<a></a>\n<b></b>";
  await expect(prettier.format(input, { parser: "babel" })).rejects.toThrowErrorMatchingSnapshot();
});
