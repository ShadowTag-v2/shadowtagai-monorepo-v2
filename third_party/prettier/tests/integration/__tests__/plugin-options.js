...
test("show external options with `--help`", async () => {
  ...
  const pluggedStdout = await runCli("plugins/options", ["--help", "--plugin=./plugin.cjs"]).stdout;
...
describe("show detailed external option with `--help foo-option`", () => {
  runCli("plugins/options", ["--plugin=./plugin.cjs", "--help", "foo-option"]).test({
    status: 0,
  });
});
...
describe("external options from CLI should work", () => {
  runCli(
    "plugins/options",
    ["--plugin=./plugin.cjs", "--stdin-filepath", "example.foo", "--foo-option", "baz"],
    { input: "hello-world" },
  ).test({
    stdout: "foo:baz",
    stderr: "",
...
describe("external options from config file should work", () => {
  runCli("plugins/options", ["--config=./config.json", "--stdin-filepath", "example.foo"], {
    input: "hello-world",
  }).test({
    stdout: "foo:baz",
    stderr: "",
