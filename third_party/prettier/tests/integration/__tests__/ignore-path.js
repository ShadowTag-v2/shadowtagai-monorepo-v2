...
].map((file) => new URL(`../cli/ignore-path/ignore-path-test/${file}`, import.meta.url));
const clean = () => Promise.all(files.map((file) => fs.rm(file, { force: true })));
const setup = () => Promise.all(files.map((file) => fs.writeFile(file, "   a+   b")));
...
const getUnformattedFiles = async (args) => {
  const { stdout } = await runCli("cli/ignore-path/ignore-path-test/", ["**/*.js", "-l", ...args]);
  return stdout ? stdout.split("\n").sort() : [];
};

test("custom ignore path", async () => {
  expect(await getUnformattedFiles(["--ignore-path", ".customignore"])).toStrictEqual([
    "ignored-by-gitignore.js",
    "ignored-by-prettierignore.js",
  ]);
});
...
test("ignore files by .prettierignore and .gitignore by default", async () => {
  expect(await getUnformattedFiles(["--ignore-path", ".non-exists-ignore-file"])).toStrictEqual([
    "ignored-by-customignore.js",
    "ignored-by-gitignore.js",
    "ignored-by-prettierignore.js",
  ]);
