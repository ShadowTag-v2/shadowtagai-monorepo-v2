...
test("shared util has correct structure", () => {
  ...
  expect(typeof utilities.getNextNonSpaceNonCommentCharacterIndex).toBe("function");
...
test("sharedUtil.getAlignmentSize", () => {
  ...
  expect(getAlignmentSize("\t\t", /* tabWidth */ 3, /* startIndex */ 1)).toBe(3);
});
...
test("sharedUtil.getNextNonSpaceNonCommentCharacter and sharedUtil.getNextNonSpaceNonCommentCharacterIndex", () => {
  const { getNextNonSpaceNonCommentCharacter, getNextNonSpaceNonCommentCharacterIndex } = utilities;
...

  {
    ...
    expect(getNextNonSpaceNonCommentCharacter(text, endOfIdentifierA)).toBe("b");
    expect(getNextNonSpaceNonCommentCharacterIndex(text, endOfIdentifierA)).toBe(
      indexOfIdentifierB,
    );
    expect(getNextNonSpaceNonCommentCharacterIndex(text, FAKE_NODE, locEnd)).toBe(
      indexOfIdentifierB,
    );
  }

  {
    ...
    expect(getNextNonSpaceNonCommentCharacterIndex(text, endOfIdentifierA)).toBe(text.length);
    expect(getNextNonSpaceNonCommentCharacterIndex(text, FAKE_NODE, locEnd)).toBe(text.length);
  }

  {
    ...
    expect(getNextNonSpaceNonCommentCharacterIndex(text, startIndex)).toBe(false);
    expect(getNextNonSpaceNonCommentCharacterIndex(text, FAKE_NODE, locEnd)).toBe(false);
  }
});

test("sharedUtil.isPreviousLineEmpty, sharedUtil.isNextLineEmpty and sharedUtil.isNextLineEmptyAfterIndex", () => {
  const { isPreviousLineEmpty, isNextLineEmpty, isNextLineEmptyAfterIndex } = utilities;
...

  {
    ...
    expect(isPreviousLineEmpty(text, FAKE_NODE_B, () => startOfIdentifierB)).toBe(true);
    expect(isNextLineEmpty(text, endOfIdentifierA)).toBe(true);
    expect(isNextLineEmptyAfterIndex(text, endOfIdentifierA)).toBe(true);
    expect(isNextLineEmpty(text, FAKE_NODE_A, () => endOfIdentifierA)).toBe(true);
  }

  {
    ...
    expect(isPreviousLineEmpty(text, FAKE_NODE_B, () => startOfIdentifierB)).toBe(false);
    expect(isNextLineEmpty(text, endOfIdentifierA)).toBe(false);
    expect(isNextLineEmptyAfterIndex(text, endOfIdentifierA)).toBe(false);
    expect(isNextLineEmpty(text, FAKE_NODE_A, () => endOfIdentifierA)).toBe(false);
  }
...
test("sharedUtil.makeString", () => {
  ...
  expect(makeString(String.raw`\a`, SINGLE_QUOTE, /* unescapeUnnecessaryEscapes */ true)).toBe(
...
test("sharedUtil.getPreferredQuote", () => {
  ...
  expect(getPreferredQuote(`'Hello' "World" 'Test'`, SINGLE_QUOTE)).toBe(DOUBLE_QUOTE);
  expect(getPreferredQuote(`"Hello" 'World' "Test"`, DOUBLE_QUOTE)).toBe(SINGLE_QUOTE);
  expect(getPreferredQuote(`"Hello" "World" "Test"`, SINGLE_QUOTE)).toBe(SINGLE_QUOTE);
  expect(getPreferredQuote(`'Hello' 'World' 'Test'`, DOUBLE_QUOTE)).toBe(DOUBLE_QUOTE);
