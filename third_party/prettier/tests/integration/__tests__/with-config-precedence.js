...
test("CLI overrides take precedence --config-precedence", async () => {
  const withoutFlag = await runCli("cli/config-precedence/valid-config", ["foo.js"]).stdout;
...
describe("CLI validate options with --config-precedence cli-override", () => {
  runCli("cli/config-precedence/invalid-config", ["--config-precedence", "cli-override"]).test({
    status: "non-zero",
  });
});

describe("CLI validate options with --config-precedence file-override", () => {
  runCli("cli/config-precedence/invalid-config", ["--config-precedence", "file-override"]).test({
    status: "non-zero",
  });
});

describe("CLI validate options with --config-precedence prefer-file", () => {
  runCli("cli/config-precedence/invalid-config", ["--config-precedence", "prefer-file"]).test({
    status: "non-zero",
  });
});
...
describe("CLI --stdin-filepath works with --config-precedence file-override", () => {
  runCli(
    "cli/config-precedence/overrides",
    ["--stdin-filepath=abc.ts", "--no-semi", "--config-precedence=file-override"],
    { input: "let x: keyof Y = foo<typeof X>()" }, // typescript
  ).test({
    stderr: "",
    status: 0,
...
describe("CLI --stdin-filepath works with --config-precedence cli-override", () => {
  runCli(
    "cli/config-precedence/overrides",
    ["--stdin-filepath=abc.ts", "--no-semi", "--config-precedence=cli-override"],
    { input: "let x: keyof Y = foo<typeof X>()" }, // typescript
  ).test({
    stderr: "",
    status: 0,
