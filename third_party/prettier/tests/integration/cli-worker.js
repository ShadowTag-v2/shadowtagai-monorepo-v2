...
import { prettierCliEntry, prettierCliMockableEntry, prettierMainEntry } from "./env.js";

const normalizeToPosix =
  path.sep === "\\" ? (filepath) => replaceAll(filepath, "\\", "/") : (filepath) => filepath;
const hasOwn =
  Object.hasOwn ??
  ((object, property) =>
...
const replaceAll = (text, find, replacement) =>
  text.replaceAll ? text.replaceAll(find, replacement) : text.split(find).join(replacement);
...
async function mockImplementations(options) {
  const [apiMockable, cliMockable] = await Promise.all([getApiMockable(), getCliMockable()]);
  ...
  cliMockable.mockImplementations({
    ...
    async writeFormattedFile(filename, content) {
      ...
      if (options.mockWriteFileErrors && hasOwn(options.mockWriteFileErrors, filename)) {
...
async function run(options) {
  ...
  const { __promise: promise } = await import(url.pathToFileURL(prettierCliEntry));
  await promise;
}
