...
async function loadUmdPrettier() {
  ...
  const [, esmPlugins] = await Promise.all([
    Promise.all([umdFiles.prettier, ...umdPluginFiles].map((file) => loadScript(`/${file}`))),
    Promise.all(esmPluginFiles.map((file) => import(`/${file}`))),
  ]);

  const { prettier } = globalThis;
  const builtinPlugins = [...Object.values(globalThis.prettierPlugins), ...esmPlugins];
...
function createPrettier(prettierBundleFormat) {
  ...
  function loadPrettier() {
    if (!cache) {
      cache = prettierBundleFormat === "esm" ? loadEsmPrettier() : loadUmdPrettier();
    }
