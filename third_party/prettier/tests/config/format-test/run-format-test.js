...
import { isErrorTest as isErrorTestDirectory, normalizeDirectory } from "./utilities.js";
...
function runFormatTest(rawFixtures, explicitParsers, rawOptions) {
  ...
  if (filename !== FORMAT_SCRIPT_FILENAME) {
    throw new Error(`Format test should run in file named '${FORMAT_SCRIPT_FILENAME}'.`);
  }

  const dirname = normalizeDirectory(path.dirname(url.fileURLToPath(importMeta.url)));
