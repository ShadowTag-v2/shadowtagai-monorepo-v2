...
function testFixture(fixture) {
  ...
  describe(title, () => {
    ...
    const testCaseForSnapshot = testCases.find(
      (testCase) => !testCase.expectFail && typeof testCase.expectedOutput !== "string",
    );
    ...
    for (const functionality of [
      {
        ...
        test: {
          run: (testCase, name) => testFormat.run(testCase, name, testCaseForSnapshot),
        },
      },
      ...
      for (const testCase of testCases) {
        if (functionality.skip?.(testCase) || functionality.test.skip?.(testCase)) {
          continue;
        }
...
function getTestCase(fixture, parser) {
  ...
  const { text: code, options: formatOptions } = replacePlaceholders(originalText, {
    filepath,
    ...context.options,
    parser,
  });
  ...
  return {
    ...
    isEmpty: code.trim() === "",
    runFormat: () => (promise === undefined ? (promise = format(code, formatOptions)) : promise),
  };
}
