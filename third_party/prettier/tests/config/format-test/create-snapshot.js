...
function printOptions(options) {
  ...
  return keys.map((key) => `${key}: ${stringify(snapshotOptions[key])}`).join("\n");
}
...
function printWidthIndicator(printWidth) {
  if (!(printWidth === undefined || (Number.isSafeInteger(printWidth) && printWidth > 0))) {
    return "";
  }

  const widthIndicator =
    printWidth === undefined ? defaultWidthIndicator : makeWidthIndicator(printWidth);

  return widthIndicator;
}

function createSnapshot(formatResult, { parsers, formatOptions }) {
  let { inputWithCursor: input, outputWithCursor: output, options } = formatResult;
  ...
  return raw(
    [
      ...
      input,
      addOffset([printSeparator("output"), output, printSeparator()].join("\n"), codeOffset),
    ].join("\n"),
  );
}
