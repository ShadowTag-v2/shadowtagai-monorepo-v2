...
const unstableTests = new Map(
  [
    ["js/ignore/semi/class-expression-decorator.js", (options) => options.semi === false],
...
const disabledTests = new Map(
  Object.entries({
    ...
    acorn: ["js/explicit-resource-management/valid-await-using-asi-assignment.js"],
    meriyah: [
...
const isUnstable = (filepath, options) => unstableTests.get(filepath)?.(options);

const isAstUnstable = (filepath, options) => unstableAstTests.get(filepath)?.(options);

const shouldDisable = (directoryOrFile, parser) => disabledTests.get(parser)?.has(directoryOrFile);
