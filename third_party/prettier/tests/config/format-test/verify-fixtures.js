...
const categoryParsers = new Map([
  ...
  [
    "flow",
    {
      ...
      verifyParsers: ["flow", "babel-flow", "typescript", "babel-ts", "hermes", "babel"],
...
const verifyParsers = (context) => {
  ...
  const { parsers: allowedParsers = [], verifyParsers: allowedVerifyParsers = [] } = settings;
    ...
    const suggestion =
      suggestCategories.length === 0
        ...
        : outdent`
          ...
          ${suggestCategories
            .map((category) => `- ${path.join(FORMAT_TEST_DIRECTORY, category)}`)
            .join("\n")}
