...
const hasPragma = (text) =>
  parseFrontMatter(text).content.trimStart().match(MARKDOWN_HAS_PRAGMA_REGEXP)?.index === 0;

const hasIgnorePragma = (text) =>
  parseFrontMatter(text).content.trimStart().match(MARKDOWN_HAS_IGNORE_PRAGMA_REGEXP)?.index === 0;
