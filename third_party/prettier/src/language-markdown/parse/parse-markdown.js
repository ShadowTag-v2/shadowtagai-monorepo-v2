...
import { liquidFromMarkdown, liquidSyntax } from "./micromark/micromark-extension-liquid.js";
...
function parseMarkdown(text) {
  ...
  if (frontMatter) {
    const [start, end] = [frontMatter.start, frontMatter.end].map(({ line, column, index }) => ({
      line,
      column: column + 1,
      offset: index,
    }));
