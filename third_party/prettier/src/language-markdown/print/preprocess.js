...
function addRawToText(ast, options) {
  return mapAst(ast, (node) => {
    if (node.type === "text") {
      ...
      node.raw = options.originalText.slice(node.position.start.offset, node.position.end.offset);
    }
    return node;
  });
}
...
function splitTextIntoSentences(ast) {
  ...
  return mapAst(ast, (node, index, parentStack) => {
    ...
    const paragraphIndex = parentStack.findIndex((ancestor) => ancestor?.type === "paragraph");

    const paragraphNode = paragraphIndex === -1 ? undefined : parentStack[paragraphIndex];

    if (paragraphNode) {
      if (
        parentStack.slice(paragraphIndex + 1).some((ancestor) => ancestor?.type === "blockquote")
      ) {
...
function getBlockquoteRawText(text, node) {
  ...
  const resultLines = rawLines.map((rawLine, index) => {
    ...
    const leadingTextAngleBrackets = valueLine.match(angleBracketsRegex)[0] ?? "";
