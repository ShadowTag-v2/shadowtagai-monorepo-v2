...
function printWord(path) {
  const { node } = path;
  const emphasisOrStrong = path.findAncestor((p) => p.type === "emphasis" || p.type === "strong");
  if (!emphasisOrStrong) {
    return node.value;
  }
  ...
  text = text.replaceAll(/(\\+|^|.)(\*+|_+)($|.)/g, (match, preceding, delimiterRun, following) => {
...
function printWordLegacy(path) {
  ...
  let escapedValue = node.value
    ...
    .replaceAll(
      new RegExp(
        [`(^|${PUNCTUATION_REGEXP.source})(_+)`, `(_+)(${PUNCTUATION_REGEXP.source}|$)`].join("|"),
        "gu",
      ),
      (_, text1, underscore1, underscore2, text2) =>
        (underscore1 ? `${text1}${underscore1}` : `${underscore2}${text2}`).replaceAll(
          "_",
          String.raw`\_`,
        ),
  ...
  if (
    ...
    escapedValue = escapedValue.replace(/^(\\?[*_])+/, (prefix) => prefix.replaceAll("\\", ""));
  }
