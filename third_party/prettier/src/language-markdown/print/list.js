...
import { getNthListSiblingIndex, hasGitDiffFriendlyOrderedList } from "../utilities.js";
...
function printList(path, options, print) {
  ...
  return printChildren(path, options, print, {
    processor() {
      ...
      return [
        prefix,
        align(" ".repeat(prefix.length), printListItem(path, options, print, prefix)),
      ];
        ...
        const rawPrefix = node.ordered
          ? (path.isFirst
              ...
              : isGitDiffFriendlyOrderedList
                ? 1
                : node.start + path.index) + (nthSiblingIndex % 2 === 0 ? ". " : ") ")
          : nthSiblingIndex % 2 === 0
            ? "- "
            : "* ";

        let prefix =
          node.isAligned && node.ordered ? alignListPrefix(rawPrefix, options) : rawPrefix;
  ...
  return [
    ...
    printChildren(path, options, print, {
      processor({ node, isFirst }) {
        if ((isFirst && node.type !== "list") || (node.type === "code" && node.isIndented)) {
...
function requiredIndent(path) {
  ...
  return (
    ...
    [...leadingSpaces].reduce((count, char) => count + (char === "\t" ? 4 : 1), 0) +
...
function printListLegacy(path, options, print) {
  ...
  return printChildren(path, options, print, {
    processor() {
      ...
      return [
        prefix,
        align(" ".repeat(prefix.length), printListItemLegacy(path, options, print, prefix)),
      ];
