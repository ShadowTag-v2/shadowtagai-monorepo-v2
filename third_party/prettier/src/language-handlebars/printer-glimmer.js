...
import { hasPrettierIgnore, isVoidElement, isWhitespaceNode } from "./utilities.js";
...
function print(path, options, print) {
  ...
  switch (node.type) {
    ...
    case "ElementNode": {
      const isWhitespaceSensitive = options.htmlWhitespaceSensitivity !== "ignore";

      const startingTag = [
        !isWhitespaceSensitive && path.previous?.type === "ElementNode" ? softline : "",
    ...
    case "AttrNode": {
      ...
      const quote = isText
        ...
        : value.type === "ConcatStatement"
          ? getPreferredQuote(
              value.parts.map((part) => (part.type === "TextNode" ? part.chars : "")).join(""),
              options.singleQuote,
            )
          : "";
      ...
      if (attrName) {
        ...
        if (attrName === "class") {
          ...
          if (path.parent.type === "ConcatStatement") {
            if (path.previous?.type === "MustacheStatement" && /^\s/.test(text)) {
              leadingSpace = true;
            }
            if (
...
const hasSamePathHeadName = ({ path: pathA }, { path: pathB }) =>
  [pathA, pathB].every((node) => node.type === "PathExpression" && node.head.type === "VarHead") &&
...
function isElseIfBlock(path) {
  if (
    !path.match(
      ...
      (node, key) => key === "body" && node.type === "Block" && node.body.length === 1,
...
function printElseIfBlock(path, print) {
  ...
  return group([
    ...
    indent([
      ...
      ...(isNonEmptyArray(node.program.blockParams) ? [line, printBlockParams(node.program)] : []),
    ]),
    softline,
...
function blockStatementHasOnlyWhitespaceInProgram(node) {
  return (
    node.type === "BlockStatement" && node.program.body.every((node) => isWhitespaceNode(node))
  );
}
...
function printPathExpression(node) {
  ...
  return parts
    .map((part, index) => (isPathExpressionPartNeedBrackets(part, index) ? `[${part}]` : part))
    .join(".");
}
