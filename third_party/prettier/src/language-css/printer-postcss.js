...
function genericPrint(path, options, print) {
  ...
  switch (node.type) {
    case "css-root": {
      ...
      return [
        node.frontMatter
          ? [print("frontMatter"), hardline, node.nodes.length > 0 ? hardline : ""]
          : "",
        nodes,
        after ? ` ${after}` : "",
    ...
    case "css-decl": {
      ...
      return [
        ...
        // Less variable
        (parentNode.type === "css-atrule" && parentNode.variable) || insideIcssRuleNode(path)
          ? node.prop
        ...
        node.nodes
          ? [
              " {",
              node.nodes.length > 0 ? indent([softline, printSequence(path, options, print)]) : "",
              softline,
              "}",
            ]
    ...
    case "media-feature":
      return maybeToLowerCase(adjustStrings(node.value.replaceAll(/ +/g, " "), options));
    ...
    case "selector-root":
      return group([
        insideAtRuleNode(path, "custom-selector")
          ? [path.findAncestor((node) => node.type === "css-atrule").customSelector, line]
          : "",
        join(
    ...
    case "selector-selector": {
      ...
      return group((shouldIndent ? indent : (x) => x)(path.map(print, "nodes")));
    }
    ...
    case "selector-attribute":
      return [
        ...
        node.value
          ? replaceEndOfLine(
              quoteAttributeValue(adjustStrings(node.value.trim(), options), options),
              literallineWithoutBreakParent,
            )
          : "",
        node.insensitive ? " i" : "",
    ...
    case "selector-combinator": {
      if (node.value === "+" || node.value === ">" || node.value === "~" || node.value === ">>>") {
        const parentNode = path.parent;
        const leading =
          parentNode.type === "selector-selector" && parentNode.nodes[0] === node ? "" : line;
