...
function printCommaSeparatedValueGroup(path, options, print) {
  ...
  const isGridValue =
    ...
    (declAncestorProp === "grid" || declAncestorProp.startsWith("grid-template"));
  const atRuleAncestorNode = path.findAncestor((node) => node.type === "css-atrule");
  const isControlDirective =
    atRuleAncestorNode && isSCSSControlDirectiveNode(atRuleAncestorNode, options);
  ...
  for (let i = 0; i < node.groups.length; ++i) {
    ...
    */
    ...
    if (options.parser === "less") {
      ...
      // `@var [ @@foo ][ bar ]`
      //               ^^
      if (iNode.type === "value-word" && iNode.value === "][" && iNextNode?.type === "value-word") {
        continue;
      }
    }
    ...
    if (
      ...
      ((isDivisionNode(iNextNode) && !requireSpaceBeforeOperator) ||
        ...
        isSubtractionNode(iNode)) &&
      (hasEmptyRawBefore(iNextNode) ||
        (isMathOperator && (!iPrevNode || (iPrevNode && isMathOperatorNode(iPrevNode)))))
    ) {
      continue;
    }
    ...
    if (declAncestorProp && (declAncestorProp === "font" || declAncestorProp.startsWith("--"))) {
      if (isDivisionNode(iNextNode) && hasEmptyRawBefore(iNextNode) && isPossibleFontSize(iNode)) {
        continue;
      }

      if (isDivisionNode(iNode) && hasEmptyRawBefore(iNode) && isPossibleFontSize(iPrevNode)) {
        continue;
      }
    }
