...
function quoteAttributeValue(value, options) {
  ...
  let flag = "";
  const match = value.match(/^(?<value>.+?)\s+(?<flag>[a-z])$/i);
  if (match) {
    ({ value, flag } = match.groups);
  }

  return (
    (value.includes('"') || value.includes("'") ? value : quote + value + quote) +
    (flag ? ` ${flag}` : "")
  );
}

function adjustNumbers(value) {
  return value.replaceAll(ADJUST_NUMBERS_REGEX, (match, quote, wordPart, number, unit) => {
    if (!wordPart && number) {
      unit ??= "";
