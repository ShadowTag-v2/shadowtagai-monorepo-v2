...
function massageAstNode(original, cloned, parent) {
  ...
  if (
    (original.type === "value-word" &&
      ((original.isColor && original.isHex) ||
        ["initial", "inherit", "unset", "revert"].includes(original.value.toLowerCase()))) ||
    original.type === "media-feature" ||
  ...
  if (original.type === "selector-attribute") {
    ...
    if (original.value) {
      ...
      value = value.replace(/^(?<quote>["'])(?<value>.*?)\k<quote>$/, "$<value>");
  ...
  if (
    ...
    cloned.value = cloned.value.replaceAll(/([\d+.e-]+)([a-z]*)/gi, (match, numStr, unit) => {
  ...
  if (
    ...
    original.groups.some(
      (node) =>
        (node.type === "value-atword" && (node.value.endsWith("[") || node.value.endsWith("]"))) ||
        (node.type === "value-word" && (node.value.startsWith("]") || node.value.startsWith("["))),
    )
  ) {
    return {
      type: "value-atword",
