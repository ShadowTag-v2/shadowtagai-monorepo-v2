...
function printDanglingComments(path, options, danglingCommentsPrintOptions = {}) {
  ...
  const danglingComments = new Set(
    path.node?.comments?.filter(
      (comment) =>
        !(comment.leading || comment.trailing || comment.marker !== marker || !filter(comment)),
    ),
  );
  ...
  const parts = path
    .map(
      ({ node: comment }) => (danglingComments.has(comment) ? printComment(path, options) : ""),
      "comments",
    )
    .filter(Boolean);
...
function printComments(path, doc, options, printOptions) {
  ...
  return leading || trailing ? inheritLabel(doc, (doc) => [leading, doc, trailing]) : doc;
}
