...
import { addDanglingComment, addLeadingComment, addTrailingComment } from "./utilities.js";
...
function decorateComment(node, comment, options, enclosingNode, ancestors = []) {
  ...
  while (left < right) {
    ...
    if (start <= commentStart && commentEnd <= end) {
      ...
      return decorateComment(child, comment, options, child, [child, ...ancestors]);
    }
  ...
  if (enclosingNode?.type === "TemplateLiteral") {
    ...
    if (
      precedingNode &&
      findExpressionIndexForComment(quasis, precedingNode, options) !== commentIndex
    ) {
      precedingNode = null;
    }
    if (
      followingNode &&
      findExpressionIndexForComment(quasis, followingNode, options) !== commentIndex
    ) {
      followingNode = null;
    }
  }
...
function attachComments(ast, options) {
  ...
  const attachPropertiesToComment = process.env.NODE_ENV !== "production" || !avoidAstMutation;
