...
import { hardline, join, line, replaceEndOfLine } from "../../document/index.js";
...
function isDanglingSpaceSensitiveNode(node, options) {
  return isDanglingSpaceSensitiveCssDisplay(node.cssDisplay) && !isScriptLikeTag(node, options);
}
...
function isVueNonHtmlBlock(node, options) {
  return (
    ...
    (isVueCustomBlock(node, options) || (node.attrMap.lang && node.attrMap.lang !== "html"))
  );
}
...
function getTextValueParts(node, value = node.value) {
  return node.parent.isWhitespaceSensitive
    ? node.parent.isIndentationSensitive
      ...
      : replaceEndOfLine(htmlWhitespace.dedentString(htmlTrimPreserveIndentation(value)), hardline)
