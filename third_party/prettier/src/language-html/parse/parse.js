...
import { normalizeParseOptions, toAngularHtmlParserParseOptions } from "./parse-options.js";
...
function parseVue(input, parseOptions) {
  ...
  let { rootNodes, errors } = angularHtmlParserParse(input, angularHtmlParserParseOptions);
  ...
  const getElementWithSameLocation = (node) => {
    ...
    return (
      getHtmlParseResult().rootNodes.find(
        (searching) =>
          searching.kind === "element" && searching.startSourceSpan.start.offset === offset,
      ) ?? node
    );
  };
...
function parseSubHtml(parser, text, subContent, startSpan, parseOptions, options) {
  const { offset } = startSpan;
  const textToParse = replaceNonLineBreaksWithSpace(text.slice(0, offset)) + subContent;
  const subAst = parse(
    textToParse,
    parser,
...
function parse(text, parser, parseOptions, options = {}) {
  const { frontMatter, content: textToParse } = parseOptions.shouldParseFrontMatter
  ...
  const { parseOptions: actualParseOptions, rootNodes } = parser(textToParse, parseOptions);
  ...
  if (frontMatter) {
    const [start, end] = [frontMatter.start, frontMatter.end].map(
      (location) => new ParseLocation(file, location.index, location.line - 1, location.column),
    );
    htmlFrontMatter = {
      ...frontMatter,
      kind: "frontMatter",
  ...
  const ast = postprocess(rawAst, htmlFrontMatter, actualParseOptions, (subContent, startSpan) =>
    parseSubHtml(parser, text, subContent, startSpan, actualParseOptions, options),
  );
