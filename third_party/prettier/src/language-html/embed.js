import { breakParent, group, hardline, indent, line } from "../document/index.js";
      ...
      if (isScriptLikeTag(node.parent, options)) {
        ...
        if (parser) {
          return async (textToDoc) => {
            const value =
              parser === "markdown"
                ? htmlWhitespace.dedentString(node.value.replace(/^[^\S\n]*\n/, ""))
                : node.value;
        ...
        return async (textToDoc) => {
          ...
          if (options.parser === "angular") {
            ...
            textToDocOptions.parser = isVueSfcWithTypescriptScript(path, options)
              ? "__vue_ts_expression"
              : "__vue_expression";
          } else {
