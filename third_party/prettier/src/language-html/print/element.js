...
function printElement(path, options, print) {
  ...
  const shouldHugContent =
    ...
    (node.firstChild.kind === "interpolation" || node.firstChild.kind === "angularIcuExpression") &&
    ...
    if (
      (node.lastChild.kind === "comment" ||
        (node.lastChild.kind === "text" &&
          node.isWhitespaceSensitive &&
          node.isIndentationSensitive)) &&
      new RegExp(String.raw`\n[\t ]{${options.tabWidth * (path.ancestors.length - 1)}}$`).test(
  ...
  return printTag([
    ...
    printChildrenDoc([printLineBeforeChildren(), printChildren(path, options, print)]),
