...
function genericPrint(path, options, print) {
  ...
  if (
    isNode(node, ["mapping", "sequence", "comment", "directive", "mappingItem", "sequenceItem"]) &&
  ...
  if (hasPrettierIgnore(path)) {
    parts.push(
      replaceEndOfLine(
        options.originalText.slice(node.position.start.offset, node.position.end.offset).trimEnd(),
      ),
    );
  } else {
    ...
    parts.push(
      alignWithSpaces(node.type === "sequenceItem" ? 2 : 0, [
        ...
        join(
          ...
          path.map(
            ({ node }) => [
              isPreviousLineEmpty(options.originalText, locStart(node)) ? hardline : "",
              print(),
            ],
            "endComments",
          ),
