...
import { getSourceType, SOURCE_TYPE_COMBINATIONS } from "./utilities/source-types.js";
...
function parseWithOptions(filepath, text, options) {
  ...
  for (const error of errors) {
    if (
      ...
      (error.message === "A 'return' statement can only be used within a function body." ||
...
function parseJs(text, options) {
  ...
  const combinations = (sourceType ? [sourceType] : SOURCE_TYPE_COMBINATIONS).map(
    (sourceType) => () => parseWithOptions(filepath, text, { sourceType, lang: "jsx" }),
  );
...
function parseTs(text, options) {
  ...
  const combinations = (sourceType ? [sourceType] : SOURCE_TYPE_COMBINATIONS).flatMap(
    (sourceType) =>
      languageCombinations.map(
        (lang) => () => parseWithOptions(filepath, text, { astType: "ts", sourceType, lang }),
      ),
  );
