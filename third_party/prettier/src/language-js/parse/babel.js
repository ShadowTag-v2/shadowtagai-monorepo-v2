  ...
  return (text, options = {}) => {
    ...
    if (text.includes("|>")) {
      ...
      combinations = conflictsPlugins.flatMap((pipelineOperatorPlugin) =>
        combinations.map((options) => appendPlugins([pipelineOperatorPlugin], options)),
      );
    ...
    try {
      ast = tryCombinationsSync(
        combinations.map((options) => () => parseWithOptions(parseFunction, text, options)),
      );
...
const babelFlow = /* @__PURE__ */ createBabelParser({
  optionsCombinations: [appendPlugins(["jsx", ["flow", { all: true }], "flowComments"])],
});
