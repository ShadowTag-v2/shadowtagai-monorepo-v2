...
import { printDeclareToken, printSemicolon, printTrailingComma } from "./miscellaneous.js";
...
function printSemicolonAfterExportDeclaration(node, options) {
  if (!node.declaration || (isDefaultExport(node) && !shouldOmitSemicolon(node.declaration))) {
...
function printModuleSource(path, options, print) {
  ...
  return [shouldPrintSpecifiers(node, options) ? " from" : "", " ", print("source")];
}
...
function shouldPrintSpecifiers(node, options) {
  ...
  const text = stripComments(options).slice(locStart(node), locStart(node.source));
...
function getImportAttributesKeyword(node, options) {
  const textBetweenSourceAndAttributes = stripComments(options)
    .slice(locEnd(node.source), node.attributes?.[0] ? locStart(node.attributes[0]) : locEnd(node))
    .trimStart();
