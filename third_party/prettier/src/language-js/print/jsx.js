...
function printJsxElementInternal(path, options, print) {
  ...
  const containsMultipleExpressions =
    node.children.filter((child) => child.type === "JSXExpressionContainer").length > 1;
  ...
  const whitespace = isMdxBlock ? line : ifBreak([rawJsxWhitespace, softline], " ");
    ...
    (!hasComment(node) &&
      (isArrayExpression(node) ||
        ...
        (node.type === "AwaitExpression" &&
          (shouldInline(node.argument, node) || node.argument.type === "JSXElement")) ||
...
function printJsxOpeningElement(path, options, print) {
  ...
  const nameHasComments = hasComment(node.name) || hasComment(node.typeArguments);
...
function shouldPrintBracketSameLine(node, options, nameHasComments) {
  const lastAttrHasTrailingComments =
    node.attributes.length > 0 && hasComment(node.attributes.at(-1), CommentCheckFlags.Trailing);
  return (
