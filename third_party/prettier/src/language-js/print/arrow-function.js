  ...
  if (!shouldAddParensIfNotBreakCache.has(node)) {
    shouldAddParensIfNotBreakCache.set(
      ...
      node.type === "ConditionalExpression" &&
        !startsWithNoLookaheadToken(node, (node) => node.type === "ObjectExpression"),
    );
  }
...
function mayBreakAfterShortPrefix(functionBody, bodyDoc, options) {
  return (
    ...
    (bodyDoc.label?.hug !== false &&
      (bodyDoc.label?.embed || isTemplateOnItsOwnLine(functionBody, options.originalText)))
  );
}
...
function printArrowFunctionBody(
  ...
  const trailingComma = args.expandLastArg ? printTrailingComma(options, "all") : "";
