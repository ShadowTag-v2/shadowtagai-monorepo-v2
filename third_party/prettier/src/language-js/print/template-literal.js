...
function printTaggedTemplateExpression(path, options, print) {
  ...
  /** @type {Doc} */
  let space = "";
  const quasiLeadingComment = getComments(node.quasi, CommentCheckFlags.Leading)[0];
          ...
          table.map((row) =>
            join(
              ...
              row.cells.map((cell, index) =>
                row.hasLineBreak
                  ? cell
                  : cell + " ".repeat(maxColumnWidths[index] - getStringWidth(cell)),
              ),
            ),
          ),
        ),
...
function printTemplateExpression(path, options, print) {
  ...
  let interpolationHasNewline = hasNewlineInRange(options.originalText, start, end);
  ...
  // of the `.
  let { indentSize, previousQuasiText } = getTemplateLiteralExpressionIndent(path, options);
