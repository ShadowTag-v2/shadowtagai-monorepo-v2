import { group, hardline, ifBreak, indent, line, softline } from "../../document/index.js";
import { printDanglingComments } from "../../main/comments/print.js";
import hasNewline from "../../utilities/has-newline.js";
import hasNewlineInRange from "../../utilities/has-newline-in-range.js";
import { locEnd, locStart } from "../location/index.js";
import { iterateClassMembersPath } from "../utilities/class-members.js";
import { CommentCheckFlags, getComments, hasComment } from "../utilities/comments.js";
...
function printClassBody(path, options, print) {
  ...
  if (node.type === "ObjectTypeAnnotation" && node.inexact) {
    ...
    if (hasComment(node, CommentCheckFlags.Dangling)) {
      ...
      printed = [
        hasLineComments || hasNewline(options.originalText, locEnd(getComments(node).at(-1)))
          ? hardline
          : line,
        "...",
      ];
  ...
  if (isObjectType) {
    const shouldBreak =
      ...
      (options.objectWrap === "preserve" &&
        firstMember &&
        hasNewlineInRange(options.originalText, locStart(node), locStart(firstMember)));
    ...
    if (parts.length === 0) {
      ...
      content = [openingBrace, indent([spacing, ...parts]), spacing, closingBrace];
    }
    ...
    if (
      ...
      path.match(
        undefined,
        (node, name) => node.type === "FunctionTypeParam" && name === "typeAnnotation",
        shouldHugTheOnlyParameter,
      )
    ) {
      return content;
...
function printClassMemberSemicolon(path, options) {
  ...
  if (path.callParent(isClassBody)) {
    return parent.type === "ObjectTypeAnnotation" ? ";" : printSemicolon(options);
  }
  ...
  if (parent.type === "TSTypeLiteral") {
    ...
    if (
      options.semi ||
      shouldPrintSemicolonAfterInterfaceProperty({ node: path.node, next: path.next }, options)
    ) {
      return ";";
    }
...
const isKeywordProperty = (node) => {
  ...
  return type === "Identifier" && (name === "static" || name === "get" || name === "set");
};

/**
 * @returns {boolean}
 */
function shouldPrintSemicolonAfterClassProperty({ node, next: nextNode }, options) {
...
function shouldPrintSemicolonAfterInterfaceProperty({ node, next: nextNode }, options) {
