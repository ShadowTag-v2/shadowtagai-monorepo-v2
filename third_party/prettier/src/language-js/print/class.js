import { group, hardline, ifBreak, indent, join, line, softline } from "../../document/index.js";
import {
  printComments,
...
function printClass(path, options, print) {
  ...
  if (node.type === "ClassExpression" && isNonEmptyArray(node.decorators)) {
    ...
    return needsParens ? [indent([softline, decoratorsDoc, doc]), softline] : [decoratorsDoc, doc];
  }
...
function printClassWithoutDecorators(path, options, print) {
  ...
  const keyword = isPrintingInterface ? "interface" : isPrintingRecord ? "record" : "class";
  ...
  if (node.superClass) {
    const printed = [printSuperClass(path, options, print), print("superTypeArguments")];
  ...
  if (groupMode) {
    ...
    parts.push(group([...partsGroup, indent(extendsParts)], { id: heritageGroupId }));
  } else {
...
function shouldPrintClassInGroupModeWithoutCache(path) {
  ...
  if (node.superClass) {
    ...
    return (
      !node.superTypeArguments && isMemberExpression(stripChainElementWrappers(node.superClass))
    );
  }

  const heritage = node.extends?.[0] ?? node.mixins?.[0] ?? node.implements?.[0];
  ...
  const groupMode =
    ...
    ((heritage.type === "TSClassImplements" || heritage.type === "TSInterfaceHeritage") &&
...
function shouldPrintClassInGroupMode(path) {
  ...
  if (!shouldPrintClassInGroupModeCache.has(node)) {
    shouldPrintClassInGroupModeCache.set(node, shouldPrintClassInGroupModeWithoutCache(path));
  }
...
function printHeritageClauses(path, options, print, listName) {
  ...
  if (!hasMultipleHeritage(node)) {
    const printed = [`${listName} `, printedLeadingComments, heritageClausesDoc];
...
function printClassProperty(path, options, print) {
  ...
  const isAbstractProperty =
    node.type === "TSAbstractPropertyDefinition" || node.type === "TSAbstractAccessorProperty";

  return [
    printAssignment(path, options, print, parts, " =", isAbstractProperty ? undefined : "value"),
