...
import { getCallArguments, iterateCallArgumentsPath } from "../utilities/call-arguments.js";
...
function printCallExpression(path, options, print) {
  ...
  if (
    ...
    if (!(isTemplateLiteralSingleArg && printed[0].label?.embed)) {
      return [printCallee(path, print), optional, typeArgumentsDoc, "(", join(", ", printed), ")"];
    }
  }
...
function printCallee(path, print) {
  ...
  return [node.type === "NewExpression" ? "new " : "", print("callee"), lineSuffixBoundary];
}
