...
import { isArrayExpression, isObjectExpression, isObjectType } from "../utilities/node-types.js";
import { isTestCall } from "../utilities/test-libraries.js";
import { printDanglingCommentsInList, printTrailingComma } from "./miscellaneous.js";

/** @import AstPath from "../../common/ast-path.js" */

// `ArrowFunctionExpression` has other dangling comments
const functionParameterDanglingCommentFilter = (comment) => comment.mark !== "commentBeforeArrow";
...
function printFunctionParameters(
  ...
  const typeParametersDoc =
    shouldPrintTypeParameters && functionNode.typeParameters ? print("typeParameters") : "";

  if (parameters.length === 0) {
    return [
      typeParametersDoc,
      "(",
      printDanglingCommentsInList(path, options, functionParameterDanglingCommentFilter),
      ")",
    ];
  }
  ...
  if (shouldExpandParameters && !isDecoratedFunction(path)) {
    ...
    return group([removeLines(typeParametersDoc), "(", removeLines(printed), ")"]);
  }
    ...
    (parameter.type === "ObjectPattern" ||
      ...
      (parameter.type === "AssignmentPattern" &&
        ...
        (parameter.right.type === "Identifier" ||
          (isObjectExpression(parameter.right) && parameter.right.properties.length === 0) ||
          (isArrayExpression(parameter.right) && parameter.right.elements.length === 0))))
  );
}
