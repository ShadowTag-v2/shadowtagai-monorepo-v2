...
import { group, hardline, indent, line, softline } from "../../document/index.js";
...
function printObject(path, options, print) {
  ...
  const property = isEnumBody ? "members" : isImportAttributes ? "attributes" : "properties";
  ...
  const canHaveTrailingSeparator = !(hasUnknownMembers || children.at(-1)?.type === "RestElement");
  ...
  if (
    path.match(
      (node) => node.type === "ObjectPattern" && !isNonEmptyArray(node.decorators),
...
function hasNewLineAfterOpeningBrace(node, firstProperty, options) {
  ...
  if (isPrintingImportAttributes(node)) {
    openingBraceIndex = stripComments(options).lastIndexOf("{", firstPropertyStart);
    /* c8 ignore next 3 */
