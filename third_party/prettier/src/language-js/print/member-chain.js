...
function printMemberChain(path, options, print) {
  const isExpressionStatement =
    (path.parent.type === "ChainExpression" ? path.grandparent : path.parent).type ===
    "ExpressionStatement";
  ...
  function rec() {
    ...
    } else if (node.type === "ChainExpression" && !needsParentheses(path, options)) {
      path.call(rec, "expression");
    } else if (node.type === "TSNonNullExpression" && !needsParentheses(path, options)) {
      printedNodes.unshift({
        node,
  ...
  const shouldHaveEmptyLineBeforeIndent =
    !isCallExpression(lastNodeBeforeIndent) && shouldInsertEmptyLineAfter(lastNodeBeforeIndent);
  ...
  function lastGroupWillBreakAndOtherCallsHaveFunctionArguments() {
    ...
    return (
      ...
      callExpressions.slice(0, -1).some((node) => node.arguments.some(isFunctionOrArrowExpression))
    );
  }
  ...
  if (
    ...
    (callExpressions.length > 2 &&
      callExpressions.some((expr) => !expr.arguments.every((arg) => isSimpleCallArgument(arg)))) ||
