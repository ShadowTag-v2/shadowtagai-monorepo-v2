import { group, hardline, indent, replaceEndOfLine, softline } from "../../document/index.js";
...
import { isArrayExpression, isLiteral, isObjectExpression } from "../utilities/node-types.js";
...
import { printDefiniteToken, printOptionalToken, printSemicolon } from "./miscellaneous.js";
...
import { printReturnStatement, printThrowStatement } from "./return-statement.js";
...
import { printTaggedTemplateExpression, printTemplateLiteral } from "./template-literal.js";
  ...
  switch (node.type) {
    ...
    case "ParenthesizedExpression": {
      const shouldHug =
        ...
        (isObjectExpression(node.expression) || isArrayExpression(node.expression));
      if (shouldHug) {
        return ["(", print("expression"), ")"];
      }
    ...
    case "YieldExpression":
      return [`yield${node.delegate ? "*" : ""}`, node.argument ? [" ", print("argument")] : ""];
    ...
    case "UnaryExpression": {
      ...
      if (hasComment(node.argument)) {
        parts.push(group(["(", indent([softline, argumentDoc]), softline, ")"]));
      } else {
