...
function shouldBreakClosingParen(node, parent) {
  return (
    (isMemberExpression(parent) || (parent.type === "NGPipeExpression" && parent.left === node)) &&
    !parent.computed
  );
}
...
function printTernary(path, options, print, args) {
  ...
  do {
    ...
    testNodePropertyNames.every((prop) => currentParent[prop] !== previousParent)
  );
  ...
  const isOnSameLineAsReturn =
    isReturnOrThrowStatement(parent) && !(isConsequentTernary || isAlternateTernary);
  ...
  const breakTSClosingParen = isTSConditional && needsParentheses(path, options);
  ...
  const shouldBreak =
    hasMultilineBlockComments(testNodes, consequentNode, alternateNode, options) ||
    isConsequentTernary ||
    isAlternateTernary;
  ...
  const printedTest = isConditionalExpression
    ...
    : [
        ...
        "extends",
        " ",
        isConditionalType(node.extendsType) || node.extendsType.type === "TSMappedType"
          ? print("extendsType")
