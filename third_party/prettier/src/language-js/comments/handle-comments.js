...
function handleNestedConditionalExpressionComments({
  ...
  const enclosingIsCond =
    enclosingNode?.type === "ConditionalExpression" || isConditionalType(enclosingNode);
  ...
  const followingIsCond =
    followingNode?.type === "ConditionalExpression" || isConditionalType(followingNode);
...
function handleMethodNameComments({
  ...
  if (isPropertyLikeNode(enclosingNode) && !followingNode && placement === "remaining") {
    addTrailingComment(
      getNextNonSpaceNonCommentCharacter(text, locEnd(comment)) === "("
        ? precedingNode
        : enclosingNode,
...
function handleLastFunctionParameterComments({
  ...
  if (
    ...
    const functionBody =
      enclosingNode.type === "MethodDefinition" ? enclosingNode.value.body : enclosingNode.body;
...
function handleCallExpressionComments({ comment, precedingNode, enclosingNode, options }) {
  if (
...
function handleMatchOrPatternComments({ comment, precedingNode, enclosingNode, followingNode }) {
...
function handleModuleSpecifiersComments({ comment, precedingNode, enclosingNode, text }) {
  ...
  const isImportDeclaration =
    precedingNode?.type === "ImportSpecifier" && enclosingNode?.type === "ImportDeclaration";
  const isExportDeclaration =
    precedingNode?.type === "ExportSpecifier" && enclosingNode?.type === "ExportNamedDeclaration";
...
function handleCommentsInDestructuringPattern({
  ...
  if (
    (enclosingNode?.type === "ObjectPattern" || enclosingNode?.type === "ArrayPattern") &&
...
function handleLastBinaryOperatorOperand({
  ...
  if (
    ...
    (precedingNode?.type === "LogicalExpression" || precedingNode?.type === "BinaryExpression")
  ) {
    //   !(
    //     (cond1 || cond2) // foo
    //   );
    ...
    if (
      ...
      hasNewlineInRange(text, locStart(enclosingNode.argument), locStart(precedingNode.right)) &&
...
function handlePropertySignatureComments({ enclosingNode, followingNode, comment }) {
  if (
    enclosingNode &&
    (enclosingNode.type === "TSPropertySignature" || enclosingNode.type === "ObjectTypeProperty") &&
...
*/
function isCommentBeforeArrowFunctionExpressionArrow(comment, arrowFunctionExpression, options) {
...
function handleArrowExpressionComments({
  ...
  if (enclosingNode?.type !== "ArrowFunctionExpression" || !followingNode || !precedingNode) {
    return false;
  }
