...
import { isArrayExpression, isBigIntLiteral, isStringLiteral } from "../utilities/node-types.js";
...
function massageAstNode(original, cloned, parent) {
  ...
  if (
    (isBigIntLiteral(original) || original.type === "BigIntLiteralTypeAnnotation") &&
    "bigint" in original
  ) {
  ...
  if (
    ...
    cloned.value.value = original.value.value.replaceAll(/["']|&quot;|&apos;/g, '"');
  }
  ...
  if (
    (original.type === "CallExpression" || original.type === "MemberExpression") &&
