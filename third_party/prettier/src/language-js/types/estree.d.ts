...
export type NumericLiteral = NodeMap["NumericLiteral"] | (NodeMap["Literal"] & { value: number });

export type StringLiteral = NodeMap["StringLiteral"] | (NodeMap["Literal"] & { value: string });

export type RegExpLiteral =
  | NodeMap["RegExpLiteral"]
  | (NodeMap["Literal"] & { regexp: { pattern: string; flags: string } });

export type BigIntLiteral = NodeMap["BigIntLiteral"] | (NodeMap["Literal"] & { bigint: string });
