...
const FLOW_TYPES = new URL("./node_modules/hermes-estree/dist/types.js.flow", PROJECT_ROOT);
const FLOW_TYPES_DTS = new URL("./src/language-js/types/flow-estree.d.ts", PROJECT_ROOT);

let text = await fs.readFile(FLOW_TYPES, "utf8");
text = toDts(text);

const getRelativePath = (url) => new URL(url).href.slice(PROJECT_ROOT.href.length);
text = `
// ! Do NOT edit !
...
function toDts(text) {
  ...
  text = text.replaceAll(/(?<=: )interface extends (?<type>\w+)(?= \{)/g, "$<type> & ");

  // `$ReadOnlyArray<?T>` -> `ReadonlyArray<T | null>`
  text = text.replaceAll(/\$ReadOnlyArray<\?(\w+)>/g, "ReadonlyArray<$1 | null>");
