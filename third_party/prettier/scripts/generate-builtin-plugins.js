...
const pluginFiles = [
  {
    ...
    file: new URL("main/plugins/builtin-plugins/development-plugins.js", sourceDirectory),
    getPluginName: (file) => path.dirname(file).slice("language-".length),
  },
  {
    kind: "production",
    pattern: "plugins/*.js",
    file: new URL("main/plugins/builtin-plugins/production-plugins.js", sourceDirectory),
...
async function getPluginData(file, pluginFile, getPluginName) {
  ...
  assert.ok(isValidIdentifier(pluginName), `Invalid plugin name '${pluginName}'.`);
...
function getImportStatements(plugins, file) {
  const entries = plugins.flatMap((plugin) =>
    ["languages", "options"].flatMap((property) => plugin[property]?.entries ?? []),
  );
...
function getExportStatements(plugins, file) {
  const codes = sortByUrl(plugins).map((plugin) => {
    ...
    for (const property of ["options", "languages"]) {
      ...
      const variableNames = plugin[property].entries.map(({ variableName }) => variableName);
...
async function buildPlugins({ kind, file, pattern, getPluginName }) {
  const plugins = await Array.fromAsync(fs.glob(pattern, { cwd: sourceDirectory }), (pluginFile) =>
    getPluginData(file, pluginFile, getPluginName, kind),
  );
...
async function locateLanguageOrOptions(pluginData, value, kind) {
  ...
  if (pluginData.name === "estree" && kind === "languages") {
    ...
    const languages = entries
      ...
      .toSorted((languageA, languageB) => languageA.name.localeCompare(languageB.name));
  ...
  throw new Error(`Can not locate '${kind}' entry for plugin '${pluginData.name}'.`);
}

function sortByUrl(array) {
  return array.toSorted((elementA, elementB) => elementA.url.href.localeCompare(elementB.url.href));
}
