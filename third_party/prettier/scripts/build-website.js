...
import { copyFile, DIST_DIR, PROJECT_ROOT, WEBSITE_DIR, writeFile } from "./utilities/index.js";
...
const NODE_MODULES_DIR = url.fileURLToPath(new URL("../node_modules", import.meta.url));
...
async function buildPlaygroundFiles(version) {
  ...
  const packageManifest = {
    ...
    plugins: await Promise.all(
      pluginFiles.map(async (file) => {
        ...
        const pluginModule = await import(url.pathToFileURL(path.join(packagesDirectory, file)));
  ...
  await Promise.all([
    ...[packageManifest.prettier, ...packageManifest.plugins].map(({ file }) =>
      copyFile(path.join(packagesDirectory, file), path.join(libDirectory, file)),
    ),
    writeScript(
...
async function getGitTreeInformation() {
  const { stdout: branch } = await spawn("git", ["rev-parse", "--abbrev-ref", "HEAD"]);
  ...
  const { stdout: commit } = await spawn("git", ["rev-parse", "--short", "HEAD"]);
