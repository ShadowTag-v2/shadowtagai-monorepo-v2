...
export function getEntries(dirPath, options) {
  ...
  const useFriendlyHeadingId = options?.useFriendlyHeadingId;
  const entries = fileNames.map((fileName) => {
    ...
    if (useFriendlyHeadingId) {
      // `1000` or `1000-2`
      const basename = path.basename(filePath, ".md");
      title += ` {#change-${basename}}`;
    }
...
export function replaceVersions(data, prevVer, newVer, isPatch = false) {
  ...
  return data
    .replaceAll(/prettier stable/gi, `Prettier ${isPatch ? prevVer : formatVersion(prevVer)}`)
    .replaceAll(/prettier main/gi, `Prettier ${isPatch ? newVer : formatVersion(newVer)}`);
}
...
function processTitle(title) {
  return title
    ...
    .replaceAll(/(?<![[`])#(\d{4,})/g, "[#$1](https://github.com/prettier/prettier/pull/$1)");
}
