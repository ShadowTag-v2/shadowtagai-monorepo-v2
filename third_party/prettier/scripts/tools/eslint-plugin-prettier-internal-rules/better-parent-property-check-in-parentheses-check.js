...
const parentPropertyCheckSelector = [
  "FunctionDeclaration",
  ":matches(",
  ["needsParens", "needsParentheses"].map((name) => `[id.name="${name}"]`).join(","),
  ")",
  " ",
  "BinaryExpression",
  ":matches(",
  ...
  create(context) {
    ...
    return {
      [parentPropertyCheckSelector](node) {
        ...
        const propertyText =
          property.type === "Identifier" ? `"${property.name}"` : sourceCode.getText(property);
      ...
      [keyCheckSelector](node) {
        context.report({
          ...
          fix(fixer) {
            ...
            const left = sourceCode.text.slice(node.range[0], operatorToken.range[0]);
            const right = sourceCode.text.slice(operatorToken.range[1], node.range[1]);
