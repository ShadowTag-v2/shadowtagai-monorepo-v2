...
const relativePath = file.href.slice(new URL("../", import.meta.url).href.length);
...
async function addNewChangelog(prNumber, category, content) {
  const file = new URL(`../changelog_unreleased/${category}/${prNumber}.md`, import.meta.url);
...
async function createChangelog(title, user, prNumber, category) {
  const templateFile = new URL("../changelog_unreleased/TEMPLATE.md", import.meta.url);
  ...
  return changelogTemplate
    ...
    .replace(stableCommentPart, generateComment(syntax, "Prettier stable") + "\n")
