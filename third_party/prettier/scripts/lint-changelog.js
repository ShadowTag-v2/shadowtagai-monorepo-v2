...
const template = fs.readFileSync(new URL(TEMPLATE_FILE, CHANGELOG_ROOT), "utf8");
...
for (const category of CHANGELOG_CATEGORIES) {
  ...
  for (const prFile of files) {
    ...
    const content = fs.readFileSync(new URL(`${category}/${prFile}`, CHANGELOG_ROOT), "utf8");
    ...
    for (const comment of templateComments) {
      if (comment !== "<!-- prettier-ignore -->" && content.includes(comment)) {
        showErrorMessage(
          `[${displayPath}]: Please remove ${getCommentDescription(content, comment)}`,
        );
      }
    }
...
function validateTitle(displayPath, title) {
  ...
  for (const node of children) {
    if (node.type === "html") {
      showErrorMessage(`[${displayPath}]: HTML "${node.value}" is restricted in title.`);
    }
  }
}
