...
export default defineConfig({
  ...
  define: {
    "import.meta.env.VITE_IS_PR": JSON.stringify(process.env.PULL_REQUEST === "true"),
  },
});
...
function buildCdnAlias() {
  ...
  return Object.fromEntries(
    getPackageDependencies(cdnDependencies).map(({ name, packageJson, dependencies }) => {
      const url = new URL(`https://esm.sh/${packageJson.name}@${packageJson.version}`);
