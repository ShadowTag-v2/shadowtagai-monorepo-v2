...
import { Decoration, EditorView, keymap, lineNumbers as lineNumbersExt } from "@codemirror/view";
...
function setup(props, { emit }) {
  ...
  const componentDidMount = () => {
    const options = { ...props };
    for (const property of ["ruler", "rulerColor", "value", "selection", "onChange"]) {
    ...
    const state = EditorState.create({
      ...
      extensions: [
        ...
        languageExt.of([]),
        editorTheme.of(theme.value === "dark" ? catppuccinMocha : catppuccinLatte),
        overlayField,
        history(),
        keymap.of([
          ...defaultKeymap,
  ...
  watch(
    ...
    (ruler) => {
      ...
      _codeMirror.dispatch({
        effects: rulerTheme.reconfigure(createRulerTheme(ruler, props.rulerColor)),
      });
    },
  );
  ...
  watch(
    ...
    (rulerColor) => {
      ...
      _codeMirror.dispatch({
        effects: rulerTheme.reconfigure(createRulerTheme(props.ruler, rulerColor)),
      });
    },
  );
  ...
  watch(
    ...
    (foldGutter) => {
      ...
      _codeMirror.dispatch({
        effects: foldGutterCompartment.reconfigure(foldGutter ? foldGutterExt() : []),
      });
    },
  );
  ...
  watch(theme, (newTheme) => {
    _codeMirror.dispatch({
      effects: editorTheme.reconfigure(newTheme === "dark" ? catppuccinMocha : catppuccinLatte),
    });
  });
...
export function OutputPanel(props) {
  return (
    <CodeMirrorPanel readOnly={true} lineNumbers={true} rulerColor="currentColor" {...props} />
  );
}
