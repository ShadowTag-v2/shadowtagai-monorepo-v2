...
function setup(props) {
  ...
  const rangeStartOption = props.availableOptions.find((opt) => opt.name === "rangeStart");
  const rangeEndOption = props.availableOptions.find((opt) => opt.name === "rangeEnd");
  const cursorOffsetOption = props.availableOptions.find((opt) => opt.name === "cursorOffset");
  ...
  const formatInput = () => {
    ...
    return worker
      ...
      .then(({ error, formatted, cursorOffset }) => {
        ...
        Object.assign(state, {
          content: formatted,
          selection: cursorOffset >= 0 ? { anchor: cursorOffset, head: cursorOffset } : {},
        });
      });
  };
  ...
  const insertDummyId = () => {
    ...
    const modifiedContent = content.slice(0, start) + dummyId + content.slice(end);
            ...
            <>
              <div class="editors-container">
                  ...
                  {isDocExplorer ? null : (
                    <SidebarCategory title="Range">
                      ...
                      <Option
                        ...
                        value={
                          typeof options.rangeStart === "number" ? options.rangeStart : undefined
                        }
                        onChange={handleOptionValueChange}
                      />
                      <Option
                        option={rangeEndOption}
                        value={typeof options.rangeEnd === "number" ? options.rangeEnd : undefined}
                        overrideMax={content.length}
                        onChange={handleOptionValueChange}
                      />

                      <Button onClick={setSelectionAsRange}>Set selected text as range</Button>
                    </SidebarCategory>
                  )}
                  {isDocExplorer ? null : (
                    <SidebarCategory title="Cursor">
                      <Option
                        ...
                        value={options.cursorOffset >= 0 ? options.cursorOffset : undefined}
                        onChange={handleOptionValueChange}
                      />
                      <div
                        style={{
                ...
                <div class="editors">
                  ...
                  {settings.showDoc && !isDocExplorer ? (
                    <DebugPanel name="Doc" mode="JavaScript" value={debug.doc || ""} />
                  ) : null}
