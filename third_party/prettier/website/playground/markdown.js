function formatMarkdown({ input, output, output2, doc, version, url, options, cliOptions, full }) {
  ...
  return [
    ...
    ...(output === undefined ? [] : ["", "**Output:**", codeBlock(output, syntax)]),
    ...(isIdempotent ? [] : ["", "**Second Output:**", codeBlock(output2, syntax)]),
    ...(full ? ["", "**Expected output:**", codeBlock("", syntax)] : []),
    ...(full ? ["", "**Why?**", "", "<!-- short explanation of expected output -->"] : []),
  ]
...
function formatCLIOptions(cliOptions) {
  return cliOptions.map(([name, value]) => (value === true ? name : `${name} ${value}`)).join("\n");
}

function codeBlock(content, syntax) {
  const backtickSequences = content.match(/`+/g) || [];
  const longestBacktickSequenceLength = Math.max(...backtickSequences.map(({ length }) => length));
