...
const RuleTester = eslintUnsupportedApi.FlatRuleTester ?? eslintPackage.RuleTester;
...
mdxRuleTester.run("eslint-plugin-mdx", rule, {
  valid: [
    {
      code: ["import React from 'react';", "", "<div>Hello World</div>", ""].join("\n"),
      filename: "valid.mdx",
    },
  ],
  invalid: [
...
function loadInvalidFixture(name) {
  const filename = url.fileURLToPath(new URL(`./invalid/${name}.txt`, import.meta.url));
...
function getPrettierRcJsFilename(dir, file = "dummy.js") {
  return url.fileURLToPath(new URL(`./prettierrc/${dir}/${file}`, import.meta.url));
}
