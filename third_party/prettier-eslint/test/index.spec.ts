...
test("fails with an error if the eslint module cannot be resolved.", async () => {
  const eslintPath = path.join(__dirname, "../__mocks__/non-existent-eslint-module");
...
test("fails with an error if the prettier module cannot be resolved.", async () => {
  const prettierPath = path.join(__dirname, "../__mocks__/non-existent-prettier-module");
...
test("resolves to the eslint module relative to the given filePath", async () => {
  ...
  const stateObj = {
    eslintPath: require.resolve("./fixtures/paths/node_modules/eslint/index.js"),
    prettierPath: require.resolve("./fixtures/paths/node_modules/prettier/index.js"),
  };
...
test("logs error if it cannot read the file from the filePath", async () => {
  ...
  await expect(() => format({ filePath: "/some-path.js" })).rejects.toThrow(/some error/);
  expect(logger.error).toHaveBeenCalledTimes(1);
...
function getESLintConfigWithDefaultRules(overrides?: Linter.RulesRecord): ESLintConfig {
  return {
    languageOptions: { parserOptions: { ecmaVersion: 7 } },
    rules: {
      semi: [2, "never"],
