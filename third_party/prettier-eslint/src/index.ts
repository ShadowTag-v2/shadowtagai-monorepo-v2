...
export async function analyze(options: FormatOptions): Promise<{
  ...
  const eslintFiles = eslintConfig.files?.length ? eslintConfig.files : DEFAULT_ESLINT_FILES;
  ...
  if (!formattingOptions.eslint.languageOptions.parser) {
    if ([".ts", ".tsx"].includes(fileExtension)) {
      ...
      formattingOptions.eslint.languageOptions.parser = await importModule("vue-eslint-parser");
...
function createPrettify(formatOptions: PrettierOptions, prettierPath: string) {
  return async function prettify(
    ...
    const prettier = await importModule<typeof import("prettier")>(prettierPath, "prettier");
    try {
...
async function getPrettierConfig(filePath: string | undefined, prettierPath: string) {
  const prettier = await importModule<typeof import("prettier")>(prettierPath, "prettier");
