...
import type { ESLintConfig, ESLintOptions, OptionGetter, StringLiteral } from "./types.ts";
...
function getPrettierOptionsFromESLintRules(
  ...
  if (ruleValueExists(prettierPluginOptions) && typeof prettierPluginOptions === "object") {
    prettierOptions = { ...prettierPluginOptions, ...prettierOptions };
  }

  return Object.keys(OPTION_GETTERS).reduce(
    (options, key) =>
      configureOptions(prettierOptions, fallbackPrettierOptions, key, options, rules),
    prettierOptions,
  );
}
...
export async function importModule<T>(modulePath: string, name = modulePath) {
  try {
    ...
    return typeof imported === "object" && "default" in imported! ? imported.default : imported;
  } catch (error) {
    logger.error(
      oneLine`
...
export async function getESLint(eslintPath: string, eslintOptions: ESLintOptions) {
  const { ESLint } = await importModule<typeof import("eslint")>(eslintPath, "eslint");
  try {
