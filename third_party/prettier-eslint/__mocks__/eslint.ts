...
async function mockCalculateConfigForFile(filePath: string): Promise<ESLintConfig> {
  if (
...
function mockLintText(this: MockESLint, ...args: Parameters<ESLintLintText>) {
  if ("throwError" in mockLintTextSpy && mockLintTextSpy.throwError instanceof Error) {
