...
def scan_repo(sqlite_db: str, repo_id: str, repo_root: str):
    ...
    with sqlite_conn(sqlite_db) as conn:
        ...
        for path in root.rglob("*"):
            ...
            cur.execute("INSERT INTO document_content_fts (doc_id, rel_path, title, body) VALUES (?, ?, ?, ?)", (doc_id, rel_path, path.name, body[:500000]))
            count += 1
    ...
    with sqlite_conn(sqlite_db) as conn:
        ...
        for doc_id, abs_path in rows:
            ...
            for name, kind, start, end in extract_symbols(text):
                ...
                cur.execute("INSERT OR REPLACE INTO symbols (symbol_id, doc_id, symbol_name, symbol_kind, signature, line_start, line_end, parent_symbol) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (sid, doc_id, name, kind, None, start, end, None))
                inserted += 1
    ...
    with sqlite_conn(sqlite_db) as conn:
        ...
        for doc_id, rel_path, abs_path, kind, language, sha in rows:
            ...
            for start in range(0, len(lines), step):
                ...
                cur.execute("INSERT OR REPLACE INTO chunk_manifest (chunk_id, doc_id, rel_path, chunk_type, section_path, start_line, end_line, sha256, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))", (chunk_id, doc_id, rel_path, kind, section_path, start + 1, end, sha))
    return chunks
...
def exact_search(sqlite_db: str, query: str, limit: int = 8):
    with sqlite_conn(sqlite_db) as conn:
        ...
        cur = conn.cursor()
        try:
            return cur.execute("SELECT doc_id, rel_path, title, snippet(document_content_fts, 3, '[', ']', ' … ', 16) FROM document_content_fts WHERE document_content_fts MATCH ? LIMIT ?", (query, limit)).fetchall()
        except Exception:
            return []
