#!/usr/bin/env python3
"""
shadowtag-omega-v4 Monte Carlo Valuation Simulator
...
def simulate_exit_value(base_revenue, base_margin, n_sims=SIMULATIONS):
    ...
    revenue_samples = np.random.lognormal(mean=np.log(base_revenue), sigma=REVENUE_VOLATILITY, size=n_sims)
...
def generate_report(results, projections):
    ...
    print("\n" + "=" * 60)
    print(f"shadowtag-omega-v4 Monte Carlo Simulation Results ({SIMULATIONS:,} runs)")
    print("=" * 60 + "\n")
...
def plot_distribution(results):
    ...
    # Histogram
    plt.hist(results["distribution"] / 1e9, bins=50, alpha=0.7, color="steelblue", edgecolor="black")

    # Add percentile lines
    plt.axvline(results["p10"] / 1e9, color="red", linestyle="--", linewidth=2, label="p10 (bear)")
    plt.axvline(results["p50"] / 1e9, color="green", linestyle="--", linewidth=2, label="p50 (base)")
    plt.axvline(results["p90"] / 1e9, color="orange", linestyle="--", linewidth=2, label="p90 (bull)")

    plt.xlabel("Exit Valuation ($B)", fontsize=12)
    plt.ylabel("Frequency", fontsize=12)
    plt.title(f"shadowtag-omega-v4 Exit Valuation Distribution ({SIMULATIONS:,} Monte Carlo Simulations)", fontsize=14, fontweight="bold")
