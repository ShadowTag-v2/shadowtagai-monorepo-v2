...
class EthicalComplianceMonitor:
    ...
    async def check_robots_txt(self, url: str) -> dict[str, Any]:
        ...
        try:
            ...
            async with (
                ...
                client.get(robots_url, headers={"User-Agent": self.user_agent}, timeout=aiohttp.ClientTimeout(total=5)) as response,
            ):
                if response.status == 200:
                    robots_content = await response.text()

                    # Simple robots.txt parsing (production should use robotparser)
                    is_allowed = self._parse_robots_txt(robots_content, parsed.path)

                    # Store compliance check
                    await self._store_compliance_check(
                        url, "robots_txt", is_allowed, f"robots.txt check: {'allowed' if is_allowed else 'disallowed'}"
                    )
            ...
            await self._store_compliance_check(url, "robots_txt", True, f"Error checking robots.txt: {str(e)}")
            return {
                "is_compliant": True,
    ...
    async def check_rate_limit(self, source_type: str, source_identifier: str) -> dict[str, Any]:
        """
        ...
        if limit_key in self.last_request_times:
            ...
            if elapsed < rate_limit_seconds:
                ...
                await self._store_compliance_check(limit_key, "rate_limit", False, f"Rate limit violated: {elapsed:.2f}s < {rate_limit_seconds}s")
    ...
    async def get_compliance_score(self, hours: int = 24) -> dict[str, Any]:
        """
        ...
        query = (
            select(
                ...
                func.sum(func.cast(EthicalCompliance.is_compliant, Integer)).label("compliant"),
        ...
        overall_score = (total_compliant / total_checks * 100) if total_checks > 0 else 100
    ...
    async def _store_compliance_check(self, source: str, check_type: str, is_compliant: bool, details: str):
    ...
    async def get_violations(self, hours: int = 24, limit: int = 50) -> list[dict[str, Any]]:
        """
        ...
        query = (
            ...
            .where(and_(EthicalCompliance.checked_at >= cutoff_time, not EthicalCompliance.is_compliant))
