...
class IngestionAnalyzer:
    ...
    async def analyze_runtime_efficiency(self, days: int = 7) -> dict[str, Any]:
        """
        ...
        query = (
            ...
            .where(and_(IngestionJob.started_at >= cutoff_time, IngestionJob.completed_at.isnot(None)))
        ...
        return {
            ...
            "jobs": jobs_analysis,
            "optimization_suggestions": self._generate_runtime_optimizations(jobs_analysis, overall_avg),
        }

    def _generate_runtime_optimizations(self, jobs: list[dict[str, Any]], overall_avg: float) -> list[dict[str, Any]]:
    ...
    async def check_quality_gates(self, hours: int = 24) -> dict[str, Any]:
        """
        ...
        # Gate 1: Daily items
        items_query = select(func.sum(IngestionJob.items_collected)).where(IngestionJob.started_at >= cutoff_time)
        ...
        cost_query = select(func.sum(SourceMetric.cost)).where(SourceMetric.timestamp >= cutoff_time)
        ...
        avg_score = ((quality_row.avg_relevance or 0) + (quality_row.avg_timeliness or 0) + (quality_row.avg_completeness or 0)) / 3
    ...
    async def track_monthly_costs(self, month: str | None = None) -> dict[str, Any]:
        """
    ...
    async def analyze_briefing_delivery(self, days: int = 7) -> dict[str, Any]:
        """
        ...
        query = select(BriefingDelivery).where(BriefingDelivery.delivery_date >= cutoff_time).order_by(desc(BriefingDelivery.delivery_date))
        ...
        avg_effectiveness = (
            sum(d.effectiveness_score for d in deliveries if d.effectiveness_score) / len([d for d in deliveries if d.effectiveness_score])
            if deliveries
            else 0
        )
    ...
    async def generate_ingestion_report(self, hours: int = 24) -> IngestionReport:
        """
