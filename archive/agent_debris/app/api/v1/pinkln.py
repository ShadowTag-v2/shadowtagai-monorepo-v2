...
async def compare_grpo_ppo(request: GRPOComparisonRequest) -> dict[str, Any]:
    ...
    if len(request.rewards) != grpo_sim.group_size:
        raise HTTPException(
            status_code=400,
            detail=f"Expected {grpo_sim.group_size} rewards, got {len(request.rewards)}"
        )
    ...
    return {
        ...
        "interpretation": (
            "GRPO outperforms PPO" if comparison["grpo_better"]
            else "PPO outperforms GRPO"
        )
    }
...
async def list_personas() -> list[dict[str, str]]:
    ...
    return [
        {
            "persona": UltrathinkPersona.PAUSE_BREATHE.value,
            "description": "Take a moment to think deeply"
        },
        {
            "persona": UltrathinkPersona.URGENCY.value,
            "description": "Ship it, make it happen NOW"
        },
        {
            "persona": UltrathinkPersona.BEAUTY.value,
            "description": "Insanely great design"
        },
        {
            "persona": UltrathinkPersona.DETAILS.value,
            "description": "Sweat every detail"
        },
        {
            "persona": UltrathinkPersona.SIMPLIFY.value,
            "description": "Remove everything unnecessary"
        },
        {
            "persona": UltrathinkPersona.BOY_SCOUT.value,
...
async def switch_persona(persona: UltrathinkPersona) -> dict[str, str]:
    ...
    return {
        "active_persona": mas.framework.active_persona.value,
        "description": f"Switched to {persona.value} mode at IQ {mas.persona_iq}"
    }
