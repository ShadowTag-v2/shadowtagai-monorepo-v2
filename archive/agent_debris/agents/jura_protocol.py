...
from src.shadowtag-omega-v4.services.gemini_core import GeminiAntigravity
...
class JuraProtocol:
    ...
    def __init__(self, project_id: str = None, redis_host: str = "10.85.19.187", redis_port: int = 6379):
    ...
    def administer_exam(self, candidate_id: str, current_level: int, proof_of_work: dict) -> tuple:
        ...
        try:
            ...
            response = self.jura.model.generate_content(
                prompt,
                generation_config=self.jura._get_generation_config(thinking_level="high", json_output=True)
            )
            raw = response.text.replace('```json', '').replace('```', '')
            result = json.loads(raw)

            if result.get("verdict") == "PASS":
                self.whiteboard.record_learning(candidate_id, f"Passed Level {target_level}", result)
    ...
    def quick_assess(self, code_snippet: str) -> dict:
        ...
        try:
            response = self.jura.model.generate_content(
                prompt,
                generation_config=self.jura._get_generation_config(thinking_level="low", json_output=True)
            )
