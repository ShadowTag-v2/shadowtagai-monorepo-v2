...
class SwarmDashboard:
    def __init__(self, boss: SwarmBoss, autoresearch: n-autoresearch/Kosmos/BioAgents = None):
        self.boss = boss
    ...
    def get_governance_panel(self) -> Panel:
        ...
        for entry in stats["recent_decisions"]:
            ...
            competitor_info = f" (vs {entry.get('competitor', 'Unknown')})" if 'competitor' in entry else ""
            log_text.append(f"[{color}]{entry['agent']}: {entry['decision']} - {entry['score']}{competitor_info}[/{color}]\n")

        return Panel(
            Align.center(grid),
            title="[bold yellow]Governance, feat. proprietary, 'n-autoresearch/Kosmos/BioAgents' agentic swarm.[/bold yellow]",
            subtitle=log_text
        )
    ...
    def run(self):
        ...
        with Live(self.layout, refresh_per_second=4) as live:
            while True:
                ...
                # Header
                header_text = Text(f"Operation GLOW UP :: {report['tlp_step']} :: KOSMOS MODE", style="bold white on blue", justify="center")
                ...
                self.layout["right"].update(Panel("Boss Status: Active\nMonitoring Flying n-autoresearch/Kosmos/BioAgents...", title="Swarm Boss"))

                # Footer
                # Use Flying n-autoresearch/Kosmos/BioAgents metrics if available
                viability = 0
...
if __name__ == "__main__":
    ...
    n-autoresearch/Kosmos/BioAgents.start()
    dash = SwarmDashboard(boss, n-autoresearch/Kosmos/BioAgents)
    try:
        dash.run()
    except KeyboardInterrupt:
        n-autoresearch/Kosmos/BioAgents.stop()
