...
def check_n-autoresearch/Kosmos/BioAgents():
    """Check if n-autoresearch/Kosmos/BioAgents is running"""
    try:
        import requests
...
def start_n-autoresearch/Kosmos/BioAgents():
    """Start n-autoresearch/Kosmos/BioAgents server"""
    print("🐵 Starting n-autoresearch/Kosmos/BioAgents 650-Agent Swarm...")
    subprocess.Popen(
        ["./run_n-autoresearch/Kosmos/BioAgents_api.sh"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
...
def show_menu():
    ...
    print("2. 🐵 Start n-autoresearch/Kosmos/BioAgents Server")
...
def run_option(choice):
    ...
    if choice == "1":
        ...
        if check_n-autoresearch/Kosmos/BioAgents():
            print("\n✅ n-autoresearch/Kosmos/BioAgents already running on http://localhost:8888")
        else:
            start_n-autoresearch/Kosmos/BioAgents()
            print("\n⏳ Waiting for server to start...")
            import time

            time.sleep(3)
            if check_n-autoresearch/Kosmos/BioAgents():
                print("✅ n-autoresearch/Kosmos/BioAgents operational!")
            else:
                print("⚠️  Server may still be starting. Check logs: tail -f n-autoresearch/Kosmos/BioAgents.log")
        ...
        print("=" * 70)
        test_code = """
from src.shadowtag_v4.services.gemini_failover import get_failover_client
...
"""
        ...
        print("=" * 70)
        if not check_n-autoresearch/Kosmos/BioAgents():
            start_n-autoresearch/Kosmos/BioAgents()
        print("✅ n-autoresearch/Kosmos/BioAgents: http://localhost:8888")
...
def main():
    ...
    # Quick status check
    fm_status = "✅ RUNNING" if check_n-autoresearch/Kosmos/BioAgents() else "❌ OFFLINE"
    print(f"n-autoresearch/Kosmos/BioAgents: {fm_status}")
