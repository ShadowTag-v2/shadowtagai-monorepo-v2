...
def memory_search(pg_dsn: str, repo_id: str, limit: int = 6):
    with pg_conn(pg_dsn) as conn:
        cur = conn.cursor()
        cur.execute("SELECT subject, summary, memory_kind FROM semantic_memories WHERE repo_id = %s ORDER BY created_at DESC LIMIT %s", (repo_id, limit))
...
def json_memory_search(query: str, limit: int = 6):
    ...
    return [{"title": r.get("subject",""), "content": r.get("summary") or r.get("body",""), "kind": r.get("type","memory"), "source": "jsonl"} for r in rows]
