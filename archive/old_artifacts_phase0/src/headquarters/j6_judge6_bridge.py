...
into the operational Judge6 validation pipeline per JP 3-33.

The J-6 holds doctrinal authority over all CSRMC/NIST continuous ATO
decisions. This bridge ensures that Judge6 pipeline invocations are
...
logger = logging.getLogger("J6-Judge6-Bridge")


class J6Judge6Bridge:
    """Bridge between JTF J-6 staff authority and Judge6 validation pipeline.
    ...
    This bridge wraps Judge6 pipeline calls with J-Staff routing,
    ...
    async def validate_with_authority(
        ...
        """Execute Judge6 validation under J-6 doctrinal authority.

        Wraps the Judge6 pipeline with J-6 routing authority, ensuring
        ...
        # Step 2: Execute Judge6 pipeline under authority
