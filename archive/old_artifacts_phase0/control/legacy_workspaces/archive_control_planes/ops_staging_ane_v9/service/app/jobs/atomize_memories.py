...
def extract_candidate_atoms_from_text(text: str, repo_id: str, source_type: str, source_ref: str | None = None) -> list[dict[str, Any]]:
    ...
    if "prettier-vscode" in lower:
        atoms.append({
            "repo_id": repo_id, "atom_kind": "rule", "subject": "standards",
            "predicate": "formatter", "object_text": "prettier-vscode",
            "source_type": source_type, "source_ref": source_ref, "tags": ["standards"]
        })
    if "default_inference_backend" in lower and "ane" in lower:
        atoms.append({
            "repo_id": repo_id, "atom_kind": "preference", "subject": "settings",
            "predicate": "default_inference_backend", "object_text": "ane",
            "source_type": source_type, "source_ref": source_ref, "tags": ["settings"]
        })
    if "fallback_backend" in lower and "metal" in lower:
        atoms.append({
            "repo_id": repo_id, "atom_kind": "preference", "subject": "settings",
            "predicate": "fallback_backend", "object_text": "metal",
            "source_type": source_type, "source_ref": source_ref, "tags": ["settings"]
        })
    if "hydrate_before_reasoning" in lower:
        atoms.append({
            "repo_id": repo_id, "atom_kind": "rule", "subject": "startup_contract",
            "predicate": "hydrate_before_reasoning", "object_text": "True",
            "source_type": source_type, "source_ref": source_ref, "tags": ["startup"]
        })
    if "ignore_codebase_as_authority" in lower:
        atoms.append({
            "repo_id": repo_id, "atom_kind": "rule", "subject": "startup_contract",
            "predicate": "ignore_codebase_as_authority", "object_text": "True",
            "source_type": source_type, "source_ref": source_ref, "tags": ["startup"]
        })
    return atoms
...
def atomize_jsonl_memory():
    ...
    for item in store.all():
        ...
        for atom in atoms:
            insert_atom(
                s.postgres_dsn, atom["repo_id"], atom["atom_kind"], atom["subject"], atom["predicate"], atom["object_text"],
                source_type=atom["source_type"], source_ref=atom["source_ref"], tags=atom["tags"], canonical_weight=0.7
            )
            inserted += 1
