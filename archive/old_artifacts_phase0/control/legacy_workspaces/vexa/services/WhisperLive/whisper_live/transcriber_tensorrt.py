...
from whisper_live.tensorrt_utils import load_audio, load_audio_wav_format, mel_filters, pad_or_trim
...
class WhisperEncoding:
    ...
    def get_audio_features(self, mel, mel_input_lengths, encoder_downsampling_factor=2):
        if isinstance(mel, list):
            ...
            mel = [torch.nn.functional.pad(f, (0, longest_mel - f.shape[-1]), mode="constant") for f in mel]
            mel = torch.cat(mel, dim=0).type(str_dtype_to_torch("float16")).contiguous()
        bsz, seq_len = mel.shape[0], mel.shape[2]
        position_ids = (
            torch.arange(math.ceil(seq_len / encoder_downsampling_factor), dtype=torch.int32, device=mel.device).expand(bsz, -1).contiguous()
        )
        if self.encoder_config["plugin_config"]["remove_input_padding"]:
            ...
            position_ids = remove_tensor_padding(position_ids, mel_input_lengths // encoder_downsampling_factor)
        inputs = OrderedDict()
...
class WhisperDecoding:
    ...
    def get_session(self, engine_dir, runtime_mapping, debug_mode=False):
        ...
        decoder_model_config = ModelConfig(
            ...
            kv_cache_type=KVCacheType.PAGED if self.decoder_config["plugin_config"]["paged_kv_cache"] else KVCacheType.CONTINUOUS,
    ...
    def generate(self, decoder_input_ids, encoder_outputs, encoder_max_input_length, encoder_input_lengths, eot_id, max_new_tokens=40, num_beams=1):
        ...
        cross_attention_mask = torch.ones([batch_size, decoder_max_input_length + max_new_tokens, encoder_max_input_length]).int().cuda()
        # generation config
        ...
        if self.decoder_config["plugin_config"]["remove_input_padding"]:
            ...
            if encoder_outputs.dim() == 3:
                encoder_output_lens = torch.full((encoder_outputs.shape[0],), encoder_outputs.shape[1], dtype=torch.int32, device="cuda")
...
class WhisperTRTLLM:
    def __init__(self, engine_dir, assets_dir=None, device=None, is_multilingual=False, language="en", task="transcribe"):
        world_size = 1
    ...
    def log_mel_spectrogram(self, audio: str | np.ndarray | torch.Tensor, padding: int = 0, return_duration=True):
        """
    ...
    def process_batch(
        self, mel, mel_input_lengths, text_prefix="<|startoftranscript|><|en|><|transcribe|><|notimestamps|>", num_beams=1, max_new_tokens=96
    ):
        prompt_id = self.tokenizer.encode(text_prefix, allowed_special=set(self.tokenizer.special_tokens.keys()))
    ...
    def transcribe(
        ...
        features_input_lengths = torch.full((mel.shape[0],), mel.shape[2], dtype=torch.int32, device=mel.device)
