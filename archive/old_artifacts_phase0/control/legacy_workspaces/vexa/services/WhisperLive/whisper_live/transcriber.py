...
class BatchedInferencePipeline:
    ...
    def generate_segment_batched(
        ...
        if options.multilingual:
            language_tokens = [
                tokenizer.tokenizer.token_to_id(segment_langs[0][0]) for segment_langs in self.model.model.detect_language(encoder_output)
            ]
    ...
    def transcribe(
        ...
        if multilingual and not self.model.model.is_multilingual:
            self.model.logger.warning("The current model is English-only but the multilingual parameter is set toTrue; setting to False instead.")
            multilingual = False
        ...
        if language is None:
            ...
            if not self.model.model.is_multilingual and language != "en":
                self.model.logger.warning(f"The current model is English-only but the language parameter is set to '{language}'; using 'en' instead.")
                language = "en"
...
class WhisperModel:
    ...
    def transcribe(
        ...
        if multilingual and not self.model.is_multilingual:
            self.logger.warning("The current model is English-only but the multilingual parameter is set toTrue; setting to False instead.")
            multilingual = False
        ...
        if language is None:
            ...
            if not self.model.is_multilingual and language != "en":
                self.logger.warning(f"The current model is English-only but the language parameter is set to '{language}'; using 'en' instead.")
                language = "en"
    ...
    def _split_segments_by_timestamps(
        ...
        consecutive_timestamps = [
            i for i in range(len(tokens)) if i > 0 and tokens[i] >= tokenizer.timestamp_begin and tokens[i - 1] >= tokenizer.timestamp_begin
        ]
