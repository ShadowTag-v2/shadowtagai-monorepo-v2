...
from mapping.speaker_mapper import (  # Removed direct map_speaker_to_segment and other statuses if not directly used by this file
    STATUS_ERROR,
    STATUS_UNKNOWN,
...
def verify_meeting_token(token: str) -> dict | None:
    try:
        if not token:
            return None
...
async def process_session_start_event(
    message_id: str, stream_data: dict[str, Any], db: AsyncSession, user: User | None, meeting: Meeting, redis_c: aioredis.Redis
) -> bool:
    ...
    try:
        ...
        try:
            ...
            start_timestamp = datetime.fromisoformat(start_timestamp_str).replace(tzinfo=UTC)
        ...
        stmt_session = select(MeetingSession).where(MeetingSession.meeting_id == meeting.id, MeetingSession.session_uid == session_uid)
        ...
        if meeting_session:
            ...
            meeting_session = MeetingSession(meeting_id=meeting.id, session_uid=session_uid, session_start_time=start_timestamp)
        ...
        logger.error(
            f"Error processing session_start_event for message {message_id}, meeting {meeting.id if meeting else 'Unknown'}: {e}", exc_info=True
        )
        try:
                ...
                if session_uid_from_payload:
                    ...
                    if cached_start:
                        try:
                            ...
                            session_start_utc = datetime.fromisoformat(cached_str).replace(tzinfo=UTC)
                            logger.debug(
                    ...
                    if not session_start_utc:
                        stmt_session_time = select(MeetingSession).where(
                            MeetingSession.meeting_id == internal_meeting_id, MeetingSession.session_uid == session_uid_from_payload
                        )
