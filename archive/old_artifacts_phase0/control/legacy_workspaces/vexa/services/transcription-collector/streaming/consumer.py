...
from streaming.processors import process_speaker_event_message, process_stream_message
...
async def claim_stale_messages(redis_c: aioredis.Redis):
    ...
    try:
        while True:
            pending_details = await redis_c.xpending_range(name=REDIS_STREAM_NAME, groupname=REDIS_CONSUMER_GROUP, min="-", max="+", count=100)

            if not pending_details:
                logger.debug("No more pending messages found for the group during stale check.")
                break

            stale_candidates = [msg for msg in pending_details if msg.get("idle", 0) > PENDING_MSG_TIMEOUT_MS]
            ...
            for stream_name_bytes, messages in response:
                ...
                for message_id_bytes, message_data_bytes in messages:
                    ...
                    try:
                        ...
                        logger.error(
                            f"[SpeakerConsumer] Critical error during process_speaker_event_message call for {message_id_str}: {e}", exc_info=True
                        )
