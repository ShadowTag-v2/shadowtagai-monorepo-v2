...
from shared_models.database import async_session_local, get_db
from shared_models.models import Meeting, MeetingSession, User  # <--- ADD MeetingSession and Transcription import
...
async def update_meeting_status(
    ...
    db: AsyncSession,
    completion_reason: MeetingCompletionReason | None = None,
...
class BotExitCallbackPayload(BaseModel):
    ...
    reason: str | None = Field("self_initiated_leave", description="Reason for the exit.")
    error_details: dict[str, Any] | None = Field(None, description="Detailed error information including stack trace, error message, and context.")
    platform_specific_error: str | None = Field(None, description="Platform-specific error message or details.")
...
class BotStatusChangePayload(BaseModel):
    ...
    exit_code: int | None = Field(None, description="Exit code if applicable.")
    error_details: dict[str, Any] | None = Field(None, description="Detailed error information.")
    platform_specific_error: str | None = Field(None, description="Platform-specific error message.")
    completion_reason: MeetingCompletionReason | None = Field(None, description="Reason for completion if applicable.")
    failure_stage: MeetingFailureStage | None = Field(None, description="Stage where failure occurred if applicable.")
