...
* **The Pure Circuit Breaker (`src/agents/autoresearch_pure.py`)**: Authored a Serverless-native quota watcher tailored specifically for `google-genai`. This eliminates the OpenAI (`GPT-4`) fallback completely. We now route `gemini-3.0-pro` to `gemini-3.0-flash` exclusively upon `APIError` threshold breaches.
...
* **Zombie Purge**: Severed and systematically destroyed all stalled terminal operations and legacy LLM router scripts (`autoresearch_pure.py`) related to the biological / Kosmos era.
