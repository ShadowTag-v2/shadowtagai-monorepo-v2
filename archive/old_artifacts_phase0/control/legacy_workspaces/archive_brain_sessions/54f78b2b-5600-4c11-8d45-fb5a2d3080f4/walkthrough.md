...
As requested by the Board, we have fully scrubbed the local agent ecosystem of obsolete `n-autoresearch/Kosmos/BioAgentss` architecture. A comprehensive sweep was performed across the `.beads` and `.agent` workflow directories, leading to the termination and purging of deprecated server workflows (`n-autoresearch/Kosmos/BioAgentss-server.md`).
