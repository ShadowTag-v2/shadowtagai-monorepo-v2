...
6. [Frontend Design (前端设计)](#6-frontend-design-前端设计)
...
19. [JSON Canvas (Canvas 创作)](#19-json-canvas-canvas-创作)
20. [Obsidian Markdown (Obsidian 语法)](#20-obsidian-markdown-obsidian-语法)
21. [Obsidian Bases (Obsidian 数据库)](#21-obsidian-bases-obsidian-数据库)
22. [Brainstorming (头脑风暴)](#22-brainstorming-头脑风暴)
23. [Dispatching Parallel Agents (并行分发)](#23-dispatching-parallel-agents-并行分发)
24. [Executing Plans (执行计划)](#24-executing-plans-执行计划)
25. [Finishing a Development Branch (分支收尾)](#25-finishing-a-development-branch-分支收尾)
26. [Receiving Code Review (接收审查)](#26-receiving-code-review-接收审查)
27. [Requesting Code Review (请求审查)](#27-requesting-code-review-请求审查)
28. [Subagent Driven Development (子Agent开发)](#28-subagent-driven-development-子agent开发)
29. [Systematic Debugging (系统化调试)](#29-systematic-debugging-系统化调试)
30. [Test Driven Development (测试驱动开发)](#30-test-driven-development-测试驱动开发)
31. [Using Git Worktrees (Git工作树)](#31-using-git-worktrees-git工作树)
32. [Using Superpowers (使用超能力)](#32-using-superpowers-使用超能力)
33. [Verification Before Completion (完成前验证)](#33-verification-before-completion-完成前验证)
34. [Writing Plans (编写计划)](#34-writing-plans-编写计划)
35. [Writing Skills (编写技能)](#35-writing-skills-编写技能)
36. [Planning with Files (文件任务规划)](#36-planning-with-files-文件任务规划)
37. [BDI Mental States (BDI 认知模型)](#37-bdi-mental-states-bdi-认知模型)
38. [Memory Systems (记忆系统)](#38-memory-systems-记忆系统)
39. [Filesystem Context (文件系统上下文)](#39-filesystem-context-文件系统上下文)
40. [Context Fundamentals (上下文基础)](#40-context-fundamentals-上下文基础)
41. [Context Optimization (上下文优化)](#41-context-optimization-上下文优化)
42. [Context Compression (上下文压缩)](#42-context-compression-上下文压缩)
43. [Context Degradation (上下文退化)](#43-context-degradation-上下文退化)
44. [Multi-Agent Patterns (多 Agent 模式)](#44-multi-agent-patterns-多-agent-模式)
45. [Hosted Agents (托管 Agent)](#45-hosted-agents-托管-agent)
46. [Tool Design (工具设计)](#46-tool-design-工具设计)
...
**简介**:
直接查询 Google NotebookLM 笔记本，获取基于文档的确切答案。有效减少幻觉。
...
>
> **Skill 响应**:
> 提供针对医疗行业的配色和布局建议，并给出 Tailwind 参数。
...
**简介**:
深度支持 Obsidian 增强版 Markdown 语法的创作和编辑，包括双向链接、块引用、Callouts 等。
...
**简介**:
在进行任何创造性工作之前使用的 Skill。通过对话澄清用户意图、需求和设计约束。
...
**简介**:
用于执行已制定的实施计划，按步骤引导执行并设置检查点。

**使用案例**:
> **用户**: "开始执行刚才制定的 重构数据库层 计划。"
>
> **Skill 响应**:
> 读取计划并按步骤实施修改，确认后继续下一步。
...
**简介**:
功能开发完成后引导用户进行分支收尾（合并、PR、清理）。

**使用案例**:
> **用户**: "功能开发完了，准备合并。"
>
> **Skill 响应**:
> 检查更改，推荐合并策略并生成 PR 描述。
...
**简介**:
将复杂计划拆分为独立子任务由子 Agent 执行。

**使用案例**:
> **用户**: "去查一下 Table 组件最新的 API 变更。"
>
> **Skill 响应**:
> 启动子 Agent 进行查阅并将结果返回主会话。
...
**简介**:
系统的调试方法论，包括根因分析、假设验证和修复实施。

**使用案例**:
> **用户**: "API 偶尔报 502 错误。"
>
> **Skill 响应**:
> 引导用户收集证据、提出针对性假设并验证修复。
...
**简介**:
遵循 TDD 开发模式：编写失败测试 -> 实现代码 -> 重构。

**使用案例**:
> **用户**: "写一个解析这种特殊日期格式的函数。"
>
> **Skill 响应**:
> 先创建案例脚本验证预期，逐步补全代码逻辑。
...
**简介**:
创建隔离的 Git 工作树进行并行开发而不干扰当前工作区。

**使用案例**:
> **用户**: "正在开发时线上有个急 Bug 要修。"
>
> **Skill 响应**:
> 开启 Worktree 进行修复并快速切回原任务。
...
**简介**:
Meta-Skill。用于帮助用户发现当前可用的 Skill 及其用法。
...
**简介**:
设计和协调多 Agent 协作系统（Supervisor/Swarm 等）。
