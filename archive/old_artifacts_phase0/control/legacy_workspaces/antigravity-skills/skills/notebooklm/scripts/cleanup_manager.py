...
class CleanupManager:
        ...
        if self.data_dir.exists():
            ...
            if browser_state_dir.exists():
                for item in browser_state_dir.iterdir():
                    ...
                    paths["browser_state"].append({"path": str(item), "size": size, "type": "dir" if item.is_dir() else "file"})
                    total_size += size
            ...
            for item in self.data_dir.iterdir():
                if item.name not in ["browser_state", "sessions.json", "library.json", "auth_info.json"]:
                    size = self._get_size(item)
                    paths["other"].append({"path": str(item), "size": size, "type": "dir" if item.is_dir() else "file"})
                    total_size += size

        return {"categories": paths, "total_size": total_size, "total_items": sum(len(items) for items in paths.values())}
    ...
    def perform_cleanup(self, preserve_library: bool = False, dry_run: bool = False) -> dict[str, Any]:
        ...
        if dry_run:
            return {"dry_run": True, "would_delete": cleanup_data["total_items"], "would_free": cleanup_data["total_size"]}
...
Examples:
  ...
  python cleanup_manager.py --confirm --force
    ...
    parser.add_argument("--confirm", action="store_true", help="Actually perform the cleanup (without this, only preview)")

    parser.add_argument("--preserve-library", action="store_true", help="Keep the notebook library (library.json)")
