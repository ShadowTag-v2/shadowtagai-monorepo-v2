...
class DOCXSchemaValidator(BaseSchemaValidator):
        ...
        for xml_file in self.xml_files:
            ...
            try:
                ...
                for instr_elem in root.xpath(".//w:del//w:instrText", namespaces=namespaces):
                    text_preview = repr(instr_elem.text or "")[:50] + "..." if len(repr(instr_elem.text or "")) > 50 else repr(instr_elem.text or "")
                    errors.append(
                        f"  {xml_file.relative_to(self.unpacked_dir)}: "
        ...
        for xml_file in self.xml_files:
            ...
            try:
                ...
                for elem in invalid_elements:
                    ...
                    errors.append(f"  {xml_file.relative_to(self.unpacked_dir)}: Line {elem.sourceline}: <w:delText> within <w:ins>: {text_preview}")
    ...
    def validate_comment_markers(self):
        ...
        try:
            ...
            range_starts = {elem.get(f"{{{self.WORD_2006_NAMESPACE}}}id") for elem in doc_root.xpath(".//w:commentRangeStart", namespaces=namespaces)}
            range_ends = {elem.get(f"{{{self.WORD_2006_NAMESPACE}}}id") for elem in doc_root.xpath(".//w:commentRangeEnd", namespaces=namespaces)}
            references = {elem.get(f"{{{self.WORD_2006_NAMESPACE}}}id") for elem in doc_root.xpath(".//w:commentReference", namespaces=namespaces)}
            ...
            if comments_xml and comments_xml.exists():
                ...
                comment_ids = {elem.get(f"{{{self.WORD_2006_NAMESPACE}}}id") for elem in comments_root.xpath(".//w:comment", namespaces=namespaces)}
