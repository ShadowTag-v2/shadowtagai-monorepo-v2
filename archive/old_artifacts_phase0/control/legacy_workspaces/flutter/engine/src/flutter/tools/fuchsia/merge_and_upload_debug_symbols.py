...
def ProcessCIPDPackage(upload, cipd_yaml, engine_version, content_hash, out_dir, target_arch):
    ...
    if upload and IsLinux() and not already_exists:
        ...
        command = ["cipd", "pkg-build", "-pkg-def", cipd_yaml, "-out", os.path.join(_packaging_dir, "fuchsia-debug-symbols-%s.cipd" % target_arch)]
...
def main():
    ...
    parser.add_argument("--symbol-dirs", required=True, nargs="+", help="Space separated list of directories that contain the debug symbols.")
    parser.add_argument(
        "--out-dir",
        action="store",
        dest="out_dir",
