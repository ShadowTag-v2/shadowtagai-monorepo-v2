...
def GetDartSdkSemanticVersion(buildroot):
    path.join(buildroot, "third_party", "dart")
    return subprocess.check_output(["git", "-C", GetDartPath(buildroot), "describe", "--abbrev=0"]).strip()
...
def GetFuchsiaSdkVersion(buildroot):
    with open(path.join(buildroot, "third_party", "fuchsia-sdk", "sdk", "meta", "manifest.json")) as fuchsia_sdk_manifest:
...
def main():
    ...
    parser.add_argument("--buildroot", action="store", help="path to the flutter engine buildroot", required=True)
    parser.add_argument("--engine-version", action="store", help="Flutter engine commit hash", required=True)
