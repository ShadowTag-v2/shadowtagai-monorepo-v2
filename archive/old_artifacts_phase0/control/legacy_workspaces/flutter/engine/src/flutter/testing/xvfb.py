...
def xvfb_pid_filename(child_build_name):
    ...
    return os.path.join(tempfile.gettempdir(), "xvfb-" + xvfb_display_index(child_build_name) + ".pid")
...
def start_virtual_x(child_build_name, build_dir):
    ...
    if xdisplaycheck_path and os.path.exists(xdisplaycheck_path):
        ...
        xdisplayproc = subprocess.Popen([xdisplaycheck_path, "--noserver"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env)
    ...
    proc = subprocess.Popen(
        [cmd, display, "-screen", "0", "1280x800x24", "-ac", "-dpi", "96", "-maxclients", "512", "-extension", "MIT-SHM"],
    ...
    if xdisplaycheck_path and os.path.exists(xdisplaycheck_path):
        ...
        checkstarttime = time.time()
        xdisplayproc = subprocess.Popen([xdisplaycheck_path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        # Wait for xdisplaycheck to exit.
        logs = xdisplayproc.communicate()[0]
