...
def main():
    parser = argparse.ArgumentParser("Verifies that all .dart files are included in sources, and sources don't include nonexsitent files")
    parser.add_argument("--source_dir", help="Path to the directory containing the package sources", required=True)
    ...
    for dirpath, dirnames, filenames in os.walk(src_dir_path, topdown=True):
        ...
        actual_sources.update(
            os.path.normpath(relpath_to_src_root.joinpath(filename)) for filename in filenames if pathlib.Path(filename).suffix == ".dart"
        )
