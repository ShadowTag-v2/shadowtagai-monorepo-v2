...
def main():
    ...
    if not os.path.exists(args.android_source_root):
        print("This script must be run at the root of the Flutter source tree, or the --android-source-root must be set.")
        return 1

    if not os.path.exists(args.out_dir):
        os.makedirs(args.out_dir)

    android_jar_path = os.path.join(args.src_dir, "flutter", "third_party", "android_tools", "sdk", "platforms", "android-36", "android.jar")
