...
def main():
    ...
    parser.add_argument("filename", action="store", help="Path to the JSON output from Google Benchmark")
    parser.add_argument(
        "-o", "--output-pdf", dest="output_pdf", action="store", default="output.pdf", help="Filename to output the PDF of graphs to."
    )
    parser.add_argument("-c", "--output-csv", dest="output_csv", action="store", default="output.csv", help="Filename to output the CSV data to.")
...
def process_benchmark_data(benchmark_json, output_pdf, output_csv):
    ...
    for benchmark_result in benchmark_json:
        ...
        if benchmark_name not in benchmark_results_data:
            benchmark_results_data[benchmark_name] = BenchmarkResult(benchmark_name, benchmark_backend, benchmark_unit, benchmark_draw_call_count)

        for key in optional_keys:
            if key in benchmark_result:
                benchmark_results_data[benchmark_name].add_optional_value(key, benchmark_seeded_value, benchmark_result[key])

        benchmark_results_data[benchmark_name].add_data_point(benchmark_family_index, benchmark_seeded_value, benchmark_real_time)
        benchmark_results_data[benchmark_name].set_family_label(benchmark_family_index, benchmark_family_label)
