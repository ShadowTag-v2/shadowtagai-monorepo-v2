...
def main():
    parser = argparse.ArgumentParser(
        description=("Creates Flutter.framework, Flutter.xcframework and copies architecture-dependent gen_snapshot binaries to output dir")
    )
    ...
    parser.add_argument("--no-extension-safe-frameworks", dest="include_extension_safe_frameworks", action="store_false", default=True)

    args = parser.parse_args()

    dst = args.dst if os.path.isabs(args.dst) else sky_utils.buildroot_relative_path(args.dst)
    arm64_out_dir = args.arm64_out_dir if os.path.isabs(args.arm64_out_dir) else sky_utils.buildroot_relative_path(args.arm64_out_dir)

    x64_out_dir = None
    if args.x64_out_dir:
        x64_out_dir = args.x64_out_dir if os.path.isabs(args.x64_out_dir) else sky_utils.buildroot_relative_path(args.x64_out_dir)

    simulator_x64_out_dir = None
    if args.simulator_x64_out_dir:
        simulator_x64_out_dir = (
            args.simulator_x64_out_dir if os.path.isabs(args.simulator_x64_out_dir) else sky_utils.buildroot_relative_path(args.simulator_x64_out_dir)
        )
    ...
    if args.simulator_arm64_out_dir:
        simulator_arm64_out_dir = (
            ...
            else sky_utils.buildroot_relative_path(args.simulator_arm64_out_dir)
        )
...
def create_framework(  # pylint: disable=too-many-arguments
    args, dst, framework, arm64_framework, simulator_framework, simulator_x64_framework, simulator_arm64_framework
):
...
def zip_archive(dst, args):
    ...
    unsigned_binaries = []
    if args.dsym:
        unsigned_binaries.append("Flutter.xcframework/ios-arm64/dSYMs/Flutter.framework.dSYM/Contents/Resources/DWARF/Flutter")
        if args.include_extension_safe_frameworks:
            unsigned_binaries.append("extension_safe/Flutter.xcframework/ios-arm64/dSYMs/Flutter.framework.dSYM/Contents/Resources/DWARF/Flutter")
    ...
    sky_utils.assert_valid_codesign_config(dst, zip_contents, with_entitlements, without_entitlements, unsigned_binaries)
