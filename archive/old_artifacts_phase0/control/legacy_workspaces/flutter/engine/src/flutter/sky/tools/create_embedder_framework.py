...
DSYMUTIL = os.path.join(os.path.dirname(__file__), "..", "..", "buildtools", ARCH_SUBPATH, "clang", "bin", "dsymutil")
...
def main():
    ...
    arm64_out_dir = args.arm64_out_dir if os.path.isabs(args.arm64_out_dir) else os.path.join(buildroot_dir, args.arm64_out_dir)
    x64_out_dir = args.x64_out_dir if os.path.isabs(args.x64_out_dir) else os.path.join(buildroot_dir, args.x64_out_dir)
    ...
    subprocess.check_call(["lipo", arm64_dylib, x64_dylib, "-create", "-output", fat_framework_binary])
...
def regenerate_symlinks(fat_framework):
    ...
    os.symlink(os.path.join("Versions", "Current", "FlutterEmbedder"), os.path.join(fat_framework, "FlutterEmbedder"))
    ...
    os.symlink(os.path.join("Versions", "Current", "Resources"), os.path.join(fat_framework, "Resources"))
