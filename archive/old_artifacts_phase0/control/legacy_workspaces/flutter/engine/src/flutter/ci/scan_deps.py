...
def extract_deps(deps_file):
    ...
    for _, dep in deps.items():
        ...
        if ancestor_result:
            filtered_osv_deps.append({"package": {"name": ancestor_result[1], "commit": ancestor_result[0]}})
    ...
    osv_result = {"packageSource": {"path": deps_file, "type": "lockfile"}, "packages": filtered_osv_deps}
    return osv_result
...
def get_common_ancestor(dep, deps_list):
    ...
    try:
        ...
        print(f"attempting to add upstream remote from: {upstream}")
        ...
        default_branch = subprocess.check_output("git remote show upstream " + "| sed -n '/HEAD branch/s/.*: //p'", cwd=temp_dep_dir, shell=True)
        default_branch = byte_str_decode(default_branch)
        default_branch = default_branch.strip()

        # Make upstream branch track the upstream dep.
        subprocess.check_output(["git", "checkout", "--force", "-b", "upstream", "--track", "upstream/" + default_branch], cwd=temp_dep_dir)
        # Get the most recent commit from default branch of upstream.
        commit = subprocess.check_output("git for-each-ref " + "--format='%(objectname:short)' refs/heads/upstream", cwd=temp_dep_dir, shell=True)
        commit = byte_str_decode(commit)
        commit = commit.strip()

        # Perform merge-base on most recent default branch commit and pinned mirror commit.
        ancestor_commit = subprocess.check_output(f"git merge-base {commit} {dep[1]}", cwd=temp_dep_dir, shell=True)
        ...
        print(f"Subprocess command '{error.cmd}' failed with exit code: {str(error.returncode)}.")
        if error.output:
            print(f"Subprocess error output: '{error.output}'")
    return None
...
def parse_args(args):
    ...
    parser.add_argument("--deps", "-d", type=str, help="Input DEPS file to extract.", default=os.path.join(CHECKOUT_ROOT, "DEPS"))
    parser.add_argument(
        "--output", "-o", type=str, help="Output osv-scanner compatible deps file.", default=os.path.join(CHECKOUT_ROOT, "osv-lockfile.json")
    )
