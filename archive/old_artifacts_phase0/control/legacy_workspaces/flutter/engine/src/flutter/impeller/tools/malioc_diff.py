...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        help=("A relative path calculated from the engine src directory to a directory tree containing new malioc results in json files"),
    )
    parser.add_argument(
        "--before-relative-to-src",
        type=str,
        help=("A relative path calculated from the engine src directory to a json file containing existing malioc results in json files"),
    )
    parser.add_argument(
        "--print-diff",
        "-p",
        ...
        if variant_name in afters:
            ...
            for variant_key, before_variant_val in before_variant.items():
                ...
                if variant_key == "performance":
                    ...
                    differences += [
                        f"In variant {variant_name}:\n  {variant_key}: {before_variant_val} <- before\n  {variant_key}: {after_variant_val} <- after"
                    ]
    return differences
...
def compare_shaders(malioc_tree, before_shader, after_shader):
    ...
    if bool(differences):
        ...
        print(f"Changes found in shader {filename} on core {core}:")
        for diff in differences:
            print(diff)
        print(f"\nFor a full report, run:\n  $ malioc --{typ.lower()} --core {core} {build_gen_dir}/{filename}\n")
...
def main(argv):
    ...
    if not bool(after_json):
        print(f"Did not find any malioc results under {args.after}.")
        return 1
    ...
    for filename, shaders in before_json.items():
        if filename not in after_json.keys():
            print(f'Shader "{filename}" has been removed.')
            changed = True
            continue
    ...
    for filename, shaders in after_json.items():
        if filename not in before_json:
            print(f'Shader "{filename}" is new.')
            changed = True
    ...
    if changed:
        print(
            ...
            f"$ ./flutter/impeller/tools/malioc_diff.py --before {args.before} --after {args.after} --update"
        )
        if args.print_diff:
