...
def main():
    parser = argparse.ArgumentParser("Merges sources of a Dart target and its dependencies", fromfile_prefix_chars="@")
    parser.add_argument("--output", help="Path to output the final list", type=argparse.FileType("w"), required=True)
    parser.add_argument("--depfile", help="Path to the depfile to generate", type=argparse.FileType("w"), required=True)
    parser.add_argument(
        "--sources",
