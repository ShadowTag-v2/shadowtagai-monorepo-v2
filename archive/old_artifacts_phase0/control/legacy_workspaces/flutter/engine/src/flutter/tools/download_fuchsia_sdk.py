...
def DownloadFuchsiaSDKFromGCS(sdk_path, verbose):
    ...
    url = f"https://storage.googleapis.com/fuchsia-artifacts/{sdk_path}"
    ...
    if curl_result.returncode == 0 and verbose:
        print(f"curl output:stdout:\n{curl_result.stdout}\nstderr:\n{curl_result.stderr}")
    elif curl_result.returncode != 0:
        eprint(f"Failed to download: stdout:\n{curl_result.stdout}\nstderr:\n{curl_result.stderr}")
        return None
...
def Main():
    ...
    parser.add_argument(
        "--fail-loudly", action="store_true", default=False, help="Return an error code if a prebuilt couldn't be fetched and extracted"
    )

    parser.add_argument("--verbose", action="store_true", default="LUCI_CONTEXT" in os.environ, help="Emit verbose output")
