...
def generate_doxyfile(section, output_dir, log_file, doxy_file):
    doxyfile = open("docs/Doxyfile.template").read()
    ...
    doxyfile = doxyfile.replace("@@PROJECT_NAME@@", f"Flutter {section.title}")
    doxyfile = doxyfile.replace("@@DOCSET_FEEDNAME@@", f"Flutter {section.title} Documentation")
...
def process_section(name, section, destination):
    ...
    log_file = os.path.join(destination, f"{name}-doxygen.log")
    zip_file = os.path.join(destination, f"{name}-docs.zip")
    ...
    print(f"Wrote ZIP file for {section} to {zip_file}")
    print(f"Preserving log file in {log_file}")
    shutil.rmtree(output_dir, ignore_errors=True)


def generate_docs(argv):
    if len(argv) != 2:
        print('Error: Argument specifying output directory required. Directory may be an absolute path, or a relative path from the "src" directory.')
        exit(1)
