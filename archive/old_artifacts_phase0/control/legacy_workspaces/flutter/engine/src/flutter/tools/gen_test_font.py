...
def create_no_path_glyph(codepoint, advance_percentage):
    name = (
        "Zero Advance" if advance_percentage == 0 else ("Full Advance" if advance_percentage == 1 else f"1/{(int)(1 / advance_percentage)} Advance")
    )
...
print(rf"|     \ Script <br />Glyph | {' | '.join(script_list)} |")
...
for glyph in font.glyphs():
    ...
    def describe_codepoint_range(codepoints):
        if not codepoints:
            return ""
        codepoints.sort()
        codepoint_ranges = [list(map(itemgetter(1), group)) for key, group in groupby(enumerate(codepoints), lambda x: x[0] - x[1])]
        ...
        return (
            "**codepoint(s):** "
            + ", ".join([f"{hex(r[0])}-{hex(r[-1])}" if len(r) > 1 else hex(r[0]) for r in codepoint_ranges])
            + "<br />"
            + "**character(s):** "
            + full_list
        )

    print(f"| {glyph.glyphname} | {' | '.join([describe_codepoint_range(l) for l in codepoints_by_script])} |")
