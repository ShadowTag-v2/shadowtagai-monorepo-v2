...
class GNTestCase(unittest.TestCase):
    ...
    def test_get_out_dir(self):
        ...
        self._expect_build_dir(["--ios", "--darwin-extension-safe"], os.path.join("out", "ios_debug_extension_safe"))
        self._expect_build_dir(["--ios", "--runtime-mode", "release"], os.path.join("out", "ios_release"))
        self._expect_build_dir(["--ios", "--darwin-extension-safe", "--runtime-mode", "release"], os.path.join("out", "ios_release_extension_safe"))
        self._expect_build_dir(["--android"], os.path.join("out", "android_debug"))
        self._expect_build_dir(["--android", "--runtime-mode", "release"], os.path.join("out", "android_release"))
    ...
    def test_to_gn_args(self):
        ...
        self.assertEqual(self._gn_args(["--ios", "--simulator", "--simulator-cpu", "x64"])["target_cpu"], "x64")
