...
def launch_package(package_name, activity_name, adb_path="adb"):
    ...
    logcat = subprocess.Popen([adb_path, "logcat"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)

    print("Launching %s (%s)" % (package_name, activity_name))
    subprocess.check_output([adb_path, "shell", "am ", "start", "-n", "%s/%s" % (package_name, activity_name)], stderr=subprocess.STDOUT)
...
def collect_and_validate_trace(adb_path="adb"):
    ...
    subprocess.check_output([adb_path, "shell", "perfetto", "--attach", PERFETTO_SESSION_KEY, "--stop"], stderr=subprocess.STDOUT)
    subprocess.check_output([adb_path, "pull", PERFETTO_TRACE_FILE, "trace.pb"], stderr=subprocess.STDOUT)

    print("Validating trace")
    traceconv = os.path.join(BUILDROOT_DIR, "flutter", "third_party", "android_tools", "trace_to_text", "trace_to_text")
...
def main():
    ...
    parser.add_argument("--apk-path", dest="apk_path", action="store", help="Provide the path to the APK to install")
    parser.add_argument("--package-name", dest="package_name", action="store", help="The package name of the APK, e.g. dev.flutter.scenarios")
    parser.add_argument(
        "--activity-name",
        dest="activity_name",
        action="store",
        help="The activity to launch as it appears in AndroidManifest.xml, e.g. .PlatformViewsActivity",
    )
    parser.add_argument(
        "--adb-path",
        dest="adb_path",
    ...
    android_api_level = subprocess.check_output([args.adb_path, "shell", "getprop", "ro.build.version.sdk"], text=True).strip()
