...
def main():
    ...
    parser.add_argument("--symbol-name", type=str, required=True, help="The name of the symbol referencing the data.")
    parser.add_argument("--output-header", type=str, required=True, help="The header file containing the symbol reference.")
    parser.add_argument("--output-source", type=str, required=True, help="The source file containing the file bytes.")
    parser.add_argument("--source", type=str, required=True, help="The source file whose contents to embed in the output source file.")
    ...
    with open(args.source, "rb") as source, open(output_source, "w") as output:
        ...
        output.write(f"alignas(std::max_align_t) const unsigned char impeller_{args.symbol_name}_data[] =\n")
        output.write("{\n")
        while True:
            byte = source.read(1)
            if not byte:
