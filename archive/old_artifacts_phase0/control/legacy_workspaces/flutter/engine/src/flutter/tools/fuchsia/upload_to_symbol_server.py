...
def process_symbols(should_upload, symbol_dir):
    ...
    for file in files:
        remote_path = "gs://%s/%s/%s" % (FUCHSIA_ARTIFACTS_BUCKET_NAME, FUCHSIA_ARTIFACTS_DEBUG_NAMESPACE, remote_filename(file))
...
def main():
    ...
    parser.add_argument("--symbol-dir", required=True, help="Directory that contain the debug symbols.")
    parser.add_argument("--engine-version", required=True, help="Specifies the flutter engine SHA.")
    parser.add_argument("--upload", default=False, action="store_true", help="If set, uploads symbols to the server.")
