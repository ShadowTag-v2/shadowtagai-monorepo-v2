...
def CopyGenSnapshotIfExists(source, destination):
    ...
    FindFileAndCopyTo("kernel_compiler.dart.snapshot", source_root, destination_base, "kernel_compiler.snapshot")
    FindFileAndCopyTo("frontend_server.dart.snapshot", source_root, destination_base, "flutter_frontend_server.snapshot")
    FindFileAndCopyTo("list_libraries.dart.snapshot", source_root, destination_base, "list_libraries.snapshot")
...
def CopyBuildToBucket(runtime_mode, arch, optimized, product):
    ...
    CopyPath(os.path.join(_src_root_dir, "flutter/sky/packages/sky_engine/LICENSE"), os.path.join(bucket_root, "LICENSE"))
...
def ProcessCIPDPackage(upload, engine_version, content_hash):
    if not upload or not IsLinux():
        RunCIPDCommandWithRetries(["cipd", "pkg-build", "-pkg-def", "fuchsia.cipd.yaml", "-out", os.path.join(_bucket_directory, "fuchsia.cipd")])
        return
...
def main():
    ...
    parser.add_argument("--cipd-dry-run", default=False, action="store_true", help="If set, creates the CIPD package but does not upload it.")

    parser.add_argument("--upload", default=False, action="store_true", help="If set, uploads the CIPD package and tags it as the latest.")

    parser.add_argument("--engine-version", required=False, help="Specifies the flutter engine SHA.")

    parser.add_argument("--unoptimized", action="store_true", default=False, help="If set, disables compiler optimization for the build.")

    parser.add_argument("--runtime-mode", type=str, choices=["debug", "profile", "release", "all"], default="all")

    parser.add_argument("--archs", type=str, choices=["x64", "arm64", "all"], default="all")

    parser.add_argument(
        "--asan", action="store_true", default=False, help="If set, enables address sanitization (including leak sanitization) for the build."
    )

    parser.add_argument("--no-lto", action="store_true", default=False, help="If set, disables LTO for the build.")

    parser.add_argument("--no-legacy", action="store_true", default=False, help="If set, disables legacy code for the build.")

    parser.add_argument("--targets", default="", help=("Comma-separated list; adds additional targets to build for Fuchsia."))

    parser.add_argument("--no-dart-version-git-info", action="store_true", default=False, help="If set, turns off the Dart SDK git hash check.")
