...
def print_divider(char: str = "=", logger: logging.Logger | None = None) -> None:
    if logger is None:
        logger = _logger
    logger.info("\n")
    for _ in range(4):
...
def run_cmd(  # pylint: disable=too-many-arguments
    cmd: list[str],
    cwd: str | None = None,
    forbidden_output: list[str] | None = None,
    expect_failure: bool = False,
    env: dict[str, str] | None = None,
    allowed_failure_output: list[str] | None = None,
    ...
    **kwargs,
    ...
    process = subprocess.Popen(cmd, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env, universal_newlines=True, **kwargs)
    output = ""
    ...
    if process.returncode != 0 and not expect_failure:
        ...
        logger.error("Failed Command:\n\n%s\n\nExit Code: %s\n\nOutput:\n%s", command_string, process.returncode, output)
        ...
        if not allowed_failure:
            raise RuntimeError(f'Command "{command_string}" (in {cwd}) exited with code {process.returncode}.')
    ...
    for forbidden_string in forbidden_output:
        if forbidden_string in output:
            ...
            raise RuntimeError(f'command "{command_string}" contained forbidden string "{forbidden_string}": {matches}')

    logger.info("Command run successfully in %.2f seconds: %s (in %s)", end_time - start_time, command_string, cwd)
...
def vulkan_validation_env(build_dir: str) -> dict[str, str]:
    extra_env = {
...
def build_engine_executable_command(
    build_dir: str, executable_name: str, flags: list[str] | None = None, coverage: bool = False, gtest: bool = False
) -> list[str]:
    if flags is None:
        flags = []
    ...
    if coverage:
        ...
        if gtest:
            gtest_parallel = os.path.join(BUILDROOT_DIR, "flutter", "third_party", "gtest-parallel", "gtest-parallel")
...
def run_engine_executable(  # pylint: disable=too-many-arguments
    build_dir: str,
    executable_name: str,
    executable_filter: list[str] | None,
...
class EngineExecutableTask:  # pylint: disable=too-many-instance-attributes
    ...
    def __call__(self, *args: typing.Any) -> tuple[Exception | None, list[str]]:
    ...
    def __str__(self) -> str:
        command = build_engine_executable_command(self.build_dir, self.executable_name, flags=self.flags, coverage=self.coverage)
        return " ".join(command)
...
def run_cc_tests(build_dir: str, executable_filter: list[str] | None, coverage: bool, capture_core_dump: bool) -> None:
    ...
    def make_test(name: str, flags: list[str] | None = None, extra_env: dict[str, str] | None = None) -> tuple[str, list[str], dict[str, str]]:
        if flags is None:
            flags = repeat_flags
        if extra_env is None:
            extra_env = {}
    ...
    try:
        ...
        for test, flags, extra_env in unittests:
            run_engine_executable(build_dir, test, executable_filter, flags, coverage=coverage, extra_env=extra_env, gtest=True)
    finally:
        if is_linux():
    ...
    if is_mac():
        ...
        run_engine_executable(build_dir, "flutter_desktop_darwin_swift_unittests", executable_filter, shuffle_flags, coverage=coverage)
        ...
        if not os.path.basename(build_dir).startswith("host_debug"):
            ...
            run_engine_executable(build_dir, "flutter_desktop_darwin_unittests", executable_filter, shuffle_flags, coverage=coverage)
...
def run_engine_benchmarks(build_dir: str, executable_filter: list[str] | None) -> None:
...
class FlutterTesterOptions:
    ...
    def apply_args(self, command_args: list[str]) -> None:
...
def gather_dart_test(build_dir: str, dart_file: str, options: FlutterTesterOptions) -> EngineExecutableTask:
    kernel_file_name = os.path.basename(dart_file) + ".dill"
    kernel_file_output = os.path.join(build_dir, "gen", kernel_file_name)
    error_message = f"{kernel_file_output} doesn't exist. Please run the build that populates {build_dir}"
    ...
    with open(dart_file, encoding="utf-8") as dart_file_contents:
    ...
    _logger.info("Running test '%s' using '%s' (%s, %s)", kernel_file_name, tester_name, options.threading_description(), options.impeller_enabled())
...
def ensure_ios_tests_are_built(ios_out_dir: str) -> None:
    ...
    joined_message = "\n".join(message)
    final_message = f"{ios_out_dir} or {ios_test_lib} doesn't exist. Please run the following commands: \n{joined_message}"
...
def assert_expected_xcode_version() -> None:
    ...
    version_output_str = version_output if isinstance(version_output, str) else version_output.decode(ENCODING)
...
def run_java_tests(executable_filter: str | None, android_variant: str = "android_debug_unopt") -> None:
    ...
    gradle_bin = os.path.join(BUILDROOT_DIR, "flutter", "third_party", "gradle", "bin", "gradle.bat" if is_windows() else "gradle")
...
def run_android_tests(android_variant: str = "android_debug_unopt", adb_path: str | None = None) -> None:
    if adb_path is None:
        adb_path = "adb"
...
def run_objc_tests(ios_variant: str = "ios_debug_sim_unopt", test_filter: str | None = None) -> None:
    ...
    create_simulator = [f"xcrun simctl create {new_simulator_name} com.apple.CoreSimulator.SimDeviceType.iPhone-11"]
...
def gather_dart_tests(build_dir: str, test_filter: list[str] | None) -> typing.Generator[EngineExecutableTask]:
    ...
    dart_vm_service_tests = [] if "release" in build_dir else glob.glob(f"{dart_tests_dir}/vm_service/*_test.dart")
    ...
    for dart_test_file, enable_vm_service in (*((test, True) for test in dart_vm_service_tests), *((test, False) for test in dart_tests)):
...
def gather_dart_smoke_test(build_dir: str, test_filter: list[str] | None) -> typing.Generator[EngineExecutableTask]:
    ...
    if test_filter is not None and os.path.basename(smoke_test) not in test_filter:
        ...
        yield gather_dart_test(build_dir, smoke_test, FlutterTesterOptions(multithreaded=True, expect_failure=True))
        yield gather_dart_test(build_dir, smoke_test, FlutterTesterOptions(multithreaded=False, expect_failure=True))


def gather_dart_package_tests(build_dir: str, package_path: str) -> typing.Generator[EngineExecutableTask]:
    if uses_package_test_runner(package_path):
        ...
        for dart_test_file in dart_tests:
            opts = [dart_test_file]
            yield EngineExecutableTask(build_dir, os.path.join("dart-sdk", "bin", "dart"), None, flags=opts, cwd=package_path)
...
def uses_package_test_runner(package: str) -> bool:
    ...
    if not os.path.exists(pubspec):
        return False
    with open(pubspec, encoding="utf-8") as file:
...
def build_dart_host_test_list() -> list[str]:
    dart_host_tests = [
...
def run_benchmark_tests(build_dir: str) -> None:
    ...
    for dart_test_file in dart_tests:
        opts = [dart_test_file]
        run_engine_executable(build_dir, os.path.join("dart-sdk", "bin", "dart"), None, flags=opts, cwd=test_dir)
...
def run_engine_tasks_in_parallel(tasks: list[EngineExecutableTask]) -> bool:
    ...
    failures = []
    try:
        with multiprocessing.Pool(max_processes, worker_init, [queue, _logger.getEffectiveLevel()]) as pool:
...
class DirectoryChange:
    ...
    def __exit__(
        self, exception_type: type[BaseException] | None, exception_value: BaseException | None, exception_traceback: typing.Any | None
    ) -> None:
...
def run_impeller_golden_tests(build_dir: str, require_skia_gold: bool = False):
    ...
    if not os.path.exists(tests_path):
        raise Exception(f'Cannot find the "impeller_golden_tests" executable in "{build_dir}". You may need to build it.')
    harvester_path: Path = Path(SCRIPT_DIR).parent.joinpath("tools").joinpath("golden_tests_harvester")
...
"""
            ...
            print_divider("<")
            print("Skipping the SkiaGoldClient invocation as the GOLDCTL environment variable is not set.")
            return
...
"""
    ...
    parser.add_argument(
        "--variant", dest="variant", action="store", default="host_debug_unopt", help="The engine build variant to run the tests for."
    )
    parser.add_argument("--type", type=str, default="all", help=f'A list of test types, default is "all" (equivalent to "{",".join(all_types)}")')
    parser.add_argument("--engine-filter", type=str, default="", help="A list of engine test executables to run.")
    parser.add_argument(
        "--dart-filter",
        type=str,
        default="",
        help='A list of Dart test script base file names to run in flutter_tester (example: "image_filter_test.dart").',
    )
    parser.add_argument("--dart-host-filter", type=str, default="", help="A list of Dart test scripts to run with the Dart CLI.")
    parser.add_argument("--java-filter", type=str, default="", help='A single Java test class to run (example: "io.flutter.SmokeTest")')
    parser.add_argument(
        "--android-variant",
    ...
    parser.add_argument(
        "--verbose-dart-snapshot", dest="verbose_dart_snapshot", action="store_true", default=False, help="Show extra dart snapshot logging."
    )
    parser.add_argument(
        "--objc-filter",
        type=str,
    ...
    parser.add_argument("--coverage", action="store_true", default=None, help="Generate coverage reports for each unit test framework run.")
    parser.add_argument(
    ...
    parser.add_argument("--quiet", dest="quiet", action="store_true", default=False, help="Only emit output when there is an error.")
    parser.add_argument(
        "--logs-dir",
        dest="logs_dir",
        type=str,
    ...
    if args.sanitizer_suppressions:
        assert is_linux() or is_mac(), "The sanitizer suppressions flag is only supported on Linux and Mac."
        file_dir = os.path.dirname(os.path.abspath(__file__))
        command = ["env", "-i", "bash", "-c", f"source {file_dir}/sanitizer_suppressions.sh >/dev/null && env"]
    ...
    if "java" in types:
        ...
        if "," in java_filter or "*" in java_filter:
            _logger.warning('Can only filter JUnit4 tests by single entire class name, eg "io.flutter.SmokeTest". Ignoring filter=%s', java_filter)
            java_filter = None
