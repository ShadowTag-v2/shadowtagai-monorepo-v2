...
COMPARE_TESTS = (
    ...
    (True, "2.ttf", MATERIAL_TTF, [r"0xE003", r"optional:0xE004"]),  # Optional codepoint that is found
    (
        True,
        "2.ttf",
        MATERIAL_TTF,
    ...
    (False, "1variable.ttf", VARIABLE_MATERIAL_TTF, [r"57348"]),  # False because different codepoint
...
def run_cmd(cmd, codepoints, fail=False):
    ...
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE, cwd=SRC_DIR)
    stdout_data, stderr_data = p.communicate(input=" ".join(codepoints).encode())
    if p.returncode != 0 and not fail:
        print("FAILURE: %s" % p.returncode)
        print("STDOUT:")
        print(stdout_data)
        print("STDERR:")
        print(stderr_data)
    elif p.returncode == 0 and fail:
...
def main():
    ...
    font_subset_zip = os.path.join(SRC_DIR, "out", variant, "zip_archives", platform_to_path(sys.platform, args.target_cpu), "font-subset.zip")
    if not os.path.isfile(font_subset):
        raise Exception("Could not locate font-subset%s in %s - build before running this script." % (exe, variant))
