...
def GetBuildIdParts(exec_path, read_elf):
    ...
    if build_id_line[0] != b"Build" or build_id_line[1] != b"ID:" or not sha1_pattern.match(str(build_id_line[-1])) or not len(build_id_line[-1]) > 2:
    ...
    return {"build_id": build_id.decode("utf-8"), "prefix_dir": build_id[:2].decode("utf-8"), "exec_name": build_id[2:].decode("utf-8")}
...
def main():
    ...
    parser.add_argument("--executable-path", dest="exec_path", action="store", required=True, help="Path to the executable on the filesystem.")
    parser.add_argument(
        "--destination-base",
        dest="dest",
        action="store",
    ...
    parser.add_argument("--stripped", dest="stripped", action="store_true", default=True, help="Executable at the specified path is stripped.")
    parser.add_argument("--unstripped", dest="stripped", action="store_false", help="Executable at the specified path is unstripped.")
    parser.add_argument("--read-elf", dest="read_elf", action="store", required=True, help="Path to read-elf executable.")
