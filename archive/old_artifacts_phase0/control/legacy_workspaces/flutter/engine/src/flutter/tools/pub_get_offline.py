...
def fetch_package(pub, package):
    try:
        ...
        print('"%s" failed in "%s" with status %d:\n%s' % (" ".join(pub), package, err.returncode, err.output))
        return 1
    return 0
...
def check_package_config(package):
    ...
    with open(package_config) as config_file:
        ...
        for package_data in packages_data:
            ...
            if ".pub-cache" in package_uri and ("pub.dartlang.org" in package_uri or "pub.dev" in package_uri):
    ...
    if pub_count > 0:
        ...
        print("Please fix the pubspec.yaml for %s so that all dependencies are path dependencies" % package)
    return pub_count
...
def delete_config_files():
    ...
    files_to_delete = subprocess.check_output(gitcmd, cwd=ENGINE_DIR, stderr=subprocess.STDOUT, text=True).splitlines()
...
def main():
    ...
    dart_sdk_bin = os.path.join(SRC_ROOT, "flutter", "third_party", "dart", "tools", "sdks", "dart-sdk", "bin")
