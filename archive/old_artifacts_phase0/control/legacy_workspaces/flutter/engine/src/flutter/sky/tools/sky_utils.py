...
def assert_valid_codesign_config(framework_dir, zip_contents, entitlements, without_entitlements, unsigned_binaries):
    ...
    if _contains_duplicates(entitlements + without_entitlements + unsigned_binaries):
        log_error("ERROR: duplicate value(s) found between entitlements.txt, without_entitlements.txt, unsigned_binaries.txt")
    ...
    if listed_binaries != binaries:
        ...
        if not_listed:
            log_error("Binaries NOT LISTED in entitlements.txt, without_entitlements.txt, unsigned_binaries.txt:")
            for file in not_listed:
                log_error("    " + file)

        not_found = sorted(listed_binaries - binaries)
        if not_found:
            log_error("Binaries listed in entitlements.txt, without_entitlements.txt, unsigned_binaries.txt but NOT FOUND:")
...
def create_fat_macos_framework(args, dst, fat_framework, arm64_framework, x64_framework):
    ...
    lipo([get_mac_framework_dylib_path(arm64_framework), get_mac_framework_dylib_path(x64_framework)], framework_dylib)
...
def _regenerate_symlinks(framework_dir):
    ...
    os.symlink(os.path.join("Versions", "Current", framework_name), os.path.join(framework_dir, framework_name))
    ...
    os.symlink(os.path.join("Versions", "Current", "Resources"), os.path.join(framework_dir, "Resources"))
...
def _set_framework_permissions(framework_dir):
    ...
    find_subprocess = subprocess.Popen(["find", versions_path, "-perm", "-100", "-print0"], stdout=subprocess.PIPE)
    # Add execute permission for other and group for all files that had it for owner.
    xargs_subprocess = subprocess.Popen(["xargs", "-0", "chmod", "og+x"], stdin=find_subprocess.stdout)
...
def extract_dsym(binary_path, dsym_out_path):
    ...
    dsymutil = buildroot_relative_path(os.path.join("flutter", "buildtools", arch_dir, "clang", "bin", "dsymutil"))
