...
def GitHashArg(value):
    ...
    if not re.match(r"^[0-9a-f]{40}$", value):
        raise argparse.ArgumentTypeError(f"'{value}' is not a valid full git hash. Expected a 40-character hexadecimal string.")
    return value


def ParseArgs(args):
    args = args[1:]
    parser = argparse.ArgumentParser(description="A script to generate updated dart dependencies for flutter DEPS.")
    parser.add_argument("--dart_deps", "-d", type=str, help="Dart DEPS file.", default=DART_DEPS)
    parser.add_argument("--flutter_deps", "-f", type=str, help="Flutter DEPS file.", default=FLUTTER_DEPS)
    parser.add_argument("--dart_revision", "-r", type=GitHashArg, help="Dart revision to update to.")
