...
class CodeAnalyzer:
    ...
    def _extract_python_class(self, node: ast.ClassDef) -> ClassSignature:
        ...
        return ClassSignature(name=node.name, base_classes=bases, methods=methods, docstring=docstring, line_number=node.lineno)
...
'''
    ...
    if result.get("classes"):
        ...
        for method in cls["methods"]:
            params = ", ".join([f"{p['name']}: {p['type_hint']}" + (f" = {p['default']}" if p.get("default") else "") for p in method["parameters"]])
