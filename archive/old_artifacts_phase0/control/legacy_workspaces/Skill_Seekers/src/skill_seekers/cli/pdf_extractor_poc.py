...
class PDFExtractor:
    ...
    def merge_continued_code_blocks(self, pages):
        ...
        for i in range(len(pages) - 1):
            ...
            if last_code["language"] == first_next_code["language"] and last_code["detection_method"] == first_next_code["detection_method"]:
    ...
    def extract_all(self):
        ...
        for chunk in chunks:
            if chunk["chapter_title"]:
                chapters.append({"title": chunk["chapter_title"], "start_page": chunk["start_page"], "end_page": chunk["end_page"]})
...
Examples:
  ...
  python3 pdf_extractor_poc.py docs/python.pdf -o python_extracted.json -v
    ...
    parser.add_argument("--min-quality", type=float, default=0.0, help="Minimum code quality score (0-10, default: 0 = no filtering)")
    parser.add_argument("--extract-images", action="store_true", help="Extract images to files (NEW in B1.5)")
    parser.add_argument("--image-dir", type=str, default=None, help="Directory to save extracted images (default: output/{pdf_name}_images)")
    parser.add_argument("--min-image-size", type=int, default=100, help="Minimum image dimension in pixels (filters icons, default: 100)")
