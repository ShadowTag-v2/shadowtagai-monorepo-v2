...
class ReactomeClient:
    ...
    def get_pathway_entities(self, pathway_id: str) -> list[dict]:
        ...
        response = requests.get(f"{self.CONTENT_BASE}/data/event/{pathway_id}/participatingPhysicalEntities")
        response.raise_for_status()
        return response.json()
    ...
    def analyze_genes(self, gene_list: list[str]) -> dict:
        ...
        data = "\n".join(gene_list)
        response = requests.post(f"{self.ANALYSIS_BASE}/identifiers/", headers={"Content-Type": "text/plain"}, data=data)
        response.raise_for_status()
        return response.json()
