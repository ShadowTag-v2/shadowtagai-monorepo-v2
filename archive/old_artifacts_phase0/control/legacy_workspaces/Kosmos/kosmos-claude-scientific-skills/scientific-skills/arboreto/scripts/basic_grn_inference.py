...
if __name__ == "__main__":
    ...
    parser.add_argument("expression_file", help="Path to expression matrix (TSV format, genes as columns)")
    parser.add_argument("output_file", help="Path for output network (TSV format)")
    parser.add_argument("--tf-file", help="Path to transcription factors file (one per line)", default=None)
    parser.add_argument("--seed", help="Random seed for reproducibility (default: 777)", type=int, default=777)

    args = parser.parse_args()

    run_grn_inference(expression_file=args.expression_file, output_file=args.output_file, tf_file=args.tf_file, seed=args.seed)
