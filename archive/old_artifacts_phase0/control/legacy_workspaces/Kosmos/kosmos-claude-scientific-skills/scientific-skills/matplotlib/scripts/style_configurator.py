...
def generate_preview_data():
    ...
    return {"x": x, "y1": y1, "y2": y2, "scatter_x": scatter_x, "scatter_y": scatter_y, "categories": categories, "bar_values": bar_values}
...
def create_style_preview(style_dict=None):
    ...
    scatter = ax2.scatter(data["scatter_x"], data["scatter_y"], c=colors, cmap="viridis", alpha=0.6, s=50)
...
def save_style_file(style_dict, filename):
    ...
    with open(filename, "w") as f:
        ...
        for category, prefixes in categories.items():
            category_items = {k: v for k, v in style_dict.items() if any(k.startswith(p) for p in prefixes)}
            if category_items:
                f.write(f"# {category}\n")
...
Examples:
  ...
  python style_configurator.py --interactive
    ...
    parser.add_argument("--preset", type=str, choices=list(STYLE_PRESETS.keys()), help="Use a predefined style preset")
