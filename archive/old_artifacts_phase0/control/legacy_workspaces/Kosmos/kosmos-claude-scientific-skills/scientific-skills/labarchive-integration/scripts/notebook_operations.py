...
def get_user_id(client, config):
    ...
    login_params = {"login_or_email": config["user_email"], "password": config["user_external_password"]}
...
def backup_notebook(client, uid, nbid, output_dir="backups", json_format=False, no_attachments=False):
    """Backup a notebook"""
    ...
    # Prepare parameters
    params = {"uid": uid, "nbid": nbid, "json": "true" if json_format else "false", "no_attachments": "true" if no_attachments else "false"}
...
def backup_all_notebooks(client, uid, output_dir="backups", json_format=False, no_attachments=False):
...
Examples:
  ...
  python3 notebook_operations.py backup --nbid 12345 --output my_backups/
    ...
    backup_parser.add_argument("--output", default="backups", help="Output directory (default: backups)")
    backup_parser.add_argument("--json", action="store_true", help="Return data in JSON format instead of XML")
    ...
    backup_all_parser.add_argument("--output", default="backups", help="Output directory (default: backups)")
    backup_all_parser.add_argument("--json", action="store_true", help="Return data in JSON format instead of XML")
