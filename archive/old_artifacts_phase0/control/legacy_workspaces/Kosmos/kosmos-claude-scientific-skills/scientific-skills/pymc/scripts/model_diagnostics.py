...
def compare_prior_posterior(idata, prior_idata, var_names=None, output_path=None):
    ...
    for idx, var in enumerate(var_names if var_names else list(idata.posterior.data_vars)[:3]):
        # Plot prior
        az.plot_dist(prior_idata.prior[var].values.flatten(), label="Prior", ax=axes[idx], color="blue", alpha=0.3)

        # Plot posterior
        az.plot_dist(idata.posterior[var].values.flatten(), label="Posterior", ax=axes[idx], color="green", alpha=0.3)
