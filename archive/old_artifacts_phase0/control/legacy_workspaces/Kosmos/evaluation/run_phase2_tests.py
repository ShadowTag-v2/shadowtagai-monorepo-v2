...
def test_component(name, func):
    ...
    try:
        ...
        elapsed = time.time() - start
        result = {"status": "FAIL", "error": str(e), "traceback": traceback.format_exc(), "duration_seconds": round(elapsed, 2)}
...
def test_hypothesis_generation(research_question=None, domain=None):
    ...
    agent = HypothesisGeneratorAgent(config={"use_literature_context": False, "num_hypotheses": 3})

    response = agent.generate_hypotheses(research_question=rq, domain=dom, num_hypotheses=3, store_in_db=False)
    ...
    result = {"count": len(hypotheses), "response_type": type(response).__name__, "research_question": rq, "domain": dom, "hypotheses": []}
...
def test_literature_search(research_question=None, domain=None):
    ...
    result = {"count": len(papers), "search_query": query, "research_question": rq, "papers": []}
...
def test_experiment_design(research_question=None, domain=None):
    ...
    protocol = agent.design_experiment(
        hypothesis=hypothesis, hypothesis_id="test-hyp-001", preferred_experiment_type=ExperimentType.COMPUTATIONAL, store_in_db=False
    )

    protocols = [protocol] if not isinstance(protocol, list) else protocol

    result = {"count": len(protocols), "research_question": rq, "domain": dom, "protocols": []}
    ...
    protocol = ExperimentProtocol(
        ...
        steps=[
            ...
            ProtocolStep(
                step_number=2,
                title="Run t-test",
                description="Perform independent samples t-test comparing groups",
                action="Perform independent samples t-test",
            ),
            ProtocolStep(
                step_number=3,
                title="Effect size",
                description="Calculate Cohen's d effect size for the comparison",
                action="Calculate Cohen's d effect size",
            ),
        ],
        variables={
        ...
        resource_requirements=ResourceRequirements(estimated_duration_minutes=5, estimated_cost_usd=0.01, compute_hours=0.1),
    )
...
def test_data_analysis(research_question=None, domain=None):
    ...
    from kosmos.models.result import ExecutionMetadata, ExperimentResult, ResultStatus, StatisticalTestResult
    ...
    interpretation = agent.interpret_results(result=mock_result, hypothesis=hypothesis, literature_context=None)
...
def test_convergence_detection(research_question=None, domain=None):
    ...
    decision1 = detector.check_convergence(research_plan=plan, hypotheses=[], results=[])

    result = {
        "research_question": rq,
        "domain": dom,
        "test_max_iterations": {"should_stop": decision1.should_stop, "reason": str(decision1.reason), "details": getattr(decision1, "details", "")},
    }
    ...
    plan2 = ResearchPlan(research_question=rq, domain=dom, max_iterations=20, iteration_count=6)
    ...
    decision2 = detector2.check_convergence(research_plan=plan2, hypotheses=hypotheses, results=[])
