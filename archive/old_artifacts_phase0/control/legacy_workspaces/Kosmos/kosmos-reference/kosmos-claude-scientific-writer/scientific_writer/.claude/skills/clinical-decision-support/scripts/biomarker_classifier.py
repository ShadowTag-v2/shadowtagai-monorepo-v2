...
def stratify_cohort_report(data, stratification_var, output_dir="stratification_report"):
    ...
    for var in continuous_vars[:5]:  # Limit to first 5 for demo
        ...
        if len(distribution) == 2:
            ...
            results.append({"Variable": var, "Test": "Mann-Whitney U", "p_value": p_value, "Significant": "Yes" if p_value < 0.05 else "No"})
...
def main():
    ...
    parser.add_argument("input_file", type=str, nargs="?", default=None, help="CSV file with patient and biomarker data")
    ...
    if args.example or args.input_file is None:
        ...
        data.loc[data["pd_l1_tps"] >= 50, "response"] = np.random.choice(["Yes", "No"], (data["pd_l1_tps"] >= 50).sum(), p=[0.65, 0.35])
    else:
