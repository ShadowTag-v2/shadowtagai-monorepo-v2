...
class PublicationVisualizer:
    ...
    def volcano_plot(
        ...
        colors = [COLORS["red"] if (abs(fc) > fc_threshold and p < p_threshold) else COLORS["gray"] for fc, p in zip(log2fc, p_values)]

        # Scatter plot
        ax.scatter(log2fc, log_p, c=colors, alpha=0.7, s=60, edgecolors="none")

        # Threshold lines
        ax.axhline(y=-np.log10(p_threshold), color=COLORS["black"], linestyle="--", alpha=0.7, linewidth=1.5, label=f"p={p_threshold}")
        ...
        if labels is not None:
            for i, (fc, p, label) in enumerate(zip(log2fc, p_values, labels)):
                if abs(fc) > fc_threshold and p < p_threshold:
                    ax.annotate(label, (fc, -np.log10(p + 1e-300)), xytext=(5, 5), textcoords="offset points", fontsize=8, ha="left", alpha=0.8)
    ...
    def scatter_with_regression(
        self, x: np.ndarray, y: np.ndarray, x_label: str, y_label: str, title: str, output_path: str | None = None, show_plot: bool = False
    ) -> str:
        """
        ...
        ax.scatter(x, y, alpha=0.7, s=60, color=COLORS["neutral"], edgecolors="black", linewidth=0.5)
        ...
        ax.plot(
            x_trend, y_trend, color=COLORS["red"], linestyle="--", alpha=0.8, linewidth=2, label=f"Linear fit (r = {r_value:.3f}, p = {p_value:.4f})"
        )
    ...
    def box_plot_with_points(
        self, data: dict[str, np.ndarray], title: str = "Box Plot", y_label: str = "Value", output_path: str | None = None, show_plot: bool = False
    ) -> str:
        """
        ...
        for i, (label, values) in enumerate(data.items()):
            ...
            ax.scatter(x_jitter, values, alpha=0.5, s=30, color=COLORS["blue_dark"], edgecolors="black", linewidth=0.5)
    ...
    def violin_plot(
        self, data: dict[str, np.ndarray], title: str = "Violin Plot", y_label: str = "Value", output_path: str | None = None, show_plot: bool = False
    ) -> str:
        """
        ...
        parts = ax.violinplot(data.values(), positions=positions, widths=0.7, showmeans=True, showmedians=True)
    ...
    def qq_plot(self, data: np.ndarray, title: str = "Q-Q Plot", output_path: str | None = None, show_plot: bool = False) -> str:
        """
        ...
        # Reference line
        ax.plot(osm, slope * osm + intercept, color=COLORS["red"], linestyle="--", linewidth=2, label=f"Normal (R²={r**2:.3f})")
    ...
    def select_plot_types(self, result: ExperimentResult) -> list[dict[str, Any]]:
        ...
        if result.statistical_tests:
            ...
            if "t-test" in primary_test or "anova" in primary_test:
                plots.append({"type": "box_plot_with_points", "description": "Box plot for group comparison"})

            # Correlation → Scatter with regression
            if "correlation" in primary_test or "regression" in primary_test:
                plots.append({"type": "scatter_with_regression", "description": "Scatter plot with linear regression"})

            # Multiple tests → Volcano plot (if fold changes available)
            if len(result.statistical_tests) >= 3:
                plots.append({"type": "volcano_plot", "description": "Volcano plot for multiple comparisons"})

        # Check for multiple variables → Heatmap
        if result.variable_results and len(result.variable_results) >= 3:
            plots.append({"type": "custom_heatmap", "description": "Heatmap for multi-variable comparison"})

        # Default: If no specific plots selected, suggest basic visualizations
        if not plots:
            plots.append({"type": "box_plot_with_points", "description": "Default: Box plot for data visualization"})
