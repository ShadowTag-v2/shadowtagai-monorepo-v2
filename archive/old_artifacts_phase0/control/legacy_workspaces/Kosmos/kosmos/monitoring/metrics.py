...
class MetricsCollector:
    ...
    def __init__(self, registry: Any | None = None):
        ...
        self.research_cycles_total = Counter(
            "kosmos_research_cycles_total", "Total number of research cycles started", ["domain", "status"], registry=self.registry
        )

        self.research_iterations_total = Counter(
            "kosmos_research_iterations_total", "Total number of research iterations", ["domain"], registry=self.registry
        )
        ...
        self.hypotheses_generated_total = Counter(
            "kosmos_hypotheses_generated_total", "Total number of hypotheses generated", ["domain", "strategy"], registry=self.registry
        )

        self.hypotheses_tested_total = Counter(
            "kosmos_hypotheses_tested_total", "Total number of hypotheses tested", ["domain", "outcome"], registry=self.registry
        )
        ...
        self.experiments_total = Counter(
            "kosmos_experiments_total", "Total number of experiments executed", ["domain", "experiment_type", "status"], registry=self.registry
        )

        self.experiments_active = Gauge("kosmos_experiments_active", "Number of currently active experiments", registry=self.registry)
        ...
        # API call metrics
        self.api_calls_total = Counter("kosmos_api_calls_total", "Total number of API calls", ["api", "model", "status"], registry=self.registry)

        self.api_tokens_total = Counter(
            "kosmos_api_tokens_total", "Total tokens used in API calls", ["api", "model", "token_type"], registry=self.registry
        )

        self.api_cost_usd_total = Counter("kosmos_api_cost_usd_total", "Total API cost in USD", ["api", "model"], registry=self.registry)

        self.api_latency_seconds = Histogram(
            "kosmos_api_latency_seconds", "API call latency", ["api", "model"], buckets=[0.1, 0.5, 1, 2, 5, 10, 30], registry=self.registry
        )

        # Cache metrics
        self.cache_operations_total = Counter(
            "kosmos_cache_operations_total", "Total cache operations", ["operation", "cache_type", "status"], registry=self.registry
        )

        self.cache_hit_ratio = Gauge("kosmos_cache_hit_ratio", "Cache hit ratio (0.0-1.0)", ["cache_type"], registry=self.registry)

        self.cache_size_bytes = Gauge("kosmos_cache_size_bytes", "Current cache size in bytes", ["cache_type"], registry=self.registry)

        self.cache_entries = Gauge("kosmos_cache_entries", "Number of entries in cache", ["cache_type"], registry=self.registry)

        # Database metrics
        self.database_queries_total = Counter(
            "kosmos_database_queries_total", "Total database queries", ["operation", "table", "status"], registry=self.registry
        )
        ...
        self.database_connections_active = Gauge(
            "kosmos_database_connections_active", "Number of active database connections", registry=self.registry
        )

        # System resource metrics
        self.cpu_usage_percent = Gauge("kosmos_cpu_usage_percent", "CPU usage percentage", registry=self.registry)
        ...
        self.result_quality_score = Summary("kosmos_result_quality_score", "Result quality score distribution", ["domain"], registry=self.registry)
    ...
    def track_experiment_complete(self, domain: str, experiment_type: str, status: str, duration: float):
        ...
        self.experiments_total.labels(domain=domain, experiment_type=experiment_type, status=status).inc()
        self.experiments_active.dec()
        self.experiment_duration_seconds.labels(experiment_type=experiment_type).observe(duration)

    # API tracking
    def track_api_call(self, api: str, model: str, status: str, latency: float, input_tokens: int = 0, output_tokens: int = 0, cost_usd: float = 0.0):
    ...
    def update_system_metrics(self, cpu_percent: float, memory_rss: int, memory_vms: int, disk_total: int, disk_used: int, disk_free: int):
