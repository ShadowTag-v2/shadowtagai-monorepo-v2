...
from kosmos.core.providers.base import LLMProvider, LLMResponse, Message, ProviderAPIError, UsageStats
...
class AnthropicProvider(LLMProvider):
        ...
        **kwargs,
        ...
        try:
            ...
            if model_override:
                ...
                logger.info(
                    f"Auto-selected {selected_model} (complexity: {complexity_analysis['complexity_score']}, reason: {complexity_analysis['reason']})"
                )
            else:
                # Track model usage
            ...
            if self.cache and not bypass_cache:
                ...
                cached_response = self.cache.get(prompt=prompt, model=selected_model, bypass=False, **cache_key_params)
                ...
                if cached_response is not None:
                    ...
                    return LLMResponse(content=response_text, usage=usage, model=selected_model, finish_reason="stop", metadata={"cache_hit": True})
                else:
                    self.cache_misses += 1
            ...
            if self.cache and not bypass_cache:
                ...
                self.cache.set(prompt=prompt, model=selected_model, response=text, metadata=metadata, **cache_key_params)
        ...
        try:
            ...
            try:
                ...
                if get_config().logging.log_llm_calls:
                    logger.debug("[LLM] Anthropic async: model=%s, in=%d, out=%d, duration=%.2fs", self.model, input_tokens, output_tokens, duration)
            except Exception:
                pass
    ...
    def generate_structured(
        ...
        try:
            ...
            try:
                ...
                raise ProviderAPIError("anthropic", f"Invalid JSON response: {e.message}", raw_error=e, recoverable=False)
    ...
    def get_usage_stats(self) -> dict[str, Any]:
        ...
        cache_hit_rate = (self.cache_hits / total_requests_with_cache * 100) if total_requests_with_cache > 0 else 0.0
        ...
        if self.enable_auto_model_selection:
            ...
            stats["model_selection"] = {
                ...
                "haiku_percent": round((self.haiku_requests / total_model_requests * 100) if total_model_requests > 0 else 0, 2),
    ...
    def generate_stream(
        ...
        **kwargs,
        ...
        try:
            ...
            from kosmos.core.events import EventType, LLMEvent
        ...
        # Emit start event
        if emit_events:
            event_bus.publish_sync(LLMEvent(type=EventType.LLM_CALL_STARTED, process_id=process_id, model=selected_model, provider="anthropic"))
        ...
        try:
                ...
                for text in stream.text_stream:
                    ...
                    if emit_events:
                        event_bus.publish_sync(
                            LLMEvent(type=EventType.LLM_TOKEN, process_id=process_id, model=selected_model, provider="anthropic", token=text)
                        )
            ...
            if emit_events:
                event_bus.publish_sync(LLMEvent(type=EventType.LLM_CALL_FAILED, process_id=process_id, model=selected_model, provider="anthropic"))
    ...
    async def generate_stream_async(
        ...
        **kwargs,
        ...
        # Emit start event
        if emit_events:
            await event_bus.publish(LLMEvent(type=EventType.LLM_CALL_STARTED, process_id=process_id, model=selected_model, provider="anthropic"))
        ...
        try:
            ...
            if emit_events:
                await event_bus.publish(LLMEvent(type=EventType.LLM_CALL_FAILED, process_id=process_id, model=selected_model, provider="anthropic"))
