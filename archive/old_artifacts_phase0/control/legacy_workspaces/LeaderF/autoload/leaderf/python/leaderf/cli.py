...
class LfCli:
    ...
    def _buildPopupPrompt(self):
        ...
        input_win_width += 2 * (len(sep) - int(lfEval(f"strdisplaywidth('{escQuote(sep)}')")))
        input_win_width += len(pattern) - int(lfEval(f"strdisplaywidth('{escQuote(pattern)}')"))
        input_win_width += len(spin) - int(lfEval(f"strdisplaywidth('{spin}')"))
