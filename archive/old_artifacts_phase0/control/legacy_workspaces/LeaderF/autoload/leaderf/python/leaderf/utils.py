...
def lfPrintTraceback(msg=""):
    ...
    lfCmd(f"echom '{escQuote(error)}' | echohl None")
...
def lfActualLineCount(buffer, start, end, col_width):
    ...
    for i in buffer[start:end]:
        try:
            num += (int(lfEval(f"strdisplaywidth('{escQuote(i)}')")) + col_width - 1) // col_width
        except:
            num += (int(lfEval("strdisplaywidth('{}')".format(escQuote(i).replace("\x00", "\x01")))) + col_width - 1) // col_width
    return num
    ...
    if 0 and lfEval("has('patch-8.0.1508')") == "1":
        if type == "tab":
            if line_num:
                ...
                lfCmd(f"keepj tab drop {escSpecial(file_name)}")
        else:
            if line_num:
                lfCmd("keepj hide drop %s | %d" % (escSpecial(file_name), line_num))
            else:
                lfCmd(f"keepj hide drop {escSpecial(file_name)}")
    else:
        if type == "tab":
            for tp, w in ((tp, window) for tp in vim.tabpages for window in tp.windows):
                ...
                if line_num:
                    ...
                    lfCmd(f"tabe {escSpecial(file_name)}")
        else:
            for w in vim.windows:
                ...
                if line_num:
                    ...
                    lfCmd(f"hide edit {escSpecial(file_name)}")
