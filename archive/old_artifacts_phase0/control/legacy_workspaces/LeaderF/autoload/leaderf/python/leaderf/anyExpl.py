    ...
    def getContent(self, *args, **kwargs):
        ...
        if format_line:
            try:
                ...
                raise Exception(f"Error occurred in user defined {str(format_line)}: {err}")
...
class AnyExplManager(Manager):
    ...
    def _defineMaps(self):
        lfCmd(f"call leaderf#Any#Maps('{self._category}')")
    ...
    def _needExit(self, line, arguments):
        ...
        if need_exit:
            try:
                ...
                raise Exception(f"Error occurred in user defined {str(need_exit)}: {err}")
        else:
            return True
    ...
    def _acceptSelection(self, *args, **kwargs):
        ...
        if accept:
            try:
                ...
                raise Exception(f"Error occurred in user defined {str(accept)}: {err}")
    ...
    def _getDigest(self, line, mode):
        ...
        if get_digest:
            try:
                ...
                raise Exception(f"Error occurred in user defined {str(get_digest)}: {err}")
        else:
            return line
    ...
    def _beforeEnter(self):
        ...
        if before_enter:
            try:
                ...
                raise Exception(f"Error occurred in user defined {str(before_enter)}: {err}")
    ...
    def _afterEnter(self):
        ...
        if self._getInstance().getWinPos() == "popup":
            ...
            for group, pattern in highlights_def.items():
                id = int(lfEval(f"matchadd('{group}', '{escQuote(pattern)}')"))
                self._match_ids.append(id)
        ...
        if highlight:
            try:
                ...
                raise Exception(f"Error occurred in user defined {str(highlight)}: {err}")
    ...
    def _afterExit(self):
        ...
        if after_exit:
            try:
                ...
                raise Exception(f"Error occurred in user defined {str(after_exit)}: {err}")
