...
import logging
from src.ShadowTag-v2.services.gemini_failover import GeminiFailoverError
import uuid
...
logger = logging.getLogger("n-autoresearch/Kosmos/BioAgentss_api")
...
# Decorator for graceful error handling of Gemini failures
def handle_gemini_errors(func):
    async def wrapper(*args, **kwargs):
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail="Gemini service temporarily unavailable. Please retry later."
            )
        except Exception:
...
def hunt_async(request: HuntRequest, background_tasks: BackgroundTasks):
    ...
    return JobResponse(
        job_id=job_id,
        status="queued",
        message=f"Hunt started. Poll /status/{job_id}"
    )
...
from src.ShadowTag-v2.services.vertex_ai_client import generate_text

@app.get("/vertex_test")
def vertex_test(prompt: str = "Hello, Vertex AI!"):
    """Simple test endpoint that calls Vertex AI directly.
    ...
    try:
        ...
        logger.exception("Vertex AI test failed")
