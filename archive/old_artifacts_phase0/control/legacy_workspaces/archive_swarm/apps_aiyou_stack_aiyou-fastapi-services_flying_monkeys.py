    ...
    if reporting:
        ...
        if gcs_bucket_name and storage:
            try:
                ...
                logger.info(f"📊 [n-autoresearch/Kosmos/BioAgents] Report uploaded to {gcs_path}")
            except Exception as e:
...
async def capture_visual_evidence(target_url: str) -> str:
    ...
    if not async_playwright:
        logger.error("❌ [n-autoresearch/Kosmos/BioAgents] Playwright not installed. Cannot capture evidence.")
        return "error_playwright_missing"

    logger.info(f"🦅 [n-autoresearch/Kosmos/BioAgents] Launching Headless Scout (Playwright) for {target_url}...")
    async with async_playwright() as p:
        ...
        await browser.close()
    logger.info(f"🦅 [n-autoresearch/Kosmos/BioAgents] Visual evidence secured: {filename}")
    return filename
