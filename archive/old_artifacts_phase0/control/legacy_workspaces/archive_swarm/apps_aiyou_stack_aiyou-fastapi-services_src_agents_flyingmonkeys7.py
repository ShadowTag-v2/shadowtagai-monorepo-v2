    ...
    def execute(self, code: str) -> str:
        ...
        try:
            with redirect_stdout(f):
                ...
                exec(code, {"__builtins__": __builtins__, "re": __import__("re")}, self.local_scope)
            return f.getvalue()
...
class LiteratureSearchAgent(AgentUnit):
    ...
    def execute_task(self, task: Task, repl: KosmosREPL | None = None) -> Finding:
        ...
        return Finding(
            ...
            provenance=Provenance(
                "Literature", f"DOI:10.1038/nature{random.randint(1000, 9999)}", time.time()
            ),
...
class n-autoresearch/Kosmos/BioAgents7:
    ...
    def director_plan(self) -> list[Task]:
        ...
        if self.world_model.cycle_count == 0:
            ...
            tasks.append(
                Task("DataAnalysis", f"Verify hypothesis {len(self.world_model.hypotheses)}")
            )
