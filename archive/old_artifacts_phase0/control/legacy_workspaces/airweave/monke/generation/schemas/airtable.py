...
class AirtableRecordSpec(BaseModel):
    ...
    table_name: str = Field(default="MonkeTestTable", description="Name of the table (for test tracking)")


class AirtableRecordContent(BaseModel):
    """Content for an Airtable record."""

    primary_field: str = Field(description="Primary field value (name/title) - must contain token")
    description: str = Field(description="Description or notes field")
    status: str = Field(default="In Progress", description="Status field (e.g., 'In Progress', 'Done')")
    tags: list[str] = Field(default_factory=list, description="List of tags or categories")
