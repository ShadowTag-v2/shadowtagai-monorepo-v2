...
class TestLoadMcpResourceTool:
    ...
    async def test_run_async(self):
        ...
        result = await tool.run_async(args=args, tool_context=self.mock_tool_context)
    ...
    async def test_process_llm_request_injects_list(self):
        ...
        # Mock list_resources
        self.mock_mcp_toolset.list_resources = AsyncMock(return_value=["res1", "res2"])

        await tool.process_llm_request(tool_context=self.mock_tool_context, llm_request=llm_request)
    ...
    async def test_process_llm_request_loads_content_text(self):
        ...
        # Mock read_resource
        text_content = TextResourceContents(uri="file:///res1", mimeType="text/plain", text="hello content")
    ...
    async def test_process_llm_request_loads_content_binary(self):
        ...
        blob_content = BlobResourceContents(uri="file:///res1", mimeType="image/png", blob=blob_b64)
