...
import pytest
from fastapi.openapi.models import MediaType, Operation, Parameter, RequestBody, Response, Schema
            ...
            content={
                "application/json": MediaType(
                    schema=Schema(
                        type="object",
                        properties={
                            "prop1": Schema(type="string", description="Property 1"),
                            "prop2": Schema(type="integer", description="Property 2"),
                        },
                    )
                )
            },
...
def test_process_request_body_array():
                ...
                "application/json": MediaType(
                    schema=Schema(
                        ...
                        items=Schema(
                            type="object",
                            properties={
                                "item_prop1": Schema(type="string", description="Item Property 1"),
                                "item_prop2": Schema(type="integer", description="Item Property 2"),
                            },
                        ),
                    )
                )
    ...
    assert parser._params[0].param_schema.items.properties["item_prop1"].description == "Item Property 1"
    assert parser._params[0].param_schema.items.properties["item_prop2"].description == "Item Property 2"


def test_process_request_body_no_name():
    """Test _process_request_body with a schema that has no properties (unnamed)"""
    operation = Operation(requestBody=RequestBody(content={"application/json": MediaType(schema=Schema(type="string"))}))
    parser = OperationParser(operation, should_parse=False)
    parser._process_request_body()
    assert len(parser._params) == 1
...
def test_process_request_body_empty_object():
    ...
    operation = Operation(requestBody=RequestBody(content={"application/json": MediaType(schema=Schema(type="object"))}))
    parser = OperationParser(operation, should_parse=False)
    parser._process_request_body()
...
def test_process_return_value_no_2xx(sample_operation):
    ...
    operation_no_2xx = Operation(responses={"400": Response(description="Client Error")})
...
def test_process_return_value_multiple_2xx(sample_operation):
    ...
    operation_multi_2xx = Operation(
        responses={
            "201": Response(
                description="Success",
                content={"application/json": MediaType(schema=Schema(type="integer"))},
            ),
            "202": Response(
                description="Success",
            ...
            "200": Response(
                description="Success",
                content={"application/pdf": MediaType(schema=Schema(type="boolean"))},
            ),
            "400": Response(
                description="Failure",
                content={"application/xml": MediaType(schema=Schema(type="object"))},
            ),
        }
    )
...
def test_process_return_value_no_content(sample_operation):
    ...
    operation_no_content = Operation(responses={"200": Response(description="Success", content={})})
...
def test_load():
    ...
    return_value = ApiParameter(original_name="", param_location="", param_schema={"type": "string"})
    ...
    assert parser.get_function_name() == "my_op"  # Check that the operation is loaded
