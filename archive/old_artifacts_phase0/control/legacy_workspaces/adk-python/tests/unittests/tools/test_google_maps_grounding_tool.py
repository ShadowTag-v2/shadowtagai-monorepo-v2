...
class TestGoogleMapsGroundingTool:
    ...
    @pytest.mark.asyncio
    async def test_process_llm_request_with_non_gemini_and_disabled_check(self, monkeypatch):
        monkeypatch.setenv("ADK_DISABLE_GEMINI_MODEL_ID_CHECK", "true")
        tool = GoogleMapsGroundingTool()
        tool_context = await _create_tool_context()
        llm_request = LlmRequest(model="internal-model-v1", config=types.GenerateContentConfig())
