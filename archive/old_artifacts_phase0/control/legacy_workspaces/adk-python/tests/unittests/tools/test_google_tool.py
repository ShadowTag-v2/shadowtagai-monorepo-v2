...
class TestGoogleTool:
    ...
    def async_sample_function(self):
        ...
        async def async_sample_func(param1: str, credentials: Credentials = None) -> dict:
    ...
    def test_tool_initialization_with_credentials(self, sample_function, credentials_config):
        ...
        """
        tool = GoogleTool(func=sample_function, credentials_config=credentials_config)
    ...
    @pytest.mark.asyncio
    async def test_run_async_with_valid_credentials(self, sample_function, credentials_config, mock_tool_context):
        ...
        with patch.object(
            ...
            result = await tool.run_async(args={"param1": "test_value"}, tool_context=mock_tool_context)
    ...
    @pytest.mark.asyncio
    async def test_run_async_oauth_flow_in_progress(self, sample_function, credentials_config, mock_tool_context):
        ...
        with patch.object(tool._credentials_manager, "get_valid_credentials", return_value=None) as mock_get_creds:
            result = await tool.run_async(args={"param1": "test_value"}, tool_context=mock_tool_context)
    ...
    @pytest.mark.asyncio
    async def test_run_async_without_credentials_manager(self, sample_function, mock_tool_context):
    ...
    @pytest.mark.asyncio
    async def test_run_async_with_async_function(self, async_sample_function, credentials_config, mock_tool_context):
        ...
        """
        tool = GoogleTool(func=async_sample_function, credentials_config=credentials_config)
    ...
    @pytest.mark.asyncio
    async def test_run_async_exception_handling(self, credentials_config, mock_tool_context):
        ...
        tool = GoogleTool(func=failing_function, credentials_config=credentials_config)
    ...
    def test_function_signature_analysis(self, credentials_config):
        ...
        tool = GoogleTool(func=complex_function, credentials_config=credentials_config)
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                BigQueryToolConfig(write_mode="blocked", max_query_result_rows=50),
                BigQueryToolConfig(write_mode="blocked", max_query_result_rows=50),
                id="with_provided_config",
            ),
        ],
    )
    def test_tool_bigquery_config_initialization(self, input_settings, expected_settings):
    ...
    def test_tool_spanner_settings_initialization(self, input_settings, expected_settings):
