...
from google.adk.tools.spanner.settings import SpannerToolSettings, SpannerVectorStoreSettings, TableColumn, VectorSearchIndexSettings
...
def test_add_contents_successful(
    ...
    mock_embed_contents.assert_called_once_with("textembedding", contents, 768, mock.ANY)
...
def test_add_contents_with_metadata(
    ...
    mock_batch.__enter__.return_value = mock_batch
    mock_spanner_database.batch.return_value = mock_batch
    spanner_tool_settings.vector_store_settings.additional_columns_to_setup = [TableColumn(name="metadata", type="JSON")]
...
def test_add_contents_empty_contents(spanner_tool_settings, mock_spanner_client, mock_spanner_database):
...
@mock.patch.object(spanner_utils, "embed_contents", autospec=True)
def test_add_contents_additional_columns_list_mismatch(mock_embed_contents, spanner_tool_settings, mock_spanner_client):
...
@mock.patch.object(spanner_utils, "embed_contents", autospec=True)
def test_add_contents_embedding_fails(mock_embed_contents, spanner_tool_settings, mock_spanner_client):
...
def test_init_raises_error_if_vector_store_settings_not_set():
    ...
    settings = SpannerToolSettings()
    with pytest.raises(ValueError, match="Spanner vector store settings are not set."):
...
def test_create_vector_store_table_ddl(spanner_tool_settings, mock_spanner_client, dialect, expected_ddl):
...
def test_create_ann_vector_search_index_ddl_raises_error_for_postgresql(spanner_tool_settings, vector_store_settings, mock_spanner_client):
    ...
    with mock.patch.object(
        ...
        with pytest.raises(
            ...
            vector_store._create_ann_vector_search_index_ddl(DatabaseDialect.POSTGRESQL)


def test_create_vector_store(spanner_tool_settings, mock_spanner_client, mock_spanner_database):
...
def test_create_vector_search_index_no_settings(spanner_tool_settings, mock_spanner_client, mock_spanner_database):
    """Test that create_vector_search_index does nothing if settings are not present."""
    spanner_tool_settings.vector_store_settings.vector_search_index_settings = None
    with mock.patch.object(
        spanner_utils.client,
        "get_spanner_client",
        autospec=True,
...
def test_create_vector_search_index_successful_google_sql(
    ...
    vector_store_settings.vector_search_index_settings = VectorSearchIndexSettings(
...
def test_create_vector_search_index_fails(
    ...
    vector_store_settings.vector_search_index_settings = VectorSearchIndexSettings(index_name="test_vector_index")
    with mock.patch.object(
        spanner_utils.client,
        "get_spanner_client",
        autospec=True,
