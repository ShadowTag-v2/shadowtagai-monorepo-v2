...
import pytest
from google.adk.tools._gemini_schema_util import _sanitize_schema_formats_for_gemini, _to_gemini_schema, _to_snake_case
...
class TestToGeminiSchema:
    ...
    def test_to_gemini_schema_nested_objects(self):
        ...
        assert gemini_schema.properties["address"].properties["street"].type == Type.STRING
        assert gemini_schema.properties["address"].properties["city"].type == Type.STRING
    ...
    def test_to_gemini_schema_nested_dict(self):
        ...
        assert gemini_schema.properties["metadata"].type == Type.OBJECT  # add object type by default
        assert len(gemini_schema.properties["metadata"].properties) == 2
        assert gemini_schema.properties["metadata"].properties["key1"].type == Type.OBJECT
        assert gemini_schema.properties["metadata"].properties["key2"].type == Type.STRING
    ...
    def test_to_gemini_schema_nested_dict_with_defs_and_ref(self):
        ...
        assert gemini_schema.properties["payload"].properties["adDomain"].type == Type.ARRAY
        assert gemini_schema.properties["payload"].properties["adDomain"].items.type == Type.STRING
        assert gemini_schema.properties["payload"].properties["device"].type == Type.STRING
    ...
    def test_sanitize_nested_formats(self):
        ...
        # Check nested object
        assert gemini_schema.properties["nested"].properties["date_str"].format is None
        assert gemini_schema.properties["nested"].properties["int_field"].format == "int64"
        # Check array items
    ...
    def test_sanitize_schema_formats_for_gemini(self):
        schema = {
            ...
            "unknownTopLevelKey": ("discard_me_too"),  # Another unsupported top-level key
        }
        ...
        assert "format" not in sanitized_props["invalid_format_prop"]  # Invalid format removed
        ...
        schema_with_list_type = {
            "type": "object",
            "properties": {"nullable_field": {"type": ["string", "null"], "format": "uuid"}},
        }
        sanitized_list_type = _sanitize_schema_formats_for_gemini(schema_with_list_type)
