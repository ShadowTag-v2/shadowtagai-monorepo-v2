...
import pytest
import requests
from fastapi.openapi.models import APIKey, APIKeyIn, HTTPBase, HTTPBearer, OAuth2, OpenIdConnect
from google.adk.auth.auth_credential import AuthCredential, AuthCredentialTypes, HttpAuth, HttpCredentials, ServiceAccount, ServiceAccountCredential
...
def test_token_to_scheme_credential_api_key_header():
    scheme, credential = token_to_scheme_credential("apikey", "header", "X-API-Key", "test_key")

    assert isinstance(scheme, APIKey)
    assert scheme.type_ == AuthSchemeType.apiKey
    assert scheme.in_ == APIKeyIn.header
    assert scheme.name == "X-API-Key"
    assert credential == AuthCredential(auth_type=AuthCredentialTypes.API_KEY, api_key="test_key")


def test_token_to_scheme_credential_api_key_query():
    scheme, credential = token_to_scheme_credential("apikey", "query", "api_key", "test_key")
...
def test_token_to_scheme_credential_api_key_cookie():
    scheme, credential = token_to_scheme_credential("apikey", "cookie", "session_id", "test_key")
...
def test_token_to_scheme_credential_api_key_no_credential():
    scheme, credential = token_to_scheme_credential("apikey", "cookie", "session_id")
...
def test_token_to_scheme_credential_oauth2_token():
    scheme, credential = token_to_scheme_credential("oauth2Token", "header", "Authorization", "test_token")

    assert isinstance(scheme, HTTPBearer)
    assert scheme.bearerFormat == "JWT"
    assert credential == AuthCredential(
        auth_type=AuthCredentialTypes.HTTP,
        http=HttpAuth(scheme="bearer", credentials=HttpCredentials(token="test_token")),
    )


def test_token_to_scheme_credential_oauth2_no_credential():
    scheme, credential = token_to_scheme_credential("oauth2Token", "header", "Authorization")
...
def test_service_account_dict_to_scheme_credential():
    ...
    assert credential.service_account.service_account_credential.project_id == "project_id"
...
def test_openid_dict_to_scheme_credential():
    ...
    scheme, credential = openid_dict_to_scheme_credential(config_dict, scopes, credential_dict)
...
def test_openid_url_to_scheme_credential(mock_get):
    ...
    scheme, credential = openid_url_to_scheme_credential("openid_url", scopes, credential_dict)
...
def test_openid_url_to_scheme_credential_request_exception(mock_get):
    ...
    with pytest.raises(ValueError, match="Failed to fetch OpenID configuration from openid_url"):
...
def test_openid_url_to_scheme_credential_invalid_json(mock_get):
    ...
    with pytest.raises(
        ValueError,
        match=("Invalid JSON response from OpenID configuration endpoint openid_url"),
    ):
        openid_url_to_scheme_credential("openid_url", [], credential_dict)


def test_credential_to_param_api_key_header():
    auth_scheme = APIKey(**{"type": "apiKey", "in": "header", "name": "X-API-Key"})
    auth_credential = AuthCredential(auth_type=AuthCredentialTypes.API_KEY, api_key="test_key")
...
def test_credential_to_param_api_key_cookie():
    auth_scheme = APIKey(**{"type": "apiKey", "in": "cookie", "name": "session_id"})
    auth_credential = AuthCredential(auth_type=AuthCredentialTypes.API_KEY, api_key="test_key")
...
def test_credential_to_param_http_basic_not_supported():
    ...
    with pytest.raises(NotImplementedError, match="Basic Authentication is not supported."):
...
def test_dict_to_auth_scheme_missing_type():
    ...
    with pytest.raises(ValueError, match="Missing 'type' field in security scheme dictionary."):
