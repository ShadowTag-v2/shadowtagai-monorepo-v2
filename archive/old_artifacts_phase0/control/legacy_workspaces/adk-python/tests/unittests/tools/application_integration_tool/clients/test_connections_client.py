...
class TestConnectionsClient:
    def test_initialization(self, project, location, connection_name):
        ...
        client = ConnectionsClient(project, location, connection_name, json.dumps(credentials))
    ...
    def test_execute_api_call_success(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        ...
        with (
            mock.patch.object(client, "_get_access_token", return_value=mock_credentials.token),
    ...
    def test_execute_api_call_credential_error(self, project, location, connection_name):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        with (
            mock.patch.object(
            ...
            pytest.raises(PermissionError, match="Credentials error: Test"),
        ):
    ...
    def test_execute_api_call_request_error_not_found_or_bad_request(
        ...
        mock_response.raise_for_status.side_effect = exceptions.HTTPError(f"HTTP error {status_code}: {response_text}")

        with (
            mock.patch.object(client, "_get_access_token", return_value=mock_credentials.token),
            mock.patch("requests.get", return_value=mock_response),
            pytest.raises(ValueError, match="Invalid request. Please check the provided"),
        ):
            client._execute_api_call("https://test.url")

    def test_execute_api_call_other_request_error(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        mock_response = mock.MagicMock()
        mock_response.status_code = 500
        mock_response.raise_for_status.side_effect = exceptions.HTTPError("Internal Server Error")

        with (
            mock.patch.object(client, "_get_access_token", return_value=mock_credentials.token),
            mock.patch("requests.get", return_value=mock_response),
            pytest.raises(ValueError, match="Request error: "),
        ):
            client._execute_api_call("https://test.url")

    def test_execute_api_call_unexpected_error(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        with (
            mock.patch.object(client, "_get_access_token", return_value=mock_credentials.token),
            mock.patch("requests.get", side_effect=Exception("Something went wrong")),
            pytest.raises(Exception, match="An unexpected error occurred: Something went wrong"),
        ):
            client._execute_api_call("https://test.url")

    def test_get_connection_details_success_with_host(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        mock_response = mock.MagicMock()
        mock_response.status_code = 200
        ...
        with mock.patch.object(client, "_execute_api_call", return_value=mock_response):
    ...
    def test_get_connection_details_success_without_host(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        mock_response = mock.MagicMock()
        mock_response.status_code = 200
    ...
    def test_get_connection_details_without_name(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        mock_response = mock.MagicMock()
        mock_response.status_code = 200
    ...
    def test_get_connection_details_error(self, project, location, connection_name):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        with (
            mock.patch.object(client, "_execute_api_call", side_effect=ValueError("Request error")),
            pytest.raises(ValueError, match="Request error"),
        ):
            client.get_connection_details()

    def test_get_entity_schema_and_operations_success(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        mock_execute_response_initial = mock.MagicMock()
        mock_execute_response_initial.status_code = 200
        mock_execute_response_initial.json.return_value = {"name": "operations/test_op"}
        ...
        with mock.patch.object(
            ...
            assert mock.call("https://connectors.googleapis.com/v1/operations/test_op") in client._execute_api_call.mock_calls

    def test_get_entity_schema_and_operations_no_operation_id(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        ...
        with (
            mock.patch.object(client, "_execute_api_call", return_value=mock_execute_response),
            pytest.raises(
                ValueError,
                match=("Failed to get entity schema and operations for entity: entity1"),
            ),
        ):
            client.get_entity_schema_and_operations("entity1")

    def test_get_entity_schema_and_operations_execute_api_call_error(self, project, location, connection_name):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        with (
            mock.patch.object(client, "_execute_api_call", side_effect=ValueError("Request error")),
    ...
    def test_get_action_schema_success(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        mock_execute_response_initial = mock.MagicMock()
        mock_execute_response_initial.status_code = 200
    ...
    def test_get_action_schema_no_operation_id(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        mock_execute_response = mock.MagicMock()
        mock_execute_response.status_code = 200
        ...
        with (
            mock.patch.object(client, "_execute_api_call", return_value=mock_execute_response),
            pytest.raises(ValueError, match="Failed to get action schema for action: action1"),
        ):
            client.get_action_schema("action1")

    def test_get_action_schema_execute_api_call_error(self, project, location, connection_name):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        with (
            mock.patch.object(client, "_execute_api_call", side_effect=ValueError("Request error")),
    ...
    def test_get_action_operation(self):
        operation = ConnectionsClient.get_action_operation("TestAction", "EXECUTE_ACTION", "TestActionDisplayName", "test_tool")
    ...
    def test_list_operation(self):
        operation = ConnectionsClient.list_operation("Entity1", '{"type": "object"}', "test_tool")
        assert "post" in operation
    ...
    def test_get_operation_static(self):
        operation = ConnectionsClient.get_operation("Entity1", '{"type": "object"}', "test_tool")
        assert "post" in operation
    ...
    def test_get_access_token_uses_cached_token(self, project, location, connection_name, mock_credentials):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
    ...
    def test_get_access_token_with_service_account_credentials(self, project, location, connection_name):
        ...
        client = ConnectionsClient(project, location, connection_name, service_account_json)
    ...
    def test_get_access_token_with_default_credentials(self, project, location, connection_name, mock_credentials):
    ...
    def test_get_access_token_no_valid_credentials(self, project, location, connection_name):
        client = ConnectionsClient(project, location, connection_name, None)
        with (
            mock.patch(
                "google.adk.tools.application_integration_tool.clients.connections_client.default_service_credential",
            ...
            pytest.raises(
                ValueError,
                match=("Please provide a service account that has the required permissions"),
            ),
        ):
            client._get_access_token()

    def test_get_access_token_refreshes_expired_token(self, project, location, connection_name, mock_credentials):
        client = ConnectionsClient(project, location, connection_name, None)
