...
def test_vertex_rag_retrieval_for_gemini_1_x():
    ...
    agent = Agent(
        ...
        tools=[
            VertexAiRagRetrieval(
                ...
                rag_corpora=["projects/123456789/locations/us-central1/ragCorpora/1234567890"],
            )
        ],
    )
    ...
    assert mockModel.requests[0].config.tools[0].function_declarations[0].name == "rag_retrieval"
...
def test_vertex_rag_retrieval_for_gemini_1_x_with_another_function_tool():
    ...
    assert mockModel.requests[0].config.tools[0].function_declarations[0].name == "rag_retrieval"
    assert mockModel.requests[0].config.tools[0].function_declarations[1].name == "noop_tool"
    assert mockModel.requests[0].tools_dict["rag_retrieval"] is not None
    ...
    assert mockModel.requests[0].config.tools == [
        types.Tool(
            retrieval=types.Retrieval(
                vertex_rag_store=types.VertexRagStore(rag_corpora=["projects/123456789/locations/us-central1/ragCorpora/1234567890"])
            )
        )
    ]
