...
from google.adk.tools.bigquery.bigquery_credentials import BIGQUERY_TOKEN_CACHE_KEY, BigQueryCredentialsConfig
...
class TestGoogleCredentialsManager:
    ...
    @pytest.mark.asyncio
    async def test_get_valid_credentials_with_valid_existing_creds(self, manager, mock_tool_context, credentials_class):
    ...
    @pytest.mark.asyncio
    async def test_get_valid_credentials_with_existing_non_oauth_creds(self, manager, mock_tool_context, valid):
    ...
    @pytest.mark.asyncio
    async def test_get_credentials_from_cache_when_none_in_manager(self, manager, mock_tool_context):
        ...
        with patch("google.oauth2.credentials.Credentials.from_authorized_user_info") as mock_from_json:
            ...
            mock_from_json.assert_called_once_with(json.loads(mock_cached_creds_json), manager.credentials_config.scopes)
    ...
    @pytest.mark.asyncio
    async def test_no_credentials_in_manager_or_cache(self, manager, mock_tool_context):
    ...
    async def test_refresh_cached_credentials_success(self, mock_request_class, manager, mock_tool_context):
        ...
        with patch("google.oauth2.credentials.Credentials.from_authorized_user_info") as mock_from_json:
            ...
            assert json.loads(mock_tool_context.state[BIGQUERY_TOKEN_CACHE_KEY])["token"] == "new_token"
            assert result == mock_cached_creds

    @pytest.mark.asyncio
    @patch("google.auth.transport.requests.Request")
    async def test_get_valid_credentials_with_refresh_success(self, mock_request_class, manager, mock_tool_context):
    ...
    @pytest.mark.asyncio
    @patch("google.auth.transport.requests.Request")
    async def test_get_valid_credentials_with_refresh_failure(self, mock_request_class, manager, mock_tool_context):
    ...
    @pytest.mark.asyncio
    async def test_oauth_flow_completion_with_caching(self, manager, mock_tool_context):
        ...
        with patch(
            ...
            assert mock_tool_context.state[BIGQUERY_TOKEN_CACHE_KEY] == mock_creds_json
    ...
    @pytest.mark.asyncio
    async def test_cache_persistence_across_manager_instances(self, credentials_config, mock_tool_context):
        ...
        with patch("google.adk.tools._google_credentials.google.oauth2.credentials.Credentials.from_authorized_user_info") as mock_from_json:
            mock_cached_creds = Mock(spec=OAuthCredentials)
            mock_cached_creds.valid = True
            mock_from_json.return_value = mock_cached_creds
            ...
            assert cached_creds_json == mock_tool_context.state[BIGQUERY_TOKEN_CACHE_KEY]
            ...
            actual_data = actual_json_arg if isinstance(actual_json_arg, dict) else json.loads(actual_json_arg)
