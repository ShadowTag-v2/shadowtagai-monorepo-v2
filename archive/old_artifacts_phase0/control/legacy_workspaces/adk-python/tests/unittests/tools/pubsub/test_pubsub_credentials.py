...
import pytest
from google.adk.tools.pubsub.pubsub_credentials import PUBSUB_DEFAULT_SCOPE, PubSubCredentialsConfig
...
def test_pubsub_credentials_config_oauth2_creds():
    ...
    """
    mock_creds = mock.create_autospec(google.oauth2.credentials.Credentials, instance=True)
...
def test_pubsub_credentials_config_validation_errors(credentials, client_id, client_secret):
    ...
    with pytest.raises(
        ValueError,
        match=("Must provide one of credentials, external_access_token_key, or client_id and client_secret pair."),
    ):
...
def test_pubsub_credentials_config_both_credentials_and_client_provided():
    ...
    with pytest.raises(
        ValueError,
        match=("If credentials are provided, external_access_token_key, client_id, client_secret, and scopes must not be provided."),
    ):
        PubSubCredentialsConfig(
