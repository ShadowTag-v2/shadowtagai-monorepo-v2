...
def test_bigquery_client_project_set_explicit():
    ...
    with mock.patch.dict(os.environ, {}, clear=True), mock.patch.object(google.auth, "default", autospec=True) as mock_default_auth:
...
def test_bigquery_client_project_set_with_env():
    ...
    with (
        mock.patch.dict(os.environ, {"GOOGLE_CLOUD_PROJECT": "test-gcp-project"}, clear=True),
        mock.patch.object(google.auth, "default", autospec=True) as mock_default_auth,
    ):
        # Simulate exception from default auth
        mock_default_auth.side_effect = DefaultCredentialsError("Your default credentials were not found")
...
def test_bigquery_client_user_agent_default():
    ...
    with mock.patch.object(bigquery_client, "Connection", autospec=True) as mock_connection:
        # Trigger the BigQuery client creation
        get_bigquery_client(
            project="test-gcp-project",
            credentials=mock.create_autospec(Credentials, instance=True),
