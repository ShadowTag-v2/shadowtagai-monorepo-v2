...
import pytest
import yaml
from fastapi.openapi.models import APIKey, APIKeyIn, MediaType, OAuth2, ParameterInType, SecuritySchemeType
...
def test_openapi_toolset_tool_existing(openapi_spec: dict):
    ...
    assert isinstance(tool.operation.requestBody.content["application/json"], MediaType)
    ...
    assert (
        tool.operation.parameters[0].description == "Calendar identifier. To retrieve calendar IDs call the"
