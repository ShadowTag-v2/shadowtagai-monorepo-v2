...
from google.adk.tools.bigquery import BigQueryCredentialsConfig, BigQueryToolset, query_tool
...
async def get_tool(name: str, tool_settings: BigQueryToolConfig | None = None) -> BaseTool:
    ...
    Returns:
      The tool.
    """
    credentials_config = BigQueryCredentialsConfig(client_id="abc", client_secret="def")
...
def test_execute_sql_select_stmt(write_mode):
    ...
    with mock.patch.object(bigquery, "Client", autospec=True) as Client:
        ...
        # Test the tool
        result = query_tool.execute_sql(project, query, credentials, tool_settings, tool_context)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "CREATE MODEL my_dataset.my_model (model_type='linear_reg', input_label_cols=['label_col']) AS SELECT * FROM my_dataset.my_table",
            "CREATE_MODEL",
            id="create-model",
        ),
        pytest.param(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "CREATE TEMP MODEL my_model (model_type='linear_reg', input_label_cols=['label_col']) AS SELECT * FROM my_dataset.my_table",
            "CREATE_MODEL",
            id="create-model",
        ),
        pytest.param(
...
def test_execute_sql_non_select_stmt_write_protected_persistent_target(query, statement_type):
    """Test execute_sql tool for non-SELECT query when writes are protected.
...
def test_execute_sql_dry_run_true():
    ...
    with mock.patch.object(bigquery, "Client", autospec=True) as Client:
        ...
        result = query_tool.execute_sql(project, query, credentials, tool_settings, tool_context, dry_run=True)
        ...
        assert mock_kwargs["job_config"].dry_run
...
def test_execute_sql_no_default_auth(mock_default_auth, mock_query, mock_query_and_wait, write_mode):
    ...
    mock_default_auth.side_effect = DefaultCredentialsError("Your default credentials were not found")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("SELECT TRUE AS x", [{"x": True}], [{"x": True}], id="BOOL"),
        pytest.param(
        ...
        pytest.param("SELECT 1.23 AS x", [{"x": 1.23}], [{"x": 1.23}], id="FLOAT64"),
        pytest.param(
            ...
            [{"x": ("{'start': datetime.date(2023, 1, 1), 'end': datetime.date(2023, 1, 31)}")}],
            id="RANGE",
        ),
        pytest.param("SELECT 'abc' AS x", [{"x": "abc"}], [{"x": "abc"}], id="STRING"),
        pytest.param(
        ...
        pytest.param(
            ...
            [{"x": datetime.datetime(2025, 7, 21, 17, 30, 45, tzinfo=datetime.UTC)}],
            [{"x": "2025-07-21 17:30:45+00:00"}],
            id="TIMESTAMP",
        ),
        pytest.param("SELECT NULL AS x", [{"x": None}], [{"x": None}], id="NULL"),
    ],
)
@mock.patch.dict(os.environ, {}, clear=True)
@mock.patch.object(bigquery.Client, "query_and_wait", autospec=True)
@mock.patch.object(bigquery.Client, "query", autospec=True)
def test_execute_sql_result_dtype(mock_query, mock_query_and_wait, query, query_result, tool_result_rows):
...
def test_execute_sql_bq_client_creation(mock_get_bigquery_client):
    ...
    tool_context = mock.create_autospec(ToolContext, instance=True)
    query_tool.execute_sql(project, query, credentials, tool_settings, tool_context)
...
def test_execute_sql_unexpected_project_id():
    ...
    result = query_tool.execute_sql(tool_call_project_id, query, credentials, tool_settings, tool_context)
    assert result == {
        "status": "ERROR",
        "error_details": (
...
def test_analyze_contribution_with_invalid_dimension_id_cols():
    ...
    assert result["status"] == "ERROR"
    assert "All elements in dimension_id_cols must be strings." in result["error_details"]
...
def test_detect_anomalies_with_invalid_id_cols():
    ...
    assert result["status"] == "ERROR"
    assert "All elements in times_series_id_cols must be strings." in result["error_details"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(WriteMode.PROTECTED, True, 3, 0, id="write-protected-dry-run"),
    ],
)
def test_execute_sql_job_labels(write_mode, dry_run, query_call_count, query_and_wait_call_count):
    """Test execute_sql tool for job label."""
    project = "my_project"
    query = "SELECT 123 AS num"
    statement_type = "SELECT"
    credentials = mock.create_autospec(Credentials, instance=True)
    tool_settings = BigQueryToolConfig(write_mode=write_mode, application_name="test-app")
    tool_context = mock.create_autospec(ToolContext, instance=True)
...
def test_ml_tool_job_labels(tool_call, expected_tool_label):
    ...
    with mock.patch.object(bigquery, "Client", autospec=True) as Client:
        ...
        for call_args_list in [
            ...
            for call_args in call_args_list:
                _, mock_kwargs = call_args
                assert mock_kwargs["job_config"].labels == {"adk-bigquery-tool": expected_tool_label}
    ...
    [
        pytest.param(
            lambda tool_context: query_tool.forecast(
                ...
                data_col="data_col",
                credentials=mock.create_autospec(Credentials, instance=True),
                settings=BigQueryToolConfig(write_mode=WriteMode.ALLOWED, application_name="test-app"),
                tool_context=tool_context,
            ),
            "forecast",
...
def test_execute_sql_max_rows_config():
    ...
    with mock.patch.object(bigquery, "Client", autospec=True) as Client:
        ...
        result = query_tool.execute_sql(project, query, credentials, tool_config, tool_context)
...
def test_execute_sql_maximum_bytes_billed_config():
    ...
    with mock.patch.object(bigquery, "Client", autospec=True) as Client:
        ...
        query_tool.execute_sql(project, query, credentials, tool_config, tool_context)
