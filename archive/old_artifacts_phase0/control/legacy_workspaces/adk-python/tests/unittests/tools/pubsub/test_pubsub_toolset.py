...
from google.adk.tools.pubsub import PubSubCredentialsConfig, PubSubToolset
...
async def test_pubsub_toolset_tools_default():
    ...
    specified.
    """
    credentials_config = PubSubCredentialsConfig(client_id="abc", client_secret="def")
    toolset = PubSubToolset(credentials_config=credentials_config, pubsub_tool_config=None)
...
async def test_pubsub_toolset_tools_selective(selected_tools):
    ...
    """
    credentials_config = PubSubCredentialsConfig(client_id="abc", client_secret="def")
    toolset = PubSubToolset(credentials_config=credentials_config, tool_filter=selected_tools)
    tools = await toolset.get_tools()
    assert tools is not None
