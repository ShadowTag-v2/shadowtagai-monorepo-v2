...
from google.adk.utils.model_name_utils import extract_model_name, is_gemini_1_model, is_gemini_model
from google.genai import types

VERTEX_SEARCH_TOOL_LOGGER_NAME = "google_adk.google.adk.tools.vertex_ai_search_tool"
...
class TestVertexAiSearchTool:
    ...
    def test_init_with_engine_and_specs(self):
        ...
        specs = [types.VertexAISearchDataStoreSpec(dataStore=("projects/p/locations/l/collections/c/dataStores/spec_store"))]
        engine_id = "projects/p/locations/l/collections/c/engines/test_search_engine"
    ...
    def test_init_with_both_raises_error(self):
        ...
        with pytest.raises(
            ...
            VertexAiSearchTool(data_store_id="test_data_store", search_engine_id="test_search_engine")
    ...
    def test_init_with_specs_but_no_engine_raises_error(self):
        ...
        with pytest.raises(
            ValueError,
            match=("search_engine_id must be specified if data_store_specs is specified"),
        ):
            VertexAiSearchTool(data_store_id="test_data_store", data_store_specs=specs)
    ...
    async def test_process_llm_request_with_simple_gemini_model(self, caplog):
        ...
        tool = VertexAiSearchTool(data_store_id="test_data_store", filter="f", max_results=5)
        tool_context = await _create_tool_context()

        llm_request = LlmRequest(model="gemini-2.5-pro", config=types.GenerateContentConfig())
        ...
        assert retrieval_tool.retrieval.vertex_ai_search.datastore == "test_data_store"
        ...
        # Verify debug log
        debug_records = [r for r in caplog.records if "Adding Vertex AI Search tool config" in r.message]
    ...
    async def test_process_llm_request_with_gemini_1_and_other_tools_raises_error(
        ...
        existing_tool = types.Tool(function_declarations=[types.FunctionDeclaration(name="test_function", description="test")])
        ...
        with pytest.raises(
            ValueError,
            match=("Vertex AI search tool cannot be used with other tools in Gemini 1.x"),
        ):
            await tool.process_llm_request(tool_context=tool_context, llm_request=llm_request)
    ...
    async def test_process_llm_request_with_non_gemini_model_raises_error(self):
        ...
        llm_request = LlmRequest(model="claude-3-sonnet", config=types.GenerateContentConfig())

        with pytest.raises(
            ValueError,
            match=("Vertex AI search tool is not supported for model claude-3-sonnet"),
        ):
            await tool.process_llm_request(tool_context=tool_context, llm_request=llm_request)
    ...
    async def test_process_llm_request_with_path_based_non_gemini_model_raises_error(
        ...
        non_gemini_path = "projects/265104255505/locations/us-central1/publishers/google/models/claude-3-sonnet"
        llm_request = LlmRequest(model=non_gemini_path, config=types.GenerateContentConfig())

        with pytest.raises(
            ValueError,
            match=(f"Vertex AI search tool is not supported for model {non_gemini_path}"),
        ):
            await tool.process_llm_request(tool_context=tool_context, llm_request=llm_request)

    @pytest.mark.asyncio
    async def test_process_llm_request_with_gemini_2_and_other_tools_succeeds(self, caplog):
