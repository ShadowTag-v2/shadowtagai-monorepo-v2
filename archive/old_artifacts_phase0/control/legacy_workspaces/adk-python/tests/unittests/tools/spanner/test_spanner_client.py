...
def test_spanner_client_project_set_explicit():
    ...
    with mock.patch.dict(os.environ, {}, clear=True), mock.patch("google.auth.default", autospec=True) as mock_default_auth:
...
def test_spanner_client_project_set_with_env():
    ...
    with (
        ...
        mock.patch("google.auth.default", autospec=True) as mock_default_auth,
    ):
        # Simulate exception from default auth
        mock_default_auth.side_effect = DefaultCredentialsError("Your default credentials were not found")
...
def test_spanner_client_user_agent():
    ...
    with mock.patch("google.cloud.spanner.Client", autospec=True) as mock_client_class:
