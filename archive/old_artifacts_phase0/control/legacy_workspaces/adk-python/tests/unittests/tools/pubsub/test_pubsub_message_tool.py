...
def test_publish_message(mock_get_publisher_client, mock_publish):
    ...
    mock_publisher_client = mock.create_autospec(pubsub_v1.PublisherClient, instance=True)
    ...
    result = message_tool.publish_message(topic_name, message, mock_credentials, tool_settings)
...
@mock.patch.object(pubsub_v1.PublisherClient, "publish", autospec=True)
@mock.patch.object(pubsub_client_lib, "get_publisher_client", autospec=True)
def test_publish_message_with_ordering_key(mock_get_publisher_client, mock_publish):
...
@mock.patch.object(pubsub_v1.PublisherClient, "publish", autospec=True)
@mock.patch.object(pubsub_client_lib, "get_publisher_client", autospec=True)
def test_publish_message_with_attributes(mock_get_publisher_client, mock_publish):
...
def test_pull_messages(mock_get_subscriber_client):
    ...
    mock_subscriber_client = mock.create_autospec(pubsub_v1.SubscriberClient, instance=True)
    ...
    result = message_tool.pull_messages(subscription_name, mock_credentials, tool_settings)
    ...
    mock_subscriber_client.pull.assert_called_once_with(subscription=subscription_name, max_messages=1)
...
def test_pull_messages_auto_ack(mock_get_subscriber_client):
    ...
    mock_get_subscriber_client.assert_called_once()
    mock_subscriber_client.pull.assert_called_once_with(subscription=subscription_name, max_messages=5)
    mock_subscriber_client.acknowledge.assert_called_once_with(subscription=subscription_name, ack_ids=["ack_123"])
...
def test_acknowledge_messages(mock_get_subscriber_client):
    ...
    result = message_tool.acknowledge_messages(subscription_name, ack_ids, mock_credentials, tool_settings)

    assert result["status"] == "SUCCESS"
    mock_get_subscriber_client.assert_called_once()
    mock_subscriber_client.acknowledge.assert_called_once_with(subscription=subscription_name, ack_ids=ack_ids)
