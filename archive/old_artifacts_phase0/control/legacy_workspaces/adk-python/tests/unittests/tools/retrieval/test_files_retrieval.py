...
import pytest
from google.adk.tools.retrieval.files_retrieval import FilesRetrieval, _get_default_embedding_model
...
class TestFilesRetrieval:
    ...
    @mock.patch("google.adk.tools.retrieval.files_retrieval._get_default_embedding_model")
    def test_files_retrieval_uses_default_embedding(self, mock_get_default_embedding, tmp_path):
    ...
    def test_get_default_embedding_model_import_error(self):
        ...
        def mock_import(name, *args, **kwargs):
            if name == "llama_index.embeddings.google_genai":
                raise ImportError("No module named 'llama_index.embeddings.google_genai'")
        ...
        with mock.patch("builtins.__import__", side_effect=mock_import):
            ...
            assert "llama-index-embeddings-google-genai package not found" in str(exc_info.value)
            assert "pip install llama-index-embeddings-google-genai" in str(exc_info.value)
    ...
    def test_get_default_embedding_model_success(self):
        ...
        with mock.patch.dict("sys.modules", {"llama_index.embeddings.google_genai": mock_module}):
            result = _get_default_embedding_model()

            mock_module.GoogleGenAIEmbedding.assert_called_once_with(model_name="text-embedding-004")
    ...
    def test_backward_compatibility(self, tmp_path):
        ...
        with mock.patch("google.adk.tools.retrieval.files_retrieval._get_default_embedding_model") as mock_get_default:
