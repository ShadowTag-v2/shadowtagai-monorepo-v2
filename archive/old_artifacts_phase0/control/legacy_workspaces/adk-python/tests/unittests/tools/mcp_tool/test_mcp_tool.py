...
import pytest
from google.adk.auth.auth_credential import AuthCredential, AuthCredentialTypes, HttpAuth, HttpCredentials, OAuth2Auth, ServiceAccount

# Skip all tests in this module if Python version is less than 3.10
pytestmark = pytest.mark.skipif(sys.version_info < (3, 10), reason="MCP tool requires Python 3.10+")
...
class TestMCPTool:
    ...
    def setup_method(self):
        ...
        self.mock_session_manager.create_session = AsyncMock(return_value=self.mock_session)
    ...
    def test_init_with_auth(self):
        ...
        assert tool._credentials_manager._auth_config.raw_auth_credential == auth_credential
    ...
    def test_get_declaration_with_json_schema_for_func_decl_enabled(self, monkeypatch):
    ...
    def test_get_declaration_with_output_schema_and_json_schema_for_func_decl_enabled(self, monkeypatch):
    ...
    def test_get_declaration_with_empty_output_schema_and_json_schema_for_func_decl_enabled(self, monkeypatch):
    ...
    async def test_run_async_impl_no_auth(self):
        ...
        mcp_response = CallToolResult(content=[TextContent(type="text", text="success")])
        self.mock_session.call_tool = AsyncMock(return_value=mcp_response)

        tool_context = Mock(spec=ToolContext)
        args = {"param1": "test_value"}

        result = await tool._run_async_impl(args=args, tool_context=tool_context, credential=None)

        # Verify the result matches the model_dump output
        assert result == mcp_response.model_dump(exclude_none=True, mode="json")
        self.mock_session_manager.create_session.assert_called_once_with(headers=None)
        # Fix: call_tool uses 'arguments' parameter, not positional args
        self.mock_session.call_tool.assert_called_once_with("test_tool", arguments=args)
    ...
    async def test_run_async_impl_with_oauth2(self):
        ...
        credential = AuthCredential(auth_type=AuthCredentialTypes.OAUTH2, oauth2=oauth2_auth)
        ...
        result = await tool._run_async_impl(args=args, tool_context=tool_context, credential=credential)
    ...
    async def test_get_headers_http_bearer(self):
        ...
        http_auth = HttpAuth(scheme="bearer", credentials=HttpCredentials(token="bearer_token"))
        credential = AuthCredential(auth_type=AuthCredentialTypes.HTTP, http=http_auth)
    ...
    async def test_get_headers_api_key_with_valid_header_scheme(self):
        ...
        auth_credential = AuthCredential(auth_type=AuthCredentialTypes.API_KEY, api_key="my_api_key")
    ...
    async def test_get_headers_api_key_without_auth_config_raises_error(self):
        ...
        credential = AuthCredential(auth_type=AuthCredentialTypes.API_KEY, api_key="my_api_key")
        tool_context = Mock(spec=ToolContext)
    ...
    async def test_run_async_impl_with_api_key_header_auth(self):
        ...
        auth_credential = AuthCredential(auth_type=AuthCredentialTypes.API_KEY, api_key="test_service_key")
        ...
        # Mock the session response - must return CallToolResult
        mcp_response = CallToolResult(content=[TextContent(type="text", text="authenticated_success")])
        self.mock_session.call_tool = AsyncMock(return_value=mcp_response)

        tool_context = Mock(spec=ToolContext)
        args = {"param1": "test_value"}

        result = await tool._run_async_impl(args=args, tool_context=tool_context, credential=auth_credential)
    ...
    async def test_get_headers_http_custom_scheme(self):
        ...
        http_auth = HttpAuth(scheme="custom", credentials=HttpCredentials(token="custom_token"))
        credential = AuthCredential(auth_type=AuthCredentialTypes.HTTP, http=http_auth)
    ...
    async def test_get_headers_api_key_error_logging(self):
        ...
        with patch("google.adk.tools.mcp_tool.mcp_tool.logger") as mock_logger:
            ...
            assert "McpTool only supports header-based API key authentication" in logged_message
    ...
    async def test_run_async_require_confirmation_true_confirmed(self):
        ...
        with patch(
            ...
            mock_super_run_async.assert_called_once_with(args=args, tool_context=tool_context)
    ...
    async def test_run_async_impl_with_header_provider_no_auth(self):
        ...
        self.mock_session_manager.create_session.assert_called_once_with(headers=expected_headers)
        self.mock_session.call_tool.assert_called_once_with("test_tool", arguments=args)
