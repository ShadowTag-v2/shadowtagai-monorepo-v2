...
class TestSessionContext:
    ...
    async def test_async_context_manager(self):
        ...
        with patch("google.adk.tools.mcp_tool.session_context.ClientSession") as mock_session_class:
            ...
            async with SessionContext(mock_client, timeout=5.0, sse_read_timeout=None) as session:
                assert session == mock_session
    ...
    async def test_stdio_client_with_read_timeout(self):
        ...
        mock_client = MockClient()
        session_context = SessionContext(mock_client, timeout=5.0, sse_read_timeout=None, is_stdio=True)
    ...
    async def test_non_stdio_client_without_read_timeout(self):
        ...
        mock_client = MockClient()
        session_context = SessionContext(mock_client, timeout=5.0, sse_read_timeout=None, is_stdio=False)
    ...
    async def test_sse_read_timeout_passed_to_client_session(self):
        ...
        mock_client = MockClient()
        session_context = SessionContext(mock_client, timeout=5.0, sse_read_timeout=300.0, is_stdio=False)
    ...
    async def test_close_before_start_ends(self):
        ...
        assert isinstance(start_task.exception(), ConnectionError) and "task cancelled" in str(start_task.exception())
