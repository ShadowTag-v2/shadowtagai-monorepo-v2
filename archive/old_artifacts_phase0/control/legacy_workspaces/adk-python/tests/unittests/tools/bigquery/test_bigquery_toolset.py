...
async def test_bigquery_toolset_tools_default():
    ...
    toolset = BigQueryToolset(credentials_config=credentials_config, bigquery_tool_config=None)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(["list_dataset_ids", "get_dataset_info"], id="dataset-metadata"),
...
async def test_bigquery_toolset_tools_selective(selected_tools):
    ...
    """
    credentials_config = BigQueryCredentialsConfig(client_id="abc", client_secret="def")
    toolset = BigQueryToolset(credentials_config=credentials_config, tool_filter=selected_tools)
    tools = await toolset.get_tools()
    assert tools is not None
