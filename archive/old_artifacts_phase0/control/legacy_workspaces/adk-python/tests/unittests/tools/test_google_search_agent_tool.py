...
async def test_grounding_metadata_is_stored_in_state_during_invocation():
    ...
    tool_agent_model = testing_utils.MockModel.create(
        responses=[
            LlmResponse(
                content=types.Content(parts=[Part.from_text(text="response from tool")]),
    ...
    tool_result = await agent_tool.run_async(args=function_call_no_schema.function_call.args, tool_context=tool_context)

    # Verify the tool result
    assert tool_result == "response from tool"

    # Verify grounding_metadata is stored in the state
    assert tool_context.state["temp:_adk_grounding_metadata"] == (grounding_metadata)
