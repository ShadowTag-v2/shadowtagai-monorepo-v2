...
from google.adk.tools.openapi_tool.openapi_spec_parser.openapi_spec_parser import OperationEndpoint, ParsedOperation


@pytest.fixture
def mock_integration_client():
    with mock.patch("google.adk.tools.application_integration_tool.application_integration_toolset.IntegrationClient") as mock_client:
        yield mock_client


@pytest.fixture
def mock_connections_client():
    with mock.patch("google.adk.tools.application_integration_tool.application_integration_toolset.ConnectionsClient") as mock_client:
        yield mock_client


@pytest.fixture
def mock_openapi_toolset():
    with mock.patch("google.adk.tools.application_integration_tool.application_integration_toolset.OpenAPIToolset") as mock_toolset:
...
def mock_openapi_entity_spec_parser():
    with mock.patch("google.adk.tools.application_integration_tool.application_integration_toolset.OpenApiSpecParser") as mock_spec_parser:
        mock_openapi_spec_parser_instance = get_mocked_parsed_operation("list_issues", {"x-entity": "Issues", "x-operation": "LIST_ENTITIES"})
...
async def test_initialization_with_integration_and_empty_trigger_list(
    ...
    integration_name = "test-integration"
    toolset = ApplicationIntegrationToolset(project, location, integration=integration_name)
    mock_integration_client.assert_called_once_with(project, location, integration_name, None, None, None, None, None)
    mock_integration_client.return_value.get_openapi_spec_for_integration.assert_called_once()
    mock_connections_client.assert_not_called()
    mock_openapi_toolset_with_multiple_tools_and_no_tools.assert_called_once()
    tools = await toolset.get_tools()
...
async def test_initialization_with_connection_and_entity_operations(
    ...
    mock_connections_client.return_value.get_connection_details.return_value = connection_details
...
async def test_initialization_with_connection_and_actions(
    ...
    mock_integration_client.assert_called_once_with(project, location, None, None, connection_name, None, actions_list, None)
    mock_connections_client.assert_called_once_with(project, location, connection_name, None)
    mock_connections_client.return_value.get_connection_details.assert_called_once()
    mock_integration_client.return_value.get_openapi_spec_for_connection.assert_called_once_with(tool_name, tool_instructions)
...
def test_initialization_without_required_params(project, location):
    with pytest.raises(
        ValueError,
        match=("Invalid request, Either integration or \\(connection and \\(entity_operations or actions\\)\\) should be provided."),
    ):
...
def test_initialization_with_service_account_credentials(project, location, mock_integration_client, mock_openapi_toolset):
    ...
    assert kwargs["auth_credential"].service_account.service_account_credential.client_email == "test@example.com"
...
@pytest.mark.asyncio
async def test_get_tools(project, location, mock_integration_client, mock_openapi_toolset):
    integration_name = "test-integration"
    triggers = ["test-trigger"]
    toolset = ApplicationIntegrationToolset(project, location, integration=integration_name, triggers=triggers)
    tools = await toolset.get_tools()
...
async def test_init_with_connection_and_custom_auth(
    ...
    tool_name = "My Actions Tool"
    tool_instructions = "Perform actions using this tool."
    mock_connections_client.return_value.get_connection_details.return_value = connection_details_auth_override_enabled
