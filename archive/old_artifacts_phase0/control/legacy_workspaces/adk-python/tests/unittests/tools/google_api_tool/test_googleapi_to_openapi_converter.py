...
def calendar_api_spec():
    ...
    return {
        ...
        "auth": {
            "oauth2": {
                "scopes": {
                    "https://www.googleapis.com/auth/calendar": {"description": "Full access to Google Calendar"},
                    "https://www.googleapis.com/auth/calendar.readonly": {"description": "Read-only access to Google Calendar"},
                }
            }
        },
        "schemas": {
...
class TestGoogleApiToOpenApiConverter:
    ...
    def test_fetch_google_api_spec(self, converter_with_patched_build, calendar_api_spec):
    ...
    def test_fetch_google_api_spec_error(self, monkeypatch, converter):
        ...
        mock_build = MagicMock(side_effect=HttpError(resp=MagicMock(status=404), content=b"Not Found"))
        monkeypatch.setattr(
            "google.adk.tools.google_api_tool.googleapi_to_openapi_converter.build",
            mock_build,
        )
    ...
    def test_convert_security_schemes(self, prepared_converter):
        ...
        # Verify the results
        security_schemes = prepared_converter._openapi_spec["components"]["securitySchemes"]
    ...
    def test_convert_schema_object(self, converter, schema_def, expected_type, expected_attrs):
    ...
    def test_convert_parameter_schema(self, converter, param_data, expected_result):
    ...
    def test_convert_methods(self, prepared_converter, calendar_api_spec):
        ...
        assert post_method["requestBody"]["content"]["application/json"]["schema"]["$ref"] == "#/components/schemas/Calendar"

        # Check response
        assert post_method["responses"]["200"]["content"]["application/json"]["schema"]["$ref"] == "#/components/schemas/Calendar"
    ...
    def test_integration_calendar_api(self, converter_with_patched_build):
        ...
        assert openapi_spec["servers"][0]["url"] == "https://www.googleapis.com/calendar/v3"
