...
import pytest
from google.adk.tools import bash_tool, tool_context
...
def workspace(tmp_path):
    ...
    scripts.mkdir()
    (scripts / "extract_form_structure.py").write_text("import sys; print(f'extracting from {sys.argv[1]}')")
    (scripts / "fill_pdf_form_with_annotations.py").write_text("print('filling form')")
...
class TestValidateCommand:
    ...
    def test_restricted_policy_blocks_others(self):
        ...
        assert "Permitted prefixes are: ls, cat" in bash_tool._validate_command("tree", policy)


class TestExecuteBashTool:
    @pytest.mark.asyncio
    async def test_requests_confirmation(self, workspace, tool_context_no_confirmation):
    ...
    async def test_rejected(self, workspace, tool_context_rejected):
        tool = bash_tool.ExecuteBashTool(workspace=workspace)
        result = await tool.run_async(args={"command": "ls"}, tool_context=tool_context_rejected)
        assert result == {"error": "This tool call is rejected."}

    @pytest.mark.asyncio
    async def test_executes_when_confirmed(self, workspace, tool_context_confirmed):
        tool = bash_tool.ExecuteBashTool(workspace=workspace)
        result = await tool.run_async(
            args={"command": "ls"},
    ...
    async def test_python_script(self, workspace, tool_context_confirmed):
        tool = bash_tool.ExecuteBashTool(workspace=workspace)
        result = await tool.run_async(
            args={"command": "python3 pdf/scripts/extract_form_structure.py test.pdf"},
            tool_context=tool_context_confirmed,
        )
    ...
    @pytest.mark.asyncio
    async def test_blocks_disallowed_by_policy(self, workspace, tool_context_no_confirmation):
