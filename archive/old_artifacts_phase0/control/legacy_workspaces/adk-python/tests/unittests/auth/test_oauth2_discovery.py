...
import httpx
import pytest
from google.adk.auth.oauth2_discovery import AuthorizationServerMetadata, OAuth2DiscoveryManager, ProtectedResourceMetadata
...
class TestOAuth2Discovery:
    ...
    def mock_invalid_json_response(self):
        ...
        response = Mock()
        response.json.side_effect = json.decoder.JSONDecodeError("Invalid JSON", "invalid_json", 0)
        return response
    ...
    async def test_discover_auth_server_metadata_failed(
        ...
        result = await discovery_manager.discover_auth_server_metadata("https://auth.example.com")
        assert not result
    ...
    async def test_discover_auth_server_metadata_with_path(
        ...
        discovery_manager = OAuth2DiscoveryManager()
        result = await discovery_manager.discover_auth_server_metadata("https://auth.example.com/oauth")
        assert result == auth_server_metadata
        mock_get.assert_has_calls(
            [
                call(
    ...
    async def test_discover_auth_server_metadata_discard_mismatched_issuer(
        ...
        bad_auth_server_metadata = auth_server_metadata.model_copy(update={"issuer": "https://bad.example.com"})
        mock_get.side_effect = [
    ...
    async def test_discover_resource_metadata_failed(
        ...
        discovery_manager = OAuth2DiscoveryManager()
        result = await discovery_manager.discover_resource_metadata("https://resource.example.com")
        assert not result
    ...
    @patch("httpx.AsyncClient.get")
    @pytest.mark.asyncio
    async def test_discover_resource_metadata_without_path(self, mock_get, resource_metadata):
    ...
    @patch("httpx.AsyncClient.get")
    @pytest.mark.asyncio
    async def test_discover_resource_metadata_with_path(self, mock_get, resource_metadata):
        ...
        mock_get.return_value = self.mock_success_response(resource_metadata)
        discovery_manager = OAuth2DiscoveryManager()
        result = await discovery_manager.discover_resource_metadata("https://resource.example.com/tenant1")
        assert result == resource_metadata
        mock_get.assert_called_once_with(
