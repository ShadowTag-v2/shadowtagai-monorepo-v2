...
class MockCredentialExchanger(BaseCredentialExchanger):
    ...
    def __init__(self, exchange_result: AuthCredential | None = None):
        self.exchange_result = exchange_result or AuthCredential(auth_type=AuthCredentialTypes.HTTP)
...
class TestCredentialExchangerRegistry:
    ...
    def test_register_multiple_exchangers(self):
        ...
        registry.register(AuthCredentialTypes.SERVICE_ACCOUNT, service_account_exchanger)

        # Verify all exchangers were registered correctly
        assert registry.get_exchanger(AuthCredentialTypes.API_KEY) is api_key_exchanger
        assert registry.get_exchanger(AuthCredentialTypes.OAUTH2) is oauth2_exchanger
    ...
    def test_register_overwrites_existing_exchanger(self):
        ...
        assert registry.get_exchanger(AuthCredentialTypes.API_KEY) is first_exchanger

        # Register second exchanger for the same type
        registry.register(AuthCredentialTypes.API_KEY, second_exchanger)
        assert registry.get_exchanger(AuthCredentialTypes.API_KEY) is second_exchanger
        assert registry.get_exchanger(AuthCredentialTypes.API_KEY) is not first_exchanger
    ...
    def test_registry_isolation(self):
        ...
        assert registry1.get_exchanger(AuthCredentialTypes.API_KEY) is not exchanger2
        assert registry2.get_exchanger(AuthCredentialTypes.API_KEY) is not exchanger1
