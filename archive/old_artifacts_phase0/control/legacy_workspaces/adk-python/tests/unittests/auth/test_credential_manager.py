...
import pytest
from fastapi.openapi.models import OAuth2, OAuthFlowAuthorizationCode, OAuthFlowImplicit, OAuthFlows
from google.adk.auth.auth_credential import AuthCredential, AuthCredentialTypes, OAuth2Auth, ServiceAccount, ServiceAccountCredential
from google.adk.auth.auth_schemes import AuthScheme, AuthSchemeType, ExtendedOAuth2
from google.adk.auth.auth_tool import AuthConfig
from google.adk.auth.credential_manager import CredentialManager, ServiceAccountCredentialExchanger
...
class TestCredentialManager:
    ...
    async def test_load_auth_credentials_success(self):
        ...
        manager._exchange_credential = AsyncMock(return_value=(mock_credential, False))
        manager._refresh_credential = AsyncMock(return_value=(mock_credential, False))
        ...
        manager._save_credential.assert_called_once_with(callback_context, mock_credential)
    ...
    async def test_load_existing_credential_with_credential_service(self):
        ...
        manager = CredentialManager(auth_config)
        manager._load_from_credential_service = AsyncMock(return_value=mock_credential)

        result = await manager._load_existing_credential(callback_context)

        manager._load_from_credential_service.assert_called_once_with(callback_context)
        assert result == mock_credential
    ...
    async def test_refresh_credential_oauth2(self):
        ...
        mock_refresher.is_refresh_needed.assert_called_once_with(mock_credential, auth_config.auth_scheme)
        mock_refresher.refresh.assert_called_once_with(mock_credential, auth_config.auth_scheme)
        assert result == mock_credential
    ...
    @pytest.mark.asyncio
    async def test_validate_credential_oauth2_missing_scheme_info(self, extended_oauth2_scheme):
    ...
    @pytest.mark.asyncio
    async def test_exchange_credentials_service_account(self, service_account_credential, oauth2_auth_scheme):
        ...
        with patch.object(
            ...
            result, was_exchanged = await manager._exchange_credential(service_account_credential)

            mock_exchange_credential.assert_called_once_with(ANY, oauth2_auth_scheme, service_account_credential)
    ...
    async def test_exchange_credential_no_exchanger(self):
        ...
        with patch.object(
            ...
            result, was_exchanged = await manager._exchange_credential(mock_credential)
    ...
    def implicit_oauth2_scheme(self):
        ...
        return OAuth2(flows=OAuthFlows(implicit=OAuthFlowImplicit(authorizationUrl="https://auth.example.com/authorize")))

    @pytest.mark.asyncio
    async def test_populate_auth_scheme_success(self, auth_server_metadata, extended_oauth2_scheme):
        ...
        assert manager._auth_config.auth_scheme.flows.authorizationCode.authorizationUrl == "https://auth.example.com/authorize"
        assert manager._auth_config.auth_scheme.flows.authorizationCode.tokenUrl == "https://auth.example.com/token"
    ...
    async def test_populate_auth_scheme_fail(self, extended_oauth2_scheme):
        ...
        assert not manager._auth_config.auth_scheme.flows.authorizationCode.authorizationUrl
    ...
    def test_is_client_credentials_flow_oauth2_with_client_credentials(self):
        ...
        auth_scheme = OAuth2(flows=OAuthFlows(clientCredentials=OAuthFlowClientCredentials(tokenUrl="https://example.com/token")))
