...
from google.adk.flows.llm_flows.functions import REQUEST_EUC_FUNCTION_CALL_NAME, build_auth_request_event
...
class TestResolveToolsetAuth:
    ...
    @pytest.mark.asyncio
    async def test_no_tools_returns_no_events(self, mock_invocation_context, mock_agent):
        """Test that no events are yielded when agent has no tools."""
        mock_agent.tools = []

        events = []
        async for event in _resolve_toolset_auth(mock_invocation_context, mock_agent):
            events.append(event)
    ...
    @pytest.mark.asyncio
    async def test_toolset_without_auth_config_skipped(self, mock_invocation_context, mock_agent):
    ...
    @pytest.mark.asyncio
    async def test_toolset_with_credential_available_populates_config(self, mock_invocation_context, mock_agent):
        ...
        with patch("google.adk.flows.llm_flows.base_llm_flow.CredentialManager") as MockCredentialManager:
    ...
    @pytest.mark.asyncio
    async def test_toolset_without_credential_yields_auth_event(self, mock_invocation_context, mock_agent):
        ...
        assert fc.args["functionCallId"].startswith(TOOLSET_AUTH_CREDENTIAL_ID_PREFIX)
        assert "MockToolset" in fc.args["functionCallId"]

    @pytest.mark.asyncio
    async def test_multiple_toolsets_needing_auth(self, mock_invocation_context, mock_agent):
...
class TestAuthPreprocessorToolsetAuthSkip:
    ...
    def test_toolset_auth_prefix_skipped(self):
        ...
        assert toolset_function_call_id.startswith(TOOLSET_AUTH_CREDENTIAL_ID_PREFIX)

        # Regular tool auth function_call_id should NOT start with prefix
        regular_function_call_id = "call_123"
        assert not regular_function_call_id.startswith(TOOLSET_AUTH_CREDENTIAL_ID_PREFIX)
...
class TestCallbackContextGetAuthResponse:
    ...
    def test_get_auth_response_returns_none_when_no_response(self, mock_invocation_context):
    ...
    def test_get_auth_response_delegates_to_auth_handler(self, mock_invocation_context):
...
class TestBuildAuthRequestEvent:
    ...
    def test_multiple_auth_requests_create_multiple_parts(self, mock_invocation_context):
        ...
        function_call_ids = {p.function_call.args["functionCallId"] for p in event.content.parts}
    ...
    def test_custom_author_overrides_default(self, mock_invocation_context):
        ...
        event = build_auth_request_event(mock_invocation_context, auth_requests, author="custom-author")
    ...
    def test_role_is_set_in_content(self, mock_invocation_context):
        ...
        event = build_auth_request_event(mock_invocation_context, auth_requests, role="model")
