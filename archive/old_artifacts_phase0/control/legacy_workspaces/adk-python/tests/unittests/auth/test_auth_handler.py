...
from fastapi.openapi.models import APIKey, APIKeyIn, OAuth2, OAuthFlowAuthorizationCode, OAuthFlows
...
@pytest.fixture
def auth_config_with_exchanged(oauth2_auth_scheme, oauth2_credentials, oauth2_credentials_with_auth_uri):
...
@pytest.fixture
def auth_config_with_auth_code(oauth2_auth_scheme, oauth2_credentials, oauth2_credentials_with_auth_code):
...
class TestGenerateAuthUri:
    ...
    def test_generate_auth_uri_oauth2(self, auth_config):
        ...
        assert result.oauth2.auth_uri.startswith("https://example.com/oauth2/authorize")
    ...
    @patch("google.adk.auth.auth_handler.OAuth2Session", MockOAuth2Session)
    def test_generate_auth_uri_with_audience_and_prompt(self, openid_auth_scheme, oauth2_credentials):
    ...
    @patch("google.adk.auth.auth_handler.OAuth2Session", MockOAuth2Session)
    def test_generate_auth_uri_openid(self, openid_auth_scheme, oauth2_credentials):
...
class TestGenerateAuthRequest:
    ...
    def test_non_oauth_scheme(self):
        ...
        credential = AuthCredential(auth_type=AuthCredentialTypes.API_KEY, api_key="test_api_key")
    ...
    def test_with_existing_auth_uri(self, auth_config_with_exchanged):
        ...
        assert result.exchanged_auth_credential.oauth2.auth_uri == auth_config_with_exchanged.exchanged_auth_credential.oauth2.auth_uri
    ...
    def test_auth_uri_in_raw_credential(self, oauth2_auth_scheme, oauth2_credentials_with_auth_uri):
        """Test when auth_uri exists in raw_credential."""
        config = AuthConfig(
            auth_scheme=oauth2_auth_scheme,
            raw_auth_credential=oauth2_credentials_with_auth_uri,
            exchanged_auth_credential=oauth2_credentials_with_auth_uri.model_copy(deep=True),
        )
        handler = AuthHandler(config)
        result = handler.generate_auth_request()

        assert result.exchanged_auth_credential.oauth2.auth_uri == oauth2_credentials_with_auth_uri.oauth2.auth_uri
    ...
    def test_missing_client_credentials(self, oauth2_auth_scheme):
        ...
        with pytest.raises(ValueError, match="requires both client_id and client_secret"):
            handler.generate_auth_request()
...
class TestGetAuthResponse:
    ...
    def test_get_auth_response_exists(self, auth_config, oauth2_credentials_with_auth_uri):
...
class TestParseAndStoreAuthResponse:
    ...
    async def test_non_oauth_scheme(self, auth_config_with_exchanged):
        ...
        auth_config.auth_scheme = APIKey(**{"name": "test_api_key", "in": APIKeyIn.header})
        ...
        credential_key = auth_config.credential_key
        assert state["temp:" + credential_key] == auth_config.exchanged_auth_credential

    @patch("google.adk.auth.auth_handler.AuthHandler.exchange_auth_token")
    @pytest.mark.asyncio
    async def test_oauth_scheme(self, mock_exchange_token, auth_config_with_exchanged):
...
class TestExchangeAuthToken:
    ...
    @pytest.mark.asyncio
    async def test_token_exchange_not_supported(self, auth_config_with_auth_code, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_openid_missing_token_endpoint(self, openid_auth_scheme, oauth2_credentials_with_auth_code):
    ...
    @pytest.mark.asyncio
    async def test_oauth2_missing_token_url(self, oauth2_auth_scheme, oauth2_credentials_with_auth_code):
    ...
    @pytest.mark.asyncio
    async def test_credentials_with_token(self, auth_config, oauth2_credentials_with_token):
    ...
    @pytest.mark.asyncio
    async def test_successful_token_exchange(self, mock_oauth2_session, auth_config_with_auth_code):
