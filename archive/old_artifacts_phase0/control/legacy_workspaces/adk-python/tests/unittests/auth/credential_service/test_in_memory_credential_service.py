...
class TestInMemoryCredentialService:
    ...
    @pytest.mark.asyncio
    async def test_load_credential_not_found(self, credential_service, auth_config, callback_context):
        """Test loading a credential that doesn't exist returns None."""
        result = await credential_service.load_credential(auth_config, callback_context)
        assert result is None

    @pytest.mark.asyncio
    async def test_save_and_load_credential(self, credential_service, auth_config, callback_context):
    ...
    async def test_credentials_isolated_by_context(
        ...
        result = await credential_service.load_credential(auth_config, another_callback_context)
        assert result is None
    ...
    @pytest.mark.asyncio
    async def test_multiple_credentials_same_context(self, credential_service, callback_context, oauth2_auth_scheme):
        ...
        result1 = await credential_service.load_credential(auth_config1, callback_context)
        result2 = await credential_service.load_credential(auth_config2, callback_context)
    ...
    def test_get_bucket_for_current_context_creates_nested_structure(self, credential_service, callback_context):
        """Test that _get_bucket_for_current_context creates the proper nested structure."""
        storage = credential_service._get_bucket_for_current_context(callback_context)
    ...
    def test_get_bucket_for_current_context_reuses_existing(self, credential_service, callback_context):
        """Test that _get_bucket_for_current_context reuses existing structure."""
        # Create initial structure
        storage1 = credential_service._get_bucket_for_current_context(callback_context)
        storage1["test_key"] = "test_value"

        # Get storage again
        storage2 = credential_service._get_bucket_for_current_context(callback_context)

        # Verify it's the same storage instance
        assert storage1 is storage2
        assert storage2["test_key"] == "test_value"

    def test_get_storage_different_apps(self, credential_service, callback_context, another_callback_context):
        """Test that different apps get different storage instances."""
        storage1 = credential_service._get_bucket_for_current_context(callback_context)
        storage2 = credential_service._get_bucket_for_current_context(another_callback_context)
    ...
    @pytest.mark.asyncio
    async def test_same_user_different_apps(self, credential_service, auth_config):
    ...
    @pytest.mark.asyncio
    async def test_same_app_different_users(self, credential_service, auth_config):
