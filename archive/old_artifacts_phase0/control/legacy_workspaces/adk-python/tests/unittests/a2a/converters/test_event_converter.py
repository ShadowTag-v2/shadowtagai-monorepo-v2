...
try:
    from a2a.types import DataPart, Message, Role, Task, TaskState, TaskStatusUpdateEvent
...
class TestEventConverter:
    ...
    def test_create_artifact_id(self):
        ...
        result = _create_artifact_id(app_name, user_id, session_id, filename, version)
    ...
    def test_process_long_running_tool_marks_tool(self):
        ...
        with (
            ...
            patch("google.adk.a2a.converters.event_converter._get_adk_metadata_key") as mock_get_key,
        ):
    ...
    def test_convert_event_to_a2a_events_full_scenario(
        ...
        result = convert_event_to_a2a_events(self.mock_event, self.mock_invocation_context)

        # Verify error event - now called with task_id and context_id parameters
        mock_create_error.assert_called_once_with(self.mock_event, self.mock_invocation_context, None, None)

        # Verify running event - now called with task_id and context_id parameters
        mock_create_running.assert_called_once_with(mock_message, self.mock_invocation_context, self.mock_event, None, None)
    ...
    def test_convert_event_to_a2a_events_message_only(self, mock_convert_message):
        ...
        with patch("google.adk.a2a.converters.event_converter._create_status_update_event") as mock_create_running:
            mock_running_event = Mock()
            mock_create_running.return_value = mock_running_event
    ...
    def test_convert_event_to_a2a_events_exception_handling(self, mock_logger):
        ...
        with patch("google.adk.a2a.converters.event_converter.convert_event_to_a2a_message") as mock_convert_message:
            mock_convert_message.side_effect = Exception("Test exception")

            with pytest.raises(Exception):
                convert_event_to_a2a_events(self.mock_event, self.mock_invocation_context)
    ...
    def test_convert_event_to_a2a_events_with_task_id_and_context_id(self):
        ...
        with patch("google.adk.a2a.converters.event_converter.convert_event_to_a2a_message") as mock_convert_message:
            ...
            with patch("google.adk.a2a.converters.event_converter._create_status_update_event") as mock_create_running:
                ...
                result = convert_event_to_a2a_events(self.mock_event, self.mock_invocation_context, task_id, context_id)
    ...
    def test_create_status_update_event_with_auth_required_state(self):
        ...
        with patch("google.adk.a2a.converters.event_converter.datetime") as mock_datetime:
    ...
    def test_convert_event_to_a2a_message_with_multiple_parts_returned(self):
        ...
        self.mock_event.content = genai_types.Content(parts=[mock_genai_part], role="model")
...
class TestA2AToEventConverters:
    ...
    def test_convert_a2a_task_to_event_with_artifacts_priority(self):
        ...
        from a2a.types import Artifact, Part, TaskStatus, TextPart
        ...
        with patch("google.adk.a2a.converters.event_converter.convert_a2a_message_to_event") as mock_convert_message:
            mock_event = Mock(spec=Event)
            mock_convert_message.return_value = mock_event

            result = convert_a2a_task_to_event(mock_task, "test-author", self.mock_invocation_context)
    ...
    def test_convert_a2a_task_to_event_with_status_message(self):
        ...
        from a2a.types import Part, TaskStatus, TextPart
    ...
    def test_convert_a2a_message_to_event_empty_parts(self):
        ...
        result = convert_a2a_message_to_event(mock_message, "test-author", self.mock_invocation_context)
