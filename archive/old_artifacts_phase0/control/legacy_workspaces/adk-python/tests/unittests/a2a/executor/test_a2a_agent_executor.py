...
class TestA2aAgentExecutor:
    ...
    def setup_method(self):
        ...
        self.executor = A2aAgentExecutor(runner=self.mock_runner, config=self.mock_config)
    ...
    async def test_execute_success_new_task(self):
        ...
        self.mock_request_converter.assert_called_once_with(self.mock_context, self.mock_a2a_part_converter)
        ...
        submitted_event = self.mock_event_queue.enqueue_event.call_args_list[0][0][0]
        assert submitted_event.status.state == TaskState.submitted
        assert not submitted_event.final
        ...
        assert not working_event.final
    ...
    async def test_prepare_session_new_session(self):
        ...
        self.mock_runner.session_service.create_session = AsyncMock(return_value=mock_session)

        # Execute
        result = await self.executor._prepare_session(self.mock_context, run_args, self.mock_runner)
    ...
    async def test_handle_request_integration(self):
        ...
        with patch("google.adk.a2a.executor.a2a_agent_executor.TaskResultAggregator") as mock_aggregator_class:
            ...
            # Execute
            await self.executor._handle_request(self.mock_context, self.mock_event_queue)

            # Verify working event was enqueued
            working_events = [
                call[0][0]
                for call in self.mock_event_queue.enqueue_event.call_args_list
                if hasattr(call[0][0], "status") and call[0][0].status.state == TaskState.working
            ]
            ...
            final_events = [
                call[0][0] for call in self.mock_event_queue.enqueue_event.call_args_list if hasattr(call[0][0], "final") and call[0][0].final
            ]
    ...
    async def test_execute_with_exception_handling(self):
        ...
        self.mock_context.task_id = "test-task-id"
        self.mock_context.current_task = None  # Make sure it goes through submitted event creation
    ...
    async def test_handle_request_with_working_state_publishes_artifact_and_completed(
        ...
        with patch("google.adk.a2a.executor.a2a_agent_executor.TaskResultAggregator") as mock_aggregator_class:
            ...
            artifact_events = [
                call[0][0] for call in self.mock_event_queue.enqueue_event.call_args_list if hasattr(call[0][0], "artifact") and call[0][0].last_chunk
            ]
