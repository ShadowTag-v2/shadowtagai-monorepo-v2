...
from google.adk.evaluation.base_eval_service import EvaluateConfig, EvaluateRequest, InferenceConfig, InferenceRequest, InferenceResult
...
from google.adk.evaluation.eval_metrics import EvalMetricResult, EvalMetricResultPerInvocation, EvalStatus
...
def test_log_eval_summary(mocker):
    statuses = [EvalStatus.PASSED] * 3 + [EvalStatus.FAILED] * 2 + [EvalStatus.NOT_EVALUATED]
    expected_log = "Evaluation summary: 3 PASSED, 2 FAILED, 1 OTHER"

    eval_results = [mocker.MagicMock(spec=EvalCaseResult, final_eval_status=status) for status in statuses]
    mock_logger = mocker.patch("google.adk.optimization.local_eval_sampler.logger")
...
def test_local_eval_service_interface_init(mocker, config_kwargs, expected_attrs):
    ...
    config = LocalEvalSamplerConfig(eval_config=EvalConfig(), app_name="test_app", **config_kwargs)
...
async def test_evaluate_agent(mocker):
    ...
    mock_eval_service_cls = mocker.patch("google.adk.optimization.local_eval_sampler.LocalEvalService")
    ...
    results = await interface._evaluate_agent(mocker.MagicMock(spec=Agent), "train_set", ["t1"])
...
async def test_extract_eval_data(mocker):
    ...
    assert inv["eval_metric_results"] == [{"metric_name": "test_metric", "score": 0.85, "eval_status": "PASSED"}]
...
async def test_sample_and_score(mocker):
    ...
    mock_log_summary = mocker.patch("google.adk.optimization.local_eval_sampler._log_eval_summary")
    mock_extract_data = mocker.patch.object(interface, "_extract_eval_data", return_value={"t1": {}, "t2": {}})
