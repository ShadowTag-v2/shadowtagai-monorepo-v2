...
def span_exporter(monkeypatch: pytest.MonkeyPatch) -> InMemorySpanExporter:
    ...
    def do_replace(tracer):
        monkeypatch.setattr(tracer, "start_as_current_span", real_tracer.start_as_current_span)
...
async def test_tracer_start_as_current_span(
    ...
    def wrapped_firstiter(coro):
        nonlocal firstiter
        assert any(
            isinstance(referrer, Aclosing) or isinstance(indirect_referrer, Aclosing)
...
@pytest.mark.asyncio
async def test_exception_preserves_attributes(test_model: BaseLlm, span_exporter: InMemorySpanExporter):
