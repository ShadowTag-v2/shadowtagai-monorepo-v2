...
def mock_event_fixture():
    ...
    event_mock.model_dumps_json.return_value = '{"default_event_key": "default_event_value"}'
    return event_mock
...
async def test_trace_call_llm(monkeypatch, mock_span_fixture):
    ...
    monkeypatch.setattr("opentelemetry.trace.get_current_span", lambda: mock_span_fixture)
...
@pytest.mark.asyncio
async def test_trace_call_llm_with_binary_content(monkeypatch, mock_span_fixture):
    ...
    assert llm_request_json_str is not None, "Attribute 'gcp.vertex.agent.llm_request' was not set on the span."

    assert llm_request_json_str.count("<not serializable>") == 2


def test_trace_tool_call_with_scalar_response(monkeypatch, mock_span_fixture, mock_tool_fixture, mock_event_fixture):
    monkeypatch.setattr("opentelemetry.trace.get_current_span", lambda: mock_span_fixture)
...
def test_trace_tool_call_with_dict_response(monkeypatch, mock_span_fixture, mock_tool_fixture, mock_event_fixture):
    # Arrange
    monkeypatch.setattr("opentelemetry.trace.get_current_span", lambda: mock_span_fixture)
    ...
    expected_calls = [
        ...
        mock.call("gcp.vertex.agent.tool_call_args", json.dumps(test_args)),
        mock.call("gcp.vertex.agent.event_id", test_event_id),
        mock.call("gcp.vertex.agent.tool_response", json.dumps(dict_function_response)),
        mock.call("gcp.vertex.agent.llm_request", "{}"),
        mock.call("gcp.vertex.agent.llm_response", "{}"),
    ]
...
def test_trace_merged_tool_calls_sets_correct_attributes(monkeypatch, mock_span_fixture, mock_event_fixture):
    monkeypatch.setattr("opentelemetry.trace.get_current_span", lambda: mock_span_fixture)
...
@pytest.mark.asyncio
async def test_call_llm_disabling_request_response_content(monkeypatch, mock_span_fixture):
    ...
    # Assert
    assert not any(
        call_obj.args[0] == "gcp.vertex.agent.llm_request" and call_obj.args[1] != {} for call_obj in mock_span_fixture.set_attribute.call_args_list
    ), "Attribute 'gcp.vertex.agent.llm_request' was incorrectly set on the span."

    assert not any(
        call_obj.args[0] == "gcp.vertex.agent.llm_response" and call_obj.args[1] != {} for call_obj in mock_span_fixture.set_attribute.call_args_list
    ), "Attribute 'gcp.vertex.agent.llm_response' was incorrectly set on the span."
...
def test_trace_tool_call_disabling_request_response_content(
    ...
    # Assert
    assert not any(
        call_obj.args[0] == "gcp.vertex.agent.tool_call_args" and call_obj.args[1] != {}
        for call_obj in mock_span_fixture.set_attribute.call_args_list
    ), "Attribute 'gcp.vertex.agent.tool_call_args' was incorrectly set on the span."

    assert not any(
        call_obj.args[0] == "gcp.vertex.agent.tool_response" and call_obj.args[1] != {} for call_obj in mock_span_fixture.set_attribute.call_args_list
    ), "Attribute 'gcp.vertex.agent.tool_response' was incorrectly set on the span."
