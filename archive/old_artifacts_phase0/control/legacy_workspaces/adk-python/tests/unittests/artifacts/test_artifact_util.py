...
def test_get_session_scoped_artifact_uri():
    ...
    assert uri == "artifact://apps/app1/users/user1/sessions/session1/artifacts/file1/versions/123"


def test_get_user_scoped_artifact_uri():
    """Tests constructing a user-scoped artifact URI."""
    uri = artifact_util.get_artifact_uri(app_name="app2", user_id="user2", filename="file2", version=456)
...
def test_is_artifact_ref_true():
    ...
    artifact = types.Part(file_data=types.FileData(file_uri="artifact://apps/a/u/s/f/v/1", mime_type="text/plain"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        types.Part(file_data=types.FileData(file_uri="http://example.com", mime_type="text/plain")),
        types.Part(),
    ],
)
