...
from google.adk.evaluation.eval_case import IntermediateData, Invocation, InvocationEvent, InvocationEvents
...
def test_format_auto_rater_prompt_with_basic_invocation(
    ...
    invocation = Invocation(
        user_content=genai_types.Content(parts=[genai_types.Part(text="User input here.")]),
        final_response=genai_types.Content(parts=[genai_types.Part(text="Final agent response.")]),
    )
...
def test_format_auto_rater_prompt_with_app_details(
    ...
    tool = genai_types.Tool(function_declarations=[genai_types.FunctionDeclaration(name="test_func", description="A test function.")])
    ...
    invocation = Invocation(
        ...
        intermediate_data=InvocationEvents(invocation_events=[InvocationEvent(author="agent1", content=None)]),
    )
    prompt = evaluator.format_auto_rater_prompt(invocation, None)
...
def test_format_auto_rater_prompt_with_intermediate_data(
    ...
    tool_call = genai_types.FunctionCall(name="test_func", args={"arg1": "val1"}, id="call1")
    tool_response = genai_types.FunctionResponse(name="test_func", response={"result": "ok"}, id="call1")
    intermediate_data = IntermediateData(tool_uses=[tool_call], tool_responses=[tool_response])
    invocation = Invocation(
        user_content=genai_types.Content(parts=[genai_types.Part(text="User input here.")]),
        final_response=genai_types.Content(parts=[genai_types.Part(text="Final agent response.")]),
