...
from google.adk.evaluation.eval_metrics import EvalMetric, JudgeModelOptions, PrebuiltMetrics, RubricsBasedCriterion
from google.adk.evaluation.eval_rubrics import Rubric, RubricContent, RubricScore
from google.adk.evaluation.evaluator import EvalStatus, PerInvocationResult
...
class FakeRubricBasedEvaluator(RubricBasedEvaluator):
    ...
    def format_auto_rater_prompt(self, actual: Invocation, expected: Invocation) -> str:
...
def _create_per_invocation_result(
    ...
    return PerInvocationResult(
        actual_invocation=Invocation(user_content=genai_types.Content(parts=[genai_types.Part(text="part_1")])),
        expected_invocation=Invocation(user_content=genai_types.Content(parts=[genai_types.Part(text="part_2")])),
...
class TestMajorityVotePerInvocationResultsAggregator:
    def test_aggregate_per_invocation_samples_with_no_rubric_scores(
        ...
        result = MajorityVotePerInvocationResultsAggregator().aggregate(samples, threshold=0.5)
    ...
    def test_aggregate_per_invocation_samples_with_all_none_scores(
        ...
        samples = [
            _create_per_invocation_result([RubricScore(rubric_id="1", score=None, rationale="r1")]),
            _create_per_invocation_result([RubricScore(rubric_id="1", score=None, rationale="r2")]),
        ]
    ...
    def test_aggregate_per_invocation_samples_with_multiple_rubrics(
        ...
        rubric1_score = next((s for s in result.rubric_scores if s.rubric_id == "1"), None)
        rubric2_score = next((s for s in result.rubric_scores if s.rubric_id == "2"), None)
...
class TestMeanInvocationResultsSummarizer:
    ...
    def test_summarize_with_no_rubric_scores(
        ...
        result = MeanInvocationResultsSummarizer().summarize(invocations, threshold=0.5)
        assert result.overall_score is None
        assert result.overall_rubric_scores == []
    ...
    def test_summarize_with_single_invocation(
        ...
        rubric1_score = next(s for s in result.overall_rubric_scores if s.rubric_id == "1")
        rubric2_score = next(s for s in result.overall_rubric_scores if s.rubric_id == "2")
        assert rubric1_score.score == 1.0
        assert rubric2_score.score == 0.0
...
class TestRubricBasedEvaluator:
    ...
    def evaluator(self) -> FakeRubricBasedEvaluator:
        ...
        criterion = RubricsBasedCriterion(threshold=0.5, rubrics=rubrics, judge_model_options=judge_model_options)
        metric = EvalMetric(
    ...
    def test_convert_auto_rater_response_to_score_with_empty_response(
        ...
        response = LlmResponse(content=genai_types.Content(parts=[genai_types.Part(text="")]))
    ...
    def test_convert_auto_rater_response_to_score_with_malformed_response(
        ...
        response = LlmResponse(content=genai_types.Content(parts=[genai_types.Part(text="This is not a valid format.")]))
        auto_rater_score = evaluator.convert_auto_rater_response_to_score(response)
        assert auto_rater_score.score is None
        assert auto_rater_score.rubric_scores == []
    ...
    Verdict: no
    """
        response = LlmResponse(content=genai_types.Content(parts=[genai_types.Part(text=response_text)]))
        auto_rater_score = evaluator.convert_auto_rater_response_to_score(response)
