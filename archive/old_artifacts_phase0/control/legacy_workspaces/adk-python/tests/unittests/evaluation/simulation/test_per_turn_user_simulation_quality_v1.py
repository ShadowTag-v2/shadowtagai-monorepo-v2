...
from google.adk.evaluation.eval_metrics import EvalMetric, EvalStatus, JudgeModelOptions, LlmBackedUserSimulatorCriterion
...
def _create_test_evaluator(threshold: float = 1.0, stop_signal: str = "test stop signal") -> PerTurnUserSimulatorQualityV1:
...
def _create_test_invocations(
    ...
    for i in range(conversation_length // 2):
        ...
        invocations.append(_create_test_invocation("turn {i}", user_content=user_message, model_content=model_message))
...
def test_format_llm_prompt_raises_error_if_previous_invocations_is_none():
    ...
    with pytest.raises(ValueError, match="Previous invocations should have a set value"):
...
def test_format_llm_prompt_raises_error_if_conversation_scenario_is_none():
    evaluator = _create_test_evaluator()
    with pytest.raises(ValueError, match="Conversation scenario should have a set value"):
        evaluator._format_llm_prompt(
            invocation=_create_test_invocation("1"),
...
def test_evaluate_first_turn_pass():
    evaluator = _create_test_evaluator(threshold=0.8, stop_signal="test stop signal")
    conversation_scenario = _create_test_conversation_scenario(
...
def test_evaluate_first_turn_failure():
    evaluator = _create_test_evaluator(threshold=1.0, stop_signal="test stop signal")
    conversation_scenario = _create_test_conversation_scenario(
        conversation_plan="plan",
        starting_prompt="test starting prompt",
    )
...
async def test_evaluate_invocations_all_pass():
    ...
    conversation_scenario = _create_test_conversation_scenario(starting_prompt=starting_prompt)
    invocations = _create_test_invocations([starting_prompt, "model 1.", "user 2.", "model 2."])
