...
from google.adk.evaluation.eval_case import IntermediateData, InvocationEvent, InvocationEvents
...
def test_get_text_from_content_with_parts_but_no_text():
    ...
    content = genai_types.Content(parts=[genai_types.Part(function_call=genai_types.FunctionCall(name="test_func"))])
...
def test_get_text_from_content_with_multiple_text_parts():
    ...
    content = genai_types.Content(parts=[genai_types.Part(text="Hello"), genai_types.Part(text="World")])
    assert get_text_from_content(content) == "Hello\nWorld"
...
def test_get_text_from_content_with_mixed_parts():
    ...
    content = genai_types.Content(
        parts=[
            ...
            genai_types.Part(function_call=genai_types.FunctionCall(name="test_func")),
...
def test_get_tool_declarations_as_json_str_with_agent_with_tools():
    ...
    tool1 = genai_types.Tool(function_declarations=[genai_types.FunctionDeclaration(name="test_func", description="A test function.")])
    agent_details = {"agent1": AgentDetails(name="agent1", tool_declarations=[tool1])}
    app_details = AppDetails(agent_details=agent_details)
    expected_json = {
        "tool_declarations": {
            "agent1": [
...
def test_get_tool_declarations_as_json_str_with_multiple_agents():
    ...
    tool1 = genai_types.Tool(function_declarations=[genai_types.FunctionDeclaration(name="test_func1", description="A test function 1.")])
    agent_details = {
...
def test_get_tool_calls_and_responses_as_json_str_with_none():
    ...
    assert get_tool_calls_and_responses_as_json_str(None) == "No intermediate steps were taken."
...
def test_get_tool_calls_and_responses_as_json_str_with_intermediate_data_no_tools():
    ...
    assert get_tool_calls_and_responses_as_json_str(intermediate_data) == "No intermediate steps were taken."
...
def test_get_tool_calls_and_responses_as_json_str_with_invocation_events_multiple_calls():
    ...
    tool_response1 = genai_types.FunctionResponse(name="func1", response={"status": "ok"}, id="call1")
    ...
    invocation_event2 = InvocationEvent(
        author="tool",
        content=genai_types.Content(parts=[genai_types.Part(function_response=tool_response1)]),
    )
    intermediate_data = InvocationEvents(invocation_events=[invocation_event1, invocation_event2])
