...
from google.adk.evaluation import conversation_scenarios, eval_case
...
_TEST_CONVERSATION_SCENARIO = conversation_scenarios.ConversationScenario(starting_prompt="Hello!", conversation_plan="test plan")
...
class TestUserSimulatorProvider:
    ...
    def test_provide_llm_backed_user_simulator(self, mocker):
        ...
        provider = user_simulator_provider.UserSimulatorProvider(user_simulator_config=llm_config)
        simulator = provider.provide(test_eval_case)
        assert isinstance(simulator, LlmBackedUserSimulator)
        assert simulator._conversation_scenario == _TEST_CONVERSATION_SCENARIO
