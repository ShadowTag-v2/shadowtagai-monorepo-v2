...
from .mock_gcs_utils import MockBucket, MockClient
...
def _get_test_eval_case_results():
    ...
    actual_invocation_1 = Invocation(
        ...
        user_content=genai_types.Content(parts=[genai_types.Part(text="input_1")]),
    )
    expected_invocation_1 = Invocation(
        invocation_id="expected_1",
        user_content=genai_types.Content(parts=[genai_types.Part(text="expected_input_1")]),
    )
    actual_invocation_2 = Invocation(
        invocation_id="actual_2",
        user_content=genai_types.Content(parts=[genai_types.Part(text="input_2")]),
    )
    expected_invocation_2 = Invocation(
        invocation_id="expected_2",
        user_content=genai_types.Content(parts=[genai_types.Part(text="expected_input_2")]),
    )
    ...
    return [
        EvalCaseResult(
            ...
            eval_metric_result_per_invocation=[eval_metric_result_per_invocation_1],
        ...
        EvalCaseResult(
            ...
            eval_metric_result_per_invocation=[eval_metric_result_per_invocation_2],
...
class TestGcsEvalSetResultsManager:
    ...
    def test_save_eval_set_result(self, gcs_eval_set_results_manager, mocker):
        ...
        blob_name = gcs_eval_set_results_manager._get_eval_set_result_blob_name(app_name, eval_set_result.eval_set_result_id)
        mock_write_eval_set_result = mocker.patch.object(
            gcs_eval_set_results_manager,
            "_write_eval_set_result",
        )
        gcs_eval_set_results_manager.save_eval_set_result(app_name, eval_set_id, eval_case_results)
    ...
    def test_get_eval_set_result(self, gcs_eval_set_results_manager, mocker):
        ...
        gcs_eval_set_results_manager.save_eval_set_result(app_name, eval_set_id, eval_case_results)
        eval_set_result = create_eval_set_result(app_name, eval_set_id, eval_case_results)
        retrieved_eval_set_result = gcs_eval_set_results_manager.get_eval_set_result(app_name, eval_set_result.eval_set_result_id)
    ...
    def test_list_eval_set_results(self, gcs_eval_set_results_manager, mocker):
        ...
        for eval_set_id in eval_set_ids:
            eval_case_results = _get_test_eval_case_results()
            gcs_eval_set_results_manager.save_eval_set_result(app_name, eval_set_id, eval_case_results)
        retrieved_eval_set_result_ids = gcs_eval_set_results_manager.list_eval_set_results(app_name)
