...
from google.adk.evaluation.eval_metrics import EvalMetric, HallucinationsCriterion, JudgeModelOptions
from google.adk.evaluation.evaluator import EvalStatus
from google.adk.evaluation.hallucinations_v1 import HallucinationsV1Evaluator, _parse_sentences, _parse_validation_results
...
def mock_llm_registry(mocker):
    ...
    MockLLMRegistry = mocker.patch("google.adk.evaluation.hallucinations_v1.LLMRegistry")
...
def hallucinations_metric(mock_llm_registry):
    ...
    eval_metric = EvalMetric(metric_name="hallucinations_v1", threshold=0.5, criterion=criterion)
...
class TestParseSentences:
    ...
    def test_parse_sentences_multiple_sentences(self):
        ...
        text_multiple_sentences = "<sentence>Sentence 1.</sentence><sentence>Sentence 2.</sentence>"
...
<sentence>This sentence won't be parsed because tag is misspelled</stenence>"""
        assert _parse_sentences(text_with_newlines) == ["This is a sentence with\n\n\nnewlines."]
...
class TestEvaluateNlResponse:
    ...
    def _create_genai_response(self, text, mocker):
        ...
        response_mock.content = genai_types.Content(parts=[genai_types.Part(text=text)])
        return response_mock

    @pytest.mark.asyncio
    async def test_evaluate_nl_response_unexpected_labels(self, hallucinations_metric, mocker):
        """Tests _evaluate_nl_response with unexpected labels."""
        metric = hallucinations_metric
        seg_response = self._create_genai_response("<sentence>sentence 1</sentence><sentence>sentence 2</sentence>", mocker)
...
"""
    ...
    @pytest.mark.asyncio
    async def test_evaluate_nl_response_missing_label(self, hallucinations_metric, mocker):
        """Tests _evaluate_nl_response with missing labels in validation results."""
        metric = hallucinations_metric
        seg_response = self._create_genai_response("<sentence>sentence 1</sentence>", mocker)
    ...
    app_details = AppDetails(
        agent_details={
            "root": AgentDetails(
                ...
                tool_declarations=[genai_types.Tool(function_declarations=[genai_types.FunctionDeclaration(name="tool1")])],
            ),
        },
    )
    user_content = genai_types.Content(parts=[genai_types.Part(text="User query.")])
    events = [
        InvocationEvent(
            author="root",
            content=genai_types.Content(parts=[genai_types.Part(function_call=genai_types.FunctionCall(id="1", name="tool1", args={}))]),
        ),
        InvocationEvent(
            author="root",
        ...
        InvocationEvent(
            ...
            content=genai_types.Content(
                parts=[
                    ...
                    genai_types.Part(function_call=genai_types.FunctionCall(id="2", name="tool1", args={})),
                ]
            ),
        ),
        InvocationEvent(
...
class TestCreateContext:
    ...
    def test_create_context_for_intermediate_step(self, hallucinations_metric, create_context_data):
        """Tests _create_context_for_step method."""
        app_details, events, invocation = create_context_data
        context = hallucinations_metric._create_context_for_step(app_details, invocation, events[:2])
...
[
  {
    ...
    def test_create_context_for_final_step(self, hallucinations_metric, create_context_data):
        """Tests _create_context_for_step method."""
        app_details, events, invocation = create_context_data
        context = hallucinations_metric._create_context_for_step(app_details, invocation, events)
        expected_context = R"""Developer instructions:
root:
Root agent instructions.
...
def agent_tree_data():
    ...
    app_details = AppDetails(
        agent_details={
            "root": AgentDetails(
                name="root",
                instructions="Root agent instructions.",
                tool_declarations=[genai_types.Tool(function_declarations=[genai_types.FunctionDeclaration(name="tool_root")])],
            ),
            "agent1": AgentDetails(
                name="agent1",
                instructions="Agent1 instructions.",
                tool_declarations=[genai_types.Tool(function_declarations=[genai_types.FunctionDeclaration(name="tool_agent1")])],
            ),
            "agent2": AgentDetails(
                name="agent2",
    ...
    user_content = genai_types.Content(parts=[genai_types.Part(text="User query for agent tree.")])
    events = [
        InvocationEvent(
            author="root",
            content=genai_types.Content(parts=[genai_types.Part(text="Hi, I am root.")]),
        ),
        InvocationEvent(
            author="root",
            content=genai_types.Content(parts=[genai_types.Part(function_call=genai_types.FunctionCall(name="tool_root", args={}))]),
        ),
        InvocationEvent(
            author="root",
            content=genai_types.Content(
        ...
        InvocationEvent(
            author="agent1",
            content=genai_types.Content(parts=[genai_types.Part(function_call=genai_types.FunctionCall(name="tool_agent1", args={"q": 1}))]),
        ),
        InvocationEvent(
            author="agent1",
            content=genai_types.Content(
                parts=[genai_types.Part(function_response=genai_types.FunctionResponse(name="tool_agent1", response={"r": 2}))]
            ),
        ),
        InvocationEvent(
            author="agent2",
            content=genai_types.Content(parts=[genai_types.Part(text="Agent2 response.")]),
        ),
    ]
    invocation = Invocation(
        app_details=app_details,
        user_content=user_content,
        intermediate_data=InvocationEvents(invocation_events=events),
        final_response=genai_types.Content(parts=[genai_types.Part(text="Final agent tree response.")]),
    )
...
class TestEvaluateInvocationsAgentTree:
    ...
    @pytest.mark.asyncio
    async def test_evaluate_invocations_multi_agents(self, hallucinations_metric, agent_tree_data, mocker):
...
"""
        ...
        async def mock_evaluate_nl_response(nl_response, context):
            if nl_response == "Hi, I am root.":
                ...
                return 1.0, json.dumps([{"sentence": "Hi, I am root.", "label": "supported"}])
            elif nl_response == "Agent2 response.":
                assert context.strip() == expected_context5.strip()
                return 0.5, json.dumps([{"sentence": "Agent2 response.", "label": "unsupported"}])
        ...
        result = await metric.evaluate_invocations([invocation], [expected_invocation])
    ...
    @pytest.mark.asyncio
    async def test_evaluate_invocations_agent_tree_skip_intermediate(self, mock_llm_registry, agent_tree_data, mocker):
...
def time_weather_data():
    ...
    app_details = AppDetails(
        agent_details={
            "root": AgentDetails(
                name="root",
                instructions=("You are an agent that can get the current time and weather."),
                tool_declarations=[
                    genai_types.Tool(
                        function_declarations=[
    ...
    user_content = genai_types.Content(parts=[genai_types.Part(text="Get the current time and weather of San Francisco.")])
    response1 = "The time in San Francisco is currently 10:30am PST. The date is September 21, 2025. I will now get the weather."
    response2 = "It is currently September 19, 2025, 10:30am PST in San Francisco. The weather is 65F with partly cloudy skies."
    events = [
        InvocationEvent(
            author="root",
            content=genai_types.Content(
    ...
    invocation = Invocation(
        ...
        user_content=user_content,
        intermediate_data=InvocationEvents(invocation_events=events),
        final_response=genai_types.Content(parts=[genai_types.Part(text=response2)]),
    )
...
class TestEvaluateInvocationsTimeWeather:
    ...
    @pytest.mark.asyncio
    async def test_evaluate_invocations_time_weather(self, hallucinations_metric, time_weather_data, mocker):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_evaluate_invocations_time_weather_skip_intermediate(self, mock_llm_registry, time_weather_data, mocker):
        """Tests evaluate_invocations with time/weather agent."""
...
async def test_evaluate_invocations_success_path(hallucinations_metric, mocker):
        ...
        intermediate_data=InvocationEvents(
            invocation_events=[
                ...
                InvocationEvent(
                    ...
                    content=genai_types.Content(
                        parts=[
                            genai_types.Part(text="Another intermediate NL response."),
                        ]
                    ),
                ),
            ]
        ),
        final_response=genai_types.Content(parts=[genai_types.Part(text="Final response.")]),
    )
    expected_invocation = Invocation(
        app_details=app_details,
        user_content=user_content,
    ...
    async def mock_evaluate_nl_response(nl_response, context):
        if nl_response == "Intermediate NL response.":
            return 1.0, json.dumps([{"sentence": "Intermediate NL response.", "label": "supported"}])
            ...
            return 0.0, json.dumps([{"sentence": "Final response.", "label": "contradictory"}])
        return None, "error"
    ...
    result = await metric.evaluate_invocations([actual_invocation], [expected_invocation])
    ...
    actual_invocation = Invocation(
        ...
        intermediate_data=InvocationEvents(
            invocation_events=[
                InvocationEvent(
                    author="root",
                    content=genai_types.Content(parts=[genai_types.Part(function_call=genai_types.FunctionCall(name="tool1", args={}))]),
                ),
            ]
        ),
        final_response=None,
...
@pytest.mark.asyncio
async def test_evaluate_all_invocations_not_evaluated(hallucinations_metric, mocker):
    metric = hallucinations_metric
    app_details = AppDetails(
        agent_details={
            "root": AgentDetails(
...
@pytest.mark.asyncio
async def test_evaluate_invocations_partial_failure(hallucinations_metric, mocker):
    metric = hallucinations_metric
    app_details = AppDetails(
        agent_details={
            "root": AgentDetails(
    ...
    async def mock_evaluate_nl_response(nl_response, context):
        if nl_response == "Intermediate NL response.":
            return 0.8, json.dumps([{"sentence": "Intermediate NL response.", "label": "supported"}])
        elif nl_response == "Final response.":
