...
from google.adk.evaluation.simulation.llm_backed_user_simulator import LlmBackedUserSimulator, LlmBackedUserSimulatorConfig
...
_INPUT_EVENTS = [
    Event(
        author="user",
        content=types.Content(parts=[types.Part(text="Can you help me?")], role="user"),
        invocation_id="inv1",
    ),
    Event(
        ...
        content=types.Content(
            parts=[
                ...
                types.Part(function_call=types.FunctionCall(name="get_user_name")),
                types.Part(
...
_INPUT_EVENTS_LONG = _INPUT_EVENTS + [
    Event(
        author="user",
        content=types.Content(parts=[types.Part(text="I need to book a flight.")], role="user"),
        invocation_id="inv2",
    ),
    Event(
        author="helpful_assistant",
...
_EXPECTED_REWRITTEN_DIALOGUE_LONG = (
    _EXPECTED_REWRITTEN_DIALOGUE
    + """
...
class TestHelperMethods:
    ...
    def test_convert_conversation_to_user_sim_pov(self):
        ...
        rewritten_dialogue = LlmBackedUserSimulator._summarize_conversation(_INPUT_EVENTS)
        assert rewritten_dialogue == _EXPECTED_REWRITTEN_DIALOGUE
        rewritten_dialogue = LlmBackedUserSimulator._summarize_conversation(_INPUT_EVENTS_LONG)
...
def mock_llm_agent(mocker):
    ...
    mock_llm_registry_cls = mocker.patch("google.adk.evaluation.simulation.llm_backed_user_simulator.LLMRegistry")
...
def conversation_scenario():
    ...
    return conversation_scenarios.ConversationScenario(starting_prompt="Hello", conversation_plan="test plan")
...
def simulator(mock_llm_agent, conversation_scenario):
    ...
    sim = LlmBackedUserSimulator(config=config, conversation_scenario=conversation_scenario)
...
class TestLlmBackedUserSimulator:
    ...
    @pytest.mark.asyncio
    async def test_get_llm_response_return_value(self, simulator, mock_llm_agent, mocker):
        ...
        mock_llm_agent.generate_content_async.return_value = to_async_iter([mock_llm_response])
        response = await simulator._get_llm_response(rewritten_dialogue="")
        assert response == "Hello world!"

    @pytest.mark.asyncio
    async def test_get_next_user_message_first_invocation(self, simulator, mock_llm_agent, conversation_scenario):
    ...
    async def test_turn_limit_reached(self, conversation_scenario):
        ...
        simulator = LlmBackedUserSimulator(config=config, conversation_scenario=conversation_scenario)
        simulator._invocation_count = 1

        next_user_message = await simulator.get_next_user_message(events=_INPUT_EVENTS)
    ...
    async def test_stop_signal_detected(self, simulator, mock_llm_agent, mocker):
        ...
        mock_llm_response = mocker.MagicMock()
        mock_llm_response.content = types.Content(parts=[types.Part(text="Thanks! Bye!</finished>")])
        mock_llm_agent.generate_content_async.return_value = to_async_iter([mock_llm_response])
    ...
    @pytest.mark.asyncio
    async def test_get_next_user_message_success(self, simulator, mock_llm_agent, mocker):
        """Tests get_next_user_message when the user message is generated successfully."""
        mock_llm_response = mocker.MagicMock()
        mock_llm_response.content = types.Content(parts=[types.Part(text="I need to book a flight.")])
        mock_llm_agent.generate_content_async.return_value = to_async_iter([mock_llm_response])

        next_user_message = await simulator.get_next_user_message(events=_INPUT_EVENTS)

        expected_user_message = types.Content(parts=[types.Part(text="I need to book a flight.")], role="user")
