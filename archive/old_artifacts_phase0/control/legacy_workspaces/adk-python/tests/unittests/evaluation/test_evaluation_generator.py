...
from google.adk.evaluation.simulation.user_simulator import NextUserMessage, UserSimulator
...
def _build_event(author: str, parts: list[types.Part], invocation_id: str) -> Event:
...
class TestConvertEventsToEvalInvocation:
    ...
    def test_multi_agent(
        ...
        events = [
            ...
            _build_event(
                "sub_agent_2",
                [types.Part(function_call=types.FunctionCall(name="tool4", args={}))],
                "inv1",
            ),
...
class TestGetAppDetailsByInvocationId:
    ...
    def test_get_app_details_by_invocation_id_empty(self, mocker):
        ...
        app_details = EvaluationGenerator._get_app_details_by_invocation_id([], mock_request_intercepter)
        assert app_details == {}
    ...
    def test_get_app_details_by_invocation_id_no_model_requests(self, mocker):
        ...
        app_details = EvaluationGenerator._get_app_details_by_invocation_id(events, mock_request_intercepter)
        assert app_details == {"inv1": AppDetails(agent_details={})}
        mock_request_intercepter.get_model_request.assert_called_once_with(events[1])
    ...
    def test_get_app_details_multiple_invocations_multiple_agents(self, mocker):
        ...
        def get_model_request_side_effect(event):
            ...
            if event.invocation_id == "inv1" and event.author == "agent1":
                mock_llm_request.config.system_instruction = "instruction1"
                mock_llm_request.config.tools = [types.Tool(function_declarations=[types.FunctionDeclaration(name="tool1")])]
        ...
        mock_request_intercepter.get_model_request.side_effect = get_model_request_side_effect
        ...
        events = [
            ...
            _build_event("agent1", [types.Part(text="Hi again from agent1")], "inv2"),  # no request
        ]
        app_details = EvaluationGenerator._get_app_details_by_invocation_id(events, mock_request_intercepter)
            ...
            "inv1": AppDetails(
                agent_details={
                    "agent1": AgentDetails(
                        name="agent1",
                        instructions="instruction1",
                        tool_declarations=[types.Tool(function_declarations=[types.FunctionDeclaration(name="tool1")])],
                    )
                }
            ),
            "inv2": AppDetails(
...
class TestGenerateInferencesForSingleUserInvocation:
    ...
    async def test_generate_inferences_with_mock_runner(self, mocker):
        ...
        events = [
            event
            async for event in EvaluationGenerator._generate_inferences_for_single_user_invocation(runner, "test_user", "test_session", user_content)
        ]
...
def mock_runner(mocker):
    ...
    mock_runner_cls = mocker.patch("google.adk.evaluation.evaluation_generator.Runner")
...
def mock_session_service(mocker):
    ...
    mock_session_service_cls = mocker.patch("google.adk.evaluation.evaluation_generator.InMemorySessionService")
...
class TestGenerateInferencesFromRootAgent:
    ...
    @pytest.mark.asyncio
    async def test_generates_inferences_with_user_simulator(self, mocker, mock_runner, mock_session_service):
        ...
        mock_user_sim.get_next_user_message = mocker.AsyncMock(side_effect=get_next_user_message_side_effect)
        ...
        mocker.patch("google.adk.evaluation.evaluation_generator.EvaluationGenerator._get_app_details_by_invocation_id")
        mocker.patch("google.adk.evaluation.evaluation_generator.EvaluationGenerator.convert_events_to_eval_invocations")

        # Each call to _generate_inferences_for_single_user_invocation will
        # yield one user and one agent event.
        async def mock_generate_inferences_side_effect(runner, user_id, session_id, user_content):
