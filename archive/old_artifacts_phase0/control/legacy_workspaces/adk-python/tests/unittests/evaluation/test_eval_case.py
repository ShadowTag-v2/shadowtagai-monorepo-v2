...
def test_get_all_tool_calls_with_intermediate_data():
    ...
    tool_call1 = genai_types.FunctionCall(name="search", args={"query": "weather"})
...
def test_get_all_tool_calls_with_invocation_events_no_tools():
    ...
    invocation_event = InvocationEvent(
        author="agent",
        content=genai_types.Content(parts=[genai_types.Part(text="Thinking...")], role="model"),
    )
...
def test_get_all_tool_calls_with_unsupported_type():
    ...
    with pytest.raises(ValueError, match="Unsupported type for intermediate_data"):
...
def test_get_all_tool_responses_with_invocation_events():
    ...
    tool_response1 = genai_types.FunctionResponse(name="search", response={"result": "weather is good"})
    tool_response2 = genai_types.FunctionResponse(name="lookup", response={"id": "123"})
    invocation_event1 = InvocationEvent(
        author="agent1",
        content=genai_types.Content(
...
def test_get_all_tool_calls_with_responses_with_intermediate_data_with_tool_calls():
    ...
    tool_call1 = genai_types.FunctionCall(name="search", args={"query": "weather"}, id="call1")
    tool_response1 = genai_types.FunctionResponse(name="search", response={"result": "sunny"}, id="call1")
    tool_call2 = genai_types.FunctionCall(name="lookup", args={"id": "123"}, id="call2")
    intermediate_data = IntermediateData(tool_uses=[tool_call1, tool_call2], tool_responses=[tool_response1])
    assert get_all_tool_calls_with_responses(intermediate_data) == [
...
def test_conversation_and_conversation_scenario_mutual_exclusion():
    ...
    test_conversation_scenario = ConversationScenario(starting_prompt="", conversation_plan="")

    with pytest.raises(
        ValueError,
        match=("Exactly one of conversation and conversation_scenario must be provided in an EvalCase."),
    ):
