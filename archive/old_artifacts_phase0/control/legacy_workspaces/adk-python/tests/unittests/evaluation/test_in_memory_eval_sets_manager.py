...
def test_add_eval_case_set_not_found(manager, app_name, eval_set_id, eval_case_id):
    eval_case = EvalCase(eval_id=eval_case_id, conversation=[])
    with pytest.raises(NotFoundError):
        manager.add_eval_case(app_name, eval_set_id, eval_case)


def test_add_eval_case_already_exists(manager, app_name, eval_set_id, eval_case_id):
    manager.create_eval_set(app_name, eval_set_id)
    eval_case = EvalCase(eval_id=eval_case_id, conversation=[])
    manager.add_eval_case(app_name, eval_set_id, eval_case)
    with pytest.raises(ValueError):
...
def test_get_eval_case_not_found(manager, app_name, eval_set_id):
    manager.create_eval_set(app_name, eval_set_id)
    retrieved_case = manager.get_eval_case(app_name, eval_set_id, "nonexistent_case")
    assert retrieved_case is None


def test_get_eval_case_set_not_found(manager, app_name, eval_case_id):
    retrieved_case = manager.get_eval_case(app_name, "nonexistent_set", eval_case_id)
    assert retrieved_case is None
...
def test_update_eval_case(manager, app_name, eval_set_id, eval_case_id):
    ...
    updated_eval_case = EvalCase(eval_id=eval_case_id, conversation=[], creation_timestamp=time.time())
    manager.update_eval_case(app_name, eval_set_id, updated_eval_case)

    retrieved_case = manager.get_eval_case(app_name, eval_set_id, eval_case_id)
    assert retrieved_case is not None
    assert retrieved_case.creation_timestamp != 0.0
    assert retrieved_case.creation_timestamp == updated_eval_case.creation_timestamp

    eval_set = manager.get_eval_set(app_name, eval_set_id)
    assert len(eval_set.eval_cases) == 1
    assert eval_set.eval_cases[0].creation_timestamp == updated_eval_case.creation_timestamp


def test_update_eval_case_not_found(manager, app_name, eval_set_id, eval_case_id):
    manager.create_eval_set(app_name, eval_set_id)
...
def test_delete_eval_case_not_found(manager, app_name, eval_set_id, eval_case_id):
    manager.create_eval_set(app_name, eval_set_id)
    with pytest.raises(NotFoundError):
        manager.delete_eval_case(app_name, eval_set_id, eval_case_id)
