...
from google.adk.evaluation.eval_metrics import EvalMetric, EvalMetricResult, Interval, MetricInfo, MetricValueInfo
...
@pytest.fixture
def eval_service(dummy_agent, mock_eval_sets_manager, mock_eval_set_results_manager):
    DEFAULT_METRIC_EVALUATOR_REGISTRY.register_evaluator(metric_info=FakeEvaluator.get_metric_info(), evaluator=FakeEvaluator)
...
async def test_perform_inference_success(
    ...
    eval_service._perform_inference_single_eval_item = mocker.AsyncMock(return_value=mock_inference_result)
    ...
    mock_eval_sets_manager.get_eval_set.assert_called_once_with(app_name="test_app", eval_set_id="test_eval_set")
...
@pytest.mark.asyncio
async def test_evaluate_success(eval_service, mock_eval_sets_manager, mock_eval_set_results_manager, mocker):
    invocation = Invocation(
        user_content=genai_types.Content(parts=[genai_types.Part(text="test user content.")]),
        final_response=genai_types.Content(parts=[genai_types.Part(text="test final response.")]),
    )
    inference_results = [
        InferenceResult(
            app_name="test_app",
...
@pytest.mark.asyncio
async def test_evaluate_single_inference_result(eval_service, mock_eval_sets_manager, mock_eval_set_results_manager, mocker):
    invocation = Invocation(
        user_content=genai_types.Content(parts=[genai_types.Part(text="test user content.")]),
        final_response=genai_types.Content(parts=[genai_types.Part(text="test final response.")]),
    )
    ...
    _, result = await eval_service._evaluate_single_inference_result(inference_result=inference_result, evaluate_config=evaluate_config)
    ...
    mock_eval_sets_manager.get_eval_case.assert_called_once_with(app_name="test_app", eval_set_id="test_eval_set", eval_case_id="case1")
    ...
    for i in range(3):
        ...
        assert invocation_result.expected_invocation == mock_eval_case.conversation[i]
...
@pytest.mark.asyncio
async def test_evaluate_single_inference_result_for_conversation_scenario(eval_service, mock_eval_sets_manager, mocker):
    ...
    eval_metric = EvalMetric(metric_name="fake_single_sided_metric", threshold=0.5)
    evaluate_config = EvaluateConfig(eval_metrics=[eval_metric], parallelism=1)
    ...
    assert len(result.overall_eval_metric_results) == 1
    assert result.overall_eval_metric_results[0].metric_name == "fake_single_sided_metric"
...
@pytest.mark.asyncio
async def test_evaluate_single_inference_result_for_conversation_scenario_with_unsupported_metric(eval_service, mock_eval_sets_manager, mocker):
    """To be removed once evaluation is implemented for conversation scenarios."""
    invocation = Invocation(
        user_content=genai_types.Content(parts=[genai_types.Part(text="test user content.")]),
        final_response=genai_types.Content(parts=[genai_types.Part(text="test final response.")]),
...
def test_generate_final_eval_status_doesn_t_throw_on(eval_service):
    ...
    for status in EvalStatus:
        eval_metric_result = EvalMetricResult(metric_name="metric1", threshold=0.5, eval_status=status)
        eval_service._generate_final_eval_status([eval_metric_result])


@pytest.mark.asyncio
@pytest.mark.skipif(sys.version_info < (3, 10), reason="MCP tool requires Python 3.10+")
async def test_mcp_stdio_agent_no_runtime_error(mocker):
    ...
    mock_responses = [genai_types.Content(parts=[genai_types.Part(text="Mocked response from test agent")])]
    ...
    try:
        ...
        test_eval_case = EvalCase(
            ...
            conversation=[
                Invocation(
                    user_content=genai_types.Content(parts=[genai_types.Part(text="List directory contents")]),
                )
            ],
        )
        ...
        try:
            ...
            if hasattr(e, "args") and len(e.args) > 0 and "cancel scope" in str(e.args[0]):
