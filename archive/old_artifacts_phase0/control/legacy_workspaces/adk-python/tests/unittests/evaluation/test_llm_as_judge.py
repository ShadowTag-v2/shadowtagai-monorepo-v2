...
from google.adk.evaluation.eval_metrics import EvalMetric, JudgeModelOptions, LlmAsAJudgeCriterion
from google.adk.evaluation.evaluator import EvalStatus, EvaluationResult, PerInvocationResult
from google.adk.evaluation.llm_as_judge import AutoRaterScore, LlmAsJudge
from google.adk.evaluation.llm_as_judge_utils import get_eval_status, get_text_from_content
from google.adk.models.llm_response import LlmResponse
from google.genai import types as genai_types


class MockLlmAsJudge(LlmAsJudge):
    def format_auto_rater_prompt(self, actual_invocation: Invocation, expected_invocation: Invocation) -> str:
...
def test_get_text_from_content():
    ...
    assert get_text_from_content(content) == "This is a test text.\nThis is another test text."
...
@pytest.mark.asyncio
async def test_evaluate_invocations_with_mock(mock_llm_as_judge, mock_judge_model, mocker):
    mock_llm_as_judge._judge_model = mock_judge_model

    mock_format_auto_rater_prompt = mocker.MagicMock(wraps=mock_llm_as_judge.format_auto_rater_prompt)
    mock_llm_as_judge.format_auto_rater_prompt = mock_format_auto_rater_prompt

    mock_convert_auto_rater_response_to_score = mocker.MagicMock(wraps=mock_llm_as_judge.convert_auto_rater_response_to_score)
    mock_llm_as_judge.convert_auto_rater_response_to_score = mock_convert_auto_rater_response_to_score

    mock_aggregate_per_invocation_samples = mocker.MagicMock(wraps=mock_llm_as_judge.aggregate_per_invocation_samples)
    mock_llm_as_judge.aggregate_per_invocation_samples = mock_aggregate_per_invocation_samples

    mock_aggregate_invocation_results = mocker.MagicMock(wraps=mock_llm_as_judge.aggregate_invocation_results)
    mock_llm_as_judge.aggregate_invocation_results = mock_aggregate_invocation_results
    ...
    result = await mock_llm_as_judge.evaluate_invocations(actual_invocations, expected_invocations)
