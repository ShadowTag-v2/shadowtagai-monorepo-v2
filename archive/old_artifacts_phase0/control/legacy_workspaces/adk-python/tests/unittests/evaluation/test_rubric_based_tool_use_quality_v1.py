...
def evaluator() -> RubricBasedToolUseV1Evaluator:
    ...
    rubrics = [
        Rubric(
            rubric_id="1",
            rubric_content=RubricContent(text_property="Did the agent use the correct tool?"),
        ),
        Rubric(
            rubric_id="2",
            rubric_content=RubricContent(text_property="Were the tool parameters correct?"),
        ),
    ]
...
def test_get_metric_info(evaluator: RubricBasedToolUseV1Evaluator):
    ...
    assert metric_info.metric_name == PrebuiltMetrics.RUBRIC_BASED_TOOL_USE_QUALITY_V1.value
