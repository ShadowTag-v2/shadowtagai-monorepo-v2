...
from google.adk.evaluation.eval_metrics import EvalMetric, PrebuiltMetrics, ToolTrajectoryCriterion
...
def test_get_metric_info():
    ...
    assert metric_info.metric_name == PrebuiltMetrics.TOOL_TRAJECTORY_AVG_SCORE.value
...
def test_evaluate_invocations_equal_tool_calls(evaluator: TrajectoryEvaluator):
    ...
    invocation = Invocation(user_content=_USER_CONTENT, intermediate_data=intermediate_data)
...
def test_evaluate_invocations_no_tool_calls(evaluator: TrajectoryEvaluator):
    ...
    invocation = Invocation(user_content=_USER_CONTENT, intermediate_data=IntermediateData())
    result = evaluator.evaluate_invocations([invocation], [invocation])
    assert result.overall_score == 1.0
    assert result.overall_eval_status == EvalStatus.PASSED
    assert result.per_invocation_results[0].score == 1.0
...
def test_evaluate_invocations_multiple_invocations(
    ...
    result = evaluator.evaluate_invocations([inv1_actual, inv2_actual], [inv1_expected, inv2_expected])
...
def test_evaluate_invocations_in_order_match_with_extra_tool_calls(
    ...
    actual_invocation = Invocation(
        user_content=_USER_CONTENT,
        intermediate_data=IntermediateData(tool_uses=[t1, t1_1, t2, t2_1, t3, t3_1]),
    )
    expected_invocation = Invocation(
        user_content=_USER_CONTENT,
        intermediate_data=IntermediateData(tool_uses=[t1, t2, t3]),
    )
    result = in_order_evaluator.evaluate_invocations([actual_invocation], [expected_invocation])
    assert result.overall_score == 1.0
    assert result.overall_eval_status == EvalStatus.PASSED
    assert result.per_invocation_results[0].score == 1.0
    assert result.per_invocation_results[0].eval_status == EvalStatus.PASSED
...
def test_evaluate_invocations_any_order_match_with_extra_tool_calls_different_order(
    ...
    actual_invocation = Invocation(
        user_content=_USER_CONTENT,
        intermediate_data=IntermediateData(tool_uses=[t2, t2_1, t1, t1_1, t3, t3_1]),
    )
    expected_invocation = Invocation(
        user_content=_USER_CONTENT,
        intermediate_data=IntermediateData(tool_uses=[t1, t2, t3]),
    )
    result = any_order_evaluator.evaluate_invocations([actual_invocation], [expected_invocation])
    assert result.overall_score == 1.0
    assert result.overall_eval_status == EvalStatus.PASSED
    assert result.per_invocation_results[0].score == 1.0
    assert result.per_invocation_results[0].eval_status == EvalStatus.PASSED
