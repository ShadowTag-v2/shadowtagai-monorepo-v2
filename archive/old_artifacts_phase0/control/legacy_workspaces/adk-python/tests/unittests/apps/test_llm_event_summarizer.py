...
class TestLlmEventSummarizer(unittest.IsolatedAsyncioTestCase):
    ...
    def _create_event(self, timestamp: float, text: str, author: str = "user") -> Event:
        return Event(
            timestamp=timestamp,
            author=author,
    ...
    async def test_maybe_compact_events_success(self):
        ...
        expected_prompt = self.compactor._DEFAULT_PROMPT_TEMPLATE.format(conversation_history=expected_conversation_history)
    ...
    def test_format_events_for_prompt(self):
        events = [
            ...
            Event(
                timestamp=7.0,
                author="model",
                content=Content(parts=[Part(function_call=FunctionCall(id="call_1", name="tool", args={}))]),
            ),
