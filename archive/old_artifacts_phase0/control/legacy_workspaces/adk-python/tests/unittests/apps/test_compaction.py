...
@pytest.mark.parametrize("env_variables", ["GOOGLE_AI", "VERTEX"], indirect=True)
class TestCompaction(unittest.IsolatedAsyncioTestCase):
    ...
    def _create_event(self, timestamp: float, invocation_id: str, text: str) -> Event:
        return Event(
            timestamp=timestamp,
    ...
    def _create_compacted_event(self, start_ts: float, end_ts: float, summary_text: str) -> Event:
    ...
    async def test_run_compaction_for_sliding_window_no_events(self):
        ...
        await _run_compaction_for_sliding_window(app, session, self.mock_session_service)
    ...
    async def test_run_compaction_for_sliding_window_first_compaction(self):
        ...
        mock_compacted_event = self._create_compacted_event(1.0, 4.0, "Summary inv1-inv4")
        self.mock_compactor.maybe_summarize_events.return_value = mock_compacted_event

        await _run_compaction_for_sliding_window(app, session, self.mock_session_service)

        # Expected events to compact: inv1, inv2, inv3, inv4
        compacted_events_arg = self.mock_compactor.maybe_summarize_events.call_args[1]["events"]
        self.assertEqual(
            [e.invocation_id for e in compacted_events_arg],
            ["inv1", "inv2", "inv3", "inv4"],
        )
        self.mock_session_service.append_event.assert_called_once_with(session=session, event=mock_compacted_event)
    ...
    async def test_run_compaction_for_sliding_window_with_overlap(self):
        ...
        mock_compacted_event = self._create_compacted_event(2.0, 5.0, "Summary inv2-inv5")
        self.mock_compactor.maybe_summarize_events.return_value = mock_compacted_event
