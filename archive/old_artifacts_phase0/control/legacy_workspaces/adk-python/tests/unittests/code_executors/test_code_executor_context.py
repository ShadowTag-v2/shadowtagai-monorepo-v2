...
def context_with_data() -> CodeExecutorContext:
    ...
    state_data = {
        ...
        "_code_executor_input_files": [{"name": "input1.txt", "content": "YQ==", "mime_type": "text/plain"}],
...
def test_add_input_files_append(context_with_data: CodeExecutorContext):
    ...
    assert context_with_data._session_state["_code_executor_input_files"] == expected_files_data
...
def test_clear_input_files_when_not_exist(empty_state: State):
    ...
    assert "_code_execution_context" not in empty_state or not empty_state["_code_execution_context"].get("processed_input_files")
...
def test_increment_error_count_existing_invocation(
    ...
    assert context_with_data._session_state["_code_executor_error_counts"]["invocationA"] == 3
...
def test_reset_error_count_exists(context_with_data: CodeExecutorContext):
    ...
    assert "invocationA" not in (context_with_data._session_state["_code_executor_error_counts"])
...
def test_reset_error_count_not_exists(context_with_data: CodeExecutorContext):
    ...
    assert "invocationB" not in (context_with_data._session_state["_code_executor_error_counts"])
...
def test_update_code_execution_result_append(
    ...
    context_with_data.update_code_execution_result("invocationX", "new_code", "new_out", "new_err")
    results = context_with_data._session_state["_code_execution_results"]["invocationX"]
