...
from typing import override  # <-- FIX: Add this import
...
def test_live_streaming_multi_agent_single_tool():
    ...
    delegation_to_roll_agent = types.Part.from_function_call(name="transfer_to_agent", args={"agent_name": "roll_agent"})
    ...
    mock_root_model = testing_utils.MockModel.create([root_response1, root_response2])

    # Mock response for the roll_agent to call its `roll_die` tool.
    function_call = types.Part.from_function_call(name="roll_die", args={"sides": 20})
    ...
    mock_roll_model = testing_utils.MockModel.create([roll_agent_response1, roll_agent_response2])
    ...
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"Roll a 20-sided die", mime_type="audio/pcm"))
...
def test_live_streaming_connection_error_on_connect():
    ...
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"Initial audio chunk", mime_type="audio/pcm"))
    ...
    assert "is not supported in the live api" in excinfo.value.rcvd.reason, "The exception reason should match the simulated server error."
