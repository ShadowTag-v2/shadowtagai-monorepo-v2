...
class UserContent(types.Content):
    def __init__(self, text_or_part: str):
        parts = [types.Part.from_text(text=text_or_part) if isinstance(text_or_part, str) else text_or_part]
...
async def create_invocation_context(
    ...
    invocation_context = InvocationContext(
        ...
        user_content=types.Content(role="user", parts=[types.Part.from_text(text=user_content)]),
        run_config=run_config or RunConfig(),
    )
    if user_content:
        append_user_content(invocation_context, [types.Part.from_text(text=user_content)])
    return invocation_context


def append_user_content(invocation_context: InvocationContext, parts: list[types.Part]) -> Event:
...
def simplify_events(events: list[Event]) -> list[(str, types.Part)]:
    return [(event.author, simplify_content(event.content)) for event in events if event.content]
...
class TestInMemoryRunner(AfInMemoryRunner):
    ...
    async def run_async_with_new_session(self, new_message: types.ContentUnion) -> list[Event]:
    ...
    async def run_async_with_new_session_agen(self, new_message: types.ContentUnion) -> AsyncGenerator[Event]:
        session = await self.session_service.create_session(app_name="InMemoryRunner", user_id="test_user")
        agen = self.run_async(
...
class InMemoryRunner:
    ...
    def __init__(
        self,
        root_agent: Agent | LlmAgent | None = None,
    ...
    def session(self) -> Session:
        if not self.session_id:
            session = self.runner.session_service.create_session_sync(app_name=self.app_name, user_id="test_user")
            self.session_id = session.id
            return session
        return self.runner.session_service.get_session_sync(app_name=self.app_name, user_id="test_user", session_id=self.session_id)
    ...
    async def run_async(
        self,
        new_message: types.ContentUnion | None = None,
...
class MockModel(BaseLlm):
    ...
    def create(
        cls,
        responses: list[types.Part] | list[LlmResponse] | list[str] | list[list[types.Part]],
        error: Exception | None = None,
    ):
    ...
    def generate_content(self, llm_request: LlmRequest, stream: bool = False) -> Generator[LlmResponse]:
...
class MockLlmConnection(BaseLlmConnection):
    ...
    async def receive(self) -> AsyncGenerator[LlmResponse]:
