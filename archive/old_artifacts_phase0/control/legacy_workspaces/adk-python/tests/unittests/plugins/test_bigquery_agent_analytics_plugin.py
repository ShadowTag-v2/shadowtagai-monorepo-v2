...
DEFAULT_STREAM_NAME = f"projects/{PROJECT_ID}/datasets/{DATASET_ID}/tables/{TABLE_ID}/_default"
...
@pytest.fixture
def mock_session():
    mock_s = mock.create_autospec(session_lib.Session, instance=True, spec_set=True)
...
@pytest.fixture
def mock_agent():
    mock_a = mock.create_autospec(base_agent.BaseAgent, instance=True, spec_set=True)
...
def invocation_context(mock_agent, mock_session):
    mock_session_service = mock.create_autospec(base_session_service_lib.BaseSessionService, instance=True, spec_set=True)
    mock_plugin_manager = mock.create_autospec(plugin_manager_lib.PluginManager, instance=True, spec_set=True)
...
def callback_context(invocation_context):
    return callback_context_lib.CallbackContext(invocation_context=invocation_context)
...
def mock_auth_default():
    mock_creds = mock.create_autospec(google.auth.credentials.Credentials, instance=True, spec_set=True)
...
def mock_write_client():
    with mock.patch.object(bigquery_agent_analytics_plugin, "BigQueryWriteAsyncClient", autospec=True) as mock_cls:
...
def mock_asyncio_to_thread():
    ...
    with mock.patch("asyncio.to_thread", side_effect=fake_to_thread) as mock_async:
        yield mock_async
...
def _get_captured_event_dict(mock_write_client, expected_schema):
    ...
    assert table.schema.equals(expected_schema), f"Schema mismatch: Expected {expected_schema}, got {table.schema}"
...
class TestBigQueryAgentAnalyticsPlugin:
    ...
    async def test_event_allowlist(
        ...
        plugin = bigquery_agent_analytics_plugin.BigQueryAgentAnalyticsPlugin(PROJECT_ID, DATASET_ID, TABLE_ID, config)
    ...
    async def test_max_content_length(
        ...
        user_message = types.Content(parts=[types.Part(text="12345678901234567890123456789012345678901")])  # 41 chars
        await plugin.on_user_message_callback(invocation_context=invocation_context, user_message=user_message)
        await asyncio.sleep(0.01)
        mock_write_client.append_rows.assert_called_once()
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
        assert log_entry["content"] == "User Content: text: '1234567890123456789012345678901234567890...' "
        ...
        llm_request = llm_request_lib.LlmRequest(
            model="gemini-pro",
            config=types.GenerateContentConfig(system_instruction=types.Content(parts=[types.Part(text="System Instruction")])),
    ...
    async def test_max_content_length_tool_args(
        ...
        mock_tool = mock.create_autospec(base_tool_lib.BaseTool, instance=True, spec_set=True)
    ...
    async def test_on_user_message_callback_logs_correctly(
        ...
        user_message = types.Content(parts=[types.Part(text="What is up?")])
        await bq_plugin_inst.on_user_message_callback(invocation_context=invocation_context, user_message=user_message)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
    ...
    async def test_on_event_callback_tool_call(
        ...
        event = event_lib.Event(
            ...
            timestamp=datetime.datetime(2025, 10, 22, 10, 0, 0, tzinfo=datetime.UTC).timestamp(),
        )
        await bq_plugin_inst.on_event_callback(invocation_context=invocation_context, event=event)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
        _assert_common_fields(log_entry, "TOOL_CALL", agent="MyTestAgent")
        assert "call: get_weather" in log_entry["content"]
        assert log_entry["timestamp"] == datetime.datetime(2025, 10, 22, 10, 0, 0, tzinfo=datetime.UTC)
    ...
    async def test_on_event_callback_model_response(
        ...
        event = event_lib.Event(
            ...
            timestamp=datetime.datetime(2025, 10, 22, 11, 0, 0, tzinfo=datetime.UTC).timestamp(),
        )
        await bq_plugin_inst.on_event_callback(invocation_context=invocation_context, event=event)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
        ...
        assert log_entry["timestamp"] == datetime.datetime(2025, 10, 22, 11, 0, 0, tzinfo=datetime.UTC)
    ...
    async def test_bigquery_client_initialization_failure(
        ...
        mock_auth_default.side_effect = auth_exceptions.GoogleAuthError("Auth failed")
        plugin_with_fail = bigquery_agent_analytics_plugin.BigQueryAgentAnalyticsPlugin(
            project_id=PROJECT_ID,
            dataset_id=DATASET_ID,
            table_id=TABLE_ID,
        )
    ...
    @pytest.mark.asyncio
    async def test_bigquery_insert_error_does_not_raise(self, bq_plugin_inst, mock_write_client, invocation_context):
        ...
        with mock.patch.object(logging, "error") as mock_log_error:
            ...
            await asyncio.sleep(0.01)
            mock_log_error.assert_called_with("BQ Plugin: Write Error: %s", "Test BQ Error")
        mock_write_client.append_rows.assert_called_once()

    @pytest.mark.asyncio
    async def test_schema_mismatch_error_handling(self, bq_plugin_inst, mock_write_client, invocation_context):
        async def fake_append_rows_with_schema_error(requests, **kwargs):
            ...
            mock_resp.error.message = "Schema mismatch: Field 'new_field' not found in table."
            return _async_gen(mock_resp)

        mock_write_client.append_rows.side_effect = fake_append_rows_with_schema_error
    ...
    async def test_before_agent_callback_logs_correctly(
        ...
        await bq_plugin_inst.before_agent_callback(agent=mock_agent, callback_context=callback_context)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
    ...
    async def test_after_agent_callback_logs_correctly(
        ...
        await bq_plugin_inst.after_agent_callback(agent=mock_agent, callback_context=callback_context)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
    ...
    async def test_before_model_callback_logs_correctly(
        ...
        await bq_plugin_inst.before_model_callback(callback_context=callback_context, llm_request=llm_request)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
        _assert_common_fields(log_entry, "LLM_REQUEST")
        assert log_entry["content"] == "Model: gemini-pro | Prompt: user: text: 'Prompt' | System Prompt: Empty"
    ...
    async def test_after_model_callback_text_response(
        ...
        llm_response = llm_response_lib.LlmResponse(
            ...
            usage_metadata=types.UsageMetadata(prompt_token_count=10, total_token_count=15),
        )
        await bq_plugin_inst.after_model_callback(callback_context=callback_context, llm_response=llm_response)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
        _assert_common_fields(log_entry, "LLM_RESPONSE")
        assert "Tool Name: text_response, text: 'Model response'" in log_entry["content"]
    ...
    @pytest.mark.asyncio
    async def test_before_tool_callback_logs_correctly(self, bq_plugin_inst, mock_write_client, tool_context, dummy_arrow_schema):
        mock_tool = mock.create_autospec(base_tool_lib.BaseTool, instance=True, spec_set=True)
        type(mock_tool).name = mock.PropertyMock(return_value="MyTool")
        type(mock_tool).description = mock.PropertyMock(return_value="Description")
        await bq_plugin_inst.before_tool_callback(tool=mock_tool, tool_args={"param": "value"}, tool_context=tool_context)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
        _assert_common_fields(log_entry, "TOOL_STARTING")
        assert log_entry["content"] == 'Tool Name: MyTool, Description: Description, Arguments: {"param": "value"}'

    @pytest.mark.asyncio
    async def test_after_tool_callback_logs_correctly(self, bq_plugin_inst, mock_write_client, tool_context, dummy_arrow_schema):
        mock_tool = mock.create_autospec(base_tool_lib.BaseTool, instance=True, spec_set=True)
        type(mock_tool).name = mock.PropertyMock(return_value="MyTool")
        type(mock_tool).description = mock.PropertyMock(return_value="Description")
        ...
        assert log_entry["content"] == 'Tool Name: MyTool, Result: {"status": "success"}'
    ...
    async def test_on_model_error_callback_logs_correctly(
        ...
        await bq_plugin_inst.on_model_error_callback(callback_context=callback_context, llm_request=llm_request, error=error)
        await asyncio.sleep(0.01)
        log_entry = _get_captured_event_dict(mock_write_client, dummy_arrow_schema)
    ...
    @pytest.mark.asyncio
    async def test_on_tool_error_callback_logs_correctly(self, bq_plugin_inst, mock_write_client, tool_context, dummy_arrow_schema):
        mock_tool = mock.create_autospec(base_tool_lib.BaseTool, instance=True, spec_set=True)
        type(mock_tool).name = mock.PropertyMock(return_value="MyTool")
        type(mock_tool).description = mock.PropertyMock(return_value="Description")
        ...
        assert log_entry["content"] == 'Tool Name: MyTool, Arguments: {"param": "value"}'
    ...
    async def test_table_creation_options(
        ...
        plugin = bigquery_agent_analytics_plugin.BigQueryAgentAnalyticsPlugin(PROJECT_ID, DATASET_ID, TABLE_ID)
        await plugin._ensure_init()
        ...
        assert timestamp_field.description == "The UTC time at which the event was logged."
