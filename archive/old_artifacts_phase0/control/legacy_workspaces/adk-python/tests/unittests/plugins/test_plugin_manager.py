...
from google.adk.plugins.plugin_manager import PluginCallbackName, PluginManager
...
def test_register_duplicate_plugin_name_raises_value_error(service: PluginManager, plugin1: TestPlugin):
    """Tests that registering a plugin with a duplicate name raises an error."""
    plugin1_duplicate = TestPlugin(name="plugin1")
    service.register_plugin(plugin1)

    with pytest.raises(ValueError, match="Plugin with name 'plugin1' already registered."):
        service.register_plugin(plugin1_duplicate)


@pytest.mark.asyncio
async def test_early_exit_stops_subsequent_plugins(service: PluginManager, plugin1: TestPlugin, plugin2: TestPlugin):
...
@pytest.mark.asyncio
async def test_normal_flow_all_plugins_are_called(service: PluginManager, plugin1: TestPlugin, plugin2: TestPlugin):
...
@pytest.mark.asyncio
async def test_plugin_exception_is_wrapped_in_runtime_error(service: PluginManager, plugin1: TestPlugin):
...
@pytest.mark.asyncio
async def test_all_callbacks_are_supported(service: PluginManager, plugin1: TestPlugin):
    ...
    # Test all callbacks
    await service.run_on_user_message_callback(user_message=mock_user_message, invocation_context=mock_context)
    ...
    await service.run_on_event_callback(invocation_context=mock_context, event=mock_context)
    await service.run_before_agent_callback(agent=mock_context, callback_context=mock_context)
    await service.run_after_agent_callback(agent=mock_context, callback_context=mock_context)
    await service.run_before_tool_callback(tool=mock_context, tool_args={}, tool_context=mock_context)
    await service.run_after_tool_callback(tool=mock_context, tool_args={}, tool_context=mock_context, result={})
    ...
    await service.run_before_model_callback(callback_context=mock_context, llm_request=mock_context)
    await service.run_after_model_callback(callback_context=mock_context, llm_response=mock_context)
...
@pytest.mark.asyncio
async def test_close_calls_plugin_close(service: PluginManager, plugin1: TestPlugin):
...
@pytest.mark.asyncio
async def test_close_raises_runtime_error_on_plugin_exception(service: PluginManager, plugin1: TestPlugin):
    ...
    with pytest.raises(RuntimeError, match="Failed to close plugins: 'plugin1': ValueError"):
        await service.close()
...
async def test_close_with_timeout(plugin1: TestPlugin):
    ...
    assert "Failed to close plugins: 'plugin1': TimeoutError" in str(excinfo.value)
