...
async def test_global_instruction_plugin_with_string():
    ...
    plugin = GlobalInstructionPlugin(global_instruction=("You are a helpful assistant with a friendly personality."))

    # Create mock objects
    mock_session = Session(app_name="test_app", user_id="test_user", id="test_session", state={})
    ...
    assert "You are a helpful assistant with a friendly personality." in llm_request.config.system_instruction
...
async def test_global_instruction_plugin_with_instruction_provider():
    ...
    # Create mock objects
    mock_session = Session(app_name="test_app", user_id="alice", id="test_session", state={})
    ...
    assert "You are assistant for user alice." in llm_request.config.system_instruction
...
async def test_global_instruction_plugin_empty_instruction():
    ...
    llm_request = LlmRequest(
        model="gemini-1.5-flash",
        config=types.GenerateContentConfig(system_instruction="Original instruction"),
    )
...
async def test_global_instruction_plugin_leads_existing():
    ...
    plugin = GlobalInstructionPlugin(global_instruction="You are a helpful assistant.")
    ...
    llm_request = LlmRequest(
        model="gemini-1.5-flash",
        config=types.GenerateContentConfig(system_instruction="Existing instructions."),
    )
...
async def test_global_instruction_plugin_prepends_to_list():
    ...
    llm_request = LlmRequest(
        model="gemini-1.5-flash",
        config=types.GenerateContentConfig(system_instruction=["Existing instruction."]),
    )
