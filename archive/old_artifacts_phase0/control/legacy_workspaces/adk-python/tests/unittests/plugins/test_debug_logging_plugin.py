...
class TestDebugLoggingPluginCallbacks:
    ...
    async def test_before_run_callback_initializes_state(self, debug_output_file, mock_invocation_context):
    ...
    async def test_on_user_message_callback_logs_message(self, debug_output_file, mock_invocation_context):
        ...
        user_message = types.Content(role="user", parts=[types.Part.from_text(text="Hello, world!")])

        result = await plugin.on_user_message_callback(invocation_context=mock_invocation_context, user_message=user_message)

        assert result is None
        state = plugin._invocation_states[mock_invocation_context.invocation_id]
        user_message_entries = [e for e in state.entries if e.entry_type == "user_message"]
        assert len(user_message_entries) == 1
        assert user_message_entries[0].data["content"]["role"] == "user"
        assert user_message_entries[0].data["content"]["parts"][0]["text"] == ("Hello, world!")

    async def test_before_model_callback_logs_request(self, debug_output_file, mock_invocation_context, mock_callback_context):
        ...
        result = await plugin.before_model_callback(callback_context=mock_callback_context, llm_request=llm_request)
        ...
        assert llm_entries[0].data["config"]["system_instruction"] == "You are a helpful assistant."

    async def test_after_model_callback_logs_response(self, debug_output_file, mock_invocation_context, mock_callback_context):
        ...
        result = await plugin.after_model_callback(callback_context=mock_callback_context, llm_response=llm_response)
    ...
    async def test_before_tool_callback_logs_tool_call(self, debug_output_file, mock_invocation_context, mock_tool_context):
    ...
    async def test_after_tool_callback_logs_tool_response(self, debug_output_file, mock_invocation_context, mock_tool_context):
    ...
    async def test_on_event_callback_logs_event(self, debug_output_file, mock_invocation_context):
        ...
        result = await plugin.on_event_callback(invocation_context=mock_invocation_context, event=event)
    ...
    async def test_on_model_error_callback_logs_error(self, debug_output_file, mock_invocation_context, mock_callback_context):
    ...
    async def test_on_tool_error_callback_logs_error(self, debug_output_file, mock_invocation_context, mock_tool_context):
...
class TestDebugLoggingPluginFileOutput:
    ...
    async def test_after_run_callback_writes_to_file(self, debug_output_file, mock_invocation_context):
        ...
        # Add some entries
        user_message = types.Content(role="user", parts=[types.Part.from_text(text="Test message")])
        await plugin.on_user_message_callback(invocation_context=mock_invocation_context, user_message=user_message)
        ...
        assert len(data["entries"]) >= 2  # At least invocation_start and user_message

    async def test_after_run_callback_includes_session_state(self, debug_output_file, mock_invocation_context):
        """Test that session state is included when enabled."""
        plugin = DebugLoggingPlugin(output_path=str(debug_output_file), include_session_state=True)
        ...
        data = documents[0]
        session_state_entries = [e for e in data["entries"] if e["entry_type"] == "session_state_snapshot"]
        assert len(session_state_entries) == 1
        assert session_state_entries[0]["data"]["state"]["key1"] == "value1"

    async def test_after_run_callback_excludes_session_state_when_disabled(self, debug_output_file, mock_invocation_context):
        """Test that session state is excluded when disabled."""
        plugin = DebugLoggingPlugin(output_path=str(debug_output_file), include_session_state=False)
    ...
    async def test_multiple_invocations_append_to_file(self, debug_output_file, mock_session):
    ...
    async def test_after_run_callback_cleans_up_state(self, debug_output_file, mock_invocation_context):
        ...
        await plugin.after_run_callback(invocation_context=mock_invocation_context)
        assert mock_invocation_context.invocation_id not in plugin._invocation_states
...
class TestDebugLoggingPluginSerialization:
    ...
    def test_serialize_content_with_function_call(self):
        ...
        content = types.Content(
            role="model",
            parts=[types.Part(function_call=types.FunctionCall(id="fc-1", name="test_func", args={"arg1": "val1"}))],
        )
...
class TestDebugLoggingPluginSystemInstructionConfig:
    ...
    async def test_system_instruction_included_when_enabled(self, debug_output_file, mock_invocation_context, mock_callback_context):
        """Test that full system instruction is included when enabled."""
        plugin = DebugLoggingPlugin(output_path=str(debug_output_file), include_system_instruction=True)
        ...
        state = plugin._invocation_states[mock_invocation_context.invocation_id]
        llm_entries = [e for e in state.entries if e.entry_type == "llm_request"]
        assert llm_entries[0].data["config"]["system_instruction"] == "Full system instruction text"

    async def test_system_instruction_length_only_when_disabled(self, debug_output_file, mock_invocation_context, mock_callback_context):
        """Test that only length is included when system instruction is disabled."""
        plugin = DebugLoggingPlugin(output_path=str(debug_output_file), include_system_instruction=False)
