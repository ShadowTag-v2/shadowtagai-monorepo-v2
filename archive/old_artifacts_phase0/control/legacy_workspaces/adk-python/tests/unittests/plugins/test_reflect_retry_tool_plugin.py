...
from google.adk.plugins.reflect_retry_tool_plugin import REFLECT_AND_RETRY_RESPONSE_TYPE, ReflectAndRetryToolPlugin
...
class TestReflectAndRetryToolPlugin(IsolatedAsyncioTestCase):
    ...
    async def test_max_retries_exceeded_with_exception(self):
        ...
        mock_tool_context = self.get_mock_tool_context()
        sample_tool_args = self.get_sample_tool_args()
        plugin = ReflectAndRetryToolPlugin(max_retries=1, throw_exception_if_retry_exceeded=True)
    ...
    async def test_max_retries_exceeded_without_exception(self):
        ...
        mock_tool_context = self.get_mock_tool_context()
        sample_tool_args = self.get_sample_tool_args()
        plugin = ReflectAndRetryToolPlugin(max_retries=2, throw_exception_if_retry_exceeded=False)
        ...
        self.assertIn("the retry limit has been exceeded", result["reflection_guidance"])
    ...
    async def test_max_retries_parameter_behavior(self):
        ...
        plugin = ReflectAndRetryToolPlugin(max_retries=1, throw_exception_if_retry_exceeded=False)
        error = ValueError("Test error")
    ...
    async def test_custom_error_detection_and_success_handling(self):
        ...
        mock_tool_context = self.get_mock_tool_context()
        sample_tool_args = self.get_sample_tool_args()
        custom_error_plugin.set_error_condition(lambda result: result if result.get("status") == "error" else None)
        ...
        self.assertEqual(callback_result["response_type"], REFLECT_AND_RETRY_RESPONSE_TYPE)
    ...
    async def test_retry_state_management(self):
        ...
        mock_tool_context = self.get_mock_tool_context()
        sample_tool_args = self.get_sample_tool_args()
        custom_error_plugin.set_error_condition(lambda result: result if result.get("failed") else None)
    ...
    async def test_hallucinating_tool_name(self):
        ...
        wrong_function_call = types.Part.from_function_call(name="increase_by_one", args={"x": 1})
        correct_function_call = types.Part.from_function_call(name="increase", args={"x": 1})
        ...
        runner = testing_utils.TestInMemoryRunner(agent=agent, plugins=[self.get_plugin()])
        ...
        assert events[1].content.parts[0].function_response.response["retry_count"] == 1
        assert "Wrong Function Name" in events[1].content.parts[0].function_response.response["reflection_guidance"]
