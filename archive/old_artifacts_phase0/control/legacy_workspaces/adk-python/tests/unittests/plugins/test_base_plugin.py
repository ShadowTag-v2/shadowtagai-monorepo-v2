...
async def test_base_plugin_default_callbacks_return_none():
    ...
    assert await plugin.before_run_callback(invocation_context=mock_context) is None
    assert await plugin.after_run_callback(invocation_context=mock_context) is None
    assert await plugin.on_event_callback(invocation_context=mock_context, event=mock_context) is None
    assert await plugin.before_agent_callback(agent=mock_context, callback_context=mock_context) is None
    assert await plugin.after_agent_callback(agent=mock_context, callback_context=mock_context) is None
    assert await plugin.before_tool_callback(tool=mock_context, tool_args={}, tool_context=mock_context) is None
    assert await plugin.after_tool_callback(tool=mock_context, tool_args={}, tool_context=mock_context, result={}) is None
    assert (
    ...
    assert await plugin.before_model_callback(callback_context=mock_context, llm_request=mock_context) is None
    assert await plugin.after_model_callback(callback_context=mock_context, llm_response=mock_context) is None
    assert (
...
async def test_base_plugin_all_callbacks_can_be_overridden():
    ...
    assert await plugin.before_run_callback(invocation_context=mock_invocation_context) == "overridden_before_run"
    assert await plugin.after_run_callback(invocation_context=mock_invocation_context) == "overridden_after_run"
    assert await plugin.on_event_callback(invocation_context=mock_invocation_context, event=mock_event) == "overridden_on_event"
    assert await plugin.before_agent_callback(agent=mock_agent, callback_context=mock_callback_context) == "overridden_before_agent"
    assert await plugin.after_agent_callback(agent=mock_agent, callback_context=mock_callback_context) == "overridden_after_agent"
    assert await plugin.before_model_callback(callback_context=mock_callback_context, llm_request=mock_llm_request) == "overridden_before_model"
    assert await plugin.after_model_callback(callback_context=mock_callback_context, llm_response=mock_llm_response) == "overridden_after_model"
    assert await plugin.before_tool_callback(tool=mock_tool, tool_args={}, tool_context=mock_tool_context) == "overridden_before_tool"
    assert (
