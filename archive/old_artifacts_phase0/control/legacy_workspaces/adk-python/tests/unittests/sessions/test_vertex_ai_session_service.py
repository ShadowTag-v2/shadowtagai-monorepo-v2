...
MOCK_EVENT_JSON_2 = [
    {
        "name": ("projects/test-project/locations/test-location/reasoningEngines/123/sessions/2/events/123"),
        "invocation_id": "222",
        "author": "user",
        "timestamp": "2024-12-12T12:12:12.123456Z",
    },
...
MOCK_EVENT_JSON_3 = [
    {
        "name": ("projects/test-project/locations/test-location/reasoningEngines/123/sessions/2/events/456"),
        "invocation_id": "333",
        "author": "user",
...
MOCK_SESSION_JSON_PAGE1 = {
    "name": ("projects/test-project/locations/test-location/reasoningEngines/123/sessions/page1"),
    "update_time": "2024-12-15T12:12:12.123456Z",
    "user_id": "user_with_pages",
}
MOCK_SESSION_JSON_PAGE2 = {
    "name": ("projects/test-project/locations/test-location/reasoningEngines/123/sessions/page2"),
...
def _generate_mock_events_for_session_5(num_events):
    ...
    for i in range(num_events):
        ...
        events.append(
            {
                "name": (f"projects/test-project/locations/test-location/reasoningEngines/123/sessions/5/events/{i}"),
    ...
    def model_dump(self, exclude_none=True, mode="python"):
        ...
        for k, v in self.__dict__.items():
            ...
            if isinstance(v, PydanticNamespace):
                ...
                d[k] = [i.model_dump(exclude_none=exclude_none, mode=mode) if isinstance(i, PydanticNamespace) else i for i in v]
            else:
                d[k] = v
        return d
...
class MockAsyncClient:
    ...
    def __init__(self) -> None:
        ...
        self.event_dict: dict[str, tuple[list[Any], str | None]] = {}
    ...
    async def _list_sessions(self, name: str, config: dict[str, Any]):
        ...
        if user_id_match:
            ...
            return [_convert_to_object(session) for session in self.session_dict.values() if session["user_id"] == user_id]

        # No user filter, return all sessions
        return [_convert_to_object(session) for session in self.session_dict.values()]

    async def _delete_session(self, name: str):
        session_id = name.split("/")[-1]
        self.session_dict.pop(session_id)

    async def _create_session(self, name: str, user_id: str, config: dict[str, Any]):
        self.last_create_session_config = config
        new_session_id = "4"
        self.session_dict[new_session_id] = {
            "name": ("projects/test-project/locations/test-location/reasoningEngines/123/sessions/" + new_session_id),
            "user_id": user_id,
        ...
        return _convert_to_object(
            {
                "name": ("projects/test_project/locations/test_location/reasoningEngines/123/sessions/" + new_session_id + "/operations/111"),
                "done": True,
    ...
    async def _list_events(self, name: str, **kwargs):
        ...
        if filter_str:
            ...
            if match:
                ...
                events = [event for event in events if isoparse(event["timestamp"]) >= after_timestamp]
...
async def test_initialize_with_project_location_and_api_key_error():
    ...
    assert "Cannot specify project or location and express_mode_api_key. Either use project and location, or just the express_mode_api_key." in str(
        excinfo.value
    )
...
async def test_get_empty_session(agent_engine_id):
    ...
    with pytest.raises(api_core_exceptions.NotFound) as excinfo:
        await session_service.get_session(app_name="123", user_id="user", session_id="0")
...
async def test_get_another_user_session(agent_engine_id):
    session_service = mock_vertex_ai_session_service(agent_engine_id)
    with pytest.raises(ValueError) as excinfo:
        await session_service.get_session(app_name="123", user_id="user2", session_id="1")
...
async def test_get_and_delete_session():
    ...
    assert await session_service.get_session(app_name="123", user_id="user", session_id="1") == MOCK_SESSION

    await session_service.delete_session(app_name="123", user_id="user", session_id="1")
    with pytest.raises(api_core_exceptions.NotFound) as excinfo:
        await session_service.get_session(app_name="123", user_id="user", session_id="1")
...
async def test_get_session_with_page_token():
    ...
    assert await session_service.get_session(app_name="123", user_id="user", session_id="2") == MOCK_SESSION_2
...
async def test_get_session_with_after_timestamp_filter():
    ...
    session = await session_service.get_session(
        ...
        user_id="user",
        session_id="2",
        config=GetSessionConfig(after_timestamp=isoparse("2024-12-12T12:12:13.0Z").timestamp()),
    )
...
async def test_get_session_keeps_events_newer_than_update_time(
    ...
    future_event_time = isoparse(MOCK_SESSION_JSON_1["update_time"]) + datetime.timedelta(seconds=1)
    event = mock_api_client_instance.event_dict["1"][0][0]
    event["timestamp"] = future_event_time.isoformat().replace("+00:00", "Z")
    session_service = mock_vertex_ai_session_service()

    session = await session_service.get_session(app_name="123", user_id="user", session_id="1")

    assert session is not None
    assert len(session.events) == 1
    assert session.events[0].timestamp == future_event_time.timestamp()
    assert session.events[0].timestamp > session.last_update_time, "Event timestamp should exceed session update_time to guard against filtering."
...
async def test_get_session_with_many_events(mock_api_client_instance):
    ...
    session_service = mock_vertex_ai_session_service()
    session = await session_service.get_session(app_name="123", user_id="user_with_many_events", session_id="5")
    assert session is not None
...
async def test_list_sessions_with_pagination():
    session_service = mock_vertex_ai_session_service()
    sessions = await session_service.list_sessions(app_name="123", user_id="user_with_pages")
    assert len(sessions.sessions) == 2
...
async def test_create_session():
    ...
    state = {"key": "value"}
    session = await session_service.create_session(app_name="123", user_id="user", state=state)
    ...
    assert session == await session_service.get_session(app_name="123", user_id="user", session_id=session_id)
...
async def test_create_session_with_custom_session_id():
    ...
    with pytest.raises(ValueError) as excinfo:
        await session_service.create_session(app_name="123", user_id="user", session_id="1")
    assert str(excinfo.value) == ("User-provided Session id is not supported for VertexAISessionService.")
...
async def test_create_session_with_custom_config(mock_api_client_instance):
    ...
    await session_service.create_session(app_name="123", user_id="user", expire_time=expire_time)
...
async def test_append_event():
    session_service = mock_vertex_ai_session_service()
    session_before_append = await session_service.get_session(app_name="123", user_id="user", session_id="1")
    ...
    retrieved_session = await session_service.get_session(app_name="123", user_id="user", session_id="1")
