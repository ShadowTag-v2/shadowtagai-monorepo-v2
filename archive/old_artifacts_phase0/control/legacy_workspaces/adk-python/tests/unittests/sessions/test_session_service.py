...
async def test_get_empty_session(service_type, tmp_path):
    ...
    assert not await session_service.get_session(app_name="my_app", user_id="test_user", session_id="123")
...
async def test_create_get_session(service_type, tmp_path):
    ...
    assert session.last_update_time <= datetime.now().astimezone(UTC).timestamp()
    ...
    assert got_session.last_update_time <= datetime.now().astimezone(UTC).timestamp()

    session_id = session.id
    await session_service.delete_session(app_name=app_name, user_id=user_id, session_id=session_id)

    assert await session_service.get_session(app_name=app_name, user_id=user_id, session_id=session.id) is None
...
async def test_list_sessions_all_users(service_type, tmp_path):
    ...
    list_sessions_response_1 = await session_service.list_sessions(app_name=app_name, user_id=user_id_1)
    ...
    list_sessions_response_2 = await session_service.list_sessions(app_name=app_name, user_id=user_id_2)
    ...
    list_sessions_response_all = await session_service.list_sessions(app_name=app_name, user_id=None)
...
async def test_app_state_is_shared_by_all_users_of_app(service_type, tmp_path):
    ...
    session1 = await session_service.create_session(app_name=app_name, user_id="u1", session_id="s1", state={"app:k1": "v1"})
    ...
    session2 = await session_service.create_session(app_name=app_name, user_id="u2", session_id="s2")
    assert session2.state == {"app:k1": "v1", "app:k2": "v2"}

    # If we get session session1 again, it should also see both
    session1_got = await session_service.get_session(app_name=app_name, user_id="u1", session_id="s1")
...
async def test_user_state_is_shared_only_by_user_sessions(service_type, tmp_path):
    session_service = get_session_service(service_type, tmp_path)
    app_name = "my_app"
    # User 1 creates a session, establishing user:k1 for user 1
    session1 = await session_service.create_session(app_name=app_name, user_id="u1", session_id="s1", state={"user:k1": "v1"})
    ...
    session1b = await session_service.create_session(app_name=app_name, user_id="u1", session_id="s1b")
...
async def test_session_state_is_not_shared(service_type, tmp_path):
    ...
    session1 = await session_service.create_session(app_name=app_name, user_id="u1", session_id="s1", state={"sk1": "v1"})
...
async def test_temp_state_is_not_persisted_in_state_or_events(service_type, tmp_path):
    session_service = get_session_service(service_type, tmp_path)
    app_name = "my_app"
    user_id = "u1"
    session = await session_service.create_session(app_name=app_name, user_id=user_id, session_id="s1")
    event = Event(
        invocation_id="inv1",
        author="user",
    ...
    session_got = await session_service.get_session(app_name=app_name, user_id=user_id, session_id="s1")
...
async def test_get_session_respects_user_id(service_type, tmp_path):
    ...
    session1 = await session_service.create_session(app_name=app_name, user_id="u1", session_id="s1")
    ...
    await session_service.create_session(app_name=app_name, user_id="u2", session_id="s1")
    # Check that getting s1 for u2 returns u2's session (with no events)
    # not u1's session.
    session2_got = await session_service.get_session(app_name=app_name, user_id="u2", session_id="s1")
...
async def test_create_session_with_existing_id_raises_error(service_type, tmp_path):
    session_service = get_session_service(service_type, tmp_path)
    app_name = "my_app"
    user_id = "test_user"
...
async def test_append_event_bytes(service_type, tmp_path):
    ...
    test_grounding_metadata = types.GroundingMetadata(search_entry_point=types.SearchEntryPoint(sdk_blob=b"test_sdk_blob"))
    event = Event(
...
async def test_append_event_complete(service_type, tmp_path):
    ...
    event = Event(
        ...
        usage_metadata=types.GenerateContentResponseUsageMetadata(prompt_token_count=1, candidates_token_count=1, total_token_count=2),
...
async def test_session_last_update_time_updates_on_event(service_type, tmp_path):
    session_service = get_session_service(service_type, tmp_path)
    app_name = "my_app"
    user_id = "user"
    ...
    refreshed_session = await session_service.get_session(app_name=app_name, user_id=user_id, session_id=session.id)
    assert refreshed_session is not None
    assert refreshed_session.last_update_time == pytest.approx(event_timestamp, abs=1e-6)
...
async def test_get_session_with_config(service_type, tmp_path):
    ...
    session = await session_service.get_session(app_name=app_name, user_id=user_id, session_id=session.id, config=config)
    events = session.events
...
async def test_partial_events_are_not_persisted(service_type, tmp_path):
    ...
    session_got = await session_service.get_session(app_name=app_name, user_id=user_id, session_id=session.id)
