...
FEATURE_CONFIG_EXPERIMENTAL_DISABLED = FeatureConfig(FeatureStage.EXPERIMENTAL, default_on=False)
FEATURE_CONFIG_EXPERIMENTAL_ENABLED = FeatureConfig(FeatureStage.EXPERIMENTAL, default_on=True)
...
class TestGetFeatureConfig:
    ...
    def test_feature_in_registry(self):
        ...
        assert _get_feature_config("MY_FEATURE") == FEATURE_CONFIG_EXPERIMENTAL_ENABLED
...
class TestIsFeatureEnabled:
    ...
    def test_wip_feature_enabled(self):
        """WIP features are disabled by default."""
        _register_feature("WIP_FEATURE", FeatureConfig(FeatureStage.WIP, default_on=True))
        with warnings.catch_warnings(record=True) as w:
    ...
    def test_experimental_enabled_feature(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert len(w) == 1
            assert "[EXPERIMENTAL] feature EXP_ENABLED is enabled." in str(w[0].message)
    ...
    def test_enable_env_var_takes_precedence(self, monkeypatch):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert len(w) == 1
            assert "[EXPERIMENTAL] feature DISABLED_FEATURE is enabled." in str(w[0].message)
