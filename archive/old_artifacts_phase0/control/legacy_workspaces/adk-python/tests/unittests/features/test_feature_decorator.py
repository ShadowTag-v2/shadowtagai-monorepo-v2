...
def test_working_in_progress_stage_mismatch():
    ...
    try:
        ...
        assert (
            ...
            " 'FeatureStage.EXPERIMENTAL'." in str(e)
        )
...
def test_disabled_experimental_class_bypass_with_env_var(monkeypatch):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        feature.run()
        assert len(w) == 1
        assert "[EXPERIMENTAL] feature EXPERIMENTAL_CLASS is enabled." in str(w[0].message)
...
def test_enabled_experimental_function_does_not_raise_error():
    ...
    with warnings.catch_warnings(record=True) as w:
        experimental_function()
        assert len(w) == 1
        assert "[EXPERIMENTAL] feature EXPERIMENTAL_FUNCTION is enabled." in str(w[0].message)
