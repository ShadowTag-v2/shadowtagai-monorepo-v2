...
class TestInvocationContext:
    ...
    def test_get_events_returns_all_events_by_default(self, mock_invocation_context, mock_events):
        """Tests that get_events returns all events when no filters are applied."""
        events = mock_invocation_context._get_events()
        assert events == mock_events

    def test_get_events_filters_by_current_invocation(self, mock_invocation_context, mock_events):
    ...
    def test_get_events_filters_by_current_branch(self, mock_invocation_context, mock_events):
    ...
    def test_get_events_filters_by_invocation_and_branch(self, mock_invocation_context, mock_events):
...
class TestInvocationContextWithAppResumablity:
    ...
    def event_to_pause(self, long_running_function_call) -> Event:
        ...
        return Event(
            invocation_id="inv_1",
            author="agent",
            content=testing_utils.ModelContent([Part(function_call=long_running_function_call)]),
            long_running_tool_ids=[long_running_function_call.id],
        )

    def _create_test_invocation_context(self, resumability_config) -> InvocationContext:
        """Create a mock invocation context for testing."""
        ctx = InvocationContext(
            session_service=Mock(spec=BaseSessionService),
            agent=Mock(spec=BaseAgent),
    ...
    def test_should_pause_invocation_with_resumable_app(self, event_to_pause):
        ...
        mock_invocation_context = self._create_test_invocation_context(ResumabilityConfig(is_resumable=True))

        assert mock_invocation_context.should_pause_invocation(event_to_pause)

    def test_should_not_pause_invocation_with_non_resumable_app(self, event_to_pause):
        """Tests should_pause_invocation with a non-resumable app."""
        invocation_context = self._create_test_invocation_context(ResumabilityConfig(is_resumable=False))

        assert not invocation_context.should_pause_invocation(event_to_pause)

    def test_should_not_pause_invocation_with_no_long_running_tool_ids(self, event_to_pause):
        """Tests should_pause_invocation with no long running tools."""
        invocation_context = self._create_test_invocation_context(ResumabilityConfig(is_resumable=True))
        nonpausable_event = event_to_pause.model_copy(update={"long_running_tool_ids": []})

        assert not invocation_context.should_pause_invocation(nonpausable_event)

    def test_should_not_pause_invocation_with_no_function_calls(self, event_to_pause):
        """Tests should_pause_invocation with a non-model event."""
        mock_invocation_context = self._create_test_invocation_context(ResumabilityConfig(is_resumable=True))
        nonpausable_event = event_to_pause.model_copy(update={"content": testing_utils.UserContent("test text part")})

        assert not mock_invocation_context.should_pause_invocation(nonpausable_event)
    ...
    def test_set_agent_state_with_agent_state(self):
        ...
        assert invocation_context.agent_states["agent1"] == agent_state.model_dump(mode="json")
    ...
    def test_reset_sub_agent_states(self):
        ...
        root_agent = BaseAgent(name="root_agent", sub_agents=[sub_agent_1, sub_agent_2])

        invocation_context = self._create_test_invocation_context(ResumabilityConfig(is_resumable=True))
        invocation_context.agent = root_agent
        invocation_context.set_agent_state("sub_agent_1", agent_state=BaseAgentState())
        invocation_context.set_agent_state("sub_agent_2", end_of_agent=True)
        invocation_context.set_agent_state("sub_sub_agent_1", agent_state=BaseAgentState())
...
class TestFindMatchingFunctionCall:
    ...
    def test_find_matching_function_call_found(self, test_invocation_context):
        ...
        matching_fc_event = invocation_context._find_matching_function_call(fr_event)
        assert testing_utils.simplify_content(matching_fc_event.content) == testing_utils.simplify_content(fc_event.content)
    ...
    def test_find_matching_function_call_no_call_events(self, test_invocation_context):
    ...
    def test_find_matching_function_call_no_response_in_event(self, test_invocation_context):
