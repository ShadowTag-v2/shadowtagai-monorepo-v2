...
class _TestingAgentWithEscalateAction(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event]:
        ...
        yield Event(
            author=self.name,
            invocation_id=ctx.invocation_id,
            content=types.Content(parts=[types.Part(text="I have done my job after escalation!!")]),
        )


async def _create_parent_invocation_context(test_name: str, agent: BaseAgent, resumable: bool = False) -> InvocationContext:
...
async def test_run_async(request: pytest.FixtureRequest, resumable: bool):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, loop_agent, resumable=resumable)
...
async def test_resume_async(request: pytest.FixtureRequest):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, loop_agent, resumable=True)
    parent_ctx.agent_states[loop_agent.name] = LoopAgentState(current_sub_agent=agent_2.name, times_looped=1).model_dump(mode="json")
...
async def test_run_async_skip_if_no_sub_agent(request: pytest.FixtureRequest):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, loop_agent)
    events = [e async for e in loop_agent.run_async(parent_ctx)]
    assert not events
...
@pytest.mark.asyncio
@pytest.mark.parametrize("resumable", [True, False])
async def test_run_async_with_escalate_action(request: pytest.FixtureRequest, resumable: bool):
    non_escalating_agent = _TestingAgent(name=f"{request.function.__name__}_test_non_escalating_agent")
    escalating_agent = _TestingAgentWithEscalateAction(name=f"{request.function.__name__}_test_escalating_agent")
    ignored_agent = _TestingAgent(name=f"{request.function.__name__}_test_ignored_agent")
    loop_agent = LoopAgent(
        name=f"{request.function.__name__}_test_loop_agent",
