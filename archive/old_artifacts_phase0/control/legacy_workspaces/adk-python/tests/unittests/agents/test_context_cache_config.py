...
class TestContextCacheConfig:
    ...
    def test_custom_values(self):
        ...
        config = ContextCacheConfig(cache_intervals=15, ttl_seconds=3600, min_tokens=1024)
    ...
    def test_str_representation(self):
        ...
        expected = "ContextCacheConfig(cache_intervals=15, ttl=3600s, min_tokens=1024)"
    ...
    def test_field_descriptions(self):
        ...
        assert "Maximum number of invocations" in schema["properties"]["cache_intervals"]["description"]

        assert "ttl_seconds" in schema["properties"]
        assert "Time-to-live for cache" in schema["properties"]["ttl_seconds"]["description"]

        assert "min_tokens" in schema["properties"]
        assert "Minimum estimated request tokens" in schema["properties"]["min_tokens"]["description"]
