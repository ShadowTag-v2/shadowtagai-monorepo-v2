...
def test_agent_not_found_shows_all_agents():
    ...
    sub_agents = [LlmAgent(name=f"agent_{i}", model="gemini-2.0-flash") for i in range(100)]

    root_agent = LlmAgent(name="root", model="gemini-2.0-flash", sub_agents=sub_agents)
