...
def test_validate_max_llm_calls_too_large():
    with pytest.raises(ValueError, match=f"max_llm_calls should be less than {sys.maxsize}."):
...
def test_audio_transcription_configs_are_not_shared_between_instances():
    ...
    assert config1.output_audio_transcription is not config2.output_audio_transcription
    ...
    assert config1.input_audio_transcription is not config2.input_audio_transcription
