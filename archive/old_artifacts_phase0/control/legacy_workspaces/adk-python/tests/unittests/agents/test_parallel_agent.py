...
async def _create_parent_invocation_context(test_name: str, agent: BaseAgent, is_resumable: bool = False) -> InvocationContext:
...
async def test_run_async(request: pytest.FixtureRequest, is_resumable: bool):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, parallel_agent, is_resumable=is_resumable)
...
async def test_resume_async_branches(request: pytest.FixtureRequest):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, parallel_agent, is_resumable=True)
    parent_ctx.agent_states[parallel_agent.name] = BaseAgentState().model_dump(mode="json")
    parent_ctx.agent_states[sequential_agent.name] = SequentialAgentState(current_sub_agent=agent3.name).model_dump(mode="json")
...
async def test_generating_one_event_per_agent_at_once(
    ...
    agent1 = _TestingAgentWithMultipleEvents(name=f"{request.function.__name__}_test_agent_1")
    agent2 = _TestingAgentWithMultipleEvents(name=f"{request.function.__name__}_test_agent_2")
    parallel_agent = ParallelAgent(
        name=f"{request.function.__name__}_test_parallel_agent",
        sub_agents=[
            agent1,
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, parallel_agent)
...
async def test_stop_agent_if_sub_agent_fails(
    ...
    agent1 = _TestingAgentWithException(name=f"{request.function.__name__}_test_agent_1")
    agent2 = _TestingAgentInfiniteEvents(name=f"{request.function.__name__}_test_agent_2")
    parallel_agent = ParallelAgent(
        name=f"{request.function.__name__}_test_parallel_agent",
        sub_agents=[
            agent1,
