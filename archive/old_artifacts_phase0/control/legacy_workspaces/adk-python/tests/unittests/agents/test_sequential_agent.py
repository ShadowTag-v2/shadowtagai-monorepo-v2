...
async def test_run_async(request: pytest.FixtureRequest):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, sequential_agent)
...
async def test_run_async_with_resumability(request: pytest.FixtureRequest):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, sequential_agent, resumable=True)
    events = [e async for e in sequential_agent.run_async(parent_ctx)]
...
async def test_resume_async(request: pytest.FixtureRequest):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, sequential_agent, resumable=True)
    parent_ctx.agent_states[sequential_agent.name] = SequentialAgentState(current_sub_agent=agent_2.name).model_dump(mode="json")
