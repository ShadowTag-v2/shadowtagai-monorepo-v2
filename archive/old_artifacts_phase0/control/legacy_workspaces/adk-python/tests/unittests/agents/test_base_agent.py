...
def _after_agent_callback_append_agent_reply(
    ...
    return types.Content(parts=[types.Part(text="Agent reply from after agent callback.")])
...
async def test_run_async(request: pytest.FixtureRequest):
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, agent)
...
async def test_run_async_with_branch(request: pytest.FixtureRequest):
    agent = _TestingAgent(name=f"{request.function.__name__}_test_agent")
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, agent, branch="parent_branch")
...
async def test_run_async_before_agent_callback_use_plugin(
    ...
    parent_ctx = await _create_parent_invocation_context(request.function.__name__, agent, plugins=[mock_plugin])
...
async def test_before_agent_callbacks_chain(
    ...
    for response, callback_type in callbacks:
        if callback_type == CallbackType.ASYNC:
            mock_cb = mock.AsyncMock(side_effect=partial(mock_async_agent_cb_side_effect, ret_value=response))
        else:
            mock_cb = mock.Mock(side_effect=partial(mock_sync_agent_cb_side_effect, ret_value=response))
    ...
    assert testing_utils.simplify_events(result) == [(f"{request.function.__name__}_test_agent", response) for response in expected_responses]
...
def test_find_agent(request: pytest.FixtureRequest):
    grand_sub_agent_1 = _TestingAgent(name=f"{request.function.__name__}__grand_sub_agent_1")
    grand_sub_agent_2 = _TestingAgent(name=f"{request.function.__name__}__grand_sub_agent_2")
