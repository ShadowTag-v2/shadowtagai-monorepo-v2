...
pytestmark = pytest.mark.skipif(sys.version_info < (3, 10), reason="A2A requires Python 3.10+")
...
try:
    ...
    from google.adk.agents.remote_a2a_agent import A2A_METADATA_PREFIX, AgentCardResolutionError, RemoteA2aAgent
...
class TestRemoteA2aAgentInit:
    ...
    def test_init_with_agent_card_object(self):
        ...
        agent = RemoteA2aAgent(name="test_agent", agent_card=agent_card, description="Test description")
    ...
    def test_init_with_url_string(self):
        ...
        agent = RemoteA2aAgent(name="test_agent", agent_card="https://example.com/agent.json")
...
class TestRemoteA2aAgentResolution:
    ...
    async def test_ensure_httpx_client_creates_new_client(self):
        ...
        agent = RemoteA2aAgent(name="test_agent", agent_card=create_test_agent_card())
    ...
    async def test_resolve_agent_card_from_url_success(self):
        ...
        with patch.object(agent, "_ensure_httpx_client") as mock_ensure_client:
            ...
            with patch("google.adk.agents.remote_a2a_agent.A2ACardResolver") as mock_resolver_class:
                mock_resolver = AsyncMock()
                mock_resolver.get_agent_card.return_value = self.agent_card
                mock_resolver_class.return_value = mock_resolver

                result = await agent._resolve_agent_card_from_url("https://example.com/agent.json")

                assert result == self.agent_card
                mock_resolver_class.assert_called_once_with(httpx_client=mock_client, base_url="https://example.com")
                mock_resolver.get_agent_card.assert_called_once_with(relative_card_path="/agent.json")
    ...
    async def test_resolve_agent_card_from_file_not_found(self):
        ...
        agent = RemoteA2aAgent(name="test_agent", agent_card="/path/to/nonexistent.json")

        with pytest.raises(AgentCardResolutionError, match="Agent card file not found"):
    ...
    async def test_validate_agent_card_no_url(self):
        ...
        with pytest.raises(AgentCardResolutionError, match="Agent card must have a valid URL"):
    ...
    async def test_ensure_resolved_with_direct_agent_card(self):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            with patch("google.adk.agents.remote_a2a_agent.A2AClientFactory") as mock_factory_class:
                mock_factory = Mock()
    ...
    async def test_ensure_resolved_with_url_source(self):
        ...
        with patch.object(agent, "_resolve_agent_card") as mock_resolve:
            ...
            with patch.object(agent, "_ensure_httpx_client") as mock_ensure_client:
                ...
                with patch("google.adk.agents.remote_a2a_agent.A2AClient") as mock_client_class:
...
class TestRemoteA2aAgentMessageHandling:
    ...
    def test_create_a2a_request_for_user_function_response_no_function_call(self):
        ...
        with patch("google.adk.agents.remote_a2a_agent.find_matching_function_call") as mock_find:
            mock_find.return_value = None

            result = self.agent._create_a2a_request_for_user_function_response(self.mock_context)
    ...
    def test_create_a2a_request_for_user_function_response_success(self):
        ...
        mock_function_event.custom_metadata = {A2A_METADATA_PREFIX + "task_id": "task-123"}
        ...
        with patch("google.adk.agents.remote_a2a_agent.find_matching_function_call") as mock_find:
            ...
            with patch("google.adk.agents.remote_a2a_agent.convert_event_to_a2a_message") as mock_convert:
    ...
    def test_construct_message_parts_from_session_success(self):
        ...
        with patch("google.adk.agents.remote_a2a_agent._present_other_agent_message") as mock_convert:
            mock_convert.return_value = mock_event

            mock_a2a_part = Mock()
            self.mock_genai_part_converter.return_value = mock_a2a_part

            parts, context_id = self.agent._construct_message_parts_from_session(self.mock_context)
    ...
    async def test_handle_a2a_response_success_with_message(self):
        ...
        with patch("google.adk.agents.remote_a2a_agent.convert_a2a_message_to_event") as mock_convert:
            mock_convert.return_value = mock_event

            result = await self.agent._handle_a2a_response(mock_a2a_message, self.mock_context)
    ...
    async def test_handle_a2a_response_with_task_completed_and_no_update(self):
        ...
        with patch("google.adk.agents.remote_a2a_agent.convert_a2a_task_to_event") as mock_convert:
            mock_convert.return_value = mock_event

            result = await self.agent._handle_a2a_response((mock_a2a_task, None), self.mock_context)
    ...
    def test_construct_message_parts_from_session_preserves_order(self):
        ...
        with patch("google.adk.agents.remote_a2a_agent._present_other_agent_message") as mock_present:
            ...
            assert converted_parts[2].original_text == "[other_agent] said: Response text"
    ...
    async def test_handle_a2a_response_with_task_status_update_with_message(self):
        ...
        with patch("google.adk.agents.remote_a2a_agent.convert_a2a_message_to_event") as mock_convert:
            ...
            result = await self.agent._handle_a2a_response((mock_a2a_task, mock_update), self.mock_context)
...
class TestRemoteA2aAgentMessageHandlingFromFactory:
    ...
    def test_construct_message_parts_from_session_success(self):
        ...
        with patch("google.adk.agents.remote_a2a_agent._present_other_agent_message") as mock_convert:
            ...
            with patch.object(self.agent, "_genai_part_converter") as mock_convert_part:
                mock_a2a_part = Mock()
...
class TestRemoteA2aAgentExecution:
    ...
    async def test_run_async_impl_no_message_parts(self):
        ...
        with (
            patch.object(self.agent, "_ensure_resolved"),
            patch.object(self.agent, "_create_a2a_request_for_user_function_response") as mock_create_func,
        ):
            mock_create_func.return_value = None

            with patch.object(self.agent, "_construct_message_parts_from_session") as mock_construct:
        ...
        with (
            ...
            with patch.object(self.agent, "_construct_message_parts_from_session") as mock_construct:
                ...
                with patch.object(self.agent, "_handle_a2a_response") as mock_handle:
                    ...
                    with (
                        patch("google.adk.agents.remote_a2a_agent.build_a2a_request_log") as mock_req_log,
                        patch("google.adk.agents.remote_a2a_agent.build_a2a_response_log") as mock_resp_log,
                    ):
                        mock_req_log.return_value = "Mock request log"
                        mock_resp_log.return_value = "Mock response log"

                        # Mock the A2AMessage constructor
                        with patch("google.adk.agents.remote_a2a_agent.A2AMessage") as mock_message_class:
                            ...
                            assert A2A_METADATA_PREFIX + "request" in mock_event.custom_metadata
    ...
    async def test_run_async_impl_a2a_client_error(self):
        ...
        with (
            ...
            with patch.object(self.agent, "_construct_message_parts_from_session") as mock_construct:
                ...
                # Mock the logging functions to avoid iteration issues
                with patch("google.adk.agents.remote_a2a_agent.build_a2a_request_log") as mock_req_log:
                    mock_req_log.return_value = "Mock request log"
    ...
    async def test_run_live_impl_not_implemented(self):
        ...
        with pytest.raises(NotImplementedError, match="_run_live_impl.*not implemented"):
    ...
    async def test_run_async_impl_with_meta_provider(self):
        ...
        with patch.object(agent, "_ensure_resolved"), patch.object(agent, "_create_a2a_request_for_user_function_response") as mock_create_func:
            mock_create_func.return_value = None

            with patch.object(agent, "_construct_message_parts_from_session") as mock_construct:
                # Create proper A2A part mocks
                from a2a.client import Client as A2AClient
                from a2a.types import TextPart
                ...
                with patch.object(agent, "_handle_a2a_response") as mock_handle:
                    ...
                    with (
                        ...
                        with patch("google.adk.agents.remote_a2a_agent.A2AMessage") as mock_message_class:
                            ...
                            assert len(events) == 1
                            mock_meta_provider.assert_called_once_with(self.mock_context, mock_message)
                ...
                with patch.object(self.agent, "_handle_a2a_response") as mock_handle:
                    ...
                    with (
                        ...
                        with patch("google.adk.agents.remote_a2a_agent.A2AMessage") as mock_message_class:
                            ...
                            # Add model_dump to mock_response for metadata
                            mock_response.root.model_dump.return_value = {"test": "response"}
...
class TestRemoteA2aAgentCleanup:
    ...
    async def test_cleanup_does_not_own_httpx_client_factory(self):
        ...
        agent = RemoteA2aAgent(
            name="test_agent",
            agent_card=self.agent_card,
            a2a_client_factory=ClientFactory(config=ClientConfig(httpx_client=shared_client)),
        )
...
class TestRemoteA2aAgentIntegration:
    ...
    async def test_full_workflow_with_direct_agent_card(self):
        ...
        with patch("google.adk.agents.remote_a2a_agent._present_other_agent_message") as mock_convert:
            ...
            with patch("google.adk.agents.remote_a2a_agent.convert_genai_part_to_a2a_part") as mock_convert_part:
                from a2a.types import TextPart
                ...
                with patch("httpx.AsyncClient") as mock_httpx_client_class:
                    ...
                    with patch.object(agent, "_a2a_client") as mock_a2a_client:
                        ...
                        with patch("google.adk.agents.remote_a2a_agent.convert_a2a_message_to_event") as mock_convert_event:
                            ...
                            with (
                                ...
                                assert A2A_METADATA_PREFIX + "request" in mock_result_event.custom_metadata
