...
class TestGeminiContextCacheManager:
    ...
    def create_llm_request(self, cache_metadata=None, contents_count=3):
        ...
        for i in range(contents_count):
            contents.append(types.Content(role="user", parts=[types.Part(text=f"Test message {i}")]))
                ...
                function_declarations=[
                    types.FunctionDeclaration(
                        ...
                        parameters=types.Schema(
                            ...
                            properties={"param": types.Schema(type=types.Type.STRING)},
                        ),
                    )
                ]
            )
    ...
    def create_cache_metadata(self, invocations_used=0, expired=False, contents_count=3):
    ...
    async def test_handle_context_caching_no_existing_cache(self):
        ...
        with patch.object(self.manager, "_generate_cache_fingerprint", return_value="test_fp"):
    ...
    async def test_handle_context_caching_valid_existing_cache(self):
        ...
        assert result.invocations_used == existing_cache.invocations_used  # Should preserve original invocations_used
        assert result.expire_time == existing_cache.expire_time  # Should preserve original expire_time
        assert result.fingerprint == existing_cache.fingerprint  # Should preserve original fingerprint
        assert result.created_at == existing_cache.created_at  # Should preserve original created_at
    ...
    async def test_handle_context_caching_invalid_cache_fingerprint_match(self):
        ...
        mock_cached_content.name = "projects/test/locations/us-central1/cachedContents/new456"
        self.manager.genai_client.aio.caches.create = AsyncMock(return_value=mock_cached_content)

        # Create request with invalid existing cache
        existing_cache = self.create_cache_metadata(invocations_used=15)  # Exceeds cache_intervals
        llm_request = self.create_llm_request(cache_metadata=existing_cache)
        llm_request.cacheable_contents_token_count = 2048  # Add token count for cache creation
    ...
    async def test_handle_context_caching_invalid_cache_fingerprint_mismatch(
        ...
        # Create request with invalid existing cache
        existing_cache = self.create_cache_metadata(invocations_used=15, contents_count=3)  # Exceeds cache_intervals
        llm_request = self.create_llm_request(cache_metadata=existing_cache, contents_count=5)
    ...
    async def test_is_cache_valid_fingerprint_only_metadata(self):
        ...
        assert result is False  # Fingerprint-only metadata is not a valid active cache

    async def test_is_cache_valid_cache_intervals_exceeded(self):
        """Test cache validation with max invocations exceeded."""
        cache_metadata = self.create_cache_metadata(invocations_used=15)  # Exceeds cache_intervals=10
        llm_request = self.create_llm_request(cache_metadata=cache_metadata)
    ...
    async def test_is_cache_valid_all_checks_pass(self):
        ...
        cache_metadata = self.create_cache_metadata(invocations_used=5)  # Within cache_intervals=10
        llm_request = self.create_llm_request(cache_metadata=cache_metadata)
    ...
    async def test_cleanup_cache(self):
        ...
        self.manager.genai_client.aio.caches.delete.assert_called_once_with(name=cache_name)
    ...
    def test_generate_cache_fingerprint(self):
        ...
        fingerprint1 = self.manager._generate_cache_fingerprint(llm_request, cache_contents_count)
        fingerprint2 = self.manager._generate_cache_fingerprint(llm_request, cache_contents_count)
        ...
        llm_request_no_tools = LlmRequest(
            ...
            config=types.GenerateContentConfig(system_instruction="Test instruction"),
            cache_config=self.cache_config,
        )

        fingerprint_no_tools = self.manager._generate_cache_fingerprint(llm_request_no_tools, cache_contents_count)
    ...
    def test_generate_cache_fingerprint_different_requests(self):
        ...
        llm_request2 = LlmRequest(
            model="gemini-2.0-flash",
            contents=[types.Content(role="user", parts=[types.Part(text="Different message")])],
            config=types.GenerateContentConfig(system_instruction="Different instruction"),
            cache_config=self.cache_config,
        )

        cache_contents_count = 2
        fingerprint1 = self.manager._generate_cache_fingerprint(llm_request1, cache_contents_count)
        fingerprint2 = self.manager._generate_cache_fingerprint(llm_request2, cache_contents_count)
    ...
    def test_generate_cache_fingerprint_tool_config_variations(self):
        ...
        tool_config_none = types.ToolConfig(function_calling_config=types.FunctionCallingConfig(mode="NONE"))
        ...
        cache_contents_count = 2
        fingerprint_auto = self.manager._generate_cache_fingerprint(llm_request_auto, cache_contents_count)
        fingerprint_none = self.manager._generate_cache_fingerprint(llm_request_none, cache_contents_count)
    ...
    async def test_populate_cache_metadata_in_response_no_invocations_increment(
        ...
        self.manager.populate_cache_metadata_in_response(llm_response, cache_metadata)
        ...
        assert updated_metadata.invocations_used == 3  # Should preserve original value
    ...
    async def test_create_new_cache_with_proper_ttl(self):
        ...
        mock_cached_content = AsyncMock()
        mock_cached_content.name = "projects/test/locations/us-central1/cachedContents/test123"
        self.manager.genai_client.aio.caches.create = AsyncMock(return_value=mock_cached_content)
    ...
    def test_edge_cases(self):
        ...
        cache_contents_count = 2
        fingerprint = self.manager._generate_cache_fingerprint(llm_request_no_config, cache_contents_count)
        ...
        fingerprint = self.manager._generate_cache_fingerprint(llm_request_empty, empty_cache_contents_count)
        assert isinstance(fingerprint, str)
    ...
    def test_parameter_types_enforcement(self):
        ...
        assert hasattr(llm_response, "usage_metadata")  # LlmResponse should have this
        assert not hasattr(cache_metadata, "usage_metadata")  # CacheMetadata should NOT have this

    def create_llm_request_with_token_count(self, token_count=None, cache_metadata=None):
