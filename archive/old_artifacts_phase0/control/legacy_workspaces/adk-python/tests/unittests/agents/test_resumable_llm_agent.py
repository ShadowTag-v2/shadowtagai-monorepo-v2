...
def transfer_call_part(agent_name: str) -> Part:
    return Part.from_function_call(name="transfer_to_agent", args={"agent_name": agent_name})


TRANSFER_RESPONSE_PART = Part.from_function_response(name="transfer_to_agent", response={"result": None})
...
async def _create_resumable_invocation_context(invocation_id: str, agent: BaseAgent, events: list[Event]) -> InvocationContext:
...
async def _resume_and_get_events(agent: BaseAgent, invocation_context: InvocationContext) -> list[(str, Part | str)]:
    events = []
...
class TestResumableLlmAgent:
    ...
    async def resumable_invocation_context(self):
        ...
        async def factory(agent: BaseAgent, events: list[Event]):
            return await _create_resumable_invocation_context(invocation_id="test_invocation", agent=agent, events=events)
    ...
    @pytest.mark.asyncio
    async def test_resume_from_transfer_call(self, resumable_invocation_context, mock_model):
    ...
    @pytest.mark.asyncio
    async def test_resume_from_transfer_response(self, resumable_invocation_context, mock_model):
        """Tests that the agent resumes from the correct sub-agent after a transfer."""
        sub_agent_1 = LlmAgent(
            name="sub_agent_1",
            model=mock_model(
        ...
        ctx: InvocationContext = await resumable_invocation_context(root_agent, past_events)
        # Initialize the agent state for the root agent.
        ctx.agent_states[root_agent.name] = BaseAgentState().model_dump(mode="json")
    ...
    @pytest.mark.asyncio
    async def test_resume_from_model_response(self, resumable_invocation_context, mock_model):
    ...
    @pytest.mark.asyncio
    async def test_resume_from_tool_call(self, resumable_invocation_context, mock_model):
    ...
    @pytest.mark.asyncio
    async def test_resume_after_tool_response(self, resumable_invocation_context, mock_model):
    ...
    @pytest.mark.asyncio
    async def test_resume_root_agent_on_user_provided_function_response(self, resumable_invocation_context, mock_model):
        ...
        ctx.agent_states[root_agent.name] = BaseAgentState().model_dump(mode="json")
        ctx.agent_states[sub_agent_1.name] = BaseAgentState().model_dump(mode="json")
    ...
    @pytest.mark.asyncio
    async def test_resume_subagent_on_user_provided_function_response(self, resumable_invocation_context, mock_model):
        """Tests that the agent resumes the correct sub-agent after a user responds to its tool call."""
