...
from google.adk.models.anthropic_llm import AnthropicLlm, Claude, content_to_message_param, function_declaration_to_tool_param
...
def generate_content_response():
    return anthropic_types.Message(
        id="msg_vrtx_testid",
        content=[anthropic_types.TextBlock(citations=None, text="Hi! How can I help you today?", type="text")],
...
async def test_function_declaration_to_tool_param(_, function_declaration, expected_tool_param):
    ...
    assert function_declaration_to_tool_param(function_declaration) == expected_tool_param


@pytest.mark.asyncio
async def test_generate_content_async(claude_llm, llm_request, generate_content_response, generate_llm_response):
    with (
        mock.patch.object(claude_llm, "_anthropic_client") as mock_client,
        mock.patch.object(
            anthropic_llm,
        ...
        responses = [resp async for resp in claude_llm.generate_content_async(llm_request, stream=False)]
...
@pytest.mark.asyncio
async def test_anthropic_llm_generate_content_async(llm_request, generate_content_response, generate_llm_response):
    anthropic_llm_instance = AnthropicLlm(model="claude-sonnet-4-20250514")
    with (
        mock.patch.object(anthropic_llm_instance, "_anthropic_client") as mock_client,
        mock.patch.object(
            anthropic_llm,
            "message_to_generate_content_response",
            return_value=generate_llm_response,
        ...
        responses = [resp async for resp in anthropic_llm_instance.generate_content_async(llm_request, stream=False)]
        assert len(responses) == 1
        assert isinstance(responses[0], LlmResponse)
        assert responses[0].content.parts[0].text == "Hello, how can I help you?"
...
async def test_generate_content_async_with_max_tokens(llm_request, generate_content_response, generate_llm_response):
    ...
    with (
        ...
        _ = [resp async for resp in claude_llm.generate_content_async(llm_request, stream=False)]
...
def test_part_to_message_block_with_content():
    ...
    assert '{"name":"root","node_type":"folder","children":[]}' in result["content"]
    ...
    (
        ...
        Content(
            ...
            parts=[
                ...
                Part(inline_data=types.Blob(mime_type="image/jpeg", data=b"fake_image_data")),
            ],
        ),
        "user",
    ...
    (
        ...
        Content(
            ...
            parts=[
                ...
                Part(inline_data=types.Blob(mime_type="image/png", data=b"fake_image_data")),
            ],
        ),
        "assistant",
    ...
    (
        ...
        Content(
            ...
            parts=[
                ...
                Part(inline_data=types.Blob(mime_type="image/webp", data=b"fake_image_data")),
            ],
        ),
        "assistant",
        1,  # Image filtered out, only text remains
...
def test_content_to_message_param_with_images(_, content, expected_role, expected_content_length, should_log_warning):
    ...
    with mock.patch("google.adk.models.anthropic_llm.logger") as mock_logger:
        ...
        if should_log_warning:
            mock_logger.warning.assert_called_once_with("Image data is not supported in Claude for assistant turns.")
        else:
