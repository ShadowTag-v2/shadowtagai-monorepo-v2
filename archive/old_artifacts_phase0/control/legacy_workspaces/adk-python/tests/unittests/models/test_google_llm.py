...
from google.adk.utils._client_labels_utils import _AGENT_ENGINE_TELEMETRY_ENV_VARIABLE_NAME, _AGENT_ENGINE_TELEMETRY_TAG
...
def llm_request_with_cache(cache_metadata):
    return LlmRequest(
        ...
        cache_config=ContextCacheConfig(cache_intervals=10, ttl_seconds=3600, min_tokens=100),
...
def test_supported_models():
    ...
    assert models[3] == r"projects\/.+\/locations\/.+\/publishers\/google\/models\/gemini.+"
...
def test_client_version_header():
    ...
    x_goog_api_client_header = client._api_client._http_options.headers["x-goog-api-client"]
    ...
    assert any(sdk in x_goog_api_client_header for sdk in ["google-genai-sdk/", "vertex-genai-modules/"])
    assert any(sdk in user_agent_header for sdk in ["google-genai-sdk/", "vertex-genai-modules/"])


def test_client_version_header_with_agent_engine(monkeypatch):
    monkeypatch.setenv(_AGENT_ENGINE_TELEMETRY_ENV_VARIABLE_NAME, "my_test_project")
    model = Gemini(model="gemini-1.5-flash")
    client = model.api_client

    # Check that ADK version with telemetry tag and Python version are present in
    # headers
    adk_version_with_telemetry = f"google-adk/{adk_version.__version__}+{_AGENT_ENGINE_TELEMETRY_TAG}"
    python_version_string = f"gl-python/{sys.version.split()[0]}"
...
def test_maybe_append_user_content(gemini_llm, llm_request):
    ...
    llm_request.contents.append(Content(role="model", parts=[Part.from_text(text="Response")]))
    gemini_llm._maybe_append_user_content(llm_request)
...
@pytest.mark.asyncio
async def test_generate_content_async(gemini_llm, llm_request, generate_content_response):
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        responses = [resp async for resp in gemini_llm.generate_content_async(llm_request, stream=False)]
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        mock_responses = [
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(role="model", parts=[Part.from_text(text=", how")]),
                        finish_reason=None,
                    )
                ]
            ),
...
@pytest.mark.asyncio
async def test_generate_content_async_stream_preserves_thinking_and_text_parts(gemini_llm, llm_request):
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
...
@pytest.mark.asyncio
async def test_generate_content_async_resource_exhausted_error(stream, gemini_llm, llm_request):
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        with pytest.raises(_ResourceExhaustedError) as excinfo:
            responses = []
            async for resp in gemini_llm.generate_content_async(llm_request, stream=stream):
...
@pytest.mark.parametrize("stream", [True, False])
@pytest.mark.asyncio
async def test_generate_content_async_other_client_error(stream, gemini_llm, llm_request):
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
...
@pytest.mark.asyncio
async def test_generate_content_async_with_custom_headers(gemini_llm, llm_request, generate_content_response):
...
@pytest.mark.asyncio
async def test_generate_content_async_stream_with_custom_headers(gemini_llm, llm_request):
...
@pytest.mark.parametrize("stream", [True, False])
@pytest.mark.asyncio
async def test_generate_content_async_patches_tracking_headers(stream, gemini_llm, llm_request, generate_content_response):
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        responses = [resp async for resp in gemini_llm.generate_content_async(llm_request, stream=stream)]
        ...
        assert final_config.http_options.headers["x-goog-api-client"] == gemini_llm._tracking_headers()["x-goog-api-client"]
...
def test_live_api_version_gemini_api(gemini_llm):
    ...
    with mock.patch.object(gemini_llm, "_api_backend", GoogleLLMVariant.GEMINI_API):
...
async def test_connect_with_custom_headers(gemini_llm, llm_request):
    ...
    llm_request.live_connect_config = types.LiveConnectConfig(http_options=types.HttpOptions(headers=custom_headers))
...
@pytest.mark.parametrize(
    ("api_backend, expected_file_display_name, expected_inline_display_name, expected_labels"),
    [
        (
...
async def test_preprocess_request_handles_backend_specific_fields(
    ...
    with mock.patch.object(Gemini, "_api_backend", new_callable=mock.PropertyMock) as mock_backend:
        ...
        for finish_reason in test_cases:
            mock_responses = [
                ...
                types.GenerateContentResponse(
                    candidates=[
                        types.Candidate(
                            content=Content(role="model", parts=[Part.from_text(text=" world")]),
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        mock_responses = [
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(role="model", parts=[Part(text="Think1", thought=True)]),
                        finish_reason=None,
                    )
                ]
            ),
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(
                            role="model",
                            parts=[Part(function_call=types.FunctionCall(name="test", args={}))],
                        ),
                        finish_reason=types.FinishReason.STOP,
                    )
                ]
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        mock_responses = [
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(role="model", parts=[Part.from_text(text="First text")]),
                        finish_reason=None,
                    )
                ]
            ),
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(
                            role="model",
                            parts=[Part(function_call=types.FunctionCall(name="test_func", args={}))],
                        ),
                        finish_reason=None,
                    )
                ]
        ...
        assert responses[1].partial is None  # Aggregated responses don't have partial flag
        ...
        assert responses[4].error_code is None  # STOP finish reason should have None error_code
...
async def test_generate_content_async_stream_complex_mixed_thought_text_function():
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(
                            role="model",
                            parts=[Part(function_call=types.FunctionCall(name="lookup", args={}))],
                        ),
                        finish_reason=None,
                    )
                ]
        ...
        aggregated_responses = [r for r in responses if r.partial is None and r.content and len(r.content.parts) > 1]
        assert len(aggregated_responses) > 0  # Should have at least one aggregated response with multiple parts

        # Final aggregated response should have both thought and text
        final_response = responses[-1]
        assert final_response.error_code is None  # STOP finish reason should have None error_code
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        mock_responses = [
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(role="model", parts=[Part.from_text(text="First")]),
                        finish_reason=None,
                    )
                ]
            ),
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(
                            role="model",
                            parts=[Part(function_call=types.FunctionCall(name="divide", args={}))],
                        ),
                        finish_reason=None,
                    )
                ]
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(role="model", parts=[Part.from_text(text="Second")]),
                        finish_reason=None,
                    )
                ]
            ),
        ...
        aggregated_text_responses = [
            r
            for r in responses
            if (r.partial is None and r.content and r.content.parts and r.content.parts[0].text and not r.content.parts[0].function_call)
        ]
        ...
        assert final_aggregation.error_code is None  # STOP finish reason should have None error_code

        # Verify the function call is preserved between aggregations
        function_call_responses = [r for r in responses if (r.content and r.content.parts and r.content.parts[0].function_call)]
        assert len(function_call_responses) == 1
        assert function_call_responses[0].content.parts[0].function_call.name == "divide"
...
async def test_computer_use_preserves_system_instruction_when_no_computer_use():
    ...
    llm_request = LlmRequest(
        ...
        config=types.GenerateContentConfig(
            ...
            tools=[types.Tool(function_declarations=[types.FunctionDeclaration(name="test", description="test")])],
        ),
    )
...
@pytest.mark.asyncio
async def test_generate_content_async_with_cache_metadata_integration(gemini_llm, llm_request_with_cache, cache_metadata):
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        with mock.patch("google.adk.models.gemini_context_cache_manager.GeminiContextCacheManager") as MockCacheManagerClass:
            mock_cache_manager = MockCacheManagerClass.return_value
            # Configure cache manager to handle context caching
            mock_cache_manager.handle_context_caching = AsyncMock(return_value=cache_metadata)

            responses = [resp async for resp in gemini_llm.generate_content_async(llm_request_with_cache, stream=False)]
            ...
            call_args = mock_cache_manager.populate_cache_metadata_in_response.call_args
            ...
            assert isinstance(first_arg, LlmResponse), (
                f"First parameter should be LlmResponse, got {type(first_arg)}. This indicates parameters are in wrong order."
            )
            assert isinstance(second_arg, CacheMetadata), (
                f"Second parameter should be CacheMetadata, got {type(second_arg)}. This indicates parameters are in wrong order."
            )
...
def test_build_function_declaration_log():
    ...
    func_decl1 = types.FunctionDeclaration(
        ...
        parameters=types.Schema(
            ...
            properties={"param1": types.Schema(type=types.Type.STRING, description="param1 desc")},
        ),
    ...
    assert log2 == ("test_func2: {'type': 'object', 'properties': {'param2': {'type': 'integer'}}} -> {'type': 'string'}")
...
def test_build_request_log_with_config_multiple_tool_types():
    ...
    assert "'system_instruction'" not in log_output.split("Contents:")[0].split("Config:")[1]
...
def test_build_request_log_fallback_to_repr_on_all_failures(monkeypatch):
    ...
    monkeypatch.setattr(types.GenerateContentConfig, "model_dump", mock_model_dump)
...
@pytest.mark.asyncio
async def test_connect_uses_gemini_speech_config_when_request_is_none(gemini_llm, llm_request):
    ...
    llm_request.live_connect_config = types.LiveConnectConfig()  # speech_config is None
...
@pytest.mark.asyncio
async def test_connect_uses_request_speech_config_when_gemini_is_none(gemini_llm, llm_request):
    ...
    llm_request.live_connect_config = types.LiveConnectConfig(speech_config=request_speech_config)
...
@pytest.mark.asyncio
async def test_connect_request_gemini_config_overrides_speech_config(gemini_llm, llm_request):
...
@pytest.mark.asyncio
async def test_connect_speech_config_remains_none_when_both_are_none(gemini_llm, llm_request):
