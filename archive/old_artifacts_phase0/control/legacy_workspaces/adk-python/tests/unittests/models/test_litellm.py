...
from litellm.types.utils import ChatCompletionDeltaToolCall, Choices, Delta, ModelResponse, StreamingChoices
from pydantic import BaseModel, Field

LLM_REQUEST_WITH_FUNCTION_DECLARATION = LlmRequest(
    contents=[types.Content(role="user", parts=[types.Part.from_text(text="Test prompt")])],
...
def test_get_completion_inputs_formats_pydantic_schema_for_litellm():
    llm_request = LlmRequest(config=types.GenerateContentConfig(response_schema=_StructuredOutput))
...
def test_to_litellm_response_format_handles_genai_schema_instance():
    ...
    assert formatted["type"] == "json_object"
    assert formatted["response_schema"] == schema_instance.model_dump(exclude_none=True, mode="json")
...
def test_schema_to_dict_filters_none_enum_values():
    ...
    nested_schema = types.Schema.model_construct(
        type=types.Type.OBJECT,
        properties={
            "status": types.Schema.model_construct(type=types.Type.STRING, enum=["READY", None, "DONE"]),
        },
    )
...
STREAM_WITH_EMPTY_CHUNK = [
    ...
    ModelResponse(choices=[StreamingChoices(finish_reason="tool_calls", delta=Delta())]),
]
...
class MockLLMClient(LiteLLMClient):
    ...
    async def acompletion(self, model, messages, tools, **kwargs):
        if kwargs.get("stream", False):
            ...
            return await self.acompletion_mock(model=model, messages=messages, tools=tools, **kwargs)

    def completion(self, model, messages, tools, stream, **kwargs):
        return self.completion_mock(model=model, messages=messages, tools=tools, stream=stream, **kwargs)
...
async def test_generate_content_async(mock_acompletion, lite_llm_instance):
    ...
    async for response in lite_llm_instance.generate_content_async(LLM_REQUEST_WITH_FUNCTION_DECLARATION):
        ...
        assert response.content.parts[1].function_call.args == {"test_arg": "test_value"}
    ...
    assert kwargs["tools"][0]["function"]["description"] == "Test function description"
    assert kwargs["tools"][0]["function"]["parameters"]["properties"]["test_arg"]["type"] == "string"


@pytest.mark.asyncio
async def test_generate_content_async_with_model_override(mock_acompletion, lite_llm_instance):
...
@pytest.mark.asyncio
async def test_generate_content_async_without_model_override(mock_acompletion, lite_llm_instance):
    llm_request = LlmRequest(
        model=None,
        contents=[types.Content(role="user", parts=[types.Part.from_text(text="Test prompt")])],
    )
...
@pytest.mark.asyncio
async def test_generate_content_async_adds_fallback_user_message(mock_acompletion, lite_llm_instance):
    llm_request = LlmRequest(
        contents=[
            types.Content(
                role="user",
    ...
    _, kwargs = mock_acompletion.call_args
    user_messages = [message for message in kwargs["messages"] if message["role"] == "user"]
    assert any(message.get("content") == "Handle the requests as specified in the System Instruction." for message in user_messages)
    assert sum(1 for content in llm_request.contents if content.role == "user") == 1
    assert llm_request.contents[-1].parts[0].text == ("Handle the requests as specified in the System Instruction.")
...
@pytest.mark.parametrize("llm_request, expected_output", litellm_append_user_content_test_cases)
            ...
            parameters=types.Schema(
                ...
                properties={
                    "array_arg": types.Schema(
                        ...
                        items=types.Schema(
                            type=types.Type.OBJECT,
                            properties={"nested_key": types.Schema(type=types.Type.STRING)},
                        ),
                    ),
                },
            ),
...
def test_function_declaration_to_tool_param(_, function_declaration, expected_output):
    assert _function_declaration_to_tool_param(function_declaration) == expected_output
...
@pytest.mark.asyncio
async def test_generate_content_async_with_system_instruction(lite_llm_instance, mock_acompletion):
    ...
    llm_request = LlmRequest(
        contents=[types.Content(role="user", parts=[types.Part.from_text(text="Test prompt")])],
        config=types.GenerateContentConfig(system_instruction="Test system instruction"),
    )
...
@pytest.mark.asyncio
async def test_generate_content_async_with_tool_response(lite_llm_instance, mock_acompletion):
...
@pytest.mark.asyncio
async def test_generate_content_async_with_usage_metadata(lite_llm_instance, mock_acompletion):
...
def test_content_to_message_param_user_message_with_file_uri(file_uri, mime_type):
...
@pytest.mark.parametrize("file_uri,mime_type", FILE_URI_TEST_CASES)
def test_content_to_message_param_user_message_file_uri_only(file_uri, mime_type):
    file_part = types.Part.from_uri(file_uri=file_uri, mime_type=mime_type)
    content = types.Content(
        role="user",
        parts=[
...
def test_content_to_message_param_assistant_message():
    content = types.Content(role="assistant", parts=[types.Part.from_text(text="Test response")])
    message = _content_to_message_param(content)
...
def test_content_to_message_param_function_call():
    content = types.Content(
        ...
        parts=[
            ...
            types.Part.from_function_call(name="test_function", args={"test_arg": "test_value"}),
        ],
    )
...
def test_message_to_generate_content_response_tool_call():
    ...
    assert response.content.parts[0].function_call.args == {"test_arg": "test_value"}
...
def test_message_to_generate_content_response_inline_tool_call_text():
    message = ChatCompletionAssistantMessage(
        role="assistant",
        content=('{"id":"inline_call","name":"get_current_time","arguments":{"timezone_str":"Asia/Taipei"}} <|im_end|>system'),
    )
...
def test_message_to_generate_content_response_with_model():
    ...
    response = _message_to_generate_content_response(message, model_version="gemini-2.5-pro")
    assert response.content.role == "model"
    assert response.content.parts[0].text == "Test response"
...
def test_parse_tool_calls_from_text_multiple_calls():
    ...
    assert json.loads(tool_calls[1].function.arguments) == {"timezone": "Asia/Taipei"}
...
def test_split_message_content_and_tool_calls_inline_text():
    message = {
        "role": "assistant",
        "content": ('Intro {"name":"alpha","arguments":{"value":1}} trailing content'),
    }
...
def test_get_content_text_inline_data_single_part():
    parts = [types.Part.from_bytes(data=b"Inline text", mime_type="text/plain")]
    content = _get_content(parts)
...
def test_get_content_text_inline_data_multiple_parts():
    parts = [
        types.Part.from_bytes(data=b"First part", mime_type="text/plain"),
...
def test_get_content_image():
    ...
    assert content[0]["image_url"]["url"] == "data:image/png;base64,dGVzdF9pbWFnZV9kYXRh"
    assert "format" not in content[0]["image_url"]


def test_get_content_video():
    parts = [types.Part.from_bytes(data=b"test_video_data", mime_type="video/mp4")]
    content = _get_content(parts)
    assert content[0]["type"] == "video_url"
    assert content[0]["video_url"]["url"] == "data:video/mp4;base64,dGVzdF92aWRlb19kYXRh"
    assert "format" not in content[0]["video_url"]


@pytest.mark.parametrize("file_data,mime_type,expected_base64", FILE_BYTES_TEST_CASES)
...
def test_get_content_audio():
    parts = [types.Part.from_bytes(data=b"test_audio_data", mime_type="audio/mpeg")]
    content = _get_content(parts)
    assert content[0]["type"] == "audio_url"
    assert content[0]["audio_url"]["url"] == "data:audio/mpeg;base64,dGVzdF9hdWRpb19kYXRh"
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            UsageMetadataChunk(prompt_tokens=0, completion_tokens=0, total_tokens=0),
            "stop",
        ),
        (
            ModelResponse(
            ...
            [TextChunk(text="this is a test")],
            UsageMetadataChunk(prompt_tokens=3, completion_tokens=5, total_tokens=8),
            "stop",
        ),
        (
            ModelResponse(
        ...
        (
                ...
                choices=[
                    {
                        ...
                        "message": {
                            "role": "assistant",
                            "content": ('{"id":"call_1","name":"get_current_time","arguments":{"timezone_str":"Asia/Taipei"}}'),
                        },
                    }
                ],
                usage={
            ...
            UsageMetadataChunk(prompt_tokens=7, completion_tokens=9, total_tokens=16),
            "tool_calls",
        ),
        (
            ModelResponse(
                choices=[
                    {
                        ...
                        "message": {
                            "role": "assistant",
                            "content": ('Intro {"id":"call_2","name":"alpha","arguments":{"foo":"bar"}} wrap'),
                        },
                    }
                ],
                usage={
            ...
            UsageMetadataChunk(prompt_tokens=11, completion_tokens=13, total_tokens=24),
            "tool_calls",
        ),
    ],
)
def test_model_response_to_chunk(response, expected_chunks, expected_usage_chunk, expected_finished):
...
async def test_acompletion_with_drop_params(mock_acompletion, mock_client):
    lite_llm_instance = LiteLlm(model="test_model", llm_client=mock_client, drop_params=True)

    async for _ in lite_llm_instance.generate_content_async(LLM_REQUEST_WITH_FUNCTION_DECLARATION):
        pass
...
async def test_completion_additional_args(mock_completion, mock_client):
    ...
    responses = [response async for response in lite_llm_instance.generate_content_async(LLM_REQUEST_WITH_FUNCTION_DECLARATION, stream=True)]
...
@pytest.mark.asyncio
async def test_generate_content_async_stream(mock_completion, lite_llm_instance):
    ...
    assert responses[3].content.parts[-1].function_call.args == {"test_arg": "test_value"}
...
@pytest.mark.asyncio
async def test_generate_content_async_stream_with_usage_metadata(mock_completion, lite_llm_instance):
    ...
    mock_completion.return_value = iter(streaming_model_response_with_usage_metadata)
...
@pytest.mark.asyncio
async def test_generate_content_async_multiple_function_calls(mock_completion, lite_llm_instance):
    ...
    responses = []
    async for response in lite_llm_instance.generate_content_async(llm_request, stream=True):
...
@pytest.mark.asyncio
async def test_generate_content_async_non_compliant_multiple_function_calls(mock_completion, lite_llm_instance):
...
@pytest.mark.asyncio
async def test_generate_content_async_stream_with_empty_chunk(mock_completion, lite_llm_instance):
...
@pytest.mark.asyncio
async def test_finish_reason_unknown_maps_to_other(mock_acompletion, lite_llm_instance):
