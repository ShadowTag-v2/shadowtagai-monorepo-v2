...
def test_llm_response_create_with_logprobs():
    ...
    avg_logprobs = -0.75
    logprobs_result = types.LogprobsResult(chosen_candidates=[], top_candidates=[])
...
def test_llm_response_create_with_concrete_logprobs_result():
    ...
    chosen_candidates = [
        types.LogprobsResultCandidate(token="The", log_probability=-0.1, token_id=123),
        types.LogprobsResultCandidate(token=" capital", log_probability=-0.5, token_id=456),
        types.LogprobsResultCandidate(token=" of", log_probability=-0.2, token_id=789),
    ]
    ...
    top_candidates = [
        types.LogprobsResultTopCandidates(
            candidates=[
                types.LogprobsResultCandidate(token="The", log_probability=-0.1, token_id=123),
                types.LogprobsResultCandidate(token="A", log_probability=-2.3, token_id=124),
                types.LogprobsResultCandidate(token="This", log_probability=-3.1, token_id=125),
            ]
        ),
        types.LogprobsResultTopCandidates(
            candidates=[
                types.LogprobsResultCandidate(token=" capital", log_probability=-0.5, token_id=456),
                types.LogprobsResultCandidate(token=" city", log_probability=-1.2, token_id=457),
                types.LogprobsResultCandidate(token=" main", log_probability=-2.8, token_id=458),
            ]
        ),
    ]

    avg_logprobs = -0.27  # Average of -0.1, -0.5, -0.2
    logprobs_result = types.LogprobsResult(chosen_candidates=chosen_candidates, top_candidates=top_candidates)

    generate_content_response = types.GenerateContentResponse(
        candidates=[
            types.Candidate(
                content=types.Content(parts=[types.Part(text="The capital of France is Paris.")]),
                finish_reason=types.FinishReason.STOP,
                avg_logprobs=avg_logprobs,
                logprobs_result=logprobs_result,
            )
    ...
    assert len(response.logprobs_result.top_candidates[0].candidates) == 3  # 3 alternatives for first token
    assert response.logprobs_result.top_candidates[0].candidates[0].token == "The"
    assert response.logprobs_result.top_candidates[0].candidates[0].token_id == 123
    assert response.logprobs_result.top_candidates[0].candidates[1].token == "A"
    assert response.logprobs_result.top_candidates[0].candidates[1].token_id == 124
    assert response.logprobs_result.top_candidates[0].candidates[2].token == "This"
    assert response.logprobs_result.top_candidates[0].candidates[2].token_id == 125
...
def test_llm_response_create_with_partial_logprobs_result():
    ...
    chosen_candidates = [
        types.LogprobsResultCandidate(token="Hello", log_probability=-0.05, token_id=111),
        types.LogprobsResultCandidate(token=" world", log_probability=-0.8, token_id=222),
    ]
