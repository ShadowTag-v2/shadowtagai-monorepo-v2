...
async def test_construct_payload_basic_payload(client, llm_request):
    ...
    with mock.patch.object(httpx.AsyncClient, "post", return_value=mock_response) as mock_post:
        _ = [r async for r in client.generate_content_async(llm_request, stream=False)]
...
async def test_construct_payload_system_instruction(client, llm_request):
    llm_request.config = types.GenerateContentConfig(system_instruction="You are a helpful assistant.")
    mock_response = AsyncMock(spec=httpx.Response)
    mock_response.json.return_value = {"choices": [{"message": {"role": "assistant", "content": "Hi"}}]}
    mock_response.status_code = 200
...
async def test_construct_payload_multimodal_content(client):
    ...
    mock_response = AsyncMock(spec=httpx.Response)
    mock_response.json.return_value = {"choices": [{"message": {"role": "assistant", "content": "It is an image"}}]}
    ...
    with mock.patch.object(httpx.AsyncClient, "post", return_value=mock_response) as mock_post:
        ...
        assert message["content"][1]["image_url"]["url"] == ("data:image/jpeg;base64,ZmFrZV9pbWFnZV9ieXRlcw==")
...
@pytest.mark.asyncio
async def test_generate_content_async_function_call_response(client, llm_request):
    ...
    with mock.patch.object(httpx.AsyncClient, "post", return_value=mock_response):
        responses = [r async for r in client.generate_content_async(llm_request, stream=False)]
...
async def test_construct_payload_response_format(
    ...
    mock_response = AsyncMock(spec=httpx.Response)
    mock_response.json.return_value = {"choices": [{"message": {"role": "assistant", "content": "{}"}}]}
    mock_response.status_code = 200
...
@pytest.mark.asyncio
async def test_generate_content_async_invalid_tool_call_type_raises_error(client, llm_request):
...
async def test_generate_content_async_streaming_function_call():
    ...
    with mock.patch.object(httpx.AsyncClient, "stream", return_value=mock_stream_ctx):
        responses = [r async for r in local_client.generate_content_async(llm_request, stream=True)]
        ...
        assert responses[1].content.parts[0].function_call.args == {"location": "London"}
...
async def test_generate_content_async_streaming_multiple_function_calls():
    ...
    chunk_data_3 = {"choices": [{"index": 0, "delta": {}, "finish_reason": "tool_calls"}]}
...
async def test_generate_content_async_streaming_parse_lines(chunks, expected_response_count):
    local_client = CompletionsHTTPClient(base_url="https://localhost")
    llm_request = LlmRequest(
        model="apigee/test",
        contents=[types.Content(role="user", parts=[types.Part.from_text(text="hi")])],
