...
async def test_preprocess_request_with_tools(llm_request_with_tools):
    ...
    assert """{"description":"Search the web for a query.","name":"search_web",""" in sys_instruct_text
    assert """{"description":"Gets the current time.","name":"get_current_time","parameters":{"properties":{}""" in sys_instruct_text


@pytest.mark.asyncio
async def test_preprocess_request_with_function_response():
    # Simulate an LlmRequest with a function response
    func_response_data = types.FunctionResponse(name="search_web", response={"results": [{"title": "ADK"}]})
    ...
    assert llm_request.contents[0].parts[0].text == 'Invoking tool `search_web` produced: `{"results": [{"title": "ADK"}]}`.'
...
async def test_preprocess_request_with_function_call():
    ...
    llm_request = LlmRequest(
        model="gemma-3-1b-it",
        contents=[types.Content(role="user", parts=[types.Part(function_call=func_call_data)])],
    )
...
async def test_preprocess_request_with_mixed_content():
    ...
    func_response = types.FunctionResponse(name="get_weather", response={"temp": "15C"})
    ...
    assert llm_request.contents[1].parts[0].text == func_call.model_dump_json(exclude_none=True)
    ...
    assert llm_request.contents[2].parts[0].text == 'Invoking tool `get_weather` produced: `{"temp": "15C"}`.'
...
def test_process_response():
    ...
    json_function_call_str = '{"name": "search_web", "parameters": {"query": "latest news"}}'
    llm_response = LlmResponse(content=Content(role="model", parts=[Part.from_text(text=json_function_call_str)]))
    ...
    expected_function_call = types.FunctionCall(name="search_web", args={"query": "latest news"})
...
def test_process_response_invalid_json_text():
    ...
    llm_response = LlmResponse(content=Content(role="model", parts=[Part.from_text(text=original_text)]))
...
def test_process_response_malformed_json():
    ...
    llm_response = LlmResponse(content=Content(role="model", parts=[Part.from_text(text=malformed_json_str)]))
    gemma = Gemma()
    gemma._extract_function_calls_from_response(llm_response=llm_response)
...
def test_process_response_empty_content_or_multiple_parts():
    ...
    gemma._extract_function_calls_from_response(llm_response=llm_response_no_content)
    assert llm_response_no_content.content is None

    # Test case 2: LlmResponse with empty parts list
    llm_response_empty_parts = LlmResponse(content=Content(role="model", parts=[]))
    gemma._extract_function_calls_from_response(llm_response=llm_response_empty_parts)
    ...
    original_parts = list(llm_response_multiple_parts.content.parts)  # Copy for comparison
    gemma._extract_function_calls_from_response(llm_response=llm_response_multiple_parts)
    assert llm_response_multiple_parts.content
    assert llm_response_multiple_parts.content.parts == original_parts  # Should remain unchanged

    # Test case 4: LlmResponse with one part, but empty text
    llm_response_empty_text_part = LlmResponse(content=Content(role="model", parts=[Part.from_text(text="")]))
    gemma._extract_function_calls_from_response(llm_response=llm_response_empty_text_part)
...
def test_process_response_with_embedded_json():
    ...
    embedded_json_str = 'Please call the tool: {"name": "search_web", "parameters": {"query": "new features"}} thanks!'
    llm_response = LlmResponse(content=Content(role="model", parts=[Part.from_text(text=embedded_json_str)]))
...
def test_process_response_flexible_parsing():
    ...
    llm_response = LlmResponse(content=Content(role="model", parts=[Part.from_text(text=flexible_json_str)]))
...
def test_process_response_last_json_object():
    ...
    llm_response = LlmResponse(content=Content(role="model", parts=[Part.from_text(text=multiple_json_str)]))
