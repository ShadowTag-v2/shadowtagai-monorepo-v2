...
async def test_generate_content_async_non_streaming(mock_client_constructor, llm_request):
...
@pytest.mark.asyncio
@mock.patch("google.genai.Client")
async def test_generate_content_async_streaming(mock_client_constructor, llm_request):
    ...
    mock_client_instance.aio.models.generate_content_stream = AsyncMock(return_value=mock_stream_generator())
    mock_client_constructor.return_value = mock_client_instance

    response_generator = apigee_llm_instance.generate_content_async(llm_request, stream=True)
    responses = [resp async for resp in response_generator]
...
@pytest.mark.asyncio
@mock.patch("google.genai.Client")
async def test_generate_content_async_with_custom_headers(mock_client_constructor, llm_request):
...
async def test_proxy_url_from_env_variable(mock_client_constructor):
    ...
    with mock.patch.dict(os.environ, {"APIGEE_PROXY_URL": "https://env.proxy.url"}):
...
def test_vertex_model_missing_project_or_location_raises_error(model_string, env_vars):
    """Tests that ValueError is raised for Vertex models if project or location is missing."""
    with mock.patch.dict(os.environ, env_vars, clear=True), pytest.raises(ValueError, match="environment variable must be set"):
...
async def test_model_string_parsing_and_client_initialization(
    ...
    with mock.patch.dict(os.environ, env_vars, clear=True):
        ...
        mock_client_instance = mock.Mock()
        mock_client_instance.aio.models.generate_content = AsyncMock(
            return_value=types.GenerateContentResponse(candidates=[types.Candidate(content=Content(parts=[Part.from_text(text="")]))])
        )
        mock_client_constructor.return_value = mock_client_instance
...
async def test_invalid_model_strings_raise_value_error(invalid_model_string):
    ...
    with pytest.raises(ValueError, match=f"Invalid model string: {invalid_model_string}"):
