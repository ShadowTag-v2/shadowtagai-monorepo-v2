...
class MockAgent(BaseAgent):
    ...
    async def _run_async_impl(self, invocation_context):
        yield Event(
            ...
            author=self.name,
            content=types.Content(role="model", parts=[types.Part(text="Test response")]),
        )
...
class MockLlmAgent(LlmAgent):
    ...
    async def _run_async_impl(self, invocation_context):
        yield Event(
            invocation_id=invocation_context.invocation_id,
            author=self.name,
            content=types.Content(role="model", parts=[types.Part(text="Test LLM response")]),
        )
...
async def test_runner_allows_nested_agent_directories(tmp_path, monkeypatch):
        ...
        session = Session(
            ...
            events=[
                Event(
                    invocation_id="inv1",
                    author="root_agent",
                    content=types.Content(role="model", parts=[types.Part(text="Root response")]),
                )
            ],
        )
        ...
        session = Session(
            ...
            events=[
                Event(
                    invocation_id="inv1",
                    author="sub_agent1",
                    content=types.Content(role="model", parts=[types.Part(text="Sub agent response")]),
                )
            ],
        )
    ...
    def test_find_agent_to_run_function_response_takes_precedence(self):
        ...
        function_response = types.FunctionResponse(id="func_456", name="test_func", response={})
    ...
    def test_is_transferable_across_agent_tree_with_non_transferable_agent(self):
        ...
        result = self.runner._is_transferable_across_agent_tree(self.non_transferable_agent)
...
class TestRunnerWithPlugins:
    ...
    async def run_test(self, original_user_input="Hello") -> list[Event]:
        ...
        await self.session_service.create_session(app_name=TEST_APP_ID, user_id=TEST_USER_ID, session_id=TEST_SESSION_ID)
        events = []
        async for event in self.runner.run_async(
            user_id=TEST_USER_ID,
            session_id=TEST_SESSION_ID,
            new_message=types.Content(role="user", parts=[types.Part(text=original_user_input)]),
        ):
            events.append(event)
        return events
    ...
    async def test_runner_modifies_user_message_before_execution(self):
        ...
        session = await self.session_service.get_session(app_name=TEST_APP_ID, user_id=TEST_USER_ID, session_id=TEST_SESSION_ID)
        ...
        assert self.plugin.user_content_seen_in_before_run_callback.parts[0].text == MockPlugin.ON_USER_CALLBACK_MSG
...
class TestRunnerCacheConfig:
    ...
    def test_runner_extracts_cache_config_from_app(self):
        ...
        cache_config = ContextCacheConfig(cache_intervals=15, ttl_seconds=3600, min_tokens=1024)
    ...
    def test_runner_with_app_without_cache_config(self):
        ...
        app = App(name="test_app", root_agent=self.root_agent, context_cache_config=None)
    ...
    def test_runner_cache_config_passed_to_invocation_context(self):
        ...
        cache_config = ContextCacheConfig(cache_intervals=20, ttl_seconds=7200, min_tokens=2048)
    ...
    def test_runner_validate_params_return_order(self):
        ...
        app_name, agent, context_cache_config, resumability_config, plugins = runner._validate_runner_params(app, None, None, None)
    ...
    def test_runner_validate_params_without_app(self):
        ...
        app_name, agent, context_cache_config, resumability_config, plugins = runner._validate_runner_params(None, "test_app", self.root_agent, None)
    ...
    def test_runner_realistic_cache_config_scenario(self):
        ...
        expected_str = "ContextCacheConfig(cache_intervals=30, ttl=14400s, min_tokens=4096)"
...
class TestRunnerShouldAppendEvent:
    ...
    def test_should_not_append_event_live_model_audio(self):
        event = Event(
            invocation_id="inv1",
            author="model",
            content=types.Content(parts=[types.Part(inline_data=types.Blob(data=b"123", mime_type="audio/pcm"))]),
        )
