...
async def test_resume_invocation_from_sub_agent():
    ...
    root_agent = LlmAgent(
        name="root_agent",
        model=testing_utils.MockModel.create(responses=[transfer_call_part(sub_agent.name)]),
    ...
    assert testing_utils.simplify_resumable_app_events(copy.deepcopy(invocation_1_events)) == [
        (
            root_agent.name,
    ...
    assert testing_utils.simplify_resumable_app_events(copy.deepcopy(invocation_2_events)) == [
        (
            sub_agent.name,
            "second response from sub_agent",
        ),
    ...
    # already final.
    assert not await runner.run_async(invocation_id=invocation_2_events[0].invocation_id)
    ...
    session = await runner.runner.session_service.get_session(app_name="test_app", user_id="test_user", session_id=session_id)
    new_session = await runner.runner.session_service.create_session(app_name=session.app_name, user_id=session.user_id)
    ...
    resumed_invocation_2_events = await runner.run_async(invocation_id=invocation_2_events[0].invocation_id)
    assert testing_utils.simplify_resumable_app_events(copy.deepcopy(resumed_invocation_2_events)) == [
        (
            sub_agent.name,
            "third response from sub_agent",
        ),
...
async def test_resume_any_invocation():
    ...
    assert testing_utils.simplify_resumable_app_events(copy.deepcopy(invocation_1_events)) == [
        ...
        (
            root_agent.name,
            Part.from_function_response(name="test_tool", response={"result": "test tool result"}),
        ),
    ]
    ...
    assert testing_utils.simplify_resumable_app_events(copy.deepcopy(invocation_3_events)) == [
        (
            root_agent.name,
            Part.from_function_call(name="test_tool", args={}),
        ),
    ...
    assert testing_utils.simplify_resumable_app_events(copy.deepcopy(resumed_invocation_1_events)) == [
        (
            root_agent.name,
            "llm response after resuming invocation 1",
        ),
