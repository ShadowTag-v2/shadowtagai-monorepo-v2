...
from google.adk.agents.sequential_agent import SequentialAgent, SequentialAgentState
from google.adk.apps.app import App, ResumabilityConfig
from google.adk.flows.llm_flows.functions import REQUEST_CONFIRMATION_FUNCTION_CALL_NAME
from google.adk.tools.function_tool import FunctionTool
from google.adk.tools.tool_context import ToolContext
from google.genai.types import FunctionCall, FunctionResponse, GenerateContentResponse, Part

from .. import testing_utils

HINT_TEXT = "Please approve or reject the tool call _test_function() by responding with a FunctionResponse with an expected ToolConfirmation payload."

TOOL_CALL_ERROR_RESPONSE = {"error": "This tool call requires confirmation, please approve or reject."}
...
def _create_llm_response_from_tools(
    ...
    parts = [Part(function_call=FunctionCall(name=tool.name, args={})) for tool in tools]
...
def _create_llm_response_from_text(text: str) -> GenerateContentResponse:
    ...
    return testing_utils.LlmResponse(content=testing_utils.ModelContent(parts=[Part(text=text)]))
...
class TestHITLConfirmationFlowWithSingleAgent(BaseHITLTest):
    ...
    @pytest.fixture
    def llm_responses(self, tools: list[FunctionTool]) -> list[GenerateContentResponse]:
    ...
    @pytest.fixture
    def mock_model(self, llm_responses: list[GenerateContentResponse]) -> testing_utils.MockModel:
        """Provides a mock model with predefined responses."""
        return testing_utils.MockModel(responses=llm_responses)

    @pytest.fixture
    def agent(self, mock_model: testing_utils.MockModel, tools: list[FunctionTool]) -> LlmAgent:
    ...
    async def test_confirmation_flow(
        ...
        expected_parts = [
            ...
            (
                agent.name,
                Part(function_response=FunctionResponse(name=tools[0].name, response=TOOL_CALL_ERROR_RESPONSE)),
            ),
        ]
        ...
        ask_for_confirmation_function_call_id = events[1].content.parts[0].function_call.id
        ...
        expected_parts_final = [
            (
                ...
                Part(
                    function_response=FunctionResponse(
                        name=tools[0].name,
                        response={"result": f"confirmed={tool_call_confirmed}"} if tool_call_confirmed else {"error": "This tool call is rejected."},
                    )
                ),
            ),
        ...
        assert testing_utils.simplify_events(copy.deepcopy(events)) == expected_parts_final
...
class TestHITLConfirmationFlowWithCustomPayloadSchema(BaseHITLTest):
    ...
    def tools(self) -> list[FunctionTool]:
        """Provides the tools for the agent."""
        return [FunctionTool(func=_test_request_confirmation_function_with_custom_schema)]
    ...
    def llm_responses(self, tools: list[FunctionTool]) -> list[GenerateContentResponse]:
        ...
        return [
            _create_llm_response_from_tools(tools),
            _create_llm_response_from_text("test llm response after tool call"),
            _create_llm_response_from_text("test llm response after final tool call"),
        ]
...
class TestHITLConfirmationFlowWithResumableApp:
    ...
    async def test_pause_and_resume_on_request_confirmation(
        ...
        assert testing_utils.simplify_resumable_app_events(copy.deepcopy(events)) == [
            (
                agent.name,
            ...
            (
                agent.name,
                Part(function_response=FunctionResponse(name=agent.tools[0].name, response=TOOL_CALL_ERROR_RESPONSE)),
            ),
        ]
        ask_for_confirmation_function_call_id = events[1].content.parts[0].function_call.id
        invocation_id = events[1].invocation_id
        ...
        events = await runner.run_async(user_confirmation, invocation_id=invocation_id)
        expected_parts_final = [
            (
                agent.name,
                Part(
        ...
        for event in events:
            assert event.invocation_id == invocation_id
        assert testing_utils.simplify_resumable_app_events(copy.deepcopy(events)) == expected_parts_final
...
class TestHITLConfirmationFlowWithSequentialAgentAndResumableApp:
    ...
    @pytest.fixture
    def agent(self, mock_model: testing_utils.MockModel, tools: list[FunctionTool]) -> SequentialAgent:
        """Provides a single LlmAgent for the test."""
    ...
    async def test_pause_and_resume_on_request_confirmation(
        ...
        assert testing_utils.simplify_resumable_app_events(copy.deepcopy(events)) == [
            (
                agent.name,
                SequentialAgentState(current_sub_agent=sub_agent1.name).model_dump(mode="json"),
            ),
            (
                sub_agent1.name,
                Part(function_call=FunctionCall(name=sub_agent1.tools[0].name, args={})),
            ),
            (
                sub_agent1.name,
                Part(
        ...
        ask_for_confirmation_function_call_id = events[2].content.parts[0].function_call.id
        ...
        expected_parts_final = [
            ...
            (
                agent.name,
                SequentialAgentState(current_sub_agent=sub_agent2.name).model_dump(mode="json"),
            ),
...
class TestHITLConfirmationFlowWithParallelAgentAndResumableApp:
    ...
    async def test_pause_and_resume_on_request_confirmation(
        ...
        sub_agent1_branch_events = [event for event in events if event.branch == f"{agent.name}.{sub_agent1.name}"]
        sub_agent2_branch_events = [event for event in events if event.branch == f"{agent.name}.{sub_agent2.name}"]
        assert testing_utils.simplify_resumable_app_events(copy.deepcopy(root_agent_events)) == [
            (
                agent.name,
                BaseAgentState().model_dump(mode="json"),
            ),
        ]
        assert testing_utils.simplify_resumable_app_events(copy.deepcopy(sub_agent1_branch_events)) == [
            (
                sub_agent1.name,
                Part(function_call=FunctionCall(name=sub_agent1.tools[0].name, args={})),
            ),
        ...
        assert testing_utils.simplify_resumable_app_events(copy.deepcopy(sub_agent2_branch_events)) == [
            (
                sub_agent2.name,
                Part(function_call=FunctionCall(name=sub_agent2.tools[0].name, args={})),
            ),
            (
                sub_agent2.name,
                Part(
        ...
        assert sub_agent1_branch_events[1].invocation_id == sub_agent2_branch_events[1].invocation_id
        ...
        events = await runner.run_async(user_confirmations[0], invocation_id=invocation_id)
        for event in events:
            assert event.invocation_id == invocation_id
        ...
        events = await runner.run_async(user_confirmations[1], invocation_id=invocation_id)
        for event in events:
            assert event.invocation_id == invocation_id
