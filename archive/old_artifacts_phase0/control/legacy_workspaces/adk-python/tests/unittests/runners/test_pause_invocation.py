...
class _TestingAgent(BaseAgent):
    ...
    def event(self, ctx: InvocationContext):
        return Event(
            ...
            content=testing_utils.ModelContent(parts=[Part.from_text(text="Delayed message")]),
        )
...
_TRANSFER_RESPONSE_PART = Part.from_function_response(name="transfer_to_agent", response={"result": None})
...
class TestPauseInvocationWithSingleLlmAgent(BasePauseInvocationTest):
    ...
    def agent(self) -> BaseAgent:
        ...
        return LlmAgent(
            name="root_agent",
            model=self.mock_model(responses=[Part.from_function_call(name="test_tool", args={})]),
    ...
    def test_pause_on_long_running_function_call(
        ...
        assert testing_utils.simplify_resumable_app_events(runner.run("test")) == [
            ...
            (
                "root_agent",
                Part.from_function_response(name="test_tool", response={"result": "result"}),
            ),
        ]
...
class TestPauseInvocationWithSequentialAgent(BasePauseInvocationTest):
    ...
    def test_pause_first_agent_on_long_running_function_call(
        ...
        assert testing_utils.simplify_resumable_app_events(runner.run("test")) == [
            (
                "root_agent",
                SequentialAgentState(current_sub_agent="sub_agent_1").model_dump(mode="json"),
            ),
    ...
    def test_pause_second_agent_on_long_running_function_call(
        ...
        assert testing_utils.simplify_resumable_app_events(runner.run("test")) == [
            ...
            (
                "root_agent",
                SequentialAgentState(current_sub_agent="sub_agent_2").model_dump(mode="json"),
            ),
...
class TestPauseInvocationWithParallelAgent(BasePauseInvocationTest):
    ...
    def test_pause_on_long_running_function_call(
        ...
        simplified_event_parts = testing_utils.simplify_resumable_app_events(runner.run("test"))
        assert (
            "sub_agent_1",
            Part.from_function_call(name="test_tool", args={}),
...
class TestPauseInvocationWithNestedParallelAgent(BasePauseInvocationTest):
    ...
    def test_pause_on_multiple_long_running_function_calls(
        ...
        simplified_events = testing_utils.simplify_resumable_app_events(runner.run("test"))
        assert (
            "sub_agent_1",
            Part.from_function_call(name="test_tool", args={}),
...
class TestPauseInvocationWithLoopAgent(BasePauseInvocationTest):
    ...
    def test_pause_on_long_running_function_call(
        ...
        assert testing_utils.simplify_resumable_app_events(runner.run("test")) == [
            (
                "root_agent",
                LoopAgentState(current_sub_agent="sub_agent_1").model_dump(mode="json"),
            ),
            ("sub_agent_1", "sub agent 1 response"),
            ("sub_agent_1", END_OF_AGENT),
            (
                "root_agent",
                LoopAgentState(current_sub_agent="sub_agent_2").model_dump(mode="json"),
            ),
            ("sub_agent_2", Part.from_function_call(name="test_tool", args={})),
            (
                "sub_agent_2",
