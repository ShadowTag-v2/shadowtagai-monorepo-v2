...
class TestRunDebug:
    ...
    async def test_run_debug_multiple_queries(self):
        ...
        with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
            ...
            events = await runner.run_debug(["First query", "Second query"], quiet=True)
    ...
    async def test_run_debug_custom_session_id(self):
        ...
        with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
            ...
            await runner.run_debug("Query", session_id="custom_debug_session", quiet=True)
    ...
    async def test_run_debug_with_run_config(self):
        ...
        with mock.patch.object(runner, "run_async", side_effect=mock_run_async) as mock_method:
    ...
    async def test_run_debug_with_existing_session(self):
        ...
        with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
            ...
            events = await runner.run_debug("Query", session_id="existing_session", quiet=True)
    ...
    async def test_run_debug_verbose_flag_false(self, capsys):
        ...
        async def mock_run_async(*args, **kwargs):
            ...
            mock_text_event.author = "test_agent"
            mock_text_event.content = mock.Mock()
            mock_text_event.content.parts = [mock.Mock(text="The weather in Tokyo is clear and 25°C.")]
            yield mock_text_event
    ...
    async def test_run_debug_verbose_flag_true(self, capsys):
        ...
        with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
            ...
            assert "[Calling tool: calculate({'expression': '42 * 3.14'})]" in captured.out
