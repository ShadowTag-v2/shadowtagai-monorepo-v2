...
from google.adk.utils.model_name_utils import extract_model_name, is_gemini_1_model, is_gemini_2_or_above, is_gemini_model
    ...
    def test_model_classification_consistency(self):
        ...
        for model in test_models:
            ...
            if is_gemini_1_model(model):
                assert not is_gemini_2_or_above(model), f"Model {model} classified as both Gemini 1.x and 2.0+"
                assert is_gemini_model(model), f"Model {model} is Gemini 1.x but not classified as Gemini"

            if is_gemini_2_or_above(model):
                assert not is_gemini_1_model(model), f"Model {model} classified as both Gemini 1.x and 2.0+"
                assert is_gemini_model(model), f"Model {model} is Gemini 2.0+ but not classified as Gemini"
            ...
            if not is_gemini_1_model(model) and not is_gemini_2_or_above(model):
                if model and "gemini-" not in extract_model_name(model):
                    assert not is_gemini_model(model), f"Non-Gemini model {model} classified as Gemini"
    ...
    def test_path_vs_simple_model_consistency(self):
        ...
        for simple_model, path_model in model_pairs:
            ...
            assert is_gemini_model(simple_model) == is_gemini_model(path_model), (
                f"Inconsistent Gemini classification for {simple_model} vs {path_model}"
            )
            assert is_gemini_1_model(simple_model) == is_gemini_1_model(path_model), (
                f"Inconsistent Gemini 1.x classification for {simple_model} vs {path_model}"
            )
            assert is_gemini_2_or_above(simple_model) == is_gemini_2_or_above(path_model), (
                f"Inconsistent Gemini 2.0+ classification for {simple_model} vs {path_model}"
            )
