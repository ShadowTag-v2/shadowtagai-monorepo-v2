...
def test_load_dotenv_for_agent_preserves_explicit_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (agent_dir / ".env").write_text(f"{explicit_key}=from_dotenv\n{from_dotenv_key}=from_dotenv\n")
...
def test_load_dotenv_for_agent_overrides_previous_dotenv(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    agents_dir = tmp_path / "agents"
...
def test_load_dotenv_for_agent_respects_disable_flag(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    agents_dir = tmp_path / "agents"
    agent_dir = agents_dir / "agent1"
    agent_dir.mkdir(parents=True)
