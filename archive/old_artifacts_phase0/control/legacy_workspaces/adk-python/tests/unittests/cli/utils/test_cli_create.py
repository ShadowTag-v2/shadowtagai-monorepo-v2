...
def test_generate_files_permission_error(monkeypatch: pytest.MonkeyPatch, agent_folder: Path) -> None:
    ...
    with pytest.raises(PermissionError):
        cli_create._generate_files(str(agent_folder), model="gemini-2.0-flash-001", type="code")
...
def test_run_cmd_with_type_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
