...
class TestAgentChangeEventHandler:
    ...
    def mock_agent_loader(self):
        ...
        return mock.create_autospec(agent_loader.AgentLoader, instance=True, spec_set=True)
    ...
    def test_on_modified_triggers_reload_for_supported_extensions(self, handler, mock_agent_loader, file_path):
        """Verify that .py, .yaml, and .yml files trigger agent reload."""
        event = FileModifiedEvent(src_path=file_path)

        handler.on_modified(event)

        mock_agent_loader.remove_agent_from_cache.assert_called_once_with("test_agent")
        assert "test_agent" in handler.runners_to_clean, f"Expected 'test_agent' in runners_to_clean for {file_path}"
    ...
    def test_on_modified_ignores_unsupported_extensions(self, handler, mock_agent_loader, file_path):
        ...
        assert not handler.runners_to_clean, f"Expected runners_to_clean to be empty for {file_path}, got {handler.runners_to_clean}"
