...
from google.adk.flows.llm_flows.functions import find_matching_function_call, merge_parallel_function_response_events
...
def test_simple_function():
    function_call_1 = types.Part.from_function_call(name="increase_by_one", args={"x": 1})
    function_responses_2 = types.Part.from_function_response(name="increase_by_one", response={"result": 2})
    ...
    assert testing_utils.simplify_contents(mock_model.requests[0].contents) == [("user", "test")]
...
async def test_async_function():
    ...
    function_responses = [
        types.Part.from_function_response(name="increase_by_one", response={"result": 2}),
        types.Part.from_function_response(name="multiple_by_two", response={"result": 4}),
        types.Part.from_function_response(name="multiple_by_two_sync", response={"result": 6}),
    ]
...
def test_find_function_call_event_function_response_but_no_matching_call():
    ...
    function_response = types.FunctionResponse(id="func_123", name="test_func", response={})
...
def test_find_function_call_event_function_response_with_matching_call():
    ...
    response_event = Event(
        invocation_id="inv2",
        author="user",
        content=types.Content(role="user", parts=[types.Part(function_response=function_response)]),
    )
...
def test_find_function_call_event_multiple_function_responses():
    ...
    function_response1 = types.FunctionResponse(id="func_123", name="test_func1", response={})
    function_response2 = types.FunctionResponse(id="func_456", name="test_func2", response={})

    call_event1 = Event(
        invocation_id="inv1",
        author="agent1",
        content=types.Content(role="model", parts=[types.Part(function_call=function_call1)]),
    )

    call_event2 = Event(
        invocation_id="inv2",
        author="agent2",
        content=types.Content(role="model", parts=[types.Part(function_call=function_call2)]),
    )
...
async def test_function_call_args_not_modified():
    ...
    from google.adk.flows.llm_flows.functions import handle_function_calls_async, handle_function_calls_live
...
async def test_function_call_args_copy_behavior():
    ...
    assert function_call.args["test_param"] == "original_value"  # Original unchanged
...
def test_shallow_vs_deep_copy_demonstration():
    ...
    assert original["nested_dict"]["inner"]["value"] == "modified"  # Original is affected!
    ...
    assert original["nested_dict"]["inner"]["value"] == "original"  # Original unchanged
    assert "new_item" not in original["list_param"]  # Original unchanged
    assert deep_copy["nested_dict"]["inner"]["value"] == "modified"  # Copy is modified
...
async def test_parallel_function_execution_timing():
    ...
    function_responses = [
        types.Part.from_function_response(name="slow_function_1", response={"result": "function_1_result"}),
        types.Part.from_function_response(name="slow_function_2", response={"result": "function_2_result"}),
    ]
    ...
    assert start_time_diff < 0.01, f"Functions started too far apart: {start_time_diff}s"
    ...
    assert total_time < sequential_time, f"Execution took too long: {total_time}s, expected < {sequential_time}s (sequential time)"
...
async def test_parallel_state_modifications_thread_safety():
    ...
    function_response_event = events[1]  # Second event should be the function response
...
async def test_sync_function_blocks_async_functions():
    ...
    async def yielding_async_function() -> dict:
        ...
        await asyncio.sleep(0.001)  # This should yield, but can't if event loop is blocked
...
async def test_async_function_without_yield_blocks_others():
    ...
    # Create function calls
    function_calls = [
        types.Part.from_function_call(name="non_yielding_async_function", args={}),
        types.Part.from_function_call(name="yielding_async_function", args={}),
    ]
...
def test_merge_parallel_function_response_events_preserves_invocation_id():
    ...
    function_response1 = types.FunctionResponse(id="func_123", name="test_function1", response={"result": "success1"})

    function_response2 = types.FunctionResponse(id="func_456", name="test_function2", response={"result": "success2"})

    event1 = Event(
        invocation_id=invocation_id,
        author="test_agent",
        content=types.Content(role="user", parts=[types.Part(function_response=function_response1)]),
    )

    event2 = Event(
        invocation_id="different_invocation_456",  # Different invocation ID
        author="test_agent",
        content=types.Content(role="user", parts=[types.Part(function_response=function_response2)]),
    )
...
def test_merge_parallel_function_response_events_single_event():
    ...
    function_response = types.FunctionResponse(id="func_123", name="test_function", response={"result": "success"})
