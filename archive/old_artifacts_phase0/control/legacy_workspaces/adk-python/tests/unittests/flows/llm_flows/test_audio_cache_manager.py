...
import pytest
from google.adk.flows.llm_flows.audio_cache_manager import AudioCacheConfig, AudioCacheManager
...
class TestAudioCacheManager:
    ...
    async def test_cache_input_audio(self):
        ...
        invocation_context = await testing_utils.create_invocation_context(testing_utils.create_test_agent())
    ...
    async def test_flush_caches_selective(self):
        ...
        await self.manager.flush_caches(invocation_context, flush_user_audio=True, flush_model_audio=False)
    ...
    async def test_filename_uses_first_chunk_timestamp(self):
        ...
        assert f"adk_live_audio_storage_input_audio_{expected_timestamp_ms}.pcm" == filename
        ...
        assert filename.startswith(f"adk_live_audio_storage_input_audio_{expected_timestamp_ms}")
