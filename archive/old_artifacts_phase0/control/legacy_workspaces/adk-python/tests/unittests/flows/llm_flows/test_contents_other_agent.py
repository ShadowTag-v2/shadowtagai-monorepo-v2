...
async def test_other_agent_function_calls():
    ...
    function_call = types.FunctionCall(id="func_123", name="search_tool", args={"query": "test query"})
    other_agent_event = Event(
        invocation_id="test_inv",
        author="other_agent",
...
async def test_other_agent_function_responses():
    ...
    assert llm_request.contents[0].parts == [
        types.Part(text="For context:"),
        types.Part(text=("[other_agent] `search_tool` tool returned result: {'results': ['item1', 'item2']}")),
    ]
...
async def test_other_agent_function_call_response():
    ...
    function_call = types.FunctionCall(id="func_123", name="calc_tool", args={"query": "6x7"})
    call_event = Event(
    ...
    function_response = types.FunctionResponse(id="func_123", name="calc_tool", response={"result": 42})
    response_event = Event(
        invocation_id="test_inv2",
        author="other_agent",
        content=types.UserContent(parts=[types.Part(function_response=function_response)]),
    )
    ...
    assert llm_request.contents[0].parts == [
        ...
        types.Part(text=("[other_agent] called tool `calc_tool` with parameters: {'query': '6x7'}")),
    ]
    # Function response from other agent
    assert llm_request.contents[1].role == "user"
    assert llm_request.contents[1].parts == [
        types.Part(text="For context:"),
        types.Part(text="[other_agent] `calc_tool` tool returned result: {'result': 42}"),
    ]
