...
async def test_output_schema_request_processor(output_schema_with_tools_allowed, mocker):
...
async def test_set_model_response_tool():
    ...
    from google.adk.tools.set_model_response_tool import MODEL_JSON_RESPONSE_KEY, SetModelResponseTool
    ...
    stored_response = invocation_context.session.state.get(MODEL_JSON_RESPONSE_KEY)
...
async def test_output_schema_helper_functions():
    ...
    from google.adk.flows.llm_flows._output_schema_processor import create_final_model_response_event, get_structured_model_response
    ...
    function_response_event = Event(
        ...
        content=types.Content(
            role="user",
            parts=[types.Part(function_response=types.FunctionResponse(name="set_model_response", response=test_dict))],
        ),
    )
    ...
    other_function_response_event = Event(
        ...
        content=types.Content(
            role="user",
            parts=[types.Part(function_response=types.FunctionResponse(name="other_tool", response={"result": "other response"}))],
        ),
    )
...
async def test_flow_yields_both_events_for_set_model_response():
    ...
    events = []
    async for event in flow._postprocess_handle_function_calls_async(invocation_context, function_call_event, llm_request):
        events.append(event)
    ...
    assert second_event.content.parts[0].text == '{"name": "Test User", "age": 30, "city": "Test City"}'
...
async def test_flow_yields_only_function_response_for_normal_tools():
    ...
    function_call_event = Event(
        ...
        content=types.Content(
            role="model",
            parts=[types.Part(function_call=types.FunctionCall(name="dummy_tool", args={"query": "test query"}))],
        ),
    )
    ...
    assert first_event.get_function_responses()[0].response == {"result": "Searched for: test query"}
