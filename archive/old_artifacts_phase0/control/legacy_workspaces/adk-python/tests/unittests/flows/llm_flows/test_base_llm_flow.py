...
async def test_preprocess_calls_toolset_process_llm_request():
    ...
    mock_response = LlmResponse(
        content=types.Content(role="model", parts=[types.Part.from_text(text="Test response")]),
        partial=False,
    )
    ...
    invocation_context = await testing_utils.create_invocation_context(agent=agent, user_content="test message")
...
async def test_preprocess_handles_mixed_tools_and_toolsets():
    ...
    agent = Agent(name="test_agent", tools=[mock_tool, _test_function, mock_toolset])
...
async def test_preprocess_with_google_search_workaround():
    ...
    agent = Agent(name="test_agent", model="gemini-pro", tools=[_my_tool, google_search])
    invocation_context = await testing_utils.create_invocation_context(agent=agent, user_content="test message")
    flow = BaseLlmFlowForTesting()
    llm_request = LlmRequest(model="gemini-pro")
    async for _ in flow._preprocess_async(invocation_context, llm_request):
...
async def test_preprocess_calls_convert_tool_union_to_tools():
    ...
    with mock.patch(
        ...
        agent = Agent(name="test_agent", model=model, tools=[_my_tool, google_search])
        invocation_context = await testing_utils.create_invocation_context(agent=agent, user_content="test message")
        flow = BaseLlmFlowForTesting()
...
@pytest.mark.asyncio
async def test_handle_after_model_callback_grounding_with_no_callbacks(tools, state_metadata, expect_metadata):
    ...
    if state_metadata:
        invocation_context.session.state["temp:_adk_grounding_metadata"] = state_metadata

    llm_response = LlmResponse(content=types.Content(parts=[types.Part.from_text(text="response")]))
    event = Event(
        id=Event.new_id(),
    ...
    result = await flow._handle_after_model_callback(invocation_context, llm_response, event)
...
@pytest.mark.asyncio
async def test_handle_after_model_callback_grounding_with_callback_override(tools, state_metadata, expect_metadata):
    """Test handling grounding metadata when there is a callback override."""
    agent_response = LlmResponse(content=types.Content(parts=[types.Part.from_text(text="agent")]))
    agent_callback = AsyncMock(return_value=agent_response)

    agent = Agent(name="test_agent", tools=tools, after_model_callback=[agent_callback])
    invocation_context = await testing_utils.create_invocation_context(agent=agent)
    if state_metadata:
        invocation_context.session.state["temp:_adk_grounding_metadata"] = state_metadata
...
@pytest.mark.asyncio
async def test_handle_after_model_callback_grounding_with_plugin_override(tools, state_metadata, expect_metadata):
    """Test handling grounding metadata when there is a plugin override."""
    plugin_response = LlmResponse(content=types.Content(parts=[types.Part.from_text(text="plugin")]))
    ...
    plugin = _MockPlugin()
    agent = Agent(name="test_agent", tools=tools)
    invocation_context = await testing_utils.create_invocation_context(agent=agent, plugins=[plugin])
    if state_metadata:
        invocation_context.session.state["temp:_adk_grounding_metadata"] = state_metadata
...
async def test_handle_after_model_callback_caches_canonical_tools():
    ...
    with mock.patch.object(type(agent), "canonical_tools", new=mock_canonical_tools):
        ...
        invocation_context.session.state["temp:_adk_grounding_metadata"] = {"foo": "bar"}
        ...
        result1 = await flow._handle_after_model_callback(invocation_context, llm_response, event)
        result2 = await flow._handle_after_model_callback(invocation_context, llm_response, event)
        result3 = await flow._handle_after_model_callback(invocation_context, llm_response, event)

        assert canonical_tools_call_count == 1, f"canonical_tools should be called once, but was called {canonical_tools_call_count} times"
