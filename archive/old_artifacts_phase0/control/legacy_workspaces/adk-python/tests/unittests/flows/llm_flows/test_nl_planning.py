...
async def test_plan_react_planner_instruction_appended():
    ...
    planner.build_planning_instruction = MagicMock(return_value="Test instruction")
    agent = Agent(name="test_agent", planner=planner)
    invocation_context = await testing_utils.create_invocation_context(agent=agent, user_content="test message")
...
async def test_remove_thought_from_request_with_thoughts():
    ...
    assert all(part.thought is None for content in llm_request.contents for part in content.parts or [])
