...
from google.adk.flows.llm_flows import compaction, contents
...
async def test_compaction_request_processor_runs_token_compaction():
    ...
    compacted_events_arg = mock_summarizer.maybe_summarize_events.call_args[1]["events"]
    assert [event.invocation_id for event in compacted_events_arg] == [
        "inv1",
        "inv2",
    ]
    session_service.append_event.assert_called_once_with(session=session, event=compacted_event)
    ...
    session = Session(
        ...
        events=[
            ...
            Event(
                ...
                content=Content(
                    role="model",
                    parts=[Part(function_call=types.FunctionCall(id="call-1", name="tool", args={}))],
                ),
            ),
            Event(
                timestamp=4.0,
