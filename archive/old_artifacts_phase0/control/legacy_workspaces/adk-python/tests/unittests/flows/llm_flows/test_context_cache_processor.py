...
class TestContextCacheRequestProcessor:
    ...
    def setup_method(self):
        ...
        self.cache_config = ContextCacheConfig(cache_intervals=10, ttl_seconds=1800, min_tokens=1024)
    ...
    def create_cache_metadata(self, invocations_used=1, cache_name="test-cache", contents_count=3):
    ...
    async def test_no_cache_config(self):
        ...
        invocation_context = self.create_invocation_context(agent, context_cache_config=None)
        ...
        events = []
        async for event in self.processor.run_async(invocation_context, llm_request):
            events.append(event)
    ...
    async def test_with_cache_config_no_session_events(self):
        ...
        agent = LlmAgent(name="test_agent")
        invocation_context = self.create_invocation_context(agent, context_cache_config=self.cache_config)
    ...
    async def test_cache_metadata_agent_filtering(self):
        ...
        other_cache = self.create_cache_metadata(invocations_used=7, cache_name="other")
    ...
    async def test_latest_cache_metadata_selected(self):
        ...
        agent = LlmAgent(name="test_agent")
        older_cache = self.create_cache_metadata(invocations_used=2, cache_name="older")
        newer_cache = self.create_cache_metadata(invocations_used=5, cache_name="newer")
