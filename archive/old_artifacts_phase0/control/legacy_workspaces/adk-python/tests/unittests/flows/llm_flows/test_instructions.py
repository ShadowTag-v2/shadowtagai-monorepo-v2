...
async def _create_invocation_context(agent: LlmAgent, state: dict[str, Any] | None = None) -> InvocationContext:
    """Helper to create InvocationContext with session."""
    session_service = InMemorySessionService()
    session = await session_service.create_session(app_name="test_app", user_id="test_user", state=state)
...
async def test_global_system_instruction():
    ...
    invocation_context = await testing_utils.create_invocation_context(agent=sub_agent)
    invocation_context.session = Session(
        app_name="test_app",
        user_id="test_user",
        id="test_id",
    ...
    assert request.config.system_instruction == ("This is the global instruction.\n\nThis is the sub agent instruction.")
...
async def test_build_system_instruction_with_namespace():
    ...
    agent = Agent(
        model="gemini-1.5-flash",
        name="agent",
        instruction=("""Use the echo_info tool to echo { customerId }, {app:key}, {user:key}, {a:key}."""),
    )
    invocation_context = await testing_utils.create_invocation_context(agent=agent)
    invocation_context.session = Session(
        app_name="test_app",
    ...
    assert request.config.system_instruction == ("""Use the echo_info tool to echo 1234567890, app_value, user_value, {a:key}.""")
...
async def test_instruction_processor_respects_bypass_state_injection():
    ...
    raw_si, bypass_flag = await agent.canonical_instruction(ReadonlyContext(invocation_context))
    assert bypass_flag
    assert raw_si == "instruction with state: test_value"

    # Run the instruction processor
    async for _ in instructions.request_processor.run_async(invocation_context, request):
        pass

    # System instruction should be exactly what the provider returned
    # (no additional state injection should occur)
    assert request.config.system_instruction == "instruction with state: test_value"
...
async def test_global_instruction_processor_respects_bypass_state_injection():
    ...
    raw_gi, bypass_flag = await root_agent.canonical_global_instruction(ReadonlyContext(invocation_context))
    assert bypass_flag
    ...
    assert request.config.system_instruction == "global instruction with state: test_value\n\nSub agent instruction"
...
async def test_string_global_instruction_respects_bypass_state_injection():
    ...
    assert request.config.system_instruction == "Global instruction with test_value\n\nSub agent instruction"
...
def test_static_instruction_field_exists(llm_backend):
    ...
    static_content = types.Content(role="user", parts=[types.Part(text="This is a static instruction")])
...
def test_static_instruction_supports_multiple_parts(llm_backend):
    ...
    static_content = types.Content(
        ...
        parts=[
            ...
            types.Part(inline_data=types.Blob(data=b"fake_file_content", mime_type="text/plain")),
...
async def test_static_instruction_added_to_contents(llm_backend):
    ...
    static_content = types.Content(role="user", parts=[types.Part(text="Static instruction content")])
    agent = LlmAgent(name="test_agent", static_instruction=static_content)
...
async def test_static_instruction_string_added_to_system(llm_backend):
    ...
    agent = LlmAgent(name="test_agent", static_instruction="Static instruction as string")
...
async def test_static_instruction_list_of_strings_converted_to_system(
    ...
    assert llm_request.config.system_instruction == "First instruction\n\nSecond instruction"
...
async def test_dynamic_instructions_added_to_user_content(llm_backend):
    ...
    static_content = types.Content(role="user", parts=[types.Part(text="Static instruction")])
    agent = LlmAgent(
        name="test_agent",
    ...
    llm_request.contents.append(types.Content(role="user", parts=[types.Part(text="Hello world")]))
...
async def test_no_dynamic_instructions_when_no_static(llm_backend):
    ...
    original_content = types.Content(role="user", parts=[types.Part(text="Hello world")])
...
async def test_instructions_insert_after_function_response():
    ...
    tool_call = types.Part.from_function_call(name="echo_tool", args={"echo": "value"})
    tool_response = types.Part.from_function_response(name="echo_tool", response={"result": "value"})
    ...
    instruction_contents = [types.Content(role="user", parts=[types.Part.from_text(text="Dynamic instruction")])]

    await _add_instructions_to_user_content(invocation_context, llm_request, instruction_contents)
...
async def test_static_instruction_with_files_and_text(llm_backend):
    ...
    static_content = types.Content(
        ...
        parts=[
            ...
            types.Part(inline_data=types.Blob(data=b"fake_image_data", mime_type="image/png")),
    ...
    assert len(llm_request.contents) == 1
    assert (
        llm_request.config.system_instruction == "Analyze this image:\n\n[Reference to inline binary data:"
    ...
    assert llm_request.contents[0].parts[0].text == "Referenced inline data: inline_data_0"
...
async def test_static_instruction_reference_id_generation(llm_backend):
    ...
    static_content = types.Content(
        ...
        parts=[
            ...
            types.Part(inline_data=types.Blob(data=b"data1", mime_type="image/png")),
            types.Part(file_data=types.FileData(file_uri="files/test1", mime_type="text/plain")),
            types.Part(inline_data=types.Blob(data=b"data2", mime_type="image/jpeg")),
        ],
    )
    agent = LlmAgent(name="test_agent", static_instruction=static_content)
...
async def test_static_instruction_only_non_text_parts(llm_backend):
    ...
    static_content = types.Content(
        role="user",
        parts=[
            types.Part(inline_data=types.Blob(data=b"data", mime_type="image/png")),
            types.Part(file_data=types.FileData(file_uri="files/test", mime_type="text/plain")),
        ],
    )
    agent = LlmAgent(name="test_agent", static_instruction=static_content)
