...
def test_auto_to_single(is_resumable: bool):
    ...
    root_agent = Agent(name="root_agent", model=mock_model, sub_agents=[sub_agent_1])
    app = App(
        name="test_app",
        root_agent=root_agent,
        resumability_config=ResumabilityConfig(is_resumable=is_resumable),
...
def test_auto_to_auto_to_single(is_resumable: bool):
    ...
    sub_agent_1 = Agent(name="sub_agent_1", model=mock_model, sub_agents=[sub_agent_1_1])
    root_agent = Agent(name="root_agent", model=mock_model, sub_agents=[sub_agent_1])
    app = App(
        name="test_app",
        root_agent=root_agent,
...
def test_auto_to_sequential(is_resumable: bool):
    ...
    if not is_resumable:
        ...
        assert testing_utils.simplify_resumable_app_events(runner.run("test1")) == [
            ...
            (
                "sub_agent_1",
                SequentialAgentState(current_sub_agent="sub_agent_1_1").model_dump(mode="json"),
            ),
            ("sub_agent_1_1", "response1"),
            ("sub_agent_1_1", END_OF_AGENT),
            (
                "sub_agent_1",
                SequentialAgentState(current_sub_agent="sub_agent_1_2").model_dump(mode="json"),
            ),
            ("sub_agent_1_2", "response2"),
...
def test_auto_to_sequential_to_auto(is_resumable: bool):
    ...
    if not is_resumable:
        ...
        assert testing_utils.simplify_resumable_app_events(runner.run("test1")) == [
            ...
            (
                "sub_agent_1",
                SequentialAgentState(current_sub_agent="sub_agent_1_3").model_dump(mode="json"),
            ),
            ("sub_agent_1_3", "response3"),
...
def test_auto_to_loop(is_resumable: bool):
    ...
    if not is_resumable:
        ...
        assert testing_utils.simplify_events(runner.run("test1")) == [
            ...
            (
                "sub_agent_1_2",
                Part.from_function_response(name="exit_loop", response={"result": None}),
            ),
        ]
        ...
        assert testing_utils.simplify_resumable_app_events(runner.run("test1")) == [
            ...
            (
                "sub_agent_1",
                LoopAgentState(current_sub_agent="sub_agent_1_1").model_dump(mode="json"),
            ),
            ("sub_agent_1_1", "response1"),
            ("sub_agent_1_1", END_OF_AGENT),
            (
                "sub_agent_1",
                LoopAgentState(current_sub_agent="sub_agent_1_2").model_dump(mode="json"),
            ),
            ("sub_agent_1_2", "response2"),
            ("sub_agent_1_2", END_OF_AGENT),
            (
                "sub_agent_1",
                LoopAgentState(current_sub_agent="sub_agent_1_1", times_looped=1).model_dump(mode="json"),
            ),
            ("sub_agent_1_1", "response3"),
            ("sub_agent_1_1", END_OF_AGENT),
            (
                "sub_agent_1",
                LoopAgentState(current_sub_agent="sub_agent_1_2", times_looped=1).model_dump(mode="json"),
            ),
            # Exits.
            ("sub_agent_1_2", Part.from_function_call(name="exit_loop", args={})),
            (
