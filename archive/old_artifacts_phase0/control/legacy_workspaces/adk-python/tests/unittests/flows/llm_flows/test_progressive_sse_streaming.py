...
class StreamingMockModel(BaseLlm):
    ...
    async def generate_content_async(self, llm_request: LlmRequest, stream: bool = False) -> AsyncGenerator[LlmResponse]:
        ...
        for chunk in self.stream_chunks:
            ...
            async for processed_chunk in aggregator.process_response(self._llm_response_to_generate_content_response(chunk)):
    ...
    def _llm_response_to_generate_content_response(self, llm_response: LlmResponse) -> types.GenerateContentResponse:
...
def test_progressive_sse_streaming_function_calls():
    ...
    response1 = LlmResponse(
        content=types.Content(role="model", parts=[types.Part.from_text(text="Checking weather...")]),
    )

    response2 = LlmResponse(
        content=types.Content(
            role="model",
            parts=[types.Part.from_function_call(name="get_weather", args={"location": "Tokyo"})],
        ),
    )
    ...
    response3 = LlmResponse(
        content=types.Content(
            role="model",
            parts=[types.Part.from_function_call(name="get_weather", args={"location": "New York"})],
        ),
        finish_reason=types.FinishReason.STOP,
    )

    # Create a streaming mock that yields all chunks in one call
    mock_model = StreamingMockModel(stream_chunks=[response1, response2, response3])
    ...
    session = runner.session_service.create_session_sync(app_name=runner.app_name, user_id="test_user")
    ...
    assert events[4].content.parts[0].function_response.response["location"] == "Tokyo"
    assert events[4].content.parts[1].function_response.name == "get_weather"
    assert events[4].content.parts[1].function_response.response["location"] == "New York"
...
def test_progressive_sse_preserves_part_ordering():
    ...
    chunk4 = LlmResponse(content=types.Content(role="model", parts=[types.Part.from_text(text="And New York too.")]))
    ...
    chunk6 = LlmResponse(
        content=types.Content(
            role="model",
            parts=[types.Part(text="Now processing second thought part 1. ", thought=True)],
        )
    )
    ...
    for event in events:
        if not event.partial and event.author == "ordering_test_agent" and event.content and len(event.content.parts) > 2:
    ...
    assert parts[3].text == "Now processing second thought part 1. Second thought part 2."
...
def test_progressive_sse_streaming_function_call_arguments():
    ...
    chunk1_fc = types.FunctionCall(
        name="get_weather",
        id="fc_001",
        partial_args=[types.PartialArg(json_path="$.location", string_value="New ")],
        will_continue=True,
    )
    chunk1 = types.GenerateContentResponse(
        candidates=[types.Candidate(content=types.Content(role="model", parts=[types.Part(function_call=chunk1_fc)]))]
    )

    # Chunk 2: Continue streaming location argument
    chunk2_fc = types.FunctionCall(
        partial_args=[types.PartialArg(json_path="$.location", string_value="York")],
        will_continue=True,
    )
    chunk2 = types.GenerateContentResponse(
        candidates=[types.Candidate(content=types.Content(role="model", parts=[types.Part(function_call=chunk2_fc)]))]
    )

    # Chunk 3: Add unit argument, FC complete
    chunk3_fc = types.FunctionCall(
        partial_args=[types.PartialArg(json_path="$.unit", string_value="celsius")],
        will_continue=False,  # FC complete
    )
    chunk3 = types.GenerateContentResponse(
        candidates=[
            types.Candidate(
                content=types.Content(role="model", parts=[types.Part(function_call=chunk3_fc)]),
                finish_reason=types.FinishReason.STOP,
            )
        ]
    )
...
def test_progressive_sse_preserves_thought_signature():
    ...
    chunk_part = types.Part(function_call=chunk_fc, thought_signature=test_thought_signature)
...
def test_progressive_sse_handles_empty_function_call():
    ...
    chunk2 = types.GenerateContentResponse(
        candidates=[
            types.Candidate(
                content=types.Content(role="model", parts=[types.Part(function_call=chunk2_fc)]),
                finish_reason=types.FinishReason.STOP,
            )
        ]
    )
