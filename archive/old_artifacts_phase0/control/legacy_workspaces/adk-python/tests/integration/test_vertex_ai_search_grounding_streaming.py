...
import pytest
from google.adk.features._feature_registry import FeatureName, temporary_feature_override
...
class TestIntegrationVertexAiSearchGrounding:
    ...
    @pytest.mark.asyncio
    async def test_grounding_metadata_with_sse_streaming(self, project_id, data_store_resource, progressive_sse, label):
        ...
        with temporary_feature_override(FeatureName.PROGRESSIVE_SSE_STREAMING, progressive_sse):
            all_events, saved_events = await self._run_agent_streaming(agent, project_id)

        self._report_events(label, all_events, saved_events)

        saved_with_grounding = [e for e in saved_events if e["has_grounding"]]
        assert saved_with_grounding, f"No saved (non-partial) events have grounding_metadata with {label}."

    @pytest.mark.parametrize("llm_backend", ["VERTEX"], indirect=True)
    @pytest.mark.asyncio
    async def test_grounding_metadata_without_streaming(self, project_id, data_store_resource):
        ...
        async with Aclosing(
            runner.run_async(
                ...
                new_message=types.Content(
                    role="user",
                    parts=[types.Part.from_text(text="What built-in tools does ADK provide?")],
                ),
        ...
        for i, e in enumerate(events):
            print(f"  Event {i}: author={e['author']}, partial={e['partial']}, grounding={e['has_grounding']}, content={e['has_content']}")
    ...
    async def _run_agent_streaming(self, agent, project_id):
        ...
        async with Aclosing(
            runner.run_async(
                ...
                new_message=types.Content(
                    role="user",
                    parts=[types.Part.from_text(text="What is ADK and when was it first released?")],
                ),
                run_config=run_config,
            )
        ) as agen:
    ...
    def _report_events(self, label, all_events, saved_events):
        ...
        for i, e in enumerate(saved_events):
            print(f"  Event {i}: author={e['author']}, partial={e['partial']}, grounding={e['has_grounding']}, content={e['has_content']}")
        partial_with_grounding = [e for e in all_events if e["partial"] is True and e["has_grounding"]]
        if partial_with_grounding:
            print(f"\n  NOTE: {len(partial_with_grounding)} partial event(s) had grounding_metadata but were NOT saved to session.")
