...
from pytest import FixtureRequest, Metafunc, fixture, hookimpl
...
def load_env_for_tests():
    ...
    if not os.path.exists(dotenv_path):
        warnings.warn(f"Missing .env file at {dotenv_path}. See dotenv.sample for an example.")
    else:
        load_dotenv(dotenv_path, override=True, verbose=True)
    if "GOOGLE_API_KEY" not in os.environ:
        warnings.warn("Missing GOOGLE_API_KEY in the environment variables. GOOGLE_AI backend integration tests will fail.")
    for env_var in [
        "GOOGLE_CLOUD_PROJECT",
        "GOOGLE_CLOUD_LOCATION",
    ]:
        if env_var not in os.environ:
            warnings.warn(f"Missing {env_var} in the environment variables. Vertex backend integration tests will fail.")
    ...
    if llm_backend.__name__ in metafunc.fixturenames:
        if not _is_explicitly_marked(llm_backend.__name__, metafunc):
            ...
            if test_backend == "GOOGLE_AI_ONLY":
                ...
                metafunc.parametrize(llm_backend.__name__, ["GOOGLE_AI", "VERTEX"], indirect=True)
            else:
                raise ValueError(f"Invalid TEST_BACKEND value: {test_backend}, should be one of [GOOGLE_AI_ONLY, VERTEX_ONLY, BOTH]")
