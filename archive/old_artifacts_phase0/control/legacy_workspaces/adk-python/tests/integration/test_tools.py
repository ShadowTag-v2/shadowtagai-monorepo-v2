...
def test_agent_tools_success(agent_runner: TestRunner):
    ...
    _call_function_and_assert(agent_runner, "no_input_schema_agent", "Tools", "Tools_success")
...
def _assert_function_output(agent_runner: TestRunner, query: str, expected):
    ...
    assert expected.lower() in model_response_event.content.parts[0].text.strip().lower()
