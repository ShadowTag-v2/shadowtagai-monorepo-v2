...
class IntegrationConnectorTool(BaseTool):
    ...
    * Attaches auth credentials to API call.
    ...
    def _prepare_dynamic_euc(self, auth_credential: AuthCredential) -> str:
        if auth_credential and auth_credential.http and auth_credential.http.credentials and auth_credential.http.credentials.token:
    ...
    async def run_async(self, *, args: dict[str, Any], tool_context: ToolContext | None) -> dict[str, Any]:
        ...
        tool_auth_handler = ToolAuthHandler.from_tool_context(tool_context, self._auth_scheme, self._auth_credential)
        ...
        if auth_result.auth_credential:
            # Attach parameters from auth into main parameters list
            auth_credential_token = self._prepare_dynamic_euc(auth_result.auth_credential)
            if auth_credential_token:
                args["dynamic_auth_config"] = {"oauth2_auth_code_flow.access_token": auth_credential_token}
            else:
