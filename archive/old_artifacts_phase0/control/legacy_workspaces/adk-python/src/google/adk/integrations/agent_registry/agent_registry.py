...
class AgentRegistry:
    ...
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(f"Failed to get default Google Cloud credentials: {e}") from e
    ...
    def _get_auth_headers(self) -> dict[str, str]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to refresh Google Cloud credentials: {e}") from e

    def _make_request(self, path: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    def get_mcp_toolset(self, mcp_server_name: str) -> McpToolset:
        ...
        endpoint_uri = self._get_connection_uri(server_details, protocol_binding=A2ATransport.jsonrpc) or self._get_connection_uri(
            server_details, protocol_binding=A2ATransport.http_json
        )
        if not endpoint_uri:
            raise ValueError(f"MCP Server endpoint URI not found for: {mcp_server_name}")

        connection_params = StreamableHTTPConnectionParams(url=endpoint_uri, headers=self._get_auth_headers())
        return McpToolset(
    ...
    def get_remote_a2a_agent(self, agent_name: str) -> RemoteA2aAgent:
        ...
        url = self._get_connection_uri(agent_info, protocol_type=_ProtocolType.A2A_AGENT)
        if not url:
