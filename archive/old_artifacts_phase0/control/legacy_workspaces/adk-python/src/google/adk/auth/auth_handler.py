...
from .auth_schemes import AuthSchemeType, OpenIdConnectWithConfig
...
class AuthHandler:
    ...
    async def exchange_auth_token(
        ...
        return await exchanger.exchange(self.auth_config.exchanged_auth_credential, self.auth_config.auth_scheme)
    ...
    async def parse_and_store_auth_response(self, state: State) -> None:
        ...
        if not isinstance(self.auth_config.auth_scheme, SecurityBase) or self.auth_config.auth_scheme.type_ not in (
    ...
    def generate_auth_request(self) -> AuthConfig:
        ...
        if not self.auth_config.raw_auth_credential:
            raise ValueError(f"Auth Scheme {self.auth_config.auth_scheme.type_} requires auth_credential.")

        # Check if oauth2 exists in raw_auth_credential
        if not self.auth_config.raw_auth_credential.oauth2:
            raise ValueError(f"Auth Scheme {self.auth_config.auth_scheme.type_} requires oauth2 in auth_credential.")
        ...
        if self.auth_config.raw_auth_credential.oauth2.auth_uri:
            return AuthConfig(
                ...
                exchanged_auth_credential=self.auth_config.raw_auth_credential.model_copy(deep=True),
            )

        # Check for client_id and client_secret
        if not self.auth_config.raw_auth_credential.oauth2.client_id or not self.auth_config.raw_auth_credential.oauth2.client_secret:
            raise ValueError(f"Auth Scheme {self.auth_config.auth_scheme.type_} requires both client_id and client_secret in auth_credential.oauth2.")
    ...
    def generate_auth_uri(
        ...
        if not AUTHLIB_AVAILABLE:
            return self.auth_config.raw_auth_credential.model_copy(deep=True) if self.auth_config.raw_auth_credential else None
        ...
        uri, state = client.create_authorization_url(url=authorization_endpoint, **params)
