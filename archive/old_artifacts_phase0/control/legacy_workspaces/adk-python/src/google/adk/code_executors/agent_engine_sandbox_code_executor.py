...
from .code_execution_utils import CodeExecutionInput, CodeExecutionResult, File
...
class AgentEngineSandboxCodeExecutor(BaseCodeExecutor):
    ...
    def __init__(
        ...
        **data,
        ...
        if sandbox_resource_name is not None:
            ...
            self._project_id, self._location = self._get_project_id_and_location_from_resource_name(
            ...
            operation = self._get_api_client().agent_engines.sandboxes.create(
                ...
                config=types.CreateAgentEngineSandboxConfig(display_name="default_sandbox"),
            )
            self.sandbox_resource_name = operation.response.name
        else:
            raise ValueError("Either sandbox_resource_name or agent_engine_resource_name must be set.")
    ...
    def execute_code(
        ...
        code_execution_response = self._get_api_client().agent_engines.sandboxes.execute_code(
        ...
        for output in code_execution_response.outputs:
            if output.mime_type == "application/json" and (
                output.metadata is None or output.metadata.attributes is None or "file_name" not in output.metadata.attributes
            ):
    ...
    def _get_project_id_and_location_from_resource_name(self, resource_name: str, pattern: str) -> tuple[str, str]:
