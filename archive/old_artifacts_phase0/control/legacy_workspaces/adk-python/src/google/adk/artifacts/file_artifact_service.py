...
from pydantic import ConfigDict, Field, ValidationError, alias_generators
from typing import override

from ..errors.input_validation_error import InputValidationError
from .base_artifact_service import ArtifactVersion, BaseArtifactService
...
def _resolve_scoped_artifact_path(scope_root: Path, filename: str) -> tuple[Path, Path]:
    ...
    if pure_path.is_absolute():
        raise InputValidationError(f"Absolute artifact filename {filename!r} is not permitted; provide a path relative to the storage scope.")
    ...
    try:
        ...
        raise InputValidationError(f"Artifact filename {filename!r} escapes storage directory {scope_root_resolved}") from exc
...
def _is_user_scoped(session_id: str | None, filename: str) -> bool:
...
class FileArtifactService(BaseArtifactService):
    ...
    def _build_artifact_version(
        ...
        *,
        ...
        filename: str,
        version: int,
        metadata: FileArtifactVersion | None,
    ) -> ArtifactVersion:
    ...
    def _latest_metadata(self, artifact_dir: Path) -> FileArtifactVersion | None:
    ...
    def _save_artifact_sync(
        ...
        if artifact.inline_data:
            ...
            mime_type = artifact.inline_data.mime_type if artifact.inline_data.mime_type else "application/octet-stream"
    ...
    def _load_artifact_sync(
        ...
        if mime_type:
            if not content_path.exists():
                logger.warning("Binary artifact %s missing at %s", filename, content_path)
                return None
...
def _read_metadata(path: Path) -> FileArtifactVersion | None:
    """Loads a metadata payload from disk."""
    if not path.exists():
        return None
    try:
        return FileArtifactVersion.model_validate_json(path.read_text(encoding="utf-8"))
