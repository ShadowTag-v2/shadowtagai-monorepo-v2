...
class OAuth2DiscoveryManager:
    ...
    async def discover_auth_server_metadata(self, issuer_url: str) -> AuthorizationServerMetadata | None:
    ...
    async def discover_resource_metadata(self, resource_url: str) -> ProtectedResourceMetadata | None:
        ...
        if path and path != "/":
            well_known_endpoint = f"{base_url}/.well-known/oauth-protected-resource{path}"
        else:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                logger.debug("Failed to fetch metadata from %s: %s", well_known_endpoint, e)
            except (json.decoder.JSONDecodeError, ValidationError) as e:
                logger.debug("Failed to parse metadata from %s: %s", well_known_endpoint, e)
