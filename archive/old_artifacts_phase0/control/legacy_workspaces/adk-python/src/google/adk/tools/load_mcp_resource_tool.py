...
NOTE: Call when you need access to resources.""",
    ...
    async def process_llm_request(self, *, tool_context: ToolContext, llm_request: LlmRequest) -> None:
        ...
        await self._append_resources_to_llm_request(tool_context=tool_context, llm_request=llm_request)

    async def _append_resources_to_llm_request(self, *, tool_context: ToolContext, llm_request: LlmRequest):
        try:
                        ...
                        for content in contents:
                            ...
                            llm_request.contents.append(
                                types.Content(
                                    role="user",
                                    parts=[
                                        types.Part.from_text(text=f"Resource {resource_name} is:"),
                                        part,
                                    ],
                                )
                            )
                    except Exception as e:
                        logger.warning("Failed to read MCP resource '%s': %s", resource_name, e)
                        continue

    def _mcp_content_to_part(self, content: Any, resource_name: str) -> types.Part:
        if hasattr(content, "text") and content.text is not None:
            ...
            try:
                ...
                return types.Part.from_text(text=f"[Binary content for {resource_name} could not be decoded]")
        else:
            return types.Part.from_text(text=f"[Unknown content type for {resource_name}]")
