...
class GeminiLlmConnection(BaseLlmConnection):
    ...
    async def send_history(self, history: list[types.Content]):
        ...
        contents = [content for content in history if content.parts and content.parts[0].text]
            ...
            async for message in agen:
                ...
                if message.server_content:
                    ...
                    if content and content.parts:
                        llm_response = LlmResponse(content=content, interrupted=message.server_content.interrupted)
                    ...
                    if message.server_content.output_transcription:
                        if message.server_content.output_transcription.text:
                            self._output_transcription_text += message.server_content.output_transcription.text
                            yield LlmResponse(
                    ...
                    if self._api_backend == GoogleLLMVariant.GEMINI_API and (
                        message.server_content.interrupted or message.server_content.turn_complete or message.server_content.generation_complete
                    ):
                ...
                if message.tool_call:
                    if text:
                        yield self.__build_full_text_response(text)
                        text = ""
                    parts = [types.Part(function_call=function_call) for function_call in message.tool_call.function_calls]
                    yield LlmResponse(content=types.Content(role="model", parts=parts))
                if message.session_resumption_update:
                    logger.debug("Received session resumption message: %s", message)
                    yield (LlmResponse(live_session_resumption_update=message.session_resumption_update))
