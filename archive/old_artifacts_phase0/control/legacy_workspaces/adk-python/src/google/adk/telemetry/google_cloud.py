...
def get_gcp_exporters(
    ...
    credentials, project_id = google_auth if google_auth is not None else google.auth.default()
    if TYPE_CHECKING:
        credentials = cast(Credentials, credentials)
        project_id = cast(str, project_id)

    if not project_id:
        logger.warning("Cannot determine GCP Project. OTel GCP Exporters cannot be set up. Please make sure to log into correct GCP Project.")
        return OTelHooks()
...
def _get_gcp_logs_exporter(project_id: str) -> LogRecordProcessor:
    ...
    default_log_name = os.environ.get(_GCP_LOG_NAME_ENV_VARIABLE_NAME, _DEFAULT_LOG_NAME)
    return BatchLogRecordProcessor(
        CloudLoggingExporter(project_id=project_id, default_log_name=default_log_name),
    )
...
def get_gcp_resource(project_id: str | None = None) -> Resource:
    ...
    """
    resource = Resource(attributes={"gcp.project_id": project_id} if project_id is not None else {})
    resource = resource.merge(OTELResourceDetector().detect())
    try:
        from opentelemetry.resourcedetector.gcp_resource_detector import GoogleCloudResourceDetector

        resource = resource.merge(GoogleCloudResourceDetector(raise_on_error=False).detect())
    except ImportError:
        logger.warning(
            "Cloud not import opentelemetry.resourcedetector.gcp_resource_detector GCE, GKE or CloudRun related resource attributes may be missing"
        )
    return resource
