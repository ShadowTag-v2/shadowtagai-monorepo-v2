...
from . import cli_create, cli_deploy
from .cli import run_cli
from .fast_api import get_fast_api_app
from .utils import envs, evals, logs
...
@click.option(
    ...
    default="replay",
    show_default=True,
    help=("Test mode: 'replay' verifies against recorded interactions, 'live' runs evaluation-based verification."),
)
@click.pass_context
...
def eval_options():
    ...
    def decorator(func):
        @click.option(
            "--eval_storage_uri",
            type=str,
            help=("Optional. The evals storage URI to store agent evals, supported URIs: gs://<bucket name>."),
            default=None,
        )
        @click.option(
            "--log_level",
...
def cli_eval(
    ...
    *Eval Set ID*
    ...
    try:
        ...
        from ..evaluation.eval_config import get_eval_metrics_from_config, get_evaluation_criteria_or_default
        from ..evaluation.eval_result import EvalCaseResult
        from ..evaluation.evaluator import EvalStatus
        from ..evaluation.in_memory_eval_sets_manager import InMemoryEvalSetsManager
        from ..evaluation.local_eval_service import LocalEvalService
        from ..evaluation.local_eval_set_results_manager import LocalEvalSetResultsManager
        from ..evaluation.local_eval_sets_manager import LocalEvalSetsManager, load_eval_set_from_file
        from ..evaluation.simulation.user_simulator_provider import UserSimulatorProvider
        from .cli_eval import _collect_eval_results, _collect_inferences, get_root_agent, parse_and_get_evals_to_run, pretty_print_eval_result
    ...
    inference_requests = []
    eval_set_file_or_id_to_evals = parse_and_get_evals_to_run(eval_set_file_path_or_id)
    ...
    # block.
    if eval_set_file_or_id_to_evals and os.path.exists(list(eval_set_file_or_id_to_evals.keys())[0]):
        ...
        for (
            ...
            try:
                ...
                raise click.ClickException(f"`{eval_set_file_path}` should be a valid eval set file.") from fne
        ...
        eval_sets_manager = eval_sets_manager if eval_storage_uri else LocalEvalSetsManager(agents_dir=agents_dir)
    ...
    try:
        ...
        inference_results = asyncio.run(_collect_inferences(inference_requests=inference_requests, eval_service=eval_service))
...
def cli_add_eval_case(
    ...
    try:
        with open(session_input_file) as f:
            session_input = SessionInput.model_validate_json(f.read())

        with open(scenarios_file) as f:
            conversation_scenarios = ConversationScenarios.model_validate_json(f.read())
        ...
        for scenario in conversation_scenarios.scenarios:
            ...
            if eval_sets_manager.get_eval_case(app_name=app_name, eval_set_id=eval_set_id, eval_case_id=eval_id) is None:
                eval_sets_manager.add_eval_case(app_name=app_name, eval_set_id=eval_set_id, eval_case=eval_case)
                click.echo(f"Eval case '{eval_case.eval_id}' added to eval set '{eval_set_id}'.")
            else:
                click.echo(f"Eval case '{eval_case.eval_id}' already exists in eval set '{eval_set_id}', skipped adding.")
...
def web_options():
    ...
    def decorator(func):
        ...
        @click.option(
            "--logo-image-url",
            type=str,
            help=("Optional. The URL of the image to display in the logo of the web UI."),
            default=None,
        )
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
...
def deprecated_adk_services_options():
    ...
    def warn(alternative_param, ctx, param, value):
        if value:
            click.echo(
                click.style(
                    f"WARNING: Deprecated option --{param.name} is used. Please use {alternative_param} instead.",
                    fg="yellow",
                ),
                err=True,
            )
...
def fast_api_common_options():
    ...
    def decorator(func):
        ...
        @click.option(
            ...
            show_default=True,
            default=False,
            help=("EXPERIMENTAL Optional. Whether to write OTel data to Google Cloud Observability services - Cloud Trace and Cloud Logging."),
        )
        @click.option(
            "--reload/--no-reload",
            default=True,
            help=("Optional. Whether to enable auto reload for server. Not supported for Cloud Run."),
        )
        @click.option(
            "--a2a",
            is_flag=True,
        ...
        def wrapper(ctx, *args, **kwargs):
            ...
            if kwargs.pop("verbose", False) and log_level_source == ParameterSource.DEFAULT:
...
@click.option(
    "--project",
    type=str,
    help=("Required. Google Cloud project to deploy the agent. When absent, default project from gcloud config is used."),
)
@click.option(
    "--region",
    type=str,
...
@click.option(
    ...
    help=("Optional. Temp folder for the generated Cloud Run source files (default: a timestamped folder in the system temp directory)."),
)
@click.option(
    "--log_level",
    type=LOG_LEVELS,
...
@click.option(
    ...
    show_default=True,
    help=("Optional. The ADK version used in Cloud Run deployment. (default: the version in the dev environment)"),
)
@click.option(
    "--a2a",
    is_flag=True,
...
def cli_migrate_session(*, source_db_url: str, dest_db_url: str, log_level: str):
...
@click.option(
    ...
    type=str,
    default=None,
    help=("Optional. GCS bucket for staging the deployment artifacts. It will be ignored if api_key is set."),
)
@click.option(
    "--agent_engine_id",
    type=str,
...
@click.option(
    ...
    help=("Optional. Python file for defining the ADK application (default: a file named agent_engine_app.py)"),
)
@click.option(
    "--temp_folder",
    type=str,
...
@click.option(
    ...
    type=str,
    default=None,
    help=("Optional. Python object corresponding to the root ADK agent or app. It can only be `root_agent` or `app`. (default: `root_agent`)"),
)
@click.option(
    "--env_file",
    type=str,
    default="",
    help=("Optional. The filepath to the `.env` file for environment variables. (default: the `.env` file in the `agent` directory, if any.)"),
)
@click.option(
    "--requirements_file",
    type=str,
    default="",
    help=("Optional. The filepath to the `requirements.txt` file to use. (default: the `requirements.txt` file in the `agent` directory, if any.)"),
)
@click.option(
    "--absolutize_imports",
    type=bool,
...
@click.option(
    ...
    type=str,
    default="adk-default-service-name",
    help=("Optional. The service name to use in GKE (default: 'adk-default-service-name')."),
)
@click.option(
    "--app_name",
    type=str,
...
@click.option(
    ...
    help=("Optional. Temp folder for the generated GKE source files (default: a timestamped folder in the system temp directory)."),
)
@click.option(
    "--adk_version",
    type=str,
    default=version.__version__,
    show_default=True,
    help=("Optional. The ADK version used in GKE deployment. (default: the version in the dev environment)"),
)
@adk_services_options()
@click.argument(
    "agent",
