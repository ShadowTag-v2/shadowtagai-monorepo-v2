...
class GemmaFunctionCallModel(BaseModel):
    ...
    parameters: dict[str, Any] = Field(validation_alias=AliasChoices("parameters", "args"))
...
class Gemma(Gemini):
    ...
    **SHOULD** be replaced. Special care is warranted here.
    ...
    def _move_function_calls_into_system_instruction(self, llm_request: LlmRequest):
        ...
        if all_function_declarations:
            system_instruction = _build_gemma_function_system_instruction(all_function_declarations)
    ...
    def _extract_function_calls_from_response(self, llm_response: LlmResponse):
        ...
        try:
            ...
            markdown_code_block_pattern = re.compile(r"```(?:(json|tool_code))?\s*(.*?)\s*```", re.DOTALL)
            ...
            function_call_parsed = GemmaFunctionCallModel.model_validate_json(json_candidate)
            ...
            logger.debug(
                "Error attempting to parse JSON into function call. Leaving as text response. %s",
                e,
            )
    ...
    async def generate_content_async(self, llm_request: LlmRequest, stream: bool = False) -> AsyncGenerator[LlmResponse]:
        ...
        """
        # print(f'{llm_request=}')
        assert llm_request.model.startswith("gemma-"), f"Requesting a non-Gemma model ({llm_request.model}) with the Gemma LLM is not supported."
...
def _convert_content_parts_for_gemma(
    ...
    for part in content_item.parts:
        if func_response := part.function_response:
            ...
            response_text = f"Invoking tool `{func_response.name}` produced: `{json.dumps(func_response.response)}`."
            new_parts.append(Part.from_text(text=response_text))
        elif func_call := part.function_call:
            has_function_call_part = True
            new_parts.append(Part.from_text(text=func_call.model_dump_json(exclude_none=True)))
        else:
