...
from ..a2a.converters.event_converter import convert_a2a_message_to_event, convert_a2a_task_to_event, convert_event_to_a2a_message
from ..a2a.converters.part_converter import (
...
from ..a2a.logs.log_utils import build_a2a_request_log, build_a2a_response_log
from ..agents.invocation_context import InvocationContext
from ..events.event import Event
from ..flows.llm_flows.contents import _is_other_agent_reply, _present_other_agent_message
...
class RemoteA2aAgent(BaseAgent):
    ...
    def __init__(
        ...
        *,
        ...
        a2a_request_meta_provider: Callable[[InvocationContext, A2AMessage], dict[str, Any]] | None = None,
        **kwargs: Any,
    ) -> None:
        ...
        self._agent_card_source: str | None = None
        ...
        self._a2a_client_factory: A2AClientFactory | None = a2a_client_factory
        ...
        if isinstance(agent_card, AgentCard):
            ...
            raise TypeError(f"agent_card must be AgentCard, URL string, or file path string, got {type(agent_card)}")

    async def _ensure_httpx_client(self) -> httpx.AsyncClient:
        """Ensure HTTP client is available and properly configured."""
        if not self._httpx_client:
            self._httpx_client = httpx.AsyncClient(timeout=httpx.Timeout(timeout=self._timeout))
    ...
    async def _resolve_agent_card_from_url(self, url: str) -> AgentCard:
        ...
        try:
            ...
            raise AgentCardResolutionError(f"Failed to resolve AgentCard from URL {url}: {e}") from e
    ...
    async def _resolve_agent_card_from_file(self, file_path: str) -> AgentCard:
        ...
        try:
            ...
            raise AgentCardResolutionError(f"Invalid JSON in agent card file {file_path}: {e}") from e
        except Exception as e:
            raise AgentCardResolutionError(f"Failed to resolve AgentCard from file {file_path}: {e}") from e
    ...
    async def _validate_agent_card(self, agent_card: AgentCard) -> None:
        ...
        if not agent_card.url:
            raise AgentCardResolutionError("Agent card must have a valid URL for RPC communication")
        ...
        try:
            ...
            raise AgentCardResolutionError(f"Invalid RPC URL in agent card: {agent_card.url}, error: {e}") from e
    ...
    async def _ensure_resolved(self) -> None:
        ...
        try:
            ...
            raise AgentCardResolutionError(f"Failed to initialize remote A2A agent {self.name}: {e}") from e

    def _create_a2a_request_for_user_function_response(self, ctx: InvocationContext) -> A2AMessage | None:
        ...
        a2a_message = convert_event_to_a2a_message(ctx.session.events[-1], ctx, Role.user, self._genai_part_converter)
    ...
    def _construct_message_parts_from_session(self, ctx: InvocationContext) -> tuple[list[A2APart], str | None]:
    ...
    async def _handle_a2a_response(self, a2a_response: A2AClientEvent | A2AMessage, ctx: InvocationContext) -> Event | None:
        ...
        try:
            if isinstance(a2a_response, tuple):
                ...
                if update is None:
                    ...
                    event = convert_a2a_task_to_event(task, self.name, ctx, self._a2a_part_converter)
                ...
                elif isinstance(update, A2ATaskStatusUpdateEvent) and update.status and update.status.message:
                    # This is a streaming task status update with a message.
                    event = convert_a2a_message_to_event(update.status.message, self.name, ctx, self._a2a_part_converter)
                    if event.content and update.status.state in [
                        ...
                        for part in event.content.parts:
                            part.thought = True
                elif isinstance(update, A2ATaskArtifactUpdateEvent) and (not update.append or update.last_chunk):
            ...
            # Otherwise, it's a regular A2AMessage for non-streaming responses.
                ...
                event = convert_a2a_message_to_event(a2a_response, self.name, ctx, self._a2a_part_converter)
                event.custom_metadata = event.custom_metadata or {}

                if a2a_response.context_id:
                    event.custom_metadata[A2A_METADATA_PREFIX + "context_id"] = a2a_response.context_id
            else:
                event = Event(
                    author=self.name,
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event]:
        ...
        try:
            ...
            async for a2a_response in self._a2a_client.send_message(
                ...
                event.custom_metadata = event.custom_metadata or {}
                event.custom_metadata[A2A_METADATA_PREFIX + "request"] = a2a_request.model_dump(exclude_none=True, by_alias=True)
                # If the response is a ClientEvent, record the task state; otherwise,
                # record the message object.
                if isinstance(a2a_response, tuple):
                    event.custom_metadata[A2A_METADATA_PREFIX + "response"] = a2a_response[0].model_dump(exclude_none=True, by_alias=True)
                else:
                    event.custom_metadata[A2A_METADATA_PREFIX + "response"] = a2a_response.model_dump(exclude_none=True, by_alias=True)
            ...
            yield Event(
                ...
                branch=ctx.branch,
                custom_metadata={
                    A2A_METADATA_PREFIX + "request": a2a_request.model_dump(exclude_none=True, by_alias=True),
                    A2A_METADATA_PREFIX + "error": error_message,
                },
            )

    async def _run_live_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event]:
        """Core implementation for live agent execution (not implemented)."""
        raise NotImplementedError(f"_run_live_impl for {type(self)} via A2A is not implemented.")
