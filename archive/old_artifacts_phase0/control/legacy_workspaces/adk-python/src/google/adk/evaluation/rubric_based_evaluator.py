...
from .eval_metrics import BaseCriterion, EvalMetric
from .eval_rubrics import Rubric, RubricScore
from .evaluator import EvaluationResult, PerInvocationResult
from .llm_as_judge import AutoRaterScore, LlmAsJudge
from .llm_as_judge_utils import get_average_rubric_score, get_eval_status, get_text_from_content
...
class DefaultAutoRaterResponseParser(AutoRaterResponseParser):
    ...
    def parse(self, auto_rater_response: str) -> list[RubricResponse]:
        ...
        for p, r, s in zip(properties, rationales, scores):
            rubric_responses.append(RubricResponse(property_text=p.strip(), rationale=r.strip(), score=s))
...
class MajorityVotePerInvocationResultsAggregator(PerInvocationResultsAggregator):
    ...
    def aggregate(
        ...
        aggregated_overall_score = get_average_rubric_score(aggregated_rubric_scores)
...
class InvocationResultsSummarizer(abc.ABC):
    ...
    @abc.abstractmethod
    def summarize(self, per_invocation_results: list[PerInvocationResult], threshold: float) -> EvaluationResult:
...
class MeanInvocationResultsSummarizer(InvocationResultsSummarizer):
    ...
    def summarize(self, per_invocation_results: list[PerInvocationResult], threshold: float) -> EvaluationResult:
        ...
        aggregated_overall_score = get_average_rubric_score(unaggregated_rubric_scores)
...
class RubricBasedEvaluator(LlmAsJudge):
    ...
    def __init__(
        ...
        self._normalized_rubric_to_id_map = {_normalize_text(r.rubric_content.text_property): r.rubric_id for r in self._rubrics}

    @override
    def convert_auto_rater_response_to_score(self, auto_rater_response: LlmResponse) -> AutoRaterScore:
        ...
        for rubric_response in rubric_responses:
            ...
            if rubric_id:
                ...
                logger.warning(f"Rubric {rubric_response.property_text} not found in the rubrics provided to the metric.")
    ...
    def aggregate_per_invocation_samples(
        ...
        The aggregator helps convert those multiple samples into a single result.
        """
        return self._per_invocation_results_aggregator.aggregate(per_invocation_samples, self._eval_metric.threshold)

    @override
    def aggregate_invocation_results(self, per_invocation_results: list[PerInvocationResult]) -> EvaluationResult:
        """Summarizes per invocation evaluation results into a single score."""
        return self._invocation_results_summarizer.summarize(per_invocation_results, self._eval_metric.threshold)
