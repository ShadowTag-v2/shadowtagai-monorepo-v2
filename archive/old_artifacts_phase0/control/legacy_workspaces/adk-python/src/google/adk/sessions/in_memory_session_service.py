...
class InMemorySessionService(BaseSessionService):
    ...
    def _create_session_impl(
        ...
        *,
        ...
        if session_id and self._get_session_impl(app_name=app_name, user_id=user_id, session_id=session_id):
        ...
        if user_state_delta:
            self.user_state.setdefault(app_name, {}).setdefault(user_id, {}).update(user_state_delta)
    ...
    def _merge_state(self, app_name: str, user_id: str, copied_session: Session) -> Session:
        ...
        if app_name in self.app_state:
            for key in self.app_state[app_name].keys():
                copied_session.state[State.APP_PREFIX + key] = self.app_state[app_name][key]

        if app_name not in self.user_state or user_id not in self.user_state[app_name]:
            return copied_session

        # Merge session state with user state.
        for key in self.user_state[app_name][user_id].keys():
            copied_session.state[State.USER_PREFIX + key] = self.user_state[app_name][user_id][key]
        return copied_session
    ...
    def list_sessions_sync(self, *, app_name: str, user_id: str | None = None) -> ListSessionsResponse:
        logger.warning("Deprecated. Please migrate to the async method.")
        return self._list_sessions_impl(app_name=app_name, user_id=user_id)

    def _list_sessions_impl(self, *, app_name: str, user_id: str | None = None) -> ListSessionsResponse:
    ...
    def _delete_session_impl(self, *, app_name: str, user_id: str, session_id: str) -> None:
        if self._get_session_impl(app_name=app_name, user_id=user_id, session_id=session_id) is None:
            return
