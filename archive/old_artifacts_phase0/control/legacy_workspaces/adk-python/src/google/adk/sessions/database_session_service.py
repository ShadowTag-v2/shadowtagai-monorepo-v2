...
from sqlalchemy import Dialect, ForeignKeyConstraint, Text, delete, event, func, inspect, select
...
class DatabaseSessionService(BaseSessionService):
    ...
    def __init__(self, db_url: str, **kwargs: Any):
        ...
        self.database_session_factory: async_sessionmaker[DatabaseSessionFactory] = async_sessionmaker(bind=self.db_engine, expire_on_commit=False)
        ...
        async with self._table_creation_lock:
            ...
            if not self._tables_created:
                ...
                async with self.database_session_factory() as sql_session:
                    version, is_v01 = await sql_session.run_sync(_schema_check.get_version_and_v01_status_sync)
                ...
                async with self.database_session_factory() as sql_session:
                    metadata = await sql_session.get(StorageMetadata, _schema_check.SCHEMA_VERSION_KEY)
                    if not metadata:
                        sql_session.add(
                            StorageMetadata(
    ...
    async def create_session(
        ...
        *,
        ...
        async with self.database_session_factory() as sql_session:
            if session_id and await sql_session.get(StorageSession, (app_name, user_id, session_id)):
            ...
            merged_state = _merge_state(storage_app_state.state, storage_user_state.state, session_state)
    ...
    async def list_sessions(self, *, app_name: str, user_id: str | None = None) -> ListSessionsResponse:
        ...
        async with self.database_session_factory() as sql_session:
            ...
            if user_id is not None:
                ...
                user_state_stmt = select(StorageUserState).filter(StorageUserState.app_name == app_name)
    ...
    async def append_event(self, session: Session, event: Event) -> Event:
        ...
        async with self.database_session_factory() as sql_session:
            ...
            if storage_session._dialect_name == "sqlite":
                update_time = datetime.fromtimestamp(event.timestamp, UTC).replace(tzinfo=None)
            else:
