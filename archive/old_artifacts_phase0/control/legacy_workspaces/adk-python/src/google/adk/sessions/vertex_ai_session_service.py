...
class VertexAiSessionService(BaseSessionService):
    ...
    def __init__(
        ...
        *,
        ...
        self._express_mode_api_key = get_express_mode_api_key(project, location, express_mode_api_key)
        ...
        async with self._get_api_client() as api_client:
            ...
            if config and not config.num_recent_events and config.after_timestamp:
                ...
                list_events_kwargs["config"] = {
                    "filter": 'timestamp>="{}"'.format(datetime.datetime.fromtimestamp(config.after_timestamp, tz=datetime.UTC).isoformat())
                }
    ...
    async def append_event(self, session: Session, event: Event) -> Event:
        ...
        config = {}
        if event.content:
            config["content"] = event.content.model_dump(exclude_none=True, mode="json")
        if event.actions:
            config["actions"] = {
                ...
                "requested_auth_configs": {
                    k: json.loads(v.model_dump_json(exclude_none=True, by_alias=True)) for k, v in event.actions.requested_auth_configs.items()
                },
        ...
        if event.grounding_metadata:
            metadata_dict["grounding_metadata"] = event.grounding_metadata.model_dump(exclude_none=True, mode="json")
        ...
        async with self._get_api_client() as api_client:
            await api_client.agent_engines.sessions.events.append(
                ...
                timestamp=datetime.datetime.fromtimestamp(event.timestamp, tz=datetime.UTC),
                config=config,
            )
        return event
    ...
    def _get_reasoning_engine_id(self, app_name: str):
        ...
        if not match:
            raise ValueError(
                f"App name {app_name} is not valid. It should either be the full ReasoningEngine resource name, or the reasoning engine id."
            )
    ...
    def _api_client_http_options_override(
        self,
    ) -> types.HttpOptions | types.HttpOptionsDict | None:
        return None
...
def _from_api_event(api_event_obj: vertexai.types.SessionEvent) -> Event:
    ...
    if actions:
        ...
        renamed_actions_dict = {rename_map.get(k, k): v for k, v in actions_dict.items()}
    ...
    if event_metadata:
        long_running_tool_ids_list = getattr(event_metadata, "long_running_tool_ids", None)
        long_running_tool_ids = set(long_running_tool_ids_list) if long_running_tool_ids_list else None
    ...
    return Event(
        ...
        content=_session_util.decode_model(getattr(api_event_obj, "content", None), types.Content),
