...
from .auth_credential import AuthCredential, AuthCredentialTypes
from .auth_schemes import AuthSchemeType, ExtendedOAuth2, OpenIdConnectWithConfig
...
class CredentialManager:
    ...
    def __init__(
        ...
        self._refresher_registry.register(AuthCredentialTypes.OAUTH2, oauth2_refresher)
        self._refresher_registry.register(AuthCredentialTypes.OPEN_ID_CONNECT, oauth2_refresher)
    ...
    async def get_auth_credential(self, callback_context: CallbackContext) -> AuthCredential | None:
    ...
    async def _load_existing_credential(self, callback_context: CallbackContext) -> AuthCredential | None:
    ...
    async def _load_from_credential_service(self, callback_context: CallbackContext) -> AuthCredential | None:
    ...
    async def _load_from_auth_response(self, callback_context: CallbackContext) -> AuthCredential | None:
        """Load credential from auth response in callback context."""
        return callback_context.get_auth_response(self._auth_config)

    async def _exchange_credential(self, credential: AuthCredential) -> tuple[AuthCredential, bool]:
        ...
        if isinstance(exchanger, ServiceAccountCredentialExchanger):
            exchanged_credential = exchanger.exchange_credential(self._auth_config.auth_scheme, credential)
        else:
            exchanged_credential = await exchanger.exchange(credential, self._auth_config.auth_scheme)

        return exchanged_credential, True

    async def _refresh_credential(self, credential: AuthCredential) -> tuple[AuthCredential, bool]:
        ...
        if await refresher.is_refresh_needed(credential, self._auth_config.auth_scheme):
            refreshed_credential = await refresher.refresh(credential, self._auth_config.auth_scheme)
    ...
    async def _validate_credential(self) -> None:
        ...
        if not self._auth_config.raw_auth_credential:
            if self._auth_config.auth_scheme.type_ in (
                ...
                raise ValueError(f"raw_auth_credential is required for auth_scheme type {self._auth_config.auth_scheme.type_}")
        ...
        if raw_credential:
            if (
                ...
                raise ValueError(f"auth_config.raw_credential.oauth2 required for credential type {raw_credential.auth_type}")

        if self._missing_oauth_info() and not await self._populate_auth_scheme():
            raise ValueError("OAuth scheme info is missing, and auto-discovery has failed to fill them in.")

        # Additional validation can be added here

    async def _save_credential(self, callback_context: CallbackContext, credential: AuthCredential) -> None:
    ...
    async def _populate_auth_scheme(self) -> bool:
        ...
        if not isinstance(auth_scheme, ExtendedOAuth2) or not auth_scheme.issuer_url:
            logger.warning("No issuer_url was provided for auto-discovery.")
            return False

        metadata = await self._discovery_manager.discover_auth_server_metadata(auth_scheme.issuer_url)
        if not metadata:
    ...
    def _is_client_credentials_flow(self) -> bool:
        ...
        if isinstance(auth_scheme, OpenIdConnectWithConfig):
            return auth_scheme.grant_types_supported is not None and "client_credentials" in auth_scheme.grant_types_supported
