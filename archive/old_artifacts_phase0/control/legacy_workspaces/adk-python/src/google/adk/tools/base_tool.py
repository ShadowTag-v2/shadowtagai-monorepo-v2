...
from ..utils.variant_utils import GoogleLLMVariant, get_google_llm_variant
...
class BaseTool(ABC):
  ...
  """
    ...
    @classmethod
    def from_config(cls: type[SelfTool], config: ToolArgsConfig, config_abs_path: str) -> SelfTool:
        ...
        for param_name, _ in sig.parameters.items():
            ...
            if param_name in config_dict:
                ...
                if param_type in (int, str, bool, float):
                ...
                elif inspect.isclass(param_type) and issubclass(param_type, BaseModel) and value is not None:
                    kwargs[param_name] = param_type.model_validate(value)
                elif param_type is Callable or get_origin(param_type) is Callable:
                    kwargs[param_name] = config_agent_utils.resolve_fully_qualified_name(value)
                    ...
                    if issubclass(list_args[0], BaseModel):
                        kwargs[param_name] = [list_args[0].model_validate(item) for item in value]
                    elif list_args[0] in (int, str, bool, float):
                        kwargs[param_name] = value
                    elif list_args[0] is Callable or get_origin(list_args[0]) is Callable:
                        kwargs[param_name] = [config_agent_utils.resolve_fully_qualified_name(item) for item in value]
                    else:
                        logger.warning("Unsupported parsing for list argument: %s.", param_name)
                else:
