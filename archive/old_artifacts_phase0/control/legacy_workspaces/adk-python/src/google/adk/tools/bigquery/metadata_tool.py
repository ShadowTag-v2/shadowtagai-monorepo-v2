...
def list_dataset_ids(project_id: str, credentials: Credentials, settings: BigQueryToolConfig) -> list[str]:
...
def get_dataset_info(
    ...
    try:
        ...
        dataset = bq_client.get_dataset(bigquery.DatasetReference(project_id, dataset_id))
...
def list_table_ids(
    ...
    try:
        ...
        tables = []
        for table in bq_client.list_tables(bigquery.DatasetReference(project_id, dataset_id)):
...
def get_table_info(
    ...
    try:
        ...
        return bq_client.get_table(bigquery.TableReference(bigquery.DatasetReference(project_id, dataset_id), table_id)).to_api_repr()
    except Exception as ex:
        return {
            "status": "ERROR",
            "error_details": str(ex),
