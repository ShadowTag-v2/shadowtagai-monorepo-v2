...
async def _run_compaction_for_sliding_window(app: App, session: Session, session_service: BaseSessionService):
    ...
    for event in reversed(events):
        if event.actions and event.actions.compaction and event.actions.compaction.end_timestamp:
    ...
    new_invocation_ids = [inv_id for inv_id in unique_invocation_ids if invocation_latest_timestamps[inv_id] > last_compacted_end_timestamp]
    ...
    start_idx = max(0, first_new_inv_idx - app.events_compaction_config.overlap_size)
    ...
    if last_event_idx != -1:
        ...
        if first_event_start_inv_idx != -1:
            ...
            events_to_compact = [e for e in events_to_compact if not (e.actions and e.actions.compaction)]

    if not events_to_compact:
        return None

    if not app.events_compaction_config.summarizer:
        app.events_compaction_config.summarizer = LlmEventSummarizer(llm=app.root_agent.canonical_model)

    compaction_event = await app.events_compaction_config.summarizer.maybe_summarize_events(events=events_to_compact)
    if compaction_event:
