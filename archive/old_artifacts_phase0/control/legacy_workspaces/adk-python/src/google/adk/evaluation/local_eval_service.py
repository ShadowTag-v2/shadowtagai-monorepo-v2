...
from ..utils._client_labels_utils import EVAL_CLIENT_LABEL, client_label_context
from ..utils.feature_decorator import experimental
from .base_eval_service import BaseEvalService, EvaluateConfig, EvaluateRequest, InferenceRequest, InferenceResult, InferenceStatus
from .eval_case import Invocation
from .eval_metrics import EvalMetric, EvalMetricResult, EvalMetricResultDetails, EvalMetricResultPerInvocation
from .eval_result import EvalCaseResult
from .eval_set import EvalCase
from .eval_set_results_manager import EvalSetResultsManager
from .eval_sets_manager import EvalSetsManager
from .evaluation_generator import EvaluationGenerator
from .evaluator import EvalStatus, EvaluationResult, PerInvocationResult
from .metric_evaluator_registry import DEFAULT_METRIC_EVALUATOR_REGISTRY, MetricEvaluatorRegistry
...
class LocalEvalService(BaseEvalService):
    ...
    def __init__(
        ...
        metric_evaluator_registry = metric_evaluator_registry or DEFAULT_METRIC_EVALUATOR_REGISTRY
    ...
    async def perform_inference(
        ...
        if not eval_set:
            raise NotFoundError(f"Eval set with id {inference_request.eval_set_id} not found for app {inference_request.app_name}")
    ...
    async def evaluate(
        ...
        evaluation_tasks = [run_evaluation(inference_result) for inference_result in evaluate_request.inference_results]
    ...
    async def _evaluate_single_inference_result(
        ...
        user_id = eval_case.session_input.user_id if eval_case.session_input and eval_case.session_input.user_id else "test_user_id"

        if eval_case.conversation_scenario is None and len(inference_result.inferences) != len(eval_case.conversation):
            raise ValueError(
        ...
        for idx, actual in enumerate(inference_result.inferences):
            eval_metric_result_per_invocation.append(
                EvalMetricResultPerInvocation(
                    ...
                    expected_invocation=eval_case.conversation[idx] if eval_case.conversation else None,
        ...
        for eval_metric in evaluate_config.eval_metrics:
            ...
            eval_metric_result_details = EvalMetricResultDetails(rubric_scores=evaluation_result.overall_rubric_scores)
            ...
            if evaluation_result.overall_eval_status != EvalStatus.NOT_EVALUATED and len(evaluation_result.per_invocation_results) != len(
                eval_metric_result_per_invocation
            ):
                raise ValueError(
            ...
            for idx, invocation in enumerate(eval_metric_result_per_invocation):
                ...
                eval_metric_result_details = EvalMetricResultDetails(rubric_scores=invocation_result.rubric_scores)
    ...
    async def _evaluate_metric(
        ...
        metric_evaluator = self._metric_evaluator_registry.get_evaluator(eval_metric=eval_metric)
