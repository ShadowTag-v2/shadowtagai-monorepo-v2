...
from ...telemetry.tracing import trace_call_llm, trace_send_data, tracer
from ...tools.base_toolset import BaseToolset
from ...tools.tool_context import ToolContext
from ...utils.context_utils import Aclosing
from . import _output_schema_processor, functions
...
class BaseLlmFlow(ABC):
        ...
        while True:
            try:
                ...
                if invocation_context.live_session_resumption_handle:
                    ...
                    llm_request.live_connect_config.session_resumption.handle = invocation_context.live_session_resumption_handle
                        ...
                        async with Aclosing(
                            ...
                            async for event in agen:
                                ...
                                if (
                                    ...
                                    and event.content.parts
                                    and event.content.parts[0].function_response
                                    and event.content.parts[0].function_response.name == "task_completed"
                                ):
                ...
                logger.error("An unexpected error occurred in live flow: %s", e, exc_info=True)
                raise
    ...
    async def _send_to_model(
        ...
        while True:
            ...
            try:
                ...
                live_request = await asyncio.wait_for(live_request_queue.get(), timeout=DEFAULT_REQUEST_QUEUE_TIMEOUT)
                ...
                if invocation_context.active_streaming_tools:
                    for active_streaming_tool in (invocation_context.active_streaming_tools).values():
            ...
            if live_request.activity_start:
                ...
                self.audio_cache_manager.cache_audio(invocation_context, live_request.blob, cache_type="input")
                ...
                async with Aclosing(llm_connection.receive()) as agen:
                    async for llm_response in agen:
                        if llm_response.live_session_resumption_update:
                            logger.info(f"Update session resumption handle: {llm_response.live_session_resumption_update}.")
                            invocation_context.live_session_resumption_handle = llm_response.live_session_resumption_update.new_handle
                        ...
                        async with Aclosing(
                            ...
                            async for event in agen:
                                ...
                                if (
                                    ...
                                    self.audio_cache_manager.cache_audio(invocation_context, audio_blob, cache_type="output")
    ...
    async def run_async(self, invocation_context: InvocationContext) -> AsyncGenerator[Event]:
        """Runs the flow."""
        while True:
            last_event = None
    ...
    async def _run_one_step_async(
        ...
        if (
            ...
            # one text responses.
            and (invocation_context.should_pause_invocation(events[-1]) or invocation_context.should_pause_invocation(events[-2]))
        ):
            return

        if invocation_context.is_resumable and events and events[-1].get_function_calls():
            model_response_event = events[-1]
            async with Aclosing(self._postprocess_handle_function_calls_async(invocation_context, model_response_event, llm_request)) as agen:
                async for event in agen:
        ...
        async with Aclosing(self._call_llm_async(invocation_context, llm_request, model_response_event)) as agen:
            async for llm_response in agen:
    ...
    async def _preprocess_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event]:
    ...
    async def _postprocess_async(
        ...
        # Runs processors.
        async with Aclosing(self._postprocess_run_processors_async(invocation_context, llm_response)) as agen:
            async for event in agen:
                yield event

        # Skip the model response event if there is no content and no error code.
        # This is needed for the code executor to trigger another loop.
        if not llm_response.content and not llm_response.error_code and not llm_response.interrupted:
            return

        # Builds the event.
        model_response_event = self._finalize_model_response_event(llm_request, llm_response, model_response_event)
        ...
        if model_response_event.get_function_calls():
            if is_feature_enabled(FeatureName.PROGRESSIVE_SSE_STREAMING):
                ...
                if invocation_context.run_config.streaming_mode == StreamingMode.SSE and model_response_event.partial:
                    return
    ...
    async def _postprocess_live(
        ...
        if llm_response.output_transcription:
            model_response_event.output_transcription = llm_response.output_transcription
            model_response_event.partial = llm_response.partial
            yield model_response_event
            return
        ...
        if invocation_context.run_config.save_live_blob:
            flushed_events = await self._handle_control_event_flush(invocation_context, llm_response)
        ...
        # Handles function calls.
        if model_response_event.get_function_calls():
            function_response_event = await functions.handle_function_calls_live(invocation_context, model_response_event, llm_request.tools_dict)
            # Always yield the function response event first
            yield function_response_event

            # Check if this is a set_model_response function response
            if json_response := _output_schema_processor.get_structured_model_response(function_response_event):
                # Create and yield a final model response event
                final_event = _output_schema_processor.create_final_model_response_event(invocation_context, json_response)
                yield final_event

    async def _postprocess_run_processors_async(self, invocation_context: InvocationContext, llm_response: LlmResponse) -> AsyncGenerator[Event]:
        for processor in self.response_processors:
            async with Aclosing(processor.run_async(invocation_context, llm_response)) as agen:
                async for event in agen:
                    yield event
    ...
    async def _postprocess_handle_function_calls_async(
        ...
        if function_response_event := await functions.handle_function_calls_async(invocation_context, function_call_event, llm_request.tools_dict):
            auth_event = functions.generate_auth_event(invocation_context, function_response_event)
            if auth_event:
                yield auth_event

            tool_confirmation_event = functions.generate_request_confirmation_event(invocation_context, function_call_event, function_response_event)
    ...
    def _get_agent_to_run(self, invocation_context: InvocationContext, agent_name: str) -> BaseAgent:
    ...
    async def _call_llm_async(
        ...
        if response := await self._handle_before_model_callback(invocation_context, llm_request, model_response_event):
            yield response
            return
        ...
        if _ADK_AGENT_NAME_LABEL_KEY not in llm_request.config.labels:
            llm_request.config.labels[_ADK_AGENT_NAME_LABEL_KEY] = invocation_context.agent.name

        # Calls the LLM.
        llm = self.__get_llm(invocation_context)

        async def _call_llm_with_tracing() -> AsyncGenerator[LlmResponse]:
                ...
                if invocation_context.run_config.support_cfc:
                    ...
                    async with Aclosing(
                        ...
                        async for llm_response in agen:
                            ...
                            if invocation_context.run_config.streaming_mode == StreamingMode.SSE or not llm_response.partial:
                                yield llm_response
    ...
    async def _handle_before_model_callback(
        ...
        callback_context = CallbackContext(invocation_context, event_actions=model_response_event.actions)

        # First run callbacks from the plugins.
        callback_response = await invocation_context.plugin_manager.run_before_model_callback(
        ...
        for callback in agent.canonical_before_model_callbacks:
            callback_response = callback(callback_context=callback_context, llm_request=llm_request)
    ...
    async def _handle_after_model_callback(
        ...
        async def _maybe_add_grounding_metadata(
            ...
            if not any(tool.name == "google_search_agent" for tool in tools):
                return response
            ground_metadata = invocation_context.session.state.get("temp:_adk_grounding_metadata", None)
        ...
        # First run callbacks from the plugins.
        callback_response = await invocation_context.plugin_manager.run_after_model_callback(
        ...
        for callback in agent.canonical_after_model_callbacks:
            callback_response = callback(callback_context=callback_context, llm_response=llm_response)
            if inspect.isawaitable(callback_response):
                callback_response = await callback_response
            if callback_response:
                return await _maybe_add_grounding_metadata(callback_response)
    ...
    def _finalize_model_response_event(
        ...
        if model_response_event.content:
            ...
            if function_calls:
                ...
                model_response_event.long_running_tool_ids = functions.get_long_running_function_calls(function_calls, llm_request.tools_dict)

        return model_response_event

    async def _handle_control_event_flush(self, invocation_context: InvocationContext, llm_response: LlmResponse) -> list[Event]:
    ...
    async def _run_and_handle_error(
        self,
        response_generator: AsyncGenerator[LlmResponse],
        invocation_context: InvocationContext,
        llm_request: LlmRequest,
        model_response_event: Event,
    ) -> AsyncGenerator[LlmResponse]:
        ...
        async def _run_on_model_error_callbacks(
            *,
            ...
            error_response = await invocation_context.plugin_manager.run_on_model_error_callback(
                callback_context=callback_context,
                llm_request=llm_request,
                error=error,
            )
