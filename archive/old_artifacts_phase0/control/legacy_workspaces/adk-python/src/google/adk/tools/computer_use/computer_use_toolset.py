...
class ComputerUseToolset(BaseToolset):
    ...
    async def adapt_computer_use_tool(
        method_name: str,
        adapter_func: Callable[[Callable[..., Any]], Callable[..., Any]] | Callable[[Callable[..., Any]], Any],
        ...
        if method_name in EXCLUDED_METHODS:
            logger.warning("Method %s is not a valid BaseComputer method", method_name)
            return
    ...
    async def process_llm_request(self, *, tool_context: ToolContext, llm_request: LlmRequest) -> None:
        ...
        try:
            ...
            llm_request.config.tools.append(types.Tool(computer_use=types.ComputerUse(environment=environment)))
            logger.debug(
