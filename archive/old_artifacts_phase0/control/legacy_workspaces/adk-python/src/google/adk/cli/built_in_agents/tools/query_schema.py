...
async def query_schema(
    ...
    try:
        ...
        if query_type == "overview":
            ...
            return {
                "error": (f"Invalid query_type '{query_type}' or missing required parameters"),
                "supported_queries": [
                    "overview - Get high-level schema structure",
                    ("component - Get details for specific component (requires component parameter)"),
                    ("field - Get details for specific field (requires field_path parameter)"),
...
def _get_component_details(schema: dict[str, Any], component: str) -> dict[str, Any]:
...
def _get_field_details(schema: dict[str, Any], field_path: str) -> dict[str, Any]:
    ...
    for i, part in enumerate(path_parts):
        if not isinstance(current, dict) or part not in current:
            return {
                ...
                "available_at_level": (list(current.keys()) if isinstance(current, dict) else []),
            }
        ...
        elif field_info.get("type") == "array" and field_info.get("items", {}).get("type") == "object":
        ...
        for key, value in obj.items():
            if key == "properties" and isinstance(value, dict):
                for prop_name, prop_details in value.items():
                    ...
                    elif prop_details.get("type") == "array" and prop_details.get("items", {}).get("type") == "object":
