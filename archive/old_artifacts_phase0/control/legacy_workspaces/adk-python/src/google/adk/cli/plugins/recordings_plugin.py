...
class RecordingsPlugin(BasePlugin):
    ...
    async def before_model_callback(self, *, callback_context: CallbackContext, llm_request: LlmRequest) -> LlmResponse | None:
        ...
        if (state := self._get_invocation_state(callback_context)) is None:
            raise ValueError("Recording state not initialized. Ensure before_run created it.")
        ...
        state.pending_llm_recordings[callback_context.agent_name] = pending_recording
        ...
        *,
        ...
        pending_recording = Recording(
            ...
            tool_recording=ToolRecording(
                tool_call=types.FunctionCall(id=function_call_id, name=tool.name, args=tool_args),
                tool_response=None,
            ),
        )
    ...
    async def after_tool_callback(
        ...
        *,
        ...
        if not (function_call_id := tool_context.function_call_id):
            logger.warning(
                "No function_call_id provided for tool %s result, skipping completion",
                tool.name,
            )
            return None  # Continue tool execution
        ...
        if pending_recording := state.pending_tool_recordings.pop(function_call_id, None):
    ...
    def _get_invocation_state(self, callback_context: CallbackContext) -> _InvocationRecordingState | None:
        """Get existing recording state for this invocation."""
        invocation_id = callback_context.invocation_id
        return self._invocation_states.get(invocation_id)

    def _create_invocation_state(self, callback_context: CallbackContext) -> _InvocationRecordingState:
        ...
        if recordings_file.exists():
            try:
                ...
                logger.error("Failed to load recordings from %s: %s", recordings_file, e)
