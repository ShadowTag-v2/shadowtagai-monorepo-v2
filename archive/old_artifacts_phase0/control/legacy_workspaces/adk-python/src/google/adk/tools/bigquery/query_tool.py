...
def _execute_sql(
    ...
    try:
        ...
        if not settings or settings.write_mode == WriteMode.BLOCKED:
            ...
            if bq_session_info:
                ...
                session_creator_job = bq_client.query(
                    "SELECT 1",
                    project=project_id,
                    job_config=bigquery.QueryJobConfig(dry_run=True, create_session=True, labels=bq_job_labels),
                )
            ...
            bq_connection_properties.append(bigquery.ConnectionProperty("session_id", bq_session_id))
        ...
        if settings.max_query_result_rows is not None and len(rows) == settings.max_query_result_rows:
...
def forecast(
    ...
    *,
    ...
    if trimmed_upper_history_data.startswith("SELECT") or trimmed_upper_history_data.startswith("WITH"):
...
def analyze_contribution(
    ...
    model_name = f"contribution_analysis_model_{str(uuid.uuid4()).replace('-', '_')}"
    ...
    if trimmed_upper_input_data.startswith("SELECT") or trimmed_upper_input_data.startswith("WITH"):
  ...
  """
    ...
    try:
        ...
        if execute_sql_settings.write_mode == WriteMode.BLOCKED:
            ...
            # session.
            execute_sql_settings = settings.model_copy(update={"write_mode": WriteMode.PROTECTED})
...
def detect_anomalies(
    ...
    *,
    ...
    if times_series_id_cols:
        if not all(isinstance(item, str) for item in times_series_id_cols):
            return {
                "status": "ERROR",
                "error_details": ("All elements in times_series_id_cols must be strings."),
            }
        times_series_id_cols_str = "[" + ", ".join([f"'{col}'" for col in times_series_id_cols]) + "]"
  ...
  """
    order_by_id_cols = ", ".join(col for col in times_series_id_cols) + ", " if times_series_id_cols else ""
  ...
  """
    if target_data:
        ...
        if trimmed_upper_target_data.startswith("SELECT") or trimmed_upper_target_data.startswith("WITH"):
