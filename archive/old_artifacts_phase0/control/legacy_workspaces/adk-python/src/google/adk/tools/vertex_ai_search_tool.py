...
from ..utils.model_name_utils import is_gemini_1_model, is_gemini_model
...
class VertexAiSearchTool(BaseTool):
    ...
    def __init__(
        ...
        *,
        ...
        if (data_store_id is None and search_engine_id is None) or (data_store_id is not None and search_engine_id is not None):
        ...
        if data_store_specs is not None and search_engine_id is None:
            raise ValueError("search_engine_id must be specified if data_store_specs is specified.")
        ...
        *,
        ...
        if is_gemini_model(llm_request.model):
            if is_gemini_1_model(llm_request.model) and llm_request.config.tools:
                raise ValueError("Vertex AI search tool cannot be used with other tools in Gemini 1.x.")
            llm_request.config = llm_request.config or types.GenerateContentConfig()
            llm_request.config.tools = llm_request.config.tools or []

            # Format data_store_specs concisely for logging
            if self.data_store_specs:
                spec_ids = [spec.data_store.split("/")[-1] if spec.data_store else "unnamed" for spec in self.data_store_specs]
                specs_info = f"{len(self.data_store_specs)} spec(s): [{', '.join(spec_ids)}]"
            else:
                specs_info = None

            logger.debug(
                "Adding Vertex AI Search tool config to LLM request: datastore=%s, engine=%s, filter=%s, max_results=%s, data_store_specs=%s",
                self.data_store_id,
            ...
            raise ValueError(f"Vertex AI search tool is not supported for model {llm_request.model}")
