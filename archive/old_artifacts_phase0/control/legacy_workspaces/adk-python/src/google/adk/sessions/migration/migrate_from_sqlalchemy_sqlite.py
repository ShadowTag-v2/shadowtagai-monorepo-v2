        ...
        for item in app_states:
            dest_cursor.execute(
                ...
                (
                    ...
                    item.update_time.replace(tzinfo=UTC).timestamp(),
                ),
            )
        ...
        for item in sessions:
            dest_cursor.execute(
                ...
                (
                    ...
                    item.id,
                    json.dumps(item.state),
                    item.create_time.replace(tzinfo=UTC).timestamp(),
                    item.update_time.replace(tzinfo=UTC).timestamp(),
                ),
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=("Migrate ADK sessions from an existing SQLAlchemy-based SQLite database to a new SQLite database with JSON events.")
    )
    parser.add_argument(
        "--source_db_path",
        required=True,
