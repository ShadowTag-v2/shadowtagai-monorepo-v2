...
class LoggingPlugin(BasePlugin):
    ...
    async def on_user_message_callback(
        ...
        *,
        ...
        self._log(f"   Root Agent: {invocation_context.agent.name if hasattr(invocation_context.agent, 'name') else 'Unknown'}")
    ...
    async def before_run_callback(self, *, invocation_context: InvocationContext) -> types.Content | None:
        ...
        self._log(f"   Invocation ID: {invocation_context.invocation_id}")
        self._log(f"   Starting Agent: {invocation_context.agent.name if hasattr(invocation_context.agent, 'name') else 'Unknown'}")
        return None
    ...
    async def after_run_callback(self, *, invocation_context: InvocationContext) -> Optional[None]:
        """Log invocation completion."""
        self._log("✅ INVOCATION COMPLETED")
        self._log(f"   Invocation ID: {invocation_context.invocation_id}")
        self._log(f"   Final Agent: {invocation_context.agent.name if hasattr(invocation_context.agent, 'name') else 'Unknown'}")
        return None
    ...
    def _format_content(self, content: types.Content | None, max_length: int = 200) -> str:
