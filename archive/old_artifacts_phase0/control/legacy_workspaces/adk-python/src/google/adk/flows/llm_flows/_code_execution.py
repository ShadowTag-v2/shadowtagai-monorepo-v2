...
from ...code_executors.code_execution_utils import CodeExecutionInput, CodeExecutionResult, CodeExecutionUtils, File
...
def _extract_and_replace_inline_files(
    ...
    for i in range(len(llm_request.contents)):
        ...
        for j in range(len(content.parts)):
            ...
            llm_request.contents[i].parts[j] = types.Part(text=f"\nAvailable file: `{file_name}`\n")
