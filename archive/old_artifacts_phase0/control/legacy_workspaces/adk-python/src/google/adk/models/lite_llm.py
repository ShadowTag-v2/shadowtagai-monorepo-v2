...
_SUPPORTED_FILE_CONTENT_MIME_TYPES = set(["application/pdf", "application/json"])
...
def _convert_reasoning_value_to_parts(reasoning_value: Any) -> list[types.Part]:
    ...
    return [types.Part(text=text, thought=True) for text in _iter_reasoning_texts(reasoning_value) if text]
...
class LiteLLMClient:
    ...
    async def acompletion(self, model, messages, tools, **kwargs) -> ModelResponse | CustomStreamWrapper:
    ...
    def completion(self, model, messages, tools, stream=False, **kwargs) -> ModelResponse | CustomStreamWrapper:
...
def _append_fallback_user_content_if_missing(
    ...
    for content in reversed(llm_request.contents):
        if content.role == "user":
            ...
            if not parts:
                content.parts = []
            content.parts.append(types.Part.from_text(text="Handle the requests as specified in the System Instruction."))
            return
    llm_request.contents.append(
        types.Content(
            role="user",
            parts=[
                types.Part.from_text(text=("Handle the requests as specified in the System Instruction.")),
            ],
        )
    )
...
def _extract_cached_prompt_tokens(usage: Any) -> int:
    ...
    try:
        ...
        if isinstance(details, dict):
            ...
            total = sum(item.get("cached_tokens", 0) for item in details if isinstance(item, dict) and isinstance(item.get("cached_tokens"), int))
            if total > 0:
                return total
...
def _content_to_message_param(
    ...
    if role == "user":
        ...
        if final_content and isinstance(final_content, list):
            ...
            final_content = final_content[0].get("text", "") if final_content[0].get("type", None) == "text" else final_content
    ...
    for part in parts:
        if part.text:
            ...
            if part.inline_data.mime_type.startswith("image"):
                ...
                raise ValueError(f"LiteLlm(BaseLlm) does not support content part with MIME type {part.inline_data.mime_type}.")
...
def _build_tool_call_from_json_dict(candidate: Any, *, index: int) -> ChatCompletionMessageToolCall | None:
...
def _parse_tool_calls_from_text(
    text_block: str,
) -> tuple[list[ChatCompletionMessageToolCall], str | None]:
...
def _split_message_content_and_tool_calls(
    message: Message,
) -> tuple[OpenAIMessageContent | None, list[ChatCompletionMessageToolCall]]:
    """Returns message content and tool calls, parsing inline JSON when needed."""

    existing_tool_calls = message.get("tool_calls") or []
    normalized_tool_calls = list(existing_tool_calls) if existing_tool_calls else []
...
def _to_litellm_role(role: str | None) -> Literal["user", "assistant"]:
...
def _schema_to_dict(schema: types.Schema | dict[str, Any]) -> dict:
    ...
    """
    schema_dict = schema.model_dump(exclude_none=True) if isinstance(schema, types.Schema) else dict(schema)
    ...
    if "items" in schema_dict:
        ...
        schema_dict["items"] = _schema_to_dict(items) if isinstance(items, (types.Schema, dict)) else items
...
def _function_declaration_to_tool_param(
    ...
    required_fields = getattr(function_declaration.parameters, "required", None) if function_declaration.parameters else None
    if required_fields:
...
def _model_response_to_chunk(
...
) -> Generator[tuple[TextChunk | FunctionChunk | UsageMetadataChunk | ReasoningChunk | None, str | None]]:
    ...
    if response.get("choices", None):
        ...
        message_content: OpenAIMessageContent | None = None
...
def _model_response_to_generate_content_response(
    ...
    thought_parts = _convert_reasoning_value_to_parts(_extract_reasoning_value(message))
    ...
    if finish_reason:
        ...
        if isinstance(finish_reason, types.FinishReason):
            ...
            llm_response.finish_reason = _FINISH_REASON_MAPPING.get(finish_reason_str, types.FinishReason.OTHER)
    if response.get("usage", None):
        llm_response.usage_metadata = types.GenerateContentResponseUsageMetadata(
            ...
            cached_content_token_count=_extract_cached_prompt_tokens(response["usage"]),
        )
    return llm_response
...
def _to_litellm_response_format(
    ...
    if isinstance(response_schema, dict):
        ...
        if isinstance(schema_type, str) and schema_type.lower() in _LITELLM_STRUCTURED_TYPES:
...
def _get_completion_inputs(
    ...
    if llm_request.config and llm_request.config.tools and llm_request.config.tools[0].function_declarations:
        tools = [_function_declaration_to_tool_param(tool) for tool in llm_request.config.tools[0].function_declarations]
...
def _build_function_declaration_log(
    ...
    if func_decl.parameters and func_decl.parameters.properties:
        param_str = str({k: v.model_dump(exclude_none=True) for k, v in func_decl.parameters.properties.items()})
    return_str = "None"
...
def _build_request_log(req: LlmRequest) -> str:
    ...
    function_logs = [_build_function_declaration_log(func_decl) for func_decl in function_decls] if function_decls else []
    contents_logs = [
        content.model_dump_json(
            exclude_none=True,
            exclude={"parts": {i: _EXCLUDED_PART_FIELD for i in range(len(content.parts))}},
        )
...
def _warn_gemini_via_litellm(model_string: str) -> None:
    ...
    if os.environ.get("ADK_SUPPRESS_GEMINI_LITELLM_WARNINGS", "").strip().lower() in ("1", "true", "yes", "on"):
        return
...
class LiteLlm(BaseLlm):
    ...
    async def generate_content_async(self, llm_request: LlmRequest, stream: bool = False) -> AsyncGenerator[LlmResponse]:
        ...
        if stream:
            ...
            async for part in await self.llm_client.acompletion(**completion_args):
                for chunk, finish_reason in _model_response_to_chunk(part):
                    ...
                    if (finish_reason == "tool_calls" or finish_reason == "stop") and function_calls:
                        tool_calls = []
                        ...
                        aggregated_llm_response = _message_to_generate_content_response(
                            ChatCompletionAssistantMessage(role="assistant", content=message_content),
                            model_version=part.model,
                            thought_parts=list(reasoning_parts) if reasoning_parts else None,
                        )
                        text = ""
