...
if TYPE_CHECKING:
    ...
    from google.genai.interactions_types import Interaction, InteractionSSEEvent
...
def convert_part_to_interaction_content(part: types.Part) -> dict | None:
    ...
    if part.text is not None:
        ...
        if part.thought_signature is not None:
            result["thought_signature"] = base64.b64encode(part.thought_signature).decode("utf-8")
        return result
        ...
        if part.thought_signature is not None:
            result["signature"] = base64.b64encode(part.thought_signature).decode("utf-8")
        return result
                ...
                if func_decl.parameters:
                    ...
                    if func_decl.parameters.properties:
                        ...
                        if func_decl.parameters.required:
                            func_tool["parameters"]["required"] = list(func_decl.parameters.required)
...
def convert_interaction_output_to_part(output: Output) -> types.Part | None:
    ...
    if output_type == "text":
    ...
    elif output_type == "image" or output_type == "audio":
        if output.data:
            return types.Part(
        ...
        return types.Part(
            code_execution_result=types.CodeExecutionResult(
                ...
                outcome=types.Outcome.OUTCOME_FAILED if output.is_error else types.Outcome.OUTCOME_OK,
            )
        )
...
def convert_interaction_to_llm_response(
    ...
    if interaction.usage:
        usage_metadata = types.GenerateContentResponseUsageMetadata(
            ...
            total_token_count=((interaction.usage.total_input_tokens or 0) + (interaction.usage.total_output_tokens or 0)),
        )
...
def convert_interaction_event_to_llm_response(
    ...
    if event_type == "content.delta":
        ...
        if status in ("completed", "requires_action"):
            return LlmResponse(
                content=types.Content(role="model", parts=list(aggregated_parts)) if aggregated_parts else None,
                partial=False,
                turn_complete=True,
                finish_reason=types.FinishReason.STOP,
                interaction_id=interaction_id,
...
def build_interactions_request_log(
    ...
    config_str = json.dumps(generation_config, default=str) if generation_config else "{}"
...
async def generate_content_via_interactions(
    ...
    # Log the request
    logger.info(
        "Sending request via interactions API, model: %s, stream: %s, previous_interaction_id: %s",
        llm_request.model,
        stream,
    ...
    if stream:
        ...
        async for event in responses:
            ...
            llm_response = convert_interaction_event_to_llm_response(event, aggregated_parts, current_interaction_id)
            if llm_response:
                yield llm_response
