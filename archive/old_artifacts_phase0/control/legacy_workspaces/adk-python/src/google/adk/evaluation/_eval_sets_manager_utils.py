...
def get_eval_case_from_eval_set(eval_set: EvalSet, eval_case_id: str) -> EvalCase | None:
...
def add_eval_case_to_eval_set(eval_set: EvalSet, eval_case: EvalCase) -> EvalSet:
    ...
    if [x for x in eval_set.eval_cases if x.eval_id == eval_case_id]:
        raise ValueError(
            f"Eval id `{eval_case_id}` already exists in `{eval_set.eval_set_id}` eval set.",
        )
...
def update_eval_case_in_eval_set(eval_set: EvalSet, updated_eval_case: EvalCase) -> EvalSet:
    ...
    if not eval_case_to_update:
        raise NotFoundError(f"Eval case `{eval_case_id}` not found in eval set `{eval_set.eval_set_id}`.")
