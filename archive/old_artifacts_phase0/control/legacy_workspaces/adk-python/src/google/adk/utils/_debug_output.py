        ...
        if part.text:
            ...
            if verbose:
                ...
                if part.function_call:
                    print(f"{event.author} > [Calling tool: {part.function_call.name}({_truncate(str(part.function_call.args), _ARGS_MAX_LEN)})]")
                # Handle function response parts (tool results)
                elif part.function_response:
                    print(f"{event.author} > [Tool result: {_truncate(str(part.function_response.response), _RESPONSE_MAX_LEN)}]")
                ...
                # Handle code execution result parts
                    ...
                    print(f"{event.author} > [Code output: {_truncate(str(output), _CODE_OUTPUT_MAX_LEN)}]")
