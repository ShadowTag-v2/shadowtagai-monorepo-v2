...
from .service_registry import get_service_registry, load_services_module
from .utils import envs, evals
...
def get_fast_api_app(
    *,
    ...
    if memory_service_uri:
        memory_service = service_registry.create_memory_service(memory_service_uri, agents_dir=agents_dir)
    ...
    if session_service_uri:
        ...
        session_service = service_registry.create_session_service(session_service_uri, agents_dir=agents_dir, **session_kwargs)
        if not session_service:
            ...
            session_service = DatabaseSessionService(db_url=session_service_uri, **session_kwargs)
    else:
        session_service = InMemorySessionService()

    # Build the Artifact service
    if artifact_service_uri:
        artifact_service = service_registry.create_artifact_service(artifact_service_uri, agents_dir=agents_dir)
    ...
    async def builder_build(files: list[UploadFile], tmp: bool | None = False) -> bool:
