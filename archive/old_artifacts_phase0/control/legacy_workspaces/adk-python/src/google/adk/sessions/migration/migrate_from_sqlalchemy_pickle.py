...
from sqlalchemy import Boolean, ForeignKeyConstraint, Text, create_engine, func, text
from sqlalchemy.dialects import mysql
from sqlalchemy.ext.mutable import MutableDict
from sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column, sessionmaker
from sqlalchemy.types import PickleType, String, TypeDecorator
...
class OldStorageSession(OldBase):
    ...
    app_name: Mapped[str] = mapped_column(String(dss.DEFAULT_MAX_KEY_LENGTH), primary_key=True)
    user_id: Mapped[str] = mapped_column(String(dss.DEFAULT_MAX_KEY_LENGTH), primary_key=True)
    id: Mapped[str] = mapped_column(String(dss.DEFAULT_MAX_KEY_LENGTH), primary_key=True)
    state: Mapped[MutableDict[str, Any]] = mapped_column(MutableDict.as_mutable(dss.DynamicJSON), default={})
    create_time: Mapped[datetime] = mapped_column(dss.PreciseTimestamp, default=func.now())
    update_time: Mapped[datetime] = mapped_column(dss.PreciseTimestamp, default=func.now(), onupdate=func.now())
...
class OldStorageEvent(OldBase):
    ...
    app_name: Mapped[str] = mapped_column(String(dss.DEFAULT_MAX_KEY_LENGTH), primary_key=True)
    user_id: Mapped[str] = mapped_column(String(dss.DEFAULT_MAX_KEY_LENGTH), primary_key=True)
    session_id: Mapped[str] = mapped_column(String(dss.DEFAULT_MAX_KEY_LENGTH), primary_key=True)
    invocation_id: Mapped[str] = mapped_column(String(dss.DEFAULT_MAX_VARCHAR_LENGTH))
    ...
    timestamp: Mapped[datetime] = mapped_column(dss.PreciseTimestamp, default=func.now())
    content: Mapped[dict[str, Any]] = mapped_column(dss.DynamicJSON, nullable=True)
    grounding_metadata: Mapped[dict[str, Any]] = mapped_column(dss.DynamicJSON, nullable=True)
    custom_metadata: Mapped[dict[str, Any]] = mapped_column(dss.DynamicJSON, nullable=True)
    usage_metadata: Mapped[dict[str, Any]] = mapped_column(dss.DynamicJSON, nullable=True)
    citation_metadata: Mapped[dict[str, Any]] = mapped_column(dss.DynamicJSON, nullable=True)
    ...
    input_transcription: Mapped[dict[str, Any]] = mapped_column(dss.DynamicJSON, nullable=True)
    output_transcription: Mapped[dict[str, Any]] = mapped_column(dss.DynamicJSON, nullable=True)
    __table_args__ = (
...
def _row_to_event(row: dict) -> Event:
    ...
    if actions_val is not None:
        try:
            ...
            logger.warning(f"Failed to unpickle actions for event {row.get('id')}: {e}")
            actions = None
    ...
    if long_running_tool_ids_json:
        try:
            ...
            logger.warning(f"Failed to decode long_running_tool_ids_json for event {row.get('id')}")
            long_running_tool_ids = set()
    ...
    return Event(
        ...
        actions=actions,
        timestamp=timestamp.replace(tzinfo=UTC).timestamp(),
        ...
        grounding_metadata=_session_util.decode_model(grounding_metadata_dict, types.GroundingMetadata),
        usage_metadata=_session_util.decode_model(usage_metadata_dict, types.GenerateContentResponseUsageMetadata),
        citation_metadata=_session_util.decode_model(citation_metadata_dict, types.CitationMetadata),
        input_transcription=_session_util.decode_model(input_transcription_dict, types.Transcription),
        output_transcription=_session_util.decode_model(output_transcription_dict, types.Transcription),
    )
...
def _get_state_dict(state_val: Any) -> dict:
    ...
    if isinstance(state_val, str):
        try:
            ...
            logger.warning("Failed to parse state JSON string, defaulting to empty dict.")
            return {}
    return {}
    ...
    with SourceSession() as source_session, DestSession() as dest_session:
        ...
        try:
            ...
            if inspector.has_table("app_states"):
                rows = source_session.execute(text("SELECT * FROM app_states")).mappings().all()
                for row in rows:
                    dest_session.merge(
                        dss.StorageAppState(
            ...
            if inspector.has_table("user_states"):
                rows = source_session.execute(text("SELECT * FROM user_states")).mappings().all()
                for row in rows:
                    dest_session.merge(
            ...
            if inspector.has_table("events"):
                rows = source_session.execute(text("SELECT * FROM events")).mappings().all()
                for row in rows:
                    try:
                        ...
                        new_event = dss.StorageEvent(
                            ...
                            timestamp=datetime.fromtimestamp(event_obj.timestamp, UTC).replace(tzinfo=None),
                            event_data=event_obj.model_dump(mode="json", exclude_none=True),
                        )
                        dest_session.merge(new_event)
                        events.append(new_event)
                    except Exception as e:
                        logger.warning(f"Failed to migrate event row {row.get('id', 'N/A')}: {e}")
                dest_session.commit()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=("Migrate ADK sessions from SQLAlchemy Pickle format to JSON format."))
    parser.add_argument("--source_db_url", required=True, help="SQLAlchemy URL of source database")
    parser.add_argument(
        "--dest_db_url",
        required=True,
