...
from .user_simulator_personas import UserBehavior, UserPersona, UserPersonaRegistry
...
class PreBuiltBehaviors(enum.Enum):
    ...
    ANSWER_RELEVANT_ONLY = UserBehavior(
        ...
        violation_rubrics=["The Agent asked a question that is not relevant to the high-level goal and the Generated User Response responds to it."],
    )
    ...
    CORRECT_AGENT = UserBehavior(
        ...
        description=("The User Simulator should catch and correct the Agent's mistakes."),
        behavior_instructions=[
    ...
    DO_NOT_CORRECT_AGENT = UserBehavior(
        ...
        description=("The User Simulator should end the conversation when the Agent provides an illogical or incorrect statement."),
        behavior_instructions=[
            ("If the Agent made an illogical or incorrect statement, end the conversation with `{{ stop_signal }}`."),
        ],
        violation_rubrics=["The Agent makes a mistake or an assumption and the Generated User Response corrects the Agent."],
    )
    ...
    TONE_PROFESSIONAL = UserBehavior(
        ...
        description=("The User Simulator use clear, technical language. NOTE: `{{ stop_signal }}` is appropriate language."),
        behavior_instructions=[
    ...
    TONE_CONVERSATIONAL = UserBehavior(
        ...
        description=("The User Simulator sounds informal. NOTE: `{{ stop_signal }}` is appropriate language."),
        behavior_instructions=[
...
class _PreBuiltPersonas(enum.Enum):
    ...
    EVALUATOR = UserPersona(
        id="EVALUATOR",
        description=("An Evaluator is trying to assess whether the Agent can help accomplish the goals in the Conversation Plan."),
        behaviors=[
            PreBuiltBehaviors.ADVANCE_DETAIL_ORIENTED.value,
            PreBuiltBehaviors.ANSWER_RELEVANT_ONLY.value,
            PreBuiltBehaviors.END_NO_TROUBLESHOOTING.value,
...
def get_default_persona_registry() -> UserPersonaRegistry:
    ...
    registry.register_persona(_PreBuiltPersonas.EXPERT.value.id, _PreBuiltPersonas.EXPERT.value)
    registry.register_persona(_PreBuiltPersonas.NOVICE.value.id, _PreBuiltPersonas.NOVICE.value)
    registry.register_persona(_PreBuiltPersonas.EVALUATOR.value.id, _PreBuiltPersonas.EVALUATOR.value)
