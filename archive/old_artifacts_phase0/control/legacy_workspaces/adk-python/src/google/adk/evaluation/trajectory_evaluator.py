...
from .eval_case import Invocation, get_all_tool_calls
from .eval_metrics import EvalMetric, Interval, MetricInfo, MetricValueInfo, PrebuiltMetrics, ToolTrajectoryCriterion
...
class TrajectoryEvaluator(Evaluator):
    ...
    def __init__(
        ...
        if eval_metric and eval_metric.criterion:
            try:
                criterion = TrajectoryEvaluator.criterion_type.model_validate(eval_metric.criterion.model_dump())
                self._threshold = criterion.threshold
                self._match_type = criterion.match_type
            except ValidationError as e:
                expected_criterion_type_error = ValueError(
                    f"`{eval_metric.metric_name}` metric expects a criterion of type `{TrajectoryEvaluator.criterion_type}`."
                )
    ...
    def _calculate_tool_use_accuracy(
        ...
        if self._match_type == ToolTrajectoryCriterion.MatchType.EXACT:
            tool_use_match_status = self._are_tool_calls_exact_match(actual_tool_uses, expected_tool_uses)
        elif self._match_type == ToolTrajectoryCriterion.MatchType.IN_ORDER:
            tool_use_match_status = self._are_tool_calls_in_order_match(actual_tool_uses, expected_tool_uses)
        elif self._match_type == ToolTrajectoryCriterion.MatchType.ANY_ORDER:
            tool_use_match_status = self._are_tool_calls_any_order_match(actual_tool_uses, expected_tool_uses)
        else:
    ...
    def _are_tool_calls_in_order_match(
        ...
        try:
            ...
            for actual in actual_tool_calls:
                if actual.name == current_expected.name and actual.args == current_expected.args:
                    current_expected = next(expected_it)
