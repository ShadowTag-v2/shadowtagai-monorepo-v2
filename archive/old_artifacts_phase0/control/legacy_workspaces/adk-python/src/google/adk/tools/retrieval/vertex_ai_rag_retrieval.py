    ...
    async def process_llm_request(
        ...
        *,
        ...
        if is_gemini_2_or_above(llm_request.model):
            llm_request.config = types.GenerateContentConfig() if not llm_request.config else llm_request.config
            llm_request.config.tools = [] if not llm_request.config.tools else llm_request.config.tools
            llm_request.config.tools.append(types.Tool(retrieval=types.Retrieval(vertex_rag_store=self.vertex_rag_store)))
        else:
