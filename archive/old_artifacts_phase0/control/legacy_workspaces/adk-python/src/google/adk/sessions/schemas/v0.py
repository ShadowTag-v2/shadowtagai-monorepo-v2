...
from sqlalchemy import Boolean, ForeignKeyConstraint, Text, func, inspect
...
from .shared import DEFAULT_MAX_KEY_LENGTH, DEFAULT_MAX_VARCHAR_LENGTH, DynamicJSON, PreciseTimestamp
...
class StorageSession(Base):
    ...
    def update_timestamp_tz(self) -> float:
        ...
        is_sqlite = bool(sqlalchemy_session and sqlalchemy_session.bind and sqlalchemy_session.bind.dialect.name == "sqlite")
...
class StorageEvent(Base):
    ...
    citation_metadata: Mapped[dict[str, Any]] = mapped_column(DynamicJSON, nullable=True)
    ...
    input_transcription: Mapped[dict[str, Any]] = mapped_column(DynamicJSON, nullable=True)
    output_transcription: Mapped[dict[str, Any]] = mapped_column(DynamicJSON, nullable=True)
    ...
    def from_event(cls, session: Session, event: Event) -> StorageEvent:
        ...
        if event.citation_metadata:
            storage_event.citation_metadata = event.citation_metadata.model_dump(exclude_none=True, mode="json")
        if event.input_transcription:
            storage_event.input_transcription = event.input_transcription.model_dump(exclude_none=True, mode="json")
        if event.output_transcription:
            storage_event.output_transcription = event.output_transcription.model_dump(exclude_none=True, mode="json")
    ...
    def to_event(self) -> Event:
        return Event(
            ...
            grounding_metadata=_session_util.decode_model(self.grounding_metadata, types.GroundingMetadata),
            usage_metadata=_session_util.decode_model(self.usage_metadata, types.GenerateContentResponseUsageMetadata),
            citation_metadata=_session_util.decode_model(self.citation_metadata, types.CitationMetadata),
            input_transcription=_session_util.decode_model(self.input_transcription, types.Transcription),
            output_transcription=_session_util.decode_model(self.output_transcription, types.Transcription),
        )
