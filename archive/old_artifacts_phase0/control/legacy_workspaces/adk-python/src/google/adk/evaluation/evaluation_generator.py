...
from .app_details import AgentDetails, AppDetails
from .eval_case import EvalCase, Invocation, InvocationEvent, InvocationEvents, SessionInput
...
class EvaluationGenerator:
    ...
    async def _process_query(
        ...
        initial_session: SessionInput | None = None,
    ) -> list[Invocation]:
    ...
    async def _generate_inferences_from_root_agent(
        ...
        request_intercepter_plugin = _RequestIntercepterPlugin(name="request_intercepter_plugin")
        ...
        ensure_retry_options_plugin = EnsureRetryOptionsPlugin(name="ensure_retry_options")
        async with Runner(
            app_name=app_name,
            agent=root_agent,
            ...
            events = []
            while True:
                next_user_message = await user_simulator.get_next_user_message(copy.deepcopy(events))
            ...
            app_details_by_invocation_id = EvaluationGenerator._get_app_details_by_invocation_id(events, request_intercepter_plugin)
            return EvaluationGenerator.convert_events_to_eval_invocations(events, app_details_by_invocation_id)

    @staticmethod
    def convert_events_to_eval_invocations(
        events: list[Event],
        app_details_per_invocation: dict[str, AppDetails] | None = None,
    ) -> list[Invocation]:
        ...
        for invocation_id, events in events_by_invocation_id.items():
            ...
            app_details = None
            if app_details_per_invocation and invocation_id in app_details_per_invocation:
            ...
            invocation_events = [InvocationEvent(author=e.author, content=e.content) for e in events_to_add]
            invocations.append(
                Invocation(
                    invocation_id=invocation_id,
    ...
    @staticmethod
    def _get_app_details_by_invocation_id(events: list[Event], request_intercepter: _RequestIntercepterPlugin) -> dict[str, AppDetails]:
    ...
    def _process_query_with_session(session_data, data):
        ...
        for index, eval_entry in enumerate(responses):
            ...
            for event in session_data.events:
                ...
                if event.author == "user" and event.content and event.content.parts and event.content.parts[0].text == query:
                    ...
                    for subsequent_event in session_data.events:
                        if subsequent_event.invocation_id == event.invocation_id:
                            ...
                            if subsequent_event.content.parts[0].function_call:
                                ...
                                actual_tool_uses.append({"tool_name": call.name, "tool_input": call.args})
