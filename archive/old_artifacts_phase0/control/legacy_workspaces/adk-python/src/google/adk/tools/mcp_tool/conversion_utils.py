...
def gemini_to_json_schema(gemini_schema: Schema) -> dict[str, Any]:
    ...
    # Map Nullable
    if getattr(gemini_schema, "nullable", None):
    ...
    if gemini_type == Type.OBJECT:
        ...
        if properties_dict is not None:
            json_schema_dict["properties"] = {prop_name: gemini_to_json_schema(prop_schema) for prop_name, prop_schema in properties_dict.items()}
    ...
    if any_of_list is not None:
        json_schema_dict["anyOf"] = [gemini_to_json_schema(sub_schema) for sub_schema in any_of_list]
