...
from .auth_schemes import AuthScheme, OpenIdConnectWithConfig
...
def create_oauth2_session(
...
) -> tuple[OAuth2Session | None, str | None]:
    ...
    if isinstance(auth_scheme, OpenIdConnectWithConfig):
        ...
        if auth_scheme.flows.authorizationCode and auth_scheme.flows.authorizationCode.tokenUrl:
            token_endpoint = auth_scheme.flows.authorizationCode.tokenUrl
            scopes = list(auth_scheme.flows.authorizationCode.scopes.keys())
        elif auth_scheme.flows.clientCredentials and auth_scheme.flows.clientCredentials.tokenUrl:
    ...
    if not auth_credential or not auth_credential.oauth2 or not auth_credential.oauth2.client_id or not auth_credential.oauth2.client_secret:
        return None, None
...
@experimental
def update_credential_with_tokens(auth_credential: AuthCredential, tokens: OAuth2Token) -> None:
    ...
    auth_credential.oauth2.expires_at = int(tokens.get("expires_at")) if tokens.get("expires_at") else None
    auth_credential.oauth2.expires_in = int(tokens.get("expires_in")) if tokens.get("expires_in") else None
