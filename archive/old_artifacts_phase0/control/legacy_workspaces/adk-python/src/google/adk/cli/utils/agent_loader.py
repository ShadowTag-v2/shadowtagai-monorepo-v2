...
SPECIAL_AGENTS_DIR = os.path.join(os.path.dirname(__file__), "..", "built_in_agents")
...
class AgentLoader(BaseAgentLoader):
    ...
    def __init__(self, agents_dir: str):
        ...
        self._agent_cache: dict[str, BaseAgent | App] = {}

    def _load_from_module_or_package(self, agent_name: str) -> BaseAgent | App | None:
        ...
        try:
            ...
            if hasattr(e, "msg"):
                e.msg = f"Fail to load '{agent_name}' module. " + e.msg
                raise e
            e.args = (f"Fail to load '{agent_name}' module. {e.args[0] if e.args else ''}",) + e.args[1:]
            raise e

        return None

    def _load_from_submodule(self, agent_name: str) -> Optional[BaseAgent, App]:
        ...
        try:
            ...
            if hasattr(e, "msg"):
                e.msg = f"Fail to load '{agent_name}.agent' module. " + e.msg
                raise e
            e.args = ((f"Fail to load '{agent_name}.agent' module. {e.args[0] if e.args else ''}"),) + e.args[1:]
            raise e

        return None

    @experimental
    def _load_from_yaml_config(self, agent_name: str, agents_dir: str) -> BaseAgent | None:
        ...
        try:
            ...
            if hasattr(e, "msg"):
                ...
                raise e
            e.args = (f"Fail to load '{config_path}' config. {e.args[0] if e.args else ''}",) + e.args[1:]
            raise e

    def _perform_load(self, agent_name: str) -> BaseAgent | App:
        ...
        if agents_path.joinpath("agent.py").is_file() or agents_path.joinpath("root_agent.yaml").is_file():
            hint = (
        ...
        *,
        ...
        if module_name:
            ...
            if spec and spec.origin:
                ...
                origin_path = module_origin.parent if module_origin.is_file() else module_origin

        if origin_path is None:
            candidate = Path(agents_dir, expected_app_name)
            origin_path = candidate if candidate.exists() else Path(agents_dir)

        def _attach_metadata(target: BaseAgent | App) -> None:
            target._adk_origin_app_name = expected_app_name
            target._adk_origin_path = origin_path
    ...
    def list_agents(self) -> list[str]:
        ...
        agent_names = [x for x in os.listdir(base_path) if os.path.isdir(os.path.join(base_path, x)) and not x.startswith(".") and x != "__pycache__"]
        agent_names.sort()
        return agent_names
    ...
    def _determine_agent_language(self, agent_name: str) -> Literal["yaml", "python"]:
        ...
        if (base_path / "root_agent.yaml").exists():
            return "yaml"
        elif (base_path / "agent.py").exists() or (base_path / "__init__.py").exists():
            return "python"
    ...
    def remove_agent_from_cache(self, agent_name: str):
        ...
        keys_to_delete = [module_name for module_name in sys.modules if module_name == agent_name or module_name.startswith(f"{agent_name}.")]
