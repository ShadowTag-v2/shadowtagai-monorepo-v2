    ...
    async def process_llm_request(
        ...
        *,
        ...
        if is_gemini_1_model(llm_request.model):
            raise ValueError("Google Maps grounding tool cannot be used with Gemini 1.x models.")
        elif is_gemini_model(llm_request.model):
            llm_request.config.tools.append(types.Tool(google_maps=types.GoogleMaps()))
        else:
            raise ValueError(f"Google maps tool is not supported for model {llm_request.model}")
