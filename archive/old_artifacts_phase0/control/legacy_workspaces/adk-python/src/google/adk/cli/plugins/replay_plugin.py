...
from .recordings_schema import LlmRecording, Recording, Recordings, ToolRecording
...
class ReplayPlugin(BasePlugin):
    ...
    async def before_model_callback(self, *, callback_context: CallbackContext, llm_request: LlmRequest) -> LlmResponse | None:
        ...
        if (state := self._get_invocation_state(callback_context)) is None:
            raise ReplayConfigError("Replay state not initialized. Ensure before_run created it.")

        agent_name = callback_context.agent_name

        # Verify and get the next LLM recording for this specific agent
        recording = self._verify_and_get_next_llm_recording_for_agent(state, agent_name, llm_request)
    ...
    async def before_tool_callback(
        ...
        *,
        ...
        recording = self._verify_and_get_next_tool_recording_for_agent(state, agent_name, tool.name, tool_args)
    ...
    def _get_invocation_state(self, callback_context: CallbackContext) -> _InvocationReplayState | None:
        """Get existing replay state for this invocation."""
        invocation_id = callback_context.invocation_id
        return self._invocation_states.get(invocation_id)

    def _load_invocation_state(self, callback_context: CallbackContext) -> _InvocationReplayState:
        ...
        if not case_dir or msg_index is None:
            raise ReplayConfigError("Replay parameters are missing from session state")
        ...
        try:
            ...
            raise ReplayConfigError(f"Failed to load recordings from {recordings_file}: {e}") from e
        ...
        logger.debug(
            "Loaded replay state for invocation %s: case_dir=%s, msg_index=%s, recordings=%d",
            invocation_id,
            case_dir,
            msg_index,
    ...
    def _get_next_recording_for_agent(
        ...
        agent_recordings = [
            ...
            if (recording.agent_name == agent_name and recording.user_message_index == state.user_message_index)
        ]
    ...
    def _verify_and_get_next_llm_recording_for_agent(
        ...
        if not expected_recording.llm_recording:
            raise ReplayVerificationError(f"Expected LLM recording for agent '{agent_name}' at index {current_agent_index}, but found tool recording")
    ...
    def _verify_and_get_next_tool_recording_for_agent(
        ...
        if not expected_recording.tool_recording:
            raise ReplayVerificationError(f"Expected tool recording for agent '{agent_name}' at index {current_agent_index}, but found LLM recording")
    ...
    def _verify_llm_request_match(
        ...
        recorded_dict = recorded_request.model_dump(exclude_none=True, exclude=excluded_fields, exclude_defaults=True)
        current_dict = current_request.model_dump(exclude_none=True, exclude=excluded_fields, exclude_defaults=True)
