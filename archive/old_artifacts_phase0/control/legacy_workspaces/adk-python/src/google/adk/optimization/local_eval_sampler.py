...
from ..evaluation.base_eval_service import EvaluateConfig, EvaluateRequest, InferenceConfig, InferenceRequest, InferenceResult
from ..evaluation.eval_case import IntermediateData, Invocation, InvocationEvents, get_all_tool_calls_with_responses
from ..evaluation.eval_config import EvalConfig, get_eval_metrics_from_config
...
class LocalEvalSamplerConfig(BaseModel):
    ...
    train_eval_case_ids: list[str] | None = Field(
        default=None,
        description=("The ids of the eval cases to use for optimization. If not provided, all eval cases in the train_eval_set will be used."),
    )

    validation_eval_set: str | None = Field(
        default=None,
        description=(
    ...
    validation_eval_case_ids: list[str] | None = Field(
        default=None,
        description=(
...
class LocalEvalSampler(Sampler[UnstructuredSamplingResult]):
    ...
    def __init__(
        ...
        self._train_eval_case_ids = self._config.train_eval_case_ids or self._get_eval_case_ids(self._train_eval_set)

        self._validation_eval_set = self._config.validation_eval_set or self._train_eval_set
        if self._config.validation_eval_case_ids:
            ...
            self._validation_eval_case_ids = self._get_eval_case_ids(self._validation_eval_set)
        else:
            self._validation_eval_case_ids = self._train_eval_case_ids

    def _get_selected_example_set_id(self, example_set: Literal[Sampler.TRAIN_SET, Sampler.VALIDATION_SET]) -> str:
    ...
    def _get_all_example_ids(self, example_set: Literal[Sampler.TRAIN_SET, Sampler.VALIDATION_SET]) -> list[str]:
    ...
    def _get_eval_case_ids(self, eval_set_id: str) -> list[str]:
        ...
        if eval_set:
            ...
            raise ValueError(f"Eval set `{eval_set_id}` does not exist for app `{self._config.app_name}`.")
    ...
    async def _evaluate_agent(
        ...
        user_simulator_provider = UserSimulatorProvider(self._config.eval_config.user_simulator_config)
        ...
        async with Aclosing(eval_service.perform_inference(inference_request=inference_request)) as agen:
            inference_results: list[InferenceResult] = [inference_result async for inference_result in agen]
        ...
        async with Aclosing(eval_service.evaluate(evaluate_request=evaluate_request)) as agen:
            eval_results: list[EvalCaseResult] = [eval_result async for eval_result in agen]
    ...
    def _extract_eval_data(
        ...
        for eval_result in eval_results:
            ...
            if eval_case and eval_case.conversation_scenario:
                eval_result_dict["conversation_scenario"] = eval_case.conversation_scenario
            ...
            for per_invocation_result in eval_result.eval_metric_result_per_invocation:
                ...
                per_invocation_result_dict = {
                    "actual_invocation": extract_single_invocation_info(per_invocation_result.actual_invocation),
                    "eval_metric_results": eval_metric_results,
                }
                if per_invocation_result.expected_invocation:
                    per_invocation_result_dict["expected_invocation"] = extract_single_invocation_info(per_invocation_result.expected_invocation)
    ...
    async def sample_and_score(
        self,
        candidate: Agent,
        example_set: Literal[Sampler.TRAIN_SET, Sampler.VALIDATION_SET] = Sampler.VALIDATION_SET,
        ...
        scores = {eval_result.eval_id: (1.0 if eval_result.final_eval_status == EvalStatus.PASSED else 0.0) for eval_result in eval_results}

        eval_data = self._extract_eval_data(eval_set_id, eval_results) if capture_full_eval_data else None
