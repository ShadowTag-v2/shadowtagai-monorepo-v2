        ...
        *,
        ...
        if is_gemini_1_model(llm_request.model):
            if llm_request.config.tools:
                raise ValueError("Google search tool cannot be used with other tools in Gemini 1.x.")
            llm_request.config.tools.append(types.Tool(google_search_retrieval=types.GoogleSearchRetrieval()))
        elif is_gemini_model(llm_request.model):
            llm_request.config.tools.append(types.Tool(google_search=types.GoogleSearch()))
        else:
            raise ValueError(f"Google search tool is not supported for model {llm_request.model}")
