...
from .eval_metrics import BaseCriterion, EvalMetric, RubricScore
...
class LlmAsJudge(Evaluator):
    ...
    def __init__(
        ...
        expected_criterion_type_error = ValueError(f"`{eval_metric.metric_name}` metric expects a criterion of type `{criterion_type}`.")

        try:
            if self._eval_metric.criterion is None:
                raise expected_criterion_type_error

            self._criterion = criterion_type.model_validate(self._eval_metric.criterion.model_dump())
    ...
    @abstractmethod
    def format_auto_rater_prompt(self, actual: Invocation, expected: Invocation | None) -> str:
    ...
    async def evaluate_invocations(
        ...
        for actual, expected in zip(actual_invocations, expected_invocations):
            ...
            for _ in range(num_samples):
                async with Aclosing(self._judge_model.generate_content_async(llm_request)) as agen:
                    async for llm_response in agen:
                        # Non-streaming call, so there is only one response content.
                        auto_rater_score = self.convert_auto_rater_response_to_score(llm_response)
                        invocation_result_samples.append(
                            PerInvocationResult(
                                actual_invocation=actual,
                                expected_invocation=expected,
                                score=auto_rater_score.score,
                                eval_status=get_eval_status(auto_rater_score.score, self._eval_metric.threshold),
                                rubric_scores=auto_rater_score.rubric_scores,
                            )
                        )
            if not invocation_result_samples:
                continue
            per_invocation_results.append(self.aggregate_per_invocation_samples(invocation_result_samples))
