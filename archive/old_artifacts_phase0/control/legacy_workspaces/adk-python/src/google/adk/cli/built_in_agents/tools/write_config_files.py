...
async def write_config_files(
    ...
    for file_path, config_content in configs.items():
        ...
        file_result = _validate_single_config(normalized_input_path, config_content, project_folder_name)
        ...
        if file_result.get("success", False):
            ...
            if rename_applied and sanitized_name and sanitized_name != agent_name:
                warnings = file_result.get("warnings", [])
                warnings.append(f"Agent name normalized for filesystem compatibility: '{agent_name}' -> '{sanitized_name}'")
                file_result["warnings"] = warnings
            ...
            if normalized_key in normalized_path_to_original:
                ...
                file_result["error"] = f"Multiple agent configs target the same file path after normalization: '{conflict_source}' and '{file_path}'"
                result["success"] = False
                continue
            ...
            if canonical_key in canonical_path_to_original:
                ...
                file_result["error_type"] = "FILE_PATH_CONFLICT"
                file_result["error"] = (
                    f"Multiple agent configs resolve to the same file path after normalization: '{conflict_source}' and '{file_path}'"
                )
                result["success"] = False
                continue
...
def _update_sub_agent_references(config_dict: dict[str, Any], reference_map: dict[str, str]) -> None:
...
def _canonical_path_key(path: str, session_state: dict[str, Any] | None) -> str:
    ...
    try:
        ...
        if not validation_result["valid"]:
            return {
                ...
                "retry_suggestion": _generate_retry_suggestion(validation_result["errors"]),
            }
        ...
        project_scope_result = _enforce_project_scoped_references(config_dict, project_folder_name, path)
            ...
            "errors": [
                {
                    ...
                    "constraint": (e.schema.get("type") or e.schema.get("enum") or "unknown"),
                }
            ],
        }
...
def _validate_structure(config: dict[str, Any], file_path: Path) -> dict[str, Any]:
    ...
    if config.get("agent_class", "LlmAgent") == "LlmAgent" and not instruction:
        warnings.append("LlmAgent has empty instruction which may result in poor performance")
        ...
        if isinstance(sub_agent, dict) and "config_path" in sub_agent:
            ...
            if not config_path.startswith("/"):
                ...
                if not referenced_path.exists():
                    warnings.append(f"Referenced sub-agent file may not exist: {config_path}")

            # Check file extension
            if not config_path.endswith((".yaml", ".yml")):
                warnings.append(f"Sub-agent config_path should end with .yaml or .yml: {config_path}")
    ...
    if has_object_format and has_string_format:
        warnings.append("Mixed tool formats detected - consider using consistent object format")
...
def _generate_retry_suggestion(
    ...
    for error in errors:
        ...
        if "required" in message.lower():
            if "name" in message:
                suggestions.append("Add required 'name' field with a descriptive agent name")
            elif "instruction" in message:
                suggestions.append("Add required 'instruction' field with clear agent instructions")
            else:
                suggestions.append(f"Add missing required field mentioned in error at '{path}'")

        elif "enum" in message.lower() or "not one of" in message.lower():
            suggestions.append(f"Use valid enum value for field '{path}' - check schema for allowed values")
            ...
            suggestions.append(f"Remove unrecognized field '{path}' or check for typos")

    if not suggestions:
        suggestions.append("Please fix the validation errors and regenerate the configuration")

    return " | ".join(suggestions[:3])  # Limit to top 3 suggestions


def _require_llm_agent_model(config: dict[str, Any], file_path: Path) -> dict[str, Any] | None:
    ...
    return {
        ...
        "error": (f"LlmAgent '{agent_name}' in '{file_path}' must define a 'model' field. LlmAgents cannot rely on implicit defaults."),
        "file_path": str(file_path),
        "validation_step": "structure_validation",
        "retry_suggestion": ("Add a 'model' field with the user-confirmed model (for example, 'model: gemini-2.5-flash')."),
    }


def _require_valid_agent_name(config: dict[str, Any], file_path: Path) -> dict[str, Any] | None:
    ...
    return {
        ...
        "file_path": str(file_path),
        "validation_step": "structure_validation",
        "retry_suggestion": ("Rename the agent using only letters, digits, and underscores (e.g., 'Paper_Analyzer')."),
    }


def _normalize_agent_name_field(config: dict[str, Any], file_path: Path) -> str | None:
    """Normalize agent name to snake_case and update the config in-place."""
    agent_name = config.get("name")
    if not isinstance(agent_name, str):
        return None

    sanitized_name, normalization_warning = _sanitize_agent_name_for_filename(agent_name)
    if not sanitized_name:
        return normalization_warning

    if sanitized_name != agent_name:
        config["name"] = sanitized_name
        return f"Agent name normalized to snake_case in '{file_path.name}': '{agent_name}' -> '{sanitized_name}'"
...
def _enforce_project_scoped_references(
    ...
    def _normalize_reference_value(value: str, descriptor: str) -> tuple[str, list[str], list[str]]:
        ...
        if value.lower().startswith(prefix.lower()):
            local_errors.append(f"{descriptor} '{value}' must use exact-case prefix '{prefix}'.")
            return new_value, local_warnings, local_errors

        if value.startswith("callbacks.") or value.startswith("tools."):
            new_value = prefix + value
            local_warnings.append(f"{descriptor} '{value}' updated to '{new_value}' to include project prefix.")
            return new_value, local_warnings, local_errors
    ...
    if isinstance(tools, list):
        for index, tool in enumerate(tools):
            if isinstance(tool, str):
                updated, local_warnings, local_errors = _normalize_reference_value(tool, "Tool reference")
                if updated != tool:
                    tools[index] = updated
                ...
                if isinstance(name, str):
                    updated, local_warnings, local_errors = _normalize_reference_value(name, "Tool reference")
                    if updated != name:
                        tool["name"] = updated
                    warnings.extend(local_warnings)
                    errors.extend(local_errors)
    ...
    for field_name in CALLBACK_FIELD_NAMES:
        ...
        items = callbacks_field if isinstance(callbacks_field, list) else [callbacks_field]

        for idx, item in enumerate(items):
            if isinstance(item, str):
                updated, local_warnings, local_errors = _normalize_reference_value(item, f"{field_name} entry")
                if updated != item:
                ...
                name = item.get("name")
                if isinstance(name, str):
                    updated, local_warnings, local_errors = _normalize_reference_value(name, f"{field_name} entry")
                    if updated != name:
                        item["name"] = updated
                    warnings.extend(local_warnings)
                    errors.extend(local_errors)
...
def _determine_target_file_path(file_path: str, agent_name: str | None) -> tuple[str, bool, str | None, str | None]:
    ...
    sanitized_name, sanitize_warning = _sanitize_agent_name_for_filename(agent_name)
    if not sanitized_name:
        return (
            file_path,
            False,
            None,
            ("Agent name could not be converted into a valid file name; original path preserved"),
        )
...
def _sanitize_agent_name_for_filename(
    ...
    if replacements_made:
        warning = f"Agent name normalized to snake_case: '{agent_name}' -> '{sanitized_name}'"
