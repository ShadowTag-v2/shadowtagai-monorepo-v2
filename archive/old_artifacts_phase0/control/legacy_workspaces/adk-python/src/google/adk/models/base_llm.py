...
class BaseLlm(BaseModel):
    ...
    async def generate_content_async(self, llm_request: LlmRequest, stream: bool = False) -> AsyncGenerator[LlmResponse]:
        ...
        """
        raise NotImplementedError(f"Async generation is not supported for {self.model}.")
    ...
    def _maybe_append_user_content(self, llm_request: LlmRequest):
        ...
        if not llm_request.contents:
            llm_request.contents.append(
                types.Content(
                    role="user",
                    parts=[types.Part(text=("Handle the requests as specified in the System Instruction."))],
                )
            )
            return
