...
def _get_code_interpreter_extension(resource_name: str = None):
    ...
    if resource_name:
        ...
        logger.info("No CODE_INTERPRETER_ID found in the environment. Create a new one.")
        new_code_interpreter = Extension.from_hub("code_interpreter")
        os.environ["CODE_INTERPRETER_EXTENSION_NAME"] = new_code_interpreter.gca_resource.name
  ...
  """
    ...
    def __init__(
        ...
        **data,
        ...
        self._code_interpreter_extension = _get_code_interpreter_extension(self.resource_name)
    ...
    def _execute_code_interpreter(
        ...
        if input_files:
            operation_params["files"] = [{"name": f.name, "contents": f.content} for f in input_files]
        if session_id:
