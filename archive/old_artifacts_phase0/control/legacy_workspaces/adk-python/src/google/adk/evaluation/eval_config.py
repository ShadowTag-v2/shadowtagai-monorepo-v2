...
from .eval_metrics import BaseCriterion, Threshold
...
class EvalConfig(BaseModel):
    ...
    criteria: dict[str, Threshold | BaseCriterion] = Field(
...
""",
    ...
    user_simulator_config: BaseUserSimulatorConfig | None = Field(
        default=None,
        description="Config to be used by the user simulator.",
    )


_DEFAULT_EVAL_CONFIG = EvalConfig(criteria={"tool_trajectory_avg_score": 1.0, "response_match_score": 0.8})


def get_evaluation_criteria_or_default(
    eval_config_file_path: str | None,
) -> EvalConfig:
    ...
    if eval_config_file_path and os.path.exists(eval_config_file_path):
        with open(eval_config_file_path, encoding="utf-8") as f:
            content = f.read()
    ...
    if eval_config.criteria:
        for metric_name, criterion in eval_config.criteria.items():
            if isinstance(criterion, float):
                ...
                raise ValueError(f"Unexpected criterion type. {type(criterion).__name__} not supported.")
