...
class ToolContextCredentialStore:
    ...
    def get_credential_key(
        ...
        scheme_name = f"{auth_scheme.type_.name}_{hash(auth_scheme.model_dump_json())}" if auth_scheme else ""
        credential_name = f"{auth_credential.auth_type.value}_{hash(auth_credential.model_dump_json())}" if auth_credential else ""
    ...
    def store_credential(
        ...
        if self.tool_context:
            self.tool_context.state[key] = auth_credential.model_dump(exclude_none=True)
...
class ToolAuthHandler:
    ...
    def __init__(
        ...
        auth_scheme: AuthScheme | None,
        auth_credential: AuthCredential | None,
        credential_exchanger: BaseAuthCredentialExchanger | None = None,
        credential_store: ToolContextCredentialStore | None = None,
    ):
        self.tool_context = tool_context
        self.auth_scheme = auth_scheme.model_copy(deep=True) if auth_scheme else None
        self.auth_credential = auth_credential.model_copy(deep=True) if auth_credential else None
        self.credential_exchanger = credential_exchanger or AutoAuthCredentialExchanger()
    ...
    async def _get_existing_credential(
        ...
        if self.credential_store:
            existing_credential = self.credential_store.get_credential(self.auth_scheme, self.auth_credential)
            if existing_credential:
                if existing_credential.oauth2:
                    ...
                    if await refresher.is_refresh_needed(existing_credential):
                        existing_credential = await refresher.refresh(existing_credential, self.auth_scheme)
                return existing_credential
        return None

    def _exchange_credential(self, auth_credential: AuthCredential) -> AuthPreparationResult | None:
        """Handles an OpenID Connect authorization response."""

        exchanged_credential = None
        try:
            exchanged_credential = self.credential_exchanger.exchange_credential(self.auth_scheme, auth_credential)
    ...
    def _store_credential(self, auth_credential: AuthCredential) -> None:
        ...
        if self.credential_store:
            key = self.credential_store.get_credential_key(self.auth_scheme, self.auth_credential)
    ...
    def _request_credential(self) -> None:
        ...
        if self.auth_scheme.type_ in (
            ...
            if not self.auth_credential or not self.auth_credential.oauth2:
                raise ValueError(f"auth_credential is empty for scheme {self.auth_scheme.type_}.Please create AuthCredential using OAuth2Auth.")

            if not self.auth_credential.oauth2.client_id:
                raise AuthCredentialMissingError("OAuth2 credentials client_id is missing.")

            if not self.auth_credential.oauth2.client_secret:
                raise AuthCredentialMissingError("OAuth2 credentials client_secret is missing.")
