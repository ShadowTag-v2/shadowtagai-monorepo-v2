...
def _is_transcription(event: Event) -> bool:
    return event.input_transcription is not None or event.output_transcription is not None
...
class Runner:
    ...
    def __init__(
        ...
        *,
        ...
        self.plugin_manager = PluginManager(plugins=plugins, close_timeout=plugin_close_timeout)
        (
            self._agent_origin_app_name,
            self._agent_origin_dir,
        ) = self._infer_agent_origin(self.agent)
        self._app_name_alignment_hint: str | None = None
    ...
    def _validate_runner_params(
        ...
        if app:
            if app_name:
                raise ValueError("When app is provided, app_name should not be provided.")
            if agent:
                raise ValueError("When app is provided, agent should not be provided.")
            if plugins:
                raise ValueError("When app is provided, plugins should not be provided and should be provided in the app instead.")
            app_name = app.name
        ...
        if plugins:
            warnings.warn(
                "The `plugins` argument is deprecated. Please use the `app` argument to provide plugins instead.",
                DeprecationWarning,
            )
        return app_name, agent, context_cache_config, resumability_config, plugins

    def _infer_agent_origin(self, agent: BaseAgent) -> tuple[str | None, Path | None]:
    ...
    def _enforce_app_name_alignment(self) -> None:
        ...
        resolution = "Ensure the runner app_name matches that directory or pass app_name explicitly when constructing the runner."
    ...
    def _format_session_not_found_message(self, session_id: str) -> str:
        ...
        if not self._app_name_alignment_hint:
            return message
        return f"{message}. {self._app_name_alignment_hint} The mismatch prevents the runner from locating the session."
    ...
    async def _compute_state_delta_for_rewind(self, session: Session, rewind_event_index: int) -> dict[str, Any]:
    ...
    async def _compute_artifact_delta_for_rewind(self, session: Session, rewind_event_index: int) -> dict[str, int]:
        ...
        for filename, vn in current_versions.items():
            ...
            if vt is None:
                ...
                artifact = types.Part(inline_data=types.Blob(mime_type="application/octet-stream", data=b""))
            else:
    ...
    async def _exec_with_plugin(
        ...
        session: Session,
        execute_fn: Callable[[InvocationContext], AsyncGenerator[Event]],
        ...
        early_exit_result = await plugin_manager.run_before_run_callback(invocation_context=invocation_context)
        if isinstance(early_exit_result, types.Content):
            ...
            async with Aclosing(execute_fn(invocation_context)) as agen:
                async for event in agen:
                    if is_live_call:
                        ...
                        if event.partial is not True:
                            if _is_transcription(event) and (
                                ...
                                logger.debug("Appending transcription finished event: %s", event)
                    ...
                    modified_event = await plugin_manager.run_on_event_callback(invocation_context=invocation_context, event=event)
        ...
        *,
        ...
        if self.artifact_service and save_input_blobs_as_artifacts:
            ...
            for i, part in enumerate(new_message.parts):
                ...
                new_message.parts[i] = types.Part(text=f"Uploaded file: {file_name}. It is saved into artifacts")
    ...
    async def run_live(
        ...
        *   **Live Model Audio Events with Inline Data:** Events containing raw
        ...
        if session is None and (user_id is None or session_id is None):
            raise ValueError("Either session or user_id and session_id must be provided.")
        if session is not None:
            warnings.warn(
                "The `session` parameter is deprecated. Please use `user_id` and `session_id` instead.",
                DeprecationWarning,
                stacklevel=2,
            )
        if not session:
                ...
                for param in inspect.signature(callable_to_inspect).parameters.values():
                    if param.annotation is LiveRequestQueue:
                        if not invocation_context.active_streaming_tools:
                            invocation_context.active_streaming_tools = {}
                        active_streaming_tool = ActiveStreamingTool(stream=LiveRequestQueue())
                        invocation_context.active_streaming_tools[tool.__name__] = active_streaming_tool
    ...
    async def _setup_context_for_resumed_invocation(
        ...
        *,
        ...
        user_message = new_message or self._find_user_message_for_invocation(session.events, invocation_id)
        if not user_message:
            raise ValueError(f"No user message available for resuming invocation: {invocation_id}")
    ...
    def _find_user_message_for_invocation(self, events: list[Event], invocation_id: str) -> types.Content | None:
        ...
        *,
        ...
        if run_config.support_cfc and isinstance(self.agent, LlmAgent):
            ...
            if not model_name.startswith("gemini-2"):
                raise ValueError(f"CFC is not supported for model: {model_name} in agent: {self.agent.name}")
    ...
    async def _cleanup_toolsets(self, toolsets_to_close: set[BaseToolset]):
        ...
        for toolset in toolsets_to_close:
            try:
                ...
                logger.warning("Toolset %s cleanup cancelled: %s", type(toolset).__name__, e)
