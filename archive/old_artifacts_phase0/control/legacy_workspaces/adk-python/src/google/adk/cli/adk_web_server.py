...
from ..agents.live_request_queue import LiveRequest, LiveRequestQueue
from ..agents.run_config import RunConfig, StreamingMode
from ..apps.app import App
from ..artifacts.base_artifact_service import ArtifactVersion, BaseArtifactService
from ..auth.credential_service.base_credential_service import BaseCredentialService
from ..errors.already_exists_error import AlreadyExistsError
from ..errors.input_validation_error import InputValidationError
from ..errors.not_found_error import NotFoundError
from ..evaluation.base_eval_service import InferenceConfig, InferenceRequest
from ..evaluation.constants import MISSING_EVAL_DEPENDENCIES_MESSAGE
from ..evaluation.eval_case import EvalCase, SessionInput
from ..evaluation.eval_metrics import EvalMetric, EvalMetricResult, EvalMetricResultPerInvocation, EvalStatus, MetricInfo
...
from .utils import cleanup, common, envs, evals
...
class ApiServerSpanExporter(export_lib.SpanExporter):
    ...
    def export(self, spans: typing.Sequence[ReadableSpan]) -> export_lib.SpanExportResult:
        for span in spans:
            if span.name == "call_llm" or span.name == "send_data" or span.name.startswith("execute_tool"):
                attributes = dict(span.attributes)
                attributes["trace_id"] = span.get_span_context().trace_id
                attributes["span_id"] = span.get_span_context().span_id
                if attributes.get("gcp.vertex.agent.event_id"):
...
class CreateSessionRequest(common.BaseModel):
    ...
    state: dict[str, Any] | None = Field(default=None, description="The initial state of the session.")
...
class SaveArtifactRequest(common.BaseModel):
    ...
    artifact: types.Part = Field(description="Artifact payload encoded as google.genai.types.Part.")
...
def _setup_gcp_telemetry_experimental(
    ...
    from ..telemetry.google_cloud import get_gcp_exporters, get_gcp_resource
...
def _setup_instrumentation_lib_if_installed():
    ...
    try:
        ...
        logger.warning("Unable to import GoogleGenAiSdkInstrumentor - some telemetry will be disabled. Make sure to install google-adk[otel-gcp]")
...
class AdkWebServer:
    ...
    def _setup_runtime_config(self, web_assets_dir: str):
        ...
        runtime_config_path = os.path.join(web_assets_dir, "assets", "config", "runtime-config.json")
        runtime_config = {}
        try:
        ...
        if self.logo_text or self.logo_image_url:
            if not self.logo_text or not self.logo_image_url:
                raise ValueError("Both --logo-text and --logo-image-url must be defined when using logo config.")
            runtime_config["logo"] = {
                "text": self.logo_text,
        ...
        try:
            ...
            logger.error("Failed to write runtime config file %s: %s", runtime_config_path, e)
    ...
    async def _create_session(
        ...
        *,
        ...
        try:
            ...
            raise HTTPException(status_code=409, detail=f"Session already exists: {session_id}") from e
        except Exception as e:
    ...
    def get_fast_api_app(
        ...
        web_assets_dir: str | None = None,
        setup_observer: Callable[[Observer, AdkWebServer], None] = lambda o, s: None,
        tear_down_observer: Callable[[Observer, AdkWebServer], None] = lambda o, s: None,
        ...
        @app.get("/list-apps")
        async def list_apps(detailed: bool = Query(default=False, description="Return detailed app information")) -> list[str] | ListAppsResponse:
            if detailed:
        ...
        async def get_trace_dict(event_id: str) -> Any:
            event_dict = trace_dict.get(event_id)
        ...
        async def list_sessions(app_name: str, user_id: str) -> list[Session]:
            list_sessions_response = await self.session_service.list_sessions(app_name=app_name, user_id=user_id)
            return [
                session
        ...
        async def update_session(
            ...
            await self.session_service.append_event(session=session, event=state_update_event)
        ...
        async def create_eval_set(app_name: str, create_eval_set_request: CreateEvalSetRequest) -> EvalSet:
            try:
        ...
        @deprecated("Please use create_eval_set instead. This will be removed in future releases.")
        @app.post(
        ...
        async def create_eval_set_legacy(
            ...
            await create_eval_set(
                app_name=app_name,
                create_eval_set_request=CreateEvalSetRequest(eval_set=EvalSet(eval_set_id=eval_set_id, eval_cases=[])),
            )
        ...
        @deprecated("Please use list_eval_sets instead. This will be removed in future releases.")
        @app.get(
        ...
        async def add_session_to_eval_set(app_name: str, eval_set_id: str, req: AddSessionToEvalSetRequest):
            # Get the session
            session = await self.session_service.get_session(app_name=app_name, user_id=req.user_id, session_id=req.session_id)
            ...
            try:
                self.eval_sets_manager.add_eval_case(app_name, eval_set_id, new_eval_case)
            except ValueError as ve:
        ...
        async def get_eval(app_name: str, eval_set_id: str, eval_case_id: str) -> EvalCase:
            """Gets an eval case in an eval set."""
            eval_case_to_find = self.eval_sets_manager.get_eval_case(app_name, eval_set_id, eval_case_id)
        ...
        async def update_eval(
            ...
            try:
                self.eval_sets_manager.update_eval_case(app_name, eval_set_id, updated_eval_case)
        ...
        async def delete_eval(app_name: str, eval_set_id: str, eval_case_id: str) -> None:
            try:
                self.eval_sets_manager.delete_eval_case(app_name, eval_set_id, eval_case_id)
            except NotFoundError as nfe:
                raise HTTPException(status_code=404, detail=str(nfe)) from nfe

        @deprecated("Please use run_eval instead. This will be removed in future releases.")
        @app.post(
            "/apps/{app_name}/eval_sets/{eval_set_id}/run_eval",
            response_model_exclude_none=True,
            tags=[TAG_EVALUATION],
        )
        async def run_eval_legacy(app_name: str, eval_set_id: str, req: RunEvalRequest) -> list[RunEvalResult]:
            run_eval_response = await run_eval(app_name=app_name, eval_set_id=eval_set_id, req=req)
        ...
        async def run_eval(app_name: str, eval_set_id: str, req: RunEvalRequest) -> RunEvalResponse:
            """Runs an eval given the details in the eval request."""
            # Create a mapping from eval set file to all the evals that needed to be
            # run.
            try:
                from ..evaluation.local_eval_service import LocalEvalService
                from .cli_eval import _collect_eval_results, _collect_inferences
        ...
        async def get_eval_result(
            ...
            try:
                eval_set_result = self.eval_set_results_manager.get_eval_set_result(app_name, eval_result_id)
        ...
        @deprecated("Please use get_eval_result instead. This will be removed in future releases.")
        @app.get(
        ...
        async def get_eval_result_legacy(
            ...
            try:
                return self.eval_set_results_manager.get_eval_set_result(app_name, eval_result_id)
            except ValueError as ve:
                raise HTTPException(status_code=404, detail=str(ve)) from ve
            except ValidationError as ve:
                raise HTTPException(status_code=500, detail=str(ve)) from ve
        ...
        async def list_eval_results(app_name: str) -> ListEvalResultsResponse:
            ...
            eval_result_ids = self.eval_set_results_manager.list_eval_set_results(app_name)
            return ListEvalResultsResponse(eval_result_ids=eval_result_ids)

        @deprecated("Please use list_eval_results instead. This will be removed in future releases.")
        @app.get(
        ...
        async def save_artifact(
            ...
            try:
                ...
                logger.error(
                    "Internal error while saving artifact %s for app=%s user=%s session=%s: %s",
                    req.filename,
                    app_name,
                    user_id,
                    session_id,
            ...
            if artifact_version is None:
                raise HTTPException(status_code=500, detail="Artifact metadata unavailable")
        ...
        async def list_artifact_names(app_name: str, user_id: str, session_id: str) -> list[str]:
            return await self.artifact_service.list_artifact_keys(app_name=app_name, user_id=user_id, session_id=session_id)
        ...
        async def delete_artifact(app_name: str, user_id: str, session_id: str, artifact_name: str) -> None:
        ...
        async def patch_memory(app_name: str, user_id: str, update_memory_request: UpdateMemoryRequest) -> None:
        ...
        async def run_agent(req: RunAgentRequest) -> list[Event]:
            session = await self.session_service.get_session(app_name=req.app_name, user_id=req.user_id, session_id=req.session_id)
            if not session:
                raise HTTPException(status_code=404, detail="Session not found")
        ...
        async def get_event_graph(app_name: str, user_id: str, session_id: str, event_id: str):
            ...
            if function_calls:
                ...
                for function_call in function_calls:
                    ...
                    dot_graph = await agent_graph.get_agent_graph(root_agent, function_call_highlights)
                ...
                for function_response in function_responses:
                    ...
                    dot_graph = await agent_graph.get_agent_graph(root_agent, function_responses_highlights)
            else:
                from_name = event.author
                to_name = ""
                dot_graph = await agent_graph.get_agent_graph(root_agent, [(from_name, to_name)])
        ...
        async def run_agent_live(
            ...
            async def forward_events():
                ...
                async with Aclosing(runner.run_live(session=session, live_request_queue=live_request_queue)) as agen:
                    async for event in agen:
        ...
        if web_assets_dir:
            ...
            redirect_dev_ui_url = self.url_prefix + "/dev-ui/" if self.url_prefix else "/dev-ui/"
