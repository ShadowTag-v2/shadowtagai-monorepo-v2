...
def _create_branch_ctx_for_sub_agent(
    ...
    invocation_context.branch = f"{invocation_context.branch}.{branch_suffix}" if invocation_context.branch else branch_suffix
    return invocation_context


# TODO - remove once Python <3.11 is no longer supported.
async def _merge_agent_run_pre_3_11(
    agent_runs: list[AsyncGenerator[Event]],
...
class ParallelAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event]:
        ...
        try:
            ...
            if ctx.is_resumable and all(ctx.end_of_agents.get(sub_agent.name) for sub_agent in self.sub_agents):
