...
from .eval_result import EvalCaseResult, EvalSetResult
...
class GcsEvalSetResultsManager(EvalSetResultsManager):
    ...
    def __init__(self, bucket_name: str, **kwargs):
        ...
        if not self.bucket.exists():
            raise ValueError(f"Bucket `{self.bucket_name}` does not exist. Please create it before using the GcsEvalSetsManager.")

    def _get_eval_history_dir(self, app_name: str) -> str:
        return f"{app_name}/{_EVAL_HISTORY_DIR}"

    def _get_eval_set_result_blob_name(self, app_name: str, eval_set_result_id: str) -> str:
    ...
    def _write_eval_set_result(self, blob_name: str, eval_set_result: EvalSetResult):
    ...
    def save_eval_set_result(
        ...
        eval_set_result_blob_name = self._get_eval_set_result_blob_name(app_name, eval_set_result.eval_set_result_id)
    ...
    def get_eval_set_result(self, app_name: str, eval_set_result_id: str) -> EvalSetResult:
        ...
        eval_set_result_blob_name = self._get_eval_set_result_blob_name(app_name, eval_set_result_id)
    ...
    def list_eval_set_results(self, app_name: str) -> list[str]:
        ...
        try:
            for blob in self.bucket.list_blobs(prefix=eval_history_dir):
                eval_set_result_id = blob.name.split("/")[-1].removesuffix(_EVAL_SET_RESULT_FILE_EXTENSION)
                eval_set_results.append(eval_set_result_id)
            return sorted(eval_set_results)
        except cloud_exceptions.NotFound as e:
            raise ValueError(f"App `{app_name}` not found in GCS bucket `{self.bucket_name}`.") from e
