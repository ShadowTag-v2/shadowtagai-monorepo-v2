...
def from_config(config_path: str) -> BaseAgent:
    ...
    if type(agent_config) is BaseAgentConfig:
        ...
        agent_config = agent_class.config_type.model_validate(agent_config.model_dump())
...
def _resolve_agent_class(agent_class: str) -> type[BaseAgent]:
    ...
    raise ValueError(f"Invalid agent class `{agent_class_name}`. It must be a subclass of BaseAgent.")
...
@experimental
def resolve_agent_reference(ref_config: AgentRefConfig, referencing_agent_config_abs_path: str) -> BaseAgent:
