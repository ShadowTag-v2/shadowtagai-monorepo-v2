...
from .utils.service_factory import create_artifact_service_from_options, create_session_service_from_options
...
async def run_input_file(
    app_name: str,
    user_id: str,
    agent_or_app: LlmAgent | App,
    artifact_service: BaseArtifactService,
    session_service: BaseSessionService,
    credential_service: BaseCredentialService,
    input_path: str,
) -> Session:
    app = agent_or_app if isinstance(agent_or_app, App) else App(name=app_name, root_agent=agent_or_app)
    runner = Runner(
        app=app,
    ...
    session = await session_service.create_session(app_name=app_name, user_id=user_id, state=input_file.state)
...
async def run_interactively(
    root_agent_or_app: LlmAgent | App,
    artifact_service: BaseArtifactService,
    session: Session,
    session_service: BaseSessionService,
    credential_service: BaseCredentialService,
) -> None:
    app = root_agent_or_app if isinstance(root_agent_or_app, App) else App(name=session.app_name, root_agent=root_agent_or_app)
    runner = Runner(
        app=app,
        artifact_service=artifact_service,
        session_service=session_service,
...
async def run_cli(
    *,
    ...
    agent_or_app = AgentLoader(agents_dir=agents_dir).load_agent(agent_folder_name)
    session_app_name = agent_or_app.name if isinstance(agent_or_app, App) else agent_folder_name
    ...
    if input_file:
        ...
        session = await session_service.create_session(app_name=session_app_name, user_id=user_id)
