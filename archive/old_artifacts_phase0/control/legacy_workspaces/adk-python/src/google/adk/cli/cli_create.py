...
def _prompt_str(
    prompt_prefix: str,
    *,
    prior_msg: str | None = None,
    default_value: str | None = None,
) -> str:
    if prior_msg:
        click.secho(prior_msg, fg="green")
    while True:
        value: str = click.prompt(prompt_prefix, default=default_value or None, type=str)
    ...
    *,
    ...
    if not google_api_key and not (google_cloud_project and google_cloud_region):
        if model.startswith("gemini"):
            google_api_key, google_cloud_project, google_cloud_region = _prompt_to_choose_backend(
