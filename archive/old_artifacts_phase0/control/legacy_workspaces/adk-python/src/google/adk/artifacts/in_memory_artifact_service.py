...
class InMemoryArtifactService(BaseArtifactService, BaseModel):
    ...
    async def save_artifact(
        ...
        *,
        ...
        if artifact.inline_data is not None:
            ...
            if artifact_util.is_artifact_ref(artifact):
                if not artifact_util.parse_artifact_uri(artifact.file_data.file_uri):
                    raise InputValidationError(f"Invalid artifact reference URI: {artifact.file_data.file_uri}")
        ...
        self.artifacts[path].append(_ArtifactEntry(data=artifact, artifact_version=artifact_version))
        return version
    ...
    async def load_artifact(
        ...
        *,
        ...
        if artifact_util.is_artifact_ref(artifact_data):
            parsed_uri = artifact_util.parse_artifact_uri(artifact_data.file_data.file_uri)
            if not parsed_uri:
                raise InputValidationError(f"Invalid artifact reference URI: {artifact_data.file_data.file_uri}")
