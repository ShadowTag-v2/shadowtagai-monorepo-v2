...
_SingleAgentCallback: TypeAlias = Callable[
    [CallbackContext],
    Awaitable[types.Content | None] | types.Content | None,
]

BeforeAgentCallback: TypeAlias = _SingleAgentCallback | list[_SingleAgentCallback]

AfterAgentCallback: TypeAlias = _SingleAgentCallback | list[_SingleAgentCallback]
  ...
  """
    ...
    def clone(self: SelfAgent, update: Mapping[str, Any] | None = None) -> SelfAgent:
        ...
        if update is not None and "parent_agent" in update:
            raise ValueError(
                "Cannot update `parent_agent` field in clone. Parent agent is set only when the parent agent is instantiated with the sub-agents."
            )
        ...
        if update is not None:
            ...
            if invalid_fields:
                raise ValueError(f"Cannot update nonexistent fields in {self.__class__.__name__}: {invalid_fields}")
    ...
    async def _handle_before_agent_callback(self, ctx: InvocationContext) -> Event | None:
        ...
        before_agent_callback_content = await ctx.plugin_manager.run_before_agent_callback(agent=self, callback_context=callback_context)
    ...
    async def _handle_after_agent_callback(self, invocation_context: InvocationContext) -> Event | None:
        ...
        # Run callbacks from the plugins.
        after_agent_callback_content = await invocation_context.plugin_manager.run_after_agent_callback(agent=self, callback_context=callback_context)
    ...
    def validate_sub_agents_unique_names(cls, value: list[BaseAgent]) -> list[BaseAgent]:
        ...
        if duplicates:
            ...
            logger.warning(
                "Found duplicate sub-agent names: %s. All sub-agents must have unique names.",
                duplicate_names_str,
            )
    ...
    def __create_kwargs(
        ...
        from .config_agent_utils import resolve_agent_reference, resolve_callbacks
        ...
        if config.before_agent_callbacks:
            kwargs["before_agent_callback"] = resolve_callbacks(config.before_agent_callbacks)
        if config.after_agent_callbacks:
            kwargs["after_agent_callback"] = resolve_callbacks(config.after_agent_callbacks)
        return kwargs
