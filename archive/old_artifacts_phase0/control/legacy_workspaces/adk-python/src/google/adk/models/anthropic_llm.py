...
def part_to_message_block(
    part: types.Part,
) -> anthropic_types.TextBlockParam | anthropic_types.ImageBlockParam | anthropic_types.ToolUseBlockParam | anthropic_types.ToolResultBlockParam:
    if part.text:
        ...
        return anthropic_types.ImageBlockParam(
            type="image",
            source=dict(type="base64", media_type=part.inline_data.mime_type, data=data),
        )
...
def content_block_to_part(
    ...
    if isinstance(content_block, anthropic_types.ToolUseBlock):
        ...
        part = types.Part.from_function_call(name=content_block.name, args=content_block.input)
...
class AnthropicLlm(BaseLlm):
    ...
    async def generate_content_async(self, llm_request: LlmRequest, stream: bool = False) -> AsyncGenerator[LlmResponse]:
        messages = [content_to_message_param(content) for content in llm_request.contents or []]
        tools = NOT_GIVEN
        if llm_request.config and llm_request.config.tools and llm_request.config.tools[0].function_declarations:
            tools = [function_declaration_to_tool_param(tool) for tool in llm_request.config.tools[0].function_declarations]
        tool_choice = anthropic_types.ToolChoiceAutoParam(type="auto") if llm_request.tools_dict else NOT_GIVEN
...
class Claude(AnthropicLlm):
    ...
    def _anthropic_client(self) -> AsyncAnthropicVertex:
        if "GOOGLE_CLOUD_PROJECT" not in os.environ or "GOOGLE_CLOUD_LOCATION" not in os.environ:
            raise ValueError("GOOGLE_CLOUD_PROJECT and GOOGLE_CLOUD_LOCATION must be set for using Anthropic on Vertex.")
