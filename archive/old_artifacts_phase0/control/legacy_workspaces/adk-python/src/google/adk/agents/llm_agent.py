...
BeforeModelCallback: TypeAlias = _SingleBeforeModelCallback | list[_SingleBeforeModelCallback]
...
AfterModelCallback: TypeAlias = _SingleAfterModelCallback | list[_SingleAfterModelCallback]
...
OnModelErrorCallback: TypeAlias = _SingleOnModelErrorCallback | list[_SingleOnModelErrorCallback]

_SingleBeforeToolCallback: TypeAlias = Callable[
    [BaseTool, dict[str, Any], ToolContext],
    Awaitable[dict | None] | dict | None,
]

BeforeToolCallback: TypeAlias = _SingleBeforeToolCallback | list[_SingleBeforeToolCallback]
...
AfterToolCallback: TypeAlias = _SingleAfterToolCallback | list[_SingleAfterToolCallback]
...
OnToolErrorCallback: TypeAlias = _SingleOnToolErrorCallback | list[_SingleOnToolErrorCallback]

InstructionProvider: TypeAlias = Callable[[ReadonlyContext], str | Awaitable[str]]
...
async def _convert_tool_union_to_tools(
    ...
    if multiple_tools and isinstance(tool_union, GoogleSearchTool):
        from ..tools.google_search_agent_tool import GoogleSearchAgentTool, create_google_search_agent
  ...
  """
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event]:
        ...
        if ctx.is_resumable:
            ...
            if events and (ctx.should_pause_invocation(events[-1]) or ctx.should_pause_invocation(events[-2])):
                return
    ...
    async def canonical_global_instruction(self, ctx: ReadonlyContext) -> tuple[str, bool]:
    ...
    async def canonical_tools(self, ctx: ReadonlyContext = None) -> list[BaseTool]:
        ...
        for tool_union in self.tools:
            resolved_tools.extend(await _convert_tool_union_to_tools(tool_union, ctx, self.model, multiple_tools))
    ...
    def _get_subagent_to_resume(self, ctx: InvocationContext) -> BaseAgent | None:
        ...
        if last_event.author == "user":
            ...
            if not function_call_event:
                raise ValueError(f"No agent to transfer to for resuming agent from function response {self.name}")
    ...
    def __get_transfer_to_agent_or_none(self, event: Event, from_agent: str) -> BaseAgent | None:
        """Returns the agent to run if the event is a transfer to agent response."""
        function_responses = event.get_function_responses()
        if not function_responses:
            return None
        for function_response in function_responses:
            if function_response.name == "transfer_to_agent" and event.author == from_agent and event.actions.transfer_to_agent != from_agent:
    ...
    @classmethod
    def validate_generate_content_config(cls, generate_content_config: types.GenerateContentConfig | None) -> types.GenerateContentConfig:
    ...
    @classmethod
    @experimental
    def _resolve_tools(cls, tool_configs: list[ToolConfig], config_abs_path: str) -> list[Any]:
        ...
        for tool_config in tool_configs:
            ...
            if isinstance(obj, BaseTool) or isinstance(obj, BaseToolset):
                logger.debug("Tool %s is an instance of BaseTool/BaseToolset.", tool_config.name)
                resolved_tools.append(obj)
            elif inspect.isclass(obj) and (issubclass(obj, BaseTool) or issubclass(obj, BaseToolset)):
                logger.debug("Tool %s is a sub-class of BaseTool/BaseToolset.", tool_config.name)
                resolved_tools.append(obj.from_config(tool_config.args, config_abs_path))
            elif callable(obj):
                if tool_config.args:
                    logger.debug(
                    ...
                    logger.debug("Tool %s is a user-defined function tool.", tool_config.name)
                    resolved_tools.append(obj)
            else:
    ...
    def _parse_config(
        ...
        from .config_agent_utils import resolve_callbacks, resolve_code_reference
        ...
        if config.before_model_callbacks:
            kwargs["before_model_callback"] = resolve_callbacks(config.before_model_callbacks)
        if config.after_model_callbacks:
            kwargs["after_model_callback"] = resolve_callbacks(config.after_model_callbacks)
        if config.before_tool_callbacks:
            kwargs["before_tool_callback"] = resolve_callbacks(config.before_tool_callbacks)
        if config.after_tool_callbacks:
            kwargs["after_tool_callback"] = resolve_callbacks(config.after_tool_callbacks)
