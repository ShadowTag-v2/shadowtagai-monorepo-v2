  ...
  """
    ...
    async def generate_content_async(self, llm_request: LlmRequest, stream: bool = False) -> AsyncGenerator[LlmResponse]:
        ...
        if llm_request.config:
            ...
            llm_request.config.http_options.headers = self._merge_tracking_headers(llm_request.config.http_options.headers)
            ...
            if stream:
                ...
                async with Aclosing(responses) as agen:
                    async for response in agen:
                        ...
                        async with Aclosing(aggregator.process_response(response)) as aggregator_gen:
                ...
                if (close_result := aggregator.close()) is not None:
                    ...
                    # streaming
                    if cache_metadata:
                        cache_manager.populate_cache_metadata_in_response(close_result, cache_metadata)
                    yield close_result
                ...
                if cache_metadata:
                    cache_manager.populate_cache_metadata_in_response(llm_response, cache_metadata)
                yield llm_response
    ...
    def _api_backend(self) -> GoogleLLMVariant:
        return GoogleLLMVariant.VERTEX_AI if self.api_client.vertexai else GoogleLLMVariant.GEMINI_API
    ...
    def _live_api_client(self) -> Client:
        ...
        return Client(http_options=types.HttpOptions(headers=self._tracking_headers(), api_version=self._live_api_version))
    ...
    async def connect(self, llm_request: LlmRequest) -> BaseLlmConnection:
        ...
        if llm_request.live_connect_config and llm_request.live_connect_config.http_options:
            ...
            llm_request.live_connect_config.http_options.headers.update(self._tracking_headers())
        ...
        if llm_request.live_connect_config.session_resumption and llm_request.live_connect_config.session_resumption.transparent:
            logger.debug(
            ...
            if self._api_backend == GoogleLLMVariant.GEMINI_API:
                raise ValueError("Transparent session resumption is only supported for Vertex AI backend. Please use Vertex AI backend.")
        ...
        async with self._live_api_client.aio.live.connect(model=llm_request.model, config=llm_request.live_connect_config) as live_session:
    ...
    async def _adapt_computer_use_tool(self, llm_request: LlmRequest) -> None:
        ...
        await ComputerUseToolset.adapt_computer_use_tool("wait", convert_wait_to_wait_5_seconds, llm_request)
...
def _build_request_log(req: LlmRequest) -> str:
    ...
    function_decl_tool_index: int | None = None

    if req.config.tools:
        for idx, tool in enumerate(req.config.tools):
            if tool.function_declarations:
                function_decls = cast(list[types.FunctionDeclaration], tool.function_declarations)
    ...
    tools_exclusion = {function_decl_tool_index: {"function_declarations"}} if function_decl_tool_index is not None else True
...
def _build_response_log(resp: types.GenerateContentResponse) -> str:
    ...
    if function_calls := resp.function_calls:
        for func_call in function_calls:
            function_calls_text.append(f"name: {func_call.name}, args: {func_call.args}")
    return f"""
LLM Response:
-----------------------------------------------------------
Text:
...
def _remove_display_name_if_present(
    data_obj: types.Blob | types.FileData | None,
):
