...
class ExampleTool(BaseTool):
    ...
    def __init__(self, examples: list[Example] | BaseExampleProvider):
        ...
        self.examples = TypeAdapter(list[Example]).validate_python(examples) if isinstance(examples, list) else examples
    ...
    async def process_llm_request(self, *, tool_context: ToolContext, llm_request: LlmRequest) -> None:
        ...
        llm_request.append_instructions([example_util.build_example_si(self.examples, parts[0].text, llm_request.model)])

    @override
    @classmethod
    def from_config(cls: type[ExampleTool], config: ToolArgsConfig, config_abs_path: str) -> ExampleTool:
        ...
        if isinstance(example_tool_config.examples, str):
            example_provider = config_agent_utils.resolve_fully_qualified_name(example_tool_config.examples)
            if not isinstance(example_provider, BaseExampleProvider):
                raise ValueError("Example provider must be an instance of BaseExampleProvider.")
            return cls(example_provider)
        elif isinstance(example_tool_config.examples, list):
            return cls(example_tool_config.examples)
        else:
            raise ValueError("Example tool config must be a list of examples or a fully-qualified name to a BaseExampleProvider object in code.")
