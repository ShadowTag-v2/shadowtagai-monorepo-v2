...
class SessionContext:
    ...
    async def start(self) -> ClientSession:
        ...
        async with self._task_lock:
            if self._session:
                logger.debug("Session has already been created, returning existing session")
                return self._session

            if self._close_event.is_set():
                raise ConnectionError("Failed to create MCP session: session already closed")
        ...
        if self._task.done() and self._task.exception():
            raise ConnectionError(f"Failed to create MCP session: {self._task.exception()}") from self._task.exception()
            ...
            async with AsyncExitStack() as exit_stack:
                ...
                if self._is_stdio:
                    session = await exit_stack.enter_async_context(
                        ClientSession(
                            *transports[:2],
                            read_timeout_seconds=timedelta(seconds=self._timeout) if self._timeout is not None else None,
                        )
                    )
                else:
                    ...
                    session = await exit_stack.enter_async_context(
                        ClientSession(
                            *transports[:2],
                            read_timeout_seconds=timedelta(seconds=self._sse_read_timeout) if self._sse_read_timeout is not None else None,
                        )
                    )
