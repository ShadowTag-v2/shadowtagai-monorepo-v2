...
from ._generated_file_utils import load_recorded_session, load_test_case
from ._replay_validators import compare_events, compare_session
...
class ConformanceTestRunner:
    ...
    def _discover_test_cases(self) -> list[TestCase]:
        ...
        for test_path in self.test_paths:
            ...
            for spec_file in test_path.rglob("spec.yaml"):
                ...
                if self.mode == "replay" and not (test_case_dir / "generated-recordings.yaml").exists():
                    click.secho(
    ...
    async def _run_user_messages(self, session_id: str, test_case: TestCase) -> None:
        """Run all user messages for a test case."""
        function_call_name_to_id_map = {}
        for user_message_index, user_message in enumerate(test_case.test_spec.user_messages):
            ...
            if user_message.content is not None:
                ...
                if (
                    ...
                    if user_message.content.parts[0].function_response.name not in function_call_name_to_id_map:
                        raise ValueError(
                            f"Function response for {user_message.content.parts[0].function_response.name} does not match any pending function call."
                        )
                    content.parts[0].function_response.id = function_call_name_to_id_map[user_message.content.parts[0].function_response.name]
            elif user_message.text is not None:
                content = types.UserContent(parts=[types.Part(text=user_message.text)])
            else:
                raise ValueError(f"UserMessage at index {user_message_index} has neither text nor content")
                ...
                if event.content and event.content.parts:
                    for part in event.content.parts:
                        if part.function_call:
                            function_call_name_to_id_map[part.function_call.name] = part.function_call.id

    async def _validate_test_results(self, session_id: str, test_case: TestCase) -> _TestResult:
        ...
        events_result = compare_events(final_session.events, recorded_session.events)
