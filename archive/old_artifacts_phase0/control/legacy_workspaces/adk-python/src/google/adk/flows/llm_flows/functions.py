...
from ...telemetry.tracing import trace_merged_tool_calls, trace_tool_call, tracer
...
def remove_client_function_call_id(content: types.Content | None) -> None:
    ...
    if content and content.parts:
        for part in content.parts:
            if part.function_call and part.function_call.id and part.function_call.id.startswith(AF_FUNCTION_CALL_ID_PREFIX):
                part.function_call.id = None
            if part.function_response and part.function_response.id and part.function_response.id.startswith(AF_FUNCTION_CALL_ID_PREFIX):
...
def get_long_running_function_calls(
    ...
    for function_call in function_calls:
        if function_call.name in tools_dict and tools_dict[function_call.name].is_long_running:
...
def generate_request_confirmation_event(
    ...
    for (
        ...
        original_function_call = next((fc for fc in function_calls if fc.id == function_call_id), None)
        if not original_function_call:
            continue
        request_confirmation_function_call = types.FunctionCall(
            name=REQUEST_CONFIRMATION_FUNCTION_CALL_NAME,
            args={
                "originalFunctionCall": original_function_call.model_dump(exclude_none=True, by_alias=True),
...
async def handle_function_calls_async(
    ...
    tool_confirmation_dict: dict[str, ToolConfirmation] | None = None,
) -> Event | None:
...
async def handle_function_call_list_async(
    ...
    function_response_events = [event for event in function_response_events if event is not None]
...
async def _execute_single_function_call_async(
    ...
    try:
        ...
        if error_response is not None:
            return __build_response_event(tool, error_response, tool_context, invocation_context)
        else:
            raise tool_error
    ...
    async def _run_with_trace():
        ...
        # response.
        function_response = await invocation_context.plugin_manager.run_before_tool_callback(
        ...
        # response.
        altered_function_response = await invocation_context.plugin_manager.run_after_tool_callback(
            tool=tool,
            tool_args=function_args,
            tool_context=tool_context,
...
async def _execute_single_function_call_live(
    ...
    tool, tool_context = _get_tool_and_context(invocation_context, function_call, tools_dict)
...
async def _process_function_live_helper(
    ...
    if function_call.name == "stop_streaming" and "function_name" in function_args:
        ...
        async with streaming_lock:
            ...
            if active_tasks and function_name in active_tasks and active_tasks[function_name].task and not active_tasks[function_name].task.done():
        ...
        if task:
            ...
            try:
                ...
                if task.cancelled():
                    ...
                    function_response = {"status": f"The task is not cancelled yet for {function_name}."}
            if not function_response:
                # Clean up the reference under lock
                async with streaming_lock:
                    if invocation_context.active_streaming_tools and function_name in invocation_context.active_streaming_tools:
                        invocation_context.active_streaming_tools[function_name].task = None

                function_response = {"status": f"Successfully stopped streaming function {function_name}"}
        else:
            function_response = {"status": f"No active streaming function named {function_name} found"}
                ...
                async with Aclosing(
                    ...
                    async for result in agen:
                        updated_content = types.Content(
                            role="user",
                            parts=[types.Part.from_text(text=f"Function {tool.name} returned: {result}")],
                        )
        ...
        task = asyncio.create_task(run_tool_and_update_queue(tool, function_args, tool_context))
        ...
        async with streaming_lock:
            ...
            if tool.name in invocation_context.active_streaming_tools:
                ...
                invocation_context.active_streaming_tools[tool.name] = ActiveStreamingTool(task=task)

        # Immediately return a pending response.
        # This is required by current live model.
        function_response = {"status": ("The function is running asynchronously and the results are pending.")}
    else:
        function_response = await __call_tool_async(tool, args=function_args, tool_context=tool_context)
...
def find_matching_function_call(
    ...
    last_event = events[-1]
    if last_event.content and last_event.content.parts and any(part.function_response for part in last_event.content.parts):
        function_call_id = next(part.function_response.id for part in last_event.content.parts if part.function_response)
