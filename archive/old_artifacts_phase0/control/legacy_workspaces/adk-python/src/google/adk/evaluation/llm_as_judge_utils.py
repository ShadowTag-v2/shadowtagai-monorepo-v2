...
from .eval_case import IntermediateDataType, get_all_tool_calls_with_responses
...
def get_eval_status(score: float | None, threshold: float) -> EvalStatus:
    if score is None:
...
def get_average_rubric_score(
    ...
    aggregated value.
    """
    rubric_scores = [rubric_score.score for rubric_score in rubric_scores if rubric_score.score is not None]
...
def get_tool_declarations_as_json_str(
    ...
    """
    tool_declarations = _ToolDeclarations(tool_declarations=app_details.get_tools_by_agent_name())
...
class _ToolCallAndResponse(EvalBaseModel):
    ...
    tool_response: genai_types.FunctionResponse | str
...
def get_tool_calls_and_responses_as_json_str(
    ...
    The output of this method is usually intended to be sent to the LLM.
    """
    raw_tool_calls_and_response = get_all_tool_calls_with_responses(intermediate_data)
    ...
    internal_tool_calls_and_responses = _ToolCallsAndResponses(tool_calls_and_response=tool_calls_and_responses)
