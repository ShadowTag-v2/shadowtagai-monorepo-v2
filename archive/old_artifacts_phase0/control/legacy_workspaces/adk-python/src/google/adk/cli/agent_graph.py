...
async def build_graph(
    ...
    def get_node_name(tool_or_agent: BaseAgent | BaseTool):
    ...
    def get_node_caption(tool_or_agent: BaseAgent | BaseTool):

        if isinstance(tool_or_agent, BaseAgent):
            return "🤖 " + tool_or_agent.name
        elif retrieval_tool_module_loaded and isinstance(tool_or_agent, BaseRetrievalTool):
            return "🔎 " + tool_or_agent.name
    ...
    def get_node_shape(tool_or_agent: BaseAgent | BaseTool):
        if isinstance(tool_or_agent, BaseAgent):
            return "ellipse"

        elif retrieval_tool_module_loaded and isinstance(tool_or_agent, BaseRetrievalTool):
            return "cylinder"
        elif isinstance(tool_or_agent, FunctionTool) or isinstance(tool_or_agent, BaseTool):
            return "box"
        else:
            logger.warning(
                "Unsupported tool, type: %s, obj: %s",
    ...
    def should_build_agent_cluster(tool_or_agent: BaseAgent | BaseTool):
        if isinstance(tool_or_agent, BaseAgent):
            if isinstance(tool_or_agent, SequentialAgent) or isinstance(tool_or_agent, LoopAgent) or isinstance(tool_or_agent, ParallelAgent):
                return True
            else:
                return False
        elif (
            retrieval_tool_module_loaded
            and isinstance(tool_or_agent, BaseRetrievalTool)
            or isinstance(tool_or_agent, FunctionTool)
            or isinstance(tool_or_agent, BaseTool)
        ):
            return False
        else:
            logger.warning(
        ...
        if isinstance(agent, LoopAgent):
            ...
            for sub_agent_int_sequential in agent.sub_agents:
                ...
                draw_edge(
                    ...
                    agent.sub_agents[0 if curr_length == length - 1 else curr_length + 1].name,
                )
                curr_length += 1
    ...
    async def draw_node(tool_or_agent: BaseAgent | BaseTool):
