...
def _build_directory_tree(root_path: Path, max_depth: int = 3) -> dict[str, Any]:
    """Build a directory tree representation."""

    def build_tree_recursive(path: Path, current_depth: int = 0) -> dict[str, Any]:
...
def _generate_path_suggestions(root_path: Path, existing_configs: list[dict[str, Any]]) -> dict[str, Any]:
    """Generate suggested file paths for new components."""

    # Suggest main agent names if none exist
    root_agent_suggestions = []
    if not any(config.get("agent_class") != "LlmAgent" or not config.get("has_sub_agents", False) for config in existing_configs):
        root_agent_suggestions = [
            "root_agent.yaml",
        ]
