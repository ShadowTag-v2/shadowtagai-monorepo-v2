...
class CodeExecutionUtils:
    ...
    def extract_code_and_truncate_content(
        ...
        for idx, part in enumerate(content.parts):
            if part.executable_code and (idx == len(content.parts) - 1 or not content.parts[idx + 1].code_execution_result):
        ...
        content.parts.append(CodeExecutionUtils.build_executable_code_part(code_str))
    ...
    def build_code_execution_result_part(
        ...
        if code_execution_result.stdout or not code_execution_result.output_files:
            final_result.append("Code execution result:\n" + f"{code_execution_result.stdout}\n")
        if code_execution_result.output_files:
            final_result.append("Saved artifacts:\n" + ",".join([f"`{f.name}`" for f in code_execution_result.output_files]))
        return types.Part.from_code_execution_result(
            outcome="OUTCOME_OK",
            output="\n\n".join(final_result),
        )
    ...
    def convert_code_execution_parts(
        ...
        if content.parts[-1].executable_code:
            content.parts[-1] = types.Part(text=(code_block_delimiter[0] + content.parts[-1].executable_code.code + code_block_delimiter[1]))
        ...
        # likely generated by the model.
            ...
            content.parts[-1] = types.Part(
                text=execution_result_delimiters[0] + content.parts[-1].code_execution_result.output + execution_result_delimiters[1]
            )
            content.role = "user"
