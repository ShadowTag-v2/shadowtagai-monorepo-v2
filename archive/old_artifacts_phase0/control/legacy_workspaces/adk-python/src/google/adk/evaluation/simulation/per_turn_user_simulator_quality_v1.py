...
from ..eval_case import ConversationScenario, Invocation
from ..eval_metrics import BaseCriterion, EvalMetric, EvalStatus, LlmBackedUserSimulatorCriterion
from ..evaluator import EvaluationResult, Evaluator, PerInvocationResult
from ..llm_as_judge import AutoRaterScore
from ..llm_as_judge_utils import Label, get_eval_status, get_text_from_content
...
def _format_conversation_history(invocations: list[Invocation]) -> str:
    ...
    for invocation in invocations:
        if invocation.user_content is not None and invocation.user_content.parts:
            conversation_history.append(f"user: {get_text_from_content(invocation.user_content)}")

        final_response = invocation.final_response
        if final_response is not None:
            conversation_history.append(f"{final_response.role}: {get_text_from_content(final_response)}")
...
def _get_stop_signal_invocation(stop_signal: str) -> Invocation:
    return Invocation(
        ...
        user_content=genai_types.Content(parts=[genai_types.Part(text=stop_signal)]),
    )
...
class PerTurnUserSimulatorQualityV1(Evaluator):
    ...
    criterion_type: ClassVar[type[LlmBackedUserSimulatorCriterion]] = LlmBackedUserSimulatorCriterion
    ...
    def _deserialize_criterion(self, eval_metric: EvalMetric) -> BaseCriterion:
        expected_criterion_type_error = ValueError(f"`{eval_metric.metric_name}` metric expects a criterion of type `{self.criterion_type}`.")
        try:
            if self._eval_metric.criterion is None:
                raise expected_criterion_type_error

            return self.criterion_type.model_validate(self._eval_metric.criterion.model_dump())
    ...
    async def evaluate_invocations(
        ...
        results = [self._evaluate_first_turn(actual_invocations[0], conversation_scenario)]
    ...
    def _format_llm_prompt(
        ...
        previous_invocations: list[Invocation] | None,
    ) -> str:
        if previous_invocations is None:
            raise ValueError(f"Previous invocations should have a set value when formatting the LLM prompt. Encountered: {previous_invocations}")

        if conversation_scenario is None:
            raise ValueError(f"Conversation scenario should have a set value when formatting the LLM prompt. Encountered: {conversation_scenario}")
    ...
    def _convert_llm_response_to_score(self, auto_rater_response: LlmResponse) -> AutoRaterScore:
    ...
    def _aggregate_conversation_results(self, per_invocation_results: list[PerInvocationResult]) -> EvaluationResult:
    ...
    def _evaluate_first_turn(
        ...
        score = int(get_text_from_content(first_invocation.user_content).strip() == conversation_scenario.starting_prompt.strip())
        return PerInvocationResult(
            actual_invocation=first_invocation,
            score=score,
    ...
    async def _evaluate_intermediate_turn(
        ...
        for _ in range(num_samples):
            ...
            samples.append(
                PerInvocationResult(
                    eval_status=get_eval_status(llm_score.score, self._eval_metric.threshold),
