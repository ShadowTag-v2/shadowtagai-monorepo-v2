...
from ._eval_sets_manager_utils import (
...
from .eval_case import EvalCase, IntermediateData, Invocation, SessionInput
...
def _convert_invocation_to_pydantic_schema(
    ...
    for old_tool_use in invocation_in_json_format.get("expected_tool_use", []):
        expected_tool_use.append(genai_types.FunctionCall(name=old_tool_use["tool_name"], args=old_tool_use["tool_input"]))

    for old_intermediate_response in invocation_in_json_format.get("expected_intermediate_agent_responses", []):
    ...
    return Invocation(
        ...
        user_content=genai_types.Content(parts=[genai_types.Part.from_text(text=query)], role="user"),
        final_response=genai_types.Content(parts=[genai_types.Part.from_text(text=reference)], role="model"),
...
def convert_eval_set_to_pydantic_schema(
    ...
    for old_eval_case in eval_set_in_json_format:
        ...
        for old_invocation in old_eval_case["data"]:
            new_invocations.append(_convert_invocation_to_pydantic_schema(old_invocation))

        session_input = None
        if "initial_session" in old_eval_case and len(old_eval_case["initial_session"]) > 0:
...
def load_eval_set_from_file(eval_set_file_path: str, eval_set_id: str) -> EvalSet:
    ...
    with open(eval_set_file_path, encoding="utf-8") as f:
        content = f.read()
        try:
            ...
            return convert_eval_set_to_pydantic_schema(eval_set_id, json.loads(content))
...
class LocalEvalSetsManager(EvalSetsManager):
    ...
    def list_eval_sets(self, app_name: str) -> list[str]:
        ...
        try:
            ...
            raise NotFoundError(f"Eval directory for app `{app_name}` not found.") from e
