...
from ._google_credentials import BaseGoogleCredentialsConfig, GoogleCredentialsManager
...
class GoogleTool(FunctionTool):
    ...
    def __init__(
        ...
        func: Callable[..., Any],
        *,
        credentials_config: BaseGoogleCredentialsConfig | None = None,
        ...
        self._credentials_manager = GoogleCredentialsManager(credentials_config) if credentials_config else None
    ...
    async def run_async(self, *, args: dict[str, Any], tool_context: ToolContext) -> Any:
        ...
        try:
            ...
            credentials = await self._credentials_manager.get_valid_credentials(tool_context) if self._credentials_manager else None

            if credentials is None and self._credentials_manager:
                # OAuth flow in progress
                return f"User authorization is required to access Google services for {self.name}. Please complete the authorization flow."

            # Execute the tool's specific logic with valid credentials

            return await self._run_async_with_credential(credentials, self._tool_settings, args, tool_context)
