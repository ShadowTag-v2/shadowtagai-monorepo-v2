...
from .eval_case import IntermediateDataType, Invocation, get_all_tool_calls
from .eval_config import EvalConfig, get_eval_metrics_from_config, get_evaluation_criteria_or_default
from .eval_metrics import BaseCriterion, EvalMetric, EvalMetricResult, PrebuiltMetrics
...
class AgentEvaluator:
    ...
    async def evaluate_eval_set(
        ...
        if not print_detailed_results:
            failure_message += (
                " If you looking to get more details on the failures, then please re-run this test with `print_detailed_results` set to `True`."
            )
        failure_message += "\n" + "\n".join(failures)
    ...
    async def evaluate(
        ...
        """
        test_files = []
        if isinstance(eval_dataset_file_path_or_dir, str) and os.path.isdir(eval_dataset_file_path_or_dir):
        ...
        for test_file in test_files:
            ...
            eval_set = AgentEvaluator._load_eval_set_from_file(test_file, eval_config, initial_session)
    ...
    def migrate_eval_data_to_new_schema(
        ...
        if not old_eval_data_file or not new_eval_data_file:
            raise ValueError("One of old_eval_data_file or new_eval_data_file is empty.")

        eval_config = AgentEvaluator.find_config_for_test_file(old_eval_data_file)
        initial_session = AgentEvaluator._get_initial_session(initial_session_file)

        eval_set = AgentEvaluator._get_eval_set_from_old_format(old_eval_data_file, eval_config, initial_session)
    ...
    def _load_eval_set_from_file(
        ...
        return AgentEvaluator._get_eval_set_from_old_format(eval_set_file, eval_config, initial_session)
    ...
    def _get_eval_set_from_old_format(
        ...
        return convert_eval_set_to_pydantic_schema(eval_set_id=str(uuid.uuid4()), eval_set_in_json_format=[eval_data])
    ...
    def _validate_input(eval_dataset, criteria):
        ...
        for key in criteria:
            if key not in ALLOWED_CRITERIA:
                raise ValueError(f"Invalid criteria key: {key}. Expected one of {ALLOWED_CRITERIA}.")
        ...
        if not isinstance(sample, list) and not isinstance(first_query, dict):
            raise ValueError(f"Each evaluation dataset sample must be list of dictionary. But it's {eval_dataset}")

        if TOOL_TRAJECTORY_SCORE_KEY in criteria:
            if QUERY_COLUMN not in first_query or EXPECTED_TOOL_USE_COLUMN not in first_query:
                raise ValueError(
        ...
        if RESPONSE_EVALUATION_SCORE_KEY in criteria:
            if QUERY_COLUMN not in first_query:
                raise ValueError(f"Samples for {RESPONSE_EVALUATION_SCORE_KEY} must include '{QUERY_COLUMN}' key. The sample is {sample}.")

        if RESPONSE_MATCH_SCORE_KEY in criteria:
            if QUERY_COLUMN not in first_query or REFERENCE_COLUMN not in first_query:
                raise ValueError(
                    f"Samples for {RESPONSE_MATCH_SCORE_KEY} must include '{QUERY_COLUMN}' and '{REFERENCE_COLUMN}' keys. The sample is {sample}."
                )

    @staticmethod
    def _print_details(
        eval_metric_result_with_invocations: list[_EvalMetricResultWithInvocation],
        ...
        print(f"Summary: `{overall_eval_status}` for Metric: `{metric_name}`. Expected threshold: `{threshold}`, actual value: `{overall_score}`.")
            ...
            data.append(
                {
                    ...
                    "threshold": threshold,
                    "prompt": AgentEvaluator._convert_content_to_text(per_invocation_result.expected_invocation.user_content),
                    "expected_response": AgentEvaluator._convert_content_to_text(per_invocation_result.expected_invocation.final_response),
                    "actual_response": AgentEvaluator._convert_content_to_text(per_invocation_result.actual_invocation.final_response),
                    "expected_tool_calls": AgentEvaluator._convert_tool_calls_to_text(per_invocation_result.expected_invocation.intermediate_data),
                    "actual_tool_calls": AgentEvaluator._convert_tool_calls_to_text(per_invocation_result.actual_invocation.intermediate_data),
                }
            )

        print(tabulate(pd.DataFrame(data), headers="keys", tablefmt="grid", maxcolwidths=25))
    ...
    def _convert_tool_calls_to_text(
        intermediate_data: IntermediateDataType | None,
    ) -> str:
    ...
    @staticmethod
    async def _get_agent_for_eval(module_name: str, agent_name: str | None = None) -> BaseAgent:
        ...
        if not (hasattr(agent_module, "agent") or module_name.endswith(".agent")):
            raise ValueError(f"Module {module_name} does not have a member named `agent` or the name should endwith `.agent`.")

        agent_module_with_agent = agent_module.agent if hasattr(agent_module, "agent") else agent_module
        if hasattr(agent_module_with_agent, "root_agent"):
            ...
            raise ValueError(f"Module {module_name} does not have a root_agent or get_agent_async method.")
    ...
    async def _get_eval_results_by_eval_id(
        ...
        """
        try:
            from .base_eval_service import EvaluateConfig, EvaluateRequest, InferenceConfig, InferenceRequest
        ...
        eval_service = LocalEvalService(
            ...
            eval_sets_manager=AgentEvaluator._get_eval_sets_manager(app_name=app_name, eval_set=eval_set),
            user_simulator_provider=user_simulator_provider,
        )
        ...
        for inference_request in inference_requests:
            async with Aclosing(eval_service.perform_inference(inference_request=inference_request)) as agen:
        ...
        async with Aclosing(eval_service.evaluate(evaluate_request=evaluate_request)) as agen:
    ...
    def _process_metrics_and_get_failures(
        ...
        for (
            ...
            scores = [m.eval_metric_result.score for m in eval_metric_results_with_invocations if m.eval_metric_result.score is not None]
            ...
            if overall_eval_status != EvalStatus.PASSED:
                ...
                failures.append(f"{metric_name} for {agent_module} Failed. Expected {threshold}, but got {overall_score}.")
