...
from ..evaluation.base_eval_service import BaseEvalService, EvaluateConfig, EvaluateRequest, InferenceRequest, InferenceResult
from ..evaluation.constants import MISSING_EVAL_DEPENDENCIES_MESSAGE
from ..evaluation.eval_case import IntermediateDataType, get_all_tool_calls
...
def pretty_print_eval_result(eval_result: EvalCaseResult):
    ...
    for metric_result in eval_result.overall_eval_metric_results:
        ...
        if metric_result.details and metric_result.details.rubric_scores:
            ...
            rubrics_by_id = {r["rubric_id"]: r["rubric_content"]["text_property"] for r in metric_result.criterion.rubrics}
            for rubric_score in metric_result.details.rubric_scores:
                rubric = rubrics_by_id.get(rubric_score.rubric_id)
                click.echo(f"Rubric: {rubric}, Score: {rubric_score.score}, Reasoning: {rubric_score.rationale}")
    ...
    for per_invocation_result in eval_result.eval_metric_result_per_invocation:
        ...
        for metric_result in per_invocation_result.eval_metric_results:
            row_data[metric_result.metric_name] = f"Status: {metric_result.eval_status.name}, Score: {metric_result.score}"
            if metric_result.details and metric_result.details.rubric_scores:
                rubrics_by_id = {r["rubric_id"]: r["rubric_content"]["text_property"] for r in metric_result.criterion.rubrics}
                for rubric_score in metric_result.details.rubric_scores:
                    rubric = rubrics_by_id.get(rubric_score.rubric_id)
                    row_data[f"Rubric: {rubric}"] = f"Reasoning: {rubric_score.rationale}, Score: {rubric_score.score}"
    ...
    if data:
        ...
        click.echo(tabulate(df_result, headers="keys", tablefmt="grid", maxcolwidths=25))
        click.echo("\n\n")  # Few empty lines for visual clarity


def get_eval_sets_manager(eval_storage_uri: str | None, agents_dir: str) -> EvalSetsManager:
