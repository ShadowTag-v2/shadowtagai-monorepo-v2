...
from ..tool_configs import BaseToolConfig, ToolArgsConfig
from .mcp_session_manager import MCPSessionManager, SseConnectionParams, StdioConnectionParams, StreamableHTTPConnectionParams, retry_on_errors
...
class McpToolset(BaseToolset):
    ...
    async def get_tools(
        ...
        timeout_in_seconds = self._connection_params.timeout if hasattr(self._connection_params, "timeout") else None
        try:
            tools_response: ListToolsResult = await asyncio.wait_for(session.list_tools(), timeout=timeout_in_seconds)
    ...
    @override
    @classmethod
    def from_config(cls: type[McpToolset], config: ToolArgsConfig, config_abs_path: str) -> McpToolset:
...
class McpToolsetConfig(BaseToolConfig):
    ...
    stdio_server_params: StdioServerParameters | None = None

    stdio_connection_params: StdioConnectionParams | None = None

    sse_connection_params: SseConnectionParams | None = None

    streamable_http_connection_params: StreamableHTTPConnectionParams | None = None
    ...
    def _check_only_one_params_field(self):
        ...
        if len(populated_fields) != 1:
            raise ValueError(
                "Exactly one of stdio_server_params, stdio_connection_params, sse_connection_params, streamable_http_connection_params must be set."
            )
        return self
