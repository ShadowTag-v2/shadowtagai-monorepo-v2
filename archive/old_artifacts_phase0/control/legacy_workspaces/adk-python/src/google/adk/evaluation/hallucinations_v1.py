...
from .eval_case import Invocation, InvocationEvent, InvocationEvents
from .eval_metrics import EvalMetric, HallucinationsCriterion, Interval, MetricInfo, MetricValueInfo, PrebuiltMetrics
from .evaluator import EvalStatus, EvaluationResult, Evaluator, PerInvocationResult
from .llm_as_judge_utils import get_eval_status, get_text_from_content, get_tool_declarations_as_json_str
            ...
            results.append(
                {
                    ...
                    "supporting_excerpt": (sup_exc.strip() if sup_exc.strip().lower() != "null" else None),
                    "contradicting_excerpt": (con_exc.strip() if con_exc.strip().lower() != "null" else None),
                }
            )
        except Exception:  # pylint: disable=broad-except
            logger.warning("Failed to parse sentence validation block: %s", match.group(0))
    return results
...
class HallucinationsV1Evaluator(Evaluator):
    ...
    criterion_type: ClassVar[type[HallucinationsCriterion]] = HallucinationsCriterion

    def __init__(self, eval_metric: EvalMetric):
        self._eval_metric = eval_metric

        expected_criterion_type_error = ValueError(
            f"`{eval_metric.metric_name}` metric expects a criterion of type `{HallucinationsV1Evaluator.criterion_type}`."
        )
        ...
        try:
            ...
            self._criterion = HallucinationsV1Evaluator.criterion_type.model_validate(self._eval_metric.criterion.model_dump())
    ...
    def _create_context_for_step(
        ...
        for event in events:
            if not event.content or not event.content.parts:
                continue
            tool_calls = [part.function_call for part in event.content.parts if part.function_call]
            tool_responses = [part.function_response for part in event.content.parts if part.function_response]
            ...
            if tool_responses:
                ...
                context_parts.append(
                    json.dumps(
                        [tool_response.model_dump(exclude_none=True) for tool_response in tool_responses],
                        indent=2,
                    )
                    + "\n"
                )
    ...
    async def _evaluate_nl_response(self, nl_response: str, context: str) -> tuple[float | None, str]:
        ...
        segmenter_llm_request = LlmRequest(
            ...
            contents=[
                genai_types.Content(
                    parts=[genai_types.Part(text=self.segmenter_prompt.format(response=nl_response))],
                    role="user",
                )
            ],
        ...
        try:
            async with Aclosing(self._judge_model.generate_content_async(segmenter_llm_request)) as agen:
                segmenter_response = await agen.__anext__()
                sentences = _parse_sentences(get_text_from_content(segmenter_response.content))
        ...
        validator_llm_request = LlmRequest(
            ...
            contents=[
                genai_types.Content(
                    parts=[genai_types.Part(text=self.sentence_validator_prompt.format(context=context, sentences=sentences_str))],
                    role="user",
                )
            ],
            config=self._model_config,
        ...
        try:
            async with Aclosing(self._judge_model.generate_content_async(validator_llm_request)) as agen:
                validator_response = await agen.__anext__()
                validation_results = _parse_validation_results(get_text_from_content(validator_response.content))
        except Exception as e:
    ...
    def _get_steps_to_evaluate(self, actual: Invocation) -> list[EvaluationStep]:
        ...
        if self._criterion.evaluate_intermediate_nl_responses:
            for event in all_events:
                nl_parts = [p.text for p in event.content.parts if p.text] if event.content and event.content.parts else []
                if nl_parts:
                    context = self._create_context_for_step(actual.app_details, actual, events_for_context)
                    for nl_response in nl_parts:
                        step_evaluations.append(EvaluationStep(nl_response=nl_response, context=context))
        ...
        if final_response_text:
            context = self._create_context_for_step(actual.app_details, actual, events_for_context)
            step_evaluations.append(EvaluationStep(nl_response=final_response_text, context=context))
    ...
    def _aggregate_invocation_results(
        ...
        return EvaluationResult(
            ...
            overall_eval_status=get_eval_status(overall_fs_score, self._eval_metric.threshold),
            per_invocation_results=per_invocation_results,
        )
    ...
    async def evaluate_invocations(
        ...
        for actual, expected in zip(actual_invocations, expected_invocations):
            ...
            for step in step_evaluations:
                fs_score, _ = await self._evaluate_nl_response(step.nl_response, step.context)
