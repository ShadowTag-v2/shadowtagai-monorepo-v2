...
class FunctionTool(BaseTool):
    ...
    def __init__(
        ...
        *,
        ...
        elif hasattr(func, "__call__") and hasattr(func.__call__, "__doc__") and func.__call__.__doc__:
                ...
                if inspect.isclass(target_type) and issubclass(target_type, pydantic.BaseModel):
                    ...
                    if not isinstance(args[param_name], target_type):
                        try:
                            ...
                            logger.warning(f"Failed to convert argument '{param_name}' to Pydantic model {target_type.__name__}: {e}")
...
You could retry calling this tool, but it is IMPORTANT for you to provide all the mandatory parameters."""
        ...
        if isinstance(self._require_confirmation, Callable):
            require_confirmation = await self._invoke_callable(self._require_confirmation, args_to_call)
        else:
    ...
    async def _call_live(
        ...
        *,
        ...
        signature = inspect.signature(self.func)
        if self.name in invocation_context.active_streaming_tools and invocation_context.active_streaming_tools[self.name].stream:
            args_to_call["input_stream"] = invocation_context.active_streaming_tools[self.name].stream
