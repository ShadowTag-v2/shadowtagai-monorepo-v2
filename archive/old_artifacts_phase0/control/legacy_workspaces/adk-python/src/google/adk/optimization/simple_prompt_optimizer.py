...
from google.adk.optimization.data_types import BaseAgentWithScores, OptimizerResult, UnstructuredSamplingResult
...
class SimplePromptOptimizerConfig(BaseModel):
    ...
    optimizer_model: str = Field(
        ...
        description=("The model used to analyze the eval results and optimize the agent."),
    )
    ...
    batch_size: int = Field(
        default=5,
        description=("The number of training examples to use for scoring each candidate."),
    )


class SimplePromptOptimizer(AgentOptimizer[UnstructuredSamplingResult, BaseAgentWithScores]):
    ...
    async def _generate_candidate_prompt(self, best_agent: Agent, best_score: float) -> str:
    ...
    async def _score_agent_on_batch(
        ...
        eval_results = await sampler.sample_and_score(agent, "train", eval_batch, capture_full_eval_data=False)
    ...
    async def _run_optimization_iterations(
        ...
        best_score = await self._score_agent_on_batch(best_agent, sampler, train_example_ids)
        ...
        for i in range(self._config.num_iterations):
            ...
            new_prompt_text = await self._generate_candidate_prompt(best_agent, best_score)
            candidate_agent = best_agent.clone(update={"instruction": new_prompt_text})
            logger.info("Generated new candidate prompt:\n%s", new_prompt_text)
            candidate_score = await self._score_agent_on_batch(candidate_agent, sampler, train_example_ids)
            logger.info(
    ...
    async def _run_final_validation(
        ...
        validation_results = await sampler.sample_and_score(best_agent, "validation")
        if not validation_results.scores:
            return 0.0
        return sum(validation_results.scores.values()) / len(validation_results.scores)
    ...
    async def optimize(
        ...
        if self._config.batch_size > len(train_example_ids):
            logger.warning(
                "Batch size (%d) is larger than the number of training examples (%d). Using all training examples for each evaluation.",
                self._config.batch_size,
                len(train_example_ids),
            )
            self._config.batch_size = len(train_example_ids)

        best_agent, _ = await self._run_optimization_iterations(initial_agent, sampler, train_example_ids)

        final_score = await self._run_final_validation(best_agent, sampler)
        logger.info("Final validation score: %f", final_score)

        return OptimizerResult(optimized_agents=[BaseAgentWithScores(optimized_agent=best_agent, overall_score=final_score)])
