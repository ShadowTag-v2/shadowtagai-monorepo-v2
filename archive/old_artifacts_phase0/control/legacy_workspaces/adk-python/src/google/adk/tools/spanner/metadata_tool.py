  ...
  """
    ...
    try:
        ...
        with database.snapshot(multi_use=True) as snapshot:
            result_set = snapshot.execute_sql(columns_query, params=params, param_types=param_types)
            ...
            key_column_result_set = snapshot.execute_sql(key_column_usage_query, params=params, param_types=param_types)
            for row in key_column_result_set:
                ...
                if column_name in results["schema"]:
                    results["schema"][column_name].setdefault("KEY_COLUMN_USAGE", []).append(key_column_properties)

            table_metadata_result_set = snapshot.execute_sql(table_metadata_query, params=params, param_types=param_types)
...
def list_table_indexes(
    ...
    try:
        ...
        with database.snapshot() as snapshot:
            result_set = snapshot.execute_sql(sql_query, params=params, param_types=param_types)
            for row in result_set:
                index_info = {}
