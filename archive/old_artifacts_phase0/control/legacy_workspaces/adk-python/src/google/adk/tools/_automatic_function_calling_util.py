...
def _map_pydantic_type_to_property_schema(property_schema: dict):
    if "type" in property_schema:
        property_schema["type"] = _py_type_2_schema_type.get(property_schema["type"], "TYPE_UNSPECIFIED")
        if property_schema["type"] == "ARRAY":
            _map_pydantic_type_to_property_schema(property_schema["items"])
    for type_ in property_schema.get("anyOf", []):
        if "type" in type_:
            type_["type"] = _py_type_2_schema_type.get(type_["type"], "TYPE_UNSPECIFIED")
...
def build_function_declaration(
    ...
    if should_update_signature:
        ...
        if isinstance(func, type):
            fields = {name: (param.annotation, param.default) for name, param in signature.parameters.items() if name not in ignore_params}
    ...
    return from_function_with_options(func, variant) if not should_update_signature else from_function_with_options(new_func, variant)


def build_function_declaration_for_langchain(vertexai: bool, name, description, func, param_pydantic_schema) -> types.FunctionDeclaration:
    param_pydantic_schema = _process_pydantic_schema(vertexai, {"properties": param_pydantic_schema})["properties"]
    ...
    return build_function_declaration_util(vertexai, name, description, func, before_param_pydantic_schema)


def build_function_declaration_for_params_for_crewai(vertexai: bool, name, description, func, param_pydantic_schema) -> types.FunctionDeclaration:
    param_pydantic_schema = _process_pydantic_schema(vertexai, param_pydantic_schema)
    param_copy = param_pydantic_schema.copy()
    return build_function_declaration_util(vertexai, name, description, func, param_copy)


def build_function_declaration_util(vertexai: bool, name, description, func, before_param_pydantic_schema) -> types.FunctionDeclaration:
...
def from_function_with_options(
    ...
    try:
        for name, param in inspect.signature(func).parameters.items():
            if param.kind in (
                ...
                param = _function_parameter_parse_util._handle_params_as_deferred_annotations(param, annotation_under_future, name)

                schema = _function_parameter_parse_util._parse_schema_from_parameter(variant, param, func.__name__)
        ...
        for name, param in inspect.signature(func).parameters.items():
            if param.kind in (
                ...
                try:
                    ...
                    json_schema_dict = _function_parameter_parse_util._generate_json_schema_for_parameter(param)

                    parameters_json_schema[name] = types.Schema.model_validate(json_schema_dict)
                except Exception as e:
                    _function_parameter_parse_util._raise_for_unsupported_param(param, func.__name__, e)
    ...
    if parameters_properties:
        ...
        declaration.parameters.required = _function_parameter_parse_util._get_required_fields(declaration.parameters)
    ...
    if return_annotation is inspect._empty:
        ...
        declaration.response = _function_parameter_parse_util._parse_schema_from_parameter(
            variant,
            return_value,
            func.__name__,
        )
    ...
    if return_annotation is None or return_annotation is type(None) or (isinstance(return_annotation, str) and return_annotation == "None"):
    ...
    response_json_schema: dict[str, Any] | types.Schema | None = None
    try:
        response_schema = _function_parameter_parse_util._parse_schema_from_parameter(
            variant,
            return_value,
            func.__name__,
        )
        ...
        try:
            response_json_schema = _function_parameter_parse_util._generate_json_schema_for_parameter(return_value)
            response_json_schema = types.Schema.model_validate(response_json_schema)
        except Exception as e:
            _function_parameter_parse_util._raise_for_unsupported_param(return_value, func.__name__, e)
    if response_schema:
