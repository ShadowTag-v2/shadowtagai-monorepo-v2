...
class GkeCodeExecutor(BaseCodeExecutor):
        ...
        **data,
        ...
        if self.kubeconfig_path:
            try:
                ...
                config.load_kube_config(config_file=self.kubeconfig_path, context=self.kubeconfig_context)
    ...
    def execute_code(
        ...
        try:
            ...
            created_job = self._batch_v1.create_namespaced_job(body=job_manifest, namespace=self.namespace)
            ...
            logger.error(
                f"A Kubernetes API error occurred during job '{job_name}': {e.reason}",
                exc_info=True,
            )
    ...
    def _create_job_manifest(
        ...
        container = k8s.client.V1Container(
            ...
            volume_mounts=[k8s.client.V1VolumeMount(name="code-volume", mount_path="/app")],
        ...
        pod_spec = k8s.client.V1PodSpec(
            ...
            volumes=[
                k8s.client.V1Volume(
                    name="code-volume",
                    config_map=k8s.client.V1ConfigMapVolumeSource(name=configmap_name),
                )
            ],
        ...
        annotations = {"adk.agent.google.com/invocation-id": invocation_context.invocation_id}
    ...
    def _watch_job_completion(self, job_name: str) -> CodeExecutionResult:
        ...
        try:
            ...
            raise TimeoutError(f"Job '{job_name}' did not complete within {self.timeout_seconds}s.")
        finally:
            watch.stop()
    ...
    def _get_pod_logs(self, job_name: str) -> str:
        ...
        try:
            ...
            return self._core_v1.read_namespaced_pod_log(name=pod_name, namespace=self.namespace)
        except ApiException as e:
    ...
    def _add_owner_reference(self, owner_job: k8s.client.V1Job, configmap_name: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to set ownerReference on ConfigMap '{configmap_name}'. Manual cleanup is required. Reason: {e.reason}")
