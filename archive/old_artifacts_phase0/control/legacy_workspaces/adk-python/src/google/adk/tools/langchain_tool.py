...
class LangchainTool(FunctionTool):
    ...
    _langchain_tool: LangchainBaseTool | object
    """The wrapped langchain tool."""

    def __init__(
        self,
        tool: LangchainBaseTool | object,
        name: str | None = None,
        description: str | None = None,
    ):
        if not hasattr(tool, "run") and not hasattr(tool, "_run"):
            raise ValueError("Tool must be a Langchain tool, have a 'run' or '_run' method.")
        ...
        if isinstance(tool, StructuredTool):
            ...
            raise ValueError(
                "This is not supported. Tool must be a Langchain tool, have a 'run' or '_run' method. The tool is: ",
                type(tool),
            )
    ...
    def _get_declaration(self) -> types.FunctionDeclaration:
        ...
        try:
            ...
            raise ValueError(f"Failed to build function declaration for Langchain tool: {e}") from e

    @override
    @classmethod
    def from_config(cls: type[LangchainTool], config: ToolArgsConfig, config_abs_path: str) -> LangchainTool:
        from ..agents import config_agent_utils

        langchain_tool_config = LangchainToolConfig.model_validate(config.model_dump())
        tool = config_agent_utils.resolve_fully_qualified_name(langchain_tool_config.tool)
