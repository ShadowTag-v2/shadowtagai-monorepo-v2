...
from ...auth.auth_credential import AuthCredential, AuthCredentialTypes, ServiceAccount, ServiceAccountCredential
...
class ApplicationIntegrationToolset(BaseToolset):
    ...
    def __init__(
        ...
        entity_operations: str | None = None,
        ...
        if integration:
            ...
            connections_client = ConnectionsClient(project, location, connection, service_account_json)
            ...
            raise ValueError("Invalid request, Either integration or (connection and (entity_operations or actions)) should be provided.")
    ...
    def _parse_spec_to_toolset(self, spec_dict, connection_details):
        ...
        if self._service_account_json:
            sa_credential = ServiceAccountCredential.model_validate_json(self._service_account_json)
            service_account = ServiceAccount(
                service_account_credential=sa_credential,
                scopes=["https://www.googleapis.com/auth/cloud-platform"],
            )
            auth_scheme, auth_credential = service_account_scheme_credential(config=service_account)
        else:
        ...
        for open_api_operation in operations:
            ...
            auth_override_enabled = connection_details.get("authOverrideEnabled", False)

            if self._auth_scheme and self._auth_credential and not auth_override_enabled:
                # Case: Auth provided, but override is OFF. Don't use provided auth.
                logger.warning("Authentication schema and credentials are not used because authOverrideEnabled is not enabled in the connection.")
    ...
    async def get_tools(
        ...
        return (
            [tool for tool in self._tools if self._is_tool_selected(tool, readonly_context)]
