...
IntermediateDataType: TypeAlias = IntermediateData | InvocationEvents
...
class Invocation(EvalBaseModel):
    ...
    final_response: genai_types.Content | None = None
    """Final response from the agent."""

    intermediate_data: IntermediateDataType | None = None
  ...
  """
    ...
    rubrics: list[Rubric] | None = Field(
        default=None,
    )
    """A list of rubrics that are applicable to only this invocation."""

    app_details: AppDetails | None = Field(default=None)
...
class EvalCase(EvalBaseModel):
    ...
    conversation: StaticConversation | None = None
  ...
  """
    ...
    final_session_state: SessionState | None = Field(default_factory=dict)
    ...
    def ensure_conversation_xor_conversation_scenario(self) -> EvalCase:
        if (self.conversation is None) == (self.conversation_scenario is None):
            raise ValueError("Exactly one of conversation and conversation_scenario must be provided in an EvalCase.")
        return self
...
def get_all_tool_calls(
    ...
    if isinstance(intermediate_data, IntermediateData):
        ...
        raise ValueError(f"Unsupported type for intermediate_data `{intermediate_data}`")
...
ToolCallAndResponse: TypeAlias = tuple[genai_types.FunctionCall, genai_types.FunctionResponse | None]
...
def get_all_tool_calls_with_responses(
    ...
    tool_responses_by_call_id: dict[str, genai_types.FunctionResponse] = {
        tool_response.id: tool_response for tool_response in get_all_tool_responses(intermediate_data)
    }

    tool_call_and_responses: list[ToolCallAndResponse] = []

    for tool_call in get_all_tool_calls(intermediate_data):
        response = tool_responses_by_call_id.get(tool_call.id)
