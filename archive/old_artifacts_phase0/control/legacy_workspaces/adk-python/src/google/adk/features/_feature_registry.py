...
_FEATURE_REGISTRY: dict[FeatureName, FeatureConfig] = {
    FeatureName.COMPUTER_USE: FeatureConfig(FeatureStage.EXPERIMENTAL, default_on=True),
    FeatureName.JSON_SCHEMA_FOR_FUNC_DECL: FeatureConfig(FeatureStage.WIP, default_on=False),
    FeatureName.PROGRESSIVE_SSE_STREAMING: FeatureConfig(FeatureStage.WIP, default_on=False),
}
...
def _get_feature_config(
    ...
    """
    return _FEATURE_REGISTRY.get(feature_name)
...
def is_feature_enabled(feature_name: FeatureName) -> bool:
    ...
    feature_name_str = feature_name.value if isinstance(feature_name, FeatureName) else feature_name
