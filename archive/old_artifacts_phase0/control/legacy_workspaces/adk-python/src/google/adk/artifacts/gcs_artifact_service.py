...
class GcsArtifactService(BaseArtifactService):
    ...
    def _get_blob_name(
        ...
        """
        return f"{self._get_blob_prefix(app_name, user_id, filename, session_id)}/{version}"
    ...
    def _save_artifact(
        ...
        blob_name = self._get_blob_name(app_name, user_id, filename, version, session_id)
        ...
        if artifact.inline_data:
            ...
            raise NotImplementedError("Saving artifact with file_data is not supported yet in GcsArtifactService.")
        else:
    ...
    def _list_artifact_keys(self, app_name: str, user_id: str, session_id: str | None) -> list[str]:
        filenames = set()

        if session_id:
            session_prefix = f"{app_name}/{user_id}/{session_id}/"
            session_blobs = self.storage_client.list_blobs(self.bucket, prefix=session_prefix)
        ...
        user_namespace_blobs = self.storage_client.list_blobs(self.bucket, prefix=user_namespace_prefix)
