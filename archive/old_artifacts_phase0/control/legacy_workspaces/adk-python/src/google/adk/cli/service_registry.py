...
class ServiceFactory(Protocol):
    ...
    def __call__(self, uri: str, **kwargs) -> BaseSessionService | BaseArtifactService | BaseMemoryService: ...
...
class ServiceRegistry:
    ...
    def register_session_service(self, scheme: str, factory: ServiceFactory) -> None:
    ...
    def register_artifact_service(self, scheme: str, factory: ServiceFactory) -> None:
        """Register a factory for a custom artifact service URI scheme."""
        self._artifact_factories[scheme] = factory

    def register_memory_service(self, scheme: str, factory: ServiceFactory) -> None:
        """Register a factory for a custom memory service URI scheme."""
        self._memory_factories[scheme] = factory

    def create_session_service(self, uri: str, **kwargs) -> BaseSessionService | None:
    ...
    def create_artifact_service(self, uri: str, **kwargs) -> BaseArtifactService | None:
    ...
    def create_memory_service(self, uri: str, **kwargs) -> BaseMemoryService | None:
    ...
    for yaml_file in ["services.yaml", "services.yml"]:
        ...
        if os.path.exists(yaml_path):
            try:
                ...
                logger.debug("Loaded custom services from %s in %s.", yaml_file, agents_dir)
...
def _register_builtin_services(registry: ServiceRegistry) -> None:
    ...
    def agentengine_session_factory(uri: str, **kwargs):
        ...
        parsed = urlparse(uri)
        params = _parse_agent_engine_kwargs(parsed.netloc + parsed.path, kwargs.get("agents_dir"))
    ...
    def rag_memory_factory(uri: str, **kwargs):
        ...
        return VertexAiRagMemoryService(rag_corpus=(f"projects/{project}/locations/{location}/ragCorpora/{rag_corpus}"))
...
def _load_gcp_config(agents_dir: str | None, service_name: str) -> tuple[str, str]:
...
def _parse_agent_engine_kwargs(uri_part: str, agents_dir: str | None) -> dict[str, Any]:
    """Helper to parse agent engine resource name."""
    if not uri_part:
        raise ValueError("Agent engine resource name or resource id cannot be empty.")

    # If uri_part is just an ID, load project/location from env
    if "/" not in uri_part:
        project, location = _load_gcp_config(agents_dir, "short-form agent engine IDs")
        return {
            "project": project,
            "location": location,
    ...
    if not (len(parts) == 6 and parts[0] == "projects" and parts[2] == "locations" and parts[4] == "reasoningEngines"):
        raise ValueError(
...
def _register_services_from_yaml_config(config: dict[str, Any], registry: ServiceRegistry) -> None:
