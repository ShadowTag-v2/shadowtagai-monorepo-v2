...
class CachePerformanceAnalyzer:
    ...
    async def _get_agent_cache_history(
        ...
        for event in session.events:
            ...
            if event.cache_metadata is not None and (agent_name is None or event.author == agent_name):
                cache_history.append(event.cache_metadata)

        return cache_history

    async def analyze_agent_cache_performance(self, session_id: str, user_id: str, app_name: str, agent_name: str) -> dict[str, Any]:
        ...
        """
        cache_history = await self._get_agent_cache_history(session_id, user_id, app_name, agent_name)
        ...
        cache_hit_ratio_percent = (total_cached_tokens / total_prompt_tokens) * 100 if total_prompt_tokens > 0 else 0.0

        cache_utilization_ratio_percent = (requests_with_cache_hits / total_requests) * 100 if total_requests > 0 else 0.0

        avg_cached_tokens_per_request = total_cached_tokens / total_requests if total_requests > 0 else 0.0
        ...
        return {
            ...
            "avg_invocations_used": (sum(invocations_used) / len(invocations_used) if invocations_used else 0),
