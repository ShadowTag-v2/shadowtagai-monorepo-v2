...
from .functions import REQUEST_CONFIRMATION_FUNCTION_CALL_NAME, REQUEST_EUC_FUNCTION_CALL_NAME, remove_client_function_call_id
...
class _ContentLlmRequestProcessor(BaseLlmRequestProcessor):
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event]:
        ...
        await _add_instructions_to_user_content(invocation_context, llm_request, instruction_related_contents)
        ...
        if event.get_function_responses():
            ...
            for function_call in event.get_function_calls():
                ...
                if function_call_id in function_call_id_to_response_events_index:
                    function_response_events_indices.add(function_call_id_to_response_events_index[function_call_id])
            ...
            if len(function_response_events_indices) == 1:
                ...
                result_events.append(_merge_function_response_events([events[i] for i in sorted(function_response_events_indices)]))
            continue
        else:
            result_events.append(event)
...
def _rearrange_events_for_latest_function_response(
        ...
        if function_calls:
            for function_call in function_calls:
                if function_call.id in function_responses_ids:
                    ...
                    function_call_ids = {function_call.id for function_call in function_calls}
    ...
    if function_call_event_idx == -1:
        logger.debug(
            "No function call event found for function responses ids: %s in event list: %s",
            function_responses_ids,
            events,
        )
        raise ValueError(f"No function call event found for function responses ids: {function_responses_ids}")
    ...
    for idx in range(function_call_event_idx + 1, len(events) - 1):
        event = events[idx]
        function_responses = event.get_function_responses()
        if function_responses and any([function_response.id in function_responses_ids for function_response in function_responses]):
    ...
    result_events.append(_merge_function_response_events(function_response_events))
...
def _contains_empty_content(event: Event) -> bool:
    ...
    return (
        ...
        or all(
            not p.text and not p.inline_data and not p.file_data and not p.function_call and not p.function_response for p in [event.content.parts[0]]
        )
    ) and (not event.output_transcription and not event.input_transcription)


def _should_include_event_in_context(current_branch: str | None, event: Event) -> bool:
...
def _process_compaction_events(events: list[Event]) -> list[Event]:
    ...
    for event in reversed(events):
        if event.actions and event.actions.compaction:
            ...
            if compaction.start_timestamp is not None and compaction.end_timestamp is not None:
...
def _get_contents(current_branch: str | None, events: list[Event], agent_name: str = "") -> list[types.Content]:
    ...
    raw_filtered_events = [e for e in rewind_filtered_events if _should_include_event_in_context(current_branch, e)]

    has_compaction_events = any(e.actions and e.actions.compaction for e in raw_filtered_events)
...
def _get_current_turn_contents(current_branch: str | None, events: list[Event], agent_name: str = "") -> list[types.Content]:
    ...
    for i in range(len(events) - 1, -1, -1):
        event = events[i]
        if _should_include_event_in_context(current_branch, event) and (event.author == "user" or _is_other_agent_reply(agent_name, event)):
...
def _is_other_agent_reply(current_agent_name: str, event: Event) -> bool:
    ...
    return bool(current_agent_name and event.author != current_agent_name and event.author != "user")


def _present_other_agent_message(event: Event) -> Event | None:
    ...
    for part in event.content.parts:
        if part.thought:
            ...
            content.parts.append(types.Part(text=f"[{event.author}] said: {part.text}"))
        elif part.function_call:
            content.parts.append(
                types.Part(text=(f"[{event.author}] called tool `{part.function_call.name}` with parameters: {part.function_call.args}"))
            )
        elif part.function_response:
            # Otherwise, create a new text part.
            content.parts.append(
                types.Part(text=(f"[{event.author}] `{part.function_response.name}` tool returned result: {part.function_response.response}"))
            )
...
def _is_event_belongs_to_branch(invocation_branch: str | None, event: Event) -> bool:
    ...
    return invocation_branch == event.branch or invocation_branch.startswith(f"{event.branch}.")
