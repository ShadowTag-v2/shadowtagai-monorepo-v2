                ...
                "choices": [
                    {
                        "message": {
                            "role": "assistant",
                            "content": (f"The instrumentation tool responded with: {tool_summary}"),
                        },
                        "finish_reason": "stop",
                    }
                ],
...
def get_current_time(timezone_str: str) -> dict[str, str]:
    ...
    return {
        "status": "success",
        "report": (f"The current time in {timezone_str} is {now.strftime('%Y-%m-%d %H:%M:%S %Z')}."),
    }
...
root_agent = Agent(
    ...
    model=_mock_model,
    description=("Demonstrates LiteLLM inline JSON tool-call parsing without an external VLLM deployment."),
    instruction=(
