...
async def get_weather(city: str, tool_context: ToolContext) -> dict:
    ...
    tool_context.state["weather_requests"].append({"city": city, "timestamp": time.time(), "result": result})
...
async def calculate_distance(city1: str, city2: str, tool_context: ToolContext) -> dict:
    ...
    if city1 in city_coords and city2 in city_coords:
        ...
        distance = int(((coord1[0] - coord2[0]) ** 2 + (coord1[1] - coord2[1]) ** 2) ** 0.5 * 111)  # rough km conversion
    else:
...
async def get_population(cities: list[str], tool_context: ToolContext) -> dict:
    ...
    tool_context.state["population_requests"].append({"cities": cities, "results": results, "timestamp": time.time()})
