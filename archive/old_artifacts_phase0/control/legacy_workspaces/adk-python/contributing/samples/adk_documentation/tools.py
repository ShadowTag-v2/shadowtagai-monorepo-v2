...
from adk_documentation.utils import error_response, get_paginated_request, get_request, patch_request, post_request
...
def get_changed_files_between_releases(repo_owner: str, repo_name: str, start_tag: str, end_tag: str) -> dict[str, Any]:
            ...
            formatted_files.append(
                {
                    ...
                    "patch": file_data.get("patch", "No patch available."),  # The diff content
                }
            )
    ...
    try:
        ...
        if os.path.exists(local_path):
            ...
            if os.path.isdir(git_dir_path):
                ...
                return error_response(f"Path '{local_path}' exists but is not a Git repository.")
        else:
...
def search_local_git_repo(
    ...
    """
    print(f"Attempting to search for pattern: {pattern} in directory: {directory_path}, with extensions: {extensions}")
    try:
        ...
        if grep_process.stdout:
            for line in grep_process.stdout.strip().split("\n"):
                try:
                    ...
                    return error_response(f"Error: Failed to parse line: {line} from git grep output.")
        return {
            "status": "success",
            "matches": matches,
        }
