...
async def call_agent_async(
    ...
    async for event in runner.run_async(
        ...
        if event.content and event.content.parts and event.author != "user" and not event.partial:
...
async def test_basic_text_generation(runner: Runner, session_id: str):
    ...
    response, interaction_id = await call_agent_async(runner, USER_ID, session_id, "Hello! What can you help me with?")
...
async def test_multi_turn_conversation(runner: Runner, session_id: str):
    ...
    assert "59" in response2 or "london" in response2.lower() or "cloudy" in response2.lower(), (
    ...
    assert "blue" in response3.lower(), f"Expected agent to remember the color 'blue': {response3}"
...
async def test_google_search_tool(runner: Runner, session_id: str):
    ...
    assert response, "Expected a non-empty response"
    assert "orwell" in response.lower() or "george" in response.lower(), f"Expected George Orwell in response: {response}"
...
async def test_custom_function_tool(runner: Runner, session_id: str):
    ...
    assert "68" in response or "partly" in response.lower() or "tokyo" in response.lower(), f"Expected weather info for Tokyo in response: {response}"
...
def main():
    parser = argparse.ArgumentParser(description="Test the Interactions API integration")
    parser.add_argument(
        "--mode",
        choices=["test", "interactive"],
        default="test",
        help=("Run mode: 'test' runs automated tests, 'interactive' for manual testing"),
    )
    parser.add_argument(
        "--debug",
        action="store_true",
