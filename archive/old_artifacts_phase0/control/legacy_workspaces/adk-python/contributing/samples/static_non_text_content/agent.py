...
SAMPLE_IMAGE_DATA = base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==")
...
def create_static_instruction_with_file_upload():
    ...
    if api_variant == GoogleLLMVariant.VERTEX_AI:
        ...
        additional_text = " You also have access to a Gemma research paper from GCS and an AI research paper from HTTPS URL."
        ...
        if uploaded_file is None:
            ...
            try:
                ...
                print(f"Uploaded new file: {uploaded_file.name} ({uploaded_file.display_name})")
            finally:
    ...
    parts = [
        ...
        types.Part.from_text(text=f"This is a sample chart showing color data.{additional_text}"),
    ]
...
root_agent = Agent(
    ...
    description=("Demonstrates static instructions with non-text content (inline_data and file_data features)"),
    static_instruction=create_static_instruction_with_file_upload(),
    instruction=(
        "Please analyze the user's question and provide helpful insights. Reference the materials provided in your static instructions when relevant."
    ),
)
