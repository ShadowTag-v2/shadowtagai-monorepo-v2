...
from tau_bench.envs.retail import tasks_dev, tasks_test, tasks_train
...
def run_tau_bench_rollouts(
    ...
    for i in range(config.num_trials):
        ...
        def _run_with_retry(idx: int) -> EnvRunResult:
            ...
            rating = rater(res.messages[1:] if len(res.messages) > 1 else res.messages) if rater else None
            info = dict(res.info)
        ...
        for comp in components_to_update:
            ...
            for trace_instance in trace_instances:
                ...
                d = {
                    ...
                    "Feedback": json.dumps(rollout["task_info"]["feedback"], indent=2, default=str),
                }
                items.append(d)
            if items:
                ret_d[comp] = items
        assert ret_d, f"empty reflective dataset for components {[comp for comp in components_to_update]}"
        return ret_d
...
SEED_SYSTEM_INSTRUCTION = "you are a customer support agent helping customers resolve their issues by using the right tools"
