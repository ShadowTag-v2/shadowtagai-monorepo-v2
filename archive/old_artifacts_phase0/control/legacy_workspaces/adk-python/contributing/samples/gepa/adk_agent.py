...
from google.adk.agents import base_agent, llm_agent, loop_agent
...
class _Tool(base_tool.BaseTool):
    ...
    async def run_async(self, *, args: dict[str, Any], tool_context: Any) -> str:
        ...
        env_response = self._env.step(types.Part(function_call=types.FunctionCall(name=self.name, args=args)))
...
def _adk_agent(
    ...
    return llm_agent.LlmAgent(
        ...
        planner=built_in_planner.BuiltInPlanner(thinking_config=types.ThinkingConfig(thinking_budget=-1, include_thoughts=False)),
...
class _UserAgent(base_agent.BaseAgent):
    ...
    async def _run_async_impl(self, ctx: Any) -> Any:
        ...
        if not ctx.session.events:
            raise ValueError("No prior session events, this is unexpected as the user agent cannot be the first step in the interaction loop.")
        ...
        output_event = event_lib.Event(
            content=types.Content(parts=[types.Part(text=env_response.observation)], role="user"),
            author="user",
        )
        if env_response.done:
...
def run_environment_loop(
    ...
    async def _async_run():
        ...
        initial_message = types.Content(role="user", parts=[types.Part(text=env_reset_res.observation)])
