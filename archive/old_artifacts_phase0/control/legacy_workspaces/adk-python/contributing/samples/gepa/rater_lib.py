    ...
    for turn in conv:
        ...
        for part in turn["parts"]:
            if role == "user" and (txt := part.get("text")):
                ...
                res = res + f"The agent called the function {fc['name']} with the following function arguments: {fc['args']}.\n"
            elif fc := part.get("function_response"):
                res = res + f"The execution result from the agent of function {fc['name']} is: \n{fc['response']}\n"
    return res
...
class Rater:
    ...
    def __init__(
        ...
        logging.info("Loaded rubric validate template from path=%s", validation_template_path)
        self._rubric = rubric
    ...
    def __call__(self, messages: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        env.globals["user_input"] = messages[0].get("parts", [{}])[0].get("text", "") if messages else ""
