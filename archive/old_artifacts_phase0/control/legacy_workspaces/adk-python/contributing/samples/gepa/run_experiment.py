...
_NUM_TEST_RECORDS = flags.DEFINE_integer(
    "num_test_records",
    None,
    "Size of the test set for final evaluation of the optimized prompt. If None, uses all available test tasks.",
)
_NUM_EVAL_TRIALS = flags.DEFINE_integer(
    "num_eval_trials",
    4,
    "Number of times each task is run during evaluation. Higher values give more stable evaluation metrics but increase runtime. Recommended: 4-8.",
)
_MAX_CONCURRENCY = flags.DEFINE_integer(
    "max_concurrency",
    8,
    "Maximum number of parallel agent-environment interactions. Increase if you have sufficient API quota.",
)
...
def main(argv: Sequence[str]) -> None:
    ...
    output_dir = os.path.join(_OUTPUT_DIR.value, datetime.now().strftime("%Y%m%d%H%M%S%f"))
    ...
    eval_dir = os.path.join(output_dir, "evals", datetime.now().strftime("%Y%m%d%H%M%S%f"))
