...
try:
    ...
    from .utils import get_test_prompts, run_experiment_batch
except ImportError:
...
async def run_cache_comparison_experiment(
    ...
    session_cached = await runner_cached.session_service.create_session(app_name=runner_cached.app_name, user_id=USER_ID)
    session_uncached = await runner_uncached.session_service.create_session(app_name=runner_uncached.app_name, user_id=USER_ID)
    ...
    cached_cache_utilization_ratio = cached_analysis.get("cache_utilization_ratio_percent", 0.0)
    cached_avg_cached_tokens_per_request = cached_analysis.get("avg_cached_tokens_per_request", 0.0)
    ...
    uncached_cache_utilization_ratio = uncached_analysis.get("cache_utilization_ratio_percent", 0.0)
    uncached_avg_cached_tokens_per_request = uncached_analysis.get("avg_cached_tokens_per_request", 0.0)
    ...
    summary = {
        ...
        "cache_analysis": {
            ...
            "uncached_experiment": {
                ...
                "avg_cached_tokens_per_request": (uncached_avg_cached_tokens_per_request),
    ...
    print(f"      Cache Hit Ratio: {cached_cache_hit_ratio:.1f}% ({cached_total_cached_tokens:,} / {cached_total_prompt_tokens:,} tokens)")
    print(f"      Cache Utilization: {cached_cache_utilization_ratio:.1f}% ({cached_requests_with_hits}/{total_cached_requests} requests)")
    print(f"      Avg Cached Tokens/Request: {cached_avg_cached_tokens_per_request:.0f}")
    print(f"   ❄️  {uncached_label}:")
    print(f"      Cache Hit Ratio: {uncached_cache_hit_ratio:.1f}% ({uncached_total_cached_tokens:,} / {uncached_total_prompt_tokens:,} tokens)")
    print(f"      Cache Utilization: {uncached_cache_utilization_ratio:.1f}% ({uncached_requests_with_hits}/{total_uncached_requests} requests)")
    print(f"      Avg Cached Tokens/Request: {uncached_avg_cached_tokens_per_request:.0f}")
    print()
...
async def analyze_cache_performance_from_sessions(
    ...
    try:
        ...
        print("  🔥 Cached Experiment Analysis:")
        ...
        if cached_analysis["status"] == "active":
            ...
            print(f"     Avg Cached Tokens/Request: {cached_analysis['avg_cached_tokens_per_request']:.0f}")
            print(f"     Requests with cache: {cached_analysis['requests_with_cache']}")
            print(f"     Avg invocations used: {cached_analysis['avg_invocations_used']:.1f}")
    ...
    try:
        ...
        print("  ❄️  Uncached Experiment Analysis:")
        ...
        if uncached_analysis["status"] == "active":
            ...
            print(f"     Avg Cached Tokens/Request: {uncached_analysis['avg_cached_tokens_per_request']:.0f}")
            print(f"     Requests with cache: {uncached_analysis['requests_with_cache']}")
            print(f"     Avg invocations used: {uncached_analysis['avg_invocations_used']:.1f}")
...
def calculate_averaged_results(all_results: list[dict[str, Any]], model_name: str) -> dict[str, Any]:
    """Calculate averaged results from multiple experiment runs."""
    if not all_results:
        raise ValueError("No results to average")

    # Calculate average cache metrics
    cache_hit_ratios = [r["cache_analysis"]["cache_hit_ratio_percent"] for r in all_results]
    cache_utilization_ratios = [r["cache_analysis"]["cache_utilization_ratio_percent"] for r in all_results]
    total_prompt_tokens = [r["cache_analysis"]["total_prompt_tokens"] for r in all_results]
    total_cached_tokens = [r["cache_analysis"]["total_cached_tokens"] for r in all_results]
    avg_cached_tokens_per_request = [r["cache_analysis"]["avg_cached_tokens_per_request"] for r in all_results]
    requests_with_cache_hits = [r["cache_analysis"]["requests_with_cache_hits"] for r in all_results]
    ...
    averaged_result = {
        ...
        "averaged_cache_analysis": {
            ...
            "avg_cached_tokens_per_request": safe_average(avg_cached_tokens_per_request),
        ...
        "statistics": {
            ...
            "cached_tokens_per_request_std": _calculate_std(avg_cached_tokens_per_request),
        },
    }
    ...
    print(f"   Average Cache Hit Ratio: {avg_cache['cache_hit_ratio_percent']:.1f}% (±{stats['cache_hit_ratio_std']:.1f}%)")
    print(f"   Average Cache Utilization: {avg_cache['cache_utilization_ratio_percent']:.1f}% (±{stats['cache_utilization_std']:.1f}%)")
    print(f"   Average Cached Tokens/Request: {avg_cache['avg_cached_tokens_per_request']:.0f} (±{stats['cached_tokens_per_request_std']:.0f})")
    print()
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help=("Number of times to repeat each experiment for averaged results (default: 1)"),
    )
    parser.add_argument(
        "--cached-first",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help=("Delay in seconds between API requests to avoid overloading (default: 2.0)"),
    )
    parser.add_argument(
        "--log-level",
    ...
    if not args.output:
        args.output = f"cache_{args.model.replace('.', '_').replace('-', '_')}_results.json"
    ...
    if args.repeat == 1:
        ...
        print(f"    Cache Utilization: {cached_exp['cache_utilization_ratio_percent']:.1f}%")
        print(f"    Cached Tokens/Request: {cached_exp['avg_cached_tokens_per_request']:.0f}")
        print(f"  ❄️  {labels['uncached_label']}:")
        print(f"    Cache Hit Ratio: {uncached_exp['cache_hit_ratio_percent']:.1f}%")
        print(f"    Cache Utilization: {uncached_exp['cache_utilization_ratio_percent']:.1f}%")
        print(f"    Cached Tokens/Request: {uncached_exp['avg_cached_tokens_per_request']:.0f}")
    else:
        ...
        print(f"    Cache Hit Ratio: {cached_exp['cache_hit_ratio_percent']:.1f}% vs {uncached_exp['cache_hit_ratio_percent']:.1f}%")
        print(
            f"    Cache Utilization: {cached_exp['cache_utilization_ratio_percent']:.1f}% vs {uncached_exp['cache_utilization_ratio_percent']:.1f}%"
        )
