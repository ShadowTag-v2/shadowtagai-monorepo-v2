...
try:
    ...
    print("WARNING: Sounddevice or numpy not found. Audio RECORDING will be disabled.")
...
def init_pyaudio_playback():
    ...
    if not AUDIO_PLAYBACK_ENABLED:  # If already marked as disabled (e.g. previous attempt failed)
        logging.warning("PyAudio playback init skipped as it's marked disabled.")
        return False
    try:
        pya_interface_instance = pyaudio.PyAudio()
        logging.info(f"Initializing PyAudio output stream: Rate={PYAUDIO_PLAY_RATE}, Channels={PYAUDIO_PLAY_CHANNELS}, Format=paInt16")
        ...
        print(
            f"ERROR: Failed to initialize PyAudio for playback: {e}. Check PortAudio installation if on Linux/macOS.",
            flush=True,
        )
        ...
        try:
            ...
            while is_streaming_audio:
                try:
                    ...
                    logging.warning(f"WebSocket connection closed while sending audio stream: {e}")
                    is_streaming_audio = False
                    break
                except Exception as e:
                    logging.error(f"Error in audio streaming send loop: {e}", exc_info=True)
                    is_streaming_audio = False
                    break
        except Exception as e:
            logging.error(f"Failed to start or run audio InputStream: {e}", exc_info=True)
            ...
            if global_input_stream:
                try:
                    ...
                    logging.error(f"Error stopping/closing Sounddevice InputStream: {e_sd_close}")
...
class AgentResponseAudioPlayer:
    def cleanup_pyaudio_playback(self):
        ...
        if pya_interface_instance:
            try:
                ...
                logging.error(f"Error terminating PyAudio interface: {e}", exc_info=True)
            finally:
                pya_interface_instance = None
    ...
    def _play_audio_pyaudio_handler(self, audio_bytes: bytes, mime_type_full: str):
        if not AUDIO_PLAYBACK_ENABLED or not pya_output_stream_instance:
            logging.warning("PyAudio stream not available or playback disabled. Cannot play audio.")
            return
        try:
            logging.debug(f"PyAudio handler: Mime='{mime_type_full}', Size={len(audio_bytes)}")
            ...
            if mime_type_base == "audio/pcm":
                ...
                if match:
                    try:
                        ...
                        logging.warning(f"Could not parse rate from '{mime_type_full}', using stream default {PYAUDIO_PLAY_RATE}Hz.")
                ...
                logging.info(f"Preparing raw PCM for PyAudio stream (target rate {PYAUDIO_PLAY_RATE}Hz).")
            else:
                logging.warning(f"Unsupported MIME type for PyAudio playback: {mime_type_full}")
                return

            if playable_data_bytes:
                pya_output_stream_instance.write(playable_data_bytes)
                logging.info(f"Audio chunk written to PyAudio stream (Size: {len(playable_data_bytes)} bytes).")
            else:
                logging.warning("No playable bytes prepared for PyAudio.")

        except Exception as e:
            logging.error(f"Error in _blocking_play_audio_pyaudio_handler: {e}", exc_info=True)

    async def play_audio_data(self, audio_bytes: bytes, mime_type: str):
        if not AUDIO_PLAYBACK_ENABLED:
            logging.debug("PyAudio Playback is disabled, skipping play_audio_data call.")
            return
        print(f"Scheduling PyAudio playback for {mime_type} audio.")
        await asyncio.to_thread(self._play_audio_pyaudio_handler, audio_bytes, mime_type)
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response_get.status_code == 200:
                ...
                logging.info(f"Session '{session_id}' not found. Attempting to create via POST.")
                response_post = await client.post(session_url, json={}, timeout=10)
                if response_post.status_code == 200:
                    logging.info(f"Session '{session_id}' created.")
                    return True
                else:
                    logging.error(f"Failed to create session '{session_id}'. POST Status: {response_post.status_code}")
                    return False
            else:
                logging.warning(f"Could not verify session '{session_id}'. GET Status: {response_get.status_code}")
                return False
    except Exception as e:
...
async def websocket_client():
    ...
    if APP_NAME == "hello_world" or USER_ID.startswith("your_user_id") or SESSION_ID.startswith("your_session_id"):
        logging.warning("Using default/example APP_NAME, USER_ID, or SESSION_ID.")

    session_ok = await ensure_session_exists(APP_NAME, USER_ID, SESSION_ID, SERVER_HOST, SERVER_PORT)
    if not session_ok:
    ...
    try:
        async with websockets.connect(uri, open_timeout=10, close_timeout=10) as websocket:
                ...
                try:
                    async for message in websocket:
                        ...
                        try:
                            ...
                            logging.info(f"<<< Parsed event from server: (Keys: {list(event_data.keys())})")
                                    ...
                                    for part in content_obj["parts"]:
                                        if isinstance(part, dict) and "inlineData" in part:
                                            ...
                                            if (
                                                ...
                                                logging.info(f"Audio part found: Mime='{mime_type_full}', Base64Len={len(audio_b64)}")
                                                try:
                                                    standard_b64_string = audio_b64.replace("-", "+").replace("_", "/")
                                                    missing_padding = len(standard_b64_string) % 4
                                                    if missing_padding:
                                                        standard_b64_string += "=" * (4 - missing_padding)

                                                    audio_bytes_decoded = base64.b64decode(standard_b64_string)

                                                    if audio_bytes_decoded:
                                                        await agent_response_audio_player.play_audio_data(audio_bytes_decoded, mime_type_full)
                                                    else:
                                                        logging.warning("Decoded audio data is empty after sanitization and padding.")

                                                except base64.binascii.Error as b64e:
                                                    # Log details if decoding still fails
                                                    logging.error(f"Base64 decode error after sanitization and padding. Error: {b64e}")
                                                except Exception as e:
                                                    logging.error(
                                                        f"Error processing audio for playback (original string prefix: '{audio_b64[:50]}...'): {e}",
                                                        exc_info=True,
                                                    )
                    ...
                    logging.warning(f"Receiver: Connection closed (Code: {e.code}, Reason: '{e.reason if e.reason else 'N/A'}')")
                except Exception:
            ...
            async def send_messages_local(ws: websockets.WebSocketClientProtocol):
                ...
                logging.info("Sender task started: Type 'start_stream', 'stop_stream', text,sendfile, or 'quit'.")
                while True:
                    await asyncio.sleep(10**-12)
                    try:
                        ...
                        if user_input.lower() == "quit":
                            ...
                            if audio_stream_task and not audio_stream_task.done():
                                logging.info("Sender: Stopping active audio stream due to quit command.")
                            ...
                            if audio_stream_task and not audio_stream_task.done():
                                ...
                                continue
                            audio_stream_task = asyncio.create_task(audio_streaming_component.start_audio_streaming(ws))
                            ...
                            if audio_stream_task and not audio_stream_task.done():
                                ...
                                logging.warning("Sender: Audio stream is not currently running or already stopped.")
                        ...
                        # If you need it, uncomment the block from my previous response.
                            ...
                            if audio_stream_task and isinstance(audio_stream_task, asyncio.Task) and not audio_stream_task.done():
                                logging.warning("Please stop the current audio stream with 'stop_stream' before sending a file.")
                                continue
                            ...
                            if not filepath.lower().endswith(".wav"):
                                logging.warning(f"File {filepath} does not end with .wav. Attempting to send anyway.")
                                print(f"Warning: File '{filepath}' is not a .wav file. Ensure it's a compatible WAV.")
                            ...
                            try:
                                ...
                                payload_str = create_audio_request_payload(audio_file_bytes, REC_AUDIO_MIME_TYPE)
                                logging.info(
                                    ">>> Sending audio file"
                        ...
                        logging.warning(f"Sender: WebSocket connection closed. Code: {e.code}, Reason: {e.reason}")
                        if audio_stream_task and not audio_stream_task.done():
                        ...
                        logging.error(f"Sender: Unhandled error: {e_send_loop}", exc_info=True)
                        if audio_stream_task and not audio_stream_task.done():
                            await audio_streaming_component.stop_audio_streaming()
                            await audio_stream_task
                            audio_stream_task = None
            ...
            receive_task = asyncio.create_task(receive_messages(websocket), name="ReceiverThread")
            send_task = asyncio.create_task(send_messages_local(websocket), name="SenderThread")

            done, pending = await asyncio.wait([receive_task, send_task], return_when=asyncio.FIRST_COMPLETED)
            logging.info(f"Main task completion: Done={len(done)}, Pending={len(pending)}")

            current_active_audio_task = audio_stream_task
            if current_active_audio_task and not current_active_audio_task.done():
                logging.info("A main task finished. Ensuring audio stream is stopped if active.")
                await audio_streaming_component.stop_audio_streaming()
                try:
                    await asyncio.wait_for(current_active_audio_task, timeout=5.0)
                    logging.info("Audio streaming task gracefully stopped after main task completion.")
                except TimeoutError:
                    logging.warning("Timeout waiting for audio stream to stop post main task. Cancelling.")
                    current_active_audio_task.cancel()
                except Exception as e_stream_stop:
                    logging.error(f"Error during audio stream stop after main task: {e_stream_stop}")
            ...
            for task in all_tasks_to_await:
                try:
                    ...
                    logging.error(f"Error awaiting main task {task.get_name()}: {e}", exc_info=True)
...
if __name__ == "__main__":
    ...
    if APP_NAME == "hello_world" or USER_ID.startswith("your_user_id") or SESSION_ID.startswith("your_session_id"):
        print(
            "WARNING: Using default/example APP_NAME, USER_ID, or SESSION_ID. Please update these.",
            flush=True,
        )
    ...
    try:
        ...
        logging.critical("A critical unhandled exception occurred in __main__.", exc_info=True)
