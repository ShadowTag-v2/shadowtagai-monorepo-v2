...
from adk_answering_agent.settings import OWNER, REPO
from adk_answering_agent.utils import call_agent_async, parse_number_string, run_graphql_query
...
async def list_most_recent_discussions(
    ...
    """
    print(f"Attempting to fetch the {count} most recently updated discussions from {OWNER}/{REPO}...")
    ...
    try:
        ...
        discussions = response.get("data", {}).get("repository", {}).get("discussions", {}).get("nodes", [])
...
async def main():
    ...
    if args.recent:
        ...
        if not discussion_number:
            print(f"Error: Invalid discussion number received: {args.discussion_number}.")
            return
        ...
        try:
            ...
            if args.discussion:
                ...
                with open(args.discussion_file, encoding="utf-8") as f:
    ...
    for discussion_number in discussion_numbers:
        ...
        if discussion_json_data:
            ...
            prompt = f"Please check discussion #{discussion_number} see if you can help answer the question or provide some information!"
...
if __name__ == "__main__":
    ...
    print(f"Start Q&A checking on {OWNER}/{REPO} at {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(start_time))}")
    print("-" * 80)
    asyncio.run(main())
    print("-" * 80)
    end_time = time.time()
    print(
        f"Q&A checking finished at {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(end_time))}",
    )
