...
from adk_answering_agent.settings import GITHUB_GRAPHQL_URL, GITHUB_TOKEN
...
def run_graphql_query(query: str, variables: dict[str, Any]) -> dict[str, Any]:
    ...
    response = requests.post(GITHUB_GRAPHQL_URL, headers=headers, json=payload, timeout=60)
...
def parse_number_string(number_str: str | None, default_value: int = 0) -> int:
    ...
    try:
        ...
        print(
            f"Warning: Invalid number string: {number_str}. Defaulting to {default_value}.",
            file=sys.stderr,
        )
        return default_value
...
def convert_gcs_to_https(gcs_uri: str) -> str | None:
    ...
    try:
        ...
        if len(parts) < 3:
            raise ValueError("GCS link must contain a bucket, prefix, and relative_path.")
