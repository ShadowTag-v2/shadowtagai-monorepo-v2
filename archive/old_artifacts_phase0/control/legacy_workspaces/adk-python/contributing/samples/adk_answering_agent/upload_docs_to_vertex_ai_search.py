...
from adk_answering_agent.settings import ADK_DOCS_ROOT_PATH, ADK_PYTHON_ROOT_PATH, GCS_BUCKET_NAME, GOOGLE_CLOUD_PROJECT, VERTEXAI_DATASTORE_ID
...
def upload_directory_to_gcs(source_directory: str, project_id: str, bucket_name: str, prefix: str) -> bool:
    """Upload the whole directory into GCS."""
    print(f"Start uploading directory {source_directory} to GCS: gs://{bucket_name}/{prefix}...")
        ...
        for filename in files:
            ...
            try:
                ...
                if filename.lower().endswith(".md"):
                    ...
                    bucket.blob(gcs_path).upload_from_string(html_content, content_type=content_type)
                    ...
                    bucket.blob(gcs_path).upload_from_string(yaml_content, content_type=content_type)
                else:  # Python files
                    bucket.blob(gcs_path).upload_from_filename(local_path, content_type=content_type)
                type_msg = f"(type {content_type})" if content_type else "(type auto-detect)"
                print(f"  - Uploaded {type_msg}: {local_path} -> gs://{bucket_name}/{gcs_path}")
                file_count += 1
            except GoogleAPICallError as e:
                print(f"[ERROR] Error uploading file {local_path}: {e}", file=sys.stderr)
                return False
...
def import_from_gcs_to_vertex_ai(
    ...
    try:
        ...
        print(f"Successfully started full sync import operation.Operation Name: {operation.operation.name}")
        return True
