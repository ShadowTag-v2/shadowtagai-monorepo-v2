...
from adk_answering_agent.utils import convert_gcs_to_https, error_response, run_graphql_query
...
def get_discussion_and_comments(discussion_number: int) -> dict[str, Any]:
    ...
    try:
        ...
        discussion_data = response.get("data", {}).get("repository", {}).get("discussion")
...
def add_comment_to_discussion(discussion_id: str, comment_body: str) -> dict[str, Any]:
    ...
    if not comment_body.startswith("**Response from ADK Answering Agent"):
        comment_body = "**Response from ADK Answering Agent (experimental, answer may be inaccurate)**\n\n" + comment_body
    ...
    try:
        ...
        if "errors" in response:
            return error_response(str(response["errors"]))
        new_comment = response.get("data", {}).get("addDiscussionComment", {}).get("comment")
...
def get_label_id(label_name: str) -> str | None:
    ...
    try:
        response = run_graphql_query(query, variables)
        if "errors" in response:
            print(f"[Warning] Error from GitHub API response for label '{label_name}': {response['errors']}")
            return None
...
def convert_gcs_links_to_https(gcs_uris: list[str]) -> dict[str, str | None]:
