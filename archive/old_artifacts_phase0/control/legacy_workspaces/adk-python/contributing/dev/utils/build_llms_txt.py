...
def expand_code_snippets(content: str, project_root: Path) -> str:
    ...
    def replace_snippet(match):
        ...
        snippet_path_str = match.group(2)  # Capture the file path (e.g., "examples/python/snippets/file.py")
        section_name = match.group(3)  # Capture the section name if present (e.g., "init")
        snippet_full_path = project_root / snippet_path_str  # Changed from base_path to project_root
            ...
            try:
                ...
                if section_name:
                    ...
                    if start_index != -1 and end_index != -1 and start_index < end_index:
                        ...
                        extracted_lines = []
                        for line in lines:
                            if not line.strip().startswith("# --8<--") and not line.strip().startswith("## --8<--") and line.strip() != "":
                                extracted_lines.append(line)
                        extracted_content = "".join(extracted_lines).strip("\n")

                        return textwrap.indent(extracted_content, indent)
                    else:
                        print(f"Warning: Section '{section_name}' not found or markers malformed in {snippet_full_path}")
...
def build_index(docs: Path) -> str:
    ...
    if adk_readme:
        ...
        lines.append("")
        lines.append("**Source:** [adk-python repository](https://github.com/google/adk-python)")
        lines.append("")
    ...
    for md in sorted(docs.rglob("*.md")):
        ...
        url = f"https://github.com/google/adk-docs/blob/main/docs/{rel}".replace(" ", "%20")
...
def build_full(docs: Path) -> str:
    ...
    if adk_readme:
        ...
        expanded_adk_readme = expand_code_snippets(strip_java(adk_readme), project_root)  # Pass project_root
    ...
    for md in sorted(docs.rglob("*.md")):
        ...
        expanded_md_content = expand_code_snippets(strip_java(md_content), project_root)  # Changed back to project_root
