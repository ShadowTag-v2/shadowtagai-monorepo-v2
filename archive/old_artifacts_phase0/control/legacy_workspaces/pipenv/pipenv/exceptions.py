...
class SystemUsageError(PipenvOptionsError):
    def __init__(self, option_name="system", message=None, ctx=None, **kwargs):
        extra = kwargs.pop("extra", [])
        extra += [
            "{}: --system is intended to be used for Pipfile installation, not installation of specific packages. Aborting.".format(
...
class VirtualenvActivationException(VirtualenvException):
    def __init__(self, message=None, **kwargs):
        if not message:
            message = "activate_this.py not found. Your environment is most certainly not activated. Continuing anyway..."
        self.message = message
...
class DependencyConflict(PipenvException):
    def __init__(self, message):
        extra = ["[bold red]The operation failed...[/bold red] [red]A dependency conflict was detected and could not be resolved.[/red]"]
...
class ResolutionFailure(PipenvException):
    def __init__(self, message, no_version_found=False):
        ...
        if "no version found at all" in message_str:
            message += "[cyan]Please check your version specifier and version number. See PEP440 for more information.[/cyan]"
