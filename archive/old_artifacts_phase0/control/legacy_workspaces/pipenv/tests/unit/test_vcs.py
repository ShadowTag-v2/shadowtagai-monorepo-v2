...
def test_install_req_from_pipfile_vcs_with_env_vars():
    ...
    os.environ.update({"GIT_HOST": "github.com", "GIT_ORG": "testorg", "GIT_REPO": "testrepo"})

    pipfile = {"git": "https://${GIT_HOST}/${GIT_ORG}/${GIT_REPO}.git", "ref": "master", "extras": ["test"]}
...
def test_install_req_from_pipfile_with_auth():
    ...
    os.environ.update({"GIT_USER": "testuser", "GIT_TOKEN": "testtoken"})

    pipfile = {"git": "https://${GIT_USER}:${GIT_TOKEN}@github.com/org/repo.git", "ref": "main"}
...
def test_install_req_from_pipfile_editable():
    ...
    pipfile = {"git": "https://${REPO_URL}.git", "editable": True, "ref": "develop"}
...
def test_install_req_from_pipfile_subdirectory():
    ...
    pipfile = {"git": "https://github.com/${REPO_PATH}.git", "subdirectory": "subdir", "ref": "main"}
...
@pytest.mark.parametrize(
    "url_format,expected_url,expected_req",
    [
        ("git+https://${HOST}/${REPO}.git", "https://github.com/org/repo.git", "package-name @ git+https://${HOST}/${REPO}.git@main"),
        (
...
def test_various_vcs_url_formats(url_format, expected_url, expected_req):
    ...
    os.environ.update({"HOST": "github.com", "REPO": "org/repo", "USER": "git"})
...
def test_git_ssh_shorthand_format():
    ...
    os.environ.update({"HOST": "github.com", "REPO": "org/repo"})
...
def test_git_url_format_variations():
    ...
    test_cases = [
        {"git": "https://${HOST}/${REPO}.git", "expected_vars": ["${HOST}", "${REPO}"]},
        {"git": "git+https://${HOST}/${REPO}.git", "expected_vars": ["${HOST}", "${REPO}"]},
        {"git": "git+ssh://${USER}@${HOST}/${REPO}.git", "expected_vars": ["${USER}", "${HOST}", "${REPO}"]},
        {"git": "ssh://git@${HOST}/${REPO}.git", "expected_vars": ["${HOST}", "${REPO}"]},
    ]
...
def test_ssh_protocol_variations():
    ...
    test_cases = ["git+ssh://git@${HOST}/${REPO}.git", "ssh://git@${HOST}/${REPO}.git", "git@${{HOST}}:${{REPO}}.git"]
...
def test_complex_ssh_url_handling():
    ...
    pipfile = {"git": "git+ssh://git@${HOST}:${PORT}/${REPO}.git", "ref": "main"}

    os.environ.update({"HOST": "github.com", "PORT": "22", "REPO": "org/repo"})
...
def test_git_protocol_handling():
    ...
    pipfile = {"git": "git://${HOST}/${REPO}.git", "ref": "main"}
