...
type CopyObjectSafeParams = z.infer<z.ZodObject<typeof inputSchema>>;

export async function copyObjectSafe(params: CopyObjectSafeParams): Promise<CallToolResult> {
  try {
    logger.info(
      `Copying safe object: ${params.source_object_name} from bucket: ${params.source_bucket_name} to ${params.destination_object_name} in bucket: ${params.destination_bucket_name}`,
    );
    ...
    const [copiedFile] = await storage
      ...
      .copy(destinationFile, { preconditionOpts: { ifGenerationMatch: 0 } });
...
export const registerCopyObjectSafeTool = (server: McpServer) => {
  server.registerTool(
    'copy_object_safe',
    {
      description:
