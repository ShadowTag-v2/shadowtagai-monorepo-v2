...
def test_sync_load_tool_success(test_tool_str_schema, sync_client, mock_transport):
    ...
    manifest = ManifestSchema(serverVersion="0.0.0", tools={TOOL_NAME: test_tool_str_schema})
    mock_transport.tool_get_mock.return_value = manifest
    mock_transport.tool_invoke_mock.return_value = "sync_tool_ok"
    sync_client._ToolboxSyncClient__async_client._ToolboxClient__transport = mock_transport
    ...
    assert list(sig.parameters.keys()) == [p.name for p in test_tool_str_schema.parameters]
    result = loaded_tool(param1="some value")
    assert result == "sync_tool_ok"


def test_sync_load_toolset_success(test_tool_str_schema, test_tool_int_bool_schema, sync_client, mock_transport):
...
def test_sync_invoke_tool_server_error(test_tool_str_schema, sync_client, mock_transport):
...
def test_sync_load_tool_not_found_in_manifest(test_tool_str_schema, sync_client, mock_transport):
    ACTUAL_TOOL_IN_MANIFEST = "actual_tool_sync_abc"
    REQUESTED_TOOL_NAME = "non_existent_tool_sync_xyz"
    mismatched_manifest = ManifestSchema(serverVersion="0.0.0", tools={ACTUAL_TOOL_IN_MANIFEST: test_tool_str_schema})
...
class TestSyncClientLifecycle:
    ...
    def test_sync_client_creation_in_isolated_env(self, sync_client):
        ...
        assert sync_client._ToolboxSyncClient__thread is not None, "Thread should be created"
        assert sync_client._ToolboxSyncClient__thread.is_alive(), "Thread should be running"
        assert isinstance(sync_client._ToolboxSyncClient__async_client, ToolboxClient), "Async client should be ToolboxClient instance"
    ...
    def test_sync_client_context_manager(self, tool_schema_minimal, mock_transport):
        ...
        """
        with patch.object(ToolboxSyncClient, "close", wraps=ToolboxSyncClient.close, autospec=True) as mock_close_method:
            with ToolboxSyncClient(TEST_BASE_URL) as client:
                client._ToolboxSyncClient__async_client._ToolboxClient__transport = mock_transport
        ...
        with patch.object(ToolboxSyncClient, "close", wraps=ToolboxSyncClient.close, autospec=True) as mock_close_method_exc:
            with pytest.raises(ValueError, match="Test exception"), ToolboxSyncClient(TEST_BASE_URL):
    ...
    def test_load_tool_raises_if_loop_or_thread_none(self):
        ...
        assert original_class_loop is not None, "Loop should have been created by client init"
        assert original_class_thread is not None, "Thread should have been created by client init"
        ...
        ToolboxSyncClient._ToolboxSyncClient__loop = original_class_loop  # Restore for next check
...
class TestSyncClientHeaders:
    ...
    def test_sync_add_headers_success(self, test_tool_str_schema, sync_client, mock_transport):
    ...
    def test_sync_add_headers_duplicate_fail(self):
        ...
        with patch("toolbox_core.sync_client.ToolboxClient", return_value=mock_async_client):
...
class TestSyncAuth:
    ...
    def test_auth_with_load_tool_success(
        ...
        manifest = ManifestSchema(serverVersion="0.0.0", tools={tool_name_auth: test_tool_auth_schema})
        mock_transport.tool_get_mock.return_value = manifest
        ...
        tool = sync_client.load_tool(tool_name_auth, auth_token_getters={"my-auth-service": token_handler})
        result = tool(argA=5)
    ...
    def test_auth_with_load_tool_fail_no_token(self, tool_name_auth, test_tool_auth_schema, sync_client, mock_transport):
        manifest = ManifestSchema(serverVersion="0.0.0", tools={tool_name_auth: test_tool_auth_schema})
        mock_transport.tool_get_mock.return_value = manifest
        sync_client._ToolboxSyncClient__async_client._ToolboxClient__transport = mock_transport
    ...
    def test_add_auth_token_getters_duplicate_fail(self, tool_name_auth, test_tool_auth_schema, sync_client, mock_transport):
        manifest = ManifestSchema(serverVersion="0.0.0", tools={tool_name_auth: test_tool_auth_schema})
        mock_transport.tool_get_mock.return_value = manifest
        sync_client._ToolboxSyncClient__async_client._ToolboxClient__transport = mock_transport
    ...
    def test_add_auth_token_getters_missing_fail(self, tool_name_auth, test_tool_auth_schema, sync_client, mock_transport):
        manifest = ManifestSchema(serverVersion="0.0.0", tools={tool_name_auth: test_tool_auth_schema})
        mock_transport.tool_get_mock.return_value = manifest
        sync_client._ToolboxSyncClient__async_client._ToolboxClient__transport = mock_transport
    ...
    def test_constructor_getters_missing_fail(self, tool_name_auth, test_tool_auth_schema, sync_client, mock_transport):
        manifest = ManifestSchema(serverVersion="0.0.0", tools={tool_name_auth: test_tool_auth_schema})
        mock_transport.tool_get_mock.return_value = manifest
        sync_client._ToolboxSyncClient__async_client._ToolboxClient__transport = mock_transport
