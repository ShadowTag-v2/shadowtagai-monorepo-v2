...
import { Button, Card, FloatingLabel, Form, Image, Modal, Spinner, Table } from 'react-bootstrap';
  ...
  const handleAddEditFormSubmit = (e) => {
    ...
    if (itemPrice.value && itemName.value) {
      if (addEditFormType === 'Add') {
        setIsLoading(true);
        FirestoreService.AddNewMenuItem(itemName.value, itemCategory.value, itemPrice.value)
          .then(() => {
                  ...
                  >
                    {menuCategories &&
                      menuCategories.map((menuCategory, index) => (
                        ...
                        <option
                          key={index}
                          value={menuCategory.doc.data.value.mapValue.fields.catName.stringValue}
                        >
          ...
          <Card style={{ margin: 24 }}>
                  ...
                  {menuItems &&
                    menuItems.map((menuItem, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          {menuItem.doc.data.value.mapValue.fields.itemPrice.doubleValue
                            ? menuItem.doc.data.value.mapValue.fields.itemPrice.doubleValue
                            : menuItem.doc.data.value.mapValue.fields.itemPrice.integerValue}
                        </td>
                        <td>
                          <Button
                            variant="primary"
                            onClick={() => {
                              ...
                              setCurrentMenuItem({
                                itemName:
                                  menuItem.doc.data.value.mapValue.fields.itemName.stringValue,
                                itemCategory:
                                  menuItem.doc.data.value.mapValue.fields.itemCategory.stringValue,
                                itemPrice: menuItem.doc.data.value.mapValue.fields.itemPrice
                                  .doubleValue
                                  ? menuItem.doc.data.value.mapValue.fields.itemPrice.doubleValue
                                  : menuItem.doc.data.value.mapValue.fields.itemPrice.integerValue,
                              });
            ...
            <Card.Footer className="d-flex justify-content-between align-items-center">
              <p style={{ marginTop: 8, fontSize: 12, color: '#A1A1A1' }}>
                SaltNPepper Menu v1.0.0 - <a href="/login">Logout</a>
              </p>
            </Card.Footer>
          </Card>
        </>
