...
import type { DocumentData, QuerySnapshot, VectorValue } from '@google-cloud/firestore';
...
describe.skipClassic('Query and Pipeline Compare - Enterprise DB', () => {
  ...
  async function compareVectorQueryAndPipeline(query: VectorQuery): Promise<VectorQuerySnapshot> {
    const queryResults = await query.get();
  ...
  it('supports NaN and Null', () => {
    ...
    return ref
      ...
      .then(() => {
        return compareQueryAndPipeline(randomCol.where('foo', '==', NaN).where('bar', '==', null));
      })
      .then((res) => {
  ...
  it('supports array-contains', () => {
    return Promise.all([randomCol.add({ foo: ['bar'] }), randomCol.add({ foo: [] })])
      .then(() => compareQueryAndPipeline(randomCol.where('foo', 'array-contains', 'bar')))
      .then((res) => {
  ...
  it('supports findNearest limits', async () => {
    ...
    const res = await compareVectorQueryAndPipeline(vectorQuery);
    expect(res.size).to.equal(1);
    expect((res.docs[0].get('embedding') as VectorValue).toArray()).to.deep.equal(embeddingVector);
  });
  ...
  it.skipEnterprise('supports !=', async () => {
    ...
    let res = await compareQueryAndPipeline(randomCol.where('zip', '!=', 98101));
    expectDocs(
      res,
      { zip: NaN },
      { zip: 91102 },
  ...
  it('supports not-in', async () => {
    ...
    let res = await compareQueryAndPipeline(
      randomCol.where('zip', 'not-in', [98101, 98103]).orderBy(FieldPath.documentId()),
    );
    expectDocs(
      res,
      { zip: 91102 },
    ...
    res = await compareQueryAndPipeline(
      randomCol.where('zip', 'not-in', [NaN]).orderBy('zip').orderBy(FieldPath.documentId()),
    );
    expectDocs(
      res,
      { zip: 91102 },
  ...
  it.skipEnterprise('supports "in"', async () => {
    ...
    const res = await compareQueryAndPipeline(randomCol.where('zip', 'in', [98101, 98103]));
  ...
  it('can query by FieldPath.documentId()', () => {
    ...
    return ref
      .set({})
      .then(() => {
        return compareQueryAndPipeline(randomCol.where(FieldPath.documentId(), '>=', 'bar'));
      })
      .then((res) => {
  ...
  it('can order by FieldPath.documentId()', () => {
    ...
    return Promise.all([ref1.set({ foo: 'a' }), ref2.set({ foo: 'b' })])
      .then(() => {
        return compareQueryAndPipeline(randomCol.orderBy(FieldPath.documentId()));
      })
      .then((res) => {
  ...
  it('has limit() method on get()', async () => {
    await addDocs({ foo: 'a' }, { foo: 'b' });
    const res = await compareQueryAndPipeline(randomCol.orderBy('foo').limit(1));
  ...
  it('can run limit(num), where num is larger than the collection size on get()', async () => {
    await addDocs({ foo: 'a' }, { foo: 'b' });
    const res = await compareQueryAndPipeline(randomCol.orderBy('foo').limit(3));
    expectDocs(res, { foo: 'a' }, { foo: 'b' });
  });
  ...
  it('can use offset() method with get()', async () => {
    ...
    await addDocs({ foo: 'a' }, { foo: 'b' });
    const res = await compareQueryAndPipeline(randomCol.orderBy('foo').offset(1));
  ...
  it('can run offset(num), where num is larger than the collection size on get()', async () => {
    await addDocs({ foo: 'a' }, { foo: 'b' });
    const res = await compareQueryAndPipeline(randomCol.orderBy('foo').offset(3));
    expect(res.empty);
  });
  ...
  it.skipEnterprise('can query collection groups', async () => {
    ...
    const querySnapshot = await compareQueryAndPipeline(firestore.collectionGroup(collectionGroup));
  ...
  it.skipEnterprise('supports OR queries', async () => {
    ...
    expectDocs(
      await compareQueryAndPipeline(
        collection.where(Filter.or(Filter.where('a', '==', 1), Filter.where('b', '==', 1))),
      ),
      'doc1',
      'doc2',
      'doc4',
  ...
  it.skipEnterprise('supports OR queries with composite indexes', async () => {
    ...
    expectDocs(
      await compareQueryAndPipeline(
        collection.where(Filter.or(Filter.where('a', '>', 2), Filter.where('b', '==', 1))),
      ),
      'doc5',
      'doc2',
      'doc3',
    ...
    expectDocs(
      await compareQueryAndPipeline(
        collection.where(Filter.or(Filter.where('a', '==', 1), Filter.where('b', '>', 0))).limit(2),
      ),
      'doc1',
      'doc2',
    );
  ...
  it.skipEnterprise('supports OR queries with array membership', async () => {
    ...
    expectDocs(
      await compareQueryAndPipeline(
        collection.where(
          Filter.or(Filter.where('a', '==', 2), Filter.where('b', 'array-contains', 7)),
        ),
      ),
      'doc3',
      'doc4',
    ...
    () => {
      it('supports multiple inequality queries', async () => {
        ...
        let results = await compareQueryAndPipeline(
          collection.where('key', '!=', 'a').where('sort', '<=', 2).where('v', '>', 2),
        );
        expectDocs(results, 'doc3');

        // Duplicate inequality fields
        results = await compareQueryAndPipeline(
          collection.where('key', '!=', 'a').where('sort', '<=', 2).where('sort', '>', 1),
        );
        ...
        // With NOT-IN
        results = await compareQueryAndPipeline(
          collection.where('key', '>=', 'a').where('sort', '<=', 2).where('v', 'not-in', [2, 4, 5]),
        );
        expectDocs(results, 'doc1', 'doc3');

        // With orderby
        results = await compareQueryAndPipeline(
          collection.where('key', '>=', 'a').where('sort', '<=', 2).orderBy('v', 'desc'),
        );
        expectDocs(results, 'doc3', 'doc4', 'doc1');

        // With limit
        results = await compareQueryAndPipeline(
          collection.where('key', '>=', 'a').where('sort', '<=', 2).orderBy('v', 'desc').limit(2),
        );
      ...
      it('can use on special values', async () => {
        ...
        results = await compareQueryAndPipeline(
          collection.where('key', '!=', 'a').where('sort', '<=', 2).where('v', '<=', 1),
        );
      ...
      it('can use with array membership', async () => {
        ...
        let results = await compareQueryAndPipeline(
          collection.where('key', '!=', 'a').where('sort', '>=', 1).where('v', 'array-contains', 0),
        );
      ...
      it('can use with nested composite filters', async () => {
        ...
        let queryWithCursor = query.startAt(docSnap);
        expectDocs(await compareQueryAndPipeline(query), 'doc1', 'doc6', 'doc5', 'doc4');
        ...
        queryWithCursor = query.startAt(docSnap);
        expectDocs(await compareQueryAndPipeline(query), 'doc5', 'doc4', 'doc1', 'doc6');
      ...
      it('inequality fields will be implicitly ordered lexicographically by the server', async () => {
        ...
        let query = collection
          ...
          .where('v', 'in', [1, 2, 3, 4]);
        let queryWithCursor = query.startAt(docSnap);
        expectDocs(await compareQueryAndPipeline(query), 'doc2', 'doc4', 'doc5', 'doc3');
      ...
      it('can use multiple explicit order by field', async () => {
        ...
        let queryWithCursor = query.startAt(docSnap);
        expectDocs(await compareQueryAndPipeline(query), 'doc2', 'doc4', 'doc3', 'doc5');
        ...
        query = collection
          ...
          .orderBy('sort');
        queryWithCursor = query.startAt(docSnap);
        expectDocs(await compareQueryAndPipeline(query), 'doc2', 'doc5', 'doc4', 'doc3');
        ...
        queryWithCursor = query.startAt(docSnap);
        expectDocs(await compareQueryAndPipeline(query), 'doc5', 'doc3', 'doc4', 'doc2');
        ...
        query = collection
          ...
          .orderBy('sort');
        queryWithCursor = query.startAt(docSnap);
        expectDocs(await compareQueryAndPipeline(query), 'doc5', 'doc4', 'doc3', 'doc2');
