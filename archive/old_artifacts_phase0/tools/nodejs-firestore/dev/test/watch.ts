 ...
 */
function docsEqual(actual: QueryDocumentSnapshot[], expected: QueryDocumentSnapshot[]): void {
...
function snapshotsEqual(
  ...
  for (let i = 0; i < expected.docChanges.length; i++) {
    ...
    expect(actualDocChanges[i].doc.data()).to.deep.eq(expected.docChanges[i].doc.data());
    const readVersion = actualDocChanges[i].type === 'removed' ? version - 1 : version;
    expect(actualDocChanges[i].doc.readTime.isEqual(new Timestamp(0, readVersion))).to.be.true;
 ...
 */
function snapshot(ref: DocumentReference, data: DocumentData): QueryDocumentSnapshot {
...
const added = (ref: DocumentReference, data: DocumentData) => docChange('added', ref, data);
const modified = (ref: DocumentReference, data: DocumentData) => docChange('modified', ref, data);
const removed = (ref: DocumentReference, data: DocumentData) => docChange('removed', ref, data);
...
class WatchHelper<T = QuerySnapshot | DocumentSnapshot> {
   ...
   */
  runTest(expectedRequest: api.IListenRequest, func: () => Promise<unknown>): Promise<void> {
    this.startWatch();
...
describe('Query watch', () => {
  ...
  const resumeTokenQuery: (resumeToken: Buffer) => api.IListenRequest = (resumeToken: Buffer) => {
    return {
      database: `projects/${PROJECT_ID}/databases/(default)`,
      addTarget: {
        query: {
  ...
  it('handles invalid target change protos', () => {
    return watchHelper.runFailedTest(
      ...
      'Unknown target change type: {"targetChangeType":"INVALID",' + '"targetIds":[65261]}',
    );
  });
      ...
      return watchHelper
        ...
        .then((results) => {
          snapshotsEqual(lastSnapshot, 3, results, {
            ...
            docChanges: [modified(doc2, { foo: 'd' }), modified(doc1, { foo: 'c' })],
          });
        });
    });
  });
  ...
  describe('supports isEqual', () => {
    ...
    function initialSnapshot(watchTest: (initialSnapshot: QuerySnapshot) => Promise<void> | void) {
      return watchHelper.runTest(collQueryJSON(), () => {
        watchHelper.sendAddTarget();
        watchHelper.sendCurrent();
        watchHelper.sendSnapshot(++snapshotVersion);
        return watchHelper.await('snapshot').then((snapshot) => watchTest(snapshot));
      });
    }
    ...
    it('for snapshots with different kind of changes', () => {
      ...
      return initialSnapshot((snapshot) => {
        return nextSnapshot(snapshot, () => {
          ...
          firstSnapshot = snapshot;
          expect(snapshot.docChanges()[0].isEqual(firstSnapshot.docChanges()[0])).to.be.true;
        });
      }).then(() =>
        initialSnapshot((snapshot) => {
          return nextSnapshot(snapshot, () => {
            ...
            expect(snapshot.isEqual(firstSnapshot)).to.be.false;
            expect(snapshot.docChanges()[0].isEqual(firstSnapshot.docChanges()[0])).to.be.false;
          });
        }),
      );
    });
    ...
    it('for snapshots with different data types', () => {
      ...
      return initialSnapshot((snapshot) => {
        return nextSnapshot(snapshot, () => watchHelper.sendDoc(doc1, { foo: '1' })).then(
          (snapshot) => {
    ...
    it('for objects with different type', () => {
      return initialSnapshot((snapshot) => {
        ...
        expect(snapshot.isEqual(new GeoPoint(0, 0) as InvalidApiUsage)).to.be.false;
      });
    });
  });
