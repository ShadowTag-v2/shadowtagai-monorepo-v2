...
function renderGoogleSignInButton(toolId, clientId, authProfileName) {
  ...
  const setupGisBtn = document.querySelector(
    `#google-auth-details-${UNIQUE_ID_BASE} .btn--setup-gis`,
  );
  ...
  if (window.google && window.google.accounts && window.google.accounts.id) {
    try {
      const handleResponse = (response) =>
        handleCredentialResponse(response, toolId, authProfileName);
...
function handleCredentialResponse(response, toolId, authProfileName) {
  ...
  const UNIQUE_ID_BASE = `${toolId}-${authProfileName}`;
  const setupGisBtn = document.querySelector(
    `#google-auth-details-${UNIQUE_ID_BASE} .setup-gis-btn`,
  );
