...
export async function main() {
  ...
  const conversationHistory = [{ role: 'system', content: [{ text: systemPrompt }] }];
      ...
      const toolResponses = await Promise.all(
        toolRequests.map(async (call) => {
          try {
            ...
            return {
              role: 'tool',
              content: [{ toolResponse: { name: call.name, output: toolOutput } }],
            };
          } catch (e) {
            console.error(`Error executing tool ${call.name}:`, e);
            return {
              role: 'tool',
              content: [{ toolResponse: { name: call.name, output: { error: e.message } } }],
            };
          }
        }),
      );
