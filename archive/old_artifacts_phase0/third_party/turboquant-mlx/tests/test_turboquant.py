...
class TestWHT:
    ...
    def test_hadamard_preserves_norm(self):
        ...
        assert abs(original_norm - transformed_norm) < 1e-5, f"Norm changed: {original_norm} -> {transformed_norm}"
    ...
    def test_wht_rotation_orthogonal(self):
        ...
        assert abs(original_dot - rotated_dot) < 1e-4, f"Inner product changed: {original_dot} -> {rotated_dot}"
...
class TestOllamaIntegration:
    ...
    def test_patch_ollama_env(self):
        ...
        # Save original env
        original_env = {k: os.environ.get(k) for k in ["OLLAMA_NUM_PARALLEL", "OLLAMA_FLASH_ATTENTION", "OLLAMA_KEEP_ALIVE", "OLLAMA_NUM_CTX"]}

        try:
            env = patch_ollama_env(num_parallel=8, num_ctx=16384, flash_attention=True, keep_alive="12h")
...
class TestGroveIntegration:
    ...
    def test_sparse_kv_delta_topk_sparsity(self):
        ...
        assert abs(actual_ratio - expected_ratio) < 0.02, f"Sparsity ratio {actual_ratio} != expected {expected_ratio}"
