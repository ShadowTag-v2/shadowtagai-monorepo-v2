...
## Objective
Upgrade the current workspace to fully implement the "Antigravity Workspace Template" standards, focusing on the "Autoresearch" agent swarm and "Judge #6" governance engine.
...
**Goal**: Connect `src/agent.py` to the `Autoresearch` swarm logic.

- [ ] Update `src/agent.py` to import `Autoresearch` from `agents/autoresearch.py`.
