...
def encode_video(
  ...
  bits_per_channel: int = typer.Option(
    2, "--bits", "-b", help="LSBs per channel (1-4)"
  ),
  encrypt: bool = typer.Option(True, "--encrypt/--no-encrypt", help="Encrypt payload"),
  error_correction: bool = typer.Option(
    True, "--error-correction/--no-error-correction", help="Enable error correction"
  ),
  create_receipt: bool = typer.Option(
    True, "--receipt/--no-receipt", help="Create receipt chain entry"
  ),
):
  """
...
def _create_encode_receipt(
  media_type: str, input_file: Path, payload: bytes, stats: dict, config: object
):
  ...
  receipt = Receipt(
    operation_id=hashlib.sha256(
      f"{datetime.now(timezone.utc).isoformat()}_{media_hash}".encode()
    ).hexdigest()[:16],
