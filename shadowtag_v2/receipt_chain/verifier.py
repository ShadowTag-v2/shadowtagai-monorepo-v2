...
class ChainVerifier:
  ...
  def _verify_cryptography(
    self, chain: ReceiptChain, result: VerificationResult
  ) -> None:
  ...
  def _verify_temporal(self, chain: ReceiptChain, result: VerificationResult) -> None:
    ...
    for i in range(1, len(chain.blocks)):
      ...
      if time_gap > 365:
        result.add_warning(
          f"Block {i} has large time gap ({time_gap} days) from previous block"
        )
  ...
  def verify_receipt_pair(
    self, encode_receipt: Receipt, decode_receipt: Receipt
  ) -> tuple[bool, list[str]]:
    """
