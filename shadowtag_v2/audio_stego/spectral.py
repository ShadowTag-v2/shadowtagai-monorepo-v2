...
class SpectralProcessor:
  ...
  def get_magnitude_phase(
    self, stft_matrix: np.ndarray
  ...
  def apply_spectral_mask(
    self, stft_matrix: np.ndarray, mask: np.ndarray
  ) -> np.ndarray:
    """
  ...
  def find_perceptual_bins(
    self, magnitude: np.ndarray, threshold: float = 0.1
  ) -> np.ndarray:
    """
