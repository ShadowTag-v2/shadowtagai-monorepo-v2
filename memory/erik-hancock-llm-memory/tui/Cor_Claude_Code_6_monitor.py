...
"""
Claude_Code_6 Governance Enforcement Monitor
Inspired by Bottom's architecture: https://github.com/ClementTsang/bottom

PATTERN MAPPINGS (Bottom → Claude_Code_6):
- Bottom's CPU sparkline → Claude_Code_6 decision latency sparkline
- Bottom's process table → Violation events table
- Bottom's update loop → Enforcement decision loop
- Bottom's p99 <50ms target → Claude_Code_6 p99 ≤90ms SLA

USAGE:
    python3 tui/Cor_Claude_Code_6_monitor.py --mode=mock    # Simulated decisions
    python3 tui/Cor_Claude_Code_6_monitor.py --mode=live    # Connect to real Claude_Code_6
"""
...
class LatencyStats:
  """Bottom-style latency tracking (Cor_Claude_Code_6/runtime/profiling.py + enhancements)"""
...
class MockCor_Claude_Code_6Engine:
  """Simulated Claude_Code_6 enforcement engine for latency testing
...
class DecisionRateWidget(Static):
  ...
  def __init__(
    self, engine: MockCor_Claude_Code_6Engine, stats: LatencyStats, **kwargs
  ):
    super().__init__(**kwargs)
    self.engine = engine
    self.stats = stats
...
class Cor_Claude_Code_6MonitorApp(App):
  """Claude_Code_6 Governance Enforcement Monitor (Bottom-inspired architecture)"""
  ...
  def __init__(self, mode: str = "mock", **kwargs):
    ...
    if mode == "live":
      try:
        from tui.Cor_Claude_Code_6_engine_live import RealCor_Claude_Code_6Engine

        self.engine = RealCor_Claude_Code_6Engine()
        self.title = "Claude_Code_6 Monitor (LIVE)"
      except ImportError as e:
        self.notify(f"Failed to load live engine: {e}", severity="error")
        self.engine = MockCor_Claude_Code_6Engine()
        self.mode = "mock"
        self.title = "Claude_Code_6 Monitor (Mock - Live Failed)"
    elif mode == "edgequeue":
      try:
        from tui.Cor_Claude_Code_6_engine_edgequeue import EdgeQueueEngine

        self.engine = EdgeQueueEngine()
        self.title = "Claude_Code_6 Monitor (EdgeQueue)"
      except ImportError as e:
        self.notify(f"Failed to load EdgeQueue engine: {e}", severity="error")
        self.engine = MockCor_Claude_Code_6Engine()
        self.mode = "mock"
        self.title = "Claude_Code_6 Monitor (Mock - EdgeQueue Failed)"
    else:
      self.engine = MockCor_Claude_Code_6Engine()
      self.title = "Claude_Code_6 Monitor (Mock)"
...
if __name__ == "__main__":
  ...
  app = Cor_Claude_Code_6MonitorApp(mode=mode)
  app.run()
