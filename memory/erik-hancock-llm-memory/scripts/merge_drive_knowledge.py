...
def load_extracted_documents() -> list[dict]:
  ...
  for metadata_file in metadata_dir.glob("*.json"):
    ...
    documents.append(
      {"hash": file_hash, "metadata": metadata, "text": text, "embedding": embedding}
    )
...
def save_updated_memory(memory: dict):
  ...
  snapshot_file = (
    snapshots_dir
    / f"memory_drive_merge_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
  )
...
def generate_statistics(memory: dict):
  ...
  print(
    f"   With embeddings: {with_embeddings} ({with_embeddings / len(knowledge) * 100:.1f}%)"
  )
