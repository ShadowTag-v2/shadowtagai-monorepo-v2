...
def parse_web_conversations(extractions: list[dict]) -> list[dict]:
    ...
    for extraction in extractions:
        ...
        if sources.get("localStorage", {}).get("data"):
            ls_convos = parse_localstorage_conversations(sources["localStorage"]["data"], platform)
...
def parse_indexeddb_conversations(idb_data: dict, platform: str) -> list[dict]:
    ...
    for store_name, records in idb_data.items():
        ...
        for record in records:
            ...
            convo_id = record.get("id") or record.get("conversationId") or record.get("uuid") or generate_id(str(record))
...
def parse_localstorage_conversations(ls_data: dict, platform: str) -> list[dict]:
    ...
    for key, value in ls_data.items():
        ...
        if has_messages:
            ...
            messages = []
            msg_data = value.get("messages", value.get("conversation", {}).get("messages", []))
...
def save_updated_memory(memory: dict):
    ...
    snapshot_file = snapshots_dir / f"memory_web_merge_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    # 4. Merge
    memory["conversations"] = merge_conversations(memory.get("conversations", []), new_conversations)
