...
### 4.4 600-Agent Swarm Scaling (FlyingMonkeys Enhancement)

**Current State:** FlyingMonkeys running on :8600 (570 Flash + 30 Pro)
...
| File | Changes |
|------|---------|
| `agents/flying_monkeys.py` | Add CrewAI task delegation |
| `lib/flyingmonkeys-swarm.js` | Enhanced voting engine |
...
class LongShortRouter:
    ...
    def __init__(self):
        self.short_model = "gemini-1.5-flash"  # Fast, cheap
...
| File | Changes |
|------|---------|
| `k8s/aiyoujr-governance/Claude_Code_6-enforcer.yaml` | Right-size CPU/Memory |
...
### Files to Modify
- `k8s/aiyoujr-governance/Claude_Code_6-enforcer.yaml` - Right-size
...
1. **FlyingMonkeys Upgraded to Gemini 2.5**
...
**FlyingMonkeys as Default Planner:**
...
║  ✅ FastGPT/GPTRAM (src/aiyou/services/fastgpt_client.py)                    ║
...
╠═══════════════════════════════════════════════════════════════════════════════╣
║ Multi-Model Architecture with FlyingMonkeys Orchestration                     ║
║                                                                               ║
...
6. **FlyingMonkeys Integration** - Connect to 600-agent swarm
...
1. **Training Job Dashboard** - Rich terminal UI for GPU cluster job submission (@ramith__ pattern)
...
| 11 | **Beware MCP** | FlyingMonkeys whitelisted, JURA approval gate |
...
- agents/ - FlyingMonkeys swarm (600 agents)
...
```bash
/compact "Preserve: FlyingMonkeys on :8600, Phase 11 implementation,
...
*Phase 12 Added: Gemini CLI as Claude Code's Subordinate (@ImSh4yy Pattern)*
*Best Practices: Integrated @wquguru's 12 Gemini CLI guidelines*
*Pro Tip: Custom /compact instructions for context preservation*
