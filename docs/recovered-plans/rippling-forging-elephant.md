...
|-----------|--------|---------|
| Runner Name | `sovereign` | Registered to ehanc69/aiyou-fastapi-services |
...
gh api repos/ehanc69/aiyou-fastapi-services/actions/runners --jq '.runners[]'
```

---

### 2. Flying Monkeys (Cloud Run)

| Component | Status | Details |
|-----------|--------|---------|
| Service | `flyingmonkeys-server` | 600-agent swarm |
| URL | `https://flyingmonkeys-server-215390634092.us-central1.run.app` | Production endpoint |
| Image | `us-central1-docker.pkg.dev/acquired-jet-478701-b3/flyingmonkeys/server:gemini25flash` | Artifact Registry |
...
curl -H "Authorization: Bearer $(gcloud auth print-identity-token)" \
  https://flyingmonkeys-server-215390634092.us-central1.run.app/health
```
...
│  │ (sovereign)      │──triggers───▶│ flyingmonkeys-server     │         │
│  │ v2.329.0         │              │ 600 agents               │         │
│  │ $0/month         │              │ Scale 1-10               │         │
│  └──────────────────┘              └──────────────────────────┘         │
│           │                                   │                          │
...
- Compliance Framework (Risk Management) → Governance (Judge 6)
- TC 18-20 (SSE) → Data Harvesting

### Deliverable 2: BCTL Template ✅
- Executive Summary (RED/AMBER/GREEN readiness)
- Methodology (Compliance Framework + ICTL attribution)
...
- **SOP-C:** Governance (Compliance Framework 5-Step Loop + Protocol 2511)
...
DOCTRINE REF: ADP 7-0, Compliance Framework, ADP 3-90
...
- Compliance Framework (Risk Management): 5-step composite risk management
...
**D. RISK ASSESSMENT (Compliance Framework):**
- Hazard: [Threat]
...
## 5. RISK MATRIX (Compliance Framework)
...
## 1. MISSION STATEMENT
This repository houses the FlyingMonkeys RSTA Squadron, a sovereign, event-driven AI swarm designed to identify revenue gaps, build solutions, and enforce "Ranger Standards" on code quality.
...
## Phase 6: Integrate Doctrine into Judge & FlyingMonkeys ✅ COMPLETE
...
| `pnkln/governance/judge_architecture.py` | ✅ | Added doctrine imports, RiskLevel mapping methods, Layer 0 Compliance Framework CRM, Layer 0.5 FM 6-0 MDMP |
| `agents/flying_monkeys.py` | ✅ | Added TLPPipeline integration, risk-based consensus thresholds in PRTP, `prtp_with_battle_drills()` |
...
| `voice_consensus/consensus_orchestrator.py` | ✅ | Added Compliance Framework thresholds, `assess_risk_and_set_threshold()`, `check_consensus_reached()`, `execute_full_consensus_with_doctrine()` |

### Integration Overview

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    DOCTRINE → JUDGE → FLYINGMONKEYS                      │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
...
#### Step 3: FlyingMonkeys TLP Integration
...
| FM 6-0 TLP 8-Step | FlyingMonkeys | Rapid audit cycles |
...
2. **Integration Tests**: Verify Judge → FlyingMonkeys flow with doctrine
...
### Current Status
- **Image Built**: `us-central1-docker.pkg.dev/acquired-jet-478701-b3/flyingmonkeys/server:gemini25flash` ✅
- **Region**: `us-central1`
...
gcloud run deploy flyingmonkeys-server \
  --image us-central1-docker.pkg.dev/acquired-jet-478701-b3/flyingmonkeys/server:gemini25flash \
  --region us-central1 \
  --project acquired-jet-478701-b3 \
  --platform managed \
...
```bash
# Get service URL
gcloud run services describe flyingmonkeys-server \
  --region us-central1 \
  --project acquired-jet-478701-b3 \
  --format 'value(status.url)'

# Test health endpoint
curl $(gcloud run services describe flyingmonkeys-server --region us-central1 --format 'value(status.url)')/health
```

### Expected Outcome
- Cloud Run service at `https://flyingmonkeys-server-XXXXX-uc.a.run.app`
