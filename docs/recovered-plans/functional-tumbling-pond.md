...
| **FlyingMonkeys Server** | ✅ Full combat posture | `bin/flyingmonkeys-server` |
...
### Location
```
/Users/pikeymickey/aiyou-fastapi-services/erik-hancock-llm-memory/
```
...
agents/flying_monkeys.py              # 600 agents, full combat posture
agents/cavalry_squadron.py            # Troop structure
agents/gemini_executor.py             # Real Gemini API calls
bin/flyingmonkeys-server              # HTTP server :8600
```

### CLAUDE.md (Project Memory)
```
/Users/pikeymickey/Documents/Claude Code/Code/Claude Demo/aiyou-fastapi-services/CLAUDE.md
```
...
| File | Action | Purpose |
|------|--------|---------|
| `agents/flying_monkeys.py` | MODIFY | Remove shift rotation |
...
| `bin/flyingmonkeys-server` | MODIFY | Remove shift endpoints, add skill loader |
...
- Server integration (`bin/flyingmonkeys-server`)

**Phase 2 Deliverables**:
1. Antigravity system prompt file
2. METT-TC integration for dynamic prompts
3. Wire prompts into FlyingMonkeys execution
...
| `bin/flyingmonkeys-server` | Wire prompts into `/mission` endpoint |
...
## CAVALRY SQUADRON "FLYINGMONKEYS" (600 agents)
...
class OPORDGenerator:
    """
    Generates OPORDs for FlyingMonkeys Cavalry Squadron.
...
```python
# Proposed mapping in bin/flyingmonkeys-server
...
1. **`bin/flyingmonkeys-server`** - Add battalion structure
2. **`agents/flying_monkeys.py`** - Add company/platoon routing
3. **`src/aiyou/jura/limiter.py`** - Map companies to JURA tiers
4. **`.claude/docs/battalion-structure.md`** - New doctrine document
5. **`prompts/flyingmonkeys_universal.txt`** - Add OPORD awareness

---

## Automation: Display "🐒 MONKEYS THINKING" Status

### Option 1: Add to CLAUDE.md (Recommended)

```markdown
## FlyingMonkeys Integration

When processing tasks, always display status:
- "🐒 MONKEYS THINKING" when sending to FlyingMonkeys
- Include: tier, agents assigned, model, latency
```

### Option 2: Hook in `.claude/hooks/`

Create `.claude/hooks/flyingmonkeys-status.sh` to inject status automatically.
...
Currently active in:
- `bin/flyingmonkeys-server` (BULK + GOVERNANCE tiers)
- `src/aiyou/services/gemini_core.py` (GeminiAntigravity with fallback)
...
3. gemini-3.1-flash (SUPEREGO tier - guaranteed)
...
| `bin/flyingmonkeys-server` | Add squadron routing, status endpoint |
| `agents/flying_monkeys.py` | Integrate squadron dispatch |
| `src/aiyou/jura/limiter.py` | Map vehicle types to JURA tiers |
