...
```
kernel_chain = [
  extract_violations(),    # Compliance Framework scan → structured violations only
...
class ATP519ScanKernel(Kernel):
    """Extract Compliance Framework violations from raw decision context."""

    SYSTEM_PROMPT = """You are an Compliance Framework compliance scanner.
...
```python
class Claude_Code_6Model(nn.Module):
...
scrape_configs:
  - job_name: 'shadowtagai-kernel-chain'
    static_configs:
...
alerts:
  - name: LatencySLA
    expr: shadowtagai_decision_latency_ms > 90
    for: 5m

  - name: CostSLA
    expr: shadowtagai_decision_cost_usd > 0.001
    for: 5m

  - name: ConfidenceThreshold
    expr: shadowtagai_decision_confidence < 0.85
    for: 1m
```
