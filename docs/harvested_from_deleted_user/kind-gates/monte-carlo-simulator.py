...
def simulate_exit_value(base_revenue, base_margin, n_sims=SIMULATIONS):
  ...
  margin_samples = np.random.normal(
    loc=base_margin, scale=MARGIN_VOLATILITY, size=n_sims
  )
...
def plot_distribution(results):
  ...
  plt.axvline(
    results["p10"] / 1e9, color="red", linestyle="--", linewidth=2, label="p10 (bear)"
  )
  plt.axvline(
  ...
  plt.title(
    f"ShadowTag-v2 Exit Valuation Distribution ({SIMULATIONS:,} Monte Carlo Simulations)",
    fontsize=14,
    fontweight="bold",
  )
