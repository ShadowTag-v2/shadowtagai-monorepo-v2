...
class MultiModelRouter:
  ...
  def cost_comparison(self) -> Dict[str, Any]:
    ...
    if_claude_cost = (
      self.gemini_tokens.total_tokens / 1_000_000
    ) * 15.0  # If used Claude
...
class FlyingMonkeysV8:
  ...
  def _call_opus(
    self, prompt: str, system: str = None, tools: List[Dict] = None
  ) -> str:
...
def main():
  ...
  if choice == "1":
    ...
    topic = (
      input("Topic to brainstorm: ").strip() or "Ways to monetize AI agent platform"
    )
    fm.brainstorm(topic)
    print(f"\n📊 {fm.tokens.display()}")
