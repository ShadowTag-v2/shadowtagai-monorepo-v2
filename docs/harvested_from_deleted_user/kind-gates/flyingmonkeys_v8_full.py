...
"""
FlyingMonkeys v8 FULL - Claude Code Integration
Research + Governance + Advanced Tool Use

Features:
- Web Search (research capabilities)
- Claude_Code_6 Governance (Purpose/Reasons/Brakes)
...
class Claude_Code_6:
    """
    Claude_Code_6 Governance - Purpose/Reasons/Brakes validation
    """

    DANGEROUS_KEYWORDS = {
        'delete', 'remove', 'drop', 'destroy', 'kill', 'terminate',
        'admin', 'root', 'sudo', 'exec', 'eval', 'system', 'rm -rf'
    }
...
class ToolRegistry:
    ...
    def search(self, query: str, limit: int = 5) -> List[ToolDef]:
        query = query.lower()
        return [t for n, t in self._tools.items() if query in n.lower() or query in t.description.lower()][:limit]
...
class SandboxExecutor:
    ...
    def execute(self, code: str) -> Dict[str, Any]:
        ...
        safe_builtins = {n: getattr(builtins, n) for n in self.ALLOWED_BUILTINS if hasattr(builtins, n)}
        ...
        try:
            ...
            return {"output": stdout.getvalue(), "results": context['results'], "log": self.log, "error": None}
...
class Monkey:
    ...
    latency_ms: float = 0
    approved: bool = False
    governance: Optional[JRValidation] = None


class FlyingMonkeysV8Full:
    """
    FlyingMonkeys v8 FULL - Integrated Research + Governance
    """

    def __init__(self, api_key: Optional[str] = None, model: str = MODEL):
        self.api_key = api_key or os.getenv("ANTHROPIC_API_KEY")
        self.model = model
        self.client = anthropic.Anthropic(api_key=self.api_key) if self.api_key and anthropic else None
    ...
    def _register_tools(self):
        """Register all tools"""
        # Research tools
        self.registry.register(ToolDef("web_search", "Search the web", self.research.web_search, "research"))
        self.registry.register(ToolDef("market_analysis", "Analyze market", self.research.analyze_market, "research"))
        self.registry.register(ToolDef("competitive_intel", "Get competitor intel", self.research.competitive_intel, "research"))

        # Math tools
        self.registry.register(ToolDef("calculator", "Calculate expression", lambda expr: eval(expr, {"__builtins__": {}}, {"math": __import__('math')}), "math"))
        self.registry.register(ToolDef("fibonacci", "Fibonacci number", lambda n: (lambda f: f(f, n))(lambda s, x: x if x <= 1 else s(s, x-1) + s(s, x-2)), "math"))

    def _call_llm(self, prompt: str, system: str = None) -> Tuple[str, Dict]:
        """Call LLM with token tracking"""
        if not self.client:
            return '{"mock": true, "recommendation": "Mock response", "score": 75}', {"input": 100, "output": 50}
        ...
        return response.content[0].text, {"input": response.usage.input_tokens, "output": response.usage.output_tokens}

    def execute_with_governance(self, action: str, args: Dict, context: str = "") -> Dict:
        """Execute action with Claude_Code_6 governance"""
    ...
    def research_task(self, query: str) -> Dict:
        ...
        return {
            ...
            "governance": {"approved": True, "scores": {
        ...
        for action, args in tasks:
            ...
            if validation.result == ValidationResult.APPROVED:
                if action in ["research", "market_analysis", "competitive_intel"]:
                    result = self.registry.execute(action.replace("_", " ").split()[0] + "_" + action.split("_")[-1] if "_" in action else action, **args)
                else:
                    result = {"simulated": True, "action": action}
                results.append({"action": action, "result": result, "approved": True})
            else:
                results.append({"action": action, "blocked": True, "reason": validation.explanation})

        print("="*60)
        governance_status = self.judge.get_status()
        print(f"⚖️  Governance: {governance_status['approved']}/{governance_status['total_validations']} approved ({governance_status['approval_rate']:.0f}%)")
        print(f"📊 {self.tokens.display()}")
    ...
    def execute_code(self, code: str) -> Dict:
        ...
        validation = self.judge.validate("code_execution", {"code": code[:100]}, "programmatic tool calling")
...
def run_in_claude_code():
    ...
    This runs the full FlyingMonkeys v8 stack.
    """
    print("""
    ╔═══════════════════════════════════════════════════════════╗
    ║         🐵 FLYING MONKEYS v8 FULL 🐵                      ║
