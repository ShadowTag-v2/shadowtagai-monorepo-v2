...
class Claude_Code_6:
  ...
  def validate(
    self, action: str, args: Dict[str, Any], context: str = ""
  ) -> JRValidation:
  ...
  def _check_purpose(self, action: str, args: Dict, context: str) -> float:
    ...
    if any(
      w in action.lower() for w in ["research", "analyze", "search", "find", "hunt"]
    ):
  ...
  def get_status(self) -> Dict:
    ...
    return {
      ...
      "approved": approved,
      "blocked": blocked,
      "approval_rate": (approved / len(self.audit_log) * 100)
      if self.audit_log
      else 100,
    }
...
class FlyingMonkeysV8Full:
  ...
  def _register_tools(self):
    ...
    self.registry.register(
      ToolDef(
        "market_analysis", "Analyze market", self.research.analyze_market, "research"
      )
    )
    self.registry.register(
      ToolDef(
