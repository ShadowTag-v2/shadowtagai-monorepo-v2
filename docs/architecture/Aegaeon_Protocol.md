...
1. The Pointer: Instead of packing the system prompt and the Monorepo state into each of the 7 requests, the router attaches the exact same Context Cache ID to all 7 payloads.
1. The Payload: The only unique data sent to each instance is the diff or the specific user query (typically < 1000 tokens).

1. Disaggregating Decode (Flash-Tiering)
...
2. Disaggregating Prefill: The MLX / Metal KV-Cache Slab
   ...
   The Translation:
3. The Slab: We run a one-time prompt evaluation of the Sovereign Doctrine and .beads using llama-server or an MLX script.
4. The Cache: We export the resultant activation states as a physical .bin or .safetensors KV-cache file. This file sits in Unified Memory.

5. Disaggregating Decode: The "Pickle Rick" ANE Bridge
...
6. Model Resident: The LLM weights (e.g., Llama-3-8B-Instruct.gguf) are loaded into Unified Memory EXACTLY ONCE.
7. The Swarm Dispatch: When a GitHub PR drops, ane_bridge.py spawns 7 lightweight asynchronous completion requests.
8. The Pointer (The Magic): Every single request passes the argument --prompt-cache /path/to/shared/beads_cache.bin.
9. Hardware Execution: The Metal GPU skips the prefill phase entirely for all 7 requests. It only computes the delta (the PR diff) because the unified memory pointer instantly provides the pre-computed K and V tensors.
