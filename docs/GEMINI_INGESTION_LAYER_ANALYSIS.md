...
This document specifies the Gemini Ingestion Layer Analysis Prompt, adapted from the Judge #6 validation framework for intelligence collection pipeline analysis. The ingestion layer operates as a proactive collector feeding the PNKLN Core Stack™, contrasting with Judge #6's reactive enforcement role.
...
**PNKLN Stack Impact**: Trust-building for entire platform, reduces litigation risk
...
containers:
  - name: crawler-orchestrator
    image: pnkln/crawler-orchestrator:v2.1
    resources:
      requests: {cpu: "500m", memory: "1Gi"}
      limits: {cpu: "2", memory: "4Gi"}

  - name: gemini-classifier
    image: pnkln/gemini-classifier:v1.3
    resources:
      requests: {cpu: "1", memory: "2Gi"}
      limits: {cpu: "4", memory: "8Gi"}

  - name: storage-writer
    image: pnkln/storage-writer:v1.0
    resources:
...
gcs_buckets:
  - ${PROJECT_ID}-pnkln-raw-intel/        # Unprocessed artifacts
  - ${PROJECT_ID}-pnkln-classified-intel/ # Tier-sorted outputs
...
**Definition**: Semantic alignment between ingested content and PNKLN vertical priorities
...
- Grafana dashboard: `PNKLN Ingestion Health` (15-panel overview)
...
- Formal approval from PNKLN Core Stack™ governance board
...
```markdown
You are analyzing the PNKLN Gemini Ingestion Layer, a pre-production intelligence
...
**Related Docs**: `PNKLN_THREAD_ROLLUP_COMPREHENSIVE.md`, `judge_six_analysis_prompt.md`
