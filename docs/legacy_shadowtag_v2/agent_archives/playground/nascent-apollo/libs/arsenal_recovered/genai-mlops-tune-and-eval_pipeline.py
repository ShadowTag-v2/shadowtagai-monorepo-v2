...
def model_comparison_component(
    ...
    def pairwise_greater(
        ...
        choice = baseline if result["pairwise_question_answering_quality/pairwise_choice"] == "BASELINE" else candidate
    ...
    def pointwise_eval(
        ...
        eval_task = EvalTask(dataset=eval_dataset, metrics=eval_metrics, experiment=experiment_name)
        results = eval_task.evaluate(
            prompt_template="{instruction} \n {context}",
    ...
    def rank_responses(instruction: str, context: str, responses: list[str]) -> tuple:
        ...
        """
        cmp_f = partial(pairwise_greater, instruction, context, project, location, experiment_name)
        ...
        qa_metrics = pointwise_metric.metrics_table[
            [col for col in pointwise_metric.metrics_table.columns if ("question_answering" in col) or ("groundedness" in col)]
        ].to_dict("records")[0]
...
def gemini_tuning_pipeline(
    ...
    source_model_name: str = "gemini-3.1-flash",
    ...
    baseline_model_endpoint: str = "gemini-3.1-flash",
