...
def display_projects() -> None:
    ...
    Returns:
        None
    """
    st.session_state.product_category = st.selectbox("Select a project", st.session_state.product_categories)
...
def initialize_all_session_state(reinitialize: bool = False) -> None:
    ...
    for key, value in session_state_defaults.items():
        if (reinitialize is False and key not in st.session_state) or reinitialize is True:
...
def page_setup(page_cfg: dict) -> None:
    ...
    if "initialize_session_state" not in st.session_state or st.session_state.initialize_session_state is False:
