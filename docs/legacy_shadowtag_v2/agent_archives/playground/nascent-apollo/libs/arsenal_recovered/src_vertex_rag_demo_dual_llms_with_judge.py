...
DEFAULT_VERTEX_SEARCH_MODEL = "gemini-3.1-flash-001"
...
JUDGE_MODEL_NAME = "gemini-3.1-pro-001"  # Added constant
...
def create_gemini_client() -> genai.Client:
    ...
    if os.environ.get("GOOGLE_VERTEXAI", "").lower() == "true":
        ...
        if not api_key:
            raise ValueError("Gemini API requires GOOGLE_API_KEY environment variable set.")
...
def get_rephraser_prompt(query: str) -> str | None:
    """
...
def get_summarizer_prompt(documents: list[str], query: str) -> str | None:
    """
...
def initialize_models() -> tuple[list[str], list[str]]:
    ...
    try:
        ...
        gemini_models = [model.name.split("/")[-1] for model in response.page if any(prefix in model.name for prefix in GEMINI_MODEL_PREFIXES)]
...
def generate_gemini_response(prompt: str, model_config: dict[str, Any], col: st.container) -> str:
    """
...
def generate_ollama_response(prompt: str, model_config: dict[str, Any], col: st.container) -> str:
    """
...
def generate_rephraser_response(model_config: dict[str, Any], question: str, col: st.container) -> str:
    """
    ...
    with column:
        ...
        if model_type == "3P_Models":
            ...
            if not available_models:
                ...
                model_name = "gemini-3.1-family"  # Fallback default
            else:
                default_index = 0
                try:
                    # Try to find a reasonable default like flash
                    default_index = available_models.index("gemini-3.1-family")
                except ValueError:
...
def _get_discoveryengine_client_options(location: str) -> ClientOptions | None:
...
def list_data_stores(project_id: str, location: str = DEFAULT_DATA_STORE_LOCATION) -> list[str]:
    """
    ...
    try:
        ...
        parent_path = client.collection_path(project_id, location, collection=DEFAULT_COLLECTION)
...
def setup_retriever_sidebar() -> VertexAI | None:
    """
    ...
    with st.sidebar:
        ...
        if "datastore_ids" not in st.session_state:
            st.session_state.datastore_ids = list_data_stores(project_id, DEFAULT_DATA_STORE_LOCATION)
        ...
        if st.button("Test Connection"):
            if data_store_id:
                docs = list_documents(project_id, DEFAULT_DATA_STORE_LOCATION, data_store_id)
...
def main(args: argparse.Namespace):  # <-- Pass parsed args to main
    ...
    if user_question:
        ...
        left_response_data = generate_summarizer_response(left_config, left_final_query, left_context, left_col)
        right_response_data = generate_summarizer_response(right_config, right_final_query, right_context, right_col)
...
if __name__ == "__main__":
    # --- Argument Parsing ---
    parser = argparse.ArgumentParser(description="Vertex AI RAG Comparator with optional Judge Model.")
    parser.add_argument(
        "--judge",
