...
def create_db(
    ...
    database_id: str = typer.Option(app_settings.google_cloud.firestore_db_name, help="The ID of the Firestore database."),
):
    """
    ...
    try:
        ...
        try:
            ...
            console.log(f"[green]Successfully created Firestore database '{database_id}' in location '{location}'.[/green]")
            db = firestore.Client(project=project_id, credentials=credentials, database=database_id)
        except Exception as e:
...
@app.command()
def update_field(
    field_path: str = typer.Argument(..., help="Dot-separated path to the field to update (e.g., 'clientDetails.name.firstName')."),
