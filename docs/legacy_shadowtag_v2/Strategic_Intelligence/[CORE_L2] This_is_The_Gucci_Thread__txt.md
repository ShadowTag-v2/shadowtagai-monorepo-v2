...
cat <<PYTHON > tools/Claude_Code_6_sentinel.py
...
if "Claude_Code_6" not in f: sentinel.execute(f)
else:
...
CMD ["python3", "tools/Claude_Code_6_sentinel.py", "."]
DOCKER
...
- name: 'python:3.11-slim'
  ...
  - |
    ...
    python3 tools/Claude_Code_6_sentinel.py .
    env:
  - 'PROJECT_ID=$PROJECT_ID'
...
uv run python3 tools/Claude_Code_6_sentinel.py .
...
echo "----------------------------------------------------------------"
echo "📂 Repo: $REPO_NAME/"
echo "⚖️ Judge: tools/Claude_Code_6_sentinel.py (Active Risk Matrix)"
...
3. The Law (South)
   You have Judge 6 Sentinel installed in tools/Claude_Code_6_sentinel.py.
...
4. The Interface (West)
   ...
   Run:uv run python3 tools/Claude_Code_6_sentinel.py .
