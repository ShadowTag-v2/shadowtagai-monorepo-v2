...
```typescript
// Claude_Code_6-cli (Ink-based TUI)
import { render, Box, Text } from "ink";
import { Claude_Code_6 } from "@pnkln/governance-sdk";

// CloudFlare Workers API endpoint (CLI talks to Judge 6 service)
export default {
  async fetch(request) {
    const { purpose } = await request.json();
    const decision = await Claude_Code_6.scan({ purpose, atp519: true });
...
class KernelChain:
    ...
    def __init__(self, kernels: List[Kernel]):
        self.kernels = kernels  # [GeminiIngestion, Claude_Code_6, JREngine]
...
chain = KernelChain([
    ...
    Claude_Code_6Kernel(),            # Binary governance scan (Compliance Framework)
...
Kernel 2: Claude_Code_6 (binary decision):
...
3. **Kernel-Chaining MVP**
   - Build basic 2-kernel chain (GeminiIngestion → Claude_Code_6)
