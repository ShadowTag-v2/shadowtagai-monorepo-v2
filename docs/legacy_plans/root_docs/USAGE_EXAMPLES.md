...
class ClaudeMemoryClient {
  ...
  async getRecentChats(limit = 20) {
    const response = await fetch(`${this.baseUrl}/conversations/recent?limit=${limit}&project_id=${this.projectId}`);
    return response.json();
  }
}
...
(async () => {
  ...
  await client.createMemory("Meeting Request", "User wants to schedule a meeting for tomorrow", "context");
