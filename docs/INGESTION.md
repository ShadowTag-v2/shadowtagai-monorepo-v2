...
- Clear user agent: `ShadowTag-v2-Ingestion/1.0 (Educational; +https://github.com/ehanc69)`
...
kubectl get cronjob gemini-ingestion-pipeline -n ShadowTag-v2-llm-serving

# View latest job
kubectl get jobs -n ShadowTag-v2-llm-serving | grep gemini-ingestion

# View logs
kubectl logs -f job/gemini-ingestion-pipeline-<timestamp> -n ShadowTag-v2-llm-serving

# Manual trigger (for testing)
kubectl create job --from=cronjob/gemini-ingestion-pipeline manual-run-1 -n ShadowTag-v2-llm-serving
```
...
kubectl exec -it <pod-name> -n ShadowTag-v2-llm-serving -- cat /output/daily_briefing.md

# Via HTTP (if briefing-server deployed)
kubectl port-forward svc/ingestion-briefing-service 8080:80 -n ShadowTag-v2-llm-serving
...
- GitHub Issues: [Create an issue](https://github.com/ehanc69/ShadowTag-v2-fastapi-services/issues)
