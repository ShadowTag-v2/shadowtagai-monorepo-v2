...
**PNKLN Intelligence Pipeline - Complete Platform**
**Date**: 2025-11-18
...
This guide walks you through deploying the complete PNKLN Intelligence Platform on macOS, including:
...
- **YouAi Governance Service** (EU AI Act, DSA, NIST RMF, ISO 42001 compliance)
...
## Part 1: YouAi Governance Service + MCP Batch API
...
git checkout claude/pnkln-intelligence-pipeline-01DwB3v8zwZaHZC3HogNeRXt
```
...
# PostgreSQL (optional for YouAi)
POSTGRES_HOST=localhost
POSTGRES_PORT=5432
POSTGRES_DB=youai_governance
POSTGRES_USER=postgres
POSTGRES_PASSWORD=your_password

# Redis (optional for YouAi)
...
0 2 * * * cd ~/Projects/aiyou-fastapi-services/nightly_intel_pipeline && /usr/bin/python3 main.py >> logs/cron.log 2>&1
```
...
```bash
cd ~/Projects/aiyou-fastapi-services/erik-hancock-llm-memory
```
...
### 4.1 PostgreSQL (for YouAi Governance)
```bash
# Install
...
# Create database
createdb youai_governance

# Run migrations (if available)
# alembic upgrade head
```

### 4.2 Redis (for YouAi Caching)
```bash
# Install
brew install redis
...
# Build Docker image
docker build -t gcr.io/PROJECT_ID/pnkln-platform:latest .

# Push to GCR
docker push gcr.io/PROJECT_ID/pnkln-platform:latest
...
```bash
# Deploy YouAi FastAPI
gcloud run deploy youai-governance \
  --source=. \
  --region=us-central1 \
...
|-----------|-------------|-------|
| **YouAi FastAPI** | $0 | Local, no infrastructure |
...
**Documentation**:
- YouAi: `docs/youai-governance-README.md`
