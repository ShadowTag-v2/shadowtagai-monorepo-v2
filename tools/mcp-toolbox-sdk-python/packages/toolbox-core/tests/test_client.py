...
def test_tool_str():
  return ToolSchema(
    ...
    parameters=[
      ParameterSchema(name="param1", type="string", description="Description of Param1")
    ],
  )
...
def test_toolbox_client_no_warning_on_mcp():
  ...
  with (
    patch("toolbox_core.client.McpHttpTransportV20250618"),
...
def test_toolbox_client_no_warning_on_explicit_mcp_version():
  ...
  with (
    patch("toolbox_core.client.McpHttpTransportV20251125"),
    warnings.catch_warnings(record=True) as w,
  ):
    warnings.simplefilter("always")
