...
def test_tool_str():
    return ToolSchema(
        ...
        parameters=[ParameterSchema(name="param1", type="string", description="Description of Param1")],
    )
...
async def test_load_tool_success(mock_transport, test_tool_str):
    ...
    async with ToolboxClient(TEST_BASE_URL) as client:
        ...
        mock_transport.tool_invoke_mock.assert_awaited_once_with(TOOL_NAME, {"param1": "some value"}, {})
...
async def test_load_toolset_success(mock_transport, test_tool_str, test_tool_int_bool):
    ...
    TOOL1 = "tool1"
    TOOL2 = "tool2"
    manifest = ManifestSchema(serverVersion="0.0.0", tools={TOOL1: test_tool_str, TOOL2: test_tool_int_bool})
...
async def test_load_tool_not_found_in_manifest(mock_transport, test_tool_str):
    ...
    mismatched_manifest = ManifestSchema(serverVersion="0.0.0", tools={ACTUAL_TOOL_IN_MANIFEST: test_tool_str})
...
class TestAuth:
    ...
    @pytest.mark.asyncio
    async def test_auth_with_load_tool_success(self, tool_name, expected_header, mock_transport_auth):
        ...
        async with ToolboxClient(TEST_BASE_URL) as client:
            ...
            tool = await client.load_tool(tool_name, auth_token_getters={"my-auth-service": token_handler})
            await tool(5)
            mock_transport_auth.tool_invoke_mock.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_auth_with_add_token_success(self, tool_name, expected_header, mock_transport_auth):
    ...
    @pytest.mark.asyncio
    async def test_add_auth_token_getters_duplicate_fail(self, tool_name, mock_transport_auth):
        """
    ...
    @pytest.mark.asyncio
    async def test_add_auth_token_getters_missing_fail(self, tool_name, mock_transport_auth):
        """
    ...
    @pytest.mark.asyncio
    async def test_constructor_getters_missing_fail(self, tool_name, mock_transport_auth):
        """
        ...
        async with ToolboxClient(TEST_BASE_URL) as client:
            ...
            with pytest.raises(
                ...
                await client.load_tool(tool_name, auth_token_getters={AUTH_SERVICE: lambda: "token"})
...
class TestValidation:
    ...
    @pytest.mark.asyncio
    async def test_load_tool_with_bound_param_success(self, mock_transport, tool_schema_with_param_P):
        """Tests loading a tool with a successfully bound parameter."""
        TOOL_NAME = "tool_with_p"
        BOUND_VALUE = "this_is_bound"
        manifest = ManifestSchema(serverVersion="0.0.0", tools={TOOL_NAME: tool_schema_with_param_P})
        mock_transport.tool_get_mock.return_value = manifest

        async with ToolboxClient(TEST_BASE_URL) as client:
            client._ToolboxClient__transport = mock_transport
            tool = await client.load_tool(TOOL_NAME, bound_params={"param_P": BOUND_VALUE})
            ...
            await tool()
            mock_transport.tool_invoke_mock.assert_awaited_once_with(TOOL_NAME, {"param_P": BOUND_VALUE}, {})

    @pytest.mark.asyncio
    async def test_load_tool_with_unused_bound_param_fail(self, mock_transport, tool_schema_minimal):
        """Tests that load_tool fails if a bound_param is unused."""
        TOOL_NAME = "minimal_tool"
        manifest = ManifestSchema(serverVersion="0.0.0", tools={TOOL_NAME: tool_schema_minimal})
        mock_transport.tool_get_mock.return_value = manifest
        ...
        async with ToolboxClient(TEST_BASE_URL) as client:
            ...
            with pytest.raises(
                ...
                await client.load_tool(TOOL_NAME, bound_params={"unused_param": "some_value"})

    @pytest.mark.asyncio
    async def test_load_toolset_strict_with_partially_used_bound_param_fail(self, mock_transport, tool_schema_with_param_P, tool_schema_minimal):
        ...
        async with ToolboxClient(TEST_BASE_URL) as client:
            ...
            with pytest.raises(
                ...
                await client.load_toolset(bound_params={"param_P": "some_value"}, strict=True)

    @pytest.mark.asyncio
    async def test_load_toolset_non_strict_with_unused_bound_param_fail(self, mock_transport, tool_schema_minimal):
        """Tests that load_toolset fails in non-strict mode if a bound_param is used by no tools."""
        manifest = ManifestSchema(serverVersion="0.0.0", tools={"tool1": tool_schema_minimal})
        mock_transport.tools_list_mock.return_value = manifest
        TOOLSET_NAME = "my_set"
        ...
        async with ToolboxClient(TEST_BASE_URL) as client:
            ...
            with pytest.raises(
                ...
                await client.load_toolset(TOOLSET_NAME, bound_params={"param_Z": "some_value"})

    @pytest.mark.asyncio
    async def test_load_toolset_strict_with_partially_used_auth_fail(self, mock_transport, tool_schema_requires_auth_X, tool_schema_minimal):
        ...
        async with ToolboxClient(TEST_BASE_URL) as client:
            ...
            with pytest.raises(
                ...
                await client.load_toolset(auth_token_getters={"auth_service_X": lambda: "token"}, strict=True)

    @pytest.mark.asyncio
    async def test_load_toolset_non_strict_with_unused_auth_fail(self, mock_transport, tool_schema_minimal):
...
class TestClientHeaders:
    ...
    async def test_add_headers_success(self, mock_transport, tool_schema_minimal):
        ...
        mock_transport.tool_get_mock.assert_awaited_once_with(TOOL_NAME, {"X-Test-Header": "TestValue"})
        await client.close()
    ...
    async def test_load_tool_with_sync_callable_headers(
        ...
        async with ToolboxClient(TEST_BASE_URL, client_headers=sync_callable_header) as client:
            ...
            mock_transport.tool_get_mock.assert_awaited_once_with(tool_name, resolved_header)
            mock_transport.tool_invoke_mock.assert_awaited_once_with(tool_name, {"param1": "test"}, resolved_header)
    ...
    @pytest.mark.asyncio
    async def test_load_toolset_with_headers(self, mock_transport, test_tool_str, static_header):
        ...
        async with ToolboxClient(TEST_BASE_URL, client_headers=static_header) as client:
            ...
            mock_transport.tools_list_mock.assert_awaited_once_with(toolset_name, static_header)
    ...
    async def test_add_headers_duplicate_fail(self, static_header):
        ...
        async with ToolboxClient(TEST_BASE_URL, client_headers=static_header) as client:
            with pytest.warns(
                ...
                with pytest.raises(
                    ValueError,
                    match="Client header\\(s\\) `X-Static-Header` already registered",
                ):
