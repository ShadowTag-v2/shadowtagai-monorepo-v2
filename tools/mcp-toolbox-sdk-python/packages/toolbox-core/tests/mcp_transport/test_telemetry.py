...
class TestGetTracer:
  def test_returns_tracer_when_available(self):
    ...
    with (
      patch.object(telemetry_module, "TELEMETRY_AVAILABLE", True),
      patch.object(telemetry_module, "trace") as mock_trace,
    ):
  ...
  def test_raises_when_unavailable(self):
    with (
      patch.object(telemetry_module, "TELEMETRY_AVAILABLE", False),
      pytest.raises(RuntimeError, match="pip install toolbox-core"),
    ):
      get_tracer()
...
class TestGetMeter:
  def test_returns_meter_when_available(self):
    ...
    with (
      patch.object(telemetry_module, "TELEMETRY_AVAILABLE", True),
      patch.object(telemetry_module, "metrics") as mock_metrics,
    ):
