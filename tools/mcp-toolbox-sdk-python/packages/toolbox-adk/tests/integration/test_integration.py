...
class TestToolboxAdkIntegration:
  ...
  async def test_3lo_flow_simulation(self):
    ...
    try:
      ...
      assert isinstance(result_first, dict) and "error" in result_first, (
      ...
      try:
        ...
        err_msg = str(e).lower()
        assert any(x in err_msg for x in ["401", "403", "unauthorized", "forbidden"]), (
  ...
  async def test_header_collision(self):
    ...
    assert auth_header == creds_token, (
...
class TestAuth:
  async def test_run_tool_unauth_with_auth(self, auth_token2: str):
    ...
    try:
      with pytest.raises(
        ValueError,
        match=r"Validation failed for list of tools 'get-row-by-id': unused auth tokens could not be applied to any tool: my-test-auth",
      ):
        await toolset.get_tools()
    finally:
      await toolset.close()
  ...
  async def test_run_multiple_tools_unauth_with_auth(self, auth_token2: str):
    ...
    try:
      with pytest.raises(
        ValueError,
        match=r"Validation failed for list of tools 'get-row-by-id, search-rows': unused auth tokens could not be applied to any tool: my-test-auth",
      ):
        await toolset.get_tools()
    finally:
      await toolset.close()
  ...
  async def test_complex_params_e2e(self):
    ...
    async for event in runner.run_async(
      ...
      new_message=types.Content(
        role="user",
        parts=[
          types.Part(text="Use my_tool with array ['a'] and object {'nested_str': 'b'}")
        ],
      ),
    ):
      event_count += 1
