...
function parseStateValue(key, rawValue) {
	...
	if (NUMERIC_KEYS.has(key)) {
		...
		if (!Number.isFinite(parsed)) {
			throw new Error(
				`Expected numeric value for ${key}. Received: ${rawValue}`,
			);
		}
		return parsed;
	}
	if (key === "step") {
		if (!STEP_VALUES.includes(rawValue)) {
			throw new Error(
				`Invalid step "${rawValue}". Allowed values: ${STEP_VALUES.join(", ")}`,
			);
		}
		return rawValue;
	}
...
export function updateState(key, rawValue, sessionDir) {
	...
	console.log(
		`Successfully updated ${key} to ${String(value)} in ${statePath}`,
	);
}
if (
	process.argv[1] &&
	path.basename(process.argv[1]).startsWith("update-state")
) {
