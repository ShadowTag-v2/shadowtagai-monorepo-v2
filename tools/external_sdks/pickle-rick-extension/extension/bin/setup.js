...
function sanitizeSessionName(name) {
	...
	if (!slug) {
		throw new Error(
			"Invalid --name value. Use letters, numbers, dots, underscores, or hyphens.",
		);
	}
	return slug;
}
...
function loadDefaults(extensionRoot) {
	...
	try {
		...
		if (typeof settings.default_max_iterations === "number") {
			defaults.loopLimit = Math.max(
				0,
				Math.floor(settings.default_max_iterations),
			);
		}
		if (typeof settings.default_max_time_minutes === "number") {
			defaults.timeLimit = Math.max(
				0,
				Math.floor(settings.default_max_time_minutes),
			);
		}
		if (typeof settings.default_worker_timeout_seconds === "number") {
			defaults.workerTimeout = Math.max(
				0,
				Math.floor(settings.default_worker_timeout_seconds),
			);
		}
	} catch {
...
${Style.YELLOW}⚠️  STRICT EXIT CONDITION ACTIVE${Style.RESET}`);
		console.log(
			`   You must output: <promise>${params.promiseToken}</promise>\n`,
		);
	}
}
async function main() {
	...
	if (args.resume) {
		...
		if (!mappedSession) {
			die(
				"No active session found for current directory. Provide --resume <PATH> to continue.",
			);
		}
		...
		state = {
			...
			completion_promise: args.provided.completionPromise
				? (args.completionPromise ?? null)
				: null,
