...
function createLogger(extensionDir: string, sessionDir?: string) {
	...
	const sessionHooksLog = sessionDir
		? path.join(sessionDir, "hooks.log")
		: null;
...
async function main() {
	const extensionDir =
		...
		path.join(os.homedir(), ".gemini/extensions/pickle-rick");
	...
	const isTaskFinished = responseText.includes(
		"<promise>TASK_COMPLETED</promise>",
	);
	const isWorkerDone =
		isWorker && responseText.includes("<promise>I AM DONE</promise>");

	const isPrdDone =
		!isWorker && responseText.includes("<promise>PRD_COMPLETE</promise>");
	const isBreakdownDone =
		!isWorker && responseText.includes("<promise>BREAKDOWN_COMPLETE</promise>");
	const isTicketSelected =
		!isWorker && responseText.includes("<promise>TICKET_SELECTED</promise>");
	const isTicketDone =
		!isWorker && responseText.includes("<promise>TICKET_COMPLETE</promise>");
	const isTaskDone =
		!isWorker && responseText.includes("<promise>TASK_COMPLETE</promise>");
