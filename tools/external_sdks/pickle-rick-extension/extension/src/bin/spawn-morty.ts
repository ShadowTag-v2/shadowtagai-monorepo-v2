...
function clampWorkerTimeout(
	...
	try {
		const state = JSON.parse(
			fs.readFileSync(timeoutStatePath, "utf8"),
		) as Record<string, unknown>;
		const maxMins =
			typeof state.max_time_minutes === "number" ? state.max_time_minutes : 0;
		const startEpoch =
			typeof state.start_time_epoch === "number" ? state.start_time_epoch : 0;

		if (maxMins > 0 && startEpoch > 0) {
			const remainingSeconds = Math.floor(
				maxMins * 60 - (Date.now() / 1000 - startEpoch),
			);
...
async function main() {
	...
	proc.on("close", (code) => {
		...
		const output = fs.existsSync(sessionLog)
			? fs.readFileSync(sessionLog, "utf8")
			: "";
...
if (
	process.argv[1] &&
	path.basename(process.argv[1]).startsWith("spawn-morty")
) {
