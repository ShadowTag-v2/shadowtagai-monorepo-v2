...
export function get_branch_name(task_id: string): string {
	...
	const type = ["fix", "bug", "patch", "issue"].some((x) => lowerId.includes(x))
		? "fix"
		: "feat";
...
export function update_ticket_status(
	...
	if (!ticket_path) {
		throw new Error(
			`Ticket linear_ticket_${ticket_id}.md not found in ${session_dir}`,
		);
	}
	...
	console.log(
		`Successfully updated ticket ${ticket_id} to status "${new_status}"`,
	);
}
...
if (process.argv[1] && path.basename(process.argv[1]).includes("git-utils")) {
	...
	if (args.includes("--update-status")) {
		...
		if (!ticketId || !status || !sessionDir) {
			console.error(
				"Usage: node git_utils.js --update-status <id> <status> <session_dir>",
			);
			process.exit(1);
		}
