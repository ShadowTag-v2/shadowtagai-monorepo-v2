...
export function loadSessionsMap(extensionRoot: string): SessionsMap {
	...
	try {
		...
		for (const [cwd, sessionPath] of Object.entries(
...
export function findMappedSessionPath(
	map: SessionsMap,
	cwd: string,
): string | null {
...
export function readStateFile(stateFile: string): State | null {
	...
	try {
		const raw = JSON.parse(fs.readFileSync(stateFile, "utf8")) as Record<
			string,
			unknown
		>;
		...
		const state: State = {
			...
			working_dir:
				typeof raw.working_dir === "string" ? raw.working_dir : process.cwd(),
			...
			original_prompt:
				typeof raw.original_prompt === "string" ? raw.original_prompt : "",
			current_ticket:
				raw.current_ticket == null ? null : String(raw.current_ticket),
