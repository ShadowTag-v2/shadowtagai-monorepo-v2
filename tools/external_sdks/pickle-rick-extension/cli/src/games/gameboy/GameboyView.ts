...
export function isGameboyActive(): boolean {
	try {
		...
		logError(
			`Game boy binding error: ${e instanceof Error ? e.message : String(e)}`,
		);
		return false;
	}
}
...
/**
 * Launch the game boy emulator with a black background overlay
 */
export function launchGameboy(
	...
	try {
		...
		logError(
			`Failed to launch Game boy: ${e instanceof Error ? e.message : String(e)}`,
		);
