...
def create_diverse_plots(results: dict[str, Any], filepath: Path) -> bool:
    ...
    try:
        ...
        fig.suptitle(f"LangExtract Benchmark - {results['timestamp']}", fontsize=14, y=0.98)
...
def _plot_tokenization_throughput(ax, results):
    ...
    if config.TOKENIZATION_KEY not in results or not results[config.TOKENIZATION_KEY]:
...
def _plot_summary_table(ax, results):
    ...
    success_count = sum(1 for r in results.get(config.RESULTS_KEY, []) if r.get("success"))
    ...
    if success_count > 0:
        avg_time = sum(r.get("time_seconds", 0) for r in results.get(config.RESULTS_KEY, []) if r.get("success")) / success_count
...
def _plot_time_comparison(ax, all_results):
    ...
    for result in all_results:
        ...
        if config.RESULTS_KEY in result:
            times = [r.get("time_seconds", 0) for r in result[config.RESULTS_KEY] if r.get("success")]
...
def _plot_success_rate_comparison(ax, all_results):
    ...
    x_pos = np.arange(len(runs))
    colors = ["green" if rate == 100 else "orange" if rate >= 75 else "red" for rate in success_rates]
...
def _plot_token_rate_by_language(ax, all_results):
    ...
    if config.RESULTS_KEY in latest_result:
        for lang in languages:
            lang_results = [r for r in latest_result[config.RESULTS_KEY] if r.get("text_type") == lang and r.get("success")]
            if lang_results and config.TOKENIZATION_KEY in lang_results[0]:
                rate = lang_results[0][config.TOKENIZATION_KEY].get("tokens_per_char", 0)
                token_rates.append(rate)
                colors.append("red" if rate < 0.1 else "orange" if rate < 0.2 else "green")
            else:
...
def _plot_timeline(ax, all_results):
    ...
    for result in all_results:
        ...
        if config.RESULTS_KEY in result:
            total_entities = sum(r.get("entity_count", 0) for r in result[config.RESULTS_KEY] if r.get("success"))
