...
Usage:
  ...
  python benchmarks/benchmark.py --model gemini-3.1-flash-lite-preview
...
from langextract import core, data, visualize

# Load API key from environment
dotenv.load_dotenv(override=True)
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", os.environ.get("LANGEXTRACT_API_KEY"))
...
class BenchmarkRunner:
    ...
    def benchmark_tokenization(self) -> list[dict[str, Any]]:
        ...
        for word_count in config.TOKENIZATION.default_text_sizes:
            ...
            word_str = f"{word_count // 1000:,}k" if word_count >= 1000 else f"{word_count:,}"

            print(f"{word_str:>6} words: {avg_ms:7.2f}ms  ({tokens_per_sec / 1e6:.1f}M tokens/sec)")
    ...
    def test_single_extraction(
        ...
        Args:
          model_id: Model identifier (e.g., 'gemini-3.1-flash-lite-preview', 'gemma2:2b').
        ...
        try:
            ...
            tokenization_analysis = utils.analyze_tokenization(test_text, self.tokenizer)
            print(f"   Tokenization: {utils.format_tokenization_summary(tokenization_analysis)}")
            ...
            example = data.ExampleData(
                ...
                extractions=[
                    data.Extraction(extraction_text="Macbeth", extraction_class="Character"),
            ...
            if result.extractions:
                for extraction in result.extractions:
                    is_grounded = (
                        extraction.char_interval and extraction.char_interval.start_pos is not None and extraction.char_interval.end_pos is not None
                    )
            ...
            if unique_grounded:
                ...
                sample_str = ", ".join(sample) + ("..." if len(unique_grounded) > 5 else "")
    ...
    def test_diverse_text_types(self, models: list[str] | None = None) -> list[dict[str, Any]]:
        ...
        for model_id in models:
            ...
            for text_type in config.TextTypes:
                ...
                if result.get("success"):
                    test_count += 1
                    if test_count % 3 == 0:
                        print(f"   Rate limit delay ({config.MODELS.gemini_rate_limit_delay}s)...")
    ...
    def save_results(self, results: dict[str, Any]):
        ...
        json_path = config.PATHS.get_result_path(self.timestamp, "").with_suffix(".json")
...
def main():
    ...
    if args.compare:
        ...
        if len(json_files) < 2:
            print(f"Need at least 2 benchmark results for comparison, found {len(json_files)}")
            return
        ...
        comparison_path = results_dir / f"comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    if "gemini" in model_to_test.lower() and not GEMINI_API_KEY:
        print(f"Error: {model_to_test} requires GEMINI_API_KEY or LANGEXTRACT_API_KEY")
        return
