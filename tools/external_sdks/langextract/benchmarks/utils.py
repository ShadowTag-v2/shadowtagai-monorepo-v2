...
def extract_text_content(full_text: str) -> str:
    ...
    if start_idx != -1 and end_idx != -1:
        ...
        if line_end != -1 and line_end < content_start + 100:  # Ensure marker is on same line.
            content_start = full_text.find("\n", line_end) + 1
...
def get_optimal_text_size(text: str, model_id: str) -> str:
    ...
    """
    if ":" in model_id or "gemma" in model_id.lower() or "llama" in model_id.lower():
...
def analyze_tokenization(text: str, tokenizer_inst: tokenizer.Tokenizer | None = None) -> dict[str, Any]:
...
def format_tokenization_summary(analysis: dict[str, Any]) -> str:
    ...
    """
    return f"{analysis['num_tokens']} tokens, {analysis['tokens_per_char']:.3f} tok/char"
