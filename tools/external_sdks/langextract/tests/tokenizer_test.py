...
class UnicodeTokenizerTest(parameterized.TestCase):
    ...
    def test_special_unicode_and_punctuation_handling(self, input_text, expected_tokens, expected_first_after_newline):
        ...
        self.assertLen(
            tokenized.tokens,
            len(expected_tokens),
            f"Expected {len(expected_tokens)} tokens for edge case test, but got {len(tokenized.tokens)}",
        )
        ...
        for i, (
            ...
            expected_newline,
        ) in enumerate(zip(tokenized.tokens, expected_tokens, expected_first_after_newline)):
    ...
    def test_mixed_script_and_emoji(self):
        ...
        for i, (expected_text, expected_type) in enumerate(expected_tokens):
            ...
            actual_text = tokenized.text[token.char_interval.start_pos : token.char_interval.end_pos]
    ...
    def test_script_boundary_grouping(self):
        ...
        t1_text = text[tokenized.tokens[0].char_interval.start_pos : tokenized.tokens[0].char_interval.end_pos]
        t2_text = text[tokenized.tokens[1].char_interval.start_pos : tokenized.tokens[1].char_interval.end_pos]
    ...
    def test_non_spaced_scripts_no_grouping(self):
        ...
        self.assertGreater(len(tokenized.tokens), 1, "Should not be grouped into a single token")
    ...
    def test_punctuation_merging_identical_only(self):
        ...
        self.assertEqual(
            [input_text_mixed[t.char_interval.start_pos : t.char_interval.end_pos] for t in tokens_mixed],
...
class ExceptionTest(absltest.TestCase):
    ...
    def test_sentence_range_errors(self):
        ...
        with self.assertRaisesRegex(tokenizer.SentenceRangeError, "start_token_index=-1 out of range"):
...
class NegativeTestCases(parameterized.TestCase):
    ...
    def test_invalid_and_edge_case_unicode(self, input_text, expected_tokens):
        ...
        self.assertLen(
            tokenized.tokens,
            len(expected_tokens),
            f"Expected {len(expected_tokens)} tokens for edge case '{input_text}', but got {len(tokenized.tokens)}",
        )

        for i, (token, (expected_text, expected_type)) in enumerate(zip(tokenized.tokens, expected_tokens)):
    ...
    def test_whitespace_only_string(self):
        ...
        for whitespace in test_cases:
            ...
            self.assertEmpty(
                tokenized.tokens,
                f"Whitespace-only string '{repr(whitespace)}' should produce no tokens",
            )
