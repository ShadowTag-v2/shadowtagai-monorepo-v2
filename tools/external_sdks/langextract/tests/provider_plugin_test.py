...
class PluginSmokeTest(absltest.TestCase):
    ...
    def test_plugin_discovery_and_usage(self):
        ...
        with mock.patch.object(metadata, "entry_points", return_value=_create_mock_entry_points([ep])):
    ...
    def test_load_plugins_once_is_idempotent(self):
        ...
        with mock.patch.object(metadata, "entry_points", return_value=_create_mock_entry_points([ep])) as m:
            lx.providers.load_plugins_once()
    ...
    def test_non_subclass_entry_point_does_not_crash(self):
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(lx.exceptions.InferenceConfigError, "No provider registered"):
    ...
    def test_plugin_priority_override_core_provider(self):
        ...
        resolved = lx.providers.registry.resolve("gemini-3.1-flash-lite-preview")
            ...
            class SchemaE2EProvider(base_model.BaseLanguageModel):
                ...
                def infer(self, batch_prompts, **kwargs):
                    ...
                    if self.schema_config:
                        output = '{"extractions": [{"entity": "test", "entity_attributes": {"schema": "applied"}}]}'
                    else:
                        output = '{"extractions": []}'
                    return [[types.ScoredOutput(score=1.0, output=output)]]
