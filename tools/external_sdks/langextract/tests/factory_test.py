...
class FactoryTest(absltest.TestCase):  # pylint: disable=too-many-public-methods
    ...
    def test_create_model_basic(self):
        ...
        config = factory.ModelConfig(model_id="gemini-3.1-family", provider_kwargs={"api_key": "test-key"})

        model = factory.create_model(config)
        self.assertIsInstance(model, FakeGeminiProvider)
        self.assertEqual(model.model_id, "gemini-3.1-family")
    ...
    def test_uses_gemini_api_key_from_environment(self):
        ...
        config = factory.ModelConfig(model_id="gemini-3.1-family")
    ...
    @mock.patch.dict(os.environ, {"LANGEXTRACT_API_KEY": "env-langextract-key"}, clear=True)
    ...
    def test_explicit_kwargs_override_env(self):
        ...
        with mock.patch.dict(os.environ, {"GEMINI_API_KEY": "env-key"}):
            config = factory.ModelConfig(model_id="gemini-3.1-family", provider_kwargs={"api_key": "explicit-key"})
    ...
    def test_model_config_fields_are_immutable(self):
        ...
        config = factory.ModelConfig(model_id="gemini-3.1-family", provider_kwargs={"api_key": "test"})
    ...
    def test_explicit_provider_overrides_pattern_matching(self):
        ...
        config = factory.ModelConfig(model_id="gemini-3.1-family", provider="AnotherProvider")
        model = factory.create_model(config)
    ...
    def test_gemini_vertexai_with_credentials(self):
        ...
        try:
            ...
            class MockGeminiWithCredentials(base_model.BaseLanguageModel):  # pylint: disable=unused-variable
                def __init__(self, model_id="gemini-3.1-flash-lite-preview", credentials=None, **kwargs):
                    self.model_id = model_id
                    self.credentials = credentials
                    super().__init__()
