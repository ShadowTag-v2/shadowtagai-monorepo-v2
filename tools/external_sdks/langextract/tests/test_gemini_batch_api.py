...
class TestGeminiBatchAPI(absltest.TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.mock_storage_cls = self.enter_context(mock.patch.object(gb.storage, "Client", autospec=True))
    ...
    def test_batch_with_schema(self, mock_client_cls):
        ...
        output_blob.open.return_value.__enter__.return_value = io.StringIO(_create_batch_response(0, {"name": "test"}))
        self.mock_bucket.list_blobs.return_value = [output_blob]
    ...
    def test_max_prompts_per_job(self, mock_client_cls):
        ...
        blob2.open.return_value.__enter__.return_value = io.StringIO(_create_batch_response(0, "r5"))
    ...
    def test_batch_item_error(self, mock_client_cls):
        ...
        output_blob = mock.create_autospec(gb.storage.Blob, instance=True)
        output_blob.name = f"output{gb._EXT_JSONL}"
        output_blob.open.return_value.__enter__.return_value = io.StringIO(_create_batch_error(0, 13, "Internal error"))
        self.mock_bucket.list_blobs.return_value = [output_blob]
...
class EmptyAndPaddingTest(absltest.TestCase):
    ...
    def test_file_pad_to_expected_count(self, mock_client_cls):
        ...
        with mock.patch.object(gb.storage, "Client", autospec=True) as mock_storage:
            ...
            output_blob = mock.create_autospec(gb.storage.Blob, instance=True)
            output_blob.name = f"output{gb._EXT_JSONL}"
            output_blob.open.return_value.__enter__.return_value = io.StringIO(_create_batch_response(0, "only_one"))
...
class GCSBatchCachingTest(absltest.TestCase):
    ...
    def test_partial_cache_hit(self, mock_client_cls):
        ...
        with mock.patch.object(gb.GCSBatchCache, "_compute_hash") as mock_hash:
            ...
            output_blob = mock.create_autospec(gb.storage.Blob, instance=True)
            output_blob.name = f"output{gb._EXT_JSONL}"
            output_blob.open.return_value.__enter__.return_value = io.StringIO(_create_batch_response(0, "new_response"))
            self.mock_bucket.list_blobs.return_value = [output_blob]
            ...
            upload_calls = [call for call in blob_miss.upload_from_string.mock_calls if "new_response" in str(call)]
    ...
    def test_project_passed_to_storage_client(self, mock_client_cls):
        ...
        output_blob = mock.create_autospec(gb.storage.Blob, instance=True)
        output_blob.name = f"output{gb._EXT_JSONL}"
        output_blob.open.return_value.__enter__.return_value = io.StringIO(_create_batch_response(0, {"result": "ok"}))
        self.mock_bucket.list_blobs.return_value = [output_blob]
        ...
        project_calls = [call for call in storage_calls if call.kwargs.get("project") == test_project]

        self.assertGreaterEqual(
            len(project_calls),
            1,
            f"storage.Client should be called with project={test_project}, " f"but was called with: {[call.kwargs for call in storage_calls]}",
        )
