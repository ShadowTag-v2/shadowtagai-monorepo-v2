...
skip_if_no_vertex = pytest.mark.skipif(
    ...
    reason=("Vertex AI credentials not available (set GOOGLE_CLOUD_PROJECT and configure gcloud auth)"),
)
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        print(f"\nRetryable error ({type(e).__name__}) on attempt {attempt + 1}/{max_retries + 1}: {e}")
                        time.sleep(delay)
...
class TestLiveAPIGemini(unittest.TestCase):
    ...
    def test_multilingual_medication_extraction(self):
        ...
        medication_extractions = [e for e in result.extractions if e.extraction_class == _CLASS_MEDICATION]
    ...
    """)
        ...
        for extraction in result.extractions:
            ...
            assert "medication_group" in extraction.attributes, f"Missing medication_group for {extraction.extraction_text}"
        ...
        for med_name, extractions in medication_groups.items():
            ...
            assert _CLASS_MEDICATION in extraction_classes, f"{med_name} group missing medication entity"
            # Dosage is expected but might be formatted differently
            assert any(c in extraction_classes for c in [_CLASS_DOSAGE, "dose"]), f"{med_name} group missing dosage"
...
class TestCrossChunkContext(unittest.TestCase):
    ...
    def test_context_window_extracts_from_both_chunks(self):
        ...
        input_text = "Dr. Sarah Chen is the lead researcher at the institute. " "She published groundbreaking work on neural networks last year."
        prompt = textwrap.dedent(
            """\
        ...
        examples = [
            lx.data.ExampleData(
                text=("Professor James Miller leads the physics department. He won the Nobel Prize in 2020."),
                extractions=[
                    lx.data.Extraction(
        ...
        all_extraction_text = " ".join(str(e.extraction_text) + " " + str(e.attributes) for e in result.extractions).lower()

        has_chunk1_content = any(term in all_extraction_text for term in ("sarah", "chen", "researcher", "lead"))
        has_chunk2_content = any(term in all_extraction_text for term in ("published", "groundbreaking", "neural", "networks"))
