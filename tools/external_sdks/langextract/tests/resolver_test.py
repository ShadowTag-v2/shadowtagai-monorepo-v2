    ...
    for extraction in extractions:
        if extraction.alignment_status == data.AlignmentStatus.MATCH_EXACT:
            ...
            test_case.assertEqual(
                ...
                f"Extraction '{extraction.extraction_text}' does not match extracted" f" '{extracted}' using char_interval {char_int}",
            )
...
class ExtractOrderedEntitiesTest(parameterized.TestCase):
    ...
    def test_extract_ordered_extractions_success(
        ...
        if resolver is None:
            resolver = resolver_lib.Resolver(extraction_index_suffix=resolver_lib.DEFAULT_INDEX_SUFFIX)
    ...
    def test_extract_ordered_extractions_exceptions(self, resolver, test_input, expected_exception, expected_regex):
        with self.assertRaisesRegex(expected_exception, expected_regex):
...
class AlignEntitiesTest(parameterized.TestCase):
    ...
    _SOURCE_TEXT_THREE_CONDITIONS_AND_MEDS = "Patient with arthritis, fever, and inflammation is prescribed Naprosyn, prednisone, and ibuprofen."
    _SOURCE_TEXT_MULTI_WORD_EXTRACTIONS = "Pt was prescribed Naprosyn as needed for pain and prednisone for one month."
    ...
    @parameterized.named_parameters(
        (
            "basic_alignment",
            [
                [data.Extraction(extraction_class="medication", extraction_text="Naprosyn")],
                [
                    data.Extraction(
                        extraction_class="medication",
                        extraction_text="prednisone",
        ...
        (
            ...
            [
                [data.Extraction(extraction_class="condition", extraction_text="arthritis")],
                ...
                [data.Extraction(extraction_class="medication", extraction_text="Naprosyn")],
                [data.Extraction(extraction_class="medication", extraction_text="ibuprofen")],
                [
                    data.Extraction(
                        extraction_class="medication",
                        extraction_text="prednisone",
        ...
        (
            ...
            [[data.Extraction(extraction_class="medication", extraction_text="Napro-syn")]],
        ...
        (
            ...
            [
                [data.Extraction(extraction_class="condition", extraction_text="high blood")],
                [
                    data.Extraction(
                        extraction_class="condition",
        ...
        (
            ...
            [
                [data.Extraction(extraction_class="medication", extraction_text="Naprosyn")],
                [data.Extraction(extraction_class="frequency", extraction_text="as needed")],
    ...
    def test_extraction_alignment(
        ...
        if expected_output is ValueError:
            with self.assertRaises(ValueError):
                self.aligner.align_extractions(extractions, source_text, enable_fuzzy_alignment=False)
        else:
...
class FenceFallbackTest(parameterized.TestCase):
    ...
    def test_rejects_multiple_fenced_blocks(self):
        ...
        with self.assertRaisesRegex(resolver_lib.ResolverParsingError, "Multiple fenced blocks found"):
            resolver.string_to_extraction_data(test_input)
