...
class ExtractParameterPrecedenceTest(absltest.TestCase):
    ...
    def test_language_model_type_only_emits_warning_and_works(self, mock_create_model, mock_annotator_cls):
        ...
        mock_model_config.assert_called_once()
        _, kwargs = mock_model_config.call_args
        self.assertEqual(kwargs["model_id"], "gemini-3.1-flash-lite-preview")
        mock_create_model.assert_called_once()
        self.assertEqual(result, "ok")
    ...
    def test_use_schema_constraints_warns_with_config(self, mock_create_model, mock_annotator_cls):
        ...
        config = factory.ModelConfig(model_id="gemini-3.1-flash-lite-preview", provider_kwargs={"api_key": "test-key"})
        ...
        mock_create_model.assert_called_once()
        called_config = mock_create_model.call_args[1]["config"]
        self.assertEqual(called_config.model_id, "gemini-3.1-flash-lite-preview")
        self.assertEqual(result, "ok")
