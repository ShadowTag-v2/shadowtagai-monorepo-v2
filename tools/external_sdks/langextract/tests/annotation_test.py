...
class AnnotatorTest(absltest.TestCase):
    def setUp(self):
        super().setUp()
        self.mock_language_model = self.enter_context(mock.patch.object(gemini, "GeminiLanguageModel", autospec=True))
    ...
    def assert_char_interval_match_source(self, source_text: str, extractions: Sequence[data.Extraction]):
        ...
        for extraction in extractions:
            if extraction.alignment_status == data.AlignmentStatus.MATCH_EXACT:
                assert extraction.char_interval is not None, "char_interval should not be None for AlignmentStatus.MATCH_EXACT"
                ...
                self.assertEqual(
                    ...
                    f"Extraction '{extraction.extraction_text}' does not match" f" extracted '{extracted}' using char_interval {char_int}",
                )

    def test_annotate_text_single_chunk(self):
        text = "Patient Jane Doe, ID 67890, received 10mg of Lisinopril daily for hypertension diagnosed on 2023-03-15."
...
class AnnotatorMultipleDocumentTest(parameterized.TestCase):
    ...
    def test_annotate_documents(
        ...
        mock_language_model = self.enter_context(mock.patch.object(gemini, "GeminiLanguageModel", autospec=True))
        ...
        for actual_annotation, expected_annotation in zip(actual_annotations, expected_result):
    ...
    def test_annotate_documents_exceptions(
        ...
        document_objects = [data.Document(text=doc["text"], document_id=doc["document_id"]) for doc in documents]
...
class AnnotatorMultiPassTest(absltest.TestCase):
    ...
    def test_multipass_extraction_non_overlapping(self):
        ...
        result = self.annotator.annotate_text(text, resolver=resolver, extraction_passes=2, debug=False)

        self.assertLen(result.extractions, 4)
        extraction_classes = [e.extraction_class for e in result.extractions]
        self.assertCountEqual(extraction_classes, ["patient", "condition", "medication", "frequency"])
...
class MultiPassHelperFunctionsTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ...
        dict(
            ...
            all_extractions=[
                [
                    ...
                    data.Extraction("class2", "text2", char_interval=data.CharInterval(10, 15)),
                ]
            ],
            expected_count=2,
        ...
        dict(
            ...
            all_extractions=[
                ...
                [
                    data.Extraction("class2", "text2", char_interval=data.CharInterval(5, 15)),  # Overlaps
                    data.Extraction("class3", "text3", char_interval=data.CharInterval(20, 25)),  # No overlap
                ],
            ],
            expected_count=2,
            expected_classes=[
    ...
    def test_merge_non_overlapping_extractions(self, all_extractions, expected_count, expected_classes):
...
class AnnotateDocumentsGeneratorTest(absltest.TestCase):
    ...
    def test_yields_documents_not_generators(self):
        ...
        meds_doc1 = {e.extraction_text for e in results[0].extractions if e.extraction_class == "medication"}
        meds_doc2 = {e.extraction_text for e in results[1].extractions if e.extraction_class == "medication"}
...
class CrossChunkContextTest(absltest.TestCase):
    ...
    def test_context_window_per_document_isolation(self):
        ...
        empty_response = [[types.ScoredOutput(score=1.0, output=f"```yaml\n{data.EXTRACTIONS_KEY}: []\n```")]]
        self.mock_language_model.infer.side_effect = [
