...
_COMMON_SCRIPTS_PATTERN = regex.compile("|".join(rf"(?P<{script}>\p{{Script={script}}})" for script in _COMMON_SCRIPTS))
        ...
        for match in regex.finditer(r"\X", text):
            ...
            if grapheme.isspace():
                if current_type is not None:
                    self._emit_token(tokens, text, current_start, start, current_type, previous_end)
                    previous_end = start
                    current_type = None
                    current_script = None
            ...
            if current_type is not None:
                if current_type == g_type:
                    if current_type == TokenType.WORD:
                        ...
                        elif _CJK_PATTERN.match(first_char) or _NON_SPACED_PATTERN.match(first_char):
                            should_merge = False
                        ...
                        if last_grapheme == grapheme:
                            should_merge = True
                        elif len(last_grapheme) >= len(grapheme) and last_grapheme.endswith(grapheme):
                            should_merge = True
...
def _is_end_of_sentence_token(
    ...
    if _END_OF_SENTENCE_PATTERN.search(current_token_text):
        if current_idx > 0:
            prev_token_text = text[tokens[current_idx - 1].char_interval.start_pos : tokens[current_idx - 1].char_interval.end_pos]
        ...
        if tokens[i].token_type == TokenType.PUNCTUATION:
            if _is_end_of_sentence_token(text, tokens, i, known_abbreviations):
                ...
                while end_index < len(tokens):
                    next_token_text = text[tokens[end_index].char_interval.start_pos : tokens[end_index].char_interval.end_pos]
                    if tokens[end_index].token_type == TokenType.PUNCTUATION and next_token_text in _CLOSING_PUNCTUATION:
                        end_index += 1
                    else:
                        break
