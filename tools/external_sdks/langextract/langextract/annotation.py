...
class Annotator:
    ...
    def _annotate_documents_single_pass(
        ...
        **kwargs,
        ...
        chunk_iter = _document_chunk_iterator(_capture_docs(documents), max_char_buffer, tokenizer=tokenizer)
        ...
        batch_iter = progress.create_extraction_progress_bar(batches, model_info=model_info, disable=not show_progress)
        ...
        try:
            for batch in batch_iter:
                ...
                prompts = [prompt_builder.build_prompt(chunk.chunk_text, chunk.document_id, chunk.additional_context) for chunk in batch]
                ...
                for text_chunk, scored_outputs in zip(batch, outputs):
                    ...
                    if not scored_outputs:
                        raise exceptions.InferenceOutputError("No scored outputs from language model.")

                    resolved_extractions = resolver.resolve(scored_outputs[0].output, debug=debug, **kwargs)

                    token_offset = text_chunk.token_interval.start_index if text_chunk.token_interval else 0
                    char_offset = text_chunk.char_interval.start_pos if text_chunk.char_interval else 0
                    ...
                    if show_progress and text_chunk.char_interval is not None:
                        chars_processed += text_chunk.char_interval.end_pos - text_chunk.char_interval.start_pos
        ...
        **kwargs,
        ...
        for doc in document_list:
            ...
            if debug:
                ...
                logging.info(
                    "Document %s: Merged %d extractions from %d passes into %d non-overlapping extractions.",
                    doc_id,
                    total_extractions,
                    extraction_passes,
