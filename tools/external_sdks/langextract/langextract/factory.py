...
class ModelConfig:
    ...
    Attributes:
      model_id: The model identifier (e.g., "gemini-3.1-flash-lite-preview", "gpt-4o").
            ...
            if provider_prefix in model_lower:
                ...
                if found_keys:
                    ...
                    if len(found_keys) > 1:
                        ...
                        warnings.warn(
                            f"Multiple API keys detected in environment: {keys_list}. " f"Using {found_keys[0][0]} and ignoring others.",
                            UserWarning,
                            stacklevel=3,
                        )
                break
...
def create_model(
    ...
    try:
        ...
        raise exceptions.InferenceConfigError(
            "Failed to load provider. " "This may be due to missing dependencies. " f"Check that all required packages are installed. Error: {e}"
        ) from e
...
def create_model_from_id(
    ...
    **provider_kwargs: typing.Any,
    ...
    Args:
      model_id: The model identifier (e.g., "gemini-3.1-flash-lite-preview").
