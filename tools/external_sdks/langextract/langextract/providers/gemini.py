...
_DEFAULT_MODEL_ID = "gemini-3.1-flash-lite-preview"
...
class GeminiLanguageModel(base_model.BaseLanguageModel):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            raise exceptions.InferenceConfigError("google-genai is required for Gemini. Install it with: pip install google-genai") from e
        ...
        if self.api_key and self.vertexai:
            logging.warning("Both API key and Vertex AI configuration provided. API key will take precedence for authentication.")
    ...
    def _validate_schema_config(self) -> None:
        ...
        if self.gemini_schema and self.format_type != data.FormatType.JSON:
            raise exceptions.InferenceConfigError(
                "Gemini structured output only supports JSON format. " "Set format_type=JSON or use_schema_constraints=False."
            )
    ...
    def infer(self, batch_prompts: Sequence[str], **kwargs) -> Iterator[Sequence[core_types.ScoredOutput]]:
        ...
        if self._batch_cfg and self._batch_cfg.enabled:
            if len(batch_prompts) >= self._batch_cfg.threshold:
                try:
                    ...
                    raise exceptions.InferenceRuntimeError(f"Gemini Batch API error: {e}", original=e) from e
        ...
        if len(batch_prompts) > 1 and self.max_workers > 1:
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(self.max_workers, len(batch_prompts))) as executor:
                future_to_index = {executor.submit(self._process_single_prompt, prompt, config.copy()): i for i, prompt in enumerate(batch_prompts)}
