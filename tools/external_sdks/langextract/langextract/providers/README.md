...
─────────                    ───────────                      ────────
    ...
    |                             |                              |
    |                    registry.resolve("gemini-3.1-flash-lite-preview")    |
...
config = lx.factory.ModelConfig(
    provider="GeminiLanguageModel",  # Will use default gemini-3.1-flash-lite-preview
    provider_kwargs={"api_key": "..."}
)
model = lx.factory.create_model(config)
...
config = lx.factory.ModelConfig(
    model_id="gemini-3.1-flash-lite-preview"  # Provider auto-detected
)
model = lx.factory.create_model(config)
```
...
When you call `lx.extract(model_id="gemini-3.1-flash-lite-preview", ...)`, here's what happens:

1. **Factory receives model_id**: "gemini-3.1-flash-lite-preview"
...
@lx.providers.registry.register(
    ...
    r'^gemini',                # Prefix: model_id="gemini-3.1-flash-lite-preview"
