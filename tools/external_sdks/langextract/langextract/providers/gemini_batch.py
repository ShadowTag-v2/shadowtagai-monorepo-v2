    ...
    def __post_init__(self):
        ...
        if self.enabled:
            if self.enable_caching is _UNSET:
                raise ValueError("batch.enable_caching must be explicitly set when batch is enabled")
            if self.retention_days is _UNSET:
                raise ValueError("batch.retention_days must be explicitly set when batch is enabled (use None for permanent)")
            if self.retention_days is not None and self.retention_days <= 0:
                raise ValueError("batch.retention_days must be > 0 or None (for permanent). 0 (immediate delete) is not allowed.")
...
def _default_job_create_callback(job: Any) -> None:
    ...
    try:
        ...
        if len(parts) >= 6:
            ...
            project = parts[1]
            logging.info(
                "Job Console URL:" " https://console.cloud.google.com/vertex-ai/locations/%s/batch-predictions/%s?project=%s",
                location,
                job_id,
                project,
            )
...
def _ensure_bucket_lifecycle(bucket: storage.Bucket, retention_days: int | None) -> None:
    ...
    for rule in bucket.lifecycle_rules:
        if rule.get("action", {}).get("type") == "Delete" and rule.get("condition", {}).get("age") == retention_days:
            return
...
def _submit_file(
    ...
    Args:
      ...
      model_id: Model identifier (e.g., "gemini-3.1-flash-lite-preview").
    ...
    path = None
    try:
        with tempfile.NamedTemporaryFile("w", suffix=_EXT_JSONL, delete=False, encoding="utf-8") as f:
            path = f.name
...
def _poll_completion(client: genai.Client, job: genai.types.BatchJob, cfg: BatchConfig) -> genai.types.BatchJob:
    ...
    while True:
        ...
        if state in _TERMINAL_FAIL:
            ...
            raise exceptions.InferenceRuntimeError(f"Batch job failed: state={state.name}, name={name}, error={error_details}")
        ...
        if time.time() - start > cfg.timeout:
            ...
            raise exceptions.InferenceRuntimeError(f"Batch job timed out after {cfg.timeout}s: {name}")
...
def infer_batch(
    ...
    std_logging.getLogger("google.auth.transport.requests").setLevel(std_logging.WARNING)
    ...
    def _process_batch(batch_items: Sequence[tuple[int, str]], display: str) -> dict[int, str]:
        ...
        requests = [_build_request(p, schema_dict, gen_config, system_instruction, safety_settings) for p in batch_prompts]
        job = _submit_file(
            client,
            model_id,
            requests,
