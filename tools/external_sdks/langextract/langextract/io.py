...
def save_annotated_documents(
    ...
    # Create progress bar
    progress_bar = progress.create_save_progress_bar(output_path=str(output_file), disable=not show_progress)
...
def _read_csv(filepath: pathlib.Path, column_names: list[str], delimiter: str = ",") -> Iterator[dict[str, Any]]:
...
def download_text_from_url(
    ...
    try:
        ...
        raise requests.RequestException(f"Failed to download from {url}: {str(e)}") from e
