...
def create_extraction_progress_bar(iterable: Any, model_info: str | None = None, disable: bool = False) -> tqdm.tqdm:
    ...
    Args:
      ...
      model_info: Optional model information to display (e.g., "gemini-3.1-family").
...
def print_download_complete(char_count: int, word_count: int, filename: str) -> None:
    ...
    """
    print(
        f"{GREEN}✓{RESET} Downloaded {BOLD}{char_count:,}{RESET} characters " f"({BOLD}{word_count:,}{RESET} words) from {BLUE}{filename}{RESET}",
        flush=True,
    )
...
def print_extraction_summary(
    ...
    """
    print(
        f"{GREEN}✓{RESET} Extracted {BOLD}{num_extractions}{RESET} entities " f"({BOLD}{unique_classes}{RESET} unique types)",
        flush=True,
    )
...
def format_extraction_progress(
    ...
    Args:
      model_info: Optional model information (e.g., "gemini-3.1-flash-lite-preview").
