...
def _filter_valid_extractions(
    ...
    return [e for e in extractions if (e.char_interval and e.char_interval.start_pos is not None and e.char_interval.end_pos is not None)]
...
def _build_highlighted_text(
    ...
    for point in points:
        ...
        if point.tag_type == TagType.START:
            ...
            span_html = f'<span class="lx-highlight{highlight_class}"' f' data-idx="{point.span_idx}" style="background-color:{colour};">'
...
def _build_legend_html(color_map: dict[str, str]) -> str:
    ...
    for extraction_class, colour in color_map.items():
        legend_items.append(f'<span class="lx-label" style="background-color:{colour};">{html.escape(extraction_class)}</span>')
...
def _format_attributes(attributes: dict | None) -> str:
    ...
    for key, value in valid_attrs.items():
        ...
        if isinstance(value, list):
            ...
            value_str = str(value)
        attrs_parts.append(
            f'<span class="lx-attr-key">{html.escape(str(key))}</span>: <span' f' class="lx-attr-value">{html.escape(value_str)}</span>'
        )
...
def _build_visualization_html(
    ...
    assert (
        first_extraction.char_interval and first_extraction.char_interval.start_pos is not None and first_extraction.char_interval.end_pos is not None
