            ...
            for extraction_class, extraction_value in group.items():
                ...
                if index_suffix:
                    ...
                    if extraction_index is None:
                        logging.debug("No index value for %s. Skipping extraction.", extraction_class)
                        continue
                else:
                    extraction_index += 1
...
class WordAligner:
    ...
    def _fuzzy_align_extraction(
        ...
        extraction_tokens = list(_tokenize_with_lowercase(extraction.extraction_text, tokenizer_inst=tokenizer_impl))
    ...
    def align_extractions(
        ...
        for group_index, group in enumerate(extraction_groups):
            ...
            for extraction in group:
                ...
                extraction_text_tokens = list(_tokenize_with_lowercase(extraction.extraction_text, tokenizer_inst=tokenizer_impl))
        ...
        tokenized_text = tokenizer_impl.tokenize(source_text) if tokenizer_impl else tokenizer_lib.tokenize(source_text)
        ...
        for i, j, n in self._get_matching_blocks()[:-1]:
            ...
            if extraction is None:
                logging.debug(
                    "No clean start index found for extraction index=%d iterating Difflib matching_blocks",
                    j,
                )
                continue
            ...
            extraction_text_len = len(list(_tokenize_with_lowercase(extraction.extraction_text, tokenizer_inst=tokenizer_impl)))
            if extraction_text_len < n:
                raise ValueError(
                    "Delimiter prevents blocks greater than extraction length: " f"extraction_text_len={extraction_text_len}, block_size={n}"
                )
            if extraction_text_len == n:
