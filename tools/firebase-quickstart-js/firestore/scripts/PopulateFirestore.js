...
const signInWithAnonCredentials = async () => {
  try {
    await signInWithEmailAndPassword(auth, 'Anonymous@Anonymous.com', 'AnonymousPassword');
  } catch (userNotCreatedError) {
    await createUserWithEmailAndPassword(auth, 'Anonymous@Anonymous.com', 'AnonymousPassword');
  }
};
...
const addMockRestaurants = async (restaurntsArr) => {
  ...
  for (var i = 0; i < restaurntsArr.length; i++) {
    const promise = addFakeRestaurant(restaurntsArr[i]);
    if (!promise) {
...
const addMockRating = async (restaurantId, avgRating) => {
  ...
  await addDoc(collection(db, `restaurants/${restaurantId}/ratings`), newRating);
};
