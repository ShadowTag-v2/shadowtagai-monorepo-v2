...
const JsonViewer: React.FC<{ data: object }> = ({ data }) => {
  try {
    ...
    return <pre className={styles.jsonPre}>Error: Could not display JSON data.</pre>;
  }
};
  ...
  return (
    <div className={styles.responseContainer}>
      ...
      <div className={styles.responseOutput}>
        {/* Loading Overlay */}
        {showLoadingOverlay && <div className={styles.loadingIndicator}>Generating...</div>}

        {/* Main Response Text */}
        {/* Render the responseText if it exists, even during loading to support streaming output */}
        {responseText && <pre className={styles.responseText}>{responseText}</pre>}

        {/* Placeholder Text */}
        {showPlaceholder && <div className={styles.placeholder}>Output will appear here.</div>}
      </div>
      ...
      {responseJson && !isLoading && (
        <details className={styles.jsonDetails}>
          <summary className={styles.jsonSummary}>View Raw Response JSON</summary>
