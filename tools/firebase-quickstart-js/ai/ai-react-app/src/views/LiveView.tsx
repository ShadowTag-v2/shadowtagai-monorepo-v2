...
const LiveView: React.FC<LiveViewProps> = ({ aiInstance }) => {
  ...
  const [controller, setController] = useState<AudioConversationController | null>(null);
  ...
  useEffect(() => {
    return () => {
      if (controller) {
        console.log('[LiveView] Component unmounting, stopping active conversation.');
        controller.stop();
      }
    };
  }, [controller]);
  ...
  return (
    <div className={styles.liveViewContainer}>
      ...
      <p className={styles.instructions}>
        Click the button below to start a real-time voice conversation with the model. Your browser
        will ask for microphone permissions.
      </p>
      ...
      <button
        className={`${styles.controlButton} ${conversationState === 'active' ? styles.stop : ''}`}
        onClick={conversationState === 'active' ? handleStopConversation : handleStartConversation}
        disabled={false} // The button is never truly disabled, it just toggles state
      >
        {conversationState === 'active' ? 'Stop Conversation' : 'Start Conversation'}
      </button>
