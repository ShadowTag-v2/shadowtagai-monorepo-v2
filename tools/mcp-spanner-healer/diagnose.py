...
def run_diagnostic(dry_run: bool = True) -> str:
    ...
    for b in bottlenecks:
        status = "⚠️" if b.latency_ms > LATENCY_THRESHOLD_MS or b.lock_wait_ms > LOCK_WAIT_THRESHOLD_MS else "✅"
        print(f"  {status} {b.table_name}: {b.execution_plan} | {b.latency_ms}ms latency | {b.lock_wait_ms}ms lock wait")
