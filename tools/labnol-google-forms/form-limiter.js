...
function Initialize() {
  ...
  if (FORM_OPEN_DATE !== '' && new Date().getTime() < parseDate_(FORM_OPEN_DATE).getTime()) {
    closeForm();
    ScriptApp.newTrigger('openForm').timeBased().at(parseDate_(FORM_OPEN_DATE)).create();
  }

  if (FORM_CLOSE_DATE !== '') {
    ScriptApp.newTrigger('closeForm').timeBased().at(parseDate_(FORM_CLOSE_DATE)).create();
  }
