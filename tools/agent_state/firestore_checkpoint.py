...
class CheckpointStore:
    ...
    def __init__(self, project_id: str = "shadowtag-omega-v4", local_mode: bool = False):
        ...
        if not local_mode:
            try:
                ...
                print("[WARN] google-cloud-firestore not installed. Falling back to local mode.")
    ...
    def list_active(self, agent_id: str | None = None) -> list[AgentCheckpoint]:
        ...
        if self.local_mode:
            ...
            query = self._db.collection(self.COLLECTION).where("status", "in", ["running", "paused"])
            if agent_id:
    ...
    def advance(self, agent_id: str, task_id: str, new_state: dict[str, Any] | None = None) -> AgentCheckpoint | None:
...
def main():
    ...
    parser.add_argument("--local", action="store_true", help="Use local JSON store instead of Firestore")
