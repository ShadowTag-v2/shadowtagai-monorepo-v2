...
import { displayResults, handleRunTool } from './runTool.js';
 ...
 */
function createHeaderEditorModal(
  toolId,
  currentHeaders,
  toolParameters,
  authRequired,
...
export function renderToolInterface(tool, containerElement) {
  ...
  const updateCurrentHeaders = (newHeaders) => {
    ...
    const newModal = createHeaderEditorModal(
      TOOL_ID,
      currentHeaders,
      tool.parameters,
      tool.authRequired,
  ...
  const DISLCAIMER_INFO =
    '*Checked parameters are sent with the value from their text field. Empty fields will be sent as an empty string. To exclude a parameter, uncheck it.';
  ...
  const headerModal = createHeaderEditorModal(
    TOOL_ID,
    currentHeaders,
    tool.parameters,
    tool.authRequired,
