...
def _analyze_with_gca(diff: str, pr_number: int, mode: str) -> list[dict]:
    ...
    for i, line in enumerate(lines):
        ...
        if any(pattern in line.lower() for pattern in ["api_key =", "secret =", "password =", "token ="]):
...
def _verify_finding(finding: dict) -> bool:
    ...
    router_path = REPO_ROOT / "apps" / "aiyou_stack" / "aiyou-fastapi-services" / "zero_cpu_router.py"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="full-review", choices=["full-review", "quick-scan", "security-only"])
