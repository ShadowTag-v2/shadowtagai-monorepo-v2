...
ANE_BRIDGE_PATH = Path(__file__).parent / "ane_bridge.py"  # or absolute: /path/to/apps/aiyou_stack/aiyou-fastapi-services/ane_bridge.py
...
def dispatch_compute(
    ...
    if (
        ...
        try:
            ...
            return {"status": "error", "source": "ANE_EDGE", "error": "ANE task timed out after 300s"}
    ...
    # ------------------------------------------------------------------
        ...
        while not outbox_path.exists():
            if time.time() - start > timeout:
                return {"status": "error", "source": "COLAB_CLOUD", "error": "Timeout waiting for Colab result"}
            time.sleep(0.5)
        ...
        try:
            ...
            return {"status": "error", "source": "COLAB_CLOUD", "error": f"Failed to parse Colab result: {e}"}
...
def main():
    ...
    parser.add_argument("code_file", help="Path to Python file containing the compute payload")
    parser.add_argument("--estimate-bytes", type=int, default=None, help="Override byte estimate")
    parser.add_argument("--force-colab", action="store_true", help="Force Tier 2 Colab even if small")
