...
def launch_local_llm_stack() -> bool:
  ...
  try:
    ...
    print(
      "❌ Harbor not found. Please install it first (https://github.com/harbor/harbor or brew)."
    )
    return False
...
def launch_claurst(extra_args: List[str] = None) -> None:
  ...
  print(
    f"   Environment: ANTIGRAVITY_MODE=sovereign, ZERO_CPU_ROUTER_PATH={ZERO_CPU_ROUTER}"
  )
...
def main():
  ...
  parser.add_argument(
    "--launch-claurst",
    action="store_true",
    help="Launch Claurst with Antigravity hooks",
  )
  parser.add_argument(
    "--with-gemma", action="store_true", help="Also start local Gemma-4-31B stack"
  )
  parser.add_argument(
    "--example-pr-review",
    type=int,
    metavar="PR_NUMBER",
    help="Run example sovereign PR review",
  )
  parser.add_argument(
    "--setup-only",
    action="store_true",
    help="Only set up environment and IPC dirs, do not launch",
  )
  ...
  if not any(
    [args.launch_claurst, args.with_gemma, args.example_pr_review, args.setup_only]
  ):
    print(__doc__)
    print(
