...
def analyze_large_files(threshold: int = 800, max_results: int = 25) -> List[Tuple[str, int]]:
    ...
    for base in search_dirs:
        ...
        for file_path in base_path.rglob("*"):
            if file_path.is_file() and not is_excluded(file_path):
                if file_path.suffix.lower() in {".py", ".ts", ".tsx", ".js", ".jsx", ".go", ".rs"}:
...
def main():
    ...
    print("**Monorepo:** shadowtagai-monorepo-v2  |  **Scope:** apps/ libs/ packages/ tools/ infra/\n")

    if any(kw in query for kw in ["large", "800", "loc", "monolithic", "chunk", "split", "module"]):
        print("## 📏 Large Files (>800 LOC) — Modularization Candidates\n")
        large = analyze_large_files()
        if large:
            for f, loc in large:
                suggestion = "Split into /components/ + /hooks/ + /utils/" if "tsx" in f or "jsx" in f else "Extract service layer + domain models"
        ...
        print(f"- Recommended next action: `python tools/scripts/ui_consistency_auditor.py`")

    print("\n---\n*HUD Query Memory v10X — part of 10X Vibe Coding Matrix*")
    print("*Tip: Pass any natural language query, e.g. `python hud_query_memory.py \"find all Python services over 500 lines\"`*")
