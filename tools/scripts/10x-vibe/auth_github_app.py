...
def main():
  ...
  parser.add_argument(
    "--renew-token",
    action="store_true",
    help="Mint and cache new token (stdout or /tmp)",
  )
  parser.add_argument(
    "--dry-run", action="store_true", help="Show what would be pushed without executing"
  )
  parser.add_argument(
    "--print-token",
    action="store_true",
    help="Print token (disabled by default for security)",
  )
