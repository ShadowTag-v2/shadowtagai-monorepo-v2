...
class AgentDelegatePoC:
  ...
  def delegate_task(
    self, task_description: str, tools_allowed: list = None, timeout_seconds: int = 30
  ) -> str:
    """
    ...
    logger.info(
      f"Spawning sub-agent {sub_agent_id} for task: {task_description[:50]}..."
    )

    self.sub_agents[sub_agent_id] = {
      "status": "running",
      "task": task_description,
      "tools": tools_allowed or ["view_file", "grep_search"],
    }

    future = self.executor.submit(
      self._simulate_sub_agent_execution, sub_agent_id, task_description, tools_allowed
    )
    ...
    return json.dumps(
      {"sub_agent_id": sub_agent_id, "status": status, "report": result}
    )
...
if __name__ == "__main__":
  ...
  while True:
    try:
      ...
      tools_input = input(
        "Enter allowed tools (comma separated, default: view_file,grep_search): "
      ).strip()
      if tools_input:
