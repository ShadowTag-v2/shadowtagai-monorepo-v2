...
import { getFormState, getFormValue, mixinFormAssociated } from './form-associated.js';

describe('mixinFormAssociated()', () => {
  @customElement('test-form-associated')
  class TestFormAssociated extends mixinFormAssociated(mixinElementInternals(LitElement)) {
    @property() value = '';
  ...
  function setupTestWithoutForm() {
    const root = env.render(html`<test-form-associated></test-form-associated>`);

    const element = root.querySelector('test-form-associated') as TestFormAssociated;

    return { root, element };
  }

  function setupTestWithForm() {
    const root = env.render(html`<form><test-form-associated></test-form-associated></form>`);
  ...
  it('should add `static formAssociated = true`', () => {
    ...
    expect((element.constructor as typeof TestFormAssociated).formAssociated).toBeTrue();
  });
  ...
  describe('name', () => {
    ...
    it('should set name attribute synchronously', () => {
      const { element } = setupTestWithoutForm();
      element.name = 'foo';
      expect(element.getAttribute('name')).withContext('name attribute value').toBe('foo');
    });

    it('should add empty name attribute if set to an empty string', () => {
      const { element } = setupTestWithoutForm();
      element.name = '';
      expect(element.hasAttribute('name')).withContext('has name attribute').toBeTrue();
      expect(element.getAttribute('name')).withContext('name attribute value').toBe('');
    });
  ...
  describe('disabled', () => {
    ...
    it('should set disabled attribute synchronously', () => {
      ...
      expect(element.hasAttribute('disabled')).withContext('has disabled attribute').toBeTrue();
    });
    ...
    it('should remove attribute if set to false', () => {
      ...
      expect(element.hasAttribute('disabled')).withContext('has disabled attribute').toBeFalse();
    });
  ...
  describe('[getFormState]()', () => {
    it('should return form value by default', () => {
      ...
      expect(element[getFormState]()).withContext('[getFormState]() return').toBe('value');
    });
  ...
  describe('formDisabledCallback()', () => {
    it('should set `element.disabled` to the provided value', () => {
      ...
      expect(element.disabled).withContext('element.disabled after callback with true').toBeTrue();
