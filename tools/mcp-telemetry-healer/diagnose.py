...
def run_diagnostic() -> DiagnosticReport:
    ...
    for r in l1_results:
        icon = {"healthy": "✅", "degraded": "⚠️", "down": "🔴", "unknown": "❓"}.get(r.status, "❓")
    ...
    sample_logs = [
        {"message": "Client network socket disconnected before secure TLS handshake", "severity": "ERROR"},
        {"message": "ECONNRESET on connection to developerknowledge.googleapis.com", "severity": "ERROR"},
        {"message": "ECONNRESET on connection to developerknowledge.googleapis.com", "severity": "ERROR"},
        {"message": "DEADLINE_EXCEEDED: Cloud Spanner query timeout after 30s", "severity": "WARNING"},
        {"message": "Request completed successfully", "severity": "INFO"},
        {"message": "TLS handshake timeout connecting to remote MCP endpoint", "severity": "ERROR"},
    ]
    ...
    audit_path = Path(__file__).parent.parent.parent / ".beads" / "telemetry_diagnostics.jsonl"
