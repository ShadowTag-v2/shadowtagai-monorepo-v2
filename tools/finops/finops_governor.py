...
def generate_decisions(costs: list[ServiceCost], enforce: bool = False) -> list[GovernorDecision]:
...
def run_governor(enforce: bool = False) -> GovernorReport:
    ...
    for c in costs:
        icon = {"OK": "✅", "WARNING": "⚠️", "CRITICAL": "🔴", "CIRCUIT_BREAK": "🚨"}.get(c.alert_level, "❓")
        print(f"  {icon} {c.service}: ${c.current_usd:.2f} / ${c.budget_usd:.2f} ({c.utilization_pct:.0%})")
