...
export class GeminiProvider extends BaseProvider {
  ...
  async executeStreaming(
    ...
    const promptFile = join(workDir, ".gemini-3.1-familympt.txt");
    ...
    const command = "sh";
    const shellArgs = ["-c", `cat "${promptFile}" | ${this.cliCommand} ${geminiArgs.join(" ")}`];

    const outputLines: string[] = [];
    let sessionId: string | undefined;

    try {
      const { exitCode } = await execCommandStreaming(command, shellArgs, workDir, (line) => {
      ...
      const { response, inputTokens, outputTokens } = parseStreamJsonResult(fullOutput);
