...
export class DiffViewDialog {
  ...
  constructor(renderer: CliRenderer, events: DiffViewDialogEvents) {
    ...
    this.fileSelect.on(SelectRenderableEvents.SELECTION_CHANGED, (index: number) => {
      this.currentFileIndex = index;
      this.loadFileDiff();
    });

    this.fileSelect.on(SelectRenderableEvents.ITEM_SELECTED, (index: number) => {
      this.currentFileIndex = index;
      this.loadFileDiff();
    });
  }
  ...
  private async loadChangedFiles() {
    ...
    try {
      ...
      const options: SelectOption[] = this.changedFiles.map((file) => {
        ...
        const stats =
          file.additions > 0 || file.deletions > 0 ? `+${file.additions} -${file.deletions}` : '';
        return {
