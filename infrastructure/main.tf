/**
 * Corrected Terraform Configuration for Cor.Claude_Code_6 Infrastructure
 *
 * FIXES APPLIED:
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "Cor.Claude_Code_6/terraform/state"
  }
}
...
# Cor.Claude_Code_6 service account for GKE workloads
resource "google_service_account" "Cor.Claude_Code_6_sa" {
  account_id   = "Cor.Claude_Code_6"
  display_name = "Cor.Claude_Code_6 Service Account"
  description  = "Service account for Cor.Claude_Code_6 inference workloads"
  project      = var.project_id
...
resource "google_service_account" "workbench_sa" {
  account_id   = "Cor.Claude_Code_6-workbench"
  display_name = "Cor.Claude_Code_6 Workbench Service Account"
...
# IAM bindings for Cor.Claude_Code_6 service account
resource "google_project_iam_member" "Cor.Claude_Code_6_vertex_user" {
  project = var.project_id
  role    = "roles/aiplatform.user"
  member  = "serviceAccount:${google_service_account.Cor.Claude_Code_6_sa.email}"
}

resource "google_project_iam_member" "Cor.Claude_Code_6_storage_object_viewer" {
  project = var.project_id
  role    = "roles/storage.objectViewer"
  member  = "serviceAccount:${google_service_account.Cor.Claude_Code_6_sa.email}"
}

resource "google_project_iam_member" "Cor.Claude_Code_6_logging_writer" {
  project = var.project_id
  role    = "roles/logging.logWriter"
  member  = "serviceAccount:${google_service_account.Cor.Claude_Code_6_sa.email}"
}

resource "google_project_iam_member" "Cor.Claude_Code_6_monitoring_writer" {
  project = var.project_id
...
resource "google_artifact_registry_repository" "Cor.Claude_Code_6" {
  provider      = google-beta
  location      = var.region
  repository_id = "Cor.Claude_Code_6"
  description   = "Docker repository for Cor.Claude_Code_6 container images"
  format        = "DOCKER"
...
# Grant Cor.Claude_Code_6 SA permission to pull images
resource "google_artifact_registry_repository_iam_member" "Cor.Claude_Code_6_reader" {
  provider   = google-beta
  location   = google_artifact_registry_repository.Cor.Claude_Code_6.location
  repository = google_artifact_registry_repository.Cor.Claude_Code_6.name
...
resource "google_vertex_ai_featurestore" "Cor.Claude_Code_6_featurestore" {
  provider = google-beta
  name     = "Cor.Claude_Code_6_featurestore"
  region   = var.region
  project  = var.project_id
...
resource "google_vertex_ai_endpoint" "Cor.Claude_Code_6_gemini" {
  provider     = google-beta
  name         = "Cor.Claude_Code_6-gemini"
  display_name = "Cor.Claude_Code_6 Gemini Endpoint"
  description  = "Endpoint for Cor.Claude_Code_6 Gemini model inference"
  region       = var.region
  project      = var.project_id
...
resource "google_notebooks_instance" "Cor.Claude_Code_6_workbench" {
  provider     = google-beta
  name         = "Cor.Claude_Code_6-workbench"
...
resource "google_service_account_iam_member" "Cor.Claude_Code_6_workload_identity" {
  service_account_id = google_service_account.Cor.Claude_Code_6_sa.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "serviceAccount:${var.project_id}.svc.id.goog[Cor.Claude_Code_6-system/Cor.Claude_Code_6-sa]"

  depends_on = [google_service_account.Cor.Claude_Code_6_sa]
}
...
output "Cor.Claude_Code_6_service_account_email" {
  description = "Cor.Claude_Code_6 service account email"
...
output "artifact_registry_repository" {
  ...
  value       = google_artifact_registry_repository.Cor.Claude_Code_6.name
}
...
output "vertex_endpoint_id" {
  ...
  value       = google_vertex_ai_endpoint.Cor.Claude_Code_6_gemini.id
}
