...
class TestBudgetStatus:
  ...
  def test_yellow_at_warn(self):
    with (
      patch.object(gov, "query_current_spend", return_value=400.0),
  ...
  def test_red_over_halt(self):
    with (
      ...
      patch.object(gov, "publish_alert"),
      patch.object(
        gov, "scale_down_services", return_value=["database-events-handler"]
      ),
    ):
      report = gov.evaluate_budget()
  ...
  def test_green_no_alert(self):
    with (
      patch.object(gov, "query_current_spend", return_value=50.0),
  ...
  def test_zero_budget_no_division_error(self):
    with (
      patch.object(gov, "BUDGET_MONTHLY_USD", 0),
      patch.object(gov, "query_current_spend", return_value=0.0),
    ):
      report = gov.evaluate_budget()
