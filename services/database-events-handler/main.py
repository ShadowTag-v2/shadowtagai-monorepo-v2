...
def _route_to_healing(
  event: dict[str, Any], publisher: pubsub_v1.PublisherClient
) -> None:
  ...
  logger.info(
    "Forwarded healing request for %s: %s", table_name, future.result(timeout=10)
  )


def _route_payment_event(
  event: dict[str, Any], publisher: pubsub_v1.PublisherClient
) -> None:
  ...
  if event.get("changeType") == "DELETE":
    logger.warning(
      "DELETE on payment table %s — logging only, no Stripe action", table_name
    )
    return
  ...
  logger.info(
    "Forwarded payment event for %s: %s", table_name, future.result(timeout=10)
  )
...
def _run_test():
  ...
  print(
    f"Is schema-critical: {event.get('tableName', '').lower() in SCHEMA_CRITICAL_TABLES}"
  )
