...
def _phase_heal(diagnosis: dict) -> tuple[HealingAction, list[str]]:
  ...
  for rec in diagnosis["recommendations"]:
    ...
    if rec_type == "QUERY_REWRITE":
      actions_taken.append(f"QUERY_REWRITE: Suggested for {rec.get('table', '?')}")
  ...
  try:
    ...
    memory_event = {
      ...
      "diagnosis_summary": {
        ...
        "recommendation_count": len(result.diagnosis.get("recommendations", [])),
      },
    ...
    future = publisher.publish(topic_path, json.dumps(memory_event).encode("utf-8"))
    logger.info(
      "Documented pipeline run to Epistemic Engine: %s", future.result(timeout=10)
    )
