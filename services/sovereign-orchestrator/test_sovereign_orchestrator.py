...
def _make_healing_request(table: str = "users", change_type: str = "INSERT") -> orch.HealingRequest:
...
class TestHealPhase:
    ...
    def test_index_recommendation_detected(self):
        diagnosis = {
            "table": "users",
            "recommendations": [{"type": "INDEX_RECOMMENDATION", "reason": "slow query", "table": "users"}],
        }
        action, actions = orch._phase_heal(diagnosis)
...
class TestFullPipeline:
    ...
    def test_red_budget_aborts_pipeline(self):
        ...
        with (
            patch.object(orch, "_phase_diagnose", return_value={"table": "users", "health": "DEGRADED", "recommendations": []}),
    ...
    def test_heal_rate_limit_aborts(self):
        ...
        with (
            patch.object(
                orch,
                "_phase_diagnose",
                return_value={"table": "transactions", "health": "DEGRADED", "recommendations": [{"type": "INDEX_RECOMMENDATION"}]},
            ),
...
class TestHTTPHandler:
    ...
    def test_returns_200_on_success(self):
        ...
        with (
            patch.object(orch, "_phase_diagnose", return_value={"table": "users", "health": "HEALTHY", "recommendations": []}),
            patch.object(orch, "_phase_budget_check", return_value="GREEN"),
            patch.object(orch, "_phase_document"),
        ):
    ...
    def test_handles_pubsub_envelope(self):
        ...
        with (
            patch.object(orch, "_phase_diagnose", return_value={"table": "transactions", "health": "HEALTHY", "recommendations": []}),
            patch.object(orch, "_phase_budget_check", return_value="GREEN"),
            patch.object(orch, "_phase_document"),
        ):
            response, status = orch.handle_healing_request(request)
