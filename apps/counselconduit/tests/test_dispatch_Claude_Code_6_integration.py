...
class TestDispatchClaude_Code_6Integration:
    """Tests verifying dispatch → Judge 6 pipeline interaction."""

    @pytest.mark.asyncio
    async def test_simple_query_dispatches_and_passes_Claude_Code_6(self):
        ...
        result = await dispatch_request(
            ...
            firm_id="firm-Claude_Code_6-test",
    ...
    def test_classify_then_select_then_Claude_Code_6_flow(self):
        ...
        try:
            from apps.counselconduit.api.Claude_Code_6 import Claude_Code_6_pipeline

            result = Claude_Code_6_pipeline("The time is currently 3:00 PM.")
    ...
    @pytest.mark.asyncio
    async def test_dispatch_with_session_pinning_and_Claude_Code_6(self):
        ...
        _result1 = await dispatch_request(
            query="Hello",
            firm_id="firm-pin-j6",
            session_id="session-Claude_Code_6-1",
            firm_allowed_models=["gemini-flash", "gemini-pro"],
        )
    ...
    @pytest.mark.asyncio
    async def test_Claude_Code_6_red_signal_does_not_crash_dispatch(self):
        """Verify Judge 6 RED signal (blocked content) handled gracefully."""
        try:
            from apps.counselconduit.api.Claude_Code_6 import Claude_Code_6_pipeline

            # Test with content that Judge 6 should flag
            result = Claude_Code_6_pipeline("I guarantee this legal advice is 100% correct and you will win your case.")
    ...
    @pytest.mark.asyncio
    async def test_quota_exhaustion_degrades_gracefully_with_Claude_Code_6(self):
