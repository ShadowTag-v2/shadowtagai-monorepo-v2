...
class TestCor_Claude_Code_6Pipeline:
  ...
  def test_clean_output_passes(self):
    result = evaluate(
      "The court held in Smith v. Jones that qualified immunity applies."
    )
  ...
  def test_absolute_guarantee_blocked(self):
    result = evaluate(
      "I guarantee you will win this case. It is certainly going to succeed."
    )
    assert result.assessment.risk_level == RiskLevel.RED
    assert result.assessment.approved is False
    assert (
      "guarantee" in result.output_text.lower()
      or "blocked" in result.output_text.lower()
    )
