...
class TestStripeIDFormats:
    ...
    def test_product_id_format(self, product_id: str, name: str) -> None:
        ...
        assert self._PRODUCT_RE.match(product_id), f"{name} product ID '{product_id}' does not match expected format"
    ...
    def test_price_id_format(self, price_id: str, name: str) -> None:
        ...
        assert self._PRICE_RE.match(price_id), f"{name} price ID '{price_id}' does not match expected format"

    def test_webhook_endpoint_id_format(self) -> None:
        """Webhook endpoint ID must match Stripe's we_ prefix format."""
        assert self._WEBHOOK_RE.match(WEBHOOK_ENDPOINT_ID), f"Webhook ID '{WEBHOOK_ENDPOINT_ID}' does not match expected format"

    def test_portal_config_id_format(self) -> None:
        """Portal config ID must match Stripe's bpc_ prefix format."""
        assert self._PORTAL_RE.match(PORTAL_CONFIG_ID), f"Portal ID '{PORTAL_CONFIG_ID}' does not match expected format"
...
class TestTierTokenLimits:
    ...
    def test_professional_middle(self) -> None:
        ...
        assert TIER_TOKEN_LIMITS["trial"] < TIER_TOKEN_LIMITS["professional"] < TIER_TOKEN_LIMITS["enterprise"]
