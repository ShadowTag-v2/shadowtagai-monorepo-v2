...
export default function () {
  ...
  const payload = JSON.stringify({
    query:
      "Analyze the privilege implications under Upjohn v. United States for in-house counsel communications.",
    firm_id: `k6-firm-${__VU}`,
    session_id: `k6-session-${__VU}-${__ITER}`,
  });

  const dispatch = http.post(`${BASE_URL}/api/v1/dispatch`, payload, {
    headers: { "Content-Type": "application/json" },
  });
  dispatchDuration.add(dispatch.timings.duration);

  const dispatchOk = check(dispatch, {
    "dispatch status 200": (r) => r.status === 200,
    "dispatch has model": (r) => {
      try {
        return JSON.parse(r.body).model !== undefined;
      } catch {
        return false;
      }
    },
    "dispatch has tier": (r) => {
      try {
        return ["simple", "complex", "agentic"].includes(JSON.parse(r.body).tier);
      } catch {
        return false;
      }
    },
