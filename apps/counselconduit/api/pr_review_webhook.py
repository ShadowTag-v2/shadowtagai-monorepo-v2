...
def _verify_github_signature(payload: bytes, signature: str) -> bool:
  ...
  if not GITHUB_WEBHOOK_SECRET:
    logger.warning(
      "GITHUB_WEBHOOK_SECRET not set — skipping signature check (dev mode)"
    )
    return True
  expected = (
    "sha256="
...
def _run_logic_verification(changed_files: list[str]) -> list[dict]:
  ...
  for f in python_files:
    ...
    if "/tests/" in f:
      ...
      test_path = f.replace("/api/", "/tests/test_").replace(
        "/services/", "/tests/test_"
      )
...
async def run_swarm(pr_number: int, branch: str, changed_files: list[str]) -> None:
  ...
  gpu_files = [
    f
    for f in changed_files
    if any(kw in f for kw in ["cuda", "gpu", "colab", "training", "fine_tune"])
  ]
  if gpu_files:
    all_findings.append(
      {
        "severity": "🟡 Nit",
        "message": (
          f"GPU-bound changes detected. Colab T4 notebook generated at: labs/uphillsnowball/notebooks/pr_review_{pr_number}.ipynb"
        ),
      }
    )
...
async def _post_review_comments(pr_number: int, findings: list[dict]) -> None:
  ...
  try:
    ...
    spec = importlib.util.spec_from_file_location(
      "auth_github_app",
      os.path.join(
        os.path.dirname(__file__), "..", "..", "..", "scripts", "auth_github_app.py"
      ),
    )
    ...
    logic_ran = (
      any("Test" in str(f) or "Logic" not in str(f) for f in findings) or not critical
    )
