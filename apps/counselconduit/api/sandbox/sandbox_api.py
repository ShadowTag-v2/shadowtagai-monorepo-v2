...
async def get_session_diffs(
  ...
  try:
    ...
    diffs = bridge.compute_diffs(
      ...
      privilege_map=session.config.privilege_map
      if hasattr(session.config, "privilege_map")
      else None,
    )
...
async def commit_session(
  ...
  try:
    ...
    new_state = (
      SessionState.COMMITTED if action != CommitAction.REJECT else SessionState.REJECTED
    )
    try:
