...
class MetricsRegistry:
    ...
    def record_latency(self, operation: str, duration_ms: float, *, error: bool = False) -> None:
    ...
    def render_prometheus(self) -> str:
        ...
        with self._lock:
            ...
            lines.append("# HELP sandbox_store_operation_duration_ms Operation duration in milliseconds")
            lines.append("# TYPE sandbox_store_operation_duration_ms summary")
            for op in sorted(self._invocations.keys()):
                samples = self._durations.get(op, [])
                pcts = _compute_percentiles(samples)
                labels = f'operation="{op}"'
                lines.append(f'sandbox_store_operation_duration_ms{{quantile="0.5",{labels}}} {pcts["p50_ms"]}')
                lines.append(f'sandbox_store_operation_duration_ms{{quantile="0.95",{labels}}} {pcts["p95_ms"]}')
                lines.append(f'sandbox_store_operation_duration_ms{{quantile="0.99",{labels}}} {pcts["p99_ms"]}')
                lines.append(f"sandbox_store_operation_duration_ms_count{{{labels}}} {self._invocations[op]}")
                lines.append(f"sandbox_store_operation_duration_ms_sum{{{labels}}} {pcts['sum_ms']}")
            lines.append("")
            ...
            for op in sorted(self._errors.keys()):
                lines.append(f'sandbox_store_operation_errors_total{{operation="{op}"}} {self._errors[op]}')
            lines.append("")
