...
class FirestoreSessionStore:
  ...
  async def _write_overlay_files(
    self, session_id: str, overlay_files: dict[str, str]
  ) -> None:
    """Write overlay files as sub-collection documents.
  ...
  async def _read_overlay_files(self, session_id: str) -> dict[str, str]:
    ...
    return {
      doc.to_dict().get("file_path", ""): doc.to_dict().get("content", "")
      for doc in docs
    }
