...
AVAILABLE_MODELS: dict[str, ModelConfig] = {
  ...
  "gemini-pro": ModelConfig(
    provider=ModelProvider.GEMINI,
    model_id="gemini-3.1-flash-lite-preview-thinking",
    display_name="Gemini Flash-Lite Thinking (Balanced)",
...
def get_pinned_model(session_id: str) -> str | None:
  ...
  if time.time() - pinned_at > SESSION_PIN_TTL_SECONDS:
    ...
    logger.info(
      "Session pin expired for %s (TTL=%ds)", session_id, SESSION_PIN_TTL_SECONDS
    )
    return None
  return model_key
...
def select_model(req: ModelRequest) -> ModelConfig:
  ...
  for candidate in tier_candidates:
    ...
    if (
      fallback and fallback in req.firm_allowed_models and fallback in AVAILABLE_MODELS
    ):
...
async def dispatch_request(
  ...
  if any(
    sig in model.model_id.lower() for sig in _NON_REASONING_SIGNALS
  ) and not model.model_id.lower().endswith("-thinking"):
