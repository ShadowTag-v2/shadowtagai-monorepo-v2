...
class DispatchRequest(BaseModel):
  ...
  session_id: str = Field(
    default="", max_length=128, description="Session ID for model pinning"
  )
  preferred_model: str | None = Field(
    default=None, description="Override auto-routing with specific model"
  )
...
def _record_circuit_error() -> None:
  ...
  if _circuit_state["errors"] >= CIRCUIT_BREAKER_THRESHOLD:
    ...
    logger.warning(
      "Circuit breaker OPEN — %d errors in window", _circuit_state["errors"]
    )
...
def cleanup_expired_session_pins() -> int:
  ...
  now = time.time()
  expired = [
    sid for sid, (_, ts) in _session_pins.items() if now - ts > SESSION_PIN_TTL_SECONDS
  ]
  for sid in expired:
...
async def dispatch_endpoint(
  ...
  try:
    ...
    if body.firm_id:
      ...
      asyncio.create_task(
        track_token_usage(
          body.firm_id, x_user_tier, result.get("estimated_tokens", 100)
        )
      )
...
async def provider_health(
  ...
  for model_key, model_cfg in AVAILABLE_MODELS.items():
    ...
    cb_status = (
      "open" if failures >= 5 else ("half-open" if failures >= 3 else "closed")
    )
    providers[model_key] = {
...
def record_provider_failure(model_key: str) -> None:
  ...
  if cb["failures"] >= 5:
    ...
    logger.warning(
      "Circuit breaker OPEN for %s (failures=%d)", model_key, cb["failures"]
    )
...
async def slo_compliance_report(
  ...
  return {
    ...
    "report": {
      ...
      "circuit_breakers": {
        k: v.get("failures", 0) for k, v in _circuit_breakers.items()
      },
    },
