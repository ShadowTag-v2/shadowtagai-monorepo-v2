...
async def store_matter(firm_id: str, matter: dict[str, Any]) -> str:
  ...
  doc_ref = (
    db.collection("firms").document(firm_id).collection("matters").document(matter_id)
  )
  await doc_ref.set(
    {
      **matter,
...
async def consume_matter_token(firm_id: str, matter_id: str) -> bool:
  ...
  await doc_ref.update(
    {"consumed": True, "_consumed_at": datetime.now(UTC).isoformat()}
  )
  return True
...
async def store_gdpr_request(firm_id: str, request: dict[str, Any]) -> str:
  ...
  doc_ref = (
    db.collection("firms").document(firm_id).collection("gdpr").document(request_id)
  )
  await doc_ref.set(
    {
      **request,
...
async def store_session(firm_id: str, session: dict[str, Any]) -> str:
  ...
  doc_ref = (
    db.collection("firms").document(firm_id).collection("sessions").document(session_id)
  )
  await doc_ref.set(
    {
      **session,
