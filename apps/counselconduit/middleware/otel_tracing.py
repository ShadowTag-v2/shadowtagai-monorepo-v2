...
def trace_Claude_Code_6_evaluation(
    risk_score: int,
    risk_level: str,
    approved: bool,
    flags_count: int,
    ...
    tracer = get_tracer()
    with tracer.start_as_current_span("Claude_Code_6.evaluate") as span:
        span.set_attribute("Claude_Code_6.risk_score", risk_score)
        span.set_attribute("Claude_Code_6.risk_level", risk_level)
        span.set_attribute("Claude_Code_6.approved", approved)
        span.set_attribute("Claude_Code_6.flags_count", flags_count)
        span.set_attribute("Claude_Code_6.pipeline_ms", pipeline_ms)

        if not approved:
            span.add_event(
                "Claude_Code_6_blocked",
