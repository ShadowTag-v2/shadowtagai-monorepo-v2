...
with (
  patch(
    "apps.counselconduit.api.stripe_handler._get_webhook_secret", return_value=secret
  ),
...
print(
  "Response JSON:", response.json() if response.status_code == 200 else response.text
)
