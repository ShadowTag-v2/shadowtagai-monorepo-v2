...
namespace TsubameViewer.Presentation.Views.EBookControls
{
    public sealed partial class EPubRenderer : UserControl
    {
        ...
        private void EPubRenderer_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            WindowsObservable.FromEventPattern<object, WindowSizeChangedEventArgs>(
                h => App.Current.Window.SizeChanged += h,
                h => App.Current.Window.SizeChanged -= h
                )
            ...
            .Skip(1)
            .Subscribe(_ => { var __ = dispatcher.TryEnqueue(() => ReloadPageHtml()); })
            .AddTo(_compositeDisposable);
        }
        ...
        private void WebView_WebResourceRequested(CoreWebView2 sender, CoreWebView2WebResourceRequestedEventArgs args)
        {
        ...
        private async void WebView_DOMContentLoaded(CoreWebView2 sender, CoreWebView2DOMContentLoadedEventArgs args)
        {
            ...
            {
                // 縦書きかをチェック
                var writingModeString = await WebView.ExecuteScriptAsync(@"
            ...
            if (IsVerticalLayout)
            {
                ...
                await WebView.ExecuteScriptAsync($"document.body.style = \"width: 100vw; overflow: hidden; max-height: {WebView.ActualHeight}px; column-count: {columnCount}; column-rule-width: 0px; font-size:{FontSize}px; \";");
            }
            else
            {
                // Note: -8は下側と右側の見切れ対策
                await WebView.ExecuteScriptAsync($"document.body.style = \"overflow: hidden; width:{WebView.ActualWidth - 8}; max-height:{WebView.ActualHeight - 8}px; column-count: {columnCount}; column-rule-width: 0px; font-size:{FontSize}px; \";");
            }
            ...
            //
            {
                ...
                var sizeList = await WebView.ExecuteScriptAsync(
                    $@"
                ...
                if (pageRealSize > heroPageHeight)
                {
                    ...
                    var pCount = int.Parse(await WebView.ExecuteScriptAsync("document.querySelectorAll('p').length.toString();"));
        ...
        private async Task SetScrollPositionAsync()
        {
            ...
            await WebView.ExecuteScriptAsync($"window.scrollTo({position}, 0);" );
        ...
        {
            var widthText = await WebView.ExecuteScriptAsync("Math.max(document.body.scrollWidth, document.documentElement.scrollWidth,document.body.offsetWidth, document.documentElement.offsetWidth,document.body.clientWidth, document.documentElement.clientWidth).toString();");
            return double.TryParse(widthText, out var value) ? value : 0;
        }

        private async Task<double> GetScrollableHeight()
        {
            var heightText = await WebView.ExecuteScriptAsync("Math.max(document.body.scrollHeight, document.documentElement.scrollHeight,document.body.offsetHeight, document.documentElement.offsetHeight,document.body.clientHeight, document.documentElement.clientHeight).toString();");
            return double.TryParse(heightText, out var value) ? value : 0;
        }

        private async Task<double> GetPageWidth()
        {
            var widthText = await WebView.ExecuteScriptAsync("window.innerWidth.toString()");
            return double.TryParse(widthText, out var value) ? value : 0;
        }

        private async Task<double> GetPageHeight()
        {
            var heightText = await WebView.ExecuteScriptAsync("window.innerHeight.toString()");
            return double.TryParse(heightText, out var value) ? value : 0;
        }

        private async Task<double> GetScrollLeft()
        {
            var XOffsetText = await WebView.ExecuteScriptAsync("window.pageXOffset.toString()");
        ...
        private async Task<double> GetScrollTop()
        {
            ...
            var scrollTop = await WebView.ExecuteScriptAsync("window.pageYOffset.toString()");
            return double.TryParse(scrollTop, out var value) ? value : 0;
        }

        private void WebView_WebResourceRequested(WebView2 sender, Microsoft.Web.WebView2.Core.CoreWebView2WebMessageReceivedEventArgs args)
        {
