...
{
    public sealed class OpenListupCommand : RelayCommandBase
    {
        ...
        protected override async void Execute(object parameter)
        {
            ...
            if (parameter is IImageSource imageSource)
            {
                ...
                if (type == StorageItemTypes.Archive)
                {
                    ...
                    var collectionContext = await _messenger.WorkWithBusyWallAsync(ct => _imageCollectionManager.GetArchiveImageCollectionContextAsync((imageSource as StorageItemImageSource).StorageItem as StorageFile, null, ct), ct);
                    try
                    {
