...
def git_log(repo: git.Repo, max_count: int = 10, start_timestamp: str | None = None, end_timestamp: str | None = None) -> list[str]:
    if start_timestamp or end_timestamp:
        ...
        for i in range(0, len(log_output), 4):
            if i + 3 < len(log_output) and len(log) < max_count:
                log.append(f"Commit: {log_output[i]}\nAuthor: {log_output[i + 1]}\nDate: {log_output[i + 2]}\nMessage: {log_output[i + 3]}\n")
        return log
    else:
        ...
        for commit in commits:
            log.append(f"Commit: {commit.hexsha!r}\nAuthor: {commit.author!r}\nDate: {commit.authored_datetime}\nMessage: {commit.message!r}\n")
        return log
...
def git_show(repo: git.Repo, revision: str) -> str:
    ...
    output = [f"Commit: {commit.hexsha!r}\nAuthor: {commit.author!r}\nDate: {commit.authored_datetime!r}\nMessage: {commit.message!r}\n"]
    if commit.parents:
...
def validate_repo_path(repo_path: Path, allowed_repository: Path | None) -> None:
    ...
    try:
        ...
        raise ValueError(f"Repository path '{repo_path}' is outside the allowed repository '{allowed_repository}'")
