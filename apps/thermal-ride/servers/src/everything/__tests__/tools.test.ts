...
import { EchoSchema, registerEchoTool } from '../tools/echo.js';
...
import { MCP_TINY_IMAGE, registerGetTinyImageTool } from '../tools/get-tiny-image.js';
  ...
  describe('gzip-file-as-resource', () => {
    it('should compress data URI and return resource link', async () => {
      ...
      (mockServer.registerTool as any).mockImplementation(
        (name: string, config: unknown, h: Function) => {
          handler = h;
        },
      );
