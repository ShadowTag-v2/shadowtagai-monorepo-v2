...
const DEMO_RECEIPT: KovelReceipt = {
  ...
  session_id: "sess_7b3e9a1f",
  hmac_hash:
    "HMAC-SHA256:a8f3b2c1d9e7f6a5b4c3d2e1f0a9b8c7d6e5f4a3b2c1d0e9f8a7b6c5d4e3f2",
...
const DEMO_MODELS: ModelConfig[] = [
  { id: "gemini-flash", name: "Gemini 3.1 Flash Lite", provider: "Google", enabled: true, tier: "primary" },
  { id: "claude-sonnet", name: "Claude Sonnet 4.5", provider: "Anthropic", enabled: true, tier: "fallback" },
  ...
  { id: "perplexity", name: "Perplexity Sonar", provider: "Perplexity", enabled: false, tier: "research" },
  { id: "deepseek-r1", name: "DeepSeek R1", provider: "DeepSeek", enabled: false, tier: "research" },
];

const DEMO_CLIENTS: ClientInfo[] = [
  { client_id: "cl_001", name: "Martinez Corp", email: "legal@martinez-corp.com", last_session: "2026-04-18", total_sessions: 15, status: "authorized" },
  { client_id: "cl_002", name: "Blackwell Industries", email: "jblackwell@blackwell-ind.com", last_session: "2026-04-18", total_sessions: 28, status: "authorized" },
  { client_id: "cl_003", name: "Chen & Associates", email: "mchen@chenlaw.co", last_session: "2026-04-17", total_sessions: 7, status: "authorized" },
  { client_id: "cl_004", name: "Rivera Holdings", email: "arivera@riverahold.com", last_session: "2026-04-16", total_sessions: 3, status: "pending" },
  { client_id: "cl_005", name: "Thompson Legal Group", email: "kthompson@tlg.law", last_session: "—", total_sessions: 0, status: "revoked" },
];
          ...
          <tbody>
            {DEMO_SESSIONS.slice(0, 3).map((s) => (
              <tr key={s.session_id}>
                ...
                <td><span className="badge badge-model">{s.model_used}</span></td>
                <td>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="tab-bar">
        ...
        <button className={`tab-btn ${tab === "receipt" ? "active" : ""}`} onClick={() => setTab("receipt")}>
          Kovel Receipt
        </button>
        <button className={`tab-btn ${tab === "memo" ? "active" : ""}`} onClick={() => setTab("memo")}>
          Oracle Memo
        </button>
      </div>
      ...
      {tab === "receipt" && (
        <div className="glass-panel receipt-card">
          ...
          <div className="receipt-field">
            ...
            <span className="receipt-value" style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem" }}>
      ...
      {tab === "memo" && (
        <div className="glass-panel oracle-memo">
          ...
          <p>
            The client inquiry concerns the federal tax treatment of a proposed merger between Martinez Corp
            and TechVenture Inc. Based on the facts presented, the transaction appears to qualify as a
            <span className="memo-citation"> IRC § 368(a)(1)(A)</span> tax-free reorganization.
          </p>

          <h3>Key Legal Framework</h3>
          <p>
            The governing authority for tax-free mergers is <span className="memo-citation">IRC § 368</span>,
            as interpreted by <span className="memo-citation">Treas. Reg. § 1.368-1</span> and relevant case law.
            Two primary doctrines must be satisfied:
          </p>
          <ol style={{ paddingLeft: "var(--space-lg)", color: "var(--text-secondary)" }}>
            <li style={{ marginBottom: "var(--space-sm)" }}>
              <strong>Continuity of Interest</strong> — Per <span className="memo-citation">Rev. Proc. 77-37</span>,
              at least 40% of consideration must be stock.
            </li>
            <li style={{ marginBottom: "var(--space-sm)" }}>
              <strong>Continuity of Business Enterprise</strong> — Under
              <span className="memo-citation"> Treas. Reg. § 1.368-1(d)</span>, the acquiring corporation must
              continue historic business or use a significant portion of historic assets.
            </li>
          </ol>

          <h3>NOL Carryforward Limitation</h3>
          <p>
            Post-merger NOL utilization is subject to <span className="memo-citation">IRC § 382</span> annual
            limitations. The current long-term tax-exempt rate of approximately 5.2% (April 2026) would cap annual
            usage at the equity value of the loss corporation multiplied by this rate.
          </p>

          <h3>Recommended Actions</h3>
          <ol style={{ paddingLeft: "var(--space-lg)", color: "var(--text-secondary)" }}>
            <li style={{ marginBottom: "var(--space-sm)" }}>Confirm consideration structure achieves ≥40% stock ratio</li>
            <li style={{ marginBottom: "var(--space-sm)" }}>Obtain § 382 valuation from independent appraiser</li>
            <li style={{ marginBottom: "var(--space-sm)" }}>Review step transaction risk for any planned post-merger dispositions</li>
            <li>Coordinate with M&amp;A counsel on SRLY rules for consolidated return filing</li>
          </ol>

          <div style={{ marginTop: "var(--space-lg)", padding: "var(--space-md)", background: "var(--accent-gold-dim)", borderRadius: "var(--radius-sm)", borderLeft: "3px solid var(--accent-gold)" }}>
            <strong style={{ color: "var(--accent-gold)", fontSize: "0.8rem" }}>PRIVILEGE NOTICE</strong>
            <p style={{ fontSize: "0.8rem", marginTop: "var(--space-xs)" }}>
              This memo is generated within a Kovel-privileged session and constitutes attorney work product
              protected under <em>United States v. Heppner</em> (S.D.N.Y., Feb. 10, 2026). Do not distribute
          ...
          <tbody>
            {DEMO_SESSIONS.map((s) => (
              <tr key={s.session_id} onClick={() => setSelectedSession(s)}>
                ...
                <td style={{ fontFamily: "var(--font-mono)", fontSize: "0.73rem", color: "var(--accent-gold)" }}>
    ...
    <>
      <div className="section-header">
        <div>
          ...
          <div className="section-subtitle">Authorized clients for privileged AI research sessions</div>
        </div>
          ...
          <div className="glass-panel client-card animate-fade-in" key={c.client_id}>
            ...
            <div className="client-info" style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: "var(--space-sm)", marginTop: 6 }}>
                <span className={`badge ${c.status === "authorized" ? "badge-active" : c.status === "pending" ? "badge-completed" : "badge-expired"}`}>
                  {c.status}
                </span>
                <span style={{ fontSize: "0.73rem", color: "var(--text-muted)" }}>
    ...
    <>
      <div className="section-header">
        <div>
          ...
          <div className="section-subtitle">
            Control which LLM providers are available to clients. All routing passes through Judge #6.
          </div>
        </div>
      </div>
        ...
        {models.map((m) => (
          ...
          >
            ...
            <div style={{ marginTop: "var(--space-sm)" }}>
              <span className={`badge ${m.tier === "primary" ? "badge-active" : m.tier === "fallback" ? "badge-completed" : "badge-privileged"}`}>
                {m.tier}
              </span>
            </div>
            <div
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "var(--space-md)" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "0.73rem", color: "var(--text-tertiary)" }}>
              All model routing decisions pass through ATP 5-19 policy evaluation.
              Hallucination circuit breaker (RKILL) is armed.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="detail-grid">
        ...
        <div className="glass-panel animate-fade-in">
          ...
          <div className="receipt-field">
            ...
            <span className="receipt-value" style={{ fontWeight: 700, color: "var(--success)" }}>$1,240.00</span>
          </div>
          <div className="receipt-field">
...
export default function App() {
  ...
  const viewTitle: Record<View, { title: string; sub: string }> = {
    overview: { title: "Attorney Dashboard", sub: "CounselConduit — Privilege-Preserving AI Research" },
    sessions: { title: "Sessions", sub: "Client research sessions with Kovel privilege protection" },
