...
export const GavelHero: React.FC = () => {
  ...
  if (authLoading)
    return <div style={{ color: theme.colors.onSurface }}>Authenticating Lakeport Core...</div>;
  if (!user)
    return (
      <div style={{ color: theme.colors.error }}>Identity required. Firebase Auth pending.</div>
    );
  if (!isAuthorized)
    return (
      <div style={{ color: theme.colors.error }}>
        Spanner Ledger Denies Access. Awaiting Realtime CDC.
      </div>
    );
  ...
  return (
    ...
    >
      {!loaded && (
        <div
          style={{ color: theme.colors.onSurface, fontFamily: theme.typography.displayLarge }}
          className="absolute animate-pulse"
        >
          Loading V19 Relay OS...
        </div>
      )}
      <canvas
        ref={canvasRef}
      ...
      <button
        ...
        className="absolute bottom-10 px-6 py-2 shadow-md"
      >
        Execute Archon-Boot
      </button>
    </div>
