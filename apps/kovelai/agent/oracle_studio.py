...
class OracleStudio:
    ...
    def _classify_argument(self, text: str) -> str:
        ...
        if any(w in text_lower for w in ["court", "motion", "filing", "hearing", "procedur"]):
            return "procedural"
        if any(w in text_lower for w in ["policy", "interest", "purpose", "intent", "legislative"]):
            return "policy"
        return "factual"
    ...
    def _audit_assumptions(self, arguments: list[Argument], text: str) -> list[Assumption]:
    ...
    def _filter_relevance(
        ...
        for arg in arguments:
            ...
            critical_assumptions = [a for a in related_assumptions if a.risk_level == "critical"]
    ...
    def _steelman_challenge(
        ...
        for arg in arguments:
            ...
            if composite >= 0.7 and len(attacks) <= 1:
                ...
                assessment = "WEAKENED — argument has exploitable vulnerabilities but may survive"
            else:
    ...
    def _build_permanent_notes(
        ...
        summary_content = f"**Murder Board Summary** — {len(arguments)} arguments analyzed\n\n"
