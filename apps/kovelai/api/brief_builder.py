...
class BriefMetadata(BaseModel):
  ...
  privilege_designation: str = (
    "Attorney Work-Product — Not Subject to Compulsory Disclosure"
  )
  pages: int = 0
...
def _build_citation_appendix(citations: list[dict]) -> str:
  ...
  for citation in citations:
    ...
    lines.append(
      f"| {citation.get('index', '')} | {citation.get('authority', '')} | {citation.get('type', '')} | {rel} | {icon} {status} |"
    )
