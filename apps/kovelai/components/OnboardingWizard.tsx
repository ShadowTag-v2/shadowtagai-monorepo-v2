...
const TIER_INFO = {
  ...
  practice: {
    name: 'Practice',
    price: '$599/mo',
    features: ['Up to 5 attorneys', '2M tokens/day', 'Murder Board', 'Oracle Memos', 'Client portal'],
  },
  enterprise: {
    name: 'Enterprise',
    price: '$999/mo',
    features: ['Unlimited attorneys', '10M tokens/day', 'BYOK', 'Custom models', 'SOC 2 reports', 'Dedicated support'],
  },
};
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {step === 1 && (
          <>
            <h2 style={{ fontFamily: 'Space Grotesk', fontSize: '24px', fontWeight: 600, marginBottom: '8px', letterSpacing: '-0.02em' }}>
              Firm Information
            </h2>
            ...
            <input style={inputStyle} value={firmData.firmName} onChange={(e) => updateField('firmName', e.target.value)} placeholder="e.g., Smith & Associates LLP" />
            <label style={labelStyle}>Contact Email</label>
            <input style={inputStyle} type="email" value={firmData.contactEmail} onChange={(e) => updateField('contactEmail', e.target.value)} placeholder="partner@firm.com" />
            <label style={labelStyle}>Bar Number</label>
            <input style={inputStyle} value={firmData.barNumber} onChange={(e) => updateField('barNumber', e.target.value)} placeholder="e.g., NY-1234567" />
            <label style={labelStyle}>Primary Jurisdiction</label>
            <input style={inputStyle} value={firmData.jurisdiction} onChange={(e) => updateField('jurisdiction', e.target.value)} placeholder="e.g., New York, Southern District" />
          </>
        )}
          ...
          <>
            ...
            {(Object.entries(TIER_INFO) as [string, typeof TIER_INFO.solo][]).map(([key, tier]) => (
              <div
                ...
                role="button"
                tabIndex={0}
                onKeyDown={(e) => { if (e.key === 'Enter') updateField('tier', key); }}
                style={{
                  padding: '20px',
                  marginBottom: '12px',
                  ...
                  borderRadius: '6px',
                  cursor: 'pointer',
                  border: firmData.tier === key ? '1px solid rgba(0, 212, 255, 0.3)' : '1px solid transparent',
                  transition: 'all 0.2s',
                }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <span style={{ fontWeight: 600, fontSize: '16px' }}>{tier.name}</span>
                  <span style={{ color: '#00d4ff', fontFamily: 'Space Grotesk', fontWeight: 600 }}>{tier.price}</span>
                </div>
        ...
        {step === 3 && (
          <>
            ...
            <div style={{ padding: '40px', textAlign: 'center', background: '#111125', borderRadius: '6px' }}>
              ...
              <p style={{ color: '#859398', fontSize: '12px', marginTop: '8px' }}>
                Beta coupon <code style={{ color: '#00d4ff' }}>3wseBY7Z</code> applied — 50% off for 3 months
              </p>
            </div>
          </>
        )}
        ...
        {step === 4 && (
          <>
            ...
            <div style={{ padding: '20px', background: '#111125', borderRadius: '6px', marginBottom: '16px' }}>
              <p style={{ color: '#bbc9cf', fontSize: '13px' }}>
                🔐 Keys are encrypted in your browser using AES-256-GCM before transmission.
        ...
        {step === 5 && (
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ padding: '16px', background: '#111125', borderRadius: '6px', textAlign: 'left' }}>
              <div style={{ fontSize: '13px', color: '#859398', marginBottom: '8px' }}>Quick Summary:</div>
              <div style={{ fontSize: '14px' }}>• <strong>Firm:</strong> {firmData.firmName}</div>
              <div style={{ fontSize: '14px' }}>• <strong>Plan:</strong> {TIER_INFO[firmData.tier].name} ({TIER_INFO[firmData.tier].price})</div>
              <div style={{ fontSize: '14px' }}>• <strong>Jurisdiction:</strong> {firmData.jurisdiction}</div>
            </div>
          </div>
        )}
