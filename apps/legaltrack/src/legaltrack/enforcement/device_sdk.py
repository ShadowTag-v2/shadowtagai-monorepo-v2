...
class DeviceEnforcementSDK:
  ...
  async def _execute_ambient_prod(self, event_data: dict[str, Any]):
    ...
    """
    logger.info(
      f"Executing Ambient Prod to User {self.user_id}: {event_data.get('message')}"
    )
  ...
  async def _execute_aggressive_prod(self, event_data: dict[str, Any]):
    ...
    if self.intensity_level == "no-slack":
      from legaltrack.enforcement.tesla_oem import TeslaOEMController

      tesla_controller = TeslaOEMController(vehicle_id="USER_VEHICLE_1")
      await tesla_controller.execute_no_slack_protocol()
