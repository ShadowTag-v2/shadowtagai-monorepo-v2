...
async def _orchestrate_departure(schedule: CEOTrackSchedule):
  ...
  print(
    f"Schiznit Hardware Orchestration complete for {schedule.task_name}. Car prepped."
  )


@router.post("/schiznit/nudge")
async def trigger_schiznit_nudge(
  schedule: CEOTrackSchedule, background_tasks: BackgroundTasks
):
