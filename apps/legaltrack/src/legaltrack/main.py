...
async def process_court_filing(payload: IngestionPayload, bg_tasks: BackgroundTasks):
  ...
  await event_bus.publish(
    "filing_received", {"id": payload.source_email_id, "route": path}
  )
  ...
  bg_tasks.add_task(
    device_sdk.dispatch_prod,
    {"message": f"DUE {calculated_deadline} - {rule_found}"},
  )

  # 5. Log ROI projection
  roi_calc.log_processing_event(
    tokens_used=4200, latency_ms=75, estimated_human_minutes=45
  )
