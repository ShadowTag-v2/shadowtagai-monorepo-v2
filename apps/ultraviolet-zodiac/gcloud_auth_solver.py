...
def run_command(command: str) -> subprocess.CompletedProcess:
  return subprocess.run(
    command, shell=True, text=True, capture_output=True, env=os.environ
  )
...
def solve_auth():
  ...
  if verify_token():
    ...
    print(
      json.dumps({"status": "success", "method": "unchanged", "timestamp": time.time()})
    )
    return
  ...
  for strategy in ["service_account_key", "application_default"]:
    ...
    if attempt_auth_strategy(strategy):
      if verify_token():
        ...
        print(
          json.dumps(
            {"status": "success", "method": strategy, "timestamp": time.time()}
          ),
        )
        return
  ...
  print(
    json.dumps(
      {"status": "failed", "error": "All strategies failed", "timestamp": time.time()}
    ),
  )
  sys.exit(1)
