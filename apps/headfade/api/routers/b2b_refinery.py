...
class HDISignal(BaseModel):
  artifact: str = Field(
    description="Visual artifact e.g., temporal_flicker, impossible_geometry"
  )
...
async def process_b2b_dataset(daily_comments: list[str]):
  ...
  try:
    ...
    formatted_rows = [
      {"artifact": row[0], "count": row[1].count} for row in anonymized_data
    ]
    errors = client.insert_rows_json(
      "shadowtag-omega-v4.b2b.hdi_dataset", formatted_rows
    )
