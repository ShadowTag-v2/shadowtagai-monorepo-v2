...
_EXEMPT_PATHS = frozenset({"/health", "/api/health", "/docs", "/openapi.json", "/redoc"})
...
async def app_check_middleware(request: Request, call_next):
    ...
    if not token:
        if _ENFORCE:
            logger.warning("App Check: missing token on %s from %s", path, request.client.host if request.client else "unknown")
            return JSONResponse(
                status_code=401,
                content={"detail": "Missing App Check token. Enable Firebase App Check in your client."},
            )
    ...
    try:
        ...
        logger.warning("App Check: firebase_admin.app_check not available, skipping verification")
