...
async function triggerJulesSession(repoUrl, taskDescription) {
  try {
    ...
    const interval = setInterval(async () => {
      checks++;
      try {
        const activityRes = await fetch(`${JULES_API_URL}/sessions/${sessionId}/activities`, { headers });
        if (activityRes.ok) {
...
// Example usage:
triggerJulesSession('https://github.com/ShadowTag-v2/Monorepo-Uphillsnowball.git', 'Verify HeadFade web app functionality');
