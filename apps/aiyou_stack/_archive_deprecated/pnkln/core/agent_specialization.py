...
class AgentSpecialization:
  ...
  def calculate_weighted_consensus(
    ...
    if weighted_opinions:
      ...
      high_weight_opinions = [
        o for o in weighted_opinions if o["weight"] >= top_weight * 0.8
      ]
    else:
      high_weight_opinions = []
  ...
  def _get_expertise_level(
    self, agent: SpecializedAgent, domain: ExpertiseDomain
  ) -> str:
  ...
  def identify_cross_domain_issues(
    self, task_domains: list[ExpertiseDomain]
  ) -> dict[str, Any]:
  ...
  def recommend_team(
    self, task_domains: list[ExpertiseDomain], team_size: int = 5
  ) -> list[str]:
