...
class LegalWhiteboard:
  ...
  def evaluate_answer(self, answer: str) -> dict[str, Any]:
    ...
    for test in self.tests:
      ...
      test.passed = passed
      results.append(
        {"test_id": test.id, "description": test.description, "passed": passed}
      )
  ...
  def add_annotation(
    self, author_id: str, target_agent_id: str, note_index: int, content: str
  ):
  ...
  def set_audience(
    self, audience: Literal["technical", "executive", "public", "legal"]
  ):
  ...
  def comprehension_check(self, agent_id: str) -> dict[str, Any]:
    ...
    total_facts = len(self.facts)
    acknowledged = sum(
      1 for fact in self.facts.values() if agent_id in fact.acknowledged_by
    )
