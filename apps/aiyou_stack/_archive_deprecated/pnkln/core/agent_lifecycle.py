...
class AgentLifecycle:
  ...
  def record_task(
    ...
    metrics.recent_accuracy = (
      old_weight * metrics.recent_accuracy + new_weight * task_accuracy
    )
  ...
  def get_temperature(self, agent_id: str) -> float:
    ...
    progress = min(
      1.0, metrics.tasks_since_regeneration / self.max_tasks_before_regeneration
    )
  ...
  def get_summary(self) -> dict[str, Any]:
    ...
    avg_accuracy = sum(m.recent_accuracy for m in self.agents.values()) / len(
      self.agents
    )
    avg_sycophancy = sum(m.sycophancy_rate for m in self.agents.values()) / len(
      self.agents
    )
