...
class QuidditchCompetition:
  ...
  def register_agent(
    self, agent_id: str, house: HogwartsHouse = None
  ) -> QuidditchAgent:
  ...
  def get_standings(self) -> dict[str, Any]:
    ...
    agent_rankings = sorted(
      self.agents.values(), key=lambda a: a.glicko_rating, reverse=True
    )
  ...
  def form_team(self, agent_ids: list[str]) -> dict[str, Any]:
    ...
    return {"team": team, "house_distribution": self._get_house_distribution(agent_ids)}
  ...
  def halftime_analysis(self) -> dict[str, Any]:
    ...
    return {
      ...
      "dominant_seeker": max(seeker_counts, key=seeker_counts.get)
      if seeker_counts
      else None,
  ...
  def create_tournament(
    self, format: str = "bracket", rounds: int = 5
  ) -> dict[str, Any]:
