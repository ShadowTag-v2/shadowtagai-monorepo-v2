...
try:
    ...
    BIG_QUERY_DATASET = (
        os.getenv("BIG_QUERY_DATASET")
        if os.getenv("BIG_QUERY_DATASET")
        else os.getenv("_CHATBOT_NAME", "quickbot").lower().replace(" ", "_").replace("-", "_")
    )
    ...
    create_table(BIG_QUERY_DATASET, CHATS_TABLE, Chat.__schema__(), project_id, bigquery_client)
    ...
    create_table(BIG_QUERY_DATASET, INTENTS_TABLE, Intent.__schema__(), project_id, bigquery_client)

    for intent in DEFAULT_INTENTS:
        try:
            insert_intent(BIG_QUERY_DATASET, INTENTS_TABLE, intent.to_insert_string(), bigquery_client)
            print("Intent 'Travel concierge' created successfully \n")
            ...
            if e.stdout:
                ...
                print("Stdout from failed command: <empty>")
            if e.stderr:
                print(f"Stderr from failed command:\n{e.stderr}")
            else:
                print("Stderr from failed command: <empty>")
