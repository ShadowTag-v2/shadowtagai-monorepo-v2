...
ENDPOINT="${ENDPOINT:-http://Claude_Code_6.pnkln.svc.cluster.local}"
MAX_RETRIES=30
RETRY_DELAY=5
...
if check_health; then
    ...
    echo "   2. Check logs: kubectl logs -n pnkln -l app=Claude_Code_6"
    echo "   3. Check service: kubectl get svc -n pnkln Claude_Code_6"
    echo ""
    exit 1
fi
