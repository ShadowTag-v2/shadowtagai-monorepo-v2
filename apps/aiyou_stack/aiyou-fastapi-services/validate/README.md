...
python test_latency.py \
  --endpoint http://Cor.Claude_Code_6.pnkln.svc.cluster.local \
  --p99-target-ms 90
...
# Custom endpoint
ENDPOINT=http://Cor.Claude_Code_6.pnkln.svc.cluster.local bash test_health.sh
```
...
{
  "status": "healthy",
  "model": "pnkln-Cor.Claude_Code_6-v1",
  "gpu": "NVIDIA L4"
}
```
...
kubectl port-forward -n pnkln svc/Cor.Claude_Code_6 8000:80
...
1. Check service is running:
   ```bash
   kubectl get svc -n pnkln Cor.Claude_Code_6
   ```

2. Verify pods are ready:
   ```bash
   kubectl get pods -n pnkln -l app=Cor.Claude_Code_6
   ```

3. Check endpoint URL:
   ```bash
   # In-cluster
   curl http://Cor.Claude_Code_6.pnkln.svc.cluster.local/health
...
2. **Review pod logs**:
   ```bash
   kubectl logs -n pnkln -l app=Cor.Claude_Code_6 --tail=100
   ```
...
4. **Verify configuration**:
   ```bash
   kubectl get cm Cor.Claude_Code_6-config -n pnkln -o yaml
   ```
...
```bash
# Region A
ENDPOINT=http://Cor.Claude_Code_6-us-central.example.com python test_latency.py

# Region B
ENDPOINT=http://Cor.Claude_Code_6-us-east.example.com python test_latency.py
```
...
1. Logs: `kubectl logs -n pnkln -l app=Cor.Claude_Code_6`
