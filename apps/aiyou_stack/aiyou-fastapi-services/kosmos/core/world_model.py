...
class KosmosWorldModel:
    ...
    def record_cost(self, tokens: int, model: str, cost: float):
        ...
        Args:
            ...
            model: Model name (gemini-3.1-flash-lite-preview, gemini-3.1-flash-lite-preview)
            cost: Cost in USD
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "top_hypotheses": [
                {"id": h.id, "text": h.text, "confidence": h.confidence}
                for h in self.get_top_hypotheses(3)
            ],
        }
    ...
    def __repr__(self) -> str:
        return (
            f"KosmosWorldModel(session={self.session_id}, phase={self.phase.value}, "
            f"hypotheses={len(self.hypotheses)}, status={self.status})"
        )
