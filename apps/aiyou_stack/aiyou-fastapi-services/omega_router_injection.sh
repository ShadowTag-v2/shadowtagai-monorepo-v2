...
mkdir -p libs/ShadowTag-v2/proxies
...
cat <<PYTHON > libs/ShadowTag-v2/proxies/routing_agent.py
import logging
...
cat <<PYTHON > apps/n-autoresearch/Kosmos/BioAgents-server/src/router_routes.py
...
if [ ! -f apps/n-autoresearch/Kosmos/BioAgents-server/src/manager_routes.py ]; then
