...
# Adjusted path to be relative to the running application or a fixed location in the container
# For Cloud Run, we might need a persistent volume or rely on Git ops (as implied by the prompt "Persisted in Git")
# For now, we place it in the local repo structure.
WHITEBOARD_PATH = Path(__file__).parent.parent.parent / "whiteboard" / "legal_state.json"


class LegalWhiteboard:
    """Single source of truth for the evolving legal agent. Persisted in Git (via file system commits)."""
