...
class Glicko2Rating:
    ...
    def _update_volatility(self, phi: float, v: float, delta: float) -> float:
        ...
        def f(x: float) -> float:
            exp_x = math.exp(x)
            phi_sq = phi**2
            return ((exp_x * (delta**2 - phi_sq - v - exp_x)) / (2 * (phi_sq + v + exp_x) ** 2)) - (
                x - a
            ) / self.TAU**2
...
class WealthAnalyzer:
    ...
    def calculate_total_leakage(leaks: list[WealthLeak]) -> dict[str, Any]:
        ...
        return {
            ...
            "leaks_by_difficulty": {
                "easy": sum(l.monthly_loss_usd for l in leaks if l.fix_difficulty == "easy"),  # noqa: E741
                "medium": sum(l.monthly_loss_usd for l in leaks if l.fix_difficulty == "medium"),  # noqa: E741
                "hard": sum(l.monthly_loss_usd for l in leaks if l.fix_difficulty == "hard"),  # noqa: E741
            },
        }
