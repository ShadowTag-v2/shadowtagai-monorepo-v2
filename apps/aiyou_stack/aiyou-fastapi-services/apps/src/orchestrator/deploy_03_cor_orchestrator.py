...
{{
  "steps": [
    {{
      "action": "call_service",
      "service": "Cor_Claude_Code_6-layer1",
      "method": "POST",
      "endpoint": "/infer",
      "payload": {{"key": "value"}},
      "timeout_ms": 100,
...
"""
    ...
    async def execute_plan(self, plan: OrchestrationPlan) -> dict[str, Any]:
        ...
        for i, step in enumerate(plan.steps):
            ...
            try:
                ...
                if asyncio.iscoroutinefunction(self._execute_step):
    ...
    async def _call_service(self, step: dict[str, Any]) -> dict[str, Any]:
        """Call a service using httpx with resilient patterns"""
        service_name = step.get("service")
        endpoint = step.get("endpoint", "/")
        method = step.get("method", "POST")
        payload = step.get("payload", {})
        timeout = step.get("timeout_ms", 1000) / 1000.0
        retry_policy = step.get("retry_policy", {"max_attempts": 3, "backoff_ms": 100})

        url = f"http://{service_name}{endpoint}"  # Assumes internal DNS resolution
        logger.info(f"Calling service: {service_name} at {url}")

        async with httpx.AsyncClient(timeout=timeout) as client:
            for attempt in range(retry_policy.get("max_attempts", 3)):
                try:
                    ...
                    if attempt < retry_policy.get("max_attempts", 3) - 1:
                        await asyncio.sleep(
                            retry_policy.get("backoff_ms", 100) / 1000.0 * (2**attempt),
                        )  # Exponential backoff
                    else:
                        logger.error(f"All attempts failed for {service_name}")
                        raise
...
def deploy_to_vertex_training(
    ...
    job.run(
        ...
        service_account=f"Cor_Claude_Code_6@{project_id}.iam.gserviceaccount.com",
    )
...
def main():
    ...
    project_id = os.getenv("PROJECT_ID", "shadowtagai-core-stack")
    ...
    job_name = deploy_to_vertex_training(
        ...
        location=location,
        display_name="Cor_Claude_Code_6-orchestrator-v1",
        container_image_uri=f"gcr.io/{project_id}/Cor_Claude_Code_6-orchestrator:latest",
