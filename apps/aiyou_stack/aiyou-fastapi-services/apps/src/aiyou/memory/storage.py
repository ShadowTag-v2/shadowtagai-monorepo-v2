...
class MemoryStorage:
    ...
    def load_from_gcs(self, blob_name: str = "memory/current.json") -> dict[str, Any]:
        ...
        if not self.storage_client:
            ...
            return {"status": "MOCK_MODE", "ui_payload": self._get_mock_payload("GCS (Workbench)")}
    ...
    def load_from_firestore(
        ...
        if not self.firestore_client:
            ...
            return {
                "status": "MOCK_MODE",
                "ui_payload": self._get_mock_payload("Firestore (Real-time)"),
            }
        ...
        try:
            ...
            return {
                "status": "EMPTY",
                "ui_payload": self._get_mock_payload("EMPTY (Firestore)"),
            }
        except Exception as e:
            logger.error(f"Failed to load memory from Firestore: {e}")
            return {"status": "ERROR", "ui_payload": self._get_mock_payload("ERROR (Firestore)")}

    def _get_mock_payload(self, source: str) -> dict[str, Any]:
        """Returns a mock A2UI payload for the renderer."""
        return {
            "type": "A2UI_PAYLOAD",
            "root_id": "root",
            "components": [
                {
                    "id": "root",
                    "type": "Text",
                    "props": {"content": f"Antigravity Console: {source}", "variant": "h1"},
                    "children": ["status_btn", "desc"],
                },
                {
                    "id": "status_btn",
                    "type": "Button",
                ...
                },
                {
                    ...
                    "type": "Text",
                    "props": {
                        "content": "The Sovereign Swarm is standing by for instructions. Use the terminal or /agent/dispatch to interact.",
                    },
                },
            ],
        }
