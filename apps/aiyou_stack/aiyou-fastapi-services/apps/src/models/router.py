...
from .circuit_breaker import get_breaker_for_model
...
class TokenLevelRouter:
    ...
    def _route_least_loaded(self, estimated_tokens: int) -> RoutingDecision:
        ...
        candidates = [
            m for m in self.registry.get_ready_models() if not get_breaker_for_model(m.name).is_open
        ]

        if not candidates:
            raise RuntimeError("No healthy models available (all circuits open or busy)")

        model = min(candidates, key=lambda m: m.metrics.active_requests)
    ...
    def _route_round_robin(self, estimated_tokens: int) -> RoutingDecision:
        ...
        # Filter out models with open circuit breakers
        candidates = [m for m in ready_models if not get_breaker_for_model(m.name).is_open]
