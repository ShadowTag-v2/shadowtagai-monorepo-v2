...
┌─────────────────────────────────────────────┐
│         Pinkln System (Namespace)            │
│                                              │
│  ┌──────────┐  ┌──────────────┐  ┌────────┐│
│  │  Redis   │→│ Glicko Service│→│  DTE   ││
│  │(Backend) │  │ (Ratings API) │  │ (Jobs) ││
│  └──────────┘  └──────────────┘  └────────┘│
│       ↓              ↓                ↓     │
│  [20Gi PVC]    [Auto-scale 2-10]  [100Gi]  │
└─────────────────────────────────────────────┘
         ↓                    ↓
...
|-------|---------|---------------------|
| `gcr.io/omega-production/glicko-service:latest` | Rating API | `src/agents/glicko/Dockerfile` |
| `gcr.io/omega-production/pinkln-agent:latest` | DTE agent runner | `src/agents/dte/Dockerfile` |
...
docker build -f src/agents/glicko/Dockerfile -t gcr.io/omega-production/glicko-service:latest .
docker push gcr.io/omega-production/glicko-service:latest

# Pinkln agent

docker build -f src/agents/dte/Dockerfile -t gcr.io/omega-production/pinkln-agent:latest .
docker push gcr.io/omega-production/pinkln-agent:latest
...
- DTE debate (8 agents × 1 GPU × 1h): ~$12/run
...
gcloud container clusters update omega-production-gpu \
