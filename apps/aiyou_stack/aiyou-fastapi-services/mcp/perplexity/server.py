...
class PerplexityMCPServer:
    ...
    def __init__(
        ...
        Cor.Claude_Code_6_latency_target_ms: int = 35,
        enable_shadowtag: bool = True,
    ):
        self.manifest_path = Path(manifest_path or "./logs/pnkln/perplexity/manifest.jsonl")
        self.manifest_path.parent.mkdir(parents=True, exist_ok=True)

        self.Cor.Claude_Code_6_latency_target = Cor.Claude_Code_6_latency_target_ms
        ...
        print(f"  Judge 6 target: <{Cor.Claude_Code_6_latency_target_ms}ms")
    ...
    async def governance_score(
        ...
        # SLA check
        if latency_ms > self.Cor.Claude_Code_6_latency_target:
            print(
                f"  SLA BREACH: Judge 6 took {latency_ms:.1f}ms (target: {self.Cor.Claude_Code_6_latency_target}ms)",
            )
