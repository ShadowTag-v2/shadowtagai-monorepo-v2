...
class TestJudgeSixClassifyKernel:
    ...
    async def test_classify_with_violations(self):
        ...
        kernel.model = MagicMock(return_value=torch.tensor([0.95]))
        ...
        assert output.metrics.latency_ms <= settings.kernel_2_max_latency_ms * 5  # Allow 5x for CPU
...
class TestAuditCompressKernel:
    ...
    async def test_compress_audit_trail(self):
        ...
        assert output.data.compression_ratio >= 1.0  # Compression achieved
