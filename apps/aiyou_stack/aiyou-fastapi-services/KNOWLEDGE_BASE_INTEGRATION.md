...
2. **Strategic Vision:** ShadowTag + ShadowTag-v4 dual-vertical business models ($15-20B valuation potential)
...
**Phase 4: ShadowTag-v4 Discovery (Weeks 11-14)**
...
1. Fetch policy configuration (ehanc69/shadowtag_v4-policy)
...
user_agent:
  pattern: "ShadowTag-v4-Ingestion-Bot/1.0 (+https://shadowtag_v4.com/bot)"
  contact: "bot@shadowtag_v4.com"
...
https://github.com/ehanc69/shadowtag_v4-policy/blob/main/policy/config/strict_policy.yml
...
- **After:** Dual-vertical strategy (ShadowTag + ShadowTag-v4) with $15-20B valuation roadmap
...
3. `strategic-business-integration.md` (1,142 lines) - ShadowTag + ShadowTag-v4 business models
...
1. `gemini-ingestion-layer.md` (448 lines) - Ingestion architecture

2. `ethical-crawling.md` (399 lines) - Ethical crawling framework

3. `tier-classification.md` (479 lines) - Intelligence classification system

**Infrastructure (config/, src/, k8s/, .github/):**

1. `config/ethical-crawling.yaml` (142 lines) - Crawling rules

2. `config/tier-classification.yaml` (256 lines) - Tier classification config

3. `src/api/ingestion.py` (531 lines) - FastAPI ingestion endpoints

4. `k8s/ingestion-cronjob.yaml` (410 lines) - K8s CronJob + deployment

5. `.github/workflows/ingest.yml` (189 lines) - Hourly ingestion workflow

**Additional:**

1. `docs/prompts/gemini-ingestion-layer-analysis.md` (314 lines) - Prompt engineering guide

2. `docs/WORKFLOW_FIX.md` (313 lines) - GitHub Actions troubleshooting
...
1. **ShadowTag-v4 Discovery Layer (Phase 4):**
...
- **ShadowTag-v4 ARR:** $275M (36-month projection)
...
- **ShadowTag-v4 Platform:** $8M (Years 1-3)
...
- ShadowTag-v4 Discovery: Multi-agent debate ranking system
