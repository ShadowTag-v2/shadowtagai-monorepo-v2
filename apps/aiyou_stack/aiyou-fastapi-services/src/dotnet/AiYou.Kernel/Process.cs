...
public static class JudgeProcessBuilder
{
    public static KernelProcess Build()
    {
        ...
        builder.OnInputEvent("Start")
            .SendEventTo(new ProcessFunctionTarget(compress, "ExtractFeaturesAsync"));

        compress.OnEvent("FeaturesExtracted")
            .SendEventTo(new ProcessFunctionTarget(risk, "EvaluateRiskAsync"));

        // Branching Logic (The "Intelligence")
        risk.OnEvent("RiskLow")
            .SendEventTo(new ProcessFunctionTarget(enforce, "Execute"));

        risk.OnEvent("RiskModerate")
            .SendEventTo(new ProcessFunctionTarget(human, "AwaitApprovalAsync"));

        risk.OnEvent("RiskPreclusive")
            .SendEventTo(new ProcessFunctionTarget(enforce, "Block"));
