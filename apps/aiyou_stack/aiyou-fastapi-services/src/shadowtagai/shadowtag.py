...
class Watermark:
    ...
    def _construct_message(self) -> str:
        ...
        return json.dumps(
            {"content": self.content, "metadata": self.metadata, "timestamp": self.timestamp},
            sort_keys=True,
        )
...
class ShadowTag:
    ...
    def watermark(self, content: str, metadata: dict[str, Any]) -> str:
        ...
        watermark = Watermark(
            content=content,
            metadata=metadata,
            signature=self._sign(content, metadata),
            merkle_root=self._compute_merkle_root(content, metadata),
        )
    ...
    def _sign(self, content: str, metadata: dict[str, Any]) -> str:
        ...
        # Construct message
        message = json.dumps(
            {"content": content, "metadata": metadata, "timestamp": datetime.now().isoformat()},
            sort_keys=True,
        )
