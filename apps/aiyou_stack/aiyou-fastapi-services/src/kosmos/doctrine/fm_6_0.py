...
class MDMPPipeline:
    ...
    async def step1_receive_mission(self, user_request: str) -> dict[str, Any]:
        ...
        self.products.append(
            MDMPProduct(
                step=MDMPStep.RECEIPT_OF_MISSION, name="Initial Mission Receipt", content=result
            )
        )
    ...
    async def step2_mission_analysis(
        ...
        self.products.append(
            MDMPProduct(
                step=MDMPStep.MISSION_ANALYSIS, name="Mission Analysis Products", content=result
            )
        )
    ...
    async def step3_coa_development(
        ...
        self.products.append(
            MDMPProduct(step=MDMPStep.COA_DEVELOPMENT, name="Courses of Action", content=self.coas)
        )
    ...
    def _identify_essential_tasks(self, specified: list[str], _implied: list[str]) -> list[str]:
