...
class ToolExecutor:
    ...
    async def _screenshot(self, page: Page, args: dict) -> tuple[bool, Any, str | None]:
        ...
        return True, {"screenshot": screenshot_b64, "size_bytes": len(screenshot_bytes)}, None
    ...
    async def _run_workflow(self, page: Page, args: dict) -> tuple[bool, Any, str | None]:
        ...
        # [POLICY INJECTION] Enforce Unified Master Prompt (v1.4) Guardrails
        if any(trigger in workflow_description.lower() for trigger in ["lyrics", "poem", "haiku"]):
            return (
                False,
                None,
                "CRITICAL_COPYRIGHT_CLAUDE: Refused. Never reproduce song lyrics or poems in ANY form.",
            )

        if any(
            trigger in workflow_description.lower() for trigger in ["illegal", "minor", "extremist"]
        ):
            return (
                False,
                None,
                "SAFETY_GROK_CLAUDE: Refused. No assistance with criminal activity or harmful content.",
            )

        # Dispatch workflow to the Temporal Heavy-Lift Swarm
        try:
            if Client is None:
                raise ImportError("temporalio not installed")

            client = await Client.connect("localhost:7233")
            payload = f"WALT Agentic Workflow Request: {workflow_description}"

            await client.execute_workflow(
                "OmegaPayloadOrchestrator",
                payload,
                id=f"walt-workflow-{uuid4().hex[:8]}",
    ...
    async def _search_and_extract(self, page: Page, args: dict) -> tuple[bool, Any, str | None]:
        ...
        # [POLICY INJECTION] Enforce Unified Master Prompt (v1.4) Guardrails
        if any(
            trigger in query.lower() for trigger in ["lyrics", "poem", "haiku", "illegal", "minor"]
        ):
            return (
                False,
                None,
                "POLICY_VIOLATION: Query blocked by Unified Master Prompt constraints.",
            )

        # Dispatch search parameters to Temporal
        try:
            if Client is None:
                raise ImportError("temporalio not installed")

            client = await Client.connect("localhost:7233")
            payload = f"WALT Search & Extract: {query} -> {extract_fields}"

            await client.execute_workflow(
                "OmegaPayloadOrchestrator",
                payload,
                id=f"walt-search-{uuid4().hex[:8]}",
                task_queue="omega-swarm-queue",
            )

            return (
                True,
                {"query": query, "fields": extract_fields, "status": "dispatched_to_swarm"},
                None,
            )
        except Exception as e:
            return False, None, f"Temporal Swarm Dispatch Failed: {e!s}"
...
def create_mcp_server():
    """Create FastMCP server for stdio usage"""
    if not FastMCP:
