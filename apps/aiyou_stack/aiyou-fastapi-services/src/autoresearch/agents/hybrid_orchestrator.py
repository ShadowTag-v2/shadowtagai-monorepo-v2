...
class ExecutionLayer(Enum):
    ...
    minionS = "minions"  # General agent swarm
...
class HybridOrchestrator:
    ...
    LAYER_ROUTING = {
        ...
        # Default → minions
        "default": ExecutionLayer.minionS,
    }
    ...
    async def execute_minions(self, task: dict[str, Any]) -> TaskResult:
        """Execute via minions agent swarm."""
        try:
            endpoint = task.get("endpoint", "task")
            response = requests.post(
                f"{self.minionS_URL}/{endpoint}",
            ...
            return TaskResult(layer=ExecutionLayer.minionS, success=False, output=str(e))
    ...
    async def execute(self, task: dict[str, Any]) -> TaskResult:
        ...
        4. Default to minions
        """
        # Allow explicit layer override
        layer = ExecutionLayer(task["layer"]) if "layer" in task else self.route_task(task)

        print(f"///▞ HYBRID :: Routing to {layer.value}")

        # Execute on appropriate layer
        if layer == ExecutionLayer.minionS:
            result = await self.execute_minions(task)
