...
def verify_zero_trust(credentials: HTTPAuthorizationCredentials = Depends(security)) -> GCPServiceIdentity:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=401,
            detail=f"Pnkln Kovel Shield: Unauthorized Identity Matrix ({str(e)})"
        )
