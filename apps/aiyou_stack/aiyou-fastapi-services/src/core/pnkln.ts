...
export class Pnkln {
  ...
  private infraAnalyzer: InfrastructureAnalyzer;
   ...
   */
  async execute(request: UserRequest): Promise<PnklnResponse> {
    ...
    try {
      ...
      if (infraComponent !== InfrastructureComponent.UNKNOWN) {
        ...
        return await this.handleInfrastructureAnalysis(
          infraComponent,
          input,
          context,
          userId,
...
\`\`\`
  ...
  /**
   * Handle infrastructure analysis requests
   */
  private async handleInfrastructureAnalysis(
    ...
    startTime: number,
  ): Promise<PnklnResponse> {
    try {
      // Perform infrastructure analysis
      const analysisResult = await this.infraAnalyzer.analyzeComponent({
        component,
        documentation: context?.documentation,
        architectureSpecs: context?.architectureSpecs,
        context,
      });

      // Format for display
      const formattedAnalysis = this.infraAnalyzer.formatAnalysisForDisplay(analysisResult);
