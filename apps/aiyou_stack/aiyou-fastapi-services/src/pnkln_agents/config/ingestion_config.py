...
from ..core.gemini_ingestion import Source, SourceTier, SourceType
...
class IngestionConfig:
    ...
    user_agent: str = "SHADOWTAGAIBot/1.0 (+https://shadowtagai.ai/bot)"
    ...
    def validate_quality_gates(self, metrics: dict[str, float]) -> dict[str, bool]:
        ...
        return {
            ...
            "cost_per_item": metrics.get("cost_per_item", float("inf"))
            <= self.target_cost_per_item,
            "relevance_score": metrics.get("relevance_score", 0) >= self.target_relevance_score,
            "runtime_minutes": metrics.get("runtime_minutes", float("inf"))
            <= self.target_runtime_minutes,
            "tier_1_percentage": metrics.get("tier_1_percentage", 0)
            >= self.target_tier_1_percentage,
        }
