...
4. VOTE: minions 650-agent consensus
...
from .vote import minionsClient
...
class AntigravityMessage:
    ...
    minions: dict[str, Any] = field(default_factory=dict)
...
class AntigravityPipeline:
    ...
    def __init__(
        ...
        github_token: str,
        minions_url: str = "https://minions-server-215390634092.us-central1.run.app",
        ...
        self.voter = minionsClient(minions_url)
    ...
    async def run(self, task: str, auto_deploy: bool = False) -> PipelineResult:
        ...
        Input → Gemini → Perplexity → Grok → Claude → minions → CodePMCS → Deploy
        """
        errors = []
        code_generated = []
        consensus_results = []
        ...
        for atom in enriched_atoms:
            try:
                ...
                code_generated.append(
                    {"atom_id": atom.id, "code": code, "reasoning": atom.reasoning_chain}
                )
            except Exception as e:
                errors.append(f"Execute error for atom {atom.id}: {str(e)}")
        print(f"[ANTIGRAVITY] Generated code for {len(code_generated)} atoms")

        # Step 4: VOTE - minions 650-agent consensus
        print("[ANTIGRAVITY] Step 4: VOTE - minions (650 agents)")
        ...
        approved_code = [
            c
            for c, r in zip(code_generated, consensus_results, strict=False)
