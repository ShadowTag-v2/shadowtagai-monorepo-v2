...
from src.antigravity.agents.base_agent import BaseAgent
...
You may delegate to multiple agents in sequence or parallel."""
    ...
    def _simple_delegate(self, task: str) -> list[dict[str, str]]:
        ...
        if any(
            word in task_lower
            for word in ["code", "implement", "build", "create", "write", "function"]
        ):
            delegations.append({"agent": "coder", "task": task})

        # Check for review-related keywords
        if any(
            word in task_lower for word in ["review", "check", "security", "quality", "analyze"]
        ):
            delegations.append({"agent": "reviewer", "task": task})

        # Check for research-related keywords
        if any(
            word in task_lower for word in ["research", "search", "find", "information", "learn"]
        ):
...
"""
        for i, (delegation, result) in enumerate(zip(delegations, results, strict=False), 1):
