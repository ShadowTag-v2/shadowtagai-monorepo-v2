...
class Settings(BaseSettings):
    ...
    gcp_storage_bucket: str = Field(
        default="gs://pnkln-policy-corpus", description="GCS Bucket for policy documents",
    )

    # Vertex AI Configuration
    vertex_ai_location: str = Field(default="us-central1", description="Vertex AI location")
    vertex_ai_model: str = Field(
        default="gemini-2.0-flash-exp", description="Gemini model for file search",
    )

    # File Search Configuration
    file_search_chunk_size: int = Field(
        default=512, description="Chunk size for document splitting",
    )
    ...
    kill_switch_file_search_p99_latency: int = Field(
        default=1000, description="P99 latency threshold in ms",
    )
    kill_switch_corpus_sync_failure_rate: float = Field(
        default=0.05, description="Max corpus sync failure rate (5%)",
    )
    kill_switch_false_positive_rate: float = Field(
        default=0.10, description="Max false positive rate (10%)",
    )

    # Judge #6 Configuration
    judge_p99_latency_target: int = Field(
        default=90, description="Judge #6 P99 latency target in ms",
    )
    judge_gemini_allocation: float = Field(
        default=0.40, description="Gemini allocation percentage for Judge #6",
    )
    ...
    google_application_credentials: str | None = Field(
        default=None, description="Path to GCP credentials JSON",
    )
