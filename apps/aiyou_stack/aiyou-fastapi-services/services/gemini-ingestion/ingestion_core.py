...
class EthicalCrawlingConfig:
    ...
    user_agent: str = "ShadowTag-v4-Ingestion-Bot/1.0 (+https://shadowtag_v4.ai/bot)"
    ...
    def get_am_briefing(self) -> dict[str, Any]:
        ...
        for item in sorted_items:
            briefing["highlights"].append(
                {
                    "title": item.title,
                    "category": item.relevance_categories[0].value
                    if item.relevance_categories
                    else "other",
