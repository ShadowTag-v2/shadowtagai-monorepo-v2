...
# REVISED VERSION - Security and reliability improvements
...
# GCS Buckets - Consider adding project ID suffix for uniqueness
...
# Set to true if you need GPU support (will change machine type)
...
# Temp directory for lifecycle configs
TEMP_DIR=$(mktemp -d)
...
prompt_if_empty() {
    ...
    if [ -z "$current_value" ]; then
        ...
        # SECURITY FIX: Use printf %q for safe variable assignment instead of eval
        # Validate input doesn't contain suspicious characters
        if [[ "$input_value" =~ [^\$] ]]; then
...
enable_apis() {
    ...
    # Enable all APIs and verify they're enabled
    for api in "${apis[@]}"; do
...
create_gcs_buckets() {
    ...
    # Make bucket names unique by adding project ID suffix
    ...
    for bucket in "${buckets[@]}"; do
        if gsutil ls -b "gs://$bucket" &> /dev/null; then
            ...
            if ! gsutil mb -p "$PROJECT_ID" -l "$REGION" "gs://$bucket"; then
                log_error "Failed to create bucket gs://$bucket"
                exit 1
            fi

            # Set lifecycle for Agent Mail (delete after 90 days)
            if [[ "$bucket" == *"agent-mail"* ]]; then
...
create_service_account() {
    ...
    # Update SA email with actual project ID
    SA_EMAIL="${SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com"
    ...
    for bucket in "$AGENT_MAIL_BUCKET" "$GOVERNANCE_BUCKET" "$ARTIFACTS_BUCKET"; do
        log_info "  Granting bucket access to gs://$bucket..."
...
create_workbench_instance() {
    ...
    log_info "  Provisioning (this takes ~5-10 minutes)..."

    # Build metadata string with REPO_URL if provided
    local metadata="proxy-mode=service_account"
    if [ -n "$REPO_URL" ]; then
        metadata="${metadata},repo-url=${REPO_URL}"
    fi

    # Add post-startup script from GCS
    metadata="${metadata},post-startup-script-url=gs://${ARTIFACTS_BUCKET}/scripts/startup.sh"
    ...
    while [ $elapsed -lt $max_wait ]; do
        local state
        state=$(gcloud notebooks instances describe "$INSTANCE_NAME" \
            --location="$ZONE" \
            --project="$PROJECT_ID" \
...
upload_supporting_docs() {
    ...
    local docs_dir="$TEMP_DIR/pnkln_docs"
    mkdir -p "$docs_dir"
...
EOF
    ...
    # Create ShadowTag-v2JR_DOCTRINE.md
    cat > "$docs_dir/ShadowTag-v2JR_DOCTRINE.md" <<'EOF'
# ShadowTag-v2JR Governance Doctrine
...
Brake violations are logged to `gs://pnkln-ShadowTag-v2jr-logs/brakes/` for audit trail.
...
generate_startup_script() {
    ...
    cat > "$TEMP_DIR/startup.sh" <<'STARTUP_EOF'
#!/bin/bash
# PNKLN Vertex AI Workbench - Instance Initialization
# This script runs automatically on instance startup
...
echo "[INFO] Starting PNKLN instance initialization..."

# Get REPO_URL from instance metadata if set
...
if ! command -v rustc &> /dev/null; then
    ...
    # Make cargo available in all shells
    echo 'source $HOME/.cargo/env' >> "$JUPYTER_HOME/.bashrc"
fi
...
cargo install cargo-tarpaulin --locked 2>/dev/null || echo "[WARN] cargo-tarpaulin already installed"
cargo install sccache --locked 2>/dev/null || echo "[WARN] sccache already installed"
cargo install cargo-watch --locked 2>/dev/null || echo "[WARN] cargo-watch already installed"

echo "[INFO] Configuring cargo to use sccache..."
export RUSTC_WRAPPER=sccache
if ! grep -q "RUSTC_WRAPPER=sccache" "$JUPYTER_HOME/.bashrc"; then
    echo 'export RUSTC_WRAPPER=sccache' >> "$JUPYTER_HOME/.bashrc"
fi

echo "[INFO] Installing Python dependencies..."
pip install -q --upgrade pip
pip install -q pyautogen google-cloud-storage google-cloud-aiplatform google-cloud-logging
...
ARTIFACTS_BUCKET="pnkln-task-artifacts-${PROJECT_ID}"

gsutil -m cp "gs://${ARTIFACTS_BUCKET}/docs/*.md" "$JUPYTER_HOME/" 2>/dev/null || \
    echo "[WARN] Could not download supporting docs from GCS"

echo "[INFO] Downloading notebook template..."
gsutil cp "gs://${ARTIFACTS_BUCKET}/notebooks/COR_MULTI_AGENT_TEMPLATE.ipynb" "$NOTEBOOKS_DIR/" 2>/dev/null || \
    echo "[WARN] Could not download notebook template"

echo "[INFO] Cloning rust_scriptbots repository..."
if [ -n "$REPO_URL" ] && [ ! -d "$RUST_SCRIPTBOTS_DIR/.git" ]; then
    echo "[INFO] Cloning from $REPO_URL..."
    git clone "$REPO_URL" "$RUST_SCRIPTBOTS_DIR" || \
        echo "[ERROR] Failed to clone repository"
else
    if [ -z "$REPO_URL" ]; then
        echo "[WARN] REPO_URL not set in instance metadata"
        echo "[WARN] Manually clone your repo: git clone <URL> $RUST_SCRIPTBOTS_DIR"
    else
...
echo "[INFO] Access JupyterLab to begin working"
STARTUP_EOF

    # Upload startup script to artifacts bucket
    gsutil cp "$TEMP_DIR/startup.sh" "gs://$ARTIFACTS_BUCKET/scripts/startup.sh"

    # Make it publicly readable so instance can fetch it
    gsutil acl ch -u AllUsers:R "gs://$ARTIFACTS_BUCKET/scripts/startup.sh"

    log_success "Startup script generated and uploaded"
}
...
display_summary() {
    ...
    echo "  • gs://$GOVERNANCE_BUCKET     (ShadowTag-v2JR Logs)"
    ...
    echo ""
    echo "  3. The startup script will auto-configure the environment"
    echo "     (Check /var/log/daemon.log for startup script logs)"
    echo ""
    if [ -z "$REPO_URL" ]; then
        echo "  4. Clone rust_scriptbots manually:"
        echo "     cd /home/jupyter"
        echo "     git clone <YOUR_REPO_URL> rust_scriptbots"
        echo ""
    fi
    echo "  5. Open ~/notebooks/COR_MULTI_AGENT_TEMPLATE.ipynb"
    echo ""
    echo "  6. Edit Cell 2: Set PROJECT_ID = \"$PROJECT_ID\""
    echo ""
    echo "  7. Run all cells to initialize multi-agent coordination"
    echo ""
    echo "═══════════════════════════════════════════════════════════════════════════"
    echo ""
}
...
main() {
    ...
    # Update SA email and bucket names after PROJECT_ID is set
    SA_EMAIL="${SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com"
...
trap 'log_error "Deployment failed at line $LINENO"; rm -rf "$TEMP_DIR"' ERR
