...
class WorkflowExecutionContext(BaseModel):
    ...
    current_action_index: int = Field(
        default=0,
        description="Index of current action being executed",
    )
...
class StartWorkflowRequest(BaseModel):
    ...
    initial_variables: dict[str, Any] | None = Field(
        ...
        description="Optional initial variables to populate the context",
    )
...
class StartWorkflowResponse(BaseModel):
    ...
    next_action: WorkflowAction | None = Field(
        None,
        description="Next action to perform if waiting for input",
    )
