...
class ArxivCrawler:
    ...
    def _is_excluded(self, paper: arxiv.Result) -> bool:
        ...
        # Check title and abstract for exclusion terms
    ...
    def _get_vertical_relevance(self, paper: arxiv.Result) -> float:
        ...
        # Map categories/content to verticals
        scores = []
        ...
        # Layer 6: Gov & Defense (revenue driver)
        if any(t in text for t in ["defense", "medical ai", "legal ai", "hipaa", "fda"]):
            scores.append(weights.get("gov_defense", 0.95))

        # Layer 3: Digital Mall
        if any(t in text for t in ["marketplace", "api", "compliance"]):
        ...
        # Layer 5: Orbital
        if any(t in text for t in ["satellite", "leo", "orbital", "ground station"]):
            scores.append(weights.get("orbital", 0.8))

        # Layer 1: Energy
        if any(t in text for t in ["offshore", "wind", "grid", "renewable", "caiso"]):
            scores.append(weights.get("energy", 0.7))

        # Quantum
        if "quant-ph" in categories or "quantum" in text:
            scores.append(weights.get("quantum", 0.6))

        return max(scores) if scores else 0.5  # Default to medium relevance
    ...
    def search_papers(
        ...
        Args:
            ...
            min_relevance: Minimum vertical relevance score (0.0-1.0)
        ...
        all_papers: dict[str, tuple[arxiv.Result, float]] = {}  # entry_id -> (paper, relevance)
        excluded_count = 0
            ...
            for search_term in search_terms:
                try:
                    ...
                    for result in search.results():
                        ...
                        # Calculate vertical relevance
                        relevance = self._get_vertical_relevance(result)
                        ...
                        # Deduplicate, keeping highest relevance score
                        if result.entry_id not in all_papers:
                            all_papers[result.entry_id] = (result, relevance)
                            logger.debug(
                                "paper_found",
                                title=result.title,
                        ...
                        elif relevance > all_papers[result.entry_id][1]:
                            all_papers[result.entry_id] = (result, relevance)
