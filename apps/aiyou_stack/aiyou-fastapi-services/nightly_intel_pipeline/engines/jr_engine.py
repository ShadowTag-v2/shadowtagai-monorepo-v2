...
"""
    ...
    def score_content(self, content: str, content_type: str, content_id: str) -> JRScore:
        ...
        try:
            ...
            logger.info(
                ...
                total_score=total_score,
                tier=tier.value,
                atp_risk=result["atp_risk_level"],
            )
    ...
    def score_intel_event(self, event: "IntelEvent") -> JRScore:
        ...
        Args:
            event: Structured IntelEvent object
        ...
        # Map document type to content type
        content_type_map = {
            "regulation": "regulatory_doc",
            "draft_bill": "regulatory_doc",
            "news": "news_article",
            "rfp": "business_doc",
            "competitor_release": "competitor_intel",
            "product_doc": "technical_doc",
            "blog": "web_content",
            "lawsuit": "legal_doc",
            "guidance": "regulatory_doc",
            "academic": "arxiv_paper",
            "announcement": "news_article",
            "unknown": "web_content",
        }
    ...
    def _build_enriched_content(self, event: "IntelEvent") -> str:
        """Build enriched scoring content from IntelEvent"""
        parts = [
            f"# {event.title}",
            f"\n**Source Type:** {event.source_type.value}",
            f"**Jurisdiction:** {event.jurisdiction or 'Unknown'}",
        ]

        if event.effective_date:
            parts.append(f"**Effective Date:** {event.effective_date.isoformat()}")
        ...
        if event.impacts:
            ...
            for imp in event.impacts:
                parts.append(f"- [{imp.severity.upper()}] {imp.description}")
                if imp.timeline:
                    parts.append(f"  Timeline: {imp.timeline}")

        if event.risk_tags:
            parts.append(f"\n**Risk Tags:** {', '.join(t.value for t in event.risk_tags)}")

        if event.topic_tags:
            parts.append(f"**Topics:** {', '.join(event.topic_tags)}")

        # Include JR hints from Gemini
        hints = event.jr_hints
        if hints.purpose_candidates:
            parts.append("\n## Gemini Purpose Analysis")
            for p in hints.purpose_candidates:
                parts.append(f"- {p}")

        if hints.reasons_candidates:
            parts.append("\n## Gemini Reasons Analysis")
            for r in hints.reasons_candidates:
                parts.append(f"- {r}")

        if hints.brakes_candidates:
            parts.append("\n## Gemini Risk Analysis (Brakes)")
            for b in hints.brakes_candidates:
                parts.append(f"- {b}")

        if hints.suggested_tier:
            parts.append(f"\n**Gemini Suggested Tier:** {hints.suggested_tier}")
            parts.append(f"**Gemini Urgency Score:** {hints.urgency_score:.2f}")

        parts.append(f"\n**Gemini Confidence:** {event.gemini_confidence:.2f}")
    ...
    def batch_score_intel_events(self, events: list["IntelEvent"]) -> list[JRScore]:
        ...
        for i, event in enumerate(events, 1):
            logger.info("batch_scoring_intel_event", index=i, total=len(events), event_id=event.id)

            score = self.score_intel_event(event)
            scores.append(score)
