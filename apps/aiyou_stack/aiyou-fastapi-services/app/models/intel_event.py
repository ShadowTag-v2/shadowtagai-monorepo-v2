...
class JRHints(BaseModel):
    ...
    reasons_candidates: list[str] = Field(
        default_factory=list, description="Candidate Reasons: defensible business impacts"
    )
    brakes_candidates: list[str] = Field(
        default_factory=list, description="Candidate Brakes: RA levels, mitigations, failure modes"
    )
    suggested_tier: int | None = Field(
        None, ge=1, le=3, description="Gemini's suggested tier (1=high, 2=medium, 3=low)"
    )
    urgency_score: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Urgency score 0-1 (1=immediate action needed)"
    )
...
class Impact(BaseModel):
    ...
    affected_area: str = Field(
        default="general", description="Area affected: pricing, operations, legal, product, sales"
    )
    severity: str = Field(
        default="medium", description="Impact severity: low, medium, high, critical"
    )
    timeline: str | None = Field(
        None, description="When this impact takes effect: immediate, 30_days, 90_days, 1_year"
    )
...
class IntelEvent(BaseModel):
    ...
    source_type: DocumentType = Field(
        default=DocumentType.UNKNOWN, description="Classified document type"
    )

    # Metadata
    jurisdiction: str | None = Field(None, description="Jurisdiction code: US, US-CA, EU, UK, etc.")
    effective_date: datetime | None = Field(
        None, description="When this regulation/change takes effect"
    )
    ...
    change_type: ChangeType = Field(
        default=ChangeType.INITIAL, description="Type of change this represents"
    )
    ...
    impacts: list[Impact] = Field(
        default_factory=list, description="Structured list of business impacts"
    )
    risk_tags: list[RiskTag] = Field(
        default_factory=list, description="Applicable risk tags for quick filtering"
    )

    # JR Engine Integration
    jr_hints: JRHints = Field(
        default_factory=JRHints, description="Pre-filled hints for JR Engine scoring"
    )

    # Provenance
    raw_text_hash: str = Field(default="", description="SHA-256 hash of raw input text for dedup")
    gemini_model: str = Field(
        default="gemini-2.0-flash", description="Gemini model used for extraction"
    )
    gemini_confidence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Gemini's confidence in extraction accuracy"
    )
    extraction_version: str = Field(
        default="1.0.0", description="Version of extraction prompt/logic"
    )

    # Timestamps
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="When this event was created"
    )
    updated_at: datetime | None = Field(None, description="Last update timestamp")

    # Delta tracking
    previous_version_id: str | None = Field(
        None, description="ID of previous version for change tracking"
    )
    ...
    raw_storage_path: str | None = Field(
        None, description="GCS path to raw document: gs://bucket/path"
    )
