...
class BottleneckDetector:
    ...
    async def analyze_slow_endpoints(self, time_window_hours: int = 24) -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            bottleneck_query = (
                select(Bottleneck)
                .where(Bottleneck.endpoint == row.endpoint)
                .order_by(desc(Bottleneck.percentage))
                .limit(3)
            )
    ...
    async def get_performance_trends(self, endpoint: str, hours: int = 24) -> dict[str, Any]:
        ...
        return {
            ...
            "data_points": [
                ...
                for ts, d in zip(timestamps[-20:], durations[-20:], strict=False)
            ],
        }
    ...
    async def generate_fix_suggestions(self, bottleneck_id: int) -> dict[str, Any]:
        ...
        if any(
            keyword in bottleneck.function_name.lower()
            for keyword in ["query", "execute", "fetch", "select"]
        ):
            suggestions.append(
                {
...
users = await db.query(User).options(selectinload(User.posts)).all()
        ...
        if any(
            keyword in bottleneck.function_name.lower()
            for keyword in ["read", "write", "open", "request", "get", "post"]
        ):
            suggestions.append(
                {
