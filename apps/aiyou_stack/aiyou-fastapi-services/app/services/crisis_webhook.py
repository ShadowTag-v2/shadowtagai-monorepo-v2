...
class WebhookConfig:
    ...
    enabled: bool = True
    events: list[WebhookEventType] = field(default_factory=lambda: [WebhookEventType.CRISIS_ALERT])
    retry_count: int = 3
...
class CrisisAlertWebhook:
    ...
    async def send_self_harm_alert(
        ...
        return await self._send_to_all_endpoints(payload, WebhookEventType.SELF_HARM_DETECTED)
    ...
    async def send_minor_explicit_alert(
        ...
        return await self._send_to_all_endpoints(payload, WebhookEventType.EXPLICIT_CONTENT_MINOR)
    ...
    async def _send_to_all_endpoints(
        ...
        for _endpoint_id, config in self.endpoints.items():
    ...
    def get_delivery_stats(self) -> dict[str, Any]:
        ...
        delivered = sum(1 for r in self.delivery_log if r.status == WebhookDeliveryStatus.DELIVERED)
        failed = sum(1 for r in self.delivery_log if r.status == WebhookDeliveryStatus.FAILED)
