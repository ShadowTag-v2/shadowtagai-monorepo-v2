...
def test_valid_email():
    assert is_valid_email('redacted@shadowtag-v4.local') is True
...
def test_login_bug():
    ...
    result = login('redacted@shadowtag-v4.local', 'pass')
...
- email.lower() → stored as 'redacted@shadowtag-v4.local' ✓

Checking login:
- db.get_user(email=email) → searches for 'redacted@shadowtag-v4.local'
- Database has 'redacted@shadowtag-v4.local'
- No match!
...
Test:
```python
# Stored: 'redacted@shadowtag-v4.local'
db.get_user('redacted@shadowtag-v4.local')  → Found ✓
db.get_user('redacted@shadowtag-v4.local')  → Not found ✓
```
