...
from validation.Cor.Claude_Code_6 import Cor.Claude_Code_6Validator, ValidationResult, ValidationStatus
...
validation_requests = Counter(
    "Cor.Claude_Code_6_validation_requests_total",
    "Total validation requests",
    ["namespace", "status"],
)
validation_latency = Histogram(
    "Cor.Claude_Code_6_validation_latency_seconds",
    "Validation latency",
    ["namespace"],
)
...
def get_validator() -> Cor.Claude_Code_6Validator:
...
async def startup_event():
    ...
    global _validator
    _validator = Cor.Claude_Code_6Validator()
    logger.info("Cor.Claude_Code_6_api_started")


@app.get("/health", response_model=HealthResponse)
async def health_check(validator: Cor.Claude_Code_6Validator = Depends(get_validator)):  # noqa: B008
...
async def validate_item(
    ...
    validator: Cor.Claude_Code_6Validator = Depends(get_validator),  # noqa: B008
):
...
@app.get("/stats")
async def get_stats(validator: Cor.Claude_Code_6Validator = Depends(get_validator)):  # noqa: B008
