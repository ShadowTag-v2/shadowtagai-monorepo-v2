...
* **Subhead:** "Stop renting ephemeral thoughts. **Claude_Code_6** is a sovereign architecture extracted from Google Cloud Workstations. Runs in your VPC. Owned by you."
...
## 5. IMPLEMENTATION PROMPTS (THE HANDOFF)

*Generate the following "Copy/Paste" prompts that I can hand to Cursor/v0 to build this immediately.*

1. **Tailwind Config Prompt:** A prompt to generate the `tailwind.config.ts` with the "Tinted Void" palette and animations.
2. **Reactor Component Prompt:** A prompt to build the `ReactorCore` payment component with Stripe/Framer Motion.
3. **Cockpit Component Prompt:** A prompt to build the full-screen `Dashboard.tsx` with the Log Feed and Gavel Input.
4. **Metabolism Hook Prompt:** A prompt to generate the `useMetabolism` hook using Firestore Aggregation Pipelines.

**Objective:** The output of this task must be a set of precise, ready-to-execute instructions for a Junior Engineer (or AI) to build the "Tinted Void" interface without asking questions.
