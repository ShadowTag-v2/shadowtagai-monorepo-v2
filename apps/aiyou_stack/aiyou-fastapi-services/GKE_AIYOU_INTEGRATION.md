...
spec:
  ...
  template:
    spec:
      ...
      - name: api
        image: us-central1-docker.pkg.dev/shadowtag_v4-prod/containers/pnkln-api:latest
        resources:
          requests:
            cpu: 500m
            memory: 1Gi
...
spec:
  ...
  template:
    spec:
      ...
      - name: Claude_Code_6
        image: us-central1-docker.pkg.dev/shadowtag_v4-prod/containers/Claude_Code_6:v2.0
        env:
...
metadata:
  name: pnkln-api-sa
  annotations:
    iam.gke.io/gcp-service-account: pnkln-api@shadowtag_v4-prod.iam.gserviceaccount.com
...
spec:
  type: Python
  mode: cluster
  image: us-central1-docker.pkg.dev/shadowtag_v4-prod/containers/spark-rapids:latest
  mainApplicationFile: "gs://shadowtag_v4-models/growth_engine.py"
  sparkConf:
...
data:
  policy.rego: |
    package shadowtag_v4.governance
...
- [ ] Create GCP project `shadowtag_v4-prod`
...
**Integration Complete:** GKE + PNKLN + ShadowTag-v4 Ecosystem Unified Platform Ready for Production 🚀
