...
ShadowTag-v4 FastAPI Services is built with **security-first architecture**. This document outlines security features, best practices, and deployment guidelines.
...
- **Password [VAPORIZED_PWD]: Only bcrypt hashes stored (never plaintext)
...
```bash
# Build
docker build -t shadowtag_v4-api:latest .

# Run
docker run -d \
  --name shadowtag_v4-api \
  -p 8000:8000 \
  --env-file .env \
  shadowtag_v4-api:latest
```
...
# Build
docker build -t gcr.io/YOUR_PROJECT_ID/shadowtag_v4-api:v1.0.0 .

# Push
docker push gcr.io/YOUR_PROJECT_ID/shadowtag_v4-api:v1.0.0
```

#### 2. Create GKE Cluster

```bash
gcloud container clusters create shadowtag_v4-cluster \
  --region us-central1 \
  --num-nodes 3 \
...
```bash
kubectl create secret generic shadowtag_v4-secrets \
...
spec:
  ...
  template:
    spec:
      containers:
      - name: api
        image: gcr.io/YOUR_PROJECT_ID/shadowtag_v4-api:v1.0.0
        envFrom:
        - secretRef:
            name: shadowtag_v4-secrets
        resources:
          requests:
            memory: "512Mi"
            cpu: "500m"
...
1. **Isolate**: Scale down to 0 replicas immediately
   ...
   ```bash
   kubectl scale deployment shadowtag_v4-api --replicas=0
   ```
...
**Security Issues**: Report to <security@shadowtag_v4.com> (replace with actual email)
...
- **Security Issues**: <security@shadowtag_v4.com>
- **Documentation**: See `README.md`
- **Support**: <support@shadowtag_v4.com>
