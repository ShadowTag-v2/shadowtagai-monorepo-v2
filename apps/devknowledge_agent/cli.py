...
async def run_query(query: str):
  ...
  if not api_key:
    print(
      "ERROR: DEVELOPER_KNOWLEDGE_API_KEY environment variable is not set.",
      file=sys.stderr,
    )
    sys.exit(1)
    ...
    async with ClientSession(read_stream, write_stream) as session:
      ...
      try:
        ...
        response = await session.call_tool(
          "search_knowledge", arguments={"query": query}
        )
...
def main():
  parser = argparse.ArgumentParser(
    description="CLI wrapper for Google Developer Knowledge MCP Server"
  )
  parser.add_argument(
    "query", help="The question or search query for the developer knowledge base"
  )
