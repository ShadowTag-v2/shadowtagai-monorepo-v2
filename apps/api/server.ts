...
const database = new Spanner({ projectId: 'shadowtag-omega-v4' }).instance('core-cluster').database('shadowtag-omega-v4');
...
const rootValue = {
  ...
  userAccess: async ({ firebaseUid }: { firebaseUid: string }) => {
    const [rows] = await database.run({ sql: `SELECT status FROM transactions WHERE user_id = @uid AND status = 'PAID'`, params: { uid: firebaseUid } });
...
app.post('/webhook/stripe', async (c) => {
    ...
    const event = stripe.webhooks.constructEvent(await c.req.text(), signature!, process.env.STRIPE_WEBHOOK_SECRET!);
    if (event.type === 'checkout.session.completed') {
        const session = event.data.object as any;
        await database.table('transactions').insert({
            user_id: session.client_reference_id, stripe_customer_id: session.customer, status: 'PAID', timestamp: Spanner.commitTimestamp()
        });
...
app.post('/webhook/workspace', async (c) => {
    ...
    let processableText = docName.endsWith('.xlsx') 
