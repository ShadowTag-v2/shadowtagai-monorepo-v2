...
type Step = 'INITIAL' | 'RESEARCHING_BAD' | 'RESEARCHING_GOOD' | 'CLAUDE_CODE_6_INTERVENTION' | 'LOCKED';

export default function WorkstationDemo() {
  const [step, setStep] = useState<Step>('INITIAL');
  const [inputValue, setInputValue] = useState('');
  const [chatLog, setChatLog] = useState<
    { sender: 'user' | 'system' | 'Claude_Code_6'; text: React.ReactNode }[]
  >([]);
  ...
  const handleUserSubmit = () => {
    ...
    // Intervention
    setTimeout(() => {
      setStep('CLAUDE_CODE_6_INTERVENTION');
      setChatLog((prev) => [
        ...prev,
        {
          sender: 'Claude_Code_6',
          text: (
            ...
            {chatLog.map((log, i) => (
              ...
              >
                <div
                  ...
                  ${log.sender === 'Claude_Code_6' ? 'bg-transparent text-white w-full' : ''}
                `}
                >
                  {log.sender === 'user' && (
              ...
              <textarea
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault();
                    if (step === 'INITIAL' || step === 'CLAUDE_CODE_6_INTERVENTION') handleUserSubmit();
                  }
                }}
                ...
                placeholder={
                  step === 'RESEARCHING_BAD'
                    ...
                    : step === 'RESEARCHING_GOOD'
                      ...
                      : step === 'CLAUDE_CODE_6_INTERVENTION'
