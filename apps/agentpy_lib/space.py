...
from .sequences import AgentIter
...
class Space(SpatialEnvironment):
  ...
  def kdtree(self):
    ...
    if self._cKDTree is None and len(self.agents) > 0:
      ...
      if self._torus:
        self._cKDTree = spatial.cKDTree(self._sorted_agent_points, boxsize=self.shape)
      else:
  ...
  def add_agents(self, agents, positions=None, random=False):
    ...
    if not positions:
      ...
      if random:
        positions = [
          [self.model.random.random() * d_max for d_max in self.shape]
  ...
  def neighbors(self, agent, distance):
    ...
    list_ids = self.kdtree.query_ball_point(self.positions[agent], distance)
