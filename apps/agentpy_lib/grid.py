...
from .tools import make_list, AgentpyError, ListDict
from .sequences import AgentSet, AgentIter
    ...
    if self.exclude:
      if isinstance(self.area, np.ndarray):
        return itertools.filterfalse(
          lambda x: x is self.exclude, itertools.chain.from_iterable(self.area.flat)
        )
      else:
        return itertools.filterfalse(lambda x: x is self.exclude, self.area)
    else:
      if isinstance(self.area, np.ndarray):
...
class Grid(SpatialEnvironment):
  ...
  def __init__(
    self, model, shape, torus=False, track_empty=False, check_border=True, **kwargs
  ):
  ...
  def _border_behavior(position, shape, torus):
    ...
    if torus:
      new_position = tuple(x % x_max for x, x_max in zip(position, shape))

    # Not connected - Stop at border
    else:
      new_position = tuple(np.clip(position, 0, np.array(shape) - 1))
  ...
  def neighbors(self, agent, distance=1):
    ...
    if self._torus:
      ...
      return AgentIter(self.model, itertools.chain.from_iterable(area_iters))

    # Case 2: Non-toroidal
    else:
      slices = tuple(
        slice(p - distance if p - distance >= 0 else 0, p + distance + 1) for p in pos
      )
  ...
  def attr_grid(self, attr_key, otypes="f", field="agents"):
    ...
    f = np.vectorize(
      lambda x: getattr(next(iter(x)), attr_key) if x else np.nan, otypes=otypes
    )
  ...
  def del_field(self, key):
    ...
    self.grid = rfs.drop_fields(self.grid, key, usemask=False, asrecarray=True)
    delattr(self, key)
