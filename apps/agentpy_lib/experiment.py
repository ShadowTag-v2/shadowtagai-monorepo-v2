...
class Experiment:
  ...
  def __init__(
    self, model_class, sample=None, iterations=1, record=False, randomize=True, **kwargs
  ):
    ...
    self.run_ids = [
      (sample_id, iteration) for sample_id in sample_range for iteration in iter_range
    ]
    ...
    if randomize and sample is not None and any(["seed" in p for p in self.sample]):
      if len_sample > 1:
        rngs = [
          rd.Random(p["seed"]) if "seed" in p else rd.Random() for p in self.sample
        ]
        self._random = {
        ...
        rng = rd.Random(seed)
        self._random = {
          (None, iteration): rng.getrandbits(128) for iteration in iter_range
        }
    else:
      self._random = None
  ...
  def run(self, n_jobs=1, pool=None, display=True, **kwargs):
    ...
    # Normal processing
      ...
      for run_id in self.run_ids:
        self._add_single_output_to_combined(self._single_sim(run_id), combined_output)
        if display:
          i += 1
