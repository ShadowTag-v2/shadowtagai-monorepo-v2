...
class DataDict(AttrDict):
    ...
    for key, output in self.items():
      if isinstance(output, pd.DataFrame):
        ...
        try:
          ...
          print(f"Warning: Object '{key}' could not be saved. (Reason: {e})")
  ...
  def _load(self, exp_name=None, exp_id=None, path="ap_output", display=True):
    ...
    if exp_id is None:
      ...
      if exp_id is None:
        raise FileNotFoundError(
          f"No experiment found with name '{exp_name}' in path '{path}'"
        )
