...
def validate_jr(metrics: dict) -> list[str]:
  ...
  # Latency check
  if (
    "p99_latency" in metrics
    and metrics["p99_latency"] > JR_CONSTRAINTS["p99_latency_ms"]
  ):
    violations.append(
  ...
  # Burn rate check
  if (
    "monthly_cost" in metrics
    and metrics["monthly_cost"] > JR_CONSTRAINTS["monthly_burn_max"]
  ):
    violations.append(
...
def main():
  parser = argparse.ArgumentParser(
    description="Validate tool usage against JR constraints"
  )
