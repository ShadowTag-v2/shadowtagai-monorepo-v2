...
class AsyncSuggestionConsumer:
    ...
    _queue: asyncio.Queue[SuggestionEntry] = field(default_factory=lambda: asyncio.Queue(maxsize=10))
    ...
    async def get_suggestion(self, *, timeout: float = 5.0) -> SuggestionEntry | None:
        ...
        while True:
            ...
            if entry.is_fresh:
                ...
                return entry
            logger.debug("Skipping stale suggestion (%.0fs old): '%s'", entry.age_seconds, entry.text[:40])
