...
class ApprovalEnvelope:
  ...
  def approval_ratio(self) -> float:
    ...
    if not self.policy.required_agents:
      return 1.0
    approved_count = sum(
      1 for v in self.votes if v.agent_id in self.policy.required_agents and v.approved
    )
...
class AgentMailbox:
  ...
  def submit_plan(
    ...
    *,
    ...
    logger.info(
      "Plan '%s' submitted for approval by %s", plan_id, envelope.policy.required_agents
    )
    return envelope
    ...
    with SpanContext(
      ...
      for existing_vote in envelope.votes:
        if existing_vote.agent_id == agent_id:
          logger.warning(
            "Agent '%s' already voted on plan '%s', ignoring duplicate",
            agent_id,
            plan_id,
          )
