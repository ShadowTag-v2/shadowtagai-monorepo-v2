...
def log_suggestion_event(*, event: str, **kwargs: Any) -> None:
  _write_event(
    f"suggestion_{event}", {k: v for k, v in kwargs.items() if v is not None}
  )


def log_speculation_event(*, event: str, **kwargs: Any) -> None:
  _write_event(
    f"speculation_{event}", {k: v for k, v in kwargs.items() if v is not None}
  )
  ...
  try:
    with open(log_file) as f:
      for line in f:
        ...
        if event_type_prefix and not entry.get("event_type", "").startswith(
          event_type_prefix
        ):
          continue
...
def compute_quality_accuracy(*, limit: int = 500) -> dict[str, Any]:
  ...
  if total > 1:
    ...
    numerator = sum(
      (q - q_mean) * (a - a_mean)
      for q, a in zip(quality_scores, acceptances, strict=True)
    )
