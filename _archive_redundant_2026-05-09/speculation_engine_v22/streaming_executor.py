...
class StreamingToolExecutor:
  ...
  def _create_synthetic_error(self, tool_id: str, reason: AbortReason) -> ToolResult:
    if reason == AbortReason.USER_INTERRUPTED:
      return ToolResult(
        tool_use_id=tool_id, content="User rejected tool use", is_error=True
      )
    ...
    async def _run() -> None:
      ...
      if self._execute_fn:
        ...
        try:
          ...
          tool.results.append(
            ToolResult(tool_use_id=tool.id, content=f"Error: {exc}", is_error=True)
          )
      else:
        tool.results.append(
          ToolResult(
            tool_use_id=tool.id, content="(no executor configured)", is_error=False
          )
        )
  ...
  async def get_remaining_results(self) -> AsyncIterator[MessageUpdate]:
    ...
    while self._has_unfinished_tools():
      ...
      if (
        ...
        executing_tasks = [
          t.task for t in self._tools if t.status == ToolStatus.EXECUTING and t.task
        ]
        if executing_tasks:
  ...
  def completed_count(self) -> int:
    return sum(
      1 for t in self._tools if t.status in (ToolStatus.COMPLETED, ToolStatus.YIELDED)
    )
