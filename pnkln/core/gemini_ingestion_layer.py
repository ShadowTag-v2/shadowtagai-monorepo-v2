...
class IngestionResult:
  ...
  def tier_1_ratio(self) -> float:
    ...
    if self.items_collected:
      tier_1_count = sum(
        1 for item in self.items_collected if item.tier == DataTier.TIER_1
      )
...
class QualityGates:
  ...
  def evaluate(self, result: IngestionResult) -> dict[str, bool]:
    ...
    gates_passed["items_per_day"] = (
      self.gates["min_items_per_day"]
      <= result.total_items
      <= self.gates["max_items_per_day"]
    )
    ...
    gates_passed["cost_per_item"] = (
      result.avg_cost_per_item <= self.gates["max_cost_per_item"]
    )
    ...
    avg_relevance = sum(
      m.avg_relevance_score for m in result.source_metrics.values()
    ) / max(len(result.source_metrics), 1)
...
class GeminiIngestionLayer:
  ...
  def _build_pipeline(self) -> None:
    ...
    async def quality_gate_validation(ctx: ExecutionContext, data: dict) -> dict:
      ...
      logger.info(
        f"Quality gates: {sum(gates_passed.values())}/{len(gates_passed)} passed ({latency_minutes:.1f} min)"
      )
  ...
  async def run_nightly_job(
    ...
    try:
      ...
      if runtime_minutes > 45:
        logger.warning(
          f"Job {job_id} exceeded 45 min target: {runtime_minutes:.1f} min"
        )
      else:
        logger.info(
...
async def example_usage():
  ...
  result = await ingestion.run_nightly_job(
    job_id="job_2025_11_15", max_items_per_source=500
  )
