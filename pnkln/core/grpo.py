...
class GRPOTrainer:
  ...
  def compute_loss(
    self, batch: GRPOBatch, new_log_probs: np.ndarray
  ) -> dict[str, float]:
    """
...
def generate_synthetic_batch(
  ...
  # Generate prompts
  prompts = [
    f"prompt_{g}" for g in range(num_groups) for _ in range(responses_per_prompt)
  ]
  ...
  # Group IDs
  group_ids = np.array(
    [g for g in range(num_groups) for _ in range(responses_per_prompt)]
  )
...
if __name__ == "__main__":
  ...
  for i in range(4):
    print(
      f"  Response {i}: reward={group_0_rewards[i]:.3f}, advantage={group_0_advantages[i]:.3f}"
    )
