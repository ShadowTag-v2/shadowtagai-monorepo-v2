...
def generate_synthetic_batch(num_groups: int = 8, responses_per_prompt: int = 4, reward_noise: float = 0.1) -> GRPOBatch:
    """
    ...
    return GRPOBatch(prompts=prompts, responses=responses, rewards=rewards, log_probs=log_probs, group_ids=group_ids)
...
def compare_ppo_grpo() -> dict[str, dict[str, Any]]:
    ...
    return {
        "ppo": {
            ...
            "hyperparameters": ["clip_epsilon", "vf_coef", "ent_coef", "lr_actor", "lr_critic"],
            "advantages": ["Well-established", "Proven on many tasks", "Good theoretical properties"],
            "disadvantages": ["Requires value network", "More hyperparameters", "Can be unstable"],
        },
        "grpo": {
            ...
            "advantages": ["Simpler implementation", "No value network needed", "More stable training", "Better sample efficiency"],
            "disadvantages": ["Requires grouped data", "Less established", "May struggle with sparse rewards"],
        },
    }
...
if __name__ == "__main__":
    ...
    # Create trainer
    config = GRPOConfig(num_groups=8, responses_per_prompt=4, learning_rate=1e-4, normalize_advantages=True)
    ...
    batch = generate_synthetic_batch(num_groups=config.num_groups, responses_per_prompt=config.responses_per_prompt)
    ...
    for i in range(4):
        print(f"  Response {i}: reward={group_0_rewards[i]:.3f}, advantage={group_0_advantages[i]:.3f}")
