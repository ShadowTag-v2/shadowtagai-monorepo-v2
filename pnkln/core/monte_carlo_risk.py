...
class ProbabilityModelA:
    ...
    async def evaluate(self, decision: dict) -> ProbabilityModelResult:
        ...
        score = sum(1 for kw in high_freq_keywords if kw in decision_text) / len(high_freq_keywords)
        ...
        return ProbabilityModelResult(probability_level=ProbabilityLevel.A_FREQUENT, severity_level=severity, score=score, evidence=evidence)
...
class ProbabilityModelB:
    ...
    async def evaluate(self, decision: dict) -> ProbabilityModelResult:
        ...
        score = sum(1 for kw in likely_keywords if kw in decision_text) / max(len(likely_keywords), 1)
        ...
        return ProbabilityModelResult(probability_level=ProbabilityLevel.B_LIKELY, severity_level=severity, score=score, evidence=evidence)
...
class ProbabilityModelC:
    ...
    async def evaluate(self, decision: dict) -> ProbabilityModelResult:
        ...
        score = sum(1 for kw in occasional_keywords if kw in decision_text) / max(len(occasional_keywords), 1)
        ...
        return ProbabilityModelResult(probability_level=ProbabilityLevel.C_OCCASIONAL, severity_level=severity, score=score, evidence=evidence)
...
class ProbabilityModelD:
    ...
    async def evaluate(self, decision: dict) -> ProbabilityModelResult:
        ...
        score = sum(1 for kw in seldom_keywords if kw in decision_text) / max(len(seldom_keywords), 1)

        severity = SeverityLevel.IV_NEGLIGIBLE
        evidence = f"Seldom probability score: {score:.2f}"

        return ProbabilityModelResult(probability_level=ProbabilityLevel.D_SELDOM, severity_level=severity, score=score, evidence=evidence)
...
class ProbabilityModelE:
    ...
    async def evaluate(self, decision: dict) -> ProbabilityModelResult:
        ...
        score = sum(1 for kw in unlikely_keywords if kw in decision_text) / max(len(unlikely_keywords), 1)

        severity = SeverityLevel.IV_NEGLIGIBLE
        evidence = f"Unlikely probability score: {score:.2f}"

        return ProbabilityModelResult(probability_level=ProbabilityLevel.E_UNLIKELY, severity_level=severity, score=score, evidence=evidence)
...
class MonteCarloRiskAssessment:
    ...
    async def evaluate_scenarios(self, decision: dict) -> MonteCarloResult:
        ...
        selected_probability = max(probability_dist.keys(), key=lambda p: probability_dist[p])
        ...
        final_risk_level = self.jr_engine.assess_risk(selected_probability, selected_severity)
...
async def example_usage():
    ...
    decision2 = {"text": "This change will constantly affect production systems repeatedly every time"}
    ...
    batch_results = await asyncio.gather(*[assessor.evaluate_scenarios(d) for d in decisions])
    ...
    for i, result in enumerate(batch_results):
        print(f"  Decision {i}: {result.final_risk_level.value} ({result.execution_time_us:.1f}μs)")
