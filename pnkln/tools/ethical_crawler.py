...
class EthicalCrawler:
    ...
    def _set_default_rate_limits(self) -> None:
        ...
        defaults = {
            "youtube": RateLimitConfig("youtube", requests_per_minute=60, requests_per_hour=1000),
            "twitter": RateLimitConfig("twitter", requests_per_minute=100, requests_per_hour=5000),
            "news_api": RateLimitConfig("news_api", requests_per_minute=30, requests_per_hour=500),
            "rss_feeds": RateLimitConfig("rss_feeds", requests_per_minute=10, requests_per_hour=100),
            "reddit": RateLimitConfig("reddit", requests_per_minute=60, requests_per_hour=1000),
        }

        for source, config in defaults.items():
            self.rate_limits[source] = config

    def configure_rate_limit(self, source_name: str, requests_per_minute: int, requests_per_hour: int = None, requests_per_day: int = None) -> None:
        """
    ...
    async def fetch_robots_txt(self, domain: str) -> RobotsTxtRules:
        ...
        rules = RobotsTxtRules(domain=domain, user_agent="*", disallowed_paths={"/admin", "/private", "/api/internal"}, crawl_delay_seconds=1.0)

        self.robots_cache[domain] = rules
        return rules

    def check_robots_txt_compliance(self, url: str, user_agent: str = "*") -> tuple[bool, str]:
        """
    ...
    def check_rate_limit_compliance(self, source_name: str) -> tuple[bool, float]:
        ...
        if source_name not in self.rate_limits:
            logger.warning(f"No rate limit configured for {source_name}, using conservative defaults")
            # Default: 10 req/min
            self.rate_limits[source_name] = RateLimitConfig(source_name=source_name, requests_per_minute=10)
    ...
    async def check_crawl_allowed(self, request: CrawlRequest) -> CrawlResult:
        ...
        robots_allowed, robots_reason = self.check_robots_txt_compliance(request.url, request.user_agent)
        ...
        if not robots_allowed:
            ...
            reason = f"Rate limit exceeded for {request.source_name}, retry in {delay_seconds:.0f}s"
        else:
...
async def example_usage():
    ...
    for i in range(5):
        request = CrawlRequest(url=f"https://youtube.com/watch?v={i}", source_name="youtube")
    ...
    print(f"  Requests last minute: {status['requests_last_minute']}/{status['limit_per_minute']}")
