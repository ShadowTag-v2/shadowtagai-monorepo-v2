...
def shadowtag_embed_video(
  ...
  coefficient_range: Annotated[
    str, "DCT coefficient range (default '15-25')"
  ] = "15-25",
  qim_delta: Annotated[int, "QIM quantization delta (default 10)"] = 10,
  output_path: Annotated[
    str | None, "Output path (default: input_watermarked.ext)"
  ] = None,
  ...
  logger.info(
    f"ShadowTag video watermark: {video_path} (block={block_size}×{block_size}, coeffs={coefficient_range})"
  )
...
def shadowtag_embed_audio(
  ...
  frequency_range: Annotated[
    str, "Ultrasonic frequency range (default '18000-22000' Hz)"
  ] = "18000-22000",
  ...
  """
  logger.info(
    f"ShadowTag audio watermark: {audio_path} (freq={frequency_range}Hz, amp={amplitude})"
  )
...
def shadowtag_verify(
  ...
  expected_payload: Annotated[
    str | None, "Expected watermark payload (for verification)"
  ] = None,
  ...
  if expected_payload and result["payload"] != expected_payload:
    logger.warning(
      f"Payload mismatch: expected {expected_payload}, got {result['payload']}"
    )

  logger.info(
    f"Verification complete: detected={result['watermark_detected']}, conf={result['confidence']:.2f}"
  )
...
def example_usage():
  ...
  video_result = shadowtag_embed_video(
    ...
    watermark_data="eyJ1c2VyX2lkIjogIjEyMyJ9",
    block_size=8,
    coefficient_range="15-25",
  )
  ...
  # Audio watermarking
  print("\n=== Audio Watermarking ===")
  audio_result = shadowtag_embed_audio(
    audio_path="/mock/audio.wav",
    watermark_data="eyJhcnRpc3QiOiAiSmFuZSJ9",
    frequency_range="18000-22000",
  )
