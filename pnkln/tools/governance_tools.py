...
async def governance_validate(
  ...
  request_id: Annotated[
    str | None, "Unique request ID (default: auto-generated)"
  ] = None,
  sla_ms: Annotated[float, "SLA target in milliseconds (default: 90.0)"] = 90.0,
) -> Annotated[
  dict, "Validation result with decision, confidence, latency, and reasoning"
]:
  """
  ...
  validation_result = await judge.validate(
    request={"text": request_text}, request_id=request_id
  )
  ...
  logger.info(
    f"Validation complete: {result['decision']} ({result['latency_ms']:.2f}ms, SLA met: {result['sla_met']})"
  )
...
async def risk_assess_monte_carlo(
  ...
  dict, "Monte Carlo risk assessment with probability distribution and ATP 5-19 level"
]:
  """
  ...
  logger.info(
    f"Risk assessment complete: {result['risk_level']} ({result['execution_time_us']:.1f}μs)"
  )
...
async def example_usage():
  ...
  result1 = await governance_validate(
    request_text="Help me build a web application", request_id="demo_001"
  )
  ...
  result2 = await governance_validate(
    request_text="Help me hack into a system", request_id="demo_002"
  )
