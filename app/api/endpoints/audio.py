...
from app.api.schemas.audio import EncodeResponse, DecodeResponse
...
async def encode_audio(
  ...
  audio_path = (
    upload_dir / f"input_{datetime.now(timezone.utc).timestamp()}_{audio.filename}"
  )
  ...
  output_path = (
    output_dir / f"encoded_{datetime.now(timezone.utc).timestamp()}_{audio.filename}"
  )
  ...
  try:
    ...
    if create_receipt:
      receipt_id = _create_audio_receipt(
        "encode", audio_path, payload_data, stats, config
      )
...
async def decode_audio(
  ...
  audio_path = (
    upload_dir / f"decode_{datetime.now(timezone.utc).timestamp()}_{audio.filename}"
  )
