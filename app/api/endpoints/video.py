...
async def encode_video(
  ...
  video_path = (
    upload_dir / f"input_{datetime.now(timezone.utc).timestamp()}_{video.filename}"
  )
  ...
  output_path = (
    output_dir / f"encoded_{datetime.now(timezone.utc).timestamp()}_{video.filename}"
  )
  ...
  try:
    ...
    if create_receipt:
      receipt_id = _create_video_receipt(
        "encode", video_path, payload_data, stats, config
      )
...
async def decode_video(
  ...
  video_path = (
    upload_dir / f"decode_{datetime.now(timezone.utc).timestamp()}_{video.filename}"
  )
...
async def estimate_capacity(
  ...
  video_path = (
    upload_dir / f"capacity_{datetime.now(timezone.utc).timestamp()}_{video.filename}"
  )
