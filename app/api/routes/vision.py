...
async def build_feature_library(request: BuildFeatureLibraryRequest):
    ...
    try:
        ...
        for face_data in request.faces:
            ...
            # Extract embedding
            faces = await tegu.recognize_faces(image_path, feature_library=None, threshold=0.0)
            if faces:
        ...
        logger.error("feature_library_build_failed", error=str(e), library_id=request.library_id)
        raise HTTPException(status_code=500, detail=f"Feature library build failed: {str(e)}")
