...
async def get_recent_conversations(
  ...
  query = RecentChatsQuery(
    limit=limit, project_id=project_id, include_incognito=include_incognito
  )
...
async def update_conversation(
  ...
  conversation = await conversation_service.update_conversation(
    db, conversation_id, user_id, conversation_data
  )
...
async def add_message(
  ...
  if message_data.conversation_id != conversation_id:
    raise HTTPException(
      status_code=status.HTTP_400_BAD_REQUEST, detail="Conversation ID mismatch"
    )
