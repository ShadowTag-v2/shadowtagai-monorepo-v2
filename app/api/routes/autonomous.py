...
async def plan_mission(request: MissionPlanningRequest):
  ...
  try:
    ...
    logger.error(
      "mission_planning_failed", error=str(e), mission_type=request.mission_type
    )
...
async def flight_command(command: FlightCommand):
  ...
  try:
    ...
    logger.error(
      "flight_command_failed", error=str(e), command_type=command.command_type
    )
...
async def _generate_mission_waypoints(
  ...
  if mission_type == "patrol":
    ...
    # Grid pattern
    return [
      Waypoint(x=x, y=y, z=2) for x in range(0, 100, 20) for y in range(0, 100, 20)
    ]
  else:
...
async def _validate_mission_safety(waypoints: list) -> list:
  ...
  return [
    {"segment": f"{i}-{i + 1}", "safety_score": 0.95} for i in range(len(waypoints) - 1)
  ]
...
async def _generate_mission_reasoning(
  ...
  avg_safety = (
    sum(v["safety_score"] for v in safety_validations) / len(safety_validations)
