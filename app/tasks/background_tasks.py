...
async def synthesize_user_memories_task(user_id: int, project_id: int = None):
  ...
  async with AsyncSessionLocal() as db:
    try:
      ...
      logger.info(
        f"Completed memory synthesis for user {user_id}: {result.total_memories} memories synthesized"
      )
...
async def summarize_conversation_task(conversation_id: int):
  ...
  async with AsyncSessionLocal() as db:
    try:
      ...
      logger.info(
        f"Completed conversation summarization for conversation {conversation_id}"
      )
...
async def auto_extract_memories_task(conversation_id: int, user_id: int):
  ...
  async with AsyncSessionLocal() as db:
    try:
      ...
      logger.error(
        f"Error extracting memories from conversation {conversation_id}: {e}"
      )
      return {"conversation_id": conversation_id, "error": str(e), "success": False}
    ...
    try:
      ...
      stmt = select(Project).where(
        and_(
          ...
          (Project.last_synthesis_at == None) | (Project.last_synthesis_at < threshold),
        )
      )
      ...
      logger.info(
        f"Periodic synthesis completed: {synthesized_count} projects synthesized"
      )
