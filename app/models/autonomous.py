...
class PathPlanningRequest(BaseModel):
  ...
  safety_margin: float = Field(
    2.0, ge=0.5, le=10.0, description="Safety margin around obstacles (meters)"
  )
  validate_safety: bool = Field(True, description="Use Judge #6 for safety validation")
  algorithm: str = Field(
    "a_star", description="Planning algorithm (a_star, rrt, rrt_star)"
  )
...
class LocalizationRequest(BaseModel):
  ...
  lidar_scan: str | None = Field(
    None, description="Base64-encoded point cloud (PCD format)"
  )
  map_id: str
...
class Pose(BaseModel):
  ...
  covariance: list[float] | None = Field(
    None, description="6x6 covariance matrix (flattened)"
  )
...
class Obstacle(BaseModel):
  ...
  classification: str | None = Field(
    None, description="Obstacle type (person, vehicle, etc.)"
  )
...
class ObstacleDetectionRequest(BaseModel):
  ...
  detection_range: float = Field(
    50.0, ge=10.0, le=100.0, description="Detection range (meters)"
  )
...
class MissionPlanningRequest(BaseModel):
  ...
  mission_type: str = Field(
    ..., description="Mission type (patrol, inspection, delivery, search_rescue)"
  )
  area_bounds: dict[str, Any] | None = Field(
    None, description="Geographic bounds {min_lat, max_lat, min_lon, max_lon}"
  )
  waypoints: list[Waypoint] | None = Field(None, description="Pre-defined waypoints")
  map_id: str
  constraints: dict[str, Any] | None = Field(
    None, description="Mission constraints (max_altitude, no_fly_zones, etc.)"
  )
...
class BuildMapRequest(BaseModel):
  ...
  map_resolution: float = Field(
    0.1, ge=0.05, le=1.0, description="Map resolution (meters)"
  )
...
class SimulationRequest(BaseModel):
  ...
  scenario: str = Field(
    ..., description="Simulation scenario (warehouse, city, outdoor)"
  )
  ...
  real_time_factor: float = Field(
    1.0, ge=0.1, le=10.0, description="Simulation speed multiplier"
  )
...
class FlightCommand(BaseModel):
  ...
  command_type: str = Field(
    ..., description="Command type (arm, disarm, takeoff, land, goto, rtl)"
  )
  parameters: dict[str, Any] | None = Field(
    None, description="Command-specific parameters"
  )
