...
class ChainExecutor:
  """
  High-level executor for the PNKLN kernel chain.
  ...
  async def execute_decision(
    ...
    try:
      ...
      if total_latency_ms > settings.max_latency_p99_ms:
        raise KernelChainError(
          f"Chain exceeded p99 latency SLA: {total_latency_ms:.2f}ms > {settings.max_latency_p99_ms}ms"
        )

      if total_cost_usd > settings.max_cost_per_decision:
        raise KernelChainError(
          f"Chain exceeded cost SLA: ${total_cost_usd:.6f} > ${settings.max_cost_per_decision}"
        )
