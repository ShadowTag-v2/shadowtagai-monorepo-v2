...
class FunnelRedesign(BaseModel):
  ...
  stage: str = Field(
    description="Funnel stage (awareness, consideration, decision, retention)"
  )
...
class LeverageStrategy(BaseModel):
  ...
  description: str
  viral_coefficient: float | None = Field(
    None, description="K-factor for viral strategies"
  )
...
class WealthAccelerator:
  ...
  def analyze_business(
    ...
    net_improvement = total_opportunity - (
      total_leak_impact * 0.3
    )  # Assume 70% leak recovery
