...
class StrategicEffect(BaseModel):
  ...
  category: str = Field(
    ..., description="Effect category (e.g., 'Efficiency', 'Economic Leverage')"
  )
  metric: str = Field(..., description="Metric name")
  improvement: str = Field(..., description="Improvement description")
  quantified_value: str | None = Field(
    None, description="Quantified value if applicable"
  )
...
class LegalPositioning(BaseModel):
  ...
  ownership_rights: list[str] = Field(
    default_factory=list, description="Ownership rights"
  )
...
class CompetitiveAdvantage(BaseModel):
  ...
  advantage: str
  description: str
  barrier_to_entry: str = Field(
    ..., description="Barrier to entry level (Low, Medium, High)"
  )
...
class PartnershipModel(BaseModel):
  ...
  partner_type: str = Field(
    ..., description="Type of partner (Provider, Customer, Both)"
  )
  relationship: str = Field(..., description="Relationship description")
  revenue_sharing: str | None = Field(
    None, description="Revenue sharing model if applicable"
  )
