...
class IngestionMetrics(BaseModel):
  ...
  average_relevance_score: float = Field(
    ..., ge=0, le=100, description="Average relevance score (0-100)"
  )
  timeliness_score: float = Field(
    ..., ge=0, le=100, description="Data timeliness score (0-100)"
  )
  completeness_percentage: float = Field(
    ..., ge=0, le=100, description="Data completeness percentage"
  )
...
class EthicalComplianceMetrics(BaseModel):
  ...
  robots_txt_compliance: float = Field(
    ..., ge=0, le=100, description="robots.txt compliance percentage"
  )
  rate_limiting_adherence: float = Field(
    ..., ge=0, le=100, description="Rate limiting adherence percentage"
  )
...
class SourceCoverage(BaseModel):
  ...
  coverage_percentage: float = Field(
    ..., ge=0, le=100, description="Coverage percentage of available data"
  )
  reliability_score: float = Field(
    ..., ge=0, le=100, description="Source reliability score"
  )
  tier_distribution: dict[DataTier, int] = Field(
    ..., description="Distribution across data tiers"
  )
...
class TierClassification(BaseModel):
  ...
  quality_score: float = Field(
    ..., ge=0, le=100, description="Quality score for this tier"
  )
...
class GKEArchitecture(BaseModel):
  ...
  cluster_name: str = Field(default="aiyou-ingestion-cluster")
  namespace: str = Field(default="intelligence-pipeline")
  cron_schedule: str = Field(
    default="0 2 * * *", description="Cron schedule (2 AM daily)"
  )
  ...
  success_rate: float = Field(
    ..., ge=0, le=100, description="Job success rate percentage"
  )
...
class QualityGates(BaseModel):
  ...
  maximum_cost_per_item: float = Field(
    ..., description="Maximum acceptable cost per item"
  )
  minimum_relevance_score: float = Field(
    ..., ge=0, le=100, description="Minimum relevance score"
  )
...
class AMBriefingMetrics(BaseModel):
  ...
  on_time_delivery_rate: float = Field(
    ..., ge=0, le=100, description="On-time delivery percentage"
  )
  average_items_per_briefing: int = Field(..., description="Average items per briefing")
  user_engagement_score: float = Field(
    ..., ge=0, le=100, description="User engagement score"
  )
  actionability_score: float = Field(
    ..., ge=0, le=100, description="Intelligence actionability score"
  )
...
class IngestionIntegration(BaseModel):
  ...
  called_by_services: list[str] = Field(
    ..., description="Services that trigger ingestion"
  )
  feeds_into_services: list[str] = Field(
    ..., description="Services that consume ingested data"
  )
  ...
  data_handoff_latency_ms: int = Field(
    ..., description="Average data handoff latency in ms"
  )
...
class GeminiIngestionLayer(BaseModel):
  ...
  confidence_score: float = Field(
    ..., ge=0, le=100, description="Overall analysis confidence (target ≥60%)"
  )
