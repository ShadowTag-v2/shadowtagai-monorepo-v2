...
class InfrastructureLayer(BaseModel):
  ...
  layer: LayerType
  platform: str = Field(
    ..., description="Platform name (e.g., Starlink LEO satellites)"
  )
...
class TechnicalMetrics(BaseModel):
  ...
  compute_utilization: float = Field(
    ..., ge=0, le=100, description="Average compute utilization percentage"
  )
  latency_reduction_ms: int = Field(
    ..., description="Latency reduction in milliseconds"
  )
  energy_efficiency_improvement: float = Field(
    ..., ge=0, le=100, description="Energy efficiency improvement percentage"
  )
...
class InfrastructureDeployment(BaseModel):
  ...
  total_capex: int = Field(..., description="Total CAPEX in USD")
  roi_period_years: float = Field(
    ..., description="Return on investment period in years"
  )
