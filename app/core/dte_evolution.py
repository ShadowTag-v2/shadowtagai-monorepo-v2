...
class DTEEngine:
  ...
  def evolve(
    ...
    """
    logger.info(
      f"Starting DTE evolution (strategy={strategy.value}, iterations={iterations})"
    )
    ...
    logger.info(
      f"Generation {self.current_generation}: best={best_score:.3f}, avg={average_score:.3f}, improvement={improvement:+.3f}"
    )
  ...
  def get_evolution_summary(self) -> dict[str, Any]:
    ...
    return {
      ...
      "strategies_used": list(
        set(r.strategy_used.value for r in self.evolution_history)
      ),
    }
...
class GRPOSimulator:
  ...
  def compare_with_ppo(
    self, rewards: list[float], epsilon: float = 0.2
  ) -> dict[str, Any]:
    """
