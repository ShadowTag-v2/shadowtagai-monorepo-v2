...
class GPTRAMMemory:
  ...
  async def store_interaction(
    ...
    try:
      ...
      await self.redis_client.setex(
        key, ttl or self.memory_ttl, json.dumps(interaction_data)
      )

      # Add to session index
      await self.redis_client.zadd(
        f"gptram:index:{session_id}", {key: datetime.now(timezone.utc).timestamp()}
      )
  ...
  async def get_memory_stats(self, session_id: str) -> dict[str, Any]:
    ...
    try:
      ...
      has_reasoning_graph = await self.redis_client.exists(
        f"gptram:rot:graph:{session_id}"
      )
