...
class SourceCoverageAnalyzer:
    ...
    async def get_source_quality(
        ...
        query = (
            ...
            .where(
                and_(
                    SourceMetric.source_type == source_type,
                    SourceMetric.timestamp >= cutoff_time
                )
            )
        )
        ...
        for source, min_count in min_thresholds.items():
            ...
            if actual_count < min_count:
                ...
                gaps.append({
                    ...
                    'recommendation': self._get_gap_recommendation(
                        source,
                        actual_count,
                        min_count
                    ),
    ...
    async def get_tier_distribution(
        ...
        tier_percentages = {
            tier: round((count / total * 100) if total > 0 else 0, 2)
        ...
        is_healthy = (
            tier_percentages['tier_1'] >= 20 and
            tier_percentages['tier_2'] >= 30 and
            tier_percentages['tier_3'] <= 50
        )
    ...
    async def get_coverage_report(self) -> dict[str, Any]:
        ...
        for source_type in SourceType:
            if getattr(coverage, source_type.value, 0) > 0:
                quality_by_source[source_type.value] = await self.get_source_quality(
                    source_type.value,
                    24
                )
