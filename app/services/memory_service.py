...
class MemoryService:
  ...
  async def create_memory(
    self, db: AsyncSession, user_id: int, memory_data: MemoryCreate
  ) -> Memory:
    """
    Create a new memory.
  ...
  async def update_memory(
    self, db: AsyncSession, memory_id: int, user_id: int, memory_data: MemoryUpdate
  ) -> Memory | None:
    """Update a memory."""
  ...
  async def synthesize_user_memories(
    self, db: AsyncSession, user_id: int, project_id: int | None = None
  ) -> MemorySynthesisResponse:
    """
    ...
    memories = await self.get_memories(
      db, user_id, project_id=project_id, limit=settings.max_memory_items_per_project
    )
