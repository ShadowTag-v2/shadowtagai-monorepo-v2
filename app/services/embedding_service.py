...
class EmbeddingService:
  ...
  async def generate_embeddings_batch(self, texts: list[str]) -> np.ndarray:
    ...
    embeddings = await loop.run_in_executor(
      None,
      lambda t: self._model.encode(
        t, normalize_embeddings=True, show_progress_bar=True
      ),
      texts,
    )
    return embeddings
  ...
  def create_index(self, embeddings: np.ndarray) -> faiss.IndexFlatIP:
    ...
    if embeddings.shape[1] != self.dimension:
      logger.warning(
        f"Embedding dimension {embeddings.shape[1]} != {self.dimension}. Updating dimension."
      )
  ...
  def search(
    self, index: faiss.IndexFlatIP, query_embedding: np.ndarray, top_k: int = 10
