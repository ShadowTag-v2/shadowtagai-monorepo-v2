...
class TeguClient:
  ...
  def __init__(
    ...
    logger.info(
      "tegu_client_initialized", model=model, gpu=gpu, model_path=self.model_path
    )
  ...
  async def classify_image(
    ...
    if self.model_name in ["resnet50", "vgg16"]:
      ...
      decoded = keras.applications.inception_v3.decode_predictions(
        predictions, top=top_k
      )[0]
  ...
  async def detect_objects(
    ...
    try:
      ...
      raise ImportError(
        "OpenCV not installed. Install with: pip install opencv-python>=4.5.0"
      )
    ...
    if self.model_name == "yolo_v3":
      net = cv2.dnn.readNetFromDarknet(
        f"{self.model_path}/yolov3.cfg", f"{self.model_path}/yolov3.weights"
      )
      ...
      net = cv2.dnn.readNetFromTensorflow(
        f"{self.model_path}/ssd_mobilenet.pb", f"{self.model_path}/ssd_mobilenet.pbtxt"
      )
  ...
  async def classify_video(
    ...
    """
    if (
      self.model_type != "video_classification"
      and self.model_type != "image_classification"
    ):
    ...
    logger.info(
      ...
      video=video_path,
      duration=duration,
      top_class=top_classes[0]["class"] if top_classes else None,
    )
