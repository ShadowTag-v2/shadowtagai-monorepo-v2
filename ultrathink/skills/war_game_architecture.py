...
class WarGameArchitectureSkill(BaseSkill):
    ...
    async def execute(self, skill_input: SkillInput) -> SkillOutput:
        ...
        plan = self._create_architecture_plan(
            problem=problem, approaches=approaches, selected=selected, evaluations=evaluations, war_game_results=war_game_results
        )
        ...
        return SkillOutput(
            ...
            metadata={"architecture_plan": plan.__dict__, "diagram": diagram, "approaches_count": len(approaches), "go_no_go": plan.go_no_go},
        )

    def get_activation_triggers(self) -> list[str]:
        """Phrases that activate this skill."""
        return ["architect this", "how should we structure", "design the system", "plan the architecture", "war game", "create architecture"]

    def _deconstruct_problem(self, problem: str, parameters: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "stakeholders": parameters.get("stakeholders", ["Users", "Developers", "Business"]),
            "constraints": parameters.get("constraints", ["Time", "Resources", "Technical"]),
            "success_criteria": parameters.get("success_criteria", ["Meets requirements", "Scalable", "Maintainable"]),
            "assumptions": ["Available resources are sufficient", "Technical constraints are understood", "Stakeholder alignment exists"],
        }

    def _generate_approaches(self, deconstruction: dict[str, Any]) -> list[dict[str, Any]]:
        """
    ...
    def _evaluate_approaches(self, approaches: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
    ...
    def _select_architecture(self, approaches: list[dict[str, Any]], evaluations: list[dict[str, Any]]) -> dict[str, Any]:
        """
        ...
        selected_approach = next(a for a in approaches if a["name"] == best_eval["approach_name"])
    ...
    def _war_game(self, architecture: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "failure_points": [
                {"scenario": "10x traffic spike", "failure": "Response time degradation", "mitigation": "Implement auto-scaling and caching"},
                {"scenario": "Database failure", "failure": "Data unavailability", "mitigation": "Add database replication and fallback"},
                {"scenario": "Network partition", "failure": "Service isolation", "mitigation": "Implement circuit breakers"},
            ],
    ...
    def _create_architecture_plan(
        ...
        return ArchitecturePlan(
            ...
            mitigation_strategies=[fp["mitigation"] for fp in war_game_results["failure_points"]],
