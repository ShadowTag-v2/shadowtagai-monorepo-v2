...
async def example_1_orchestrator_basic():
  ...
  result = await orchestrator.execute(
    task="Review this API design for elegance and simplicity",
...
async def example_2_specific_agent():
  ...
  context = AgentContext(
    ...
    metadata={
      "current_widgets": 50,
      "user_complaints": ["too complex", "overwhelming", "can't find features"],
    },
  )
...
async def example_3_skill_execution():
  ...
  result = await orchestrator.execute_skill(
    ...
    parameters={
      ...
      "current_offers": ["Email course at $97"],
    },
  )
...
async def example_4_strategic_decision():
  ...
  result = await orchestrator.execute(
    task="Should we pivot from B2C to B2B, or expand our B2C offering?",
    task_type=TaskType.STRATEGIC_DECISION,
  )
...
async def example_5_holistic_initiative():
  ...
  result = await orchestrator.execute(
    task="Launch a new SaaS product for project management",
    task_type=TaskType.HOLISTIC_INITIATIVE,
  )
