...
class UltrathinkOrchestrator:
    ...
    async def execute(self, task: str, task_type: TaskType | None = None, context: dict[str, Any] | None = None) -> dict[str, Any]:
        """
        ...
        agent_context = AgentContext(
            task=task, role=self._get_primary_role(task_type), metadata=context or {}, constraints=[], stakeholders=[], success_criteria=[]
        )
    ...
    def _classify_task(self, task: str) -> TaskType:
        ...
        # Design keywords
        if any(word in task_lower for word in ["design", "refine", "elegant", "beautiful", "audit"]):
            return TaskType.DESIGN_REVIEW

        # Architecture keywords
        if any(word in task_lower for word in ["architect", "structure", "plan", "system", "war game"]):
            return TaskType.ARCHITECTURE_PLANNING

        # Monetization keywords
        if any(word in task_lower for word in ["revenue", "monetize", "money", "pricing", "funnel"]):
            return TaskType.MONETIZATION_STRATEGY

        # Reasoning keywords
        if any(word in task_lower for word in ["analyze", "reason", "think", "solve", "decide"]):
        ...
        # Strategic keywords
        if any(word in task_lower for word in ["strategy", "strategic", "decision", "consensus"]):
            return TaskType.STRATEGIC_DECISION

        # Holistic keywords
        if any(word in task_lower for word in ["launch", "initiative", "transformation", "holistic"]):
    ...
    async def _execute_architecture_planning(self, context: AgentContext) -> AgentResponse:
    ...
    async def _execute_holistic_initiative(self, context: AgentContext) -> dict[AgentRole, AgentResponse]:
        """Execute holistic initiative via Cross-Functional Task Force."""
        return await self.task_force.execute_mission(context)

    async def execute_skill(self, skill_type: SkillType, content: str, parameters: dict[str, Any] | None = None) -> SkillOutput:
        """
        ...
        skill_input = SkillInput(skill_type=skill_type, content=content, parameters=parameters or {})
    ...
    def get_agent_status(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            role.value: {"execution_count": len(agent.get_execution_history()), "system_prompt_preview": agent.get_system_prompt()[:200] + "..."}
            for role, agent in self.agents.items()
        }

    def get_skill_status(self) -> dict[str, dict[str, Any]]:
        """Get status of all skills."""
        return {skill_type.value: skill.get_metadata() for skill_type, skill in self.skills.items()}
