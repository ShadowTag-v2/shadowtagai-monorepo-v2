...
class BaseAgent(ABC):
  ...
  def __init__(
    self, role: AgentRole, system_prompt: str, config: UltrathinkConfig | None = None
  ):
    self.role = role
  ...
  def assess_elegance(self, solution: str) -> dict[str, Any]:
    ...
    assessment = {
      criterion: {"question": question, "score": 0.0}
  ...
  def create_reasoning_path(
    self, method: ReasoningMethod, steps: list[str], confidence: float = 0.0
  ) -> ReasoningPath:
    """
