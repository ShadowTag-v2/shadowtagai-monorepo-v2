...
class MultiAgentDebate:
    ...
    async def debate(self, context: AgentContext, rounds: int = 3) -> DebateResult:
        ...
        for round_num in range(1, rounds + 1):
            ...
            msg_a = self._generate_argument(agent_a, context.task, round_num, transcript, is_affirmative=True)
            transcript.append(msg_a)

            # Agent B challenges
            msg_b = self._generate_argument(agent_b, context.task, round_num, transcript, is_affirmative=False)
    ...
    def _generate_argument(self, agent: dict, problem: str, round_num: int, transcript: list[DebateMessage], is_affirmative: bool) -> DebateMessage:
