...
class PanelGPTDebate:
    ...
    return [
      {
        "name": "Alice",
        "role": "Optimist",
        "perspective": "Sees opportunity and upside",
        "bias": "Positive, growth-oriented",
      },
      {
        "name": "Bob",
        "role": "Skeptic",
        "perspective": "Challenges assumptions, points out risks",
        "bias": "Critical, risk-aware",
      },
      {
        "name": "Carol",
        "role": "Pragmatist",
        "perspective": "Focuses on feasibility and ROI",
        "bias": "Practical, execution-focused",
      },
      {
        "name": "David",
        "role": "Moderator",
        "perspective": "Guides discussion and summarizes",
        "bias": "Neutral, synthesis-oriented",
      },
    ]
  ...
  async def debate(self, context: AgentContext, rounds: int = 3) -> DebateResult:
    ...
    for round_num in range(1, rounds + 1):
      ...
      for expert in self.experts:
        ...
        message = self._generate_expert_response(
          expert=expert,
          problem=problem,
          round_num=round_num,
  ...
  def _generate_expert_response(
    ...
    content = (
      f"{expert['name']} ({expert['role']}): Contributing {expert['perspective']}"
    )
  ...
  def _assess_debate_confidence(
    self, transcript: list[DebateMessage], consensus: str
  ) -> float:
