...
class ValidationConfig:
    ...
    endpoint: str = "https://judge6.pnkln.ai/enforce"
    iterations: int = 1000
    concurrency: int = 50
...
class LatencyValidator:
    ...
    async def validate(self) -> ValidationReport:
        ...
        timeout = httpx.Timeout(connect=self.config.connect_timeout, read=self.config.read_timeout, write=5.0, pool=5.0)

        limits = httpx.Limits(max_connections=self.config.concurrency * 2, max_keepalive_connections=self.config.concurrency)
    ...
    async def _run_with_progress(self, tasks: list):
        ...
        for coro in asyncio.as_completed(tasks):
            ...
            if completed % report_interval == 0:
                logger.info(f"Progress: {completed}/{self.config.iterations} ({100 * completed // self.config.iterations}%)")
    ...
    async def _try_request(self, client: httpx.AsyncClient, request_id: int, attempt: int) -> LatencyResult:
    ...
    def _print_report(self, report: ValidationReport):
        ...
        logger.info(f"  Successful: {report.successful_requests} ({report.success_rate * 100:.2f}%)")
        ...
        logger.info(f"  Error rate < {self.config.error_rate_threshold * 100}%: {err_status} (actual: {report.error_rate * 100:.2f}%)")
...
async def main():
    ...
    config = ValidationConfig(
        endpoint="https://judge6.pnkln.ai/enforce",
        iterations=1000,
        concurrency=50,
        connect_timeout=5.0,
        read_timeout=10.0,
