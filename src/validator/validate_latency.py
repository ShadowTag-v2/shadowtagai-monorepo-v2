...
class LatencyValidator:
  ...
  async def _run_with_progress(self, tasks: list):
    ...
    for coro in asyncio.as_completed(tasks):
      ...
      if completed % report_interval == 0:
        logger.info(
          f"Progress: {completed}/{self.config.iterations} ({100 * completed // self.config.iterations}%)"
        )
  ...
  def _print_report(self, report: ValidationReport):
    ...
    logger.info(
      f"  Failed:     {report.failed_requests} ({report.error_rate * 100:.2f}%)"
    )
    ...
    logger.info(
      f"  Throughput: {report.total_requests / report.duration_seconds:.2f} req/s"
    )
    ...
    logger.info(
      f"  Error rate < {self.config.error_rate_threshold * 100}%: {err_status} (actual: {report.error_rate * 100:.2f}%)"
    )
