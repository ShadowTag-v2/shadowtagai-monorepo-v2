...
from app.models.decision import AuditTrail, JudgeSixClassification
    ...
    try:
      ...
      if isinstance(kernel_input.data, JudgeSixClassification):
        ...
        raise KernelChainError(
          f"Invalid input type: expected JudgeSixClassification, got {type(kernel_input.data)}"
        )
      ...
      if compressed_size > self.TARGET_SIZE_BYTES:
        ...
        kernel_input.metadata["size_warning"] = (
          f"Compressed size {compressed_size}B exceeds target {self.TARGET_SIZE_BYTES}B"
        )
