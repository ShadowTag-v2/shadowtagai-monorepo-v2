...
class PromptAnalyzer:
    ...
    def analyze(self, prompt: str, expected_output_tokens: int = 1000) -> AnalysisReport:
        ...
        suggestions = self._generate_suggestions(prompt, word_count, avg_sentence_length, complexity_score, clarity_score)
    ...
    def _generate_suggestions(
        self, prompt: str, word_count: int, avg_sentence_length: float, complexity_score: float, clarity_score: float
    ) -> list[str]:
        ...
        # Sentence length
        if avg_sentence_length > 30:
            suggestions.append(f"Average sentence length is high ({avg_sentence_length:.1f} words). Break into shorter sentences for better clarity")

        # Complexity
        if complexity_score > 0.7:
            suggestions.append("High complexity detected. Simplify language and structure for better results")
        ...
        if not self._has_verification_criteria(prompt):
            suggestions.append("No verification criteria found. Add explicit success criteria (KERNEL: Easy to Verify)")
        ...
        if not self._has_constraints(prompt):
            suggestions.append("No constraints specified. Add what NOT to do, limits, and bounds (KERNEL: Explicit Constraints)")

        # If no suggestions, provide positive feedback
        if not suggestions:
            suggestions.append("Prompt looks well-structured! Consider validating with KernelValidator for detailed KERNEL compliance check")
    ...
    def compare_prompts(self, prompt1: str, prompt2: str, labels: list[str] | None = None) -> dict:
        """
        ...
        return {
            ...
            "comparison": {
                ...
                "cost_difference_usd": analysis2.estimated_cost_usd - analysis1.estimated_cost_usd,
                "time_difference_sec": analysis2.estimated_response_time_sec - analysis1.estimated_response_time_sec,
    ...
    def _determine_winner(self, a1: AnalysisReport, a2: AnalysisReport, labels: list[str]) -> str:
