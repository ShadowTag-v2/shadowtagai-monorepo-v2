...
class PromptMetrics:
    ...
    def compare_to_baseline(self, baseline: "PromptMetrics") -> dict:
        ...
        """
        return {
            "first_try_success_improvement": (self.first_try_success_rate - baseline.first_try_success_rate),
            "revision_reduction": baseline.avg_revisions - self.avg_revisions,
            "token_reduction_pct": ((baseline.avg_tokens - self.avg_tokens) / baseline.avg_tokens * 100 if baseline.avg_tokens > 0 else 0),
            "cost_reduction_pct": ((baseline.avg_cost_usd - self.avg_cost_usd) / baseline.avg_cost_usd * 100 if baseline.avg_cost_usd > 0 else 0),
            "time_reduction_pct": ((baseline.avg_time_sec - self.avg_time_sec) / baseline.avg_time_sec * 100 if baseline.avg_time_sec > 0 else 0),
        }
...
class MetricsTracker:
    ...
    def generate_comparison_report(self, before_ids: list[str], after_ids: list[str]) -> str:
        """
        ...
        report += "IMPROVEMENTS:\n"
        report += f"  First-Try Success: {comparison['first_try_success_improvement']:+.1%}\n"
