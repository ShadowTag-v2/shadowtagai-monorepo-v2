...
class KernelValidator:
  ...
  def _validate_easy_to_verify(self, prompt: str) -> KernelScore:
    ...
    if vague_found:
      ...
      recommendations.append(
        "Replace with measurable criteria (e.g., '3 examples', '<50 lines')"
      )
  ...
  def _validate_reproducible(self, prompt: str) -> KernelScore:
    ...
    if temporal_found:
      ...
      recommendations.append(
        "Replace with specific versions/dates (e.g., 'Python 3.11', '2024 data')"
      )
    else:
  ...
  def _validate_narrow_scope(self, prompt: str) -> KernelScore:
    ...
    if conjunction_count > 5:
      score -= 0.4
      feedback.append(
        f"Many conjunctions ({conjunction_count}), suggests multiple goals"
      )
    ...
    for pattern in multi_goal_patterns:
      if re.search(pattern, prompt.lower()):
        ...
        recommendations.append(
          "Use prompt chaining: separate prompts for each deliverable"
        )
        break
  ...
  def _validate_explicit_constraints(self, prompt: str) -> KernelScore:
    ...
    if constraint_count == 0:
      ...
      recommendations.append(
        "Add constraints section: what NOT to do, limits, forbidden patterns"
      )
      ...
      feedback.append(
        f"Good constraint coverage ({constraint_count} constraint keywords)"
      )
  ...
  def _validate_logical_structure(self, prompt: str) -> KernelScore:
    ...
    sections_found = [
      s
      for s in self.STRUCTURE_SECTIONS
      if s + ":" in prompt.lower() or s.upper() in prompt
    ]
  ...
  def _generate_summary(
    self, overall: float, passed: bool, scores: dict[str, KernelScore]
  ) -> str:
