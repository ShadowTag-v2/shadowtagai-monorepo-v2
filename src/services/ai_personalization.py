...
class PersonalizationEngine:
    ...
    def rules_personalize(self, user_context: dict[str, Any], video_metadata: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        - Product emphasis
        """
        personalization = {"stage": "rules", "runtime_multiplier": 1.0, "scene_sequence": "standard", "product_emphasis": "balanced", "reasons": []}
    ...
    def bandits_select_arc(
        self, video_id: str, available_arcs: list[str], user_segment: str, exploration_mode: bool = True
        ...
        if exploration_mode and random.random() < self.epsilon:
            ...
            return selected_arc, {"strategy": "exploration", "reason": "random_exploration", "epsilon": self.epsilon}
        ...
        return best_arc, {"strategy": "ucb1", "ucb_scores": ucb_scores, "total_pulls": total_pulls, "selected_score": best_score}

    def bandits_record_outcome(self, video_id: str, arc: str, user_segment: str, converted: bool, revenue: float = 0.0):
        """
    ...
    def bandits_get_performance(self, video_id: str) -> dict[str, Any]:
        ...
        for key, stats in self.arm_stats.items():
            if key.startswith(f"{video_id}:"):
                ...
                conversion_rate = (stats["conversions"] / stats["pulls"] * 100) if stats["pulls"] > 0 else 0
    ...
    def generative_create_narrative(
        self, user_profile: dict[str, Any], product_info: dict[str, Any], video_metadata: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        narrative = {
            ...
            "demo_focus": self._select_demo_focus(interests, product_category, purchase_history),
            # Custom benefits messaging
            "benefits_emphasis": self._generate_benefits(household_type, product_category, price),
            # Personalized CTA
    ...
    def _generate_intro(self, name: str, interests: list[str], household_type: str) -> str:
    ...
    def _select_demo_focus(self, interests: list[str], product_category: str, purchase_history: list[str]) -> str:
    ...
    def _generate_benefits(self, household_type: str, category: str, price: float) -> list[str]:
        ...
        if household_type == "family_with_kids":
            benefits.extend(["Safe for all ages", "Built to last through years of use", "Educational and fun"])
        elif household_type == "couple":
            benefits.extend(
                ["Premium quality that looks great in any home", "Energy efficient - saves money long-term", "Stylish design you'll both love"]
            )
        else:
            benefits.extend(
                ["Exceptional quality at a fair price", "Trusted by thousands of satisfied customers", "Hassle-free returns and warranty"]
            )
    ...
    def _generate_scene_sequence(self, user_profile: dict, product_info: dict, video_metadata: dict) -> list[dict[str, Any]]:
        """Generate personalized scene sequence"""
        scenes = [
            {"scene_id": "personalized_intro", "duration_seconds": 5, "content": "Custom greeting with user's name"},
            {"scene_id": "problem_statement", "duration_seconds": 10, "content": "Address user's specific pain point"},
            {"scene_id": "product_demo", "duration_seconds": 30, "content": "Demo focusing on user's interest areas"},
            {"scene_id": "social_proof", "duration_seconds": 15, "content": "Testimonials from similar user segment"},
            {"scene_id": "benefits", "duration_seconds": 20, "content": "Personalized benefits messaging"},
            {"scene_id": "cta", "duration_seconds": 10, "content": "Personalized call-to-action"},
        ]
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("SWIPER AI PERSONALIZATION ENGINE - EXAMPLES")
    print("=" * 80)
    ...
    print(f"Runtime Multiplier: {rules_result['runtime_multiplier']}×")
    print(f"Scene Sequence: {rules_result['scene_sequence']}")
    print(f"Product Emphasis: {rules_result['product_emphasis']}")
    print(f"Reasons: {', '.join(rules_result['reasons'])}")
    ...
    selected_arc, bandit_info = engine.bandits_select_arc("video123", available_arcs, "family_segment")

    print(f"Selected Arc: {selected_arc}")
    print(f"Strategy: {bandit_info['strategy']}")
    print(f"UCB Scores: {bandit_info.get('ucb_scores', {})}")
    ...
    generative_result = engine.generative_create_narrative(user_profile, product_info, video_metadata)

    print(f'Intro Script: "{generative_result["intro_script"]}"')
    print(f"Demo Focus: {generative_result['demo_focus']}")
    print(f"Benefits: {', '.join(generative_result['benefits_emphasis'])}")
    print(f'CTA: "{generative_result["cta_message"]}"')
    print(f"Soundtrack: {generative_result['soundtrack']}")
    print(f"Visual Style: {generative_result['visual_style']}")
    print(f"Cache Key: {generative_result['cache_key']}")

    print("\n" + "=" * 80)
    print("✅ All three personalization stages demonstrated!")
    print("=" * 80)
