...
class JurisdictionRuleEngine:
    ...
    def calculate_deadline(
        ...
        if service_method:
            service_addition = self._get_service_method_addition(jurisdiction, service_method, deadline_type)
        ...
        current_date = self._apply_jurisdiction_specific_rules(current_date, jurisdiction, deadline_type, notes, rule_citations)

        # Calculate total calendar days
        calendar_days = (current_date - trigger_date).days

        # Calculate confidence
        confidence = self._calculate_calculation_confidence(jurisdiction, deadline_type, service_method)
    ...
    def _get_service_method_addition(self, jurisdiction: str, service_method: ServiceMethod, deadline_type: str) -> int:
    ...
    def _apply_jurisdiction_specific_rules(
        self, calculated_date: date, jurisdiction: str, deadline_type: str, notes: list[str], rule_citations: list[str]
    ) -> date:
        """
        ...
        if jurisdiction == "federal":
            while self._is_weekend(calculated_date) or self._is_holiday(calculated_date, jurisdiction):
    ...
    def _calculate_calculation_confidence(self, jurisdiction: str, deadline_type: str, service_method: ServiceMethod | None) -> float:
        """
    ...
    def get_rule_citation(self, jurisdiction: str, deadline_type: str, service_method: ServiceMethod | None = None) -> str:
...
class RuleDatabase:
        ...
        self.rules["federal"] = [
            {
                ...
                "service_additions": {ServiceMethod.MAIL: 3, ServiceMethod.ELECTRONIC: 0, ServiceMethod.PERSONAL: 0},
            },
            {
        ...
        self.rules["CA"] = [
            {
                ...
                "exclude_holidays": True,
                "service_additions": {ServiceMethod.MAIL: 5, ServiceMethod.ELECTRONIC: 2, ServiceMethod.PERSONAL: 0},
            },
            {
                "deadline_type": "appeal",
                "trigger": "entry_of_judgment",
    ...
    def get_rule(self, jurisdiction: str, deadline_type: str, trigger: str | None = None) -> dict[str, Any] | None:
