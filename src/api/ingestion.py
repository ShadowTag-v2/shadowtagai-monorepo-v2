...
class JobStatusResponse(BaseModel):
  ...
  items_by_tier: dict[str, int] = Field(
    default_factory=dict, description="Items by tier"
  )
...
class TriggerRequest(BaseModel):
  ...
  sources_filter: list[SourceType] | None = Field(
    None, description="Limit to specific sources"
  )
...
class SourceConfig(BaseModel):
  ...
  rate_limit_delay: float = Field(
    default=2.0, ge=1.0, description="Rate limit delay (seconds)"
  )
  config: dict[str, Any] = Field(
    default_factory=dict, description="Source-specific config"
  )
...
async def root():
  """API root endpoint."""
  return {
    "service": "Gemini Ingestion Layer API",
    "version": "1.0.0",
    "status": "operational",
    "documentation": "/docs",
  }
...
async def health_check():
  ...
  return {
    ...
    "checks": {"database": "ok", "gcs": "ok", "redis": "ok"},
  }
...
async def trigger_ingestion(request: TriggerRequest):
  ...
  **Note**: Requires appropriate permissions.
  ...
  job_id = f"manual-{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}"
...
async def get_metrics(
  days: int = Query(
    7, ge=1, le=90, description="Number of days for metrics calculation"
  ),
):
  """
...
@app.get(
  "/ingestion/sources", response_model=list[SourceConfig], tags=["Configuration"]
)
