...
from src.workflows import SequentialWorkflowAgent, ParallelWorkflowAgent, LoopWorkflowAgent, ConditionalWorkflowAgent
...
# Create FastAPI app
app = FastAPI(title="Multi-Agent System API", description="API for interacting with Google ADK-based multi-agent systems", version="1.0.0")
...
class WorkflowRequest(BaseModel):
    ...
    workflow_type: str = Field(..., description="Type of workflow: sequential, parallel, loop, conditional")
    user_input: str = Field(..., description="User input to process")
    context: dict[str, Any] | None = Field(default_factory=dict, description="Additional context")
    max_iterations: int | None = Field(5, description="Max iterations for loop workflow")
    max_concurrent: int | None = Field(None, description="Max concurrent tasks for parallel workflow")
...
def create_default_coordinator():
    ...
    coordinator = CoordinatorAgent(name="DefaultCoordinator", sub_agents=[researcher, analyzer, task_executor])
...
async def root():
    ...
    return {
        ...
        "version": "1.0.0",
        "endpoints": {"execute": "/api/agents/execute", "workflow": "/api/workflows/execute", "hierarchy": "/api/agents/hierarchy"},
    }
...
async def execute_agent(request: AgentRequest):
    ...
    try:
        ...
        context = InvocationContext({"user_input": request.user_input, "task_type": request.task_type, **request.context})
        ...
        return AgentResponse(success=True, message=f"Successfully executed agent with {len(events)} events", events=events, agent_hierarchy=hierarchy)
...
async def execute_workflow(request: WorkflowRequest):
    ...
    try:
        ...
        if request.workflow_type == "sequential":
            ...
            workflow = ParallelWorkflowAgent(name="ParallelWorkflow", sub_agents=agents, max_concurrent=request.max_concurrent)

        elif request.workflow_type == "loop":
            agents = [TaskAgent(name="LoopTask", task_description="Iterative Task")]
            workflow = LoopWorkflowAgent(name="LoopWorkflow", sub_agents=agents, max_iterations=request.max_iterations)
        ...
        return AgentResponse(success=True, message=f"Successfully executed {request.workflow_type} workflow with {len(events)} events", events=events)
...
async def execute_custom_agent(agent_type: str, request: AgentRequest):
    ...
    try:
        ...
        return AgentResponse(success=True, message=f"Successfully executed {agent_type} agent with {len(events)} events", events=events)
