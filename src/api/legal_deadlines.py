...
class DeadlineRule(BaseModel):
    ...
    jurisdiction: str = Field(..., description="Jurisdiction (e.g., 'federal', 'CA', 'NY-SDNY')")
    ...
    service_method_additions: dict[str, int] = Field(default_factory=dict, description="Additional days based on service method")
...
class ExtractedDeadline(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid4()), description="Unique deadline ID")
    ...
    status: DeadlineStatus = Field(default=DeadlineStatus.PENDING, description="Current status")
...
async def root():
    """API root endpoint"""
    return {"service": "Zero-Touch Legal Deadline Management API", "version": "1.0.0", "status": "operational", "documentation": "/docs"}
...
@app.get("/documents/{document_id}/deadlines", response_model=list[ExtractedDeadline], tags=["Documents"])
async def get_document_deadlines(document_id: str):
    ...
    return [
        ExtractedDeadline(
            ...
            reminder_schedule=[date(2025, 11, 8), date(2025, 11, 24), date(2025, 12, 1), date(2025, 12, 7)],
        )
    ]
...
@app.get("/deadlines/{deadline_id}", response_model=ExtractedDeadline, tags=["Deadlines"])
...
@app.post("/deadlines/{deadline_id}/calendar", response_model=CalendarEntry, tags=["Calendar"])
async def sync_to_calendar(
    deadline_id: str, calendar_provider: str = Query(..., description="Calendar provider"), calendar_id: str = Query(..., description="Calendar ID")
):
    """
    ...
    return CalendarEntry(
        deadline_id=deadline_id, calendar_provider=calendar_provider, calendar_id=calendar_id, synced=True, last_synced=datetime.now(UTC)
    )
...
@app.post("/rules", response_model=DeadlineRule, status_code=status.HTTP_201_CREATED, tags=["Rules"])
...
async def get_statistics(jurisdiction: str | None = Query(None), date_from: date | None = Query(None), date_to: date | None = Query(None)):
    """
...
@app.get("/dashboard/upcoming", response_model=list[ExtractedDeadline], tags=["Dashboard"])
async def get_upcoming_deadlines(
    days: int = Query(30, ge=1, le=365, description="Days ahead to look"), assigned_to: str | None = Query(None, description="Filter by assignee")
):
    """
...
@app.get("/dashboard/critical", response_model=list[ExtractedDeadline], tags=["Dashboard"])
async def get_critical_deadlines(assigned_to: str | None = Query(None, description="Filter by assignee")):
    """
...
async def startup_event():
    ...
    print("Zero-Touch Legal Deadline Management API started")
...
async def shutdown_event():
    ...
    print("Zero-Touch Legal Deadline Management API shutting down")
...
if __name__ == "__main__":
    ...
    uvicorn.run("legal_deadlines:app", host="0.0.0.0", port=8001, reload=True, log_level="info")
