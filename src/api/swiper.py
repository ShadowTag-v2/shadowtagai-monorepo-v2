...
async def list_videos(
  ...
  if retailer_id:
    filtered_videos = [
      v for v in filtered_videos if v.get("retailer_id") == retailer_id
    ]
...
async def get_adaptive_playback(request: AdaptiveVideoRequest):
  ...
  video_products = [
    p for p in product_overlays_db.values() if p["video_id"] == request.video_id
  ]
  ...
  persuasion_points = select_persuasion_points(
    video_id=request.video_id, household_type=household_type, user_context=user_context
  )
      ...
      if retailer and retailer.get("store_locations"):
        ...
        for location in retailer["store_locations"]:
          distance = calculate_geofence_distance(
            request.location_lat, request.location_lon, location["lat"], location["lon"]
          )
...
async def get_video_analytics(video_id: str):
  ...
  unique_viewers = len(
    set(i["user_id"] for i in video_interactions if i.get("user_id"))
  )
  ...
  runtime_modes = [
    i.get("runtime_mode") for i in video_interactions if i.get("runtime_mode")
  ]
  ...
  for interaction in video_interactions:
    if interaction[
      "interaction_type"
    ] == InteractionType.CLICK_PRODUCT and interaction.get("product_id"):
      product_id = interaction["product_id"]
      product_clicks[product_id] = product_clicks.get(product_id, 0) + 1

  top_products = []
  for product_id, clicks in sorted(
    product_clicks.items(), key=lambda x: x[1], reverse=True
  )[:5]:
  ...
  beacon_interactions = [
    i for i in video_interactions if i.get("is_premium_beacon_session")
  ]
...
async def get_platform_analytics():
  ...
  beacon_interactions = [
    i for i in interactions_db if i.get("is_premium_beacon_session")
  ]
...
async def get_retailer_performance(retailer_id: str):
  ...
  retailer_videos = [
    v for v in videos_db.values() if v.get("retailer_id") == retailer_id
  ]
