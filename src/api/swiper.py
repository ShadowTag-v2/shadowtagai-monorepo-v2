...
"""
Swiper Adaptive Shoppable Video Platform - FastAPI Service
...
Strategic Impact:
- Makes Swiper the first commerce-linked streaming platform
...
sys.path.append("/home/user/aiyou-fastapi-services/src")
from models.swiper import (
    VideoCreate,
    VideoResponse,
    ProductCreate,
...
app = FastAPI(
    title="Swiper Adaptive Shoppable Video Platform",
    description="""
    ...
    Swiper vs. TikTok/Reels:
    - TikTok = discovery + impulse (awareness)
    - Swiper = consideration + conversion (closing engine)

    Together: Use TikTok to get interest → Swiper to seal the deal

    ## ✅ Investor Hook

    "Billboards nag. YouTube interrupts. Swiper entertains. Our Premium Beacons let brands
...
def select_narrative_arc(
    video_id: str, user_id: str | None, personalization_stage: PersonalizationStage, user_interests: list[ProductCategory] | None = None
) -> dict[str, Any]:
    """
    ...
    if personalization_stage == PersonalizationStage.RULES:
        if user_interests and ProductCategory.TOYS in user_interests:
            return {"arc_id": "family_fun", "scenes": ["intro", "product_demo", "family_interaction", "cta"], "emphasis": "safety_and_fun"}
        elif user_interests and ProductCategory.ELECTRONICS in user_interests:
            return {
                "arc_id": "tech_specs",
                "scenes": ["intro", "feature_showcase", "comparison", "value_prop", "cta"],
                "emphasis": "performance_and_value",
            }
        else:
            return {"arc_id": "general", "scenes": ["intro", "product_showcase", "benefits", "cta"], "emphasis": "quality"}
    ...
    # Stage 2: Multi-armed bandit (would query conversion history)
        ...
        return {
            ...
            "scenes": ["hook", "emotional_story", "product_integration", "social_proof", "cta"],
...
def select_persuasion_points(video_id: str, household_type: str | None = None, user_context: dict | None = None) -> list[dict[str, Any]]:
    """
    ...
    if household_type == "family_with_kids":
        ...
        persuasion_points = [p for p in video_points if p["target_audience"] == PersuasionTarget.PARENT]
    elif household_type == "couple":
        # Spouse → Spouse persuasion
        persuasion_points = [p for p in video_points if p["target_audience"] == PersuasionTarget.SPOUSE_PARTNER]
    elif user_context and user_context.get("is_corporate"):
        # Employee → Manager persuasion
        persuasion_points = [p for p in video_points if p["target_audience"] == PersuasionTarget.MANAGER]
    else:
        # Default to self-persuasion
        persuasion_points = [p for p in video_points if p["target_audience"] == PersuasionTarget.SELF]
...
def calculate_geofence_distance(user_lat: float, user_lon: float, store_lat: float, store_lon: float) -> float:
    """
...
@app.get("/")
async def root():
    """API root - Swiper platform overview"""
    return {
        "service": "Swiper Adaptive Shoppable Video Platform",
        "tagline": "Billboards nag. YouTube interrupts. Swiper entertains.",
        "version": "1.0.0",
        "features": [
...
async def list_videos(
    ...
    if is_premium_beacon is not None:
        filtered_videos = [v for v in filtered_videos if v["is_premium_beacon"] == is_premium_beacon]
    if retailer_id:
...
async def get_adaptive_playback(request: AdaptiveVideoRequest):
    ...
    This endpoint powers Swiper's three major innovations:
    ...
    behavioral_signals = {"scroll_speed": request.scroll_speed, "hover_time_seconds": request.hover_time_seconds}
    ...
    narrative_arc = select_narrative_arc(
        video_id=request.video_id, user_id=request.user_id, personalization_stage=video["personalization_stage"], user_interests=user_interests
    )
            ...
            products_list.append(
                {
                    ...
                    "position": {"x": overlay["position_x"], "y": overlay["position_y"], "width": overlay["width"], "height": overlay["height"]},
                    "cta_text": overlay["cta_text"],
                }
            )

    # Select persuasion points (household targeting)
    persuasion_points = select_persuasion_points(video_id=request.video_id, household_type=household_type, user_context=user_context)
        ...
        if request.location_lat and request.location_lon and video.get("retailer_id"):
            ...
            if retailer and retailer.get("store_locations"):
                ...
                for location in retailer["store_locations"]:
                    distance = calculate_geofence_distance(request.location_lat, request.location_lon, location["lat"], location["lon"])
                    if distance <= video.get("location_radius_meters", 5000):
                        unlock_condition = (
                            f"You're {int(distance)}m from the store. Watch Part 1 on your way. Buy the product to unlock Part 2 for the ride home!"
                        )
                        break
    ...
    interactions_db.append(
        {
            ...
            "metadata": {"eta_minutes": request.eta_minutes, "came_from_url": request.came_from_url, "behavioral_signals": behavioral_signals},
        }
    )
...
async def create_product_overlay(overlay: ProductOverlayCreate):
    ...
    overlay_data = {"id": overlay_id, **overlay.dict(), "clicks": 0, "conversions": 0, "click_rate": 0.0, "created_at": datetime.utcnow()}
...
async def create_persuasion_point(point: PersuasionPointCreate):
    ...
    Most ads only persuade the individual. Swiper persuades the household.
    ...
    - Unique moat: Only Swiper targets the full decision chain
    """
    ...
    point_data = {"id": point_id, **point.dict(), "impressions": 0, "conversion_lift": 0.0, "created_at": datetime.utcnow()}
...
async def list_persuasion_points(video_id: str | None = None, target_audience: PersuasionTarget | None = None):
...
async def log_interaction(interaction: InteractionCreate):
    ...
    interaction_data = {"id": interaction_id, **interaction.dict(), "timestamp": datetime.utcnow()}
...
async def get_video_analytics(video_id: str):
    ...
    # Calculate metrics
    total_views = len([i for i in video_interactions if i["interaction_type"] == InteractionType.VIEW])
    unique_viewers = len(set(i["user_id"] for i in video_interactions if i.get("user_id")))

    total_purchases = len([i for i in video_interactions if i["interaction_type"] == InteractionType.PURCHASE])
    total_revenue = sum(i.get("purchase_amount", 0) for i in video_interactions if i["interaction_type"] == InteractionType.PURCHASE)
    ...
    for interaction in video_interactions:
        if interaction["interaction_type"] == InteractionType.CLICK_PRODUCT and interaction.get("product_id"):
    ...
    for product_id, clicks in sorted(product_clicks.items(), key=lambda x: x[1], reverse=True)[:5]:
        if product_id in products_db:
            ...
            top_products.append({"product_id": product_id, "name": product["name"], "clicks": clicks, "price": product["price"]})
...
async def get_platform_analytics():
    ...
    Shows overall Swiper performance across all videos and retailers
    """
    ...
    total_revenue = sum(i.get("purchase_amount", 0) for i in interactions_db if i["interaction_type"] == InteractionType.PURCHASE)
    ...
    return {
        ...
        "personalization": {
            "rules_stage_videos": len([v for v in videos_db.values() if v["personalization_stage"] == PersonalizationStage.RULES]),
            "bandits_stage_videos": len([v for v in videos_db.values() if v["personalization_stage"] == PersonalizationStage.BANDITS]),
            "generative_stage_videos": len([v for v in videos_db.values() if v["personalization_stage"] == PersonalizationStage.GENERATIVE]),
        },
        "top_categories": [
            {"category": cat, "products": len([p for p in products_db.values() if p["category"] == cat])}
...
async def create_retailer(
    name: str, website_url: str | None = None, store_locations: list[dict] | None = None, revenue_share_percentage: float = 10.0
):
    """
...
async def get_retailer_performance(retailer_id: str):
    ...
    retailer_revenue = sum(
        ...
        if i["interaction_type"] == InteractionType.PURCHASE and i["video_id"] in [v["id"] for v in retailer_videos]
    )
    ...
    return {
        ...
        "conversion_rate_pct": round((total_conversions / total_views * 100) if total_views > 0 else 0.0, 2),
        "revenue_share_pct": retailer["revenue_share_percentage"],
        "retailer_payout_usd": round(retailer_revenue * (retailer["revenue_share_percentage"] / 100), 2),
    }
...
async def startup_event():
    ...
    """
    print("🎥 Swiper Adaptive Shoppable Video Platform starting...")
    print("✅ Billboards nag. YouTube interrupts. Swiper entertains.")
    ...
    retailers_db[walmart_id] = {
        ...
        "store_locations": [
            {"lat": 37.7749, "lon": -122.4194, "address": "San Francisco, CA", "radius": 5000},
            {"lat": 34.0522, "lon": -118.2437, "address": "Los Angeles, CA", "radius": 5000},
        ],
    ...
    videos_db[superman_video_id] = {
        ...
        "cdn_url": "https://cdn.swiper.com/videos/superman-adventure-part1/manifest.m3u8",
        "thumbnail_url": "https://cdn.swiper.com/thumbnails/superman-adventure.jpg",
        "hls_manifest_url": "https://cdn.swiper.com/videos/superman-adventure-part1/manifest.m3u8",
    ...
    products_db[superman_product_id] = {
        ...
        "image_url": "https://cdn.swiper.com/products/superman-figure.jpg",
        "in_stock": True,
        "stock_quantity": 500,
        "total_clicks": 0,
        "total_purchases": 0,
