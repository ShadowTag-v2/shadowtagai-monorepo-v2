  ...
  **Example Response**:
  ...
  try:
    ...
    raise HTTPException(
      ...
      detail=f"Judge evaluation failed: {str(e)}",
    )


@app.get("/judges/audit/{request_id}", response_model=AuditTrail, tags=["Audit"])
async def get_audit_trail(
  request_id: str = Path(..., description="Request ID to retrieve audit trail for"),
):
  """
...
@app.get(
  "/judges/metrics/{judge_type}", response_model=dict[str, Any], tags=["Metrics"]
)
async def get_judge_metrics(
  judge_type: JudgeType = Path(..., description="Judge vertical type"),
):
  """
...
@app.get(
  "/judges/decisions/recent", response_model=list[JudgeResponse], tags=["Decisions"]
)
  ...
  **System-Wide Metrics**:
  ...
  return {
    ...
    "decisions": {
      "allow": total_allow,
      "block": total_block,
      "allow_percentage": (total_allow / len(_recent_decisions)) * 100,
    },
    "latency": {
      "avg_ms": avg_latency,
      "p99_ms": p99_latency,
      "target_p99_ms": 90,
      "within_target": p99_latency <= 90,
    },
    "by_vertical": {
      jt.value: sum(1 for d in _recent_decisions if d.judge_type == jt)
      for jt in JudgeType
    },
  }
