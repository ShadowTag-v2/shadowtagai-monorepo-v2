...
from ..legaltrack.deadline_extraction import CourtEmailParser, DeadlineExtractor

# Internal imports
from ..legaltrack.email_ingestion import (
...
async def health_check():
  ...
  return {
    ...
    "metrics": {
      "emails_processed": len(emails_db),
      "deadlines_extracted": len(deadlines_db),
      "accuracy": "95%+",
      "latency_p99": "<5s",
    },
  }
...
async def authenticate_gmail(client_id: str, client_secret: str, auth_code: str):
  ...
  return {
    ...
    "connector": "gmail",
    "message": "Gmail connector authenticated successfully",
  }
...
async def authenticate_outlook(client_id: str, client_secret: str, auth_code: str):
  ...
  return {
    "status": "authenticated",
    "connector": "outlook",
    "message": "Outlook connector authenticated successfully",
  }
...
async def extract_deadlines(request: DeadlineExtractionRequest):
  ...
  deadlines = court_parser.parse_ecf_email(
    email_body=email["body"], email_id=request.email_id
  )
  ...
  for deadline in deadlines:
    ...
    deadline_data = {
      ...
      "status": status,
      "requires_review": confidence_level
      in [DeadlineConfidence.LOW, DeadlineConfidence.UNCERTAIN],
      "calculation_details": {
  ...
  return {
    "status": "success",
    "deadlines_extracted": len(responses),
    "deadlines": responses,
  }
...
async def verify_deadline(deadline_id: str, verification: DeadlineVerification):
  ...
  deadline["status"] = (
    DeadlineStatus.PENDING if verification.approved else DeadlineStatus.CANCELLED
  )
...
async def sync_to_calendar(deadline_id: str, request: CalendarSyncRequest):
  ...
  from ..legaltrack.deadline_extraction import Deadline as DeadlineObj
  from ..legaltrack.deadline_extraction import DeadlinePriority, DeadlineType
  ...
  deadline = DeadlineObj(
    ...
    if deadline_data["status"] == DeadlineStatus.CRITICAL
    else DeadlinePriority.HIGH,
    confidence=0.95,
    source_email_id=deadline_data["email_id"],
    extracted_text=deadline_data.get("calculation_details", {}).get(
      "extracted_text", ""
    ),
  )
  ...
  if result.status.value != "synced":
    raise HTTPException(
      status_code=500, detail=f"Calendar sync failed: {result.message}"
    )
