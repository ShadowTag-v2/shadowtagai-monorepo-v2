...
class FraudJudge(BaseJudge):
    ...
    def evaluate_action(self, request: JudgeRequest) -> dict[str, Any]:
        ...
        fraud_indicator_count = sum(
            [geo_mismatch, velocity_exceeded, not device_fingerprint_match, is_new_payee and amount_usd > 5000, time_since_account_creation_days < 30]
        )

        if fraud_indicator_count >= 3:
            decision = JudgeDecision.BLOCK
            reasoning_parts.append(f"Multiple fraud indicators ({fraud_indicator_count}) detected - BLOCK")
        ...
        # Rule 4: Medium fraud score with risk factors
            ...
            reasoning_parts.append(f"Medium fraud score ({fraud_score:.2f}) - requires additional verification")
        ...
        # Rule 5: New account with high-value transaction
            ...
            decision = JudgeDecision.BLOCK
            reasoning_parts.append("New account (<7 days) with high-value transaction - BLOCK")
            risk_flags.append("new_account_high_value")

        # Rule 6: Velocity check failure
        elif velocity_exceeded:
            decision = JudgeDecision.BLOCK
            reasoning_parts.append("Transaction velocity exceeded - possible account takeover")
        ...
        # Rule 7: Low fraud score - allow with monitoring
            ...
            reasoning_parts.append(f"Low fraud score ({fraud_score:.2f}), identity verified - approved with monitoring")
