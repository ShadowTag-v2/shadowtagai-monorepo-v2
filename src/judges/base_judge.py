...
from src.risk_matrix import Probability, RiskAssessment, Severity, assess_risk
...
class BaseJudge(ABC):
  ...
  def judge(self, request: JudgeRequest) -> JudgeResponse:
    ...
    probability, severity, rationale, mitigations = self.extract_risk_factors(
      request, evaluation
    )
    ...
    next_steps = self._generate_next_steps(
      final_decision, risk_assessment, approval_gate, request
    )
  ...
  def create_audit_trail(
    self, response: JudgeResponse, request: JudgeRequest
  ) -> AuditTrail:
    """
    ...
    trail_id = generate_trail_id(
      self.judge_type.value, request.request_id, response.timestamp
    )
