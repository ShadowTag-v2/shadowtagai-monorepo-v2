...
from src.risk_matrix import RiskAssessment, RiskLevel
...
class JudgeRequest(BaseModel):
  ...
  urgency: str = Field(
    default="normal", description="Request urgency: low, normal, high, critical"
  )
    ...
    json_schema_extra={
      "example": {
        ...
        "risk_assessment": {
          ...
          "mitigations": [
            "Verify vendor with external database",
        ...
        "next_steps": [
          ...
          "Request supporting documentation from requester",
        ],
...
class AuditTrail(BaseModel):
  ...
  approval_chain: list[dict[str, Any]] = Field(
    default_factory=list, description="Approval history"
  )
  timestamp: datetime = Field(
    default_factory=lambda: datetime.now(UTC), description="Trail creation time"
  )
  retention_days: int = Field(
    default=2555, description="Retention period (7 years default)"
  )
...
class PerformanceMetrics(BaseModel):
  ...
  false_positive_rate: float | None = Field(
    None, description="False positive rate (if known)"
  )
