...
class MADEngine:
    ...
    def reach_consensus(self, context: dict[str, Any], timeout_per_agent: float = 2.0) -> MADConsensus:
        """
        ...
        if self._is_unanimous(initial_votes):
            ...
            return self._build_consensus(votes=initial_votes, debate_rounds=[], unanimous=True, start_time=start_time)
        ...
        for round_num in range(1, self.max_debate_rounds + 1):
            ...
            debate_round = self._run_debate_round(
                round_number=round_num, current_votes=current_votes, context=context, timeout_per_agent=timeout_per_agent
            )
            ...
            if self._is_unanimous(current_votes):
                ...
                return self._build_consensus(votes=current_votes, debate_rounds=debate_rounds, unanimous=True, start_time=start_time)
        ...
        return self._build_consensus(votes=current_votes, debate_rounds=debate_rounds, unanimous=False, start_time=start_time)

    def _query_agents_parallel(self, context: dict[str, Any], timeout: float) -> list[AgentVote]:
        """
        ...
        with ThreadPoolExecutor(max_workers=len(self.agents)) as executor:
            ...
            for agent_id, agent in self.agents.items():
                future = executor.submit(self._query_single_agent, agent_id, agent, context, timeout)
    ...
    def _query_single_agent(self, agent_id: str, agent: Any, context: dict[str, Any], timeout: float) -> AgentVote:
        """
    ...
    def _run_debate_round(self, round_number: int, current_votes: list[AgentVote], context: dict[str, Any], timeout_per_agent: float) -> DebateRound:
        """
        ...
        debate_context = {
            **context,
            ...
            "current_votes": [
                {"agent": vote.agent_id, "decision": vote.decision.value, "confidence": vote.confidence, "reasoning": vote.reasoning}
    ...
    def _get_agent_argument(
        self, agent_id: str, current_vote: AgentVote, other_votes: list[AgentVote], context: dict[str, Any], timeout: float
    ) -> str:
        """
        ...
        other_decisions = ", ".join([f"{v.agent_id}={v.decision.value}" for v in other_votes])
        ...
        majority_decision = max(set(v.decision for v in other_votes), key=lambda d: sum(1 for v in other_votes if v.decision == d))
    ...
    def _build_consensus(self, votes: list[AgentVote], debate_rounds: list[DebateRound], unanimous: bool, start_time: float) -> MADConsensus:
        """
...
if __name__ == "__main__":
    print("=== MAD (Multi-Agent Debates) Demo ===\n")
    ...
    agents = {"gemini": MockAgent("Gemini"), "claude": MockAgent("Claude"), "gpt5": MockAgent("GPT-5")}
    ...
    # Create MAD engine
    mad = MADEngine(agents=agents, glicko_ratings=glicko_ratings, max_debate_rounds=2, require_unanimity=False)
    ...
    print(f"Latency: {consensus.total_latency_ms:.0f}ms")
    print(f"Debate rounds: {len(consensus.debate_rounds)}")

    print("\nWeighted votes:")
    for decision, weight in sorted(consensus.weighted_votes.items(), key=lambda x: x[1], reverse=True):
        print(f"  {decision.value}: {weight:.2f}")

    print("\nIndividual votes:")
    for vote in consensus.individual_votes:
        print(f"  {vote.agent_id}: {vote.decision.value} (confidence: {vote.confidence:.2f}, Glicko: {vote.glicko_rating:.0f})")
