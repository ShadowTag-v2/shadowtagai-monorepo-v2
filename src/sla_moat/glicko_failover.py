...
class GlickoEnhancedFailover(JREngineWithFailover):
  ...
  def __init__(
    ...
    logger.info(
      f"GlickoEnhancedFailover initialized with dynamic provider ranking. Initial rankings: {self._get_ranking_summary()}"
    )
  ...
  def execute_decision(self, context: dict[str, Any]) -> JudgeDecision:
    ...
    logger.warning(
      f"All preferred providers failed - using local fallback. Latency: {decision.latency_ms:.2f}ms"
    )

    return decision

  def _call_provider(
    self, provider_type: ProviderType, context: dict[str, Any], timeout: float
  ) -> JudgeDecision:
    """
  ...
  def _get_next_provider(
    ...
    ranked_providers: list[tuple[ProviderType, Glicko2Player]],
  ) -> ProviderType:
  ...
  def _update_glicko_rating(
    ...
    avg_rating = sum(
      player.get_rating() for player in self.glicko_ratings.values()
    ) / len(self.glicko_ratings)
  ...
  def _check_rebalancing(self):
    ...
    if self.decision_count % self.rebalance_interval == 0:
      ...
      if max_change > 0.05:  # 5% change in allocation
        logger.info(
          f"Rebalancing providers after {self.decision_count} decisions. Rankings: {self._get_ranking_summary()}"
        )
