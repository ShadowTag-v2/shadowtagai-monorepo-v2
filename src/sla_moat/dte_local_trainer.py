...
from .dte_evolution import DTEEvolutionEngine, BenchmarkType, BenchmarkResult
...
class DTELocalModelTrainer:
    ...
    def evolve_continuously(self, max_iterations: int = 10, save_checkpoints: bool = True) -> Any:
        """
        ...
        if save_checkpoints:
            for iteration_record in history:
                checkpoint = ModelCheckpoint(
                    ...
                    model_path=str(self.checkpoint_dir / f"checkpoint_iter_{iteration_record.iteration}.pt"),
        ...
        logger.info(f"DTE evolution complete: iterations={len(history)}, final_accuracy={history[-1].evolved_accuracy:.2%}")

        return evolved_model

    def compare_with_commercial_apis(self, test_cases: list[dict[str, Any]], commercial_decisions: dict[str, list[str]]) -> dict[str, float]:
        """
        ...
        """
        logger.info(f"Comparing local model with commercial APIs on {len(test_cases)} test cases")
        ...
        for provider, provider_decisions in commercial_decisions.items():
            if len(provider_decisions) != len(local_decisions):
                logger.warning(f"Decision count mismatch for {provider}: {len(provider_decisions)} vs {len(local_decisions)}")
                continue

            # Count agreements
            agreement_count = sum(1 for local, commercial in zip(local_decisions, provider_decisions) if local == commercial)

            agreement_pct = agreement_count / len(local_decisions)
            agreements[provider] = agreement_pct

            logger.info(f"Agreement with {provider}: {agreement_pct:.1%} ({agreement_count}/{len(local_decisions)})")
    ...
    def _save_model(self, model: Any, path: Path):
        ...
        metadata = {
            ...
            "final_glicko_rating": self.checkpoints[-1].glicko_rating if self.checkpoints else 1200,
            "timestamp": time.time(),
        }
    ...
    def get_training_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "checkpoints": [
                {"iteration": cp.iteration, "accuracy": cp.accuracy, "glicko_rating": cp.glicko_rating, "timestamp": cp.timestamp}
...
if __name__ == "__main__":
    print("=== DTE Local Model Trainer Demo ===\n")

    # Initialize trainer
    trainer = DTELocalModelTrainer(model_path="models/judge6_local_v2.pt", target_accuracy=0.80, target_commercial_agreement=0.80)

    print("Training local model with DTE evolution...\n")
    ...
    for cp in summary["checkpoints"]:
        print(f"  Iteration {cp['iteration']}: {cp['accuracy']:.2%} accuracy, {cp['glicko_rating']:.0f} rating")
    ...
    print("\n" + "=" * 70 + "\n")
    print("Comparing with commercial APIs...\n")

    # Generate test cases
    test_cases = [{"user_request": f"Test case {i}", "policies": ["policy1"]} for i in range(100)]
    ...
    print("Agreement with commercial APIs:")
    for provider, agreement in agreements.items():
        print(f"  {provider}: {agreement:.1%}")

    if agreements.get("gemini", 0) >= 0.80:
        print("\n✅ Target reached: ≥80% agreement with Gemini!")
        print("Local model ready for production use.")
    else:
        print(f"\n⚠️  Target not yet reached: {agreements.get('gemini', 0):.1%} < 80% agreement")
