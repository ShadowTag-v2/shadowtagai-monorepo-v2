...
from .cheat_sheet_fusion import CheatSheetFusion, CheatSheetLibrary
from .cheat_sheet_fusion import ProviderType as CheatProviderType
...
class IntegratedJudge:
  ...
  def __init__(
    ...
    if enable_dte:
      self.dte_trainer = DTELocalModelTrainer(
        model_path=model_path, target_accuracy=0.80, target_commercial_agreement=0.80
      )
    ...
    logger.info(
      f"IntegratedJudge initialized: Glicko={enable_glicko}, MAD={enable_mad}, CheatSheet={enable_cheat_sheet}, DTE={enable_dte}"
    )
  ...
  def decide(self, context: dict[str, Any]) -> IntegratedDecisionMetrics:
    ...
    if self.cheat_sheet_fusion and "user_request" in context:
      ...
      optimized_prompt = self.cheat_sheet_fusion.apply(
        base_prompt=original_request, provider=cheat_provider
      )
  ...
  def evolve_local_model(
    ...
    # Run evolution
    self.dte_trainer.evolve_continuously(
      max_iterations=max_iterations, save_checkpoints=save_checkpoints
    )
    ...
    logger.info(
      f"DTE evolution complete: accuracy improved {summary['initial_accuracy']:.2%} → {summary['final_accuracy']:.2%}"
    )
...
if __name__ == "__main__":
  ...
  metrics = judge.decide(
    {
      "user_request": "Update user email to new_email@example.com",
      "risk_level": "low",
      "user_id": "user_123",
    }
  )
  ...
  for i in range(1, 11):
    context = {
      "user_request": f"Process transaction {i}",
      "risk_level": "medium",
      "amount": i * 100,
    }
