...
class CheatSheetLibrary:
    ...
    def evolve_profile(self, provider: ProviderType, performance_data: dict[str, Any]) -> CheatSheet:
        """
...
class CheatSheetFusion:
    ...
    def apply(self, base_prompt: str, provider: ProviderType, use_case: str | None = None, custom_overrides: dict[str, Any] | None = None) -> str:
        """
    ...
    def _format_narrative(self, content: str, sheet: CheatSheet) -> str:
        ...
        # Opening with role
        parts.append(f"You are acting as a {sheet.act.replace('_', ' ')}. Your goal is to {sheet.objective.replace('_', ' ')}.")
        parts.append("")
        ...
        if sheet.examples > 0:
            parts.append(f"For reference, here are {sheet.examples} similar cases to guide your thinking:")
            parts.append("[Examples would be inserted here]")
            parts.append("")

        # Audience-aware tone
        if sheet.audience == "human_decision_maker":
            parts.append("Please provide a thoughtful recommendation that I can understand and use to make an informed decision.")
        else:
...
"""
    ...
    for provider in [ProviderType.GEMINI, ProviderType.CLAUDE, ProviderType.GPT5]:
        print(f"OPTIMIZED FOR {provider.value.upper()}:")
        print("-" * 70)
    ...
    print(f"Original Gemini profile: {library.get_profile(ProviderType.GEMINI).to_dict()}")
    print()

    evolved = library.evolve_profile(ProviderType.GEMINI, performance_data)
    print(f"Evolved Gemini profile: {evolved.to_dict()}")
    print()
    print("Changes:")
    print("  - Context: concise → moderate (accuracy <70%)")
    print("  - Examples: 2 → 3 (accuracy <70%)")
    print("  - Tone: professional → professional (already optimal)")
