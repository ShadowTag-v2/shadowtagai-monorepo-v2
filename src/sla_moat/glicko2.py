...
class Glicko2Player:
  ...
  def update(
    self,
    matches: list[tuple[float, "Glicko2Player"]],
    tau: float = _TAU,
    tol: float = _EPSILON,
  ):
    """
    ...
    delta_mu = self._phi**2 * sum(
      self._g(opponent._phi) * (outcome - self._E(opponent))
      for outcome, opponent in matches
    )
    self._mu += delta_mu

  def _compute_v_and_delta(
    self, matches: list[tuple[float, "Glicko2Player"]]
  ) -> tuple[float, float]:
    """
    ...
    # Variance
    v_inv = sum(
      self._g(opponent._phi) ** 2 * self._E(opponent) * (1 - self._E(opponent))
      for _, opponent in matches
    )
...
def create_provider_ratings() -> dict:
  ...
  return {
    ...
    ProviderType.LOCAL: Glicko2Player(
      mu=1200, phi=100, sigma=0.06
    ),  # Lower initial rating
  }


def update_provider_rating(
  provider: Glicko2Player,
  outcome: float,
