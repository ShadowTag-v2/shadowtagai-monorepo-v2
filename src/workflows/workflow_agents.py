...
from google.adk.events import Event, AgentEvent
import asyncio
import logging
...
class SequentialWorkflowAgent(BaseAgent):
    ...
    def __init__(self, name: str, sub_agents: list[BaseAgent], description: str | None = None, **kwargs):
        """
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        """
        logger.info(f"SequentialWorkflowAgent {self.name} starting with {len(self.sub_agents)} agents")

        yield AgentEvent(agent_name=self.name, data={"status": "started", "workflow_type": "sequential", "total_agents": len(self.sub_agents)})
        ...
        for idx, sub_agent in enumerate(self.sub_agents, 1):
            ...
            yield AgentEvent(agent_name=self.name, data={"status": "executing_step", "step": idx, "agent": sub_agent.name})
            ...
            self.results.append({"step": idx, "agent": sub_agent.name, "results": step_results})

            yield AgentEvent(agent_name=self.name, data={"status": "step_completed", "step": idx, "agent": sub_agent.name})

        yield AgentEvent(
            agent_name=self.name,
            data={"status": "completed", "workflow_type": "sequential", "total_steps": len(self.sub_agents), "results": self.results},
        )
...
class ParallelWorkflowAgent(BaseAgent):
    ...
    def __init__(self, name: str, sub_agents: list[BaseAgent], description: str | None = None, max_concurrent: int | None = None, **kwargs):
        """
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        """
        logger.info(f"ParallelWorkflowAgent {self.name} starting with {len(self.sub_agents)} agents")

        yield AgentEvent(
            agent_name=self.name,
            data={"status": "started", "workflow_type": "parallel", "total_agents": len(self.sub_agents), "max_concurrent": self.max_concurrent},
        )
        ...
        yield AgentEvent(
            agent_name=self.name,
            data={"status": "completed", "workflow_type": "parallel", "total_agents": len(self.sub_agents), "results_count": len(self.results)},
        )
...
class LoopWorkflowAgent(BaseAgent):
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        """
        logger.info(f"LoopWorkflowAgent {self.name} starting with max {self.max_iterations} iterations")

        yield AgentEvent(agent_name=self.name, data={"status": "started", "workflow_type": "loop", "max_iterations": self.max_iterations})
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            if self.condition and self.condition(context):
                ...
                yield AgentEvent(agent_name=self.name, data={"status": "condition_met", "iteration": iteration})
                break

            yield AgentEvent(agent_name=self.name, data={"status": "iteration_started", "iteration": iteration})
            ...
            self.iteration_results.append({"iteration": iteration, "events": iteration_events})

            yield AgentEvent(agent_name=self.name, data={"status": "iteration_completed", "iteration": iteration})

        yield AgentEvent(
            agent_name=self.name,
            data={"status": "completed", "workflow_type": "loop", "total_iterations": len(self.iteration_results), "results": self.iteration_results},
        )
...
class ConditionalWorkflowAgent(BaseAgent):
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        yield AgentEvent(agent_name=self.name, data={"status": "started", "workflow_type": "conditional", "conditions_count": len(self.conditions)})
        ...
        for idx, (condition, agents) in enumerate(self.conditions):
            try:
                if condition(context):
                    ...
                    yield AgentEvent(agent_name=self.name, data={"status": "condition_matched", "condition_index": idx, "agents_count": len(agents)})
                ...
                yield AgentEvent(agent_name=self.name, data={"status": "condition_error", "condition_index": idx, "error": str(e)})

        # Execute default agents if no condition matched
        if not matched and self.default_agents:
            logger.info(f"No conditions matched, executing {len(self.default_agents)} default agents")

            yield AgentEvent(agent_name=self.name, data={"status": "executing_default", "agents_count": len(self.default_agents)})

            for agent in self.default_agents:
                async for event in agent._run_async_impl(context):
                    yield event

        yield AgentEvent(agent_name=self.name, data={"status": "completed", "workflow_type": "conditional", "matched": matched})
