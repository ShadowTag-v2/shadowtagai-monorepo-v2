...
class CoordinatorAgent(MultiAgent):
    ...
    def __init__(self, name: str, model: str = "gemini-2.5-flash", sub_agents: list[BaseAgent] | None = None, **kwargs):
        super().__init__(
            name=name,
            model=model,
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event]:
        ...
        yield AgentEvent(
            ...
            data={"status": "coordinating", "task_type": task_type, "sub_agents_count": len(self.sub_agents) if self.sub_agents else 0},
        )
        ...
        yield AgentEvent(agent_name=self.name, data={"status": "completed", "message": f"Coordination completed for task: {task_type}"})
...
class ResearchAgent(MultiAgent):
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event]:
        ...
        yield AgentEvent(agent_name=self.name, data={"status": "researching", "query": query})
        ...
        research_findings = {
            ...
            "type": "research",
            "query": query,
            "findings": ["Finding 1: Relevant information discovered", "Finding 2: Additional context found", "Finding 3: Related topics identified"],
            "sources": ["Source A", "Source B", "Source C"],
        }

        yield AgentEvent(agent_name=self.name, data={"status": "completed", "results": research_findings})
...
class AnalysisAgent(MultiAgent):
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event]:
        ...
        yield AgentEvent(agent_name=self.name, data={"status": "analyzing", "data_size": len(str(data))})
        ...
        analysis_results = {
            "agent": self.name,
            "type": "analysis",
            "insights": ["Insight 1: Pattern detected in data", "Insight 2: Correlation identified", "Insight 3: Trend analysis complete"],
            "metrics": {"accuracy": 0.95, "confidence": 0.88},
        }

        yield AgentEvent(agent_name=self.name, data={"status": "completed", "results": analysis_results})
...
class TaskAgent(MultiAgent):
    ...
    def __init__(self, name: str, model: str = "gemini-2.5-flash", task_description: str | None = None, **kwargs):
        super().__init__(
            name=name,
            model=model,
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event]:
        ...
        yield AgentEvent(agent_name=self.name, data={"status": "executing", "task": task, "delegated_by": delegated_by})
        ...
        yield AgentEvent(agent_name=self.name, data={"status": "completed", "results": task_result})
...
class ValidationAgent(MultiAgent):
    ...
    def __init__(self, name: str, model: str = "gemini-2.5-flash", validation_criteria: dict[str, Any] | None = None, **kwargs):
        super().__init__(
            name=name,
            model=model,
    ...
    async def _run_async_impl(self, context: InvocationContext) -> AsyncGenerator[Event]:
        ...
        yield AgentEvent(agent_name=self.name, data={"status": "completed", "results": validation_result})
