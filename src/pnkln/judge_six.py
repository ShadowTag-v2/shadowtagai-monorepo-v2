...
class JudgeSix:
  ...
  def enforce(self, user_request: str) -> str:
    ...
    return self.caller.execute(
      prompt=user_request, validation_callback=validate_function_call
    )
  ...
  def _validate(
    ...
    if not purpose_valid:
      ...
      explanation = (
        f"Function '{fn_name}' does not advance mission: {self.mission_statement}"
      )
  ...
  def _validate_purpose(
    ...
    # Default: Check if function call aligns with mission
