...
class IngestionPipeline:
    ...
    async def collect_from_source(self, collector) -> tuple[str, list[IngestedItem], list[str]]:
        """
        ...
        try:
            ...
            items = await asyncio.wait_for(collector.collect_with_retry(), timeout=self.PER_SOURCE_TIMEOUT_SECONDS)
    ...
    async def run(self) -> dict:
        ...
        try:
            ...
            logger.info(f"   Target: {self.TARGET_RUNTIME_MINUTES} min | Status: {'✅ ON TARGET' if summary['under_target'] else '⚠️ OVER TARGET'}")
            logger.info("=" * 80)
...
async def main():
    ...
    if summary["status"] == "completed":
        ...
        print(f"Total Cost: ${summary['metrics']['total_cost']}")
        print(f"Cost/Item: ${summary['metrics']['cost_per_item']} (target: $0.0026)")
        print(f"Avg Relevance: {summary['metrics']['avg_relevance']}")
        print(f"\nQuality Gates: {summary['quality_gates']['overall_status'].upper()}")
        print(f"  Passed: {summary['quality_gates']['passed']}/{summary['quality_gates']['total_gates']}")
        print(f"  Warnings: {summary['quality_gates']['warnings']}, Critical: {summary['quality_gates']['critical']}")
