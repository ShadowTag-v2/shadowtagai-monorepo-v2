...
async def sequential_workflow_example():
    ...
    pipeline = SequentialWorkflowAgent(
        name="DataProcessingPipeline", sub_agents=[step1, step2, step3, step4], description="Sequential data processing pipeline"
    )

    # Execute
    context = InvocationContext({"user_input": "Process customer data and generate insights"})
...
async def parallel_workflow_example():
    ...
    social_fetcher = TaskAgent(name="SocialFetcher", task_description="Fetch social media")
...
async def loop_workflow_example():
    ...
    loop_processor = LoopWorkflowAgent(name="RefinementLoop", sub_agents=[refiner], max_iterations=5, description="Iterative data refinement")
    ...
    async for event in loop_processor._run_async_impl(context):
        ...
        logger.info(f"[{event.agent_name}] {status} {f'(Iteration {iteration})' if iteration else ''}")
...
async def conditional_workflow_example():
    ...
    high_priority = TaskAgent(name="HighPriorityHandler", task_description="Handle high priority")
    medium_priority = TaskAgent(name="MediumPriorityHandler", task_description="Handle medium priority")
    low_priority = TaskAgent(name="LowPriorityHandler", task_description="Handle low priority")
    default_handler = TaskAgent(name="DefaultHandler", task_description="Default handling")
    ...
    for priority in priorities:
        ...
        context = InvocationContext({"user_input": f"Process request with priority {priority}", "priority": priority})
...
async def combined_workflow_example():
    ...
    parallel_stage = ParallelWorkflowAgent(name="GatherStage", sub_agents=[fetcher1, fetcher2])
    ...
    combined_pipeline = SequentialWorkflowAgent(
        name="CombinedPipeline", sub_agents=[parallel_stage, processor, analyzer], description="Combined parallel and sequential workflow"
    )
