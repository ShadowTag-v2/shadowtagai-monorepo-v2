...
from src.integration.unified_orchestrator import create_unified_orchestrator, UnifiedPinklnOrchestrator
...
""")
    ...
    orchestrator = create_unified_orchestrator(
        api_key=os.environ.get("GOOGLE_API_KEY"), enable_jr_validation=True, enable_shadowtag=True, enable_memory=True, enable_glicko=True
    )
    ...
    start = time.time()
    result = orchestrator.execute("Analyze this military decision for ATP 5-19 compliance and provide go/no-go recommendation: " + decision_context)
...
""")
    ...
    result = orchestrator.execute("Have a panel of 3 experts debate: Should we prioritize latency or cost optimization?")
