...
import asyncio
from src.agents import CoordinatorAgent, ResearchAgent, AnalysisAgent, TaskAgent
...
async def basic_multi_agent_example():
    ...
    coordinator = CoordinatorAgent(name="MainCoordinator", sub_agents=[researcher, analyzer, task_executor])
    ...
    context = InvocationContext({"user_input": "Analyze market trends and provide insights", "task_type": "research_and_analysis"})
...
async def hierarchical_multi_agent_example():
    ...
    data_team = CoordinatorAgent(name="DataTeamCoordinator", sub_agents=[data_collector, data_validator])
    ...
    analysis_team = CoordinatorAgent(name="AnalysisTeamCoordinator", sub_agents=[insight_generator, report_builder])

    # Level 1: Top-level coordinator
    master_coordinator = CoordinatorAgent(name="MasterCoordinator", sub_agents=[data_team, analysis_team])
    ...
    # Execute the system
    context = InvocationContext({"user_input": "Complete data analysis pipeline", "task_type": "full_pipeline"})
...
async def dynamic_agent_management_example():
    ...
    context = InvocationContext({"user_input": "Execute with dynamically configured agents", "task_type": "dynamic"})
