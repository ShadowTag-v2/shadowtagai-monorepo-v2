...
from .models import DecisionPath, GovernanceRequest, RiskAssessment, RiskLevel
...
class ATP519RiskClassifier:
  ...
  def _has_approval_controls(cls, request: GovernanceRequest) -> bool:
    ...
    return (
      "approver_id" in context
      or "manager_approval" in context
      or "budget_approved" in context
    )
  ...
  def _recommend_controls(
    cls, request: GovernanceRequest, risk_level: RiskLevel
  ) -> list[str]:
    """
