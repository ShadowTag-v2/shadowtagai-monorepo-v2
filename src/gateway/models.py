...
class GovernanceRequest(BaseModel):
  ...
  context: dict[str, Any] = Field(
    default_factory=dict, description="Additional context for decision-making"
  )

  # Risk indicators
  financial_value: float | None = Field(
    None, ge=0, description="USD value if applicable"
  )
  data_sensitivity: str | None = Field(
    None, description="PII, PHI, confidential, public"
  )
...
class GovernanceResponse(BaseModel):
  ...
  policy_citations: list[str] = Field(
    default_factory=list, description="Specific policy sections applied"
  )
  ...
  hallucination_check: bool = Field(
    default=True, description="Passed hallucination detection"
  )
...
class AgentMetrics(BaseModel):
  ...
  overrides_count: int = Field(
    0, ge=0, description="Human overrides of agent decisions"
  )
