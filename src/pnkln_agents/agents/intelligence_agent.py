...
from ..core.agent_pattern import AgentStatus, AgentTask, PnklnAgent
...
class IntelligenceAgent(PnklnAgent):
  ...
  def _execute_task(
    ...
    quality_gates = self.ingestion_layer.validate_quality_gates(
      ingestion_result.metrics
    )
    ...
    verification_result = self.judge_six.verify(
