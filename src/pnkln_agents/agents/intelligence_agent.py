...
from ..core.agent_pattern import PnklnAgent, AgentTask, AgentStatus
from ..config.ingestion_config import IngestionConfig, DEFAULT_INGESTION_CONFIG
...
class IntelligenceAgent(PnklnAgent):
    ...
    def __init__(self, ingestion_layer: GeminiIngestionLayer | None = None, ingestion_config: IngestionConfig | None = None, **kwargs):
        super().__init__(**kwargs)

        # Initialize ingestion layer
        self.ingestion_config = ingestion_config or DEFAULT_INGESTION_CONFIG
        self.ingestion_layer = ingestion_layer or GeminiIngestionLayer(config=self.ingestion_config.to_dict())
    ...
    def collect_intelligence(self, task: IntelligenceTask) -> IntelligenceResult:
        ...
        if agent_result.status == AgentStatus.COMPLETED:
            ...
            return IntelligenceResult(status=agent_result.status, audit_trail=agent_result.audit_trail, execution_time_ms=execution_time_ms)
    ...
    def _execute_task(self, task: AgentTask, constraints: dict[str, Any]) -> IntelligenceResult:
        ...
        if not ingestion_result.success:
            return IntelligenceResult(status=AgentStatus.FAILED, ingestion_result=ingestion_result, metrics={"errors": ingestion_result.errors})
        ...
        briefing = self.ingestion_layer.export_am_briefing(ingestion_result.items, format="markdown")
