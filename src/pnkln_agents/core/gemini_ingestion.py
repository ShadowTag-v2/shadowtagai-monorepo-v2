...
class EthicalComplianceValidator:
  ...
  def __init__(self, config: dict[str, Any] | None = None):
    ...
    self.user_agent = self.config.get(
      "user_agent", "PNKLNBot/1.0 (+https://pnkln.ai/bot)"
    )
...
class GeminiIngestionLayer:
  ...
  def ingest(
    ...
    for source in sources:
      ...
      try:
        ...
        robots_violation = self.ethical_validator.validate_robots_txt(
          source.url, source
        )
        if robots_violation:
  ...
  def _collect_from_source(
    self, source: Source, target_count: int
  ) -> list[IngestedItem]:
    """
  ...
  def export_am_briefing(
    self, items: list[IngestedItem], format: str = "markdown"
  ) -> str:
    """
