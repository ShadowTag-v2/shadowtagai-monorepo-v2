...
class TestA2AMesh:
  ...
  def test_dispatch_task(self):
    ...
    task = mesh.dispatch_task(
      "orchestrator", AgentCapability.LINT_FIX, {"file": "main.py"}
    )
...
class TestDarwinianGate:
  ...
  def test_benchmark_speed_delta_emitted(self):
    ...
    gate.propose_mutation(
      MutationCandidate("m1", "a.py", "perf", 0.5, 0.6, benchmark_ms=100.0)
    )
