...
def orchestrator():
    ...
    return create_unified_orchestrator(
        api_key=os.environ["GOOGLE_API_KEY"], enable_jr_validation=True, enable_shadowtag=True, enable_memory=True, enable_glicko=True
    )
...
class TestLatencyBenchmarks:
    ...
    def test_kernel_chain_latency(self, orchestrator):
        ...
        start = time.time()
        result = orchestrator.execute("Analyze decision context for ATP 5-19 violations: $2M unauthorized purchase")
    ...
    def test_debate_latency(self, orchestrator):
        ...
        start = time.time()
        result = orchestrator.execute("Have 3 experts debate: Should we prioritize speed or accuracy?")
        latency_ms = (time.time() - start) * 1000
...
class TestAccuracyBenchmarks:
    ...
    def test_violation_detection_accuracy(self, orchestrator):
        ...
        test_cases = [
            {"context": "$2M purchase exceeding battalion authority ($1M limit)", "expected_violations": True},
            {"context": "$500K purchase within company authority", "expected_violations": False},
            {"context": "Missing required documentation for $1.5M contract", "expected_violations": True},
        ]
        ...
        for i, case in enumerate(test_cases, 1):
            ...
            if has_violations == case["expected_violations"]:
                ...
                print(f"  Test {i}: ❌ FAIL (expected {case['expected_violations']}, got {has_violations})")
...
class TestSelfEvolution:
    ...
    def test_continuous_improvement(self, orchestrator):
        ...
        prompts = ["Analyze data", "Analyze data effectively", "Analyze data effectively and provide insights"]
