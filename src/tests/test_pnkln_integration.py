...
def full_stack():
    ...
    # Create components
    tools = [FunctionTool(name="simple_function", description="Double a number", function=simple_function, parameters={"x": {"type": "integer"}})]

    caller = GeminiFunctionCaller(model_name="gemini-2.0-flash-exp", tools=tools)

    judge = JudgeSix(caller=caller, mission_statement="Perform mathematical operations", purpose_threshold=0.3)
