...
def judge_six():
    ...
    tools = [
        FunctionTool(name="safe_function", description="Process text safely", function=safe_function, parameters={"text": {"type": "string"}}),
        FunctionTool(
            name="dangerous_function", description="Delete a database", function=dangerous_function, parameters={"database": {"type": "string"}}
        ),
    ]
...
def test_brakes_block_sql_injection(judge_six):
    ...
    validation = judge_six._validate("safe_function", {"text": "'; DROP TABLE users; --"}, "Process some text")
