...
Aligned with PNKLN Core Stack 2025 technology refresh:
...
class IngestcionConfig:
    ...
    enabled_sources: list[str] = field(default_factory=lambda: ["youtube", "twitter", "news", "reddit", "rss_feeds"])
    ...
    gcs_bucket: str = os.getenv("GCS_BUCKET", "pnkln-ingestion-data")
    bigquery_dataset: str = os.getenv("BQ_DATASET", "pnkln_ingestion")

    # AM Briefing Configuration
    briefing_recipients: list[str] = field(default_factory=lambda: os.getenv("BRIEFING_RECIPIENTS", "").split(","))
    ...
    serving_namespaces: list[str] = field(default_factory=lambda: ["intelligence", "analytics", "reporting", "api-gateway"])
...
YOUTUBE_CONFIG = SourceConfig(
    ...
    custom_params={"video_categories": ["News", "Education", "Science & Technology"], "min_view_count": 1000, "max_video_age_days": 7},
)
...
NEWS_CONFIG = SourceConfig(
    ...
    custom_params={"trusted_domains": ["reuters.com", "apnews.com", "bbc.com", "bloomberg.com"], "categories": ["technology", "business", "science"]},
)
...
REDDIT_CONFIG = SourceConfig(
    ...
    custom_params={"subreddits": ["technology", "machinelearning", "programming"], "min_upvotes": 10, "min_comments": 5},
)
...
RSS_FEEDS_CONFIG = SourceConfig(
    ...
    max_items_per_run=1000,
    tier_default=TierLevel.TIER_2,
    custom_params={"feed_urls": ["https://news.ycombinator.com/rss", "https://techcrunch.com/feed/", "https://www.wired.com/feed/rss"]},
)

# Export default configuration
DEFAULT_CONFIG = IngestcionConfig()
