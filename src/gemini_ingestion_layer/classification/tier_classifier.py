...
class TierClassifier:
    ...
    def __init__(self, model: str = "gemini-2.5-flash-lite", confidence_threshold: float = 0.60, temperature: float = 0.3, max_tokens: int = 8192):
        """
...
Provide your classification:"""
    ...
    async def classify(self, content: dict[str, Any]) -> ClassificationResult:
        ...
        if domain in self.tier_1_domains:
            result = ClassificationResult(
                tier=TierLevel.TIER_1, confidence=0.95, reasoning=f"Known Tier 1 domain: {domain}", metadata={"method": "domain_whitelist"}
            )
        ...
        if domain in self.tier_2_domains:
            result = ClassificationResult(
                tier=TierLevel.TIER_2, confidence=0.85, reasoning=f"Known Tier 2 domain: {domain}", metadata={"method": "domain_whitelist"}
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "average_cost_per_item": round(self.stats["total_cost_usd"] / total, 6) if total > 0 else 0.0,
        }
    ...
    def check_tier_distribution(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tier_1": {"actual": tier_1_pct, "target": 30.0, "status": "pass" if tier_1_ok else "fail"},
            "tier_2": {"actual": tier_2_pct, "target": 50.0, "status": "pass" if tier_2_ok else "fail"},
            "tier_3": {"actual": tier_3_pct, "target": 20.0, "max": 25.0, "status": "pass" if tier_3_ok else "fail"},
