...
class GeminiIngestionLayer:
    ...
    def __init__(self, config: IngestcionConfig = None):
        ...
        self.classifier = TierClassifier(
            model=self.config.gemini_model, confidence_threshold=self.config.min_relevance_score, temperature=self.config.gemini_temperature
        )
    ...
    async def run(self) -> dict[str, Any]:
        ...
        for item in all_items:
            try:
                result = await self.classifier.classify(
                    {"source": item.source, "domain": item.domain, "title": item.title, "text": item.content, "author": item.author, "url": item.url}
                )
        ...
        ingestion_stats = {
            ...
            "average_relevance_score": (sum(i.relevance_score for i in classified_items) / len(classified_items) if classified_items else 0.0),
