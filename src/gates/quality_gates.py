...
class QualityGateChecker:
  ...
  def check_tier_distribution(self, items: list[IngestedItem]) -> GateResult:
    ...
    issues = []
    if not (
      self.config.tier1_min_pct
      <= tier_percentages[Tier.TIER_1]
      <= self.config.tier1_max_pct
    ):
      issues.append(
        f"T1: {tier_percentages[Tier.TIER_1]:.1f}% (target: {self.config.tier1_min_pct}-{self.config.tier1_max_pct}%)"
      )
    if not (
      self.config.tier2_min_pct
      <= tier_percentages[Tier.TIER_2]
      <= self.config.tier2_max_pct
    ):
      issues.append(
        f"T2: {tier_percentages[Tier.TIER_2]:.1f}% (target: {self.config.tier2_min_pct}-{self.config.tier2_max_pct}%)"
      )
    if tier_percentages[Tier.TIER_3] > self.config.tier3_max_pct:
      issues.append(
        f"T3: {tier_percentages[Tier.TIER_3]:.1f}% (max: {self.config.tier3_max_pct}%)"
      )
  ...
  def get_summary(self) -> dict:
    ...
    return {
      ...
      "critical": critical,
      "overall_status": (
        "critical" if critical > 0 else ("warning" if warnings > 0 else "ok")
      ),
      "results": [
        {
          "gate": r.gate,
