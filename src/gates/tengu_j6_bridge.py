...
from governance.j6_csrmc_cato import Cor_Claude_Code_6CSRMC
...
def evaluate_gate(gate_name: str) -> Any:
  ...
  if gate_name not in PYTHON_GATES:
    raise KeyError(
      f"Unregistered gate: {gate_name}. Available: {sorted(PYTHON_GATES.keys())}"
    )
...
def get_gate_diagnostics() -> dict[str, Any]:
  ...
  for name, defn in PYTHON_GATES.items():
    ...
    diagnostics[name] = {
      ...
      "effective_value": cached_value
      if cached_value is not None
      else defn.default_value,
