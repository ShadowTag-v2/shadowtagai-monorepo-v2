...
class ReasonsValidator:
    ...
    def __init__(self):
        self.evidence_keywords = {"data", "analysis", "research", "benchmark", "test", "measurement", "metric", "result", "finding", "report"}
    ...
    def validate(self, action: Action) -> ReasonsVerdict:
        ...
        score = self._calculate_score(evidence_quality, risk_reward_ratio, stakeholder_impact)
        ...
        explanation = self._generate_explanation(status, score, evidence_quality, risk_reward_ratio)
    ...
    def _assess_evidence_quality(self, action: Action) -> str:
        ...
        if "evidence" in action.context:
            ...
            evidence_count = sum(1 for keyword in self.evidence_keywords if keyword in evidence)
        ...
        evidence_count = sum(1 for keyword in self.evidence_keywords if keyword in description)
    ...
    def _generate_explanation(
        ...
        if status == VerdictStatus.APPROVED:
            return (
                f"Reasons are well-justified (score: {score}/10). Evidence quality: {evidence_quality}. Risk/reward ratio: {risk_reward_ratio:.2f}."
            )
        elif status == VerdictStatus.REJECTED:
            return (
                f"Reasons are insufficient (score: {score}/10). Weak evidence ({evidence_quality}) and/or poor risk/reward ({risk_reward_ratio:.2f})."
            )
