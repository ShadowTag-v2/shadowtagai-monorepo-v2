...
class BrakesValidator:
  ...
  def validate(self, action: Action) -> BrakesVerdict:
    ...
    explanation = self._generate_explanation(
      status, score, threats_detected, compliance_violations
    )
  ...
  def _detect_threats(self, action: Action) -> list[str]:
    ...
    if any(
      keyword in text_to_check.lower() for keyword in ["password", "secret", "api_key"]
    ):
  ...
  def _generate_explanation(
    ...
    if status == VerdictStatus.REJECTED:
      ...
      violation_list = (
        ", ".join(compliance_violations) if compliance_violations else "none"
      )
