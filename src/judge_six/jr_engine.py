...
class JREngine:
  ...
  def validate(self, action: Action) -> JRVerdict:
    ...
    overall_status = self._determine_overall_status(
      purpose_verdict, reasons_verdict, brakes_verdict
    )
  ...
  def _determine_overall_status(self, purpose, reasons, brakes) -> VerdictStatus:
    ...
    if (
      purpose.status == VerdictStatus.REJECTED
      or reasons.status == VerdictStatus.REJECTED
    ):
      return VerdictStatus.REJECTED
    ...
    if (
      purpose.status == VerdictStatus.FLAGGED
      or reasons.status == VerdictStatus.FLAGGED
      or brakes.status == VerdictStatus.FLAGGED
    ):
