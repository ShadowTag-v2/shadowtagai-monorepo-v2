...
class EthicalScraper:
    ...
    async def fetch_url(self, url: str, headers: dict | None = None) -> str | None:
        ...
        # 1. Check robots.txt
        if self.config["robots_txt"]["enabled"]:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "open_circuits": len([d for d in self.circuit_breakers.keys() if self.is_circuit_open(d)]),
            "circuit_breakers": {
                domain: {"failures": failures, "opened_at": opened_at.isoformat()} for domain, (failures, opened_at) in self.circuit_breakers.items()
            },
        }
...
DEFAULT_SCRAPING_CONFIG = {
    ...
    "error_handling": {"respect_retry_after": True, "exponential_backoff": True, "max_retries": 3, "circuit_breaker": True},
}
