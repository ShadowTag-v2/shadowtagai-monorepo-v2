...
from ..scraper.ethical_scraper import EthicalScraper, DEFAULT_SCRAPING_CONFIG
...
class IntelligenceIngestion:
    ...
    async def ingest_all(self) -> list[IntelligenceItem]:
        ...
        logger.info(f"✓ Ingestion complete: {len(self.intelligence_items)} items in {duration:.1f}s")
        ...
        try:
            ...
            for doc in results:
                item = IntelligenceItem(
                    ...
                    published_date=datetime.fromisoformat(doc.get("publication_date", datetime.now().isoformat())),
                    metadata={
    ...
    async def ingest_state_regulations(self):
        ...
        try:
            ...
            item = IntelligenceItem(
                ...
                metadata={"state": "CA", "bill_number": "AB 2885", "effective_date": "2026-01-01", "status": "Enacted"},
            )
            self.intelligence_items.append(item)
    ...
    async def ingest_arxiv_papers(self):
        ...
        queries = ["ti:AI+governance", "ti:AI+ethics", "ti:AI+safety+policy", "ti:algorithmic+accountability"]
            ...
            for source_name, feed_url in news_feeds.items():
                ...
                for entry in feed.entries[:10]:  # Limit to 10 most recent
                    ...
                    if any(
                        ...
                        for keyword in ["ai", "artificial intelligence", "machine learning", "openai", "anthropic", "regulation"]
                    ):
                        item = IntelligenceItem(
                            ...
                            metadata={"publication": source_name, "author": entry.get("author", "Unknown")},
                        )
                        self.intelligence_items.append(item)
        ...
        try:
            for channel_name, feed_url in youtube_channels.items():
                ...
                for entry in feed.entries[:5]:
                    ...
                    if any(keyword in entry.title.lower() for keyword in ["ai", "artificial intelligence", "technology policy", "regulation"]):
                        item = IntelligenceItem(
                            ...
                            content=entry.get("summary", ""),
                            published_date=datetime(*entry.published_parsed[:6]) if hasattr(entry, "published_parsed") else datetime.now(),
                            metadata={"channel": channel_name, "video_id": entry.yt_videoid if hasattr(entry, "yt_videoid") else None},
                        )
                        self.intelligence_items.append(item)
