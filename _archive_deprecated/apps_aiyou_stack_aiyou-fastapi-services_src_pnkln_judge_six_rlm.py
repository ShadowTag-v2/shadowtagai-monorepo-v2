...
class RLMDecision:
  ...
  decision_result: str  # APPROVED | BLOCKED
  confidence: float
...
class RecursiveJudge:
  ...
  def __init__(self, base_judge: JudgeSix | None = None):
    ...
    self.audit_path = os.getenv(
      "RLM_AUDIT_PATH", "/var/log/pnkln/rlm-judge-audit.jsonl"
    )
  ...
  async def validate_large_context(
    ...
    decision = RLMDecision(
      decision_result=final_decision["result"],
  ...
  async def _spawn_specialists(
    self, payload: dict, focus_areas: list[str]
  ) -> list[dict]:
    """
...
async def validate_with_rlm(
  ...
  Usage:
      ...
      if result.decision_result == ValidationResult.APPROVED:
          # proceed
  """
...
if __name__ == "__main__":
  ...
  async def test_rlm_judge():
    ...
    print(f"  Result: {result1.decision_result}")
    ...
    print(f"  Result: {result2.decision_result}")
