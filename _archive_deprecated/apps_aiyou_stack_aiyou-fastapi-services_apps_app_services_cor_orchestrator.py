...
class SessionManager:
  ...
  def cleanup_expired(self) -> int:
    ...
    expired = [
      sid for sid, data in self._sessions.items() if data["start_time"] < cutoff
    ]
    for sid in expired:
...
class CorOrchestrator:
  ...
  async def _assess_internal(
    ...
    if request.session_id:
      session_duration = self.session_manager.get_session_duration_minutes(
        request.session_id
      )
    ...
    audit_trail.append(
      self.audit_manager.log_entry(
        ...
        output_hash=hashlib.sha256(
          str(ns_output.overall_risk_score).encode()
        ).hexdigest()[:16],
    ...
    audit_trail.append(
      self.audit_manager.log_entry(
        ...
        output_hash=hashlib.sha256(str(jr_output.go_decision).encode()).hexdigest()[
          :16
        ],
    ...
    disclosure_text = (
      "I am an AI assistant. I am not a human." if disclosure_required else ""
    )
    ...
    result = CaliforniaAIAssessmentResult(
      ...
      human_review_reason=jr_output.reasoning
      if jr_output.human_review_required
      else None,
    )
    ...
    return {
      **self._stats,
      ...
      "crisis_response_rate": (
        self._stats["crisis_responses"] / total if total > 0 else 0.0
      ),
    }
