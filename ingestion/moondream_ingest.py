...
def parse_with_moondream(file_path: Path) -> dict[str, Any]:
    ...
    if ext in {".txt", ".md", ".csv", ".json"}:
        ...
        try:
            ...
            return {"text": "", "json": None, "meta": {"mode": "error", "error": "Could not read text file"}}
        ...
        return {
            ...
            "json": {"file_type": ext.lstrip("."), "size_bytes": file_path.stat().st_size, "placeholder": True},
...
def ingest_directory(root: Path, output: Path, seen_path: Path | None = None) -> int:
    ...
    exts = {".png", ".jpg", ".jpeg", ".pdf", ".tiff", ".bmp", ".webp", ".txt", ".md", ".csv", ".json", ".html"}
