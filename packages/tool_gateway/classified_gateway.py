...
_PATH_KEYS = frozenset(
  {"path", "file", "target", "TargetFile", "AbsolutePath", "filePath"}
)
...
class ClassifiedGateway:
  ...
  def _load_permissions(self, path: Path) -> None:
    ...
    if not path.exists():
      logger.warning(
        "Tool permissions not found at %s — all tools require classifier", path
      )
      return
    ...
    for key in _PATH_KEYS:
      ...
      if not result.is_allowed:
        ...
        return Decision(
          allowed=False,
          reason=(
            f"Sandbox path denied (Tier 1.25): {result.deny_reason} [path='{path_value}', resolved='{result.resolved}']"
          ),
  ...
  def check(
    ...
    if contract and contract.risk_level in ("medium", "high", "critical"):
      ...
      if has_consequential_precond and not context.get(
    ...
    if ba_result.final_verdict == BAVerdict.BLOCK:
      ...
      block_reasons = ", ".join(
        f"{r.rule_id}: {r.description}" for r in ba_result.matched_rules
      )
      self._emit(
      ...
      return Decision(
        allowed=False,
        reason=(
          f"BLOCK/ALLOW Engine BLOCKED (Tier 1.5): {block_reasons}. Reasoning: {ba_result.reasoning}"
        ),
    ...
    if tool_id in self._auto_approved:
      ...
      event = (
        ...
        else GatewayEvent.CONTRACT_BLOCKED
      )
      self._emit(
        event,
        tool_id,
    ...
    if classifier_result.verdict == ClassifierVerdict.BLOCK:
      ...
      return Decision(
        allowed=False,
        reason=(
          f"Classifier BLOCKED (stage {classifier_result.stage}): {classifier_result.reasoning}"
        ),
    ...
    if contract_decision.allowed:
      ...
      return Decision(
        allowed=True,
        reason=(
          f"Classifier ALLOWED (stage {classifier_result.stage}) AND contracts passed: {contract_decision.reason}"
        ),
