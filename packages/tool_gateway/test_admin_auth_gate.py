...
class MockVerifier:
  ...
  def __init__(
    self, claims: dict[str, Any] | None = None, *, raise_exc: Exception | None = None
  ):
...
class TestVerifyAdminClaim:
  ...
  def test_admin_in_custom_claims(self):
    verifier = MockVerifier(
      {"uid": "user2", "email": "admin2@test.com", "custom_claims": {"admin": True}}
    )
    claim = verify_admin_claim("test-token", verifier=verifier)
    assert claim.is_admin is True
  ...
  def test_custom_admin_claim_key(self):
    verifier = MockVerifier(
      {"uid": "user5", "email": "super@test.com", "superadmin": True}
    )
    claim = verify_admin_claim(
      "test-token", verifier=verifier, admin_claim_key="superadmin"
    )
    assert claim.is_admin is True
