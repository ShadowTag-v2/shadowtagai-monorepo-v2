...
"""Async Evidence Logger — Non-blocking audit entries to .beads/issues.jsonl.

Asynchronous port of evidence.py using aiofiles for non-blocking I/O.
Delivers ~64% throughput improvement on concurrent tool gateway checks
by eliminating synchronous file I/O blocking in the event loop.

Every tool gateway decision is logged as a structured JSON line for
observability (Pillar 4 of the Control Plane).
"""
