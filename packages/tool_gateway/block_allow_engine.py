...
_CREDENTIAL_PATTERNS = [
    re.compile(r"(?i)(api[_-]?key|secret[_-]?key|access[_-]?token|bearer)\s*[:=]\s*\S{8,}"),
...
_EXFIL_COMMANDS = {"curl", "wget", "nc", "ncat", "socat", "scp", "rsync", "ssh", "ftp", "sftp"}
...
class BlockAllowRuleEngine:
    ...
    def _check_data_safety(
        ...
        output_content = str(tool_input.get("content", "")) + str(tool_input.get("body", ""))
