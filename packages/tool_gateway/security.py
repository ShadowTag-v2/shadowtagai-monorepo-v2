  ...
  def check_subcommand_count(self, command: str) -> SecurityCheckResult:
    ...
    if total > MAX_SUBCOMMANDS:
      return SecurityCheckResult(
        passed=False,
        reason=(
          f"Command has {total} subcommands (cap: {MAX_SUBCOMMANDS}). Break into smaller commands or get explicit approval."
        ),
      )
    ...
    if total > MAX_SUBCOMMANDS * 0.8:
      return SecurityCheckResult(
        passed=True,
        warnings=[
          f"Command has {total} subcommands (approaching cap of {MAX_SUBCOMMANDS})"
        ],
      )
  ...
  def check_tool_result_size(self, result_text: str) -> SecurityCheckResult:
    ...
    if len(result_text) > TOOL_RESULT_MAX_CHARS:
      return SecurityCheckResult(
        passed=True,
        warnings=[
          f"Tool result is {len(result_text)} chars (truncation limit: {TOOL_RESULT_MAX_CHARS})"
        ],
      )
    return SecurityCheckResult(passed=True)
