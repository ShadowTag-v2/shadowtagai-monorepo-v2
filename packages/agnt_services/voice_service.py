...
def check_voice_dependencies() -> VoiceDependencies:
  ...
  return VoiceDependencies(
    available=False, missing=["sox (rec command)"], install_command=pm
  )
...
def check_recording_availability() -> RecordingAvailability:
  ...
  if system == "Windows":
    return RecordingAvailability(
      available=False, reason="Voice mode requires native audio module on Windows."
    )
  pm = detect_package_manager()
  if pm:
    return RecordingAvailability(
      available=False, reason=f"Voice mode requires SoX. Install: {pm}"
    )
  return RecordingAvailability(
    available=False, reason="Voice mode requires SoX for audio recording."
  )
...
class VoiceRecorder:
  ...
  def _start_arecord(
    self, on_data: Callable[[bytes], None], on_end: Callable[[], None]
  ) -> bool:
    args = [
      "arecord",
      "-f",
