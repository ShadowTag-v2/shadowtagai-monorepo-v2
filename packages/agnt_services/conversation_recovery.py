...
def migrate_legacy_attachment_types(
  ...
  *,
  ...
  if "displayPath" not in attachment:
    path = (
      attachment.get("filename") or attachment.get("path") or attachment.get("skillDir")
    )
    if path:
...
def deserialize_messages_with_interrupt_detection(
  ...
  for msg in migrated:
    if (
      msg.get("type") == "user"
      and msg.get("permissionMode") is not None
      and msg.get("permissionMode") not in PERMISSION_MODES
    ):
