...
class MemoryExtractor:
  ...
  def __init__(
    ...
    *,
    ...
    self._is_auto_mem_path = is_auto_mem_path or (
      lambda p: p.startswith(str(self._memory_dir))
    )
    ...
    self._pending_context: (
      tuple[list[dict[str, Any]], AppendSystemMessage | None] | None
    ) = None
  ...
  async def run(
    ...
    *,
    ...
    return await self._run_extraction(
      messages, append_system_message, is_trailing=False
    )
  ...
  async def _run_extraction(
    ...
    *,
    ...
    if has_memory_writes_since(
      messages, self._last_cursor_uuid, self._is_auto_mem_path
    ):
