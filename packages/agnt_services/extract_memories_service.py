...
def count_model_visible_since(messages: list[MemoryMessage], since_uuid: str | None) -> int:
...
def can_use_tool_for_extraction(tool_name: str, tool_input: dict, memory_dir: str) -> tuple[bool, str]:
...
class ExtractMemoriesService:
    def __init__(self, memory_dir: str | Path, config: ExtractMemoriesConfig | None = None):
    ...
    def should_extract(self, messages: list[MemoryMessage]) -> bool:
        ...
        threshold = (
            ...
            else int(self.config.min_messages_between_extractions * self.config.cadence_multiplier)
        )
