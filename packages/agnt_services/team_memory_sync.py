...
_SECRET_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
  ...
  (
    "Generic API Key",
    re.compile(r"(?i)api[_-]?key\s*[:=]\s*['\"][A-Za-z0-9]{20,}", re.ASCII),
  ),
...
class TeamMemoryEntry:
  ...
  @staticmethod
  def from_content(
    key: str, content: str, author: str | None = None
  ) -> TeamMemoryEntry:
...
class TeamMemorySyncService:
  ...
  def add_entry(
    self, key: str, content: str, author: str | None = None
  ) -> SecretFinding | None:
  ...
  def get_changed_entries(
    ...
    """
    return {
      k: v for k, v in self._entries.items() if remote_hashes.get(k) != v.content_hash
    }
