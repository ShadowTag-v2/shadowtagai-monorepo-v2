...
def accumulate_usage(current: TokenUsage, delta: TokenUsage) -> TokenUsage:
    ...
    return TokenUsage(
        ...
        cache_read_input_tokens=current.cache_read_input_tokens + delta.cache_read_input_tokens,
        cache_creation_input_tokens=current.cache_creation_input_tokens + delta.cache_creation_input_tokens,
...
def create_subagent_context(
    ...
    messages = overrides.messages if overrides.messages is not None else copy.deepcopy(parent_messages or [])
                    ...
                    if message.get("type") == "stream_event":
                        ...
                        if event.get("type") == "message_delta" and "usage" in event:
                            delta = TokenUsage(
                                ...
                                cache_creation_input_tokens=event["usage"].get("cache_creation_input_tokens", 0),
                            )
