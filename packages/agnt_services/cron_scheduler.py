...
def is_recurring_task_aged(
  ...
  if max_age_ms == 0:
    return False
  return bool(
    task.recurring and not task.permanent and now_ms - task.created_at >= max_age_ms
  )
...
class CronScheduler:
  ...
  def _check(self) -> None:
    ...
    jitter_cfg = (
      self._options.get_jitter_config()
      if self._options.get_jitter_config
      else DEFAULT_JITTER_CONFIG
    )
    seen: set[str] = set()
