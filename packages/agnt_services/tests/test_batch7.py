...
class TestOAuthService:
    ...
    def test_oauth_cleanup_resets_state(self):
        ...
        config = OAuthConfig(base_api_url="x", client_id="x", authorize_url="x", token_url="x", profile_url="x")
        svc = OAuthService(config)
...
class TestRemoteManagedSettings:
    ...
    def test_compute_checksum_deterministic(self):
        from packages.agnt_services.remote_managed_settings import compute_checksum_from_settings
...
class TestLSPClient:
    ...
    def test_server_capabilities_from_dict(self):
        ...
        caps = ServerCapabilities.from_dict({"hoverProvider": True, "definitionProvider": False, "extra": 42})
...
class TestVoiceService:
    ...
    def test_voice_dependencies_struct(self):
        ...
        d = VoiceDependencies(available=False, missing=["sox"], install_command="brew install sox")
...
class TestToolOrchestration:
    ...
    def test_run_tools_concurrently(self):
        ...
        results = asyncio.run(run_tools_concurrently(blocks, executor, ToolUseContext(), max_concurrency=3))
