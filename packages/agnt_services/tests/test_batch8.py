...
class TestMCPChannelService(TestCase):
  ...
  def test_filter_permission_relay_clients(self):
    from packages.agnt_services.mcp_channel_service import (
    ...
    result = filter_permission_relay_clients(
      clients, lambda n: n in ("telegram", "slack")
    )
    assert len(result) == 1
...
class TestSessionMemoryService(TestCase):
  def test_count_tool_calls_since(self):
    ...
    msgs = [
      ...
      Message(
        uuid="3", type="assistant", tool_calls=[{"name": "write"}, {"name": "edit"}]
      ),
    ]
...
class TestExtractMemoriesService(TestCase):
  ...
  def test_can_use_tool_sandbox(self):
    ...
    assert allowed
    allowed, _ = can_use_tool_for_extraction(
      "file_write", {"file_path": "/mem/note.md"}, "/mem"
    )
    assert allowed
    allowed, _ = can_use_tool_for_extraction(
      "file_write", {"file_path": "/etc/passwd"}, "/mem"
    )
    assert not allowed
...
class TestTokenEstimationService(TestCase):
  def test_estimate_tokens_from_text(self):
    from packages.agnt_services.token_estimation_service import (
...
class TestTeamMemorySyncService(TestCase):
  ...
  def test_compute_push_delta(self):
    ...
    entries = [
      TeamMemoryEntry(
        key="old.md", content="old content", checksum=hash_content("old content")
      ),
      TeamMemoryEntry(
        key="new.md", content="new content", checksum=hash_content("new content")
      ),
    ]
  ...
  def test_batch_entries_for_push(self):
    ...
    entries = [
      TeamMemoryEntry(key=f"file{i}.md", content="x" * 50000, checksum="sha256:test")
      for i in range(5)
    ]
