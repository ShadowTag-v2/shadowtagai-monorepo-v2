...
def _encode_jsonrpc(method: str, params: Any = None, request_id: int | None = None) -> bytes:
...
class LSPClient:
    ...
    def start(self, command: str, args: list[str] | None = None, *, env: dict[str, str] | None = None, cwd: str | None = None) -> None:
        import os

        full_env = {**os.environ, **(env or {})}
        self._process = subprocess.Popen(
            [command, *(args or [])], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=full_env, cwd=cwd
        )
        logger.debug("LSP client started for %s (PID %d)", self.server_name, self._process.pid)
