...
def has_thinking_blocks(messages: list[MessageBlock]) -> bool:
    return any(msg.role == "assistant" and msg.type in (ContentType.THINKING, ContentType.REDACTED_THINKING) for msg in messages)
...
class TokenEstimationService:
    ...
    def __init__(self, provider: str = "firstParty"):
        self.provider = APIProvider(provider) if provider in APIProvider.__members__.values() else APIProvider.ANTHROPIC
    ...
    def count_messages_tokens(self, messages: list[dict], tools: list[dict] | None = None) -> TokenCount:
