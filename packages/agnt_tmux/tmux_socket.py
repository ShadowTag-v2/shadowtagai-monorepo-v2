...
class TmuxSocketManager:
    ...
    def initialize(self):
        ...
        subprocess.run(["tmux", "-L", self.socket_name, "new-session", "-d", "-s", "base"], check=True, capture_output=True)

        # Set environment to skip prompt history (anti-pollution)
        subprocess.run(["tmux", "-L", self.socket_name, "set-environment", "-g", "AGNT_SKIP_PROMPT_HISTORY", "true"], check=True, capture_output=True)

        # Retrieve socket path and PID
        res = subprocess.run(
            ["tmux", "-L", self.socket_name, "display-message", "-p", "#{socket_path},#{pid}"], capture_output=True, text=True, check=True
        )
    ...
    def exec_command(self, cmd: str) -> subprocess.CompletedProcess:
        ...
        return subprocess.run(["tmux", "-L", self.socket_name] + cmd.split(), capture_output=True, text=True)
