...
@given(session_name=st.text(min_size=1), message=st.text())
def test_jules_session_approve_plan(session_name, message):
  ...
  session = JulesSession(
    client=mock_client, source_name="test_source", session_name=session_name
  )
...
@given(session_name=st.text(min_size=1), text=st.text())
...
def test_jules_session_run_auto_pr_workflow_happy_path(mock_sleep, mock_time):
  ...
  result = session.run_auto_pr_workflow(
    plan_approval_callback=mock_approval_callback, timeout=600, interval=10
  )

  assert result["state"] == "COMPLETED"
  assert callback_called is True
  mock_client.approve_plan.assert_called_once_with(
    session_name, message="Approved via auto PR workflow"
  )
