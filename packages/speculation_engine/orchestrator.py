...
class SpeculativeResearchOrchestrator:
  ...
  def async_consumer(self) -> Any:
    ...
    if self._async_consumer is None and self._flags.is_enabled(
      SpecFlags.ASYNC_CONSUMER
    ):
  ...
  def auto_route(self, query: str) -> Any:
    ...
    logger.info(
      "Pipeline auto-routed to %s for query: %s...", self._active_mode.value, query[:60]
    )
  ...
  def _semantic_classify(self, query: str) -> Any:
    ...
    threshold = 0.4
    mode = (
      PipelineMode.RESEARCH_SWEEP
      if score >= threshold
      else PipelineMode.PAIR_PROGRAMMING
    )
  ...
  def _make_cow_handler(
    ...
    async def handler(
      ...
      overlay_count = (
        len(self._engine.overlay.written_files) if self._engine.overlay else 0
      )
  ...
  def get_session_summary(self) -> dict[str, Any]:
    ...
    suggestion_count = sum(
      1 for r in self._speculation_results if r.suggestion and r.suggestion.suggestion
    )
