...
class CacheBreakDetector:
  ...
  def _record_prompt_state_inner(self, snapshot: PromptStateSnapshot) -> None:
    ...
    cache_control_hash = _compute_hash(
      [block.get("cache_control") for block in snapshot.system]
    )
    ...
    extra_body_hash = (
      _compute_hash(snapshot.extra_body_params)
      if snapshot.extra_body_params is not None
      else 0
    )

    def lazy_diffable() -> str:
      return _build_diffable_content(
        snapshot.system, snapshot.tool_schemas, snapshot.model
      )
    ...
    if any_change:
      ...
      if ts_changed:
        ...
        for name in tool_names:
          if name in prev_tool_set and new_hashes.get(name) != prev.per_tool_hashes.get(
            name
          ):
  ...
  async def _check_inner(
    ...
    if (
      cache_read_tokens >= prev_cache_read * 0.95 or token_drop < MIN_CACHE_MISS_TOKENS
    ):
      state.pending_changes = None
      return None
    ...
    if changes:
      ...
      if changes.tool_schemas_changed:
        if changes.added_tool_count > 0 or changes.removed_tool_count > 0:
          tool_diff = (
            f" (+{changes.added_tool_count}/-{changes.removed_tool_count} tools)"
          )
        else:
      ...
      if (
        changes.cache_control_changed
        and not changes.global_cache_strategy_changed
        and not changes.system_prompt_changed
      ):
    ...
    event_data: dict[str, Any] = {
      ...
      "global_cache_strategy_changed": changes.global_cache_strategy_changed
      if changes
      else False,
      ...
      "added_tools": ",".join(
        _sanitize_tool_name(t) for t in (changes.added_tools if changes else [])
      ),
      "removed_tools": ",".join(
        _sanitize_tool_name(t) for t in (changes.removed_tools if changes else [])
      ),
      "changed_tool_schemas": ",".join(
        _sanitize_tool_name(t)
        for t in (changes.changed_tool_schemas if changes else [])
      ),
      "call_number": state.call_count,
      "prev_cache_read_tokens": prev_cache_read,
      "cache_read_tokens": cache_read_tokens,
      "cache_creation_tokens": cache_creation_tokens,
      "time_since_last_assistant_msg": time_since_last
    ...
    if changes and changes._build_prev_diffable and state._build_diffable:
      diff_path = self._write_diff(
        changes._build_prev_diffable(), state._build_diffable()
      )
    ...
    summary = (
      ...
      f"creation: {cache_creation_tokens}"
