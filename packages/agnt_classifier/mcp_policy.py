...
def is_mcp_server_denied(
  ...
  for entry in policy.denied_servers:
    if entry.entry_type == PolicyEntryType.NAME:
      ...
      entry.entry_type == PolicyEntryType.URL
      and server_info
      and server_info.url
      and _url_matches_pattern(server_info.url, entry.server_url)
    ):
      return True, entry
...
def is_mcp_server_allowed_by_policy(
  ...
  if server_info:
    ...
    if server_info.command:
      if has_command_entries:
        for entry in policy.allowed_servers:
          if entry.entry_type == PolicyEntryType.COMMAND and _command_arrays_match(
            entry.server_command, server_info.command
          ):
            return PolicyResult(
              allowed=True,
    ...
    # Remote server with URL
      ...
      for entry in policy.allowed_servers:
        if entry.entry_type == PolicyEntryType.URL and _url_matches_pattern(
