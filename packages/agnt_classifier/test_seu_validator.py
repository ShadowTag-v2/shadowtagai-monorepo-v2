...
class TestLineClassification:
    ...
    def test_safety_line(self):
        v = SEUValidator()
        assert v.classify_line("WARNING: This is not a substitute for legal advice") == SEUSection.SAFETY

    def test_empathy_line(self):
        v = SEUValidator()
        assert v.classify_line("I understand your concern, that's a great question") == SEUSection.EMPATHY

    def test_utility_line(self):
        v = SEUValidator()
        assert v.classify_line("Here is the code to implement the feature") == SEUSection.UTILITY
