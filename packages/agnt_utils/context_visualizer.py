...
def _process_assistant_message(
    ...
    for block in inner.get("content", []):
        ...
        if isinstance(block, dict) and block.get("type") == "tool_use":
            ...
            breakdown.tool_calls_by_type[tool_name] = breakdown.tool_calls_by_type.get(tool_name, 0) + block_tokens
        else:
...
def _process_user_message(
    ...
    for block in content:
        ...
        if isinstance(block, dict) and block.get("type") == "tool_result":
            ...
            tool_name = tool_use_id_to_name.get(tool_use_id, "unknown") if tool_use_id else "unknown"
            breakdown.tool_results_by_type[tool_name] = breakdown.tool_results_by_type.get(tool_name, 0) + block_tokens
        else:
...
def _process_attachment(
    ...
    attach_type = attachment.get("type", "unknown") if isinstance(attachment, dict) else "unknown"
    breakdown.attachments_by_type[attach_type] = breakdown.attachments_by_type.get(attach_type, 0) + tokens
...
def analyze_context_usage(
    ...
    *,
    ...
    if deferred_tool_tokens > 0:
        cats.append(ContextCategory("MCP tools (deferred)", deferred_tool_tokens, "inactive", is_deferred=True))
    ...
    if is_auto_compact and auto_compact_threshold is not None:
        ...
        cats.append(ContextCategory(_MANUAL_COMPACT_BUFFER_NAME, reserved_tokens, "inactive"))
    ...
    if api_usage:
        api_total = api_usage.get("input_tokens", 0) + api_usage.get("cache_creation_input_tokens", 0) + api_usage.get("cache_read_input_tokens", 0)
