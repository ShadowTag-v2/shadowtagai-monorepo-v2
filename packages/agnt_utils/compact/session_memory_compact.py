...
def adjust_index_to_preserve_api_invariants(
  ...
  if all_tool_result_ids:
    ...
    needed_tool_use_ids = {
      tid for tid in all_tool_result_ids if tid not in tool_use_ids_in_kept
    }
          ...
          if isinstance(content, list):
            for block in content:
              if (
                isinstance(block, dict)
                and block.get("type") == "tool_use"
                and block.get("id") in needed_tool_use_ids
              ):
  ...
  for i in range(adjusted_index - 1, -1, -1):
    ...
    inner = msg.get("message", {})
    if (
      isinstance(inner, dict) and inner.get("id") and inner["id"] in message_ids_in_kept
    ):
      adjusted_index = i
...
def try_session_memory_compaction(
  ...
  *,
  ...
  try:
    ...
    messages_to_keep = [
      m for m in messages[start_index:] if not is_compact_boundary_message(m)
    ]
