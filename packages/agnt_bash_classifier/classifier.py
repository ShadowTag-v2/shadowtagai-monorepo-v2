...
_SIGNAL_TRAPPING_RE = re.compile(
  r"\btrap\s+['\"].*?['\"]\s+(EXIT|SIGINT|SIGTERM|DEBUG|ERR)\b"
)
_ALIAS_INJECTION_RE = re.compile(r"\balias\s+[a-zA-Z0-9_-]+=")
_CROSS_SHELL_INJECTION_RE = re.compile(
  r"(?i)\b(Invoke-Expression|iex|powershell(\.exe)?\s+-e(ncodedcommand)?|cmd(\.exe)?\s+/c)\b|%COMSPEC%"
)
_FUNCTION_HIJACKING_RE = re.compile(
  r"\bexport\s+-f\b|^\s*(function\s+)?[a-zA-Z_][a-zA-Z0-9_]*\s*\(\s*\)\s*\{"
)
...
def _check_ansi_c_quoting(command: str) -> CheckResult:
  ...
  if _ANSI_C_QUOTING_RE.search(command):
    return CheckResult(
      BashSecurityCheckId.ANSI_C_QUOTING, CheckVerdict.BLOCK, "ANSI-C quoting detected"
    )
...
def _check_shell_builtin_abuse(command: str) -> CheckResult:
  ...
  if _SHELL_BUILTIN_ABUSE_RE.search(command):
    return CheckResult(
      BashSecurityCheckId.SHELL_BUILTIN_ABUSE,
      CheckVerdict.BLOCK,
      "Shell built-in abuse (exec/eval) detected",
    )
...
def _check_signal_trapping(command: str) -> CheckResult:
  ...
  if _SIGNAL_TRAPPING_RE.search(command):
    return CheckResult(
      BashSecurityCheckId.SIGNAL_TRAPPING,
      CheckVerdict.BLOCK,
      "Signal trapping detected",
    )
...
def _check_alias_injection(command: str) -> CheckResult:
  ...
  if _ALIAS_INJECTION_RE.search(command):
    return CheckResult(
      BashSecurityCheckId.ALIAS_INJECTION,
      CheckVerdict.BLOCK,
      "Alias injection detected",
    )
...
def _check_cross_shell_injection(command: str) -> CheckResult:
  ...
  if _CROSS_SHELL_INJECTION_RE.search(command):
    return CheckResult(
      BashSecurityCheckId.CROSS_SHELL_INJECTION,
      CheckVerdict.BLOCK,
      "Cross-shell injection detected",
    )
...
def _check_function_hijacking(command: str) -> CheckResult:
  ...
  if _FUNCTION_HIJACKING_RE.search(command):
    return CheckResult(
      BashSecurityCheckId.FUNCTION_HIJACKING,
      CheckVerdict.BLOCK,
      "Function hijacking detected",
    )
...
class BashSecurityClassifier:
  ...
  def classify_for_gateway(
    ...
    return {
      "allowed": False,
      "reason": f"Bash security check BLOCKED (check {blocked.check_id.name}): {blocked.message}"
      if blocked
      else "Unknown block",
