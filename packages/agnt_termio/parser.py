...
def _is_emoji(cp: int) -> bool:
    return (
        (0x2600 <= cp <= 0x26FF) or (0x2700 <= cp <= 0x27BF) or (0x1F300 <= cp <= 0x1F9FF) or (0x1FA00 <= cp <= 0x1FAFF) or (0x1F1E0 <= cp <= 0x1F1FF)
    )
...
def _parse_csi(raw: str) -> Action | None:
    ...
    if final_byte == CSI.DECSCUSR and intermediate == " ":
        ...
        return CursorActionWrapper(action=CursorStyleAction(style=info["style"], blinking=info["blinking"]))
    ...
    if private_mode == "?" and final_byte in (CSI.SM, CSI.RM):
        ...
        if p0 == DEC.MOUSE_NORMAL:
            return ModeActionWrapper(action=MouseTrackingMode(mode="normal" if enabled else "off"))
        if p0 == DEC.MOUSE_BUTTON:
            return ModeActionWrapper(action=MouseTrackingMode(mode="button" if enabled else "off"))
        if p0 == DEC.MOUSE_ANY:
            return ModeActionWrapper(action=MouseTrackingMode(mode="any" if enabled else "off"))
