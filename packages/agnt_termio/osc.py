...
def tmux_load_buffer(text: str) -> bool:
    ...
    args = ["load-buffer", "-"] if os.environ.get("LC_TERMINAL") == "iTerm2" else ["load-buffer", "-w", "-"]
...
def tab_status(fields: TabStatusAction) -> str:
    ...
    if fields.has_indicator:
        parts.append(f"indicator={_rgb(fields.indicator) if isinstance(fields.indicator, RGBColor) else ''}")
    if fields.has_status:
        status_val = fields.status.replace("\\", "\\\\").replace(";", "\\;") if fields.status else ""
        parts.append(f"status={status_val}")
    if fields.has_status_color:
        parts.append(f"status-color={_rgb(fields.status_color) if isinstance(fields.status_color, RGBColor) else ''}")
