...
def prometheus_metrics(registry: Any | None = None) -> str:
  ...
  for name, snap in sorted(snapshots.items()):
    ...
    lines.append(
      f'circuit_breaker_state{{service="{name}"}} {state_val} {timestamp_ms}'
    )

  # --- Failures total ---
  lines.append(
    "# HELP circuit_breaker_failures_total Total failures recorded by the circuit breaker"
  )
  lines.append("# TYPE circuit_breaker_failures_total counter")
  for name, snap in sorted(snapshots.items()):
    lines.append(
      f'circuit_breaker_failures_total{{service="{name}"}} {snap.get("total_failures", 0)} {timestamp_ms}'
    )

  # --- Successes total ---
  lines.append(
    "# HELP circuit_breaker_successes_total Total successes recorded by the circuit breaker"
  )
  lines.append("# TYPE circuit_breaker_successes_total counter")
  for name, snap in sorted(snapshots.items()):
    lines.append(
      f'circuit_breaker_successes_total{{service="{name}"}} {snap.get("total_successes", 0)} {timestamp_ms}'
    )

  # --- Consecutive failures gauge ---
  lines.append(
    "# HELP circuit_breaker_consecutive_failures Current consecutive failure count"
  )
  lines.append("# TYPE circuit_breaker_consecutive_failures gauge")
  for name, snap in sorted(snapshots.items()):
    lines.append(
      f'circuit_breaker_consecutive_failures{{service="{name}"}} {snap.get("consecutive_failures", 0)} {timestamp_ms}'
    )

  # --- Seconds until probe ---
  lines.append(
    "# HELP circuit_breaker_seconds_until_probe Seconds remaining until HALF_OPEN probe (0 if not OPEN)"
  )
  lines.append("# TYPE circuit_breaker_seconds_until_probe gauge")
  for name, snap in sorted(snapshots.items()):
    lines.append(
      f'circuit_breaker_seconds_until_probe{{service="{name}"}} {snap.get("seconds_until_probe", 0):.1f} {timestamp_ms}'
    )

  # --- Backoff multiplier ---
  lines.append(
    "# HELP circuit_breaker_backoff_multiplier Exponential backoff multiplier for HALF_OPEN retries"
  )
  lines.append("# TYPE circuit_breaker_backoff_multiplier gauge")
  for name, snap in sorted(snapshots.items()):
    lines.append(
      f'circuit_breaker_backoff_multiplier{{service="{name}"}} {snap.get("backoff_multiplier", 0)} {timestamp_ms}'
    )
