...
def format_health_table(report: dict[str, Any]) -> str:
  ...
  for name, snap in sorted(services.items()):
    ...
    failures = (
      f"{snap.get('consecutive_failures', 0)}/{snap.get('failure_threshold', '?')}"
    )
    probe = (
  ...
  lines.append(
    f"{status_icon} {summary.get('closed', 0)} healthy, {summary.get('open', 0)} open, {summary.get('half_open', 0)} probing"
  )
