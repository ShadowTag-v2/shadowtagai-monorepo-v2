...
def callback_breaker() -> tuple[CircuitBreaker, MagicMock]:
  ...
  cb = MagicMock()
  b = CircuitBreaker(
    "cb_service", failure_threshold=2, reset_timeout_s=0.5, on_state_change=cb
  )
  return b, cb
...
class TestCallbacks:
  ...
  def test_callback_on_open(
    self, callback_breaker: tuple[CircuitBreaker, MagicMock]
  ) -> None:
...
class TestSlidingWindowPerformance:
  ...
  def test_record_failure_throughput_10k(self) -> None:
    ...
    assert ops_per_sec > 5_000, (
      f"record_failure throughput too low: {ops_per_sec:.0f} ops/sec (avg {avg_us:.1f}µs/op)"
    )
  ...
  def test_record_success_throughput_10k(self) -> None:
    ...
    assert ops_per_sec > 10_000, (
      f"record_success throughput too low: {ops_per_sec:.0f} ops/sec (avg {avg_us:.1f}µs/op)"
    )
