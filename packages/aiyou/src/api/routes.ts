...
: Router
{
  ...
  router.post('/graph/insert', async (req: any, res: any) => {
    try {
      ...
      console.log(`[Metric] counter: ShadowTag-v2.api.insert.success`);
      console.log(`[Metric] timer: ShadowTag-v2.api.insert_ms = ${ms}`);
  ...
  router.post('/graph/retrieve', async (req: any, res: any) => {
    try {
      ...
      console.log(`[Metric] counter: ShadowTag-v2.api.retrieve.success`);
      console.log(`[Metric] timer: ShadowTag-v2.api.retrieve_ms = ${ms}`);
...
`;
      ...
      console.log(`[ShadowTag-v2 API] Synthesized response under LegalMetaPersona constraints.`);
