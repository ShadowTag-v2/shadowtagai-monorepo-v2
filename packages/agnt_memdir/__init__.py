...
class WarmStore:
    ...
    def __init__(self, knowledge_dir: str | Path | None = None) -> None:
        self._knowledge_dir = Path(knowledge_dir) if knowledge_dir else self._default_knowledge_dir()
    ...
    def _default_knowledge_dir() -> Path:
        ...
        base = os.environ.get("ANTIGRAVITY_DATA_DIR", os.path.expanduser("~/.gemini/antigravity"))
...
class ColdStore:
    ...
    def size(self) -> int:
        if not self._archive_dir.is_dir():
            return 0
        return sum(1 for d in self._archive_dir.iterdir() if d.is_dir() and not (d / ".purged").exists())
...
class SkepticalIndex:
    ...
    def __init__(
        ...
        *,
        ...
        self.cold = ColdStore(archive_dir=cold_archive_dir, retention_days=cold_retention_days)
    ...
    def search(self, query: str, *, limit: int = 10, include_cold: bool = False) -> list[MemoryEntry]:
