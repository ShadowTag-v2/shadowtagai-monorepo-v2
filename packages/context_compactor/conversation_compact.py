...
def strip_reinjected_attachments(messages: list[Message]) -> list[Message]:
    ...
    """
    return [
        m for m in messages if not (m.get("role") == "attachment" and m.get("attachment", {}).get("type") in ("skill_discovery", "skill_listing"))
    ]
...
def truncate_head_for_ptl_retry(
    ...
    """
    inp = messages
    if inp and inp[0].get("role") == "user" and inp[0].get("isMeta") and inp[0].get("content") == PTL_RETRY_MARKER:
        inp = inp[1:]
    ...
    *,
    ...
    if direction == CompactDirection.UP_TO:
        ...
        messages_to_keep = [
            ...
            if not is_compact_boundary_message(m) and not (m.get("role") == "user" and m.get("isCompactSummary"))
        ]
    else:
        messages_to_summarize = all_messages[pivot_index:]
    ...
    summary_msg = _create_user_message(
        get_compact_user_summary_message(summary, suppress_follow_up=False, transcript_path=None),
        is_compact_summary=True,
