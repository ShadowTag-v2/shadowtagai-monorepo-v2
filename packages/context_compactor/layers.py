...
class Layer2TimeBased(CompactionLayer):
  ...
  def compact(
    ...
    for msg in messages:
      ...
      is_old_assistant = (
        msg.role == "assistant" and (now - msg.timestamp) > STALE_RESULT_AGE_SECONDS * 2
      )
...
class Layer3APIManagement(CompactionLayer):
  ...
  def compact(
    ...
    clear_directives = {
      "clear_tool_uses": [
        messages[i].tool_use_id for i in clearable_tool_uses if messages[i].tool_use_id
      ],
...
class Layer4FullCompaction(CompactionLayer):
  ...
  def compact(
    ...
    try:
      ...
      result.errors.append(
        f"L4 compaction failed (attempt {self._consecutive_failures}/{MAX_CONSECUTIVE_FAILURES}): {e}"
      )
