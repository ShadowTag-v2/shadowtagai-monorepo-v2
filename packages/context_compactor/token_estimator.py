...
class TokenBudgetTracker:
    ...
    def add_tools(self, tool_definitions: list[dict[str, Any]], label: str = "tools") -> int:
...
def rough_token_estimate(content: str, bytes_per_token: int = DEFAULT_BYTES_PER_TOKEN) -> int:
...
def estimate_for_content_block(block: ContentBlock) -> int:
    ...
    match block.block_type:
        ...
        case _:
            ...
            serialized = _safe_json_serialize({"type": block.block_type, "text": block.text, "data": block.data})
...
def estimate_for_message(message: Message) -> int:
    ...
    if isinstance(message.content, list):
        return overhead + sum(estimate_for_content_block(block) for block in message.content)
