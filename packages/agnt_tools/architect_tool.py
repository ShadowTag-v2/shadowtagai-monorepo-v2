...
async def _architect_call(
  ...
  analysis = {
    ...
    "note": (
      "This is a planning artifact. No files were modified. Use implementation tools to execute the plan."
    ),
  }
    ...
    input_schema={
      ...
      "properties": {
        "task": {
          "type": "string",
          "description": (
            "Describe the architectural analysis needed. Be specific about what you want to understand or plan."
          ),
        },
        "scope": {
          "type": "string",
          "description": (
            "Optional directory or module scope to focus the analysis. e.g., 'packages/auth/' or 'src/services/'"
          ),
        },
        "constraints": {
          "type": "array",
          "items": {"type": "string"},
          "description": (
            "Optional list of constraints the plan must respect. e.g., 'No breaking changes', 'Must be backward compatible'"
          ),
        },
      },
      "required": ["task"],
