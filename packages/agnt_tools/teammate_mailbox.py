...
MAILBOX_DIR = (
  Path(
    os.environ.get(
      "REPO_ROOT", os.path.expanduser("~/.gemini/antigravity/Monorepo-Uphillsnowball")
    )
  )
  / ".beads"
  / "mailbox"
...
class TeammateMailbox:
  ...
  def broadcast(
    self, agents: list[str], subject: str, body: str, priority: int = 0
  ) -> list[str]:
    return [
      self.send(a, subject, body, priority) for a in agents if a != self.agent_name
    ]
