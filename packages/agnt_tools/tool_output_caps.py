...
DEFAULT_MAX_RESULT_SIZE_CHARS = int(os.environ.get("TOOL_MAX_RESULT_CHARS", "50000"))  # 50K per tool
MAX_TOOL_RESULTS_PER_MESSAGE_CHARS = int(os.environ.get("TOOL_MAX_AGGREGATE_CHARS", "200000"))  # 200K per turn

OVERFLOW_DIR = Path(
    os.environ.get(
        "TOOL_OVERFLOW_DIR",
        os.path.expanduser("~/.gemini/antigravity/Monorepo-Uphillsnowball/.beads/tool_overflow"),
    )
)
    ...
    def clear_overflow(self, max_age_hours: int = 24) -> int:
        ...
        for f in OVERFLOW_DIR.iterdir():
            if f.is_file() and f.suffix == ".txt":
                age_hours = (now - datetime.fromtimestamp(f.stat().st_mtime, tz=UTC)).total_seconds() / 3600
