...
async def cmd_branch(args: str, ctx: CommandContext) -> CommandResult:
    ...
    try:
        if sub in ("", "current"):
            ...
            log_output = log_result.stdout.strip() if log_result.returncode == 0 else "(unavailable)"
            return CommandResult(
...
async def cmd_mcp(args: str, ctx: CommandContext) -> CommandResult:
    ...
    """
    fleet = [
        {"name": "StitchMCP", "tools": 12, "domain": "Design systems, screen gen, UI variants"},
        {"name": "chrome-devtools-mcp", "tools": 29, "domain": "Browser, DOM, Lighthouse, perf"},
        {"name": "firebase-mcp-server", "tools": 45, "domain": "Auth, Firestore, Hosting, Functions"},
        {"name": "google-developer-knowledge", "tools": 3, "domain": "Google docs search, retrieval"},
