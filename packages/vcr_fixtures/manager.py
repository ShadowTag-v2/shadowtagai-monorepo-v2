...
class Cassette:
    ...
    def save(self) -> None:
        ...
        logger.info("Saved cassette %s with %d interactions", self.name, len(self._interactions))
        self._dirty = False
...
class FixtureManager:
    ...
    def _effective_mode(self, mode: CassetteMode | None = None) -> CassetteMode:
        ...
        if self.enforce_test_gate and not self._is_test_context():
            logger.warning("VCR disabled outside test context. Set PYTEST_CURRENT_TEST or VCR_FORCE_ENABLE.")
