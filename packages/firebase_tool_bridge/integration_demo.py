...
def fetch_weather(city: str, unit: str = "celsius") -> dict[str, Any]:
  ...
  data = weather_db.get(
    city.lower(), {"temp": 20, "conditions": "unknown", "humidity": 50}
  )
  if unit == "fahrenheit":
...
def search_recipes(
  ...
  recipes = [
    ...
    },
    {
      ...
      "description": "Roman egg and cheese pasta",
      "cook_time": "20 min",
    },
    {
      "name": "Pad Thai",
    ...
    {
      "name": "Caesar Salad",
      "description": "Romaine lettuce with anchovy dressing",
      "cook_time": "15 min",
    },
    {
      "name": "Ratatouille",
...
def delete_all_meal_plans() -> dict[str, str]:
  ...
  return {
    "status": "deleted",
    "message": "All meal plans have been permanently deleted.",
  }
...
def cli_confirmation_provider(function_name: str, args: dict[str, Any]) -> bool:
  ...
  """
  logger.info(
    "🔐 CONFIRMATION GATE: '%s' with args %s → AUTO-APPROVED (demo mode)",
    function_name,
    list(args.keys()),
  )
  return True
...
def run_demo() -> None:
  ...
  for reg in registry.list_all():
    logger.info(
      ...
      reg.name,
      reg.risk_tier.value,
      " ⚠️ REQUIRES CONFIRMATION" if reg.risk_tier.requires_confirmation else "",
    )
  ...
  logger.info(
    "   ✅ Model: %s (temperature=%.1f, deterministic tool selection)",
    config.model_name,
    config.temperature,
  )
  ...
  logger.info(
    "   User prompt: 'What's the weather in San Francisco? Suggest some recipes.'"
  )
  ...
  current_content: list[dict[str, Any]] = [
    {"text": "What's the weather in San Francisco? Suggest some recipes."}
  ]
  ...
  while turn < max_turns:
    ...
    logger.info(
      "   🔧 Model proposed %d function call(s):", len(response.function_calls)
    )
  ...
  if evidence_file.exists():
    ...
    logger.info(
      "   ⚠️ No evidence file found (evidence logging may not have written yet)"
    )
  ...
  logger.info(
    "   Evidence records:     %d",
    len(evidence_file.read_text().strip().splitlines())
    if evidence_file.exists()
    else 0,
  )
  logger.info(
    "   Model configuration:  %s (temp=%.1f)", config.model_name, config.temperature
  )
  logger.info("=" * 70)
