...
def build_bridge(tmp_dir: Path) -> ToolBridge:
  ...
  return ToolBridge(
    reg, evidence=EvidenceLogger(repo_root=tmp_dir), confirmation=_AutoApprove()
  )
...
def run_profile(n_calls: int = 200) -> None:
  ...
  print(f"  {'-' * 60}")
  print(
    f"  {'Sync':<20} {sync_time * 1000:>12.2f} {sync_per_call_us:>15.1f} {'baseline':>10}"
  )
  print(
    f"  {'Async':<20} {async_time * 1000:>12.2f} {async_per_call_us:>15.1f} {f'-{delta_pct:.1f}%':>10}"
  )
  print(
