...
def test_allowlisted_functions_always_approve(
  ...
  for fn_name in allowed:
    ...
    assert result is True, (
      f"Allowlisted '{fn_name}' was denied with risk={risk_tier}, tags={action_tags}"
    )
...
def test_non_allowlisted_functions_always_deny(
  ...
  assert result is False, (
    f"Non-allowlisted '{intruder}' was approved when allowed={allowed}"
  )
...
def test_allowlist_is_immutable(allowed: frozenset[str]) -> None:
  ...
  assert isinstance(internal, frozenset), (
    f"Expected frozenset, got {type(internal).__name__}"
  )
...
def test_risk_tier_and_tags_do_not_affect_decision(
  ...
  assert result_a == result_b, (
    f"Decision changed for '{fn_name}': {risk_tier_a}/{tags_a} → {result_a}, {risk_tier_b}/{tags_b} → {result_b}"
  )
...
def test_superset_monotonicity(
  ...
  for fn_name in allowed:
    small_result = provider_small.request_confirmation(
      fn_name, args, risk_tier, action_tags
    )
    large_result = provider_large.request_confirmation(
      fn_name, args, risk_tier, action_tags
    )
    # If small approves, large MUST also approve (monotonicity)
    if small_result:
      assert large_result, (
        f"Monotonicity violated: '{fn_name}' approved by subset but denied by superset"
      )
...
def test_set_and_frozenset_construction_equivalent(
  ...
  tags = frozenset[str]()
  assert from_set.request_confirmation(
    fn_name, args, risk_tier, tags
  ) == from_frozen.request_confirmation(fn_name, args, risk_tier, tags)
