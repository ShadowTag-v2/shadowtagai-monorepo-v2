...
def send(
  ...
  *,
  ...
  if len(payload) > MAX_MESSAGE_SIZE:
    raise ValueError(
      f"Message payload exceeds {MAX_MESSAGE_SIZE} bytes: {len(payload)}"
    )
  ...
  filename = (
    f"{ts.strftime('%Y-%m-%dT%H-%M-%S')}_{from_}_{to}_{envelope.msg_type.value}.json"
  )
...
def preview(msg: DaemonMessage) -> str:
  ...
  if isinstance(variant, Completed):
    ...
    return (
      f"completed {truncate(variant.summary, 40)} | {len(variant.files_changed)} files"
    )
