...
@mcp.tool()
def jules_execute_task(source_name: str, task_description: str, automation_mode: str = "AUTO_CREATE_PR") -> dict[str, Any]:
    """
    ...
    try:
        # Create session
        session = client.create_session(source_name=source_name, automation_mode=automation_mode, task_description=task_description)
        session_name = session.get("name")
        if not session_name:
            return {"error": "Session creation succeeded but returned no session name.", "response": session}
