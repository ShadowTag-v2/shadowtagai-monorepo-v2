...
class PlanModeInterview:
    ...
    def evaluate_uncertainty(self, uncertainty_score: float, topic: str) -> str | None:
        ...
        if uncertainty_score > self.uncertainty_threshold:
            logger.info(f"Uncertainty ({uncertainty_score}) exceeds threshold ({self.uncertainty_threshold}) for topic: {topic}")
            return f"I need some clarification on {topic}. Could you please provide more details?"
        return None
