...
class ASTSecurityPipeline:
    ...
    def _traverse_ast(self, node):
        ...
        if node.type == 'command_name' and node_text in ['history', 'rm ~/.bash_history', 'sudo', 'su', 'chsh']:
            self.block_reason = f"CRITICAL: Unauthorized escalation/manipulation ({node_text})"
            return
