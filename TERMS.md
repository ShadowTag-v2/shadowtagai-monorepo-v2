# TERMS.md — ShadowTag AI Terms of Service

**Last Updated:** April 14, 2026
**Effective Date:** April 14, 2026
**Version:** 1.0.0

---

## 1. Acceptance of Terms

By accessing or using any ShadowTag AI service ("Service"), including **ShadowTag AI** (shadowtagai.com), **KovelAI** (kovelai.com), and any associated APIs, you agree to these Terms of Service ("Terms"). If you do not agree, do not use the Service.

## 2. Service Description

ShadowTag AI provides:
- **Judge 6 Shield**: AI liability mitigation and compliance firewall
- **KovelAI / CounselConduit**: Privileged client AI intake and legal workflow automation
- **UphillSnowball**: Infrastructure platform for AI governance and retrieval

## 3. Eligibility

You must be at least 18 years old and have the legal capacity to enter into contracts. If using the Service on behalf of an organization, you represent that you have authority to bind that organization.

## 4. Account Registration

- You are responsible for maintaining the security of your account credentials
- You must provide accurate and complete registration information
- You must notify us immediately of any unauthorized access
- One person or entity per account unless otherwise agreed in writing

## 5. Subscription and Payment

### 5.1 Pricing Tiers
- **Consumer Syndicate**: $149/month
- **Enterprise Base SLA**: $20,000/month
- **Enterprise EU26 Premium**: $28,333/month
- **Sovereign Scale**: Custom pricing (100% compute pass-through + base license)

### 5.2 Payment Terms
- All fees are in USD and non-refundable unless otherwise stated
- Payments processed via Stripe; you agree to Stripe's terms
- Enterprise customers: Net-30 invoicing available upon agreement
- We reserve the right to change pricing with 30 days notice

### 5.3 Free Tier
- Limited access may be available for evaluation purposes
- Free tier usage is subject to rate limits and quota restrictions
- We may discontinue free tier access at any time

## 6. Acceptable Use

You agree NOT to:
- Use the Service for illegal purposes or to facilitate illegal activity
- Attempt to bypass rate limits, security controls, or access restrictions
- Reverse engineer, decompile, or derive source code from the Service
- Use the Service to develop a competing product
- Submit content that infringes intellectual property rights
- Attempt to access other users' data or accounts
- Use the Service to generate content that violates applicable law

### 6.1 AI-Specific Restrictions
- You may not use AI-generated outputs as legal advice without human attorney review
- Outputs from CounselConduit/KovelAI do not constitute attorney-client privilege unless used through a licensed attorney's account
- You are responsible for reviewing and validating all AI-generated content

## 7. Intellectual Property

### 7.1 Our IP
- The Service, including software, algorithms, documentation, and branding, is owned by ShadowTag AI
- Open-source components are licensed under their respective licenses (see repository)

### 7.2 Your Content
- You retain ownership of content you submit to the Service
- You grant us a limited license to process your content solely to provide the Service
- We do not use your content to train models or share it with third parties

### 7.3 Output Ownership
- AI-generated outputs belong to you, subject to applicable law
- We make no warranty regarding the intellectual property status of generated content

## 8. Data Handling

- See our [Privacy Policy](PRIVACY.md) for detailed data practices
- Enterprise customers: data isolation via dedicated GCP sidecars
- All data encrypted in transit (TLS 1.3) and at rest (GCP managed keys)
- We comply with GDPR, CCPA, and applicable data protection laws

## 9. Service Level Agreement (SLA)

### 9.1 Consumer Tier
- Target availability: 99.5%
- No formal SLA guarantees
- Best-effort support via email

### 9.2 Enterprise Tier
- Target availability: 99.95%
- Target latency: p99 ≤ 90ms application path
- Dedicated support channel
- Incident response: 4-hour acknowledgment

## 10. Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW:

- THE SERVICE IS PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND
- WE ARE NOT LIABLE FOR INDIRECT, INCIDENTAL, SPECIAL, OR CONSEQUENTIAL DAMAGES
- OUR TOTAL LIABILITY SHALL NOT EXCEED THE FEES PAID BY YOU IN THE PRECEDING 12 MONTHS
- WE ARE NOT RESPONSIBLE FOR LEGAL OUTCOMES BASED ON AI-GENERATED CONTENT

## 11. Indemnification

You agree to indemnify and hold harmless ShadowTag AI from claims arising from:
- Your use of the Service
- Your violation of these Terms
- Your violation of any third-party rights
- Content you submit to the Service

## 12. Termination

- You may terminate your account at any time
- We may suspend or terminate access for Terms violations
- Upon termination, your right to use the Service ceases immediately
- Enterprise customers: 30-day wind-down period for data export

## 13. Modifications

We may modify these Terms at any time. Material changes will be communicated via:
- Email notification to registered users
- A notice on our websites
- A commit to this repository (open-source transparency)

Continued use after modification constitutes acceptance.

## 14. Governing Law

These Terms are governed by the laws of the State of California, USA. Disputes shall be resolved in the courts of Los Angeles County, California.

## 15. Miscellaneous

- **Severability**: If any provision is unenforceable, the remainder stays in effect
- **Waiver**: Failure to enforce any right does not constitute a waiver
- **Entire Agreement**: These Terms, together with the Privacy Policy, constitute the entire agreement
- **Assignment**: You may not assign your rights; we may assign ours

## 16. Contact

- **General**: legal@shadowtagai.com
- **Privacy**: privacy@shadowtagai.com
- **Enterprise Sales**: sales@shadowtagai.com
- **GitHub**: [ShadowTag-v2/Monorepo-Uphillsnowball](https://github.com/ShadowTag-v2/Monorepo-Uphillsnowball)

---

**License**: This document is licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).
