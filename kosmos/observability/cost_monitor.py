...
class CostMonitor:
    ...
    def record_usage(
        ...
        logger.debug(f"Recorded usage: {tokens} tokens, ${cost:.4f} ({model}) [daily: ${self.daily_burn:.2f}, monthly: ${self.monthly_burn:.2f}]")
    ...
    def get_budget_status(self) -> dict[str, any]:
        ...
        return {
            "daily": {
                ...
                "utilization_pct": (self.daily_burn / self.daily_budget * 100) if self.daily_budget > 0 else 0,
            },
            "monthly": {
                "budget": self.monthly_budget,
                "burned": self.monthly_burn,
                "remaining": self.monthly_budget - self.monthly_burn,
                "utilization_pct": (self.monthly_burn / self.monthly_budget * 100) if self.monthly_budget > 0 else 0,
            },
            "sessions": {
    ...
    def _reset_if_needed(self):
        ...
        if today > self.last_reset_date:
            logger.info(f"Daily budget reset: burned ${self.daily_burn:.2f} on {self.last_reset_date}")
            self.daily_burn = 0.0
            self.last_reset_date = today

        # Reset monthly if new month
        if current_month != self.last_reset_month:
            logger.info(f"Monthly budget reset: burned ${self.monthly_burn:.2f} in month {self.last_reset_month}")
