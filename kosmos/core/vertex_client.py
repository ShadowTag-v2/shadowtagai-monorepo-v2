...
class GeminiModel(Enum):
  ...
  FLASH = (
    "gemini-2.0-flash-exp"  # Fast, cheap (was gemini-2.5-flash, using latest available)
  )
...
class VertexAIClient:
    ...
    **kwargs,
    ...
    if self.cost_tracker:
      estimated_cost = self._estimate_cost(
        estimated_input_tokens, max_output_tokens, model
      )
    ...
    try:
      ...
      logger.info(
        f"Generated {output_tokens} tokens with {model.value} (cost: ${cost:.4f}, input: {input_tokens} tokens)"
      )
  ...
  def _estimate_cost(
    self, input_tokens: int, output_tokens: int, model: GeminiModel
  ) -> float:
    """
