...
class GeminiModel(Enum):
    ...
    FLASH = "gemini-2.0-flash-exp"  # Fast, cheap (was gemini-2.5-flash, using latest available)
...
class VertexAIClient:
    ...
    def generate(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(f"Generated {output_tokens} tokens with {model.value} (cost: ${cost:.4f}, input: {input_tokens} tokens)")
            ...
            return GenerationResult(
                ...
                cost=cost,
                finish_reason=getattr(response.candidates[0], "finish_reason", None) if hasattr(response, "candidates") else None,
            )
    ...
    def generate_with_auto_routing(
        ...
        **kwargs,
        ...
        if context_length < 5000 and complexity == "low":
            ...
            logger.debug(f"Auto-routing: Selected Pro (context={context_length}, complexity={complexity})")
    ...
    def __repr__(self) -> str:
        return f"VertexAIClient(project={self.project_id}, location={self.location}, default_model={self.default_model.value})"
