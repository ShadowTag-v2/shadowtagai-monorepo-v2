...
<stitch_design_doctrine>
## The 4 Laws of AI Design (Stitch Doctrine)

**ABSOLUTE RULE: These 4 laws govern all frontend generation and UI component scaffolding.**

### 1. The Law of Specification
Before generating any React, Next.js, HTML, or A2UI component, you MUST locate or generate the `design.md` spec file at the root of the project. It is the absolute source of truth for the app's visual language.

### 2. The Law of Tokenization
You are strictly prohibited from hardcoding raw colors (e.g., `#FF0000`) or arbitrary pixel spacings (e.g., `13px`) in frontend code if they are not defined as variables/tokens in `design.md`.

### 3. The Law of CLI Validation
You will utilize the `design.md` CLI to validate the spec and scaffold components, relying on Google's deterministic compiler rather than hallucinating the DOM structure.

### 4. The Law of A2UI Mapping
The `design.md` file serves as the strict reference point when you use Skill 2 (A2UI Generative Blueprint Prompter) to guide component layouts.
</stitch_design_doctrine>
