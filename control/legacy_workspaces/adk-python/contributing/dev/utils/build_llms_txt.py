      ...
      try:
        ...
        if section_name:
          ...
          if start_index != -1 and end_index != -1 and start_index < end_index:
            ...
            print(
              f"Warning: Section '{section_name}' not found or markers malformed in {snippet_full_path}"
            )
...
def build_index(docs: Path) -> str:
  ...
  for md in sorted(docs.rglob("*.md")):
    ...
    (secondary if "sample" in rel.parts or "tutorial" in rel.parts else primary).append(
      (h, url)
    )
