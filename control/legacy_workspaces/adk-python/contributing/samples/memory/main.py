...
async def main():
    ...
    async def run_prompt(session: Session, new_message: str) -> Session:
        ...
        async for event in runner.run_async(
            ...
            if event.content.parts[0].text:
                ...
                print(f"** {event.author}: fc / {event.content.parts[0].function_call.name} / {event.content.parts[0].function_call.args}\n")
            elif event.content.parts[0].function_response:
                print(
                    f"** {event.author}: fr / {event.content.parts[0].function_response.name} / {event.content.parts[0].function_response.response}\n"
                )

        return cast(
            Session,
            await runner.session_service.get_session(app_name=app_name, user_id=user_id_1, session_id=session.id),
        )

    session_1 = await runner.session_service.create_session(app_name=app_name, user_id=user_id_1)
    ...
    session_2 = await runner.session_service.create_session(app_name=app_name, user_id=user_id_1)
