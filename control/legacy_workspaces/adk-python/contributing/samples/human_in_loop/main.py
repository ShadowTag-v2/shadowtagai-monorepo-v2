...
async def main():
  ...
  async def call_agent(query: str):
      ...
      if event.content and event.content.parts:
        for i, part in enumerate(event.content.parts):
          ...
          if part.function_call:
            print(
              f"    Part {i} [FunctionCall]: {part.function_call.name}({part.function_call.args}) ID: {part.function_call.id}"
            )
            if not long_running_function_call and part.function_call.id in (
              ...
              print(
                f"      (Captured as long_running_function_call for '{part.function_call.name}')"
              )
          if part.function_response:
            ...
            if (
              ...
              print(
                f"      (Captured as initial_tool_response for '{part.function_response.name}', Ticket ID: {ticket_id})"
              )
    ...
    if (
      ...
      print(
        f"--- Sending updated tool result to agent for call ID {long_running_function_call.id}: {updated_tool_output_data} ---"
      )
      ...
      print(
        f"--- Long running function '{long_running_function_call.name}' was called, but its initial response was not captured. ---"
      )
