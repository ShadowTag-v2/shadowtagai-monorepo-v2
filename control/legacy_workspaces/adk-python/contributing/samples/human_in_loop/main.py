...
async def main():
    ...
    async def call_agent(query: str):
        ...
        initial_tool_response: types.FunctionResponse | None = None
            ...
            if event.content and event.content.parts:
                for i, part in enumerate(event.content.parts):
                    ...
                    if part.function_call:
                        print(f"    Part {i} [FunctionCall]: {part.function_call.name}({part.function_call.args}) ID: {part.function_call.id}")
                        if not long_running_function_call and part.function_call.id in (event.long_running_tool_ids or []):
                            long_running_function_call = part.function_call
                            print(f"      (Captured as long_running_function_call for '{part.function_call.name}')")
                    if part.function_response:
                        ...
                        if long_running_function_call and part.function_response.id == long_running_function_call.id:
                            ...
                            print(f"      (Captured as initial_tool_response for '{part.function_response.name}', Ticket ID: {ticket_id})")

        print("--- End of agent's initial turn ---\n")

        if long_running_function_call and initial_tool_response and initial_tool_response.response.get("status") == "pending":
            print(f"--- Simulating external approval for ticket: {ticket_id} ---\n")

            updated_tool_output_data = {
                "status": "approved",
                "ticketId": ticket_id,
                "approver_feedback": "Approved by manager at " + str(asyncio.get_event_loop().time()),
            }
            ...
            print(f"--- Sending updated tool result to agent for call ID {long_running_function_call.id}: {updated_tool_output_data} ---")
            print("--- Running agent's turn AFTER receiving updated tool result ---")

            async for event in runner.run_async(
                session_id=session.id,
                user_id=USER_ID,
                new_message=types.Content(parts=[updated_function_response_part], role="user"),
            ):
                if event.content and event.content.parts:
                    for i, part in enumerate(event.content.parts):
                        if part.text:
            ...
            print(f"--- Long running function '{long_running_function_call.name}' was called, but its initial response was not captured. ---")
        elif not long_running_function_call:
            print("--- No long running function call was detected in the initial turn. ---")
