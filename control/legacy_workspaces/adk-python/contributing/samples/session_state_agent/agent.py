...
async def assert_session_values(
  ...
  title: str,
  *,
  keys_in_ctx_session: list[str] | None = None,
  keys_in_service_session: list[str] | None = None,
  keys_not_in_service_session: list[str] | None = None,
):
  ...
  print(
    f"** Asserting keys are already persisted in session: {keys_in_service_session}",
    end=" ",
  )
  ...
  print(
    f"** Asserting keys are not persisted in session yet: {keys_not_in_service_session}",
    end=" ",
  )
...
root_agent = Agent(
  ...
  instruction=(
    "Log all users query with `log_query` tool. Must always remind user you cannot answer second query because your setup."
  ),
