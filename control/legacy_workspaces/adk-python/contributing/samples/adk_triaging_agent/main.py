...
async def fetch_specific_issue_details(issue_number: int):
  ...
  try:
    ...
    if has_planned and not has_component:
      ...
      print(
        f"Issue #{issue_number} is already triaged or doesn't have 'planned' label. Skipping."
      )
      return None
...
async def main():
  ...
  if EVENT_NAME == "issues" and ISSUE_NUMBER:
    ...
    if specific_issue is None:
      print(
        f"No issue details found for #{issue_number} that needs triaging, or an error occurred. Skipping agent interaction."
      )
      return
...
if __name__ == "__main__":
  ...
  print(
    f"Start triaging {OWNER}/{REPO} issues at {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(start_time))}"
  )
  print("-" * 80)
  asyncio.run(main())
  print("-" * 80)
