...
def get_changed_files_between_releases(
  ...
  url = (
    f"{GITHUB_BASE_URL}/repos/{repo_owner}/{repo_name}/compare/{start_tag}...{end_tag}"
  )
...
def search_local_git_repo(
  ...
  """
  print(
    f"Attempting to search for pattern: {pattern} in directory: {directory_path}, with extensions: {extensions}"
  )
  try:
