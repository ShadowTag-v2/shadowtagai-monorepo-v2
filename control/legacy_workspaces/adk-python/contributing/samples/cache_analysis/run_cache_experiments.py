...
async def run_cache_comparison_experiment(
  ...
  print(
    f"      Cache Hit Ratio: {cached_cache_hit_ratio:.1f}% ({cached_total_cached_tokens:,} / {cached_total_prompt_tokens:,} tokens)"
  )
  print(
    f"      Cache Utilization: {cached_cache_utilization_ratio:.1f}% ({cached_requests_with_hits}/{total_cached_requests} requests)"
  )
  print(f"      Avg Cached Tokens/Request: {cached_avg_cached_tokens_per_request:.0f}")
  print(f"   ❄️  {uncached_label}:")
  print(
    f"      Cache Hit Ratio: {uncached_cache_hit_ratio:.1f}% ({uncached_total_cached_tokens:,} / {uncached_total_prompt_tokens:,} tokens)"
  )
  print(
    f"      Cache Utilization: {uncached_cache_utilization_ratio:.1f}% ({uncached_requests_with_hits}/{total_uncached_requests} requests)"
  )
  print(
    f"      Avg Cached Tokens/Request: {uncached_avg_cached_tokens_per_request:.0f}"
  )
  print()
...
async def analyze_cache_performance_from_sessions(
  ...
  try:
    ...
    if cached_analysis["status"] == "active":
      ...
      print(
        f"     Avg Cached Tokens/Request: {cached_analysis['avg_cached_tokens_per_request']:.0f}"
      )
  ...
  try:
    ...
    if uncached_analysis["status"] == "active":
      ...
      print(
        f"     Avg Cached Tokens/Request: {uncached_analysis['avg_cached_tokens_per_request']:.0f}"
      )
      print(f"     Requests with cache: {uncached_analysis['requests_with_cache']}")
      print(
        f"     Avg invocations used: {uncached_analysis['avg_invocations_used']:.1f}"
      )
...
def calculate_averaged_results(
  ...
  print(
    f"   Average Cache Hit Ratio: {avg_cache['cache_hit_ratio_percent']:.1f}% (±{stats['cache_hit_ratio_std']:.1f}%)"
  )
  print(
    f"   Average Cache Utilization: {avg_cache['cache_utilization_ratio_percent']:.1f}% (±{stats['cache_utilization_std']:.1f}%)"
  )
  print(
    f"   Average Cached Tokens/Request: {avg_cache['avg_cached_tokens_per_request']:.0f} (±{stats['cached_tokens_per_request_std']:.0f})"
  )
  print()
...
async def main():
  ...
  parser.add_argument(
    ...
    default=1,
    help=(
      "Number of times to repeat each experiment for averaged results (default: 1)"
    ),
  )
  parser.add_argument(
    "--cached-first",
  ...
  if args.repeat == 1:
    ...
    print(
      f"    Cache Utilization: {cached_exp['cache_utilization_ratio_percent']:.1f}%"
    )
    print(
      f"    Cached Tokens/Request: {cached_exp['avg_cached_tokens_per_request']:.0f}"
    )
    print(f"  ❄️  {labels['uncached_label']}:")
    print(f"    Cache Hit Ratio: {uncached_exp['cache_hit_ratio_percent']:.1f}%")
    print(
      f"    Cache Utilization: {uncached_exp['cache_utilization_ratio_percent']:.1f}%"
    )
    print(
      f"    Cached Tokens/Request: {uncached_exp['avg_cached_tokens_per_request']:.0f}"
    )
  else:
    ...
    print(
      f"    Cache Hit Ratio: {cached_exp['cache_hit_ratio_percent']:.1f}% vs {uncached_exp['cache_hit_ratio_percent']:.1f}%"
    )
    print(
