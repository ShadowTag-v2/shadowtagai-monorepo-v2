...
async def call_agent_async(runner: InMemoryRunner, user_id: str, session_id: str, prompt: str) -> dict[str, Any]:
    ...
    async for event in runner.run_async(
        ...
        if event.usage_metadata:
            if hasattr(event.usage_metadata, "prompt_token_count") and event.usage_metadata.prompt_token_count:
                token_usage["prompt_token_count"] += event.usage_metadata.prompt_token_count
            if hasattr(event.usage_metadata, "candidates_token_count") and event.usage_metadata.candidates_token_count:
                token_usage["candidates_token_count"] += event.usage_metadata.candidates_token_count
            if hasattr(event.usage_metadata, "cached_content_token_count") and event.usage_metadata.cached_content_token_count:
                token_usage["cached_content_token_count"] += event.usage_metadata.cached_content_token_count
            if hasattr(event.usage_metadata, "total_token_count") and event.usage_metadata.total_token_count:
...
def get_test_prompts() -> list[str]:
    ...
    return [
        ...
        ("Run market_research_analysis for industry='fintech', focus_areas=['user_experience', 'security'], report_depth='comprehensive'."),
        ("Execute competitive_analysis with competitors=['Netflix', 'Disney+'], analysis_type='feature_comparison', output_format='detailed'."),
        ("Perform content_performance_evaluation on content_type='video', platform='social_media', success_metrics=['views', 'engagement']."),
    ]
...
async def run_experiment_batch(
    ...
    for i, prompt in enumerate(prompts, 1):
        ...
        try:
            ...
            cached_tokens = agent_response["token_usage"].get("cached_content_token_count", 0)

            print(f"✅ Completed (Response: {len(agent_response['response_text'])} chars)")
            print(f"   📊 Tokens - Prompt: {prompt_tokens:,}, Cached: {cached_tokens:,}")
    ...
    total_prompt_tokens = sum(r.get("token_usage", {}).get("prompt_token_count", 0) for r in results)
    total_cached_tokens = sum(r.get("token_usage", {}).get("cached_content_token_count", 0) for r in results)
    ...
    requests_with_cache_hits = sum(1 for r in results if r.get("token_usage", {}).get("cached_content_token_count", 0) > 0)
    cache_utilization_ratio = (requests_with_cache_hits / len(prompts)) * 100 if prompts else 0.0

    # Average cached tokens per request
    avg_cached_tokens_per_request = total_cached_tokens / len(prompts) if prompts else 0.0
    ...
    print(f"      Cache Hit Ratio: {cache_hit_ratio:.1f}% ({total_cached_tokens:,} / {total_prompt_tokens:,} tokens)")
    print(f"      Cache Utilization: {cache_utilization_ratio:.1f}% ({requests_with_cache_hits}/{len(prompts)} requests)")
