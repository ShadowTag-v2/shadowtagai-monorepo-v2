...
async def run_experiment_batch(
  ...
  print(
    f"      Cache Hit Ratio: {cache_hit_ratio:.1f}% ({total_cached_tokens:,} / {total_prompt_tokens:,} tokens)"
  )
  print(
    f"      Cache Utilization: {cache_utilization_ratio:.1f}% ({requests_with_cache_hits}/{len(prompts)} requests)"
  )
