  ...
  try:
    ...
    if (
      ...
      if last_bot_alert_time and last_bot_alert_time > state["last_activity_time"]:
        logger.info(
          f"#{item_number}: Silent edit detected, but Bot already alerted. No spam."
        )
      else:
    ...
    logger.debug(
      f"#{item_number} VERDICT: Role={state['last_action_role']}, Idle={days_since_activity:.2f}d"
    )
...
def remove_label_from_issue(item_number: int, label_name: str) -> dict[str, Any]:
  ...
  url = (
    f"{GITHUB_BASE_URL}/repos/{OWNER}/{REPO}/issues/{item_number}/labels/{label_name}"
  )
  try:
    delete_request(url)
    return {"status": "success"}
