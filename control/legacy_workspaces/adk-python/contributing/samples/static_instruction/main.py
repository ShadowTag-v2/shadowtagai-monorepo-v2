...
async def main():
    ...
    print(f"Static Personality Configured: {agent.root_agent.static_instruction is not None}")
    print(f"Dynamic Mood System Configured: {agent.root_agent.instruction is not None}")
    print()
...
if __name__ == "__main__":
    ...
    print(f"🐕 Starting Bingo Digital Pet Session at {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(start_time))}")
    print("-" * 80)
    ...
    print("-" * 80)
    end_time = time.time()
    print(f"🐕 Pet session ended at {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(end_time))}")
