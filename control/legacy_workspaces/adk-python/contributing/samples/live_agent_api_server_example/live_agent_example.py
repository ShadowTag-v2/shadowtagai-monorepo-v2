...
def init_pyaudio_playback():
  ...
  try:
    ...
    logging.info(
      f"Initializing PyAudio output stream: Rate={PYAUDIO_PLAY_RATE}, Channels={PYAUDIO_PLAY_CHANNELS}, Format=paInt16"
    )
...
class AgentResponseAudioPlayer:
  ...
  def _play_audio_pyaudio_handler(self, audio_bytes: bytes, mime_type_full: str):
    if not AUDIO_PLAYBACK_ENABLED or not pya_output_stream_instance:
      logging.warning(
        "PyAudio stream not available or playback disabled. Cannot play audio."
      )
      return
    try:
      logging.debug(
      ...
      if mime_type_base == "audio/pcm":
        ...
        if match:
          try:
            ...
            logging.warning(
              f"Could not parse rate from '{mime_type_full}', using stream default {PYAUDIO_PLAY_RATE}Hz."
            )
        ...
        logging.info(
          f"Preparing raw PCM for PyAudio stream (target rate {PYAUDIO_PLAY_RATE}Hz)."
        )
      else:
      ...
      if playable_data_bytes:
        ...
        logging.info(
          f"Audio chunk written to PyAudio stream (Size: {len(playable_data_bytes)} bytes)."
        )
      else:
    ...
    async with httpx.AsyncClient() as client:
      ...
      if response_get.status_code == 200:
        ...
        if response_post.status_code == 200:
          ...
          logging.error(
            f"Failed to create session '{session_id}'. POST Status: {response_post.status_code}"
          )
          return False
      else:
        logging.warning(
          f"Could not verify session '{session_id}'. GET Status: {response_get.status_code}"
        )
        return False
  except Exception as e:
  ...
  try:
    async with websockets.connect(uri, open_timeout=10, close_timeout=10) as websocket:
        ...
        try:
          async for message in websocket:
            ...
            try:
              ...
              logging.info(
                f"<<< Parsed event from server: (Keys: {list(event_data.keys())})"
              )
                  ...
                  for part in content_obj["parts"]:
                    if isinstance(part, dict) and "inlineData" in part:
                      ...
                      if (
                        ...
                        logging.info(
                          f"Audio part found: Mime='{mime_type_full}', Base64Len={len(audio_b64)}"
                        )
                        try:
                          standard_b64_string = audio_b64.replace("-", "+").replace(
                            "_", "/"
                          )
                          ...
                          if audio_bytes_decoded:
                            ...
                            logging.warning(
                              "Decoded audio data is empty after sanitization and padding."
                            )

                        except base64.binascii.Error as b64e:
                          # Log details if decoding still fails
                          logging.error(
                            f"Base64 decode error after sanitization and padding. Error: {b64e}"
                          )
                        except Exception as e:
                          logging.error(
          ...
          logging.warning(
            f"Receiver: Connection closed (Code: {e.code}, Reason: '{e.reason if e.reason else 'N/A'}')"
          )
        except Exception:
      ...
      async def send_messages_local(ws: websockets.WebSocketClientProtocol):
        ...
        logging.info(
          "Sender task started: Type 'start_stream', 'stop_stream', text,sendfile, or 'quit'."
        )
        while True:
          await asyncio.sleep(10**-12)
          try:
            ...
            if user_input.lower() == "quit":
              ...
              if audio_stream_task and not audio_stream_task.done():
                ...
                logging.warning(
                  "Sender: Audio stream is not currently running or already stopped."
                )
            ...
            # If you need it, uncomment the block from my previous response.
              ...
              if (
                ...
                logging.warning(
                  "Please stop the current audio stream with 'stop_stream' before sending a file."
                )
                continue
              ...
              if not filepath.lower().endswith(".wav"):
                logging.warning(
                  f"File {filepath} does not end with .wav. Attempting to send anyway."
                )
                print(
                  f"Warning: File '{filepath}' is not a .wav file. Ensure it's a compatible WAV."
                )
            ...
            logging.warning(
              f"Sender: WebSocket connection closed. Code: {e.code}, Reason: {e.reason}"
            )
            if audio_stream_task and not audio_stream_task.done():
      ...
      if current_active_audio_task and not current_active_audio_task.done():
        ...
        try:
          ...
          logging.info(
            "Audio streaming task gracefully stopped after main task completion."
          )
        except TimeoutError:
          logging.warning(
            "Timeout waiting for audio stream to stop post main task. Cancelling."
          )
