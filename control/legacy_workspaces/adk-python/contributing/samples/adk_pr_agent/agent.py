...
def get_github_pr_info_http(pr_number: int) -> str | None:
  ...
  try:
    ...
    print(
      f"HTTP Error fetching PR details: {e.response.status_code} -  {e.response.text}"
    )
    return None
  ...
  if commits_url:
    ...
    while True:
      ...
      try:
        ...
        print(
          f"HTTP Error fetching PR commits (page {page}): {e.response.status_code} - {e.response.text}"
        )
        break
      except requests.exceptions.RequestException as e:
