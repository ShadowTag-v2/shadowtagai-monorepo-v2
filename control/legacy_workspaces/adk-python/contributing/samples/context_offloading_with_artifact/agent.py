...
class CustomLoadArtifactsTool(LoadArtifactsTool):
    ...
    async def _append_artifacts_to_llm_request(self, *, tool_context: ToolContext, llm_request: LlmRequest):
        ...
        artifacts_with_summaries = [
            f"{name}: {summaries.get(name)}" if name in summaries and summaries.get(name) else name for name in artifact_names
        ]
...
Regional Breakdown:
""" + "".join([f"Sub-region {i + 1} performance metric: {random.random() * 100:.2f}\n" for i in range(500)])
    ...
    return {
        "message": (f"Sales data for {region} for Q3 2025 is saved as artifact '{artifact_name}'."),
