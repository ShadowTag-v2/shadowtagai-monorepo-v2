...
def get_voting_options(
  tool_context: ToolContext, round_id: str | None = None
) -> dict[str, Any]:
...
def store_vote_to_bigquery(
  ...
  """
  print(
    f"Tool called: store_vote_to_bigquery - vote={vote_choice}, user={user_id}, round={round_id or CURRENT_ROUND}"
  )
  ...
  if LOCAL_MODE:
    ...
    return {
      "success": True,
      "message": (
        f"✅ Vote recorded for Option {vote}: {active_options[vote]['title']}!"
      ),
