...
async def append_logprobs_to_response(callback_context: CallbackContext, llm_response: LlmResponse) -> LlmResponse:
    ...
    if llm_response.avg_logprobs is None:
        ...
        confidence_level = "High" if llm_response.avg_logprobs >= -0.5 else "Medium" if llm_response.avg_logprobs >= -1.0 else "Low"
...
📈 Confidence Score: {100 * (2**llm_response.avg_logprobs):.1f}%"""
        ...
        if llm_response.logprobs_result and llm_response.logprobs_result.top_candidates:
            logprobs_info += f"\n🔍 Top alternatives analyzed: {len(llm_response.logprobs_result.top_candidates)}"
...
root_agent = Agent(
    ...
    description=("A simple agent that demonstrates log probability extraction and display."),
    instruction="""
