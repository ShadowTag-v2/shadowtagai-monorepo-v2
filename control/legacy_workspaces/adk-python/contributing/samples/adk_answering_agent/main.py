...
async def list_most_recent_discussions(
  ...
  """
  print(
    f"Attempting to fetch the {count} most recently updated discussions from {OWNER}/{REPO}..."
  )
...
if __name__ == "__main__":
  ...
  print(
    f"Start Q&A checking on {OWNER}/{REPO} at {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(start_time))}"
  )
  print("-" * 80)
  asyncio.run(main())
  print("-" * 80)
