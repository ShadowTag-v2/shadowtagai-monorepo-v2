...
async def call_agent_async(runner: InMemoryRunner, user_id: str, session_id: str, prompt: str) -> list[Event]:
            ...
            if event.content and event.author and event.author != "user":
                for part in event.content.parts:
                    if part.text:
                        ...
                        print(f"    📦 Tool Response: {highlight(part.function_response.response)}")
...
async def main():
    ...
    await call_agent_async(runner, USER_ID, session.id, "save artifact file1 with content version1")

    # 2. Check current state and artifact
    print("\n\n===== STATE BEFORE UPDATE =====")
    await call_agent_async(runner, USER_ID, session.id, "what is the value of state `color`?")
    ...
    events_update_state = await call_agent_async(runner, USER_ID, session.id, "update state key color to blue")
    rewind_invocation_id = events_update_state[0].invocation_id
    print(f"Will rewind before invocation: {rewind_invocation_id}")

    await call_agent_async(runner, USER_ID, session.id, "save artifact file1 with content version2")

    # 4. Check state and artifact after update
    print("\n\n===== STATE AFTER UPDATE =====")
    await call_agent_async(runner, USER_ID, session.id, "what is the value of state key color?")
    await call_agent_async(runner, USER_ID, session.id, "load artifact file1")
    ...
    print("🔧 If rewind was successful, color should be 'red' and file1 content should contain 'version1' in the final check.")
