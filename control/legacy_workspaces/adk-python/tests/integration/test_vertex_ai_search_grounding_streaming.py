...
class TestIntegrationVertexAiSearchGrounding:
  ...
  async def test_grounding_metadata_without_streaming(
    ...
    for i, e in enumerate(events):
      print(
        f"  Event {i}: author={e['author']}, partial={e['partial']}, grounding={e['has_grounding']}, content={e['has_content']}"
      )
    ...
    async with Aclosing(
      runner.run_async(
        ...
        new_message=types.Content(
          role="user",
          parts=[
            types.Part.from_text(text="What is ADK and when was it first released?")
          ],
        ),
        run_config=run_config,
      )
