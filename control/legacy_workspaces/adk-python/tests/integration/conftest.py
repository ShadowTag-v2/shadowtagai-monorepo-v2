...
def load_env_for_tests():
  ...
  if "GOOGLE_API_KEY" not in os.environ:
    warnings.warn(
      "Missing GOOGLE_API_KEY in the environment variables. GOOGLE_AI backend integration tests will fail."
    )
  for env_var in [
    ...
    if env_var not in os.environ:
      warnings.warn(
        f"Missing {env_var} in the environment variables. Vertex backend integration tests will fail."
      )
  ...
  if llm_backend.__name__ in metafunc.fixturenames:
    if not _is_explicitly_marked(llm_backend.__name__, metafunc):
      ...
      if test_backend == "GOOGLE_AI_ONLY":
        ...
        raise ValueError(
          f"Invalid TEST_BACKEND value: {test_backend}, should be one of [GOOGLE_AI_ONLY, VERTEX_ONLY, BOTH]"
        )
