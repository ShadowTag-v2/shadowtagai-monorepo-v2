...
MOCK_SESSION = Session(
    ...
    events=[
        ...
        Event(
            ...
            author="agent",
            timestamp=23456,
            content=types.Content(parts=[types.Part(function_call=types.FunctionCall(name="test_function"))]),
        ),
    ],
)
...
async def test_search_memory(mock_vertexai_client):
    ...
    retrieved_memory.memory.update_time = datetime(2024, 12, 12, 12, 12, 12, 123456)

    mock_vertexai_client.agent_engines.memories.retrieve.return_value = [retrieved_memory]
    memory_service = mock_vertex_ai_memory_bank_service()

    result = await memory_service.search_memory(app_name=MOCK_APP_NAME, user_id=MOCK_USER_ID, query="query")
