...
MOCK_SESSION_1 = Session(
    ...
    events=[
        ...
        Event(
            ...
            author="model",
            timestamp=12347,
            content=types.Content(parts=[types.Part(text="I agree. The Agent Development Kit (ADK) rocks!")]),
        ),
    ],
)
...
async def test_search_memory_simple_match():
    ...
    result = await memory_service.search_memory(app_name=MOCK_APP_NAME, user_id=MOCK_USER_ID, query="Python")
...
async def test_search_memory_case_insensitive_match():
    ...
    result = await memory_service.search_memory(app_name=MOCK_APP_NAME, user_id=MOCK_USER_ID, query="development")

    assert len(result.memories) == 1
    assert result.memories[0].content.parts[0].text == "I agree. The Agent Development Kit (ADK) rocks!"
...
async def test_search_memory_multiple_matches():
    ...
    result = await memory_service.search_memory(app_name=MOCK_APP_NAME, user_id=MOCK_USER_ID, query="How about ADK?")
...
async def test_search_memory_no_match():
    ...
    result = await memory_service.search_memory(app_name=MOCK_APP_NAME, user_id=MOCK_USER_ID, query="nonexistent")
...
async def test_search_memory_is_scoped_by_user():
    ...
    result = await memory_service.search_memory(app_name=MOCK_APP_NAME, user_id=MOCK_USER_ID, query="secret")

    # No results should be returned for MOCK_USER_ID
    assert not result.memories

    # The result should be found when searching as the correct user
    result_other_user = await memory_service.search_memory(app_name=MOCK_APP_NAME, user_id=MOCK_OTHER_USER_ID, query="secret")
