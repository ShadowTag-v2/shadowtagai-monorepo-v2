...
class TestAuthLlmRequestProcessor:
    ...
    @pytest.fixture
    def mock_user_event_with_auth_response(self, mock_function_response_with_auth):
        """Create a mock user event with auth response."""
        event = Mock(spec=Event)
        event.author = "user"
        event.content = Mock()  # Non-None content
        event.get_function_responses.return_value = [mock_function_response_with_auth]
        return event

    @pytest.fixture
    def mock_user_event_without_auth_response(self, mock_function_response_without_auth):
        """Create a mock user event without auth response."""
        event = Mock(spec=Event)
        event.author = "user"
        event.content = Mock()  # Non-None content
        event.get_function_responses.return_value = [mock_function_response_without_auth]
        return event
    ...
    @pytest.mark.asyncio
    async def test_non_llm_agent_returns_early(self, processor, mock_llm_request, mock_session):
    ...
    @pytest.mark.asyncio
    async def test_empty_events_returns_early(self, processor, mock_invocation_context, mock_llm_request):
        """Test that empty events list returns early."""
        mock_invocation_context.session.events = []

        result = []
        async for event in processor.run_async(mock_invocation_context, mock_llm_request):
            result.append(event)
    ...
    async def test_last_event_no_auth_responses_returns_early(
        ...
        mock_invocation_context.session.events = [mock_user_event_without_auth_response]
    ...
    async def test_processes_auth_response_successfully(
        ...
        mock_invocation_context.session.events = [mock_user_event_with_auth_response]
        ...
        mock_auth_handler_class.assert_called_once_with(auth_config=mock_auth_config)

        # Verify parse_and_store_auth_response was called
        mock_auth_handler.parse_and_store_auth_response.assert_called_once_with(state=mock_invocation_context.session.state)
    ...
    @pytest.mark.asyncio
    async def test_isinstance_check_for_llm_agent(self, processor, mock_llm_request, mock_session):
