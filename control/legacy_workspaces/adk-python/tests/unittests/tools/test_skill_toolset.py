...
@pytest.mark.asyncio
async def test_list_skills_tool(mock_skill1, mock_skill2, tool_context_instance):
    toolset = skill_toolset.SkillToolset([mock_skill1, mock_skill2])
...
async def test_load_skill_run_async(mock_skill1, tool_context_instance, args, expected_result):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {
                "error": ("Resource 'references/other.md' not found in skill 'skill1'."),
...
async def test_load_resource_run_async(mock_skill1, tool_context_instance, args, expected_result):
    toolset = skill_toolset.SkillToolset([mock_skill1])
...
@pytest.mark.asyncio
async def test_process_llm_request(mock_skill1, mock_skill2, tool_context_instance):
    toolset = skill_toolset.SkillToolset([mock_skill1, mock_skill2])
    llm_req = mock.create_autospec(llm_request_model.LlmRequest, instance=True)

    await toolset.process_llm_request(tool_context=tool_context_instance, llm_request=llm_req)
...
def test_default_skill_system_instruction_warning():
    with pytest.warns(UserWarning, match="DEFAULT_SKILL_SYSTEM_INSTRUCTION is experimental"):
...
def _make_mock_executor(stdout="", stderr=""):
    ...
    executor.execute_code.return_value = CodeExecutionResult(stdout=stdout, stderr=stderr)
    return executor
...
async def test_execute_script_missing_params(mock_skill1, args, expected_error_code):
...
async def test_execute_script_python_success(mock_skill1):
    ...
    assert "runpy.run_path('scripts/run.py', run_name='__main__')" in code_input.code
...
async def test_execute_script_with_input_args_python(mock_skill1):
    ...
    call_args = executor.execute_code.call_args
    code_input = call_args[0][1]
    assert "['scripts/run.py', '--verbose', 'True', '--count', '3']" in code_input.code
...
async def test_execute_script_toolset_executor_priority(mock_skill1):
    ...
    toolset = skill_toolset.SkillToolset([mock_skill1], code_executor=toolset_executor)
    tool = skill_toolset.RunSkillScriptTool(toolset)
    agent = mock.MagicMock()
...
async def test_execute_script_shell_includes_timeout(mock_skill1):
    ...
    toolset = skill_toolset.SkillToolset([mock_skill1], code_executor=executor, script_timeout=60)
    tool = skill_toolset.RunSkillScriptTool(toolset)
    ctx = _make_tool_context_with_agent()
    result = await tool.run_async(
        args={"skill_name": "skill1", "script_path": "setup.sh"},
...
def test_system_instruction_references_run_skill_script():
    ...
    assert "execute_skill_script" not in skill_toolset.DEFAULT_SKILL_SYSTEM_INSTRUCTION
...
async def test_execute_script_empty_files_mounted():
    ...
    skill.resources.get_reference.side_effect = lambda n: "" if n == "empty.md" else None
    skill.resources.get_asset.side_effect = lambda n: "" if n == "empty.cfg" else None
    skill.resources.get_script.side_effect = lambda n: models.Script(src="") if n == "run.py" else None
...
async def test_execute_script_binary_content_packaged():
    ...
    skill.resources.get_reference.side_effect = lambda n: b"\x00\x01\x02" if n == "data.bin" else None
    skill.resources.get_asset.return_value = None
    skill.resources.get_script.side_effect = lambda n: models.Script(src="print('ok')") if n == "run.py" else None
