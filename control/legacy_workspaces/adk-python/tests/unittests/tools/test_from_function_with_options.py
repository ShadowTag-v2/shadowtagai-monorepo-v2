...
def test_from_function_with_options_no_return_annotation_gemini():
    ...
    declaration = _automatic_function_calling_util.from_function_with_options(test_function, GoogleLLMVariant.GEMINI_API)
...
def test_from_function_with_options_no_params():
    ...
    assert declaration.parameters is None or len(declaration.parameters.properties) == 0
...
def test_from_function_with_collections_type_parameter():
    ...
    assert declaration.parameters.properties["artifact_key"].type == types.Type.STRING
    assert declaration.parameters.properties["input_edit_ids"].type == types.Type.ARRAY
    assert declaration.parameters.properties["input_edit_ids"].items.type == types.Type.STRING
    assert declaration.response.type == types.Type.STRING
