...
class TestComputerUseTool:
    ...
    def test_init_with_invalid_screen_size(self, mock_computer_function):
        ...
        with pytest.raises(ValueError, match="screen_size dimensions must be positive"):
    ...
    def test_init_with_invalid_virtual_screen_size(self, mock_computer_function):
        ...
        with pytest.raises(ValueError, match="virtual_screen_size dimensions must be positive"):
            ComputerUseTool(
                func=mock_computer_function,
                screen_size=(1920, 1080),
    ...
    def test_normalize_x(self, mock_computer_function):
        ...
        tool = ComputerUseTool(func=mock_computer_function, screen_size=(1920, 1080))
    ...
    def test_normalize_with_custom_virtual_screen_size(self, mock_computer_function):
    ...
    @pytest.mark.asyncio
    async def test_run_async_with_coordinates(self, mock_computer_function, tool_context):
        ...
        calls = []
        mock_state = ComputerState(screenshot=b"test_screenshot", url="https://example.com")
    ...
    @pytest.mark.asyncio
    async def test_run_async_with_drag_and_drop_coordinates(self, mock_computer_function, tool_context):
        ...
        async def specific_mock_func(x: int, y: int, destination_x: int, destination_y: int):
        ...
        def assert_called_once_with(x, y, destination_x, destination_y):
            assert len(calls) == 1, f"Expected 1 call, got {len(calls)}"
            assert calls[0] == (x, y, destination_x, destination_y), f"Expected ({x}, {y}, {destination_x}, {destination_y}), got {calls[0]}"
    ...
    @pytest.mark.asyncio
    async def test_run_async_with_non_computer_state_result(self, mock_computer_function, tool_context):
    ...
    @pytest.mark.asyncio
    async def test_run_async_without_coordinates(self, mock_computer_function, tool_context):
        ...
        def assert_called_once_with(direction):
            assert len(calls) == 1, f"Expected 1 call, got {len(calls)}"
            assert calls[0] == (direction,), f"Expected ({direction},), got {calls[0]}"
    ...
    @pytest.mark.asyncio
    async def test_run_async_with_error(self, mock_computer_function, tool_context):
    ...
    @pytest.mark.asyncio
    async def test_process_llm_request(self, mock_computer_function, tool_context):
    ...
    def test_custom_screen_size_with_custom_virtual_screen_size(self, mock_computer_function):
        ...
        # Test bounds
        assert tool._normalize_x(800) == 2559  # 800/800 * 2560, clamped to screen bounds
        assert tool._normalize_y(600) == 1439  # 600/600 * 1440, clamped to screen bounds

    @pytest.mark.asyncio
    async def test_coordinate_logging(self, mock_computer_function, tool_context, caplog):
