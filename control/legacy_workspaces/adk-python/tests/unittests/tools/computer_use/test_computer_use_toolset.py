...
class TestComputerUseToolset:
    ...
    @pytest.mark.asyncio
    async def test_get_tools_creates_tools_with_correct_methods(self, toolset, mock_computer):
    ...
    @pytest.mark.asyncio
    async def test_multiple_get_tools_calls_return_cached_instances(self, toolset):
    ...
    @pytest.mark.asyncio
    async def test_toolset_handles_computer_initialization_failure(self, mock_computer):
    ...
    async def test_process_llm_request(self, toolset, mock_computer):
        ...
        await toolset.process_llm_request(tool_context=MagicMock(), llm_request=llm_request)
        ...
        computer_use_tools = [tool for tool in llm_request.config.tools if hasattr(tool, "computer_use") and tool.computer_use]
        assert len(computer_use_tools) == 1

        # Should have correct environment
        computer_use_tool = computer_use_tools[0]
        assert computer_use_tool.computer_use.environment == types.Environment.ENVIRONMENT_BROWSER

    @pytest.mark.asyncio
    async def test_process_llm_request_with_existing_computer_use(self, toolset, mock_computer):
    ...
    async def test_adapt_computer_use_tool_sync_adapter(self):
        ...
        await ComputerUseToolset.adapt_computer_use_tool("wait", sync_adapter, llm_request)
    ...
    async def test_adapt_computer_use_tool_async_adapter(self):
        ...
        # Call the adaptation method
        await ComputerUseToolset.adapt_computer_use_tool("wait", async_adapter, llm_request)
    ...
    async def test_adapt_computer_use_tool_invalid_method(self):
        ...
        await ComputerUseToolset.adapt_computer_use_tool("invalid_method", adapter, llm_request)
    ...
    async def test_adapt_computer_use_tool_excluded_method(self):
        ...
        # Should not raise an exception, just log a warning
        await ComputerUseToolset.adapt_computer_use_tool("screen_size", adapter, llm_request)
    ...
    async def test_adapt_computer_use_tool_method_not_in_tools_dict(self):
        ...
        # Should not raise an exception, just log a warning
        await ComputerUseToolset.adapt_computer_use_tool("wait", adapter, llm_request)
