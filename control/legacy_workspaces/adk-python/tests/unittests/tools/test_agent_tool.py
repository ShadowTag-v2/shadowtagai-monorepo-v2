...
function_call_custom = Part.from_function_call(name="tool_agent", args={"custom_input": "test1"})

function_call_no_schema = Part.from_function_call(name="tool_agent", args={"request": "test1"})

function_response_custom = Part.from_function_response(name="tool_agent", response={"custom_output": "response1"})

function_response_no_schema = Part.from_function_response(name="tool_agent", response={"result": "response1"})
        ...
        async def after_model_callback(
            ...
            *,
            ...
            if response_text:
                ...
                self.model_responses[callback_context.agent_name].append("".join(response_text))
...
async def test_update_artifacts():
    ...
    async def before_tool_agent(callback_context: CallbackContext):
        ...
        await callback_context.save_artifact("artifact_2", Part.from_text(text=artifact.text + " 2"))
    ...
    async def after_main_agent(callback_context: CallbackContext):
        ...
        await callback_context.save_artifact("artifact_3", Part.from_text(text=artifact_2.text + " 3"))

    mock_model = testing_utils.MockModel.create(responses=[function_call_no_schema, "response2"])
    root_agent = Agent(
        name="root_agent",
    ...
    assert await runner.runner.artifact_service.list_artifact_keys(app_name="test_app", user_id="test_user", session_id=runner.session_id) == [
        "artifact_1",
        "artifact_2",
        "artifact_3",
    ]
...
def test_include_plugins_default_true():
    ...
    mock_model = testing_utils.MockModel.create(responses=[function_call_no_schema, "response1", "response2"])
