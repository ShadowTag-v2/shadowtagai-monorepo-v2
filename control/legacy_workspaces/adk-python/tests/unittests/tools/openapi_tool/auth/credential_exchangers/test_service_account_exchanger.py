...
def test_exchange_credential_success(service_account_exchanger, auth_scheme, monkeypatch):
    ...
    target_path = (
        "google.adk.tools.openapi_tool.auth.credential_exchangers.service_account_exchanger.service_account.Credentials.from_service_account_info"
    )
    monkeypatch.setattr(
        target_path,
    ...
    result = service_account_exchanger.exchange_credential(auth_scheme, auth_credential)
...
def test_exchange_credential_use_default_credential_success(service_account_exchanger, auth_scheme, monkeypatch):
    """Test successful exchange of service account credentials using default credential."""
    mock_credentials = MagicMock()
    mock_credentials.token = "mock_access_token"
    mock_google_auth_default = MagicMock(return_value=(mock_credentials, "test_project"))
...
def test_exchange_credential_missing_auth_credential(service_account_exchanger, auth_scheme):
...
def test_exchange_credential_missing_service_account_info(service_account_exchanger, auth_scheme):
...
def test_exchange_credential_exchange_failure(service_account_exchanger, auth_scheme, monkeypatch):
    """Test failure during service account token exchange."""
    mock_from_service_account_info = MagicMock(side_effect=Exception("Failed to load credentials"))
    target_path = (
        "google.adk.tools.openapi_tool.auth.credential_exchangers.service_account_exchanger.service_account.Credentials.from_service_account_info"
    )
    monkeypatch.setattr(
