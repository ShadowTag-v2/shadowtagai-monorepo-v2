...
from google.adk.tools.openapi_tool.auth.credential_exchangers.oauth2_exchanger import (
...
def test_init_with_custom_exchangers():
  ...
  assert (
    auto_exchanger.exchangers[AuthCredentialTypes.API_KEY] == MockCredentialExchanger
  )
  assert (
...
def test_exchange_credential_open_id_connect(auto_exchanger, auth_scheme):
  ...
  auto_exchanger.exchangers[AuthCredentialTypes.OPEN_ID_CONNECT] = lambda: (
    mock_exchanger
  )
...
def test_exchange_credential_service_account(auto_exchanger, auth_scheme):
  ...
  auto_exchanger.exchangers[AuthCredentialTypes.SERVICE_ACCOUNT] = lambda: (
    mock_exchanger
  )
