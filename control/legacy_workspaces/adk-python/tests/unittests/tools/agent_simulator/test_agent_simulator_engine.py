...
@patch("google.adk.tools.agent_simulator.agent_simulator_engine.ToolConnectionAnalyzer")
@patch("google.adk.tools.agent_simulator.agent_simulator_engine._create_mock_strategy")
@pytest.mark.asyncio
class TestAgentSimulatorEngineSimulate:
    """Test cases for the simulate method of AgentSimulatorEngine."""

    async def test_simulate_no_op_for_unconfigured_tool(self, mock_create_strategy, mock_analyzer):
    ...
    async def test_injection_with_matching_args(self, mock_create_strategy, mock_analyzer):
    ...
    async def test_injection_not_applied_with_mismatched_args(self, mock_create_strategy, mock_analyzer):
        ...
        mock_tool = MagicMock()
        mock_tool.name = "test_tool"
        result = await engine.simulate(mock_tool, {"param": "different_value"}, MagicMock())
    ...
    async def test_no_op_when_no_injection_hit_and_unspecified_strategy(self, mock_create_strategy, mock_analyzer, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="agent_simulator_logger"):
            result = await engine.simulate(mock_tool, {"param": "different_value"}, MagicMock())
            assert result is None
            assert "did not hit any injection config and has no mock strategy" in caplog.text
        mock_create_strategy.assert_not_called()

    async def test_injection_with_random_seed_is_deterministic(self, mock_create_strategy, mock_analyzer):
