...
class TestConnectionsClient:
    ...
    def test_execute_api_call_request_error_not_found_or_bad_request(
        ...
        with (
            ...
            with pytest.raises(ValueError, match="Invalid request. Please check the provided"):
                client._execute_api_call("https://test.url")
    ...
    def test_execute_api_call_unexpected_error(self, project, location, connection_name, mock_credentials):
        ...
        with (
            ...
            with pytest.raises(Exception, match="An unexpected error occurred: Something went wrong"):
                client._execute_api_call("https://test.url")
    ...
    def test_get_connection_details_error(self, project, location, connection_name):
        credentials = {"email": "test@example.com"}
        client = ConnectionsClient(project, location, connection_name, credentials)
        with mock.patch.object(client, "_execute_api_call", side_effect=ValueError("Request error")):
            with pytest.raises(ValueError, match="Request error"):
    ...
    def test_get_entity_schema_and_operations_no_operation_id(self, project, location, connection_name, mock_credentials):
        ...
        with mock.patch.object(client, "_execute_api_call", return_value=mock_execute_response):
            with pytest.raises(
                ValueError,
    ...
    def test_get_action_schema_no_operation_id(self, project, location, connection_name, mock_credentials):
        ...
        with mock.patch.object(client, "_execute_api_call", return_value=mock_execute_response):
            with pytest.raises(ValueError, match="Failed to get action schema for action: action1"):
