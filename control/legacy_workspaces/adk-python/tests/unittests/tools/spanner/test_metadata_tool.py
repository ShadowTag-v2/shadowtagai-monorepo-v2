...
def test_list_table_names_success(mock_get_spanner_client, mock_spanner_ids, mock_credentials):
...
@patch("google.adk.tools.spanner.client.get_spanner_client")
def test_list_table_names_error(mock_get_spanner_client, mock_spanner_ids, mock_credentials):
...
@patch("google.adk.tools.spanner.client.get_spanner_client")
def test_get_table_schema_success(mock_get_spanner_client, mock_spanner_ids, mock_credentials):
    ...
    assert result["results"]["schema"]["col1"]["KEY_COLUMN_USAGE"][0]["CONSTRAINT_NAME"] == "PK_Table"
    assert "metadata" in result["results"]
    assert result["results"]["metadata"][0]["TABLE_NAME"] == "test_table"
    assert result["results"]["metadata"][0]["ROW_DELETION_POLICY_EXPRESSION"] == "OLDER_THAN(CreatedAt, INTERVAL 1 DAY)"


@patch("google.adk.tools.spanner.client.get_spanner_client")
def test_list_table_indexes_success(mock_get_spanner_client, mock_spanner_ids, mock_credentials):
...
@patch("google.adk.tools.spanner.client.get_spanner_client")
def test_list_table_index_columns_success(mock_get_spanner_client, mock_spanner_ids, mock_credentials):
...
@patch("google.adk.tools.spanner.client.get_spanner_client")
def test_list_named_schemas_success(mock_get_spanner_client, mock_spanner_ids, mock_credentials):
