...
class TestGoogleSearchTool:
    ...
    async def test_process_llm_request_with_gemini_1_model(self):
        ...
        llm_request = LlmRequest(model="gemini-1.5-flash", config=types.GenerateContentConfig())
    ...
    async def test_process_llm_request_with_gemini_1_0_model(self):
        ...
        llm_request = LlmRequest(model="gemini-1.0-pro", config=types.GenerateContentConfig())
    ...
    async def test_process_llm_request_with_gemini_2_model(self):
        ...
        llm_request = LlmRequest(model="gemini-2.0-flash", config=types.GenerateContentConfig())
    ...
    async def test_process_llm_request_with_gemini_1_model_and_existing_tools_raises_error(
        ...
        with pytest.raises(
            ValueError,
            match=("Google search tool cannot be used with other tools in Gemini 1.x"),
        ):
            await tool.process_llm_request(tool_context=tool_context, llm_request=llm_request)
    ...
    async def test_process_llm_request_with_path_based_non_gemini_model_raises_error(
        ...
        with pytest.raises(
            ValueError,
            match=(f"Google search tool is not supported for model {non_gemini_path}"),
        ):
            await tool.process_llm_request(tool_context=tool_context, llm_request=llm_request)
    ...
    async def test_process_llm_request_with_none_model_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Google search tool is not supported for model None"):
            await tool.process_llm_request(tool_context=tool_context, llm_request=llm_request)
    ...
    async def test_process_llm_request_with_empty_model_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Google search tool is not supported for model "):
            await tool.process_llm_request(tool_context=tool_context, llm_request=llm_request)
    ...
    async def test_process_llm_request_with_none_tools(self):
        ...
        llm_request = LlmRequest(model="gemini-2.0-flash", config=types.GenerateContentConfig(tools=None))
    ...
    async def test_process_llm_request_edge_cases(self):
        ...
        for model in edge_cases:
            llm_request = LlmRequest(model=model, config=types.GenerateContentConfig())
