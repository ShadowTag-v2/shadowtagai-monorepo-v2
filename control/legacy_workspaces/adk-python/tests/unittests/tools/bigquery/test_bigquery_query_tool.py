...
async def get_tool(
  name: str, tool_settings: BigQueryToolConfig | None = None
) -> BaseTool:
  ...
  [
    ...
    pytest.param(
      ...
      [
        {
          "x": (
            "{'start': datetime.date(2023, 1, 1), 'end': datetime.date(2023, 1, 31)}"
          )
        }
      ],
      id="RANGE",
...
def test_detect_anomalies_with_invalid_id_cols():
  ...
  assert result["status"] == "ERROR"
  assert (
    "All elements in times_series_id_cols must be strings." in result["error_details"]
  )
