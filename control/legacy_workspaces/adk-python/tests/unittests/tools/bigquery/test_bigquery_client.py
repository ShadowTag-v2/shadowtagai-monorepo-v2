...
def test_bigquery_client_project_set_explicit():
    ...
    with mock.patch.dict(os.environ, {}, clear=True):
        with mock.patch.object(google.auth, "default", autospec=True) as mock_default_auth:
...
def test_bigquery_client_project_set_with_env():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_PROJECT": "test-gcp-project"}, clear=True):
        with mock.patch.object(google.auth, "default", autospec=True) as mock_default_auth:
            # Simulate exception from default auth
            mock_default_auth.side_effect = DefaultCredentialsError("Your default credentials were not found")
