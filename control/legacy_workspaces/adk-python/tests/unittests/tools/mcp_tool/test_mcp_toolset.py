...
class TestMCPToolset:
    ...
    def test_init_with_stdio_connection_params(self):
        ...
        stdio_params = StdioConnectionParams(server_params=self.mock_stdio_params, timeout=10.0)
    ...
    def test_init_with_sse_connection_params(self):
        ...
        sse_params = SseConnectionParams(url="https://example.com/mcp", headers={"Authorization": "Bearer token"})
    ...
    def test_init_with_tool_filter_list(self):
        ...
        toolset = MCPToolset(connection_params=self.mock_stdio_params, tool_filter=tool_filter)
    ...
    async def test_get_tools_basic(self):
        ...
        self.mock_session.list_tools = AsyncMock(return_value=MockListToolsResult(mock_tools))
    ...
    async def test_get_tools_with_function_filter(self):
        ...
        toolset = MCPToolset(connection_params=self.mock_stdio_params, tool_filter=file_tools_filter)
        toolset._mcp_session_manager = self.mock_session_manager
    ...
    async def test_close_with_exception(self):
        ...
        self.mock_session_manager.close = AsyncMock(side_effect=Exception("Cleanup error"))
    ...
    async def test_get_tools_with_timeout(self):
        ...
        stdio_params = StdioConnectionParams(server_params=self.mock_stdio_params, timeout=0.01)
        toolset = MCPToolset(connection_params=stdio_params)
        toolset._mcp_session_manager = self.mock_session_manager
        ...
        with pytest.raises(ConnectionError, match="Failed to get tools from MCP server."):
