...
class TestMCPSessionManager:
    ...
    def setup_method(self):
        ...
        self.mock_stdio_params = StdioServerParameters(command="test_command", args=[])
        self.mock_stdio_connection_params = StdioConnectionParams(server_params=self.mock_stdio_params, timeout=5.0)

    def test_init_with_stdio_server_parameters(self):
        """Test initialization with StdioServerParameters (deprecated)."""
        with patch("google.adk.tools.mcp_tool.mcp_session_manager.logger") as mock_logger:
            manager = MCPSessionManager(self.mock_stdio_params)

            # Should log deprecation warning
            mock_logger.warning.assert_called_once()
            assert "StdioServerParameters is not recommended" in str(mock_logger.warning.call_args)
    ...
    def test_init_with_streamable_http_params(self):
        ...
        http_params = StreamableHTTPConnectionParams(url="https://example.com/mcp", timeout=15.0)
    ...
    def test_merge_headers_sse(self):
        ...
        sse_params = SseConnectionParams(url="https://example.com/mcp", headers=base_headers)
        manager = MCPSessionManager(sse_params)
    ...
    async def test_create_session_stdio_new(self):
        ...
        with patch("google.adk.tools.mcp_tool.mcp_session_manager.stdio_client") as mock_stdio:
            with patch("google.adk.tools.mcp_tool.mcp_session_manager.AsyncExitStack") as mock_exit_stack_class:
                with patch("google.adk.tools.mcp_tool.mcp_session_manager.ClientSession") as mock_session_class:
                    # Setup mocks
    ...
    async def test_create_session_timeout(self, mock_session_class, mock_exit_stack_class, mock_stdio):
        ...
        mock_session.initialize.side_effect = TimeoutError("Test timeout")
        ...
        mock_session_class.assert_called_with(
            "read",
            "write",
            read_timeout_seconds=timedelta(seconds=manager._connection_params.timeout),
        )
