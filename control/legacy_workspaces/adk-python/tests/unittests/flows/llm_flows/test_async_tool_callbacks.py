...
async def invoke_tool_with_callbacks(before_cb=None, after_cb=None) -> Event | None:
...
def mock_async_before_cb_side_effect(
  ...
  args: dict[str, Any],
  tool_context: ToolContext,
  ret_value: dict[str, Any] | None = None,
):
  if ret_value:
    return ret_value
  return None
...
async def test_before_tool_callbacks_chain(
  callbacks: list[tuple[dict[str, Any] | None, int]],
  ...
  for response, callback_type in callbacks:
    if callback_type == CallbackType.ASYNC:
      mock_cb = mock.AsyncMock(
        side_effect=partial(mock_async_before_cb_side_effect, ret_value=response)
      )
    else:
      mock_cb = mock.Mock(
        side_effect=partial(mock_sync_before_cb_side_effect, ret_value=response)
      )
...
async def test_after_tool_callbacks_chain(
  ...
  for response, callback_type in callbacks:
    if callback_type == CallbackType.ASYNC:
      mock_cb = mock.AsyncMock(
        side_effect=partial(mock_async_after_cb_side_effect, ret_value=response)
      )
    else:
      mock_cb = mock.Mock(
        side_effect=partial(mock_sync_after_cb_side_effect, ret_value=response)
      )
