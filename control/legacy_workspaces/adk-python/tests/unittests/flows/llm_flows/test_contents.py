...
async def test_include_contents_default_full_history():
    ...
    agent = Agent(model="gemini-2.5-flash", name="test_agent", include_contents="default")
...
async def test_include_contents_none_multi_agent_current_turn():
    ...
    agent = Agent(model="gemini-2.5-flash", name="current_agent", include_contents="none")
    llm_request = LlmRequest(model="gemini-2.5-flash")
    invocation_context = await testing_utils.create_invocation_context(agent=agent)
...
async def test_authentication_events_are_filtered():
    ...
    auth_response = types.FunctionResponse(
        id="auth_123",
        name=REQUEST_EUC_FUNCTION_CALL_NAME,
        response={"auth_config": {"exchanged_auth_credential": {"token": "secret"}}},
    )
    ...
    events = [
        ...
        Event(
            invocation_id="inv2",
            author="test_agent",
            content=types.ModelContent([types.Part(function_call=auth_function_call)]),
        ),
        Event(
            invocation_id="inv3",
            author="user",
            content=types.Content(parts=[types.Part(function_response=auth_response)], role="user"),
        ),
        Event(
            invocation_id="inv4",
            author="user",
...
async def test_confirmation_events_are_filtered():
    ...
    events = [
        ...
        Event(
            invocation_id="inv2",
            author="test_agent",
            content=types.ModelContent([types.Part(function_call=confirmation_function_call)]),
        ),
        Event(
            invocation_id="inv3",
            author="user",
...
async def test_events_with_empty_content_are_skipped():
    ...
    events = [
        ...
        Event(
            ...
            author="user",
            content=types.Content(
                parts=[types.Part(inline_data=types.Blob(data=b"test", mime_type="image/png"))],
                role="user",
            ),
        ),
