...
async def test_request_confirmation_processor_no_events():
    ...
    events = []
    async for event in request_processor.run_async(invocation_context, llm_request):
        events.append(event)
...
async def test_request_confirmation_processor_no_function_responses():
    ...
    invocation_context.session.events.append(Event(author="user", content=types.Content()))
...
async def test_request_confirmation_processor_no_confirmation_function_response():
    ...
    invocation_context.session.events.append(
        Event(
            author="user",
            content=types.Content(parts=[types.Part(function_response=types.FunctionResponse(name="other_function", response={}))]),
        )
    )
...
async def test_request_confirmation_processor_success():
    ...
    original_function_call = types.FunctionCall(name=MOCK_TOOL_NAME, args={"param1": "test"}, id=MOCK_FUNCTION_CALL_ID)
    ...
    with patch("google.adk.flows.llm_flows.functions.handle_function_call_list_async") as mock_handle_function_call_list_async:
        ...
        assert args[4][MOCK_FUNCTION_CALL_ID] == user_confirmation  # tool_confirmation_dict
