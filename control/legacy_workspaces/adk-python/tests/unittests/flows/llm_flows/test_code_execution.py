...
async def test_builtin_code_executor_image_artifact_creation(mock_datetime):
    ...
    invocation_context.artifact_service.save_artifact = AsyncMock(return_value="v1")
    llm_response = LlmResponse(
        content=types.Content(
            parts=[
                ...
                types.Part(inline_data=types.Blob(mime_type="image/jpeg", data=b"image2")),
            ]
        )
    )

    events = []
    async for event in response_processor.run_async(invocation_context, llm_response):
        events.append(event)
    ...
    assert llm_response.content.parts[0].text == "Saved as artifact: image_1.png. "
...
async def test_logs_executed_code(mock_logger):
    ...
    mock_code_executor.execute_code.return_value = CodeExecutionResult(stdout="hello")
    ...
    _ = [event async for event in response_processor.run_async(invocation_context, llm_response)]

    mock_code_executor.execute_code.assert_called_once()
    mock_logger.debug.assert_called_once_with("Executed code:\n```\n%s\n```", 'print("hello")')
