...
def test_function_request_euc():
    ...
    def call_external_api1(tool_context: ToolContext) -> int | None:
        tool_context.request_credential(auth_config1)

    def call_external_api2(tool_context: ToolContext) -> int | None:
    ...
    assert exchanged_auth_config1.raw_auth_credential == auth_config1.raw_auth_credential
    assert exchanged_auth_config1.exchanged_auth_credential.oauth2.auth_uri is not None
    assert exchanged_auth_config2.auth_scheme == auth_config2.auth_scheme
    assert exchanged_auth_config2.raw_auth_credential == auth_config2.raw_auth_credential
    assert exchanged_auth_config2.exchanged_auth_credential.oauth2.auth_uri is not None
    ...
    for idx, part in enumerate(events[1].content.parts):
        ...
        assert request_euc_function_call.name == functions.REQUEST_EUC_FUNCTION_CALL_NAME
        ...
        assert args.auth_config.raw_auth_credential == auth_configs[idx].raw_auth_credential
...
def test_function_get_auth_response():
    ...
    function_response1.id = request_euc_function_call_event.content.parts[0].function_call.id
    ...
    function_response2.id = request_euc_function_call_event.content.parts[1].function_call.id
    runner.run(
