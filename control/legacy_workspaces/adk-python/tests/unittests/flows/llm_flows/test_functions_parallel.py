...
async def test_parallel_function_calls_with_state_change():
    function_calls = [
        ...
        types.Part.from_function_call(name="transfer_to_agent", args={"agent_name": "test_sub_agent"}),
    ]
    function_responses = [
        types.Part.from_function_response(name="update_session_state", response={"result": None}),
        types.Part.from_function_response(name="update_session_state", response={"result": None}),
        types.Part.from_function_response(name="transfer_to_agent", response={"result": None}),
    ]
    ...
    async def update_session_state(key: str, value: str, tool_context: ToolContext) -> None:
        nonlocal function_called
        function_called += 1
        tool_context.state.update({key: value})
        return

    async def transfer_to_agent(agent_name: str, tool_context: ToolContext) -> None:
        nonlocal function_called
        function_called += 1
