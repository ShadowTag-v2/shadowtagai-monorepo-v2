...
def test_before_tool_callback():
    ...
    agent = Agent(
        name="root_agent",
        model=mock_model,
        before_tool_callback=MockBeforeToolCallback(mock_response={"test": "before_tool_callback"}),
    ...
    assert testing_utils.simplify_events(runner.run("test")) == [
        ...
        (
            "root_agent",
            Part.from_function_response(name="simple_function", response={"test": "before_tool_callback"}),
        ),
...
def test_before_tool_callback_noop():
    ...
    responses = [
        types.Part.from_function_call(name="simple_function", args={"input_str": "simple_function_call"}),
        "response1",
    ]
    mock_model = testing_utils.MockModel.create(responses=responses)
    agent = Agent(
    ...
    assert testing_utils.simplify_events(runner.run("test")) == [
        (
            "root_agent",
            Part.from_function_call(name="simple_function", args={"input_str": "simple_function_call"}),
        ),
        (
            "root_agent",
...
def test_after_tool_callback():
    ...
    agent = Agent(
        name="root_agent",
        model=mock_model,
        after_tool_callback=MockAfterToolCallback(mock_response={"test": "after_tool_callback"}),
        tools=[simple_function],
    )
    ...
    assert testing_utils.simplify_events(runner.run("test")) == [
        ...
        (
            "root_agent",
            Part.from_function_response(name="simple_function", response={"test": "after_tool_callback"}),
        ),
        ("root_agent", "response1"),
    ]
