...
def function_response(response: dict[str, Any]) -> types.Part:
    return types.Part.from_function_response(name="increase_by_one", response=response)
...
def test_sequential_calls():
    ...
    # 1 item: user content
    assert testing_utils.simplify_contents(mockModel.requests[0].contents) == [("user", "test")]
