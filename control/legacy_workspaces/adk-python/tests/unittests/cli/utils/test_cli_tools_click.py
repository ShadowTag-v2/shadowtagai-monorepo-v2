...
def mock_load_eval_set_from_file():
    with mock.patch("google.adk.evaluation.local_eval_sets_manager.load_eval_set_from_file") as mock_func:
        yield mock_func
...
def test_cli_deploy_cloud_run_passthrough_args(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cli_deploy_cloud_run_rejects_args_without_separator(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cli_deploy_cloud_run_rejects_args_before_separator(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.exit_code == 2
    assert "Unexpected arguments after agent path and before '--':" in result.output
    assert "unexpected_arg" in result.output
    assert not rec.calls, "cli_deploy.to_cloud_run should not be called"


def test_cli_deploy_cloud_run_allows_empty_gcloud_args(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cli_deploy_agent_engine_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
# cli deploy gke
def test_cli_deploy_gke_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cli_web_invokes_uvicorn(tmp_path: Path, _patch_uvicorn: _Recorder, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cli_api_server_invokes_uvicorn(tmp_path: Path, _patch_uvicorn: _Recorder, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cli_web_passes_service_uris(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, _patch_uvicorn: _Recorder) -> None:
...
def test_cli_eval_with_eval_set_file_path(
    ...
    eval_set_results_manager = LocalEvalSetResultsManager(agents_dir=str(tmp_path))
    eval_set_results = eval_set_results_manager.list_eval_set_results(app_name="my_agent")
...
def test_cli_eval_with_eval_set_id(
    ...
    # Assert that we wrote eval set results
    eval_set_results_manager = LocalEvalSetResultsManager(agents_dir=str(tmp_path))
    eval_set_results = eval_set_results_manager.list_eval_set_results(app_name=app_name)
...
def test_cli_add_eval_case_with_session(tmp_path: Path):
    ...
    scenarios_file.write_text('{"scenarios": [{"starting_prompt": "hello", "conversation_plan": "world"}]}')
    session_file = tmp_path / "session2.json"
    session_file.write_text('{"app_name": "test_app_add_2", "user_id": "test_user", "state": {}}')
...
def test_cli_add_eval_case_skip_existing(tmp_path: Path):
    ...
    session_file.write_text('{"app_name": "test_app_add_3", "user_id": "test_user", "state": {}}')
...
def test_cli_deploy_cloud_run_gcloud_arg_conflict(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _mock_to_cloud_run(*_a, **kwargs):
        ...
        _validate_gcloud_extra_args(kwargs.get("extra_gcloud_args"), adk_managed_args)

    monkeypatch.setattr(cli_tools_click.cli_deploy, "to_cloud_run", _mock_to_cloud_run)
