...
@pytest.mark.asyncio
async def test_run_cli_loads_services_module(fake_agent, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cli, "load_services_module", lambda path: loaded_dirs.append(path))
...
@pytest.mark.asyncio
async def test_run_cli_app_uses_app_name_for_sessions(fake_app_agent, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cli, "create_session_service_from_options", _session_factory)
...
def test_create_artifact_service_respects_memory_uri(tmp_path: Path) -> None:
    ...
    service = create_artifact_service_from_options(base_dir=tmp_path, artifact_service_uri="memory://")
...
def test_create_artifact_service_accepts_file_uri(tmp_path: Path) -> None:
    ...
    service = create_artifact_service_from_options(base_dir=tmp_path, artifact_service_uri=custom_root.as_uri())
    assert isinstance(service, FileArtifactService)
...
@pytest.mark.asyncio
async def test_run_cli_accepts_memory_scheme(fake_agent, tmp_path: Path) -> None:
...
async def test_run_interactively_whitespace_and_exit(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    await cli.run_interactively(root_agent, artifact_service, sess, session_service, credential_service)
