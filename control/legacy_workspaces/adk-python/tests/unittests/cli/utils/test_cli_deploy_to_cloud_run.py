...
def test_to_cloud_run_happy_path(
    ...
    """
    src_dir = agent_dir(include_requirements=include_requirements, include_env=False)
    ...
    assert (agent_dest_path / "requirements.txt").is_file() == include_requirements
    ...
    assert 'RUN adduser --disabled-password --gecos "" myuser' in dockerfile_content
    ...
    if include_requirements:
        assert 'RUN pip install -r "/app/agents/agent/requirements.txt"' in dockerfile_content
    else:
        assert "# No requirements.txt found." in dockerfile_content

    assert "--allow_origins=http://localhost:3000,https://my-app.com" in dockerfile_content
