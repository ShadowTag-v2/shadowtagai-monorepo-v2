...
class TestAgentLoader:
  ...
  def test_agent_not_found_error(self):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      expected_msg_part_2 = "Searched in 'nonexistent_agent.agent.root_agent', 'nonexistent_agent.root_agent' and 'nonexistent_agent/root_agent.yaml'."
      expected_msg_part_3 = (
  ...
  def test_agent_internal_syntax_error(self):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      with (
        pytest.raises(SyntaxError) as exc_info
      ):  # Or potentially ImportError depending on Python version specifics with importlib
        loader.load_agent(agent_name)
  ...
  def test_yaml_agent_not_found_error(self):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      expected_msg_part_3 = (
        f"Ensure '{agents_dir}/nonexistent_yaml_agent' is structured correctly"
      )
  ...
  def test_special_agent_not_found_error(self):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
      ...
      try:
        ...
        expected_msg_part_2 = "Searched in 'nonexistent_special.agent.root_agent', 'nonexistent_special.root_agent' and 'nonexistent_special/root_agent.yaml'."
        expected_msg_part_3 = (
          f"Ensure '{special_agents_dir}/nonexistent_special' is structured correctly"
        )
