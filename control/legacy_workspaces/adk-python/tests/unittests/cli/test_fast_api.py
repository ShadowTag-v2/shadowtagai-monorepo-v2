...
def mock_artifact_service():
  ...
  def _artifact_key(
    app_name: str, user_id: str, session_id: str | None, filename: str
  ) -> str:
...
def test_update_session(test_app, create_test_session):
  ...
  state_patch_events = [
    event for event in events if event.get("invocationId", "").startswith("p-")
  ]

  assert len(state_patch_events) == 1, (
    f"Expected 1 state_patch event, found {len(state_patch_events)}. Events: {events}"
  )
...
def test_agent_run(test_app, create_test_session):
  ...
  assert (
    data[1]["content"]["parts"][0]["inlineData"]["mimeType"] == "audio/pcm;rate=24000"
  )
...
def test_save_artifact(test_app, create_test_session, mock_artifact_service):
  ...
  key = (
    f"{info['app_name']}:{info['user_id']}:{info['session_id']}:{payload['filename']}"
  )
