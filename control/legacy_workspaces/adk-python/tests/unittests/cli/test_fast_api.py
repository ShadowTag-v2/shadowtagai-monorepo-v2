...
def _event_1():
    return Event(
        ...
        content=types.Content(role="model", parts=[types.Part(text="LLM reply", inline_data=None)]),
    )
    ...
    return Event(
        ...
        content=types.Content(
            ...
            parts=[
                types.Part(
                    text=None,
                    inline_data=types.Blob(mime_type="audio/pcm;rate=24000", data=b"\x00\xff"),
                )
            ],
        ),
    )
...
def _event_3():
    return Event(author="dummy agent", invocation_id="invocation_id", interrupted=True)
...
def mock_artifact_service():
    ...
    def _artifact_key(app_name: str, user_id: str, session_id: str | None, filename: str) -> str:
    ...
    class MockArtifactService:
        def __init__(self):
            ...
            self.save_artifact_side_effect: BaseException | None = None
            ...
            *,
            ...
            if self.save_artifact_side_effect is not None:
                ...
                if isinstance(effect, BaseException):
                    raise effect
                raise TypeError("save_artifact_side_effect must be an exception instance.")
            key = _artifact_key(app_name, user_id, session_id, filename)
            entries = artifacts.setdefault(key, [])
            version = len(entries)
            artifact_version = ArtifactVersion(
                version=version,
                canonical_uri=_canonical_uri(app_name, user_id, session_id, filename, version),
        ...
        async def list_artifact_keys(self, app_name, user_id, session_id):
            ...
            return [key.split(":")[-1] for key in artifacts.keys() if key.startswith(prefix)]
...
def mock_eval_set_results_manager():
    ...
    class MockEvalSetResultsManager:
        ...
        def get_eval_set_result(self, app_name, eval_set_result_id):
            ...
            if eval_set_result_id not in eval_set_results[app_name]:
                raise ValueError(f"Eval set result {eval_set_result_id} not found in app {app_name}.")
...
@pytest.fixture
async def create_test_session(test_app, test_session_info, mock_session_service):
...
@pytest.fixture
async def create_test_eval_set(test_app, test_session_info, mock_eval_sets_manager):
...
def test_app_with_a2a(
    ...
    with (
        ...
        patch("google.adk.a2a.executor.a2a_agent_executor.A2aAgentExecutor") as mock_executor,
        patch("a2a.server.request_handlers.DefaultRequestHandler") as mock_handler,
        ...
        mock_app_instance.routes.return_value = []  # Return empty routes for testing
...
def test_update_session(test_app, create_test_session):
    ...
    state_patch_events = [event for event in events if event.get("invocationId", "").startswith("p-")]

    assert len(state_patch_events) == 1, f"Expected 1 state_patch event, found {len(state_patch_events)}. Events: {events}"
...
def test_agent_run(test_app, create_test_session):
    ...
    assert data[1]["content"]["parts"][0]["inlineData"]["mimeType"] == "audio/pcm;rate=24000"
...
def test_save_artifact(test_app, create_test_session, mock_artifact_service):
    ...
    assert data["canonicalUri"].endswith(f"/sessions/{info['session_id']}/artifacts/{payload['filename']}/versions/0")
    assert isinstance(data["createTime"], float)

    key = f"{info['app_name']}:{info['user_id']}:{info['session_id']}:{payload['filename']}"
    stored = mock_artifact_service._artifacts[key][0]
    assert stored["artifact"].text == "hello world"


def test_save_artifact_returns_400_on_validation_error(test_app, create_test_session, mock_artifact_service):
    ...
    mock_artifact_service.save_artifact_side_effect = InputValidationError("invalid artifact")
...
def test_save_artifact_returns_500_on_unexpected_error(test_app, create_test_session, mock_artifact_service):
    ...
    mock_artifact_service.save_artifact_side_effect = RuntimeError("unexpected failure")
...
def test_run_eval(test_app, create_test_eval_set):
    ...
    payload = {
        ...
        "eval_metrics": [{"metric_name": "tool_trajectory_avg_score", "threshold": 0.5}],
    }
    response = test_app.post(url, json=payload)
...
def test_get_event_graph_returns_dot_src_for_app_agent():
    ...
    response = client.get(f"/apps/test_app/users/user/sessions/session_id/events/{event_id}/graph")
    assert response.status_code == 200
    assert "dotSrc" in response.json()


@pytest.mark.skipif(sys.version_info < (3, 10), reason="A2A requires Python 3.10+")
