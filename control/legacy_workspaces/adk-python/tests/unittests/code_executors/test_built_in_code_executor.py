...
def test_process_llm_request_gemini_2_model_config_none(
    ...
    assert llm_request.config.tools == [types.Tool(code_execution=types.ToolCodeExecution())]
...
def test_process_llm_request_gemini_2_model_tools_none(
    ...
    llm_request = LlmRequest(model="gemini-2.0-pro", config=types.GenerateContentConfig())
    built_in_executor.process_llm_request(llm_request)
    assert llm_request.config.tools == [types.Tool(code_execution=types.ToolCodeExecution())]
...
def test_process_llm_request_gemini_2_model_with_existing_tools(
    ...
    existing_tool = types.Tool(function_declarations=[types.FunctionDeclaration(name="test_func", description="A test func")])
    llm_request = LlmRequest(
    ...
    assert types.Tool(code_execution=types.ToolCodeExecution()) in llm_request.config.tools
...
def test_process_llm_request_non_gemini_2_model(
    ...
    with pytest.raises(ValueError) as excinfo:
        built_in_executor.process_llm_request(llm_request)
    assert "Gemini code execution tool is not supported for model gemini-1.5-flash" in str(excinfo.value)
...
def test_process_llm_request_no_model_name(
    ...
    with pytest.raises(ValueError) as excinfo:
        built_in_executor.process_llm_request(llm_request)
    assert "Gemini code execution tool is not supported for model None" in str(excinfo.value)
