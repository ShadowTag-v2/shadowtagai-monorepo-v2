...
class TestSaveFilesAsArtifactsPlugin:
    ...
    def setup_method(self):
        ...
        artifact_service.get_artifact_version = AsyncMock(side_effect=_mock_get_artifact_version)
    ...
    async def test_save_files_with_display_name(self):
        ...
        result = await self.plugin.on_user_message_callback(invocation_context=self.mock_context, user_message=user_message)
        ...
        assert result.parts[1].file_data.file_uri == "gs://mock-bucket/test_document.pdf/versions/0"
    ...
    async def test_save_files_without_display_name(self):
        ...
        inline_data = types.Blob(display_name=None, data=b"test data", mime_type="application/pdf")
        ...
        assert result.parts[1].file_data
        assert result.parts[1].file_data.file_uri == "gs://mock-bucket/artifact_test_invocation_123_0/versions/0"
    ...
    async def test_multiple_files_in_message(self):
        ...
        inline_data1 = types.Blob(display_name="file1.txt", data=b"file1 content", mime_type="text/plain")
        inline_data2 = types.Blob(display_name="file2.jpg", data=b"file2 content", mime_type="image/jpeg")
        ...
        first_call = self.mock_context.artifact_service.save_artifact.call_args_list[0]
        second_call = self.mock_context.artifact_service.save_artifact.call_args_list[1]
        ...
        assert result.parts[1].file_data
        assert result.parts[1].file_data.file_uri == "gs://mock-bucket/file1.txt/versions/0"
        ...
        assert result.parts[4].file_data.file_uri == "gs://mock-bucket/file2.jpg/versions/0"
    ...
    async def test_no_artifact_service(self):
        ...
        inline_data = types.Blob(display_name="test.pdf", data=b"test data", mime_type="application/pdf")
    ...
    async def test_parts_without_inline_data(self):
        ...
        user_message = types.Content(parts=[types.Part(text="Hello world"), types.Part(text="No files here")])
    ...
    async def test_save_artifact_failure(self):
        ...
        self.mock_context.artifact_service.save_artifact.side_effect = Exception("Storage error")
    ...
    async def test_mixed_success_and_failure(self):
        ...
        self.mock_context.artifact_service.save_artifact.side_effect = _save_side_effect
        ...
        user_message = types.Content(parts=[types.Part(inline_data=inline_data1), original_part2])
