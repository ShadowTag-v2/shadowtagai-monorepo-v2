...
async def test_base_plugin_default_callbacks_return_none():
  ...
  assert (
    await plugin.on_event_callback(invocation_context=mock_context, event=mock_context)
    is None
  )
  assert (
  ...
  assert (
    await plugin.after_agent_callback(agent=mock_context, callback_context=mock_context)
    is None
  )
  assert (
