...
class TestTaskResultAggregator:
    ...
    def test_process_auth_required_event(self):
        ...
        event = TaskStatusUpdateEvent(
            task_id="test-task",
            context_id="test-context",
            status=TaskStatus(state=TaskState.auth_required, message=status_message),
            final=False,
        )
    ...
    def test_working_event_updates_message_regardless_of_state(self):
        ...
        self.aggregator.process_event(working_event)
        assert self.aggregator.task_state == TaskState.auth_required  # State unchanged
        assert self.aggregator.task_status_message == auth_message  # Message unchanged because task state is not working
