...
class TestConvertEvalSetToPydanticSchema:
  ...
  def test_convert_eval_set_to_pydantic_schema_complete(self):
    ...
    eval_set_in_json_format = [
      {
        ...
        "data": [
          ...
          },
          {
            ...
            "reference": (
              "I have rolled a 17 sided die twice. The first roll was 13 and the second roll was 4.\n"
            ),
          },
        ],
        "initial_session": {
    ...
    assert (
      len(
        eval_set.eval_cases[0].conversation[1].intermediate_data.intermediate_responses
      )
      == 1
    )
  ...
  def test_convert_eval_set_to_pydantic_schema_minimal(self):
    ...
    assert (
      eval_set.eval_cases[0].conversation[0].final_response.parts[0].text == "World"
    )
  ...
  def test_convert_eval_set_to_pydantic_schema_empty_tool_use_and_intermediate_responses(
    ...
    assert (
      len(
        eval_set.eval_cases[0].conversation[0].intermediate_data.intermediate_responses
      )
      == 0
    )
          ...
          conversation=[
            Invocation(
              ...
              final_response=genai_types.Content(
                parts=[genai_types.Part(text="New Format Reference")]
              ),
            )
          ],
        )
