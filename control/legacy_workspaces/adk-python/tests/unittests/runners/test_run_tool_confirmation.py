        ...
        Part(
          function_call=FunctionCall(
            ...
            args={
              ...
              "toolConfirmation": {
                "hint": (
                  "test hint for request_confirmation with custom payload schema"
                ),
                "confirmed": False,
                "payload": {
                  "test_custom_payload": {
...
class TestHITLConfirmationFlowWithParallelAgentAndResumableApp:
  ...
  async def test_pause_and_resume_on_request_confirmation(
    ...
    sub_agent1_branch_events = [
      event for event in events if event.branch == f"{agent.name}.{sub_agent1.name}"
    ]
    sub_agent2_branch_events = [
      event for event in events if event.branch == f"{agent.name}.{sub_agent2.name}"
    ]
