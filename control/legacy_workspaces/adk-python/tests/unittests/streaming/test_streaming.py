...
def test_streaming():
    ...
    runner = testing_utils.InMemoryRunner(root_agent=root_agent, response_modalities=["AUDIO"])
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"\x00\xff", mime_type="audio/pcm"))
    res_events = runner.run_live(live_request_queue)

    assert res_events is not None, "Expected a list of events, got None."
    assert len(res_events) > 0, "Expected at least one response, but got an empty list."
...
def test_live_streaming_function_call_single():
    ...
    function_call = types.Part.from_function_call(name="get_weather", args={"location": "San Francisco", "unit": "celsius"})
    ...
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"What is the weather in San Francisco?", mime_type="audio/pcm"))
...
def test_live_streaming_function_call_multiple():
    ...
    function_call1 = types.Part.from_function_call(name="get_weather", args={"location": "San Francisco"})
    function_call2 = types.Part.from_function_call(name="get_time", args={"timezone": "PST"})
    ...
    runner = CustomTestRunner(root_agent=root_agent)
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"What is the weather and current time?", mime_type="audio/pcm"))
    ...
    assert weather_call_found or time_call_found, "Expected at least one function call."
...
def test_live_streaming_function_call_parallel():
    ...
    function_call1 = types.Part.from_function_call(name="get_weather", args={"location": "San Francisco"})
    function_call2 = types.Part.from_function_call(name="get_weather", args={"location": "New York"})

    # Create LLM response with parallel function calls
    response1 = LlmResponse(
        content=types.Content(role="model", parts=[function_call1, function_call2]),
        turn_complete=False,
    )
    response2 = LlmResponse(
        turn_complete=True,
    ...
    runner = CustomTestRunner(root_agent=root_agent)
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"Compare weather in SF and NYC", mime_type="audio/pcm"))
    ...
    assert sf_call_found and nyc_call_found, "Expected both location function calls."


def test_live_streaming_function_call_with_error():
    """Test live streaming with function call that returns an error."""
    # Create a function call response
    function_call = types.Part.from_function_call(name="get_weather", args={"location": "Invalid Location"})
    ...
    runner = CustomTestRunner(root_agent=root_agent)
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"What is weather in Invalid Location?", mime_type="audio/pcm"))
...
def test_live_streaming_function_call_sync_tool():
    ...
    # Create a function call response
    function_call = types.Part.from_function_call(name="calculate", args={"x": 5, "y": 3})
    ...
    runner = CustomTestRunner(root_agent=root_agent)
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"Calculate 5 plus 3", mime_type="audio/pcm"))
...
def test_live_streaming_simple_streaming_tool():
    ...
    function_call = types.Part.from_function_call(name="monitor_stock_price", args={"stock_symbol": "AAPL"})
    ...
    runner = CustomTestRunner(root_agent=root_agent)
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"Monitor AAPL stock price", mime_type="audio/pcm"))
    ...
    for event in res_events:
        if event.content and event.content.parts:
            for part in event.content.parts:
                if part.function_call and part.function_call.name == "monitor_stock_price":
                    function_call_found = True
                    assert part.function_call.args["stock_symbol"] == "AAPL"

    assert function_call_found, "Expected monitor_stock_price function call event."
...
def test_live_streaming_video_streaming_tool():
    ...
    function_call = types.Part.from_function_call(name="monitor_video_stream", args={})
    ...
    live_request_queue.send_realtime(blob=types.Blob(data=b"fake_jpeg_data_1", mime_type="image/jpeg"))
    live_request_queue.send_realtime(blob=types.Blob(data=b"fake_jpeg_data_2", mime_type="image/jpeg"))
    live_request_queue.send_realtime(blob=types.Blob(data=b"Monitor video stream", mime_type="audio/pcm"))
    ...
    for event in res_events:
        if event.content and event.content.parts:
            for part in event.content.parts:
                if part.function_call and part.function_call.name == "monitor_video_stream":
                    function_call_found = True

    assert function_call_found, "Expected monitor_video_stream function call event."
...
def test_live_streaming_stop_streaming_tool():
    ...
    start_function_call = types.Part.from_function_call(name="monitor_stock_price", args={"stock_symbol": "TSLA"})
    stop_function_call = types.Part.from_function_call(name="stop_streaming", args={"function_name": "monitor_stock_price"})
    ...
    runner = CustomTestRunner(root_agent=root_agent)
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"Monitor TSLA and then stop", mime_type="audio/pcm"))
            ...
            for part in event.content.parts:
                if part.function_call:
                    if part.function_call.name == "monitor_stock_price":
                        ...
                        assert part.function_call.args["function_name"] == "monitor_stock_price"
...
def test_live_streaming_multiple_streaming_tools():
    ...
    stock_function_call = types.Part.from_function_call(name="monitor_stock_price", args={"stock_symbol": "NVDA"})
    video_function_call = types.Part.from_function_call(name="monitor_video_stream", args={})

    # Create LLM responses: start both streaming tools
    response1 = LlmResponse(
        content=types.Content(role="model", parts=[stock_function_call, video_function_call]),
        turn_complete=False,
    )
    response2 = LlmResponse(
        turn_complete=True,
    ...
    runner = CustomTestRunner(root_agent=root_agent)
    live_request_queue = LiveRequestQueue()
    live_request_queue.send_realtime(blob=types.Blob(data=b"Monitor both stock and video", mime_type="audio/pcm"))
