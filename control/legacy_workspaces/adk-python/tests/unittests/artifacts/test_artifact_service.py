...
class MockClient:
    ...
    def list_blobs(self, bucket: MockBucket, prefix: str | None = None):
        """Mocks listing blobs in a bucket, optionally with a prefix."""
        if prefix:
            return [blob for name, blob in bucket.blobs.items() if name.startswith(prefix) and blob.content is not None]
...
async def test_list_keys(service_type, artifact_service_factory):
    ...
    assert await artifact_service.list_artifact_keys(app_name=app_name, user_id=user_id, session_id=session_id) == filenames
...
async def test_list_versions(service_type, artifact_service_factory):
    ...
    versions = [types.Part.from_bytes(data=i.to_bytes(2, byteorder="big"), mime_type="text/plain") for i in range(3)]
...
async def test_list_keys_preserves_user_prefix(service_type, artifact_service_factory):
    ...
    artifact_keys = await artifact_service.list_artifact_keys(app_name=app_name, user_id=user_id, session_id=session_id)
...
async def test_list_artifact_versions_and_get_artifact_version(service_type, artifact_service_factory):
    ...
    session_id = "123"
    filename = "filename"
    versions = [types.Part.from_bytes(data=i.to_bytes(2, byteorder="big"), mime_type="text/plain") for i in range(4)]

    with patch("google.adk.artifacts.base_artifact_service.datetime") as mock_datetime:
        ...
        for i in range(4):
            ...
            if service_type == ArtifactServiceType.GCS:
                uri = f"gs://test_bucket/{app_name}/{user_id}/{session_id}/{filename}/{i}"
            else:
...
@pytest.mark.asyncio
@pytest.mark.parametrize("service_type", [ArtifactServiceType.IN_MEMORY, ArtifactServiceType.GCS])
async def test_list_artifact_versions_with_user_prefix(service_type, artifact_service_factory):
...
@pytest.mark.asyncio
@pytest.mark.parametrize("service_type", [ArtifactServiceType.IN_MEMORY, ArtifactServiceType.GCS])
async def test_get_artifact_version_artifact_does_not_exist(service_type, artifact_service_factory):
...
@pytest.mark.asyncio
@pytest.mark.parametrize("service_type", [ArtifactServiceType.IN_MEMORY, ArtifactServiceType.GCS])
async def test_get_artifact_version_out_of_index(service_type, artifact_service_factory):
...
async def test_file_metadata_camelcase(tmp_path, artifact_service_factory):
    ...
    artifact = types.Part.from_bytes(data=b"binary-content", mime_type="application/octet-stream")
    await artifact_service.save_artifact(
        app_name="myapp",
        user_id="user123",
        session_id="sess789",
...
async def test_file_list_artifact_versions(tmp_path, artifact_service_factory):
    ...
    version_payload_path = (
        tmp_path / "artifacts" / "users" / "user123" / "sessions" / "sess789" / "artifacts" / "docs" / "report.txt" / "versions" / "0" / "report.txt"
    ).resolve()
...
async def test_file_save_artifact_rejects_out_of_scope_paths(tmp_path, filename, session_id):
...
async def test_file_save_artifact_rejects_absolute_path_within_scope(tmp_path):
    ...
    artifact_service = FileArtifactService(root_dir=tmp_path / "artifacts")
    absolute_in_scope = tmp_path / "artifacts" / "apps" / "myapp" / "users" / "user123" / "artifacts" / "diagram.png"
    part = types.Part(text="content")
    with pytest.raises(InputValidationError):
        await artifact_service.save_artifact(
            app_name="myapp",
