...
async def test_generate_content_async(mock_acompletion, lite_llm_instance):
  ...
  assert (
    kwargs["tools"][0]["function"]["parameters"]["properties"]["test_arg"]["type"]
    == "string"
  )
...
def test_message_to_generate_content_response_inline_tool_call_text():
  message = ChatCompletionAssistantMessage(
    role="assistant",
    content=(
      '{"id":"inline_call","name":"get_current_time","arguments":{"timezone_str":"Asia/Taipei"}} <|im_end|>system'
    ),
  )
        ...
        choices=[
          {
            ...
            "message": {
              "role": "assistant",
              "content": (
                '{"id":"call_1","name":"get_current_time","arguments":{"timezone_str":"Asia/Taipei"}}'
              ),
            },
          }
        ],
        ...
        choices=[
          {
            ...
            "message": {
              "role": "assistant",
              "content": (
                'Intro {"id":"call_2","name":"alpha","arguments":{"foo":"bar"}} wrap'
              ),
            },
          }
        ],
