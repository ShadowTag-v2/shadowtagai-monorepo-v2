...
def test_append_tools_with_existing_tools():
    ...
    existing_declaration = types.FunctionDeclaration(name="existing_tool", description="An existing tool", parameters={})
    request.config.tools = [types.Tool(function_declarations=[existing_declaration])]
    ...
    decl_names = {decl.name for decl in request.config.tools[0].function_declarations}
...
def test_append_tools_tool_with_no_declaration():
    ...
    class NoDeclarationTool(BaseTool):
        def __init__(self):
            super().__init__(name="no_decl_tool", description="A tool with no declaration")
...
def test_append_instructions_with_content():
    ...
    content = types.Content(role="user", parts=[types.Part(text="This is content-based instruction")])
...
def test_append_instructions_with_content_multiple_calls():
    ...
    existing_content = types.Content(role="user", parts=[types.Part(text="Existing content")])
    request.contents.append(existing_content)

    # First Content instruction
    content1 = types.Content(role="user", parts=[types.Part(text="First instruction")])
    ...
    content2 = types.Content(role="user", parts=[types.Part(text="Second instruction")])
...
def test_append_instructions_with_content_multipart():
    ...
    assert request.config.system_instruction.parts[1].text == "Additional text part"
...
def test_append_instructions_mixed_string_and_content():
    ...
    content = types.Content(role="user", parts=[types.Part(text="Content instruction")])
    request.append_instructions(content)
    ...
    assert request.config.system_instruction.parts[1].text == "Content instruction"
...
def test_append_instructions_invalid_input():
    ...
    with pytest.raises(TypeError, match="instructions must be list\\[str\\] or types.Content"):
    ...
    with pytest.raises(TypeError, match="instructions must be list\\[str\\] or types.Content"):
        request.append_instructions(["valid string", 123])  # Mixed valid/invalid in list
...
async def test_process_llm_request_consolidates_declarations_in_single_tool():
    ...
    await tool1.process_llm_request(tool_context=tool_context, llm_request=request)
    await tool2.process_llm_request(tool_context=tool_context, llm_request=request)
    await tool3.process_llm_request(tool_context=tool_context, llm_request=request)
...
async def test_append_tools_and_process_llm_request_consistent_behavior():
    ...
    await tool4.process_llm_request(tool_context=tool_context, llm_request=request2)
    await tool5.process_llm_request(tool_context=tool_context, llm_request=request2)
    await tool6.process_llm_request(tool_context=tool_context, llm_request=request2)
    ...
    decl_names1 = {decl.name for decl in request1.config.tools[0].function_declarations}
    decl_names2 = {decl.name for decl in request2.config.tools[0].function_declarations}
...
def test_append_instructions_with_content():
    ...
    assert len(request.contents) == 0
    assert request.config.system_instruction == "This is content-based instruction"
...
def test_append_instructions_content_with_non_text_parts():
    ...
    content = types.Content(
        ...
        parts=[
            types.Part(text="Text instruction"),
            types.Part(inline_data=types.Blob(data=b"file_data", mime_type="text/plain")),
    ...
    assert user_contents[0].parts[0].text == "Referenced inline data: inline_data_0"
...
def test_append_instructions_warning_unsupported_system_instruction_type(
    ...
    assert "Cannot append to system_instruction of unsupported type" in caplog.text
