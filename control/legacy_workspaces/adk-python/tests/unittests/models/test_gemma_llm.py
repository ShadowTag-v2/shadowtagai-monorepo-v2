...
async def test_preprocess_request_with_function_call():
  ...
  llm_request = LlmRequest(
    model="gemma-3-1b-it",
    contents=[
      types.Content(role="user", parts=[types.Part(function_call=func_call_data)])
    ],
  )
...
def test_process_response():
  ...
  llm_response = LlmResponse(
    content=Content(role="model", parts=[Part.from_text(text=json_function_call_str)])
  )
...
def test_process_response_malformed_json():
  ...
  llm_response = LlmResponse(
    content=Content(role="model", parts=[Part.from_text(text=malformed_json_str)])
  )
  gemma = Gemma()
  gemma._extract_function_calls_from_response(llm_response=llm_response)
...
def test_process_response_with_embedded_json():
  ...
  llm_response = LlmResponse(
    content=Content(role="model", parts=[Part.from_text(text=embedded_json_str)])
  )
...
def test_process_response_flexible_parsing():
  ...
  llm_response = LlmResponse(
    content=Content(role="model", parts=[Part.from_text(text=flexible_json_str)])
  )
...
def test_process_response_last_json_object():
  ...
  multiple_json_str = 'I thought about {"name": "first_call", "parameters": {"a": 1}} but then decided to call: {"name": "second_call", "parameters": {"b": 2}}'
  llm_response = LlmResponse(
    content=Content(role="model", parts=[Part.from_text(text=multiple_json_str)])
  )
