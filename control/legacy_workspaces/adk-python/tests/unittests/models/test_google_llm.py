...
def test_supported_models():
  ...
  assert (
    models[3] == r"projects\/.+\/locations\/.+\/publishers\/google\/models\/gemini.+"
  )
...
def test_client_version_header():
  ...
  assert any(
    sdk in user_agent_header for sdk in ["google-genai-sdk/", "vertex-genai-modules/"]
  )
...
async def test_generate_content_async(
  ...
  with mock.patch.object(gemini_llm, "api_client") as mock_client:
    ...
    responses = [
      resp
      async for resp in gemini_llm.generate_content_async(llm_request, stream=False)
    ]
...
async def test_generate_content_async_stream(gemini_llm, llm_request):
  with mock.patch.object(gemini_llm, "api_client") as mock_client:
    ...
    responses = [
      resp async for resp in gemini_llm.generate_content_async(llm_request, stream=True)
    ]
...
async def test_generate_content_async_stream_aggregated_content_regardless_of_finish_reason():
  ...
  with mock.patch.object(gemini_llm, "api_client") as mock_client:
    ...
    for finish_reason in test_cases:
      ...
      responses = [
        resp
        async for resp in gemini_llm.generate_content_async(llm_request, stream=True)
      ]
...
def test_build_function_declaration_log():
  ...
  func_decl1 = types.FunctionDeclaration(
    ...
    parameters=types.Schema(
      ...
      properties={
        "param1": types.Schema(type=types.Type.STRING, description="param1 desc")
      },
    ),
  ...
  assert log2 == (
    "test_func2: {'type': 'object', 'properties': {'param2': {'type': 'integer'}}} -> {'type': 'string'}"
  )
...
def test_build_request_log_with_config_multiple_tool_types():
  ...
  assert (
    "'system_instruction'" not in log_output.split("Contents:")[0].split("Config:")[1]
  )
...
async def test_connect_uses_gemini_speech_config_when_request_is_none(
  ...
  with mock.patch.object(gemini_llm, "_live_api_client") as mock_live_client:
    ...
    async with gemini_llm.connect(llm_request) as connection:
      ...
      assert (
        config_arg.speech_config.voice_config.prebuilt_voice_config.voice_name == "Kore"
      )
      assert isinstance(connection, GeminiLlmConnection)
...
async def test_connect_request_gemini_config_overrides_speech_config(
  ...
  with mock.patch.object(gemini_llm, "_live_api_client") as mock_live_client:
    ...
    async with gemini_llm.connect(llm_request) as connection:
      ...
      assert config_arg.speech_config is not None
      assert (
        config_arg.speech_config.voice_config.prebuilt_voice_config.voice_name == "Puck"
      )
      assert isinstance(connection, GeminiLlmConnection)
