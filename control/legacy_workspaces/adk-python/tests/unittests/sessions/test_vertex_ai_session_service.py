...
MOCK_SESSION_JSON_PAGE1 = {
  "name": (
    "projects/test-project/locations/test-location/reasoningEngines/123/sessions/page1"
  ),
...
MOCK_SESSION_JSON_PAGE2 = {
  "name": (
    "projects/test-project/locations/test-location/reasoningEngines/123/sessions/page2"
  ),
...
def _generate_mock_events_for_session_5(num_events):
  ...
  for i in range(num_events):
    ...
    events.append(
      {
        "name": (
          f"projects/test-project/locations/test-location/reasoningEngines/123/sessions/5/events/{i}"
        ),
...
class MockAsyncClient:
  ...
  async def _create_session(self, name: str, user_id: str, config: dict[str, Any]):
    ...
    self.session_dict[new_session_id] = {
      "name": (
        "projects/test-project/locations/test-location/reasoningEngines/123/sessions/"
        + new_session_id
      ),
      "user_id": user_id,
    ...
    return _convert_to_object(
      {
        "name": (
          "projects/test_project/locations/test_location/reasoningEngines/123/sessions/"
          + new_session_id
          + "/operations/111"
        ),
        "done": True,
  ...
  async def _list_events(self, name: str, **kwargs):
    ...
    if filter_str:
      ...
      if match:
        ...
        events = [
          event for event in events if isoparse(event["timestamp"]) >= after_timestamp
        ]
...
async def test_get_session_with_page_token():
  ...
  assert (
    await session_service.get_session(app_name="123", user_id="user", session_id="2")
    == MOCK_SESSION_2
  )
...
async def test_get_session_keeps_events_newer_than_update_time(
  ...
  future_event_time = isoparse(MOCK_SESSION_JSON_1["update_time"]) + datetime.timedelta(
    seconds=1
  )
...
async def test_create_session_with_custom_config(mock_api_client_instance):
  ...
  assert (
    mock_api_client_instance.last_create_session_config["expire_time"] == expire_time
  )
