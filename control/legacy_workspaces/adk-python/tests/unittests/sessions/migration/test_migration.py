...
def test_migrate_from_sqlalchemy_pickle(tmp_path):
    ...
    app_state = v0.StorageAppState(app_name="app1", state={"akey": 1}, update_time=now)
    user_state = v0.StorageUserState(app_name="app1", user_id="user1", state={"ukey": 2}, update_time=now)
...
def test_migrate_from_sqlalchemy_pickle_with_async_driver_urls(tmp_path):
    ...
    # Populate source data
    now = datetime.now(UTC)
    app_state = v0.StorageAppState(app_name="async_app", state={"key": "value"}, update_time=now)
    session = v0.StorageSession(
        app_name="async_app",
        user_id="async_user",
        id="async_session",
...
def _assert_update_timestamp_tz_is_utc_timestamp(schema_module) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert fetched.update_timestamp_tz == update_time.replace(tzinfo=UTC).timestamp()
