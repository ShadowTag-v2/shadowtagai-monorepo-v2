...
class TestInMemoryRunner(AfInMemoryRunner):
    ...
    async def run_async_with_new_session_agen(self, new_message: types.ContentUnion) -> AsyncGenerator[Event, None]:
...
class MockModel(BaseLlm):
    ...
    def generate_content(self, llm_request: LlmRequest, stream: bool = False) -> Generator[LlmResponse, None, None]:
