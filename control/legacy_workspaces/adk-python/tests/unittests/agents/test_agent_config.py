...
def test_agent_config_discriminator_parallel_agent(agent_class_value: str, tmp_path: Path):
    yaml_content = f"""\
agent_class: {agent_class_value}
name: CodePipelineAgent
description: Executes a sequence of code writing, reviewing, and refactoring.
...
def test_agent_config_discriminator_sequential_agent(agent_class_value: str, tmp_path: Path):
    yaml_content = f"""\
agent_class: {agent_class_value}
name: CodePipelineAgent
description: Executes a sequence of code writing, reviewing, and refactoring.
...
def test_agent_config_discriminator_with_sub_agents(agent_class_value: str, expected_agent_type: type[BaseAgent], tmp_path: Path):
...
def test_agent_config_discriminator_llm_agent_with_sub_agents(agent_class_value: str, expected_agent_type: type[BaseAgent], tmp_path: Path):
    # Create sub-agent config files
    sub_agent_dir = tmp_path / "sub_agents"
    sub_agent_dir.mkdir()
    sub_agent_config = """\
...
"""
    ...
    assert agent.model._additional_args.get("api_base") == ("https://proxy.litellm.ai/v1")
...
def test_resolve_agent_reference_resolves_relative_paths(
    ...
    agent = config_agent_utils.resolve_agent_reference(ref_config, str(config_file))
...
def test_resolve_agent_reference_uses_windows_dirname():
    ...
    expected_path = ntpath.join(ntpath.dirname(referencing), ref_config.config_path)
