...
class TestLlmAgentOutputSave:
    ...
    def test_maybe_save_output_to_state_skips_different_author(self, caplog):
        ...
        llm_agent_logger = logging.getLogger("google_adk.google.adk.agents.llm_agent")
        ...
        try:
            ...
            # Should log the skip
            assert "Skipping output save for agent agent_a: event authored by agent_b" in caplog.text
        finally:
    ...
    def test_maybe_save_output_to_state_not_final_response(self):
        ...
        agent = LlmAgent(name="test_agent", output_key="result")
        event = create_test_event(author="test_agent", content_text="Partial response", is_final=False)
    ...
    def test_maybe_save_output_to_state_with_output_schema(self):
        ...
        agent = LlmAgent(name="test_agent", output_key="result", output_schema=MockOutputSchema)
    ...
    def test_maybe_save_output_to_state_agent_transfer_scenario(self, caplog):
        ...
        try:
            ...
            agent_b_event = create_test_event(author="billing_agent", content_text="Your bill is $100")
            ...
            assert "Skipping output save for agent support_agent: event authored by billing_agent" in caplog.text
        finally:
            # Restore original logger level
            llm_agent_logger.setLevel(original_level)
    ...
    def test_maybe_save_output_to_state_case_sensitive_names(self, caplog):
        ...
        try:
            ...
            assert len(event.actions.state_delta) == 0
            assert "Skipping output save for agent TestAgent: event authored by testagent" in caplog.text
        finally:
            # Restore original logger level
            llm_agent_logger.setLevel(original_level)
    ...
    def test_maybe_save_output_to_state_handles_empty_final_chunk_with_schema(self, empty_content):
        ...
        event = create_test_event(author="test_agent", content_text=empty_content, is_final=True)
