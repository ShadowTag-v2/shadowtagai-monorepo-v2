...
class TestContextInitialization:
    ...
    def test_initialization_without_function_call_id(self, mock_invocation_context):
    ...
    def test_initialization_with_tool_confirmation(self, mock_invocation_context):
        ...
        tool_confirmation = ToolConfirmation(hint="test hint", payload={"key": "value"})
        context = Context(
            mock_invocation_context,
            function_call_id="test-function-call-id",
...
class TestContextListArtifacts:
    ...
    async def test_list_artifacts_returns_artifact_keys(self, mock_invocation_context, mock_artifact_service):
    ...
    async def test_list_artifacts_raises_value_error_when_service_is_none(self, mock_invocation_context):
...
class TestContextCredentialMethods:
    ...
    async def test_save_credential_with_service(self, mock_invocation_context, mock_auth_config):
        ...
        credential_service.save_credential.assert_called_once_with(mock_auth_config, context)

    async def test_save_credential_no_service(self, mock_invocation_context, mock_auth_config):
        """Test save_credential when credential service is not available."""
        mock_invocation_context.credential_service = None

        context = Context(mock_invocation_context)

        with pytest.raises(ValueError, match="Credential service is not initialized"):
            await context.save_credential(mock_auth_config)

    async def test_load_credential_with_service(self, mock_invocation_context, mock_auth_config, mock_auth_credential):
        ...
        credential_service.load_credential.assert_called_once_with(mock_auth_config, context)
        assert result == mock_auth_credential

    async def test_load_credential_no_service(self, mock_invocation_context, mock_auth_config):
...
class TestContextGetAuthResponse:
    ...
    def test_get_auth_response(self, mock_invocation_context, mock_auth_config):
        ...
        with patch.object(auth_handler, "AuthHandler", autospec=True) as mock_auth_handler:
            ...
            mock_handler_instance.get_auth_response.assert_called_once_with(context.state)
...
class TestContextRequestCredential:
    ...
    def test_request_credential_with_function_call_id(self, mock_invocation_context, mock_auth_config):
        ...
        with patch.object(auth_handler, "AuthHandler", autospec=True) as mock_auth_handler:
            ...
            assert context.actions.requested_auth_configs["test-function-call-id"] == "auth-request"

    def test_request_credential_without_function_call_id_raises(self, mock_invocation_context, mock_auth_config):
...
class TestContextRequestConfirmation:
    ...
    def test_request_confirmation_with_function_call_id(self, mock_invocation_context):
        ...
        context.request_confirmation(hint="Please confirm", payload={"action": "delete"})

        confirmation = context.actions.requested_tool_confirmations["test-function-call-id"]
    ...
    def test_request_confirmation_without_function_call_id_raises(self, mock_invocation_context):
...
class TestContextMemoryMethods:
    ...
    async def test_add_session_to_memory_success(self, mock_invocation_context):
        ...
        memory_service.add_session_to_memory.assert_called_once_with(mock_invocation_context.session)

    async def test_add_session_to_memory_no_service_raises(self, mock_invocation_context):
        ...
        with pytest.raises(
            ValueError,
            match=(r"Cannot add session to memory: memory service is not available\."),
        ):
            await context.add_session_to_memory()
    ...
    async def test_search_memory_success(self, mock_invocation_context, mocker):
        ...
        memory_service = AsyncMock()
        mock_search_response = mocker.create_autospec(SearchMemoryResponse, instance=True)
    ...
    async def test_add_events_to_memory_no_service_raises(self, mock_invocation_context):
    ...
    async def test_add_memory_forwards_metadata(self, mock_invocation_context):
        ...
        memory_service = AsyncMock()
        mock_invocation_context.memory_service = memory_service
        memories = [MemoryEntry(content=types.Content(parts=[types.Part(text="fact one")]))]
        metadata = {"ttl": "6000s"}
    ...
    @pytest.mark.asyncio
    async def test_add_memory_accepts_memory_entries(self, mock_invocation_context):
        """Tests that add_memory forwards MemoryEntry inputs unchanged."""
        memory_service = AsyncMock()
        mock_invocation_context.memory_service = memory_service
        memory_entry = MemoryEntry(content=types.Content(parts=[types.Part(text="fact one")]))
    ...
    async def test_add_memory_no_service_raises(self, mock_invocation_context):
        ...
        with pytest.raises(
            ...
            await context.add_memory(memories=[MemoryEntry(content=types.Content(parts=[types.Part(text="fact one")]))])
