...
async def _create_readonly_context(agent: LlmAgent, state: dict[str, Any] | None = None) -> ReadonlyContext:
...
def test_canonical_model_inherit():
    ...
    parent_agent = LlmAgent(name="parent_agent", model="gemini-pro", sub_agents=[sub_agent])
...
async def test_canonical_instruction_str():
    ...
    canonical_instruction, bypass_state_injection = await agent.canonical_instruction(ctx)
...
async def test_canonical_global_instruction_str():
    ...
    canonical_instruction, bypass_state_injection = await agent.canonical_global_instruction(ctx)
...
async def test_canonical_global_instruction():
    ...
    agent = LlmAgent(name="test_agent", global_instruction=_global_instruction_provider)
    ctx = await _create_readonly_context(agent, state={"state_var": "state_value"})

    canonical_global_instruction, bypass_state_injection = await agent.canonical_global_instruction(ctx)
...
def test_validate_generate_content_config_thinking_config_throw():
    with pytest.raises(ValueError):
        _ = LlmAgent(
            name="test_agent",
            generate_content_config=types.GenerateContentConfig(thinking_config=types.ThinkingConfig()),
        )
...
def test_validate_generate_content_config_tools_throw():
    with pytest.raises(ValueError):
        _ = LlmAgent(
            name="test_agent",
            generate_content_config=types.GenerateContentConfig(tools=[types.Tool(function_declarations=[])]),
        )
...
def test_validate_generate_content_config_system_instruction_throw():
    with pytest.raises(ValueError):
        _ = LlmAgent(
            name="test_agent",
            generate_content_config=types.GenerateContentConfig(system_instruction="system instruction"),
        )
