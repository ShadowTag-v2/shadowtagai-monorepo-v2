...
def test_three_level_nested_agent():
    ...
    middle_agent1 = SequentialAgent(name="middle1", description="First middle agent", sub_agents=[leaf_agent1])
...
def test_clone_with_sub_agents_update():
    ...
    cloned = original.clone(update={"name": "cloned_agent", "sub_agents": [new_sub1, new_sub2]})
...
def test_clone_shallow_copies_lists_with_sub_agents():
    ...
    assert original_sub_agent.before_agent_callback is not cloned_sub_agent.before_agent_callback
    assert original_sub_agent.after_agent_callback is not cloned_sub_agent.after_agent_callback
    assert original_sub_agent.before_model_callback is not cloned_sub_agent.before_model_callback
    assert original_sub_agent.after_model_callback is not cloned_sub_agent.after_model_callback
    assert original_sub_agent.before_tool_callback is not cloned_sub_agent.before_tool_callback
    assert original_sub_agent.after_tool_callback is not cloned_sub_agent.after_tool_callback
    ...
    _check_lists_contain_same_contents(tools, original_sub_agent.tools, cloned_sub_agent.tools)
