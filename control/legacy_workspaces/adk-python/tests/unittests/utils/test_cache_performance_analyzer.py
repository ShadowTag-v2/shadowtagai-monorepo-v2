...
class TestCachePerformanceAnalyzer:
    ...
    def create_cache_metadata(self, invocations_used=1, cache_name="test-cache", contents_count=5):
        """Helper to create test CacheMetadata."""
        return CacheMetadata(
            cache_name=(f"projects/test/locations/us-central1/cachedContents/{cache_name}"),
    ...
    def create_mock_usage_metadata(self, prompt_tokens=1000, cached_tokens=500, candidates_tokens=100):
    ...
    def create_mock_event(self, author="test_agent", cache_metadata=None, usage_metadata=None):
    ...
    async def test_get_agent_cache_history_empty_session(self):
        ...
        result = await self.analyzer._get_agent_cache_history("test_session", "test_user", "test_app", "test_agent")
    ...
    async def test_get_agent_cache_history_all_agents(self):
        ...
        result = await self.analyzer._get_agent_cache_history("test_session", "test_user", "test_app", None)
    ...
    async def test_analyze_agent_cache_performance_no_cache_data(self):
        ...
        result = await self.analyzer.analyze_agent_cache_performance("test_session", "test_user", "test_app", "test_agent")
    ...
    async def test_analyze_agent_cache_performance_with_cache_data(self):
        ...
        usage1 = self.create_mock_usage_metadata(prompt_tokens=1000, cached_tokens=800)
        usage2 = self.create_mock_usage_metadata(prompt_tokens=1500, cached_tokens=1200)
        usage3 = self.create_mock_usage_metadata(prompt_tokens=800, cached_tokens=0)

        events = [
            self.create_mock_event(author="test_agent", cache_metadata=cache1, usage_metadata=usage1),
            self.create_mock_event(author="other_agent", cache_metadata=cache2),
            self.create_mock_event(author="test_agent", cache_metadata=cache2, usage_metadata=usage2),
            self.create_mock_event(author="test_agent", cache_metadata=cache3, usage_metadata=usage3),
        ]
        ...
        assert result["requests_with_cache_hits"] == 2  # Only first two have cached tokens
        ...
        assert abs(result["cache_utilization_ratio_percent"] - expected_utilization) < 0.01
        assert abs(result["avg_cached_tokens_per_request"] - expected_avg_cached) < 0.01

    async def test_analyze_agent_cache_performance_single_cache(self):
        """Test analysis with single cache instance."""
        cache = self.create_cache_metadata(invocations_used=10, cache_name="single_cache")
        usage = self.create_mock_usage_metadata(prompt_tokens=2000, cached_tokens=1500)

        events = [
            self.create_mock_event(author="test_agent", cache_metadata=cache, usage_metadata=usage),
        ]
    ...
    async def test_analyze_agent_cache_performance_zero_invocations(self):
        ...
        cache = self.create_cache_metadata(invocations_used=0, cache_name="zero_cache")
        usage = self.create_mock_usage_metadata(prompt_tokens=1000, cached_tokens=500)
    ...
    async def test_session_service_integration(self):
        ...
        events = [
            self.create_mock_event(author="integration_agent", cache_metadata=cache_metadata),
        ]
    ...
    async def test_mixed_agents_filtering(self):
        ...
        target_cache = self.create_cache_metadata(invocations_used=3, cache_name="target")
        other_cache = self.create_cache_metadata(invocations_used=5, cache_name="other")

        target_usage = self.create_mock_usage_metadata(prompt_tokens=1000, cached_tokens=800)
        other_usage = self.create_mock_usage_metadata(prompt_tokens=2000, cached_tokens=1600)
        ...
        result = await self.analyzer.analyze_agent_cache_performance("test_session", "test_user", "test_app", "target_agent")
