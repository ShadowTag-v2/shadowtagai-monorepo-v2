...
class TestStreamingResponseAggregator:
  ...
  async def test_process_response_with_text(self):
    ...
    response = types.GenerateContentResponse(
      candidates=[
        types.Candidate(content=types.Content(parts=[types.Part(text="Hello")]))
      ]
    )
    results = []
  ...
  async def test_process_response_with_thought(self):
    ...
    response = types.GenerateContentResponse(
      candidates=[
        types.Candidate(
          content=types.Content(parts=[types.Part(text="Thinking...", thought=True)])
        )
      ]
    )
    results = []
  ...
  async def test_process_response_multiple(self):
    ...
    response2 = types.GenerateContentResponse(
      candidates=[
        types.Candidate(content=types.Content(parts=[types.Part(text="World!")]))
      ]
    )
  ...
  async def test_process_response_interleaved_thought_and_text(self):
    ...
    response2 = types.GenerateContentResponse(
      candidates=[
        types.Candidate(
          content=types.Content(parts=[types.Part(text="Okay, I have a result.")])
        )
      ]
    )
    response3 = types.GenerateContentResponse(
      candidates=[
        types.Candidate(
          content=types.Content(parts=[types.Part(text=" The result is 42.")])
        )
      ]
    )
