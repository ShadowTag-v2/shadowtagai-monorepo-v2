...
class LocalEvalSamplerConfig(BaseModel):
  ...
  train_eval_case_ids: list[str] | None = Field(
    default=None,
    description=(
      "The ids of the eval cases to use for optimization. If not provided, all eval cases in the train_eval_set will be used."
    ),
  )
...
class LocalEvalSampler(Sampler[UnstructuredSamplingResult]):
  ...
  def __init__(
    ...
    self._train_eval_case_ids = (
      self._config.train_eval_case_ids or self._get_eval_case_ids(self._train_eval_set)
    )
  ...
  def _get_eval_case_ids(self, eval_set_id: str) -> list[str]:
    ...
    if eval_set:
      ...
      raise ValueError(
        f"Eval set `{eval_set_id}` does not exist for app `{self._config.app_name}`."
      )
      ...
      for per_invocation_result in eval_result.eval_metric_result_per_invocation:
        ...
        if per_invocation_result.expected_invocation:
          per_invocation_result_dict["expected_invocation"] = (
            extract_single_invocation_info(per_invocation_result.expected_invocation)
          )
