...
class SqliteSessionService(BaseSessionService):
    ...
    async def create_session(
        ...
        *,
        ...
        async with self._get_db_connection() as db:
            ...
            if user_state_delta:
                await self._upsert_user_state(db, app_name, user_id, user_state_delta, now)
          ...
          """,
            ...
            merged_state = _merge_state(storage_app_state, storage_user_state, session_state)
            return Session(
                app_name=app_name,
                user_id=user_id,
                id=session_id,
        ...
        async with self._get_db_connection() as db:
            ...
            if user_id:
                session_rows = await db.execute_fetchall(
                    "SELECT id, user_id, state, update_time FROM sessions WHERE app_name=? AND user_id=?",
                    (app_name, user_id),
                )
            else:
                session_rows = await db.execute_fetchall(
    ...
    async def append_event(self, session: Session, event: Event) -> Event:
        ...
        async with self._get_db_connection() as db:
            ...
            if event.actions and event.actions.state_delta:
                ...
                if app_state_delta:
                    await self._upsert_app_state(db, session.app_name, app_state_delta, event_timestamp)
                if user_state_delta:
    ...
    async def _get_state(self, db: aiosqlite.Connection, query: str, params: tuple) -> dict[str, Any]:
        """Fetches and deserializes a JSON state column from a single row."""
        async with db.execute(query, params) as cursor:
            row = await cursor.fetchone()
            return json.loads(row["state"]) if row else {}

    async def _get_app_state(self, db: aiosqlite.Connection, app_name: str) -> dict[str, Any]:
        return await self._get_state(db, "SELECT state FROM app_states WHERE app_name=?", (app_name,))

    async def _get_user_state(self, db: aiosqlite.Connection, app_name: str, user_id: str) -> dict[str, Any]:
    ...
    async def _upsert_app_state(self, db: aiosqlite.Connection, app_name: str, delta: dict, now: float) -> None:
    ...
    async def _update_session_state_in_db(
        ...
        await db.execute(
            "UPDATE sessions SET state=json_patch(state, ?), update_time=? WHERE app_name=? AND user_id=? AND id=?",
            (
                json.dumps(delta),
                now,
                app_name,
    ...
    def _is_migration_needed(self) -> bool:
        ...
        try:
            ...
            raise RuntimeError(f"Error accessing database {self._db_path}: {e}") from e
