...
def _row_to_event(row: dict) -> Event:
  ...
  if long_running_tool_ids_json:
    try:
      ...
      logger.warning(
        f"Failed to decode long_running_tool_ids_json for event {row.get('id')}"
      )
      long_running_tool_ids = set()
  ...
  with SourceSession() as source_session, DestSession() as dest_session:
    ...
    try:
      ...
      if inspector.has_table("user_states"):
        rows = (
          source_session.execute(text("SELECT * FROM user_states")).mappings().all()
        )
        for row in rows:
          dest_session.merge(
        ...
        for row in rows:
          try:
            ...
            new_event = dss.StorageEvent(
              ...
              timestamp=datetime.fromtimestamp(event_obj.timestamp, UTC).replace(
                tzinfo=None
              ),
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(
    description=("Migrate ADK sessions from SQLAlchemy Pickle format to JSON format.")
  )
  parser.add_argument(
