...
def get_gcp_exporters(
  ...
  if not project_id:
    logger.warning(
      "Cannot determine GCP Project. OTel GCP Exporters cannot be set up. Please make sure to log into correct GCP Project."
    )
    return OTelHooks()
...
def get_gcp_resource(project_id: str | None = None) -> Resource:
  ...
  """
  resource = Resource(
    attributes={"gcp.project_id": project_id} if project_id is not None else {}
  )
