...
def get_content_capturing_mode() -> str:
    return os.getenv(OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT, "").upper()
...
def _to_finish_reason(
    ...
    if (
        ...
        finish_reason is types.FinishReason.FINISH_REASON_UNSPECIFIED or finish_reason is types.FinishReason.OTHER
    ):
        return "error"
...
def _to_part(part: types.Part, idx: int) -> Part | None:
    ...
    if data := part.inline_data:
        return Blob(mime_type=data.mime_type or "", data=data.data or b"", type="blob")
...
def _to_system_instructions(
    ...
    parts = (_to_part(part, idx) for idx, part in enumerate(sys_instr.parts or []))
...
async def set_operation_details_attributes_from_request(
    ...
    input_messages = _to_input_messages(transformers.t_contents(llm_request.contents))
...
def set_operation_details_attributes_from_response(
    ...
    if finish_reason := llm_response.finish_reason:
        operation_details_common_attributes[GEN_AI_RESPONSE_FINISH_REASONS] = [_to_finish_reason(finish_reason)]
    if usage_metadata := llm_response.usage_metadata:
        if usage_metadata.prompt_token_count is not None:
            operation_details_common_attributes[GEN_AI_USAGE_INPUT_TOKENS] = usage_metadata.prompt_token_count
        if usage_metadata.candidates_token_count is not None:
            operation_details_common_attributes[GEN_AI_USAGE_OUTPUT_TOKENS] = usage_metadata.candidates_token_count
...
def maybe_log_completion_details(
    ...
    if capturing_mode in ["EVENT_ONLY", "SPAN_AND_EVENT"]:
        ...
        final_attributes = final_attributes | _operation_details_attributes_no_content(operation_details_attributes)
    ...
    if capturing_mode in ["SPAN_ONLY", "SPAN_AND_EVENT"]:
        ...
        for key, value in _operation_details_attributes_no_content(operation_details_attributes).items():
            span.set_attribute(key, _safe_json_serialize_no_whitespaces(value))
