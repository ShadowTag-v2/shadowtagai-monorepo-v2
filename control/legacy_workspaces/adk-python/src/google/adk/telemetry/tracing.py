...
def trace_agent_invocation(span: trace.Span, agent: BaseAgent, ctx: InvocationContext) -> None:
...
def trace_tool_call(
    ...
    if function_response_event is not None and function_response_event.content is not None and function_response_event.content.parts:
...
def trace_send_data(
    ...
    if _should_add_request_response_to_spans():
        span.set_attribute(
            ...
            _safe_json_serialize([types.Content(role=content.role, parts=content.parts).model_dump(exclude_none=True) for content in data]),
        )
    else:
...
def _should_add_request_response_to_spans() -> bool:
    disabled_via_env_var = os.getenv(ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS, "true").lower() in ("false", "0")
