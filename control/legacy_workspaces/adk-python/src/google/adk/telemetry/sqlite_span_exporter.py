...
class SqliteSpanExporter(SpanExporter):
        ...
        try:
            with self._lock:
                ...
                for span in spans:
                    ...
                    session_id = attributes.get("gcp.vertex.agent.session_id") or attributes.get("gen_ai.conversation.id")
    ...
    def get_all_spans_for_session(self, session_id: str) -> list[ReadableSpan]:
        ...
        rows = self._query(
            f"SELECT * FROM spans WHERE trace_id IN ({placeholders}) ORDER BY start_time_unix_nano",
            trace_ids,
        )
