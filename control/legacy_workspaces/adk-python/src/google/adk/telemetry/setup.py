...
def _get_otel_exporters() -> OTelHooks:
    ...
    metric_readers = []
    if os.getenv(otel_env.OTEL_EXPORTER_OTLP_ENDPOINT) or os.getenv(otel_env.OTEL_EXPORTER_OTLP_METRICS_ENDPOINT):
        metric_readers.append(_get_otel_metrics_exporter())

    log_record_processors = []
    if os.getenv(otel_env.OTEL_EXPORTER_OTLP_ENDPOINT) or os.getenv(otel_env.OTEL_EXPORTER_OTLP_LOGS_ENDPOINT):
