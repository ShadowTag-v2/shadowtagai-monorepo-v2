...
class InMemoryEvalSetsManager(EvalSetsManager):
  ...
  def add_eval_case(self, app_name: str, eval_set_id: str, eval_case: EvalCase):
    ...
    if eval_case.eval_id in self._eval_cases[app_name][eval_set_id]:
      raise ValueError(
        f"EvalCase {eval_case.eval_id} already exists in EvalSet {eval_set_id} for app {app_name}."
      )
  ...
  def update_eval_case(
    ...
    if updated_eval_case.eval_id not in self._eval_cases[app_name][eval_set_id]:
      raise NotFoundError(
        f"EvalCase {updated_eval_case.eval_id} not found in EvalSet {eval_set_id} for app {app_name}."
      )

    # Full replace
    self._eval_cases[app_name][eval_set_id][updated_eval_case.eval_id] = (
      updated_eval_case
    )
  ...
  def delete_eval_case(self, app_name: str, eval_set_id: str, eval_case_id: str):
    ...
    if eval_case_id not in self._eval_cases[app_name][eval_set_id]:
      raise NotFoundError(
        f"EvalCase {eval_case_id} not found in EvalSet {eval_set_id} for app {app_name}."
      )
