...
class AgentEvaluator:
  ...
  async def evaluate_eval_set(
    ...
    if not print_detailed_results:
      failure_message += " If you looking to get more details on the failures, then please re-run this test with `print_detailed_results` set to `True`."
    failure_message += "\n" + "\n".join(failures)
  ...
  def _validate_input(eval_dataset, criteria):
    ...
    if not isinstance(sample, list) and not isinstance(first_query, dict):
      raise ValueError(
        f"Each evaluation dataset sample must be list of dictionary. But it's {eval_dataset}"
      )
    ...
    if RESPONSE_EVALUATION_SCORE_KEY in criteria:
      if QUERY_COLUMN not in first_query:
        raise ValueError(
          f"Samples for {RESPONSE_EVALUATION_SCORE_KEY} must include '{QUERY_COLUMN}' key. The sample is {sample}."
        )
  ...
  def _print_details(
    ...
    print(
      f"Summary: `{overall_eval_status}` for Metric: `{metric_name}`. Expected threshold: `{threshold}`, actual value: `{overall_score}`."
    )
    ...
    print(
      tabulate(pd.DataFrame(data), headers="keys", tablefmt="grid", maxcolwidths=25)
    )
  ...
  async def _get_agent_for_eval(
    ...
    if not (hasattr(agent_module, "agent") or module_name.endswith(".agent")):
      raise ValueError(
        f"Module {module_name} does not have a member named `agent` or the name should endwith `.agent`."
      )
    ...
    if hasattr(agent_module_with_agent, "root_agent"):
      ...
      raise ValueError(
        f"Module {module_name} does not have a root_agent or get_agent_async method."
      )
  ...
  def _process_metrics_and_get_failures(
    ...
    for (
      ...
      if overall_eval_status != EvalStatus.PASSED:
        ...
        failures.append(
          f"{metric_name} for {agent_module} Failed. Expected {threshold}, but got {overall_score}."
        )
