...
class ResponseEvaluator(Evaluator):
    ...
    def __init__(
        ...
        if (threshold is not None and eval_metric) or (metric_name is not None and eval_metric):
            raise ValueError("Either eval_metric should be specified or both threshold and metric_name should be specified.")
    ...
    def get_metric_info(metric_name: str) -> MetricInfo:
        ...
        if PrebuiltMetrics.RESPONSE_EVALUATION_SCORE.value == metric_name:
            return MetricInfo(
                ...
                metric_value_info=MetricValueInfo(interval=Interval(min_value=1.0, max_value=5.0)),
            )
        elif PrebuiltMetrics.RESPONSE_MATCH_SCORE.value == metric_name:
    ...
    def evaluate_invocations(
        ...
        if self._metric_name == PrebuiltMetrics.RESPONSE_MATCH_SCORE.value:
            rouge_evaluator = RougeEvaluator(EvalMetric(metric_name=self._metric_name, threshold=self._threshold))
            return rouge_evaluator.evaluate_invocations(actual_invocations, expected_invocations)
