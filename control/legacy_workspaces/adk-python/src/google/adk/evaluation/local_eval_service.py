...
class LocalEvalService(BaseEvalService):
  ...
  async def perform_inference(
    ...
    if not eval_set:
      raise NotFoundError(
        f"Eval set with id {inference_request.eval_set_id} not found for app {inference_request.app_name}"
      )
  ...
  async def _evaluate_single_inference_result(
    ...
    for eval_metric in evaluate_config.eval_metrics:
      ...
      if evaluation_result.overall_eval_status != EvalStatus.NOT_EVALUATED and len(
