...
class HallucinationsV1Evaluator(Evaluator):
  ...
  def _create_context_for_step(
    ...
    for event in events:
      ...
      tool_responses = [
        part.function_response for part in event.content.parts if part.function_response
      ]
  ...
  async def _evaluate_nl_response(
    ...
    segmenter_llm_request = LlmRequest(
      ...
      contents=[
        genai_types.Content(
          parts=[
            genai_types.Part(text=self.segmenter_prompt.format(response=nl_response))
          ],
          role="user",
        )
      ],
