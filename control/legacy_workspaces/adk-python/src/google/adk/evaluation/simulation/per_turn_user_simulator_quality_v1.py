...
from .per_turn_user_simulator_quality_prompts import (
...
class PerTurnUserSimulatorQualityV1(Evaluator):
  ...
  def _deserialize_criterion(self, eval_metric: EvalMetric) -> BaseCriterion:
    expected_criterion_type_error = ValueError(
      f"`{eval_metric.metric_name}` metric expects a criterion of type `{self.criterion_type}`."
    )
    try:
  ...
  def _format_llm_prompt(
    ...
    if previous_invocations is None:
      raise ValueError(
        f"Previous invocations should have a set value when formatting the LLM prompt. Encountered: {previous_invocations}"
      )

    if conversation_scenario is None:
      raise ValueError(
        f"Conversation scenario should have a set value when formatting the LLM prompt. Encountered: {conversation_scenario}"
      )
