...
class InMemoryMemoryService(BaseMemoryService):
    ...
    async def add_session_to_memory(self, session: Session):
        ...
        with self._lock:
            ...
            self._session_events[user_key][session.id] = [event for event in session.events if event.content and event.content.parts]
        ...
        for session_events in session_event_lists.values():
            for event in session_events:
                if not event.content or not event.content.parts:
                    continue
                words_in_event = _extract_words_lower(" ".join([part.text for part in event.content.parts if part.text]))
