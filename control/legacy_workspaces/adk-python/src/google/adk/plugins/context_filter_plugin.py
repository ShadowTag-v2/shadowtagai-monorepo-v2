...
class ContextFilterPlugin(BasePlugin):
    ...
    def __init__(
        self,
        num_invocations_to_keep: int | None = None,
        custom_filter: Callable[[list[Event]], list[Event]] | None = None,
    ...
    async def before_model_callback(self, *, callback_context: CallbackContext, llm_request: LlmRequest) -> LlmResponse | None:
        ...
        try:
            ...
            if self._num_invocations_to_keep is not None and self._num_invocations_to_keep > 0:
                    ...
                    for i in range(len(contents) - 1, -1, -1):
                        if contents[i].role == "model":
                            ...
                            if model_turns_to_find == 0:
                                start_index = i
                                while start_index > 0 and contents[start_index - 1].role == "user":
                                    start_index -= 1
