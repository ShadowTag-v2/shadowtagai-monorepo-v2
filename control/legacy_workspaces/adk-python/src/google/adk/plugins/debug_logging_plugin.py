...
class DebugLoggingPlugin(BasePlugin):
    ...
    def _serialize_content(self, content: types.Content | None) -> dict[str, Any] | None:
    ...
    def _add_entry(
        ...
        **data: Any,
        ...
        if invocation_id not in self._invocation_states:
            logger.warning("No debug state for invocation %s, skipping entry", invocation_id)
            return
    ...
    async def on_event_callback(self, *, invocation_context: InvocationContext, event: Event) -> Event | None:
        ...
        if event.actions:
            ...
            if event.actions.state_delta:
                actions_data["state_delta"] = self._safe_serialize(event.actions.state_delta)
            ...
            if event.actions.requested_auth_configs:
                actions_data["requested_auth_configs"] = len(event.actions.requested_auth_configs)
            if actions_data:
    ...
    async def after_run_callback(self, *, invocation_context: InvocationContext) -> None:
        ...
        if invocation_id not in self._invocation_states:
            logger.warning("No debug state for invocation %s, skipping write", invocation_id)
            return
    ...
    async def after_model_callback(self, *, callback_context: CallbackContext, llm_response: LlmResponse) -> LlmResponse | None:
        ...
        if llm_response.usage_metadata:
            response_data["usage_metadata"] = {
                ...
                "cached_content_token_count": (llm_response.usage_metadata.cached_content_token_count),
            }
