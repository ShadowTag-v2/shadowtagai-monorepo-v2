    ...
    **Example:**
    ...
    async def after_tool_callback(
        ...
        *,
        ...
        """
        if isinstance(result, dict) and result.get("response_type") == REFLECT_AND_RETRY_RESPONSE_TYPE:
            return None

        error = await self.extract_error_from_result(tool=tool, tool_args=tool_args, tool_context=tool_context, result=result)
    ...
    async def _handle_tool_error(
        ...
        async with self._lock:
            tool_failure_counter = self._scoped_failure_counters.setdefault(scope_key, {})
            current_retries = tool_failure_counter.get(tool.name, 0) + 1
            tool_failure_counter[tool.name] = current_retries

            if current_retries <= self.max_retries:
                return self._create_tool_reflection_response(tool, tool_args, error, current_retries)
    ...
    async def _reset_failures_for_tool(self, tool_context: ToolContext, tool_name: str) -> None:
