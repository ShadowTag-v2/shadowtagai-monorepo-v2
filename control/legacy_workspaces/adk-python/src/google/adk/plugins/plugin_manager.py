...
class PluginManager:
    ...
    async def run_before_run_callback(self, *, invocation_context: InvocationContext) -> types.Content | None:
        """Runs the `before_run_callback` for all plugins."""
        return await self._run_callbacks("before_run_callback", invocation_context=invocation_context)

    async def run_after_run_callback(self, *, invocation_context: InvocationContext) -> Optional[None]:
        """Runs the `after_run_callback` for all plugins."""
        return await self._run_callbacks("after_run_callback", invocation_context=invocation_context)

    async def run_on_event_callback(self, *, invocation_context: InvocationContext, event: Event) -> Event | None:
    ...
    async def run_before_agent_callback(self, *, agent: BaseAgent, callback_context: CallbackContext) -> types.Content | None:
    ...
    async def run_after_agent_callback(self, *, agent: BaseAgent, callback_context: CallbackContext) -> types.Content | None:
    ...
    async def run_before_model_callback(self, *, callback_context: CallbackContext, llm_request: LlmRequest) -> LlmResponse | None:
    ...
    async def run_after_model_callback(self, *, callback_context: CallbackContext, llm_response: LlmResponse) -> LlmResponse | None:
    ...
    async def _run_callbacks(self, callback_name: PluginCallbackName, **kwargs: Any) -> Any | None:
        ...
        for plugin in self.plugins:
            ...
            try:
                ...
                error_message = f"Error in plugin '{plugin.name}' during '{callback_name}' callback: {e}"
    ...
    async def close(self) -> None:
        ...
        for plugin in self.plugins:
            try:
                ...
                if isinstance(e, (asyncio.TimeoutError, asyncio.CancelledError)):
                    logger.warning("Timeout/Cancelled while closing plugin: %s", plugin.name)
                else:
                    logger.error(
        ...
        if exceptions:
            error_summary = ", ".join(f"'{name}': {type(exc).__name__}" for name, exc in exceptions.items())
