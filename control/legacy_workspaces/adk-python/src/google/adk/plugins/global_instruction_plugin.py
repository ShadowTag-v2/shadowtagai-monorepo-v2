...
class GlobalInstructionPlugin(BasePlugin):
    ...
    async def before_model_callback(self, *, callback_context: CallbackContext, llm_request: LlmRequest) -> LlmResponse | None:
        ...
        final_global_instruction = await self._resolve_global_instruction(callback_context)
        ...
        if isinstance(existing_instruction, str):
            llm_request.config.system_instruction = f"{final_global_instruction}\n\n{existing_instruction}"
    ...
    async def _resolve_global_instruction(self, readonly_context: ReadonlyContext) -> str:
        ...
        if isinstance(self.global_instruction, str):
            ...
            return await instructions_utils.inject_session_state(self.global_instruction, readonly_context)
        else:
