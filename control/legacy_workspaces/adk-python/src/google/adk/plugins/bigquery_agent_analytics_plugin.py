...
def to_arrow_schema(bq_schema_list):
    ...
    for bq_field in bq_schema_list:
        ...
        if af:
            ...
            logging.warning("Failed to convert schema due to field '%s'.", bq_field.name)
            return None
...
class BigQueryLoggerConfig:
    ...
    content_formatter: Callable[[Any], str] | None = None
...
def _format_content(content: types.Content | None, max_len: int = 500) -> tuple[str, bool]:
    ...
    for p in content.parts:
        if p.text:
            parts.append(f"text: '{p.text[:max_len]}...' " if len(p.text) > max_len else f"text: '{p.text}'")
            ...
            parts.append("other")
    return " | ".join(parts), any(len(p.text) > max_len for p in content.parts if p.text)


def _format_args(args: dict[str, Any], *, max_len: int = 1000) -> tuple[str, bool]:
...
class BigQueryAgentAnalyticsPlugin(BasePlugin):
        ...
        **kwargs,
        ...
        self._schema = [
            ...
            bigquery.SchemaField(
                ...
                "BOOLEAN",
                mode="NULLABLE",
                description=("Indicates if the content field was truncated due to size limits."),
            ),
        ]

    def _format_content_safely(self, content: types.Content | None) -> tuple[str | None, bool]:
    ...
    async def _ensure_init(self):
        ...
        async with self._init_lock:
            ...
            try:
                ...
                client_info = gapic_client_info.ClientInfo(user_agent=f"google-adk-bq-logger/{version.__version__}")
                self._bq_client = bigquery.Client(project=self._project_id, credentials=creds, client_info=client_info)
    ...
    async def _perform_write(self, row: dict):
        ...
        try:
            ...
            req.arrow_rows.writer_schema.serialized_schema = self._arrow_schema.serialize().to_pybytes()
            req.arrow_rows.rows.serialized_record_batch = batch.serialize().to_pybytes()

            # Write with protection against immediate cancellation
            async for resp in await asyncio.shield(self._write_client.append_rows(iter([req]))):
                if resp.error.code != 0:
                    msg = resp.error.message
                    # Check for common schema mismatch indicators
                    if "schema mismatch" in msg.lower() or "field" in msg.lower() or "type" in msg.lower():
                        logging.error(
    ...
    async def _log(self, data: dict):
        ...
        if self._config.event_denylist and event_type in self._config.event_denylist:
            return
        if self._config.event_allowlist and event_type not in self._config.event_allowlist:
            return
    ...
    async def close(self):
        ...
        if self._background_tasks:
            logging.info("BQ Plugin: Flushing %s pending logs...", len(self._background_tasks))
            try:
                await asyncio.wait(self._background_tasks, timeout=self._config.shutdown_timeout)
            except TimeoutError:
    ...
    async def before_model_callback(self, *, callback_context: CallbackContext, llm_request: LlmRequest) -> None:
        ...
        if llm_request.tools_dict:
            content_parts.append(f"Available Tools: {list(llm_request.tools_dict.keys())}")
    ...
    async def after_model_callback(self, *, callback_context: CallbackContext, llm_response: LlmResponse) -> None:
        ...
        if is_tool_call:
            fc_names = []
            if content and content.parts:
                fc_names = [part.function_call.name for part in content.parts if part.function_call]
            content_parts.append(f"Tool Name: {', '.join(fc_names)}")
        else:
            text_content, truncated = self._format_content_safely(llm_response.content)
        ...
        if llm_response.usage_metadata:
            ...
            total_tokens = getattr(llm_response.usage_metadata, "total_token_count", "N/A")
            token_usage_str = f"Token Usage: {{prompt: {prompt_tokens}, candidates: {candidates_tokens}, total: {total_tokens}}}"
    ...
    async def before_tool_callback(
        ...
        *,
        ...
        If the content length exceeds `max_content_length`, it is truncated.
        """
        args_str, truncated = _format_args(tool_args, max_len=self._config.max_content_length)
        content = f"Tool Name: {tool.name}, Description: {tool.description}, Arguments: {args_str}"
    ...
    async def after_tool_callback(
        ...
        *,
        ...
        If the content length exceeds `max_content_length`, it is truncated.
        """
        result_str, truncated = _format_args(result, max_len=self._config.max_content_length)
