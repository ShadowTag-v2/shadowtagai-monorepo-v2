...
def _format_content(
  content: types.Content | None, max_len: int = 500
) -> tuple[str, bool]:
...
class BigQueryAgentAnalyticsPlugin(BasePlugin):
    ...
    **kwargs,
    ...
    self._schema = [
      ...
      bigquery.SchemaField(
        ...
        mode="NULLABLE",
        description=(
          "Indicates the type of event being logged (e.g., 'LLM_REQUEST', 'TOOL_COMPLETED')."
        ),
      ),
      bigquery.SchemaField(
        "agent",
      ...
      bigquery.SchemaField(
        ...
        mode="NULLABLE",
        description=(
          "A unique identifier to group events within a single conversation or user session."
        ),
      ),
      bigquery.SchemaField(
        "invocation_id",
        ...
        mode="NULLABLE",
        description=(
          "A unique identifier for each individual agent execution or turn within a session."
        ),
      ),
      bigquery.SchemaField(
        "user_id",
      ...
      bigquery.SchemaField(
        ...
        mode="NULLABLE",
        description=(
          "Populated if an error occurs during the processing of the event."
        ),
      ),
      bigquery.SchemaField(
        "is_truncated",
        ...
        mode="NULLABLE",
        description=(
          "Indicates if the content field was truncated due to size limits."
        ),
      ),
    ]
  ...
  async def after_model_callback(
    ...
    if is_tool_call:
      ...
      if content and content.parts:
        fc_names = [
          part.function_call.name for part in content.parts if part.function_call
        ]
  ...
  async def before_tool_callback(
    ...
    *,
    ...
    content = (
      f"Tool Name: {tool.name}, Description: {tool.description}, Arguments: {args_str}"
    )
