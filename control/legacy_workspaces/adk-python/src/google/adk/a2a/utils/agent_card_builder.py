...
class AgentCardBuilder:
    ...
    async def build(self) -> AgentCard:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to build agent card for {self._agent.name}: {e}") from e
...
async def _build_sub_agent_skills(agent: BaseAgent) -> list[AgentSkill]:
    ...
    for sub_agent in agent.sub_agents:
        try:
            ...
            print(f"Warning: Failed to build skills for sub-agent {sub_agent.name}: {e}")
            continue
...
def _build_orchestration_skill(agent: BaseAgent, agent_type: str) -> AgentSkill | None:
...
def _get_workflow_description(agent: BaseAgent) -> str | None:
...
def _get_input_modes(agent: BaseAgent) -> list[str] | None:
...
def _get_output_modes(agent: BaseAgent) -> list[str] | None:
    """Get output modes from Agent.generate_content_config.response_modalities."""
    if not isinstance(agent, LlmAgent):
        return None

    if hasattr(agent, "generate_content_config") and agent.generate_content_config and hasattr(agent.generate_content_config, "response_modalities"):
