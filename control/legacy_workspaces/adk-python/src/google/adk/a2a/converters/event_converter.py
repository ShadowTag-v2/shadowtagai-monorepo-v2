    ...
    try:
        ...
        for field_name, field_value in optional_fields:
            if field_value is not None:
                metadata[_get_adk_metadata_key(field_name)] = _serialize_metadata_value(field_value)
...
def _process_long_running_tool(a2a_part: A2APart, event: Event) -> None:
    ...
    if (
        ...
        and a2a_part.root.metadata.get(_get_adk_metadata_key(A2A_DATA_PART_METADATA_TYPE_KEY)) == A2A_DATA_PART_METADATA_TYPE_FUNCTION_CALL
        and a2a_part.root.data.get("id") in event.long_running_tool_ids
    ):
        a2a_part.root.metadata[_get_adk_metadata_key(A2A_DATA_PART_METADATA_IS_LONG_RUNNING_KEY)] = True
    ...
    try:
        ...
        for a2a_part in a2a_message.parts:
            try:
                ...
                if a2a_part.root.metadata and a2a_part.root.metadata.get(_get_adk_metadata_key(A2A_DATA_PART_METADATA_IS_LONG_RUNNING_KEY)) is True:
                    for part in parts:
...
def convert_event_to_a2a_message(
    ...
    try:
        ...
        if output_parts:
            return Message(message_id=str(uuid.uuid4()), role=role, parts=output_parts)
    ...
    return TaskStatusUpdateEvent(
        ...
        status=TaskStatus(
            ...
            message=Message(
                ...
                metadata={_get_adk_metadata_key("error_code"): str(event.error_code)} if event.error_code else {},
            ),
...
def _create_status_update_event(
    ...
    if any(
        part.root.metadata.get(_get_adk_metadata_key(A2A_DATA_PART_METADATA_TYPE_KEY)) == A2A_DATA_PART_METADATA_TYPE_FUNCTION_CALL
        and part.root.metadata.get(_get_adk_metadata_key(A2A_DATA_PART_METADATA_IS_LONG_RUNNING_KEY)) is True
...
def convert_event_to_a2a_events(
    ...
    try:
        ...
        message = convert_event_to_a2a_message(event, invocation_context, part_converter=part_converter)
        if message:
