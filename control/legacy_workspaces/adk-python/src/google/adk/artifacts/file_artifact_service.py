...
def _resolve_scoped_artifact_path(scope_root: Path, filename: str) -> tuple[Path, Path]:
  ...
  if pure_path.is_absolute():
    raise InputValidationError(
      f"Absolute artifact filename {filename!r} is not permitted; provide a path relative to the storage scope."
    )
  ...
  try:
    ...
    raise InputValidationError(
      f"Artifact filename {filename!r} escapes storage directory {scope_root_resolved}"
    ) from exc
