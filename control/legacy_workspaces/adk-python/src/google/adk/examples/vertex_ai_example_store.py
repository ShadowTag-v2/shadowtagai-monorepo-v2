    ...
    def get_examples(self, query: str) -> list[Example]:
        ...
        for result in response.results:
            if result.similarity_score < 0.5:
                continue
            expected_contents = [content.content for content in result.example.stored_contents_example.contents_example.expected_contents]
            expected_output = []
            for content in expected_contents:
                ...
                expected_output.append(types.Content(role=content.role, parts=expected_parts))

            returned_examples.append(
                Example(
                    input=types.Content(
                        role="user",
                        parts=[types.Part.from_text(text=result.example.stored_contents_example.search_key)],
                    ),
