...
class _NlPlanningRequestProcessor(BaseLlmRequestProcessor):
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event, None]:
        ...
        if isinstance(planner, BuiltInPlanner):
            ...
            if planning_instruction := planner.build_planning_instruction(ReadonlyContext(invocation_context), llm_request):
...
class _NlPlanningResponse(BaseLlmResponseProcessor):
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_response: LlmResponse) -> AsyncGenerator[Event, None]:
        ...
        processed_parts = planner.process_planning_response(callback_context, llm_response.content.parts)
        if processed_parts:
