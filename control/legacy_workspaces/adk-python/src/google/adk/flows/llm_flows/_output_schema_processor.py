...
class _OutputSchemaRequestProcessor(BaseLlmRequestProcessor):
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event, None]:
        ...
        # schema with tools
        if not agent.output_schema or not agent.tools or can_use_output_schema_with_tools(agent.model):
            return
...
def create_final_model_response_event(invocation_context: InvocationContext, json_response: str) -> Event:
    ...
    final_event.content = types.Content(role="model", parts=[types.Part(text=json_response)])
    return final_event
