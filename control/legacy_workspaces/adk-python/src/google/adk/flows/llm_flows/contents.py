...
def _rearrange_events_for_latest_function_response(
  ...
  if function_call_event_idx == -1:
    ...
    raise ValueError(
      f"No function call event found for function responses ids: {function_responses_ids}"
    )
...
def _process_compaction_events(events: list[Event]) -> list[Event]:
  ...
  for event in reversed(events):
    if event.actions and event.actions.compaction:
      ...
      if (
        compaction.start_timestamp is not None and compaction.end_timestamp is not None
      ):
...
def _get_contents(
  current_branch: str | None, events: list[Event], agent_name: str = ""
) -> list[types.Content]:
...
def _is_other_agent_reply(current_agent_name: str, event: Event) -> bool:
  ...
  return bool(
    current_agent_name and event.author != current_agent_name and event.author != "user"
  )
  ...
  for part in event.content.parts:
    if part.thought:
      ...
      content.parts.append(
        types.Part(
          text=(
            f"[{event.author}] called tool `{part.function_call.name}` with parameters: {part.function_call.args}"
          )
        )
      )
      ...
      content.parts.append(
        types.Part(
          text=(
            f"[{event.author}] `{part.function_response.name}` tool returned result: {part.function_response.response}"
          )
        )
      )
