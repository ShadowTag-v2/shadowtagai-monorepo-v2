...
class _RequestConfirmationLlmRequestProcessor(BaseLlmRequestProcessor):
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event, None]:
        ...
        request_confirmation_function_responses = dict()  # {function call id, tool confirmation}
        ...
        for k in range(len(events) - 1, -1, -1):
            ...
            for function_response in responses:
                ...
                # confirmation
                if function_response.response and len(function_response.response.values()) == 1 and "response" in function_response.response.keys():
                    # ADK web client will send a request that is always encapsulated in a
                    # 'response' key.
                    tool_confirmation = ToolConfirmation.model_validate(json.loads(function_response.response["response"]))
                else:
                    tool_confirmation = ToolConfirmation.model_validate(function_response.response)
                request_confirmation_function_responses[function_response.id] = tool_confirmation
        ...
        for i in range(len(events) - 2, -1, -1):
            ...
            tools_to_resume_with_confirmation = dict()  # {Function call id, tool confirmation}
            tools_to_resume_with_args = dict()  # {Function call id, function calls}

            for function_call in function_calls:
                if function_call.id not in request_confirmation_function_responses.keys():
                    continue

                args = function_call.args
                if "originalFunctionCall" not in args:
                    continue
                original_function_call = types.FunctionCall(**args["originalFunctionCall"])
                tools_to_resume_with_confirmation[original_function_call.id] = request_confirmation_function_responses[function_call.id]
                tools_to_resume_with_args[original_function_call.id] = original_function_call
            ...
            if function_response_event := await functions.handle_function_call_list_async(
                ...
                {tool.name: tool for tool in await agent.canonical_tools(ReadonlyContext(invocation_context))},
