...
class ContextCacheRequestProcessor(BaseLlmRequestProcessor):
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event, None]:
        ...
        latest_cache_metadata, previous_token_count = self._find_cache_info_from_events(
    ...
    def _find_cache_info_from_events(
    ...
    ) -> tuple[CacheMetadata | None, int | None]:
        ...
        for i in range(len(events) - 1, -1, -1):
            ...
            if cache_metadata is None and event.cache_metadata is not None:
                ...
                if event.invocation_id and event.invocation_id != current_invocation_id and event.cache_metadata.cache_name is not None:
                    # Different invocation with active cache - increment invocations_used
                    cache_metadata = event.cache_metadata.model_copy(update={"invocations_used": event.cache_metadata.invocations_used + 1})
                else:
            ...
            if previous_token_count is None and event.usage_metadata and event.usage_metadata.prompt_token_count is not None:
