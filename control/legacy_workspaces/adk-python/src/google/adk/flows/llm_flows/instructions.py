...
class _InstructionsLlmRequestProcessor(BaseLlmRequestProcessor):
    ...
    async def _process_agent_instruction(self, agent, invocation_context: InvocationContext) -> str:
        ...
        """
        raw_si, bypass_state_injection = await agent.canonical_instruction(ReadonlyContext(invocation_context))
        si = raw_si
        if not bypass_state_injection:
            si = await instructions_utils.inject_session_state(raw_si, ReadonlyContext(invocation_context))
        return si
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event, None]:
        ...
        if isinstance(root_agent, LlmAgent) and root_agent.global_instruction:
            raw_si, bypass_state_injection = await root_agent.canonical_global_instruction(ReadonlyContext(invocation_context))
            si = raw_si
            if not bypass_state_injection:
                si = await instructions_utils.inject_session_state(raw_si, ReadonlyContext(invocation_context))
