...
class AudioCacheManager:
  ...
  async def _flush_cache_to_services(
    ...
    try:
      ...
      filename = (
        f"adk_live_audio_storage_{cache_type}_{timestamp}.{mime_type.split('/')[-1]}"
      )
      ...
      audio_event = Event(
        ...
        content=types.Content(
          ...
          parts=[
            types.Part(
              file_data=types.FileData(file_uri=artifact_ref, mime_type=mime_type)
            )
          ],
        ),
  ...
  def get_cache_stats(self, invocation_context: InvocationContext) -> dict[str, int]:
    ...
    input_bytes = sum(
      len(entry.data.data) for entry in (invocation_context.input_realtime_cache or [])
    )
    output_bytes = sum(
      len(entry.data.data) for entry in (invocation_context.output_realtime_cache or [])
    )
