...
class AudioCacheManager:
    ...
    def cache_audio(
        ...
        audio_entry = RealtimeCacheEntry(role=role, data=audio_blob, timestamp=time.time())
    ...
    async def _flush_cache_to_services(
        ...
        try:
            ...
            combined_audio_part = types.Part(inline_data=types.Blob(data=combined_audio_data, mime_type=mime_type))
            ...
            audio_event = Event(
                ...
                content=types.Content(
                    ...
                    parts=[types.Part(file_data=types.FileData(file_uri=artifact_ref, mime_type=mime_type))],
                ),
    ...
    def get_cache_stats(self, invocation_context: InvocationContext) -> dict[str, int]:
        ...
        input_bytes = sum(len(entry.data.data) for entry in (invocation_context.input_realtime_cache or []))
        output_bytes = sum(len(entry.data.data) for entry in (invocation_context.output_realtime_cache or []))
