...
class _CodeExecutionRequestProcessor(BaseLlmRequestProcessor):
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event, None]:
        ...
        async with Aclosing(_run_pre_processor(invocation_context, llm_request)) as agen:
...
class _CodeExecutionResponseProcessor(BaseLlmResponseProcessor):
    ...
    @override
    async def run_async(self, invocation_context: InvocationContext, llm_response: LlmResponse) -> AsyncGenerator[Event, None]:
        # Skip if the response is partial (streaming).
        if llm_response.partial:
            return

        async with Aclosing(_run_post_processor(invocation_context, llm_response)) as agen:
            async for event in agen:
                yield event
...
async def _run_pre_processor(
    ...
    for file in files_to_process:
        ...
        execution_result_event = await _post_process_code_execution_result(invocation_context, code_executor_context, code_execution_result)
...
async def _run_post_processor(
    ...
    code_str = CodeExecutionUtils.extract_code_and_truncate_content(response_content, code_executor.code_block_delimiters)
    ...
    yield await _post_process_code_execution_result(invocation_context, code_executor_context, code_execution_result)
...
def _extract_and_replace_inline_files(
    ...
    for i in range(len(llm_request.contents)):
        ...
        for j in range(len(content.parts)):
            ...
            llm_request.contents[i].parts[j] = types.Part(text="\nAvailable file: `%s`\n" % file_name)
...
async def _post_process_code_execution_result(
    ...
    result_content = types.Content(
        role="model",
        parts=[
            CodeExecutionUtils.build_code_execution_result_part(code_execution_result),
        ],
    )
