...
class CacheMetadata(BaseModel):
  ...
  invocations_used: int | None = Field(
    ...
    ge=0,
    description=(
      "Number of invocations this cache has been used for (None if no active cache)"
    ),
  )
  ...
  contents_count: int = Field(
    ge=0,
    description=(
      "Number of contents (cached contents when active cache exists, total contents in request when no active cache)"
    ),
  )
  ...
  def __str__(self) -> str:
    ...
    if self.cache_name is None:
      return f"Fingerprint-only: {self.contents_count} contents, fingerprint={self.fingerprint[:8]}..."
