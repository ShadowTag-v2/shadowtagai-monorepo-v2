  ...
  for part in parts:
    if part.text:
      ...
      if part.inline_data.mime_type.startswith("image"):
        ...
        raise ValueError(
          f"LiteLlm(BaseLlm) does not support content part with MIME type {part.inline_data.mime_type}."
        )
...
def _to_litellm_response_format(
  ...
  if isinstance(response_schema, dict):
    ...
    if (
      isinstance(schema_type, str) and schema_type.lower() in _LITELLM_STRUCTURED_TYPES
    ):
...
def _warn_gemini_via_litellm(model_string: str) -> None:
  ...
  if os.environ.get("ADK_SUPPRESS_GEMINI_LITELLM_WARNINGS", "").strip().lower() in (
    "1",
    "true",
    "yes",
    "on",
