...
class GeminiContextCacheManager:
    ...
    async def handle_context_caching(self, llm_request: LlmRequest) -> CacheMetadata | None:
        ...
        if llm_request.cache_metadata:
            ...
            if await self._is_cache_valid(llm_request):
                ...
                fingerprint_for_all = self._generate_cache_fingerprint(llm_request, total_contents_count)
    ...
    async def _create_new_cache_with_contents(self, llm_request: LlmRequest, cache_contents_count: int) -> CacheMetadata | None:
