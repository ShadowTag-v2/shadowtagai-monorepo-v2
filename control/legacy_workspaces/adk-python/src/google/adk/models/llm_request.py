...
def _find_tool_with_function_declarations(
    ...
    return next(
        (tool for tool in llm_request.config.tools if isinstance(tool, types.Tool) and tool.function_declarations),
        None,
    )
...
class LlmRequest(BaseModel):
  ...
  """
    ...
    cacheable_contents_token_count: int | None = None
    ...
    def append_instructions(self, instructions: list[str] | types.Content) -> list[types.Content]:
        ...
        if isinstance(instructions, types.Content):
            ...
            for part in instructions.parts:
                if part.text:
                    ...
                    reference_text = f"[Reference to inline binary data: {reference_id}{display_text}]"
                    ...
                    user_content = types.Content(
                        role="user",
                        parts=[
                            types.Part.from_text(text=f"Referenced inline data: {reference_id}"),
                    ...
                    display_text = f" ({', '.join(display_info)})" if display_info else ""
                    reference_text = f"[Reference to file data: {reference_id}{display_text}]"
                    text_parts.append(reference_text)
                    ...
                    user_content = types.Content(
                        role="user",
                        parts=[
                            types.Part.from_text(text=f"Referenced file data: {reference_id}"),
            ...
            if text_parts:
                ...
                if not self.config.system_instruction:
                    ...
                    logging.warning(
                        "Cannot append to system_instruction of unsupported type: %s. Only string system_instruction is supported.",
        ...
        if isinstance(instructions, list) and all(isinstance(inst, str) for inst in instructions):
    ...
    def append_tools(self, tools: list[BaseTool]) -> None:
        ...
        if declarations:
            ...
            if tool_with_function_declarations := _find_tool_with_function_declarations(self):
                if tool_with_function_declarations.function_declarations is None:
                    tool_with_function_declarations.function_declarations = []
                tool_with_function_declarations.function_declarations.extend(declarations)
            else:
