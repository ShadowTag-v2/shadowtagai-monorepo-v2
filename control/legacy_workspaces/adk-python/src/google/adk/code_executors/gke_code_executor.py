...
class GkeCodeExecutor(BaseCodeExecutor):
  ...
  def _add_owner_reference(
    ...
    try:
      ...
      logger.info(
        f"Set Job '{owner_job.metadata.name}' as owner of ConfigMap '{configmap_name}'."
      )
    except ApiException as e:
      logger.warning(
        f"Failed to set ownerReference on ConfigMap '{configmap_name}'. Manual cleanup is required. Reason: {e.reason}"
      )
