...
class APIHubClient(BaseAPIHubClient):
    ...
    def get_spec_content(self, path: str) -> str:
        ...
        """
        apihub_resource_name, api_version_resource_name, api_spec_resource_name = self._extract_resource_name(path)

        if apihub_resource_name and not api_version_resource_name:
            api = self.get_api(apihub_resource_name)
            versions = api.get("versions", [])
            if not versions:
                raise ValueError(f"No versions found in API Hub resource: {apihub_resource_name}")
    ...
    def _extract_resource_name(self, url_or_path: str) -> tuple[str, str, str]:
        ...
        api_version_resource_name = f"{api_resource_name}/versions/{version_id}" if version_id else None
        api_spec_resource_name = f"{api_version_resource_name}/specs/{spec_id}" if version_id and spec_id else None
    ...
    def _get_access_token(self) -> str:
        ...
        if not credentials:
            raise ValueError("Please provide a service account or an access token to API Hub client.")
