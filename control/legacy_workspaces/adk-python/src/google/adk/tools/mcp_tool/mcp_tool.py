...
try:
    ...
    if sys.version_info < (3, 10):
        raise ImportError("MCP Tool requires Python 3.10 or above. Please upgrade your Python version.") from e
    else:
        raise e
...
class McpTool(BaseAuthenticatedTool):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            auth_config=AuthConfig(auth_scheme=auth_scheme, raw_auth_credential=auth_credential) if auth_scheme else None,
        )
    ...
    async def _invoke_callable(self, target: Callable[..., Any], args_to_call: dict[str, Any]) -> Any:
        ...
        is_async = inspect.iscoroutinefunction(target) or (hasattr(target, "__call__") and inspect.iscoroutinefunction(target.__call__))
        if is_async:
    ...
    async def run_async(self, *, args: dict[str, Any], tool_context: ToolContext) -> Any:
        if isinstance(self._require_confirmation, Callable):
            require_confirmation = await self._invoke_callable(self._require_confirmation, args)
        else:
    ...
    @retry_on_errors
    @override
    async def _run_async_impl(self, *, args, tool_context: ToolContext, credential: AuthCredential) -> dict[str, Any]:
    ...
    async def _get_headers(self, tool_context: ToolContext, credential: AuthCredential) -> dict[str, str] | None:
        ...
        if credential:
            if credential.oauth2:
                ...
                if credential.http.scheme.lower() == "bearer" and credential.http.credentials.token:
                    headers = {"Authorization": f"Bearer {credential.http.credentials.token}"}
                elif credential.http.scheme.lower() == "basic":
                    # Handle basic auth
                    if credential.http.credentials.username and credential.http.credentials.password:
                ...
                if not self._credentials_manager or not self._credentials_manager._auth_config:
                    error_msg = f"Cannot find corresponding auth scheme for API key credential {credential}"
                    logger.error(error_msg)
                    raise ValueError(error_msg)
                elif self._credentials_manager._auth_config.auth_scheme.in_ != APIKeyIn.header:
                    error_msg = (
                    ...
                    headers = {self._credentials_manager._auth_config.auth_scheme.name: (credential.api_key)}
            elif credential.service_account:
                # Service accounts should be exchanged for access tokens before reaching this point
                logger.warning("Service account credentials should be exchanged before MCP session creation")
