...
class InjectionConfig(BaseModel):
  ...
  def check_injected_error_or_response(self) -> Self:
    ...
    if bool(self.injected_error) == bool(self.injected_response):
      raise ValueError(
        "Either injected_error or injected_response must be set, but not both, and not neither."
      )
    return self
...
class ToolSimulationConfig(BaseModel):
  ...
  def check_mock_strategy_type(self) -> Self:
    ...
    if (
      ...
      raise ValueError(
        "If injection_configs is empty, mock_strategy_type cannot be MOCK_STRATEGY_UNSPECIFIED."
      )
    return self
