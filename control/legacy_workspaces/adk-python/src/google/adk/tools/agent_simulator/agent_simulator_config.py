...
class InjectionConfig(BaseModel):
  ...
  the interceptor is applied as after tool callback."""
    ...
    injected_error: InjectedError | None = None
    """The injected error."""

    injected_response: dict[str, Any] | None = None
    ...
    def check_injected_error_or_response(self) -> Self:
        ...
        if bool(self.injected_error) == bool(self.injected_response):
            raise ValueError("Either injected_error or injected_response must be set, but not both, and not neither.")
        return self
...
class ToolSimulationConfig(BaseModel):
  ...
  the mock_strategy will be applied if no injection config is hit."""
    ...
    def check_mock_strategy_type(self) -> Self:
        ...
        if not self.injection_configs and self.mock_strategy_type == MockStrategy.MOCK_STRATEGY_UNSPECIFIED:
            raise ValueError("If injection_configs is empty, mock_strategy_type cannot be MOCK_STRATEGY_UNSPECIFIED.")
        return self
...
class AgentSimulatorConfig(BaseModel):
    ...
    tool_simulation_configs: list[ToolSimulationConfig] = Field(default_factory=list)
    ...
    def check_tool_simulation_configs(cls, v: list[ToolSimulationConfig]):
        ...
        for tool_sim_config in v:
            if tool_sim_config.tool_name in seen_tool_names:
                raise ValueError(f"Duplicate tool_name found: {tool_sim_config.tool_name}")
