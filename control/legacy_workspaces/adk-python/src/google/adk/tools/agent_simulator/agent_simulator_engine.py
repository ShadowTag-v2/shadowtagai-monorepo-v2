...
class AgentSimulatorEngine:
    ...
    def __init__(self, config: AgentSimulatorConfig):
        self._config = config
        self._tool_sim_configs = {c.tool_name: c for c in config.tool_simulation_configs}
        self._is_analyzed = False
        self._tool_connection_map: ToolConnectionMap | None = None
    ...
    async def simulate(self, tool: BaseTool, args: dict[str, Any], tool_context: Any) -> dict[str, Any] | None:
        ...
        if not self._is_analyzed and any(
            c.mock_strategy_type != MockStrategyEnum.MOCK_STRATEGY_UNSPECIFIED for c in self._config.tool_simulation_configs
        ):
        ...
        for injection_config in tool_sim_config.injection_configs:
            if injection_config.match_args:
                if not all(item in args.items() for item in injection_config.match_args.items()):
                    continue

            if injection_config.random_seed is not None:
                self._random_generator.seed(injection_config.random_seed)

            if self._random_generator.random() < injection_config.injection_probability:
                time.sleep(injection_config.injected_latency_seconds)
                if injection_config.injected_error:
                    return {
                        "error_code": (injection_config.injected_error.injected_http_error_code),
        ...
        if tool_sim_config.mock_strategy_type == MockStrategyEnum.MOCK_STRATEGY_UNSPECIFIED:
            agent_simulator_logger.warning(
                "Tool '%s' did not hit any injection config and has no mock strategy configured. Returning no-op.",
                tool.name,
            )
            return None
