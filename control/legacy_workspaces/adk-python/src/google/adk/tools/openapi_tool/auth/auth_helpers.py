...
def openid_dict_to_scheme_credential(
  ...
  missing_fields = [
    field for field in required_credential_fields if field not in credential_dict
  ]
  if missing_fields:
    raise ValueError(
      f"Missing required fields in credential_dict: {', '.join(missing_fields)}"
    )
...
def openid_url_to_scheme_credential(
  ...
  try:
    ...
    raise ValueError(
      f"Invalid JSON response from OpenID configuration endpoint {openid_url}: {e}"
    ) from e
