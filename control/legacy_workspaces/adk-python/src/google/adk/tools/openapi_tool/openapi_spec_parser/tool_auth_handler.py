...
from ..auth.credential_exchangers.auto_auth_credential_exchanger import (
  AutoAuthCredentialExchanger,
)
from ..auth.credential_exchangers.base_credential_exchanger import (
  ...
  def _request_credential(self) -> None:
    ...
    if self.auth_scheme.type_ in (
      ...
      if not self.auth_credential or not self.auth_credential.oauth2:
        raise ValueError(
          f"auth_credential is empty for scheme {self.auth_scheme.type_}.Please create AuthCredential using OAuth2Auth."
        )
