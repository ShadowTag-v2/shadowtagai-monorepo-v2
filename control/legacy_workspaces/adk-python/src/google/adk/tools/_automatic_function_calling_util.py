...
def build_function_declaration(
  ...
  if should_update_signature:
    new_params = [
      param for name, param in signature.parameters.items() if name not in ignore_params
    ]
    ...
    for name, param in inspect.signature(func).parameters.items():
      if param.kind in (
        ...
        try:
          ...
          json_schema_dict = (
            _function_parameter_parse_util._generate_json_schema_for_parameter(param)
          )
  ...
  try:
    ...
    try:
      response_json_schema = (
        _function_parameter_parse_util._generate_json_schema_for_parameter(return_value)
      )
