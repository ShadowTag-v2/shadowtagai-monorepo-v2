...
def _raise_for_invalid_enum_value(param: inspect.Parameter):
  ...
  if inspect.isclass(param.annotation) and issubclass(param.annotation, Enum):
    if param.default is not inspect.Parameter.empty and param.default not in [
      ...
      raise ValueError(
        f"Default value {param.default} is not a valid enum value for {param.annotation}."
      )
...
def _is_default_value_compatible(
  ...
  if (
    ...
    if origin in (Union, typing_types.UnionType):
      return any(
        _is_default_value_compatible(default_value, arg) for arg in get_args(annotation)
      )
    ...
    if origin is list:
      ...
      return all(
        any(_is_default_value_compatible(item, arg) for arg in get_args(annotation))
...
def _parse_schema_from_parameter(
  ...
  if inspect.isclass(param.annotation) and issubclass(param.annotation, ToolContext):
    raise ValueError(
      f"`ToolContext` parameter must be named as `tool_context`. Found `{param.name}` instead in function `{func_name}`."
    )
