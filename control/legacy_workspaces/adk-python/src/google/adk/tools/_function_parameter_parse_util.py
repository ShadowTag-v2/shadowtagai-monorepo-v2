...
def _add_unevaluated_items_to_fixed_len_tuple_schema(
    ...
    if (
        ...
        and (json_schema.get("prefixItems") and len(json_schema["prefixItems"]) == json_schema["maxItems"])
...
def _raise_for_invalid_enum_value(param: inspect.Parameter):
    ...
    if inspect.isclass(param.annotation) and issubclass(param.annotation, Enum):
        if param.default is not inspect.Parameter.empty and param.default not in [e.value for e in param.annotation]:
            raise ValueError(f"Default value {param.default} is not a valid enum value for {param.annotation}.")
...
def _generate_json_schema_for_parameter(
    ...
    json_schema_dict = _add_unevaluated_items_to_fixed_len_tuple_schema(json_schema_dict)
...
def _raise_for_any_of_if_mldev(schema: types.Schema):
    if schema.any_of:
        raise ValueError("AnyOf is not supported in function declaration schema for Google AI.")
...
def _update_for_default_if_mldev(schema: types.Schema):
    if schema.default is not None:
        ...
        schema.default = None
        logger.warning("Default value is not supported in function declaration schema for Google AI.")
...
def _is_default_value_compatible(default_value: Any, annotation: inspect.Parameter.annotation) -> bool:
    ...
    if isinstance(annotation, _GenericAlias) or isinstance(annotation, typing_types.GenericAlias) or isinstance(annotation, typing_types.UnionType):
        ...
        if origin is list:
            ...
            return all(any(_is_default_value_compatible(item, arg) for arg in get_args(annotation)) for item in default_value)
...
def _parse_schema_from_parameter(variant: GoogleLLMVariant, param: inspect.Parameter, func_name: str) -> types.Schema:
    ...
    if (
        ...
        and all((_is_builtin_primitive_or_compound(arg) or arg is type(None)) for arg in get_args(param.annotation))
    ):
        ...
        _raise_if_schema_unsupported(variant, schema)
        return schema
    if isinstance(param.annotation, _GenericAlias) or isinstance(param.annotation, typing_types.GenericAlias):
        ...
        if origin is Literal:
            if not all(isinstance(arg, str) for arg in args):
                raise ValueError(f"Literal type {param.annotation} must be a list of strings.")
            schema.type = types.Type.STRING
        ...
        if origin is Union:
            ...
            for arg in args:
                ...
                if len(param.annotation.__args__) == 2 and type(None) in param.annotation.__args__:  # Optional type
    ...
    if (
        ...
        _raise_if_schema_unsupported(variant, schema)
        return schema
    if inspect.isclass(param.annotation) and issubclass(param.annotation, ToolContext):
        raise ValueError(f"`ToolContext` parameter must be named as `tool_context`. Found `{param.name}` instead in function `{func_name}`.")
...
def _get_required_fields(schema: types.Schema) -> list[str]:
    if not schema.properties:
        return
    return [field_name for field_name, field_schema in schema.properties.items() if not field_schema.nullable and field_schema.default is None]
