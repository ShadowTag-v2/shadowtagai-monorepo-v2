...
class GoogleApiToOpenApiConverter:
  ...
  def _convert_servers(self) -> None:
    """Convert server information."""
    base_url = self._google_api_spec.get("rootUrl", "") + self._google_api_spec.get(
      "servicePath", ""
    )
  ...
  def _convert_methods(self, methods: dict[str, Any], resource_path: str) -> None:
    ...
    for method_name, method_data in methods.items():
      ...
      self._openapi_spec["paths"][rest_path][http_method] = self._convert_operation(
        method_data, path_params
      )
...
def main():
  ...
  parser = argparse.ArgumentParser(
    description=("Convert Google API Discovery documents to OpenAPI v3 specifications")
  )
