...
class SpannerToolset(BaseToolset):
  ...
  def __init__(
    ...
    *,
    ...
    self._tool_settings = (
      spanner_tool_settings if spanner_tool_settings else SpannerToolSettings()
    )
  ...
  async def get_tools(
    ...
    # Query tools
    if (
      self._tool_settings and Capabilities.DATA_READ in self._tool_settings.capabilities
    ):
      all_tools.append(
        GoogleTool(
