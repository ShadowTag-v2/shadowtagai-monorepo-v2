...
def _get_embedding_for_query(
  ...
  spanner_embedding_model_name: str | None,
...
def _generate_sql_for_ann(
  ...
  if dialect == DatabaseDialect.POSTGRESQL:
    raise NotImplementedError(
      f"{_APPROXIMATE_NEAREST_NEIGHBORS} is not supported for PostgreSQL dialect."
    )
...
def similarity_search(
  ...
  try:
    ...
    if (
      ...
      raise ValueError(
        f"embedding_options['{_SPANNER_EMBEDDING_MODEL_NAME}'] must be specified for GoogleSQL dialect."
      )
    ...
    if (
      ...
      raise ValueError(
        f"embedding_options['{_VERTEX_AI_EMBEDDING_MODEL_ENDPOINT}'] must be specified for PostgreSQL dialect."
      )
    ...
    if nearest_neighbors_algorithm not in (
      ...
      raise NotImplementedError(
        f"Unsupported search_options['{_NEAREST_NEIGHBORS_ALGORITHM}']: {nearest_neighbors_algorithm}"
      )
