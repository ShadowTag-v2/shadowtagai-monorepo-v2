...
class ExampleTool(BaseTool):
  ...
  async def process_llm_request(
    ...
    llm_request.append_instructions(
      [example_util.build_example_si(self.examples, parts[0].text, llm_request.model)]
    )
  ...
  def from_config(
    ...
    if isinstance(example_tool_config.examples, str):
      ...
      raise ValueError(
        "Example tool config must be a list of examples or a fully-qualified name to a BaseExampleProvider object in code."
      )
