...
try:
    ...
    if sys.version_info < (3, 10):
        raise ImportError("Crewai Tools require Python 3.10+. Please upgrade your Python version.") from e
    else:
        raise ImportError("Crewai Tools require pip install 'google-adk[extensions]'.") from e
...
You could retry calling this tool, but it is IMPORTANT for you to provide all the mandatory parameters."""
    ...
    @override
    @classmethod
    def from_config(cls: type[CrewaiTool], config: ToolArgsConfig, config_abs_path: str) -> CrewaiTool:
        from ..agents import config_agent_utils

        crewai_tool_config = CrewaiToolConfig.model_validate(config.model_dump())
        tool = config_agent_utils.resolve_fully_qualified_name(crewai_tool_config.tool)
