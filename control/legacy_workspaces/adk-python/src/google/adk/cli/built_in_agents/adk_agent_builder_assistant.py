...
class AgentBuilderAssistant:
  ...
  def create_agent(
    ...
    agent = LlmAgent(
      ...
      description=(
        "Intelligent assistant for building ADK multi-agent systems using YAML configurations"
      ),
  ...
  def _load_instruction_with_schema(
    ...
    model_str = (
      str(model) if isinstance(model, str) else getattr(model, "model_name", str(model))
    )
