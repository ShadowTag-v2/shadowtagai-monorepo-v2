...
class AgentBuilderAssistant:
    ...
    def create_agent(
        ...
        custom_tools = [
            ...
            FunctionTool(write_config_files),  # Write/validate multiple YAML configs
    ...
    def _build_schema_reference(schema: dict[str, Any]) -> str:
        ...
        add(
            "before_agent_callbacks / after_agent_callbacks: list of CodeConfig entries that run before or after the agent loop.",
            indent=4,
        )
        add("model: string model id (required in practice).", indent=4)
        add(
            "disallow_transfer_to_parent / disallow_transfer_to_peers: booleans to restrict automatic transfer.",
            indent=4,
        )
        add(
            "input_schema / output_schema: JSON schema objects to validate inputs and outputs.",
            indent=4,
        )
        ...
        add(
            "before_model_callbacks / after_model_callbacks: list of CodeConfig entries around LLM calls.",
            indent=4,
        )
        add(
            "before_tool_callbacks / after_tool_callbacks: list of CodeConfig entries around tool calls.",
            indent=4,
        )
        add(
        ...
        add("ArgumentConfig")
        add(
            "Represents a single argument. value is required and may be any JSON type. name is optional (null allowed). Often used in callback args.",
            indent=2,
        )
    ...
    def _extract_core_schema(schema: dict[str, Any]) -> dict[str, Any]:
        ...
        if gen_config:
            ...
            filtered_defs["GenerateContentConfig"] = {
                ...
                "description": ("Common LLM generation knobs exposed by the Agent Builder."),
                "type": "object",
                "additionalProperties": False,
                "properties": {key: properties[key] for key in AgentBuilderAssistant._GEN_CONFIG_FIELDS if key in properties},
            }
    ...
    def _load_instruction_with_schema(
        ...
        instruction_template = AgentBuilderAssistant._load_embedded_schema_instruction_template()
        schema_content = AgentBuilderAssistant._load_schema()

        # Get model string for template replacement
        model_str = str(model) if isinstance(model, str) else getattr(model, "model_name", str(model))
        ...
        def instruction_provider(context: ReadonlyContext) -> str:
            ...
            project_folder_name = AgentBuilderAssistant._extract_project_folder_name(context)
    ...
    def _load_embedded_schema_instruction_template() -> str:
        ...
        if not template_path.exists():
            raise FileNotFoundError(f"Instruction template not found at {template_path}")
