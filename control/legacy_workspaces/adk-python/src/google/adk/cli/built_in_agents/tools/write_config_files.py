  ...
  for file_path, config_content in configs.items():
    ...
    if file_result.get("success", False):
      ...
      if rename_applied and sanitized_name and sanitized_name != agent_name:
        warnings = file_result.get("warnings", [])
        warnings.append(
          f"Agent name normalized for filesystem compatibility: '{agent_name}' -> '{sanitized_name}'"
        )
        file_result["warnings"] = warnings
      ...
      if normalized_key in normalized_path_to_original:
        ...
        file_result["error"] = (
          f"Multiple agent configs target the same file path after normalization: '{conflict_source}' and '{file_path}'"
        )
        result["success"] = False
        continue
  ...
  for sub_agent in sub_agents:
    if isinstance(sub_agent, dict) and "config_path" in sub_agent:
      ...
      if not config_path.endswith((".yaml", ".yml")):
        warnings.append(
          f"Sub-agent config_path should end with .yaml or .yml: {config_path}"
        )
...
def _generate_retry_suggestion(
  ...
  for error in errors:
    ...
    if "required" in message.lower():
      ...
      suggestions.append(
        f"Use valid enum value for field '{path}' - check schema for allowed values"
      )
...
def _require_llm_agent_model(
  ...
  return {
    ...
    "error": (
      f"LlmAgent '{agent_name}' in '{file_path}' must define a 'model' field. LlmAgents cannot rely on implicit defaults."
    ),
    "file_path": str(file_path),
    "validation_step": "structure_validation",
    "retry_suggestion": (
      "Add a 'model' field with the user-confirmed model (for example, 'model: gemini-2.5-flash')."
    ),
  }
...
def _require_valid_agent_name(
  ...
  return {
    ...
    "validation_step": "structure_validation",
    "retry_suggestion": (
      "Rename the agent using only letters, digits, and underscores (e.g., 'Paper_Analyzer')."
    ),
  }
  ...
  def _normalize_reference_value(
    ...
    if value.startswith("callbacks.") or value.startswith("tools."):
      ...
      local_warnings.append(
        f"{descriptor} '{value}' updated to '{new_value}' to include project prefix."
      )
      return new_value, local_warnings, local_errors
...
def _determine_target_file_path(
  file_path: str, agent_name: str | None
...
def _sanitize_agent_name_for_filename(
  ...
  if replacements_made:
    warning = (
      f"Agent name normalized to snake_case: '{agent_name}' -> '{sanitized_name}'"
    )
