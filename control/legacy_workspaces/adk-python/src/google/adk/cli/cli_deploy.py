...
def _resolve_project(project_in_option: str | None) -> str:
...
def _validate_gcloud_extra_args(extra_gcloud_args: tuple[str, ...] | None, adk_managed_args: set[str]) -> None:
...
def _get_service_option_by_adk_version(
    ...
    if parsed_version >= parse("1.3.0"):
        session_option = f"--session_service_uri={session_uri}" if session_uri else ""
        artifact_option = f"--artifact_service_uri={artifact_uri}" if artifact_uri else ""
        ...
        artifact_option = f"--artifact_storage_uri={artifact_uri}" if artifact_uri else ""
...
def to_cloud_run(
    ...
    * agents/[app_name]/
    ...
    try:
        ...
        allow_origins_option = f"--allow_origins={','.join(allow_origins)}" if allow_origins else ""
...
def to_agent_engine(
    *,
    ...
    adk_app: str,
    staging_bucket: str,
    trace_to_cloud: bool | None = None,
    api_key: str | None = None,
    adk_app_object: str | None = None,
    ...
    agent_engine_config_file: str | None = None,
):
    ...
    if adk_app_object not in ["root_agent", "app"]:
        click.echo(f'Invalid adk_app_object: {adk_app_object}. Please use "root_agent" or "app".')
        return
    ...
    try:
        ...
        if os.path.exists(agent_engine_config_file):
            ...
            with open(agent_engine_config_file) as f:
                agent_config = json.load(f)
        if display_name:
            if "display_name" in agent_config:
                click.echo(f"Overriding display_name in agent engine config with {display_name}")
            agent_config["display_name"] = display_name
        if description:
            if "description" in agent_config:
                click.echo(f"Overriding description in agent engine config with {description}")
        ...
        if not requirements_file:
            ...
            if "requirements_file" in agent_config:
                click.echo(f"Overriding requirements in agent engine config with {requirements_file}")
        ...
        if env_vars:
            if "env_vars" in agent_config:
                click.echo(f"Overriding env_vars in agent engine config with {env_vars}")
        ...
        if project and region:
            ...
            click.echo("No project/region or api_key provided. Please specify either project/region or api_key.")
            return
        ...
        if absolutize_imports:
            click.echo("Agent Engine deployments have switched to source-based deployment, so it is no longer necessary to absolutize imports.")
...
def to_gke(
    *,
    ...
    try:
        ...
        shutil.copytree(agent_folder, agent_src_path)
        requirements_txt_path = os.path.join(agent_src_path, "requirements.txt")
        install_agent_deps = f'RUN pip install -r "/app/agents/{app_name}/requirements.txt"' if os.path.exists(requirements_txt_path) else ""
        ...
        click.echo("  (This may take a few minutes. Raw logs from gcloud will be shown below.)")
        project = _resolve_project(project)
...
"""
        ...
        click.secho("  - The following resources were applied to the cluster:", fg="green")
