...
def _validate_gcloud_extra_args(
  extra_gcloud_args: tuple[str, ...] | None, adk_managed_args: set[str]
) -> None:
...
def to_agent_engine(
  *,
  ...
  if adk_app_object not in ["root_agent", "app"]:
    click.echo(
      f'Invalid adk_app_object: {adk_app_object}. Please use "root_agent" or "app".'
    )
    return
  ...
  try:
    ...
    if display_name:
      if "display_name" in agent_config:
        click.echo(
          f"Overriding display_name in agent engine config with {display_name}"
        )
    ...
    if not requirements_file:
      ...
      if "requirements_file" in agent_config:
        click.echo(
          f"Overriding requirements in agent engine config with {requirements_file}"
        )
    ...
    if project and region:
      ...
      click.echo(
        "No project/region or api_key provided. Please specify either project/region or api_key."
      )
      return
    ...
    if absolutize_imports:
      click.echo(
        "Agent Engine deployments have switched to source-based deployment, so it is no longer necessary to absolutize imports."
      )
...
def to_gke(
  *,
  ...
  try:
    ...
    click.echo(
      "  (This may take a few minutes. Raw logs from gcloud will be shown below.)"
    )
    project = _resolve_project(project)
