...
def _make_feature_decorator(
    *,
    ...
    def decorator_factory(message_or_obj=None):
        ...
        if message_or_obj is not None and (isinstance(message_or_obj, type) or callable(message_or_obj)):
            return _create_decorator(default_message, label, block_usage, bypass_env_var)(message_or_obj)

        # Case 2: Used as @decorator() with or without message
        # message_or_obj is either None or a string message
        message = message_or_obj if isinstance(message_or_obj, str) else default_message
        return _create_decorator(message, label, block_usage, bypass_env_var)

    return decorator_factory


def _create_decorator(message: str, label: str, block_usage: bool, bypass_env_var: str | None) -> Callable[[T], T]:
    def decorator(obj: T) -> T:
        ...
        if isinstance(obj, type):  # decorating a class
            ...
            def new_init(self, *args, **kwargs):
                ...
                should_bypass = bypass_env_var is not None and _is_truthy_env(bypass_env_var)
            ...
            raise TypeError(f"@{label} can only be applied to classes or callable objects")
...
working_in_progress = _make_feature_decorator(
    label="WIP",
    default_message=("This feature is a work in progress and is not working completely. ADK users are not supposed to use it."),
    block_usage=True,
