...
class StreamingResponseAggregator:
    ...
    def __init__(self):
        ...
        self._current_text_is_thought: bool | None = None
        ...
        self._current_thought_signature: str | None = None
    ...
    def _flush_text_buffer_to_sequence(self):
        ...
        if self._current_text_buffer:
            if self._current_text_is_thought:
                self._parts_sequence.append(types.Part(text=self._current_text_buffer, thought=True))
            else:
                self._parts_sequence.append(types.Part.from_text(text=self._current_text_buffer))
            self._current_text_buffer = ""
            self._current_text_is_thought = None

    def _get_value_from_partial_arg(self, partial_arg: types.PartialArg, json_path: str):
        ...
        if partial_arg.string_value is not None:
            ...
            path_without_prefix = json_path[2:] if json_path.startswith("$.") else json_path
    ...
    async def process_response(self, response: types.GenerateContentResponse) -> AsyncGenerator[LlmResponse, None]:
    ...
    def close(self) -> LlmResponse | None:
            ...
            if self._response and self._response.candidates:
                ...
                if final_parts:
                    ...
                    return LlmResponse(
                        ...
                        error_code=None if finish_reason == types.FinishReason.STOP else finish_reason,
                        error_message=None if finish_reason == types.FinishReason.STOP else candidate.finish_message,
        ...
        # ========== Non-Progressive SSE Streaming (old behavior) ==========
        if (self._text or self._thought_text) and self._response and self._response.candidates:
            parts = []
            if self._thought_text:
                parts.append(types.Part(text=self._thought_text, thought=True))
            if self._text:
            ...
            return LlmResponse(
                content=types.ModelContent(parts=parts),
                error_code=None if candidate.finish_reason == types.FinishReason.STOP else candidate.finish_reason,
