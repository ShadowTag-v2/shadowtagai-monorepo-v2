...
class AuthHandler:
  ...
  def generate_auth_request(self) -> AuthConfig:
    ...
    if not self.auth_config.raw_auth_credential:
      raise ValueError(
        f"Auth Scheme {self.auth_config.auth_scheme.type_} requires auth_credential."
      )

    # Check if oauth2 exists in raw_auth_credential
    if not self.auth_config.raw_auth_credential.oauth2:
      raise ValueError(
        f"Auth Scheme {self.auth_config.auth_scheme.type_} requires oauth2 in auth_credential."
      )
    ...
    if (
      ...
      raise ValueError(
        f"Auth Scheme {self.auth_config.auth_scheme.type_} requires both client_id and client_secret in auth_credential.oauth2."
      )
