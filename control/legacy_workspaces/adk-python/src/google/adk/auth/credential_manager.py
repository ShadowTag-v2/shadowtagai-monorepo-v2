...
from ..tools.openapi_tool.auth.credential_exchangers.service_account_exchanger import (
  ...
  async def _validate_credential(self) -> None:
    ...
    if not self._auth_config.raw_auth_credential:
      if self._auth_config.auth_scheme.type_ in (
        ...
        raise ValueError(
          f"raw_auth_credential is required for auth_scheme type {self._auth_config.auth_scheme.type_}"
        )
    ...
    if raw_credential:
      if (
        ...
        raise ValueError(
          f"auth_config.raw_credential.oauth2 required for credential type {raw_credential.auth_type}"
        )
