...
class _AuthLlmRequestProcessor(BaseLlmRequestProcessor):
    ...
    async def run_async(self, invocation_context: InvocationContext, llm_request: LlmRequest) -> AsyncGenerator[Event, None]:
        ...
        for function_call_response in responses:
            ...
            await AuthHandler(auth_config=auth_config).parse_and_store_auth_response(state=invocation_context.session.state)
        ...
        for i in range(len(events) - 2, -1, -1):
            ...
            for j in range(i - 1, -1, -1):
                ...
                if any([function_call.id in tools_to_resume for function_call in function_calls]):
