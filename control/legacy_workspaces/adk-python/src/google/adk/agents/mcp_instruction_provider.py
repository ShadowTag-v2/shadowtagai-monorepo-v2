...
try:
    ...
    if sys.version_info < (3, 10):
        raise ImportError("MCP Session Manager requires Python 3.10 or above. Please upgrade your Python version.") from e
    else:
        raise e
...
class McpInstructionProvider(InstructionProvider):
    ...
    async def __call__(self, context: ReadonlyContext) -> str:
        ...
        if prompt_definition and prompt_definition.arguments:
            ...
            prompt_args = {k: v for k, v in (context.state or {}).items() if k in arg_names}

        # Fetch the specific prompt by name with arguments from context state
        prompt_result: types.GetPromptResult = await session.get_prompt(self.prompt_name, arguments=prompt_args)

        if prompt_result and prompt_result.messages:
            # Concatenate content of all messages to form the instruction.
            instruction = "".join(message.content.text for message in prompt_result.messages if message.content.type == "text")
            return instruction
        else:
