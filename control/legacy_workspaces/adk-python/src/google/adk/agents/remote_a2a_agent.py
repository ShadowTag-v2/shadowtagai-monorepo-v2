...
class RemoteA2aAgent(BaseAgent):
  ...
  def __init__(
    ...
    *,
    ...
    a2a_request_meta_provider: Callable[[InvocationContext, A2AMessage], dict[str, Any]]
    | None = None,
    **kwargs: Any,
  ) -> None:
    ...
    if isinstance(agent_card, AgentCard):
      ...
      raise TypeError(
        f"agent_card must be AgentCard, URL string, or file path string, got {type(agent_card)}"
      )
  ...
  async def _run_async_impl(
    ...
    try:
      ...
      async for a2a_response in self._a2a_client.send_message(
        ...
        event.custom_metadata = event.custom_metadata or {}
        event.custom_metadata[A2A_METADATA_PREFIX + "request"] = a2a_request.model_dump(
          exclude_none=True, by_alias=True
        )
        # If the response is a ClientEvent, record the task state; otherwise,
        # record the message object.
        if isinstance(a2a_response, tuple):
          event.custom_metadata[A2A_METADATA_PREFIX + "response"] = a2a_response[
            0
      ...
      yield Event(
        ...
        branch=ctx.branch,
        custom_metadata={
          A2A_METADATA_PREFIX + "request": a2a_request.model_dump(
            exclude_none=True, by_alias=True
          ),
