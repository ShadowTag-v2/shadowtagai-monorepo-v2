...
class BaseAgent(BaseModel):
  ...
  def clone(self: SelfAgent, update: Mapping[str, Any] | None = None) -> SelfAgent:
    ...
    if update is not None:
      ...
      if invalid_fields:
        raise ValueError(
          f"Cannot update nonexistent fields in {self.__class__.__name__}: {invalid_fields}"
        )
  ...
  async def _handle_before_agent_callback(self, ctx: InvocationContext) -> Event | None:
    ...
    before_agent_callback_content = await ctx.plugin_manager.run_before_agent_callback(
