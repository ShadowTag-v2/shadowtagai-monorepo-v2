...
class LlmAgent(BaseAgent):
  ...
  **Examples:**
  ...
  async def canonical_tools(self, ctx: ReadonlyContext = None) -> list[BaseTool]:
    ...
    for tool_union in self.tools:
      resolved_tools.extend(
        await _convert_tool_union_to_tools(tool_union, ctx, self.model, multiple_tools)
      )
  ...
  def _get_subagent_to_resume(self, ctx: InvocationContext) -> BaseAgent | None:
    ...
    if last_event.author == "user":
      ...
      if not function_call_event:
        raise ValueError(
          f"No agent to transfer to for resuming agent from function response {self.name}"
        )
  ...
  def validate_generate_content_config(
    cls, generate_content_config: types.GenerateContentConfig | None
