...
class _InvocationCostManager(BaseModel):
    ...
    def increment_and_enforce_llm_calls_limit(self, run_config: RunConfig | None):
        ...
        if run_config and run_config.max_llm_calls > 0 and self._number_of_llm_calls > run_config.max_llm_calls:
            # We only enforce the limit if the limit is a positive number.
            raise LlmCallsLimitExceededError(f"Max number of llm calls limit of `{run_config.max_llm_calls}` exceeded")
...
class InvocationContext(BaseModel):
  ...
  Set to True in callbacks or tools to terminate this invocation."""
    ...
    _invocation_cost_manager: _InvocationCostManager = PrivateAttr(default_factory=_InvocationCostManager)
  ...
  """
    ...
    def is_resumable(self) -> bool:
        ...
        return self.resumability_config is not None and self.resumability_config.is_resumable
    ...
    def populate_invocation_agent_states(self) -> None:
        ...
        for event in self._get_events(current_invocation=True):
            ...
            elif event.author != "user" and event.content and not self.agent_states.get(event.author):
    ...
    def increment_llm_call_count(
        ...
        Raises:
          LlmCallsLimitExceededError: If number of llm calls made exceed the set
            threshold.
        """
        self._invocation_cost_manager.increment_and_enforce_llm_calls_limit(self.run_config)
    ...
    def _get_events(
        ...
        *,
        ...
        if current_invocation:
            results = [event for event in results if event.invocation_id == self.invocation_id]
        if current_branch:
    ...
    def _find_matching_function_call(self, function_response_event: Event) -> Event | None:
