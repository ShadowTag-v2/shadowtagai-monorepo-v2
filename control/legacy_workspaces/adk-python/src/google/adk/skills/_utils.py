...
def _load_skill_from_dir(skill_dir: str | pathlib.Path) -> models.Skill:
    ...
    if skill_dir.name != frontmatter.name:
        raise ValueError(f"Skill name '{frontmatter.name}' does not match directory name '{skill_dir.name}'.")

    references = _load_dir(skill_dir / "references")
    assets = _load_dir(skill_dir / "assets")
    raw_scripts = _load_dir(skill_dir / "scripts")
    scripts = {name: models.Script(src=content) for name, content in raw_scripts.items()}
...
def _validate_skill_dir(
    ...
    if skill_dir.name != frontmatter.name:
        problems.append(f"Skill name '{frontmatter.name}' does not match directory name '{skill_dir.name}'.")
