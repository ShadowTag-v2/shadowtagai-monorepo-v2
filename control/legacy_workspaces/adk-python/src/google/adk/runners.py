...
class Runner:
  ...
  def _validate_runner_params(
    ...
    if app:
      ...
      if plugins:
        raise ValueError(
          "When app is provided, plugins should not be provided and should be provided in the app instead."
        )
      app_name = app.name
    ...
    *,
    ...
    if run_config.support_cfc and isinstance(self.agent, LlmAgent):
      ...
      if not model_name.startswith("gemini-2"):
        raise ValueError(
          f"CFC is not supported for model: {model_name} in agent: {self.agent.name}"
        )
