...
import { type IGitCommandManager, MinimumGitSparseCheckoutVersion } from "./git-command-manager";
...
export async function getSource(settings: IGitSourceSettings): Promise<void> {
  ...
  try {
    ...
    const checkoutInfo = await refHelper.getCheckoutInfo(git, settings.ref, settings.commit);
    core.endGroup();
...
export async function cleanup(repositoryPath: string): Promise<void> {
  // Repo exists?
  if (!repositoryPath || !fsHelper.fileExistsSync(path.join(repositoryPath, ".git", "config"))) {
    return;
  }
  ...
  try {
    if (stateHelper.PostSetSafeDirectory) {
      ...
      await git.config("safe.directory", repositoryPath, true, true).catch((error) => {
        core.info(`Failed to initialize safe directory with error: ${error}`);
      });
    }
