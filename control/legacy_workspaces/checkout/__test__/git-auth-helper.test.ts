...
import type { IGitCommandManager } from "../lib/git-command-manager";
import type { IGitSourceSettings } from "../lib/git-source-settings";
...
describe("git-auth-helper tests", () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(stateHelper, "setSshKnownHostsPath").mockImplementation(jest.fn());
  });
  ...
  async function testAuthHeader(testName: string, serverUrl: string | undefined = undefined) {
    ...
    const localConfigContent = (await fs.promises.readFile(localGitConfigPath)).toString();
    expect(localConfigContent.indexOf("includeIf.gitdir:")).toBeGreaterThanOrEqual(0);
    ...
    const credentialsContent = (await fs.promises.readFile(credentialsConfigPath)).toString();
    const basicCredential = Buffer.from(`x-access-token:${settings.authToken}`, "utf8").toString(
      "base64",
    );
    expect(
  ...
  it(configureAuth_AcceptsGitHubServerUrlSetToGHEC, async () => {
    await testAuthHeader(configureAuth_AcceptsGitHubServerUrlSetToGHEC, "https://github.com");
  });
  ...
  it(configureAuth_configuresAuthHeaderEvenWhenPersistCredentialsFalse, async () => {
    // Arrange
    await setup(configureAuth_configuresAuthHeaderEvenWhenPersistCredentialsFalse);
    expect(settings.authToken).toBeTruthy(); // sanity check
    ...
    const credentialsContent = (await fs.promises.readFile(credentialsConfigPath)).toString();
    expect(
      credentialsContent.indexOf(`http.https://github.com/.extraheader AUTHORIZATION`),
    ).toBeGreaterThanOrEqual(0);
  });
  ...
  it(configureAuth_registersBasicCredentialAsSecret, async () => {
    ...
    const expectedSecret = Buffer.from(`x-access-token:${settings.authToken}`, "utf8").toString(
      "base64",
    );
  ...
  it(setsSshCommandEnvVarWhenPersistCredentialsFalse, async () => {
    ...
    expect(gitConfigLines.some((line) => line.indexOf("includeIf.gitdir:") >= 0)).toBeTruthy();
  });
  ...
  it(configureAuth_writesSshKeyAndImplicitKnownHosts, async () => {
    ...
    const actualSshKeyContent = (await fs.promises.readFile(actualSshKeyPath)).toString();
    ...
    if (!isWindows) {
      ...
      expect((await fs.promises.stat(actualSshKeyPath)).mode & 0o777).toBe(0o600);
    }
  ...
  it(configureGlobalAuth_copiesGlobalGitConfig, async () => {
    ...
    let configContent = (await fs.promises.readFile(globalGitConfigPath)).toString();
  ...
  it(configureGlobalAuth_createsNewGlobalGitConfigWhenGlobalDoesNotExist, async () => {
    // Arrange
    await setup(configureGlobalAuth_createsNewGlobalGitConfigWhenGlobalDoesNotExist);
  ...
  it(
    ...
    async () => {
      ...
      expect(mockSubmoduleForeach.mock.calls[0][0] as string).toMatch(/unset-all.*insteadOf/);
    },
  );
  ...
  it(
    ...
    async () => {
      ...
      expect(mockSubmoduleForeach.mock.calls[0][0]).toMatch(/unset-all.*insteadOf/);
      expect(mockSubmoduleForeach.mock.calls[1][0]).toMatch(/url.*insteadOf.*git@github.com:/);
  ...
  it(removeAuth_removesSshCommand, async () => {
    ...
    const authHelper = gitAuthHelper.createAuthHelper(git, settings);
    await authHelper.configureAuth();
    let gitConfigContent = (await fs.promises.readFile(localGitConfigPath)).toString();
  ...
  it(removeAuth_removesToken, async () => {
    ...
    let localConfigContent = (await fs.promises.readFile(localGitConfigPath)).toString();
    expect(localConfigContent.indexOf("includeIf.gitdir:")).toBeGreaterThanOrEqual(0);
    ...
    const credentialsContent = (await fs.promises.readFile(credentialsFilePath)).toString();
    const basicCredential = Buffer.from(`x-access-token:${settings.authToken}`, "utf8").toString(
      "base64",
    );
    expect(
    ...
    expect(localConfigContent.indexOf(credentialsFilePath)).toBeGreaterThanOrEqual(0);
    expect(localConfigContent.indexOf(containerCredentialsPath)).toBeGreaterThanOrEqual(0);

    // Act
    await authHelper.removeAuth();

    // Assert all includeIf entries removed from local git config
    localConfigContent = (await fs.promises.readFile(localGitConfigPath)).toString();
    expect(localConfigContent.indexOf("includeIf.gitdir:")).toBeLessThan(0);
    expect(localConfigContent.indexOf(`includeIf.gitdir:${hostGitDir}.path`)).toBeLessThan(0);
    expect(localConfigContent.indexOf("includeIf.gitdir:/github/workspace/.git.path")).toBeLessThan(
      0,
    );
  ...
  const removeAuth_removesTokenFromSubmodules = "removeAuth removes token from submodules";
  it(removeAuth_removesTokenFromSubmodules, async () => {
    ...
    const mockGetSubmoduleConfigPaths = git.getSubmoduleConfigPaths as jest.Mock<any, any>;
    mockGetSubmoduleConfigPaths.mockResolvedValue([submodule1ConfigPath, submodule2ConfigPath]);
    ...
    let submodule1Content = (await fs.promises.readFile(submodule1ConfigPath)).toString();
    const submodule1GitDir = submodule1Dir.replace(/\\/g, "/");
    expect(
      submodule1Content.indexOf(`includeIf.gitdir:${submodule1GitDir}.path`),
    ).toBeGreaterThanOrEqual(0);
    expect(submodule1Content.indexOf(credentialsFilePath)).toBeGreaterThanOrEqual(0);

    // Verify submodule 2 config has includeIf entries
    let submodule2Content = (await fs.promises.readFile(submodule2ConfigPath)).toString();
    const submodule2GitDir = submodule2Dir.replace(/\\/g, "/");
    expect(
      submodule2Content.indexOf(`includeIf.gitdir:${submodule2GitDir}.path`),
    ).toBeGreaterThanOrEqual(0);
    expect(submodule2Content.indexOf(credentialsFilePath)).toBeGreaterThanOrEqual(0);
    ...
    expect(submodule1Content.indexOf(containerCredentialsPath)).toBeGreaterThanOrEqual(0);
    expect(submodule2Content.indexOf(containerCredentialsPath)).toBeGreaterThanOrEqual(0);
    ...
    submodule1Content = (await fs.promises.readFile(submodule1ConfigPath)).toString();
    ...
    submodule2Content = (await fs.promises.readFile(submodule2ConfigPath)).toString();
  ...
  const removeGlobalConfig_removesOverride = "removeGlobalConfig removes override";
  ...
  it(testCredentialsConfigPath_matchesCredentialsConfigPaths, async () => {
    ...
    // Test invalid paths
    expect((authHelper as any).testCredentialsConfigPath("/some/path/other-config.config")).toBe(
      false,
    );
    expect(
      (authHelper as any).testCredentialsConfigPath("/some/path/git-credentials-invalid.config"),
    ).toBe(false);
    expect(
      (authHelper as any).testCredentialsConfigPath("/some/path/git-credentials-.config"),
    ).toBe(false);
...
async function setup(testName: string): Promise<void> {
  ...
  git = {
    ...
    configExists: jest.fn(async (key: string, globalConfig?: boolean): Promise<boolean> => {
    ...
    tagExists: jest.fn(),
    tryClean: jest.fn(),
    tryConfigUnset: jest.fn(async (key: string, globalConfig?: boolean): Promise<boolean> => {
      const configPath = globalConfig
        ? path.join(git.env["HOME"] || tempHomedir, ".gitconfig")
        : localGitConfigPath;
      const content = await fs.promises.readFile(configPath);
    ...
    tryGetConfigValues: jest.fn(
      async (key: string, globalConfig?: boolean, configPath?: string): Promise<string[]> => {
        const targetConfigPath =
          configPath ||
          (globalConfig
            ? path.join(git.env["HOME"] || tempHomedir, ".gitconfig")
    ...
    tryGetConfigKeys: jest.fn(
      async (pattern: string, globalConfig?: boolean, configPath?: string): Promise<string[]> => {
        const targetConfigPath =
          configPath ||
          (globalConfig
            ? path.join(git.env["HOME"] || tempHomedir, ".gitconfig")
