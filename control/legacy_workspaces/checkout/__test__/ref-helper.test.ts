...
describe("ref-helper tests", () => {
  ...
  it("getCheckoutInfo refs/heads/", async () => {
    const checkoutInfo = await refHelper.getCheckoutInfo(git, "refs/heads/my/branch", commit);
  ...
  it("getCheckoutInfo refs/pull/", async () => {
    const checkoutInfo = await refHelper.getCheckoutInfo(git, "refs/pull/123/merge", commit);
    expect(checkoutInfo.ref).toBe("refs/remotes/pull/123/merge");
    expect(checkoutInfo.startPoint).toBeFalsy();
  });

  it("getCheckoutInfo refs/tags/", async () => {
    const checkoutInfo = await refHelper.getCheckoutInfo(git, "refs/tags/my-tag", commit);
    expect(checkoutInfo.ref).toBe("refs/tags/my-tag");
    expect(checkoutInfo.startPoint).toBeFalsy();
  });

  it("getCheckoutInfo refs/", async () => {
    const checkoutInfo = await refHelper.getCheckoutInfo(git, "refs/gh/queue/main/pr-123", commit);
    expect(checkoutInfo.ref).toBe(commit);
    expect(checkoutInfo.startPoint).toBeFalsy();
  });

  it("getCheckoutInfo refs/ without commit", async () => {
    const checkoutInfo = await refHelper.getCheckoutInfo(git, "refs/non-standard-ref", "");
