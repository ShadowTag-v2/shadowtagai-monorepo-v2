import { inject, type Ref, shallowRef, watchEffect } from "vue";
import { doFind, type Match, useSetupParser } from "./lang";
import { astGrepStateKey, Mode, type State, useSgState } from "./state";
...
function useDoFind(langLoaded: Ref<boolean>, state: State) {
  ...
  const yaml = shallowRef<YAML | null>(null);
  yamlImport.then((yml) => {
    yaml.value = yml;
  });
...
export function usePattern() {
  const { query, rewrite, selector, strictness } = inject(astGrepStateKey)!;
  return {
    query,
    rewrite,
    selector,
