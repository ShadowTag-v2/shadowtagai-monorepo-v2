...
import type { SendStreamingMessageSuccessResponse, TaskStatusUpdateEvent } from "@a2a-js/sdk";
...
describe("E2E Tests", () => {
  ...
  it("should handle tool calls that do not require approval", async () => {
    ...
    const res = await agent
      .post("/")
      .send(
        createStreamMessageRequest("run a tool without approval", "a2a-no-approval-test-message"),
      )
      .set("Content-Type", "application/json")
      .expect(200);
