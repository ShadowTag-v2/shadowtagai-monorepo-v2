...
import { ESC_TIMEOUT, KeypressProvider, useKeypressContext } from "./KeypressContext.js";
...
describe("KeypressContext", () => {
  ...
  it.each([
    1,
    ESC_TIMEOUT - 1,
  ])("should handle sequences arriving character by character with %s ms delay", async (delay) => {
    const keyHandler = vi.fn();
    const { result } = renderHook(() => useKeypressContext(), { wrapper });
