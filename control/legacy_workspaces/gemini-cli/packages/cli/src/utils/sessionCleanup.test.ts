...
import { type Config, debugLogger, SESSION_FILE_PREFIX } from "@google/gemini-cli-core";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import type { Settings } from "../config/settings.js";
import { cleanupExpiredSessions } from "./sessionCleanup.js";
import { getAllSessionFiles, type SessionInfo } from "./sessionUtils.js";
  ...
  describe("Specific cleanup scenarios", () => {
    it("should delete sessions that exceed the cutoff date", async () => {
      ...
      expect(unlinkCalls).toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}8d.json`),
      );
      expect(unlinkCalls).toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}15d.json`),
      );
      expect(unlinkCalls).not.toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}5d.json`),
      );
    });
    ...
    it("should keep N most recent deletable sessions", async () => {
      ...
      const unlinkCalls = mockFs.unlink.mock.calls.map((call) => call[0]);
      expect(unlinkCalls).toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}3d.json`),
      );
      expect(unlinkCalls).toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}4d.json`),
      );
      expect(unlinkCalls).toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}5d.json`),
      );
      ...
      expect(unlinkCalls).not.toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}current.json`),
      );
      expect(unlinkCalls).not.toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}1d.json`),
      );
      expect(unlinkCalls).not.toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}2d.json`),
      );
    });
    ...
    it("should handle combined maxAge and maxCount retention (most restrictive wins)", async () => {
      ...
      expect(unlinkCalls).toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}7d.json`),
      );
      expect(unlinkCalls).toContain(
        path.join("/tmp/test-project", "chats", `${SESSION_FILE_PREFIX}12d.json`),
      );
