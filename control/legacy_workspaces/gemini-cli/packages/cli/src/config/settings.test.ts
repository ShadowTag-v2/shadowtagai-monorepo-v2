...
describe("Settings Loading and Merging", () => {
  ...
  describe("loadSettings", () => {
    ...
    it("should correctly migrate a complex legacy (v1) settings file", () => {
      ...
      (fs.readFileSync as Mock).mockImplementation((p: fs.PathOrFileDescriptor) => {
        if (p === USER_SETTINGS_PATH) return JSON.stringify(legacySettingsContent);
        return "{}";
      });
    ...
    it("should correctly merge and migrate legacy array properties from multiple scopes", () => {
      ...
      (fs.readFileSync as Mock).mockImplementation((p: fs.PathOrFileDescriptor) => {
        ...
        if (p === MOCK_WORKSPACE_SETTINGS_PATH) return JSON.stringify(legacyWorkspaceSettings);
        return "{}";
      });
      ...
      expect(settings.merged.context?.includeDirectories).toEqual(["/user/dir", "/workspace/dir"]);

      // Verify excludeTools are concatenated and de-duped
      expect(settings.merged.tools?.exclude).toEqual(["user-tool", "workspace-tool"]);
  ...
  describe("excludedProjectEnvVars integration", () => {
    ...
    it("should exclude DEBUG and DEBUG_MODE from project .env files by default", () => {
      ...
      const originalFindEnvFile = (loadSettings as unknown as { findEnvFile: () => string })
        .findEnvFile;
    ...
    it("should respect custom excludedProjectEnvVars from user settings", () => {
      ...
      expect(settings.merged.advanced?.excludedEnvVars).toEqual(["NODE_ENV", "DEBUG"]);
    });
  ...
  describe("migrateDeprecatedSettings", () => {
    ...
    it("should migrate disabled extensions from user and workspace settings", () => {
      ...
      (mockFsReadFileSync as Mock).mockImplementation((p: fs.PathOrFileDescriptor) => {
        if (p === USER_SETTINGS_PATH) return JSON.stringify(userSettingsContent);
        if (p === MOCK_WORKSPACE_SETTINGS_PATH) return JSON.stringify(workspaceSettingsContent);
        return "{}";
      });
