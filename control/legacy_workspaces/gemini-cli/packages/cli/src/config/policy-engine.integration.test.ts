...
describe("Policy Engine Integration Tests", () => {
  describe("Policy configuration produces valid PolicyEngine config", () => {
    it("should create a working PolicyEngine from basic settings", async () => {
      ...
      expect((await engine.check({ name: "run_shell_command" }, undefined)).decision).toBe(
    ...
    it("should handle MCP server wildcard patterns correctly", async () => {
      ...
      // Tools from allowed server should be allowed
      expect((await engine.check({ name: "allowed-server__tool1" }, undefined)).decision).toBe(
        PolicyDecision.ALLOW,
      );
      expect(
        (await engine.check({ name: "allowed-server__another_tool" }, undefined)).decision,
      ).toBe(PolicyDecision.ALLOW);

      // Tools from trusted server should be allowed
      expect((await engine.check({ name: "trusted-server__tool1" }, undefined)).decision).toBe(
        PolicyDecision.ALLOW,
      );
      expect(
        (await engine.check({ name: "trusted-server__special_tool" }, undefined)).decision,
      ).toBe(PolicyDecision.ALLOW);
      ...
      expect((await engine.check({ name: "blocked-server__any_tool" }, undefined)).decision).toBe(
        PolicyDecision.DENY,
      );

      // Tools from unknown servers should use default
      expect((await engine.check({ name: "unknown-server__tool" }, undefined)).decision).toBe(
        PolicyDecision.ASK_USER,
      );
    });
    ...
    it("should handle complex mixed configurations", async () => {
      ...
      expect((await engine.check({ name: "my-server__special-tool" }, undefined)).decision).toBe(
        PolicyDecision.ALLOW,
      );

      // MCP server tools
      expect((await engine.check({ name: "allowed-server__tool" }, undefined)).decision).toBe(
        PolicyDecision.ALLOW,
      );
      expect((await engine.check({ name: "trusted-server__tool" }, undefined)).decision).toBe(
        PolicyDecision.ALLOW,
      );
      expect((await engine.check({ name: "blocked-server__tool" }, undefined)).decision).toBe(
        PolicyDecision.DENY,
      );
    ...
    it("should verify priority ordering works correctly in practice", async () => {
      ...
      expect((await engine.check({ name: "blocked-server__any" }, undefined)).decision).toBe(
        PolicyDecision.DENY,
      );
      expect((await engine.check({ name: "specific-tool" }, undefined)).decision).toBe(
        PolicyDecision.ALLOW,
      );
      expect((await engine.check({ name: "trusted-server__any" }, undefined)).decision).toBe(
        PolicyDecision.ALLOW,
      );
      expect((await engine.check({ name: "mcp-server__any" }, undefined)).decision).toBe(
        PolicyDecision.ALLOW,
      );
    ...
    it("should handle edge case: MCP server with both trust and exclusion", async () => {
      ...
      expect((await engine.check({ name: "conflicted-server__tool" }, undefined)).decision).toBe(
        PolicyDecision.DENY,
      );
    });
    ...
    it("should handle edge case: specific tool allowed but server excluded", async () => {
      ...
      expect((await engine.check({ name: "my-server__other-tool" }, undefined)).decision).toBe(
        PolicyDecision.DENY,
      );
    });
