...
import { type CliArgs, loadCliConfig, parseArguments } from "./config.js";
import { ExtensionManager } from "./extension-manager.js";
import type { Settings } from "./settings.js";
import { isWorkspaceTrusted } from "./trustedFolders.js";

vi.mock("./trustedFolders.js", () => ({
  isWorkspaceTrusted: vi.fn().mockReturnValue({ isTrusted: true, source: "file" }), // Default to trusted
}));
...
vi.mock("@google/gemini-cli-core", async () => {
  ...
  return {
    ...
    loadServerHierarchicalMemory: vi.fn(
      (cwd, dirs, debug, fileService, extensionLoader: ExtensionLoader, _maxDirs) => {
...
describe("parseArguments", () => {
  it.each([
  ...
  ])("should throw an error when using conflicting prompt flags ($description)", async ({
    argv,
  }) => {
    process.argv = argv;
    ...
    await expect(parseArguments({} as Settings)).rejects.toThrow("process.exit called");
  ...
  ])("should throw an error when using conflicting yolo/approval-mode flags ($description)", async ({
    argv,
  }) => {
    process.argv = argv;
...
describe("loadCliConfig fileFiltering", () => {
  ...
  it.each(testCases)("should pass $property from settings to config when $value", async ({
    property,
    getter,
    value,
  }) => {
