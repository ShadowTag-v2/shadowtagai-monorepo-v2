...
export class OAuthCredentialStorage {
  ...
  static async loadCredentials(): Promise<Credentials | null> {
    try {
      const credentials = await OAuthCredentialStorage.storage.getCredentials(MAIN_ACCOUNT_KEY);
      ...
      return await OAuthCredentialStorage.migrateFromFileStorage();
    } catch (error: unknown) {
  ...
  static async saveCredentials(credentials: Credentials): Promise<void> {
    ...
    await OAuthCredentialStorage.storage.setCredentials(mcpCredentials);
  }
  ...
  static async clearCredentials(): Promise<void> {
    try {
      await OAuthCredentialStorage.storage.deleteCredentials(MAIN_ACCOUNT_KEY);
