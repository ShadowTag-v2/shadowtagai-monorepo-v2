...
import { type ChatCompressionInfo, CompressionStatus, GeminiEventType, Turn } from "./turn.js";
...
describe("Gemini Client (client.ts)", () => {
  ...
  describe("sendMessageStream", () => {
    ...
    it.each([
      ...
      { compressionStatus: CompressionStatus.NOOP },
    ])("does not emit a compression event when the status is $compressionStatus", async ({
      compressionStatus,
    }) => {
      // Arrange
...
\`\`\`
    ...
    describe("Editor context delta", () => {
      ...
      it.each(testCases)("$description", async ({
        previousActiveFile,
        currentActiveFile,
        shouldSendContext,
      }) => {
