...
import { DEFAULT_GEMINI_MODEL, getEffectiveModel, PREVIEW_GEMINI_MODEL } from "../config/models.js";
...
describe("Core System Prompt (prompts.ts)", () => {
  ...
  describe("GEMINI_SYSTEM_MD environment variable", () => {
    ...
    it.each([
      "true",
      "1",
    ])('should read from default path when GEMINI_SYSTEM_MD is "%s"', (value) => {
  ...
  describe("GEMINI_WRITE_SYSTEM_MD environment variable", () => {
    it.each([
      "false",
      "0",
    ])('should not write to file when GEMINI_WRITE_SYSTEM_MD is "%s"', (value) => {
    ...
    it.each([
      "true",
      "1",
    ])('should write to default path when GEMINI_WRITE_SYSTEM_MD is "%s"', (value) => {
      const defaultPath = path.resolve(path.join(GEMINI_DIR, "system.md"));
      vi.stubEnv("GEMINI_WRITE_SYSTEM_MD", value);
      getCoreSystemPrompt(mockConfig);
