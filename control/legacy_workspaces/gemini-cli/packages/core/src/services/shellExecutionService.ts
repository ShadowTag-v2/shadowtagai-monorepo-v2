...
import { type AnsiOutput, serializeTerminalToObject } from "../utils/terminalSerializer.js";
...
export class ShellExecutionService {
  ...
  static async execute(
    ...
    return ShellExecutionService.childProcessFallback(
      commandToExecute,
      cwd,
      onOutputEvent,
      abortSignal,
      ...
      const result = new Promise<ShellExecutionResult>((resolve) => {
        ...
        const handleOutput = (data: Buffer, stream: "stdout" | "stderr") => {
          ...
          if (isStreamingRawContent) {
            ...
            if (stream === "stdout") {
              const { newBuffer, truncated } = ShellExecutionService.appendAndTruncate(
                stdout,
                decodedChunk,
        ...
        child.on("exit", (code, signal) => {
          if (child.pid) {
            ShellExecutionService.activePtys.delete(child.pid);
          }
  ...
  private static executeWithPty(
    ...
    try {
      ...
      const result = new Promise<ShellExecutionResult>((resolve) => {
        ...
        ShellExecutionService.activePtys.set(ptyProcess.pid, { ptyProcess, headlessTerminal });
