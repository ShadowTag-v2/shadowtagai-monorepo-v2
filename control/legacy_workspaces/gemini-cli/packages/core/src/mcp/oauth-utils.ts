...
export class OAuthUtils {
  ...
  static async discoverAuthorizationServerMetadata(
    ...
    for (const endpoint of endpointsToTry) {
      const authServerMetadata = await OAuthUtils.fetchAuthorizationServerMetadata(endpoint);
  ...
  static async discoverOAuthConfig(serverUrl: string): Promise<MCPOAuthConfig | null> {
    try {
      ...
      const wellKnownUrls = OAuthUtils.buildWellKnownUrls(serverUrl, false);

      // Try to get the protected resource metadata at root
      let resourceMetadata = await OAuthUtils.fetchProtectedResourceMetadata(
      ...
      if (!resourceMetadata) {
        ...
        if (url.pathname && url.pathname !== "/") {
          const pathBasedUrls = OAuthUtils.buildWellKnownUrls(serverUrl, true);
          resourceMetadata = await OAuthUtils.fetchProtectedResourceMetadata(
  ...
  static async discoverOAuthFromWWWAuthenticate(
    ...
    const resourceMetadata = await OAuthUtils.fetchProtectedResourceMetadata(resourceMetadataUri);
    if (!resourceMetadata?.authorization_servers?.length) {
      return null;
    }

    const authServerUrl = resourceMetadata.authorization_servers[0];
    const authServerMetadata = await OAuthUtils.discoverAuthorizationServerMetadata(authServerUrl);

    if (authServerMetadata) {
      return OAuthUtils.metadataToOAuthConfig(authServerMetadata);
    }
