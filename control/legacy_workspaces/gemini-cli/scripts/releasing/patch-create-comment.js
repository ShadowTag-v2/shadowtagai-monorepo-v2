...
async function main() {
  ...
  const exitCode =
    argv.exitCode !== undefined ? argv.exitCode : parseInt(process.env.EXIT_CODE || "1");
...
- [View branch on GitHub](https://github.com/${repository}/tree/${branch})`;
    ...
    if (branchMatch) {
      ...
      if (testMode) {
        ...
        const hasConflicts =
          logContent.includes("Cherry-pick has conflicts") || logContent.includes("[CONFLICTS]");
...
- [View hotfix PR #${mockPrNumber}](${mockPrUrl})`;
        ...
        try {
          ...
          if (prList.length > 0) {
            const pr = prList[0];
            const hasConflicts =
              logContent.includes("Cherry-pick has conflicts") || pr.title.includes("[CONFLICTS]");
