...
export class TestRig {
  ...
  run(
    ...
    if (typeof promptOrOptions === "object" && !promptOrOptions.stdinDoesNotEnd) {
      child.stdin!.end();
    }
  ...
  async waitForTelemetryEvent(eventName: string, timeout?: number) {
    ...
    return poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes && logData.attributes["event.name"] === `gemini_cli.${eventName}`,
        );
      },
      timeout,
      100,
  ...
  async expectToolCallSuccess(
    ...
    expect(success, `Expected to find successful toolCalls for ${JSON.stringify(toolNames)}`).toBe(
      true,
    );
  }
  ...
  readAllApiRequest(): ParsedLog[] {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes && logData.attributes["event.name"] === "gemini_cli.api_request",
    );
    return apiRequests;
  }
  ...
  async runInteractive(...args: string[]): Promise<InteractiveRun> {
    ...
    const options: pty.IPtyForkOptions = {
      ...
      rows: 80,
      cwd: this.testDir!,
      env: Object.fromEntries(Object.entries(env).filter(([, v]) => v !== undefined)) as {
        [key: string]: string;
      },
    };
