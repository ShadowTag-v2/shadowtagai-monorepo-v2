...
import { printDebugInfo, TestRig, validateModelOutput } from "./test-helper.js";
...
describe.skip("stdin context", () => {
  it("should be able to use stdin as context for a prompt", async () => {
    ...
    expect(promptIndex, `Expected prompt to be present in conversation history`).toBeGreaterThan(
      -1,
    );
