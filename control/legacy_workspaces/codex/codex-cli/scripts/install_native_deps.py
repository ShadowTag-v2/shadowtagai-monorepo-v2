...
def parse_args() -> argparse.Namespace:
  ...
  parser.add_argument(
    "--workflow-url",
    help=(
      "GitHub Actions workflow URL that produced the artifacts. Defaults to a known good run when omitted."
    ),
  )
  parser.add_argument(
    "--component",
    ...
    help=(
      "Limit installation to the specified components. May be repeated. Defaults to 'codex' and 'rg'."
    ),
  )
  parser.add_argument(
    "root",
    ...
    type=Path,
    help=(
      "Directory containing package.json for the staged package. If omitted, the repository checkout is used."
    ),
  )
...
def install_binary_components(
  ...
  selected_components = [
    BINARY_COMPONENTS[name] for name in component_names if name in BINARY_COMPONENTS
  ]
