...
class Judge6Integrator:
    ...
    def _extract_metric_value(self, metric_name: str, metrics: dict) -> float | None:
        ...
        mapping = {
            "tier_1_percentage": lambda m: m.get("tier_distribution", {}).get("tier_1", {}).get("percentage", 0),
            "total_items_minimum": lambda m: m.get("coverage_stats", {}).get("total_items_collected", 0),
            "compliance_score": lambda m: m.get("compliance_stats", {}).get("allowed_percentage", 0),
    ...
    async def analyze_handoff(
        ...
        throughput_mbps = (data_size_bytes / 1024 / 1024 / latency_seconds) if latency_seconds > 0 else 0
    ...
    def get_unified_metrics(self, ingestion_metrics: dict, validation_results: list[ValidationResult]) -> dict:
        """
        ...
        return {
            ...
            "validation": {
                ...
                "success_rate": (status_counts["pass"] / len(validation_results) * 100) if validation_results else 0,
            },
            "handoff": {
                "avg_latency_seconds": sum(self.handoff_latencies) / len(self.handoff_latencies) if self.handoff_latencies else 0,
                "failure_count": self.handoff_failures,
            },
            "failed_rules": [
                {
    ...
    def _calculate_health_score(self, validation_results: list[ValidationResult], metrics: dict) -> float:
        """
    ...
    def generate_validation_report(self, validation_results: list[ValidationResult]) -> str:
        ...
        # Critical failures
        critical_fails = [r for r in validation_results if r.status == ValidationStatus.FAIL and r.severity == "critical"]
        if critical_fails:
        ...
        high_issues = [r for r in validation_results if r.status != ValidationStatus.PASS and r.severity == "high"]
        if high_issues:
