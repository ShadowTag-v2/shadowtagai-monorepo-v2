...
from .sources import SourceManager, DEFAULT_SOURCES
...
class IngestionPipeline:
    ...
    def __init__(
        ...
        ethics_checker: EthicalComplianceChecker = None,
        tier_classifier: TierClassifier = None,
        briefing_generator: BriefingGenerator = None,
    ):
    ...
    async def run_nightly_ingestion(
        ...
        try:
            ...
            logger.info(f"  - Tier 1 (High): {tier_dist['tier_1']['count']} ({tier_dist['tier_1']['percentage']:.1f}%)")
            logger.info(f"  - Tier 2 (Med):  {tier_dist['tier_2']['count']} ({tier_dist['tier_2']['percentage']:.1f}%)")
            logger.info(f"  - Tier 3 (Low):  {tier_dist['tier_3']['count']} ({tier_dist['tier_3']['percentage']:.1f}%)")
