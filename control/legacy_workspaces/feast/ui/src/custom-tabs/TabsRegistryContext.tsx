...
const useGenericCustomTabsNavigation = <T extends CustomTabRegistrationInterface>(
  entries: T[],
  ...
  if (duplicatedPaths.length) {
    throw new Error(`More than one tabs registered for path url: ${duplicatedPaths.join(", ")}`);
  }
...
const useRegularFeatureViewCustomTabs = (navigate: NavigateFunction) => {
  const { RegularFeatureViewCustomTabs } = React.useContext(TabsRegistryContext);
...
const useOnDemandFeatureViewCustomTabs = (navigate: NavigateFunction) => {
  const { OnDemandFeatureViewCustomTabs } = React.useContext(TabsRegistryContext);
...
const useFeatureCustomTabRoutes = () => {
  ...
  return genericCustomTabRoutes(FeatureCustomTabs || [], FeatureCustomTabLoadingWrapper);
};

const useDatasetCustomTabRoutes = () => {
  const { DatasetCustomTabs } = React.useContext(TabsRegistryContext);

  return genericCustomTabRoutes(DatasetCustomTabs || [], DatasetCustomTabLoadingWrapper);
};
