...
const AllProjectsDashboard = () => {
  ...
  const projectStats = projects.map((project) => {
    const projectFVs =
      registryData.objects.featureViews?.filter((fv: any) => fv?.spec?.project === project.id) ||
      [];
    const projectEntities =
      registryData.objects.entities?.filter((e: any) => e?.spec?.project === project.id) || [];
  ...
  return (
    <EuiPageTemplate panelled>
      <EuiPageTemplate.Section>
        ...
        <EuiText>
          <p>View aggregated statistics and explore data across all your Feast projects.</p>
        </EuiText>
        <EuiSpacer size="l" />
        ...
        <EuiFlexGroup gutterSize="l" wrap>
          {projectStats.map((project) => (
            <EuiFlexItem key={project.id} style={{ minWidth: "300px", maxWidth: "400px" }}>
              <EuiCard
                title={project.name}
                ...
                <EuiFlexGroup justifyContent="spaceAround" gutterSize="s">
                  <EuiFlexItem grow={false}>
                    <EuiText size="s" textAlign="center">
                      ...
                      <br />
                      <span style={{ fontSize: "0.85em", color: "#69707D" }}>Feature Views</span>
                    </EuiText>
                  </EuiFlexItem>
                  <EuiFlexItem grow={false}>
                    <EuiText size="s" textAlign="center">
                      ...
                      <br />
                      <span style={{ fontSize: "0.85em", color: "#69707D" }}>Entities</span>
                    </EuiText>
                  </EuiFlexItem>
                  <EuiFlexItem grow={false}>
                    <EuiText size="s" textAlign="center">
                      ...
                      <br />
                      <span style={{ fontSize: "0.85em", color: "#69707D" }}>Features</span>
                    </EuiText>
                  </EuiFlexItem>
                </EuiFlexGroup>
              </EuiCard>
...
const ProjectOverviewPage = () => {
  ...
  const { isLoading, isSuccess, isError, data } = useLoadRegistry(registryUrl, projectName);
  ...
  const categories = [
    ...
    },
    {
      ...
      getLink: (item: any) => {
        ...
        return serviceName ? `/p/${projectName}/feature-service/${serviceName}` : "#";
      },
    },
  ];
          ...
          <EuiFlexItem grow={2}>
            ...
            {isError && (
              <EuiEmptyPrompt
                ...
                body={
                  <p>
                    There was an error loading the Project Configurations. Please check that{" "}
            ...
            {isSuccess &&
              (data?.description ? (
                ...
                <EuiText>
                  <p>
                    Welcome to your new Feast project. In this UI, you can see Data Sources,
                    Entities, Features, Feature Views, and Feature Services registered in Feast.
                  </p>
                  <p>
                    It looks like this project already has some objects registered. If you are new
                    to this project, we suggest starting by exploring the Feature Services, as they
                    represent the collection of Feature Views serving a particular model.
                  </p>
                  <p>
                    <strong>Note</strong>: We encourage you to replace this welcome message with
                    more suitable content for your team. You can do so by specifying a{" "}
                    <code>project_description</code> in your <code>feature_store.yaml</code> file.
                  </p>
                </EuiText>
              ))}
            <ObjectsCountStats />
