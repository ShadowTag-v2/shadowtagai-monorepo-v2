...
type FeatureColumn = EuiTableFieldDataColumnType<Feature> | EuiTableComputedColumnType<Feature>;

const FeatureListPage = () => {
  const { projectName } = useParams();
  const registryUrl = useContext(RegistryPathContext);
  const { data, isLoading, isError } = useLoadRegistry(registryUrl, projectName);
  ...
  const paginatedFeatures = sortedFeatures.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize);
  ...
  return (
    <EuiPageTemplate panelled>
      <EuiPageTemplate.Header
        ...
        rightSideItems={[<ExportButton data={filteredFeatures} fileName="features" />]}
      />
