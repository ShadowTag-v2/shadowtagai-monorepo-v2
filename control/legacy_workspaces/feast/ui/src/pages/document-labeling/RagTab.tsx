...
The final paragraph contains information about feature stores and real-time machine learning systems. This text can be used to test the highlighting functionality and ensure that labels are properly stored and displayed in the user interface.`;
      ...
      setError(err instanceof Error ? err.message : "An error occurred while loading the document");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderDocumentWithHighlights = (content: string): (string | React.ReactElement)[] => {
  ...
  return (
    <EuiPageSection>
      <EuiCallOut title="Label document chunks for RAG" color="primary" iconType="iInCircle">
        <p>
          Load a document and highlight text chunks to label them for chunk extraction/retrieval.
          Add prompt and query context, then provide ground truth labels for generation evaluation.
        </p>
      </EuiCallOut>
      ...
      <EuiFlexGroup>
        ...
        <EuiFlexItem grow={false}>
          <EuiFormRow hasEmptyLabelSpace>
            <EuiButton fill onClick={loadDocument} disabled={!filePath} isLoading={isLoading}>
              Load Document
            </EuiButton>
          </EuiFormRow>
        </EuiFlexItem>
      ...
      {error && (
        <EuiCallOut title="Error loading document" color="danger" iconType="alert">
          <p>{error}</p>
        </EuiCallOut>
      )}
      ...
      {documentContent && (
        <>
          <EuiPanel paddingSize="l">
            ...
            <EuiFlexGroup>
              <EuiFlexItem>
                <EuiFormRow label="Prompt" helpText="System context for the RAG system">
                  <EuiTextArea
              ...
              <EuiFlexItem>
                <EuiFormRow label="Query" helpText="User query for retrieval testing">
                  <EuiTextArea
          ...
          <EuiFlexGroup>
            ...
            <EuiFlexItem grow={false}>
              <EuiFormRow hasEmptyLabelSpace>
                <EuiButton fill onClick={handleLabelSelection} disabled={!selectedText}>
                  Label Selected Text
                </EuiButton>
              </EuiFormRow>
            </EuiFlexItem>
          ...
          {selectedText && (
            <EuiCallOut title="Text selected for labeling" color="primary" size="s">
          ...
          <EuiFormRow label="Ground Truth Label" helpText="Text for generation evaluation">
            <EuiTextArea
          ...
          {(labels.length > 0 || groundTruthLabel || prompt || query) && (
            <>
              <EuiCallOut title="Ready to save" color="success" iconType="check" size="s">
          ...
          {hasUnsavedChanges && (
            <>
              <EuiCallOut title="Unsaved changes" color="warning" iconType="alert" size="s">
              ...
              <EuiPanel paddingSize="l">
                ...
                {labels.map((label, index) => (
                  <EuiFlexGroup key={index} alignItems="center" gutterSize="s">
                    <EuiFlexItem grow={false}>
                      <EuiCode color={label.label === "relevant" ? "success" : "danger"}>
                        Chunk: {label.label}
                      </EuiCode>
                    </EuiFlexItem>
                    {label.groundTruthLabel && (
                      <EuiFlexItem grow={false}>
                        <EuiCode color="primary">GT: {label.groundTruthLabel}</EuiCode>
                      </EuiFlexItem>
                    )}
                    <EuiFlexItem>
                      <EuiText size="s">
                    ...
                    <EuiFlexItem grow={false}>
                      <EuiButton size="s" color="danger" onClick={() => handleRemoveLabel(index)}>
                        Remove
                      </EuiButton>
                    </EuiFlexItem>
                  </EuiFlexGroup>
