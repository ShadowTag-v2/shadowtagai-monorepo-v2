...
const ClassificationTab = () => {
  ...
  const loadCsvData = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "An error occurred while loading the CSV data");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleClassChange = (id: number, newClass: string) => {
    setData(data.map((item) => (item.id === id ? { ...item, currentClass: newClass } : item)));
  };
  ...
  const resetChanges = () => {
    setData(data.map((item) => ({ ...item, currentClass: item.originalClass || "" })));
  };
  ...
  return (
    <EuiPageSection>
      <EuiCallOut title="Classify sample data" color="primary" iconType="iInCircle">
        <p>
          Load a CSV file containing text samples and edit their classification labels. This helps
          improve your classification models by providing corrected training data.
        </p>
      </EuiCallOut>
      ...
      <EuiFlexGroup>
        ...
        <EuiFlexItem grow={false}>
          <EuiFormRow hasEmptyLabelSpace>
            <EuiButton fill onClick={loadCsvData} disabled={!csvPath} isLoading={isLoading}>
              Load CSV Data
            </EuiButton>
          </EuiFormRow>
        </EuiFlexItem>
      ...
      {error && (
        <EuiCallOut title="Error loading CSV data" color="danger" iconType="alert">
          <p>{error}</p>
        </EuiCallOut>
      )}
          ...
          <EuiFlexGroup alignItems="center" justifyContent="spaceBetween">
            ...
            <EuiFlexItem grow={false}>
              <EuiFlexGroup gutterSize="s">
                ...
                <EuiFlexItem grow={false}>
                  <EuiButton fill onClick={saveChanges} disabled={getChangedItems().length === 0}>
                  ...
                  <EuiTableRow key={item.id}>
                    ...
                    <EuiTableRowCell>
                      <EuiSelect
                        ...
                        onChange={(e) => handleClassChange(item.id, e.target.value)}
                        compressed
                      />
                    </EuiTableRowCell>
                  </EuiTableRow>
          ...
          {getChangedItems().length > 0 && (
            <>
              ...
              <EuiCallOut
                ...
                iconType="alert"
              >
                <p>You have unsaved changes. Click "Save Changes" to persist your modifications.</p>
              </EuiCallOut>
            </>
          )}
        </>
