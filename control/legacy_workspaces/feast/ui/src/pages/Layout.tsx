...
import RegistrySearch, { type RegistrySearchRef } from "../components/RegistrySearch";
...
const Layout = () => {
  ...
  const globalRegistryPath = projectsData?.projects?.[0]?.registryPath || registryPath;
    ...
    ? [
        ...
        },
        {
          ...
          getLink: (item: any) => {
            const serviceName = item?.name || item?.spec?.name;
            const project = item?.projectId || getProjectId(item);
            return serviceName ? `/p/${project}/feature-service/${serviceName}` : "#";
          },
        },
      ]
    : [];
                ...
                >
                  <EuiFlexGroup justifyContent="center">
                    <EuiFlexItem grow={false} style={{ width: "600px", maxWidth: "90%" }}>
                      <RegistrySearch ref={searchRef} categories={globalCategories} />
                    </EuiFlexItem>
                  </EuiFlexGroup>
                </div>
              )}
