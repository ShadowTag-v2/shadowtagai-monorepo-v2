  ...
  const data = relationshipQuery.data
    ? fvList.reduce((memo: Record<string, string[]>, fvName) => {
        if (relationshipQuery.data) {
          memo[fvName] = relationshipQuery.data.filter(whereFSconsumesThisFv(fvName)).map((fs) => {
  ...
  const columns = [
    {
      ...
      render: ({ name }: { name: string }) => {
        return <EuiCustomLink to={`/p/${projectName}/feature-view/${name}`}>{name}</EuiCustomLink>;
      },
    },
    {
      name: "FS Consumers",
