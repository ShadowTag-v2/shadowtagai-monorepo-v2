...
const FeatureServiceOverviewTab = () => {
  const { featureServiceName, projectName } = useParams();

  const fsName = featureServiceName === undefined ? "" : featureServiceName;

  const { isLoading, isSuccess, isError, data, entities } = useLoadFeatureService(fsName);
      ...
      {isSuccess && data && (
        <EuiFlexGroup direction="column">
          <EuiFlexGroup alignItems="center">
            ...
            <EuiFlexItem>
              <EuiStat title={`${numFeatureViews}`} description="Feature Views" />
            </EuiFlexItem>
            {data?.meta?.lastUpdatedTimestamp ? (
              ...
              <EuiText>No last updated timestamp specified on this feature service.</EuiText>
            )}
          </EuiFlexGroup>
          <EuiFlexGroup>
            <EuiFlexItem grow={2}>
              <EuiPanel hasBorder={true}>
                ...
                {data?.spec?.features ? (
                  ...
                  <EuiText>No features specified for this feature service.</EuiText>
                )}
              </EuiPanel>
            </EuiFlexItem>
            <EuiFlexItem grow={1}>
                      ...
                      return (
                        <EuiFlexItem grow={false} key={entity.name}>
                          <EuiBadge
                            color="primary"
                            onClick={() => {
                              navigate(`/p/${projectName}/entity/${entity.name}`);
                            }}
