...
const DatasourcesListingTable = ({ dataSources }: DatasourcesListingTableProps) => {
  ...
  const columns = [
    {
      ...
      render: (name: string, item: feast.core.IDataSource) => {
        ...
        return <EuiCustomLink to={`/p/${itemProject}/data-source/${name}`}>{name}</EuiCustomLink>;
      },
    },
    {
      name: "Type",
  ...
  return <EuiBasicTable columns={columns} items={dataSources} rowProps={getRowProps} />;
};
