...
const whereFSconsumesThisFv = (fvName: string) => {
  return (r: EntityRelation) => {
    return r.source.name === fvName && r.target.type === FEAST_FCO_TYPES.featureService;
  };
};

const OnDemandFeatureViewOverviewTab = ({ data }: OnDemandFeatureViewOverviewTabProps) => {
  ...
  const fsNames = relationshipQuery.data
    ? relationshipQuery.data.filter(whereFSconsumesThisFv(data?.spec?.name!)).map((fs) => {
            ...
            <EuiFlexGroup direction="column">
              {inputs.map(([key, inputGroup]) => {
                if ((inputGroup as feast.core.IOnDemandSource).requestDataSource) {
                  return (
                    <EuiFlexItem key={key}>
                      <RequestDataDisplayPanel {...(inputGroup as feast.core.IOnDemandSource)} />
                    </EuiFlexItem>
                  );
                }
