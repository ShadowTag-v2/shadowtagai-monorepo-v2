...
import type { RegularFeatureViewCustomTabProps } from "../../custom-tabs/types";

const CurlGeneratorTab = ({ feastObjectQuery }: RegularFeatureViewCustomTabProps) => {
    ...
    <React.Fragment>
      <EuiPanel hasBorder={true}>
        ...
        <EuiText size="s" color="subdued">
          <p>
            Generate a CURL command to fetch online features from the feature server. The command is
            pre-populated with all features and entities from this feature view.
          </p>
        </EuiText>
        <EuiSpacer size="m" />
        ...
        {data?.features && data.features.length > 0 && (
          <>
            <EuiFlexGroup justifyContent="spaceBetween" alignItems="center">
              <EuiFlexItem grow={false}>
                <EuiTitle size="xs">
                  <h3>
                    Features to Include (
                    {Object.values(selectedFeatures).filter((v) => v !== false).length}/
            ...
            <EuiPanel color="subdued" paddingSize="s">
              {Array.from({ length: Math.ceil(data.features.length / 5) }, (_, rowIndex) => (
                <EuiFlexGroup
                  key={rowIndex}
                  direction="row"
                  gutterSize="s"
                  style={{
                    marginBottom: rowIndex < Math.ceil(data.features.length / 5) - 1 ? "8px" : "0",
                  }}
                >
                  {data.features.slice(rowIndex * 5, (rowIndex + 1) * 5).map((feature: any) => (
                    <EuiFlexItem key={feature.name} grow={false} style={{ minWidth: "180px" }}>
                      <EuiCheckbox
        ...
        {data?.object?.spec?.entities && data.object.spec.entities.length > 0 && (
          <>
            <EuiTitle size="xs">
        ...
        <CodeBlock text={curlCommand} language="bash" showLineNumbers={false} theme={github} />

        <EuiSpacer size="m" />

        <EuiText size="s" color="subdued">
          <p>
            <strong>Features included:</strong> {generateFeatureNames().join(", ")}
          </p>
