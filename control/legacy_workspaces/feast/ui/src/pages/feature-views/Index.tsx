...
import type { genericFVType, regularFVInterface } from "../../parsers/mergedFVTypes";
...
const useLoadFeatureViews = () => {
  ...
  const data = registryQuery.data === undefined ? undefined : registryQuery.data.mergedFVList;
...
const shouldIncludeFVsGivenTokenGroups = (
  ...
  return Object.entries(tagTokenGroups).every(([key, values]) => {
    const entryTagValue = entry?.object?.spec!.tags ? entry.object.spec.tags[key] : undefined;
...
const Index = () => {
  ...
  const filterResult = data ? filterFn(data, { tagTokenGroups, searchTokens }) : data;
  ...
  return (
    <EuiPageTemplate panelled>
      <EuiPageTemplate.Header
        ...
        rightSideItems={[
          <ExportButton data={filterResult ?? []} fileName="feature_views" formats={["json"]} />,
        ]}
      />
      <EuiPageTemplate.Section>
        ...
        {isSuccess && data && data.length > 0 && filterResult && (
          <React.Fragment>
            ...
            <EuiSpacer size="m" />
            <FeatureViewListingTable tagKeysSet={tagKeysSet} featureViews={filterResult} />
          </React.Fragment>
        )}
      </EuiPageTemplate.Section>
    </EuiPageTemplate>
