...
const buildTagCollection = <T>(
  ...
  const tagCollection = array.reduce((memo: Record<string, Record<string, T[]>>, fco: T) => {
...
const useFeatureViewTagsAggregation = () => {
  ...
  const data = useMemo(() => {
    return query.data && query.data.objects && query.data.objects.featureViews
      ? buildTagCollection<feast.core.IFeatureView>(query.data.objects.featureViews!, (fv) => {
          return fv.spec?.tags!;
        })
      : undefined;
  }, [query.data]);
...
const useFeatureServiceTagsAggregation = () => {
  ...
  const data = useMemo(() => {
    return query.data && query.data.objects && query.data.objects.featureServices
      ? buildTagCollection<feast.core.IFeatureService>(query.data.objects.featureServices, (fs) => {
          return fs.spec?.tags!;
        })
      : undefined;
  }, [query.data]);
...
export { useFeatureServiceTagsAggregation, useFeatureViewTagsAggregation };
