...
const createSearchLink = (FCOType: FEAST_FCO_TYPES, key: string, value: string) => {
...
const sortTagByUniqueValues = <T>(tagAggregation: Record<string, Record<string, T[]>>) => {
  return Object.entries(tagAggregation).sort(([a, valuesOfA], [b, valuesOfB]) => {
    return Object.keys(valuesOfB).length - Object.keys(valuesOfA).length;
  });
};

const sortTagsByTotalUsage = <T>(tagAggregation: Record<string, Record<string, T[]>>) => {
  return Object.entries(tagAggregation).sort(([a, valuesOfA], [b, valuesOfB]) => {
...
const generateExplorationSuggestions = (
  ...
  if (tagAggregation) {
    const SortedCandidates = sortTagByUniqueValues<feast.core.IFeatureView>(tagAggregation);
...
const useFCOExploreSuggestions = () => {
  ...
  let data: ExplorationSuggestion[] | undefined;
...
export { generateExplorationSuggestions, sortTagByUniqueValues, sortTagsByTotalUsage };
