...
const generateEmptyStateSuggestions = (tagsAggregationData: tagAggregationRecordType) => {
  if (tagsAggregationData) {
    return sortTagsByTotalUsage(tagsAggregationData).map(([tagKey, recordOfTagValues]) => {
      const uniqueValues = Object.keys(recordOfTagValues).length;
      const totalEntries = Object.values(recordOfTagValues).reduce((memo, current) => {
...
function getAllSpacePositions(s: string) {
  ...
  while (
    s.indexOf(" ", indices.length ? indices[indices.length - 1] + 1 : 0) !== -1 &&
    indices.length < 100
  ) {
...
const parseTokenInput = (
  ...
  if (cursorPosition) {
    tokenInFocusIndex = allSpacePositions.findIndex((value, index) => {
      return cursorPosition >= value && cursorPosition <= allSpacePositions[index + 1];
    });
  }
...
const useTagsWithSuggestions = (tagsAggregationData: tagAggregationRecordType) => {
  const [rawtagsString, setTagsStringParam] = useQueryParam("tags", StringParam);
  ...
  const { chunks, tokenInFocusIndex, currentTag } = parseTokenInput(cursorPosition, tagsString);

  const suggestionMode: SuggestionModes = currentTag.indexOf(":") < 0 ? "KEY" : "VALUE";
  ...
  const setSuggestionAtPositionInTagsString = (suggestion: string, position: number) => {
  ...
  const tagTokenGroups = tagTokens.reduce((memo: Record<string, string[]>, current) => {
    if (memo[current.key]) {
...
export type { filterInputInterface, SuggestionModes, TagSuggestionInstance, tagTokenGroupsType };
export { useSearchQuery, useTagsWithSuggestions };
