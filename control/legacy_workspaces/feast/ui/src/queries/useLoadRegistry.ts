...
import mergedFVTypes, { type genericFVType } from "../parsers/mergedFVTypes";
import parseEntityRelationships, { type EntityRelation } from "../parsers/parseEntityRelationships";
      ...
      return fetch(url, {
        ...
        .then<FeatureStoreAllData>((data) => {
          ...
          if (projectName && projectName !== "all") {
            ...
            const shouldFilter = projectsInRegistry.size > 1 || projectsInRegistry.has(projectName);
            ...
            if (shouldFilter && projectsInRegistry.has(projectName)) {
              ...
              if (objects.permissions) {
                objects.permissions = objects.permissions.filter(
                  (perm: any) => perm?.spec?.project === projectName || !perm?.spec?.project,
                );
              }
            }
          }
          ...
          const indirectRelationships = parseIndirectRelationships(relationships, objects);
