...
const FeaturesList = ({ projectName, featureViewName, features, link }: FeaturesListProps) => {
  const columns: { name: string; render?: any; field: any }[] = [
    {
      name: "Name",
      field: "name",
      render: (item: string) => (
        <EuiCustomLink to={`/p/${projectName}/feature-view/${featureViewName}/feature/${item}`}>
          {item}
        </EuiCustomLink>
      ),
    },
  ...
  return <EuiBasicTable columns={columns} items={features} rowProps={getRowProps} />;
};
