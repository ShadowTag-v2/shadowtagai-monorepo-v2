    ...
    <>
      ...
      {searchText && (
        <div style={searchResultsStyles.searchResults}>
          ...
          <EuiSpacer size="xs" />
          {searchResults.filter((result) => result.items.length > 0).length > 0 ? (
            searchResults
              .filter((result) => result.items.length > 0)
              .map((result) => (
                <div key={result.title} style={searchResultsStyles.categoryGroup}>
                  <EuiPanel hasBorder={true} paddingSize="m">
                    ...
                    {result.items.map((item, idx) => (
                      ...
                      >
                        <EuiFlexGroup>
                          <EuiFlexItem>
                            <EuiCustomLink to={item.link} onClick={() => setSearchText("")}>
