...
import { EuiLoadingSpinner, EuiPanel, EuiSpacer, EuiTitle, EuiToolTip } from "@elastic/eui";
...
const getLayoutedElements = (
  ...
  Object.entries(groupedIsolatedNodes).forEach(([type, typeNodes]) => {
    ...
    currentY +=
      Math.ceil(typeNodes.length / nodesPerRow) * (nodeHeight + isolatedNodesPadding) + 100;
  });
...
const Legend = () => {
  ...
  const boxShadow = isDarkMode ? "0 2px 5px rgba(0,0,0,0.3)" : "0 2px 5px rgba(0,0,0,0.1)";
    ...
    nodes.push({
      ...
      data: {
        ...
        permissions: permissions
          ? getEntityPermissions(permissions, FEAST_FCO_TYPES.featureService, fs.spec?.name)
          : [],
      },
      position: { x: 0, y: 0 },
    });
  ...
  objects.featureViews?.forEach((fv) => {
    nodes.push({
      ...
      data: {
        ...
        metadata: fv,
        permissions: permissions
          ? getEntityPermissions(permissions, FEAST_FCO_TYPES.featureView, fv.spec?.name)
          : [],
      },
      position: { x: 0, y: 0 },
    });
  ...
  objects.onDemandFeatureViews?.forEach((odfv) => {
    nodes.push({
      ...
      data: {
        ...
        metadata: odfv,
        permissions: permissions
          ? getEntityPermissions(permissions, FEAST_FCO_TYPES.featureView, odfv.spec?.name)
          : [],
      },
      position: { x: 0, y: 0 },
    });
  ...
  objects.streamFeatureViews?.forEach((sfv) => {
    nodes.push({
      ...
      data: {
        ...
        metadata: sfv,
        permissions: permissions
          ? getEntityPermissions(permissions, FEAST_FCO_TYPES.featureView, sfv.spec?.name)
          : [],
      },
      position: { x: 0, y: 0 },
    });
  ...
  objects.entities?.forEach((entity) => {
    nodes.push({
      ...
      data: {
        ...
        metadata: entity,
        permissions: permissions
          ? getEntityPermissions(permissions, FEAST_FCO_TYPES.entity, entity.spec?.name)
          : [],
      },
      position: { x: 0, y: 0 },
    });
  ...
  Array.from(dataSources).forEach((dsName) => {
    nodes.push({
      ...
      data: {
        ...
        permissions: permissions
          ? getEntityPermissions(permissions, FEAST_FCO_TYPES.dataSource, dsName)
          : [],
      },
      position: { x: 0, y: 0 },
    });
