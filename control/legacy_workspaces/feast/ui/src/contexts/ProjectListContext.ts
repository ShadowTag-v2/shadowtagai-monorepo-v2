...
const ProjectListContext = React.createContext<ProjectsListContextInterface | undefined>(undefined);
...
const anticipatedProjectListErrors = (err: Error, isCustomProjectList: boolean) => {
  ...
  return useQuery(
    ...
    () => {
      return projectListPromise?.projectsListPromise
        .catch((e) => {
          const anticipatedError = anticipatedProjectListErrors(e, projectListPromise.isCustom);
          setQueryError(() => {
            if (anticipatedError) {
...
export type { ProjectsListContextInterface, ProjectsListType };
