...
if (current_branch !== "master") {
  ...
  if (!is_valid) {
    throw new Error(
      `Invalid branch name: ${current_branch}. Must be in release branch form like v0.18-branch or master`,
    );
  }
}
  ...
  plugins: [
    ...
    [
      "@semantic-release/exec",
      {
        ...
        verifyReleaseCmd:
          "./infra/scripts/validate-release.sh  ${nextRelease.type} " + current_branch,

        // Bump all version files and build UI / update yarn.lock / helm charts
        prepareCmd:
          "python ./infra/scripts/release/bump_file_versions.py ${lastRelease.version} ${nextRelease.version}; make build-ui; make build-helm-docs; make -C infra/feast-operator build-installer bundle; rm -rf infra/feast-operator/bin",
      },
    ],
