...
function startRecording(data) {
  ...
  socket.onmessage = async (event) => {
    ...
    await browser.runtime.sendMessage({ action: "transcript", data: event.data }).catch((error) => {
...
function initPopupElement() {
  ...
  popupContainer.style.cssText =
    "position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; color: black; padding: 16px; border-radius: 10px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.5); display: none; text-align: center;";
...
function init_element() {
  ...
  elem_container.style.cssText =
    "padding-top:16px;font-size:18px;line-height:18px;position:fixed;top:85%;left:50%;transform:translate(-50%,-50%);width:500px;height:90px;opacity:0.9;z-index:100;background:black;border-radius:10px;color:white;";
