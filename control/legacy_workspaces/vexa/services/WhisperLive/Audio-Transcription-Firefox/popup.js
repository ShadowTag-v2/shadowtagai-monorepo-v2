document.addEventListener("DOMContentLoaded", () => {
  ...
  startButton.addEventListener("click", () => {
    ...
    browser.tabs
      ...
      .then((tabs) => {
        ...
        toggleCaptureButtons(true);
        browser.storage.local.set({ capturingState: { isCapturing: true } }).catch((error) => {
  ...
  stopButton.addEventListener("click", () => {
    browser.tabs
      ...
      .then((tabs) => {
        browser.tabs
          .sendMessage(tabs[0].id, { action: "stopCapture" })
          .then((response) => {
            toggleCaptureButtons(false);
            browser.storage.local.set({ capturingState: { isCapturing: false } }).catch((error) => {
              console.error("Error storing capturing state:", error);
            });
          })
          .catch((error) => {
