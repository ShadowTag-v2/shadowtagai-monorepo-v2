...
def write_error_stats(  # noqa: C901
    ...
    if enable_log:
        logging.info(f"[{test_set_name}] %WER {tot_errs / ref_len:.2%} [{tot_errs} / {ref_len}, {ins_errs} ins, {del_errs} del, {sub_errs} sub ]")

    print(f"%WER = {tot_err_rate}", file=f)
    print(
        f"Errors: {ins_errs} insertions, {del_errs} deletions, {sub_errs} substitutions, over {ref_len} reference words ({num_corr} correct)",
        file=f,
    )
    print(
    ...
    for cut_id, ref, hyp in results:
        ...
        print(
            f"{cut_id}:\t" + " ".join((ref_word if ref_word == hyp_word else f"({ref_word}->{hyp_word})" for ref_word, hyp_word in ali)),
            file=f,
        )
