...
class TranscriptionCollectorClient:
  ...
  def __init__(self, redis_stream_url=None):
    ...
    self.redis_url = (
      redis_stream_url or os.getenv("REDIS_STREAM_URL") or "redis://localhost:6379/0"
    )
    logging.info(
...
class TranscriptionServer:
  ...
  def __init__(self):
    ...
    self.collector_client: TranscriptionCollectorClient | None = None
  ...
  def handle_new_connection(
    ...
    try:
      ...
      missing_fields = [
        field for field in required_fields if field not in options or not options[field]
      ]
  ...
  def _consul_deregister_service(self):
    if not getattr(self, "_consul_enabled", False):
      return
    url = (
      f"{self._consul_http_addr}/v1/agent/service/deregister/{self._consul_service_id}"
    )
    import urllib.request as _urllib_request
      ...
      try:
        ...
        if self.circuit_breaker_enabled:
          ...
          if (now - self.server_start_ts) < self.server_warmup_s:
            ...
            speaker_active = (
              ...
              and (now - self.last_speaker_event_ts) <= self.speaker_active_window_s
            )
            ...
            if websocket_ok and redis_ok and speaker_active:
              ...
              if (
                no_tx_age is not None and no_tx_age >= self.server_speaker_no_tx_stall_s
              ):
        ...
        if websocket_ok and redis_ok:
          if self.unhealthy_streak > 0:
            logging.info(
              "Self-monitor: Service recovered. WebSocket: OK, Redis: OK. Unhealthy streak reset."
            )
          self.unhealthy_streak = 0
        else:
        ...
        logging.error(
          f"Self-monitor: Unexpected error in monitoring loop: {e}", exc_info=True
        )
        self.unhealthy_streak += 1  # Count this as an unhealthy check to be safe
        if self.unhealthy_streak >= self.max_unhealthy_streak:
          logging.critical(
            "Self-monitor: Exiting due to repeated errors in monitoring loop."
          )
          self._graceful_shutdown_and_exit()
          return  # Exit thread
  ...
  def _graceful_shutdown_and_exit(self):
    ...
    if self.health_server:
      try:
        ...
        logging.error(
          f"Self-monitor: Error shutting down HTTP health_server: {e}", exc_info=True
        )
    ...
    class HealthCheckHandler(http.server.SimpleHTTPRequestHandler):
      ...
      def __init__(
        ...
        self.redis_collector = (
          redis_collector_ref  # This is the TranscriptionCollectorClient instance
        )
      ...
      def do_GET(self):
        ...
        if self.path == "/health":
          ...
          if self.transcription_server_instance is None:
            ...
            server_id = getattr(
              self.transcription_server_instance, "_consul_service_id", "unknown"
            )
  ...
  def send_transcription_to_client(self, segments):
    ...
    try:
      ...
      try:
        ...
        last_range = (
          f"{last.get('start', 'N/A')}-{last.get('end', 'N/A')}" if last else "N/A"
        )
...
class ServeClientTensorRT(ServeClientBase):
  ...
  def __init__(
    ...
    self.websocket.send(
      json.dumps(
        {"uid": self.client_uid, "message": self.SERVER_READY, "backend": "tensorrt"}
      )
    )
    ...
    if len(segments) > 1 and segments[-1].no_speech_prob <= self.no_speech_thresh:
      for i, s in enumerate(segments[:-1]):
        ...
        self.transcript.append(
          self.format_segment(
            start, end, filtered_text, completed=True, language=self.language
          )
        )
