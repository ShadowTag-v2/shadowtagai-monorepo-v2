...
class BotExitCallbackPayload(BaseModel):
  ...
  platform_specific_error: str | None = Field(
    None, description="Platform-specific error message or details."
  )
  completion_reason: MeetingCompletionReason | None = Field(
    None, description="Reason for completion if applicable."
  )
  failure_stage: MeetingFailureStage | None = Field(
...
async def _delayed_container_stop(
  ...
  try:
    ...
    logger.error(
      f"[Delayed Stop] Error stopping container {container_id}: {e}", exc_info=True
    )
...
async def bot_status_change_callback(
  ...
  try:
    ...
    logger.error(
      f"Bot status change callback: An unexpected error occurred: {e}", exc_info=True
    )
    await db.rollback()
    raise HTTPException(
      status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
