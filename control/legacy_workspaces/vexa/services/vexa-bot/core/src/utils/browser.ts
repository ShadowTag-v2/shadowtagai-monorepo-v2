...
export class BrowserAudioService {
  ...
  async findMediaElements(retries: number = 5, delay: number = 2000): Promise<HTMLMediaElement[]> {
    for (let i = 0; i < retries; i++) {
      const mediaElements = Array.from(document.querySelectorAll("audio, video")).filter(
        (el: any) =>
          el.srcObject instanceof MediaStream && el.srcObject.getAudioTracks().length > 0,
      ) as HTMLMediaElement[];
  ...
  private resampleAudioData(inputData: Float32Array, sourceSampleRate: number): Float32Array {
    ...
    for (let i = 1; i < targetLength - 1; i++) {
      ...
      resampledData[i] =
        inputData[leftIndex] + (inputData[rightIndex] - inputData[leftIndex]) * fraction;
    }
...
export class BrowserWhisperLiveService {
  ...
  private startStubbornReconnection(): void {
    ...
    const delay = Math.min(this.retryDelayMs * 1.5 ** Math.min(this.retryCount, 10), 10000);
