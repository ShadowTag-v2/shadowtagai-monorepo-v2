          ...
          audioService
            ...
            .then(async (processor: any) => {
              ...
              audioService.setupAudioDataProcessor(
                async (audioData: Float32Array, sessionStartTime: number | null) => {
            ...
            .then(() => {
              ...
              const initializeTeamsSpeakerDetection = (
                ...
                function isElementActuallyVisible(el: HTMLElement): boolean {
                  ...
                  const occluded = !!el.closest(
                    (selectors as any).occlusionSelectors?.[0] || ".__no_occlusion__",
                  );
                  return (
                    rect.width > 0 &&
                    rect.height > 0 &&
                  ...
                  try {
                    ...
                    for (const sel of containerSelectors) {
                      document
                        ...
                        .forEach((el) => containers.push(el as HTMLElement));
                    }
                ...
                function getContainerForIndicator(indicator: HTMLElement): HTMLElement | null {
                  ...
                  let hops = 0;
                  while (parent && hops < 5) {
                    if (parent.hasAttribute("data-tid") || parent.hasAttribute("data-stream-type"))
                      return parent;
                ...
                const meetingContainer =
                  document.querySelector(selectorsTyped.meetingContainerSelectors[0]) ||
                  document.body;
                ...
                function collectAriaParticipants(): string[] {
                  try {
                    ...
                    for (const item of menuItems) {
                      const hasImg = !!(
                        item.querySelector("img") || item.querySelector('[role="img"]')
                      );
                      if (!hasImg) continue;
              ...
              const setupTeamsMeetingMonitoring = (
                ...
                const checkInterval = setInterval(() => {
                  ...
                  const currentParticipantCount = (window as any).getTeamsActiveParticipantsCount
                    ? (window as any).getTeamsActiveParticipantsCount()
                    : 0;
                  ...
                  if (currentParticipantCount !== lastParticipantCount) {
                    ...
                    const participantList = (window as any).getTeamsActiveParticipants
                      ? (window as any).getTeamsActiveParticipants()
                      : [];
                    (window as any).logBot(
