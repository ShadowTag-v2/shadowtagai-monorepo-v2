...
async def forward_request(
  ...
  if is_admin_request:
    ...
    if admin_key:
      ...
      print("DEBUG: Forwarding x-admin-api-key header")
    else:
      print("DEBUG: No x-admin-api-key header found in request")
  else:
  ...
  try:
    ...
    resp = await client.request(
      method, url, headers=headers, params=forwarded_params or None, content=content
    )
...
async def get_transcript_proxy(
  ...
  url = (
    f"{TRANSCRIPTION_COLLECTOR_URL}/transcripts/{platform.value}/{native_meeting_id}"
  )
  return await forward_request(app.state.http_client, "GET", url, request)
