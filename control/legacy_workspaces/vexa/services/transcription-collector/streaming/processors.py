...
async def process_session_start_event(
  ...
  try:
    ...
    stmt_session = select(MeetingSession).where(
      MeetingSession.meeting_id == meeting.id, MeetingSession.session_uid == session_uid
    )
  ...
  try:
    ...
    async with async_session_local() as db:
      try:
        ...
        logger.error(
          f"DB/Lookup error preparing for message {message_id}: {db_err}", exc_info=True
        )
        await db.rollback()
        return False
        ...
        if session_uid_from_payload:
          ...
          if not session_start_utc:
            ...
            if session_row and getattr(session_row, "session_start_time", None):
              ...
              try:
                await redis_c.set(
                  session_start_cache_key, session_start_utc.isoformat(), ex=7200
                )
                logger.debug(
    ...
    logger.error(
      f"Unexpected error in process_stream_message for {message_id}: {e}", exc_info=True
    )
    return False
