...
async def startup():
  ...
  try:
    ...
    await redis_client.xgroup_create(
      name=REDIS_STREAM_NAME, groupname=REDIS_CONSUMER_GROUP, id="0", mkstream=True
    )
    logger.info(
  ...
  try:
    ...
    if "BUSYGROUP Consumer Group name already exists" in str(e):
      ...
      logger.error(
        f"Failed to create Redis consumer group for speaker events: {e}", exc_info=True
      )
      return
  ...
  for i, task in enumerate(tasks_to_cancel):
    if task and not task.done():
      ...
      try:
        ...
        logger.error(
          f"Error during background task {i + 1} cancellation: {e}", exc_info=True
        )
