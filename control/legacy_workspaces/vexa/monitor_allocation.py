...
class WhisperLiveMonitor:
  ...
  def discover_servers(self) -> list[dict]:
    ...
    try:
      ...
      response = requests.get(
        f"{self.consul_url}/v1/health/service/whisperlive?passing=true", timeout=5
      )
  ...
  def display_allocation_matrix(self, servers: list[dict], loads: list[tuple]):
    ...
    print("\n" + "=" * 80)
    print(
      f"📊 WhisperLive Server Load Monitor - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print("=" * 80)
    ...
    # Header
    print(
      f"{'Server ID':<25} {'Address':<18} {'Load':<8} {'Status':<12} {'Load Bar':<20}"
    )
    print("-" * 80)
...
def main():
  ...
  parser.add_argument(
    "--once", action="store_true", help="Run once and exit (no continuous monitoring)"
  )
