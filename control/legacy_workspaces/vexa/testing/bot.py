...
class Bot:
  ...
  def __init__(
    self, user_client: VexaClient, meeting_url: str, bot_id: str | None = None
  ):
    """
  ...
  def create(
    self, bot_name: str | None = None, language: str = "en", task: str = "transcribe"
  ) -> dict[str, Any]:
    """
    ...
    try:
      ...
      if segments:
        ...
        for seg in segments:
          ...
          if abs_start:
            if first_abs_start is None or pd.to_datetime(abs_start) < pd.to_datetime(
              first_abs_start
            ):
              first_abs_start = abs_start
          if abs_end:
            if last_abs_end is None or pd.to_datetime(abs_end) > pd.to_datetime(
              last_abs_end
            ):
  ...
  def get_meeting_status(self) -> dict[str, Any] | None:
    """
  ...
  def update_config(
    self, language: str | None = None, task: str | None = None
  ) -> dict[str, Any]:
    """
