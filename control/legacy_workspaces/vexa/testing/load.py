...
class TestSuite:
    ...
    for i in range(num_users):
      try:
        ...
        user_client = self._create_vexa_client(
          base_url=self.base_url, api_key=user_api_key, user_id=user_data["id"]
        )
  ...
  def scale_to_users(
    self, target_users: int, meeting_urls: list[str], bot_name_prefix: str = "TestBot"
  ) -> dict[str, Any]:
    """
  ...
  def parse_for_pandas(
    self, snapshot_data: dict[str, Any] | None = None
  ) -> list[dict[str, Any]]:
    """
