...
def create_user_client(user_api_key=None, base_url="http://localhost:18056", max_concurrent_bots=1, admin_api_key=None):
    ...
    if user_api_key is None:
        ...
        new_user = admin_client.create_user(email=f"{random.randint(1, 1000000)}@example.com", name="test", max_concurrent_bots=max_concurrent_bots)
...
def request_bot(client, platform, native_meeting_id, passcode=None, bot_name="Vexa bot", language="en", task="transcribe"):
    """Request a bot for a meeting."""
    return client.request_bot(
        platform=platform, native_meeting_id=native_meeting_id, bot_name=bot_name, language=language, task=task, passcode=passcode
    )
