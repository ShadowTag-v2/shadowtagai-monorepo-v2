...
async function buildPlaygroundFiles() {
  ...
  const packageManifest = {
    ...
    plugins: await Promise.all(
      pluginFiles.map(async (file) => {
        ...
        const pluginModule = await import(url.pathToFileURL(path.join(PACKAGES_DIRECTORY, file)));
  ...
  await Promise.all([
    ...[packageManifest.prettier, ...packageManifest.plugins].map(({ file }) =>
      copyFile(path.join(PACKAGES_DIRECTORY, file), path.join(PLAYGROUND_LIB_DIRECTORY, file)),
    ),
    writeScript(
