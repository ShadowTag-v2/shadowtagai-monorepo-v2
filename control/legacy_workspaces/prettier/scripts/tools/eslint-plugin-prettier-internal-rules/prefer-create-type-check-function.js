...
const isMultipleTypeAccessCheck = (node, parameterName) =>
  isSetHasOrArrayIncludesCall(node) && isTypeAccess(node.arguments[0], parameterName);
    ...
    return {
      [functionSelector](functionNode) {
        ...
        const commentsInTypes =
          commentsInFunction === 0
            ? 0
            : types.reduce(
                (count, { node }) => count + sourceCode.getCommentsInside(node).length,
                0,
              );
      ...
      [setDeclareSelector](identifier) {
        const variable = sourceCode
          ...
          .find(
            (variable) =>
              variable.identifiers.length === 1 && variable.identifiers[0] === identifier,
          );
        ...
        for (const { identifier: referenceIdentifier } of references) {
          ...
          if (
            !(
              ...
              referenceIdentifier.parent.parent.callee === referenceIdentifier.parent &&
          ...
          const memberExpression =
            typeNode.type === "ChainExpression" ? typeNode.expression : typeNode;
        ...
        context.report({
          ...
          *fix(fixer) {
            ...
            for (const { setHasNode, typeNode, nodeNode } of nodes) {
              ...
              // `foo.type` -> `foo`
              yield fixer.replaceText(typeNode, `(${sourceCode.getText(nodeNode)})`);
            }
          },
        });
      },
