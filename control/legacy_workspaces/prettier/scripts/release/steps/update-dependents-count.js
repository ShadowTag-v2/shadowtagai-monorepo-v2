...
async function getNpmDependentsCount() {
  ...
  const dependentsCountNpm = Number(
    npmPage.match(/"dependentsCount":"(?<dependentsCount>\d+)",/u).groups.dependentsCount,
  );
...
async function update({ repo }) {
  ...
  ] = await Promise.allSettled([getNpmDependentsCount(), getGithubDependentsCount()]);
  ...
  if (dependentsCountNpm || dependentsCountGithub) {
    ...
    const isUpdated = await logPromise(
      ...
      async () => (await runGit(["diff", "--name-only"])).stdout === "website/src/pages/index.jsx",
    );
