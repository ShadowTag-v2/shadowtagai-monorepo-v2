...
const playgroundLink =
  process.env.NODE_ENV === "production" ? "pathname:///playground/" : "http://localhost:5173/";
...
function DraggableLogo() {
  ...
  return (
    ...
    >
      <AnimatedLogo version="wide" rolling={rolling} onAnimationEnd={() => setRolling(false)} />
    </div>
  );
}
    ...
    <section className={styles.syntaxSection}>
      ...
      >
        <div className="margin-bottom--md">
          Limited edition tshirts are now available to buy! $10 per tshirt goes to maintain the
          project.
        </div>
...
function LanguagesSection() {
  ...
  for (const language of languages) {
    ...
    if (last && last.languages.length < 2 && last.variantsCount + languageVariants < 9) {
  ...
  const showcase = users
    ...
    .map((user) => (
      <a key={user.caption} href={user.infoLink} className={styles.userItem}>
        <img src={user.greyImage} title={user.caption} className={styles.userItemImage} />
      </a>
    ));
    ...
    <div className={clsx(styles.sectionPadding, styles.userSection)}>
      ...
      <div className="container">
        ...
        <div className={styles.ecosystemGrid}>
          ...
          <div className={styles.ecosystemGridItem}>
            <Link to="https://github.com/prettier/prettier" aria-label="Prettier on GitHub">
              <img src="/images/github_grey.svg" alt="" />
            </Link>
            <div>
              <p>
                More than <strong data-placeholder="dependent-github">9.9 million</strong> dependent
          ...
          <div className={styles.ecosystemGridItem}>
            <Link to="https://npmjs.com/package/prettier" aria-label="Prettier on NPM">
              <img src="/images/npm_grey.svg" alt="" />
            </Link>
            <div>
              <p>
                More than <strong data-placeholder="dependent-npm">19.4k</strong> dependent packages
                on npm
              </p>
              <Link
                className="button button--primary"
...
export default function Home() {
  ...
  return (
    <div className="home">
      <Layout title="Prettier · Opinionated Code Formatter" description={siteConfig.tagline}>
        <HomepageHeader />
        <SyntaxSection />
        <TldrSection />
        <LanguagesSection />
