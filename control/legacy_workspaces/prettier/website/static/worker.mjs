...
async function handleFormatMessage(message) {
  ...
  const formatResult = await formatCode(message.code, options, message.debug.rethrowEmbedErrors);
  ...
  if (!isDocExplorer && message.debug.reformat) {
    response.debug.reformatted = (await formatCode(response.formatted, options)).formatted;
  }
