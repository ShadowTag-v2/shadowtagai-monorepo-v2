...
const CATEGORIES_ORDER = ["Global", "Common", "JavaScript", "Markdown", "HTML", "Special"];
...
class Playground extends React.Component {
  constructor(props) {
    ...
    this.rangeStartOption = props.availableOptions.find((opt) => opt.name === "rangeStart");
    this.rangeEndOption = props.availableOptions.find((opt) => opt.name === "rangeEnd");
    this.cursorOffsetOption = props.availableOptions.find((opt) => opt.name === "cursorOffset");
  ...
  componentDidUpdate(_, prevState) {
    ...
    if (!shallowEqual(prevState.options, this.state.options) || prevState.content !== content) {
  ...
  handleOptionValueChange(option, value) {
    this.setState((state) => {
      ...
      const content =
        state.content === "" || state.content === getCodeSample(state.options.parser)
  ...
  insertDummyId() {
    ...
    const modifiedContent = content.slice(0, range[0]) + dummyId + content.slice(range[1]);

    this.setState({
      content: modifiedContent,
      selection: convertOffsetToSelection(range[0] + dummyId.length, modifiedContent),
    });
  }
                ...
                <>
                      ...
                      {isDocExplorer ? null : (
                        <SidebarCategory title="Range">
                          ...
                          <Option
                            ...
                            value={typeof options.rangeStart === "number" ? options.rangeStart : ""}
                            onChange={this.handleOptionValueChange}
                          />
                          <Option
                            option={this.rangeEndOption}
                            value={typeof options.rangeEnd === "number" ? options.rangeEnd : ""}
                      ...
                      {isDocExplorer ? null : (
                        <SidebarCategory title="Cursor">
                          <Option
                            ...
                            value={options.cursorOffset >= 0 ? options.cursorOffset : ""}
                            onChange={this.handleOptionValueChange}
                          />
                          <div
                            style={{
                          ...
                          >
                            ...
                            {options.cursorOffset >= 0 ? (
                              <>
                                <Button
                                  onClick={() => {
                                    this.handleOptionValueChange(this.cursorOffsetOption, -1);
                                  }}
                                >
                                  Reset
                                </Button>
                    ...
                    <div className="editors">
                      ...
                      {editorState.showOutput ? (
                        <OutputPanel
                          ...
                          value={formatted}
                          ruler={options.printWidth}
                          overlayStart={cursorOffset === -1 ? undefined : cursorOffset}
                          overlayEnd={cursorOffset === -1 ? undefined : cursorOffset + 1}
                        />
                      ) : null}
                  ...
                  <div className="bottom-bar">
                    ...
                    <div className="bottom-bar-buttons bottom-bar-buttons-right">
                      <ClipboardButton copy={window.location.href}>Copy link</ClipboardButton>
                      <ClipboardButton
                        copy={() =>
                          this.getMarkdown({
                            formatted,
                      ...
                      >
                        <ClipboardButton copy={() => (showFullReport ? "" : fullReport)}>
                          Report issue
                        </ClipboardButton>
                      </a>
                    </div>
