...
const MyGenericComponent: React.VoidFunctionComponent<MyGenericComponentProps<number>> = ({
  x,
  y,
}) => {
  const a = useA();
...
const Query: FunctionComponent<QueryProps> = ({
  ...
  children(
    useQuery({ type, resource, payload }, { ...options, withDeclarativeSideEffectsSupport: true }),
  );
