...
inJSXExpressionContainer.withLoops.orBooleans.example = (
  <div>
    {items
      ? items.map((item) =>
          item.display ? <Item item={item} attr="breaks ternary but not consequent" /> : <Blank />,
        )
      : null}

    {showTheStuff &&
      (foo ? <Thing thing={foooooooooooooooooooooooooo} bar="bazzzzzz" /> : <OtherThing />)}
  </div>
);
...
inJSXExpressionContainer.withNullConditional = (
  <div>
    {isACat() ? null : <Foo />}
    {isACat() && (someReallyLongCondition || moreInThisLongCondition) ? null : <Foo />}
    {isACat() &&
