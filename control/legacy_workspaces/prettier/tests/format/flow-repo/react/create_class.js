...
const A = React.createClass({
  ...
  m() {
    (this.props.foo
    : empty) // error: string ~> empty
    (this.props.bar
    : empty) // error: number ~> empty
  },
});
...
const G = React.createClass({
  ...
  m() {
    (this.mixins
    : mixed) // error: property `mixins` not found
    (this.autobind
    : mixed) // error: property `autobind` not found
    (this.statics
    : mixed) // error: property `statics` not found
  },
});
...
React.createClass(({}: {})
) // error: spec must be [ ] exact and [x] sealed
