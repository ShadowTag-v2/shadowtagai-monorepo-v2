...
function switch_scope(x: string) {
  ...
  switch (x) {
    case "foo": {
      const a = ""; // ok: local to switch
      var b = ""; // error: string ~> number
      break;
    }
    case "bar": {
      const a = ""; // error: a already bound in switch
      break;
    }
  }
}
...
function for_in_scope_let(o: Object) {
  let a: number = 0;
  for (const a /* ok: local to init */ in o) {}
}
...
function for_of_scope_let(xs: string[]) {
  let a: number = 0;
  for (const a /* ok: local to init */ of xs) {}
}
