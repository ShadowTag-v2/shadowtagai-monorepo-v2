...
(new C1
: IFoo) // ok, we already errored at def site

interface IBar { bar: number }

class C4 implements IFoo, IBar {} // error: properties `foo`, `bar` not found
(new C4
: IBar) // ok, we already errored at def site

interface IFooBar extends IFoo { bar: number }

class C5 implements IFooBar {} // error: properties `foo`, `bar` not found
(new C5
: IFooBar) // ok, already errored at def site
(new C5
: IFoo) // ok, already errored at def site
(new C5
: IBar) // error: property `bar` not found (despite IBar < IFooBar)

class C6 extends C1 {}
(new C6
: IFoo) // ok, C1 implements IFoo
