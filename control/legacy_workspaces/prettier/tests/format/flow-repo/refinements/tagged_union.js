...
const tests = [
  ...
  (x: { str: string, num: number, bool: boolean }) => {
    if (x.str === 'str') {
  ...
  // type mismatch
  (x: { foo: 123, y: string } | { foo: 'foo', z: string }) => {
    if (x.foo === 123) {
      (x.y: string);
      x.z; // error
    } else {
  ...
  (x: { foo: number, y: string } | { foo: 'foo', z: string }) => {
    if (x.foo === 123) {
  ...
  // null
  (x: { foo: null, y: string } | { foo: 'foo', z: string }) => {
    if (x.foo === null) {
      (x.y: string);
      x.z; // error
    } else {
  ...
  // void
  (x: { foo: void, y: string } | { foo: 'foo', z: string }) => {
    if (x.foo === undefined) {
      (x.y: string);
      x.z; // error
    } else {
