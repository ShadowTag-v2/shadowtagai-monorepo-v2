...
function reduce_test() {
   ...
   */
  [0, 1, 2, 3, 4].reduce(
    (previousValue, currentValue, index, array) => previousValue + currentValue + array[index],
  );
...
function from_test() {
  var a: Array<string> = Array.from([1, 2, 3], (val, index) => index % 2 ? "foo" : String(val));
  var b: Array<string> = Array.from([1, 2, 3], (val) => String(val));
}
