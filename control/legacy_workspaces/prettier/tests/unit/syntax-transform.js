...
const file = url.fileURLToPath(new URL("../../src/__dummy.js", import.meta.url));
const shimsDirectory = url.fileURLToPath(new URL("../../scripts/build/shims", import.meta.url));
const publicDocPath = url.fileURLToPath(new URL("../../src/document/public.js", import.meta.url));
const transform = (code) =>
  ...
  transformCode(code, file, {
    ...
    .replaceAll(JSON.stringify(shimsDirectory + path.sep).slice(1, -1), "<SHIMS>/")
    .replaceAll(JSON.stringify(publicDocPath).slice(1, -1), "<PUBLIC_DOC_PATH>") +
  "\n";
...
test("Array#findLast", () => {
  ...
  // Not supported
  expect(transform("foo.findLast(callback, thisArgument)")).toMatchInlineSnapshot(`
      "
...
test("Array#findLastIndex", () => {
  ...
  // Not supported
  expect(transform("foo.findLastIndex(callback, thisArgument)")).toMatchInlineSnapshot(`
      "
...
test("String.raw", () => {
  expect(transform("String.raw`\\\\\\uINVALID`")).toMatchInlineSnapshot(String.raw`
"
"\\\\\\uINVALID";
"
`);
...
test("public doc functionality", () => {
  expect(transform('import {align, line} from "./document/index.js"')).toMatchInlineSnapshot(`
      "
