...
const CLI_WORKER_FILE = url.fileURLToPath(new URL("./cli-worker.js", import.meta.url));
const INTEGRATION_TEST_DIRECTORY = url.fileURLToPath(new URL("./", import.meta.url));
const IS_CI = Boolean(process.env.CI);
const removeFinalNewLine = (string) => (string.endsWith("\n") ? string.slice(0, -1) : string);
const SUPPORTS_DISABLE_WARNING_FLAG = Number(process.versions.node.split(".")[0]) >= 20;
const promiseWithResolvers = Promise.withResolvers
  ? Promise.withResolvers.bind(Promise)
  : () => {
      let resolve;
      let reject;
      const promise = new Promise((_resolve, _reject) => ([resolve, reject] = [_resolve, _reject]));
...
function runCliWorker(dir, args, options) {
  ...
  const createEpipeErrorHandler = (event) => (error) => {
    ...
    if (error.code === "EPIPE" && error.syscall === "write" && nodeOptions.length > 0) {
      if (IS_CI) {
        // eslint-disable-next-line no-console
        console.error(Object.assign(error, { event, dir, args, options, worker }));
      }
      return;
    }
  ...
  const worker = childProcess.fork(CLI_WORKER_FILE, args, {
    ...
    execArgv: [
      "--trace-deprecation",
      ...(SUPPORTS_DISABLE_WARNING_FLAG ? ["--disable-warning=ExperimentalWarning"] : []),
      ...nodeOptions,
    ],
    ...
    test(options.title || "", async () => {
      ...
      for (const name of ["status", "stdout", "stderr", "write"]) {
        ...
        if (name === "stdout" || name === "stderr") {
          value = result[name].replace(/\r/gu, options.ignoreLineEndings ? "" : "/*CR*/");
        }
