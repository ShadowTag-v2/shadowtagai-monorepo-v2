...
test("Plugins in options", async () => {
  ...
  const pluginUrl = new URL("../plugins/get-parse-options/plugin.js", import.meta.url);
  ...
  for (const { plugin, expected } of [
    ...
    const { name } = options.plugins.find((plugin) => plugin.__plugin_internal_name === pluginName);
