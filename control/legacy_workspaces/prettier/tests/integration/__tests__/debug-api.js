...
describe("API", () => {
  ...
  test("prettier.printToDoc", async () => {
    const hasCursor = (doc) =>
      findInDoc(doc, (doc) => (doc.type === "cursor" ? true : undefined)) ?? false;
  ...
  test("prettier.formatDoc", async () => {
    ...
    const doc2 = new Function(`{ ${Object.keys(builders)} }`, `return ${formatResultFromDoc}`)(
      builders,
    );
  ...
  test("prettier.formatDoc prints things as expected", async () => {
    const { indent, hardline, literalline, fill, ifBreak, indentIfBreak, group, line, label } =
      builders;
    ...
    expect(await formatDoc(fill(["foo", hardline, "bar", literalline, "baz"]))).toBe(
      'fill(["foo", hardline, "bar", literalline, "baz"])',
    );

    expect(await formatDoc(indentIfBreak(group(["1", line, "2"]), { groupId: "Q" }))).toBe(
      'indentIfBreak(group(["1", line, "2"]), { groupId: "Q" })',
    );
