...
test("API getFileInfo with filepath only", async () => {
  ...
  await expect(prettier.getFileInfo("tsconfig.json", { resolveConfig: false })).resolves.toEqual({
    ignored: false,
    inferredParser: "json",
  });
});
...
describe("API getFileInfo resolveConfig", () => {
  ...
  test("{resolveConfig: true}", async () => {
    await expect(prettier.getFileInfo(files.foo, { resolveConfig: true })).resolves.toEqual({
      ignored: false,
      inferredParser: "foo-parser",
    });
    await expect(prettier.getFileInfo(files.js, { resolveConfig: true })).resolves.toEqual({
      ignored: false,
      inferredParser: "override-js-parser",
    });
    await expect(prettier.getFileInfo(files.bar, { resolveConfig: true })).resolves.toEqual({
      ignored: false,
      inferredParser: null,
    });
    await expect(prettier.getFileInfo(files.css, { resolveConfig: true })).resolves.toEqual({
      ignored: false,
      inferredParser: "css",
    });
  });
...
test("extracts file-info for a JS file with no extension but a standard shebang", async () => {
  await expect(prettier.getFileInfo("tests/integration/cli/shebang/node-shebang")).resolves.toEqual(
    {
      ignored: false,
      inferredParser: "babel",
    },
