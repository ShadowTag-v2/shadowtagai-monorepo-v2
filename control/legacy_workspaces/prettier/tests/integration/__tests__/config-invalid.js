...
describe("throw error for unsupported extension", () => {
  runCli("cli/config/invalid", ["--config", "file/.prettierrc.unsupported"]).test({
    status: "non-zero",
  });
});
...
describe("throw error with invalid config precedence option (configPrecedence)", () => {
  runCli("cli/config/invalid", ["--config-precedence", "option/configPrecedence"]).test({
    status: "non-zero",
  });
});
...
describe("show warning with unknown option", () => {
  runCli("cli/config/invalid", ["--config", "option/unknown", "--parser", "babel"]).test({
    status: 0,
  });
});

describe("show warning with kebab-case option key", () => {
  runCli("cli/config/invalid", ["--config", "option/kebab-case", "--parser", "babel"]).test({
    status: 0,
  });
});
