...
describe("--cache option", () => {
  ...
  const defaultCacheFile = path.join(dir, "node_modules/.cache/prettier/.prettier-cache");
...
`;
  ...
  it("doesn't create cache directory without --cache", async () => {
    ...
    await expect(fs.stat(path.join(dir, "node_modules/.cache"))).rejects.toHaveProperty(
      "code",
      "ENOENT",
    );
  });
  ...
  it("throws error when use with --stdin-filepath", async () => {
    ...
    expect(stderr.trim()).toBe("[error] `--cache` cannot be used when formatting stdin.");
  });
  ...
  it("throws error when use `--cache-strategy` without `--cache`.", async () => {
    ...
    expect(stderr.trim()).toBe("[error] `--cache-strategy` cannot be used without `--cache`.");
  });
  ...
  it("throws error when `--cache-location` is a directory.", async () => {
    ...
    expect(stderr.trim()).toEqual(
      expect.stringMatching(/\[error\] Resolved --cache-location '.+' is a directory/u),
    );
  });

  describe("--cache-strategy metadata", () => {
    it("creates default cache file named `node_modules/.cache/prettier/.prettier-cache`", async () => {
      await expect(fs.stat(defaultCacheFile)).rejects.toHaveProperty("code", "ENOENT");
      await runCliWithoutGitignore(dir, ["--cache", "--cache-strategy", "metadata", "*.js"]);
      await expect(fs.stat(defaultCacheFile)).resolves.not.toThrow();
    });
    ...
    it("re-formats when a file has been updated.", async () => {
      const cliArguments = ["--cache", "--write", "--cache-strategy", "metadata", "*.js"];
      const { stdout: firstStdout } = await runCliWithoutGitignore(dir, cliArguments);
      expect(firstStdout.split("\n")).toEqual(
        expect.arrayContaining([
          expect.stringMatching(/^a\.js .+ms$/u),
          expect.stringMatching(/^b\.js .+ms \(unchanged\)$/u),
      ...
      const { stdout: secondStdout } = await runCliWithoutGitignore(dir, cliArguments);
      expect(secondStdout.split("\n")).toEqual(
    ...
    it("re-formats when multiple cached files are updated.", async () => {
      const cliArguments = ["--write", "--cache", "--cache-strategy", "metadata", "*.js"];
      ...
      const { stdout: thirdStdout } = await runCliWithoutGitignore(dir, cliArguments);
  ...
  describe("--cache-strategy content", () => {
    it("creates default cache file named `node_modules/.cache/prettier/.prettier-cache`", async () => {
      await expect(fs.stat(defaultCacheFile)).rejects.toHaveProperty("code", "ENOENT");
      await runCliWithoutGitignore(dir, ["--cache", "--cache-strategy", "content", "*.js"]);
      await expect(fs.stat(defaultCacheFile)).resolves.not.toThrow();
    });

    it("doesn't format when cache is available", async () => {
      const cliArguments = ["--cache", "--cache-strategy", "content", "--write", "*.js"];
      const { stdout: firstStdout } = await runCliWithoutGitignore(dir, cliArguments);
      expect(firstStdout.split("\n")).toEqual(
        expect.arrayContaining([
          expect.stringMatching(/^a\.js .+ms$/u),
    ...
    it("re-formats when multiple cached files are updated.", async () => {
      const cliArguments = ["--write", "--cache", "--cache-strategy", "content", "*.js"];
      await runCliWithoutGitignore(dir, cliArguments);
  ...
  describe("--cache-location", () => {
    ...
    describe("file", () => {
      it("creates the cache file at location specified by `--cache-location`", async () => {
        await expect(fs.stat(nonDefaultCacheFilePath)).rejects.toHaveProperty("code", "ENOENT");
        await runCliWithoutGitignore(dir, [
          "--cache",
          "--cache-location",
          nonDefaultCacheFileName,
