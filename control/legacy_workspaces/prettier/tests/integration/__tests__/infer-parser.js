...
describe("stdin with unknown path and no parser", () => {
  ...
  describe("--list-different logs error but exits with 0", () => {
    runCli("cli/infer-parser/", ["--list-different", "--stdin-filepath", "foo"], {
      input: "foo",
    }).test({
      status: 0,
      stdout: "",
...
describe("Allow plugin to override builtin plugins", () => {
  runCli("cli/infer-parser/override-builtin-plugin-languages", ["--stdin-filepath=foo.js"], {
    input: "foo(   )",
...
describe("Known/Unknown", () => {
  runCli("cli/infer-parser/known-unknown", ["--end-of-line", "lf", "--list-different", "."]).test({
    status: 1,
    stderr: "",
    write: [],
  });
...
describe("isSupported", () => {
  ...
  test("API", async () => {
    ...
    expect(await getIsSupportedReceivedFilepath({ filepath: fileUrl })).toBe(filePath);
    expect(await getIsSupportedReceivedFilepath({ filepath: fileUrl.href })).toBe(filePath);
    expect(await getIsSupportedReceivedFilepath({ filepath: filePath })).toBe(filePath);

    // Relative path
    expect(await getIsSupportedReceivedFilepath({ filepath: "./foo.unknown" })).toBe(
      "./foo.unknown",
    );
    ...
    expect(await getIsSupportedReceivedFilepath({ filepath: "file://%0" })).toBeUndefined();
