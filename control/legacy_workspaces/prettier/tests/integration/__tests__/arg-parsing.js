describe("boolean flags do not swallow the next argument", () => {
  runCli("cli/arg-parsing", ["--end-of-line", "lf", "--single-quote", "file.js"]).test({
    status: 0,
  });
});

describe("negated options work", () => {
  runCli("cli/arg-parsing", ["--end-of-line", "lf", "--no-semi", "file.js"]).test({
    status: 0,
  });
});

describe("unknown options are warned", () => {
  runCli("cli/arg-parsing", ["--end-of-line", "lf", "file.js", "--unknown"]).test({
    status: 0,
  });
});

describe("unknown negated options are warned", () => {
  runCli("cli/arg-parsing", ["--end-of-line", "lf", "file.js", "--no-unknown"]).test({
    status: 0,
  });
});
...
describe("allow overriding flags", () => {
  runCli("cli/arg-parsing", ["--tab-width=1", "--tab-width=3", "--parser=babel"], {
    input: "function a() { b }",
  }).test({
    stdout: "function a() {\n   b;\n}",
    status: 0,
...
describe("number file/dir", () => {
  ...
  for (const pattern of patterns) {
    runCli("cli/arg-parsing/number", ["--parser=babel", "--list-different", pattern]).test({
      stderr: "",
      status: 1,
      write: [],
    });
  }
  runCli("cli/arg-parsing/number", ["--parser=babel", "--list-different", ...patterns]).test({
    stderr: "",
    status: 1,
    write: [],
  });
