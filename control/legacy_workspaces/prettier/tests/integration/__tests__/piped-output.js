describe("output with --check + unformatted differs when piped", () => {
  const cli0 = runCli("cli/write", ["--write", "--check", "--no-color", "unformatted.js"], {
    stdoutIsTTY: true,
  }).test({
    status: 0,
  });

  const cli1 = runCli("cli/write", ["--write", "--check", "--no-color", "unformatted.js"], {
    stdoutIsTTY: false,
  }).test({
    status: 0,
  });
...
describe("no file diffs with --check + formatted file", () => {
  const cli0 = runCli("cli/write", ["--write", "--check", "--no-color", "formatted.js"], {
    stdoutIsTTY: true,
  }).test({
    status: 0,
  });

  const cli1 = runCli("cli/write", ["--write", "--check", "--no-color", "formatted.js"], {
    stdoutIsTTY: false,
  }).test({
    status: 0,
  });
...
describe("no file diffs with --list-different + formatted file", () => {
  const cli0 = runCli("cli/write", ["--write", "--list-different", "--no-color", "formatted.js"], {
    stdoutIsTTY: true,
  }).test({
    status: 0,
  });

  const cli1 = runCli("cli/write", ["--write", "--list-different", "--no-color", "formatted.js"], {
    stdoutIsTTY: false,
  }).test({
    status: 0,
  });
