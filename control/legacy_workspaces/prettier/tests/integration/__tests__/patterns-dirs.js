...
describe("Trailing slash", () => {
  ...
  testPatterns("run in sub dir 1", [".."], { status: 1, stderr: "" }, "cli/patterns-dirs/dir2");
  testPatterns("run in sub dir 2", ["../"], { status: 1, stderr: "" }, "cli/patterns-dirs/dir2");
  testPatterns(
    "run in sub dir 3",
    ["../dir1"],
    { status: 1, stderr: "" },
...
describe("plugins `.`", () => {
  runCliWithoutGitignore("cli/dirs/plugins", [".", "-l", "--plugin", uppercaseRocksPlugin]).test({
    write: [],
    stderr: "",
    status: 1,
  });
...
if (path.sep === "/") {
  ...
  describe("Backslashes in names", () => {
    ...
    testPatterns("", ["test-a\\"], { stdout: String.raw`test-a\/test.js` }, base);
    testPatterns("", ["test-a*/*"], { stdout: String.raw`test-a\/test.js` }, base);
    ...
    testPatterns("", ["test-b*/*"], { stdout: String.raw`test-b\?/test.js` }, base);
  });
}
...
(isSymlinkSupported() ? describe : describe.skip)("Ignore symlinks", () => {
  ...
  fs.symlinkSync(path.join(directoryA, "a.js"), path.join(directoryA, "symlink-to-file-a"));
  fs.symlinkSync(path.join(directoryB, "b.js"), path.join(directoryA, "symlink-to-file-b"));
  ...
  testPatterns(
    ...
    ["test-a/symlink-to-file-a"],
    {
      status: 2,
      stdout: "",
      stderr: '[error] Explicitly specified pattern "test-a/symlink-to-file-a" is a symbolic link.',
    },
    base,
  );
  testPatterns(
    ...
    ["test-a/symlink-to-file-b"],
    {
      status: 2,
      stdout: "",
      stderr: '[error] Explicitly specified pattern "test-a/symlink-to-file-b" is a symbolic link.',
    },
    base,
  );
  testPatterns(
    ...
    ["test-a/symlink-*"],
    {
      status: 2,
      stdout: "",
      stderr: '[error] No files matching the pattern were found: "test-a/symlink-*".',
    },
    base,
  );
  testPatterns(
  ...
  testPatterns(
    "",
    ["test-a/symlink-to-file-b", "--no-error-on-unmatched-pattern", "--log-level", "debug"],
    {
      status: 0,
      stdout: "",
      stderr:
        '[debug] normalized argv: {"_":["test-a/symlink-to-file-b"],"cache":false,"color":true,"editorconfig":true,"errorOnUnmatchedPattern":false,"logLevel":"debug","ignorePath":[".prettierignore"],"configPrecedence":"cli-override","debugRepeat":0,"plugins":[],"listDifferent":true,"__raw":{"_":["test-a/symlink-to-file-b"],"cache":false,"color":true,"editorconfig":true,"error-on-unmatched-pattern":false,"l":true,"log-level":"debug","ignore-path":".prettierignore","config-precedence":"cli-override","debug-repeat":0,"plugin":[]}}' +
        "\n" +
...
function testPatterns(namePrefix, cliArgs, expected = {}, cwd = "cli/patterns-dirs") {
  const testName =
    (namePrefix ? namePrefix + ": " : "") +
    "prettier " +
    cliArgs.map((arg) => (/^[\w./=-]+$/u.test(arg) ? arg : `'${arg}'`)).join(" ");
