...
const getCategory = (dirname) => path.relative(TESTS_ROOT, dirname).split(path.sep).shift();
...
const categoryParsers = new Map([
  ...
  ["css", { parsers: ["css"], verifyParsers: ["less", "scss"], extensions: [".css"] }],
  [
    "flow",
    {
  ...
  ["graphql", { parsers: ["graphql"], verifyParsers: [], extensions: [".graphql"] }],
  ["handlebars", { parsers: ["glimmer"], verifyParsers: [], extensions: [".hbs"] }],
  ["html", { parsers: ["html"], verifyParsers: [], extensions: [".html", ".svg"] }],
  ["mjml", { parsers: ["mjml"], verifyParsers: ["html"], extensions: [".mjml"] }],
  [
    "js",
    {
  ...
  ["markdown", { parsers: ["markdown"], verifyParsers: [], extensions: [".md"] }],
  ...
  ["stylefmt-repo", { parsers: ["css", "scss"], verifyParsers: [], extensions: [".css"] }],
  [
    "typescript",
    {
  ...
  ["vue", { parsers: ["vue"], verifyParsers: [], extensions: [".vue", ".html"] }],
...
const checkParser = ({ dirname, files }, parsers = []) => {
  ...
  if (verifyParsers.includes(parser)) {
    throw new Error(
      `verifyParsers ${JSON.stringify(verifyParsers)} should not include parser "${parser}".`,
    );
  }
    ...
    const suggestion =
      suggestCategories.length === 0
        ...
        : outdent`
          ...
          ${suggestCategories.map((category) => `- ${path.join(TESTS_ROOT, category)}`).join("\n")}

          Or config to allow use this parser in "${__filename}".
        `;

    throw new Error(
      `Parser "${parser}" should not used in "${dirname}".${suggestion ? `\n\n${suggestion}` : ""}`,
    );
  }
