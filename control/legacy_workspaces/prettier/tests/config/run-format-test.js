...
const unstableTests = new Map(
  [
    ...
    ["js/multiparser-markdown/codeblock.js", (options) => options.proseWrap === "always"],
    ...
    const [file, isUnstable = () => true] = Array.isArray(fixture) ? fixture : [fixture];
...
const isTestDirectory = (dirname, name) =>
  (dirname + path.sep).startsWith(path.join(__dirname, "../format", name) + path.sep);
...
function runFormatTest(fixtures, parsers, options) {
  ...
  const IS_PARSER_INFERENCE_TESTS = isTestDirectory(dirname, "misc/parser-inference");
  ...
  const IS_TYPESCRIPT_ONLY_TEST = isTestDirectory(dirname, "misc/typescript-only");
  ...
  for (const { name, filename, code, output } of [...files, ...snippets]) {
    const title = `${name}${stringifiedOptions ? ` - ${stringifiedOptions}` : ""}`;
    ...
    describe(title, () => {
      ...
      const shouldThrowOnMainParserFormat = shouldThrowOnFormat(name, formatOptions);
      ...
      for (const currentParser of allParsers) {
        if (
          ...
          (currentParser === "typescript" && typescriptDisabledTests.has(filename))
        ) {
          continue;
        }

        const testTitle =
          shouldThrowOnMainParserFormat || formatOptions.parser !== currentParser
...
async function runTest({
  ...
  if (mainParserFormatResult.error || mainParserFormatOptions.parser !== parser) {
  ...
  if (typeof output === "string") {
    expect(formatResult.eolVisualizedOutput).toEqual(visualizeEndOfLine(output));
    return;
  }
  ...
  if (
    ...
    const { eolVisualizedOutput: secondOutput } = await format(output, formatOptions);
    if (isUnstableTest) {
  ...
  if (code.charAt(0) !== BOM) {
    const { eolVisualizedOutput: output } = await format(BOM + code, formatOptions);
...
async function format(originalText, originalOptions) {
  const { text: input, options } = replacePlaceholders(originalText, originalOptions);
...
async function loadPlugins(options) {
  ...
  if (externalPlugins.has(options.parser)) {
    ...
    const url = new URL(
      isProduction ? `../../dist/${pluginName}/index.mjs` : `../../packages/${pluginName}/index.js`,
      import.meta.url,
    );
