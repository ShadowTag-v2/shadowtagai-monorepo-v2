...
const escapeArgument = isWindows
  ? (argument) =>
      escapeFile(
        escapeFile(
          `"${argument.replace(/(\\*)"/gu, String.raw`$1$1\"`).replace(/(\\*)$/u, "$1$1")}"`,
        ),
      )
  : (argument) => argument;
const spawn = (command, args, options) =>
  spawnSync([escapeFile(command), ...args.map((argument) => escapeArgument(argument))].join(" "), {
    ...options,
    shell: true,
  });
...
function installPrettier(packageDirectory) {
  ...
  const runNpmClient = (args) => spawn(client, args, { cwd: temporaryDirectory });
  ...
  fs.writeFileSync(path.join(temporaryDirectory, "index-proxy.mjs"), "export * from 'prettier';");
