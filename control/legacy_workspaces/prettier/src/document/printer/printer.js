  ...
  while (remainingWidth >= 0) {
    ...
    switch (docType) {
      ...
      case DOC_TYPE_GROUP: {
        ...
        const contents =
          doc.expandedStates && groupMode === MODE_BREAK ? doc.expandedStates.at(-1) : doc.contents;
...
function printDocToString(doc, options) {
      ...
      case DOC_TYPE_GROUP:
        switch (mode) {
          ...
          case MODE_BREAK: {
            ...
            if (!doc.break && fits(next, commands, remainingWidth, hasLineSuffix, groupModeMap)) {
              commands.push(next);
            } else {
              ...
              if (doc.expandedStates) {
                ...
                if (doc.break) {
                  ...
                  for (let index = 1; index < doc.expandedStates.length + 1; index++) {
  ...
  return {
    formatted,
    cursorNodeStart,
    cursorNodeText: formatted.slice(cursorNodeStart, finalCursorPositions.at(-1)),
  };
  ...
  function trim() {
    ...
    if (cursorPositions.length > 0) {
      settledCursorPositions.push(
        ...cursorPositions.map((position) => Math.min(position, settledTextLength)),
      );
