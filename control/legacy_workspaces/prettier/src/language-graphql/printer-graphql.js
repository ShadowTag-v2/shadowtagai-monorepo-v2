...
function genericPrint(path, options, print) {
  ...
  switch (node.kind) {
    ...
    case "SelectionSet":
      return [
        "{",
        indent([hardline, join(hardline, printSequence(path, options, print, "selections"))]),
        hardline,
        "}",
      ];
    ...
    case "StringValue":
      ...
      return [
        '"',
        node.value.replaceAll(/["\\]/gu, String.raw`\$&`).replaceAll("\n", String.raw`\n`),
        '"',
      ];
    ...
    case "ListValue":
      return group([
        "[",
        indent([softline, join([ifBreak("", ", "), softline], path.map(print, "values"))]),
        softline,
        "]",
      ]);
    ...
    case "InterfaceTypeDefinition": {
      ...
      if (node.fields.length > 0) {
        parts.push([
          " {",
          indent([hardline, join(hardline, printSequence(path, options, print, "fields"))]),
          hardline,
          "}",
        ]);
      }
    ...
    case "EnumTypeDefinition":
      return [
        ...
        node.values.length > 0
          ? [
              " {",
              indent([hardline, join(hardline, printSequence(path, options, print, "values"))]),
              hardline,
              "}",
            ]
          : "",
        ...
        group([
          ...
          node.types.length > 0
            ? [
                " =",
                ifBreak("", " "),
                indent([ifBreak([line, "| "]), join([line, "| "], path.map(print, "types"))]),
              ]
            : "",
        ]),
      ]);
...
function printVariableDefinitions(path, print) {
  ...
  return group([
    "(",
    indent([softline, join([ifBreak("", ", "), softline], path.map(print, "variableDefinitions"))]),
    softline,
    ")",
  ]);
}
