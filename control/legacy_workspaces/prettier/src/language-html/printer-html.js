...
import { fill, group, hardline, indent, line, replaceEndOfLine } from "../document/index.js";
  ...
  switch (node.kind) {
    ...
    case "comment":
      return [
        ...
        replaceEndOfLine(options.originalText.slice(locStart(node), locEnd(node))),
    ...
    case "attribute": {
      ...
      const quote = shouldUnquoteAttributeValue(node, options) ? "" : getPreferredQuote(value, '"');
      ...
      return [
        ...
        quote,
        replaceEndOfLine(
          quote === '"' ? value.replaceAll('"', "&quot;") : value.replaceAll("'", "&apos;"),
        ),
        quote,
      ];
    }
