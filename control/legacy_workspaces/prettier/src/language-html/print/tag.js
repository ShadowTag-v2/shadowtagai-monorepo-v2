...
import { hardline, indent, join, line, replaceEndOfLine, softline } from "../../document/index.js";
...
function printAttributes(path, options, print) {
  ...
  const ignoreAttributeData =
    node.prev?.kind === "comment" && getPrettierIgnoreAttributeCommentData(node.prev.value);
  ...
  const printedAttributes = path.map(
    ({ node: attribute }) =>
      hasPrettierIgnoreAttribute(attribute)
        ? replaceEndOfLine(options.originalText.slice(locStart(attribute), locEnd(attribute)))
        : print(),
    "attrs",
  );
  ...
  const shouldPrintAttributePerLine =
    options.singleAttributePerLine && node.attrs.length > 1 && !isVueSfcBlock(node, options);
  const attributeLine = shouldPrintAttributePerLine ? hardline : line;

  /** @type {Doc[]} */
  const parts = [
    indent([forceNotToBreakAttrContent ? " " : line, join(attributeLine, printedAttributes)]),
  ];
...
function printOpeningTagStartMarker(node, options) {
  switch (node.kind) {
    ...
    case "docType": {
      ...
      return options.originalText.slice(start, start + HTML5_DOCTYPE_START_MARKER.length);
    }
