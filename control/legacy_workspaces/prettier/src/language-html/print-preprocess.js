  ...
  ast.walk((node) => {
    if (node.children) {
      for (let i = 0; i < node.children.length; i++) {
        ...
        prevChild.value +=
          `<${child.rawName}>` + child.firstChild.value + `</${child.rawName}>` + nextChild.value;
        prevChild.sourceSpan = new ParseSourceSpan(
          prevChild.sourceSpan.start,
  ...
  ast.walk((node) => {
    ...
    if (!isWhitespaceSensitive) {
      for (let i = 0; i < children.length; i++) {
        ...
        const { leadingWhitespace, text, trailingWhitespace } = getLeadingAndTrailingHtmlWhitespace(
          child.value,
        );
...
function addIsSpaceSensitive(ast, options) {
  ast.walk((node) => {
    ...
    if (children.length === 0) {
      node.isDanglingSpaceSensitive = isDanglingSpaceSensitiveNode(node, options);
      return;
    }
