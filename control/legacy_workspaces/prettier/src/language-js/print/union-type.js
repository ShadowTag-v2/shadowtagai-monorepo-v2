import { align, group, ifBreak, indent, join, line, softline } from "../../document/index.js";
import { printComments, printCommentsSeparately } from "../../main/comments/print.js";
...
function printUnionType(path, options, print) {
  ...
  const mainParts = [ifBreak([shouldAddStartLine ? line : "", "| "]), join([line, "| "], printed)];
  ...
  if (parent.type === "TupleTypeAnnotation" || parent.type === "TSTupleType") {
    ...
    if (elementTypes.length > 1) {
      return [group([indent([ifBreak(["(", softline]), parts]), softline, ifBreak(")")]), trailing];
    }
  }
