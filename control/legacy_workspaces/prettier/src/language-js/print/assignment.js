...
function chooseLayout(path, options, print, leftDoc, rightPropertyName) {
  ...
  const shouldUseChainFormatting = path.match(
    ...
    (node) =>
      !isTail || (node.type !== "ExpressionStatement" && node.type !== "VariableDeclaration"),
  );
  ...
  if (
    path.call(() => shouldBreakAfterOperator(path, options, print, hasShortKey), rightPropertyName)
  ) {
    return "break-after-operator";
  }
...
function shouldBreakAfterOperator(path, options, print, hasShortKey) {
  ...
  switch (rightNode.type) {
    ...
    case "ConditionalExpression": {
      ...
      return (
        consequent.type === "ConditionalExpression" || alternate.type === "ConditionalExpression"
      );
    }
  ...
  if (
    ...
    path.call(() => isPoorlyBreakableMemberOrCallChain(path, options, print), ...propertiesForPath)
  ) {
    return true;
  }
...
function isObjectPropertyWithShortKey(node, keyDoc, options) {
  ...
  //   longValue2
  return (
    typeof keyDoc === "string" && getStringWidth(keyDoc) < options.tabWidth + MIN_OVERLAP_FOR_BREAK
  );
}
