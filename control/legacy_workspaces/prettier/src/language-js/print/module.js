...
function printSemicolonAfterExportDeclaration(node, options) {
  if (
    options.semi &&
    (!node.declaration || (isDefaultExport(node) && !shouldOmitSemicolon(node.declaration)))
  ) {
    return ";";
  }
...
function isShorthandSpecifier(specifier) {
  ...
  if (isStringLiteral(local)) {
    return local.value === importedOrExported.value && getRaw(local) === getRaw(importedOrExported);
  }
