import { align, dedent, group, ifBreak, indent, line, softline } from "../../document/index.js";
import {
...
function printTernaryOld(path, options, print) {
  ...
  const { parent } = path;
  const isParentTest =
    parent.type === node.type && testNodePropertyNames.some((prop) => parent[prop] === node);
  ...
  if (
    ...
    */
    const printBranch = (nodePropertyName) =>
      options.useTabs ? indent(print(nodePropertyName)) : align(2, print(nodePropertyName));
    // normal mode
    const part = [
      line,
      "? ",
