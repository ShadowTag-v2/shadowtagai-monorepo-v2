...
function printPropertyKey(path, options, print) {
  ...
  if (options.quoteProps === "consistent" && !needsQuoteProps.has(parent)) {
    const objectHasStringProp = path.siblings.some(
      (prop) =>
        !prop.computed && isStringLiteral(prop.key) && !isStringKeySafeToUnquote(prop, options),
    );
  ...
  if (shouldQuotePropertyKey(path, options)) {
    ...
    const prop = printString(
      JSON.stringify(key.type === "Identifier" ? key.name : key.value.toString()),
      options,
    );
  ...
  if (
    ...
    return path.call(
      () =>
        printComments(path, /^\d/u.test(key.value) ? printNumber(key.value) : key.value, options),
      "key",
    );
  }
