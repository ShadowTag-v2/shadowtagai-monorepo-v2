...
const isSingleLineComment = (comment, text) =>
  isLineComment(comment) || !hasNewlineInRange(text, locStart(comment), locEnd(comment));
...
function handleLastUnionElementInExpression({
  ...
  if (
    ...
    (((enclosingNode.type === "TSArrayType" || enclosingNode.type === "ArrayTypeAnnotation") &&
      !followingNode) ||
...
function handleCommentAfterArrowExpression({
  ...
  if (
    ...
    (precedingNode.type === "TSTypeAnnotation" || precedingNode.type === "TypeAnnotation")
  ) {
    addLeadingComment(followingNode, comment);
    return true;
  }
