...
function needsParentheses(path, options) {
  ...
  if (node.type === "Identifier") {
    ...
    if (node.name === "let") {
      ...
      if (
        expression &&
        startsWithNoLookaheadToken(expression, (leftmostNode) => leftmostNode === node)
      ) {
        return true;
      }
    }
  ...
  switch (node.type) {
    ...
    case "UnaryExpression":
      switch (parent.type) {
        case "UnaryExpression":
          return (
            node.operator === parent.operator && (node.operator === "+" || node.operator === "-")
          );
    ...
    case "LogicalExpression":
      switch (parent.type) {
        ...
        case "AssignmentPattern":
          return (
            key === "left" && (node.type === "TSTypeAssertion" || isBinaryCastExpression(node))
          );
    ...
    case "TypeOperator":
      return (
        ...
        (key === "objectType" &&
          (parent.type === "IndexedAccessType" || parent.type === "OptionalIndexedAccessType")) ||
    ...
    case "AssignmentExpression":
      ...
      if (
        ...
        path.match(
          undefined,
          undefined,
          (node, name) => (name === "init" || name === "update") && node.type === "ForStatement",
        )
      ) {
        return false;
      }
      ...
      if (
        ...
        path.match(
          undefined,
          undefined,
          (node, name) => name === "properties" && node.type === "ObjectPattern",
        )
      ) {
        return false;
      }
...
function shouldWrapFunctionForExportDefault(path, options) {
  ...
  if (
    ...
    (parent.type !== "ExportDefaultDeclaration" && needsParentheses(path, options))
  ) {
    return false;
  }
...
function shouldAddParenthesesToChainElement(path) {
  ...
  if (
    path.match(
      ...
      (node, name) =>
        ...
        (name === "callee" && (node.type === "CallExpression" || node.type === "NewExpression")),
    ) ||
    path.match(
      (node) => node.type === "OptionalMemberExpression" || node.type === "OptionalCallExpression",
      (node, name) => name === "expression" && node.type === "TSNonNullExpression",
  ...
  if (
    ...
    (path.match(
      ...
      (node, name) =>
        (name === "callee" &&
          ((node.type === "CallExpression" && !node.optional) || node.type === "NewExpression")) ||
...
function canDecoratorExpressionUnparenthesized(node) {
  ...
  return (
    ...
    (isCallExpression(node) && !node.optional && isDecoratorMemberExpression(node.callee))
  );
}
