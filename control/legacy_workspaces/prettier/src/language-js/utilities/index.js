    ...
    return (
      (args.length === 2
        ...
        : isFunctionOrArrowExpressionWithBody(args[1]) &&
          getFunctionParameters(args[1]).length <= 1) || isAngularTestWrapper(args[1])
    );
  }
  return false;
}
...
function isSimpleCallArgument(node, depth = 2) {
  ...
  if (
    (node.type === "UnaryExpression" && simpleCallArgumentUnaryOperators.has(node.operator)) ||
...
const isUnionType = createTypeCheckFunction(["TSUnionType", "UnionTypeAnnotation"]);
