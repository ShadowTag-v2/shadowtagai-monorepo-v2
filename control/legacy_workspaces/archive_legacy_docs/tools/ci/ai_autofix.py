...
def log_metric(
  model: str, scope: str, prompt_tokens: int, resp_max: int, note: str = ""
) -> None:
  try:
...
def attempt(goal, model, scope):
  ...
  for files in batches:
    ...
    run("git add -A")
    run(
      f'git commit -m "ci: {model} autofix scope={scope} sharded" || echo "no changes"'
    )
    ok2, _ = run_tests()
    if ok2:
      return True
...
def main():
  goal = os.getenv(
    "AIAUTOFIX_GOAL",
    "Fix tests and lint errors; minimal diffs; stay within TPM; fallback on rate limits.",
  )
  plan = [
    ("gpt-5-fast", "changed"),
    ("claude-4-sonnet", "changed"),
    ("gpt-5", "project"),
  ]
  ok, _ = run_tests()
  if ok:
