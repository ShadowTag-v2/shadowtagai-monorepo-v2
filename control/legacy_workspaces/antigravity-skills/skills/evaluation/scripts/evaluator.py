...
class AgentEvaluator:
    ...
    def evaluate(self, task: dict, output: str, ground_truth: dict = None, tool_calls: list[dict] = None) -> dict:
        """
        ...
        for dimension_name, dimension in self.rubric.items():
            ...
            score = self._evaluate_dimension(dimension=dimension, task=task, output=output, ground_truth=ground_truth, tool_calls=tool_calls)

            scores[dimension_name] = {"score": score, "weight": dimension.weight, "level": self._score_to_level(score)}

        # Calculate weighted overall
        overall = sum(s["score"] * self.rubric[k]["weight"] for k, s in scores.items())

        result = {"overall_score": overall, "dimension_scores": scores, "passed": overall >= 0.7, "timestamp": time.time()}

        self.evaluation_history.append(result)
        return result

    def _evaluate_dimension(
        self, dimension: RubricDimension, task: dict, output: str, ground_truth: dict = None, tool_calls: list[dict] = None
    ) -> float:
        """
...
class EvaluationRunner:
    ...
    def run_test(self, test: dict) -> dict:
        ...
        return {"test": test, "output": output, "evaluation": evaluation, "passed": evaluation["passed"]}
    ...
    def summarize(self) -> dict:
        ...
        return {
            ...
            "failures": [{"test": r["test"]["name"], "score": r["evaluation"]["overall_score"]} for r in self.results if not r["passed"]],
        }
...
class ProductionMonitor:
    ...
    def _generate_alerts(self, pass_rate: float, avg_score: float) -> list[dict]:
        ...
        if pass_rate < self.alert_thresholds["pass_rate_critical"]:
            alerts.append({"type": "critical", "message": f"Pass rate ({pass_rate:.2f}) below critical threshold"})
        elif pass_rate < self.alert_thresholds["pass_rate_warning"]:
            alerts.append({"type": "warning", "message": f"Pass rate ({pass_rate:.2f}) below warning threshold"})

        if avg_score < 0.6:
            alerts.append({"type": "quality", "message": f"Average score ({avg_score:.2f}) indicates quality issues"})
