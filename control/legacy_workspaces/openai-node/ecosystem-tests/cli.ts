...
async function main() {
  ...
  if (args.skip.length > 0) {
    ...
    projectNames = projectNames.filter(
      (projectName) => (args.skip as string[]).indexOf(projectName) < 0,
    );
    ...
    if (jobs > 1) {
      ...
      function showProgress() {
        ...
        process.stderr.write(
          `${spin} Running ${[...runningProjects].join(", ")}`.substring(
            0,
...
async function buildPackage() {
  ...
  assert(
    pack.length === 1,
    `Expected pack output to be an array of length 1 but got ${pack.length}`,
  );
