...
import { Blob as FormDataBlob, File as FormDataFile } from "formdata-node";
...
it(`streaming works`, async () => {
  ...
  expect(chunks.map((c) => c.choices[0]?.delta.content || "").join("")).toBeSimilarTo(
    "This is a test",
    10,
  );
});
    ...
    const file = await fetch(url)
      ...
      .then(
        (x) =>
          new File(
            [
              // @ts-expect-error array buffer can't be passed to File at the type-level with certain tsconfigs
              x,
            ],
            filename,
          ),
