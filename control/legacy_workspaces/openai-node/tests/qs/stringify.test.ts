...
describe("stringify()", () => {
  ...
  test("supports custom representations when filter=function", () => {
    ...
    var filterFunc: StringifyOptions["filter"] = (prefix, value) => {
      calls += 1;
      if (calls === 1) {
  ...
  test("can sort the keys", () => {
    // @ts-expect-error
    var sort: NonNullable<StringifyOptions["sort"]> = (a: string, b: string) => a.localeCompare(b);
  ...
  test("strictNullHandling works with custom filter", () => {
    // @ts-expect-error
    var filter = (_prefix, value) => value;
...
describe("stringifies empty keys", () => {
  empty_test_cases.forEach((testCase) => {
