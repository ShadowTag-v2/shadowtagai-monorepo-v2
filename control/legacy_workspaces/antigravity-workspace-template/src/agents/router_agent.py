...
You may delegate to multiple agents in sequence or parallel."""
  ...
  def _simple_delegate(self, task: str) -> list[dict[str, str]]:
    ...
    if any(
      word in task_lower
      for word in ["review", "check", "security", "quality", "analyze"]
    ):
    ...
    if any(
      word in task_lower
      for word in ["research", "search", "find", "information", "learn"]
    ):
  ...
  def synthesize_results(
    self, delegations: list[dict[str, str]], results: list[str]
  ) -> str:
    """
...
"""
    ...
    synthesis_prompt += (
      "Provide a concise final report summarizing what was accomplished."
    )
