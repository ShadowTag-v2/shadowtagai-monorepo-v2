...
function formatSearchResults(
  ...
  for (const result of papers) {
    ...
    r.push("");
    r.push(
      result.paper.ai_keywords ? `**AI Keywords**: ${result.paper.ai_keywords.join(", ")}` : "",
    );

    const upvotes: string =
      result.paper.upvotes && result.paper.upvotes > 0
        ? `Upvoted ${result.paper.upvotes} times`
        : "";
    ...
    if (result.numComments && result.numComments > 0) {
      if (result.isAuthorParticipating)
        ...
        r.push(`${upvotes}. There is a community discussion with ${result.numComments} comments.`);
    } else {
