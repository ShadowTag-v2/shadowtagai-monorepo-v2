...
import { formatComplexSchemaError, formatParameters } from "../utils/parameter-formatter.js";
 ...
 */
async function fetchGradioSchema(
  subdomain: string,
  isPrivate: boolean,
  hfToken?: string,
): Promise<Tool[]> {
...
function parseSchemaResponse(schemaResponse: unknown): Tool[] {
  ...
  if (Array.isArray(schemaResponse)) {
    ...
    for (const item of schemaResponse) {
      if (typeof item === "object" && item !== null && "name" in item && "inputSchema" in item) {
