import { HubApiError, modelInfo } from "@huggingface/hub";
import { z } from "zod";
  ...
  async getDetails(modelId: string, includeReadme: boolean = false): Promise<ToolResult> {
    try {
      ...
      if (error instanceof Error) {
        ...
        if (isUnauthorized) {
          throw new Error(
            `Authentication required or insufficient permissions to access model '${modelId}'.`,
          );
        }
...
function formatModelDetails(model: ModelInformation): ToolResult {
  ...
  if (model.inferenceProviders && model.inferenceProviders.length > 0) {
    ...
    r.push("");
    r.push(
      "Try this model in the [playground](https://hf.co/playground?modelId=" + model.name + ")",
    );
    r.push("");
  }
