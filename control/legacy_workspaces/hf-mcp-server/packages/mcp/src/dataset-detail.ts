  ...
  async getDetails(datasetId: string, includeReadme: boolean = false): Promise<ToolResult> {
    try {
      ...
      if (error instanceof Error) {
        ...
        if (isUnauthorized) {
          throw new Error(
            `Authentication required or insufficient permissions to access dataset '${datasetId}'.`,
          );
        }
...
function formatDatasetDetails(dataset: DatasetInformation): ToolResult {
  const r: string[] = [];
  const [authorFromName] = dataset.name.includes("/")
    ? dataset.name.split("/")
    : ["", dataset.name];
  ...
  r.push(
    `**Link:** [https://hf.co/datasets/${dataset.name}](https://hf.co/datasets/${dataset.name})`,
  );
