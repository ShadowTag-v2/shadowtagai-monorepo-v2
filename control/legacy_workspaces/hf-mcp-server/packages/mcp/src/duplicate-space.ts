...
export const DUPLICATE_SPACE_TOOL_CONFIG = {
  ...
  schema: z.object({
    ...
    newSpaceId: z
      .string()
      .optional()
      .describe("Name for the new space (optional, defaults to source space-name)"),
    hardware: z
      .enum(["freecpu", "zerogpu"])
      .optional()
      .describe(
        'Either "freecpu" or "zerogpu" (defaults based on source). Both options are in the free tier.',
      ),
    private: z
      .boolean()
      .optional()
  ...
  async duplicate(params: DuplicateSpaceParams): Promise<DuplicateSpaceResult> {
    ...
    try {
      ...
      if (typeof sourceInfo.runtime?.hardware === "object") {
        sourceHardwareStr =
          sourceInfo.runtime.hardware.current || sourceInfo.runtime.hardware.requested;
      } else {
