...
export class SpaceFilesTool {
   ...
   */
  async generateDetailedMarkdown(
    spaceName: string,
    fileType: "all" | "image" | "audio" = "all",
  ): Promise<string> {
    ...
    markdown +=
      "This space is accessible via `git` with `git clone https://huggingface.co/spaces/" +
      spaceName +
      "`\n";
    return markdown;
  }
  ...
  async listFiles(params: SpaceFilesParams): Promise<string> {
    ...
    const spaceName =
      params.spaceName || (this.username ? `${this.username}/filedrop` : "filedrop");
