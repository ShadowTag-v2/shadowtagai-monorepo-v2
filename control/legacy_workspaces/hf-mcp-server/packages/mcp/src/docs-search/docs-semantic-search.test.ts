  ...
  describe("search", () => {
    ...
    it("should return no results message with product filter", async () => {
      ...
      expect(result.formatted).toBe(
        `No documentation found for query 'nonexistent' in product 'hub'`,
      );
      expect(result.totalResults).toBe(0);
      expect(result.resultsShared).toBe(0);
    });
    ...
    it("should format results grouped by product and page", async () => {
      const sampleResults = [
        {
          ...
          heading1: "Analytics",
          source_page_url:
            "https://huggingface.co/docs/hub/enterprise-hub-analytics#export-analytics-as-csv",
      ...
      expect(result.formatted).toContain(
        "### Results from [Quickstart](https://huggingface.co/docs/dataset-viewer/quick_start)",
      );
      ...
      expect(result.formatted).toContain(
        "In this quickstart, you will learn how to use the dataset viewer REST API",
      );

      // Check footer
      expect(result.formatted).toContain(
        'Use the "' + DOC_FETCH_CONFIG.name + '" tool to fetch a document from the library.',
      );
    });
    ...
    it("should sort multiple products and pages correctly by count", async () => {
      ...
      expect(result.formatted).toContain(
        "### Results from [Page 1](https://huggingface.co/docs/hub/page1) (2 results)",
      );
    ...
    it("should group results from the same page with different anchors together", async () => {
      ...
      expect(result.formatted).toContain(
        "### Results from [Analytics](https://huggingface.co/docs/hub/analytics) (3 results)",
      );
    ...
    it("should handle API errors gracefully", async () => {
      ...
      await expect(docSearchTool.search({ query: "test" })).rejects.toThrow(
    ...
    it("should handle token budget by truncating or switching to links", async () => {
      ...
      const hasTruncation = result.formatted.includes(
