...
export const DOCS_SEMANTIC_SEARCH_CONFIG = {
  ...
  schema: z.object({
    ...
    product: z
      .string()
      .optional()
      .describe("Filter by Product. Supply when known for focused results"),
  }),
  annotations: {
...
export class DocSearchTool extends HfApiCall<DocSearchApiParams, DocSearchResult[]> {
  ...
  constructor(
    hfToken?: string,
    apiUrl = "https://hf.co/api/docs/search",
...
function groupBySection(
  pageResults: DocSearchResult[],
): Map<string | undefined, DocSearchResult[]> {
...
function formatSectionExcerpts(
  ...
  for (const result of results) {
    ...
    if (
      useTruncatedMode &&
      cleanText.length > TRUNCATE_EXCERPT_LENGTH &&
      !hasAlreadyShownTruncation
    ) {
      cleanText =
...
function formatSearchResults(
  ...
  for (const product of sortedProducts) {
    ...
    // Add product header
    const totalProductHits = Array.from(productGroup.values()).reduce(
    ...
    for (const url of sortedUrls) {
      ...
      for (const [section, sectionResults] of sectionGroups) {
        ...
        const result = formatSectionExcerpts(
          section,
          sectionResults,
          useTruncatedMode,
          hasShownTruncationMessage,
        );
  ...
  if (linkOnlyResults.length > 0) {
    ...
    for (const linkResult of linkOnlyResults) {
      ...
      lines.push(
        `- [${escapeMarkdown(linkResult.title)}](${linkResult.url})${hitText} *(${linkResult.product})*`,
      );
    }
    lines.push("");
  }
