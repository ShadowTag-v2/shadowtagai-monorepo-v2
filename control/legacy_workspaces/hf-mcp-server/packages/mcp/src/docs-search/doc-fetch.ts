...
export const DOC_FETCH_CONFIG = {
  ...
  schema: z.object({
    doc_url: z
      ...
      .describe("Documentation URL (Hugging Face or Gradio)"),
    offset: z
      .number()
      .min(0)
      .optional()
...
export class DocFetchTool {
  ...
  constructor() {
    ...
    this.turndownService.remove((node) => {
      ...
      if (nodeName === "img") {
        try {
          const src =
            (node as unknown as { getAttribute?: (name: string) => string | null }).getAttribute?.(
              "src",
            ) ??
    ...
    this.turndownService.addRule("dropHeadingAnchors", {
      filter: (node) => {
        try {
          ...
          const children =
            (n as unknown as { childNodes?: Array<{ nodeName?: string }> }).childNodes || [];
          const onlyIcons =
            children.length > 0 &&
            children.every(
              (c) =>
                (c.nodeName || "").toLowerCase() === "img" ||
                (c.nodeName || "").toLowerCase() === "svg",
            );
  ...
  validateUrl(hfUrl: string): void {
    try {
      ...
      const isHfDocs =
        (hostname === "huggingface.co" || hostname === "www.huggingface.co") &&
    ...
    try {
      ...
      if (!isPlainOrMarkdown) {
        ...
        content = content.replace(
          /\[[^\]]*(?:data:image\/svg\+xml|%3csvg|svg%2bxml)[^\]]*\]\([^)]*\)/gi,
          "",
        );
      }
