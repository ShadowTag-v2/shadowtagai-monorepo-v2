...
import { type Author, authors } from "./paper-search.js";
import { SpaceSearchTool } from "./space-search.js";
import { escapeMarkdown, formatDate, formatNumber } from "./utilities.js";
...
export const PAPER_SUMMARY_PROMPT_CONFIG = {
  ...
  schema: z.object({
    paper_id: z
      ...
      .describe(
        'arXiv paper ID in various formats (e.g., "2502.16161", "arxiv:2502.16161", or full URL)',
      )
      .max(60)
...
export class PaperSummaryPrompt extends HfApiCall<Record<string, string>, PaperDetails> {
  ...
  private async getRelatedResources(
    arxivId: string,
  ): Promise<{ models?: string; datasets?: string; spaces?: string }> {
    ...
    try {
      ...
      if (
        spaceResults &&
        !spaceResults.formatted.includes("No matching Hugging Face Spaces found")
      ) {
