...
import { cancelCommand, inspectCommand } from "./commands/inspect.js";
...
import { type AnyZodType, extractFieldDetails, formatCommandHelp } from "./schema-help.js";
...
export const HF_JOBS_TOOL_CONFIG = {
  ...
  schema: z.object({
    operation: z
      ...
      .optional()
      .describe(
        `Operation to execute. Valid values: ${OPERATION_NAMES.map((cmd) => `"${cmd}"`).join(", ")}`,
      ),
      ...
      switch (operation) {
        ...
        case "scheduled run":
          result = await scheduledRunCommand(
            parsedArgs as ScheduledRunArgs,
            this.client,
            this.hfToken,
          );
          break;
