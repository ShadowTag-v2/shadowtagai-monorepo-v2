...
const commonArgsSchema = z.object({
  namespace: z
    .string()
    .optional()
    .describe("Target namespace (username or organization). Defaults to current user."),
});
...
export const runArgsSchema = commonArgsSchema.extend({
  ...
  detach: z
    ...
    .default(false)
    .describe(
      "If true, return immediately with job ID. If false (default), tail logs for up to 10 seconds.",
    ),
});
...
export const uvArgsSchema = commonArgsSchema.extend({
  script: z
    .string()
    .describe(
      "Python script: local file path, URL, or inline code. UV will handle dependencies automatically.",
    ),
...
export const psArgsSchema = commonArgsSchema.extend({
  ...
  status: z
    .string()
    .optional()
    .describe('Filter by status ("RUNNING", "COMPLETED", "CANCELED", "ERROR", "DELETED")'),
});
...
export const scheduledRunArgsSchema = runArgsSchema.extend({
  schedule: z
    .string()
    .describe(
      "Schedule: cron expression or shorthand (@hourly, @daily, @weekly, @monthly, @yearly)",
    ),
...
export const scheduledPsArgsSchema = commonArgsSchema.extend({
  all: z
    ...
    .optional()
    .default(false)
    .describe("Show all scheduled jobs (default: hide suspended)"),
});
