...
import { type ApiClientConfig, McpApiClient } from "../../../src/server/utils/mcp-api-client.js";
import {
  ...
  describe("Complex Scenarios", () => {
    ...
    it("should handle all possible tool types in mix", async () => {
      ...
      for (const [bouquetName, bouquetConfig] of Object.entries(BOUQUETS)) {
        ...
        const expectedTools = [
          ...new Set([...userSettings.builtInTools, ...bouquetConfig.builtInTools]),
        ];
    ...
    it("should preserve gradio endpoints when mixing with all bouquet in internal API mode", async () => {
      ...
      const expectedBuiltInTools = [
        ...new Set([...userSettings.builtInTools, ...ALL_BUILTIN_TOOL_IDS]),
      ];
