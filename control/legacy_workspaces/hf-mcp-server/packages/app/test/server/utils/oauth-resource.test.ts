...
describe("buildOAuthResourceHeader", () => {
  ...
  it("appends raw query string from originalUrl", () => {
    ...
    expect(header).toBe(`Bearer resource_metadata="${OAUTH_RESOURCE_BASE_URL}?login&mix=foo"`);
  });
  ...
  it("preserves duplicate query parameters", () => {
    ...
    expect(header).toBe(`Bearer resource_metadata="${OAUTH_RESOURCE_BASE_URL}?mix=foo&mix=bar"`);
  });
});
