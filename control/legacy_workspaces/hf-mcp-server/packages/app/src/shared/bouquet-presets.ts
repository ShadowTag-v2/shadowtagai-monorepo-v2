...
import { GRADIO_IMAGE_FILTER_FLAG, README_INCLUDE_FLAG } from "./behavior-flags.js";
...
const PRESET_META: Array<Omit<BouquetPreset, "builtInTools">> = [
  {
    ...
    description:
      "Tools that work well with Hugging Face Skills (https://github.com/huggingface/skills).",
    category: "core",
...
export const BOUQUET_PRESETS: BouquetPreset[] = PRESET_META.map((preset) => {
  ...
  return {
    ...
    directParams: preset.directParams
      ? preset.directParams.map((option) => ({ ...option }))
      : undefined,
  };
});
