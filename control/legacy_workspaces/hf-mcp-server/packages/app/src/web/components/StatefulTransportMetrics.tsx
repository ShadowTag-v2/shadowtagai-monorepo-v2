...
export function StatefulTransportMetrics({ metrics }: StatefulTransportMetricsProps) {
  ...
  const createSessionColumns = (): ColumnDef<SessionData>[] => [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            ...
            title={
              session.clientInfo
                ? `${session.clientInfo.name}@${session.clientInfo.version}`
                : "unknown"
            }
          >
            {clientDisplay}
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const session = row.original;
        const status =
          session.connectionStatus || (session.isConnected ? "Connected" : "Disconnected");
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-sm">{formatRelativeTime(row.getValue<string>("connectedAt"))}</div>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-sm">{formatRelativeTime(row.getValue<string>("lastActivity"))}</div>
      ),
    },
    {
  ...
  const createClientColumns = (): ColumnDef<ClientData>[] => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-right font-mono text-sm">{row.getValue<number>("toolCallCount")}</div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-right font-mono text-sm">{row.getValue<number>("newIpCount")}</div>
      ),
    },
    {
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          {transportTypeDisplay[metrics.transport as keyof typeof transportTypeDisplay] ||
            metrics.transport}{" "}
          transport
        </CardDescription>
      </CardHeader>
          ...
          <>
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                SSE connections checked every{" "}
                {formatMilliseconds(metrics.configuration.heartbeatInterval || 30000)}, sessions
                swept every {formatMilliseconds(metrics.configuration.staleCheckInterval)}, removed
                after {formatMilliseconds(metrics.configuration.staleTimeout)} inactive
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell className="font-medium text-sm">
                  Requests per Minute (tot/3hr/hr)
                </TableCell>
                <TableCell className="text-sm font-mono">
                  {metrics.requests.averagePerMinute}/{metrics.requests.last3Hours}/
              ...
              <TableRow>
                ...
                <TableCell className="text-sm font-mono">
                  {metrics.connections.uniqueIps ?? 0}
                </TableCell>
                <TableCell className="font-medium text-sm">
