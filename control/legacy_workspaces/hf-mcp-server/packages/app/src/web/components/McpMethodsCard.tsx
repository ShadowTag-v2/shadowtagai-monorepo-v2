...
export function McpMethodsCard() {
  ...
  const filteredMethods = showOnlyToolAndPromptCalls
    ? processedMethods.filter(
        (m) => m.method.startsWith("tools/call") || m.method.startsWith("prompts/get"),
      )
    : processedMethods;
  ...
  const columns: ColumnDef<MethodData>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-right font-mono">{avgTime ? `${avgTime.toFixed(0)}ms` : "—"}</div>
        );
      },
    },
    {
        ...
        {isStdioMode && (
          <div className="flex items-center justify-center py-8">
            <div className="text-sm text-muted-foreground">
              Method tracking is not available in STDIO mode. This data is only collected for
            ...
            {processedMethods.length > 0 && (
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  ...
                  tracked since {new Date(metrics.startupTime).toLocaleString()}
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-sm font-medium">
                    Total MCP Calls:{" "}
                    <span className="font-mono">{totalMcpCalls.toLocaleString()}</span>
                  </div>
                  <div className="text-sm font-medium">
