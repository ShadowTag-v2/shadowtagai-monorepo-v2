...
function App() {
  ...
  const { data: transportInfo, error: transportError } = useSWR<TransportInfo>(
    "/api/transport",
    fetcher,
    {
  ...
  const gradioImageFilterEnabled =
    settings?.builtInTools?.includes(GRADIO_IMAGE_FILTER_FLAG) ?? false;
  ...
  const spaceTools = {
    duplicate_space: {
      ...
      description:
        ...
        "Duplicate a Hugging Face Space to your account.",
    ...
    space_files: {
      ...
      description:
        SPACE_FILES_TOOL_CONFIG.description ||
        "List all files in a static Hugging Face Space with download URLs.",
  ...
  return (
    <>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      ...
                      assistants to securely connect to external data sources and tools. This HF MCP
                      Server provides access to Hugging Face's ecosystem of models, datasets, and
                      Spaces, allowing AI assistants to search, analyze, and interact with ML
                      resources directly.
                    </p>
                  </div>
                      ...
                      <div>
                        ...
                        {(transportInfo?.externalApiMode ||
                          ...
                          <p className="text-sm text-blue-700 dark:text-blue-300 leading-relaxed">
                            {transportInfo?.externalApiMode ? (
                              <>
                                <strong>External API Mode:</strong> Tools are managed by an external
                                API. Tool Selection Tabs are not available.
                              </>
                            ) : (
                              <>
                                <strong>Shared Mode:</strong> Tool toggles in this interface affect{" "}
                                <strong>all connected MCP server instances</strong> and control tool
                                availability for all connected Hosts.
                              </>
                            )}
                          </p>
                        )}
      ...
      <ConnectionFooter
        isLoading={isLoading}
        error={error}
        transportInfo={transportInfo || { transport: "unknown" }}
      />
    </>
  );
}
