...
import { type SpaceTool, settingsService } from "../shared/settings.js";
...
export class WebServer {
  ...
  private setupMiddleware(): void {
    ...
    const allowedOrigins = (envOriginsNorm.length > 0 ? envOriginsNorm : CORS_ALLOWED_ORIGINS).map(
      normalize,
    );
  ...
  public setupApiRoutes(): void {
    ...
    this.app.get("/api/transport-metrics", (req, res) => {
      ...
      try {
        ...
        let tempLogStatus:
          | { activated: boolean; remaining: number; maxAllowed: number }
          | undefined;
        ...
        const extendedMetrics = formattedMetrics as typeof formattedMetrics & {
          tempLogStatus?: unknown;
        };
        if (tempLogStatus) {
    ...
    this.app.post("/api/settings", express.json(), (req, res) => {
      const { builtInTools, spaceTools } = req.body as {
        builtInTools?: string[];
      ...
      if (builtInTools !== undefined) {
        for (const toolId of ALL_BUILTIN_TOOL_IDS) {
          ...
          if (currentlyEnabled !== shouldBeEnabled) {
            ...
            logger.info(
              `Tool ${toolId} has been ${shouldBeEnabled ? "enabled" : "disabled"} via API`,
            );
          }
        }
      }
