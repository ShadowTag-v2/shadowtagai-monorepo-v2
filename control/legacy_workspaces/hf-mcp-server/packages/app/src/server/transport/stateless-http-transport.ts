...
export class StatelessHttpTransport extends BaseTransport {
  ...
  initialize(_options: TransportOptions): Promise<void> {
    ...
    this.app.get("/mcp", (req: Request, res: Response) => {
      ...
      if (process.env.MCP_STRICT_COMPLIANCE === "true" || !isBrowser(req.headers)) {
        ...
        logger.debug(
          "Rejected GET request to /mcp in strict compliance mode or from non-browser client",
        );
        res
          .status(405)
          .json(
            JsonRpcErrors.methodNotAllowed(
              null,
              "Method not allowed. Use POST for stateless JSON-RPC requests.",
            ),
          );
        return;
      }
  ...
  private async handleJsonRpcRequest(req: Request, res: Response): Promise<void> {
    ...
    const requestBody = req.body as
      | {
          method?: string;
          params?: { clientInfo?: unknown; capabilities?: unknown; name?: string };
        }
      | undefined;
      ...
      if (requestBody?.method === "initialize") {
        ...
        if (this.analyticsSessions.has(sessionId)) {
          ...
          if (this.tempLogCounter > 0) {
            ...
            const hfToken =
              headers["authorization"] || headers["hf-token"] || headers["x-hf-token"];
            if (hfToken) {
            ...
            console.log(
              `[TEMPLOG ${logNumber}/${this.tempLogOriginalCount}] Session Resume Failed:`,
              {
                sessionId: sessionId,
          ...
          res
            .status(404)
            .json(JsonRpcErrors.sessionNotFound(sessionId, extractJsonRpcId(req.body)));
          return;
        }
      } else {
        ...
        res
          .status(400)
          .json(JsonRpcErrors.invalidRequest(extractJsonRpcId(req.body), "Session ID required"));
        return;
      }
    }
  ...
  private async handleDeleteRequest(req: Request, res: Response): Promise<void> {
    ...
    if (!sessionId) {
      ...
      res
        .status(400)
        .json(JsonRpcErrors.invalidRequest(null, "Session ID required for DELETE requests"));
      return;
    }
  ...
  private updateAnalyticsSessionClientInfo(
    sessionId: string,
