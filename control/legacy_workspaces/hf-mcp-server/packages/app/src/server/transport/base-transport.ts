import { HubApiError, type WhoAmI, whoAmI } from "@huggingface/hub";
...
import { extractJsonRpcId, JsonRpcErrors } from "./json-rpc-errors.js";
...
export abstract class BaseTransport {
   ...
   */
  protected trackClientIpAddress(
   ...
   */
  protected extractClientInfoFromRequest(
    requestBody: unknown,
      ...
      try {
        ...
        const isUnauthorized =
          ...
          (error instanceof TypeError &&
            error.message.includes("Your access token must start with"));
...
> extends BaseTransport {
  ...
  protected readonly STALE_CHECK_INTERVAL = parseInt(
    process.env.MCP_CLIENT_CONNECTION_CHECK || "90000",
    10,
  );
  protected readonly STALE_TIMEOUT = parseInt(
    process.env.MCP_CLIENT_CONNECTION_TIMEOUT || "600000",
    10,
  );
  protected readonly HEARTBEAT_INTERVAL = parseInt(
    process.env.MCP_CLIENT_HEARTBEAT_INTERVAL || "30000",
    10,
  );
  ...
  protected readonly PING_FAILURE_THRESHOLD = parseInt(
    process.env.MCP_PING_FAILURE_THRESHOLD || "1",
    10,
  );
  ...
  protected pingSingleSession(sessionId: string): void {
    ...
    session.server.server
      ...
      .catch((error: unknown) => {
        ...
        logger.trace(
          { sessionId, error: errorMessage, failures: session.metadata.pingFailures },
          "Ping failed",
        );
      })
      .finally(() => {
