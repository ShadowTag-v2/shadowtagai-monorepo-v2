...
import { type BaseSession, StatefulTransport, type TransportOptions } from "./base-transport.js";
...
export class StreamableHttpTransport extends StatefulTransport<Session> {
  ...
  private async handleDeleteRequest(
    ...
    if (!sessionId || !this.sessions.has(sessionId)) {
      ...
      res
        .status(404)
        .json(JsonRpcErrors.sessionNotFound(sessionId || "missing", extractJsonRpcId(req.body)));
      return;
    }
