...
export class HfDatasetLogger {
  ...
  private startFlushTimer(): void {
    this.flushTimer = setInterval(() => {
      if (this.logBuffer.length > 0) {
        console.log(
          `[HF Dataset ${this.logType}] Timer flush triggered - Buffer: ${this.logBuffer.length} logs`,
        );
        void this.flush();
      }
    }, this.flushInterval);
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      this.logBuffer = [];
      console.log(
        `[HF Dataset ${this.logType}] ✅ Uploaded ${logsToUpload.length} logs to ${this.datasetId}`,
      );
    } catch (error) {
      // Keep logs in buffer for retry on next flush cycle
      console.error(
        `[HF Dataset ${this.logType}] ❌ Upload failed, will retry on next flush:`,
        error,
      );
    } finally {
