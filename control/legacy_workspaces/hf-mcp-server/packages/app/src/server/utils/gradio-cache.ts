...
class SpaceMetadataCache {
  ...
  get(spaceName: string): CachedSpaceMetadata | null {
    ...
    if (!isValid) {
      this.stats.misses++;
      logger.trace(
        { spaceName, age, ttl: CACHE_CONFIG.SPACE_METADATA_TTL },
        "Space metadata cache expired",
      );
      return null;
    }
...
export function formatCacheMetricsForAPI(): {
  ...
  const metadataHitRate =
    metadataTotal > 0 ? Math.round((stats.metadataHits / metadataTotal) * 10000) / 100 : 0;

  const schemaTotal = stats.schemaHits + stats.schemaMisses;
  const schemaHitRate =
    schemaTotal > 0 ? Math.round((stats.schemaHits / schemaTotal) * 10000) / 100 : 0;
