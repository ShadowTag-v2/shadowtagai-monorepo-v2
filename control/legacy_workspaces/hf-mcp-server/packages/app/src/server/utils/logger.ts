...
function createLogger(): Logger {
  ...
  if (hfLoggingEnabled) {
    ...
    if (!hfToken) {
      console.warn(
        "[Logger] HF Dataset logging disabled: No HF token found (set LOGGING_HF_TOKEN or DEFAULT_HF_TOKEN)",
      );
    } else {
      ...
      try {
        ...
        console.error(
          "[Logger] Failed to setup HF transport, falling back to console only:",
          error,
        );
      }
    }
