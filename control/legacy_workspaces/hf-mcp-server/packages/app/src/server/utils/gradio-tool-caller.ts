...
import { getMetricsSafeName, gradioMetrics } from "./gradio-metrics.js";
import { extractUrlFromContent, stripImageContentFromResult } from "./gradio-result-processor.js";
  ...
  if (proxiedReplica) {
    ...
    return {
      ...
      _meta: {
        ...
        responseHeaders: {
          ...(result as { _meta?: { responseHeaders?: Record<string, unknown> } })._meta
            ?.responseHeaders,
...
export function applyResultPostProcessing(
  ...
  if (options.sessionInfo?.clientInfo?.name === "openai-mcp") {
    ...
    if (extractedUrl) {
      logger.debug(
        { tool: options.toolName, url: extractedUrl },
        "Setting structuredContent with extracted URL",
      );
      (
