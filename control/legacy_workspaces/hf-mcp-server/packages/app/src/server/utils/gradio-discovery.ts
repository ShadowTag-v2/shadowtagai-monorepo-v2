  ...
  try {
    ...
    try {
      ...
      if (!metadata.private) {
        ...
        logger.debug(
          { spaceName, subdomain: metadata.subdomain },
          "Private space metadata fetched (not cached)",
        );
      }
...
async function fetchSchema(
  ...
  try {
    ...
    logger.debug(
      { spaceName, subdomain: metadata.subdomain },
      "Fetching schema from Gradio endpoint",
    );
    ...
    try {
      ...
      if (!metadata.private) {
        ...
        logger.debug(
          { spaceName, toolCount: tools.length },
          "Private space schema fetched (not cached)",
        );
      }
