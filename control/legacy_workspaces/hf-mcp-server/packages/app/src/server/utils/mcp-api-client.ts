...
export type ToolStateChangeCallback = (toolId: string, enabled: boolean) => void;
...
export class McpApiClient extends EventEmitter {
    ...
    switch (this.config.type) {
      case "polling":
        ...
        try {
          ...
          if (!response.ok) {
            logger.error(
              `Failed to fetch settings: ${response.status.toString()} ${response.statusText}`,
            );
            return withNormalizedFlags(BOUQUET_FALLBACK);
          }
      ...
      case "external":
        ...
        try {
          ...
          const apiTimeout = process.env.HF_API_TIMEOUT
            ? parseInt(process.env.HF_API_TIMEOUT, 10)
            : 12500;
          const timeoutId = setTimeout(() => controller.abort(), apiTimeout);
          logger.debug(
            `Fetching external settings from ${this.config.externalUrl} with timeout ${apiTimeout}ms`,
          );
          ...
          if (!response.ok) {
            ...
            if (response.status === 401 || response.status === 403) {
              logger.debug(
                `External config API ${response.status} ${response.statusText}: ${this.config.externalUrl}`,
              );
            }
          ...
          logger.warn(
            { error },
            "Error fetching settings from external API - defaulting to fallback bouquet",
          );
          return withNormalizedFlags(BOUQUET_FALLBACK);
        }
  ...
  async getToolStates(overrideToken?: string): Promise<Record<string, boolean> | null> {
    ...
    if (settings.spaceTools && settings.spaceTools.length > 0) {
      ...
      logger.trace(
        { gradioEndpoints: this.gradioEndpoints },
        "Updated gradio endpoints from external API",
      );
    }
  ...
  updateGradioEndpointState(index: number, enabled: boolean): void {
    if (index >= 0 && index < this.gradioEndpoints.length) {
      ...
      if (endpoint) {
        logger.info(
          `Gradio endpoint ${(index + 1).toString()} set to ${enabled ? "enabled" : "disabled"}`,
        );
      }
    }
  }
  ...
  async startPolling(onUpdate: ToolStateChangeCallback): Promise<void> {
    ...
    if (this.config.type === "external") {
      logger.debug(
        "Using external user config API - no startup fetching, will fetch on first user request",
      );
      return;
    }
      ...
      void (async () => {
        ...
        for (const [toolId, enabled] of Object.entries(states)) {
          ...
          if (cachedState !== enabled) {
            logger.info(
              `Tool ${toolId} state changed: ${String(cachedState)} -> ${String(enabled)}`,
            );
            this.cache.set(toolId, enabled);
            onUpdate(toolId, enabled);
