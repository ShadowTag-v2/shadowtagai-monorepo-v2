...
import { type WhoAmI, whoAmI } from "@huggingface/hub";
import {
  CONFIG_GUIDANCE,
...
import { type AppSettings, DEFAULT_SPACE_TOOLS } from "../shared/settings.js";
...
export const createServerFactory = (
  ...
  return async (
    ...
    if (hfToken) {
      try {
        ...
        logger.warn(
          ...
          `Failed to authenticate with Hugging Face API`,
        );
      }
    }
    ...
    const response = userDetails
      ? `You are authenticated as ${username ?? "unknown"}.`
      : CONFIG_GUIDANCE;
    server.tool(
      "hf_whoami",
      whoDescription,
    ...
    const hubInspectSchemaShape: z.ZodRawShape = hubInspectReadmeAllowed
      ? hubInspectBaseShape
      : (() => {
          const { include_readme: _omit, ...rest } = hubInspectBaseShape as unknown as Record<
            string,
            unknown
          >;
          ...
          try {
            ...
            if ("result" in result && result.result) {
              ...
              const responseContent = [
                ...warningsContent,
                ...(processedResult.content as unknown[]),
              ];
              logGradioEvent(
