...
import { z } from "zod";
import { CACHE_CONFIG, spaceMetadataCache } from "./utils/gradio-cache.js";
import { getMetricsSafeName, gradioMetrics } from "./utils/gradio-metrics.js";
import {
  applyResultPostProcessing,
  callGradioTool,
  type GradioToolCallOptions,
} from "./utils/gradio-tool-caller.js";
 ...
 */
function createToolDisplayInfo(
  connection: EndpointConnection,
  tool: Tool,
...
function createToolHandler(
  ...
  return async (params: Record<string, unknown>, extra) => {
    ...
    try {
      ...
      logger.error(
        { tool: tool.name, err: errObj, errMessage: errObj.message },
      ...
      logGradioEvent(
        connection.name || connection.endpointId,
...
export function convertJsonSchemaToZod(
  ...
  if (
    ...
    Array.isArray(jsonSchemaProperty.enum) &&
    jsonSchemaProperty.enum.length > 0
  ) {
    // Handle enum types
