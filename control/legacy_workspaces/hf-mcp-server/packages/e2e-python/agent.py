...
@fast.agent(
  name="DVe0UTvm4", instruction="You are a helpful AI Agent", servers=["test_hf"]
)
@fast.agent(
  name="all", instruction="You are a helpful AI Agent", servers=["test_all_hf"]
)
async def main():
  ...
  async with fast.run() as agent:
    ...
    await agent.anon(
      '***CALL_TOOL space_search {"query": "evalstate", "limit": 3,"mcp": true}'
    )
