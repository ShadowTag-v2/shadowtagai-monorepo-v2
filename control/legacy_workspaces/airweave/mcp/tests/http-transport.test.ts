...
describe("HTTP Transport - Session Management", () => {
  ...
  beforeEach(() => {
    ...
    app.post("/mcp", async (req, res) => {
      ...
      const sessionId =
        req.headers["mcp-session-id"]?.toString() || `session_${Date.now()}_${Math.random()}`;
  ...
  describe("API Key Authentication", () => {
    it("should require API key", async () => {
      const response = await request(app).post("/mcp").send({ method: "test" });
  ...
  describe("Session Cleanup", () => {
    it("should allow session termination via DELETE", async () => {
      ...
      // Terminate session
      const deleteResponse = await request(app).delete("/mcp").set("MCP-Session-ID", "session-123");
