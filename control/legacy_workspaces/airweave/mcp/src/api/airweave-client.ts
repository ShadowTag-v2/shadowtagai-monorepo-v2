...
import type { AirweaveConfig, SearchResponse } from "./types.js";
...
export class AirweaveClient {
  ...
  async search(searchRequest: any): Promise<SearchResponse> {
    console.log(
      `[${new Date().toISOString()}] AirweaveClient.search called with:`,
    ...
    try {
      ...
      console.log(
        `[${new Date().toISOString()}] Search successful, got ${response.results?.length || 0} results`,
      );
      return response;
    } catch (error: any) {
      ...
      if (error.statusCode) {
        ...
        throw new Error(
          `Airweave API error (${error.statusCode}): ${error.message}\nStatus code: ${error.statusCode}\nBody: ${errorBody}`,
        );
      } else {
