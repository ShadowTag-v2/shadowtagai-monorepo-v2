...
import {
  ...
  type SessionData,
  SessionMetadata,
  type SessionWithTransport,
} from "./session/redis-session-manager.js";
 ...
 */
async function createSessionObjects(
  sessionData: SessionData,
  apiKey: string,
): Promise<SessionWithTransport> {
...
app.post("/mcp", async (req, res) => {
  try {
    ...
    if (session) {
      ...
      if (!apiKeyMatches) {
        console.log(
          `[${new Date().toISOString()}] API key changed for session ${sessionId}, recreating...`,
        );
        ...
        if (session.data.clientIP && session.data.clientIP !== clientIP) {
          console.warn(
            `[${new Date().toISOString()}] Session hijacking attempt detected: IP mismatch for ${sessionId}`,
          );
          res.status(403).json({
            jsonrpc: "2.0",
            error: {
      ...
      if (sessionData) {
        ...
        if (!apiKeyMatches) {
          console.log(
            `[${new Date().toISOString()}] API key mismatch for session ${sessionId}, recreating...`,
          );
        ...
        if (!rateLimit.allowed) {
          console.warn(
            `[${new Date().toISOString()}] Rate limit exceeded for API key (${rateLimit.count} sessions/hour)`,
          );
          res.status(429).json({
            jsonrpc: "2.0",
            error: {
