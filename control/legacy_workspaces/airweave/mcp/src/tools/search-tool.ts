...
import { z } from "zod";
import type { AirweaveClient } from "../api/airweave-client.js";
import { formatErrorResponse, formatSearchResponse } from "../utils/error-handling.js";
...
export function createSearchTool(
  ...
  const searchSchema = {
    ...
    score_threshold: z
      ...
      .max(1)
      .optional()
      .describe("Minimum similarity score threshold (0..1). Only return results above this score"),
    search_method: z
      .enum(["hybrid", "neural", "keyword"])
      .optional()
      .describe(
        "Search method: 'hybrid' (combines neural + keyword), 'neural' (semantic only), 'keyword' (text matching only)",
      ),
    expansion_strategy: z
      .enum(["auto", "llm", "no_expansion"])
      .optional()
      .describe(
        "Query expansion strategy: 'auto' (generates query variations), 'llm' (AI-powered expansion), 'no_expansion' (use exact query)",
      ),
    enable_reranking: z
      .boolean()
      .optional()
      .describe("Enable LLM-based reranking to improve result relevance"),
    enable_query_interpretation: z
      .boolean()
      .optional()
      .describe("Enable automatic filter extraction from natural language query"),
  };
