...
async function generateAppGrid() {
  ...
  for (const file of files) {
    ...
    currentRow.push(
      `<img src="frontend/src/components/icons/apps/${file}" alt="${name}" width="${ICON_SIZE}" height="${ICON_SIZE}" style="margin: 4px; padding: 2px;" />`,
    );
  ...
  if (currentRow.length > 0) {
    ...
    const paddedRow = [
      ...Array(padding).fill(
        `<span style="width: ${ICON_SIZE}px; display: inline-block; margin: 4px;"></span>`,
      ),
      ...currentRow,
    ];
    rows.push(paddedRow);
