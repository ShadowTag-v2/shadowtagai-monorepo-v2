...
class TeamsBongo(BaseBongo):
  ...
  async def create_entities(self) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
      ...
      channel_name = (
        f"monke-test-{str(uuid.uuid4())[:6]}-{channel_data['display_name'][:20]}"
      )
      ...
      try:
        ...
        async def create_channel_message(idx: int):
          async with semaphore:
            ...
            self.logger.info(
              f"Creating channel message {idx + 1}/{self.entity_count} with token {message_token}"
            )
      ...
      for entity in entities:
        try:
          ...
          if entity_type == "channel_message":
            ...
            url = (
              f"{self.GRAPH_BASE_URL}/teams/{entity['team_id']}/channels/{entity_id}"
            )
