...
class SharePointBongo(BaseBongo):
  ...
  def __init__(self, credentials: dict[str, Any], **kwargs):
    ...
    if "access_token" not in credentials:
      ...
      raise ValueError(
        f"Missing 'access_token' in credentials. Available fields: {list(credentials.keys())}"
      )
  ...
  async def create_entities(self) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
      ...
      if not is_valid:
        raise RuntimeError(
          "SharePoint access token is invalid or expired. Please reconnect your SharePoint account in Composio."
        )
