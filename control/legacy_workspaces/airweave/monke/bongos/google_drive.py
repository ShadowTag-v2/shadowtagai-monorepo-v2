  ...
  async def delete_specific_entities(self, entities: list[dict[str, Any]]) -> list[str]:
    ...
    for entity in entities:
      try:
        ...
        test_file = next(
          (tf for tf in self.test_files if tf["id"] == entity["id"]), None
        )
    ...
    self.logger.info(
      "🔍 VERIFYING: Checking if files are actually deleted from Google Drive"
    )
    for entity in entities:
      if entity["id"] in deleted_ids:
        ...
        if is_deleted:
          self.logger.info(
            f"✅ File {entity['id']} confirmed deleted from Google Drive"
          )
        else:
