...
class JiraBongo(BaseBongo):
  ...
  async def update_entities(self) -> list[dict[str, Any]]:
    ...
    issues_to_update = min(
      3, self.entity_count
    )  # Update max 3 issues for any test size
  ...
  async def delete_specific_entities(self, entities: list[dict[str, Any]]) -> list[str]:
    ...
    for entity in entities:
      try:
        ...
        test_issue = next(
          (ti for ti in self.test_issues if ti["id"] == entity["id"]), None
        )
  ...
  async def _delete_test_issue(self, issue_id: str):
    ...
    async with httpx.AsyncClient() as client:
      ...
      if response.status_code != 204:
        raise Exception(
          f"Failed to delete issue: {response.status_code} - {response.text}"
        )
