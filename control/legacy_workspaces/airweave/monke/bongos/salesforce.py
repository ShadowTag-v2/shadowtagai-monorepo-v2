...
class SalesforceBongo(BaseBongo):
  ...
  async def create_entities(self) -> list[dict[str, Any]]:
    ...
    gen_results = await asyncio.gather(
      *[generate_contact_data(token) for token in tokens]
    )
  ...
  async def cleanup(self):
    ...
    try:
      ...
      if self._contacts:
        self.logger.info(
          f"🗑️ Cleaning up {len(self._contacts)} current session contacts"
        )
      ...
      self.logger.info(
        f"🧹 Cleanup completed: {cleanup_stats['contacts_deleted']} contacts deleted, {cleanup_stats['errors']} errors"
      )
    except Exception as e:
            ...
            for contact in contacts:
              try:
                ...
                if del_r.status_code in (204, 200):
                  ...
                  self.logger.info(
                    f"✅ Deleted orphaned contact: {contact.get('Email', 'unknown')}"
                  )
                else:
                  stats["errors"] += 1
              except Exception as e:
