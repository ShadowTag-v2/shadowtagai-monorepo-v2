...
class AirtableBongo(BaseBongo):
  ...
  async def create_entities(self) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
      ...
      async def create_one() -> dict[str, Any] | None:
        async with semaphore:
          try:
            ...
            self.logger.info(
              f"📝 Generated record: '{fields.get('Name', '')[:50]}...' with {len(comments)} comments"
            )
            ...
            for comment_text in comments:
              ...
              try:
                ...
                if comment_resp.status_code in (200, 201):
                  ...
                  self.logger.debug(
                    f"Added comment to record {record_id}: '{comment_text[:50]}...'"
                  )
                else:
                  self.logger.warning(
  ...
  async def cleanup(self):
    ...
    try:
      ...
      if self._records:
        self.logger.info(
          f"🗑️ Cleaning up {len(self._records)} current session records (and {len(self._comments)} comments)"
        )
  ...
  async def _ensure_base(self):
    ...
    async with httpx.AsyncClient() as client:
      try:
        ...
        if not bases:
          raise RuntimeError(
            "No accessible Airtable bases found. Please create a base and grant access to your OAuth app."
          )
