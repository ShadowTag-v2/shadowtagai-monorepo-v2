...
class OutlookMailBongo(BaseBongo):
  ...
  async def cleanup(self):
    ...
    try:
      ...
      self.logger.info(
        f"🧹 Cleanup completed: {cleanup_stats['messages_deleted']} messages deleted, {cleanup_stats['errors']} errors"
      )
    except Exception as e:
            ...
            if r.status_code == 200:
              ...
              test_messages = [
                ...
                if any(
                  ...
                  or (
                    m.get("bodyPreview") and pattern in m.get("bodyPreview", "").lower()
                  )
              ...
              if test_messages:
                self.logger.info(
                  f"🔍 Found {len(test_messages)} potential test messages (search term: {term})"
                )
                for msg in test_messages:
                  try:
                    ...
                    if del_r.status_code == 204:
                      ...
                      self.logger.info(
                        f"✅ Deleted orphaned message: {msg.get('subject', 'Unknown')[:50]}"
                      )
                    else:
                      stats["errors"] += 1
                  except Exception as e:
