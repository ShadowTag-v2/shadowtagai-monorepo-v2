...
class ConfluenceBongo(BaseBongo):
  ...
  async def create_entities(self) -> list[dict[str, Any]]:
    ...
    async def create_single_page(index: int) -> dict[str, Any]:
      ...
      page_data = await self._create_test_page(
        self.test_space_key, unique_title, content
      )
  ...
  async def delete_specific_entities(self, entities: list[dict[str, Any]]) -> list[str]:
    ...
    for entity in entities:
      try:
        ...
        test_page = next(
          (tp for tp in self.test_pages if tp["id"] == entity["id"]), None
        )
    ...
    self.logger.info(
      "🔍 VERIFYING: Checking if pages are actually deleted from Confluence"
    )
    for entity in entities:
  ...
  async def _get_cloud_id(self) -> str:
    ...
    async with httpx.AsyncClient() as client:
      ...
      if response.status_code != 200:
        raise Exception(
          f"Failed to get cloud ID: {response.status_code} - {response.text}"
        )
  ...
  async def _ensure_test_space(self):
    ...
    async with httpx.AsyncClient() as client:
      ...
      if response.status_code != 200:
        raise Exception(
          f"Failed to get spaces: {response.status_code} - {response.text}"
        )
  ...
  async def _create_test_page(
    self, space_key: str, title: str, content: str
  ) -> dict[str, Any]:
