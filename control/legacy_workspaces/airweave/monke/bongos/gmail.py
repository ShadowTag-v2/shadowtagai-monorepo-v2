...
class GmailBongo(BaseBongo):
  ...
  async def cleanup(self):
    ...
    """
    self.logger.info(
      "🧹🔥 PURGING Gmail workspace - searching for all test-related emails"
    )
    ...
    if untracked_found_ids:
      self.logger.info(
        f"🧹 Found {len(untracked_found_ids)} additional test emails to delete"
      )
    ...
    cleanup_stats["total_deleted"] = (
      cleanup_stats["tracked_deleted"] + cleanup_stats["searched_deleted"]
    )
