...
class GitLabBongo(BaseBongo):
  ...
  async def create_entities(self) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
      ...
      async def create_one_issue() -> dict[str, Any] | None:
  ...
  async def update_entities(self) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
      ...
      if self._merge_requests:
        ...
        mr_url = (
          f"{self.API_BASE}/projects/{self._project_id}/merge_requests/{mr['iid']}"
        )
        resp = await client.put(
          mr_url,
          headers=self._headers(),
      ...
      for entity in entities:
        try:
          ...
          if entity["type"] == "issue":
            ...
            issue_url = (
              f"{self.API_BASE}/projects/{self._project_id}/issues/{entity['iid']}"
            )
            r = await client.put(
              issue_url,
              headers=self._headers(),
