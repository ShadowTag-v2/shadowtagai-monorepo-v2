...
class StripeBongo(BaseBongo):
  ...
  async def update_entities(self) -> list[dict[str, Any]]:
    ...
    customers_to_update = min(
      3, self.entity_count
    )  # Update max 3 customers for any test size
  ...
  async def delete_specific_entities(self, entities: list[dict[str, Any]]) -> list[str]:
    ...
    self.logger.info(
      "🔍 VERIFYING: Checking if customers are actually deleted from Stripe"
    )
    for entity in entities:
  ...
  async def cleanup(self):
    ...
    for test_customer in self.test_customers:
      try:
        ...
        self.logger.warning(
          f"⚠️ Could not force delete customer {test_customer['id']}: {e}"
        )
