...
class ExcelBongo(BaseBongo):
  ...
  def __init__(self, credentials: dict[str, Any], **kwargs):
    ...
    self._test_workbook_id: str | None = None
    self._test_workbook_name: str | None = None
    ...
    async with httpx.AsyncClient(base_url=GRAPH, timeout=30) as client:
      try:
        ...
        if r.status_code == 204:
          ...
          self.logger.info(
            f"✅ Deleted workbook: {self._test_workbook_name} ({len(deleted)} worksheets)"
          )
  ...
  async def cleanup(self):
    ...
    try:
      ...
      self.logger.info(
        f"🧹 Cleanup completed: {cleanup_stats['workbooks_deleted']} workbooks deleted, {cleanup_stats['errors']} errors"
      )
    except Exception as e:
