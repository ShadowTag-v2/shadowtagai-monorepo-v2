...
class NotionBongo(BaseBongo):
  ...
  def __init__(self, credentials: dict[str, Any], **kwargs):
    ...
    if isinstance(raw_parent, str) and raw_parent.startswith("${"):
      raw_parent = None
    self.parent_id: str | None = raw_parent

    # Runtime state
    self._pages: list[dict[str, Any]] = []
    self._parent_page_id: str | None = None
  ...
  async def update_entities(self) -> list[dict[str, Any]]:
    ...
    for i in range(pages_to_update):
      ...
      page_update = {
        "properties": {
          "title": {"title": [{"text": {"content": f"{token} {title} (Updated)"}}]}
        }
      }
  ...
  async def delete_entities(self) -> list[str]:
    ...
    for i, page in enumerate(self._pages, 1):
      ...
      self.logger.info(
        f"✅ Trashed: {page_title} [{page_id}] - API returned: in_trash={is_trashed}, archived={is_archived}"
      )
  ...
  async def cleanup(self):
    ...
    try:
        ...
        if active_orphans:
          ...
          for page in active_orphans:
            try:
              ...
              if not is_trashed:
                self.logger.error(
                  f"   ❌ WARNING: Orphaned page {page['id']} was 'trashed' but API returned in_trash={is_trashed}!"
                )
            except Exception as e:
      ...
      self.logger.info(
        f"🧹 Cleanup completed: {cleanup_stats['pages_trashed']} pages trashed, {cleanup_stats['errors']} errors"
      )
  ...
  async def _find_monke_test_pages(self) -> list[dict[str, Any]]:
    ...
    try:
      ...
      while has_more:
        ...
        self.logger.info(
          f"🔍 _find_monke_test_pages: Search page returned {len(results)} pages (has_more={has_more})"
        )
