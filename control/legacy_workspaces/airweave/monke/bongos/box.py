...
class BoxBongo(BaseBongo):
  ...
  def __init__(self, credentials: dict[str, Any], **kwargs):
    ...
    self._test_folder_id: str | None = None
  ...
  async def create_entities(self) -> list[dict[str, Any]]:
      ...
      for i in range(self.entity_count):
        async with semaphore:
          ...
          for file_idx in range(2):  # 2 files per folder
            ...
            self.logger.info(
              f"  Creating file {file_idx + 1}/2 in folder {folder['id']} with token {file_token}"
            )
            ...
            self.logger.info(
              f"    Creating comment on file {file['id']} with token {comment_token}"
            )
    ...
    self.logger.info(
      f"✅ Created {len(self._folders)} folders, {len(self._files)} files, {len(self._comments)} comments"
    )
      ...
      for entity in entities:
        if entity["type"] == "folder":
          ...
          for subfolder in self.created_entities:
            if (
              subfolder["type"] == "folder" and subfolder.get("parent_id") == folder_id
            ):
      ...
      try:
        ...
        try:
          ...
          if resp.status_code == 200:
            ...
            for item in items:
              if item.get("type") == "folder" and item.get("name", "").startswith(
                "Airweave_Test_"
              ):
                try:
                  await self._rate_limit()
