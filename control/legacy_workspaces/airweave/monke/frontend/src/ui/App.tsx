...
export function App() {
  ...
  useEffect(() => {
    ...
    // live updates via WS
    const proto =
      API_BASE.startsWith("https") || (!API_BASE && location.protocol === "https:") ? "wss" : "ws";
    ...
    try {
      ...
      setRuns((prev) => {
        const placeholders = ids.map(
          (id) => ({ id, connector: "starting", status: "queued", progress: 0 }) as RunSummary,
        );
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-4">
      <header className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            disabled={!selected || runBusy}
            className="px-3 py-2 rounded-lg bg-[#1f2a44] hover:opacity-90 disabled:opacity-50"
            onClick={onRun}
          >
            {runBusy ? "Running…" : "Run"}
          </button>
...
function RunCard({ summary }: { summary: RunSummary }) {
  ...
  const statusClass = useMemo(
    () =>
      ...
      })[summary.status] || "text-slate-300",
    [summary.status],
  );

  useEffect(() => {
    fetch(`${API_BASE}/api/runs/${summary.id}`)
      .then((r) => r.json())
      .then((d) => {
        setDetail(d);
      });
    ...
    <div className="bg-[#12192a] border border-[#1d2740] rounded-xl p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className={`text-xs bg-[#1f2a44] px-2 py-1 rounded-full ${statusClass}`}>
            {summary.status}
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-wrap my-2">
        {detail?.steps?.map((s) => (
          <div
            key={s.index}
            className={`text-xs px-2 py-1 rounded-full border ${stepClass(s.status)}`}
          >
            {s.name}
          </div>
        ))}
      ...
      <div className="h-2 rounded-full border border-[#24314f] overflow-hidden bg-[#0f1524]">
        <div
          ...
          style={{ width: `${Math.round((detail?.progress ?? summary.progress) * 100)}%` }}
        />
      </div>
      <details
        ref={detailsRef}
        onToggle={() => setLogsOpen(!!detailsRef.current?.open)}
        className="mt-2 bg-[#0f1524] border border-[#24314f] rounded-lg p-2"
      >
        <summary>
          Logs
          <button
        ...
        <pre
          ref={logsRef}
          className="text-xs text-slate-300 whitespace-pre-wrap max-h-72 overflow-auto"
        >
