...
class TestFlow:
  ...
  def __init__(self, config: TestConfig, run_id: str | None = None):
  ...
  async def _execute_step(
    self, step_name: str, step_idx: int = 0, total_steps: int = 0
  ):
    ...
    if step_idx > 0 and total_steps > 0:
      self.logger.info("")
      self.logger.info(
        f"📍 PROGRESS: Step {step_idx}/{total_steps} - {step_name.upper()}"
      )
      self.logger.info("")
    else:
