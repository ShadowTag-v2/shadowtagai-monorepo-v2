...
class ConnectorConfig(BaseModel):
  ...
  auth_mode: str = Field(
    "composio", description="Authentication mode: 'composio' or 'direct'"
  )
  ...
  def validate_auth_fields(cls, v: dict[str, str]) -> dict[str, str]:
    ...
    for field_name, env_var in v.items():
      if not env_var.startswith("MONKE_"):
        raise ValueError(
          f"Environment variable '{env_var}' for field '{field_name}' must start with 'MONKE_'"
        )
    return v
  ...
  def validate_auth_consistency(self) -> "ConnectorConfig":
    ...
    if self.auth_mode == "composio":
      ...
      if self.auth_fields:
        raise ValueError(
          "auth_mode is 'composio' but auth_fields are provided. Cannot use both Composio and direct auth"
        )
    elif self.auth_mode == "direct":
      if self.composio_config:
        raise ValueError(
          "auth_mode is 'direct' but composio_config is provided. Cannot use both direct and Composio auth"
        )
  ...
  def resolve_auth_fields(self) -> dict[str, Any]:
    ...
    for field_name, env_var in self.auth_fields.items():
      ...
      if not value:
        raise ValueError(
          f"Required environment variable '{env_var}' not set for field '{field_name}' in {self.type}"
        )
...
class DeletionConfig(BaseModel):
  ...
  verify_partial_deletion: bool = Field(
    True, description="Verify partial deletion worked"
  )
  verify_remaining_entities: bool = Field(
    True, description="Verify remaining entities exist"
  )
  verify_complete_deletion: bool = Field(
    True, description="Verify complete deletion worked"
  )
...
class TestConfig(BaseModel):
  ...
  deletion: DeletionConfig = Field(
    default_factory=DeletionConfig, description="Deletion config"
  )
  entity_count: int = Field(10, ge=1, description="Number of entities to create")
  collection_config: dict[str, Any] = Field(
    default_factory=dict, description="Collection config"
  )
  verification_config: dict[str, Any] = Field(
    default_factory=dict, description="Verification config"
  )
  cleanup_config: dict[str, Any] = Field(
    default_factory=dict, description="Cleanup config"
  )
