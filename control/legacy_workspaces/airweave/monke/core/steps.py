...
class CreateStep(TestStep):
  ...
  async def execute(self) -> None:
    ...
    self.logger.info(
      f"🎯 Target: Create {entity_count} test entities with tracking tokens"
    )
    ...
    self.logger.info(
      f"✅ Successfully created {len(entities)} entities in {self.config.connector.type}"
    )
    ...
    if delay_seconds > 0:
      self.logger.info(
        f"⏸️ Post-create delay: Waiting {delay_seconds}s for API propagation..."
      )
      self.logger.info(
        "   (This allows the source system to index newly created entities)"
      )
      await asyncio.sleep(delay_seconds)

    self.logger.info("=" * 80)
    self.logger.info(
      f"✅ CREATE COMPLETED: {len(entities)} test entities ready for sync"
    )
    self.logger.info("=" * 80)
...
class SyncStep(TestStep):
  ...
  async def execute(self) -> None:
    ...
    if self.force_full_sync:
      self.logger.info(
        "   🌀 Force full sync enabled - will ignore cursor data and fetch all entities"
      )
    self.logger.info("=" * 80)
    ...
    self.logger.info(
      "⏳ Pre-sync delay: Waiting 30s for external API to index changes..."
    )
    self.logger.info(
      "   (This ensures newly created/modified entities are available in the source API)"
    )
    ...
    self.logger.info("=" * 80)
    self.logger.info(
      f"✅ {sync_mode} COMPLETED: Data pipeline execution finished successfully"
    )
    self.logger.info(f"📊 Job ID: {target_job_id}")
    self.logger.info(
      "📝 Note: Check backend logs for detailed sync pipeline metrics (entities processed, errors, etc.)"
    )
    self.logger.info("=" * 80)
  ...
  def _find_active_job_id(self) -> str | None:
  ...
  def _get_latest_job_id(self) -> str | None:
  ...
  try:
    ...
    if results and len(results) > 0:
      ...
      for i, r in enumerate(results[:3]):
        ...
        score = r.get("score", 0)
        name = (
          payload.get("name") or payload.get("title") or payload.get("id", "Unknown")
        )
    ...
    if expect_present:
      logger.warning(
        f"❌ Token '{token}' NOT found in vector database (expected to be present)"
      )
    else:
      logger.info(
        f"✅ Token '{token}' confirmed absent from vector database (as expected)"
      )
    return False
...
class VerifyStep(TestStep):
  ...
  async def execute(self) -> None:
    ...
    self.logger.info(
      f"🎯 Target: Verify {entity_count} entities were successfully synced"
    )
    self.logger.info(f"📦 Collection: {self.context.collection_readable_id}")
    self.logger.info(
      "🔍 Strategy: Search for unique tokens embedded in each test entity"
    )
    ...
    if errors:
      self.logger.error(
        f"❌ VERIFICATION FAILED: {verified_count}/{entity_count} entities found"
      )
      self.logger.error(
        f"📊 Summary: {len(errors)} entity(ies) missing from vector database"
      )
      for error in errors:
      ...
      self.logger.info(
        "   1. Check if entities were created successfully in the source system"
      )
      self.logger.info(
        "   2. Review backend sync logs to see if source API returned the entities"
      )
      self.logger.info(
        "   3. Verify filter configurations aren't excluding test entities"
      )
      self.logger.info(
        "   4. Ensure adequate wait time for source API indexing (post_create_sleep_seconds)"
      )
      self.logger.info("=" * 80)
      raise Exception("; ".join(errors))
    else:
      self.logger.info(
        f"✅ VERIFICATION PASSED: All {entity_count}/{entity_count} entities found!"
      )
      self.logger.info(
        "📊 Summary: Every test entity was successfully synced to the vector database"
      )
      self.logger.info("=" * 80)
...
class PartialDeleteStep(TestStep):
  ...
  async def execute(self) -> None:
    ...
    self.logger.info(
      f"🗑️ Deleting {len(entities_to_delete)} entities: {[self._display_name(e) for e in entities_to_delete]}"
    )
    self.logger.info(
      f"💾 Initially keeping {len(entities_to_keep)} entities: {[self._display_name(e) for e in entities_to_keep]}"
    )
