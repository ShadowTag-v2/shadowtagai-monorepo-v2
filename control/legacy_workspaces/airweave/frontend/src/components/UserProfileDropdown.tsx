...
const MenuItemWithIcon = ({
  ...
  <DropdownMenuItem onSelect={onClick} disabled={disabled} className={cn(menuItemClass, className)}>
...
export function UserProfileDropdown() {
  ...
  const { organizations, currentOrganization, fetchUserOrganizations, switchOrganization } =
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      case "owner":
        return <Crown className="h-3 w-3 text-brand-lime/90" />;
      case "admin":
        return <Shield className="h-3 w-3 text-brand-lime/90" />;
      default:
        return <Users className="h-3 w-3 text-brand-lime/90" />;
    }
  };
      ...
      <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
            ...
            <div className="flex items-center gap-3">
              <Avatar className="h-8 w-8">
                ...
                <AvatarFallback className="bg-primary/10 text-primary text-bold text-xs">
                  {firstName ? firstName[0] : user?.email?.substring(0, 1).toUpperCase() || "U"}
                </AvatarFallback>
              </Avatar>
              <div className="flex flex-col items-start min-w-0">
                <span className="text-foreground truncate max-w-32">{user?.name || "User"}</span>
                {currentOrganization && (
        ...
        >
          ...
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent
              className="w-72 p-1 shadow-xs animate-none max-h-96 overflow-y-auto"
              alignOffset={-120}
            >
              {isLoadingOrgs ? (
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0 flex-1">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {org.is_primary && (
                              <div className="text-xs mt-0.5 text-brand-yellow">
                        ...
                        <div className="shrink-0 ml-2">
                          {isSelected && <Check className="h-4 w-4 text-muted-foreground/60" />}
                        </div>
                      </DropdownMenuItem>
                    );
                  })}
          ...
          {currentOrganization && ["owner", "admin"].includes(currentOrganization.role) && (
            <>
              ...
              <InternalMenuLink to="/organization/settings" icon={<Settings className="h-4 w-4" />}>
          ...
          {user?.is_admin && (
            <>
              <InternalMenuLink to="/admin" icon={<Shield className="h-4 w-4" />}>
                Admin Dashboard
              </InternalMenuLink>
          ...
          {/* External Links */}
          <ExternalMenuLink href="https://airweave.ai">Blog</ExternalMenuLink>

          <ExternalMenuLink href="https://docs.airweave.ai">Documentation</ExternalMenuLink>

          <ExternalMenuLink href="https://discord.gg/484HY9Ehxt">Join Discord</ExternalMenuLink>
