import { AlertCircle, Clock, Loader2, Pencil, Play, RefreshCw } from "lucide-react";
...
import { cleanEntityName, convertDagToFlowGraph } from "@/components/collection/DagToFlow";
import type { Dag } from "@/components/sync/dag";
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        ...
        >
          <CardHeader className="p-3">
            <div className="flex justify-between items-center">
              <h3
                className={cn("text-base font-medium", isDark ? "text-gray-200" : "text-gray-700")}
              >
                Entity Graph
              </h3>
        ...
        >
          ...
          >
            <h3 className={cn("text-base font-medium", isDark ? "text-gray-200" : "text-gray-700")}>
              Entities
            </h3>
          </div>
...
const SimplifiedSourceConnectionDetailView = ({
  ...
  const fetchSyncJob = async (connection: SourceConnection) => {
    ...
    try {
      ...
      const response = await apiClient.get(
        `/source-connections/${connection.id}/jobs/${connection.last_sync_job_id}`,
      );

      if (!response.ok) {
        console.error(
          `Failed to fetch job with ID ${connection.last_sync_job_id}, status: ${response.status}`,
        );
        setSyncJob(null);
        setTotalEntities(0);
        setTotalRuntime(null);
  ...
  const updateEntityDictionary = useCallback(() => {
    ...
    const sourceName =
      entityDags[0]?.nodes?.filter((node) => node.type === "source")?.map((node) => node.name)[0] ||
      "";
  ...
  useEffect(() => {
    ...
    const matchingDag =
      exactMatch || entityDags.find((dag) => dag.name && dag.name.includes(selectedEntity));
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              >
                ...
                <div className="text-sm font-semibold">{totalEntities.toLocaleString()}</div>
              </div>
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  <Clock className={cn("w-3 h-3", isDark ? "text-gray-400" : "text-gray-500")} />
                  <div className="text-sm font-medium">
