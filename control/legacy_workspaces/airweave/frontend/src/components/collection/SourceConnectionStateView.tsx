...
import { formatTimeUntil, parseCronExpression } from "@/utils/cronParser";
...
const SourceConnectionStateView: React.FC<Props> = ({
  ...
  const [sourceConnection, setSourceConnection] = useState<SourceConnection | null>(
    sourceConnectionData || null,
  );
  ...
  const storeConnection = useEntityStateStore((store) => store.getConnection(sourceConnectionId));
  ...
  const formatTimeAgo = useCallback((dateStr: string | undefined) => {
    ...
    const hasTimezone =
      dateStr.endsWith("Z") || dateStr.match(/[T\s]\d{2}:\d{2}:\d{2}(?:\.\d+)?[+-]\d{2}:?\d{2}$/);
  ...
  const fetchSourceConnection = useCallback(
    async (regenerateAuthUrl = false) => {
  ...
  useEffect(() => {
    const isNotAuthorized =
      sourceConnectionData?.status === "pending_auth" || !sourceConnectionData?.auth?.authenticated;
    ...
    if (!isNotAuthorized) {
      ...
      Promise.all([mediator.current.initialize(), fetchSourceConnection()])
        .then(async () => {
  ...
  useEffect(() => {
    const updateLastRan = () => {
      ...
      const startedAt =
        storeConnection?.last_sync_job?.started_at || sourceConnection?.last_sync_job?.started_at;
    ...
    storeConnection?.last_sync_job?.started_at,
  ...
  useEffect(() => {
    ...
    storeConnection?.last_sync_job?.error,
    fetchSourceConnection,
  ]);
  ...
  const handleRunSync = async () => {
    if (!entitiesAllowed || isCheckingUsage) {
      toast({
        title: "Limit reached",
        description:
          entitiesCheckDetails?.reason === "usage_limit_exceeded"
    ...
    try {
      ...
      cancelTimeoutRef.current = setTimeout(() => {
        ...
        toast({
          ...
          description:
            "The cancellation request timed out. The sync may still be cancelled in the background.",
          variant: "destructive",
        });
        cancelTimeoutRef.current = null;
      }, 30000);
      ...
      const response = await apiClient.post(
        `/source-connections/${sourceConnectionId}/jobs/${jobId}/cancel`,
      );
  ...
  const getSyncStatusDisplay = () => {
    ...
    if (syncState === "CANCELLING")
      return { text: "Cancelling", color: "bg-orange-500 animate-pulse", icon: "loader" };
    if (currentSyncJob?.status === "failed")
      return { text: "Failed", color: "bg-red-500", icon: null };
    if (currentSyncJob?.status === "completed")
  ...
  const isNotAuthorized =
    sourceConnection?.status === "pending_auth" || !sourceConnection?.auth?.authenticated;
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2 flex-wrap items-center">
            ...
            {!isFederatedSource && (
              <div
                className={cn(
                  "h-8 px-3 py-1.5 border border-border rounded-md shadow-sm flex items-center gap-2 min-w-[90px]",
              ...
              >
                ...
                <span className="text-xs font-semibold text-foreground">
                  ...
                  {(
                    storeConnection?.entity_states?.reduce(
                      (sum, state) => sum + (state.total_count || 0),
                      0,
                    ) ||
              ...
              <TooltipProvider delayDuration={100}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div
                      className={cn(
                        "h-8 px-3 py-1.5 border border-border rounded-md shadow-sm flex items-center gap-2 min-w-[100px] cursor-help",
                        isDark ? "bg-gray-900" : "bg-white",
                      )}
                    >
                      <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/60">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        >
                          {(() => {
                            const parsed = parseCronExpression(sourceConnection?.schedule?.cron);
                            if (parsed) {
                              const nextRunStr = formatTimeUntil(
                                sourceConnection?.schedule?.next_run,
                              );
                              return nextRunStr
                                ? `${parsed.shortDescriptionLocal} (${nextRunStr})`
                                : parsed.shortDescriptionLocal;
                            }
                            return "Manual";
                          })()}
                        </span>
                    ...
                    <div className="text-xs space-y-1">
                      ...
                      {sourceConnection?.schedule?.next_run && (
                        <p className="text-muted-foreground">
                          Next run:{" "}
                          {new Date(sourceConnection.schedule.next_run)
                            .toISOString()
                            .replace("T", ", ")
                            .replace(".000Z", "")}{" "}
                          UTC
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="text-xs max-w-[240px]">
                      This source uses federated search. Data is queried in real-time when you
                      search instead of being synced and indexed.
                    </p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
          ...
          <div className="flex gap-1.5 items-center">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      disabled={
                        syncState === "CANCELLING" || (!entitiesAllowed && syncState === "IDLE")
                      }
                      className={cn(
                      ...
                      {syncState === "CANCELLING"
                        ...
                        : syncState === "SYNCING"
                          ...
                          : !entitiesAllowed &&
                            ...
                            : !entitiesAllowed &&
                                entitiesCheckDetails?.reason === "payment_required"
            ...
            {sourceConnection && (
              <div
                className={cn(
                  "h-8 w-8 border border-border rounded-md shadow-sm flex items-center justify-center",
                  isDark ? "bg-gray-900" : "bg-white",
                )}
              >
                <button
                  type="button"
                  onClick={() => {
                    /* Settings menu trigger logic would go here */
                  }}
          ...
          {!isFederatedSource && currentSyncJob?.status === "failed" && (
            <SyncErrorCard
              error={
                ...
                sourceConnection?.last_sync_job?.error ||
          ...
          {isFederatedSource && (
            ...
            >
              <div className="flex items-start gap-3">
                <Send
                  className={cn("h-5 w-5 mt-0.5", isDark ? "text-blue-400" : "text-blue-600")}
                />
                <div className="flex-1 space-y-2">
                  <p className={cn("text-sm", isDark ? "text-blue-200/80" : "text-blue-800/80")}>
                    This source searches the data at query time instead of syncing and indexing it
                    beforehand.
                  </p>
                </div>
              </div>
            ...
            <EntityStateList
              ...
              entityStates={
                sourceConnection?.entities
                  ...
                  : undefined
              } // Convert entities to entity_states format for EntityStateList
            />
          )}
        </>
      )}
