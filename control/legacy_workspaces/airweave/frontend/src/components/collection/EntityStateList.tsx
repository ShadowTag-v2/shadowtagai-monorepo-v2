    ...
    >
      <Tabs defaultValue="schema" className="flex flex-col flex-1 min-h-0 overflow-hidden">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {/* Tab Buttons */}
              <TabsList
                className={cn("h-7 p-0.5 rounded-md", isDark ? "bg-gray-800/60" : "bg-gray-100")}
              >
                <TabsTrigger
                  value="schema"
                  className={cn(
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          <TabsContent
            value="schema"
            className="h-full max-h-[400px] overflow-y-auto overflow-x-hidden scrollbar-thin"
          >
            {entity.definition ? (
                  ...
                  <div className="overflow-x-auto">
                    {(() => {
                      try {
                        ...
                        if (entity.definition.type === "file" && Array.isArray(schema)) {
                          return (
                            <div className={cn(DESIGN_SYSTEM.typography.sizes.body)}>
                              ...
                              <span className="font-mono">{(schema as string[]).join(", ")}</span>
                            </div>
                          );
                        }
                        ...
                        return <SimplifiedSchemaView schema={parsedSchema} isDark={isDark} />;
                      } catch (error) {
          ...
          <TabsContent
            value="example"
            className="h-full max-h-[400px] overflow-y-auto overflow-x-hidden scrollbar-thin px-4 py-3"
          >
            <div
              className={cn(
                "rounded-lg border p-8 mt-4",
                isDark ? "bg-gray-900/20 border-gray-800/50" : "bg-gray-50/50 border-gray-200/50",
              )}
            >
...
export const EntityStateList: React.FC<EntityStateListProps> = ({
    ...
    const fetchEntityDefinitions = async () => {
      ...
      try {
        ...
        const response = await apiClient.get(
          `/entities/definitions/by-source/?source_short_name=${sourceShortName}`,
        );
        if (response.ok) {
  ...
  const combinedEntities = React.useMemo(() => {
    ...
    const entityStatesToUse =
      state?.entity_states && state.entity_states.length > 0
    ...
    const entityMap = new Map<
      string,
      { name: string; count: number; definition?: EntityDefinition }
    >();
  ...
  const expandedEntityData = expandedEntity
    ? combinedEntities.find((e) => e.name === expandedEntity)
    : null;

  console.log("Expanded state:", {
    expandedEntity,
    expandedEntityData,
    combinedEntitiesCount: combinedEntities.length,
  });
