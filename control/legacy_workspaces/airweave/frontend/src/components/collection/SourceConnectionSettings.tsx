  ...
  const fetchSourceDetailsForEdit = async () => {
    ...
    try {
      ...
      if (sourceConnection.auth_provider) {
        const authProviderResponse = await apiClient.get(
  ...
  const handleScheduleSave = async () => {
    try {
      const cronExpression =
        scheduleConfig.type === "scheduled" ? buildCronExpression(scheduleConfig) : null;
      ...
      if (hasConfigChanges) {
        try {
          const syncResponse = await apiClient.post(
            `/source-connections/${updatedConnection.id}/run`,
            {},
          );
    ...
    <>
      ...
      <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
        <DropdownMenuTrigger asChild>
          <Button
            size="sm"
            variant="ghost"
            className={cn(DESIGN_SYSTEM.buttons.heights.secondary, "w-8 p-0")}
          >
      ...
      {showScheduleDialog && (
        <Dialog open={showScheduleDialog} onOpenChange={setShowScheduleDialog}>
          <DialogContent className={cn("max-w-3xl", isDark ? "bg-card-solid border-border" : "")}>
            ...
            <div className="py-4">
              <SyncSchedule value={scheduleConfig} onChange={setScheduleConfig} />
            </div>
      ...
      {showDeleteDialog && (
        <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <AlertDialogContent
            className={cn("border-border", isDark ? "bg-card-solid text-foreground" : "bg-white")}
          >
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                      ...
                      <li className="flex items-start">
                        ...
                        <div>
                          <p className="text-sm text-muted-foreground">
                            You will need to re-authenticate and reconfigure the connection to sync
                            data from this source again.
                          </p>
                        </div>
                      </li>
                      <li className="flex items-start">
                        ...
                        <div>
                          <p className="text-sm text-muted-foreground">
                            All data that was synced from this source will be permanently removed
                            from the knowledge base and cannot be recovered.
                          </p>
                        </div>
                      </li>
                    </ul>
                ...
                <div className="mt-4">
                  <Label htmlFor="confirm-delete" className="text-sm font-medium block mb-2">
                    Type <span className="font-bold">{sourceConnection.name}</span> to confirm
                    deletion
                  </Label>
                  <Input
                    id="confirm-delete"
