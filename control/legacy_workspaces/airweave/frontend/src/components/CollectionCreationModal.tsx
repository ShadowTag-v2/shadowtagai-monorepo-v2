...
export const CollectionCreationModal: React.FC = () => {
    ...
    if (isOAuthReturn) {
      if (status === "success" || status === "sync_started") {
        ...
        if (collectionId) {
          ...
          navigate(
            `/collections/${collectionId}?status=success&source_connection_id=${connectionId}`,
          );
  ...
  const handleClose = () => {
    ...
    console.log(
      "Modal closing, isOpen should be false:",
      useCollectionCreationStore.getState().isOpen,
    );
  ...
  const showVisualization = currentStep !== "source-select" && currentStep !== "oauth-redirect";
