...
const getStatusDescription = (statusKey: string, context?: TooltipContext): string | null => {
  ...
  const collectionDescriptions: Record<string, string> = {
    ACTIVE:
      "At least one source connection has completed a sync or is currently syncing. Your collection has data and is ready for queries.",
    ERROR:
      "All source connections have failed their last sync. Check your connections and authentication to resolve sync issues.",
    "NEEDS SOURCE":
      "This collection has no authenticated connections, or connections exist but haven't successfully synced yet. Configure a source or wait for the initial sync to complete.",
  };

  // API key-specific descriptions
  const apiKeyDescriptions: Record<string, string> = {
    ACTIVE:
      "This API key is valid and can be used to authenticate requests. It will remain active until its expiration date.",
    EXPIRING_SOON:
      "This API key will expire within 7 days. Create a new key before expiration to avoid service interruption.",
    EXPIRED:
      "This API key has expired and can no longer be used. Delete this key and create a new one to continue using the API.",
