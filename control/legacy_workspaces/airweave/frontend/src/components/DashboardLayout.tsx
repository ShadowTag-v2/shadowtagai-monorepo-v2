...
const CollectionsSection = memo(() => {
  ...
  const getNavItemStyles = useCallback(
    (active: boolean) =>
      cn(
...
const DashboardLayout = () => {
  ...
  const isNonScrollable = nonScrollableRoutes.some(
  ...
  const isDashboardActive = useMemo(() => location.pathname === "/", [location.pathname]);

  const isApiKeysActive = useMemo(() => location.pathname === "/api-keys", [location.pathname]);
          ...
          <div className="space-y-0 pr-3 pb-4">
                  ...
                  {(!sourceConnectionsAllowed || !entitiesAllowed) && (
                    <TooltipContent className="max-w-xs">
                      <p className="text-xs">
                        ...
                        usageCheckDetails.source_connections?.reason === "usage_limit_exceeded" ? (
                          <>
                          ...
                          usageCheckDetails.entities?.reason === "usage_limit_exceeded" ? (
                          <>
            ...
            <div>
              <NavItem
                ...
                icon={
                  <Shield className="mr-2 h-4 w-4 opacity-70 group-hover:opacity-100 transition-opacity" />
                }
              >
                Auth Providers
              </NavItem>
      ...
      <GradientCard className="h-full">
        ...
        <div className="flex h-screen overflow-hidden">
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button
                  variant="outline"
                  size="icon"
                  className="bg-background-alpha-90 rounded-lg shadow-sm hover:bg-background-alpha-100 transition-all duration-200"
                >
              ...
              <SheetContent
                side="left"
                className="w-64 p-0 bg-background-alpha-90 backdrop-blur-md"
              >
                {SidebarContent}
              </SheetContent>
            </Sheet>
          ...
          <div className="w-full lg:pl-[240px] flex flex-col h-screen">
            <div className={cn("flex-1", isNonScrollable ? "overflow-hidden" : "overflow-auto")}>
              {/* Top Navigation Bar - Now inside the scrollable area */}
              <header
                className={`h-16 sticky top-0 pr-2 backdrop-blur-sm z-10 ${resolvedTheme === "dark" ? "bg-background/80" : "bg-background/95"} border-b border-border/30`}
              >
