...
export const EditAuthProviderView: React.FC<EditAuthProviderViewProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      console.log(
        "🔍 [EditAuthProviderView] Fetching connection details for:",
        authProviderConnectionId,
      );
      setLoading(true);
      try {
        // Fetch connection details
        const connResponse = await apiClient.get(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.put(
        `/auth-providers/${authProviderConnectionId}`,
        undefined,
        updateData,
      );
                    ...
                    {authProviderDetails.auth_fields.fields.map((field: any) => (
                      <div key={field.name} className="space-y-1">
                        ...
                        <input
                          ...
                          placeholder={
                            field.secret ? "••••••••" : `Enter new ${field.title || field.name}`
                          }
                        />
                        {errors[field.name] && (
