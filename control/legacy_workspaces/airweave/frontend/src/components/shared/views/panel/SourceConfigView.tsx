...
import { getAppIconUrl, getAuthProviderIconUrl } from "@/lib/utils/icons";
...
export const SourceConfigView: React.FC<SourceConfigViewProps> = ({ context }) => {
  ...
  const { authProviderConnections, isLoadingConnections, fetchAuthProviderConnections } =
  ...
  const handleFieldChange =
    (setter: React.Dispatch<React.SetStateAction<any>>) => (key: string, value: string) => {
    ...
    <div className="p-6">
      <div className="space-y-6">
        ...
        {!useExternalAuthProvider &&
          sourceDetails?.auth_fields?.fields?.filter((f: any) => !isTokenField(f.name)).length >
            0 && (
            <div className="space-y-4 p-4 border rounded-md">
              <h3 className="font-semibold">Authentication</h3>
              {sourceDetails.auth_fields.fields
                .filter((f: any) => !isTokenField(f.name))
                .map((field: any) => (
                  <div key={field.name}>
                    ...
                    <input
                      ...
                      className={cn(
                        "w-full p-2 mt-1 rounded border",
              ...
              <div key={field.name}>
                ...
                {field.type === "array" ? (
                  <TagInput
                    ...
                    transformInput={
                      sourceDetails?.short_name === "jira" && field.name === "project_keys"
                        ? (v) => v.toUpperCase()
                        : undefined
                    }
                  />
          ...
          sourceDetails.supported_auth_providers.length > 0 && (
            <div className="space-y-4 p-4 border rounded-md">
              <div className="flex items-center justify-between">
                <div>
                  <label htmlFor="use-external-auth" className="text-sm font-medium">
                    Use external auth provider
                  </label>
                  <p className="text-xs text-muted-foreground">
                    Authenticate via a connected third-party service.
                  </p>
                </div>
                <Switch
                  id="use-external-auth"
                  checked={useExternalAuthProvider}
                  onCheckedChange={setUseExternalAuthProvider}
                />
              </div>
              {useExternalAuthProvider && (
                <div className="pt-4 border-t">
                  {isLoadingConnections ? (
                    ...
                    (() => {
                      ...
                      const filteredConnections = authProviderConnections.filter((conn) =>
                        supportedAuthProviders.includes(conn.short_name),
                      );
                        ...
                        <div className="space-y-2">
                          {filteredConnections.map((conn) => (
                            <div
                              ...
                              className={cn(
                                "h-10 flex items-center gap-2 p-2 rounded-md cursor-pointer border",
                                selectedAuthProviderConnection?.id === conn.id
                                  ? "border-primary"
                                  : "border-transparent hover:bg-accent",
                              )}
                            >
                              <img
                                src={getAuthProviderIconUrl(conn.short_name, resolvedTheme)}
                                alt={conn.name}
                                className="h-6 w-6"
                              />
