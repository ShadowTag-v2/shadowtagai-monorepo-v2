...
export const MinimalConfigureSourceView: React.FC<MinimalConfigureSourceViewProps> = ({
  ...
  console.log("🔧 [MinimalConfigureSourceView] Extracted props:", {
    sourceName,
    sourceShortName,
    sourceId,
  });
  ...
  useEffect(() => {
    ...
    console.log(
      "🔧 [MinimalConfigureSourceView] Source changed, resetting all state:",
      sourceShortName,
    );
  ...
  useEffect(() => {
    ...
    if (
      viewData?.credentialId &&
      viewData?.isAuthenticated &&
      viewData?.sourceShortName === sourceShortName
    ) {
      console.log(
        "🔑 [MinimalConfigureSourceView] Restoring authenticated state with credential:",
        viewData.credentialId,
      );
  ...
  useEffect(() => {
    console.log(
      "🔧 [MinimalConfigureSourceView] Source details effect running for:",
      sourceShortName,
    );
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.auth_fields?.fields) {
            ...
            console.log(
              "🔧 [MinimalConfigureSourceView] Initialized auth values:",
              initialAuthValues,
            );
          ...
          if (data.config_fields?.fields) {
            ...
            console.log(
              "🔧 [MinimalConfigureSourceView] Initialized config values:",
              initialConfigValues,
            );
  ...
  const hasRequiredConfigFields = (): boolean => {
    const result =
      sourceDetails?.config_fields?.fields?.some((field: any) => field.required) || false;
  ...
  const handleAuthenticate = async () => {
    ...
    try {
      ...
      console.log(
        "🔧 [MinimalConfigureSourceView] Calling authenticateSource with state:",
        dialogState,
      );
  ...
  const handleComplete = async () => {
      ...
      sourceDetails.config_fields.fields
        ...
        .forEach((field: any) => {
          if (!configValues[field.name]?.trim()) {
            console.log(
              "🔧 [MinimalConfigureSourceView] Config field validation failed:",
              field.name,
            );
    ...
    try {
      ...
      console.log(
        "🔧 [MinimalConfigureSourceView] Creating source connection:",
      ...
      console.log(
        "🔧 [MinimalConfigureSourceView] Calling onComplete with result:",
        completionResult,
      );
      ...
      if (onError)
        onError(error instanceof Error ? error : new Error(String(error)), "Connection error");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="flex-grow overflow-y-auto">
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-4">
            <DialogTitle className="text-xl font-semibold">Connect to {sourceName}</DialogTitle>
            {sourceShortName && (
              <img
