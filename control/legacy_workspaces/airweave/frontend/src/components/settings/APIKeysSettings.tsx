...
import { CheckCircle2, Copy, Key, Loader2, Plus, RotateCw, Trash2 } from "lucide-react";
...
import { type APIKey, useAPIKeysStore } from "@/lib/stores/apiKeys";
...
export function APIKeysSettings() {
  const { apiKeys, isLoading, error, fetchAPIKeys, createAPIKey, rotateAPIKey, deleteAPIKey } =
    useAPIKeysStore();
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {keyToDelete && (
            <div className="my-4 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-3">
              <code className="text-sm font-mono">{maskKey(keyToDelete.decrypted_key)}</code>
            </div>
          )}
