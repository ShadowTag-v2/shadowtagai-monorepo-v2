...
export const BillingSettings = ({ organizationId }: BillingSettingsProps) => {
  ...
  const handleUpgrade = async (plan: "developer" | "pro" | "team") => {
    try {
      ...
      const isPeriodChangeOnly =
        isCurrentPlan && targetPeriod !== (subscription?.has_yearly_prepay ? "yearly" : "monthly");
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={cn(
          "inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md border text-[10px] font-medium tracking-wider",
          config.className,
        )}
      >
        ...
        <div className="flex items-center justify-between border border-border/50 rounded-lg p-3 bg-muted/40">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="text-muted-foreground">
              Yearly prepay active • Expires{" "}
              {format(new Date(subscription.yearly_prepay_expires_at), "MMMM d, yyyy")}
            </span>
          </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <AlertDescription className="text-blue-800">
                ...
                {subscription.has_yearly_prepay ? (
                  <>
                    Changing to {getPlanDisplayName(subscription.pending_plan_change)} at end of
                    yearly prepay on{" "}
                ...
                >
                  ...
                  {isCurrentPlanAndPeriod ? (
                    <Button
                      variant="outline"
                      disabled
                      className="w-full h-9 text-xs border-border mt-auto"
                    >
                      Current Plan
                    </Button>
                  ) : isPeriodChange ? (
                    ...
                    <Button
                      onClick={() =>
                        window.open("https://cal.com/lennert-airweave/airweave-demo", "_blank")
                      }
                      variant="outline"
      ...
      <div className="border-t border-border pt-6">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>
            ...
            <a href="mailto:hello@airweave.ai" className="text-foreground hover:underline">
              hello@airweave.ai
            </a>
          </span>
        </div>
