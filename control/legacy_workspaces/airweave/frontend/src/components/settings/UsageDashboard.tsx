    ...
    <div className="py-1.5">
      <div className="flex items-center gap-3">
        ...
        <div className="h-4 w-full flex-1 max-w-[560px] rounded bg-muted overflow-hidden border-2 border-border">
          <div
            className={cn("h-full rounded", isAtLimit ? "bg-red-500" : "bg-blue-500")}
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isUnlimited && <span className="text-sm text-muted-foreground">Unlimited</span>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="border border-border/50 rounded-lg p-4">
      ...
      <div className="space-y-1">
        {metricConfig.map((metric) => {
          const currentValue = current_period.usage[
            metric.key as keyof typeof current_period.usage
          ] as number;
          const limitValue = current_period.usage[
            `max_${metric.key}` as keyof typeof current_period.usage
          ] as number | null;
