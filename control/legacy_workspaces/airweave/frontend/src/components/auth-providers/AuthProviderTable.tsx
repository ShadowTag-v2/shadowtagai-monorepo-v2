...
export const AuthProviderTable = () => {
  ...
  const [dialogMode, setDialogMode] = useState<
    "auth-provider" | "auth-provider-detail" | "auth-provider-edit"
  >("auth-provider");
  ...
  useEffect(() => {
    ...
    Promise.all([fetchAuthProviders(), fetchAuthProviderConnections()]).then(
      ([providers, connections]) => {
        console.log(
          `🔄 [AuthProviderTable] Auth providers loaded: ${providers.length} providers, ${connections.length} connections`,
        );
      },
    );
  ...
  const handleAuthProviderClick = (authProvider: any) => {
    console.log("🖱️ [AuthProviderTable] handleAuthProviderClick called:", {
      ...
      hasConnection: !!authProviderConnections.find(
  ...
  const dialogKey = useMemo(() => {
    ...
    const key = dialogOpen
      ? `auth-${selectedAuthProvider?.short_name || "none"}-${remountKey}`
      : "closed";
