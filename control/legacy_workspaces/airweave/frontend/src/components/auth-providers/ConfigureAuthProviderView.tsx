...
const AirweaveFallback: React.FC<{ isDark: boolean }> = ({ isDark }) => (
  ...
  >
    <span className={cn("text-xl font-bold", isDark ? "text-blue-400" : "text-blue-600")}>AW</span>
  </div>
);
...
export const ConfigureAuthProviderView: React.FC<ConfigureAuthProviderViewProps> = ({
  ...
  const { authProviderId, authProviderName, authProviderShortName, authProviderAuthType } =
    viewData;
  ...
  const defaultConnectionName = authProviderName
    ? `My ${authProviderName} Connection`
    : "My Connection";
    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.error(
            "❌ [ConfigureAuthProviderView] Failed to load auth provider details:",
            errorText,
          );
  ...
  const hasEmptyRequiredFields = (): boolean => {
    ...
    if (authProviderDetails?.auth_fields?.fields) {
      return authProviderDetails.auth_fields.fields.some(
        (field: any) => !authFieldValues[field.name] || authFieldValues[field.name].trim() === "",
      );
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (onNext) {
        ...
        console.log(
          "📡 [ConfigureAuthProviderView] Refreshing auth provider connections after navigation",
        );
      ...
      if (error instanceof Error) {
        ...
        if (errorMessage.includes("Failed to create auth provider connection:")) {
          const apiError = errorMessage
            .replace("Failed to create auth provider connection:", "")
            .trim();
          try {
  ...
  return (
    <div className="h-full flex flex-col max-h-[90vh]">
      {/* Fixed header */}
      <div
        className={cn("px-6 pt-6 pb-4 border-b", isDark ? "border-gray-800/50" : "border-gray-100")}
      >
        <div>
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Create a connection to {authProviderName} that can be used to authenticate to data
            sources
          </p>
        </div>
      </div>
      ...
      <div className="px-8 py-10 flex-1 overflow-auto min-h-0">
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            {/* Auth fields */}
            {authProviderDetails?.auth_fields?.fields &&
              authProviderDetails.auth_fields.fields.length > 0 && (
                <>
                  <div className="pt-2">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {(authProviderShortName === "composio" ||
                        authProviderShortName === "pipedream") && (
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p>
                                Opens{" "}
                                {authProviderShortName === "composio"
                                  ? "Composio platform to retrieve your API credentials"
                                  : "Pipedream settings to retrieve your client ID and secret"}
                              </p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
