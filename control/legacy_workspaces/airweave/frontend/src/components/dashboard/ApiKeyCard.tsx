...
export const ApiKeyCard = ({ onRequestNewKey }: ApiKeyCardProps) => {
  ...
  const { apiKeys, isLoading, fetchAPIKeys, createAPIKey } = useAPIKeysStore();
  ...
  const maskApiKey = (key: string) => {
    ...
    const masked = Array(key.length - 4)
      .fill("*")
      .join("");
            ...
            <div className="flex items-center">
              ...
              >
                <div className="relative">
                  <Copy
                    className={`h-3.5 w-3.5 transition-all duration-200 ${copySuccess ? "opacity-0 scale-75" : "opacity-100 scale-100"}`}
                  />
                  <Check
                    className={`h-3.5 w-3.5 absolute inset-0 transition-all duration-200 ${copySuccess ? "opacity-100 scale-100" : "opacity-0 scale-75"}`}
                  />
                </div>
              </Button>
            </div>
