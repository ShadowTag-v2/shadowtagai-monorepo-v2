      ...
      <CardContent>
        <div className="grid gap-4 md:grid-cols-2">
          {mockDestinations.map((destination) => (
            <div key={destination.id} className="flex items-center space-x-4 rounded-lg border p-4">
              <img
              ...
              <div className="flex-1 space-y-1">
                ...
                <p className="text-sm text-muted-foreground">Last sync: {destination.lastSync}</p>
              </div>
            </div>
          ))}
        </div>
