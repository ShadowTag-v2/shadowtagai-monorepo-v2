...
export const SourceButton = ({
  ...
  const getColorClass = (shortName: string) => {
    ...
    const index =
      shortName.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0) % colors.length;
    return colors[index];
  };
      ...
      <img
        ...
        onError={(e) => {
          ...
          e.currentTarget.parentElement!.innerHTML =
            `<span class="text-white font-semibold text-xs sm:text-sm">${shortName.substring(0, 2).toUpperCase()}</span>`;
        }}
      />
    </div>
  );
  ...
  const getTooltipContent = () => {
    ...
    if (
      source_connections &&
      !source_connections.allowed &&
      source_connections.reason === "usage_limit_exceeded"
    ) {
      return (
        <>
