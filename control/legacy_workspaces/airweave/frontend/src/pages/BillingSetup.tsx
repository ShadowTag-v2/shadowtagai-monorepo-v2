import { AlertCircle, Check, ChevronRight, CreditCard, Loader2 } from "lucide-react";
...
export default function BillingSetup() {
  ...
  const handleSetupPayment = async () => {
    ...
    try {
      ...
      if (billingInfo?.plan === "developer") {
        toast.error(
          "Stripe listener error: awaiting activation for Developer plan. Please ensure webhooks are configured.",
        );
        setIsLoading(false);
        return;
      }
  ...
  const needsPaymentMethod =
    billingInfo?.requires_payment_method && !billingInfo?.payment_method_added;
