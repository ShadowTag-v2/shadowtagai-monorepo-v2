import { Check, Eye, EyeOff, Loader2, Monitor, Moon, Sun } from "lucide-react";
...
const SemanticMcp = () => {
  ...
  const logoSrc =
    resolvedTheme === "dark" ? "/logo-airweave-darkbg.svg" : "/logo-airweave-lightbg.svg";
  ...
  useEffect(() => {
    ...
    if (savedConnectedSourcesJson) {
      try {
        ...
        console.log(
          "🔗 [SemanticMcp] Restored connected sources from session:",
          savedConnectedSources,
        );
    ...
    if (savedSourceConnectionsJson) {
      try {
        ...
        console.log(
          "📋 [SemanticMcp] Restored source connections from session:",
  ...
  useEffect(() => {
    if (connectedSources.size > 0) {
      ...
      sessionStorage.setItem(
        "semantic_mcp_connected_sources",
        JSON.stringify(connectedSourcesArray),
      );
    }
  }, [connectedSources]);
  ...
  useEffect(() => {
    ...
    const nonSensitiveFields = Object.keys(authValues).filter(
      (key) => !/(?:token|key|secret|password|credential)/i.test(key),
    );

    console.log(
      "🔐 [SemanticMcp] AuthValues updated - Total fields:",
      Object.keys(authValues).length,
      "Sensitive fields:",
  ...
  useEffect(() => {
    if (detailedSource?.auth_fields?.fields) {
      console.log(
        "📋 [SemanticMcp] All auth fields from API:",
        detailedSource.auth_fields.fields.map((f) => f.name),
      );
      console.log(
        "📋 [SemanticMcp] Filtered auth fields (shown to user):",
        detailedSource.auth_fields.fields
          .filter((field) => field.name !== "refresh_token" && field.name !== "access_token")
          .map((f) => f.name),
      );
      console.log(
        "📋 [SemanticMcp] Hidden auth fields:",
        detailedSource.auth_fields.fields
          .filter((field) => field.name === "refresh_token" || field.name === "access_token")
          .map((f) => f.name),
      );
  ...
  const handleAuthValueChange = (fieldName: string, value: string) => {
    console.log(
      `🔐 [SemanticMcp] Auth field "${fieldName}" changed to:`,
      value ? "[REDACTED]" : "[EMPTY]",
    );
    setAuthValues((prev) => ({
      ...prev,
      [fieldName]: value,
  ...
  const createCredentialNonOAuth = async (
    ...
    try {
      ...
      console.log(
        "🔐 [SemanticMcp] Creating credentials for non-OAuth2 source:",
        sourceShortName,
        "with",
      ...
      if (response.ok) {
        ...
        alert(
          `Failed to authenticate with ${detailedSource.name}: ${typeof errorData === "string" ? errorData : JSON.stringify(errorData)}`,
        );
        return false;
      }
    } catch (error) {
      alert(
        `Error authenticating with ${detailedSource.name}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  };
  ...
  const createCredentialsOAuth = async (
    ...
    try {
      ...
      alert(
        `Error starting OAuth authentication for ${detailedSource.name}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  };
  ...
  const handleAuthenticate = async () => {
    ...
    if (detailedSource.auth_type?.startsWith("oauth2")) {
      ...
      const success = await createCredentialsOAuth(
        authValues,
        configValues,
        selectedSource,
        detailedSource,
    ...
    try {
      ...
      if (!collection) {
        ...
        if (!collectionResponse.ok) {
          ...
          throw new Error(
            `Failed to create collection: ${typeof errorData === "string" ? errorData : JSON.stringify(errorData)}`,
          );
        }
      ...
      if (!connectionResponse.ok) {
        ...
        throw new Error(
          `Failed to create source connection: ${typeof errorData === "string" ? errorData : JSON.stringify(errorData)}`,
        );
      }
      ...
      alert(
        `Failed to connect to ${selectedSource.name}:\n\n${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="rounded-lg h-8 w-8 hover:bg-background-alpha-40 text-muted-foreground transition-all duration-200"
            >
              {resolvedTheme === "dark" ? (
          ...
          <div className="grid grid-cols-8 gap-1 justify-items-center mx-auto">
            {isLoading ? (
              ...
              [...sources]
                .filter(
                  (source) =>
                    source.short_name.toLowerCase() !== "ctti" &&
                    source.short_name.toLowerCase() !== "oracle",
                )
                      ...
                      <img
                        ...
                        onError={(e) => {
                          ...
                          e.currentTarget.parentElement!.innerHTML =
                            `<span class="text-white font-semibold text-sm">${connection.short_name.substring(0, 2).toUpperCase()}</span>`;
                        }}
                      />
                    </div>
                  </div>
      ...
      <div className="flex justify-center px-10 mt-6">
        ...
        >
          ...
          <div className="relative pt-5">
            ...
            {sourceConnections.length === 0 && (
              <div className="absolute inset-0 bg-gray-50/2 dark:bg-gray-900/2 backdrop-blur-[1px] rounded-lg flex items-center justify-center"></div>
            )}
          </div>
        </div>
      </div>
              ...
              {detailedSource.auth_fields?.fields?.length > 0 && (
                <div>
                  <div className="space-y-4">
                    {detailedSource.auth_fields.fields
                      .filter(
                        (field) => field.name !== "refresh_token" && field.name !== "access_token",
                      )
                      .map((field, index) => (
              ...
              <div className="flex justify-center gap-3 pt-4">
                ...
                <Button
                  ...
                  className={`px-6 flex items-center gap-2 ${
                    isAuthenticated ? "bg-green-600 hover:bg-green-700 text-white" : ""
                  }`}
                >
