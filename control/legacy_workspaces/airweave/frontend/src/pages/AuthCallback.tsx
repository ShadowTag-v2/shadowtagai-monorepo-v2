...
async function exchangeCodeForCredentials(
  ...
  const requestData: CredentialRequestData = {
    credential_name: `${
      savedState.sourceDetails?.name || savedState.detailedSource?.name || shortName
    } OAuth Credential`,
    credential_description: `OAuth credential for ${
      savedState.sourceDetails?.name || savedState.detailedSource?.name || shortName
    }`,
  };
...
async function handleOriginalOAuthCallback(
  ...
  try {
    ...
    const parsedStatePayload = parseStateParam(
