...
export const Onboarding = () => {
  ...
  const currentPlanLimit =
    SUBSCRIPTION_PLANS.find((plan) => plan.value === formData.subscriptionPlan)?.teamMemberLimit ||
    2;
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      try {
        ...
        const endpoint =
          billingPeriod === "yearly" && isEligibleForYearly
  ...
  const validateEmail = useCallback(
    (email: string) => {
      ...
      if (teamMembers.length >= currentPlanLimit - 1) {
        setEmailError(
          `You've reached the maximum team size for the ${formData.subscriptionPlan} plan`,
        );
        return;
      }
  ...
  const getStepData = (step: number): Record<string, any> => {
    switch (step) {
      case 1:
        return { org_name: formData.organizationName };
      case 2:
        return { org_size: formData.organizationSize };
      case 3:
        return { user_role: formData.userRole };
      case 4:
        return { org_type: formData.organizationType };
      case 5:
        return { subscription_plan: formData.subscriptionPlan, billing_period: billingPeriod };
      case 6:
        return { team_members_count: teamMembers.length };
      default:
        return {};
    }
  };
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Use letters, numbers, spaces, hyphens, and underscores only • You can always change
                this later
              </p>
            </div>
          </div>
        ...
        return (
          <div className="space-y-8">
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground">This helps us recommend the right plan</p>
            </div>
              ...
              {!authConfig.authEnabled && (
                <div className="mt-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                  <p className="text-xs text-amber-600 dark:text-amber-400">
                    <strong>Local Development Mode:</strong> Billing is disabled. You'll go straight
                    to the dashboard after setup.
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="relative">
                  ...
                  >
                    <p className="text-xs text-muted-foreground text-center whitespace-nowrap">
                      After 12 months, your plan will renew at monthly rates. You can re-enable
                      yearly billing anytime to keep the discount.
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-light">
                        {!authConfig.authEnabled
                          ? "Free"
                          : billingPeriod === "yearly" &&
                              (plan.value === "pro" || plan.value === "team")
                            ? plan.value === "pro"
                              ? "$16"
                              : "$239"
                            : plan.price}
        ...
        return (
          <div className="space-y-8">
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground">Get your team onboard from day one</p>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Your {formData.subscriptionPlan} plan includes up to {currentPlanLimit} team
                  members
                </p>
              </div>
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-2xl relative">
        ...
        <div className="mb-12 text-center">
          ...
          />
          <p className="text-xs text-muted-foreground">Build smarter agents</p>
        </div>
          ...
          {currentStep < totalSteps ? (
            ...
            >
              {isCreating ? (
                <>
                  <span>
                    {!authConfig.authEnabled ? "Creating Organization" : "Complete Setup"}
                  </span>
