...
export class EntityStateMediator {
  ...
  private async fetchDatabaseState(): Promise<SourceConnectionState | undefined> {
    try {
      ...
      const connectionResponse = await apiClient.get(`/source-connections/${this.connectionId}`);
      ...
      const state: SourceConnectionState = {
        ...
        entity_states: connection.entities
          ? // Convert entities object to entity_states array
            Object.entries(connection.entities.by_type || {}).map(
  ...
  private async subscribeToUpdates(jobId: string): Promise<void> {
    ...
    await this.subscribeToEntityState(jobId, this.connectionId, (update: SyncProgressUpdate) => {
          ...
          onMessage: (msg: MessageEvent) => {
            ...
            if (data.type === "entity_state") {
              ...
              if (currentConnection) {
                ...
                const entityStates = Object.entries(data.entity_counts || {}).map(
                  ([entityType, count]) => ({
              ...
              if (currentConnection) {
                ...
                const finalEntityStates = Object.entries(data.final_counts || {}).map(
                  ([entityType, count]) => ({
