...
export const SearchProcess: React.FC<SearchProcessProps> = ({
  requestId,
  events,
  className,
  isSearching = false,
  ...
  const { isLive, isAnswering, hasError } = useMemo(() => {
    ...
    const started = events.some(
      (e: any) => e?.type === "connected" || e?.type === "start" || e?.type === "operator_start",
    );
    const answering = events.some(
      (e: any) => e?.type === "completion_start" || e?.type === "completion_delta",
    );
  ...
  const displayRows = useMemo(() => {
    ...
    for (let i = 0; i < src.length; i++) {
      const event = src[i];
      if (i === 0) {
        console.log("[SearchProcess] Rendering events", {
          count: src.length,
          firstType: (event as any)?.type,
        });
      }
      ...
      if (event.type === "operator_start" && event.op === "qdrant_filter") {
        ...
        while (
          i < src.length &&
          !(src[i].type === "operator_end" && src[i].op === "qdrant_filter")
        ) {
          i++;
        }
      ...
      if (event.type === "operator_start" && event.op === "vector_search") {
        ...
        while (
          i < src.length &&
          !(src[i].type === "operator_end" && src[i].op === "vector_search")
        ) {
          i++;
        }
        continue;
      ...
      if (inReranking) {
        ...
        if (event.type === "reranking_reason_delta") {
          ...
          if (typeof text === "string" && text.trim()) {
            ...
            rows.push(
              <div
                key={`rerank-${i}-reason-${rerankingData.reasons.length}`}
                className="py-0.5 px-2 text-[11px] opacity-80"
              >
                {text.trim()}
              </div>,
            );
          ...
          if (rerankingData.rankings.length > 0) {
            rows.push(
              <div key={`${key}-rankings-header`} className="py-0.5 px-2 text-[11px] opacity-90">
                Reranked {rerankingData.rankings.length} result
      ...
      if (
        event.type === "completion_start" ||
        event.type === "completion_delta" ||
        event.type === "completion_done"
      ) {
        continue;
      }
  ...
  const statusRibbon = (
    <div className="h-1.5 w-full relative overflow-hidden">
      {isLive ? (
        ...
        <div className={cn("absolute inset-0 h-1.5", isDark ? "bg-gray-800" : "bg-gray-200")}></div>
      )}
    </div>
  );
