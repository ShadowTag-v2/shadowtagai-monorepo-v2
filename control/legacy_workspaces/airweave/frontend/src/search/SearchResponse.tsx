...
export const SearchResponse: React.FC<SearchResponseProps> = ({
  ...
  const { entitySourceMap, resultNumberMap } = useMemo(() => {
    ...
    results.forEach((result: any, idx: number) => {
      ...
      const rawSourceName =
        payload.airweave_system_metadata?.source_name || payload.source_name || "Unknown";
  ...
  const scrollToEntity = useCallback(
    (entityIndex: number, entityId: string) => {
      ...
      if (targetCard) {
        ...
        targetCard.style.backgroundColor = isDark
          ? "rgba(59, 130, 246, 0.2)"
          : "rgba(59, 130, 246, 0.15)";
        targetCard.style.border = isDark
    ...
    (entityId: string) => {
      ...
      setTimeout(() => {
        ...
        const entityIndex = results.findIndex((result: any) => {
          const payload = result.payload || result;
          const found =
            payload.entity_id === entityId || payload.id === entityId || payload._id === entityId;
  ...
  const JsonBlock: React.FC<{ value: string; isDark: boolean }> = ({ value, isDark }) => {
    ...
    return (
      <div className="relative">
        ...
        >
          {copiedLocal ? (
            <Check className={DESIGN_SYSTEM.icons.inline} />
          ) : (
  ...
  const traceRows = useMemo(() => {
    ...
    for (let i = 0; i < src.length; i++) {
      ...
      if (event.type === "operation_skipped") {
        ...
        const skipMessages: Record<string, string> = {
          no_sources_support_temporal_relevance:
            "Sources in this collection do not support recency bias",
      ...
      if (event.type === "operator_start" && event.op === "qdrant_filter") {
        ...
        rows.push(
          <div
            key={`qdrant-${i}-start`}
            className="px-2 py-1 text-[11px] flex items-center gap-1.5"
          >
        ...
        const hasExisting = !!(
          mergeDetails &&
          mergeDetails.existing &&
          typeof mergeDetails.existing === "object" &&
          Object.keys(mergeDetails.existing).length > 0
        );
        if (hasExisting) {
          rows.push(
        ...
        while (
          i < src.length &&
          !((src[i] as any).type === "operator_end" && (src[i] as any).op === "qdrant_filter")
        ) {
          i++;
        }
      ...
      if (inInterpretation) {
        ...
        if (event.type === "operator_end" && event.op === "query_interpretation") {
          ...
          if (
            interpretationData.filterApplied &&
            typeof interpretationData.filterApplied === "object"
          ) {
          ...
          if (expansionData.alternatives.length > 0) {
            rows.push(
              <div key={`${key}-alts-header`} className="py-0.5 px-2 text-[11px] opacity-90">
                Generated {expansionData.alternatives.length} alternative
                {expansionData.alternatives.length !== 1 ? "s" : ""}:
              </div>,
            );
      ...
      if (event.type === "operator_start" && event.op === "federated_search") {
        ...
        rows.push(
          <div key={`${key}-start`} className="px-2 py-1 text-[11px] flex items-center gap-1.5">
            ...
            >
              {federatedSearchData.sourceNames.join(", ")}
            </span>
            {federatedSearchData.numQueries > 1 && (
              ...
              >
                {federatedSearchData.numQueries} queries
              </span>
            )}
          </div>,
        );
        ...
        if (federatedSearchData.keywords.length > 0) {
          rows.push(
            <div key={`${key}-keywords-header`} className="py-0.5 px-2 text-[11px] opacity-90">
              Searching with {federatedSearchData.numKeywords} keyword
              {federatedSearchData.numKeywords !== 1 ? "s" : ""}:
            </div>,
          );
          federatedSearchData.keywords.forEach((keyword, idx) => {
            rows.push(
              <div
                key={`${key}-keyword-${idx}`}
                className="py-0.5 px-2 pl-4 text-[11px] opacity-80"
              >
                • {keyword}
              </div>,
            );
        ...
        if (federatedSearchData.noResults) {
          ...
          rows.push(
            <div key={`${key}-merged`} className="py-0.5 px-2 text-[11px] opacity-80">
              Merged {federatedSearchData.federatedCount} federated +{" "}
              {federatedSearchData.vectorCount} vector = {federatedSearchData.mergedCount} results
            </div>,
          );
        }
        ...
        while (
          i < src.length &&
          !((src[i] as any).type === "operator_end" && (src[i] as any).op === "federated_search")
        ) {
          i++;
        }
        continue;
      ...
      if (event.type === "operator_start" && event.op === "vector_search") {
        ...
        const VIcon =
          vMethod === "hybrid" ? FiGitMerge : vMethod === "neural" ? ChartScatter : FiType;
          ...
          if (pendingEmbedding.neuralCount && pendingEmbedding.neuralCount > 0) {
            rows.push(
              <div key={`${key}-embed-neural`} className="py-0.5 px-2 text-[11px] opacity-80">
                Embeddings: {pendingEmbedding.neuralCount} neural
                {pendingEmbedding.neuralCount !== 1 ? "s" : ""} (dim{" "}
                {pendingEmbedding.dim || "unknown"})
              </div>,
            );
          }
        ...
        if (vectorSearchData.noResults) {
          ...
          rows.push(
            <div key={`${key}-found`} className="py-0.5 px-2 text-[11px] opacity-80">
              Retrieved {vectorSearchData.finalCount} candidate result
              {vectorSearchData.finalCount !== 1 ? "s" : ""}
            </div>,
          );
        }
        ...
        while (
          i < src.length &&
          !((src[i] as any).type === "operator_end" && (src[i] as any).op === "vector_search")
        ) {
          i++;
        }
        continue;
      ...
      if (inRecency) {
        ...
        if (event.type === "recency_skipped") {
          ...
          while (
            i < src.length &&
            !((src[i] as any).type === "operator_end" && (src[i] as any).op === "recency")
          ) {
            i++;
          }
          inRecency = false;
          ...
          if (rerankingData.rankings.length > 0) {
            rows.push(
              <div key={`${key}-rankings-header`} className="py-0.5 px-2 text-[11px] opacity-90">
                Top {rerankingData.rankings.length} result
                {rerankingData.rankings.length !== 1 ? "s" : ""}:
              </div>,
            );
            const topRankings = rerankingData.rankings.slice(0, 5);
            topRankings.forEach((ranking, idx) => {
              const score =
                typeof ranking.relevance_score === "number"
      ...
      if (event.type === "answer_context_budget") {
        ...
        rows.push(
          <div key={`completion-budget-${i}`} className="py-0.5 px-2 text-[11px] opacity-80">
            Using {e.results_in_context} of {e.total_results} result
            {e.total_results !== 1 ? "s" : ""} in context
            {e.excluded > 0 && (
      ...
      if (event.type === "connected") {
        ...
        rows.push(
          <div
            key={(event.seq ?? i) + "-" + i}
            className="py-0.5 px-2 text-[11px] opacity-90 flex items-center gap-1.5"
          >
    ...
    if (
      inExpansion &&
      (expansionData.reasons.length > 0 ||
        expansionData.strategy ||
        expansionData.alternatives.length > 0)
    ) {
      const key = `exp-incomplete`;
      rows.push(
        <div key={`${key}-start`} className="py-0.5 px-2 text-[11px] opacity-90">
          • Starting query expansion
          {expansionData.strategy ? ` with strategy '${expansionData.strategy}'` : ""}
        </div>,
      );
      expansionData.reasons.forEach((reason, idx) => {
        rows.push(
  ...
  const statusRibbon = (
    <div className="h-1.5 w-full relative overflow-hidden">
      {isSearching ? (
        <>
          <div
            className={cn("absolute inset-0 h-1.5 bg-gradient-to-r from-blue-500 to-indigo-500")}
          ></div>
          <div
            className={cn(
                    ...
                    <TooltipTrigger asChild>
                      <button
                        ...
                        className={cn(
                          ...
                          isDark ? "text-gray-600 bg-gray-900/30" : "text-gray-400 bg-gray-50/50",
                        )}
                      >
                    ...
                    >
                      <div className="flex items-start gap-1.5">
                        ...
                        <p className="text-xs text-white/90 leading-relaxed">
                          To get an answer to your question, turn on "Generate answer" when
                          searching.
                        </p>
                      </div>
                    </TooltipContent>
          ...
          >
                ...
                {!events || events.length === 0 ? (
                  <div className="px-1 py-1">
                    <div className="space-y-2 animate-pulse">
                      <div
                        className={cn("h-3 rounded", isDark ? "bg-gray-800" : "bg-gray-200")}
                      ></div>
                      <div
                        className={cn("h-3 rounded", isDark ? "bg-gray-800" : "bg-gray-200")}
                      ></div>
                      <div
                        className={cn("h-3 rounded w-5/6", isDark ? "bg-gray-800" : "bg-gray-200")}
                      ></div>
                      <div className="py-1">
                      ...
                      ></div>
                      <div
                        className={cn("h-3 rounded w-2/3", isDark ? "bg-gray-800" : "bg-gray-200")}
                      ></div>
                      <div
                        className={cn("h-3 rounded w-1/2", isDark ? "bg-gray-800" : "bg-gray-200")}
                      ></div>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    {events.some((e: any) => e?.type === "cancelled") && (
                      <div className="py-0.5 px-2 text-[11px] text-red-500">Search cancelled</div>
                    )}
                    {isSearching && (
                  ...
                  >
                    {isSearching ? (
                      ...
                      <ReactMarkdown
                        ...
                        components={{
                          h1: ({ node, ...props }) => (
                            <h1 className="text-[13px] font-semibold mt-0 mb-1.5" {...props} />
                          ),
                          h2: ({ node, ...props }) => (
                            <h2 className="text-[12px] font-semibold mt-0 mb-1.5" {...props} />
                          ),
                          h3: ({ node, ...props }) => (
                            <h3 className="text-[12px] font-medium mt-0 mb-1.5" {...props} />
                          ),
                          ul: ({ node, ...props }) => (
                            <ul className="list-disc pl-4 mt-0 mb-1 space-y-0.5" {...props} />
                          ),
                          ol: ({ node, ...props }) => (
                            <ol className="list-decimal pl-4 mt-0 mb-1 space-y-0.5" {...props} />
                          ),
                          table: ({ node, ...props }) => (
                          ...
                          thead: ({ node, ...props }) => (
                            <thead
                              className={cn(isDark ? "bg-gray-800/50" : "bg-gray-50")}
                              {...props}
                            />
                          ),
                          tbody: ({ node, ...props }) => (
                            <tbody
                              className={cn(
                                "divide-y",
                                isDark
                                  ? "divide-gray-800 bg-gray-900/30"
                          ...
                          p: ({ children, ...props }) => {
                            ...
                            return (
                              <p className="mt-0 mb-1 leading-relaxed" {...props}>
                                {processedChildren}
                              </p>
                            );
                          },
                          ...
                          hr: ({ node, ...props }) => (
                            <hr
                              className={cn("my-2", isDark ? "border-gray-700" : "border-gray-300")}
                              {...props}
                            />
                          ),
                          strong: ({ node, ...props }) => (
            ...
            {activeTab === "entities" && (results.length > 0 || isSearching) && (
              ...
              >
                {isSearching ? (
                  <div
                    className={cn(DESIGN_SYSTEM.spacing.padding.default, "animate-pulse space-y-2")}
                  >
                        ...
                        <Button
                          onClick={() =>
                            setVisibleResultsCount((prev) =>
                              Math.min(prev + LOAD_MORE_INCREMENT, results.length),
                            )
                          }
                          variant="outline"
                          size="sm"
            ...
            {activeTab === "raw" &&
              (() => {
                ...
                const displayString =
                  showFullRawJson || !shouldTruncate
                    ? fullJsonString
                    ...
                    {shouldTruncate && !showFullRawJson && (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className={cn("opacity-50 font-mono text-[10px]")}>
