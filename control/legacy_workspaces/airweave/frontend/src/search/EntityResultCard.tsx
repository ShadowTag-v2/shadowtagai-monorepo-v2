...
const EntityResultCardComponent: React.FC<EntityResultCardProps> = ({
  ...
  const sourceName =
    payload.airweave_system_metadata?.source_name || payload.source_name || "Unknown Source";
  ...
  const metadata = useMemo(() => {
    ...
    const excludeKeys = [
      ...
      "source_name",
      "created_at",
      "updated_at", // Displayed in timestamp badge
      "vector",
      "vectors",
    ];
  ...
  return (
    ...
    >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    {/* Divider */}
                    <div
                      className={cn("w-px h-3 opacity-30", isDark ? "bg-gray-400" : "bg-gray-600")}
                    />
                ...
                >
                  <div className="space-y-2">
                    ...
                    {/* Similarity score */}
                    <div className={cn("text-[12px]", isDark ? "text-gray-400" : "text-gray-600")}>
                      <span className="font-medium">Similarity:</span>{" "}
                      <span className="font-semibold">
                        {isNormalizedScore ? `${(score * 100).toFixed(1)}%` : score.toFixed(3)}
                      </span>
                    </div>

                    {/* Divider */}
                    <div className={cn("h-px w-full", isDark ? "bg-gray-700/50" : "bg-gray-200")} />
      ...
      {formattedContent && (
        <div className={cn("border-t", isDark ? "border-gray-800/50" : "border-gray-100")}>
          ...
          >
            ...
            <div className="w-3.5 h-3.5 rounded-sm overflow-hidden flex items-center justify-center flex-shrink-0 opacity-60">
              <img src={airweaveLogo} alt="Airweave" className="w-full h-full object-contain" />
            </div>
            Preview
          </button>

          {isPreviewExpanded && (
            <div className={cn("px-4 pb-3", isDark ? "text-gray-200" : "text-gray-800")}>
                ...
                <div className="relative">
                  ...
                  >
                    <ReactMarkdown
                      ...
                      components={{
                        h1: ({ node, ...props }) => (
                          <h1 className="text-base font-bold mt-4 mb-2 first:mt-0" {...props} />
                        ),
                        h2: ({ node, ...props }) => (
                          <h2 className="text-sm font-bold mt-3 mb-2 first:mt-0" {...props} />
                        ),
                        h3: ({ node, ...props }) => (
                          <h3 className="text-sm font-semibold mt-3 mb-1.5 first:mt-0" {...props} />
                        ),
                        p: ({ node, ...props }) => (
                          <p className="text-[13px] leading-relaxed mb-2" {...props} />
                        ),
                        ul: ({ node, ...props }) => (
      ...
      {Object.keys(importantMetadata).length > 0 && (
        <div className={cn("border-t", isDark ? "border-gray-800/50" : "border-gray-100")}>
          ...
          {isPropertiesExpanded && (
            <div className={cn("px-4 pb-2.5", isDark ? "bg-gray-900/20" : "bg-gray-50/50")}>
                ...
                <>
                  ...
                  >
                    {isMetadataExpanded ? (
                      ...
                      <>
                        <ChevronRight className="h-3.5 w-3.5" />
                        {Object.keys(remainingMetadata).length} More Field
                        {Object.keys(remainingMetadata).length !== 1 ? "s" : ""}
                      </>
                    )}
                  </button>
                  ...
                  {isMetadataExpanded && (
                    ...
                    >
                      {Object.entries(remainingMetadata).map(([key, value]) => {
                        const { displayValue, needsTruncation, isDate } = formatFieldValue(
                          value,
                          key,
                        );
                        const isExpanded = expandedFields.has(key);
          ...
          <div className="px-4 pb-3">
            ...
            <div
              className={cn(
                ...
                isDark ? "bg-gray-900/40 border-gray-800/60" : "bg-gray-50/80 border-gray-200/60",
              )}
            >
              <button
