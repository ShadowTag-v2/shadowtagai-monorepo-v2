  ...
  const cleanupErrorMessage = (errors: any[] | null | undefined): string => {
    ...
    for (const error of errors) {
      ...
      if (error.keyword === "required" && error.params?.missingProperty) {
        errorMessages.add(
          `Missing required field: "${error.params.missingProperty}"${path ? ` at ${path}` : ""}`,
        );
      } else if (error.keyword === "additionalProperties" && error.params?.additionalProperty) {
        errorMessages.add(
          `Unknown field: "${error.params.additionalProperty}"${path ? ` at ${path}` : ""}`,
        );
      } else if (error.keyword === "type") {
        errorMessages.add(`${path || "Value"} must be ${error.params?.type}`);
      } else if (error.keyword === "enum") {
        errorMessages.add(
          `${path || "Value"} must be one of: ${error.params?.allowedValues?.join(", ")}`,
        );
      ...
      <div className="relative">
        <textarea
          ...
          className={cn(
            ...
            validationError ? "border-red-500 focus:border-red-500" : "focus:border-blue-500",
