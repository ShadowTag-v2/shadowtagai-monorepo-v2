...
export const useSyncStateStore = create<SyncStateStore>((set, get) => ({
      ...
      void fetchEventSource(sseUrl, {
        ...
        onopen: async (response) => {
          ...
          if (status === 401 || status === 403) {
            ...
            throw new Error(
              `SSE connection failed with status ${status}: ${response.statusText} - ${errorText}`,
            );
          }
        },
            ...
            if (data.is_complete || data.is_failed) {
              ...
              setTimeout(() => {
                ...
                if (sub) {
                  ...
                  console.log(
                    `📊 Subscription status changed for ${sourceConnectionId}: ${previousStatus} -> ${sub.status}`,
                  );
  ...
  restoreProgressFromStorage: (sourceConnectionId: string, jobId: string) => {
    ...
    if (storedData && storedData.jobId === jobId && storedData.status === "active") {
      console.log(
        `🔄 Restoring saved progress for ${sourceConnectionId} from session storage:`,
        storedData,
      );
