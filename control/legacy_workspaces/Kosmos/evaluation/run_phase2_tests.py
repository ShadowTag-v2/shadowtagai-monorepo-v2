...
def test_hypothesis_generation(research_question=None, domain=None):
  ...
  agent = HypothesisGeneratorAgent(
    config={"use_literature_context": False, "num_hypotheses": 3}
  )
  ...
  hypotheses = (
    getattr(response, "hypotheses", response)
  ...
  for h in hypotheses:
    ...
    print(
      f"    novelty={h_data['novelty_score']}, testability={h_data['testability_score']}"
    )
...
def test_code_execution(domain=None, data_path=None):
  ...
  try:
    ...
    result["execution"] = {
      ...
      "error": str(getattr(exec_result, "error", ""))[:500]
...
def test_convergence_detection(research_question=None, domain=None):
  ...
  print(
    f"  Max iterations test: stop={decision1.should_stop}, reason={decision1.reason}"
  )

  # Test 2: Declining novelty
  detector2 = ConvergenceDetector(config={"max_iterations": 20})
  plan2 = ResearchPlan(
    research_question=rq, domain=dom, max_iterations=20, iteration_count=6
  )
  ...
  decision2 = detector2.check_convergence(
    research_plan=plan2, hypotheses=hypotheses, results=[]
  )
  ...
  print(
    f"  Novelty decline test: stop={decision2.should_stop}, reason={decision2.reason}"
  )
...
if __name__ == "__main__":
  ...
  output_path = (
    args.output_dir or "/mnt/c/python/Kosmos/evaluation/artifacts/phase2_components"
  )
