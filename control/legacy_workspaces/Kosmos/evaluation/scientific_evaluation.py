...
async def run_phase2_smoke_test(
  research_question: str = None, domain: str = None, data_path: Path = None
) -> PhaseResult:
  ...
  try:
    ...
    director = ResearchDirectorAgent(
      ...
      or "How does temperature affect enzyme catalytic rates?",
    ...
    for i in range(max_actions):
      ...
      logger.info(
        f"  Step {i}: action={next_action.value}, state={director.workflow.current_state.value}"
      )
    ...
    advanced_past_init = (
      status.get("workflow_state") != WorkflowState.INITIALIZING.value
    )
    result.add_check(
      "workflow_advanced",
      advanced_past_init,
...
async def run_phase3_multi_iteration(
  ...
  result = PhaseResult(
    phase=3, name="Multi-Iteration Full Loop (3 iter)", status="PASS"
  )
  start = time.time()
  _reset_eval_state()
  ...
  try:
    ...
    logger.info(
      f"Phase 3: Creating ResearchDirector for {max_iterations or 3} iterations..."
    )
    director = ResearchDirectorAgent(
      research_question=research_question
      or "What is the relationship between substrate concentration and enzyme reaction velocity?",
      domain=domain or "biology",
      config=flat_config,
    )
...
async def run_phase4_dataset_test(
  research_question: str = None, domain: str = None, data_path: Path = None
) -> PhaseResult:
  ...
  if data_path is None:
    result.add_check(
      "dataset_exists", False, "No data_path provided and no default dataset"
    )
    result.status = "FAIL"
  ...
  try:
    ...
    director = ResearchDirectorAgent(
      research_question=research_question
      or "Analyze the relationship between temperature and enzyme activity",
      domain=domain or "biology",
      config=flat_config,
    )
    result.add_check(
...
def assess_output_quality(
  phase2_result: PhaseResult, phase3_result: PhaseResult
) -> PhaseResult:
  ...
  for phase_result, label in [(phase2_result, "phase2"), (phase3_result, "phase3")]:
    ...
    if hyp_count > 0:
      ...
      specificity = (
        1 if any(w in plan_lower for w in ["specific", "measurable", "quantif"]) else 0
      )
      mechanism = (
        1
        if any(w in plan_lower for w in ["mechanism", "pathway", "process", "cause"])
        else 0
      )
      testable = (
        1
        if any(w in plan_lower for w in ["test", "experiment", "measur", "observ"])
        else 0
      )
      novel = (
        1
        if any(w in plan_lower for w in ["novel", "new", "unexplor", "innovat"])
        else 0
      )

      # Score 1-10 based on indicators
      hyp_score = max(
        1, min(10, 3 + specificity * 2 + mechanism * 2 + testable * 2 + novel * 1)
      )
    ...
    design_seen = (
      "design_experiment" in phases_seen or "DESIGN_EXPERIMENT" in phases_seen
    )
    ...
    # Assess execution
    exec_seen = (
      "execute_experiment" in phases_seen or "EXECUTE_EXPERIMENT" in phases_seen
    )
...
def run_phase6_rigor_scorecard() -> PhaseResult:
  ...
  try:
    ...
    has_methods = all(
      hasattr(analyzer, m) for m in ["ttest_sample_size", "correlation_sample_size"]
    )
  ...
  try:
    ...
    metrics = get_metrics()
    has_budget = hasattr(metrics, "enforce_budget") or hasattr(
...
def run_phase7_paper_compliance(
  ...
  try:
    ...
    claims.append(
      claim(4, "World Model as central hub", "PARTIAL", f"World model init: {e}")
    )
  ...
  try:
    ...
    claims.append(
      claim(6, "~36 literature rollouts", "PARTIAL", "Agent importable but untested")
    )
  ...
  try:
    ...
    claims.append(
      claim(12, "Parallel agent instances", "PARTIAL", "Import not available")
    )
  ...
  try:
    ...
    claims.append(
      claim(15, "Reports with citations", "PARTIAL", "Summarizer not importable")
    )
...
def generate_report(report: EvaluationReport) -> str:
  ...
  lines.append("**Evaluator**: automated (scientific_evaluation.py)")
  lines.append("")
  ...
  lines.append(
    f"- **Checks passed**: {passed_checks}/{total_checks} ({100 * passed_checks // max(total_checks, 1)}%)"
  )
  lines.append(
    f"- **Total duration**: {sum(p.duration_seconds for p in report.phases):.1f}s"
  )
  lines.append("")
  ...
  for phase in report.phases:
    ...
    if phase.phase == 5 and "quality_scores" in phase.details:
      ...
      lines.append("")
      lines.append(
        f"**Average Quality Score**: {phase.details.get('average_score', 'N/A')}/10"
      )
      lines.append("")
    ...
    if phase.phase == 6 and "rigor_scores" in phase.details:
      ...
      lines.append("")
      lines.append(
        f"**Average Rigor Score**: {phase.details.get('average_rigor_score', 'N/A')}/10"
      )
      lines.append("")
...
async def main(
  ...
  p2 = await run_phase2_smoke_test(
    research_question=research_question, domain=domain, data_path=data_path
  )
  report.add_phase(p2)
  print(
    f"  -> {p2.status} ({p2.checks_passed}/{p2.checks_total} checks, {p2.duration_seconds:.1f}s)"
  )
  print()
  ...
  report.add_phase(p3)
  print(
    f"  -> {p3.status} ({p3.checks_passed}/{p3.checks_total} checks, {p3.duration_seconds:.1f}s)"
  )
  print()
  ...
  report.add_phase(p4)
  print(
    f"  -> {p4.status} ({p4.checks_passed}/{p4.checks_total} checks, {p4.duration_seconds:.1f}s)"
  )
  print()
