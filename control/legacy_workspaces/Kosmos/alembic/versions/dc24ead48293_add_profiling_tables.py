...
def upgrade() -> None:
  ...
  op.create_table(
    ...
    sa.Column(
      "profiling_mode", sa.String(length=20), nullable=False
    ),  # light, standard, full
    # Timing metrics
  ...
  op.create_table(
    ...
    sa.ForeignKeyConstraint(
      ["profile_id"], ["execution_profiles.id"], ondelete="CASCADE"
    ),
  )
...
def downgrade() -> None:
  ...
  # Drop indexes first
  op.drop_index(
    "idx_profiling_bottlenecks_function", table_name="profiling_bottlenecks"
  )
  op.drop_index(
    "idx_profiling_bottlenecks_time_percent", table_name="profiling_bottlenecks"
  )
  op.drop_index(
    "idx_profiling_bottlenecks_severity", table_name="profiling_bottlenecks"
  )
  op.drop_index(
    "idx_profiling_bottlenecks_profile_id", table_name="profiling_bottlenecks"
  )
  ...
  op.drop_index(
    "idx_execution_profiles_execution_time", table_name="execution_profiles"
  )
