...
def header_footer(canvas, doc):
  ...
  if doc.page > 1:
    ...
    canvas.line(
      inch, letter[1] - 0.55 * inch, letter[0] - inch, letter[1] - 0.55 * inch
    )

    # Footer
    canvas.drawString(
      inch, 0.5 * inch, f"Generated: {datetime.now().strftime('%B %d, %Y')}"
    )
...
def create_report(filename, report_data):
  ...
  frame = Frame(
    doc.leftMargin, doc.bottomMargin, doc.width, doc.height - 0.5 * inch, id="normal"
  )
  ...
  toc.levelStyles = [
    ParagraphStyle(
      name="TOCHeading1", fontSize=14, leftIndent=20, spaceBefore=10, spaceAfter=5
    ),
    ParagraphStyle(
      name="TOCHeading2", fontSize=12, leftIndent=40, spaceBefore=3, spaceAfter=3
    ),
  ]
  ...
  for section in report_data.get("sections", []):
    ...
    story.append(
      Paragraph(f'<a name="{section_title}"/>{section_title}', heading1_style)
    )
...
def create_section_chart(chart_data):
  ...
  if chart_type == "bar":
    ...
    for i in range(len(chart_data["data"])):
      chart.bars[i].fillColor = colors.HexColor(
        ["#3498db", "#e74c3c", "#2ecc71"][i % 3]
      )
    ...
    # Style lines
    for i in range(len(chart_data["data"])):
      chart.lines[i].strokeColor = colors.HexColor(
        ["#3498db", "#e74c3c", "#2ecc71"][i % 3]
      )
