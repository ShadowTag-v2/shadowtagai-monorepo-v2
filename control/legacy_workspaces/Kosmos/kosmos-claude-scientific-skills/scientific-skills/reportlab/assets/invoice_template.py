...
def create_invoice(
  ...
  if logo_path:
    ...
    header_data = [
      [
        logo,
        Paragraph(company_text, header_style),
        Paragraph(invoice_text, header_style),
      ]
    ]
    header_table = Table(header_data, colWidths=[1.5 * inch, 3 * inch, 2.5 * inch])
  else:
    header_data = [
      [Paragraph(company_text, header_style), Paragraph(invoice_text, header_style)]
    ]
  ...
  for item in items:
    ...
    items_data.append([desc, str(qty), f"${price:,.2f}", f"${amount:,.2f}"])

  # Create items table
  items_table = Table(
    items_data, colWidths=[3.5 * inch, 1 * inch, 1.5 * inch, 1 * inch]
  )
  ...
  terms_style = ParagraphStyle(
    "Terms", parent=styles["Normal"], fontSize=9, textColor=colors.grey
  )
