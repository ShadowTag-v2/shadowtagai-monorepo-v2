...
PRETRAINED_MODELS = {
    "chemberta": {"name": "ChemBERTa", "description": "BERT pretrained on 77M molecules from ZINC15", "model_id": "seyonec/ChemBERTa-zinc-base-v1"},
    "grover": {
        "name": "GROVER",
        "description": "Graph transformer pretrained on 10M molecules",
        "model_id": None,  # GROVER uses its own loading mechanism
    },
    "molformer": {"name": "MolFormer", "description": "Transformer pretrained on molecular structures", "model_id": "ibm/MoLFormer-XL-both-10pct"},
}
...
def main():
    ...
    parser.add_argument("--model", type=str, choices=list(PRETRAINED_MODELS.keys()), required=True, help="Pretrained model to use")
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument("--target", nargs="+", default=["target"], help="Target column name(s) for custom data")
    parser.add_argument("--smiles-col", type=str, default="smiles", help="SMILES column name for custom data")
    parser.add_argument("--task-type", type=str, choices=["classification", "regression"], default="classification", help="Type of prediction task")
    ...
    try:
        ...
        if args.model == "chemberta":
            model, results = train_chemberta(train, valid, test, task_type=task_type, n_tasks=n_tasks, n_epochs=args.epochs)
        elif args.model == "grover":
            model, results = train_grover(train, test, task_type=task_type, n_tasks=n_tasks, n_epochs=args.epochs)
        else:
