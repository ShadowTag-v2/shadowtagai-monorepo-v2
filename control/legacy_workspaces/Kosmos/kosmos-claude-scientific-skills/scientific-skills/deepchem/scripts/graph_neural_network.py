...
def create_model(model_type, n_tasks, mode="classification"):
    ...
    """
    if model_type == "gcn":
        return dc.models.GCNModel(n_tasks=n_tasks, mode=mode, batch_size=128, learning_rate=0.001, dropout=0.0)
    elif model_type == "gat":
        return dc.models.GATModel(n_tasks=n_tasks, mode=mode, batch_size=128, learning_rate=0.001)
    elif model_type == "attentivefp":
        return dc.models.AttentiveFPModel(n_tasks=n_tasks, mode=mode, batch_size=128, learning_rate=0.001)
...
def main():
    parser = argparse.ArgumentParser(description="Train graph neural networks for molecular property prediction")
    parser.add_argument("--model", type=str, choices=list(AVAILABLE_MODELS.keys()), default="gcn", help="Type of graph neural network model")
    parser.add_argument("--dataset", type=str, choices=list(MOLNET_DATASETS.keys()), default=None, help="MoleculeNet dataset to use")
    parser.add_argument("--data", type=str, default=None, help="Path to custom CSV file")
    parser.add_argument(
        "--task-type", type=str, choices=["classification", "regression"], default="classification", help="Type of prediction task (for custom data)"
    )
    parser.add_argument("--targets", nargs="+", default=["target"], help="Names of target columns (for custom data)")
    ...
    if args.dataset is None and args.data is None:
        print("Error: Must specify either --dataset (MoleculeNet) or --data (custom CSV)", file=sys.stderr)
        return 1
    ...
    try:
        if args.dataset:
            ...
            model, test_set = train_on_custom_data(
                args.data, args.model, args.task_type, args.targets, smiles_col=args.smiles_col, n_epochs=args.epochs
            )
