...
def drug_discovery_workflow(disease_efo_id: str, max_targets: int = 3):
  ...
  for idx, target in enumerate(top_targets, 1):
    print(
      f"    {idx}. {target.get('target_name', 'Unknown')} ({target.get('uniprot_id', 'N/A')})"
    )
  ...
  for seq_data in sequences:
    try:
      ...
      print(
        f"  ✓ Predicted structure for {seq_data['target'].get('target_name', 'Unknown')}"
      )
    except Exception as e:
  ...
  for struct_data in structures:
    try:
      ...
      print(
        f"  ✓ Found {len(sites)} binding sites for {struct_data['target'].get('target_name', 'Unknown')}"
      )
    except Exception as e:
        ...
        compounds = tu.run(
          {
            ...
            "arguments": {"binding_site": site, "library": "lead-like", "top_n": 10},
          }
        )
        all_hits.extend(compounds)
        print(
          f"  ✓ Found {len(compounds)} hit compounds for {site_data['target'].get('target_name', 'Unknown')}"
        )
      except Exception as e:
...
def genomics_workflow(geo_id: str):
  ...
  try:
    expression_data = tu.run(
      {"name": "GEO_download_dataset", "arguments": {"geo_id": geo_id}}
    )
  ...
  try:
    ...
    print(
      f"  ✓ Found {len(de_genes.get('significant_genes', []))} differentially expressed genes"
    )
  except Exception as e:
    print(f"  ✗ Failed: {e}")
    return
    ...
    pathways = tu.run(
      {
        ...
        "arguments": {"gene_list": de_genes["significant_genes"], "organism": "hsa"},
      }
    )
