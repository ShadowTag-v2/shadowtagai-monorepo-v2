...
def check_normality(data: np.ndarray | pd.Series | list, name: str = "data", alpha: float = 0.05, plot: bool = True) -> dict:
    ...
    interpretation = f"Data {'appear' if is_normal else 'do not appear'} normally distributed (W = {statistic:.3f}, p = {p_value:.3f})"
    ...
    return {
        ...
        "n": len(data_clean),
        "recommendation": ("Proceed with parametric test" if is_normal else "Consider non-parametric alternative or transformation"),
    }


def check_normality_per_group(data: pd.DataFrame, value_col: str, group_col: str, alpha: float = 0.05, plot: bool = True) -> pd.DataFrame:
    """
    ...
    for idx, group in enumerate(groups):
        ...
        results.append({"Group": group, "N": len(group_data), "W": stat, "p-value": p, "Normal": "Yes" if p > alpha else "No"})
...
def check_homogeneity_of_variance(data: pd.DataFrame, value_col: str, group_col: str, alpha: float = 0.05, plot: bool = True) -> dict:
    """
    ...
    return {
        ...
        "interpretation": interpretation,
        "recommendation": ("Proceed with standard test" if is_homogeneous else "Consider Welch's correction or transformation"),
    }


def check_linearity(x: np.ndarray | pd.Series, y: np.ndarray | pd.Series, x_name: str = "X", y_name: str = "Y") -> dict:
    """
    ...
    return {
        ...
        "recommendation": ("If non-linear pattern detected: Consider polynomial terms, transformations, or non-linear models"),
    }


def detect_outliers(data: np.ndarray | pd.Series | list, name: str = "data", method: str = "iqr", threshold: float = 1.5, plot: bool = True) -> dict:
    """
    ...
    if plot:
        ...
        ax2.scatter(
            x_coords[~outlier_mask], data_clean[~outlier_mask], alpha=0.6, s=50, color="steelblue", label="Normal", edgecolors="black", linewidths=0.5
        )
        if n_outliers > 0:
            ax2.scatter(
...
def comprehensive_assumption_check(data: pd.DataFrame, value_col: str, group_col: str | None = None, alpha: float = 0.05) -> dict:
    """
    ...
    print("\n1. OUTLIER DETECTION")
    print("-" * 70)
    outlier_results = detect_outliers(data[value_col].dropna(), name=value_col, method="iqr", plot=True)
    ...
    if group_col is not None:
        ...
        print("-" * 70)
        normality_results = check_normality_per_group(data, value_col, group_col, alpha=alpha, plot=True)
        ...
        print("-" * 70)
        homogeneity_results = check_homogeneity_of_variance(data, value_col, group_col, alpha=alpha, plot=True)
        ...
        print("\n2. NORMALITY CHECK")
        print("-" * 70)
        normality_results = check_normality(data[value_col].dropna(), name=value_col, alpha=alpha, plot=True)
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame({"score": np.concatenate([group_a, group_b]), "group": ["A"] * 50 + ["B"] * 50})
