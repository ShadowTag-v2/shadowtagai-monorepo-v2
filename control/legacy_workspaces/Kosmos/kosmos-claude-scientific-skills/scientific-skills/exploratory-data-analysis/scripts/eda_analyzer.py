...
def analyze_general_scientific(filepath, extension):
    ...
    try:
        if extension in ["npy"]:
            ...
            data = np.load(filepath)
            results = {"arrays": list(data.files), "array_count": len(data.files), "array_shapes": {name: data[name].shape for name in data.files}}
                ...
                def get_structure(group, prefix=""):
                    ...
                    for key in group.keys():
                        ...
                        if isinstance(group[key], h5py.Dataset):
                            items[path] = {"type": "dataset", "shape": group[key].shape, "dtype": str(group[key].dtype)}
