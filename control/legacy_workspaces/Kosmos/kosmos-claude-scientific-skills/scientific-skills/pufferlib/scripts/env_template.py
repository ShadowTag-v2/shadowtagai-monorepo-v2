...
class MultiAgentEnvironment(PufferEnv):
    ...
    def _apply_action(self, agent_idx, action):
        ...
        # Clip to grid bounds
        self.agent_positions[agent_idx] = np.clip(self.agent_positions[agent_idx], 0, self.grid_size - 1)
    ...
    def _get_obs(self, agent_idx):
        ...
        other_positions = np.concatenate([self.agent_positions[i] for i in range(self.num_agents) if i != agent_idx])
