...
class Policy(nn.Module):
    ...
    def __init__(self, observation_space, action_space, hidden_size=256):
        ...
        self.encoder = nn.Sequential(
            layer_init(nn.Linear(observation_space.shape[0], hidden_size)), nn.ReLU(), layer_init(nn.Linear(hidden_size, hidden_size)), nn.ReLU()
        )
...
def create_policy(env):
    ...
    return Policy(observation_space=env.observation_space, action_space=env.action_space, hidden_size=256)
...
def train(args):
    ...
    if args.logger == "wandb":
        ...
        logger = NeptuneLogger(project=args.project, name=args.exp_name, api_token=args.neptune_token)
    else:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.log_freq == 0:
            ...
            print(f"Iteration {iteration}/{args.num_iterations} | Mean Reward: {mean_reward:.2f} | SPS: {sps:,.0f}")
...
def main():
    ...
    parser.add_argument("--num-workers", type=int, default=8, help="Number of vectorization workers")
    ...
    parser.add_argument("--n-epochs", type=int, default=4, help="Number of training epochs per batch")
    ...
    # Logging
    parser.add_argument("--logger", type=str, default="none", choices=["wandb", "neptune", "none"], help="Logger to use")
    parser.add_argument("--project", type=str, default="pufferlib-training", help="Project name for logging")
    ...
    parser.add_argument("--save-freq", type=int, default=100, help="Checkpoint save frequency (iterations)")
