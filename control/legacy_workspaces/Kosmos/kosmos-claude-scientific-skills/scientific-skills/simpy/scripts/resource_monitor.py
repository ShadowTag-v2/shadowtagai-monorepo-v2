...
class MultiResourceMonitor:
  ...
  def summary(self):
    ...
    for name, monitor in self.monitors.items():
      print(
        f"{name:<20} {monitor.average_utilization():<12.2%} {monitor.average_queue_length():<12.2f} {monitor.average_wait_time():<12.2f}"
      )
...
class ContainerMonitor:
  ...
  def __init__(
    self, env: simpy.Environment, container: simpy.Container, name: str = "Container"
  ):
    """
