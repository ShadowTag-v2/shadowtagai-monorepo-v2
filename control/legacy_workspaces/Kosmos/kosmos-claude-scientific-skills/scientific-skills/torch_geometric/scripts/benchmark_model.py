...
def benchmark_node_classification(model_name, dataset_name, epochs, lr, weight_decay, device):
    ...
    model = model_class(num_features=dataset.num_features, hidden_channels=64, num_classes=dataset.num_classes).to(device)
        ...
        for model_name in args.models:
            ...
            try:
                if task == "node_classification":
                    result = benchmark_node_classification(model_name, args.dataset, args.epochs, args.lr, args.weight_decay, device)
                    print(f"Test Acc: {result['test_acc']:.4f}, Time: {result['train_time']:.2f}s")
                else:
                    result = benchmark_graph_classification(model_name, args.dataset, args.epochs, args.lr, device)
                    print(f"Test Acc: {result['test_acc']:.4f}, Time: {result['train_time']:.2f}s")
...
def main():
    ...
    parser.add_argument("--models", nargs="+", default=["gcn"], help="Model types to benchmark (gcn, gat, graphsage)")
    parser.add_argument("--dataset", type=str, default="Cora", help="Dataset name (Cora, CiteSeer, PubMed, ENZYMES, PROTEINS)")
    parser.add_argument("--epochs", type=int, default=200, help="Number of training epochs")
    parser.add_argument("--runs", type=int, default=5, help="Number of runs to average over")
    parser.add_argument("--lr", type=float, default=0.01, help="Learning rate")
    parser.add_argument("--weight-decay", type=float, default=5e-4, help="Weight decay for node classification")
