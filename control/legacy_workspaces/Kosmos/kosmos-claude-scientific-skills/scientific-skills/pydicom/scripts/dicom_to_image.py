...
def convert_dicom_to_image(input_path, output_path, image_format="PNG", apply_window=False, frame=0):
    """
...
Examples:
  ...
  python dicom_to_image.py multiframe.dcm frame5.png --frame 5
    ...
    parser.add_argument("--format", type=str, choices=["PNG", "JPEG", "TIFF", "BMP"], help="Output image format (default: inferred from extension)")
    parser.add_argument("--apply-windowing", action="store_true", help="Apply VOI LUT windowing if available")
    parser.add_argument("--frame", type=int, default=0, help="Frame number for multi-frame DICOM files (default: 0)")
    parser.add_argument("-v", "--verbose", action="store_true", help="Show detailed conversion information")
    ...
    success, result = convert_dicom_to_image(args.input, args.output, image_format, args.apply_windowing, args.frame)
