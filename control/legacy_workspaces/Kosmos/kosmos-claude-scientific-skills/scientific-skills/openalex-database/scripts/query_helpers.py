...
def find_author_works(author_name: str, client: OpenAlexClient, limit: int | None = None) -> list[dict[str, Any]]:
    """
    ...
    author_response = client._make_request("/authors", params={"search": author_name, "per-page": 1})
...
def find_institution_works(institution_name: str, client: OpenAlexClient, limit: int | None = None) -> list[dict[str, Any]]:
    """
    ...
    inst_response = client._make_request("/institutions", params={"search": institution_name, "per-page": 1})
...
def find_highly_cited_recent_papers(
    topic: str | None = None, years: str = ">2020", client: OpenAlexClient | None = None, limit: int = 100
) -> list[dict[str, Any]]:
    """
    ...
    params = {"filter": f"publication_year:{years}", "sort": "cited_by_count:desc", "per-page": min(limit, 200)}
...
def get_publication_trends(
    search_term: str | None = None, filter_params: dict | None = None, client: OpenAlexClient | None = None
) -> list[dict[str, Any]]:
    """
...
def analyze_research_output(
    ...
    # Works by year
    trends = client.group_by("works", "publication_year", filter_params={filter_prefix: entity_id, "publication_year": years})
