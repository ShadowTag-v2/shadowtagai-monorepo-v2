...
def create_line_plot(data, ax=None):
    ...
    ax.plot(data["x"], data["y1"], label="sin(x)", linewidth=2, marker="o", markevery=10, markersize=6)
...
def create_scatter_plot(data, ax=None):
    ...
    scatter = ax.scatter(data["scatter_x"], data["scatter_y"], c=colors, s=sizes, alpha=0.6, cmap="viridis", edgecolors="black", linewidth=0.5)
...
def create_bar_chart(data, ax=None):
    ...
    bars = ax.bar(x_pos, data["bar_values"], yerr=errors, color="steelblue", edgecolor="black", linewidth=1.5, capsize=5, alpha=0.8)
...
def create_histogram(data, ax=None):
    ...
    n, bins, patches = ax.hist(data["hist_data"], bins=30, density=True, alpha=0.7, edgecolor="black", color="steelblue")
    ...
    ax.plot(x_theory, norm.pdf(x_theory, mu, std), "r-", linewidth=2, label=f"Normal fit (μ={mu:.2f}, σ={std:.2f})")
...
def create_contour_plot(data, ax=None):
    ...
    # Contour lines
    contour = ax.contour(data["X"], data["Y"], data["Z"], levels=10, colors="black", linewidths=0.5, alpha=0.4)
