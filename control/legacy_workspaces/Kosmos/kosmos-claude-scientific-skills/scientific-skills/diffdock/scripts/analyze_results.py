  ...
  with open(output_path, "w", newline="") as f:
    ...
    for complex_name, data in results.items():
      ...
      for pred in predictions:
        ...
        writer.writerow(
          [complex_name, pred["rank"], conf_value, confidence_class, pred["path"]]
        )
...
def print_top_predictions(results, n=10):
  ...
  for i, pred in enumerate(top_preds, 1):
    ...
    print(
      f"{i:2d}. {pred['complex']:30s} | Rank {pred['rank']:2d} | Confidence: {pred['confidence']:7.3f} ({confidence_class})"
    )
