...
def register_mcp_tools(default_top_k: int = 3, max_content_chars: int | None = None):
  ...
  async def find_helpful_skills(
    task_description: str, top_k: int = default_top_k, list_documents: bool = True
  ) -> list[TextContent]:
  ...
  async def read_skill_document(
    skill_name: str, document_path: str | None = None, include_base64: bool = False
  ) -> list[TextContent]:
  ...
  if config.get("auto_update_enabled", False):
    ...
    async def update_callback():
      ...
      try:
        ...
        logger.info(
          f"Update check complete: {len(result.changed_sources)} sources changed, {result.api_calls_made} API calls made"
        )
