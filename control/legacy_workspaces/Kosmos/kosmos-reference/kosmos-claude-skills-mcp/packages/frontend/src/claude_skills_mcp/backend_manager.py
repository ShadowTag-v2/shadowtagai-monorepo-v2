...
class BackendManager:
  ...
  async def start_backend(self, backend_args: list[str]) -> str:
    ...
    try:
      ...
      asyncio.create_task(
        self._consume_stream(self.backend_process.stdout, "backend-stdout")
      )
      asyncio.create_task(
        self._consume_stream(self.backend_process.stderr, "backend-stderr")
      )
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
          ...
          if response.status_code == 200:
            ...
            if loading_complete and skills_loaded > 0:
              logger.info(
                f"Backend health check passed ({skills_loaded} skills loaded)"
              )
              return
  ...
  async def cleanup(self) -> None:
    ...
    if self.backend_process:
      ...
      try:
        logger.info(
          f"Terminating backend process group (PID: {self.backend_process.pid})"
        )
