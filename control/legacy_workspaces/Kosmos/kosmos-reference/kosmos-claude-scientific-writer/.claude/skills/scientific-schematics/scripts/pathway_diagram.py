...
class PathwayNode:
  ...
  def __init__(
    self, name: str, node_type: str = "protein", position: tuple[float, float] = (0, 0)
  ):
    self.name = name
...
class PathwayGenerator:
  ...
  def add_edge(
    self, source: str, target: str, interaction: str = "activation"
  ) -> "PathwayGenerator":
    """
  ...
  def _draw_nodes(self, ax: plt.Axes):
    ...
    for node in self.nodes.values():
      ...
      if node.node_type == "gene":
        ...
        box = Circle(
          (x, y), self.node_height / 2, facecolor=color, edgecolor="black", linewidth=2
        )
      else:
...
Examples:
  ...
  parser.add_argument(
    "--example", choices=["mapk", "simple"], help="Generate example pathway"
  )
  parser.add_argument(
    "-o", "--output", help="Output filename (default: based on example)"
  )
  parser.add_argument(
    "--dpi", type=int, default=300, help="DPI for raster output (default: 300)"
  )
