...
class TikZCompiler:
  ...
  def pdf_to_png(
    self, pdf_file: str, output: str | None = None, dpi: int = 300
  ) -> str | None:
    """
...
Examples:
  ...
  parser.add_argument(
    "--preview", action="store_true", help="Open PDF after compilation"
  )
  ...
  pdf_file = compiler.compile_to_pdf(
    args.input, output=args.output, cleanup=not args.no_cleanup
  )
