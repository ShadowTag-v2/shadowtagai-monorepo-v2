...
def generate_report(scores: dict[str, float], weights: dict[str, float], output_file: Path | None = None) -> str:
    ...
    for dimension, score in sorted(scores.items()):
        ...
        report_lines.append(f"  {dimension:25s} {score:.2f}/5.00  (weight: {percentage:4.1f}%, contribution: {weighted_contribution:.3f})")
...
def interactive_mode():
    ...
    scores = {}
    dimensions = ["problem_formulation", "literature_review", "methodology", "data_collection", "analysis", "results", "writing", "citations"]
