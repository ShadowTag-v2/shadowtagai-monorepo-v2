...
def verify_git_status(root: Path) -> None:
  ...
  if result.stdout.strip():
    raise RuntimeError(
      "Working directory has uncommitted changes. Please commit or stash changes before publishing."
    )
...
def publish_to_pypi(root: Path, dry_run: bool = False) -> None:
  ...
  if not token and not pypirc.exists():
    raise RuntimeError(
      "PyPI credentials not found. Please set UV_PUBLISH_TOKEN or TWINE_PASSWORD environment variable, or configure ~/.pypirc"
    )
...
def main() -> int:
  ...
  parser.add_argument(
    "--bump", choices=["major", "minor", "patch"], help="Bump version before publishing"
  )
  parser.add_argument(
    "--dry-run", action="store_true", help="Build package but don't publish"
  )
  parser.add_argument("--skip-tag", action="store_true", help="Skip git tag creation")
  parser.add_argument(
    "--skip-git-check", action="store_true", help="Skip git status verification"
  )
