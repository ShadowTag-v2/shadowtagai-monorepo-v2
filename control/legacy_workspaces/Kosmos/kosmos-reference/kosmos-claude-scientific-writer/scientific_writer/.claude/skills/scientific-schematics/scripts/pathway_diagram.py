...
class PathwayNode:
    ...
    def __init__(self, name: str, node_type: str = "protein", position: tuple[float, float] = (0, 0)):
        self.name = name
...
class PathwayEdge:
    ...
    def __init__(self, source: str, target: str, interaction: str = "activation"):
        self.source = source
        self.target = target
    ...
    def get_style(self) -> dict:
        ...
        return INTERACTION_STYLES.get(self.interaction, INTERACTION_STYLES["activation"])
...
class PathwayGenerator:
    ...
    def add_node(self, name: str, node_type: str = "protein", position: tuple[float, float] = (0, 0)) -> "PathwayGenerator":
        """
    ...
    def add_edge(self, source: str, target: str, interaction: str = "activation") -> "PathwayGenerator":
        """
    ...
    def _draw_nodes(self, ax: plt.Axes):
        ...
        for node in self.nodes.values():
            ...
            if node.node_type == "gene":
                ...
                box = Circle((x, y), self.node_height / 2, facecolor=color, edgecolor="black", linewidth=2)
            else:
            ...
            # Add label
            ax.text(x, y, node.name, ha="center", va="center", fontsize=self.font_size, fontweight="bold", fontfamily=self.font_family)
    ...
    def _draw_edges(self, ax: plt.Axes):
        ...
        for edge in self.edges:
            ...
            start, end = self._calculate_arrow_points((x1, y1), (x2, y2), self.node_width, self.node_height)
    ...
    def _calculate_arrow_points(
        self, start: tuple[float, float], end: tuple[float, float], box_width: float, box_height: float
...
Examples:
  ...
  python pathway_diagram.py --example mapk -o my_pathway.pdf
    ...
    parser.add_argument("--example", choices=["mapk", "simple"], help="Generate example pathway")
    parser.add_argument("-o", "--output", help="Output filename (default: based on example)")
