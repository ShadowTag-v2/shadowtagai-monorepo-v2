...
def find_existing_papers(output_folder: Path) -> list[dict[str, Any]]:
  ...
  for paper_dir in output_folder.iterdir():
    if paper_dir.is_dir():
      papers.append(
        {"path": paper_dir, "name": paper_dir.name, "mtime": paper_dir.stat().st_mtime}
      )
...
def detect_paper_reference(
  user_input: str, existing_papers: list[dict[str, Any]]
) -> Path | None:
  """
  ...
  has_continuation_keyword = any(
    keyword in user_input_lower for keyword in continuation_keywords
  )
  ...
  for paper in existing_papers:
    ...
    if len(parts) >= 3:
      ...
      matches = sum(
        1 for word in topic_words if len(word) > 3 and word in user_input_lower
      )
