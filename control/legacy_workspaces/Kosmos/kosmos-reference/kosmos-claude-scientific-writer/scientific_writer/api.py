      ...
      if hasattr(message, "content") and message.content:
        for block in message.content:
          if hasattr(block, "text"):
            ...
            stage, percentage, msg = _analyze_progress(
              accumulated_text, current_stage, current_percentage
            )
...
def _analyze_progress(text: str, current_stage: str, current_percentage: int) -> tuple:
  ...
  if (
    "research" in text_lower or "literature" in text_lower or "searching" in text_lower
  ):
...
def _find_most_recent_paper(output_folder: Path, start_time: float) -> Path | None:
  """
...
def _build_paper_result(paper_dir: Path, file_info: dict[str, Any]) -> PaperResult:
  ...
  """
  # Extract metadata
  tex_file = file_info["tex_final"] or (
    file_info["tex_drafts"][0] if file_info["tex_drafts"] else None
  )
