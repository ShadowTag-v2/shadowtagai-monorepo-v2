...
def setup_claude_skills(package_dir: Path, work_dir: Path) -> None:
  ...
  if source_claude.exists() and not dest_claude.exists():
    try:
      ...
      print("✓ Skills and system instructions (WRITER.md) ready")
...
def get_api_key(api_key: str | None = None) -> str:
  ...
  if not env_key:
    raise ValueError(
      "ANTHROPIC_API_KEY not found. Either pass api_key parameter or set ANTHROPIC_API_KEY environment variable."
    )
  return env_key
...
def load_system_instructions(work_dir: Path) -> str:
  ...
  if instructions_file.exists():
    ...
    return "You are a scientific writing assistant. Follow best practices for scientific communication and always present a plan before execution."
...
def extract_images_from_docx(
  docx_path: Path, figures_output: Path
) -> list[dict[str, Any]]:
  """
      ...
      for media_file in media_files:
        ...
        if file_ext in image_extensions:
          ...
          extracted_images.append(
            {"name": file_name, "path": str(output_path), "source_docx": docx_path.name}
          )
  ...
  for file_path in data_files:
    ...
    if file_ext in manuscript_extensions:
      ...
      processed_info["image_files"].append(
        {"name": file_name, "path": str(destination), "original": str(file_path)}
      )
    ...
    try:
      ...
      processed_info["all_files"].append(
        {"name": file_name, "type": file_type, "destination": str(destination)}
      )
...
def create_data_context_message(processed_info: dict[str, Any] | None) -> str:
  ...
  if processed_info.get("manuscript_files"):
    ...
    for file_info in processed_info["manuscript_files"]:
      context_parts.append(
        f"  - {file_info['name']} ({file_info['extension']}): {file_info['path']}"
      )
    context_parts.append(
  ...
  if processed_info.get("image_files"):
    ...
    direct_images = [
      img for img in processed_info["image_files"] if "source_docx" not in img
    ]
    extracted_images = [
      img for img in processed_info["image_files"] if "source_docx" in img
    ]
