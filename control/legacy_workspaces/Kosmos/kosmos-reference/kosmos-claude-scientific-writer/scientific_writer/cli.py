...
async def main():
  ...
  package_dir = Path(
    __file__
  ).parent.absolute()  # Package installation directory (scientific_writer/)
...
"""
  ...
  options = ClaudeAgentOptions(
    ...
    allowed_tools=[
      ...
      "Bash",
      "research-lookup",
    ],  # Default Claude Code tools + research lookup
  ...
  print(
    "  • I automatically detect when you're referring to a previous paper/presentation"
  )
  print(
    "  • Continue: 'continue', 'update', 'edit', 'the paper', 'the presentation', etc."
  )
  ...
  while True:
    try:
      ...
      is_new_paper_request = any(
        keyword in user_input.lower() for keyword in new_paper_keywords
      )
      ...
      if (
        data_files
        and not current_paper_path
        and (is_new_paper_request or not current_paper_path)
      ):
        ...
        if current_paper_path:
          ...
          if processed_info:
            ...
            if manuscript_count > 0:
              print(
                f"   ✓ Copied {manuscript_count} .tex manuscript(s) to drafts/ [EDITING MODE]"
              )
            if source_count > 0:
...
User request: {user_input}"""
        ...
        if paper_info["tex_drafts"]:
          context_parts.append(
            f"  • Draft LaTeX files: {len(paper_info['tex_drafts'])} file(s)"
          )
          for draft in paper_info["tex_drafts"]:
            context_parts.append(f"    - {Path(draft).name}")
        if paper_info["pdf_drafts"]:
          context_parts.append(
            f"  • Draft PDF files: {len(paper_info['pdf_drafts'])} file(s)"
          )
        ...
        if paper_info["sources"]:
          context_parts.append(
            f"  • Source/context files: {len(paper_info['sources'])} file(s)"
          )
        if paper_info["bibliography"]:
          context_parts.append(
            f"  • Bibliography: {Path(paper_info['bibliography']).name}"
          )
...
def _print_help():
  ...
  print(
    "  • Continue working: 'continue the paper', 'update my presentation', 'edit the poster'"
  )
  ...
  print(
    "  • Keywords like 'continue', 'update', 'edit', 'look for', 'find' trigger detection"
  )
