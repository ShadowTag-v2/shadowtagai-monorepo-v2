...
"""
  ...
  def is_r_available(self) -> bool:
    ...
    try:
      result = subprocess.run(
        [self.r_path, "--version"], capture_output=True, timeout=10
      )
  ...
  def get_r_version(self) -> str | None:
    ...
    try:
      result = subprocess.run(
        [self.r_path, "--version"], capture_output=True, text=True, timeout=10
      )
  ...
  def detect_language(self, code: str) -> str:
    ...
    """
    # Check for shebang
    if code.strip().startswith("#!/usr/bin/env Rscript") or code.strip().startswith(
      "#!/usr/bin/Rscript"
    ):
      return "r"
