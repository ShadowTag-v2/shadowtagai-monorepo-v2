...
class NotebookMetadata:
  ...
  cell_line_mappings: list[dict] | None = (
    None  # [{cell_index, start_line, end_line, code_hash}]
  )
...
class NotebookGenerator:
  ...
  def get_notebook_path(
    self, cycle: int, task_id: int, analysis_type: str, suffix: str = "ipynb"
  ) -> Path:
    """
  ...
  def create_notebook(
    ...
    try:
      ...
      logger.info(
        f"Generated notebook: {output_path} ({line_count} lines, {code_cell_count} code cells)"
      )
  ...
  def _convert_execution_result(self, execution_result: Any) -> list:
    ...
    if hasattr(execution_result, "stdout") and execution_result.stdout:
      outputs.append(
        new_output(output_type="stream", name="stdout", text=execution_result.stdout)
      )

    if hasattr(execution_result, "stderr") and execution_result.stderr:
      outputs.append(
        new_output(output_type="stream", name="stderr", text=execution_result.stderr)
      )

    if (
      hasattr(execution_result, "return_value")
      and execution_result.return_value is not None
    ):
      outputs.append(
        new_output(
  ...
  def _convert_cell_output(self, cell_output: Any) -> Any | None:
    """
    ...
    if output_type == "stream":
      ...
      return new_output(
        output_type="error", ename="Error", evalue=content, traceback=[]
      )
    elif output_type == "display_data":
      mime_type = getattr(cell_output, "mime_type", "text/plain")
      return new_output(
        output_type="display_data", data={mime_type: content}, metadata={}
      )
