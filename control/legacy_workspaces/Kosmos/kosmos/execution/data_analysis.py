...
class DataAnalyzer:
  ...
  def correlation_analysis(
    ...
    if len(df_clean) < 3:
      raise ValueError(
        f"Insufficient data after removing NaN values. Need at least 3 samples, got {len(df_clean)}"
      )
  ...
  def log_log_scaling_analysis(
    ...
    if len(df_clean) < 3:
      raise ValueError(
        f"Insufficient positive data for log-log analysis. Need at least 3 positive samples, got {len(df_clean)}"
      )
  ...
  def anova_comparison(
    ...
    group_data = [
      data[data[group_col] == g][measure_col].dropna() for g in unique_groups
    ]
    ...
    group_means = {
      str(g): float(np.mean(gd)) for g, gd in zip(unique_groups, group_data)
    }
    group_stds = {
      str(g): float(np.std(gd, ddof=1)) for g, gd in zip(unique_groups, group_data)
    }
      ...
      enriched_pathways.append(
        {
          ...
          "n_genes": int(row["Overlap"].split("/")[0])
          if "/" in str(row["Overlap"])
          else 0,
        }
      )
...
class DataLoader:
  ...
  @staticmethod
  def load_excel(
    file_path: str | Path, sheet_name: str | int = 0, **kwargs
  ) -> pd.DataFrame:
    """
  ...
  @staticmethod
  def load_parquet(
    file_path: str | Path, columns: list[str] | None = None, **kwargs
  ) -> pd.DataFrame:
    """
  ...
  def load_data(file_path: str | Path, **kwargs) -> pd.DataFrame:
    ...
    if suffix == ".csv":
      ...
      raise ValueError(
        f"Unsupported file type: {suffix}. Supported: .csv, .xlsx, .xls, .json, .h5ad, .parquet"
      )
...
class DataCleaner:
  ...
  def filter_positive(df: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
    ...
    if removed > 0:
      logger.info(
        f"Removed {removed} rows with non-positive values in {columns} ({removed / initial_rows * 100:.1f}%)"
      )
  ...
  def remove_outliers(
    ...
    if removed > 0:
      logger.info(
        f"Removed {removed} outliers from '{column}' using {method} ({removed / initial_rows * 100:.1f}%)"
      )
