...
class CellOutput:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {"output_type": self.output_type, "content": self.content, "mime_type": self.mime_type, "metadata": self.metadata}
...
class JupyterClient:
    ...
    async def execute_code(self, code: str, timeout: int = 300, capture_result: bool = True) -> ExecutionResult:
        """
...
'''
    ...
    async def execute_notebook(
        ...
        for i, cell in enumerate(cells):
            if cell.get("cell_type") == "code":
                ...
                if stop_on_error and result.status in (ExecutionStatus.FAILED, ExecutionStatus.TIMEOUT):
    ...
    async def run_script(self, script_path: str, timeout: int = 600) -> ExecutionResult:
        ...
        try:
            ...
            return ExecutionResult(status=ExecutionStatus.COMPLETED, stdout=stdout, stderr=stderr, execution_time=execution_time)
    ...
    async def check_package(self, package_name: str) -> bool:
        ...
        result = container.exec_run(["python3", "-c", f"import {package_name}"], workdir="/workspace")
    ...
    async def install_package(self, package_name: str, version: str | None = None) -> bool:
        ...
        result = container.exec_run(["pip", "install", "--quiet", "--no-cache-dir", pkg_spec], workdir="/workspace")
