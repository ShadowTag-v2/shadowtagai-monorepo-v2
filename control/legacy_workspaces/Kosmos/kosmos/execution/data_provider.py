...
class SyntheticDataGenerator:
  ...
  def _get_template(
    self, domain: str | None, experiment_type: str | None
  ) -> dict | None:
  ...
  def _generate_from_template(
    self, template: dict, n_samples: int, **kwargs
  ) -> pd.DataFrame:
    ...
    n_per_group = kwargs.get(
      "n_per_group", template.get("n_per_group", n_samples // len(groups))
    )
  ...
  def _generate_from_columns(
    self, columns: dict[str, str], n_samples: int, **kwargs
  ) -> pd.DataFrame:
...
class DataProvider:
  ...
  def __init__(self, default_data_dir: str | None = None, seed: int = 42):
    """
    ...
    **kwargs,
    ...
    if allow_synthetic:
      df = self.generator.generate(
        domain=domain, experiment_type=experiment_type, n_samples=n_samples, **kwargs
      )
  ...
  def validate_data(
    self, file_path: str | None = None, allow_synthetic: bool = True
  ) -> tuple[bool, str]:
    """
