...
class TTestComparisonCodeTemplate(CodeTemplate):
  ...
  def matches(self, protocol: ExperimentProtocol) -> bool:
    ...
    for test in protocol.statistical_tests:
      test_type_str = (
        ...
        if hasattr(test.test_type, "value")
        else str(test.test_type)
      )
  ...
  def generate(self, protocol: ExperimentProtocol) -> str:
    ...
    indep_vars = [
      v for v in protocol.variables.values() if v.type.value == "independent"
    ]
    ...
    code_lines = [
      ...
      "        df = None",
      "if df is None:",
      "    # Generate synthetic data for computational experiment",
...
class CorrelationAnalysisCodeTemplate(CodeTemplate):
  ...
  def matches(self, protocol: ExperimentProtocol) -> bool:
    ...
    for test in protocol.statistical_tests:
      ...
      if (
        "correlation" in test_type_str.lower() or "regression" in test_type_str.lower()
      ):
        return True
...
class GenericComputationalCodeTemplate(CodeTemplate):
  ...
  def generate(self, protocol: ExperimentProtocol) -> str:
    ...
    code_lines = [
      ...
      "",
      "# Data loading with synthetic fallback",
      "# Expected format: CSV with relevant columns",
      "df = None",
      "if 'data_path' in dir() and data_path:",
      "    try:",
      "        df = pd.read_csv(data_path)",
    ...
    if use_llm and llm_client is None:
      try:
        ...
        try:
          ...
          logger.warning(
            f"LiteLLM fallback also failed: {e2}. LLM generation disabled."
          )
