...
class ResultCollector:
  ...
  def collect(
    ...
    logger.info(
      f"Collected result for experiment {protocol.id}: status={status.value}, p-value={primary_p_value}"
    )
  ...
  def create_version(
    ...
    logger.info(
      f"Created result version {new_result.version} (parent: {original_result.id})"
    )
