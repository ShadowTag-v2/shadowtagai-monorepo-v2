...
from kosmos.models.result import ExperimentResult, ResultStatus, ExecutionMetadata, StatisticalTestResult, VariableResult, ResultExport
...
class ResultCollector:
    ...
    def collect(
        ...
        metadata = self._create_metadata(
            protocol=protocol, execution_output=execution_output, start_time=start_time or datetime.utcnow(), end_time=end_time or datetime.utcnow()
        )
        ...
        if statistical_tests:
            stat_test_results, primary_test, primary_p_value, primary_effect_size = self._create_statistical_test_results(statistical_tests)

        # Determine if hypothesis is supported
        supports_hypothesis = self._determine_hypothesis_support(primary_p_value, primary_effect_size, protocol)
        ...
        logger.info(f"Collected result for experiment {protocol.id}: status={status.value}, p-value={primary_p_value}")
    ...
    def _create_metadata(
        self, protocol: ExperimentProtocol, execution_output: dict[str, Any], start_time: datetime, end_time: datetime
    ) -> ExecutionMetadata:
    ...
    def _create_variable_results(self, variable_data: dict[str, pd.Series], protocol: ExperimentProtocol) -> list[VariableResult]:
    ...
    def _create_statistical_test_results(
        ...
        for i, test_data in enumerate(statistical_tests):
            ...
            statistic = test_data.get(
                "statistic",
                test_data.get("t_statistic", test_data.get("f_statistic", test_data.get("u_statistic", test_data.get("chi2_statistic", 0.0)))),
            )
    ...
    def _determine_hypothesis_support(self, p_value: float | None, effect_size: float | None, protocol: ExperimentProtocol) -> bool | None:
    ...
    def export_result(self, result: ExperimentResult, format: str = "json", output_path: str | None = None) -> str:
        """
    ...
    def create_version(
        self, original_result: ExperimentResult, new_execution_output: dict[str, Any], protocol: ExperimentProtocol
    ) -> ExperimentResult:
        """
        ...
        logger.info(f"Created result version {new_result.version} (parent: {original_result.id})")
