...
try:
  ...
  logger.warning(
    "Docker sandbox not available. Install docker package for sandboxed execution."
  )
...
class CodeExecutor:
  ...
  def __init__(
    ...
    if R_EXECUTOR_AVAILABLE:
      r_timeout = (
        self.sandbox_config.get("timeout", 300) if self.sandbox_config else 300
      )
  ...
  def execute_r(
    self, code: str, capture_results: bool = True, output_dir: str | None = None
  ) -> ExecutionResult:
    """
    ...
    r_result = self.r_executor.execute(
      code, capture_results=capture_results, output_dir=output_dir
    )
  ...
  def _execute_in_sandbox(
    ...
    sandbox_result = self.sandbox.execute(
      code, data_files=data_files if data_files else None
    )
  ...
  def execute_with_data(
    ...
    augmented_code = (
      f"# Data path injected by executor\ndata_path = {repr(data_path)}\n\n{code}"
    )
...
class CodeValidator:
  ...
  def validate(code: str, allow_file_read: bool = True) -> dict[str, Any]:
    ...
    logger.info(
      f"Code validation: {'PASSED' if is_valid else 'FAILED'}, {len(errors)} errors, {len(warnings)} warnings"
    )
...
class RetryStrategy:
  ...
  def _repair_with_llm(
    self, code: str, error: str, traceback_str: str, llm_client: Any
  ) -> str | None:
...
"""
  ...
  def _fix_file_not_found(self, code: str, error: str) -> str | None:
    """
