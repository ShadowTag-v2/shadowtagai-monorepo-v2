...
class UnifiedLiteratureSearch:
  ...
  def search_by_arxiv_id(self, arxiv_id: str) -> PaperMetadata | None:
    ...
    if PaperSource.SEMANTIC_SCHOLAR in self.clients:
      paper = self.clients[PaperSource.SEMANTIC_SCHOLAR].get_paper_by_id(
        f"arXiv:{arxiv_id}"
      )
      if paper:
        return paper
  ...
  def get_citations(
    self, paper: PaperMetadata, max_citations: int = 50
  ) -> list[PaperMetadata]:
    """
  ...
  def get_references(
    self, paper: PaperMetadata, max_references: int = 50
  ) -> list[PaperMetadata]:
    """
    ...
    for paper in papers:
      if paper.pdf_url and not paper.full_text:
        try:
          ...
          logger.warning(
            f"PDF extraction timed out after {pdf_timeout}s for {paper.id}"
          )
        except Exception as e:
