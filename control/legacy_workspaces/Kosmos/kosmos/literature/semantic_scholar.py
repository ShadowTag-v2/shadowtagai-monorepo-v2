...
class SemanticScholarClient(BaseLiteratureClient):
  ...
  def get_paper_references(
    ...
    try:
      ...
      papers = [
        self._s2_to_metadata(ref.citedPaper) for ref in result if ref.citedPaper
      ]
  ...
  def get_paper_citations(
    ...
    try:
      ...
      # Extract and convert papers
      papers = [
        self._s2_to_metadata(cite.citingPaper) for cite in result if cite.citingPaper
      ]
  ...
  def _s2_to_metadata(self, result: S2Paper) -> PaperMetadata:
    ...
    return PaperMetadata(
      ...
      journal=(
        result.journal.get("name")
        if isinstance(result.journal, dict)
        else result.journal
      )
      if result.journal
      else None,
