...
class Neo4jWorldModel(WorldModelStorage, EntityManager):
  ...
  def add_relationship(self, relationship: Relationship) -> str:
    ...
    if not self.graph.connected:
      logger.debug(
        f"Neo4j not connected, skipping add_relationship for {relationship.id}"
      )
  ...
  def _add_generic_relationship(self, relationship: Relationship) -> str:
    ...
    if not self.graph.connected:
      logger.debug(
        f"Neo4j not connected, skipping _add_generic_relationship for {relationship.id}"
      )
      return relationship.id
    ...
    if not result:
      raise ValueError(
        f"Could not create relationship: source={relationship.source_id}, target={relationship.target_id}"
      )
  ...
  def get_relationship(self, relationship_id: str) -> Relationship | None:
    ...
    if not self.graph.connected:
      logger.debug(
        f"Neo4j not connected, skipping get_relationship for {relationship_id}"
      )
      return None
  ...
  def query_related_entities(
    ...
    if not self.graph.connected:
      logger.debug(
        f"Neo4j not connected, skipping query_related_entities for {entity_id}"
      )
      return []
  ...
  def import_graph(
    ...
    if data.get("version") != EXPORT_FORMAT_VERSION:
      logger.warning(
        f"Import file version {data.get('version')} may not be compatible with current version {EXPORT_FORMAT_VERSION}"
      )
  ...
  def get_annotations(self, entity_id: str) -> list[Annotation]:
    ...
    if not self.graph.connected:
      logger.debug(
        f"Not connected to Neo4j, returning empty annotations for {entity_id}"
      )
      return []
