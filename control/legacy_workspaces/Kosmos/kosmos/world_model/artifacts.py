...
class Finding:
  ...
  notebook_metadata: dict | None = (
    None  # Notebook metadata: {kernel, line_count, cell_count, etc.}
  )
  # Issue #70: Null model validation for statistical grounding
  null_model_result: dict | None = (
    None  # Null model validation: {p_value, persists_in_noise, etc.}
  )
  # Issue #63: Failure mode detection
  failure_detection_result: dict | None = (
    None  # Failure modes: {over_interpretation, invented_metrics, rabbit_hole}
  )
  # Issue #62: Code line provenance
  code_provenance: dict | None = (
    None  # CodeProvenance: {notebook_path, cell_index, start_line, end_line, code_snippet}
  )
...
class ArtifactStateManager:
  ...
  def __init__(
    self, artifacts_dir: str = "artifacts", world_model=None, vector_store=None
  ):
    """
  ...
  async def save_finding_artifact(
    self, cycle: int, task_id: int, finding: dict
  ) -> Path:
    """
