...
class Finding:
    ...
    hypothesis_id: str | None = None  # Issue #55: Link finding to hypothesis
    ...
    figure_paths: list[str] | None = None  # List of generated figure file paths
    figure_metadata: dict | None = None  # Figure metadata: {type, caption, dpi, etc.}
    # Issue #61: Notebook tracking for research artifacts
    notebook_metadata: dict | None = None  # Notebook metadata: {kernel, line_count, cell_count, etc.}
    # Issue #70: Null model validation for statistical grounding
    null_model_result: dict | None = None  # Null model validation: {p_value, persists_in_noise, etc.}
    # Issue #63: Failure mode detection
    failure_detection_result: dict | None = None  # Failure modes: {over_interpretation, invented_metrics, rabbit_hole}
    # Issue #62: Code line provenance
    code_provenance: dict | None = None  # CodeProvenance: {notebook_path, cell_index, start_line, end_line, code_snippet}
    ...
    validation_accurate: bool | None = None  # Expert assessment of accuracy
    validation_timestamp: str | None = None  # When validation was recorded
    validation_notes: str | None = None  # Expert notes on validation
...
class ArtifactStateManager:
    ...
    def get_validated_findings(self) -> list[Finding]:
        ...
        validated = [f for f in all_findings if f.scholar_eval and f.scholar_eval.get("passes_threshold", False)]
        return validated
    ...
    async def generate_cycle_summary(self, cycle: int) -> str:
        ...
        validated = [f for f in findings if f.scholar_eval and f.scholar_eval.get("passes_threshold", False)]
        ...
        try:
            ...
            finding_entity = Entity(
                ...
                properties={
                    ...
                    "confidence": finding.scholar_eval.get("overall_score", 0) if finding.scholar_eval else 0,
            ...
            if finding.notebook_path:
                evidence_entity = Entity(type="Evidence", properties={"path": finding.notebook_path, "type": finding.evidence_type})
                evidence_id = self.world_model.add_entity(evidence_entity)

                derives_from = Relationship(
                    source_id=entity_id, target_id=evidence_id, type="DERIVES_FROM", properties={"evidence_type": finding.evidence_type}
                )
    ...
    async def add_finding_with_conflict_check(self, finding: dict) -> FindingIntegrationResult:
        """
        ...
        for existing in existing_findings:
            ...
            if new_effect != 0 and old_effect != 0 and (new_effect * old_effect < 0):
                ...
                reasoning = f"Effect direction contradiction: new effect={new_effect:.3f} vs existing effect={old_effect:.3f}"
        ...
        return FindingIntegrationResult(
            success=True, update_type=update_type, affected_hypotheses=affected, confidence=finding.get("confidence", 0.5), reasoning=reasoning
        )
