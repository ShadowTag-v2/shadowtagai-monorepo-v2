  ...
  if _world_model is None or reset:
    ...
    if mode == "simple":
      ...
      if not wm.graph.connected:
        logger.warning(
          "Neo4j unavailable, falling back to InMemoryWorldModel. Graph data will not persist across restarts."
        )
      ...
      raise ValueError(
        f"Unknown world model mode: '{mode}'. Valid options: 'simple', 'production'"
      )
