...
class SafetyReport(BaseModel):
  ...
  def has_critical_violations(self) -> bool:
    ...
    return any(
      v.severity in [RiskLevel.CRITICAL, RiskLevel.HIGH] for v in self.violations
    )
...
class EthicalGuideline(BaseModel):
  ...
  guideline_id: str
  category: (
    str  # e.g., "human_subjects", "animal_welfare", "data_privacy", "environmental"
  )
  description: str
  required: bool = True
...
class EmergencyStopStatus(BaseModel):
  ...
  def trigger(
    self, triggered_by: str, reason: str, affected_experiments: list[str] = None
  ):
