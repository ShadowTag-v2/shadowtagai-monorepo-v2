...
class ExecutionMetadata(BaseModel):
  ...
  memory_peak_mb: float | None = Field(
    None, ge=0, description="Peak memory usage in MB"
  )
  ...
  data_source: str | None = Field(
    None, description="Data source: 'file' or 'synthetic'"
  )
...
class StatisticalTestResult(BaseModel):
  ...
  effect_size_type: str | None = Field(
    None, description="Type of effect size (Cohen's d, etc.)"
  )
  ...
  confidence_level: float | None = Field(
    None, ge=0, le=1, description="CI confidence level"
  )
  ...
  is_primary: bool = Field(
    default=False, description="Whether this is the primary test"
  )
...
class VariableResult(BaseModel):
  ...
  variable_type: str = Field(
    ..., description="Variable type (independent, dependent, etc.)"
  )
...
class ExperimentResult(BaseModel):
  ...
  parent_result_id: str | None = Field(
    None, description="ID of parent result if re-run"
  )
