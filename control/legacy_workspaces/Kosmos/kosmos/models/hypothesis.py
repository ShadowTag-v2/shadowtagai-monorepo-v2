...
class HypothesisGenerationRequest(BaseModel):
  ...
  max_iterations: int = Field(
    default=1, ge=1, le=5, description="Max refinement iterations"
  )
...
class NoveltyReport(BaseModel):
  ...
  max_similarity: float = Field(
    0.0, ge=0.0, le=1.0, description="Highest similarity found"
  )
  prior_art_detected: bool = Field(
    default=False, description="True if near-duplicate found"
  )
...
class TestabilityReport(BaseModel):
  ...
  testability_score: float = Field(
    ..., ge=0.0, le=1.0, description="Overall testability score"
  )
  ...
  challenges: list[str] = Field(
    default_factory=list, description="Implementation challenges"
  )
  limitations: list[str] = Field(
    default_factory=list, description="Testing limitations"
  )
...
class PrioritizedHypothesis(BaseModel):
  ...
  priority_score: float = Field(
    ..., ge=0.0, le=1.0, description="Overall priority score"
  )
