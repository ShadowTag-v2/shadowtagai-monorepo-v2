...
class HumanReviewWorkflow:
  ...
  def __init__(
    ...
    logger.info(
      f"HumanReviewWorkflow initialized (mode={mode.value}, auto_approve_low_risk={auto_approve_low_risk})"
    )

  def request_approval(
    self, request: ApprovalRequest, user: str = "system"
  ) -> ApprovalRequest:
    """
  ...
  def _auto_approve(self, request: ApprovalRequest, user: str) -> ApprovalRequest:
    ...
    if self.auto_approve_low_risk and request.risk_level == RiskLevel.LOW:
      request.approve("automatic")
      self._audit(
        "auto_approved", "automatic", request.request_id, {"reason": "low_risk"}
      )
    else:
      ...
      if self.notification_callback:
        self.notification_callback(
          f"Auto-approved {request.risk_level.value} risk operation: {request.operation_description[:100]}"
        )
  ...
  def _blocking_approval(self, request: ApprovalRequest, user: str) -> ApprovalRequest:
    ...
    logger.info(
      f"Approval required for {request.operation_type} (risk: {request.risk_level.value})"
    )
  ...
  def _queue_approval(self, request: ApprovalRequest, user: str) -> ApprovalRequest:
    ...
    logger.info(
      f"Approval request queued: {request.request_id} ({len(self.pending_requests)} pending)"
    )
  ...
  def override_decision(
    ...
    logger.warning(
      f"Approval decision overridden by {user}: {request_id} ({old_status.value} -> {new_status.value})"
    )
