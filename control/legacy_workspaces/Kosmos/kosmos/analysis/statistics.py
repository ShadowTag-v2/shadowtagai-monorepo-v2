...
class DescriptiveStats:
  ...
  def compute_full_descriptive(data: np.ndarray) -> dict[str, float]:
    ...
    stats_dict = {
      ...
      "cv": float(np.std(clean_data, ddof=1) / np.mean(clean_data))
...
class DistributionAnalysis:
  ...
  def test_normality(data: np.ndarray, alpha: float = 0.05) -> dict[str, Any]:
    ...
    result = {
      ...
      "alpha": alpha,
      "interpretation": (
        f"Data {'appears' if p_value > alpha else 'does not appear'} normally distributed (p={p_value:.4f})"
      ),
    }
  ...
  def fit_distribution(
    data: np.ndarray, dist_names: list[str] | None = None
  ) -> dict[str, Any]:
    """
...
class StatisticalReporter:
  ...
  def generate_full_report(
    ...
    report += (
      f"**Dataset Shape:** {data.shape[0]} observations × {data.shape[1]} variables\n\n"
    )
