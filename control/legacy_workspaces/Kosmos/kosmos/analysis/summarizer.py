...
class ResultSummarizer:
  ...
  def generate_summary(
    ...
    prompt = self._build_summary_prompt(
      result, hypothesis, interpretation, literature_context
    )
  ...
  def extract_key_findings(
    self, result: ExperimentResult, max_findings: int = 5
  ) -> list[str]:
    """
  ...
  def identify_limitations(
    ...
    if result.primary_ci_lower is None or result.primary_ci_upper is None:
      limitations.append(
        "Confidence intervals not reported, limiting precision of estimate"
      )

    # Generic limitations
    limitations.append(
      "Replication in independent dataset recommended to confirm findings"
    )
    limitations.append(
      "Potential confounding variables should be considered in interpretation"
    )
...
""")
  ...
  def _create_fallback_summary(
    ...
    summary = (
      f"Experiment {result.experiment_id} completed with status {result.status}. "
    )
    ...
    return ResultSummary(
      ...
      hypothesis_comparison=self.compare_to_hypothesis(result, hypothesis)
      if hypothesis
