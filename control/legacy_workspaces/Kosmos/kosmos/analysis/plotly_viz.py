...
COLORS = {"red": "#d7191c", "blue_dark": "#2c7bb6", "blue": "#0072B2", "neutral": "#abd9e9", "gray": "#808080", "black": "#000000"}
...
class PlotlyVisualizer:
    ...
    def interactive_scatter(
        self, x: np.ndarray, y: np.ndarray, x_label: str, y_label: str, title: str, labels: list[str] | None = None, show_regression: bool = True
    ) -> go.Figure:
        """
        ...
        if labels is not None:
            hover_text = [f"{label}<br>{x_label}: {xi:.3f}<br>{y_label}: {yi:.3f}" for label, xi, yi in zip(labels, x, y)]
        else:
            hover_text = [f"{x_label}: {xi:.3f}<br>{y_label}: {yi:.3f}" for xi, yi in zip(x, y)]
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=y,
                mode="markers",
                marker=dict(size=8, color=COLORS["neutral"], line=dict(width=1, color="black"), opacity=0.7),
                text=hover_text,
    ...
    def interactive_heatmap(
        self, data: np.ndarray, row_labels: list[str], col_labels: list[str], title: str = "Heatmap", colorscale: str = "RdBu_r"
    ) -> go.Figure:
        """
    ...
    def interactive_volcano(
        ...
        significant = np.array([(abs(fc) > fc_threshold and p < p_threshold) for fc, p in zip(log2fc, p_values)])

        # Create hover text
        if labels is not None:
            hover_text = [
                f"{label}<br>Log2FC: {fc:.3f}<br>P-value: {p:.4e}<br>{'SIGNIFICANT' if sig else 'Not significant'}"
                for label, fc, p, sig in zip(labels, log2fc, p_values, significant)
            ]
        else:
            hover_text = [
                f"Log2FC: {fc:.3f}<br>P-value: {p:.4e}<br>{'SIGNIFICANT' if sig else 'Not significant'}"
        ...
        if len(sig_indices) > 0:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=log_p[sig_indices],
                    mode="markers",
                    marker=dict(size=8, color=COLORS["red"], opacity=0.7),
        ...
        if len(nonsig_indices) > 0:
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(size=8, color=COLORS["gray"], opacity=0.5),
        ...
        # Add threshold lines
        fig.add_hline(y=-np.log10(p_threshold), line_dash="dash", line_color="black", annotation_text=f"p={p_threshold}", annotation_position="right")
    ...
    def interactive_box(self, data: dict[str, np.ndarray], title: str = "Box Plot", y_label: str = "Value") -> go.Figure:
        """
        ...
        for label, values in data.items():
            ...
            hover_text = [f"{label}<br>Value: {v:.3f}<br>Mean: {mean_val:.3f}<br>Median: {median_val:.3f}<br>SD: {std_val:.3f}" for v in values]
            ...
            fig.add_trace(
                go.Box(
                    ...
                    marker=dict(size=6, color=COLORS["blue_dark"], opacity=0.6),
        ...
        fig.update_layout(
            title=title, yaxis_title=y_label, template=self.template, font=dict(family="Arial", size=12), showlegend=True, hovermode="closest"
        )
    ...
    def create_multi_panel(
        self, figures: list[go.Figure], rows: int, cols: int, subplot_titles: list[str] | None = None, main_title: str | None = None
    ) -> go.Figure:
        """
        ...
        fig.update_layout(template=self.template, font=dict(family="Arial", size=12), showlegend=True)
    ...
    def save_static_image(self, fig: go.Figure, output_path: str, width: int = 800, height: int = 600, scale: float = 2.0):
        """
