...
class PlotlyVisualizer:
  ...
  def interactive_scatter(
    ...
    if labels is not None:
      ...
      hover_text = [
        f"{x_label}: {xi:.3f}<br>{y_label}: {yi:.3f}" for xi, yi in zip(x, y)
      ]
  ...
  def interactive_volcano(
    ...
    significant = np.array(
      [(abs(fc) > fc_threshold and p < p_threshold) for fc, p in zip(log2fc, p_values)]
    )
  ...
  def interactive_box(
    ...
    for label, values in data.items():
      ...
      hover_text = [
        f"{label}<br>Value: {v:.3f}<br>Mean: {mean_val:.3f}<br>Median: {median_val:.3f}<br>SD: {std_val:.3f}"
        for v in values
      ]
  ...
  def create_multi_panel(
    ...
    fig.update_layout(
      template=self.template, font=dict(family="Arial", size=12), showlegend=True
    )
