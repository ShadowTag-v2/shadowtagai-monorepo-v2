...
class AccuracyTracker:
  ...
  def add_finding(self, finding: Any) -> None:
    ...
    if evidence_type not in self.VALID_EVIDENCE_TYPES:
      logger.warning(
        f"Unknown evidence_type '{evidence_type}' for finding {finding_id}, using 'data_analysis'"
      )
  ...
  def validate_finding(
    self, finding_id: str, is_accurate: bool, expert_notes: str | None = None
  ) -> None:
    """
  ...
  def compute_type_accuracy(self, evidence_type: str) -> TypeAccuracyResult:
    ...
    validated = [
      f for f in findings_list if getattr(f, "finding_id", None) in self.validations
    ]
    ...
    accurate = sum(
      1 for f in validated if self.validations[getattr(f, "finding_id")].is_accurate
    )
  ...
  def _generate_recommendations(
    self, by_type: dict[str, TypeAccuracyResult], overall_result: TypeAccuracyResult
  ) -> list[str]:
