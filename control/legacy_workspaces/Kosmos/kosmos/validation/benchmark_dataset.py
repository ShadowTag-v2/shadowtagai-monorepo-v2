...
class BenchmarkFinding:
  ...
  def __post_init__(self):
    ...
    if self.evidence_type not in valid_types:
      raise ValueError(
        f"Invalid evidence_type '{self.evidence_type}'. Must be one of: {valid_types}"
      )
...
class BenchmarkGenerator:
  ...
  def _generate_data_analysis_finding(
    self, finding_id: str, domain: str, is_accurate: bool
  ) -> BenchmarkFinding:
  ...
  def create_synthetic_benchmark(
    self, n_per_type: int = 30, accuracy_rates: dict[str, float] | None = None
  ) -> BenchmarkDataset:
    """
    ...
    return BenchmarkDataset(
      ...
      description=(
        f"Synthetic benchmark dataset with {len(findings)} findings ({n_per_type} per type) matching paper accuracy rates"
      ),
      metadata={
...
def create_paper_benchmark(output_path: str | None = None) -> BenchmarkDataset:
  """
