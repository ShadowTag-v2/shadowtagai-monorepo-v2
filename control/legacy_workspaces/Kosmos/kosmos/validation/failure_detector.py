...
class FailureDetectionResult:
  ...
  def get_summary(self) -> str:
    ...
    if self.over_interpretation.detected:
      failures.append(
        f"Over-interpretation (score={self.over_interpretation.score:.2f})"
      )
...
class FailureDetector:
  ...
  def _extract_metrics_from_text(self, text: str) -> list[str]:
    ...
    pattern3 = (
      r"the\s+([a-zA-Z][a-zA-Z0-9_]*)\s+(?:was|is|were|are)\s+[-+]?[0-9]*\.?[0-9]+"
    )
  ...
  def detect_rabbit_hole(
    ...
    if similarity < self.similarity_threshold:
      evidence.append(
        f"Low relevance detected (threshold: {self.similarity_threshold})"
      )
  ...
  def _compute_relevance_similarity(
    self, finding_text: str, research_question: str
  ) -> float:
    """
  ...
  def get_failure_statistics(
    self, results: list[FailureDetectionResult]
  ) -> dict[str, Any]:
    """
