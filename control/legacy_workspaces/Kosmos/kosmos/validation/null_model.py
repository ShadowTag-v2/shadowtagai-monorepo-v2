...
class NullModelResult:
  ...
  null_distribution: list[
    float
  ]  # Distribution summary (percentiles: 5, 25, 50, 75, 95)
  ...
  null_effect_sizes: list[float] | None = None  # Percentiles from permuted data
...
class NullModelValidator:
  ...
  def validate_finding(
    ...
    return NullModelResult(
      ...
      null_effect_sizes=self._summarize_distribution(null_effects)
      if null_effects is not None
      else None,
  ...
  def _extract_test_statistic(self, finding: dict[str, Any]) -> float:
    ...
    raise ValueError(
      f"No test statistic found in finding. Expected one of: {self.STATISTIC_KEYS}"
    )
  ...
  def _parametric_null(self, finding: dict[str, Any]) -> np.ndarray:
    ...
    if "t" in test_type or "t_test" in test_type:
      ...
      # F-distribution
      return sp_stats.f.rvs(
        1, max(1, df), size=self.n_permutations, random_state=self.rng
      )
    elif "chi" in test_type:
      # Chi-squared distribution
      return sp_stats.chi2.rvs(
        max(1, df), size=self.n_permutations, random_state=self.rng
      )
    elif (
      "correlation" in test_type or "pearson" in test_type or "spearman" in test_type
    ):
  ...
  def _calculate_permutation_pvalue(
    self, observed: float, null_dist: np.ndarray
  ) -> float:
    """
