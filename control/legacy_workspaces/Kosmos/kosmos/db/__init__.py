...
def init_database(
  ...
  logger.info(
    f"Database initialized successfully (pool_size={pool_size if not database_url.startswith('sqlite') else 'N/A'})"
  )
...
def init_from_config():
  ...
  if not setup_results["schema_valid"]:
    logger.warning(
      "Database schema is incomplete. Some features may not work correctly. Run 'kosmos doctor' for details."
    )
