...
class ResearchConfig(BaseSettings):
  ...
  enabled_experiment_types: Annotated[
    list[str], BeforeValidator(parse_comma_separated)
  ] = Field(
...
class DatabaseConfig(BaseSettings):
  ...
  echo: bool = Field(
    default=False, description="Enable SQL echo logging", alias="DATABASE_ECHO"
  )
...
class RedisConfig(BaseSettings):
  ...
  enabled: bool = Field(
    default=False, description="Enable Redis caching", alias="REDIS_ENABLED"
  )
...
class LoggingConfig(BaseSettings):
  ...
  debug_modules: Annotated[list[str] | None, BeforeValidator(parse_comma_separated)] = (
    Field(
      default=None,
...
class DevelopmentConfig(BaseSettings):
  ...
  test_mode: bool = Field(
    default=False, description="Test mode (uses mocks)", alias="TEST_MODE"
  )
  ...
  def validate_provider_config(self):
    ...
    if self.llm_provider == "openai":
      if not self.openai or not self.openai.api_key:
        raise ValueError(
          "OPENAI_API_KEY is required when LLM_PROVIDER=openai. Please set OPENAI_API_KEY in your environment or .env file."
        )
  ...
  def get_active_provider_config(self) -> dict:
    ...
    if provider == "litellm":
      return {
        ...
        "api_base": self.litellm.api_base,
      }
    elif provider == "anthropic":
