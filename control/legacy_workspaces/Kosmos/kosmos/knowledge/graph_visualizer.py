...
class GraphVisualizer:
  ...
  def visualize_citation_network(
    ...
    for citation in citations:
      ...
      G.add_node(
        cited_id, type="Paper", label=cited.get("title", "Unknown"), importance=0.5
      )
  ...
  def _build_networkx_graph(
    ...
    node_types_str = (
      "|".join(node_types) if node_types else "Paper|Author|Concept|Method"
    )
  ...
  def _visualize_networkx_interactive(self, G: nx.Graph, output_file: str, title: str):
    ...
    self.visualize_interactive(
      output_file=output_file, max_nodes=len(G.nodes()), title=title
    )
