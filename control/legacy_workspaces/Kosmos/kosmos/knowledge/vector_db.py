...
class PaperVectorDB:
  ...
  def __init__(
    ...
    logger.info(
      f"Initialized PaperVectorDB (collection={collection_name}, persist_dir={persist_directory}, count={self.collection.count()})"
    )
    ...
    if results and results["ids"] and len(results["ids"]) > 0:
      for i in range(len(results["ids"][0])):
        result = {
          ...
          else 1.0,  # Convert distance to similarity
  ...
  def _paper_metadata(self, paper: PaperMetadata) -> dict[str, Any]:
    ...
    metadata = {
      ...
      "domain": paper.fields[0]
      if paper.fields and len(paper.fields) > 0
      else "unknown",
    }
