...
class KnowledgeGraph:
  ...
  def get_method_papers(
    self, method_name: str, limit: int = 50
  ) -> list[dict[str, Any]]:
    """
  ...
  def get_concept_cooccurrence(
    ...
    results = self.graph.run(
      query, concept_name=concept_name, min_papers=min_papers
    ).data()

    return [
      {"concept": dict(r["c2"]), "count": r["cooccurrence_count"]} for r in results
    ]
  ...
  def get_stats(self) -> dict[str, Any]:
    ...
    for label in ["Paper", "Author", "Concept", "Method"]:
      count = self.graph.run(f"MATCH (n:{label}) RETURN count(n) as count").data()[0][
        "count"
      ]
