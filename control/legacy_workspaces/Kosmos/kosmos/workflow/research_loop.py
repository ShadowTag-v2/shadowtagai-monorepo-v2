...
class ResearchWorkflow:
  ...
  async def _execute_cycle(self, cycle: int, num_tasks: int) -> dict:
    ...
    if self.past_tasks:
      ...
      logger.info(
        f"  Novelty: {plan_novelty['novel_task_count']}/{len(plan.tasks)} novel tasks ({plan_novelty['plan_novelty_score']:.2f})"
      )
    ...
    logger.info(
      f"  Plan review: {'APPROVED' if review.approved else 'REJECTED'} (score: {review.average_score:.1f}/10)"
    )
    ...
    if completed_tasks:
      ...
      logger.info(
        f"  Compressed: {len(completed_tasks)} tasks → {len(compressed_cycle.summary)} chars"
      )
  ...
  def _compute_final_statistics(self) -> dict:
    ...
    results = {
      ...
      "validation_rate": (
        len(validated_findings) / len(all_findings) if all_findings else 0
      ),
  ...
  async def generate_report(self) -> str:
    ...
    report += (
      f"research cycles, generating {len(validated_findings)} validated findings.\n\n"
    )
