...
class ConvergenceAnalyzer:
  ...
  def analyze(
    self, run_findings: list[list[dict]], run_configs: list[RunConfig] | None = None
  ...
  def _cluster_similar_findings(
    self, all_findings: list[tuple[int, dict]]
  ...
  def _compute_finding_similarity(self, finding1: dict, finding2: dict) -> float:
    ...
    text_sim = self._compute_text_similarity(
      finding1.get("summary", ""), finding2.get("summary", "")
    )

    # Statistical similarity
    stat_sim = self._compute_statistical_similarity(
      finding1.get("statistics", {}), finding2.get("statistics", {})
    )
  ...
  def _create_finding_match(
    self, cluster: list[tuple[int, dict]], n_runs: int, cluster_idx: int
  ) -> FindingMatch:
    """
    ...
    scholar_score_std = (
      float(np.std(scholar_scores)) if len(scholar_scores) > 1 else 0.0
    )
    ...
    is_convergent = self._is_convergent(
      convergence_strength, effect_size_cv, significance_agreement, direction_agreement
    )
  ...
  def _determine_convergence_strength(
    self, replication_rate: float, n_runs: int
  ) -> str:
    """
  ...
  def _calculate_convergence_metrics(
    self, matched_findings: list[FindingMatch], n_runs: int, total_raw: int
  ) -> ConvergenceMetrics:
    ...
    findings_replicated_2_3 = sum(
      1 for m in matched_findings if 2 <= m.replication_count <= 3
    )
    findings_replicated_4_plus = sum(
      1 for m in matched_findings if m.replication_count >= 4
    )

    # Count by strength
    strong_count = sum(
      1 for m in matched_findings if m.convergence_strength == "strong"
    )
    moderate_count = sum(
      1 for m in matched_findings if m.convergence_strength == "moderate"
    )
    ...
    if matched_findings:
      ...
      sig_agreements = [
        m.significance_agreement for m in matched_findings if m.p_values
      ]
      avg_sig_agreement = float(np.mean(sig_agreements)) if sig_agreements else 0.0

      dir_agreements = [
        m.direction_agreement for m in matched_findings if m.effect_sizes
      ]
      avg_dir_agreement = float(np.mean(dir_agreements)) if dir_agreements else 0.0

      scholar_vars = [
        m.scholar_score_std**2 for m in matched_findings if m.scholar_scores
      ]
  ...
  def __init__(
    ...
    if temperatures is not None:
      if len(temperatures) != n_runs:
        raise ValueError(
          f"temperatures must have {n_runs} elements, got {len(temperatures)}"
        )
    ...
    logger.info(
      f"Initialized EnsembleRunner with {n_runs} runs, seeds={self.seeds}, temperatures={self.temperatures}"
    )

  async def run(
    self, research_objective: str, num_cycles: int = 5, tasks_per_cycle: int = 10
  ) -> EnsembleResult:
    """
    ...
    logger.info(
      f"Starting ensemble run: '{research_objective}' ({self.n_runs} runs, {num_cycles} cycles each)"
    )
    ...
    logger.info(
      f"Ensemble complete: {len(convergent)} convergent, {len(non_convergent)} non-convergent findings ({end_time - start_time:.1f}s)"
    )
...
class ConvergenceReporter:
  ...
  def generate_markdown_report(self, result: EnsembleResult) -> str:
    ...
    lines.append(
      f"**Date**: {result.end_timestamp[:10] if result.end_timestamp else 'N/A'}\n"
    )
    ...
    if metrics:
      lines.append(
        f"- **Total Unique Findings**: {metrics.total_unique_findings} (from {metrics.total_raw_findings} raw findings)\n"
      )
      lines.append(
        f"- **Strongly Convergent**: {metrics.strong_convergence_count} findings (4-5 runs)\n"
      )
      lines.append(
        f"- **Moderately Convergent**: {metrics.moderate_convergence_count} findings (3 runs)\n"
      )
      lines.append(
        f"- **Weakly Convergent**: {metrics.weak_convergence_count} findings (2 runs)\n"
      )
      lines.append(
        f"- **Non-Replicating**: {metrics.findings_replicated_1} findings (1 run only)\n"
      )
      lines.append("")
      lines.append(
        f"**Overall Replication Rate**: {metrics.overall_replication_rate:.1%}\n"
      )
    lines.append("")
    ...
    if strong_findings:
      ...
      lines.append(
        "These findings appeared in 4-5 of 5 runs and show statistical consistency.\n"
      )
      ...
      for i, match in enumerate(strong_findings, 1):
        ...
        lines.append("|--------|-------|")
        lines.append(
          f"| Replication | {match.replication_count}/{result.n_runs} runs |"
        )
        if match.effect_sizes:
          lines.append(
            f"| Effect Size | {match.effect_size_mean:.3f} +/- {match.effect_size_std:.3f} |"
          )
        if match.p_values:
          lines.append(
            f"| Significance Agreement | {match.significance_agreement:.1%} |"
          )
        if match.effect_sizes:
          lines.append(f"| Direction Agreement | {match.direction_agreement:.1%} |")
        if match.scholar_scores:
          lines.append(
            f"| ScholarEval Score | {match.scholar_score_mean:.2f} +/- {match.scholar_score_std:.2f} |"
          )
        lines.append("")
    ...
    if moderate_findings:
      ...
      for i, match in enumerate(moderate_findings, 1):
        lines.append(f"\n### {i}. {match.canonical_summary}\n")
        lines.append(
          f"- **Replication**: {match.replication_count}/{result.n_runs} runs ({match.run_indices})\n"
        )
        if match.effect_sizes:
          lines.append(
            f"- **Effect Size**: {match.effect_size_mean:.3f} +/- {match.effect_size_std:.3f}\n"
    ...
    if non_rep_findings:
      ...
      lines.append(
        "These findings appeared in only 1 run and should be treated with caution.\n"
      )
      lines.append("")
    ...
    if metrics:
      ...
      lines.append(
        f"| Significance Agreement | {metrics.avg_significance_agreement:.1%} |\n"
      )
      lines.append(f"| Direction Agreement | {metrics.avg_direction_agreement:.1%} |\n")
      lines.append(
        f"| ScholarEval Variance | {metrics.avg_scholar_score_variance:.3f} |\n"
      )
    ...
    lines.append(
      f"- **N={result.n_runs}** independent runs with seeds {[c.seed for c in result.run_configs]}\n"
    )
    if metrics:
      lines.append(f"- **Similarity Threshold**: {self._get_sim_threshold(result)}\n")
      lines.append(
        f"- **Replication Threshold**: {metrics.replication_threshold:.0%}\n"
      )
    lines.append(
      "- **Matching Algorithm**: Semantic clustering with sentence-transformers\n"
    )
  ...
  def save_report(
    ...
    if output_path is None:
      ...
      output_path = os.path.join(
        self.output_dir, f"convergence_report_{timestamp}.{ext}"
      )
