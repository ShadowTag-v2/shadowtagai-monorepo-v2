...
class LiteratureAnalyzerAgent(BaseAgent):
  ...
  def __init__(
    ...
    logger.info(
      f"Initialized {self.agent_type} (graph={self.use_knowledge_graph}, semantic={self.use_semantic_similarity})"
    )
  ...
  def _on_stop(self):
    ...
    logger.info(
      f"Agent runtime: {runtime:.2f}s, tasks completed: {self.tasks_completed}, errors: {self.errors_encountered}"
    )
  ...
  def extract_methodology(self, paper: PaperMetadata) -> dict[str, Any]:
    ...
    if (
      not methodology["experimental_methods"]
      and not methodology["computational_methods"]
    ):
