...
class SkillLoader:
  ...
  def __init__(self, skills_dir: str | None = None, auto_discover: bool = True):
    ...
    logger.info(
      f"SkillLoader initialized with {len(self.skills_cache)} skills from {self.skills_dir}"
    )
  ...
  def _find_skills_directory(self) -> str | None:
    ...
    skills_path = (
      kosmos_package_dir / "kosmos-claude-scientific-skills" / "scientific-skills"
    )
    if skills_path.exists():
  ...
  def load_skill_from_file(
    self, skill_path: Path, skill_name: str | None = None
  ) -> dict:
  ...
  def load_skills_for_task(
    ...
    return self._format_skills_for_prompt(
      loaded_skills, include_examples=include_examples
    )

  def _format_skills_for_prompt(
    self, skills: list[dict], include_examples: bool = False
  ) -> str:
    """
