...
class ExperimentDesignerAgent(BaseAgent):
  ...
  def design_experiment(
    ...
    experiment_type = self._select_experiment_type(
      hypothesis, preferred_experiment_type
    )
    ...
    try:
      ...
      for test_spec in protocol.statistical_tests:
        ...
        test_type_str = (
          test_spec.test_type.value
          if hasattr(test_spec.test_type, "value")
          else str(test_spec.test_type)
        )
        try:
          if "t_test" in test_type_str:
            required_n = power_analyzer.ttest_sample_size(
              effect_size=expected_es,
              power=test_spec.required_power,
          ...
          if required_n and (
            not protocol.sample_size or required_n > protocol.sample_size
          ):
    ...
    logger.info(
      f"Designed protocol '{protocol.name}' in {design_time:.2f}s (rigor: {rigor_score:.2f}, feasibility: {feasibility})"
    )
  ...
  def design_experiments(
    ...
    for hypothesis in hypotheses:
      try:
        ...
        logger.warning(
          f"Failed to design experiment for hypothesis {hypothesis.id}: {e}"
        )
        # Continue with remaining hypotheses

    logger.info(
      f"Designed {len(responses)} experiments for {len(hypotheses)} hypotheses"
    )
    return responses
  ...
  def _generate_from_template(
    ...
    if not template:
      logger.warning(
        f"No template found for {experiment_type.value}, falling back to LLM"
      )
  ...
  def _generate_recommendations(
    ...
    if not protocol.reproducibility_notes:
      recommendations.append(
        "Add reproducibility notes (dependencies, environment, etc.)"
      )
