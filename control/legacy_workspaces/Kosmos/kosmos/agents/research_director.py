...
class ResearchDirectorAgent(BaseAgent):
  ...
  def __init__(
    ...
    """
    super().__init__(
      agent_id=agent_id, agent_type="ResearchDirector", config=config or {}
    )
    ...
    self.convergence_detector = ConvergenceDetector(
      ...
      config={
        ...
        "cost_per_discovery_threshold": self.config.get(
          "cost_per_discovery_threshold", 1000.0
        ),
        "min_experiments_before_convergence": self.config.get(
  ...
  def _load_skills(self):
    ...
    try:
      ...
      if self.domain:
        ...
        self.skills = skill_loader.load_skills_for_task(
          task_type="research", include_examples=False, include_common=True
        )
        if self.skills:
  ...
  def _on_start(self):
    ...
    if self._start_time is None:
      ...
      logger.info(
        f"Research started at {datetime.now().isoformat()}, max runtime: {self.max_runtime_hours}h"
      )
  ...
  def _persist_protocol_to_graph(
    ...
    protocol_id: str,
    hypothesis_id: str,
    agent_name: str = "ExperimentDesignerAgent",
  ):
    """
  ...
  def _handle_error_with_recovery(
    ...
    # Log the error
    logger.error(
      f"{ERROR_RECOVERY_LOG_PREFIX} {error_source}: {error_message} (attempt {self._consecutive_errors}/{MAX_CONSECUTIVE_ERRORS})"
    )

    # Check if we've hit the circuit breaker threshold
    if self._consecutive_errors >= MAX_CONSECUTIVE_ERRORS:
      logger.error(
        f"{ERROR_RECOVERY_LOG_PREFIX} Max consecutive errors reached. Transitioning to ERROR state."
      )
    ...
    if recoverable:
      ...
      logger.info(
        f"{ERROR_RECOVERY_LOG_PREFIX} Waiting {backoff_seconds}s before retry (attempt {self._consecutive_errors + 1})"
      )
    ...
    logger.warning(
      f"{ERROR_RECOVERY_LOG_PREFIX} Non-recoverable error from {error_source}. Skipping to next action."
    )
    return None
  ...
  def _reset_error_streak(self) -> None:
    ...
    if self._consecutive_errors > 0:
      logger.debug(
        f"{ERROR_RECOVERY_LOG_PREFIX} Error streak reset (was {self._consecutive_errors} consecutive errors)"
      )
  ...
  def _handle_hypothesis_generator_response(self, message: AgentMessage):
    ...
    if message.type == MessageType.ERROR:
      recovery_action = self._handle_error_with_recovery(
        ...
        recoverable=True,
        error_details={
          "hypothesis_count_before": len(self.research_plan.hypothesis_pool)
        },
      )
      if recovery_action:
  ...
  def _handle_experiment_designer_response(self, message: AgentMessage):
    ...
    if message.type == MessageType.ERROR:
      recovery_action = self._handle_error_with_recovery(
        ...
        recoverable=True,
        error_details={
          "untested_hypotheses": len(self.research_plan.get_untested_hypotheses())
        },
      )
      if recovery_action:
        self._execute_next_action(recovery_action)
    ...
    logger.info(
      f"Received experiment design: {protocol_id} for hypothesis {hypothesis_id}"
    )
  ...
  def _handle_data_analyst_response(self, message: AgentMessage):
    ...
    logger.info(
      f"Received result interpretation for {result_id}: hypothesis {hypothesis_id} supported={hypothesis_supported}"
    )
  ...
  async def _send_to_hypothesis_generator(
    ...
    target_agent = self.agent_registry.get(
      "HypothesisGeneratorAgent", "hypothesis_generator"
    )
  ...
  async def _send_to_data_analyst(
    ...
    logger.debug(
      f"Sent interpretation request to DataAnalystAgent for result {result_id}"
    )
    return message
  ...
  async def _send_to_hypothesis_refiner(
    ...
    logger.debug(
      f"Sent {action} request to HypothesisRefiner for hypothesis {hypothesis_id}"
    )
    return message
  ...
  def _check_convergence_direct(self) -> StoppingDecision:
    ...
    try:
      ...
      with get_session() as session:
        ...
        if hyp_ids:
          db_hyps = (
            session.query(HypothesisModel).filter(HypothesisModel.id.in_(hyp_ids)).all()
          )
          hypotheses = [
            Hypothesis(
              id=h.id,
    ...
    logger.info(
      f"[CONVERGENCE] Decision: should_stop={decision.should_stop}, reason={decision.reason.value}"
    )
  ...
  def _apply_multiple_comparison_correction(self):
    ...
    if corrections_applied > 0:
      logger.info(
        f"[FDR] BH-FDR correction: {corrections_applied}/{len(p_values)} results lost significance"
      )
  ...
  async def _handle_convergence_action(self):
    ...
    if decision.should_stop:
      ...
      with self._workflow_context():
        self.workflow.transition_to(
          WorkflowState.CONVERGED, action=f"Research converged: {decision.reason.value}"
        )
      ...
      if self._actions_this_iteration > 0:
        ...
        logger.info(
          f"[ITERATION] Continuing to iteration {self.research_plan.iteration_count}"
        )
  ...
  async def _handle_generate_hypothesis_action(self):
    ...
    try:
      ...
      if count > 0:
        with self._workflow_context():
          self.workflow.transition_to(
            WorkflowState.DESIGNING_EXPERIMENTS, action=f"Generated {count} hypotheses"
          )
  ...
  async def _handle_design_experiment_action(self, hypothesis_id: str):
    ...
    try:
      ...
      logger.info(
        f"Designing experiment for hypothesis {hypothesis_id} via direct call"
      )

      response = self._experiment_designer.design_experiment(
        hypothesis_id=hypothesis_id, store_in_db=True
      )
      ...
      logger.info(
        f"Designed experiment protocol {protocol_id} for hypothesis {hypothesis_id}"
      )
  ...
  async def _handle_analyze_result_action(self, result_id: str):
    ...
    try:
      ...
      logger.info(
        f"Result {result_id} interpretation: hypothesis {hypothesis_id} supported={hypothesis_supported}"
      )
  ...
  async def _handle_refine_hypothesis_action(self, hypothesis_id: str):
    ...
    try:
      ...
      if not latest_result:
        logger.warning(
          f"No results found for hypothesis {hypothesis_id}, skipping refinement"
        )
        # Still update stats
        with self._strategy_stats_context():
          self.strategy_stats["hypothesis_refinement"]["attempts"] += 1
      ...
      if decision == RetirementDecision.RETIRE:
        ...
        refined = self._hypothesis_refiner.refine_hypothesis(
          pydantic_hyp, latest_result
        )
  ...
  async def _send_to_convergence_detector(
    ...
    """
    logger.warning(
      "[DEPRECATED] _send_to_convergence_detector is deprecated. Using direct convergence check instead (Issue #76)."
    )
    ...
    try:
      with get_session() as session:
        ...
        if hypothesis:
          ...
          related_str = (
            ", ".join(related_papers[:5]) if related_papers else "None identified"
          )
...
"""
  ...
  def execute_experiments_batch(self, protocol_ids: list[str]) -> list[dict[str, Any]]:
    ...
    try:
      ...
      return [
        {"protocol_id": pid, "success": False, "error": str(e)} for pid in protocol_ids
      ]
  ...
  async def analyze_results_concurrently(
...
"""
  ...
  def decide_next_action(self) -> NextAction:
    ...
    if current_state == WorkflowState.GENERATING_HYPOTHESES:
      ...
      if untested:
        ...
        logger.debug(
          "[DECISION] DESIGNING: No untested hypotheses but queue has %d experiments - executing",
          len(self.research_plan.experiment_queue),
        )
        ...
        logger.debug(
          "[DECISION] DESIGNING: No untested, no queue, but have %d results - analyzing",
          len(self.research_plan.results),
        )
        ...
        logger.debug(
          "[DECISION] DESIGNING: No hypotheses, queue, or results - converging"
        )
        return NextAction.CONVERGE
      ...
      if self.research_plan.experiment_queue:
        ...
        logger.debug(
          "[DECISION] EXECUTING: Queue empty but have %d results - analyzing",
          len(self.research_plan.results),
        )
        return NextAction.ANALYZE_RESULT
      else:
        ...
        logger.warning(
          "[DECISION] EXECUTING: No queue and no results - refining to recover"
        )
      ...
      if not self.research_plan.results:
        ...
        if self.research_plan.experiment_queue:
          logger.debug(
            "[DECISION] ANALYZING: Falling back to execute queued experiments"
          )
          return NextAction.EXECUTE_EXPERIMENT
        else:
    ...
    if action == NextAction.GENERATE_HYPOTHESIS:
        ...
        if self.enable_concurrent and self.async_llm_client and len(untested) > 1:
          ...
          try:
            ...
            evaluations = await asyncio.wait_for(
              self.evaluate_hypotheses_concurrently(hypothesis_batch), timeout=300
            )
            ...
            logger.warning(
              "Hypothesis evaluation timed out, falling back to sequential"
            )
            await self._handle_design_experiment_action(hypothesis_id=untested[0])
          except Exception as e:
            logger.error(f"Concurrent hypothesis evaluation failed: {e}")
      ...
      if experiment_queue:
        ...
        if (
          ...
          and self.parallel_executor
          and len(experiment_queue) > 1
        ):
        ...
        if self.enable_concurrent and self.async_llm_client and len(results) > 1:
          ...
          try:
            ...
            analyses = await asyncio.wait_for(
              self.analyze_results_concurrently(result_batch), timeout=300
            )
      ...
      self._actions_this_iteration = 0
      logger.info(
        f"[ITERATION] Completed iteration {self.research_plan.iteration_count}"
      )
      ...
      logger.info(
        f"[CONVERGE] Checking convergence at iteration {self.research_plan.iteration_count}"
      )
      await self._handle_convergence_action()
  ...
  def _should_check_convergence(self) -> bool:
    ...
    if self.research_plan.iteration_count >= self.research_plan.max_iterations:
      ...
      untested = self.research_plan.get_untested_hypotheses()
      has_testable_work = (
        len(untested) > 0 or len(self.research_plan.experiment_queue) > 0
      )
      if completed < min_exp and has_testable_work:
        logger.info(
          f"Iteration limit reached but deferring: {completed}/{min_exp} experiments, testable work remains"
        )
        return False
  ...
  def update_strategy_effectiveness(
    self, strategy: str, success: bool, cost: float = 0.0
  ):
    """
