...
class HypothesisGeneratorAgent(BaseAgent):
  ...
  def __init__(
    ...
    self.literature_search = (
      UnifiedLiteratureSearch() if self.use_literature_context else None
    )
  ...
  def generate_hypotheses(
      ...
      try:
        ...
        for hyp in validated_hypotheses:
          try:
            ...
            if (
              self.require_novelty_check
              and report.novelty_score < self.min_novelty_score
            ):
              logger.info(
                "Filtered low-novelty hypothesis (%.2f): %s",
    ...
    if validated_hypotheses:
      ...
      testability_scores = [
        ...
        if h.testability_score is not None
      ]
  ...
  def _gather_literature_context(
    ...
    try:
      ...
      papers = self.literature_search.search(
        query=query, max_results=self.max_papers_context
      )
  ...
  def _validate_hypothesis(self, hypothesis: Hypothesis) -> bool:
    ...
    try:
      ...
      if len(hypothesis.rationale) < 30:
        logger.warning(
          f"Hypothesis rationale too brief: {hypothesis.rationale[:50]}..."
        )
        return False
