...
class DataAnalystAgent(BaseAgent):
  ...
  def __init__(
    ...
    self.significance_threshold_strict = self.config.get(
      "significance_threshold_strict", 0.01
    )
...
""")
  ...
  def _parse_interpretation_response(
    ...
    try:
      ...
      anomalies = (
        self.detect_anomalies(result) if self.anomaly_detection_enabled else []
      )
  ...
  def detect_patterns_across_results(
    ...
    effect_sizes = [
      r.primary_effect_size for r in results if r.primary_effect_size is not None
    ]
  ...
  def interpret_significance(
    ...
    if p_value < 0.001:
      interpretation_parts.append(
        f"The p-value (p={p_value:.6f}) provides very strong evidence against the null hypothesis (p < 0.001)."
      )
    elif p_value < 0.01:
      interpretation_parts.append(
        f"The p-value (p={p_value:.4f}) provides strong evidence against the null hypothesis (p < 0.01)."
      )
    elif p_value < 0.05:
      interpretation_parts.append(
