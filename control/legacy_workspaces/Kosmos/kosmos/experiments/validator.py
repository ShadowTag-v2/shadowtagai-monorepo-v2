...
class ExperimentValidator:
  ...
  def validate(self, protocol: ExperimentProtocol) -> ValidationReport:
    ...
    logger.info(
      f"Validation complete: {severity} (rigor={rigor_score:.2f}, passed={checks_passed}/{len(checks)})"
    )
  ...
  def _list_bias_mitigations(self, checks: list[ValidationCheck]) -> list[str]:
    ...
    bias_checks = [
      c for c in checks if c.check_type == "bias_detection" and c.recommendation
    ]
