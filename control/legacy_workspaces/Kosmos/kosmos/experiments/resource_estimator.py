...
class ResourceEstimator:
  ...
  def _estimate_memory(
    self, experiment_type: ExperimentType, sample_size: int | None
  ) -> float:
  ...
  def _requires_gpu(
    self, experiment_type: ExperimentType, hypothesis: Hypothesis | None
  ) -> bool:
  ...
  def check_availability(
    ...
    if available_budget and requirements.estimated_cost_usd:
      if requirements.estimated_cost_usd > available_budget:
        ...
        result["blockers"].append(
          f"Cost ${requirements.estimated_cost_usd:.2f} exceeds budget ${available_budget:.2f}"
        )
      elif requirements.estimated_cost_usd > available_budget * 0.8:
        result["warnings"].append(
          f"Cost ${requirements.estimated_cost_usd:.2f} is 80%+ of budget ${available_budget:.2f}"
        )
    ...
    if requirements.can_parallelize and requirements.compute_hours > 24:
      result["recommendations"].append(
        f"Consider parallelizing across {requirements.parallelization_factor} cores to reduce duration"
      )
    ...
    if requirements.api_calls_estimated and requirements.api_calls_estimated > 100:
      ...
      result["recommendations"].append(
        f"~{requirements.api_calls_estimated} API calls estimated (~${api_cost:.2f} in API costs)"
      )
