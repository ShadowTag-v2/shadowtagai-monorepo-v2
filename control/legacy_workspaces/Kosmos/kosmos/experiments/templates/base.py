...
class TemplateMetadata(BaseModel):
  ...
  complexity_score: float = Field(
    ...
    ge=0.0,
    le=1.0,
    description="Template complexity (0=simple, 1=advanced)",
  )
  rigor_score: float = Field(
    default=0.7, ge=0.0, le=1.0, description="Expected scientific rigor"
  )
...
class TemplateBase(ABC):
  ...
  def get_applicability_score(self, hypothesis: Hypothesis) -> float:
    ...
    # Check domain match
    if (
      self.metadata.domain and self.metadata.domain.lower() == hypothesis.domain.lower()
    ):
      return 0.7
...
class TemplateRegistry:
  ...
  def register(self, template: TemplateBase, skip_validation: bool = False) -> None:
    ...
    # Add to type index
    self._templates_by_type[template.metadata.experiment_type].append(
