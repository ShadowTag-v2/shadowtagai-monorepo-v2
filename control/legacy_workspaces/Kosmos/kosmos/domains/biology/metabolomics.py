...
class MetabolomicsAnalyzer:
  ...
  def categorize_metabolite(
    self, compound_name: str, use_kegg: bool = True
  ) -> dict[str, Any]:
    """
  ...
  def analyze_pathway_pattern(
    ...
    category_filter: MetaboliteCategory | None = None,
  ) -> list[PathwayPattern]:
    """
    ...
    for (category, met_type), group in results_df.groupby(
      ["category", "metabolite_type"]
    ):
