...
class MetabolomicsAnalyzer:
    ...
    SYNTHESIS_PRODUCTS = ["AMP", "GMP", "IMP", "XMP", "CMP", "UMP", "TMP", "dAMP", "dGMP", "dCMP", "dUMP", "dTMP"]
    ...
    def categorize_metabolite(self, compound_name: str, use_kegg: bool = True) -> dict[str, Any]:
        ...
        return {"compound_name": compound_clean, "category": category, "metabolite_type": metabolite_type, "pathways": pathways}
    ...
    def _determine_category(self, compound_name: str, use_kegg: bool = True) -> MetaboliteCategory:
        ...
        # Purine patterns
        if any(pattern in compound_lower for pattern in ["adenine", "adenosine", "guanine", "guanosine", "inosine", "hypoxanthine", "xanthine"]):
            return MetaboliteCategory.PURINE

        # Pyrimidine patterns
        if any(pattern in compound_lower for pattern in ["cytosine", "cytidine", "uracil", "uridine", "thymine", "thymidine", "orotate"]):
            return MetaboliteCategory.PYRIMIDINE
        ...
        # Lipid patterns
        if any(pattern in compound_lower for pattern in ["fatty acid", "phospholipid", "sphingolipid", "cholesterol", "triglyceride"]):
            return MetaboliteCategory.LIPID

        # Carbohydrate patterns
        if any(pattern in compound_lower for pattern in ["glucose", "fructose", "galactose", "mannose", "ribose", "glycogen", "sucrose"]):
    ...
    def _determine_type(self, compound_name: str) -> MetaboliteType:
        ...
        if compound_name.endswith(("ine", "osine")) and not any(x in compound_lower for x in ["mp", "dp", "tp"]):
            return MetaboliteType.SALVAGE_PRECURSOR
        ...
        if any(pattern in compound_lower for pattern in ["uric acid", "allantoin", "urea", "beta-alanine"]):
    ...
    def analyze_group_comparison(
        ...
        if missing_group1 or missing_group2:
            raise ValueError(f"Missing samples - Group1: {missing_group1}, Group2: {missing_group2}")
    ...
    def analyze_pathway_pattern(self, results: list[MetabolomicsResult], category_filter: MetaboliteCategory | None = None) -> list[PathwayPattern]:
        """
    ...
    def compare_salvage_vs_synthesis(
        self, results: list[MetabolomicsResult], category: MetaboliteCategory = MetaboliteCategory.PURINE
        ...
        salvage = [r for r in category_results if r.metabolite_type == MetaboliteType.SALVAGE_PRECURSOR]
        synthesis = [r for r in category_results if r.metabolite_type == MetaboliteType.SYNTHESIS_PRODUCT]
