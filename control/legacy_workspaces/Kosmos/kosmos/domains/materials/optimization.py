...
class MaterialsOptimizer:
  ...
  def correlation_analysis(
    ...
    df_clean = df_clean[
      np.isfinite(df_clean[parameter]) & np.isfinite(df_clean[metric])
    ]
    ...
    logger.info(
      f"Correlation: {parameter} vs {metric}: r = {correlation:.4f}, p = {p_value:.4e} ({significance})"
    )
  ...
  def shap_analysis(
    ...
    try:
      ...
      logger.info(
        f"SHAP analysis complete: model R² = {r_squared:.4f}, top feature = {top_features[0]}"
      )
  ...
  def parameter_space_optimization(
    ...
    try:
      ...
      logger.info(
        f"Optimization complete: {'success' if optimization_success else 'failed'} after {n_iterations} iterations"
      )
