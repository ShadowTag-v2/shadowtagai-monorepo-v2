...
class NeurodegenerationResult:
      ...
      rows.append(
        {
          ...
          "temporal_stage": result.temporal_stage.value
          if result.temporal_stage
          else None,
        }
      )
  ...
  def get_summary(self) -> dict[str, Any]:
    ...
    return {
      ...
      "percent_significant": f"{self.n_significant / self.n_genes_tested * 100:.2f}%"
...
class NeurodegenerationAnalyzer:
  ...
  def differential_expression_analysis(
    ...
    control_samples = sample_metadata[
      sample_metadata[condition_column] == control_label
    ]["sample_id"].tolist()
  ...
  def cross_species_validation(
    ...
    for mouse_gene_id in common_genes:
      human_gene_id = (
        gene_mapping.get(mouse_gene_id, mouse_gene_id)
        if gene_mapping
        else mouse_gene_id
      )
  ...
  def generate_volcano_plot_data(
    ...
    for result in deg_results.gene_results:
      # -log10(p-value)
      neg_log10_p = (
        -np.log10(result.adjusted_p_value) if result.adjusted_p_value > 0 else 100
      )
