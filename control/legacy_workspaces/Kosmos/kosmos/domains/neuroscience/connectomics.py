...
class ConnectomicsResult:
  ...
  def get_scaling_summary(self) -> dict[str, Any]:
    ...
    for relationship in [
      self.length_synapses,
      self.synapses_degree,
      self.length_degree,
    ]:
      if relationship:
...
class ConnectomicsAnalyzer:
  ...
  def analyze_scaling_laws(
    ...
    if missing_cols:
      raise ValueError(
        f"Missing required columns: {missing_cols}. Available columns: {list(connectome_data.columns)}"
      )
    ...
    if len(df_clean) == 0:
      raise ValueError(
        f"No valid data after cleaning. Original size: {len(connectome_data)}, all rows had NaN or non-positive values."
      )

    # Calculate basic statistics
    mean_stats = {
      "mean_length": df_clean["Length"].mean() if "Length" in properties else None,
      "mean_synapses": df_clean["Synapses"].mean()
      if "Synapses" in properties
      else None,
  ...
  def _clean_connectome_data(
    self, data: pd.DataFrame, properties: list[str]
  ) -> pd.DataFrame:
    """
  ...
  def cross_species_comparison(
    ...
    mean_ls_exp = (
      np.mean(length_synapses_exponents) if length_synapses_exponents else None
    )
    std_ls_exp = (
      np.std(length_synapses_exponents) if length_synapses_exponents else None
    )
    mean_sd_exp = (
      np.mean(synapses_degree_exponents) if synapses_degree_exponents else None
    )
    std_sd_exp = (
      np.std(synapses_degree_exponents) if synapses_degree_exponents else None
    )
