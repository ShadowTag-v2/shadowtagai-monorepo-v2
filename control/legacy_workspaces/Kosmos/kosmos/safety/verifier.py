...
class ResultVerifier:
  ...
  def cross_validate(
    ...
    if (
      original.primary_effect_size is not None
      and replication.primary_effect_size is not None
    ):
    ...
    if (
      original.supports_hypothesis is not None
      and replication.supports_hypothesis is not None
    ):
  ...
  def detect_errors(self, result: ExperimentResult) -> list[str]:
    ...
    if hasattr(result, "metadata") and result.metadata:
      error_msg = (
        ...
        if hasattr(result.metadata, "error_message")
        else None
      )
      if error_msg:
