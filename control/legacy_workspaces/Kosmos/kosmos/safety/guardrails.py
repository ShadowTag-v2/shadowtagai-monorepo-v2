...
class SafetyGuardrails:
  ...
  def __init__(
    self, incident_log_path: str | None = None, enable_signal_handlers: bool = True
  ):
    """
  ...
  def _register_signal_handlers(self):
    ...
    def signal_handler(signum, frame):
      ...
      self.trigger_emergency_stop(
        triggered_by="signal", reason=f"Signal {signum} received"
      )
  ...
  def check_emergency_stop(self):
    ...
    if self.is_emergency_stop_active():
      raise RuntimeError(
        f"Emergency stop active (triggered by {self.emergency_stop.triggered_by}): {self.emergency_stop.reason}"
      )
  ...
  def safety_context(self, experiment_id: str | None = None):
    ...
    try:
      ...
      if self.is_emergency_stop_active():
        logger.warning(
          f"Emergency stop detected after execution (experiment: {experiment_id})"
        )

  def _log_violations(
    self, report: SafetyReport, code: str, context: dict[str, Any] | None
  ):
  ...
  def get_recent_incidents(
    ...
    if severity:
      incidents = [
        i for i in incidents if i.violation and i.violation.severity == severity
      ]
  ...
  def get_incident_summary(self) -> dict[str, Any]:
    ...
    return {
      ...
      "emergency_stops": sum(
        1 for i in self.incidents if "emergency_stop" in i.incident_id
      ),
    }
