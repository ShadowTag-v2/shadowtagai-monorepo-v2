...
class ReproducibilityManager:
  ...
  def __init__(
    ...
    logger.info(
      f"ReproducibilityManager initialized (seed={default_seed}, capture_env={capture_environment}, strict={strict_mode})"
    )
  ...
  def validate_consistency(
    ...
    if isinstance(original_result, (int, float)):
      if abs(original_result - replication_result) > tolerance:
        issues.append(
          f"Numeric results differ beyond tolerance: {original_result} vs {replication_result} (tolerance={tolerance})"
        )
  ...
  def test_determinism(
    self, experiment_function, seed: int, n_runs: int = 3, **kwargs
  ) -> bool:
    """
