...
from kosmos.cli.themes import get_domain_color, get_box_style
...
class ResultsViewer:
  ...
  def display_metrics_summary(self, metrics: dict[str, Any]):
    ...
    research_table.add_row(
      "Hypotheses Generated", str(metrics.get("hypotheses_generated", 0))
    )
    research_table.add_row(
      "Experiments Executed", str(metrics.get("experiments_executed", 0))
    )
    research_table.add_row(
      "Successful Experiments", str(metrics.get("successful_experiments", 0))
    )
    research_table.add_row(
      "Failed Experiments", str(metrics.get("failed_experiments", 0))
    )
