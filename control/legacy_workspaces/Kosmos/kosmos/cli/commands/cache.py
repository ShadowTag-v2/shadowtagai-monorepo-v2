...
def display_cache_stats(cache_manager):
  ...
  overall_table.add_row(
    "Hit Rate", create_metric_text(hit_rate / 100, format_type="percentage")
  )
...
def display_health_check(cache_manager):
  ...
  if all_healthy:
    ...
    print_error(
      "Some caches have issues. Consider running --optimize",
...
def optimize_caches(cache_manager):
  ...
  print_success(
    f"Removed {total_removed} expired entries", title="Optimization Complete"
  )
...
def clear_specific_cache(cache_manager, cache_type: str):
  ...
  print_success(
    f"{cache_type.title()} cache cleared successfully", title="Cache Cleared"
  )
