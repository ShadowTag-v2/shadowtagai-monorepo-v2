...
def show_status(
  run_id: str | None = typer.Argument(
    None, help="Research run ID to check (omit for latest)"
  ),
  watch: bool = typer.Option(
    False, "--watch", "-w", help="Watch mode - refresh every 5 seconds"
  ),
...
def display_status_live(research_data: dict, show_details: bool = False):
  ...
  console.print(
    "[info]Live status mode - refreshing every 5 seconds (Ctrl+C to exit)[/info]"
  )
  console.print()
...
def display_progress_bar(research_data: dict):
  ...
  task = progress.add_task(
    "Research Progress", total=max_iterations, completed=iteration
  )
...
def display_workflow_state(research_data: dict):
  ...
  table.add_row(
    ...
    if created_at
    else "[muted]Unknown[/muted]",
  )
  table.add_row(
    "Last Updated",
    format_timestamp(updated_at) if updated_at else "[muted]Unknown[/muted]",
  )
