...
def profile_command(
  ...
  agent_type: str | None = typer.Option(
    None, "--agent", "-a", help="Agent type to profile"
  ),
  ...
  if target not in valid_targets:
    console.print(
      f"[red]Error:[/red] Invalid target '{target}'. Must be one of: {', '.join(valid_targets)}",
      style="bold",
    )
    raise typer.Exit(1)
  ...
  if target == "experiment":
    ...
    if not agent_type:
      console.print(
        "[red]Error:[/red] --agent required for agent profiling", style="bold"
      )
      raise typer.Exit(1)
...
def _profile_experiment(
  ...
  if profile_result.function_times:
    _display_function_stats(
      profile_result.function_times, profile_result.function_calls, top_n
    )
  ...
  if compare:
    ...
    if compare_profile:
      ...
      console.print(
        f"[yellow]Warning:[/yellow] Profile {compare} not found", style="bold"
      )
...
def _profile_agent(agent_type: str, mode: str, output: Path | None, top_n: int):
  ...
  console.print(
    "[yellow]Note:[/yellow] Agent profiling not yet implemented", style="dim"
  )
...
def _profile_workflow(mode: str, output: Path | None, top_n: int):
  ...
  console.print(
    "[yellow]Note:[/yellow] Workflow profiling not yet implemented", style="dim"
  )
...
def _display_memory_timeline(snapshots):
  ...
  for i, snapshot in enumerate(snapshots[:10]):
    ...
    bar = "█" * bar_length
    console.print(
      f"{i:2d}. [{snapshot.timestamp:6.2f}s] {bar} {snapshot.current_mb:.1f} MB"
    )
...
def _display_comparison(profile1: ProfileResult, profile2: ProfileResult):
  ...
  table = Table(
    title="Profile Comparison", show_header=True, header_style="bold magenta"
  )
  ...
  mem_diff_pct = (
    (mem_diff / profile1.memory_peak_mb * 100) if profile1.memory_peak_mb > 0 else 0
  )
...
def _format_diff(
  diff: float, diff_pct: float, unit: str, lower_is_better: bool = True
) -> str:
