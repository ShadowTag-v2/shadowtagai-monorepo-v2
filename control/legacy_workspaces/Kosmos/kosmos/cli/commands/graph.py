...
def manage_graph(
  stats: bool = typer.Option(
    False, "--stats", "-s", help="Show knowledge graph statistics"
  ),
  info: bool = typer.Option(
    False, "--info", "-i", help="Show knowledge graph statistics (alias for --stats)"
  ),
  export: str | None = typer.Option(
    None, "--export", "-e", help="Export graph to JSON file"
  ),
  import_file: str | None = typer.Option(
    None, "--import", help="Import graph from JSON file"
  ),
  clear: bool = typer.Option(
    False, "--clear", "-c", help="Clear graph before import (use with --import)"
  ),
  reset: bool = typer.Option(
    False, "--reset", "-r", help="Clear all graph data (DANGEROUS)"
  ),
):
  """
...
def display_graph_stats(wm):
  ...
  console.print()
  console.print(
    f"[h2]{get_icon('graph')} Knowledge Graph Statistics[/h2]", justify="center"
  )
  console.print()
  ...
  if relationship_types:
    ...
    for rel_type, count in sorted(
      relationship_types.items(), key=lambda x: x[1], reverse=True
    ):
...
def export_graph(wm, filepath: str):
  ...
  console.print()
  console.print(
    f"[h2]{get_icon('save')} Exporting Knowledge Graph[/h2]", justify="center"
  )
  console.print()
  ...
  print_info(
    f"Use 'kosmos graph --import {filepath}' to restore this graph", title="Next Steps"
  )
...
def import_graph(wm, filepath: str, clear: bool = False):
  ...
  console.print()
  console.print(
    f"[h2]{get_icon('upload')} Importing Knowledge Graph[/h2]", justify="center"
  )
  console.print()
...
def reset_graph(wm):
  ...
  console.print()
  console.print(
    f"[h2]{get_icon('warning')} Reset Knowledge Graph[/h2]", justify="center"
  )
  console.print()
  ...
  console.print()
  print_info(
    "Graph is now empty. Run research queries to rebuild knowledge.", title="Next Steps"
  )
