...
def run_research(
  ...
  domain: str | None = typer.Option(
    ...
    "--domain",
    "-d",
    help="Research domain (biology, neuroscience, materials, etc.)",
  ),
  max_iterations: int = typer.Option(
    10, "--max-iterations", "-i", help="Maximum number of research iterations"
  ),
  budget: float | None = typer.Option(
    None, "--budget", "-b", help="Budget limit in USD"
  ),
  ...
  stream: bool = typer.Option(
    False, "--stream", "-s", help="Enable real-time event streaming display"
  ),
  stream_tokens: bool = typer.Option(
    True,
    "--stream-tokens/--no-stream-tokens",
    help="Show LLM token streaming (with --stream)",
  ),
):
  """
  ...
  if not question:
    print_error(
      "No research question provided. Use --interactive or provide a question."
    )
    raise typer.Exit(1)
...
async def run_with_progress_async(
  ...
  iteration_task = progress.add_task(
    "[bright_blue]Research progress...", total=max_iterations
  )
