...
def select_domain() -> str:
  ...
  for i, domain in enumerate(domains, 1):
    ...
    table.add_row(
      str(i), Text(domain.title(), style=color), DOMAIN_DESCRIPTIONS[domain]
    )
...
def configure_research_parameters() -> dict[str, Any]:
  ...
  console.print(
    "[muted](Automatically choose Haiku/Sonnet based on complexity)[/muted]"
  )
...
def confirm_and_start() -> bool:
  ...
  confirmed = Confirm.ask(
    "[bright_blue]Start autonomous research[/bright_blue]", default=True
  )
