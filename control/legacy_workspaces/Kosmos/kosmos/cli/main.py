...
def info():
  ...
  from kosmos.cli.utils import format_size, get_cache_dir
  from kosmos.config import get_config
...
def doctor():
  ...
  if llm_provider == "openai":
    ...
    checks.append(
      ("OpenAI API Key", "Configured" if api_key_ok else "Not set", api_key_ok)
    )
  else:
    api_key_ok = bool(os.getenv("ANTHROPIC_API_KEY"))
    checks.append(
      ("Anthropic API Key", "Configured" if api_key_ok else "Not set", api_key_ok)
    )
  ...
  if all_ok:
    print_success(
      "All checks passed! Kosmos is ready to use.", title="Diagnostics Complete"
    )
  else:
    print_error(
