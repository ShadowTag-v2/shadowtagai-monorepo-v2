...
class StreamingDisplay:
  ...
  def __init__(
    ...
    if not HAS_RICH:
      raise ImportError(
        "Rich library required for StreamingDisplay. Install with: pip install rich"
      )
  ...
  def _on_workflow_completed(self, event: WorkflowEvent) -> None:
    ...
    self.console.print(
      f"[dim]Findings:[/dim] {event.findings_count} ([green]{event.validated_count}[/green] validated)"
    )
    self.console.print()
...
class SimpleStreamingDisplay:
  ...
  def __init__(self, process_id: str | None = None):
...
def create_streaming_display(
  console: Optional["Console"] = None, process_id: str | None = None, **kwargs
) -> StreamingDisplay:
  """
