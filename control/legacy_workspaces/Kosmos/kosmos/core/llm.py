...
class ClaudeClient:
  ...
  def __init__(
    ...
    try:
      ...
      logger.info(
        f"Claude client initialized in {'CLI' if self.is_cli_mode else 'API'} mode"
      )
    ...
    try:
      ...
      if self.cache and not bypass_cache:
        ...
        if cached_response is not None:
          ...
          logger.info(
            f"Cache hit ({cached_response.get('cache_hit_type', 'exact')}): saved API call"
          )
      ...
      if stop_reason == "max_tokens":
        logger.warning(
          f"Response hit max_tokens limit ({max_tokens or self.max_tokens})"
        )
  ...
  def generate_structured(
    ...
    max_retries: int = 2,
    schema: dict[str, Any]
    | None = None,  # Alias for output_schema (provider compatibility)
  ) -> dict[str, Any]:
    """
    ...
    for attempt in range(max_retries + 1):
      ...
      try:
        ...
        if attempt < max_retries:
          ...
          logger.error(
            f"Failed to parse JSON after {e.attempts} strategies and {max_retries + 1} API calls"
          )
...
def get_client(
  reset: bool = False, use_provider_system: bool = True
