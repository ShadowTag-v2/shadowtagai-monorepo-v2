...
class ModelComplexity:
    ...
    def estimate_complexity(prompt: str, system: str | None = None) -> dict[str, Any]:
        ...
        keyword_matches = sum(1 for keyword in ModelComplexity.COMPLEX_KEYWORDS if keyword in prompt_lower)
        ...
        return {
            ...
            "reason": ("simple query" if complexity_score < 30 else "moderate complexity" if complexity_score < 60 else "high complexity task"),
        }
...
class ClaudeClient:
        ...
        try:
            ...
            if self.cache and not bypass_cache:
                ...
                if cached_response is not None:
                    ...
                    logger.info(f"Cache hit ({cached_response.get('cache_hit_type', 'exact')}): saved API call")
    ...
    def generate_structured(
        ...
        max_retries: int = 2,
        schema: dict[str, Any] | None = None,  # Alias for output_schema (provider compatibility)
    ) -> dict[str, Any]:
        """
        ...
        raise ProviderAPIError("claude", f"Invalid JSON response: {last_error.message}", raw_error=last_error, recoverable=False)
    ...
    def get_usage_stats(self) -> dict[str, Any]:
        ...
        if self.cache and self.cache_hits > 0:
            ...
            avg_input_tokens = self.total_input_tokens / self.total_requests if self.total_requests > 0 else 1000
            avg_output_tokens = self.total_output_tokens / self.total_requests if self.total_requests > 0 else 500
        ...
        if self.enable_auto_model_selection:
            ...
            if self.haiku_requests > 0 and not self.is_cli_mode:
                avg_tokens_per_request = (
                    (self.total_input_tokens + self.total_output_tokens) / self.total_requests if self.total_requests > 0 else 1500
                )
                # Estimate savings: 80% of what Sonnet would have cost
                estimated_haiku_savings = (
                    (avg_tokens_per_request / 1_000_000) * self.haiku_requests * 12 * 0.8  # ~80% savings
                )
                stats["model_selection"]["estimated_cost_saved_usd"] = round(estimated_haiku_savings, 2)
