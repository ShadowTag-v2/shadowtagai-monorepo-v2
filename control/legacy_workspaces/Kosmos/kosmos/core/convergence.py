...
class ConvergenceMetrics(BaseModel):
  ...
  novelty_trend: list[float] = Field(
    default_factory=list, description="Novelty over time"
  )
...
class ConvergenceDetector:
  ...
  def __init__(
    ...
    self.optional_criteria = optional_criteria or [
      "novelty_decline",
      "diminishing_returns",
    ]
    ...
    self.novelty_decline_window = self.config.get(
      "novelty_decline_window", min(5, max(2, max_iters - 1))
    )
    self.cost_per_discovery_threshold = self.config.get(
    ...
    logger.info(
      f"ConvergenceDetector initialized (mandatory: {self.mandatory_criteria}, optional: {self.optional_criteria})"
    )
  ...
  def check_convergence(
    ...
    if "iteration_limit" in self.mandatory_criteria:
      decision = self._check_criterion(
        "iteration_limit", research_plan, hypotheses, results, mandatory=True
      )
      if decision.should_stop:
  ...
  def calculate_novelty_decline(
    ...
    # Get novelty scores
    novelty_scores = [
      h.novelty_score for h in hypotheses if h.novelty_score is not None
    ]
  ...
  def _update_metrics(
    ...
    self.metrics.significant_results = sum(
