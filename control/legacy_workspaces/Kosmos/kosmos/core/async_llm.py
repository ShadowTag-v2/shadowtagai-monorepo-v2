...
class AsyncClaudeClient:
  ...
  def __init__(
    ...
    logger.info(
      f"Async Claude client initialized (max_concurrent={max_concurrent}, rate_limit={max_requests_per_minute}/min)"
    )
    ...
    try:
      ...
      try:
        response = await asyncio.wait_for(
          _api_call_with_retry(), timeout=timeout_seconds
        )
      except TimeoutError:
      ...
      logger.debug(
        f"Async generation completed in {execution_time:.2f}s ({len(text)} chars)"
      )
