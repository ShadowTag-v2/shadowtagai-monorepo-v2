...
class DomainRouter:
  ...
  def classify_research_question(
    ...
    try:
      ...
      logger.info(
        f"Classified to {classification.primary_domain.value} (confidence: {classification.confidence.value})"
      )
...
"""
  ...
  def route(
    ...
    logger.info(
      f"Routed to {len(selected_domains)} domain(s) using {routing_strategy} strategy"
    )
  ...
  def assess_domain_expertise(self, domain: ScientificDomain) -> DomainExpertise:
    ...
    api_score = len(capability.get_operational_apis()) / max(
      len(capability.api_clients), 1
    )
