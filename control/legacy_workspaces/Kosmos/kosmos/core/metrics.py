...
class BudgetExceededError(Exception):
  ...
  def __init__(
    ...
    super().__init__(
      message
      or f"Budget exceeded: ${current_cost:.2f} spent (limit: ${limit:.2f}, {self.usage_percent:.1f}%)"
    )
...
class MetricsCollector:
  ...
  def get_api_statistics(self) -> dict[str, Any]:
    ...
    with self._lock:
      avg_duration = (
        self.total_api_duration / self.api_calls if self.api_calls > 0 else 0
      )
  ...
  def configure_budget(
    ...
    with self._lock:
      ...
      logger.info(
        f"Budget configured: ${limit_usd or 0}/period, {limit_requests or 0} requests/period, period={period.value}"
      )
        ...
        if usage_percent >= threshold:
          ...
          if not already_triggered:
            ...
            if usage_percent >= 100:
              ...
              message = f"Budget alert: {threshold}% threshold reached. Usage: {usage:.2f}/{limit:.2f} {usage_type} ({usage_percent:.1f}%)"
