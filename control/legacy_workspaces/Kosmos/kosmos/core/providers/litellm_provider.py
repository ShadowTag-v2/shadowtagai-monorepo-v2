...
class LiteLLMProvider(LLMProvider):
  ...
  def __init__(self, config: dict[str, Any]):
    ...
    logger.info(
      f"LiteLLM provider initialized: model={self.model}, provider_type={self.provider_type}, api_base={self.api_base}"
    )
  ...
  def _build_messages(
    ...
    no_think_directive = (
      "Do not use thinking mode. Respond directly without <think> tags."
    )
  ...
  def _parse_response(self, response) -> LLMResponse:
    ...
    total_tokens = (
      getattr(usage_data, "total_tokens", input_tokens + output_tokens)
      if usage_data
      else 0
    )
...
The response must match this schema: """
    ...
    response = self.generate(
      ...
      temperature=temperature
      if temperature is not None
      else 0.3,  # Lower temp for structured output
      **kwargs,
    )
