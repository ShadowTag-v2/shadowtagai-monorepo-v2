...
class ExperimentCache:
  ...
  def __init__(
    ...
    logger.info(
      f"ExperimentCache initialized: db={self.db_path}, similarity={enable_similarity}, threshold={similarity_threshold}"
    )
    ...
    with self._lock:
      try:
        ...
        if similar_experiments:
          ...
          logger.info(
            f"Found {len(similar_experiments)} similar experiments (best match: {similar_experiments[0][1]:.3f})"
          )
