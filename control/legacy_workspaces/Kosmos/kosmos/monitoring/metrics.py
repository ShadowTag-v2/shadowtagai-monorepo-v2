...
class MetricsCollector:
  ...
  def __init__(self, registry: Any | None = None):
    ...
    self.info = Info(
      "kosmos_info", "Kosmos service information", registry=self.registry
    )
    self._set_info()
  ...
  # Research tracking
  def track_research_cycle(
    self, domain: str, status: str, duration: float | None = None
  ):
    ...
    if duration:
      self.research_duration_seconds.labels(domain=domain, status=status).observe(
        duration
      )
  ...
  def track_experiment_complete(
    ...
    self.experiment_duration_seconds.labels(experiment_type=experiment_type).observe(
      duration
    )
  ...
  def track_api_call(
    ...
    if input_tokens > 0:
      self.api_tokens_total.labels(api=api, model=model, token_type="input").inc(
        input_tokens
      )
    if output_tokens > 0:
  ...
  def update_cache_stats(
    self, cache_type: str, hit_ratio: float, size_bytes: int, entries: int
  ):
