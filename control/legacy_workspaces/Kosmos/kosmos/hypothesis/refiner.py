...
class HypothesisRefiner:
  ...
  def evaluate_hypothesis_status(
    ...
    if consecutive_failures >= self.failure_threshold:
      logger.info(
        f"Hypothesis {hypothesis.id} has {consecutive_failures} consecutive failures (threshold: {self.failure_threshold})"
      )
...
"""
  ...
  def _bayesian_confidence_update(
    ...
    # Start with prior
    prior = (
      hypothesis.confidence_score if hypothesis.confidence_score is not None else 0.5
    )
  ...
  def refine_hypothesis(
    self, hypothesis: Hypothesis, result: ExperimentResult
  ) -> Hypothesis:
    """
...
"""
      ...
      for hyp2 in hypotheses[i + 1 :]:
        ...
        if similarity >= self.similarity_threshold:
          ...
          if support1 is not None and support2 is not None and support1 != support2:
            ...
            logger.warning(
              f"Contradiction detected between {hyp1.id} and {hyp2.id} (similarity: {similarity:.2f})"
            )
  ...
  def _overall_support(self, results: list[ExperimentResult]) -> bool | None:
    ...
    supports = [
      r.supports_hypothesis for r in results if r.supports_hypothesis is not None
    ]
