...
class HypothesisPrioritizer:
  ...
  def __init__(
    ...
    self.testability_analyzer = (
      TestabilityAnalyzer() if use_testability_analyzer else None
    )
  ...
  def prioritize(
    ...
    for hyp in hypotheses:
      try:
        ...
        logger.debug(
          f"Scored hypothesis: {hyp.statement[:50]}... = {priority_score:.2f}"
        )
  ...
  def _run_analysis(self, hypothesis: Hypothesis) -> None:
    ...
    if hypothesis.novelty_score is None and self.novelty_checker:
      try:
        ...
        hypothesis.similar_work = [
          p["title"] for p in novelty_report.similar_papers[:5]
        ]
      except Exception as e:
...
{{
  ...
  def _heuristic_impact_score(self, hypothesis: Hypothesis) -> float:
    ...
    if any(
      term in statement for term in ["increase", "decrease", "improve", "reduce"]
    ) and any(char in statement for char in ["%", "fold", "factor"]):
      score += 0.15
    ...
    # Practical applications
    if any(
      term in rationale for term in ["application", "practical", "real-world", "deploy"]
    ):
      score += 0.10
    ...
    if any(
      term in statement
      for term in ["fundamental", "core", "key", "crucial", "essential"]
    ):
      score += 0.10
