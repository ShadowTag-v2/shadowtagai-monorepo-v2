...
class TestabilityAnalyzer:
  ...
  def __init__(
    self, testability_threshold: float = 0.3, use_llm_for_assessment: bool = True
  ):
    """
    ...
    logger.info(
      f"Initialized TestabilityAnalyzer with threshold={testability_threshold}"
    )
  ...
  def analyze_testability(self, hypothesis: Hypothesis) -> TestabilityReport:
    ...
    hypothesis.suggested_experiment_types = [
      exp["type"] for exp in suggested_experiments[:2]
    ]
  ...
  def _assess_basic_testability(self, hypothesis: Hypothesis) -> float:
    ...
    positive_indicators = [
      ...
      (
        r"\b(correlat|associat|affect|influence|cause)\b",
        0.10,
        "Causal/correlational claim",
      ),
    ...
    negative_indicators = [
      (
        r"\b(may|might|possibly|perhaps|potentially)\b",
        -0.15,
        "Vague/uncertain language",
      ),
      (
        r"\b(always|never|everyone|everything|all)\b",
        -0.10,
        "Absolute claim (hard to test)",
      ),
      (
  ...
  def _suggest_experiment_types(self, hypothesis: Hypothesis) -> list[dict[str, Any]]:
    ...
    experiments.append(
      {
        ...
        "feasibility": "high"
        if comp_score > 0.7
        else ("medium" if comp_score > 0.4 else "low"),
    ...
    if any(
      term in domain
      for term in ["data_science", "statistics", "epidemiology", "economics"]
    ):
      data_score += 0.1
    ...
    experiments.append(
      {
        ...
        "feasibility": "high"
        if data_score > 0.7
        else ("medium" if data_score > 0.4 else "low"),
    ...
    if any(
      ...
      lit_score += 0.3
    if any(
      term in statement for term in ["review", "synthesis", "meta-analysis", "survey"]
    ):
      lit_score += 0.3
    ...
    experiments.append(
      {
        ...
        "feasibility": "high"
        if lit_score > 0.6
        else ("medium" if lit_score > 0.3 else "low"),
  ...
  def _estimate_resources(
    ...
    if exp_type == ExperimentType.COMPUTATIONAL:
      ...
      if (
        "large" in hypothesis.statement.lower() or "big" in hypothesis.statement.lower()
      ):
        compute_hours *= 5
    ...
    # Domain-specific adjustments
    if (
      "biology" in hypothesis.domain.lower() or "medicine" in hypothesis.domain.lower()
    ):
  ...
  def _identify_challenges(self, hypothesis: Hypothesis) -> list[str]:
    ...
    if any(
      word in statement
      for word in ["maybe", "might", "possibly", "perhaps", "potentially"]
    ):
      challenges.append(
  ...
  def _llm_testability_assessment(
