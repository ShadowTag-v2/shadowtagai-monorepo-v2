...
def main(argv: Sequence[str] | None = None) -> int:
  ...
  parser.add_argument(
    ...
    default="2",
    help=(
      'The number of indent spaces or a string to be used as delimiter for indentation level e.g. 4 or "\t" (Default: 2)'
    ),
  )
  parser.add_argument(
    "--no-ensure-ascii",
  ...
  for json_file in args.filenames:
    ...
    try:
      ...
      print(
        f"Input File {json_file} is not a valid JSON, consider using check-json",
      )
      status = 1
    else:
