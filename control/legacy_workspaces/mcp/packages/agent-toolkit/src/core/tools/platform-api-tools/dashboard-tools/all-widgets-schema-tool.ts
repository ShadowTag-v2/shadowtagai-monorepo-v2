...
import { type ToolOutputType, ToolType } from "../../../tool";
...
export class AllWidgetsSchemaTool extends BaseMondayApiTool<Record<string, never>, never> {
  ...
  protected async executeInternal(): Promise<ToolOutputType<never>> {
    try {
      ...
      const res = await this.mondayApi.request<GetAllWidgetsSchemaQuery>(
        getAllWidgetsSchema,
        variables,
      );
      ...
      for (const widgetSchema of res.all_widgets_schema) {
        if (widgetSchema?.widget_type && widgetSchema?.schema) {
          const schemaObj =
            typeof widgetSchema.schema === "string"
              ? JSON.parse(widgetSchema.schema)
              : widgetSchema.schema;

          const dynamicDescription =
            schemaObj?.description ||
            schemaObj?.title ||
            `${widgetSchema.widget_type} widget for data visualization`;
