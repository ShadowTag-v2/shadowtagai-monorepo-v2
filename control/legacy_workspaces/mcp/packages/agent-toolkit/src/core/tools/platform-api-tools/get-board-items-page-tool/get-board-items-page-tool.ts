...
import { z } from "zod";
import {
  type BoardRelationValue,
  type FormulaValue,
  type GetBoardItemsPageQuery,
  type GetBoardItemsPageQueryVariables,
  ItemsOrderByDirection,
  ItemsQueryOperator,
  ItemsQueryRuleOperator,
  type SmartSearchBoardItemIdsQuery,
  type SmartSearchBoardItemIdsQueryVariables,
} from "../../../../monday-graphql/generated/graphql/graphql";
import {
  fallbackToStringifiedVersionIfNull,
  STRINGIFIED_SUFFIX,
} from "../../../../utils/microsoft-copilot.utils";
...
type Item = NonNullable<
  NonNullable<
    NonNullable<NonNullable<GetBoardItemsPageQuery["boards"]>[0]>["items_page"]
  >["items"][0]
>;
...
export const getBoardItemsPageToolSchema = {
  ...
  includeColumns: z
    ...
    .optional()
    .default(false)
    .describe(`Whether to include column values in the response.
...
export class GetBoardItemsPageTool extends BaseMondayApiTool<GetBoardItemsPageToolInput> {
  ...
  private mapItem(
    ...
    if (input.includeSubItems && "subitems" in item && item.subitems) {
      itemResult.subitems = item.subitems
        .slice(0, input.subItemLimit)
        .map((subItem) => this.mapItem(subItem!, input));
    }
  ...
  private async getItemIdsFromSmartSearchAsync(
    input: ToolInputType<GetBoardItemsPageToolInput>,
  ): Promise<number[]> {
    ...
    const itemIdsFromSmartSearch =
      smartSearchRes.search_items?.results?.map((result) => Number(result.data.id)) ?? [];
    ...
    if (itemIdsFromSmartSearch.length === 0) {
      ...
      throw new Error(
        "No items found for search term or new search is not enabled for this account",
      );
    }
