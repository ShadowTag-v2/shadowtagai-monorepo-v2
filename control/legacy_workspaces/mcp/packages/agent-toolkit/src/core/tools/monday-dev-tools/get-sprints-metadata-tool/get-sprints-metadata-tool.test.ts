...
import {
  ...
  type getSprintsMetadataToolSchema,
} from "./get-sprints-metadata-tool";
import {
...
export type InputType = Partial<z.infer<z.ZodObject<typeof getSprintsMetadataToolSchema>>> & {
...
describe("GetSprintsMetadataTool", () => {
  ...
  describe("Basic Functionality", () => {
    it("should successfully get sprints metadata with default limit", async () => {
      ...
      const itemsCall = calls.find((call) =>
        call[0].includes("query GetSprintsBoardItemsWithColumns"),
      );
