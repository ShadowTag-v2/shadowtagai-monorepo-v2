...
import {
  BaseMondayAppsTool,
  createMondayAppsAnnotations,
} from "../base-tool/base-monday-apps-tool";
import { MondayAppsToolCategory } from "../consts/apps.consts";
import { API_ENDPOINTS, HttpMethod } from "../consts/routes.consts";
import { type AppFeaturesResponse, getAppFeaturesSchema } from "./schemas/app-feature-schemas";

export class GetAppFeaturesTool extends BaseMondayAppsTool<
  typeof getAppFeaturesSchema.shape,
  AppFeaturesResponse
> {
  ...
  protected async executeInternal(
    ...
    try {
      ...
      const featuresSummary = features
        .map(
          (feature) =>
            `${feature.name} (ID: ${feature.id}, Type: ${feature.type}, State: ${feature.state})`,
        )
        .join(", ");
