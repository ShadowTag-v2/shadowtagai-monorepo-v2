...
describe("Full Board Data Tool", () => {
  ...
  it("Successfully fetches board data with items, updates, and enriches with user info", async () => {
    ...
    expect(mocks.mockApiClient.request).toHaveBeenCalledWith(
      expect.stringContaining("query getBoardData"),
      {
        boardId: "123456",
        itemsLimit: 7,
      },
    ...
    expect(mocks.mockApiClient.request).toHaveBeenCalledWith(
      expect.stringContaining("query getUsersByIds"),
      {
  ...
  it("Throws error when board is not found", async () => {
    ...
    await expect(tool.execute({ boardId: 999999 })).rejects.toThrow(
      "Board with ID 999999 not found",
    );

    // Test with null
    mocks.setResponse({ boards: null });
    await expect(tool.execute({ boardId: 888888 })).rejects.toThrow(
      "Board with ID 888888 not found",
    );
  });
