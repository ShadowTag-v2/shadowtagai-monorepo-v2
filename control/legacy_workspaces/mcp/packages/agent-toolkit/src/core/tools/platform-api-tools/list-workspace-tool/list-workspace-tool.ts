...
import { z } from "zod";
import type { ListWorkspacesQuery } from "../../../../monday-graphql/generated/graphql/graphql";
...
export const listWorkspaceToolSchema = {
  ...
  limit: z
    ...
    .describe(
      `The number of workspaces to return. Default and maximum allowed is ${DEFAULT_WORKSPACE_LIMIT}`,
    ),
  ...
  protected async executeInternal(
    ...
    if (input.searchTerm) {
      ...
      if (searchTermNormalized.length === 0) {
        throw new Error(
          "Search term did not include any alphanumeric characters. Please provide a valid search term.",
        );
      }
    }
    ...
    const shouldIncludeNoFilteringDisclaimer =
      searchTermNormalized && workspaces.length <= DEFAULT_WORKSPACE_LIMIT;
    const filteredWorkspaces = this.filterWorkspacesIfNeeded(
