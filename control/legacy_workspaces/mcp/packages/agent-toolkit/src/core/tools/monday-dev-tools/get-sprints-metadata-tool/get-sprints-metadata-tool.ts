...
import {
  ...
  type SPRINT_COLUMN_DISPLAY_NAMES,
  SPRINT_STATUS,
  type Sprint,
  validateItemColumns,
} from "../shared";
...
Requires the Main Sprints board ID of the monday-dev containing your sprints.`;
  ...
  private validateSprintsBoardSchemaFromColumns(boardColumns: Column[]): {
    isValid: boolean;
    errorMessage: string;
  } {
    ...
    if (!validation.isValid) {
      ...
      validation.missingColumns.forEach((columnId) => {
        const columnDisplayName = getSprintColumnDisplayName(
          columnId as keyof typeof SPRINT_COLUMN_DISPLAY_NAMES,
        );
