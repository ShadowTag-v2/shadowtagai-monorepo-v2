import {
  ...
  Column,
  type GetBoardInfoJustColumnsQuery,
  type GetBoardInfoQuery,
} from "../../../../monday-graphql/generated/graphql/graphql";

export type BoardInfoData = NonNullable<NonNullable<GetBoardInfoQuery["boards"]>[0]>;
export type BoardInfoJustColumnsData = NonNullable<
  NonNullable<GetBoardInfoJustColumnsQuery["boards"]>[0]
>;
...
export const formatBoardInfoAsJson = (
  ...
  return {
    ...
    filteringGuidelines: getColumnFilteringGuidelines(
      board.columns!.filter(isBaseColumnInfo) as BaseColumnInfo[],
    ),
...
export const formatBoardInfo = (
  ...
  if (board.columns && board.columns.length > 0) {
    ...
    board.columns.forEach((column, index: number) => {
      if (column) {
        sections.push(
          `${index + 1}. **${column.title || "Untitled"}** (${column.type || "unknown"})`,
        );
  ...
  if (board?.columns) {
    sections.push(
      getColumnFilteringGuidelines(board.columns!.filter(isBaseColumnInfo) as BaseColumnInfo[]),
    );
  }
...
const generateColumnsSection = (
  board: BoardInfoData | BoardInfoJustColumnsData,
  isSubItemBoard: boolean,
) => {
