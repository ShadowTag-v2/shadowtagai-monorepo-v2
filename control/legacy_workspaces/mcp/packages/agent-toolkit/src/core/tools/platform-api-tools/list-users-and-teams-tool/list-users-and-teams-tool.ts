...
import { DEFAULT_USER_LIMIT, MAX_TEAM_IDS, MAX_USER_IDS } from "./constants";
  ...
  protected async executeInternal(
    ...
    if (getMe) {
      ...
      if (!res.me) {
        return {
          content:
            "AUTHENTICATION_ERROR: Current user fetch failed. Verify API token and user permissions.",
        };
      }
    ...
    if (hasName) {
      ...
      const userList = res.users
        ...
        .map(
          (user) =>
            `• **${user!.name}** (ID: ${user!.id})${user!.title ? ` - ${user!.title}` : ""}`,
        )
        .join("\n");
    ...
    if (teamsOnly || (!hasUserIds && hasTeamIds && !includeTeams)) {
      ...
      if (includeTeamMembers) {
        ...
        res = await this.mondayApi.request<ListTeamsWithMembersQuery>(
          listTeamsWithMembers,
          variables,
        );
      } else {
