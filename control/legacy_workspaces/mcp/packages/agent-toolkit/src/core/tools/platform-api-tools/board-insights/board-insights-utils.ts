import type { ToolInputType } from "src/core/tool";
import {
  AggregateFromElementType,
  type AggregateFromTableInput,
  type AggregateGroupByElementInput,
  type AggregateSelectColumnInput,
  type AggregateSelectElementInput,
  AggregateSelectElementType,
  type AggregateSelectFunctionInput,
  AggregateSelectFunctionName,
  type ItemsQuery,
  type ItemsQueryOrderBy,
} from "src/monday-graphql/generated/graphql/graphql";
import { transformativeFunctions } from "./board-insights.consts";
import type { boardInsightsToolSchema } from "./board-insights-tool";
...
export function handleOrderBy(
  input: ToolInputType<typeof boardInsightsToolSchema>,
): ItemsQueryOrderBy[] | undefined {
...
export function handleSelectAndGroupByElements(
  ...
  const columnsWithLabelFunction = new Set<string>(
    input
      .aggregations!.filter(
        (aggregation) => aggregation.function === AggregateSelectFunctionName.Label,
      )
  ...
  const selectElements = aggregationsToBuild.map((aggregation) => {
    ...
    if (
      !groupByElements.some((groupByElement) => groupByElement.column_id === aggregation.columnId)
    ) {
      groupByElements.push({
