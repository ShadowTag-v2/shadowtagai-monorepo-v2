...
import { z } from "zod";
import { type Tool, type ToolInputType, type ToolOutputType, ToolType } from "../../tool";
...
export const manageToolsSchema = {
  ...
  toolName: z
    .string()
    .optional()
    .describe("Name of the tool to manage (required for enable/disable/status/reset)"),
};
  ...
  async execute(input?: ToolInputType<typeof manageToolsSchema>): Promise<ToolOutputType<never>> {
    ...
    switch (action) {
      ...
      case "detailed": {
        ...
        const disabledTools = Object.entries(detailedStatus).filter(
        ...
        if (enabledTools.length > 0) {
          ...
          content += enabledTools
            .map(
              ([name, status]) =>
                `  • ${name} (default: ${status.enabledByDefault ? "enabled" : "disabled"})`,
            )
            .join("\n");
        }
