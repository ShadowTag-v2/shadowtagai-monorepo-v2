...
export const getSprintColumnDisplayName = (
  columnId: keyof typeof SPRINT_COLUMN_DISPLAY_NAMES,
): string => {
...
export const getRelatedBoardIdFromRelationColumn = (
  column: NonNullable<Board["columns"]>[number],
): string | null => {
...
export const getBoardRelationColumn = (
  ...
  return (
    board.columns
      .filter((col): col is NonNullable<typeof col> => col !== null)
      .find((col) => col.id === columnId && col.type === NonDeprecatedColumnType.BoardRelation) ||
    null
  );
};
