...
import type { PeopleEntity, PeopleValue, User } from "./full-board-data.types";
...
export class FullBoardDataTool extends BaseMondayApiTool<typeof fullBoardDataToolSchema> {
  ...
  protected async executeInternal(
    ...
    try {
      ...
      const boardData = await this.mondayApi.request<GetBoardDataQuery>(
        getBoardDataQuery,
        variables,
      );
      ...
      if (userIds.size > 0) {
        ...
        const usersData = await this.mondayApi.request<GetUsersByIdsQuery>(
          getUsersByIdsQuery,
          userVariables,
        );
      ...
      const result = {
        ...
        stats: {
          ...
          total_updates: board.items_page.items.reduce(
            (sum, item) => sum + (item.updates?.length || 0),
            0,
          ),
