import { type ToolInputType, type ToolOutputType, ToolType } from "src/core/tool";
import type {
  GetBoardsQuery,
...
export const searchSchema = {
  ...
  page: z
    ...
    .optional()
    .default(1)
    .describe("The page number to get. The default value is 1."),
...
IMPORTANT: ids returned by this tool are prefixed with the type of the object (e.g doc-123, board-456, folder-789). When passing the ids to other tools, you need to remove the prefix and just pass the number.
  ...
  private async searchFoldersAsync(
    ...
    const data = this.searchAndVirtuallyPaginate(
      input,
      response.folders || [],
      (folder) => folder!.name,
    );
  ...
  private getPagingParamsForSearch(input: ToolInputType<SearchToolInput>): {
    page: number;
    limit: number;
  } {
    return {
