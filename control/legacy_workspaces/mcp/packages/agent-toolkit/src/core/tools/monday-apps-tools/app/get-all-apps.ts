...
import type { AppApiDataResponse } from "./schemas/app-schemas";
...
export class GetAllAppsTool extends BaseMondayAppsTool<undefined, AppApiDataResponse> {
  ...
  protected async executeInternal(
    _input?: ToolInputType<undefined>,
  ): Promise<ToolOutputType<AppApiDataResponse>> {
    try {
      const response = await this.executeApiRequest<AppApiDataResponse>(
        HttpMethod.GET,
        API_ENDPOINTS.APPS.GET_ALL,
      );
