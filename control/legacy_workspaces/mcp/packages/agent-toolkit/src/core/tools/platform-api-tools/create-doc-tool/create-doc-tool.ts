...
import { z } from "zod";
import {
  type AddContentToDocFromMarkdownMutation,
  type AddContentToDocFromMarkdownMutationVariables,
  BoardKind,
  type CreateColumnMutation,
  type CreateColumnMutationVariables,
  type CreateDocMutation,
  type CreateDocMutationVariables,
  type GetItemBoardQuery,
  type GetItemBoardQueryVariables,
  type UpdateDocNameMutation,
  type UpdateDocNameMutationVariables,
...
const CreateDocLocationSchema = z.discriminatedUnion("type", [
  z.object({
    ...
    doc_kind: z
      ...
      .optional()
      .describe("Document kind (public/private/share). Defaults to public."),
    folder_id: z
      .number()
      .optional()
      .describe("Optional folder ID to place the document inside a specific folder"),
  }),
  z.object({
...
export const createDocToolSchema = {
  ...
  markdown: z
    .string()
    .describe("Markdown content that will be imported into the newly created document as blocks."),
  location: z
    .enum(["workspace", "item"])
    .describe(
...
- Item doc: { location: "item", item_id: 456, column_id: "doc_col_1" , markdown: "..." }`;
  ...
  protected async executeInternal(
    input: ToolInputType<typeof createDocToolSchema>,
    ...
    if (!inputParsingResult.success) {
      return {
        content: `Required parameters were not provided for location parameter of ${input.location}`,
      };
    }
    ...
    try {
      ...
      if (parsedInput.type === DocType.enum.workspace) {
        ...
        const existingDocColumn = item.board?.columns?.find(
          (c) => c && c.type === NonDeprecatedColumnType.Doc,
        );
        ...
        if (!columnId) {
          if (existingDocColumn) {
            ...
            const columnRes: CreateColumnMutation = await this.mondayApi.request(
        ...
        const res: CreateDocMutation = await this.mondayApi.request(
          createDocMutation,
          itemVariables,
        );
        docId = res.create_doc?.id ?? undefined;
        ...
        if (input.doc_name && docId) {
          try {
            ...
            const updateRes: UpdateDocNameMutation = await this.mondayApi.request(
              updateDocName,
              updateVariables,
            );
          } catch (updateError) {
