...
export const createColumnToolSchema = {
  columnType: z
    ...
    .describe("The type of the column to be created"),
...
export type CreateColumnToolInput =
  | typeof createColumnToolSchema
...
export class CreateColumnTool extends BaseMondayApiTool<CreateColumnToolInput> {
  ...
  protected async executeInternal(
    input: ToolInputType<CreateColumnToolInput>,
  ): Promise<ToolOutputType<never>> {
    const boardId =
      this.context?.boardId ??
      (input as ToolInputType<typeof createColumnInBoardToolSchema>).boardId;
    ...
    const variables: CreateColumnMutationVariables = {
      ...
      columnSettings:
        typeof input.columnSettings === "string"
          ? JSON.parse(input.columnSettings)
