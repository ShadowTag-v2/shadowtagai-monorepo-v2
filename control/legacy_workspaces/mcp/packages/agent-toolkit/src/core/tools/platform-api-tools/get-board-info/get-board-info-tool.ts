...
export class GetBoardInfoTool extends BaseMondayApiTool<typeof getBoardInfoToolSchema | undefined> {
  ...
  private async getSubItemsBoardAsync(
    board: BoardInfoData,
  ): Promise<BoardInfoJustColumnsData | null> {
    const subTasksColumn = board.columns?.find(
      (column) => column?.type === NonDeprecatedColumnType.Subtasks,
    );
    if (!subTasksColumn) {
      return null;
    }
    ...
    const response = await this.mondayApi.request<GetBoardInfoJustColumnsQuery>(
      getBoardInfoJustColumns,
