import type { GetWorkspaceInfoQuery } from "src/monday-graphql/generated/graphql/graphql";
import type { OrganizedWorkspaceInfo } from "./types";
...
export function organizeWorkspaceInfoHierarchy(
    ...
    (folders || [])
      .filter(
        (folder): folder is NonNullable<typeof folder> =>
          folder != null && folder.id != null && folder.name != null,
      )
      .map((folder) => [
        folder.id!,
        {
  ...
  (boards || [])
    .filter(
      (board): board is NonNullable<typeof board> =>
        board != null && board.id != null && board.name != null,
    )
    .forEach((board) => {
  ...
  (docs || [])
    .filter(
      (doc): doc is NonNullable<typeof doc> => doc != null && doc.id != null && doc.name != null,
    )
    .forEach((doc) => {
