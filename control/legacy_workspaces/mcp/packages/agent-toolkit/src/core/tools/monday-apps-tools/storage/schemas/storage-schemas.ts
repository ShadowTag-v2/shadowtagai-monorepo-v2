import { z } from "zod";
import type { MondayApiResponse } from "../../base-tool/base-monday-apps-tool";
...
export const searchStorageRecordsSchema = z.object({
  appId: z
    .number()
    .describe(
      "The unique identifier of the app whose storage you want to search. Get this from get_all_apps",
    ),
  accountId: z
    .number()
    .describe(
...
export const exportStorageDataSchema = z.object({
  appId: z
    .number()
    .describe(
      "The unique identifier of the app whose storage data you want to export. Get this from get_all_apps",
    ),
  accountId: z
    .number()
    .describe(
      "The monday.com account ID to export storage from. Each account has isolated storage data",
    ),
  fileFormat: z
    .enum(["JSON", "CSV"])
    .optional()
