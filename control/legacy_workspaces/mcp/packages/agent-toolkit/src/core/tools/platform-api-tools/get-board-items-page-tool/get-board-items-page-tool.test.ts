...
import {
  ...
  createMockApiClient,
} from "../test-utils/mock-api-client";
import {
  GetBoardItemsPageTool,
  type GetBoardItemsPageToolInput,
...
describe("GetBoardItemsPageTool", () => {
  ...
  describe("getItemIdsFromSmartSearchAsync integration", () => {
    it("should call mockRequest with itemIds from smart search when no initial itemIds are provided", async () => {
      ...
      const getBoardItemsPageCall = calls.find((call) =>
        call[0].includes("query GetBoardItemsPage"),
      );
      expect(getBoardItemsPageCall).toBeDefined();
      expect(getBoardItemsPageCall[1].queryParams.ids).toEqual(
        smartSearchItemIds.map((id) => id.toString()),
      );
    });
    ...
    it("should call mockRequest with intersection of itemIds and smart search results", async () => {
      ...
      expect(getBoardItemsPageCall).toBeDefined();
      expect(getBoardItemsPageCall[1].queryParams.ids).toEqual(
        expectedIds.map((id) => id.toString()),
      );
    });
                      ...
                      linked_items: [
                        {
                          id: "123",
                          name: "Linked Item 1",
                          board: { id: "999", name: "Linked Board" },
                        },
                        {
                          id: "456",
                          name: "Linked Item 2",
