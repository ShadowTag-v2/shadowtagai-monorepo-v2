import {
  ObjectType,
  type UpdateBoardHierarchyMutation,
  type UpdateFolderMutation,
  type UpdateOverviewHierarchyMutation,
...
export const moveObjectToolSchema = {
  ...
  position_is_after: z
    .boolean()
    .optional()
    .describe("Whether to position the object after the object"),
  parentFolderId: z
    .string()
    .optional()
...
export class MoveObjectTool extends BaseMondayApiTool<MoveObjectToolInput> {
  ...
  private async executeUpdateFolder(
  ...
  private async executeUpdateBoardHierarchy(
    input: ToolInputType<MoveObjectToolInput>,
  ): Promise<ToolOutputType<never>> {
    const {
      id,
    ...
    const res = await this.mondayApi.request<UpdateBoardHierarchyMutation>(
  ...
  private async executeUpdateOverviewHierarchy(
    ...
    const res = await this.mondayApi.request<UpdateOverviewHierarchyMutation>(
