import {
  DocsOrderBy,
  type ReadDocsQuery,
  type ReadDocsQueryVariables,
} from "src/monday-graphql/generated/graphql/graphql";
import { exportMarkdownFromDoc, readDocs } from "src/monday-graphql/queries.graphql";
import { z } from "zod";
...
export const readDocsToolSchema = {
  type: QueryByIdEnum.describe(
    "Query type of ids parameter that is used query by: ids, object_ids, or workspace_ids",
  ),
  ids: z
    .array(z.string())
    .min(1)
    .describe("Array of ID values for this query type (at least 1 required)"),
  limit: z
    .number()
    .optional()
    .describe(
...
- For large searches: start with page 1, then paginate if has_more_pages=true`;
  ...
  protected async executeInternal(
    input: ToolInputType<typeof readDocsToolSchema>,
      ...
      docs
        ...
        .map(async (doc) => {
          ...
          try {
            ...
            if (
              markdownRes.export_markdown_from_doc.success &&
              markdownRes.export_markdown_from_doc.markdown
            ) {
