...
export type DeleteColumnToolInput =
  | typeof deleteColumnToolSchema
  | typeof deleteColumnInBoardToolSchema;
...
export class DeleteColumnTool extends BaseMondayApiTool<DeleteColumnToolInput> {
  ...
  protected async executeInternal(
    input: ToolInputType<DeleteColumnToolInput>,
  ): Promise<ToolOutputType<never>> {
    const boardId =
      this.context?.boardId ??
      (input as ToolInputType<typeof deleteColumnInBoardToolSchema>).boardId;
