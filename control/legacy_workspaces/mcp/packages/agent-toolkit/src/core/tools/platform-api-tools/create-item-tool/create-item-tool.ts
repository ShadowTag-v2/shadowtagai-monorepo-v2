...
export class CreateItemTool extends BaseMondayApiTool<CreateItemToolInput> {
  ...
  protected async executeInternal(
    ...
    const boardId =
      this.context?.boardId ?? (input as ToolInputType<typeof createItemInBoardToolSchema>).boardId;
  ...
  private async duplicateAndUpdateItem(
    ...
    try {
      ...
      const duplicateRes = await this.mondayApi.request<DuplicateItemMutation>(
        duplicateItem,
        duplicateVariables,
      );
