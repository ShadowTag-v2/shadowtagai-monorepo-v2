...
export const updateWorkspaceToolSchema = {
  ...
  attributeAccountProductId: z
    .number()
    .optional()
    .describe("The target account product's ID to move the workspace to"),
  attributeDescription: z
    .string()
    .optional()
    .describe("The description of the workspace to update"),
  attributeKind: z
...
export class UpdateWorkspaceTool extends BaseMondayApiTool<UpdateWorkspaceToolInput> {
  ...
  protected async executeInternal(
    input: ToolInputType<UpdateWorkspaceToolInput>,
    ...
    const res = await this.mondayApi.request<{ update_workspace: { id: string } }>(
      updateWorkspace,
      variables,
    );
