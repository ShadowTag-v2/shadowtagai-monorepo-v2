...
`;
  ...
  protected async executeInternal(
    ...
    try {
      ...
      const documentContent = await this.readSprintSummaryDocument(
        sprintMetadata.documentObjectId!,
      );
  ...
  private async readSprintSummaryDocument(documentObjectId: string) {
    try {
      ...
      const docsResponse = await this.mondayApi.request<ReadDocsQuery>(
        readSprintSummaryDocs,
        readDocsVariables,
      );
