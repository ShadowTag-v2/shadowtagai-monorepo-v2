...
import {
  ...
  handleFrom,
  handleOrderBy,
  handleSelectAndGroupByElements,
} from "./board-insights-utils";
  ...
  describe("Utility Functions", () => {
    ...
    describe("handleSelectAndGroupByElements", () => {
      ...
      it("should handle mixed aggregations with group by and add label function", () => {
        ...
        expect(result.selectElements[2].function?.function).toEqual(
      ...
      it("should add select elements for group by columns not in aggregations", () => {
        ...
        expect(
          result.selectElements.some(
            (el) => el.type === AggregateSelectElementType.Column && el.as === "status",
          ),
        ).toBe(true);
        expect(
          result.selectElements.some(
            (el) => el.type === AggregateSelectElementType.Column && el.as === "priority",
          ),
        ).toBe(true);
