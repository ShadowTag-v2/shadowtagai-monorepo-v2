...
function filterAndEnhanceTools(
  tools: ReturnType<AtpClient["getATPTools"]>,
...
export async function runAtpMcpServer(config: AtpServerConfig): Promise<void> {
  ...
  const { createServer, AgentToolProtocolClient, ToolNames, registerToolsWithMCP } =
    await loadAtpDependencies();
