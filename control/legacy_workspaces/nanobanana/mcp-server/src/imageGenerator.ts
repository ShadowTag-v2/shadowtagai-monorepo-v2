...
export class ImageGenerator {
  ...
  constructor(authConfig: AuthConfig) {
    ...
    this.modelName = process.env.NANOBANANA_MODEL || ImageGenerator.DEFAULT_MODEL;
  ...
  private async handlePreview(files: string[], request: ImageGenerationRequest): Promise<void> {
  ...
  private buildBatchPrompts(request: ImageGenerationRequest): string[] {
    ...
    if (prompts.length === 0 && request.outputCount && request.outputCount > 1) {
