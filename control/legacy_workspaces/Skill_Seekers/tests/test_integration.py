...
from skill_seekers.cli.doc_scraper import (
  DocToSkillConverter,
  load_config,
  validate_config,
)
...
class TestRealConfigFiles(unittest.TestCase):
  ...
  def test_godot_config(self):
    ...
    if os.path.exists(config_path):
      ...
      self.assertEqual(
        len(errors), 0, f"Godot config should be valid, got errors: {errors}"
      )
  ...
  def test_react_config(self):
    ...
    if os.path.exists(config_path):
      ...
      errors, _ = validate_config(config)
      self.assertEqual(
        len(errors), 0, f"React config should be valid, got errors: {errors}"
      )
  ...
  def test_vue_config(self):
    ...
    if os.path.exists(config_path):
      ...
      errors, _ = validate_config(config)
      self.assertEqual(
        len(errors), 0, f"Vue config should be valid, got errors: {errors}"
      )
  ...
  def test_django_config(self):
    ...
    if os.path.exists(config_path):
      ...
      errors, _ = validate_config(config)
      self.assertEqual(
        len(errors), 0, f"Django config should be valid, got errors: {errors}"
      )
...
"""
  ...
  def test_full_llms_txt_workflow(self):
    ...
    with patch("cli.llms_txt_downloader.requests.get") as mock_get:
      ...
      self.assertGreater(
        len(scraper.pages), 0, "Should have parsed pages from llms.txt"
      )
      ...
      self.assertTrue(
        all("content" in page for page in scraper.pages),
      ...
      self.assertIn(
        self.config["name"], skill_content, "SKILL.md should contain skill name"
      )
      self.assertGreater(
        len(skill_content), 100, "SKILL.md should have substantial content"
      )
  ...
  def test_multi_variant_download(self):
    ...
    with (
      ...
      # Run scraper
      from skill_seekers.cli.doc_scraper import (
        DocToSkillConverter as DocumentationScraper,
      )
      ...
      self.assertTrue(
        (refs_dir / "llms-small.md").exists(), "llms-small.md should exist"
      )
