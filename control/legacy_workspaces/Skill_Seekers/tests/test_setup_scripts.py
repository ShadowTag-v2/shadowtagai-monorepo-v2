...
class TestSetupMCPScript:
  ...
  def test_references_correct_mcp_directory(self, script_content):
    ...
    assert len(new_refs) >= 6, (
      f"Expected at least 6 references to 'src/skill_seekers/mcp/', found {len(new_refs)}"
    )
  ...
  def test_requirements_txt_path(self, script_content):
    ...
    old_skill_seeker_refs = re.findall(
      r"skill_seeker_mcp/requirements\.txt", script_content
    )
    old_mcp_refs = re.findall(
      r"(?<!skill_seeker_)mcp/requirements\.txt", script_content
    )
  ...
  def test_server_py_path(self, script_content):
    ...
    old_mcp_refs = re.findall(
      r"(?<!/)(?<!skill_seekers/)mcp/server\.py", script_content
    )
  ...
  def test_no_hardcoded_paths(self, script_content):
    ...
    assert len(hardcoded_paths) == 0, (
      f"Found hardcoded absolute paths: {hardcoded_paths}"
    )
...
def test_mcp_directory_structure():
  ...
  assert (mcp_dir / "__init__.py").exists(), (
    "src/skill_seekers/mcp/__init__.py should exist"
  )
