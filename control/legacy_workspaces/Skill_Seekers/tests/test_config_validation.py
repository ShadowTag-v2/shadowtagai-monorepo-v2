...
class TestConfigValidation(unittest.TestCase):
  ...
  def test_valid_complete_config(self):
    ...
    errors, _ = validate_config(config)
    self.assertEqual(
      len(errors), 0, f"Valid config should have no errors, got: {errors}"
    )
  ...
  def test_valid_url_protocols(self):
    ...
    for protocol in ["http://", "https://"]:
      ...
      errors, _ = validate_config(config)
      url_errors = [
        e for e in errors if "base_url" in e.lower() and "invalid" in e.lower()
      ]
  ...
  def test_invalid_selectors_not_dict(self):
    ...
    self.assertTrue(
      any(
        "selectors" in error.lower() and "dictionary" in error.lower()
        for error in errors
      )
    )
  ...
  def test_invalid_categories_not_dict(self):
    ...
    self.assertTrue(
      any(
        "categories" in error.lower() and "dictionary" in error.lower()
        for error in errors
      )
    )
  ...
  def test_invalid_category_keywords_not_list(self):
    ...
    self.assertTrue(
      any(
        "getting_started" in error.lower() and "list" in error.lower()
        for error in errors
      )
    )
