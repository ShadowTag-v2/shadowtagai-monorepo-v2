...
def run_specific_suite(suite_name):
    ...
    suite_map = {"config": "tests.test_config_validation", "features": "tests.test_scraper_features", "integration": "tests.test_integration"}
...
def print_summary(result):
    ...
    if total > 0:
        ...
        color = (
            ColoredTextTestResult.GREEN if success_rate == 100 else ColoredTextTestResult.YELLOW if success_rate >= 80 else ColoredTextTestResult.RED
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run tests for Skill Seeker", formatter_class=argparse.RawDescriptionHelpFormatter)
