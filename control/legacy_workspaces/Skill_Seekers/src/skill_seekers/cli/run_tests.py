...
def print_summary(result):
  ...
  if skipped > 0:
    print(
      f"{ColoredTextTestResult.YELLOW}⊘ Skipped:{ColoredTextTestResult.RESET} {skipped}"
    )
...
def main():
  ...
  parser.add_argument(
    "--quiet", "-q", action="store_true", help="Quiet output (minimal output)"
  )
  parser.add_argument(
  ...
  print(
    f"{ColoredTextTestResult.BOLD}SKILL SEEKER TEST SUITE{ColoredTextTestResult.RESET}"
  )
  ...
  if args.suite:
    ...
    print(
      f"Running {ColoredTextTestResult.BLUE}all tests{ColoredTextTestResult.RESET}\n"
    )
