  ...
  def create_router_config(self) -> dict[str, Any]:
    ...
    router_config = {
      ...
      "description": self.base_config.get(
        "description", f"{self.router_name.title()} documentation router"
      ),
...
Examples:
  ...
  parser.add_argument(
    "--name", help="Router skill name (default: inferred from sub-skills)"
  )

  parser.add_argument(
    "--output-dir", help="Output directory (default: same as input configs)"
  )
