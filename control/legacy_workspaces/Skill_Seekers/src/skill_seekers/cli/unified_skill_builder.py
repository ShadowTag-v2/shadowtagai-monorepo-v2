...
"""
    ...
    for source in self.config.get("sources", []):
      ...
      if source_type == "documentation":
        ...
        content += (
          f"  - Code Analysis: {source.get('code_analysis_depth', 'surface')}\n"
        )
    ...
    if self.conflicts:
      ...
      for conflict in self.conflicts:
        ctype = (
          conflict.type
          if hasattr(conflict, "type")
          else conflict.get("type", "unknown")
        )
  ...
  def _format_merged_apis(self) -> str:
    ...
    if code_only:
      content += "\n### 💻 Undocumented APIs\n\n"
      content += (
        f"*Found in code but not in documentation ({len(code_only)} total)*\n\n"
      )
  ...
  def _generate_conflicts_report(self):
    ...
    with open(conflicts_path, "w") as f:
      ...
      high = [
        ...
        if (hasattr(c, "severity") and c.severity == "high")
        or c.get("severity") == "high"
      ]
      medium = [
        c
        for c in self.conflicts
      ...
      low = [
        c
        for c in self.conflicts
        if (hasattr(c, "severity") and c.severity == "low")
        or c.get("severity") == "low"
      ]
