...
class QualityReport:
  ...
  def add_warning(
    self, category: str, message: str, file: str = None, line: int = None
  ):
...
class SkillQualityChecker:
  ...
  def __init__(self, skill_dir: Path):
    ...
    self.report = QualityReport(
      skill_name=self.skill_dir.name, skill_path=self.skill_dir
    )
  ...
  def _check_skill_structure(self):
    ...
    if not self.skill_md_path.exists():
      self.report.add_error(
        "structure", "SKILL.md file not found", str(self.skill_md_path)
      )
      return
  ...
  def _check_enhancement_quality(self):
    ...
    if code_blocks == 0:
      ...
      self.report.add_info(
        "enhancement", f"✓ Found {code_blocks} code examples", "SKILL.md"
      )
...
def print_report(report: QualityReport, verbose: bool = False):
  ...
  # Quality score
  print(
    f"Quality Score: {report.quality_score:.1f}/100 (Grade: {report.quality_grade})"
  )
  print()
...
""",
  ...
  parser.add_argument(
    "skill_directory", help="Path to skill directory (e.g., output/react/)"
  )

  parser.add_argument(
    "--verbose", "-v", action="store_true", help="Show all info messages"
  )
