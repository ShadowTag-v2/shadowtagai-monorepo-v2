...
class GitHubScraper:
  ...
  def scrape(self) -> dict[str, Any]:
    ...
    try:
      ...
      logger.error(
        "GitHub API rate limit exceeded. Please wait or use authentication token."
      )
      raise
...
"""
  ...
  def _format_languages(self) -> str:
    ...
    for lang, info in sorted(
      languages.items(), key=lambda x: x[1]["bytes"], reverse=True
    ):
...
Examples:
  ...
  parser.add_argument(
    "--scrape-only", action="store_true", help="Only scrape, don't build skill"
  )
  ...
  try:
    ...
    logger.info(
      f"Next step: skill-seekers-package output/{config.get('name', config['repo'].split('/')[-1])}/"
    )
