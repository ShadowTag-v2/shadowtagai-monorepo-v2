...
class ConfigValidator:
  ...
  def _validate_documentation_source(self, source: dict[str, Any], index: int):
    ...
    if "base_url" not in source:
      raise ValueError(
        f"Source {index} (documentation): Missing required field 'base_url'"
      )
    ...
    if "max_pages" in source and not isinstance(source["max_pages"], int):
      raise ValueError(
        f"Source {index} (documentation): 'max_pages' must be an integer"
      )
  ...
  def _validate_github_source(self, source: dict[str, Any], index: int):
    ...
    if "code_analysis_depth" in source:
      ...
      if depth not in self.VALID_DEPTH_LEVELS:
        raise ValueError(
          f"Source {index} (github): Invalid code_analysis_depth '{depth}'. Must be one of {self.VALID_DEPTH_LEVELS}"
        )
  ...
  def _validate_legacy(self) -> bool:
    ...
    if "base_url" in self.config:
      ...
      raise ValueError(
        "Cannot detect legacy config type (missing base_url, repo, or pdf)"
      )
