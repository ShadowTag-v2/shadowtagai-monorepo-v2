...
class UnifiedScraper:
  ...
  def _scrape_github(self, source: dict[str, Any]):
    ...
    github_config = {
      ...
      "local_repo_path": source.get(
        "local_repo_path"
      ),  # Pass local_repo_path from config
    }
  ...
  def build_skill(self, merged_data: dict | None = None):
    ...
    builder = UnifiedSkillBuilder(
      self.config, self.scraped_data, merged_data, conflicts
    )
...
Examples:
  ...
  parser.add_argument(
    "--config", "-c", required=True, help="Path to unified config JSON file"
  )
  parser.add_argument(
    "--merge-mode",
    "-m",
