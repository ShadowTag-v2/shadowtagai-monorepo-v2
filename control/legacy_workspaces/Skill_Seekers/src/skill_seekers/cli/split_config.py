...
class ConfigSplitter:
  ...
  def __init__(
    self, config_path: str, strategy: str = "auto", target_pages: int = 5000
  ):
  ...
  def get_split_strategy(self) -> str:
    ...
    if self.strategy == "auto":
      ...
      if max_pages < 5000:
        ...
        print(
          f"ℹ️  Medium documentation ({max_pages} pages) - category split recommended"
        )
        return "category"
  ...
  def split_by_size(self) -> list[dict[str, Any]]:
    ...
    print(
      f"✅ Created {len(configs)} size-based configs ({self.target_pages} pages each)"
    )
    return configs
  ...
  def save_configs(
    self, configs: list[dict[str, Any]], output_dir: Path = None
  ) -> list[Path]:
