...
class DocToSkillConverter:
  ...
  def extract_patterns(
    ...
    for elem in main.find_all(["p", "div"]):
      ...
      if any(
        word in text for word in ["example:", "pattern:", "usage:", "typical use"]
      ):
  ...
  def scrape_all(self) -> None:
    ...
    if self.dry_run:
      ...
      if len(self.visited_urls) >= preview_limit:
        logger.info(
          "   (showing first %d, actual scraping may find more)", preview_limit
        )
  ...
  def smart_categorize(
    ...
    categories: dict[str, list[dict[str, Any]]] = {
      cat: [] for cat in category_defs.keys()
    }
    categories["other"] = []
  ...
  def create_enhanced_skill_md(
    ...
    description = self.config.get(
      "description", f"Comprehensive assistance with {self.name}"
    )
...
"""
    ...
    if quick_ref:
      for i, pattern in enumerate(quick_ref[:8], 1):
        content += (
          f"**Pattern {i}:** {pattern.get('description', 'Example pattern')}\n\n"
        )
        content += "```\n"
...
def execute_enhancement(config: dict[str, Any], args: argparse.Namespace) -> None:
  ...
  if not args.enhance and not args.enhance_local:
    ...
    logger.info(
      "  API-based:            skill-seekers-enhance-api output/%s/", config["name"]
    )
