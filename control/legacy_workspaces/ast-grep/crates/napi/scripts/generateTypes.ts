...
async function fileExists(filePath: string): Promise<boolean> {
  try {
    ...
    // oxlint-disable-line eslint/no-unused-vars
    return false;
  }
}
...
function processNodeTypes(nodeTypes: NodeType[]): Record<string, NodeType> {
  const filteredNodeTypes = nodeTypes.map(filterOutUnNamedNode).filter((node) => !!node);
...
async function generateLangNodeTypes() {
  ...
  const languageCargoToml = await readFile(path.resolve(rootDir, "../language/Cargo.toml"), "utf8");
