...
test.before(async () => {
  ...
  const parserPath = require.resolve("tree-sitter-javascript/tree-sitter-javascript.wasm");
...
test("findAll", (t) => {
  const sg = parse("javascript", "console.log(123); let a = console.log.bind(console);");
...
test("dumpPattern with selector", (t) => {
  ...
  const dump = wasm.dumpPattern("javascript", "class A { $F = $I }", "field_definition");
...
test("parse multiple languages simultaneously", async (t) => {
  const pythonPath = require.resolve("tree-sitter-python/tree-sitter-python.wasm");
  await wasm.registerDynamicLanguage({
