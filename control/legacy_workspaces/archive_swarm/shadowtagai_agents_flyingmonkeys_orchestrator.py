...
class CodeGenerationTask:
    ...
    specialist_required: Optional[str] = None  # e.g., "python", "kubernetes"
...
class n-autoresearch/Kosmos/BioAgentsOrchestrator:
    ...
    def __init__(
        ...
        print(f"   Cofounder context: ${self.platform_context.valuation_2030/1e9:.1f}B valuation by 2030")
    ...
    async def initialize_agents(self):
        ...
        self.shift_manager.assign_agents_to_shifts(
            [a.agent_id for a in self.agents],
            agent_levels
        )

        print(f"\n✅ {len(self.agents)} agents initialized")
        print(f"   Specialists: {len([a for a in self.agents if a.knowledge_graph.get('agent_type') == 'specialist'])}")
        print(f"   Generalists: {len([a for a in self.agents if a.knowledge_graph.get('agent_type') == 'generalist'])}")
    ...
    async def jury_deliberation(
        ...
        for agent_id in specialist_agents[:5]:
            comment = f"I think Option {specialist_agents.index(agent_id) % 4 + 65} is best because..."
    ...
    async def _call_gemini_api(
        self,
        prompt: str,
        repl_env: Optional[Dict[str, Any]] = None
    ) -> str:
        """
