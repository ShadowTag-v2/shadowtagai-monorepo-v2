...
class AsyncSwarmVoter:
    ...
    async def _collect_parallel_votes(
        ...
        vote_coros = [
            self._async_heuristic_vote(agent, context)
    ...
    async def batch_vote(
        ...
        """
        vote_coros = [
            self.vote(intent, risk, brakes)
            for intent, risk, brakes in decisions
        ]
...
_async_voter: Optional[AsyncSwarmVoter] = None
...
async def main():
    ...
    for intent, risk, brakes in test_cases:
        ...
        print(f"  Timing: {result.total_latency_ms:.2f}ms "
