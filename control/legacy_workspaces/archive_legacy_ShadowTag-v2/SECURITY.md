...
AiYou FastAPI Services is built with **security-first architecture**. This document outlines security features, best practices, and deployment guidelines.
...
```bash
# Build
docker build -t aiyou-api:latest .
...
# Build
docker build -t gcr.io/YOUR_PROJECT_ID/aiyou-api:v1.0.0 .

# Push
docker push gcr.io/YOUR_PROJECT_ID/aiyou-api:v1.0.0
```

#### 2. Create GKE Cluster
```bash
gcloud container clusters create aiyou-cluster \
  --region us-central1 \
  --num-nodes 3 \
...
```bash
kubectl create secret generic aiyou-secrets \
...
spec:
  ...
  template:
    spec:
      containers:
      - name: api
        image: gcr.io/YOUR_PROJECT_ID/aiyou-api:v1.0.0
        envFrom:
        - secretRef:
            name: aiyou-secrets
        resources:
...
1. **Isolate**: Scale down to 0 replicas immediately
   ```bash
   kubectl scale deployment aiyou-api --replicas=0
   ```
...
**Security Issues**: Report to security@aiyou.com (replace with actual email)
...
- **Security Issues**: security@aiyou.com
- **Documentation**: See `README.md`
- **Support**: support@aiyou.com
