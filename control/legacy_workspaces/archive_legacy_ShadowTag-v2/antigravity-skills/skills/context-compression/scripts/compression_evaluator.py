...
class CompressionEvaluator:
  ...
  def _heuristic_score(
    self, criterion: dict, response: str, ground_truth: str | None
  ) -> float:
    """
  ...
  def get_summary(self) -> dict:
    ...
    avg_dimensions = {
      dim: dimension_totals[dim] / dimension_counts[dim] for dim in dimension_totals
    }
...
"""
  ...
  def _format_summary(self) -> str:
    ...
    next_steps_str = (
      "\n".join(f"{i + 1}. {s}" for i, s in enumerate(self.sections["next_steps"][-5:]))
      or "None"
    )
