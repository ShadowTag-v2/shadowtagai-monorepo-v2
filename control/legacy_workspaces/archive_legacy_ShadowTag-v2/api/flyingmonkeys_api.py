...
from flyingmonkeys_v8 import FlyingMonkeysV8, SwarmResult

app = FastAPI(
    title="FlyingMonkeys v8 API",
...
def brainstorm(request: BrainstormRequest):
    ...
    return {"mode": "brainstorm", "topic": request.topic, **swarm_result_to_dict(result)}
...
async def bulk_analyze(request: BulkAnalyzeRequest):
    ...
    return {"mode": "bulk_analyze", "model_routing": "claude_architect_gemini_specialist", **result}


@app.get("/cost_stats")
def cost_stats():
    """Get multi-model cost comparison stats"""
    return {"multi_model_router": fm.router.cost_comparison(), "claude_tokens": fm.tokens.display()}
...
def hunt_async(request: HuntRequest, background_tasks: BackgroundTasks):
    ...
    return JobResponse(job_id=job_id, status="queued", message=f"Hunt started. Poll /status/{job_id}")
