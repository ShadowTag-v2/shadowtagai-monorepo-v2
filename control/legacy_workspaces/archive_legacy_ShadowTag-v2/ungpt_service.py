...
# Initialize FastAPI
app = FastAPI(title="UnGPT Consensus Service", description="Multi-LLM consensus system with tiered routing", version="1.0.0")
...
gemini_model = genai.GenerativeModel("gemini-3.1-flash-exp")
...
class UnGPTRequest(BaseModel):
    ...
    complexity: QueryComplexity | None = Field(None, description="Force specific tier (auto-detect if None)")
...
DAILY_BUDGET = {"simple_queries": 100, "moderate_queries": 30, "complex_queries": 15, "max_daily_spend": 10.00}

QUERY_LIMITS = {"max_cost_per_query": 0.50, "require_approval_above": 0.30}


async def check_budget(user_id: str, estimated_cost: float, query_tier: str) -> tuple[bool, str | None]:
    """
...
async def execute_simple_path(query: str) -> dict[str, Any]:
    ...
    response = await asyncio.to_thread(
        anthropic_client.messages.create, model="claude-sonnet-4-20250514", max_tokens=2000, messages=[{"role": "user", "content": query}]
    )
    ...
    return {
        ...
        "total_cost": calculate_cost("claude", response.usage.input_tokens, response.usage.output_tokens),
        "models": ["claude-sonnet-4"],
        "risk_level": "RA-1",
        "reasoning_chain": [{"step": "single_model_query", "model": "claude-sonnet-4", "result": "Direct response"}],
    }
...
async def execute_moderate_path(query: str) -> dict[str, Any]:
    ...
    reasoning_chain.append({"step": "claude_initial", "model": "claude-sonnet-4", "result": claude_text[:200] + "..."})
...
Provide your independent analysis. Do you agree with Claude? What would you add or change?"""
    ...
    reasoning_chain.append({"step": "gemini_analysis", "model": "gemini-3.1-flash", "result": gemini_text[:200] + "..."})
...
Synthesize both perspectives into a final, comprehensive answer. Resolve any disagreements and provide the best combined response."""
    ...
    final_response = await asyncio.to_thread(
        anthropic_client.messages.create, model="claude-sonnet-4-20250514", max_tokens=2000, messages=[{"role": "user", "content": synthesis_prompt}]
    )
    ...
    total_cost = (
        calculate_cost("claude", claude_response.usage.input_tokens, claude_response.usage.output_tokens)
        + calculate_cost("gemini", 500, 300)  # Estimated
        + calculate_cost("claude", final_response.usage.input_tokens, final_response.usage.output_tokens)
    )

    reasoning_chain.append({"step": "final_synthesis", "model": "claude-sonnet-4", "result": "Combined response generated"})
    ...
    return {
        ...
        "models": ["claude-sonnet-4", "gemini-3.1-flash"],
        "risk_level": "RA-2",
...
Your response will be analyzed by other advanced models."""
    ...
    layer1_response = await asyncio.to_thread(
        anthropic_client.messages.create, model="claude-sonnet-4-20250514", max_tokens=2000, messages=[{"role": "user", "content": layer1_prompt}]
    )

    layer1_text = layer1_response.content[0].text
    reasoning_chain.append({"step": "layer1_reasoning", "model": "claude-sonnet-4", "result": layer1_text[:300] + "..."})
...
4. Confidence level (0.0-1.0)"""
    ...
    reasoning_chain.append({"step": "layer2_parallel", "model": "gemini-3.1-flash", "result": layer2_text[:300] + "..."})
...
Provide comprehensive, execution-ready response."""
    ...
    layer3_response = await asyncio.to_thread(
        anthropic_client.messages.create, model="claude-sonnet-4-20250514", max_tokens=3000, messages=[{"role": "user", "content": layer3_prompt}]
    )
    ...
    # Calculate costs
    total_cost = (
        calculate_cost("claude", layer1_response.usage.input_tokens, layer1_response.usage.output_tokens)
        + calculate_cost("gemini", 800, 500)
        + calculate_cost("claude", layer3_response.usage.input_tokens, layer3_response.usage.output_tokens)
    )

    reasoning_chain.append({"step": "layer3_synthesis", "model": "claude-sonnet-4", "result": "Final consensus generated"})
    ...
    return {
        ...
        "execution_time": execution_time,
        "total_cost": total_cost,
        "models": ["claude-sonnet-4", "gemini-3.1-flash"],  # Will add grok, gpt-5 when configured
        "risk_level": "RA-3",
        "reasoning_chain": reasoning_chain,
    }
...
def calculate_cost(model: str, input_tokens: int, output_tokens: int) -> float:
    ...
    cost = (input_tokens / 1_000_000 * rates[model]["input"]) + (output_tokens / 1_000_000 * rates[model]["output"])
...
async def process_query(request: UnGPTRequest, user_id: str = Depends(get_user_id)):
    ...
    # 2. Estimate cost
    cost_estimates = {QueryComplexity.SIMPLE: 0.017, QueryComplexity.MODERATE: 0.046, QueryComplexity.COMPLEX: 0.120}
    ...
    try:
        ...
        if result["total_cost"] > request.max_cost:
            raise HTTPException(status_code=402, detail=f"Query cost ${result['total_cost']:.4f} exceeds your limit ${request.max_cost}")
...
async def health_check():
    ...
    return {"status": "healthy", "service": "ungpt-consensus", "timestamp": datetime.utcnow().isoformat()}
