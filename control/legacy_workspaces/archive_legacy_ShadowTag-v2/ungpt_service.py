...
class UnGPTRequest(BaseModel):
  ...
  user_location: str = Field(
    default="US", description="ISO country code for compliance"
  )
  ...
  include_reasoning: bool = Field(
    default=False, description="Include detailed reasoning chain"
  )
...
Provide your independent analysis. Do you agree with Claude? What would you add or change?"""
  ...
  gemini_response = await asyncio.to_thread(
    gemini_model.generate_content, gemini_prompt
  )
...
4. Confidence level (0.0-1.0)"""
  ...
  layer2_response = await asyncio.to_thread(
    gemini_model.generate_content, layer2_prompt
  )
...
async def process_query(request: UnGPTRequest, user_id: str = Depends(get_user_id)):
  ...
  allowed, reason = await check_budget(
    user_id, estimated_cost, request.complexity.value
  )
  if not allowed:
