...
A "Hybrid Arbitrage" engine that trades LLM reasoning depth for grounded execution (Monkeys) and safety (Judge). It creates a synthetic engineering team with an embedded compliance officer.
...
- **Action**: Monkeys research docs/versions. GCA generates 2 variants (Conservative/Ambitious). Judge-6 sanitizes.
- **Output**: Validated Drafts -> Question #2.

**Iteration 2 (Refinement)**

- **Actor**: Monkey Swarm.
- **Action**: Monkeys test/benchmark the drafts. Vote on bugs/perf.
...
1.  **Sandbox**: Monkeys run in isolated containers (Jetski). No persistence.
