...
# Packages FlyingMonkeys Server and dependencies for transfer to Cloud Workstation
set -e
...
for path in "apps/flyingmonkeys-server/src/main.py" "bin/flyingmonkeys-server" "scripts/verify_smart_actions.py" "src/governance/judge_six" "src/intelligence/tegu_vision"; do
    if [ ! -e "$path" ]; then
...
# Create manifest
echo "Manifest:
- apps/flyingmonkeys-server/src/main.py
- bin/flyingmonkeys-server
