...
class MedicalDLPEngine:
  ...
  def _compile_patterns(self) -> None:
    ...
    self._phi_compiled = {
      phi_type: [re.compile(p, re.IGNORECASE) for p in patterns]
      for phi_type, patterns in self.PHI_PATTERNS.items()
    }
    self._clinical_compiled = {
      data_type: [re.compile(p, re.IGNORECASE) for p in patterns]
      for data_type, patterns in self.CLINICAL_PATTERNS.items()
    }
  ...
  async def scan(
      ...
      for pattern in patterns:
        for match in pattern.finditer(text):
          sensitivity = self.SENSITIVITY_MAP.get(
            phi_type, SensitivityLevel.CONFIDENTIAL
          )
    ...
    for data_type, patterns in self._clinical_compiled.items():
      for pattern in patterns:
        for match in pattern.finditer(text):
          sensitivity = self.SENSITIVITY_MAP.get(
            data_type, SensitivityLevel.CONFIDENTIAL
          )
          if self._sensitivity_rank(sensitivity) > self._sensitivity_rank(
            highest_sensitivity
          ):
  ...
  async def scan_for_disclosure(
    ...
    genetic_found = any(
      c.data_type == ClinicalDataType.GENETIC for c in result.clinical_detected
    )
    if genetic_found:
    ...
    substance_found = any(
      c.data_type == ClinicalDataType.SUBSTANCE_ABUSE for c in result.clinical_detected
    )
    if substance_found:
      warnings.append(
        "Substance abuse records detected - 42 CFR Part 2 restrictions apply"
      )
