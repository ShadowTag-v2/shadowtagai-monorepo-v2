...
class ClinicalDecisionGateway:
  ...
  async def evaluate(
    ...
    approved = decision.risk_level not in [
    ...
    if not approved:
      ...
      logger.info(
        f"CLINICAL GATEWAY FLAGGED: {decision.decision_id} - {escalation_reasons}"
      )
  ...
  def get_safe_response_template(
    self, decision_category: DecisionCategory
  ) -> dict[str, Any]:
  ...
  def generate_safety_report(self) -> dict[str, Any]:
    ...
    return {
      ...
      "escalation_rate": (escalated / total_decisions * 100)
