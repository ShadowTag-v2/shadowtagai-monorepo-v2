...
class ClinicalDecisionGateway:
    ...
    async def evaluate(
        ...
        if decision.confidence_score < threshold:
            ...
            warnings.append(f"Confidence {decision.confidence_score:.0%} below {threshold:.0%} threshold")
            ...
            for patient_med in patient_meds:
                for rec_med in recommended_meds:
                    ...
                    if interaction:
                        ...
                        warnings.append(f"Drug interaction: {patient_med} + {rec_med} = {interaction}")
        ...
        if decision.evidence_level == EvidenceLevel.E5 and decision.category == DecisionCategory.THERAPEUTIC:
        ...
        if not approved:
            logger.warning(f"CLINICAL GATEWAY BLOCKED: {decision.decision_id} - {escalation_reasons}")
    ...
    async def report_adverse_event(
        ...
        if ai_recommendation_id:
            ...
            related_result = next((r for r in self._audit_log if r.decision_id == ai_recommendation_id), None)
            if related_result:
