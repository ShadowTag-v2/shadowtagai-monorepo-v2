...
class SB243ComplianceEngine:
  ...
  def _compile_patterns(self) -> None:
    ...
    self._age_compiled = {
      ...
      for key, patterns in self.AGE_INDICATORS.items()
    }
    self._deception_compiled = [
      re.compile(p, re.IGNORECASE) for p in self.DECEPTION_PATTERNS
    ]
  ...
  def get_crisis_response(self, crisis_level: CrisisLevel) -> dict[str, Any]:
    ...
    return responses.get(
      crisis_level, {"action": "CONTINUE", "escalate_to_human": False}
    )
