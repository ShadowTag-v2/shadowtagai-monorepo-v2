...
class LiabilityShield:
    ...
    async def pre_check(
        ...
        audit_trail.append(f"DLP scan: {dlp_result.total_phi_count} PHI, {dlp_result.total_clinical_count} clinical")
        ...
        elif sb243_result.violations and any(v.severity.value in ["critical", "high"] for v in sb243_result.violations):
            action = ShieldAction.BLOCK
            required_actions.extend([f"RESOLVE: {v.violation_type.value} - {v.remediation}" for v in sb243_result.violations])
    ...
    async def mid_check(
        ...
        sb243_result = await self.sb243.check(
            ...
            context={
                **session_context,
                "crisis_resources_provided": "988" in ai_response or "crisis" in ai_response.lower(),
            },
            is_ai_response=True,
        )
        ...
        audit_trail.append(f"Gateway: approved={gateway_result.approved}, escalate={gateway_result.human_review_required}")
    ...
    async def post_log(
        ...
        had_crisis = any(r.sb243_result and r.sb243_result.crisis_level != CrisisLevel.NONE for r in session_results)
        had_escalation = any(r.gateway_result and r.gateway_result.human_review_required for r in session_results)
    ...
    def generate_liability_report(
        ...
        crisis_events = len([r for r in results if r.sb243_result and r.sb243_result.crisis_level in [CrisisLevel.CRITICAL, CrisisLevel.SEVERE]])

        blocked_decisions = len([r for r in results if r.gateway_result and not r.gateway_result.approved])
        ...
        return {
            ...
            "protection_effectiveness": (f"{((len(results) - crisis_events - blocked_decisions) / len(results) * 100):.1f}%" if results else "N/A"),
        }
