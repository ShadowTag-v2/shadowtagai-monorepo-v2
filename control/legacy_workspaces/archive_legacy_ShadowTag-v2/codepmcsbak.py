...
class JudgeSixSentinel:
  def __init__(self):
    ...
    try:
      ...
      self.grounding = Tool.from_google_search_retrieval(
        grounding.GoogleSearchRetrieval()
      )
      self.model = GenerativeModel("gemini-1.5-pro-001")
      print(
        f"{Fore.GREEN}>>> 🌍 Vertex AI Grounding: ACTIVE ({PROJECT_ID}){Style.RESET_ALL}"
      )
...
if __name__ == "__main__":
  ...
  if target == ".":
    ...
    print(
      f"{Fore.MAGENTA}>>> 👁️  Scanning {len(files)} changed files...{Style.RESET_ALL}"
    )
    for f in files:
