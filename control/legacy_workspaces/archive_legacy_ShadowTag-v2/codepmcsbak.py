...
HAZARD_DATABASE = [
    ...
    {"pattern": "import non_existent", "severity": "II", "probability": "E", "name": "Hallucination"},
...
class MissionLogger:
    ...
    def send_alert(self, target, hazards, risk):
        if not self.webhook:
            return
        payload = {"username": "JUDGE 6 OMEGA", "content": f"🚨 **ABORT** | `{target}` | Risk: {risk}\nHazards: {hazards}"}
        try:
            req = urllib.request.Request(self.webhook, data=json.dumps(payload).encode("utf-8"), headers={"Content-Type": "application/json"})
            urllib.request.urlopen(req)
        except:
            pass

    def log_mission(self, target, hazards, risk, outcome):
        entry = {"timestamp": datetime.now().isoformat(), "target": target, "hazards": [h["name"] for h in hazards], "risk": risk, "outcome": outcome}
        try:
            with open(self.log_file) as f:
                h = json.load(f)
        except:
...
class JudgeSixSentinel:
    ...
    def run_tests(self, file_path):
        ...
        if not os.path.exists(test_file):
            print(f"{Fore.YELLOW}>>> ⚠️  No unit tests found for {file_path}. Skipping functional check.{Style.RESET_ALL}")
    ...
    def assess_risk(self, code):
        matrix = {("I", "E"): 2, ("II", "E"): 2, ("III", "B"): 2, ("IV", "A"): 1}  # Simplified Matrix
        current, detected = 1, []
