# GKE + AiYou Ecosystem Integration
...
Successfully integrated the complete **AiYou Ecosystem** (Judge #6 v2.0 + 6 verticals) with **PNKLN Core Stack™** (LLM orchestration + multi-agent intelligence) on **GKE-native infrastructure**.
...
spec:
  ...
  template:
    spec:
      ...
      - name: api
        image: us-central1-docker.pkg.dev/aiyou-prod/containers/pnkln-api:latest
        resources:
          requests:
            cpu: 500m
            memory: 1Gi
...
spec:
  ...
  template:
    spec:
      ...
      - name: judge6
        image: us-central1-docker.pkg.dev/aiyou-prod/containers/judge6:v2.0
        env:
...
metadata:
  name: pnkln-api-sa
  annotations:
    iam.gke.io/gcp-service-account: pnkln-api@aiyou-prod.iam.gserviceaccount.com
...
spec:
  type: Python
  mode: cluster
  image: us-central1-docker.pkg.dev/aiyou-prod/containers/spark-rapids:latest
  mainApplicationFile: "gs://aiyou-models/growth_engine.py"
  sparkConf:
...
- `GKE_AIYOU_INTEGRATION.md` (this file)
...
**Integration Complete:** GKE + PNKLN + AiYou Ecosystem Unified Platform Ready for Production 🚀
