...
class BugBot:
  ...
  def format_triage_prompt(self, issue: dict) -> str:
    ...
    care = CARE(
      ...
      action=(
        "Classify severity, category, and priority. Estimate fix time. Suggest labels and assignee."
      ),
      result=(
        "JSON with severity (critical/high/medium/low/trivial), category, priority_score (1-10), estimated_hours, labels"
      ),
      example={
        "severity": "high",
        "category": "bug",
...
"""
  ...
  async def _check_auto_fixable(self, issue: dict) -> bool:
    ...
    return any(
      keyword in title_lower or keyword in body_lower for keyword in auto_fix_keywords
    )
  ...
  async def generate_fix(self, issue: dict, triage: TriageResult) -> str | None:
    ...
    prompt = rise.format("Generate the fix")
  ...
  def __repr__(self) -> str:
    return f"BugBot(auto_fix={self.auto_fix_enabled}, agents={self.debate_agents}, strategy={self.reasoning_strategy!r})"
