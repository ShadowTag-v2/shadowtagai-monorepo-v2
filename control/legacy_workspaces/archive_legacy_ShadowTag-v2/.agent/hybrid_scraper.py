...
class AntigravityHybridScraper:
  ...
  def safe_write(self, filename, data):
    ...
    if not any(filepath.startswith(os.path.abspath(d)) for d in allowed_dirs):
      raise PermissionError(
        "Antigravity Sandbox Violation: Write blocked outside allowed directories."
      )
  ...
  def scrape_spa(self, url, selectors):
    ...
    try:
      ...
      data = self.fc.scrape(
        url=url, formats=["html"], params={"waitFor": 3000}, timeout=60
      )
      raw_html = data["html"]
    except Exception as e:
      print(
        f" [Scraper] FireCrawl failed ({e}). Arbitraging to Scrapling DynamicFetcher..."
      )
...
if __name__ == "__main__":
  ...
  results = scraper.scrape_spa(
    ...
    selectors={
      "container": "div.vue-list-item",
      "id_field": "data-key",
      "fields": {"title": "h3.title", "price": "span.price"},
    },
  )
