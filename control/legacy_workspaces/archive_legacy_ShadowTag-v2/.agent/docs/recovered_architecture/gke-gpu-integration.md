# GKE GPU Integration Guide for YouAi
...
This document provides the complete implementation guide for deploying YouAi's GPU infrastructure on **Google Kubernetes Engine (GKE)**. It translates the strategic GPU infrastructure plan into concrete GCP-native implementations, leveraging GKE's managed Kubernetes capabilities, NVIDIA GPU support, and integration with Vertex AI.
...
|---------|-------|-----------|
| **Cluster Name** | `youai-production-gpu` | Clear naming convention |
...
export PROJECT_ID="youai-production"
export CLUSTER_NAME="youai-production-gpu"
export REGION="us-central1"
export NETWORK="youai-vpc"
export SUBNETWORK="youai-gpu-subnet"
  ...
  predictor:
    ...
    containers:
      ...
      - name: kserve-container
        image: gcr.io/youai-production/llm-inference:latest
        resources:
          requests:
            cpu: "4"
            memory: "16Gi"
        ...
        env:
          ...
          - name: STORAGE_URI
            value: "gs://youai-models/llama-70b-instruct"
...
kubectl get inferenceservice youai-llm-inference -n production

# Get endpoint

kubectl get ksvc youai-llm-inference-predictor -n production -o jsonpath='{.status.url}'
      ...
      template:
        ...
        spec:
          ...
          containers:
            ...
            - name: pytorch
              image: gcr.io/youai-production/pytorch-training:latest
              command:
                ...
                - --dataset=gs://youai-data/fine-tune-dataset
...
gcloud notebooks instances create youai-workbench \
...
aiplatform.init(project='youai-production', location='us-central1')

# Create custom training job that runs on GKE

job = aiplatform.CustomContainerTrainingJob(
    display_name='llama-finetune',
    container_uri='gcr.io/youai-production/pytorch-training:latest',
    command=['python', 'train.py'],
    model_serving_container_image_uri='gcr.io/youai-production/llm-serve:latest',
)
...
@dsl.component(
    base_image='gcr.io/youai-production/pytorch-training:latest',
    gpu_limit='2'
)
...
def training_pipeline(input_data: str):
    preprocess_task = preprocess_data(
        ...
        output_path='gs://youai-data/preprocessed'
    )

    train_task = train_model(
        data_path=preprocess_task.outputs['output_path'],
        model_output='gs://youai-models/llama-finetuned'
    )
...
pipeline_job = aiplatform.PipelineJob(
    ...
    pipeline_root='gs://youai-pipelines',
    parameter_values={'input_data': 'gs://youai-data/raw'}
)
pipeline_job.run()
...
FROM
  `youai-production.billing.gcp_billing_export_v1_*`
CROSS JOIN
...
admissionWhitelistPatterns:
  ...
  - namePattern: gcr.io/youai-production/*
defaultAdmissionRule:
  requireAttestationsBy:

    - projects/youai-production/attestors/image-signer
...
gcloud container clusters create youai-dev-gpu \
  --region=us-central1 \
  --num-nodes=1 \
  --machine-type=e2-medium
