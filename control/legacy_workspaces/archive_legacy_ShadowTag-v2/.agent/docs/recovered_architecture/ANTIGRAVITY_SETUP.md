...
**Squadron**: 650-Agent FlyingMonkeys Cavalry
**Status**: ✅ OPERATIONAL
...
**Location**: `src/aiyou/services/gemini_failover.py`
...
**Location**: `api/flyingmonkeys_api.py`
**Port**: 8888
...
```bash
# Start FlyingMonkeys
./run_flyingmonkeys_api.sh

# Or manually
python3 -m uvicorn api.flyingmonkeys_api:app --host 0.0.0.0 --port 8888 --reload
```
...
### Issue: FlyingMonkeys not responding

**Solution**:
```bash
# Check if running
ps aux | grep flyingmonkeys

# Check logs
tail -f flyingmonkeys.log

# Restart
pkill -f flyingmonkeys
./run_flyingmonkeys_api.sh
```
...
2. **Start Services**: `./run_flyingmonkeys_api.sh`
...
- **Gemini Failover**: `src/aiyou/services/gemini_failover.py` - Implementation
- **FlyingMonkeys API**: `api/flyingmonkeys_api.py` - Server code
