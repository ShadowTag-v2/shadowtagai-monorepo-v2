...
git clone https://github.com/ShadowTag-v2/aiyou-fastapi-services.git ~/aiyou-fastapi-services
...
RUNNER_TOKEN=$(gh api -X POST repos/ShadowTag-v2/aiyou-fastapi-services/actions/runners/registration-token --jq '.token')

# Configure

./config.sh --url https://github.com/ShadowTag-v2/aiyou-fastapi-services --token $RUNNER_TOKEN --name sovereign --labels self-hosted,macOS,X64 --unattended
...
gh api repos/ShadowTag-v2/aiyou-fastapi-services/actions/runners --jq '.runners[]'
...
- **URL**: `https://flyingmonkeys-server-215390634092.us-central1.run.app`
...
| Project | Path |
|---------|------|
| Main Repo | `~/aiyou-fastapi-services` |
