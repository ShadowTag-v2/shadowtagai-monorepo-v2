...
from src.kosmos.doctrine import BattleDrillRouter, DrillTrigger, MDMPPipeline
from src.kosmos.doctrine import RiskLevel as DoctrineRiskLevel
from src.kosmos.doctrine import RiskManager as DoctrineRiskManager
from src.kosmos.doctrine import TLPPipeline
...
class RegulatoryComplianceEngine:
  ...
  def _map_decision_to_frameworks(
    ...
    if (
      "minors" in decision.description.lower() or "age" in decision.description.lower()
    ):
    ...
    if (
      "mobile" in decision.description.lower() or "ios" in decision.description.lower()
    ):
  ...
  def _calculate_highest_risk(
    self, compliance_profile: dict[str, ComplianceCheck]
  ) -> RiskLevel:
...
class JudgeArchitectureMonitor:
  ...
  def log_decision(
    ...
    if iq_level == 160:
      ...
      self.iq_160_metrics["regulatory_gap_detection"].append(
        len(outcome["regulatory_gaps"])
      )
  ...
  def get_performance_summary(self) -> dict[str, Any]:
    ...
    return {
      "decision_accuracy_mean": float(
        np.mean(self.iq_160_metrics["decision_accuracy"])
      ),
      "doctrine_alignment_mean": float(
        np.mean(self.iq_160_metrics["doctrine_alignment"])
      ),
  ...
  def get_doctrine_status(self) -> dict[str, Any]:
    ...
    return {
      ...
      "consensus_thresholds": {
        ...
        "EXTREMELY_HIGH": CONSENSUS_THRESHOLDS.get(
          DoctrineRiskLevel.EXTREMELY_HIGH, 0.90
        ),
      },
