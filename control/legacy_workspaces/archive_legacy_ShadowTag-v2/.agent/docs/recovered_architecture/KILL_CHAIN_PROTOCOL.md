...
To provide an immediate, unconditional termination capability ("Rkill") for all agentic operations (FlyingMonkeys, Genkit Swarms) across both local and cloud environments. This protocol is the "Break Glass" mechanism to prevent runaway costs, security breaches, or operational hazards.
...
1. **Process Termination:** `SIGKILL` sent to all `python`, `node`, and `go` processes associated with `flyingmonkeys` or `genkit`.
