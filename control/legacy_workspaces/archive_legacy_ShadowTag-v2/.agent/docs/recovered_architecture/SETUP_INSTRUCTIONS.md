...
**Location:** `~/aiyou-stack/antigravity-tools/AntigravityQuotaWatcher/antigravity-quota-watcher-0.7.1.vsix`
...
**Location:** `~/aiyou-stack/antigravity-tools/antigravity-proxy`

**Setup:**
1. Edit the `.env` file:
   ```bash
   nano ~/aiyou-stack/antigravity-tools/antigravity-proxy/.env
   ```
   Set `GEMINI_API_KEY=your-actual-key`.

2. Start the proxy:
   ```bash
   mitmproxy -s ~/aiyou-stack/antigravity-tools/antigravity-proxy/mitmproxy-addon.py --listen-port 8080
   ```
...
**Location:** `~/aiyou-stack/antigravity-tools/antigravity-workspace-template`
