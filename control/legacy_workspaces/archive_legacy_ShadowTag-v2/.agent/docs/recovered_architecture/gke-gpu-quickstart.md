...
> **Get YouAi's GPU infrastructure running on GKE in under 30 minutes**
...
This quick start guide provides a streamlined path to deploy YouAi's GPU infrastructure on Google Kubernetes Engine (GKE). Follow these steps to go from zero to running GPU workloads.
...
- **GKE Cluster**: `youai-production-gpu` (regional, 3 zones)
...
gcloud container clusters get-credentials youai-production-gpu \
  --region us-central1 \
  --project $PROJECT_ID
...
gcloud container clusters resize youai-production-gpu \
...
echo "https://console.cloud.google.com/kubernetes/clusters/details/us-central1/youai-production-gpu/observability?project=$PROJECT_ID"
...
```bash
terraform import google_compute_network.vpc projects/$PROJECT_ID/global/networks/youai-vpc
terraform apply
...
You've successfully deployed YouAi's GPU infrastructure on GKE! 🎉
