...
kubectl apply -f k8s/deployments/flying-monkeys.yaml
...
kubectl scale deployment flying-monkeys --replicas=48

# Enable autoscaling
kubectl autoscale deployment flying-monkeys \
  --min=24 \
  --max=100 \
  --cpu-percent=70
```
...
```yaml
# k8s/deployments/flying-monkeys.yaml
nodeSelector:
