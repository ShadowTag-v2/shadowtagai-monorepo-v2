...
**Context**: Applied to `aiyou-fastapi-services` Agent Ecosystem
**Benchmark**: [Puzzle Room Challenge](demos/puzzle_room_challenge.md) - Validation Protocol

## Overview

This guide outlines how to implement Anthropic's advanced tool use patterns—**Tool Search**, **Programmatic Tool Calling (PTC)**, and **Tool Use Examples**—to optimize token usage, improve accuracy, and enable complex orchestration within the `aiyou` agent swarm.
...
### Implementation Strategy for `aiyou` (Tool Search)
...
### Implementation Strategy for `aiyou` (PTC)
...
### Implementation Strategy for `aiyou` (Examples)

Update tool definitions in `src/aiyou/tools/` or `agents/`:
