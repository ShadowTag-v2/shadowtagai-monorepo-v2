...
- "Here's our live API" → Show https://api.youai.com/docs
...
**With Branch E:**
> "We have a LIVE governance API at api.youai.com. Test it now. EU AI Act compliant, production-ready, scales on GKE. Deploy your own instance with `kubectl apply -f k8s/`."
...
git clone https://github.com/ShadowTag-v2/aiyou-fastapi-services
...
gcloud container clusters get-credentials youai-cluster --zone us-central1-a
kubectl apply -f k8s/
kubectl get pods  # ✅ 3/3 Running

# LIVE IN PRODUCTION

curl https://api.youai.com/health  # ✅ Production healthy
...
gcloud container clusters create youai-cluster \
...
# Point api.youai.com to external IP

# Configure Let's Encrypt cert

# Day 5: LIVE IN PRODUCTION

curl https://api.youai.com/health  # ✅
curl https://api.youai.com/docs    # ✅ Swagger UI

# FIRST API CALL (REVENUE POSSIBLE)

curl -X POST https://api.youai.com/api/v1/governance/assess \
  -H "Content-Type: application/json" \
