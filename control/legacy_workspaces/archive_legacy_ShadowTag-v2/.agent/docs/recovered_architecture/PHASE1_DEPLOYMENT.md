...
gcloud container clusters list --project=youai-production

# Get credentials

gcloud container clusters get-credentials youai-production-gpu \
  --region us-central1 \
  --project youai-production
...
gcloud compute project-info describe --project=youai-production | \
...
# https://console.cloud.google.com/iam-admin/quotas?project=youai-production
...
# docker build -f /tmp/Dockerfile.agent -t gcr.io/youai-production/pinkln-agent:latest .

# docker push gcr.io/youai-production/pinkln-agent:latest
...
gcloud container clusters update youai-production-gpu \
