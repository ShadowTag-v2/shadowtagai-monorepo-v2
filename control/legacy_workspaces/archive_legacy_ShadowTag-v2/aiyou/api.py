...
upload_requests = Counter(
  "aiyou_upload_requests_total", "Total upload requests", ["status"]
)
...
async def get_feed(request: FeedRequest):
  ...
  if request.include_tiers:
    candidates = [
      item for item in candidates if item.tier_score.tier in request.include_tiers
    ]
  ...
  items_data = [
    {
      ...
      "content_preview": (
        item.source_item.content[:200] if item.source_item.content else ""
      ),
  ...
  avg_rank = (
    sum(item.ai_presumed_rank for item in feed_items) / len(feed_items)
    if feed_items
    else 0.0
  )
