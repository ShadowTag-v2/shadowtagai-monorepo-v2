  ...
  def getContent(self, *args, **kwargs):
    ...
    if isinstance(source, list):
      ...
      if lfEval("has('nvim')") == "0" and isinstance(source["command"], vim.Function):
        try:
          ...
          raise Exception(
            "Error occurred in user defined %s: %s" % (str(source["command"]), err)
          )
      elif type(source["command"]) == str:
        if lfEval("has('nvim')") == "1" and source["command"].startswith("function("):
          function_name = source["command"][
            10:-2
          ]  # source["command"] is like "function('FuncName')"
          ...
          if source_cmd.count("%s") != len(positional_args):
            ...
            source_cmd = source_cmd % tuple(
              arguments[name][0] for name in positional_args
            )
      else:
        raise Exception(
          "Invalid source command `{}`, should be a string or a Funcref!".format(
            str(source["command"])
          )
        )
      ...
      if re.search(
        r"\brg\b|\bag\b|\bpt\b|\bfd\b|\bgit\b", source_cmd
      ):  # encoding of output is utf-8
    ...
    if format_line:
      try:
        ...
        if sys.version_info >= (3, 0) and lfEval("has('nvim')") == "0":
          result = (
            lfBytes2Str(format_line(line, kwargs["arguments"])) for line in result
          )
        else:
          result = (format_line(line, kwargs["arguments"]) for line in result)
      except vim.error as err:
        raise Exception(
          "Error occurred in user defined %s: %s" % (str(format_line), err)
        )
    ...
    if format_list:
      try:
        ...
        raise Exception(
          "Error occurred in user defined %s: %s" % (str(format_list), err)
        )
  ...
  def _getDigest(self, line, mode):
    ...
    if get_digest:
      try:
        ...
        raise Exception(
          "Error occurred in user defined %s: %s" % (str(get_digest), err)
        )
    else:
      return line
  ...
  def _beforeEnter(self):
    ...
    if before_enter:
      try:
        ...
        raise Exception(
          "Error occurred in user defined %s: %s" % (str(before_enter), err)
        )
  ...
  def _afterEnter(self):
    ...
    if after_enter:
      ...
      try:
        ...
        raise Exception(
          "Error occurred in user defined %s: %s" % (str(after_enter), err)
        )
    ...
    if highlight:
      try:
        if self._getInstance().getWinPos() == "popup":
          lfCmd(
            """call win_execute(%d, "let matchids = %s(%s)")"""
  ...
  def _bangEnter(self):
    ...
    if bang_enter:
      ...
      try:
        ...
        raise Exception(
          "Error occurred in user defined %s: %s" % (str(bang_enter), err)
        )
  ...
  def _beforeExit(self):
    ...
    if before_exit:
      ...
      try:
        ...
        raise Exception(
          "Error occurred in user defined %s: %s" % (str(before_exit), err)
        )
  ...
  def _afterExit(self):
    ...
    if after_exit:
      try:
        ...
        raise Exception(
          "Error occurred in user defined %s: %s" % (str(after_exit), err)
        )
        ...
        if result:
          ...
          if isinstance(filename, int):  # it is a buffer number
            ...
            if lfEval("bufloaded('%s')" % escQuote(filename)) == "1":
              filename = int(
                lfEval("bufnr('%s')" % escQuote(filename))
              )  # actually, it's a buffer number
          self._createPopupPreview(
            "",
            filename,
            line_num,
            lfBytes2Str(jump_cmd) if not self._has_nvim else jump_cmd,
          )
      except vim.error as err:
    ...
    while True:
      ...
      if self.state is None:
        ...
        if nextchar == self.state:
          ...
          self.posix and nextchar in self.escape and self.state in self.escapedquotes
        ):
    ...
    if lfEval("has_key(g:Lf_Extensions, '%s')" % category) == "1":
      if category not in self._managers:
        ...
        if (
          ...
          if lfEval("has('nvim')") == "1":
            config["source"]["command"] = vim.eval(
              "string(g:Lf_Extensions['%s']['source']['command'])" % category
            )
          else:
            config["source"]["command"] = vim.bindeval(
              "g:Lf_Extensions['%s']['source']['command']" % category
            )
      ...
      if category == "file":
        ...
        manager_id = lfFunction(
          lfEval("g:Lf_PythonExtensions['%s'].manager_id" % category)
        )()
  ...
  def start(self, arg_line, *args, **kwargs):
    if self._parser is None:
      self._parser = argparse.ArgumentParser(
        prog="Leaderf[!]",
        formatter_class=LfHelpFormatter,
        epilog="If [!] is given, enter normal mode directly.",
      )
      ...
      extensions = itertools.chain(
        lfEval("keys(g:Lf_Extensions)"), lfEval("keys(g:Lf_PythonExtensions)")
      )
      for category in itertools.chain(
        extensions, (i for i in lfEval("keys(g:Lf_Arguments)") if i not in extensions)
      ):
        positional_args = []
        if lfEval("has_key(g:Lf_Extensions, '%s')" % category) == "1":
          ...
          arg_def = lfEval(
            "get(g:Lf_PythonExtensions['%s'], 'arguments', [])" % category
          )
        else:
        ...
        if isinstance(arg_def, dict):
          ...
          for command, args in arg_def.items():
            ...
            subparser = subsubparsers.add_parser(
              command, help=help, formatter_class=LfHelpFormatter
            )
            ...
            if alias is not None:
              subparser = subsubparsers.add_parser(
                alias, help=help, formatter_class=LfHelpFormatter
              )
              group = subparser.add_argument_group("specific arguments")
              self._add_argument(group, args, positional_args)
        ...
        parser.set_defaults(
          start=partial(self._default_action, category, positional_args)
        )
    ...
    try:
      ...
      if sys.version_info < (3, 7):
        if (
          "--recall" in raw_args
          and len([i for i in raw_args if not i.startswith("-")]) == 0
        ):
          if self._last_cmd:
