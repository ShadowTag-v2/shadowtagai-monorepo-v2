...
class FileExplorer(Explorer):
  ...
  def _getFiles(self, dir):
    ...
    for dir_path, dirs, files in os.walk(
      dir, followlinks=False if lfEval("g:Lf_FollowLinks") == "0" else True
    ):
      dirs[:] = [
        i
        for i in dirs
        if True not in (fnmatch.fnmatch(i, j) for j in wildignore.get("dir", []))
      ]
      for name in files:
  ...
  def _getFileList(self, dir):
    ...
    with lfOpen(self._cache_index, "r+", errors="ignore") as f:
      ...
      if target != -1:
        ...
        f.writelines(lines)
        with lfOpen(
          os.path.join(self._cache_dir, lines[target].split(None, 2)[1]),
          "r",
          errors="ignore",
        ) as cache_file:
          if lines[target].split(None, 2)[2].strip() == dir:
            ...
            file_list = [
              line for line in cache_file.readlines() if line.startswith(dir)
            ]
            if file_list == []:
        ...
        if delta_seconds > float(lfEval("g:Lf_NeedCacheTime")):
          ...
          if len(lines) < int(lfEval("g:Lf_NumberOfCache")):
            ...
            f.writelines(lines)
          with lfOpen(
            os.path.join(self._cache_dir, cache_file_name), "w", errors="ignore"
          ) as cache_file:
  ...
  def _buildCmd(self, dir, **kwargs):
    ...
    if lfEval("exists('g:Lf_ExternalCommand')") == "1":
      if cd_cmd:
        cmd = cd_cmd + lfEval("g:Lf_ExternalCommand").replace('"%s"', "").replace(
          "%s", ""
        )
      else:
    ...
    if lfEval("g:Lf_UseVersionControlTool") == "1":
      if self._exists(dir, ".git") and lfEval("executable('git')") == "1":
        ...
        if cd_cmd:
          cmd = cd_cmd + "git ls-files %s && git ls-files --others %s %s" % (
            recurse_submodules,
            no_ignore,
            ignore,
          )
    ...
    if default_tool["rg"] and lfEval("executable('rg')") == "1":
      ...
      if cd_cmd:
        cmd = cd_cmd + "rg --no-messages --files %s %s %s %s %s" % (
          color,
          ignore,
          followlinks,
          show_hidden,
      ...
      if cd_cmd:
        cmd = cd_cmd + 'pt --nocolor %s %s %s %s -g=""' % (
          ignore,
          followlinks,
          show_hidden,
          no_ignore,
      ...
      if cd_cmd:
        cmd = cd_cmd + 'ag --nocolor --silent %s %s %s %s -g ""' % (
          ignore,
          followlinks,
          show_hidden,
          no_ignore,
      ...
      if cd_cmd:
        cmd = cd_cmd + 'find %s . -name "." -o %s %s %s -type f -print %s %s' % (
          followlinks,
          ignore_dir,
          ignore_file,
          show_hidden,
  ...
  def getContentFromMultiDirs(self, dirs, **kwargs):
    ...
    dirs = {
      os.path.abspath(os.path.expanduser(lfDecode(dir.strip('"').rstrip("\\/"))))
      for dir in dirs
    }
    if (
      arg_changes
      or lfEval("g:Lf_UseMemoryCache") == "0"
      or dirs != self._cur_dir
        ...
        if cmd.split(None, 1)[0] == "dir":
          ...
          if lfEval("get(g:, 'Lf_ShowDevIcons', 1)") == "1":
            content = executor.execute(
              cmd, encoding=lfEval("&encoding"), format_line=format_line
            )
          else:
...
class FileExplManager(Manager):
  ...
  def _bangEnter(self):
    ...
    if self._read_finished < 2:
      self._timer_id = lfEval(
        "timer_start(1, 'leaderf#File#TimerCallback', {'repeat': -1})"
      )
  ...
  def startExplorer(self, win_pos, *args, **kwargs):
    ...
    if directory and directory[0] not in [
      '""',
      "''",
    ]:  # behavior no change for `LeaderfFile <directory>`
      self._orig_cwd = None
      ...
      if kwargs.get("mode", "") == "t":
        if (
          ...
          and not (
            ...
            and len(
              [w for tp in vim.tabpages for w in tp.windows if w.buffer.name == file]
            )
            > 0
          )
        ):
