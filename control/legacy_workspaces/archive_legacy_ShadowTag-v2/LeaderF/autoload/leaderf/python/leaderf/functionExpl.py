...
class FunctionExplorer(Explorer):
  ...
  def _getFunctionResult(self, buffer):
    ...
    extra_options = self._ctags_options.get(
      lfEval("getbufvar(%d, '&filetype')" % buffer.number), ""
    )
    ...
    if buffer.options["modified"] == True:
      ...
      cmd = (
        f'{self._ctags} -n -u --fields=k {extra_options} -f- "{lfDecode(file_name)}"'
      )
      result = executor.execute(cmd, cleanup=partial(os.remove, file_name))
    else:
      cmd = (
        f'{self._ctags} -n -u --fields=k {extra_options} -f- "{lfDecode(buffer.name)}"'
      )
...
class FunctionExplManager(Manager):
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_funcKind'', ''^\w'')')"""
      ...
      id = int(
        lfEval(
          r"""matchadd('Lf_hl_funcReturnType', '^\w\t\zs.\{-}\ze\s*[~]\=\(\w\|[#:]\)\+\W\{-}[(\[]')"""
        )
      )
      self._match_ids.append(id)
      ...
      id = int(
        lfEval(
          r"""matchadd('Lf_hl_funcName', '^\w\t.\{-}\s*\zs[~]\=\(\w\|[#:]\)\+\W\{-}\ze[(\[]')"""
        )
      )
      self._match_ids.append(id)
    ...
    if "--all" in self._arguments and not self._is_content_list:
      ...
      if self._read_finished < 2:
        self._timer_id = lfEval(
          "timer_start(1, 'leaderf#Function#TimerCallback', {'repeat': -1})"
        )
    else:
