  ...
  def getContent(self, *args, **kwargs):
    ...
    if "--all" not in kwargs.get("arguments", {}):
      if "--tabpage" not in kwargs.get("arguments", {}):
        buffers = {
          b.number: b for b in vim.buffers if lfEval("buflisted(%d)" % b.number) == "1"
        }
      else:
        buffers = {
          w.buffer.number: w.buffer
      ...
      if "--tabpage" not in kwargs.get("arguments", {}):
        buffers = {
          b.number: b for b in vim.buffers if os.path.basename(b.name) != "LeaderF"
        }
      else:
        buffers = {
          w.buffer.number: w.buffer
    ...
    self._max_bufname_len = max(
      [
        int(lfEval("strdisplaywidth('%s')" % escQuote(getBasename(buffers[nr].name))))
...
class BufExplManager(Manager):
  ...
  def _getDigestStartPos(self, line, mode):
    ...
    if mode == 0:
      ...
      buf_number = int(re.sub(r"^.*?(\d+).*$", r"\1", line))
      basename = getBasename(vim.buffers[buf_number].name)
      space_num = self._getExplorer().getMaxBufnameLen() - int(
        lfEval("strdisplaywidth('%s')" % escQuote(basename))
      )
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_bufNumber'', ''^\s*\zs\d\+'')')"""
      ...
      self._match_ids.append(id)
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_bufDirname'', '' \zs".*"$'')')"""
        % self._getInstance().getPopupWinId()
      )
      id = int(lfEval("matchid"))
      self._match_ids.append(id)
      ...
      self._match_ids.append(id)
      id = int(
        lfEval(r"matchadd('Lf_hl_bufIndicators', '^\s*\d\+\s*\zsu\=\s*[#%]\=...')")
      )
      self._match_ids.append(id)
      id = int(
        lfEval(r"matchadd('Lf_hl_bufModified', '^\s*\d\+\s*u\=\s*[#%]\=.+\s*\zs.*$')")
      )
      self._match_ids.append(id)
      id = int(
        lfEval(
          r"matchadd('Lf_hl_bufNomodifiable', '^\s*\d\+\s*u\=\s*[#%]\=..-\s*\zs.*$')"
        )
      )
      self._match_ids.append(id)
    ...
    if lfEval("get(g:, 'Lf_ShowDevIcons', 1)") == "1":
      self._match_ids.extend(
        matchaddDevIconsExtension(r"__icon__\ze\s\+.\{-}\.__name__\($\|\s\)", winid)
      )
      self._match_ids.extend(
