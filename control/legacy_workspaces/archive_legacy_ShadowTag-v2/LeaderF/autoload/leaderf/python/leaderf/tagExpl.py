...
class TagExplorer(Explorer):
  ...
  def getFreshContent(self, *args, **kwargs):
    ...
    if has_new_tagfile == False and has_changed_tagfile == False:
      ...
      self._tag_list = list(
        itertools.chain.from_iterable(i[1] for i in self._file_tags.values())
      )
      return self._tag_list
...
class TagExplManager(Manager):
    ...
    try:
      if kwargs.get("mode", "") == "t":
        if (
          ...
          and lfEval("bufloaded('%s')" % escQuote(tagfile)) == "1"
        ):
          lfDrop("tab", tagfile)
        else:
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      ...
      self._match_ids.append(id)
      id = int(
        lfEval(r"""matchadd('Lf_hl_tagType', ';"\t\zs[cdefFgmpstuv]\ze\(\t\|$\)')""")
      )
      self._match_ids.append(id)
      keyword = [
        "namespace",
      ...
      for i in keyword:
        id = int(
          lfEval(r"""matchadd('Lf_hl_tagKeyword', '\(;"\t.\{-}\)\@<=%s:')""" % i)
        )
        self._match_ids.append(id)
  ...
  def _bangEnter(self):
    ...
    if instance.getWinPos() == "popup":
      ...
      lfCmd(
        "call nvim_win_set_option(%d, 'cursorline', v:true)" % instance.getPopupWinId()
      )
    else:
