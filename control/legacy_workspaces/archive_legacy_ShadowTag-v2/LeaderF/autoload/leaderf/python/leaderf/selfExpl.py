...
class SelfExplorer(Explorer):
  ...
  def getContent(self, *args, **kwargs):
    ...
    for i, (key, value) in enumerate(extra_content.items()):
      content.append(f'{length + i:<3d}{key:<24s}"{value}"')
    return content
...
class SelfExplManager(Manager):
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_selfIndex'', ''^\d\+'')')"""
        % self._getInstance().getPopupWinId()
      )
      id = int(lfEval("matchid"))
      self._match_ids.append(id)
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_selfDescription'', '' \zs".*"$'')')"""
        % self._getInstance().getPopupWinId()
      )
      id = int(lfEval("matchid"))
      self._match_ids.append(id)
