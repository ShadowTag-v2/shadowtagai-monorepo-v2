...
class LfCli:
    ...
    def _buildPrompt(self):
        ...
        if self._is_fuzzy:
            if self._is_full_path:
                lfCmd(f"echohl Constant | echon '>F> {self._additional_prompt_string}' | echohl NONE")
            else:
                lfCmd(f"echohl Constant | echon '>>> {self._additional_prompt_string}' | echohl NONE")
        elif self._is_live:
            lfCmd(f"echohl Constant | echon '>>> {self._additional_prompt_string}' | echohl NONE")
        else:
            lfCmd(f"echohl Constant | echon 'R>> {self._additional_prompt_string}' | echohl NONE")

        lfCmd("echohl Normal | echon '%s' | echohl NONE" % escQuote("".join(self._cmdline[: self._cursor_pos])))
        if self._cursor_pos < len(self._cmdline):
            lfCmd("echohl Lf_hl_cursor | echon '%s' | echohl NONE" % escQuote("".join(self._cmdline[self._cursor_pos])))
            lfCmd("echohl Normal | echon '%s' | echohl NONE" % escQuote("".join(self._cmdline[self._cursor_pos + 1 :])))
        else:
    ...
    def highlightMatches(self):
        ...
        if self._is_fuzzy:
            ...
            if self._is_full_path:
                ...
                lfCmd("syn match Lf_hl_match display /%s/ containedin=Lf_hl_nonHelp, Lf_hl_dirname, Lf_hl_filename contained" % regex)
            else:
                if self._refine:
                    idx = self._cmdline.index(self._delimiter)
                    ...
                    if regex[0] == r"\c\V" and regex[1] == r"\c\V":
                        ...
                        lfCmd("syn match Lf_hl_match display /%s/ containedin=Lf_hl_dirname, Lf_hl_filename contained" % regex[1])
                    elif regex[1] == r"\c\V":
                        lfCmd("syn match Lf_hl_match display /%s/ containedin=Lf_hl_filename contained" % regex[0])
                    else:
                        lfCmd("syn match Lf_hl_match display /%s/ containedin=Lf_hl_filename contained" % regex[0])
                        lfCmd(
                    ...
                    regex = r"\c\V" + self._join(cmdline)
                    lfCmd("syn match Lf_hl_match display /%s/ containedin=Lf_hl_filename contained" % regex)
        else:
            if self._pattern:
                ...
                try:
                    if self._instance.getWinPos() == "popup":
                        ...
                        lfCmd("syn match Lf_hl_match '%s' containedin=Lf_hl_dirname, Lf_hl_filename contained" % regex)
                except vim.error:
                    pass
