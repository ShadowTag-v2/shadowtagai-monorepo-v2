...
devicons_palette["dark"].update(
  lfEval("get(get(g:, 'Lf_DevIconsPalette', {}), 'dark', {})")
)
devicons_palette["light"].update(
  lfEval("get(get(g:, 'Lf_DevIconsPalette', {}), 'light', {})")
)
...
def _matchadd(icons, pattern, priority, winid):
  ...
  for [name, glyph] in icons.items():
    ...
    if winid:
      lfCmd(
        f"""call win_execute({winid:d}, 'let matchid = matchadd(''{group_name:s}'', ''{tmp_pattern:s}'')')"""
      )
      id = int(lfEval("matchid"))
    else:
      id = int(lfEval(f"matchadd('{group_name:s}', '{tmp_pattern:s}')"))
    ids.append(id)
  return ids
