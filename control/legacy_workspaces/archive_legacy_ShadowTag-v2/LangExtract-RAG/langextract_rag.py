...
class FixedLangExtractProcessor:
    ...
    def extract_metadata(self, documents: list[dict]) -> list[dict]:
        ...
        examples = [
            self.lx.data.ExampleData(
                ...
                extractions=[
                    self.lx.data.Extraction(extraction_class="service_name", extraction_text="Payment API", attributes={}),
                    self.lx.data.Extraction(extraction_class="version_number", extraction_text="3.0", attributes={}),
                    self.lx.data.Extraction(extraction_class="document_category", extraction_text="reference", attributes={}),
                    self.lx.data.Extraction(extraction_class="rate_limits", extraction_text="500 requests per minute", attributes={}),
                ],
            )
        ]
        ...
        for doc in documents:
            ...
            try:
                result = self.lx.extract(
                    text_or_documents=doc["content"], prompt_description=prompt, examples=examples, model_id="gemini-2.5-flash", extraction_passes=2
                )
            ...
            extracted_docs.append({"id": doc["id"], "title": doc["title"], "content": doc["content"], "metadata": metadata})
    ...
    def _process_and_normalize(self, extractions, doc: dict) -> dict:
        ...
        metadata = {"service": "unknown", "version": "unknown", "doc_type": "reference", "rate_limits": [], "deprecated": False}
