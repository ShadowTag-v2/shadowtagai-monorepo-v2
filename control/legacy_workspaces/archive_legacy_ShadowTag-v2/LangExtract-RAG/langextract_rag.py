    ...
    examples = [
      self.lx.data.ExampleData(
        ...
        extractions=[
          ...
          self.lx.data.Extraction(
            extraction_class="version_number", extraction_text="3.0", attributes={}
          ),
          self.lx.data.Extraction(
  ...
  def _enhanced_regex_extraction(self, documents: list[dict]) -> list[dict]:
    ...
    for doc in documents:
      ...
      rate_matches = re.findall(
        r"(\d+)\s*(?:requests?|req)[/\s]*(?:per\s*)?min", content.lower()
      )
  ...
  for query in test_queries:
    ...
    if with_results:
      for r in with_results:
        print(
          f"      - {r['id']}: {r['metadata']['service']} v{r['metadata']['version']} ({r['metadata']['doc_type']})"
        )
