...
class TestPKGBUILDMetadata:
    ...
    def test_arch_specified(self, content):
        ...
        assert any(a in arch_content for a in ["x86_64", "any", "aarch64"]), f"Invalid arch specification: {arch_content}"
...
class TestPKGBUILDSecurity:
    ...
    def test_config_file_permissions(self, content):
        ...
        if "config.env" in content:
            assert "600" in content or "400" in content, "Config file should have restricted permissions"
