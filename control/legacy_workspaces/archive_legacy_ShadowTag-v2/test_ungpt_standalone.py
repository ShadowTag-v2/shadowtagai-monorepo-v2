...
try:
    ...
    costs = [("Gemini", gemini_cost), ("Claude", claude_cost), ("Grok", grok_cost), ("GPT-5", gpt5_cost)]
...
try:
    ...
    tier_costs = {
        QueryComplexity.SIMPLE: {"models": ["claude"], "tokens": {"claude": {"input": 800, "output": 500}}, "total": 0.017},
        QueryComplexity.MODERATE: {
            "models": ["claude", "gemini"],
            "tokens": {"claude": {"input": 1200, "output": 800}, "gemini": {"input": 1000, "output": 600}},
            "total": 0.046,
        },
        QueryComplexity.COMPLEX: {
            "models": ["claude", "gemini", "grok"],
            "tokens": {"claude": {"input": 2000, "output": 1500}, "gemini": {"input": 1500, "output": 800}, "grok": {"input": 1800, "output": 1000}},
            "total": 0.120,
        },
    }
    ...
    for tier, details in tier_costs.items():
        ...
        calculated_total = sum(calculate_cost(model, tokens["input"], tokens["output"]) for model, tokens in details["tokens"].items())
...
try:
    ...
    for tier, percentage in distribution.items():
        ...
        print(f"      {tier.capitalize():10} {percentage * 100:3.0f}% ({count:2} queries) = ${cost:.2f}/day")
...
try:
    example_queries = [
        {"query": "What is ATP 5-19?", "tier": QueryComplexity.SIMPLE, "models": ["claude"], "cost": 0.017, "time": 2.3},
        {
...
try:
    mock_response = {
        ...
        "total_cost": 0.046,
        "models_consulted": ["claude-sonnet-4", "gemini-3.1-flash"],
        "risk_level": "RA-2",
        "query_tier": "moderate",
        "reasoning_chain": [
            {"step": "claude_initial", "model": "claude-sonnet-4"},
            {"step": "gemini_analysis", "model": "gemini-3.1-flash"},
