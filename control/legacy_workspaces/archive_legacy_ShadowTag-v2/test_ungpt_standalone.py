...
try:
  ...
  for tier, details in tier_costs.items():
    ...
    for model, tokens in details["tokens"].items():
      ...
      print(
        f"      {model:10} {tokens['input']:4}→{tokens['output']:4} tokens = ${cost:.6f}"
      )
...
try:
  ...
  print("\n   ROI Analysis:")
  print(
    f"      Time saved: {time_saved_daily} min/day ({time_saved_daily / 60:.1f} hrs)"
  )
...
try:
  ...
  for i, example in enumerate(example_queries, 1):
    ...
    print(
      f'   Query: "{example["query"][:60]}{"..." if len(example["query"]) > 60 else ""}"'
    )
