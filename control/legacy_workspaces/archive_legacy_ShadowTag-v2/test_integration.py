...
async def test_agent_fallback_proposal():
    ...
    agent = GeminiAgent(name="skeptic", persona="You are a skeptical analyst", temperature=0.5, api_key=None)
    ...
    proposal2 = await agent.propose_tier(
        title="My opinion on why AI is overhyped", content="This is just my opinion on social media about AI trends...", tags=["opinion"]
    )
...
async def test_voting_methods():
    ...
    for method in methods:
        result = await chat.classify_with_debate(
            title=test_item["title"], content=test_item["content"], tags=test_item["tags"], rounds=1, voting_method=method
        )
...
async def test_persona_variations():
    ...
    for agent_name in ["skeptic", "optimist", "neutral"]:
        ...
        result = await chat.classify_with_debate(
            title=test_content["title"], content=test_content["content"], tags=test_content["tags"], rounds=1, voting_method="weighted_confidence"
        )

        print(f"{agent_name.capitalize():10} → Tier {result.tier} (confidence: {result.confidence:.0%})")
