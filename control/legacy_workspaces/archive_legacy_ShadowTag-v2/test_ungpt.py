...
print("\n[Test 1] Import Check...")
try:
  from ungpt_service import QueryComplexity, UnGPTRequest, calculate_cost
...
try:
  request = UnGPTRequest(
    query="What is edge computing?", user_location="US", max_cost=0.50
  )
...
try:
  ...
  daily_cost = sum(
    queries_per_day * distribution[tier] * estimates[tier] for tier in estimates
  )
