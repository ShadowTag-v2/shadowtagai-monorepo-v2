...
class TestFullAccuracyPipeline:
    ...
    def test_pipeline_with_expert_notes(self, real_findings):
        ...
        tracker.validate_finding(
            "da_000", is_accurate=True, expert_notes="Well-supported statistical analysis"
        )
        tracker.validate_finding(
            "da_001", is_accurate=False, expert_notes="P-value hacking suspected"
        )
...
class TestBenchmarkValidation:
    ...
    def test_validate_against_paper_benchmark(self, paper_benchmark):
        ...
        assert (
            abs(report.by_type["data_analysis"].accuracy - benchmark_accuracy["data_analysis"])
            < 0.01
        )
        assert abs(report.by_type["literature"].accuracy - benchmark_accuracy["literature"]) < 0.01
        assert (
            abs(report.by_type["interpretation"].accuracy - benchmark_accuracy["interpretation"])
            < 0.01
        )
    ...
    def test_create_custom_benchmark_from_findings(self, real_findings):
        ...
        benchmark = generator.create_from_findings(real_findings, ground_truth, name="custom_test")
