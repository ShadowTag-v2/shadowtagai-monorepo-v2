...
from kosmos.execution.data_analysis import DataAnalyzer, DataLoader
...
class TestH5adRealWorld:
    ...
    def test_pbmc3k_analysis_workflow(self, pbmc3k_dataset):
        ...
        cols_with_variance = [
            col for col in numeric_cols if df[col].std() > 0.01 and df[col].nunique() > 2
        ]
...
class TestParquetRealWorld:
    ...
    def test_load_parquet_column_subset(self, large_parquet_file):
        ...
        df = DataLoader.load_parquet(parquet_path, columns=["user_id", "revenue", "conversion"])
    ...
    def test_parquet_with_analysis(self, large_parquet_file):
        ...
        result = DataAnalyzer.ttest_comparison(df, "conversion", "page_views", (True, False))
...
class TestMixedFormatWorkflow:
    ...
    def test_load_and_combine_formats(self, tmp_path):
        ...
        csv_data = pd.DataFrame(
            {"sample_id": ["A", "B", "C"], "condition": ["control", "treatment", "treatment"]}
        )
        csv_path = tmp_path / "metadata.csv"
        csv_data.to_csv(csv_path, index=False)

        # Parquet data
        parquet_data = pd.DataFrame({"sample_id": ["A", "B", "C"], "measurement": [1.5, 2.3, 2.1]})
...
class TestDataFormatEdgeCases:
    ...
    def test_parquet_with_compression(self, tmp_path):
        ...
        df_orig = pd.DataFrame({"a": range(1000), "b": np.random.randn(1000)})
...
class TestRealScanpyWorkflow:
    ...
    def test_differential_expression_workflow(self, synthetic_scrna_data):
        ...
        result = DataAnalyzer.ttest_comparison(
            df_subset, "obs_cluster", gene_col, ("cluster_0", "cluster_1")
        )
