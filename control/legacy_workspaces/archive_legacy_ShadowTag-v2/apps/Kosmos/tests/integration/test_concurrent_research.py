...
class TestConcurrentOperationsIntegration:
    ...
    def test_fallback_to_sequential_when_concurrent_unavailable(self):
        ...
        with (
            patch(
                "kosmos.core.async_llm.AsyncClaudeClient",
                side_effect=ImportError("No async support"),
            ),
            patch(
                "kosmos.execution.parallel.ParallelExperimentExecutor",
                side_effect=ImportError("No parallel support"),
            ),
        ):
            config = {
