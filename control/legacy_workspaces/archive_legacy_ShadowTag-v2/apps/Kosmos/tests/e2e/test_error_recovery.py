...
class TestErrorRecoverability:
    ...
    def test_retry_on_recoverable_errors(self):
        ...
        rate_limit_error = ProviderAPIError(
            provider="anthropic", message="Rate limit exceeded", recoverable=True
        )
        assert is_recoverable_error(rate_limit_error) is True

        # Timeout errors are recoverable
        timeout_error = ProviderAPIError(
            provider="anthropic", message="Request timeout", recoverable=True
        )
    ...
    def test_no_retry_on_permanent_errors(self):
        ...
        invalid_error = ProviderAPIError(
            provider="anthropic", message="Invalid request format", recoverable=False
        )
...
class TestPartialBatchFailureHandling:
    ...
    async def test_partial_batch_failure_handling(self):
        ...
        async def process_item(item_id: int) -> BatchResponse:
            ...
            if item_id in [2, 5, 8]:
                return BatchResponse(id=str(item_id), success=False, error=f"Item {item_id} failed")
            return BatchResponse(id=str(item_id), success=True, response=f"Result for {item_id}")
    ...
    async def test_batch_aggregates_statistics(self):
        ...
        async def process_with_stats(item_id: int) -> BatchResponse:
            ...
            if item_id % 3 == 0:  # Every 3rd item fails
                failed_requests[0] += 1
                return BatchResponse(id=str(item_id), success=False, error="Failed")
            else:
    ...
    async def test_batch_respects_circuit_breaker(self, circuit_breaker):
        ...
        async def process_with_circuit_breaker(item_id: int) -> BatchResponse:
            ...
            if not await circuit_breaker.can_execute():
                ...
                return BatchResponse(id=str(item_id), success=False, error="Circuit breaker open")
            ...
            if item_id < 3:
                ...
                return BatchResponse(id=str(item_id), success=False, error="Processing failed")

            await circuit_breaker.record_success()
            return BatchResponse(id=str(item_id), success=True, response=f"Result {item_id}")
