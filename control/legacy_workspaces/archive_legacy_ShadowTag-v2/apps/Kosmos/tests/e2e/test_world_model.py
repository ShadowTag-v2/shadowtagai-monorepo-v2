...
from tests.e2e.factories import EntityFactory, RelationshipFactory
...
class TestEntityPersistence:
    ...
    def test_entity_persistence_across_cycles(self, in_memory_world_model):
        ...
        protocol_entity = EntityFactory.create_protocol_entity(
            protocol_id="proto_001", hypothesis_id="hyp_001", name="Temperature experiment"
        )
        wm.add_entity(protocol_entity)

        # Create TESTS relationship
        relationship = RelationshipFactory.create_tests_relationship(
            source_id=protocol_entity.id, target_id=hypothesis_entity.id, iteration=1
        )
...
class TestRelationshipCreation:
    ...
    def test_spawned_by_relationship(self):
        ...
        relationship = RelationshipFactory.create_spawned_by_relationship(
            source_id="hyp_001", target_id="question_001", generation=1
        )
    ...
    def test_tests_relationship(self):
        ...
        relationship = RelationshipFactory.create_tests_relationship(
            source_id="proto_001", target_id="hyp_001", iteration=2
        )
    ...
    def test_supports_relationship(self):
        ...
        relationship = RelationshipFactory.create_supports_relationship(
            source_id="result_001", target_id="hyp_001", p_value=0.001, effect_size=0.85
        )
...
class TestEntityConversion:
    ...
    def test_hypothesis_to_entity_flow(self, sample_hypothesis):
        ...
        entity = Entity.from_hypothesis(sample_hypothesis, created_by="HypothesisGeneratorAgent")
    ...
    def test_result_to_entity_flow(self, sample_experiment_result):
        ...
        entity = Entity.from_result(sample_experiment_result, created_by="Executor")
        ...
        assert (
            entity.properties["supports_hypothesis"] == sample_experiment_result.supports_hypothesis
        )
...
class TestWorldModelExportImport:
    ...
    def test_world_model_export_import_roundtrip(self, e2e_artifacts_dir):
        ...
        hypothesis = EntityFactory.create_hypothesis_entity(
            hypothesis_id="hyp_export_001", statement="Export test hypothesis"
        )
        entities.append(hypothesis)

        result = EntityFactory.create_result_entity(
            result_id="result_export_001", hypothesis_id="hyp_export_001"
        )
        entities.append(result)

        # Add relationship
        rel = RelationshipFactory.create_supports_relationship(
            source_id="result_export_001", target_id="hyp_export_001"
        )
        ...
        export_data = {
            ...
            "mode": "simple",
            "project": None,
            "statistics": {"entity_count": len(entities), "relationship_count": len(relationships)},
...
class TestWorldModelStatistics:
    ...
    def test_world_model_statistics_accuracy(self, in_memory_world_model):
        ...
        for i in range(3):
            wm.add_entity(EntityFactory.create_hypothesis_entity(hypothesis_id=f"hyp_{i:03d}"))

        for i in range(2):
            wm.add_entity(EntityFactory.create_result_entity(result_id=f"result_{i:03d}"))

        # Add relationships
        wm.add_relationship(
            RelationshipFactory.create_tests_relationship(
                source_id="proto_001", target_id="hyp_001"
            )
        )
        wm.add_relationship(
            RelationshipFactory.create_supports_relationship(
                source_id="result_001", target_id="hyp_001"
            )
        )
    ...
    def test_neo4j_relationship_traversal(self):
        ...
        try:
            ...
            proto_entity = EntityFactory.create_protocol_entity(
                protocol_id=f"proto_traversal_{timestamp}", hypothesis_id=hyp_entity.id
            )
            ...
            rel = RelationshipFactory.create_tests_relationship(
                source_id=proto_entity.id, target_id=hyp_entity.id
            )
            wm.add_relationship(rel)

            # Query related entities
            related = wm.query_related_entities(
                entity_id=hyp_entity.id, relationship_type="TESTS", direction="incoming"
            )
