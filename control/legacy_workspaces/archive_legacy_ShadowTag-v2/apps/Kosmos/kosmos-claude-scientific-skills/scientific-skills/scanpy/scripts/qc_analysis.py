...
def generate_qc_plots(adata, output_prefix="qc"):
    ...
    # Scatter plots
    sc.pl.scatter(
        adata, x="total_counts", y="pct_counts_mt", save=f"_{output_prefix}_mt_scatter.pdf"
    )
    sc.pl.scatter(
        adata, x="total_counts", y="n_genes_by_counts", save=f"_{output_prefix}_genes_scatter.pdf"
    )
...
def main():
    ...
    parser.add_argument(
        "--output", default="qc_filtered.h5ad", help="Output file name (default: qc_filtered.h5ad)"
    )
    parser.add_argument(
        "--mt-threshold", type=float, default=5, help="Max mitochondrial percentage (default: 5)"
    )
    parser.add_argument(
