...
Examples:
  ...
  python dicom_to_image.py multiframe.dcm frame5.png --frame 5
    ...
    parser.add_argument(
        "--apply-windowing", action="store_true", help="Apply VOI LUT windowing if available"
    )
    parser.add_argument(
        "--frame", type=int, default=0, help="Frame number for multi-frame DICOM files (default: 0)"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Show detailed conversion information"
    )
