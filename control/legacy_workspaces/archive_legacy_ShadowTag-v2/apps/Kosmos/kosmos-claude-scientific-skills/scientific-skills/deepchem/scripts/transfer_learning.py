...
def train_chemberta(
    train_dataset, valid_dataset, test_dataset, task_type="classification", n_tasks=1, n_epochs=10
):
    """
...
def main():
    ...
    parser.add_argument(
        "--target", nargs="+", default=["target"], help="Target column name(s) for custom data"
    )
    parser.add_argument(
        "--smiles-col", type=str, default="smiles", help="SMILES column name for custom data"
    )
    parser.add_argument(
        "--task-type",
        type=str,
    ...
    try:
        ...
        if args.model == "chemberta":
            model, results = train_chemberta(
                train, valid, test, task_type=task_type, n_tasks=n_tasks, n_epochs=args.epochs
            )
