...
def benchmark_node_classification(model_name, dataset_name, epochs, lr, weight_decay, device):
    ...
    model = model_class(
        num_features=dataset.num_features, hidden_channels=64, num_classes=dataset.num_classes
    ).to(device)
            ...
            try:
                if task == "node_classification":
                    result = benchmark_node_classification(
                        model_name, args.dataset, args.epochs, args.lr, args.weight_decay, device
                    )
...
def main():
    ...
    parser.add_argument(
        "--weight-decay", type=float, default=5e-4, help="Weight decay for node classification"
    )
