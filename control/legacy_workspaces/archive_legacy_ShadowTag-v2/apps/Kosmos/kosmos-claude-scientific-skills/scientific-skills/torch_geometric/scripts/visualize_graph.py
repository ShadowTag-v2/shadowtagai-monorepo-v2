...
def visualize_data(
    ...
    if node_color_attr and isinstance(node_colors, np.ndarray):
        sm = plt.cm.ScalarMappable(
            cmap=plt.cm.viridis, norm=plt.Normalize(vmin=node_colors.min(), vmax=node_colors.max())
        )
        sm.set_array([])
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="Cora", help="Dataset name (e.g., Cora, CiteSeer, ENZYMES)"
    )
    parser.add_argument(
