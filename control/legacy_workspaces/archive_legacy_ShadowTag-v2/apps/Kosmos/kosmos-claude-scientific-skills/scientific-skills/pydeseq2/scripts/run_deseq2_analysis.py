...
def run_statistical_tests(dds, contrast, alpha=0.05, shrink_lfc=True):
    ...
    ds = DeseqStats(
        dds, contrast=contrast, alpha=alpha, cooks_filter=True, independent_filter=True, quiet=False
    )
...
Examples:
  ...
  python run_deseq2_analysis.py \\
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.05, help="Significance threshold (default: 0.05)"
    )
    parser.add_argument(
        "--no-transpose",
        action="store_true",
    ...
    parser.add_argument(
        "--n-cpus", type=int, default=1, help="Number of CPUs for parallel processing (default: 1)"
    )
