...
def main():
    ...
    parser.add_argument(
        "--query", dest="query_arg", help="Search query (alternative to positional argument)"
    )
    ...
    parser.add_argument(
        "--format", choices=["json", "bibtex"], default="json", help="Output format (default: json)"
    )
    ...
    if args.format == "json":
        output = json.dumps(
            {"query": query, "count": len(metadata_list), "results": metadata_list}, indent=2
        )
    else:  # bibtex
