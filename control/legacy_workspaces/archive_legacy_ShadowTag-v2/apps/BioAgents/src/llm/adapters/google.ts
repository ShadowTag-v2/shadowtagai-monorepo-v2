...
export class GoogleAdapter extends LLMAdapter {
  ...
  protected transformResponse(response: GenerateContentResponse): LLMResponse {
    ...
    if (candidate?.content?.parts) {
      const hasCodeExecution = candidate.content.parts.some(
        (part: any) => part.executableCode || part.codeExecutionResult,
      );
      if (hasCodeExecution) {
  ...
  private applyGroundingCitations(
    ...
    return {
      textWithCitations: updatedText,
      citedChunkIndices: Array.from(citedChunks),
    };
  }
    ...
    try {
      ...
      if (uploadedFile.state === "PROCESSING") {
        ...
        while (retries < maxRetries) {
          ...
          const fileStatus = await this.client.files.get({
