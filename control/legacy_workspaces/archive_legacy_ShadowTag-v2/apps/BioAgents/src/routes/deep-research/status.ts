...
export async function deepResearchStatusHandler(ctx: any) {
  ...
  try {
    ...
    const hasActiveSteps = Object.values(steps).some((step: any) => step.start && !step.end);
...
async function deepResearchRetryHandler(ctx: any) {
  ...
  if (!startMutex.acquired && !startMutex.fallback) {
    logger.warn({ jobId, conversationStateId }, "deep_research_retry_proceeding_without_mutex");
  }
