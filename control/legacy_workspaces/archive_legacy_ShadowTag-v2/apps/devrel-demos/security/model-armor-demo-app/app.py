...
def get_cached_auth_token():
  ...
  print("INFO: Authentication token refreshed and cached")
...
def cache_result(cache_key, result):
  ...
  if len(model_armor_cache) > 100:
    ...
    expired_keys = [
      k
      for k, (_, timestamp) in model_armor_cache.items()
...
def get_genai_client(location):
  if location not in genai_clients:
    genai_clients[location] = genai.Client(
...
def pre_initialize_clients():
  ...
  for endpoint_info in model_armor_endpoints:
    try:
      ...
      print(
        f"  - Successfully initialized Model Armor client for {endpoint_info['location']}"
      )
    except Exception as e:
      print(
        f"  - WARNING: Failed to initialize Model Armor client for {endpoint_info['location']}: {e}"
      )

  # Pre-warm Generative AI clients
  unique_locations = {
    model.get("location", "us-central1") for model in foundation_models
  }
...
model_armor_endpoints = [
  ...
  {
    "location": "us-east1",
    "endpoint": "modelarmor.us-east1.rep.googleapis.com",
    "display_name": "us-east1",
  },
  {
    "location": "europe-west4",
    "endpoint": "modelarmor.europe-west4.rep.googleapis.com",
    "display_name": "europe-west4",
  },
  {
    "location": "asia-southeast1",
    "endpoint": "modelarmor.asia-southeast1.rep.googleapis.com",
...
foundation_models = [
  {
    ...
    "provider": "Google",
    "location": "global",
    "display_name": "gemini-2.5-flash-thinking-exp-01-21",
  },
  {
  ...
  try:
    ...
    for template in page_result:
      template_info = {
        ...
        "display_name": template.name.split("/")[-1],
        "location": location,
...
def process_template_results(output_str):
  matches = re.finditer(
    r'key: "([^"]+)"[^}]*?match_state: MATCH_FOUND', output_str, re.DOTALL
  )
...
def check_sdp_transformation_for_file(response_data):
  ...
  try:
    deidentify_result = (
      response_data.get("sanitizationResult", {})
      .get("filterResults", {})
      .get("sdp", {})
      .get("sdpFilterResult", {})
...
def analyze_response_with_template(
  response_text, template_name, location, modelarmor_client, use_default_response
):
  template_display_name = template_name
  try:
    model_response_data = modelarmor_v1.DataItem()
    model_response_data.text = response_text
    response_sanitize_request = modelarmor_v1.SanitizeModelResponseRequest(
      name=get_template_path(template_name, location),
      model_response_data=model_response_data,
    )
    response_check = modelarmor_client.sanitize_model_response(
      request=response_sanitize_request
    )
    ...
    details = (
      "❌ Violations found:\n" + "\n".join(f"• {result}" for result in filter_results)
      if filter_results
    ...
    return {
      ...
      "analysis": {
        "template": template_display_name,
        "status": "error",
        "details": f"Error in Model Armor analysis: {e}",
        "matches": False,
        "raw_output": str(e),
      },
...
def sanitize_file_prompt_with_rest_api_optimized(
  file_data_base64, mime_type, template_name, location, endpoint
):
  """Optimized version with caching and connection reuse"""
  # Check cache first
  cache_key = get_cache_key(
    {"base64_data": file_data_base64, "mime_type": mime_type},
    template_name,
    location,
  )
...
def sanitize_text_prompt_optimized(text, template_name, location, endpoint_info):
  ...
  try:
    ...
    prompt_check = modelarmor_client.sanitize_user_prompt(
      request=prompt_sanitize_request
    )
...
async def analyze_prompt_async(
  prompt, file_data, prompt_template, location, endpoint_info
):
  ...
  def run_analysis():
    if file_data:
      ...
      result = sanitize_text_prompt_optimized(
        prompt, prompt_template, location, endpoint_info
      )
...
async def generate_response_async(prompt, model_info, system_instruction, file_data):
  ...
  return await loop.run_in_executor(
    None, generate_model_response, prompt, model_info, system_instruction, file_data
  )


async def analyze_response_async(
  response_text, response_template, location, endpoint_info, use_default_response
):
...
async def process_chat_async(
  ...
  if prompt_template:
    ...
    if is_file_analysis and sdp_transformed_text:
      ...
      print(
        "INFO: Constructed new prompt from redacted file content. Original file data will not be sent to LLM."
      )
  ...
  model_response = await generate_response_async(
    prompt_for_llm, model_info, system_instruction, file_data_for_llm
  )
  ...
  if response_template:
    ...
    response_result = await analyze_response_async(
      model_response,
      response_template,
      location,
      endpoint_info,
...
def home():
  ...
  prompt_templates, response_templates = fetch_model_armor_templates(
    initial_location, initial_endpoint
  )
  return render_template(
    "index.html",
    foundation_models=foundation_models,
    model_armor_endpoints=model_armor_endpoints,
...
def get_templates_for_location(location):
  endpoint_info = next(
    (e for e in model_armor_endpoints if e["location"] == location), None
  )
  if not endpoint_info:
    return jsonify({"error": "Invalid location"}), 404
  prompt_templates, response_templates = fetch_model_armor_templates(
    location, endpoint_info["endpoint"]
  )
  return jsonify(
    {"prompt_templates": prompt_templates, "response_templates": response_templates}
  )
  ...
  try:
    ...
    if "file" in request.files:
      ...
      if not file or not prompt_template or not location:
        return jsonify(
          {"error": "Missing file, template, or location for analysis"}
        ), 400
      ...
      if file_cache_key in file_cache:
        ...
        if len(file_cache) > 50:
          ...
          expired_keys = [
            k
            for k, (_, timestamp) in file_cache.items()
            if current_time - timestamp > CACHE_TTL
          ]
          for k in expired_keys:
            del file_cache[k]
    ...
    # Handle text-only scenario
      ...
      output_str = sanitize_text_prompt_optimized(
        prompt, prompt_template, location, endpoint_info
      )
      filter_results = process_template_results(output_str)
    ...
    return jsonify(
      {
        ...
        "prompt_analysis": {
          ...
          or request.get_json().get("promptTemplate"),
          "status": "error",
...
def chat():
  ...
  if "file" in request.files:
    ...
    prompt = (
      prompt_text if prompt_text else f"Please analyze this document: {file.filename}"
    )
    is_file_upload = True
  ...
  model_info = next(
    (
      ...
      if m.get("display_name") == model_name_from_ui or m["name"] == model_name_from_ui
    ),
    None,
  )
  if not model_info:
