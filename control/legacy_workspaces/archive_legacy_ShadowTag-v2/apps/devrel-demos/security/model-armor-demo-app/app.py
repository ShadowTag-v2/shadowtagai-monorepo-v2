...
def cache_result(cache_key, result):
    ...
    if len(model_armor_cache) > 100:
        current_time = time.time()
        expired_keys = [k for k, (_, timestamp) in model_armor_cache.items() if current_time - timestamp > CACHE_TTL]
...
def get_model_armor_client(location, endpoint):
    if location not in model_armor_clients:
        model_armor_clients[location] = modelarmor_v1.ModelArmorClient(transport="rest", client_options={"api_endpoint": endpoint})
...
def check_sdp_transformation_for_file(response_data):
    ...
    try:
        deidentify_result = (
            response_data.get("sanitizationResult", {}).get("filterResults", {}).get("sdp", {}).get("sdpFilterResult", {}).get("deidentifyResult", {})
        )
        ...
        if "file" in request.files:
            ...
            if file_cache_key in file_cache:
                ...
                if len(file_cache) > 50:
                    current_time = time.time()
                    expired_keys = [k for k, (_, timestamp) in file_cache.items() if current_time - timestamp > CACHE_TTL]
                    for k in expired_keys:
                        del file_cache[k]
...
def chat():
    ...
    model_info = next(
        (m for m in foundation_models if m.get("display_name") == model_name_from_ui or m["name"] == model_name_from_ui),
        None,
    )
    if not model_info:
