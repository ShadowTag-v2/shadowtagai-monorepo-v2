...
cloudEvent("index", async (cloudevent: CloudEvent<StorageObjectData>) => {
  ...
  const hashedFilePath = createHmac("sha256", secret).update(filePath).digest("hex");

  //Store with filePath as key
  await firestore.doc(`${cloudevent.data.bucket}/${hashedFilePath}`).set({
  ...
  console.log(
    `Successfully stored ${cloudevent.data.name} to Firestore (${cloudevent.data.bucket}/${hashedFilePath})`,
  );
});
