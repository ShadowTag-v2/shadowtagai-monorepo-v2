...
export default function PlayerList({ game }: { game: Game }) {
  ...
  const playerDisplayNames = Object.values(game.players)
    .sort()
    .sort((a) => (a === currentPlayerName ? -1 : 0));
  ...
  return (
    <center className="mx-auto max-w-7xl">
      {currentPlayerName && (
        <div className="mt-5">
          <div className="player-list-item">{currentPlayerName}</div>
          {" ← You!"}
        </div>
      )}
      <div className="mt-5">
        {playerDisplayNames.map((displayName) => (
          <div key={displayName} className="player-list-item">
            {displayName}
          </div>
        ))}
      </div>
