...
import { type Game, gameStates, type Question } from "@/app/types";
...
export default function QuestionPanel({
  ...
  const existingGuesses =
    currentQuestion?.playerGuesses && currentQuestion.playerGuesses[authUser.uid];
  ...
  useEffect(() => {
    setAnswersSelectedCount(
      answerSelection.reduce((correctAnswerCount, answerIsSelected) => {
  ...
  const onAnswerClick = async (answerIndex: number) => {
    if (game.state === gameStates.AWAITING_PLAYER_ANSWERS && !isGameLeader) {
      ...
      await updateAnswerAction({
        gameId,
        answerSelection: newAnswerSelection,
        tokens,
      });
    }
  };
    ...
    <div className={`grid lg:grid-cols-2`}>
        ...
        <BorderCountdownTimer game={game} gameRef={gameRef}>
          <div className="flex flex-col justify-between h-full">
            ...
            <div>
              {isShowingCorrectAnswers ? (
                <>
                  <h2 className="md:text-xl lg:text-2xl pt-5">{currentQuestion.explanation}</h2>
                </>
              ) : (
                <div>
                  <h2 className="text-xl lg:text-4xl pt-5">Pick {totalCorrectAnswerOptions}</h2>
                  {hasEverGuessed && (
              ...
              {hasEverGuessed && currentPlayer.displayName && (
                <div className="mt-2">
                  ...
                  {isShowingCorrectAnswers &&
                    ...
                    ` and you have ${currentPlayer.score} point${currentPlayer.score === 1 ? "" : "s"}`}
                </div>
              )}
            </div>
          </div>
            ...
            <>
              <div className="flex h-full">
                <div className="m-auto">
                  <div>Just getting here?</div>
                  <div>Scan the QR Code to join the game!</div>
      ...
      <div className="grid grid-rows-4 h-[50dvh] lg:h-full">
        {currentQuestion.answers.map((answer, index) => {
          const guessesForThisAnswer = Object.values(currentQuestion.playerGuesses || []).reduce(
            (playerGuesses, guess) => {
          ...
          const guessPercentageForThisAnswer =
            (guessesForThisAnswer / (totalPlayersWhoMadeAGuess || 1)) * 100;
          ...
          return (
            <div className="flex" key={`${currentQuestion.prompt} ${answer.text}`}>
              <button
              ...
              >
                  ...
                  <>
                    <div className="absolute bottom-0 right-0 border-8 border-transparent min-w-fit h-full text-right text-lg lg:text-2xl flex">
                      ...
                      <div className="bg-white pl-1 h-fit my-auto">
                        <div className="whitespace-nowrap">{answerDisplayText()}</div>
                ...
                <div
                  ...
                  style={{
                    borderColor:
                      answer.isCorrect || !isShowingCorrectAnswers
                        ? `var(--google-cloud-${color})`
                        : "#9ca3af",
                  }}
                />
