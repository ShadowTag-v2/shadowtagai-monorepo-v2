...
export const TimePerQuestionSchema = z
  .number({ invalid_type_error: "Time per question must be a number" })
  .int()
  .max(600, "Time per question must be 600 or less.")
  .min(10, "Time per question must be at least 10.");
export const TimePerAnswerSchema = z
  .number({ invalid_type_error: "Time per answer must be a number" })
  .int()
  .max(600, "Time per answer must be 600 or less.")
  .min(5, "Time per answer must be at least 5.");
...
const gameStatesOptions = [
  "NOT_STARTED",
  "SHOWING_CORRECT_ANSWERS",
