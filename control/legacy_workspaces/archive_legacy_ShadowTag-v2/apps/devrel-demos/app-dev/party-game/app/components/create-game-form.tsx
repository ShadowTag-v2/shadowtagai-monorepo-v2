...
import {
  type QuestionAdvancement,
  QuestionAdvancementEnum,
...
export default function CreateGameForm() {
  ...
  const [timePerQuestionInputValue, setTimePerQuestionInputValue] = useState<string>(
    defaultTimePerQuestion.toString(),
  );
  const [timePerAnswerInputValue, setTimePerAnswerInputValue] = useState<string>(
    defaultTimePerAnswer.toString(),
  );
  ...
  return (
    <div className="w-full max-w-lg mx-auto border-8 border-r-[var(--google-cloud-blue)] border-t-[var(--google-cloud-red)] border-b-[var(--google-cloud-green)] border-l-[var(--google-cloud-yellow)]">
      <form className="bg-white px-8 py-12" onSubmit={onCreateGameSubmit}>
        ...
        {questionAdvancement === AUTOMATIC && (
          <>
            <div className="mb-6">
              ...
              />
              <p className="text-red-500 text-xs italic">
                {timePerAnswerError ? timePerAnswerError : <>&nbsp;</>}
              </p>
            </div>
          </>
        )}
        ...
        <center>
          <BigColorBorderButton type="submit">Create Game</BigColorBorderButton>
        </center>
      </form>
    </div>
  );
