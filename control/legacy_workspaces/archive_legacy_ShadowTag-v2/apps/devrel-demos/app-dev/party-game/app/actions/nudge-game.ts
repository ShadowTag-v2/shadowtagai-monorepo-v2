...
export async function nudgeGameAction({
  ...
  switch (game.state) {
    ...
    case NOT_STARTED:
      if (
        desiredState.state === AWAITING_PLAYER_ANSWERS &&
        desiredState.currentQuestionIndex === 0
      ) {
      ...
      throw new Error(
        "The only allowed game progression when NOT_STARTED is to AWAITING_PLAYER_ANSWERS",
      );
    case SHOWING_CORRECT_ANSWERS:
      ...
      throw new Error(
        "The only allowed game progression when SHOWING_CORRECT_ANSWERS is to AWAITING_PLAYER_ANSWERS on the next question",
      );
    case AWAITING_PLAYER_ANSWERS:
      if (
        desiredState.state === SHOWING_CORRECT_ANSWERS &&
        desiredState.currentQuestionIndex === game.currentQuestionIndex
      ) {
        await gameRef.update({
      ...
      throw new Error(
        "The only allowed game progression when AWAITING_PLAYER_ANSWERS is to SHOWING_CORRECT_ANSWERS",
      );
  }
}
