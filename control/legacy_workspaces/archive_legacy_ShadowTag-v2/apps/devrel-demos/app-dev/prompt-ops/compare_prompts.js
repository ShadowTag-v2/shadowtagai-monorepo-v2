...
import { cos_sim as cosSim, pipeline } from "@xenova/transformers";
 ...
 * response.
 */
async function getSpamResponseFromAI(genAI, modelName, prompt, imagePath = null) {
 ...
 */
async function evaluateMessage(message, promptTemplates, genAI, modelName, extractor) {
  const results = {};
  for (const template of promptTemplates) {
    ...
    try {
      ...
      console.error(`Error processing prompt "${template}" for message "${message.text}":`, error);
...
function printResults(aggregatedResults) {
  ...
  for (const [template, data] of Object.entries(aggregatedResults)) {
    const accuracy = data.total > 0 ? ((data.correct / data.total) * 100).toFixed(0) : 0;
    const avgSimilarity =
      data.similarities.length > 0
        ? (data.similarities.reduce((a, b) => a + b, 0) / data.similarities.length) * 100
        : 0;
    console.log(`\nPrompt: "${template}"`);
    console.log(`  - Accuracy: ${accuracy}% (${data.correct}/${data.total})`);
    console.log(`  - Explanation Similarity: ${avgSimilarity.toFixed(0)}% (avg. cosine)`);
  }
}
...
async function runEvaluation(messages, genAI, extractor) {
  ...
  const promises = messages.map((message) =>
    evaluateMessage(message, CONFIG.PROMPT_TEMPLATES, genAI, CONFIG.MODEL_NAME, extractor),
  );
