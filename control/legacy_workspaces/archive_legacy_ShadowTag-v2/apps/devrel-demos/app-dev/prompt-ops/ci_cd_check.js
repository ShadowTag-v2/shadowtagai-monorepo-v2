...
async function evaluateMessages(genAI, messages) {
  ...
  const promises = messages.map((message) => {
    ...
    return getSpamResponseFromAI(genAI, CONFIG.MODEL_NAME, fullPrompt, message.image_path).then(
  ...
  for (const result of results) {
    if (result.status === "fulfilled") {
      ...
      if (aiResponse.is_spam === message.is_spam) {
        ...
        console.error(`Incorrect classification for message: "${message.text}"`);
        console.error(`  - Expected: ${message.is_spam}, Got: ${aiResponse.is_spam}`);
      }
    } else {
...
function reportFinalResult(correctCount, totalMessages) {
  ...
  if (accuracy >= CONFIG.ACCURACY_THRESHOLD) {
    console.log(
      `✅ Accuracy is above the ${CONFIG.ACCURACY_THRESHOLD * 100}% threshold. CI/CD check passed.`,
    );
    process.exit(0);
  } else {
    console.error(
      `❌ Accuracy is below the ${CONFIG.ACCURACY_THRESHOLD * 100}% threshold. CI/CD check failed.`,
    );
    process.exit(1);
  }
}
