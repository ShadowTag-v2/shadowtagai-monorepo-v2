...
1. Allow the service account access to this secret, replacing `MYPROJECT` with your project ID:
   1. In Secret Manager, and click on the **Permission** tab, and click
      **Grant Access**
      _ New Principals: `cloud-build-sa@MYPROJECT.iam.gserviceaccount.com`
      _ Role: Secret Manager Secret Accessor
   1. Click **Save**.
...
1.  Go to your repository, and make a visual change to `app.py` in a [new
    branch](https://docs.github.com/en/free-pro-team@latest/github/collaborating-with-issues-and-pull-requests/creating-and-deleting-branches-within-your-repository). 1. Go to `app.py`, and click the pencil icon (✎)
...
4.  After the trigger completes, a new status check named "_Deployment Preview_"
...
The `--to-latest` parameter offers the same functionality as the _Serve this
revision immediately_ checkbox in the Cloud Run page. It ensures that this
