...
The "broken AI" experience in a multi-tasking chat wasn't just an annoyance. It was a key lesson. It showed the great need for careful context engineering. To understand _why_ the AI "broke," we must look closer at when and how its performance suffered.
...
- **Hallucinations:** The AI started making up facts. It said them confidently, but they were wrong. It often mixed details from old, unrelated tasks into its current answers. For example, when debugging a `Go` error, it might invent non-existent `Python` library functions.
...
More context isn't always better. Its _quality_ is what truly matters. This idea changes how we talk to AI. It pushes us to be more thoughtful and clear.

#### **The primacy of quality over quantity**

We might want to give an AI all related information, hoping for better answers. But this can backfire, as the "Lost in the Middle" problem shows. Think of giving a chef a whole cookbook for one recipe. The sheer volume might hide the exact steps needed. This could lead to confusion.

Good context engineering means taking a precise approach:

- **Strategic filtering:** Don't just dump data. Actively filter out extra information. Focus on what's _truly_ important for the task. Remove what's related but not essential. Tools that let you choose context, or short rules, are very helpful.
- **Concise and focused context:** Give the AI a small, clear context that directly helps with the current goal. This means summarizing long documents. Pull out only key data. Turn old conversations into short summaries before giving them to the AI again. This eases the model's thinking load. It also helps the AI focus on important details.
...
By smartly dividing tasks and actively managing context, we can move past fixing "broken AIs." We can build a more planned and useful partnership with advanced language models.
