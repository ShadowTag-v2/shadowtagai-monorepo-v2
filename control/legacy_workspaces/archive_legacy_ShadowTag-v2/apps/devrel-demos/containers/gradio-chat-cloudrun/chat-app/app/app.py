...
def call_gemma_model(aggregated_message, model_temperature, top_p, max_tokens, config):
    json_message = {
...
def process_message_gemma(message, history):
    ...
    aggregated_message = history_message + new_user_message + "Assistant's Turn:\n>>> "
...
def inference_interface(
    ...
    if not doc_ref.get().exists:
        doc_ref.set({"Session start": datetime.datetime.now(datetime.UTC)})
...
max_tokens = gr.Slider(minimum=1, maximum=4096, value=1024, label="Max Tokens", render=False)
