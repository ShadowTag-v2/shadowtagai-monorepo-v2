...
class TestContacts(unittest.TestCase):
    ...
    def test_create_contact_409_status_code_duplicate_contact_with_diff_label(
        ...
        self.mocked_db.return_value.get_contacts.return_value = [create_new_contact()]
    ...
    def test_create_contact_409_status_code_duplicate_contact_with_same_label(
        ...
        self.assertEqual(response.status_code, 409)
        # assert we get correct error message
        self.assertEqual(response.data, b"contact already exists with that label")

    def test_create_contact_400_status_code_invalid_account_number_less_than_ten_digits(
        self,
    ):
    ...
    def test_create_contact_400_status_code_invalid_routing_number_more_than_nine_digits(
        self,
    ):
    ...
    def test_create_contact_400_status_code_is_external_routing_num_equals_local_routing(
        self,
    ):
    ...
    def test_get_contacts_200_list_of_contacts(self):
        ...
        # send request to test client
        response = self.test_app.get(f"/contacts/{EXAMPLE_USER}", headers=EXAMPLE_HEADERS)
        # assert 200 response code
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_contacts_401_get_contacts_invalid_auth(self):
        ...
        invalid_token_header["Authorization"] = "foo"
        # send request to test client
        response = self.test_app.get(f"/contacts/{EXAMPLE_USER}", headers=invalid_token_header)
        # assert 200 response code
        self.assertEqual(response.status_code, 401)
        # assert we get correct error message
        self.assertEqual(response.data, b"authentication denied")
