...
def create_app():
    ...
    def home():
        ...
        _populate_contact_labels(account_id, api_response[TRANSACTION_LIST_NAME], api_response[CONTACTS_NAME])
    ...
    def payment():
        ...
        try:
            ...
            msg = f"Payment failed: {user_input} is not a valid number"
    ...
    def format_currency(int_amount):
        ...
        if int_amount is None:
            return "$---"
        amount_str = f"${abs(Decimal(int_amount) / 100):0,.2f}"
        if int_amount < 0:
            amount_str = "-" + amount_str
        return amount_str

    # set up global variables
    app.config["TRANSACTIONS_URI"] = "http://{}/transactions".format(os.environ.get("TRANSACTIONS_API_ADDR"))
    app.config["USERSERVICE_URI"] = "http://{}/users".format(os.environ.get("USERSERVICE_API_ADDR"))
    app.config["BALANCES_URI"] = "http://{}/balances".format(os.environ.get("BALANCES_API_ADDR"))
    app.config["HISTORY_URI"] = "http://{}/transactions".format(os.environ.get("HISTORY_API_ADDR"))
    app.config["LOGIN_URI"] = "http://{}/login".format(os.environ.get("USERSERVICE_API_ADDR"))
    app.config["CONTACTS_URI"] = "http://{}/contacts".format(os.environ.get("CONTACTS_API_ADDR"))
    app.config["PUBLIC_KEY"] = open(os.environ.get("PUB_KEY_PATH")).read()
