...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  depositForm.addEventListener("submit", (e) => {
    ...
    if (!depositForm.checkValidity() || document.querySelector("#deposit-amount").value <= 0.0) {
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  paymentForm.addEventListener("submit", (e) => {
    ...
    if (!paymentForm.checkValidity() || document.querySelector("#payment-amount").value <= 0.0) {
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  document.querySelector("#payment-accounts").addEventListener("change", (e) => {
    RefreshModals();
  });

  // Handle new account option in Deposit modal
  document.querySelector("#accounts").addEventListener("change", (e) => {
    RefreshModals();
  });
