...
import { IconCircle, IconSquare, IconTriangle, IconX } from "@tabler/icons-react";
          ...
          if (data.replicas >= 50 && gameStateRef.current === "playing") {
            ...
            setTimeout(() => {
              if (gameStateRef.current === "playing") {
                // Double check state in case user won points simultaneously
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (gameState === "idle" && event.key === "Enter") {
        ...
        gameState === "playing" &&
        (ARROW_KEYS.includes(event.key) || ALLOWED_KEYS.includes(event.key))
      ) {
  ...
  return (
    <div style={{ fontFamily: "sans-serif", textAlign: "center", padding: "20px" }}>
      ...
      {gameState === "idle" && (
        <div style={{ marginTop: "50px" }}>
          <p style={{ fontSize: "4em" }}>
            Press <span className="bold">Enter</span> to start the race!
          </p>
          ...
          >
            ...
            >
              <h2 style={{ fontSize: "2em" }}>
                Your Points: <span style={{ color: "green" }}>{points}</span> / 50
              </h2>
            ...
            <div
              style={{
                ...
                borderRadius: "8px",
                minWidth: "200px",
                backgroundColor: replicaCount >= 50 ? "#fdd" : "transparent",
              }}
            >
              <h2>
                Kubernetes Replicas:{" "}
                <span style={{ color: replicaCount >= 50 ? "red" : "orange" }}>{replicaCount}</span>{" "}
                / 50
              </h2>
              {replicaCount < 50 && <p>Scaling...</p>}
              {replicaCount >= 50 && (
                <p style={{ color: "red", fontWeight: "bold" }}>Scale Up Complete!</p>
              )}
            </div>
          </div>
      ...
      {gameState === "finished" && (
        <div style={{ marginTop: "50px" }}>
          ...
          <p style={{ fontSize: "1.2em" }}>
            Your final score: <span style={{ fontWeight: "bold" }}>{points}</span>
          </p>
          <p style={{ fontSize: "1.2em" }}>
            Final Kubernetes replicas: <span style={{ fontWeight: "bold" }}>{replicaCount}</span>
          </p>
          <button
      ...
      <div style={{ marginTop: "40px", fontSize: "0.9em", color: "#555" }}>
        <p>
          Instructions: Press the arrow keys (↑, ↓, ←, →), (X, O, □, ∆) or (w, a, s, d) in the order
          shown to gain points.
        </p>
        <p>(↑ == w == ∆, ↓ == s == O, ← == a == X, → == d == □)</p>
