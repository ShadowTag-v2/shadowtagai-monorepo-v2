...
def create_agent_app():
    ...
    bq_plugin = BigQueryAgentAnalyticsPlugin(project_id=PROJECT_ID, dataset_id=DATASET_ID, table_id=AGENT_LOGS_TABLE_ID)
...
def deploy(agent_name: str, agent_desc: str):
    ...
    try:
        remote_agent = client.agent_engines.create(agent=agent_app, config=deployment_config)
        print("\n✅ Deployment Complete!")
...
if __name__ == "__main__":
    deploy(
        "Knowledge Agent",
        "ADK Knowledge Agent for BigQuery Knowledge Graph and Maps analytics",
    )
