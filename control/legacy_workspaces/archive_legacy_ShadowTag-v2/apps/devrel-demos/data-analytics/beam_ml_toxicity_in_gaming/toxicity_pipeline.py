...
def run(project_id, gaming_model_location, movie_model_location, pipeline_args):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        gaming_inference = read_from_pubsub | "Perform gaming inference" >> RunInference(gaming_model_handler)
        ...
        joined = ({"gaming": gaming_inference, "movie": movie_inference}) | "Join" >> beam.CoGroupByKey()

        # Simple string schema - normally not recommended
        # For brevity sake, we convert to a single string
        data_schema = {"fields": [{"name": "data_col", "type": "STRING", "mode": "NULLABLE"}]}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project_id", dest="project_id", required=True, help=("project id"))
    parser.add_argument("--gaming", dest="gaming_loc", required=True, help=("location of gaming model"))
    parser.add_argument("--movie", dest="movie_loc", required=True, help=("location of movie model"))
