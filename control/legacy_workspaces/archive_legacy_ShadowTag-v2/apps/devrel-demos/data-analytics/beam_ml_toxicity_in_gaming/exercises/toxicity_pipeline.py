...
class extendTFModelHandlerTensor(
  beam.ml.inference.tensorflow_inference.TFModelHandlerTensor
):
...
def run(project_id, gaming_model_location, movie_model_location, pipeline_args):
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    ...
    gaming_model_handler = KeyedModelHandler(
      extendTFModelHandlerTensor(gaming_model_location)
    )
    ...
    gaming_inference = read_from_pubsub | "Perform gaming inference" >> RunInference(
      gaming_model_handler
    )

    # Flag the values so we can determine if toxic or not
    nice_or_not = gaming_inference | beam.ParDo(flag_for_toxic())
    ...
    _ = (
      ...
      >> beam.Map(lambda element: bytes(str(element[1]), "UTF-8"))
      | beam.io.WriteToPubSub(topic=output_topic)
    )

    # Load the model into a handler
    movie_model_handler = KeyedModelHandler(
      extendTFModelHandlerTensor(movie_model_location)
    )
    ...
    # Use the handler to perform inference
    movie_inference = read_from_pubsub | "Perform movie inference" >> RunInference(
      movie_model_handler
    )
    ...
    data_schema = {
      "fields": [{"name": "data_col", "type": "STRING", "mode": "NULLABLE"}]
    }
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--project_id", dest="project_id", required=True, help=("project id")
  )
  parser.add_argument(
    "--gaming", dest="gaming_loc", required=True, help=("location of gaming model")
  )
  parser.add_argument(
    "--movie", dest="movie_loc", required=True, help=("location of movie model")
  )
  known_args, pipeline_args = parser.parse_known_args()
  run(
    known_args.project_id,
    known_args.gaming_loc,
    known_args.movie_loc,
    pipeline_args,
  )
