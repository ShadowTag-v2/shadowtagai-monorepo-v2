...
def create_and_run_scan(
  ...
  try:
    ...
    client.create_data_scan(
      parent=parent, data_scan=data_scan, data_scan_id=data_scan_id
  ...
  try:
    ...
    print(
      f"[SUCCESS] Job started for '{data_scan_id}'. Job ID: {run_response.job.name.split('/')[-1]}"
    )
...
def main():
  ...
  if not all([PROJECT_ID, LOCATION, DATASET_ID]):
    ...
    print(
      "Please ensure PROJECT_ID, LOCATION, and DATASET_ID are exported in your shell."
    )
    sys.exit(1)

  print(
    f"[INFO] Using Project: {PROJECT_ID}, Location: {LOCATION}, Dataset: {DATASET_ID}"
  )
  ...
  for view_name in TARGET_VIEWS:
    ...
    job = create_and_run_scan(
      client, PROJECT_ID, LOCATION, data_scan_id, target_resource
    )
    if job:
    ...
    for job in running_jobs:
      ...
      try:
        ...
        if state in (
          ...
          dataplex_v1.DataScanJob.State.CANCELING,
        ):
