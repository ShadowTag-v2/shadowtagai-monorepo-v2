...
class GeminiModelHandler(ModelHandler):
  ...
  def run_inference(
    ...
    for one_text in batch:
      contents = [
        types.Content(role="user", parts=[types.Part.from_text(text=one_text)])
      ]
      result = ""
...
def run(argv=None, save_main_session=True):
  ...
  parser.add_argument(
    "--alloydb_ip", dest="alloydb_ip", required=True, help="AlloyDB IP Address"
  )
  parser.add_argument(
    "--alloydb_port", dest="alloydb_port", default="5432", help="AlloyDB Port"
  )
  parser.add_argument(
    "--alloydb_database",
  ...
  parser.add_argument(
    "--pubsub_subscription", dest="pubsub_subscription", required=True
  )
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    pubsub_pcoll = (
      p
      | "ReadFromPubSub"
      >> ReadFromPubSub(subscription=known_args.pubsub_subscription).with_output_types(
        bytes
      )
    ...
    embedding_pcoll | "Write to jdbc" >> WriteToJdbc(
      ...
      jdbc_url=(
        f"jdbc:postgresql://{known_args.alloydb_ip}:{known_args.alloydb_port}/{known_args.alloydb_database}"
      ),
