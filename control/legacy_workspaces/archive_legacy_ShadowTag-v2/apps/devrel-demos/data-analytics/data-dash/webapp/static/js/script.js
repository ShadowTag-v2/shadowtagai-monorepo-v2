$(document).ready(() => {
  ...
  function setCheckpoints(div, checkpointsMap) {
    for (let i = 0; i < 8; i++) {
      ...
      const selector = div + " tr";
      const row = $(selector).eq(checkpoint);
      const valEl = row.find("td.checkpoint_val");
      const checkpointValue = checkpointsMap[checkpoint];
