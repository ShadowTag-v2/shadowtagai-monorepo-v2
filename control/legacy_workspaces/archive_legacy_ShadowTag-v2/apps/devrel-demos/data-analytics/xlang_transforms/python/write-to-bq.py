...
def run(input_files, transform_jar, table_name, pipeline_args):
    ...
    java_transform = JavaExternalTransform("com.google.beam.workshop.FieldPrefixAdderTransform", classpath=[transform_jar]).create(
        "rate_code", "prefix-"
    )

    write_to_bigquery = (
        JavaExternalTransform("com.google.beam.workshop.WriteToBigQuery", classpath=[transform_jar])
        .writeTo(table_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_files", dest="input_files", required=True, help=("input file pattern"))
    parser.add_argument(
        "--transform_jar",
        dest="transform_jar",
        required=True,
