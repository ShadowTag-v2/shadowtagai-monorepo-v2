...
def run(input_files, transform_jar, table_name, pipeline_args):
  pipeline_options = PipelineOptions(pipeline_args, save_main_session=True)

  java_transform = JavaExternalTransform(
    "com.google.beam.workshop.FieldPrefixAdderTransform", classpath=[transform_jar]
  ).create("rate_code", "prefix-")

  write_to_bigquery = (
    JavaExternalTransform(
      "com.google.beam.workshop.WriteToBigQuery", classpath=[transform_jar]
    )
    .writeTo(table_name)
    .usingStorageApiAtLeastOnceMethod()
  )
  ...
  with beam.Pipeline(options=pipeline_options) as pipeline:
    ...
    sql_group_by | "Print after SQL" >> beam.Map(
      lambda row: logging.info("After SQL:" + str(row))
    )

    appended_sql_group_by = sql_group_by | "Send to Java" >> java_transform

    appended_sql_group_by | "Print Converted" >> beam.Map(
      lambda row: logging.info("After Java:" + str(row))
    )
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--input_files", dest="input_files", required=True, help=("input file pattern")
  )
  parser.add_argument(
    "--transform_jar",
    dest="transform_jar",
  ...
  parser.add_argument(
    ...
    dest="table_name",
    required=True,
    help="table name to save results to",
  )
  known_args, pipeline_args = parser.parse_known_args()

  run(
    known_args.input_files,
    known_args.transform_jar,
