    ...
    for item in batch:
      ...
      try:
        ...
        if not text_to_classify:
          logging.warning(
            f"Input JSON missing 'text' key or text is empty: {json_string_for_output}"
          )
          yield PredictionResult(
            example=json_string_for_output, inference="ERROR_NO_TEXT"
          )
          continue
        ...
        yield PredictionResult(
          example=json_string_for_output, inference=classification_tag
        )

      except json.JSONDecodeError as e:
        logging.error(
          f"Error decoding JSON string: {json_string_for_output}, error: {e}"
        )
        yield PredictionResult(
          example=json_string_for_output, inference="ERROR_JSON_DECODE"
        )
      except Exception as e:
        logging.error(
          f"Error during Gemini inference for input {json_string_for_output}: {e}"
        )
        yield PredictionResult(
          example=json_string_for_output, inference="ERROR_INFERENCE"
        )


# --- GeminiAccuracyModelHandler ---
# This ModelHandler is specifically designed for the second LLM call:
# verifying the accuracy of the previous response.
# It takes the original prompt and text, and asks Gemini to score the accuracy.
#
# Input to run_inference: A batch of JSON strings, each containing
#   'original_prompt', 'original_text', and 'classification_tag'.
# Output from run_inference: A PredictionResult object with the accuracy score and explanation.
    ...
    for json_string in batch:
      try:
        ...
        if not original_prompt or not original_text:
          ...
          yield PredictionResult(
            example=json_string, inference="0.0 - ERROR_ACCURACY_INPUT"
          )
          continue
        ...
        logging.error(
          f"Error during Gemini accuracy inference for input {json_string}: {e}"
        )
        yield PredictionResult(example=json_string, inference="0.0 - ERROR_INFERENCE")


# --- StatefulCountFn ---
# This DoFn demonstrates Beam's stateful processing primitives (StateSpec and TimerSpec).
# It counts "bad" classifications within each window and collects the associated
# prompts and texts for detailed analysis.
#
# It uses:
# - CombiningValueStateSpec: To sum up the count of 'bad' elements.
# - BagStateSpec: To store a bag (unordered collection) of (prompt, text) tuples.
# - TimerSpec: To trigger output at the end of each window.
#
# Input to process: A (key, value) tuple, where key is 'good' or 'bad', and value
#   is a tuple containing (1, (original_prompt, original_text)).
class StatefulCountFn(beam.DoFn):
  """A stateful DoFn to count elements per key within a window."""

  # Step 5
  # Counting Good and Bad Outcomes with Stateful Processing
  # Replace the pass with code that will achieve your goal
  ...
  def on_window_timer(
    ...
    key=beam.DoFn.KeyParam,
    count_state=beam.DoFn.StateParam(COUNT_STATE),
    bad_prompts_state=beam.DoFn.StateParam(BAD_PROMPTS_STATE),
  ):
...
def run(argv=None):
  ...
  classification_model_handler = GeminiModelHandler(
    ...
    model_kwargs={"temperature": 0.2, "max_output_tokens": 500},
  )

  accuracy_model_handler = GeminiAccuracyModelHandler(
    project=known_args.project,
    location=known_args.location,
    model_name=known_args.model_name,
    model_kwargs={"temperature": 0.1, "max_output_tokens": 500},
  )

  def parse_accuracy_result(accuracy_string: str) -> tuple[float, str]:
    match = re.match(r"^\s*(\d\.?\d*)\s*-\s*(.*)", accuracy_string, re.DOTALL)
    if match:
      return (float(match.group(1)), match.group(2).strip())
    return (0.0, accuracy_string)
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    if known_args.test_mode:
      ...
      parsed_elements = (
        parsed_elements
        | "ConvertTestDictsToJsonAndAddTimestamps"
        >> beam.Map(
      ...
      parsed_elements = (
        ...
        | "DecodeBytes"
        >> beam.Map(lambda b: b.decode("utf-8"))  # Output is JSON string
        # Extract timestamp from JSON string for Pub/Sub messages
        | "AddTimestampsFromParsedJson"
        >> beam.Map(
          lambda s: beam.window.TimestampedValue(s, json.loads(s)["timestamp"])
        )
      )

    # 2. Classify the initial response
    classification_predictions = parsed_elements | "ClassifyWithGemini" >> RunInference(
      classification_model_handler
    )

    # Prepare the data for the accuracy verification step
    accuracy_inputs = (
      classification_predictions
      | "PrepareAccuracyInput"
      >> beam.Map(
        lambda pr: {
          "original_data_json": pr.example,  # This is the original JSON string
          # Parse the original JSON string to get the prompt and text
          "original_prompt": json.loads(pr.example).get("prompt", ""),
          "original_text": json.loads(pr.example).get("text", ""),
          "classification_tag": pr.inference,
        }
      )
      | "ConvertAccuracyInputToJson"
      >> beam.Map(json.dumps)  # Convert the dictionary to a JSON string
    )

    # 3. Call Gemini again to verify the accuracy
    accuracy_predictions = accuracy_inputs | "VerifyAccuracy" >> RunInference(
      accuracy_model_handler
    )

    # 4. Window the results and parse them into a final format
    final_results = (
      accuracy_predictions
      | "AddTimestampBackForWindowing"
      >> beam.Map(
        lambda pr: beam.window.TimestampedValue(
          pr,
          json.loads(json.loads(pr.example)["original_data_json"])["timestamp"],
        )
      )
      | "WindowIntoFixedWindows" >> beam.WindowInto(beam.window.FixedWindows(60))
      | "ParseResults"
      >> beam.Map(
        lambda pr, window=beam.DoFn.WindowParam: (
          json.loads(json.loads(pr.example)["original_data_json"]),  # original dict
          json.loads(pr.example)["classification_tag"],  # classification
          parse_accuracy_result(pr.inference)[0],  # score
          parse_accuracy_result(pr.inference)[1],  # explanation
          window,
        )
      )
    )

    # 5. Use a stateful DoFn to count "good" and "bad" results per window
    good_bad_counts = (
      final_results
      # Pass the (prompt, text) tuple along with the count
      | "AssignGoodBadKey"
      >> beam.Map(
        lambda element: (
          "good" if element[2] >= 0.7 else "bad",
          (1, (element[0].get("prompt", ""), element[0].get("text", ""))),
        )
      ).with_output_types(tuple[str, tuple[int, tuple[str, str]]])
      | "CountGoodBad" >> beam.ParDo(StatefulCountFn())
    )

    good_bad_counts | "LogBadCounts" >> beam.Map(
      lambda count_tuple, window=beam.DoFn.WindowParam: logging.info(
        f"Window: {window}, Bad Counts: {count_tuple[1]}, Bad Prompts: {count_tuple[2]}"
      )
    )
