    ...
    def run_inference(
        ...
        for item in batch:
            ...
            try:
                ...
                gemini_response = model.models.generate_content_stream(model=self._model_name, contents=contents, config=self._model_kwargs)
                classification_tag = ""
...
class StatefulCountFn(beam.DoFn):
    ...
    BAD_PROMPTS_STATE = beam.transforms.userstate.BagStateSpec(
        "bad_prompts",
        coder=beam.coders.TupleCoder([beam.coders.StrUtf8Coder(), beam.coders.StrUtf8Coder()]),
    )
...
def run(argv=None):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        classification_predictions = parsed_elements | "ClassifyWithGemini" >> RunInference(classification_model_handler)
        ...
        accuracy_inputs = (
            ...
            | "ConvertAccuracyInputToJson" >> beam.Map(json.dumps)  # Convert the dictionary to a JSON string
        )

        # 3. Call Gemini again to verify the accuracy
        accuracy_predictions = accuracy_inputs | "VerifyAccuracy" >> RunInference(accuracy_model_handler)
