...
function fetchJSON(url, parseJson = true) {
  return new Promise((resolve, reject) => {
    https
      .get(url, { headers: { "User-Agent": "Gemini-CLI-Version-Checker" } }, (res) => {
        let data = "";
    ...
    switch (ecosystem.toLowerCase()) {
      ...
      case "pypi": {
        ...
        if (pypiData.info.yanked) {
          console.log(
            `⚠️  WARNING: This release was YANKED. Reason: "${pypiData.info.yanked_reason}"`,
          );
        }
        ...
        if (
          ...
          pypiData.info.classifiers.includes("Development Status :: 7 - Inactive")
        ) {
          ...
          try {
            ...
            if (repoData) {
              ...
              if (desc.includes("deprecated") || desc.includes("moved to")) {
                console.log(
                  `⚠️  WARNING: Potential deprecation/move detected in description: "${repoData.description}"`,
                );
                return;
              }
              ...
              if (readmeData && readmeData.content) {
                const content = Buffer.from(readmeData.content, "base64")
                  .toString("utf8")
                  .toLowerCase();
                if (
                  content.includes("deprecated") ||
                  content.includes("use google.golang.org/genai")
                ) {
                  console.log(
                    `⚠️  WARNING: The README for '${repoPath}' indicates this package is DEPRECATED or superseded.`,
                  );
                }
              }
            }
