...
if __name__ == "__main__":
  ...
  try:
    ...
    print("\nDownload complete.")
    print(
      f"All .table files have been saved to the '{os.path.abspath(OUTPUT_DIR)}' directory."
    )
  except requests.exceptions.RequestException as e:
    print(f"\nAn error occurred: {e}")
    print(
      "Please check your internet connection and ensure the GitHub repository is accessible."
    )
    if "rate limit" in str(e).lower():
      print(
        "You've likely hit the GitHub API rate limit. Please ensure your GITHUB_TOKEN is valid."
      )
