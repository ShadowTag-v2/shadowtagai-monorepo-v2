...
async def main():
    ...
    await runner.session_service.create_session(user_id="repro_user", session_id="repro_session", app_name=runner.app_name)
    ...
    try:
        async for event in runner.run_async(user_id="repro_user", session_id="repro_session", new_message=message):
