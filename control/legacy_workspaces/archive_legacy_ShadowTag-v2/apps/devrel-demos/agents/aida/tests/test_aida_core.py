...
class TestAidaCore(unittest.TestCase):
  ...
  def test_04_query_library_fuzzy(self):
    ...
    self.assertTrue(
      "malware" in str(results).lower() or "adware" in str(results).lower()
    )
  ...
  def test_05_query_library_no_results(self):
    ...
    results = search_query_library(
      "definitely_not_a_real_query_term_xyz", platform="all", top_k=5
    )
    # Current implementation returns results even for nonsense
    self.assertTrue(len(results) > 0)
    print(
      "Query library nonsense query test passed (returned results as expected by current implementation)."
    )
