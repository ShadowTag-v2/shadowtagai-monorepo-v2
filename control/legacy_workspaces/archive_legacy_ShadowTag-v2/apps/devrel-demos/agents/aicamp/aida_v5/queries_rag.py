...
def search_query_library(search_terms: str | list[str], platform: str | list[str], top_k: int) -> list[DocumentResult]:
    """
    ...
    # Robustly handle list inputs from LLM (Gemini sometimes returns lists for strings)
    if isinstance(search_terms, list):
        search_terms = " ".join(str(x) for x in search_terms)
