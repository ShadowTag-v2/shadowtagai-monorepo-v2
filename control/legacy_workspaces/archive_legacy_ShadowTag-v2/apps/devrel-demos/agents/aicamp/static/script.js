...
async function sendMessage(event) {
  ...
  // Thinking animation
  const thinkBlinkInterval = setInterval(() => {
  ...
  try {
    ...
    const currentInternalAuthor = null;
    let currentDebugDiv = null;
    const currentLogEntry = null;
            ...
            try {
              ...
              if (data.type === "log") {
                ...
                if (debugMode || isVisibleByDefault) {
                  ...
                  currentDebugDiv.innerHTML =
                    `<strong>[${author}]</strong>: ` + DOMPurify.sanitize(marked.parse(newText));
                  messagesDiv.scrollTop = messagesDiv.scrollHeight;
                } else {
