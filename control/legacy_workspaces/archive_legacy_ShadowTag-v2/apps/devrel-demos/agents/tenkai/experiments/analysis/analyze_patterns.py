...
def analyze(experiment_id, alternative):
  ...
  for run_id, is_success in runs:
    ...
    cursor.execute(
      "SELECT type, payload FROM run_events WHERE run_id = ? ORDER BY id",
      (run_id,),
    )
    ...
    patterns.append({"run_id": run_id, "is_success": is_success, "workflow": workflow})
  ...
  for p in patterns:
    if not p["is_success"] and fail_count < 3:
      ...
      for i, step in enumerate(p["workflow"]):
        ...
        print(f"    Tool:    {step['tool']}({step['args']})")
        if step["output"] and (
          "error" in step["output"].lower() or "failed" in step["output"].lower()
        ):
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "experiment_id", type=int, help="The ID of the experiment to analyze."
  )
  parser.add_argument(
    "alternative", type=str, help="The name of the alternative to analyze."
  )
