...
2.  **Lint-Driven Development:** Agents used `go_lint` as a primary feedback loop. A common pattern was `file_edit` → `go_lint` → `file_edit` until the code was clean _before_ attempting compilation.
...
This alternative provided _both_ the generic core tools and the specialized Go tools. Counter-intuitively, it performed **worse** than the control group (20% vs 40%).
