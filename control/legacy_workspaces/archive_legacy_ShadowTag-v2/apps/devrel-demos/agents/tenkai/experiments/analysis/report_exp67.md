...
2.  **SDK Grounding:** A significant source of friction was the agent selecting the wrong SDK (`mark3labs` vs `modelcontextprotocol`).
    - _Evidence:_ Failed runs show repeated `go_get` calls for the wrong package.
    - _Impact:_ This wasted time and tokens. The `safe-shell-v2` agent often recovered from this by failing the verification step and searching again, whereas the `no-core` agent just failed.
...
2.  **Enhance Discovery Tools:** To fix the SDK hallucination issue, explicitly encourage or tool the agent to `search` or `go_docs` _before_ `go_get` when the package name isn't provided.
