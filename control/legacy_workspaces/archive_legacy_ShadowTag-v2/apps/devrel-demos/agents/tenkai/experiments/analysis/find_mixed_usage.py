...
# Get godoctor runs
cursor.execute(
  "SELECT id FROM run_results WHERE experiment_id = 72 AND alternative = 'godoctor'"
)
...
for run_id in run_ids:
  cursor.execute(
    "SELECT type, payload FROM run_events WHERE run_id = ? ORDER BY id", (run_id,)
  )
      ...
      try:
        ...
        if tool_name:
          if tool_name == "run_shell_command":
            ...
            tool_name.startswith("go_")
            or tool_name.startswith("file_")
            or tool_name == "safe_shell"
          ):
            used_godoctor = True
  ...
  if used_godoctor:
    ...
    if used_shell:
      print("  -> MIXED USAGE (Shell + GoDoctor)")
    else:
      print("  -> PURE GoDoctor (No Shell)")
  # else:
