...
This section filters for runs where the agent _actually used_ at least one GoDoctor tool (e.g., `smart_edit`, `read_docs`), ignoring runs where it fell back to standard shell commands.
...
4.  **Generic Prompting is Less Effective:** The `custom-context` ("use specialized tools") had the lowest adoption rate (90%). Specific instructions on _which_ tools to use and _why_ (as in the Extension Context) are far superior to generic advice.
