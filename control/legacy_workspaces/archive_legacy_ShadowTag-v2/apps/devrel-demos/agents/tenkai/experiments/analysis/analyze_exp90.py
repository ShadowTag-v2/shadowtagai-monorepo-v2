...
# Load data
with open("agents/tenkai/experiments/analysis/exp90_raw_tools.csv") as f:
...
print(
  "| Alternative | Total Runs | GD Runs | GD Success Rate | Avg Duration (s) | Avg Tokens |"
)
print("|---|---|---|---|---|---|")
...
for alt, stats in results.items():
  ...
  # Get top 3 GD tools
  gd_tools_counts = {
    k: v for k, v in stats["tool_usage"].items() if k in GODOCTOR_TOOLS
  }
  top_gd = sorted(gd_tools_counts.items(), key=lambda x: x[1], reverse=True)[:3]
  top_gd_str = ", ".join([f"{k}({v})" for k, v in top_gd])

  print(
    f"| {alt} | {stats['total_runs']} | {stats['godoctor_runs']} | {success_rate:.1f}% | {avg_dur:.1f} | {avg_tok:.0f} | {top_gd_str} |"
  )
