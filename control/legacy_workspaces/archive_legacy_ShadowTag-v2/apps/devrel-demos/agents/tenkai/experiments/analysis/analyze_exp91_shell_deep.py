...
def deep_analyze_failures():
  ...
  for alt, run_id, is_success, payload_json in rows:
    try:
      ...
      if payload.get("name") != "run_shell_command":
        continue
  ...
  print(
    "This analysis identifies 'friction events' where a shell command returned an error or a Go-specific failure message.\n"
  )
