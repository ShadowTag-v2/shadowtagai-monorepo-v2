...
def analyze_runs(cursor, alt):
  ...
  # Correlation Check
  fails_with_silent_blocks = sum(
    1 for d in run_data.values() if not d["success"] and d["silent_blocks"] > 0
  )
  success_with_silent_blocks = sum(
    1 for d in run_data.values() if d["success"] and d["silent_blocks"] > 0
  )
