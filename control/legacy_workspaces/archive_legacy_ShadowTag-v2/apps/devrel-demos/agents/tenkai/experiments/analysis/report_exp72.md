...
A deeper look at the `godoctor` alternative reveals that the agent _did_ occasionally use specialized tools, but failed to commit to a pure workflow.
...
3.  **Optimization:** The combination of _Specialized Tools Only_ (No Core) + _Safe Shell Integration_ has proven to be the optimal configuration for Go development tasks.
...
2.  **Prompt Engineering:** If generic tools _must_ remain, the System Prompt needs explicit instruction: _"ALWAYS prefer `go_test` over running `go test` in the shell."_
