...
**Observation:** This is a reversal. When the `godoctor-advanced-no-core` agent _didn't_ crash, it was **the most effective agent of all**.
...
- **Why?** Perhaps the advice/warnings in `safe-shell-v2` ("don't use ls", etc.) consume context or cognitive load, distracting the agent slightly compared to the pure "code-only" focus of `no-core`. Or, the `no-core` agent _had_ to write perfect code because it couldn't rely on shell hacks, leading to higher quality output.
...
- `no-core` (Exp 67, Advanced Config): **68.8%** ?? Wait.
  - _Correction:_ In Exp 67, `no-core` had Advanced Profile settings. In Exp 68, it also had Advanced Profile settings.
  - Why the jump from 68.8% -> 93.3%?
  - **Hypothesis:** The `no-core` in Exp 67 had the _wrong_ config (Advanced), but `safe-shell-v2` had _Standard_.
