...
def get_run_details(cursor, run_id):
  ...
  avg_input_tokens = (
    statistics.mean(input_tokens_trajectory) if input_tokens_trajectory else 0
  )
...
def analyze():
  ...
  # Get alternatives
  cursor.execute(
    "SELECT DISTINCT alternative FROM run_results WHERE experiment_id = ?",
    (EXP_ID,),
  )
  alts = [row[0] for row in cursor.fetchall()]

  print(
    f"{'Alternative':<35} | {'Turns':<6} | {'Tool Out (Chars)':<16} | {'Context Growth':<14} | {'Top Tools'}"
  )
  print("-" * 120)
  ...
  for alt in alts:
    ...
    cursor.execute(
      "SELECT id FROM run_results WHERE experiment_id = ? AND alternative = ? AND status = 'COMPLETED'",
      (EXP_ID, alt),
    )
    ...
    print(
      f"{alt:<35} | {avg_turns:<6.1f} | {avg_tool_chars:<16.0f} | {avg_growth:<14.0f} | {top_tools_str}"
    )
