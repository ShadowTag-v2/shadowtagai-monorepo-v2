...
**Hypothesis:** Agents can successfully complete tasks using _only_ GoDoctor tools for file manipulation, potentially achieving higher quality results due to the specialized nature of tools like `smart_edit`.
...
A deeper look at the tool failure patterns reveals _why_ GoDoctor outperformed the baseline.
...
1.  **Forced Adoption Works:** Disabling core tools successfully forced the agents to use GoDoctor tools (`file_create`, `smart_edit`) without hindering performance. In fact, success rates _increased_ slightly (94% vs 90%).
...
3.  **Doc Update: `file_create`:**
    ...
    - **Solution:** Explicitly state in the `file_create` description: _"Automatically creates parent directories."_
