...
def classify_shell_command(cmd):
  cmd = cmd.strip()
  if cmd.startswith("go mod init"):
    return "go mod init"
  if cmd.startswith("go mod tidy"):
    return "go mod tidy"
  ...
  if cmd.startswith("go test"):
    return "go test"
  if cmd.startswith("go list"):
    return "go list"
  if cmd.startswith("go doc"):
    return "go doc"
  if cmd.startswith("go vet"):
    return "go vet"
  if cmd.startswith("go fmt") or cmd.startswith("gofmt"):
    return "go fmt"
  if cmd.startswith("mkdir"):
    return "mkdir"
  if cmd.startswith("ls"):
    return "ls/dir"
  if cmd.startswith("cat") or cmd.startswith("read_file"):
    return "cat/read"
...
for alternative, payload_json in cursor.fetchall():
  try:
    ...
    if tool_name == "run_shell_command":
      ...
      if not is_failure:
        if category == "go test" and (
          "FAIL" in tool_output or "build failed" in tool_output
        ):
          is_failure = True
...
print(
  "| Command Category | Default (Use/Fail) | Extension (Use/Fail) | MCP (Use/Fail) | Total Uses |"
)
print("|---|---|---|---|---|")
...
for cat, total in sorted(cat_totals.items(), key=lambda x: x[1], reverse=True):
  ...
  print(
    f"| {cat} | {def_use} / {def_fail} | {ext_use} / {ext_fail} | {mcp_use} / {mcp_fail} | {total} |"
  )
