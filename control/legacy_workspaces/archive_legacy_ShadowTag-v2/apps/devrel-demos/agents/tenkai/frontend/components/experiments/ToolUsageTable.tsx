...
import type { ToolStatRow } from "@/app/api/api";
import {
  Table,
  TableBody,
  TableCell,
...
export default function ToolUsageTable({
  experimentId,
  alternatives,
  filter,
}: ToolUsageTableProps) {
  ...
  if (loading)
    return <div className="p-4 text-center text-zinc-500 animate-pulse">Loading tool stats...</div>;
  ...
  stats.forEach((s) => {
    ...
    matrix[s.tool_name][s.alternative] = {
      total: s.total_calls,
    ...
    <div className="panel overflow-hidden mt-6">
      <div className="p-4 border-b border-white/5 bg-white/[0.02]">
        <h3 className="font-bold uppercase tracking-widest text-sm">
          Tool Call Analysis (Accumulated)
        </h3>
      </div>
          ...
          <TableHeader>
            <TableRow>
              ...
              {alternatives.map((alt) => (
                <TableHead key={alt} className="text-right">
                  {alt}
                </TableHead>
              ))}
            </TableRow>
            ...
            {tools.map((tool) => (
              <TableRow key={tool}>
                ...
                {alternatives.map((alt) => {
                  ...
                  if (!data) {
                    return (
                      <TableCell key={alt} className="text-right font-mono text-zinc-700">
                        -
                      </TableCell>
                    );
                  }
