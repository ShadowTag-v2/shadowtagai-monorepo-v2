...
import type { ScenarioAsset, ValidationRule } from "@/types/domain";
...
export default function ScenarioForm({
  ...
  const [assetType, setAssetType] = useState<"none" | "folder" | "files" | "git" | "create">(
    "none",
  );
  ...
  useEffect(() => {
    ...
    const loadedVal = (initialData.validation || []).map((v: ValidationRule) => ({
      ...v,
      include_in_prompt:
        v.include_in_prompt !== undefined ? v.include_in_prompt : v.type !== "model",
    }));
  ...
  const addValidation = () => {
    ...
    const val: Partial<ValidationRule> = {
      type: newValType as any,
      include_in_prompt: newValInclude,
    };
    ...
    <form onSubmit={handleFormSubmit} className="space-y-8">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="md:col-span-2">
            <Input
              ...
              required
              placeholder="e.g. Legacy Refactoring"
            />
          </div>
          <div className="md:col-span-2">
            <TextArea
              ...
              rows={3}
              placeholder="Briefly explain what this scenario tests..."
            />
          </div>
      ...
      {(taskMode === "prompt" || (taskMode === "github" && githubTaskType === "prompt")) && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Manual instructions for the agent (excluding automated criteria)
            </CardDescription>
          </CardHeader>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            {envVars.map((e, i) => (
              <div
                key={i}
                className="flex gap-4 items-center bg-card p-2 rounded border border-border"
              >
                <span className="font-mono text-emerald-600 dark:text-emerald-400">{e.key}</span>
                <span className="text-muted-foreground">=</span>
                <span className="font-mono text-body flex-1 truncate text-foreground">
                  {e.value}
                </span>
                <button
                  type="button"
            ...
            {envVars.length === 0 && (
              <p className="text-body opacity-30 italic">No environment variables defined.</p>
            )}
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Automated rules for success and generated acceptance criteria
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-8">
          ...
          <div className="space-y-4">
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1 space-y-2">
                    <div className="flex gap-3 items-center flex-wrap">
                      ...
                      />
                      <span
                        className={`px-2 py-0.5 rounded font-bold uppercase text-xs ${v.type === "test" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : v.type === "lint" ? "bg-amber-500/10 text-amber-600 dark:text-amber-400" : v.type === "model" ? "bg-purple-500/10 text-purple-600 dark:text-purple-400" : v.type === "manual" ? "bg-blue-500/10 text-blue-400" : "bg-primary/10 text-primary"}`}
                      >
                        {v.type}
                      </span>
                      {v.include_in_prompt && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                        ...
                        {v.stdin && (
                          <div className="md:col-span-2">
                            ...
                            <pre className="font-mono text-foreground bg-muted/30 p-2 rounded mt-1 text-xs whitespace-pre-wrap">
                              {v.stdin}
                            </pre>
                          </div>
                        )}
                        {v.stdin_delay && (
                          <div className="md:col-span-2">
                            ...
                            <span className="font-mono text-foreground bg-muted/30 px-2 py-1 rounded ml-2 text-xs">
                              {v.stdin_delay}
                            </span>
                          </div>
                        )}
                        ...
                        {v.expect_output_regex && (
                          <div className="md:col-span-2">
                            ...
                            <code className="font-mono text-foreground bg-muted/30 px-2 py-1 rounded ml-2">
                              {v.expect_output_regex}
                            </code>
                          </div>
                        )}
            ...
            {validation.length === 0 && (
              <p className="text-body opacity-30 italic">No validation rules defined.</p>
            )}
          </div>

          <div
            className={`p-4 bg-muted/10 border rounded flex gap-6 items-start ${editingIdx !== null ? "border-primary" : "border-border"}`}
          >
            ...
            <div className="w-[200px] flex flex-col gap-4">
              <Button
                type="button"
                variant={editingIdx !== null ? "default" : "secondary"}
              ...
              <label
                ...
                title="Include in Generated Acceptance Criteria"
              >
                <input
                  type="checkbox"
          ...
          <div className="mt-8 pt-6 border-t border-border">
            ...
            <div className="bg-[#1e1e20] p-4 rounded-md font-mono text-sm text-zinc-300 border border-zinc-800 whitespace-pre-wrap">
              {validation.length > 0 ? (
                generateAcceptanceCriteria(validation) || (
                  <span className="text-zinc-600 italic">No active criteria selected.</span>
                )
              ) : (
                <span className="text-zinc-600 italic">No criteria generated yet.</span>
              )}
            </div>
            <p className="text-[10px] text-muted-foreground mt-2">
              This block will be automatically appended to the agent prompt.
            </p>
          </div>
        </CardContent>
      </Card>
