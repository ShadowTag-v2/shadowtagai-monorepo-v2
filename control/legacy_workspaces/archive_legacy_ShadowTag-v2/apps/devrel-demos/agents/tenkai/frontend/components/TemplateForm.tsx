...
export default function TemplateForm({
  ...
  const [description, setDescription] = useState(
    initialData?.description || initialData?.config?.description || "",
  );
  ...
  const [experimentControl, setExperimentControl] = useState(
    initialData?.config?.experiment_control || "",
  );
  ...
  const addAlternative = () => {
    ...
    const defaultAgent = { command: "", args: [] as string[] };
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-6xl mx-auto pb-20">
      ...
      <div className="flex justify-end items-center pb-4 border-b border-border">
        <div className="flex gap-4 items-center">
          {error && (
            <span className="text-red-500 font-bold text-sm tracking-tight bg-red-500/10 px-2 py-1 rounded">
              {error}
            </span>
          )}
        ...
        <div className="space-y-6">
          {alternatives.map((alt, idx) => (
            <AlternativeCard
              ...
              onSetControl={() =>
                setExperimentControl(experimentControl === alt.name ? "" : alt.name)
              }
          ...
          <Button
            ...
            disabled={isLocked}
            className="w-full py-8 border-dashed border-2 hover:border-primary hover:bg-primary/5 transition-all"
          >
