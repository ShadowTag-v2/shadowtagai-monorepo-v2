    ...
    <div className="space-y-6">
      ...
      {existingAssets.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-xs font-black uppercase tracking-tighter text-muted-foreground">
            Existing Assets
          </h4>
          <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto custom-scrollbar pr-2">
            {existingAssets.map((asset, idx) => {
              const isBinary = /\.(jpe?g|png|gif|zip|gz|exe|bin|pdf|docx?|xlsx?)$/i.test(
                asset.target,
              );
              return (
                <div
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-muted-foreground uppercase">
                        {asset.type} • {isBinary ? "binary" : "text"}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {files && files.length > 0 && (
            <div className="bg-[#161618] rounded border border-[#27272a] p-4 space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="flex justify-between items-center mb-2 border-b border-[#27272a] pb-2">
                <span className="text-xs font-bold uppercase text-[#a1a1aa]">
              ...
              <div className="max-h-64 overflow-y-auto space-y-1 pr-2 custom-scrollbar">
                {Array.from(files)
                  ...
                  .map((f: any, i) => (
                    <div
                      key={i}
                      className="flex justify-between text-xs font-mono text-[#e4e4e7] group py-0.5"
                    >
