...
export default function ScenariosPage() {
    ...
    try {
      ...
      if (original.assets && original.assets.length > 0) {
        ...
        if (asset.type === "git") {
          ...
          toast.warning(
            "Asset copying not fully supported for this type. Please re-upload assets.",
          );
        }
      } else {
        formData.append("asset_type", "none");
  ...
  const handleDeleteAll = async () => {
    if (!confirm("⚠️ WARNING: This will delete ALL scenarios. This action cannot be undone."))
      return;
    if (!confirm("Are you really sure?")) return;

    setDeletingAll(true);
    try {
      const res = await fetch("/api/scenarios/delete-all", {
        method: "DELETE",
      });
      if (res.ok) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {scenarios.map((scenario) => (
            <Link href={`/scenarios/${scenario.id}`} key={scenario.id} className="block group">
              <Card
                className={`h-full p-6 hover:border-muted transition-colors flex flex-col justify-between ${scenario.is_locked ? "border-2 border-amber-500" : "border border-border"}`}
              >
                <div>
                      ...
                      <LockToggle
                        ...
                        onToggle={async (locked) => {
                          try {
                            ...
                            setScenarios((prev) =>
                              prev.map((s) =>
                                s.id === scenario.id ? { ...s, is_locked: locked } : s,
                              ),
                            );
                            return true;
                          } catch (err) {
                  ...
                  <div className="mb-6 space-y-2 flex-1">
                    {scenario.github_issue ? (
                      ...
                      <div className="text-body font-mono">
                        <span className="font-bold text-muted-foreground mr-2 block mb-1">
                          PROMPT:
                        </span>
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-0">
                  {scenario.validation?.map((v, i) => (
                    <span
                      key={i}
                      className={`px-3 py-1 rounded-full text-mono font-bold text-xs uppercase tracking-wider border ${
                        v.type === "test"
                    ...
                    >
                      {v.type === "test"
                        ? `Test >= ${v.min_coverage ?? 0}%`
                        : v.type === "lint"
                          ? `Lint <= ${v.max_issues ?? 0}`
                          : v.type === "command"
                            ? `Cmd (Exit ${v.expected_exit_code ?? 0})`
                            : v.type}
                    </span>
                  ))}
