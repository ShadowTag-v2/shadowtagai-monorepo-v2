...
export default async function ReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return (
      <div className="p-8 text-center py-32">
        ...
        <Link href="/experiments" className="text-body hover:underline font-bold text-primary">
          Return to index
        </Link>
      </div>
    );
    ...
    return (
      <div className="p-8 text-center py-32">
        ...
        <p className="text-body mb-8">
          Experiment metadata loaded, but details could not be fetched.
        </p>
  ...
  try {
    if (experiment.config_content) configYaml = yaml.load(experiment.config_content);
  } catch (e) {}

  return (
    <div id="report-page-root" className="flex h-screen overflow-hidden">
      <RefreshOnInterval
        active={experiment.status === "running" || experiment.status === "RUNNING"}
      />
