...
import type { Checkpoint } from "@/app/api/api";
...
export default function StatusBanner({
  ...
  return (
    <div className="px-6 py-4 bg-muted/40 border-b border-border flex items-center justify-between animate-in slide-in-from-top duration-500 text-body">
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3">
          <div
            className={`w-3 h-3 rounded-full ${isRunning ? "bg-indigo-500 animate-pulse shadow-[0_0_10px_#6366f1]" : "bg-amber-500"}`}
          ></div>
        ...
        <div className="flex gap-4">
          <p className="font-bold uppercase tracking-widest text-muted-foreground">
            ...
            <span className="text-foreground">
              {checkpoint.completed_jobs} / {checkpoint.total_jobs}
            </span>
          </p>
        </div>
      </div>

      <div className="flex gap-2">{/* No controls here - centralized in Header */}</div>
    </div>
  );
}
