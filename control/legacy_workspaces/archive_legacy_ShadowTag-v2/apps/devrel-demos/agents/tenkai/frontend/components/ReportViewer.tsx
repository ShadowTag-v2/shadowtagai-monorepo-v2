...
function RevalidateExperimentButton({
  experimentId,
  isExperimentRunning,
  onStateChange,
}: {
  ...
  onStateChange?: (revalidating: boolean) => void;
}) {
  ...
  return (
    <Button
      ...
      size="sm"
      onClick={handleReval}
      disabled={loading || isExperimentRunning}
    >
      {loading ? (
...
export default function ReportViewer({
  ...
  const activeTab =
    (searchParams.get("tab") as "overview" | "investigate" | "config") || "overview";
  ...
  const handleFilterChange = async (val: string) => {
    ...
    try {
      ...
      newSummaries.forEach((row: ExperimentSummaryRecord) => {
        statObj[row.alternative] = {
          ...row,
          alternative: row.alternative,
          count: row.total_runs,
        };
      });
    ...
    <div className="flex flex-col h-full bg-background text-body">
        ...
        <div className="flex h-full">
          {[
            ...
            <button
              ...
              className={`px-6 h-full font-bold uppercase tracking-widest transition-all border-b-2 ${
                activeTab === tab.id
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-12">
                        ...
                        <TooltipTrigger asChild>
                          <div>
                            <ToggleGroupItem
                              value="all"
                              aria-label="All Runs"
                              className="text-xs data-[state=on]:bg-background data-[state=on]:text-foreground data-[state=on]:shadow-sm transition-all h-7 px-3"
                            >
                  ...
                  <ToolImpactTable
                    stats={viewStats}
                    alternatives={[
                      ...(config?.alternatives?.map((a: any) => a.name) ||
                  ...
                  <ToolUsageTable
                    experimentId={experiment.id}
                    alternatives={
                      config?.alternatives?.map((a: any) => a.name) || Object.keys(stats).sort()
                    }
                    filter={filterMode}
                  />
                </div>
                ...
                {selectedRun ? (
                  loadingDetails ? (
                    <div className="flex items-center justify-center h-full animate-pulse">
                      <p className="text-header uppercase font-bold text-zinc-700 tracking-widest">
                        Loading Telemetry...
                      </p>
                    </div>
                  ) : runDetails ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center opacity-30">
                    ...
                    <h3 className="text-header font-bold uppercase tracking-widest">
                      Select a run to investigate
                    </h3>
      ...
      {configModalContent && (
        <div
          ...
          onClick={() => setConfigModalContent(null)}
        >
          <div
            className="bg-popover border border-border rounded-lg shadow-2xl w-full max-w-4xl max-h-[80vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between items-center p-4 border-b border-border">
              <h3 className="font-bold uppercase tracking-widest text-foreground">
