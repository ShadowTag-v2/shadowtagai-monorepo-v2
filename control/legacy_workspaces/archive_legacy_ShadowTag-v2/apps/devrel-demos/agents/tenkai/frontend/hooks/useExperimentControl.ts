...
export function useExperimentControl(
  ...
  options?: UseExperimentControlOptions,
) {
  ...
  const handleExplain = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/experiments/${experimentId}/explain`, {
        method: "POST",
      });
      if (res.ok) {
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/experiments/${experimentId}/regenerate`, {
        method: "POST",
      });
      if (res.ok) {
