    ...
    <div className="flex flex-col h-full text-body">
      <div className="p-4 border-b border-border bg-muted flex flex-col gap-3">
        <div className="flex justify-between items-center">
          <h3 className="font-bold uppercase tracking-widest text-muted-foreground">
        ...
        {alternatives.map((alt) => {
          ...
          const altRuns = runs
            ...
            .filter((r) => {
              ...
              if (filter === "FAILED")
                return !r.is_success && r.status !== "QUEUED" && r.status !== "RUNNING";
                        ...
                        {run.reason && (
                          <span
                            className={`font-mono font-bold text-[10px] ${
                              run.reason.toUpperCase() === "SUCCESS"
                                ? "text-emerald-500"
                                : "text-red-400"
                            }`}
                          >
                    ...
                    <div className="flex gap-4 items-center opacity-50 font-mono text-sm">
                      <span>
                        {run.status === "RUNNING" || run.status === "QUEUED"
                          ? "---"
                          : `${(run.duration / 1e9).toFixed(1)}s`}
                      </span>
                      <span>
                        {run.tests_passed}/{run.tests_passed + run.tests_failed} Tests
                      </span>
                    </div>
                  </button>
                ))}
