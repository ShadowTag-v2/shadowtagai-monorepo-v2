...
export default function ExperimentRow({ exp }: { exp: ExperimentRecord }) {
  return (
    <tr className="hover:bg-muted/50 transition-colors text-body">
      ...
      <td className="px-6 py-4 text-body opacity-80 max-w-md truncate">{exp.description || "—"}</td>
      <td className="px-6 py-4">
        <span
          className={`inline-flex items-center px-2 py-0.5 rounded-[3px] font-mono font-bold uppercase tracking-wider ${
      ...
      <td className="px-6 py-4 text-right">
        <span
          className={`font-mono font-bold ${(exp.success_rate || 0) >= 90 ? "text-success" : "text-muted-foreground"}`}
        >
