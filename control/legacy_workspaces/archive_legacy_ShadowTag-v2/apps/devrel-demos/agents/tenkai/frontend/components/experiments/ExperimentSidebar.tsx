  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-border">
        <Link
          href="/experiments"
          className="text-body hover:text-white flex items-center mb-4 transition-colors font-bold uppercase tracking-wider justify-center md:justify-start"
        >
        ...
        {!isCollapsed ? (
          <>
            <h1 className="text-title leading-tight mb-2 truncate" title={experiment.name}>
              {experiment.name || "Unnamed"}
            </h1>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span
                  className={`inline-block w-2.5 h-2.5 rounded-full ${experiment.status?.toUpperCase() === "RUNNING" ? "bg-primary animate-pulse shadow-[0_0_8px_var(--primary)]" : experiment.status?.toUpperCase() === "COMPLETED" ? "bg-emerald-500" : "bg-muted"}`}
                ></span>
          ...
          <div className="flex flex-col items-center gap-4">
            <span
              className={`inline-block w-2.5 h-2.5 rounded-full ${experiment.status?.toUpperCase() === "RUNNING" ? "bg-primary animate-pulse" : experiment.status?.toUpperCase() === "COMPLETED" ? "bg-emerald-500" : "bg-muted"}`}
        ...
        <div className="p-4 space-y-8 overflow-y-auto flex-1 text-body">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-y-3">
              ...
              <span className="text-foreground text-right font-mono font-bold">
                {experiment.id}
              </span>

              <span className="uppercase opacity-50">Started</span>
              <span className="text-foreground text-right">
                {new Date(experiment.timestamp).toISOString().split("T")[0]}
              </span>
          ...
          <div className="space-y-4">
            ...
            <AnnotationEditor
              experimentId={experiment.id}
              initialAnnotations={experiment.annotations}
            />
          </div>
        </div>
      )}
