...
import { Input, Select } from "@/components/ui/input"; // Helper we used in BlockSelector
...
export default function AlternativeCard({
  ...
  return (
    <div
      className={cn(
        "bg-card border rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow",
        isLocked ? colorStyle.className : "border-border",
      )}
    >
      {/* Header Row */}
        ...
        >
          <div
            className={cn(
              ...
              isControl ? "bg-primary border-primary" : "border-muted-foreground",
            )}
          >
      ...
      <div className="space-y-4 mb-6">
            ...
            <BlockCombobox
              ...
              onSelect={(b) => {
                try {
                  ...
                  toast.error("Invalid Agent Block");
                }
              }}
              required
              disabled={isLocked}
        ...
        <div className="grid grid-cols-12 gap-4 items-center">
          <label className="col-span-3 font-medium text-sm text-muted-foreground">
            System Prompt
          </label>
          <div className="col-span-9">
            <BlockCombobox
        ...
        <div className="col-span-9 flex flex-wrap gap-2 items-center">
          ...
          {(alt.extensions || []).map((s: any, i: number) => (
            <OptionChip
              ...
              name={s.name || s.name}
              onRemove={() => {
                const n = [...alt.extensions];
                n.splice(i, 1);
                onUpdate("extensions", n);
              }}
              disabled={isLocked}
            />
          ))}
          ...
          {(alt.mcp_servers || []).map((s: any, i: number) => (
            <OptionChip
              ...
              name={s.name || s.name}
              onRemove={() => {
                const n = [...alt.mcp_servers];
                n.splice(i, 1);
                onUpdate("mcp_servers", n);
              }}
              disabled={isLocked}
            />
          ))}
          ...
          {(alt.settings_blocks || []).map((s: any, i: number) => (
            <OptionChip
              key={`set-${i}`}
              type="settings"
              name={s.name || "Settings"}
              onRemove={() => {
                const n = [...alt.settings_blocks];
                n.splice(i, 1);
                onUpdate("settings_blocks", n);
              }}
              disabled={isLocked}
            />
          ))}
          ...
          {!isLocked && (
            <AddOptionPopover
              blocks={blocks}
              onAdd={(type: ConfigBlockType, block: ConfigBlock) => {
                try {
                  ...
                  toast.error("Invalid block content");
                }
              }}
              disabled={isLocked}
            />
...
function OptionChip({ type, name, onRemove, disabled }: OptionChipProps) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn("pl-2 pr-1 py-1 flex items-center gap-1 font-normal", styles[type])}
    >
      {name}
      <span className="font-bold opacity-70 ml-1 text-[10px] uppercase tracking-wider">
        [{labels[type]}]
      </span>
      {!disabled && (
        <button
    ...
    >
      <PopoverTrigger asChild>
        <Button
          ...
          variant="outline"
          size="sm"
          className="h-7 w-7 p-0 rounded-full bg-muted hover:bg-primary hover:text-white transition-colors border-dashed border-zinc-400"
          disabled={disabled}
        >
          <Plus size={14} />
        </Button>
              ...
              <CommandGroup heading="Option Type">
                {typeOptions.map((opt) => (
                  <CommandItem
                    key={opt.type}
                    onSelect={() => {
                      setSelectedType(opt.type as any);
                      setStep("block");
                    }}
                  >
          ...
          <Command>
            ...
            <CommandList>
              ...
              <CommandGroup heading={selectedType?.toUpperCase()}>
                {blocks
                  .filter((b: ConfigBlock) => b.type === selectedType)
                  .map((b: ConfigBlock) => {
                    let desc = "";
                    try {
                      const c = JSON.parse(b.content);
