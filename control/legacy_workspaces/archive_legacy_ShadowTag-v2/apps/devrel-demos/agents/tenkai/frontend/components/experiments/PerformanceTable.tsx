...
import type { RunResultRecord } from "@/app/api/api";
...
export default function PerformanceTable({
  ...
  const sortedAlternatives = [...(configAlts || Object.keys(stats))].sort((a, b) => {
    ...
    if (sortConfig.key === "avg_tool_calls") {
      valA =
        statsA.total_tool_calls && statsA.total_runs
          ? statsA.total_tool_calls / statsA.total_runs
          : 0;
      valB =
        statsB.total_tool_calls && statsB.total_runs
          ? statsB.total_tool_calls / statsB.total_runs
          : 0;
    }
    if (sortConfig.key === "avg_failed_tools") {
      valA =
        statsA.failed_tool_calls && statsA.total_runs
          ? statsA.failed_tool_calls / statsA.total_runs
          : 0;
      valB =
        statsB.failed_tool_calls && statsB.total_runs
          ? statsB.failed_tool_calls / statsB.total_runs
          : 0;
    }
    ...
    <div className="rounded-md border bg-card">
      <div className="p-4 border-b bg-muted/40 flex justify-between items-center">
        ...
        <div className="flex gap-4 font-bold uppercase tracking-wider text-muted-foreground text-xs">
          <span>
            Control: <span className="text-primary">{referenceAlt}</span>
          </span>
        </div>
      </div>
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/50">
            <TableHead
              rowSpan={2}
              className="px-6 h-auto align-bottom pb-3 cursor-pointer hover:bg-accent transition-colors border-r border-border text-xs font-bold uppercase tracking-wider text-muted-foreground text-center"
              onClick={() => handleSort("alternative")}
            >
              <div className="flex items-center justify-center gap-2">
                Alternative <ArrowUpDown size={12} className="opacity-50" />
              </div>
            </TableHead>
            <TableHead
              rowSpan={2}
            ...
            >
              <div className="flex items-center justify-center gap-2">
                Success Rate <ArrowUpDown size={12} className="opacity-50" />
              </div>
            </TableHead>
            <TableHead
              rowSpan={2}
            ...
            >
              <div className="flex items-center justify-center gap-2">
                Avg Duration <ArrowUpDown size={12} className="opacity-50" />
              </div>
            </TableHead>
            <TableHead
              colSpan={4}
              className="px-6 text-center border-b border-border border-r border-border py-2 h-auto text-xs font-bold uppercase tracking-wider text-muted-foreground"
            >
              Token Usage
            </TableHead>
            <TableHead
            ...
            <TableHead
              rowSpan={2}
              className="px-6 h-auto align-bottom pb-3 cursor-pointer hover:bg-accent transition-colors border-r border-border text-xs font-bold uppercase tracking-wider text-muted-foreground text-center"
              onClick={() => handleSort("avg_lint")}
            >
              <div className="flex items-center justify-center gap-2">
                Avg Lint <ArrowUpDown size={12} className="opacity-50" />
              </div>
            </TableHead>
            <TableHead
              rowSpan={2}
              className="px-6 h-auto align-bottom pb-3 cursor-pointer hover:bg-accent transition-colors border-r border-border text-xs font-bold uppercase tracking-wider text-muted-foreground text-center"
              onClick={() => handleSort("avg_coverage")}
            >
              <div className="flex items-center justify-center gap-2">
                Avg Cov <ArrowUpDown size={12} className="opacity-50" />
              </div>
            </TableHead>
            <TableHead
              rowSpan={2}
              className="px-6 h-auto align-bottom pb-3 cursor-pointer hover:bg-accent transition-colors text-xs font-bold uppercase tracking-wider text-muted-foreground text-center"
              onClick={() => handleSort("total_runs")}
            >
              <div className="flex items-center justify-center gap-2">
                Runs <ArrowUpDown size={12} className="opacity-50" />
              </div>
            </TableHead>
          </TableRow>
          <TableRow className="bg-muted/50 border-t-0">
            <TableHead
              className="px-6 py-2 border-r border-border text-right cursor-pointer hover:bg-accent hover:text-accent-foreground text-xs font-bold text-muted-foreground"
        ...
        <TableBody>
          {alternatives.map((alt) => {
            ...
            const inputTokenSig = isControl
              ? null
              : getSigLevel(s.p_input_tokens, s.effect_input_tokens);
            const outputTokenSig = isControl
              ? null
              : getSigLevel(s.p_output_tokens, s.effect_output_tokens);
            const cachedTokenSig = isControl
              ? null
              : getSigLevel(s.p_cached_tokens, s.effect_cached_tokens);
            ...
            const coverageSig =
              !isControl && s.p_coverage !== undefined
                ? getSigLevel(s.p_coverage, s.effect_coverage)
                : null;
            ...
            const avgFailedTools =
              s.failed_tool_calls && totalRuns ? s.failed_tool_calls / totalRuns : 0;
            ...
            return (
              <TableRow key={alt}>
                ...
                <TableCell className="px-6 text-right border-r border-white/5">
                  <div className="flex items-center justify-end gap-2">
                    <span
                      className={`font-mono ${avgFailedTools > 0 ? "text-red-400 font-bold" : "text-zinc-500"}`}
                    >
