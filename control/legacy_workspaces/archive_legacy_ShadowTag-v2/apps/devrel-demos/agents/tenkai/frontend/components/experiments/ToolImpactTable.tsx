...
import type { ExperimentSummaryRecord } from "@/app/api/api";
    ...
    <div className="panel overflow-hidden mt-6 border-primary/20 shadow-sm">
      <div className="p-4 border-b border-primary/20 bg-primary/10 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <h3 className="font-bold uppercase tracking-widest text-sm text-primary">
        ...
        <span className="text-xs text-muted-foreground font-mono font-bold tracking-tighter">
          Mann-Whitney U Test (Succ vs Fail)
        </span>
      </div>
          ...
          <TableBody>
            {impacts.map((imp, i) => {
              ...
              return (
                <TableRow
                  key={i}
                  className={`hover:bg-primary/10 transition-colors border-white/5 ${rowBg}`}
                >
                  <TableCell className="px-6 font-mono text-[11px]">
                    {imp.isGlobal ? (
                      <span className="text-primary font-bold tracking-tighter uppercase">
                        ✨ Global (Combined)
                      </span>
                    ) : (
                  ...
                  <TableCell
                    className={`px-6 font-mono font-bold ${isSignificant ? "text-primary" : "text-muted-foreground"}`}
                  >
                    {imp.tool}
                  </TableCell>
                  <TableCell className="px-6 text-right font-mono">
                    <div className="flex items-center justify-end gap-2">
                      <span
                        className={
                          isSignificant ? "text-foreground font-bold" : "text-muted-foreground"
                        }
                      >
                        {imp.p.toFixed(4)}
                      </span>
                  ...
                  <TableCell
                    className={`px-6 text-right font-mono ${Math.abs(imp.corr_dur) > 0.5 ? "text-yellow-500" : "text-muted-foreground"}`}
                  >
      ...
      <div className="p-3 bg-primary/5 text-[10px] text-muted-foreground italic flex gap-6 px-6">
        <span>
          **P-Value**: Probability that tool usage frequency is independent of run success.
        </span>
