    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 bg-black/80 backdrop-blur-sm text-body">
      <div className="panel w-full max-w-4xl max-h-full flex flex-col bg-background shadow-2xl border-border animate-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-border flex justify-between items-center bg-muted">
          <div>
            <span className="font-bold text-blue-500 uppercase tracking-widest block mb-1">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          <div className="flex items-center gap-12">
            <div className="space-y-1">
              ...
              <span
                className={`px-4 py-1.5 rounded-full font-bold uppercase tracking-widest border ${item.status === "PASS" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-red-500/10 text-red-400 border-red-500/20"}`}
              >
                {item.status}
              </span>
            </div>
                ...
                return (
                  <div className="space-y-1">
                    <h4 className="font-bold text-muted-foreground uppercase tracking-widest">
                      Coverage{" "}
                      {minCoverage > 0 && (
                        <span className="text-xs normal-case opacity-70">(min {minCoverage}%)</span>
                      )}
                    </h4>
                    <span
                      className={`text-3xl font-black ${(item.coverage || 0) >= (minCoverage || 80) ? "text-emerald-400" : "text-red-500"}`}
                    >
          ...
          <div className="space-y-4">
            {(() => {
              if (!item.details)
                return (
                  <p className="text-muted-foreground italic">No technical details provided.</p>
                );
              ...
              return (
                <>
                        ...
                        item.details.split("\n").map((line: string, i: number) => (
                          <div
                            ...
                            className={
                              line.startsWith("✓")
                                ? "text-emerald-400"
                                : line.includes("does not") || line.includes("Expected")
                                  ? "text-red-400"
                                  : "text-foreground"
                            }
                          >
                  ...
                  {item.definition && (
                    <div className="space-y-2">
                      <h4 className="font-bold text-muted-foreground uppercase tracking-widest text-[10px]">
                        Definition
                      </h4>
                  ...
                  {item.error !== undefined && (
                    <div className="space-y-2">
                      <h4 className="font-bold text-red-400 uppercase tracking-widest text-[10px]">
                        Standard Error
                      </h4>
