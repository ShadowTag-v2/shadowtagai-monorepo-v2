...
import type { ExperimentRecord } from "@/app/api/api";
import { type ExperimentStatus, useExperimentControl } from "@/hooks/useExperimentControl";
...
export default function ActiveExperimentCard({
  ...
  const {
    status,
    loadingAction,
    handleControl: originalHandleControl,
  } = useExperimentControl(exp.id, normalizedStatus);
      ...
      <Card className="hover:border-primary/30 bg-card/40 cursor-pointer group transition-all duration-300 border-border">
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="flex gap-3 items-center">
            <div
              className={`text-xs font-bold uppercase tracking-widest px-2 py-1 rounded ${status === "running" ? "bg-primary/10 text-primary" : "bg-warning/10 text-warning"}`}
            >
              {status}
            </div>
          </div>
        ...
        <div className="flex justify-between items-end mb-2">
          <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
            {exp.progress ? (
              <span>
                {exp.progress.completed} / {exp.progress.total} jobs
              </span>
            ) : (
