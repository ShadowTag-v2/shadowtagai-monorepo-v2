...
import type { ConfigBlock, ConfigBlockType } from "@/types/domain";
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage reusable configuration blocks for your experiments.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="py-12 text-center border border-dashed border-border rounded-lg bg-card/50">
          ...
          <Button variant="link" onClick={handleOpenCreate}>
            Create your first {formatType(activeTab)} block
          </Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredBlocks.map((block) => (
            <div
              key={block.id}
              className="p-4 rounded-lg border border-border bg-card hover:border-primary/50 transition-colors group flex flex-col h-full"
            >
              <div className="flex items-start justify-between mb-2">
                <h3
                  className="font-bold text-lg text-card-foreground truncate pr-2"
                  title={block.name}
                >
                  {block.name}
                </h3>
        ...
        <div className="space-y-4 border p-4 rounded bg-muted/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Input
              ...
              value={structuredData.source || ""}
              onChange={(e) => handleStructuredChange("source", e.target.value)}
              placeholder={
            ...
            <>
              <div className="flex gap-4">
                <div className="flex-1">
                  <Input
                    label="Ref (Git Tag/Branch)"
                    value={structuredData.ref || ""}
                    onChange={(e) => handleStructuredChange("ref", e.target.value)}
                    placeholder="main"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={structuredData.auto_update || false}
                    onChange={(e) => handleStructuredChange("auto_update", e.target.checked)}
                    className="w-4 h-4"
                  />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={structuredData.consent || false}
                    onChange={(e) => handleStructuredChange("consent", e.target.checked)}
                    className="w-4 h-4"
                  />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={structuredData.pre_release || false}
                    onChange={(e) => handleStructuredChange("pre_release", e.target.checked)}
                    className="w-4 h-4"
                  />
      ...
      return (
        <div className="space-y-4 border p-4 rounded bg-muted/10">
          ...
          <Input
            ...
            value={structuredData.path || ""}
            onChange={(e) => handleStructuredChange("path", e.target.value)}
            placeholder="skills/my-skill"
          />

          <Select
            label="Scope"
            value={structuredData.scope || "user"}
            onChange={(e) => handleStructuredChange("scope", e.target.value)}
            options={[
              { value: "user", label: "User (Global)" },
              { value: "workspace", label: "Workspace (Project)" },
            ]}
          />
        </div>
      );
        ...
        {useStructured && (type === "extension" || type === "skill") ? (
          ...
          <>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {type === "skill" &&
                "JSON object with 'name', 'source' (URL/Path), 'path' (opt subpath), 'scope'."}
              {type === "settings" && "JSON object for settings.json."}
              {(type === "system_prompt" || type === "context") &&
