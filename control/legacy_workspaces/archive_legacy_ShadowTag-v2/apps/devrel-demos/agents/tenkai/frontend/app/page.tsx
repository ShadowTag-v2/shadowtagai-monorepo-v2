...
export default async function Home() {
  ...
  const activeExperiments = sortedExperiments.filter(
    (e) => e.status === "RUNNING" || e.status === "running",
  );
  const completedExperiments = sortedExperiments.filter(
    (e) => e.status !== "RUNNING" && e.status !== "running",
  );
        ...
        <div>
          ...
          <div className="flex gap-4 mt-1">
            <span className="text-sm font-medium text-muted-foreground uppercase tracking-widest">
              Active Processes:{" "}
              <span className={activeExperiments.length > 0 ? "text-primary" : "text-foreground"}>
