...
export default function TemplatesPage() {
  ...
  const handleDuplicate = async (e: React.MouseEvent, id: string) => {
    ...
    try {
      ...
      if (createRes.ok) {
        ...
        toast.error(`Failed to duplicate: ${err.error || "Unknown"}`, {
          id: toastId,
        });
      }
    } catch (e) {
  ...
  const handleDeleteAll = async () => {
    if (!confirm("⚠️ WARNING: This will delete ALL templates. This action cannot be undone."))
      return;
    if (!confirm("Are you really sure?")) return;

    setDeletingAll(true);
    try {
      const res = await fetch("/api/templates/delete-all", {
        method: "DELETE",
      });
      if (res.ok) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {templates.map((template) => (
            <Link href={`/templates/${template.id}`} key={template.id} className="block group">
              <Card
                className={`h-full p-6 hover:border-muted transition-colors flex flex-col justify-between ${template.is_locked ? "border-2 border-amber-500" : "border border-border"}`}
              >
                <div>
                      ...
                      <LockToggle
                        ...
                        onToggle={async (locked) => {
                          try {
                            ...
                            setTemplates((prev) =>
                              prev.map((t) =>
                                t.id === template.id ? { ...t, is_locked: locked } : t,
                              ),
                            );
                            return true;
                          } catch (err) {
                  ...
                  <div className="mb-6 space-y-2 flex-1">
                    <div className="text-body font-mono text-sm">
                      <span className="font-bold text-muted-foreground mr-2 block mb-1 uppercase tracking-tighter">
                        Alternatives:
                      </span>
