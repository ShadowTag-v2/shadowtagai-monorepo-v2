...
export type { Checkpoint, ExperimentRecord, ExperimentSummaryRecord };

const API_BASE = typeof window === "undefined" ? "http://127.0.0.1:8080/api" : "/api";
...
export async function getExperimentSummaries(
  id: number | string,
  filter?: string,
): Promise<ExperimentSummaryRecord[]> {
...
export async function getRunResults(
  ...
  return (
    (await fetchAPI<RunResultRecord[]>(
      `/experiments/${experimentId}/runs?page=${page}&limit=${limit}`,
    )) || []
  );
}
...
export async function getMessages(runId: number, page = 1, limit = 1000): Promise<MessageRecord[]> {
  return (
    (await fetchAPI<MessageRecord[]>(`/runs/${runId}/messages?page=${page}&limit=${limit}`)) || []
  );
}
