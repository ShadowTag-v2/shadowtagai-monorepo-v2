...
export function Sidebar() {
  ...
  return (
    <aside
      className={`${currentCollapsed ? "w-[60px]" : "w-[240px]"} bg-background border-r border-border h-screen fixed left-0 top-0 flex flex-col z-50 transition-all duration-300`}
    >
      {/* Header */}
      <div
        className={`h-[56px] flex items-center ${isCollapsed ? "justify-center" : "px-4"} border-b border-border relative group`}
      >
        <div className="flex items-center gap-2">
          ...
          {!isCollapsed && (
            <span className="text-header font-bold tracking-tight whitespace-nowrap overflow-hidden">
              Tenkai
            </span>
          )}
        </div>
      ...
      <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto overflow-x-hidden">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className="opacity-80 flex items-center justify-center text-lg">
                {item.icon}
              </span>
              {!isCollapsed && (
      ...
      {/* Footer */}
      <div
        className={`p-4 border-t border-border space-y-4 ${isCollapsed ? "flex flex-col items-center" : ""}`}
      >
