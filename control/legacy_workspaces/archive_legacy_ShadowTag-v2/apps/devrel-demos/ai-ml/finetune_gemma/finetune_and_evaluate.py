...
def parse_args():
  ...
  parser = argparse.ArgumentParser(
    description="Fine-tune Gemma 3 on Open Images dataset"
  )
...
def authenticate_huggingface(hf_token=None):
  ...
  if not token:
    logger.warning(
      "⚠️ No HuggingFace token provided. Ensure access to google/gemma-3-4b-it."
    )
    return
...
def load_data(train_size, eval_size, hf_token=None):
  ...
  try:
    logger.info("Loading Oxford-IIIT Pet dataset...")
    # Load train and test splits
    train_dataset_raw = load_dataset(
      "timm/oxford-iiit-pet", split="train", streaming=False
    )
    test_dataset_raw = load_dataset(
      "timm/oxford-iiit-pet", split="test", streaming=False
    )
    ...
    def process_pet_data(example):
      return {
        ...
        "caption": class_names[
          example["label"]
        ],  # Using breed as 'caption' for format consistency
      }

    train_dataset = train_dataset_raw.map(
      process_pet_data, remove_columns=train_dataset_raw.column_names
    )
    test_dataset = test_dataset_raw.map(
      process_pet_data, remove_columns=test_dataset_raw.column_names
    )

    # Shuffle and select subsets
    train_data = train_dataset.shuffle(seed=42).select(
      range(min(train_size, len(train_dataset)))
    )
    eval_data = test_dataset.shuffle(seed=42).select(
      range(min(eval_size, len(test_dataset)))
    )

    logger.info(
      f"✓ Dataset prepared (Train: {len(train_data)}, Eval: {len(eval_data)})"
    )
    gc.collect()
...
def evaluate_model(
  model, processor, eval_data, prompt, class_names, batch_size=8, hf_token=None
):
  ...
  for i in range(0, len(eval_data), batch_size):
    ...
    inputs = processor(
      text=batch_messages, images=batch_images, return_tensors="pt", padding=True
    ).to(model.device)
  ...
  if results:
    ...
    avg_acc = (
      acc_metric.compute(predictions=y_pred, references=y_true)["accuracy"] * 100
    )
    avg_f1 = (
      f1_metric.compute(predictions=y_pred, references=y_true, average="macro")["f1"]
      * 100
    )
  ...
  for i in range(min(3, len(results))):
    ...
    logger.info(
      f"    Pred: {results[i].get('predicted_label', 'N/A')} | Correct: {results[i].get('correct', 'N/A')}"
    )
...
def train_model(model, processor, train_data, eval_data, args):
  ...
  training_args = SFTConfig(
    ...
    },  # Tell SFTTrainer to use raw dataset via data_collator
...
def main():
  ...
  # Load data
  token = args.hf_token or os.environ.get("HF_TOKEN")
  train_data, eval_data, class_names = load_data(
    args.train_size, args.eval_size, hf_token=token
  )

  # Load model and processor (processor only in dry run)
  model, processor = load_model_and_processor(
    args.model_id, args.device, hf_token=token, load_model=not args.dry_run
  )
  ...
  logger.info("-" * 40)
  logger.info("  METRIC                 | BASELINE | FINAL    | IMPROVEMENT")
  logger.info("-" * 40)
  logger.info(
    f"  Accuracy               | {baseline_results['accuracy']:>7.2f}% | {final_results['accuracy']:>7.2f}% | {final_results['accuracy'] - baseline_results['accuracy']:>+7.2f}%"
  )
  logger.info(
    f"  F1 Score (Macro)       | {baseline_results['f1']:>7.2f}% | {final_results['f1']:>7.2f}% | {final_results['f1'] - baseline_results['f1']:>+7.2f}%"
  )
  logger.info(
    f"  Unknown Rate           | {baseline_results['unknown_rate']:>7.2f}% | {final_results['unknown_rate']:>7.2f}% | {final_results['unknown_rate'] - baseline_results['unknown_rate']:>+7.2f}%"
  )
  logger.info("-" * 40)
  logger.info("=" * 80)
