...
class AppConfig:
    def __init__(self):
        ...
        logger.info(f"Initializing with project_id: {self.project_id}, location: {self.location}")
        ...
        logger.info(f"Vector Search settings - endpoint: {self.index_endpoint_id}, index: {self.index_id}, deployed: {self.deployed_index_id}")
    ...
    def initialize_clients(self):
        ...
        try:
            ...
            self.index_endpoint = aiplatform.MatchingEngineIndexEndpoint(index_endpoint_name=self.index_endpoint_id)
            logger.info(f"Successfully initialized index endpoint: {self.index_endpoint_id}")
...
async def process_prompt(request: PromptRequest, config: AppConfig = Depends(get_config)):
    """
...
def prompt_vector_search(prompt: str, num_neighbors: int, config: AppConfig) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"❓ Querying Vector Search with deployed_index_id: {config.deployed_index_id}")
...
def prompt_gemini(prompt: str, config: AppConfig) -> str:
    ...
    try:
        ...
        response = client.models.generate_content(
            model="gemini-2.5-flash-thinking-exp-01-21",  # Using hardcoded model name for consistency
            contents=[prompt],
        )
