...
async def main():
  ...
  if existing_agents:
    ...
    print(
      f"❌ Error: Agent Engine '{agent_name}' not found. Please deploy with Terraform first."
    )
    return
  ...
  memory_service = VertexAiMemoryBankService(
    project=project_id, location=resource_location, agent_engine_id=agent_engine_id
  )
  ...
  print(
    "3. Ask for a blog post. The agent should retrieve your preference from the CLOUD memory."
  )

  current_session_id = "session-1"
  await session_service.create_session(
    app_name="dev-signal", user_id=user_id, session_id=current_session_id
  )
