...
We can use LSP `didChange` notifications to send file edits _before_ saving them to disk. This allows `godoctor` to "preview" the impact of an edit (compilation errors) without touching the disk, enabling a true "Sandboxed Edit" mode.
