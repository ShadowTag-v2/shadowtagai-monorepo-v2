...
def analyze():
  ...
  total_runs = cur.execute(
    "SELECT count(*) FROM run_results WHERE experiment_id = ?", (EXP_ID,)
  ).fetchone()[0]
  ...
  for row in rows:
    ...
    if "blocked" in error_lower or "blocked" in output_lower:
      ...
      details.append(
        f"Run {run_id}: Blocked - {tool_error or tool_output.split('Status:')[0]}"
      )
      continue
      ...
      if "server is closing: eof" in output_lower:
        ...
        if "invalid character 'c'" in output_lower:
          try:
            ...
            details.append(
              f"Run {run_id}: Invalid Char 'C' - Stdin starts with: {repr(stdin_sample)}"
            )
          except:
            details.append(f"Run {run_id}: Invalid Char 'C' - (parse error)")
        else:
          details.append(
            f"Run {run_id}: Generic Exit Status 1 - {tool_output[:200].replace(chr(10), ' ')}"
          )
      else:
  ...
  for row in success_samples:
    ...
    try:
      ...
      stdin = " ".join(
        [a for a in cmd_args if "{" in a]
      )  # Rough heuristic for stdin blob
      if len(stdin) > 100:
        stdin = stdin[:100] + "..."
