...
The tool attempts to fuzzy-match the context, apply the patch, and _then_ run `gofmt`/syntax check. The failures suggest that the fuzzy matcher often misaligned the replacement, resulting in unmatched braces or broken syntax scopes. The error message was helpful in identifying _that_ it failed, but not _how_ the patch was misapplied.
...
- **Tool Improvement:** The tool could perhaps offer a "dry run" or return the "bad code" (with a diff) in the error message so the agent can see _how_ the fuzzy match failed (e.g., "I replaced lines 70-75, but that left a dangling brace on line 76").
