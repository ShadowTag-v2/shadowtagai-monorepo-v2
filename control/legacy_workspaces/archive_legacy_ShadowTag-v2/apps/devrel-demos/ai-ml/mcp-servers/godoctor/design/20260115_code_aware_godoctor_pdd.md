...
- **Mechanism:**
  ...
  - _Goal:_ The LLM always sees the disk state, not a stale memory state.
...
- **Returns:** A **Skeleton View** of the file:
  ...
  - Comments.
  - _Why?_ Gives high-level structure without token bloat, forcing the LLM to use `describe` for details.
...
- **Args:**
  ...
  - _Constraint:_ At least `package` or `file` must be set.
- **Logic:**
...
- _Warning:_ "You used `auth.SignIn`, but package `auth` only exports `Login`."
