...
- **Constrain Behavior & Tool Use**: Explicitly state what the LLM _and should not_ do.
...
- **Delegation (`sub_agents`)**: The parent agent _transfers control_. The sub-agent interacts directly with the user for subsequent turns until it finishes.
- **Agent-as-a-Tool (`AgentTool`)**: The parent agent _calls_ another agent like a function. The parent remains in control, receives the sub-agent's entire interaction as a single tool result, and summarizes it for the user.
