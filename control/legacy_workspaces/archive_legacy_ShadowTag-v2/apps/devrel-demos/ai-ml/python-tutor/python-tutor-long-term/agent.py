    ...
    if state.get("user_name") is None:
        ...
        if hasattr(callback_context, "_invocation_context"):
            ...
            if hasattr(inv_ctx, "user_message") and inv_ctx.user_message:
                user_text = inv_ctx.user_message.text.lower() if hasattr(inv_ctx.user_message, "text") else ""
                ...
                for pattern in name_patterns:
                    if pattern in user_text:
                        ...
                        if len(parts) > 1:
                            ...
                            name_part = remaining_text.split()[0] if remaining_text.split() else ""

                            # Clean up punctuation
                            name_part = (
                                name_part.replace(".", "").replace(",", "").replace("!", "").replace("?", "").replace("'", "").replace('"', "")
                            )
                            ...
                            if name_part and len(name_part) > 1 and name_part.isalpha():
                                ...
                                print(f"🎯 Detected and set user name: {state['user_name']}")
                                break
...
async def auto_save_to_memory_callback(callback_context):
    ...
    try:
        ...
        print("🧠 Session auto-saved to memory bank")
