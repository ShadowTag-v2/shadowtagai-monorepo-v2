...
class CustomerRefundAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        async for event in self.parallel_agent.run_async(ctx):
            logger.info(f"[{self.name}] Event from RefundChecks: {event.model_dump_json(indent=2, exclude_none=True)}")
            yield event

        # Get results from session state
        is_eligible_str = ctx.session.state.get("is_refund_eligible", "false")
        # Convert string to boolean
        is_eligible = is_eligible_str.lower() == "true" if isinstance(is_eligible_str, str) else bool(is_eligible_str)
        ...
        if is_eligible and purchase_history:
            ...
            async for event in self.process_full_refund.run_async(ctx):
                logger.info(f"[{self.name}] Event from ProcessFullRefund: {event.model_dump_json(indent=2, exclude_none=True)}")
                yield event

            final_message = ctx.session.state.get("final_response", "")

        elif purchase_history and not is_eligible:
            logger.info(f"[{self.name}] Customer not eligible for refund. Offering store credit.")
            async for event in self.offer_store_credit.run_async(ctx):
                logger.info(f"[{self.name}] Event from OfferStoreCredit: {event.model_dump_json(indent=2, exclude_none=True)}")
                yield event

            # Process user's response to store credit offer
            async for event in self.process_store_credit_response.run_async(ctx):
                logger.info(f"[{self.name}] Event from ProcessStoreCreditResponse: {event.model_dump_json(indent=2, exclude_none=True)}")
                yield event
        ...
        async for event in final_response_agent.run_async(ctx):
            logger.info(f"[{self.name}] Final response event: {event.model_dump_json(indent=2, exclude_none=True)}")
            yield event
