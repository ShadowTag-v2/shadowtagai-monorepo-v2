...
def _init_connector_and_engine(env_vars):
  ...
  print(
    f"Creating SQLAlchemy engine for Cloud SQL instance: {env_vars['db_instance_connection_name']} (for PGVector connection handling)..."
  )
...
def _download_and_parse_json(gcs_path):
  ...
  if return_code != 0:
    print(
      f"Error downloading JSON file using gcloud. gcloud exited with code {return_code}."
    )
  ...
  if not os.path.exists(local_json_path):
    raise FileNotFoundError(
      f"Downloaded JSON file not found locally at {local_json_path}"
    )
...
def _prepare_and_chunk_documents(
  data_items, strategy, max_docs, chunk_size, chunk_overlap
):
  print(
    f"Preparing and chunking documents with strategy: {strategy}, max_docs: {max_docs}, chunk_size: {chunk_size}, overlap: {chunk_overlap}"
  )
  initial_documents = []
  for item in data_items:
    if "kwargs" in item and "page_content" in item["kwargs"]:
      ...
      print(
        f"Skipping item due to missing 'kwargs' or 'page_content': {str(item)[:100]}..."
      )
  ...
  if len(initial_documents) > max_docs:
    print(
      f"INFO: Processing only the first {max_docs} initial documents out of {len(initial_documents)} before chunking."
    )
    initial_documents = initial_documents[:max_docs]
  else:
    print(
      f"Processing all {len(initial_documents)} initial documents as it's within MAX_DOCS_TO_PROCESS limit of {max_docs}."
    )
  ...
  for chunk_doc in chunked_documents:
    ...
    chunk_doc.metadata = (
      chunk_doc.metadata if isinstance(chunk_doc.metadata, dict) else {}
    )
    final_docs_with_ids.append({"id": str(uuid.uuid4()), "document": chunk_doc})

  print(
    f"Processed {len(initial_documents)} initial documents, resulting in {len(final_docs_with_ids)} chunks for strategy '{strategy}'."
  )
  return final_docs_with_ids


def _embed_and_store_documents(
  docs_with_ids, embeddings_service, collection_name, env_vars, getconn_func
):
  ...
  print(
    f"Adding {len(documents_to_add)} documents to PGVector collection '{collection_name}'. This may take a few minutes..."
  )
...
def main():
  ...
  try:
    ...
    print(
      f"Initializing Vertex AI Embeddings (model: text-embedding-005, project: {env_vars['project_id']}, region: {env_vars['region']})..."
    )
    ...
    for strategy in chunking_strategies:
      ...
      docs_with_ids = _prepare_and_chunk_documents(
        json_data_items,
        strategy,
        MAX_DOCS_TO_PROCESS,
        CHUNK_SIZE,
      ...
      if not docs_with_ids:
        print(
          f"No documents prepared for embedding with strategy '{strategy}'. Skipping."
        )
        continue
      ...
      print(
        f"Initializing PGVector store for collection: {collection_name_for_strategy}..."
      )
      store = PGVector(
      ...
      print(
        f"Adding {len(documents_to_add)} documents to PGVector collection '{collection_name_for_strategy}'. This may take a few minutes..."
      )
      store.add_documents(documents_to_add, ids=ids_to_add)
      logging.info(
        f"Documents added successfully to PGVector collection '{collection_name_for_strategy}'."
      )
