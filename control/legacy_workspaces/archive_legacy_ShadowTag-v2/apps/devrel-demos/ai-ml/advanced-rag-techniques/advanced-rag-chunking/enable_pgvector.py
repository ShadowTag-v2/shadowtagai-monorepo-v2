...
def main():
  ...
  if not all([db_instance_connection_name, db_name, db_user, db_pass]):
    ...
    print(
      "Ensure DB_INSTANCE_CONNECTION_NAME, SQL_DATABASE_NAME, SQL_USER, SQL_PASSWORD are set."
    )
    return

  print(
    f"Connecting to Cloud SQL instance: {db_instance_connection_name}, database: {db_name}"
  )
  ...
  try:
    ...
    with pool.connect() as db_conn:
      ...
      result = db_conn.execute(
        sqlalchemy.text("SELECT extname FROM pg_extension WHERE extname = 'vector';")
      ).fetchone()
      if result:
      ...
      result_after_creation = db_conn.execute(
        sqlalchemy.text("SELECT extname FROM pg_extension WHERE extname = 'vector';")
      ).fetchone()
      if result_after_creation:
        logging.info(
          f"Successfully verified '{result_after_creation[0]}' extension is active."
        )
      else:
        logging.info(
          "Error: pgvector extension could not be verified after attempting to create it."
        )
