...
def main():
  ...
  if missing_vars:
    print(
      f"Error: Required environment variables are not fully set: {', '.join(missing_vars)}"
    )
    print(
      "Please ensure all required environment variables are set (PROJECT_ID, REGION, DB_INSTANCE_CONNECTION_NAME, SQL_DATABASE_NAME, SQL_USER, SQL_PASSWORD, PGVECTOR_BASE_COLLECTION_NAME)."
    )
    return

  print(
    f"Starting query process for database: '{db_name}', using base collection: '{base_collection_name}'"
  )
  ...
  try:
    ...
    print(
      f"Initializing Vertex AI Embeddings (model: text-embedding-005, project: {project_id}, location: {region}) for querying..."
    )
    embeddings_service = VertexAIEmbeddings(
      model_name="text-embedding-005", project=project_id, location=region
    )
    ...
    for strategy in chunking_strategies:
      ...
      print(
        f"--- Preparing to query for {strategy.upper()} Chunking Strategy (Collection: {collection_name_for_strategy}) ---"
      )

      try:
        logging.info(
          f"  Initializing PGVector store for {collection_name_for_strategy}..."
        )
        store = PGVector(
        ...
        logging.info(
          f"  Performing similarity search for collection {collection_name_for_strategy}..."
        )
        ...
        if not retrieved_docs_with_scores:
          print(
            "    No documents found. Possible reasons: Collection is empty, query is too dissimilar, or an issue with connection/setup."
          )
        else:
          logging.info(
            f"    Found {len(retrieved_docs_with_scores)} relevant documents:"
          )
          for i, (doc, score) in enumerate(retrieved_docs_with_scores):
            ...
            if doc.metadata:
              ...
              print(
                f"        Metadata: source='{source}', other_metadata_keys={list(doc.metadata.keys())}"
              )
            print("        " + "-" * 30)
      except Exception as e_store:
        print(
          f"    Error querying collection '{collection_name_for_strategy}': {e_store}"
        )
