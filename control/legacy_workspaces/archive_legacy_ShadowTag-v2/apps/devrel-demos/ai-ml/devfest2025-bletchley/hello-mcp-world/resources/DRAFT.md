...
Since AIDA only needs to ingest plain text `.table` files right now, I didn't actually _need_ `markitdown`'s document parsing capabilities. Rather than downgrading my entire Python environment, I chose a quick and dirty hack: mocking the offending module before `sqlite-rag` could try to import it.
