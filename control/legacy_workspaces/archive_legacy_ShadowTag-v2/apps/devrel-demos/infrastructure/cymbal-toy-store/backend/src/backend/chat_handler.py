...
class ChatHandler:
  ...
  def _validate_intent_output(self, intent: dict[str, Any]) -> bool:
    for key in [
      ...
      "shouldRecommendProductReasoning",
      "summary",
    ]:
  ...
  def _create_response(
    ...
    if not intent["isOnTopic"]:
      prompts[0] = self._gen_system_template(
        "Gently redirect the conversation back to toys and other products in the store. Do not use markdown for output"
      )
    elif not intent["shouldRecommendProduct"]:
      prompts[0] = self._gen_system_template(
        f"Need more information - {intent['shouldRecommendProductReasoning']}"
      )
    else:
  ...
  def respond(self, messages: dict[str, Any]) -> str:
    ...
    try:
      ...
      logging.exception(
        f"Failed to decode LLM intent classification as JSON: {intent_json}"
      )
