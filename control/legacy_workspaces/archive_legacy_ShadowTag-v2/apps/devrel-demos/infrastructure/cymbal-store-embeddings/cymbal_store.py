...
def get_products(db: sqlalchemy.engine.base.Engine, embeddings: str) -> dict:
  ...
  try:
    with db.connect() as conn:
      app_products = conn.execute(
        stmt, parameters={"embeddings": embeddings}
      ).fetchall()
...
_BACKGROUND_IMAGE_LIGHT = (
  f"url(data:image/png;base64,{bimage64_light}) center/cover no-repeat"
)

bimage_dark = open("pub/app_background_dark.png", "rb")
bimage64_dark = base64.b64encode(bimage_dark.read()).decode("ascii")
_BACKGROUND_IMAGE_DARK = (
  f"url(data:image/png;base64,{bimage64_dark}) center/cover no-repeat"
)
