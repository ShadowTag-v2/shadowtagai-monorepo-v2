...
export async function startLoader({
  ...
  checkSecretPassword(secretPassword);
  return getRequest(`${main_vm_address}/game/start/${playerName}`);
}
...
export async function getVmAllStats(secretPassword: string) {
  ...
  const gameVmSelectionIndex = parsedResponse.ALL_VMS.findIndex(
    (vmName: string) => vmName === gameVmSelection,
  );
  const vmStats = {
    statusArray,
    ...
    playerOneScore: statusArray
      .slice(0, 4)
      .reduce((agg, vmStatus) => agg + Number(vmStatus.score), 0),
