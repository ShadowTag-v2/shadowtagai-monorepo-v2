...
const playerEndPositions: [number, number, number][] = player1VmXPositions.map((xPosition) => [
  xPosition,
  vmYPositionBottom,
  0,
]);
...
export default function Play() {
  ...
  const playerOneActiveVmIndex = player1VmXPositions.findIndex(
    (value) => playerOneEnd[0] === value,
  );
  ...
  const playerTwoActiveVmIndex = player2VmXPositions.findIndex(
    (value) => playerTwoEnd[0] === value,
  );
  ...
  useEffect(() => {
    const getStartLoader = async (playerName: string) => {
      try {
        const { player_name } = await startLoader({
          playerName,
          secretPassword: localStorage.getItem("secretPassword") || "",
        });
  ...
  const addFailBlock = (failBlockStartingPosition: [number, number, number]) => {
    ...
    const uid = crypto.randomUUID();
    const side: "LEFT" | "RIGHT" = ["LEFT", "RIGHT"][Math.floor(Math.random() * 2)] as
      | "LEFT"
      | "RIGHT";
  ...
  return (
    <main className="flex h-screen flex-col items-center justify-between">
      ...
      <div className="absolute top-40">
        <span
          className={`text-9xl font-mono transition-opacity duration-1000 ${timeRemaining > 0 && timeElapsed > -1 ? "opacity-100" : "opacity-0"}`}
        >
          {timeRemaining}
        </span>
      </div>
      <div className="flex w-full overflow-clip justify-center text-7xl font-mono">
        <div
          className="flex flex-row transition-all duration-1000 justify-between px-2"
          style={{
            height: "80px",
        ...
        >
          <h2 className={`mb-3 text-2xl font-semibold w-64 text-balance`}>{playerName}</h2>
        ...
        >
          ...
          <h3 className="mb-3 text-2xl font-semibold w-56 text-right text-balance">
      ...
      <Canvas>
        ...
        <LoadBalancerBox position={playerOneLoadBalancerPosition} playerColor={colors.player1} />
        <LoadBalancerBox position={playerTwoLoadBalancerPosition} playerColor={colors.player2} />
        {playerOneBlocks.map(({ xPosition, yPosition, uuid }) => {
          return (
            <MessageIncoming
              ...
              onVmContact={
                playerOneAtMaxCapacity
                  ? () => addFailBlock([playerOneMid[0], vmYPositionTop + 0.3, playerOneMid[2]])
                  : () => {}
              }
        ...
        {successBlocks.map((block) => {
          return (
            <MessageSuccess key={block.uid} position={block.startingPosition} type={"SUCCESS"} />
          );
        })}
        <MessagePath
      ...
      <div
        className={`flex items-center justify-center transition-opacity fixed top-0 left-0 right-0 bottom-0 bg-gradient-to-t from-white via-white to-transparent duration-1000 ${timeElapsed < -1 ? "opacity-90" : "opacity-0"}  ${timeElapsed < 0 ? "z-50" : "-z-50"}`}
      >
