...
def run_billing_link(project_id, user_billing_name=None):
  ...
  try:
    ...
    if user_billing_name and user_billing_name.strip():
      ...
      print(
        f"DEBUG: Searching for user-specified billing account containing: '{target_name}'"
      )
    else:
      # Default behavior
    ...
    if not target_account:
      return (
        False,
        f"Error: Could not find a billing account matching '{target_name}'. Please check the name or ensure you have access.",
      )

    print(
      f"DEBUG: Found and selected Billing Account: {target_account.get('displayName')}"
    )
    ...
    return (
      True,
      f"Successfully linked project '{project_id}' to billing account '{acct_display}' ({acct_id})",
    )
...
def run_gcloud_script(deploy_id, project, region, password, cluster, instance):
  ...
  try:
    ...
    if process.returncode == 0:
      ...
      deployments[deploy_id]["summary"] = generate_deployment_summary(
      ...
      resources_exist = (
        "already exists" in log_text
        or "alreadyexists" in log_text
        or "creating cluster..." in log_text
      )  # Progress indicates partial success
  ...
  try:
    ...
    if not project_id:
      return jsonify(
        {
          "status": "error",
          "message": "Project ID is missing. Please enter a Project ID first.",
        }
      )
  ...
  try:
    if command_type == "get_ip":
      ...
      if result.returncode == 0:
        ...
        return jsonify(
          {"status": "success", "output": ip, "message": f"Private IP: {ip}"}
        )
      else:
        return jsonify(
          {
