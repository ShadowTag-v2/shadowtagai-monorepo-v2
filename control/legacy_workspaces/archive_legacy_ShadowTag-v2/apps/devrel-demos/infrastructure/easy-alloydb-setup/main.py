...
def run_gcloud_script(deploy_id, project, region, password, cluster, instance):
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            resources_exist = (
                "already exists" in log_text or "alreadyexists" in log_text or "creating cluster..." in log_text
    ...
    try:
        if command_type == "get_ip":
            ...
            if result.returncode == 0:
                ...
                return jsonify({"status": "success", "output": ip, "message": f"Private IP: {ip}"})
            else:
                return jsonify(
                    {
                        "status": "error",
