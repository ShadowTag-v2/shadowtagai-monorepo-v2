...
export default function Play() {
  ...
  const player1GpuPositions = [-4.9, -3.69, -2.52, -1.31];
  ...
  const yellowDataBlockTexture = useLoader(TextureLoader, "/assets/yellow-datablock.png");
  ...
  const robotBasePosition: [number, number, number] = [0.0, 1.0, 0];
  ...
  const playerTwoActiveVmIndex = player2VmXPositions.reduce((bestIndex, pos, i) => {
    return Math.abs(pos - player2FunnelX) <
      Math.abs(player2VmXPositions[bestIndex] - player2FunnelX)
      ? i
      : bestIndex;
  }, 0);
    ...
    const getVMStatus = async () => {
      if (timeElapsed < totalGameTime + 5) {
        ...
        if (timeElapsed < 0) {
          const resetStats = {
            ...vmStats,
            statusArray: vmStats.statusArray.map((vm) => ({
              ...vm,
              utilization: 0,
            })),
            playerOneScore: 0,
        ...
        const playerTwoActiveVmIndex = player2GpuPositions.reduce((bestIndex, pos, i) => {
          return Math.abs(pos - player2FunnelX) <
            Math.abs(player2GpuPositions[bestIndex] - player2FunnelX)
            ? i
            : bestIndex;
        }, 0);
        ...
        const updatedVmStats = {
          ...
          statusArray: vmStats.statusArray.map((vmDetails, index) => {
            if (index < 4) {
              ...
              if (index === playerOneActiveVmIndex) {
                return {
                  ...vmDetails,
                  utilization: Math.min(vmDetails.utilization + leftSideIncreaseRate, 1),
                };
              } else {
                return {
                  ...vmDetails,
          ...
          playerOneScore:
            ...
            vmStats.statusArray
              .slice(0, 4)
              .reduce((agg, vm) => agg + (vm.utilization > 0 ? 1 : 0), 0),
          playerTwoScore:
  ...
  const [userFunnelPosition, setUserFunnelPosition] = useState<[number, number, number]>([
  ...
  return (
    <main className="flex h-screen flex-col items-center justify-between">
      ...
      <Canvas style={{ background: "white" }}>
        ...
        <GpuGroup positions={player1GpuPositions} stats={vmStats.statusArray.slice(0, 4)} />

        {/* Right side GPUs */}
        <GpuGroupRight positions={player2GpuPositions} stats={vmStats.statusArray.slice(4, 8)} />
      </Canvas>
