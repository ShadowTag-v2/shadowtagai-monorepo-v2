  ...
  return (
    <group ref={ref} position={startPosition}>
      ...
      <mesh>
        ...
        <meshBasicMaterial color="#FADF6B" transparent={true} opacity={0.9} toneMapped={false} />
      </mesh>

      {/* Border Outline */}
      <lineSegments>
        <edgesGeometry attach="geometry" args={[new BoxGeometry(0.3, 0.3, 1.3)]} />
        <lineBasicMaterial attach="material" color="#e3a507" linewidth={3} />
      </lineSegments>
    </group>
  );
}
...
export default function Play3D() {
  ...
  const createNewCube = () => {
    ...
    if (pattern === 0) {
      ...
      angle = ((Math.floor(id / 10) % 4) * Math.PI) / 2 + Math.random() * (Math.PI / 4);
      distance = 0.01 + Math.random() * 0.02;
    }
    ...
    return {
      key: `cube-${id}`,
      startPosition: [dirX * distance, dirY * distance, -12 - Math.random() * 4] as [
        number,
        number,
        number,
      ],
