...
export default function MessageIncomingRight({ showResults, funnelX }: MessageIncomingRightProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setPhase2Blocks((prev) => (prev.length < 8 ? [...prev, crypto.randomUUID()] : prev));
