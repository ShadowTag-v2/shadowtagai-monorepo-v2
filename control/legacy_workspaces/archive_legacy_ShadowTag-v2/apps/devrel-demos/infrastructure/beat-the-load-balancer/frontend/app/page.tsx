...
export default function Home() {
  const [secretPassword, setSecretPassword] = useState<string>(
    (typeof window !== "undefined" && localStorage.getItem("secretPassword")) || "",
  );
  const [secretPasswordExists, setSecretPasswordExists] = useState<boolean>(false);
  const [showPassword, setShowPassword] = useState<boolean>(
    secretPasswordExists && !secretPassword,
  );
  ...
  return (
    <>
      ...
      <main className="flex flex-col min-h-screen items-center justify-between p-24">
        <h1 className={`mb-3 text-7xl font-mono -z-20`}>Load Balancing Blitz</h1>
        <StartButton onClick={() => setShowPlayComponent(true)} showPassword={showPassword} />
        <div>
          <p>
            Press 1, 2, 3, and 4 to distribute the incoming requests across the four virtual
            machines.
          </p>
          <p>
        ...
        {secretPasswordExists && (
          <>
            {showPassword ? (
              ...
              <button onClick={handleShowPasswordClick}>Enter Password</button>
            )}
          </>
        )}
      </main>
