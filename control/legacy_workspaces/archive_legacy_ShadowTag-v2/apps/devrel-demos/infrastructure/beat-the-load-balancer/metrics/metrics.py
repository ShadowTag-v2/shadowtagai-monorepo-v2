...
def send_system_health_message(cpu, memory, vm, status, uniqueid) -> None:
    _datetime = datetime.datetime.now(datetime.UTC).astimezone().isoformat()
    # Create the healthcheck object
    healthcheck_message = game_healthcheck.GameHealthcheck(_datetime, cpu, memory, vm, bool(status), uniqueid)
...
def send_score_message(score, vm, uniqueid, difficulty) -> None:
    ...
    score_message = game_score.GameScore(_datetime, player_type, score, vm, difficulty, uniqueid)
