...
def reset_player_game() -> dict:
    ...
    if job_mgr.is_job_running():
        ...
        response["details"] = f"Flag file {RESET_FLAG_FILE} says a job seem to be running"
    else:
        response["status"] = "ok"
...
def update_vm_select(vm_name) -> dict:
    ...
    if vm_name in config.WH_VM_NAMES:
        if vm_name != WH_VM_MGR.get_selected_vm():
            logging.warning(f" ===> vm-selection is updated! ({WH_VM_MGR.get_selected_vm()} --> {vm_name})")
...
def automatic_routing_options() -> dict:
    url_config = {vm_name: f"http://{config.VM_IPS[vm_name]}:8000/process" for vm_name in config.WH_VM_NAMES}
...
def automatic_round_robin_routing():
    ...
    logging.info(f"/rr/process - selected vm : {selected_vm} ==> redirect request to {url}")
    return redirect(url, code=302)
