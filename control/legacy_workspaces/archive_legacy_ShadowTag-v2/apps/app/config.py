...
class Settings(BaseSettings):
    ...
    shadowtag_v2_enabled: bool = Field(default=True, description="Enable ShadowTag v2 ledger")
    shadowtag_retention_years: int = Field(default=7, description="Audit log retention period")
    shadowtag_enabled: bool = Field(default=True)
    shadowtag_private_key: str | None = None
    shadowtag_chain_id: str | None = None
    ipfs_gateway_url: str | None = Field(default=None, description="IPFS gateway for hash pinning")
    ...
    mcf_pricing_tier: str = Field(default="pro", description="Default pricing tier (free, pro, enterprise)")
    ...
    mcf_rate_limit_assessments: int = Field(default=1000, description="Monthly assessment limit")
    mcf_rate_limit_api_calls: int = Field(default=50000, description="Monthly API call limit")

    # Post-Generation Validation (GPT Store Pattern)
    mcf_post_gen_validation_enabled: bool = Field(default=True)
    mcf_auto_remediation_enabled: bool = Field(default=False, description="Auto-fix non-critical violations")
