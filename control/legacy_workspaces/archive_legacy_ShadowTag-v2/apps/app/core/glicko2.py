...
class Glicko2System:
  ...
  def _calculate_variance(
    self, mu: float, opponents: list[tuple[float, float]]
  ) -> float:
...
class AgentRanking:
  ...
  def record_match(self, agent_id: str, opponent_id: str, score: float):
    ...
    self.agents[agent_id] = self.system.update(
      agent, [opponent.mu], [opponent.phi], [score]
    )
  ...
  def get_rankings(self) -> list[tuple[str, float, float]]:
    ...
    rankings = [
      (agent_id, player.mu, player.phi) for agent_id, player in self.agents.items()
    ]
