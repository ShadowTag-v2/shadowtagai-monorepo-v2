...
class ShadowTagLedger:
  ...
  def __init__(
    self, firestore_client=None, bigquery_client=None, enable_ipfs: bool = False
  ):
    """
  ...
  async def verify_chain(self, full_verification: bool = False) -> ChainStats:
    ...
    chain_valid = True
    expected_hash = (
      entries_to_check[0].previous_hash if entries_to_check else self._genesis_hash()
    )
  ...
  async def get_chain_proof(self, entry_id: str) -> dict[str, Any]:
    ...
    prev_hash = (
      self._chain[position - 1].data_hash if position > 0 else self._genesis_hash()
    )
    next_hash = (
      self._chain[position + 1].previous_hash
      if position < len(self._chain) - 1
      else None
    )
  ...
  async def generate_signed_url(
    self, entry_id: str, expires_minutes: int = 15
  ) -> str | None:
    """
