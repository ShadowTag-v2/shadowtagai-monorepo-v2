...
class Judge6Grounded:
  ...
  def __init__(
    ...
    self.project_id = project_id or os.getenv(
      "VERTEX_PROJECT_ID", "acquired-jet-478701-b3"
    )
  ...
  def _check_compliance_domains(
    self, content: str, user_region: str | None
  ) -> ComplianceFlags:
...
Be specific and cite relevant doctrine sections."""
  ...
  async def score_governance(
    ...
    risk_score = self._calculate_risk_score(
      request_type, compliance_flags, transaction_value
    )
    ...
    logger.info(
      f"Governance scored: {decision.value} (risk={risk_score}, latency={latency_ms:.0f}ms, citations={len(citations)})"
    )
