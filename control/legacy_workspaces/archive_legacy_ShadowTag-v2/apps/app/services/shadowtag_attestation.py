...
class ShadowTagClient:
  ...
  def _sign_attestation(self, attestation_data: dict) -> str:
    ...
    signature = hmac.new(
      self.config.signing_key.encode(), message, hashlib.sha256
    ).hexdigest()
    return signature
  ...
  async def verify_attestation(
    ...
    attestation_data = {
      ...
      "metadata": {
        k: v for k, v in attestation.metadata.items() if k != "compliance_data"
      },
    }

    expected_signature = await self._create_signature(
      attestation_data, attestation.level
    )
