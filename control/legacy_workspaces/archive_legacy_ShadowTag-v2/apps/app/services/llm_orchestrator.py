...
class GrokIntake:
  ...
  async def decompose_query(self, query: str) -> list[dict[str, Any]]:
    ...
    if any(
      kw in query.lower() for kw in ["classify", "tier", "intelligence", "source"]
    ):
      return [
        {
          "thread_id": "intel_1",
...
class PNKLNOrchestrator:
  ...
  async def process_query(
    ...
    tier_confidences = [
      t.tier_classification.confidence
      for t in threads
      if t.tier_classification is not None
    ]
    confidence = (
      sum(tier_confidences) / len(tier_confidences) if tier_confidences else 0.5
    )
  ...
  async def _synthesize_results(self, query: str, threads: list[Thread]) -> str:
    ...
    for thread in threads:
      ...
      if thread.tier_classification:
        synthesis_parts.append(
          f"Classification: Tier {thread.tier_classification.tier} ({thread.tier_classification.confidence:.0%} confidence)"
        )
