...
class GeminiAgent:
    ...
    def __init__(
        ...
        if genai and api_key:
            ...
            self.model = genai.GenerativeModel(model_name=model_name, system_instruction=persona, tools=self.tools)
        else:
            self.model = None
...
"""
    ...
    def _fallback_proposal(self, title: str, content: str, tags: list[str]) -> dict[str, Any]:
...
Decision Tendency: No bias, pure evidence-based classification.""",
    ...
    async def classify_with_debate(
        ...
        # Aggregate votes
        final_tier, final_confidence, reasoning = self._aggregate_votes(debate_rounds, method=voting_method)
    ...
    def _aggregate_votes(self, debate_rounds: list[list[dict]], method: str = "weighted_confidence") -> tuple[int, float, str]:
        """
        ...
        if method == "neutral_arbiter":
            ...
            majority_confidences = [p["confidence"] for p in final_round.values() if p["tier"] == majority_tier]
            ...
            tier_variance = sum(abs(p["tier"] - final_tier) * p["confidence"] for p in final_round.values()) / total_weight
    ...
    def _build_debate_summary(self, debate_rounds: list[list[dict]]) -> str:
        ...
        for round_num, round_data in enumerate(debate_rounds, 1):
            ...
            for agent_name, proposal in round_data.items():
                summary.append(f"  {agent_name.capitalize()}: Tier {proposal['tier']} ({proposal['confidence']:.0%} confidence)")
...
def create_atp_519_tools() -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "parameters": {
                "type": "object",
                "properties": {"source_id": {"type": "string", "description": "Source identifier"}},
                "required": ["source_id"],
            },
        },
    ]
...
async def example_usage():
    ...
    chat = GeminiGroupChat(api_key=os.getenv("GEMINI_API_KEY"), agents=["skeptic", "optimist", "neutral"])
