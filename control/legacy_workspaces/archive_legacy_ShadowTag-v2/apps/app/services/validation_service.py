...
class ATP519RuleEngine:
  ...
  def rate_source_reliability(
    self, domain: str, source_history: dict | None = None
  ) -> str:
  ...
  def check_relevance(self, tags: list[str], target_domains: list[str] = None) -> int:
    ...
    matches = sum(
      1 for tag in tags if any(domain in tag.lower() for domain in target_domains)
    )
...
class ValidationService:
  ...
  async def validate(
    self, request: ValidationRequest, item_data: dict
  ) -> ValidationResponse:
    """
  ...
  def _calculate_coverage(
    self, atp_scores: ATP519Scores, jr_compliance: JRCompliance
  ) -> float:
