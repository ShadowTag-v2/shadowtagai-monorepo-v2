...
class ContentEngine:
  ...
  async def verify_c2pa(
    self, request: C2PAVerificationRequest
  ) -> C2PAVerificationResponse:
    """
    ...
    has_credentials = (
      request.content_data is not None or request.content_url is not None
    )
  ...
  async def attach_credentials(
    self, content_id: str, creator_id: str
  ) -> dict[str, Any]:
  ...
  async def watermark_content(self, request: WatermarkRequest) -> WatermarkResponse:
    ...
    if request.content_type == ContentType.VIDEO:
      ...
      raise ValueError(
        f"Unsupported content type for watermarking: {request.content_type}"
      )
