...
Based on docs/cor8-aiyou-global-edge-fabric/03-technical-architecture/gemini-ingestion-layer.md
"""
...
class EthicalComplianceChecker:
    ...
    USER_AGENT = "PNKLNBot/1.0 (+https://pnkln.ai/bot; compliance@aiyou.ai)"
...
"""
    ...
    async def classify(self, title: str, summary: str, tags: list[str], domain: str) -> TierClassification:
    ...
    def _rule_based_classification(self, title: str, summary: str, tags: list[str], domain: str) -> TierClassification:
        """Fallback rule-based classification"""
        ...
        # Check for Tier 1
        if any(kw in text_lower for kw in tier1_keywords) or any(d in domain for d in tier1_domains):
            return TierClassification(
                tier=1,
                confidence=0.75,
...
class IngestionService:
    ...
    def __init__(self, gemini_api_key: str | None = None):
        ...
        self.storage: dict[str, dict] = {}  # In-memory storage (replace with Cloud Storage in production)
    ...
    async def _process_item(self, item_id: str, request: IngestionSubmitRequest):
        ...
        try:
            ...
            rate_ok = await self.ethics_checker.check_rate_limit(request.source.type, request.source.domain)
    ...
    def _generate_item_id(self, request: IngestionSubmitRequest) -> str:
        ...
        content_hash = hashlib.blake2b(request.content.full_text.encode(), digest_size=4).hexdigest()
