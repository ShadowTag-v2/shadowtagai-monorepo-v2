        ...
        if rule.trigger == TriggerCondition.ALWAYS:
            ...
            if ns_output.explicit_content_signals:
                ...
                if max_confidence >= self.config.explicit_threshold:
                    triggered = True
                    trigger_details["confidence"] = max_confidence
                    trigger_details["categories"] = [s.content_categories for s in ns_output.explicit_content_signals]
...
class JRPolicyEngine:
    ...
    async def evaluate(
        ...
        for rule in all_rules:
            ...
            if not evaluation.passed:
                ...
                self._stats["actions_by_type"][action_name] = self._stats["actions_by_type"].get(action_name, 0) + 1
        ...
        human_review = ComplianceAction.FLAG_FOR_REVIEW in required_actions or (
            self.config.ambiguous_range[0] <= ns_output.overall_risk_score <= self.config.ambiguous_range[1]
        )
    ...
    def _calculate_risk_tier(self, ns_output: NSDetectionOutput, violations: list[PolicyViolation]) -> RiskTier:
    ...
    def _generate_reasoning(
        ...
        violation_summary = []
        for v in violations:
            violation_summary.append(f"- {v.rule_id}: {v.description} (severity: {v.severity.value})")
