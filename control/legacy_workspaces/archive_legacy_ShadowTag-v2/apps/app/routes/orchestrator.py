...
class ProcessQueryRequest(BaseModel):
    ...
    enable_review_rotation: bool = Field(default=False, description="Enable 3-round peer review (slower, more accurate)")
    metadata: dict[str, Any] | None = Field(default=None, description="Optional metadata for tracking")
...
class IntelligenceClassificationRequest(BaseModel):
    ...
    enable_debate: bool = Field(default=True, description="Use multi-agent debate (3 agents, 2 rounds)")
...
async def process_query(request: ProcessQueryRequest):
    ...
    """
    try:
        result = await orchestrator.process_query(query=request.query, enable_review_rotation=request.enable_review_rotation)
