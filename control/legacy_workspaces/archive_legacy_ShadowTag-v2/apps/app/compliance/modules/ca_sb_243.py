...
class CASB243Module(ComplianceModule):
  ...
  def _define_metadata(self) -> ModuleMetadata:
    return ModuleMetadata(
      ...
      description=(
        "California law requiring AI systems to detect self-harm risks and implement protections for minors interacting with AI."
      ),
  ...
  def determine_risk_tier(self, input_data: AssessmentInput) -> RiskTier | None:
    ...
    if any(
      indicator in content_type for indicator in ["mental_health", "crisis", "therapy"]
    ):
      return RiskTier.HIGH
