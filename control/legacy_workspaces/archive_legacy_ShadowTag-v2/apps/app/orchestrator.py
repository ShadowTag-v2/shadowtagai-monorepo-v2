...
def node_critic(state: AgentState) -> dict:
    ...
    response = llm.invoke(
        [
            SystemMessage(content="You are a hostile critic. Find security flaws and logic errors."),
...
def route_after_gate(state: AgentState):
    if state["policy_issues"] or state["risk_level"] in [
        RiskLevel.RED,
        RiskLevel.AMBER,
    ]:
        return "critic"
...
workflow.add_conditional_edges("policy_gate", route_after_gate, {"critic": "critic", "arbiter": "arbiter"})
