...
async def generate_blueprint(
  ...
  try:
    ...
    logger.info(
      f"Blueprint generated with {len(blueprint.selected_modules)} modules, cost: ${blueprint.estimated_monthly_cost_usd}/mo"
    )
    return blueprint
...
async def run_assessment(
  ...
  try:
    ...
    logger.info(
      f"Assessment {result.assessment_id} completed: status={result.overall_status.value}, score={result.overall_score:.2%}"
    )
    return result
...
async def validate_content(
  ...
  try:
    ...
    logger.info(
      f"Validation {result.validation_id}: compliant={result.is_compliant}, violations={len(result.violations)}"
    )
    return result
  except Exception as e:
...
async def batch_assessment(
  ...
  try:
    results = await engine.assess_batch(
      request.inputs, max_concurrent=request.max_concurrent
    )
