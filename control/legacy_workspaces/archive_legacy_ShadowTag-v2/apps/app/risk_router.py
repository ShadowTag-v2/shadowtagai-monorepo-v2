...
# --- LOGGING SETUP ---
logging.basicConfig(
  level=logging.INFO, format="%(asctime)s - JUDGE6 - %(levelname)s - %(message)s"
)
...
class Judge6:
  ...
  def evaluate(
    self, prompt: str, file_paths: list[str] = [], mission_id: str = "default"
  ) -> Judgment:
    """
    ...
    logger.info(
      f"Mission {mission_id}: Judge6 Review - Iteration {current_iter}/3 [{phase}]"
    )
    ...
    if current_iter < 3:
      return Judgment(
        ...
        csrmc_status=CSRMCStatus(
          cato_valid=False, phase=phase, high_signal_controls=[]
        ),
      )
  ...
  def _check_csrmc_compliance(
    self, prompt: str, files: list[str], phase: str
  ) -> Judgment:
    """
    ...
    if (
      phase == "BUILD"
      and "terraform" in prompt_lower
      and "sentinel" not in prompt_lower
    ):
      # Gentle nudge to use Policy-as-Code
      pass  # In strict mode, we might mandate this.

    return Judgment(
      verdict=RiskLevel.GREEN, reason="CSRMC Check Pass", iteration_count=0
    )
  ...
  def _check_legislation(self, prompt: str) -> Judgment:
    eu_rules = (
      self.policy.get("judge6_constitution", {})
    ...
    return Judgment(
      verdict=RiskLevel.GREEN, reason="Legislative Pass", iteration_count=0
    )
...
def evaluate_risk(
  prompt: str, file_paths: list[str] = [], mission_id: str = "default"
) -> dict:
