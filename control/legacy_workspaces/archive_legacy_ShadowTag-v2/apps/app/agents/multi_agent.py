...
class MultiAgentSystem:
  ...
  def register_agent(
    self, agent_id: str, role: AgentRole, initial_rating: float = 1500.0
  ):
    ...
    logger.info(
      f"Registered agent {agent_id} ({role.value}) with rating {initial_rating}"
    )
  ...
  async def run_debate(
    ...
    """
    logger.info(
      f"Starting debate: {topic} ({num_participants} agents, {rounds} rounds)"
    )
  ...
  def _calculate_consensus(
    self, participants: list[DebateParticipant], topic: str
  ) -> str:
...
"""
  ...
  async def deep_reasoning(
    self, problem: str, use_dte_evolution: bool = True
  ) -> AgentResponse:
    """
    ...
    return [
      {
        ...
        "tasks_completed": self.agents[agent_id].get("tasks_completed", 0)
        if agent_id in self.agents
        else 0,
      }
