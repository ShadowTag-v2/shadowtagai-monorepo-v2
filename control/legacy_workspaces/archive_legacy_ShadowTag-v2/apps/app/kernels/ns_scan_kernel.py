    ...
    async def execute(self, kernel_input: KernelInput) -> KernelOutput:
        ...
        try:
            ...
            if isinstance(kernel_input.data, NSScanInput):
                ...
                raise KernelChainError(f"Invalid input type: expected NSScanInput, str, or dict, got {type(kernel_input.data)}")
...
class GeminiNSScanKernel(NSScanKernel):
    ...
    async def execute(self, kernel_input: KernelInput) -> KernelOutput:
        ...
        if self._gemini_model and 0.3 < detection_output.overall_risk_score < 0.7 and not detection_output.has_critical_signals:
