    ...
    try:
      ...
      if isinstance(kernel_input.data, NSScanInput):
        ...
        raise KernelChainError(
          f"Invalid input type: expected NSScanInput, str, or dict, got {type(kernel_input.data)}"
        )
...
def create_ns_scan_kernel(
  ...
  if gemini_api_key:
    return GeminiNSScanKernel(
      gemini_api_key=gemini_api_key, max_latency_ms=max_latency_ms
    )
