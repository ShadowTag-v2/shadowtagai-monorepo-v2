...
# Application Layer
mkdir -p apps/{flyingmonkeys-server/src,agent-manager-ui/public}
# Library Layer (The Brains)
mkdir -p libs/aiyou/{agents,governance,proxies,superpowers}
mkdir -p libs/arsenal/{shadowtag_core,tegu_vision,gaas_flight,flying_monkeys,safety_net}
...
1. **Governance:** All code changes must pass 'libs.aiyou.governance.sentinel'.
...
from aiyou.agents.summarizer import summarizer
...
from aiyou.agents.recursive_rlm import RecursiveAgent
...
from aiyou.agents.deep_research import researcher
while True:
...
echo "🧠 Brain:    libs/aiyou/agents/"
echo "⚖️  Law:      libs/aiyou/governance/"
echo "💪 Muscle:   libs/arsenal/"
echo "--------------------------------------------------------"
echo "👉 1. Run 'uv sync'"
echo "👉 2. Run 'uv run python apps/flyingmonkeys-server/src/main.py'"
