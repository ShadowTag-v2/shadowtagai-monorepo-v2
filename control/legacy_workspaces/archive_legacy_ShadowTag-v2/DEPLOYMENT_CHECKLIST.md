...
cd deploy && kubectl apply -f . && kubectl wait --for=condition=ready pod -l app=judge6 -n pnkln --timeout=300s  # Expected: All pods ready
...
- [ ] Upload model: `gsutil -m cp -r MODEL_PATH gs://BUCKET/models/judge6/`
- [ ] Verify upload: `gsutil ls gs://BUCKET/models/judge6/`
...
- [ ] Update `04-judge6-deployment.yaml` with bucket name and GPU type
- [ ] Deploy: `kubectl apply -f .`
- [ ] Wait for ready: `kubectl wait --for=condition=ready pod -l app=judge6 -n pnkln --timeout=600s`
...
### Cloud Console
- [ ] Navigate to GKE → Workloads → judge6
- [ ] Verify metrics visible (CPU, Memory, Network)
- [ ] Check logs for errors

### Prometheus (Optional)
- [ ] Port forward metrics: `kubectl port-forward -n pnkln svc/judge6-metrics 8080:8080`
...
# Check logs
kubectl logs -n pnkln -l app=judge6
...
### High Latency
**Check**: `kubectl logs -n pnkln -l app=judge6`
...
For issues:
1. Check logs: `kubectl logs -n pnkln -l app=judge6`
