...
class EthicalCrawlConfig:
    ...
    transparency_contact: str = "intelligence@pnkln.ai"
...
class TierClassifier:
    ...
    def __init__(self, gemini_api_key: str | None = None):
        ...
        if GEMINI_AVAILABLE and self.api_key:
            ...
            self.model = genai.GenerativeModel("gemini-3.1-flash-exp")
            self.enabled = True
            logger.info("Gemini tier classifier initialized")
        else:
            self.enabled = False
            logger.warning("Gemini tier classifier disabled (API key missing or package not installed)")

    def classify_item(self, title: str, content: str, source_type: SourceType) -> tuple[DataTier, float]:
        """
...
class SourceIngester:
    ...
    def __init__(self, source_type: SourceType, ethical_config: EthicalCrawlConfig, robots_parser: RobotsTxtParser):
...
class GeminiIngestionPipeline:
    ...
    def __init__(self, gemini_api_key: str | None = None, ethical_config: EthicalCrawlConfig | None = None):
        """
        ...
        self.ingesters: dict[SourceType, SourceIngester] = {
            SourceType.YOUTUBE: MockYouTubeIngester(SourceType.YOUTUBE, self.ethical_config, self.robots_parser),
            SourceType.TWITTER: MockTwitterIngester(SourceType.TWITTER, self.ethical_config, self.robots_parser),
            SourceType.NEWS: MockNewsIngester(SourceType.NEWS, self.ethical_config, self.robots_parser),
        }
    ...
    def run_ingestion_cycle(self, source_configs: dict[SourceType, dict[str, Any]]) -> IngestionMetrics:
        """
            ...
            try:
                ...
                for raw_item in raw_items:
                    ...
                    tier, relevance_score = self.tier_classifier.classify_item(
                        title=raw_item["title"], content=raw_item["content"], source_type=source_type
                    )
        ...
        tier_counts = {DataTier.TIER_1_CRITICAL: 0, DataTier.TIER_2_IMPORTANT: 0, DataTier.TIER_3_BACKGROUND: 0}

        for item in all_items:
            tier_counts[item.tier] += 1

        total_cost = sum(item.cost for item in all_items)
        avg_relevance = sum(item.relevance_score for item in all_items) / len(all_items) if all_items else 0.0
    ...
    def generate_am_briefing(self, date: str | None = None) -> str:
        ...
        items = [item for item in self.ingestion_history if item.ingestion_timestamp.startswith(target_date)]

        if not items:
            return f"# AM Briefing - {target_date}\n\nNo intelligence items for this date."

        # Sort by tier (Tier 1 first) and relevance
        tier_order = {DataTier.TIER_1_CRITICAL: 1, DataTier.TIER_2_IMPORTANT: 2, DataTier.TIER_3_BACKGROUND: 3}
    ...
    def export_metrics(self, metrics: IngestionMetrics, output_path: str = "ingestion_metrics.json") -> str:
...
def main():
    ...
    # Configure sources
    source_configs = {
        SourceType.YOUTUBE: {"channels": ["@healthcare_ai", "@fintech_news"], "keywords": ["AI", "healthcare", "fintech"]},
        SourceType.TWITTER: {"accounts": ["@tech_insider", "@health_tech"], "hashtags": ["#HealthTech", "#AIinHealthcare"]},
        SourceType.NEWS: {"feeds": ["https://news.example.com/rss"], "keywords": ["FDA", "AI", "healthcare"]},
    }
