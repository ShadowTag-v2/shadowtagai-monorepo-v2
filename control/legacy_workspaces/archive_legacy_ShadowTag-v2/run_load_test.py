...
class LoadTestRunner:
    ...
    def run_scenario(self, scenario: LoadTestScenario, headless: bool = True) -> bool:
        ...
        csv_prefix = self.results_dir / f"{scenario.name.lower().replace(' ', '_')}_{timestamp}"
    ...
    def run_custom(self, users: int, spawn_rate: int, duration: int, user_class: str = "AIYouUser", headless: bool = True) -> bool:
...
Examples:
  ...
  python run_load_test.py --scenario medium --no-headless
    ...
    parser.add_argument("--host", default="http://localhost:8000", help="Target host URL (default: http://localhost:8000)")

    parser.add_argument(
        "--scenario",
        choices=["smoke", "light", "medium", "heavy", "stress", "spike", "endurance", "breaking_point"],
    ...
    parser.add_argument(
        "--user-class", default="AIYouUser", choices=["AIYouUser", "QuickLoadUser", "StressTestUser"], help="User class to use for testing"
    )

    parser.add_argument("--breaking-point", action="store_true", help="Run breaking point test (incrementally increase load)")

    parser.add_argument("--list-scenarios", action="store_true", help="List all available scenarios")

    parser.add_argument("--no-headless", action="store_true", help="Run with web UI instead of headless mode")
    ...
    if args.users and args.spawn_rate and args.duration:
        success = runner.run_custom(
            users=args.users, spawn_rate=args.spawn_rate, duration=args.duration, user_class=args.user_class, headless=not args.no_headless
        )
        return 0 if success else 1
