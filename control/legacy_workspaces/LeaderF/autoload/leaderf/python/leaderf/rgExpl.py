...
class RgExplorer(Explorer):
    ...
    def getContent(self, *args, **kwargs):
        ...
        if "--context-separator" in arguments_dict:
            ...
            if self._context_separator.startswith('"') and self._context_separator.endswith('"'):
                self._context_separator = self._context_separator[1:-1]
            one_args_options += f'--context-separator="{self._context_separator}" '
        else:
            one_args_options += f"--context-separator={self._context_separator} "
        if "-A" in arguments_dict:
            one_args_options += "-A {} ".format(arguments_dict["-A"][0])
            self._display_multi = True
        if "-B" in arguments_dict:
            one_args_options += "-B {} ".format(arguments_dict["-B"][0])
            self._display_multi = True
        if "-C" in arguments_dict:
            one_args_options += "-C {} ".format(arguments_dict["-C"][0])
            self._display_multi = True
        if "-E" in arguments_dict:
            one_args_options += "-E {} ".format(arguments_dict["-E"][0])
        if "-M" in arguments_dict:
            one_args_options += "-M {} ".format(arguments_dict["-M"][0])
        else:
            for opt in rg_config:
        ...
        if "--max-depth" in arguments_dict:
            one_args_options += "--max-depth {} ".format(arguments_dict["--max-depth"][0])
        if "--max-filesize" in arguments_dict:
            one_args_options += "--max-filesize {} ".format(arguments_dict["--max-filesize"][0])
        if "--path-separator" in arguments_dict:
            one_args_options += "--path-separator {} ".format(arguments_dict["--path-separator"][0])
        if "--sort" in arguments_dict:
            one_args_options += "--sort {} ".format(arguments_dict["--sort"][0])
        ...
        if "-g" in arguments_dict:
            repeatable_options += "-g {} ".format(" -g ".join(arguments_dict["-g"]))
        if "--iglob" in arguments_dict:
            repeatable_options += "--iglob {} ".format(" --iglob ".join(arguments_dict["--iglob"]))
        if "--ignore-file" in arguments_dict:
            repeatable_options += "--ignore-file {} ".format(" --ignore-file ".join(arguments_dict["--ignore-file"]))
        if "--type-add" in arguments_dict:
            repeatable_options += "--type-add {} ".format(" --type-add ".join(arguments_dict["--type-add"]))
        if "-t" in arguments_dict:
            repeatable_options += "-t {} ".format(" -t ".join(arguments_dict["-t"]))
        if "-T" in arguments_dict:
            repeatable_options += "-T {} ".format(" -T ".join(arguments_dict["-T"]))
        ...
        if "--current-buffer" in arguments_dict:
            ...
            if vim.current.buffer.name:
                if not vim.current.buffer.options["modified"] and not vim.current.buffer.options["bt"]:
                    try:
                        path = f'"{os.path.relpath(lfDecode(vim.current.buffer.name))}"'
                    except ValueError:
...
class RgExplManager(Manager):
    ...
    def _acceptSelection(self, *args, **kwargs):
        ...
        try:
            ...
            if self._getExplorer().getPatternRegex():
                lfCmd(f"call search('{escQuote(self._getExplorer().getPatternRegex()[0])}', 'zW', line('.'))")
            lfCmd("norm! zz")
        ...
        if "--live" in arguments_dict:
            for arg in ("--nameOnly", "--fullPath", "--fuzzy", "--regexMode"):
                if arg in arguments_dict:
                    lfPrintError(f"error: argument --live: not allowed with argument {arg}")
                    return
    ...
    def outputToQflist(self, *args, **kwargs):
        ...
        lfCmd(f"call setqflist({json.dumps(items, ensure_ascii=False)}, 'r')")
