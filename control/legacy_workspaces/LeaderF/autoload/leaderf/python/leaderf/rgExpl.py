...
class RgExplorer(Explorer):
  ...
  def getContent(self, *args, **kwargs):
    ...
    for opt in rg_config:
      ...
      if self._display_multi == False and (
        opt.startswith("-A")
        or opt.startswith("-B")
        or opt.startswith("-C")
    ...
    if "--ignore-file" in arguments_dict:
      repeatable_options += "--ignore-file %s " % " --ignore-file ".join(
        arguments_dict["--ignore-file"]
      )
    if "--type-add" in arguments_dict:
      repeatable_options += "--type-add %s " % " --type-add ".join(
        arguments_dict["--type-add"]
      )
    ...
    if "--current-buffer" in arguments_dict:
      ...
      if vim.current.buffer.name:
        if (
          ...
          with tmp_file(
            mode="w", suffix="@LeaderF@" + str(vim.current.buffer.number), delete=False
          ) as f:
        ...
        if lfEval("buflisted(%d)" % b.number) == "1":
          if b.name:
            if b.options["modified"] == False:
              ...
              with tmp_file(
                mode="w", suffix="@LeaderF@" + str(b.number), delete=False
              ) as f:
                file_name = lfDecode(f.name)
                for line in b:
                  f.write(line + "\n")
...
class RgExplManager(Manager):
  ...
  def _acceptSelection(self, *args, **kwargs):
    ...
    try:
      ...
      if self._getExplorer().getPatternRegex():
        lfCmd(
          "call search('%s', 'zW', line('.'))"
          % escQuote(self._getExplorer().getPatternRegex()[0])
        )
      lfCmd("norm! zz")
  ...
  def setArguments(self, arguments):
    ...
    self._has_column = (
      "--column" in lfEval("get(g:, 'Lf_RgConfig', [])")
      or "--column" in self._arguments
    )
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      if "--heading" in self._arguments:
        if (
          "-A" in self._arguments or "-B" in self._arguments or "-C" in self._arguments
        ):
          lfCmd(
      ...
      if "--heading" in self._arguments:
        ...
        if (
          ...
          id = int(
            lfEval(r"matchadd('Lf_hl_rgFileName', '^.\{-}\ze\(:\d\+:\|-\d\+-\)', 10)")
          )
        else:
        ...
        if self._has_column:
          id = int(
            lfEval(r"matchadd('Lf_hl_rgColumnNumber', '^.\{-}:\d\+:\zs\d\+:', 10)")
          )
          self._match_ids.append(id)
    ...
    if "--recall" not in self._arguments:
      ...
      if self._read_finished < 2:
        self._timer_id = lfEval(
          "timer_start(1, 'leaderf#Rg#TimerCallback', {'repeat': -1})"
        )
    else:
  ...
  def startExplorer(self, win_pos, *args, **kwargs):
    ...
    if (
      "-A" in arguments_dict
      or "-B" in arguments_dict
      or "-C" in arguments_dict
    ...
    if fall_back:
      if "f" in mode:
        ...
        if cur_buf_name and not os.path.dirname(cur_buf_name).startswith(
          self._orig_cwd
        ):
          chdir(os.path.dirname(cur_buf_name))

    if "--live" in arguments_dict or (
      "--recall" in arguments_dict and "--live" in self._arguments
    ):
  ...
  def _createPopupPreview(self, title, source, line_num, jump_cmd=""):
    ...
    if (
      super()._createPopupPreview(title, source, line_num, jump_cmd)
  ...
  def outputToQflist(self, *args, **kwargs):
    ...
    lfCmd(
      "echohl WarningMsg | redraw | echo ' Output result to quickfix list.' | echohl NONE"
    )
  ...
  def outputToLoclist(self, *args, **kwargs):
    ...
    lfCmd(
      "call setloclist(%d, %s, 'r')"
      % (int(winnr), json.dumps(items, ensure_ascii=False))
    )
    lfCmd(
      "echohl WarningMsg | redraw | echo ' Output result to location list.' | echohl NONE"
    )
  ...
  def replace(self):
    ...
    try:
      ...
      text = (
        ""
        if len(self._getExplorer().getPatternRegex()) == 0
      ...
      if "--heading" in self._arguments:
        lfCmd(
          r"""%d;$s/\%%(^\d\+[:-].\{-}\)\@<=%s/%s/%s"""
          % (
            self._getInstance().helpLength + 1,
            pattern.replace("/", r"\/"),
            string.replace("/", r"\/"),
            flags,
          )
  ...
  def applyChanges(self):
    ...
    try:
      ...
      for n, line in enumerate(
        self._getInstance().buffer[self._getInstance().helpLength :]
      ):
        try:
          if line == self._getExplorer().getContextSeparator():
            continue
          ...
          if "--heading" in self._arguments:
            if (
              "-A" in self._arguments
              or "-B" in self._arguments
              or "-C" in self._arguments
            ):
              if not re.match(r"^\d+[:-]", line):
                file = line
                continue
      ...
      lfCmd(
        f"call leaderf#colorscheme#highlightBlank('{self._getExplorer().getStlCategory()}', {self._getInstance().buffer.number})"
      )
  ...
  def confirm(self):
    if self._getInstance().buffer.options["modified"]:
      selection = int(
        lfEval(
          """confirm("buffer changed, apply changes or discard?", "&apply\n&discard")"""
        )
      )
      if selection == 0:
        return
  ...
  def _writeBuffer(self):
    ...
    if self._read_finished > 0:
      ...
      if self._pattern_changed or len(self._getInstance().buffer) < min(
        cur_len, self._initial_count
      ):
        self._pattern_changed = False
        self._getInstance().setBuffer(self._content[: self._initial_count])
  ...
  def _search(self, content, is_continue=False, step=0):
    if "--live" not in self._arguments:
      super()._search(content, is_continue, step)
      return
    ...
    kill_thread = threading.Thread(
      target=self._killThread, args=(self._getExplorer()._executor,)
    )
    ...
    content = self._getExplorer().getContent(
      arguments=self._arguments, pattern=self._cli.pattern
    )
    self._reader_thread = threading.Thread(target=self._readContent, args=(content,))
    self._reader_thread.daemon = True
    self._reader_thread.start()
