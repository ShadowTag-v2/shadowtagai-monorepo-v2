...
class JumpsExplManager(Manager):
  ...
  def _acceptSelection(self, *args, **kwargs):
    ...
    if (
      ...
      or "--stayOpen" in self._arguments
      ...
      # it's text
      if int(line) <= len(orig_buf) and orig_buf[int(line) - 1].lstrip().startswith(
        file_text
      ):
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_jumpsTitle'', ''^ \D\+'')')"""
        % self._getInstance().getPopupWinId()
      )
      id = int(lfEval("matchid"))
      self._match_ids.append(id)
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_jumpsNumber'', ''^>\?\s*\zs\d\+'')')"""
        % self._getInstance().getPopupWinId()
      )
      id = int(lfEval("matchid"))
      self._match_ids.append(id)
      ...
      self._match_ids.append(id)
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_jumpsIndicator'', ''^>'')')"""
        % self._getInstance().getPopupWinId()
      )
      id = int(lfEval("matchid"))
      self._match_ids.append(id)
      ...
      self._match_ids.append(id)
      id = int(
        lfEval(r"""matchadd('Lf_hl_jumpsLineCol', '^>\?\s*\d\+\s*\zs\d\+\s*\d\+')""")
      )
      self._match_ids.append(id)
