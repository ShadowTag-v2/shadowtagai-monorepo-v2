...
class LfCli:
  ...
  def setArguments(self, arguments):
    ...
    if "--quick-select" in self._arguments:
      self._quick_select = not self._instance.isReverseOrder() and bool(
        quick_select_value
      )
    else:
      self._quick_select = not self._instance.isReverseOrder() and bool(
        int(lfEval("get(g:, 'Lf_QuickSelect', 0)"))
      )
  ...
  def clear(self):
    ...
    if self._instance and self._instance.getWinPos() == "popup":
      lfCmd(
        """call win_execute(%d, 'silent! syn clear Lf_hl_match')"""
        % self._instance.getPopupWinId()
      )
      lfCmd(
        """call win_execute(%d, 'silent! syn clear Lf_hl_match_refine')"""
        % self._instance.getPopupWinId()
      )
    else:
  ...
  def _buildPopupPrompt(self):
    ...
    if (
      lfEval("get(g:, 'Lf_PopupShowBorder', 1)") == "1" and lfEval("has('nvim')") == "0"
    ):
      input_win_width -= 2
    ...
    result_count = lfEval(f"g:Lf_{self._instance._category}_StlResultsCount")
    total = lfEval(f"g:Lf_{self._instance._category}_StlTotal")
    ...
    input_win_width += 2 * (
      len(sep) - int(lfEval("strdisplaywidth('%s')" % escQuote(sep)))
    )
    input_win_width += len(pattern) - int(
      lfEval("strdisplaywidth('%s')" % escQuote(pattern))
    )
    ...
    if self._instance.getWinPos() == "popup":
      ...
      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_prompt'})")"""
        % input_window.id
      )
      lfCmd(
      ...
      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_cursor'})")"""
        % input_window.id
      )
      lfCmd(
        ...
        % (
          input_window.id,
          lfBytesLen(prompt)
          + lfBytesLen("".join(self._cmdline[: self._cursor_pos]))
          + 1,
        )
      )

      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_total'})")"""
        % (input_window.id)
      )
      lfCmd(
      ...
      if sep != "":
        lfCmd(
          """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_%s_sep5'})")"""
      ...
      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_lineInfo'})")"""
        % (input_window.id)
      )
      lfCmd(
      ...
      if sep != "":
        lfCmd(
          """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_%s_sep4'})")"""
          % (input_window.id, self._instance._category)
        )
        lfCmd(
      ...
      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_spin'})")"""
        % (input_window.id)
      )
      if spin != "":
        lfCmd(
      ...
      cursor_pos = lfBytesLen(prompt) + lfBytesLen(
        "".join(self._cmdline[: self._cursor_pos])
      )
      if self._cursor_pos == len(self._cmdline):
  ...
  def _buildPrompt(self):
    if self._idle and datetime.now() - self._start_time < timedelta(
      milliseconds=500
    ):  # 500ms
      return
    else:
    ...
    if self._is_fuzzy:
      if self._is_full_path:
        lfCmd(
          f"echohl Constant | echon '>F> {self._additional_prompt_string}' | echohl NONE"
        )
      else:
        lfCmd(
          f"echohl Constant | echon '>>> {self._additional_prompt_string}' | echohl NONE"
        )
    elif self._is_live:
      lfCmd(
        f"echohl Constant | echon '>>> {self._additional_prompt_string}' | echohl NONE"
      ...
      lfCmd(
        f"echohl Constant | echon 'R>> {self._additional_prompt_string}' | echohl NONE"
      )

    lfCmd(
      "echohl Normal | echon '%s' | echohl NONE"
      % escQuote("".join(self._cmdline[: self._cursor_pos]))
    )
    if self._cursor_pos < len(self._cmdline):
      lfCmd(
        "echohl Lf_hl_cursor | echon '%s' | echohl NONE"
        % escQuote("".join(self._cmdline[self._cursor_pos]))
      )
      lfCmd(
        "echohl Normal | echon '%s' | echohl NONE"
  ...
  def _buildPattern(self):
    ...
    if self._is_fuzzy:
      if (
        self._and_delimiter in "".join(self._cmdline).lstrip(self._and_delimiter)
        and self._delimiter not in self._cmdline
      ):
        self._is_and_mode = True
        if case_insensitive:
          patterns = re.split(
            r"[" + self._and_delimiter + "]+",
        ...
        if (
          (self._supports_nameonly and not self._is_full_path) or self._supports_refine
        ) and self._delimiter in self._cmdline:
          self._refine = True
          idx = self._cmdline.index(self._delimiter)
          if case_insensitive:
            self._pattern = (
              "".join(self._cmdline[:idx]).lower(),
              "".join(self._cmdline[idx + 1 :]).lower(),
            )
          else:
            self._pattern = (
              "".join(self._cmdline[:idx]),
  ...
  def highlightMatches(self):
    ...
    if self._is_fuzzy:
      ...
      if (
        lfEval("exists('*matchaddpos')") == "1"
        and lfEval("g:Lf_HighlightIndividual") == "1"
      ):
        return
      cmdline = [
        r"\/" if c == "/" else r"\\" if c == "\\" else c for c in self._cmdline
      ]  # \/ for syn match
      if self._is_full_path:
        regex = r"\c\V" + self._join(cmdline)
        lfCmd(
          "syn match Lf_hl_match display /%s/ containedin=Lf_hl_nonHelp, Lf_hl_dirname, Lf_hl_filename contained"
          % regex
        )
      else:
        if self._refine:
          ...
          if regex[0] == r"\c\V" and regex[1] == r"\c\V":
            ...
            lfCmd(
              "syn match Lf_hl_match display /%s/ containedin=Lf_hl_dirname, Lf_hl_filename contained"
              % regex[1]
            )
          elif regex[1] == r"\c\V":
            lfCmd(
              "syn match Lf_hl_match display /%s/ containedin=Lf_hl_filename contained"
              % regex[0]
            )
          else:
            lfCmd(
      ...
      if self._pattern:
        ...
        regex = (
          (self._pattern + "\\")
          if len(re.search(r"\\*$", self._pattern).group(0)) % 2 == 1
          else self._pattern
        )
        ...
        try:
          if self._instance.getWinPos() == "popup":
            ...
            lfCmd(
              "syn match Lf_hl_match '%s' containedin=Lf_hl_dirname, Lf_hl_filename contained"
              % regex
            )
        except vim.error:
          pass
  ...
  def writeHistory(self, category):
    ...
    history_dir = os.path.join(
      lfEval("g:Lf_CacheDirectory"), "LeaderF", "history", category
    )
    if self._is_fuzzy:
  ...
  def isPrefix(self):  # assume that there are no \%23l, \%23c, \%23v, \%...
    ...
    if pos > 1:
      if regex[pos - 2] != "\\" and (
        regex[pos - 1].isalnum() or regex[pos - 1] in r"`$%*(-_+[\;:,. /?"
      ):
        if regex[pos - 2] == "_":
    ...
    try:
      ...
      while 1:
        ...
        if lfEval("!type(nr) && nr >= 0x20") == "1":
          ...
          self._insert(char)
          self._buildPattern()
          if self._pattern is None or (
            self._refine and self._pattern[1] == ""
          ):  # e.g. abc;
            continue
