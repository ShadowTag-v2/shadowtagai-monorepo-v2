  ...
  def getContent(self, *args, **kwargs):
    ...
    with lfOpen(mru.getCacheFileName(), "r+", errors="ignore", encoding="utf8") as f:
      ...
      if len(data_list) == 0:
        ...
        try:
          with lfOpen(
            mru.getOldCacheFileName(), "r+", errors="ignore", encoding="utf8"
          ) as old_f:
            current_time = time.time()
            data_list = [
              [int(current_time), 1, filename]
              for filename in old_f.readlines()
      ...
      if (
        "--frecency" in arguments_dict
        or lfEval("get(g:, 'Lf_MruEnableFrecency', 0)") == "1"
      ):
      ...
      f.seek(0)
      f.truncate(0)
      f.writelines([f"{data[0]} {data[1]} {data[2]}" for data in data_list])
    ...
    if "--cwd" in arguments_dict:
      ...
      lines = [
        name
        for name in lines
        if lfDecode(name).startswith(os.path.join(project_root, ""))
      ]
    ...
    lines = [
      ...
      and True
      not in (fnmatch(name, "*/" + j + "/*") for j in wildignore.get("dir", []))
    ]
    ...
    if "--no-split-path" in arguments_dict:
      ...
      if lfEval("get(g:, 'Lf_ShowDevIcons', 1)") == "1":
        lines = [
          webDevIconsGetFileTypeSymbol(getBasename(line)) + line for line in lines
        ]
      return lines
    ...
    for i, line in enumerate(lines):
      ...
      lines[i] = '{}{}{} "{}"'.format(
        icon, getBasename(line), " " * space_num, dirname if dirname else "." + os.sep
      )
    return lines
