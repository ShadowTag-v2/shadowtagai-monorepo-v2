...
class BufTagExplorer(Explorer):
  ...
  def _getTagResult(self, buffer):
    if (
      not buffer.name
      or lfEval("bufloaded(%d)" % buffer.number) == "0"
      or lfEval("&bt") != ""
    ):
      return []
    ...
    if buffer.options["modified"] == True:
      ...
      with tmp_file(
        mode="w+", suffix="_" + os.path.basename(buffer.name), delete=False
      ) as f:
        for line in buffer:
          f.write(line + "\n")
        file_name = f.name
      ...
      cmd = (
        f'{self._ctags} -n -u --fields=Ks {extra_options} -f- "{lfDecode(file_name)}"'
      )
      result = executor.execute(cmd, cleanup=partial(os.remove, file_name))
    else:
      cmd = (
        f'{self._ctags} -n -u --fields=Ks {extra_options} -f- "{lfDecode(buffer.name)}"'
      )
  ...
  def _formatResult(self, buffer, result):
    ...
    for _, item in enumerate(output):
      ...
      if self._supports_preview:
        ...
        #                                  taglen=tag_len
        #                                  )
        code = f"\t\t{buffer[int(item[2][:-2]) - 1].lstrip()}"
...
class BufTagExplManager(Manager):
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      ...
      self._match_ids.append(id)
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_buftagCode'', ''^\s\+.*'')')"""
        % self._getInstance().getPopupWinId()
      )
      id = int(lfEval("matchid"))
      self._match_ids.append(id)
      ...
      self._match_ids.append(id)
      id = int(
        lfEval(r"""matchadd('Lf_hl_buftagScopeType', '[^\t]*\t\S\+\s*\zs\w\+:')""")
      )
      self._match_ids.append(id)
      id = int(
        lfEval(
          r"""matchadd('Lf_hl_buftagScope', '^[^\t]*\t\S\+\s*\(\w\+:\)\=\zs\S\+')"""
        )
      )
      self._match_ids.append(id)
      id = int(
        lfEval(
          r"""matchadd('Lf_hl_buftagDirname', '[^\t]*\t\S\+\s*\S\+\s*\zs[^\t]\+')"""
        )
      )
      self._match_ids.append(id)
  ...
  def _fuzzyFilter(self, is_full_path, get_weight, iterable):
    ...
    if self._supports_preview:
      ...
      pairs = (
        (get_weight(getDigest(line)), (line, iterable[2 * i + 1]))
  ...
  def _refineFilter(self, first_get_weight, get_weight, iterable):
    if self._supports_preview:
      ...
      tuples = (
        (
          first_get_weight(getDigest(line, 1)),
          get_weight(getDigest(line, 2)),
          line,
          iterable[2 * i + 1],
        )
        for i, line in enumerate(iterable[::2])
      ...
      return (
        (i[0] + i[1], (i[2], i[3]))
        for i in tuples
        if i[0] > MIN_WEIGHT and i[1] > MIN_WEIGHT
      )
    else:
  ...
  def _regexFilter(self, iterable):
    if self._supports_preview:
      ...
      return super()._regexFilter(iterable)
  ...
  def _getList(self, pairs):
    ...
    if self._supports_preview:
      ...
      return super()._getList(pairs)

  def _toUp(self):
    if self._supports_preview:
      if (
        self._getInstance().isReverseOrder()
        and self._getInstance().getCurrentPos()[0] <= 3
      ):
      ...
      if self._getInstance().isReverseOrder():
        ...
        if self._getInstance().getWinPos() == "popup":
          lfCmd(
            "call win_execute(%d, 'norm! 2k')" % (self._getInstance().getPopupWinId())
          )
        else:
          lfCmd("norm! 2k")
    else:
  ...
  def _toDown(self):
    if self._supports_preview:
      if self._getInstance().isReverseOrder():
        ...
        if self._getInstance().getWinPos() == "popup":
          lfCmd(
            "call win_execute(%d, 'norm! 3jk')" % (self._getInstance().getPopupWinId())
          )
        else:
          lfCmd("norm! 3jk")
    else:
    ...
    if "--all" in self._arguments and not self._is_content_list:
      ...
      if self._read_finished < 2:
        self._timer_id = lfEval(
          "timer_start(1, 'leaderf#BufTag#TimerCallback', {'repeat': -1})"
        )
    else:
      self._relocateCursor()

  def _bangReadFinished(self):
    super()._bangReadFinished()
    self._relocateCursor()
  ...
  def _relocateCursor(self):
    ...
    if last >= 0:
      ...
      if self._getInstance().getWinPos() == "popup":
        ...
        lfCmd(
          """call win_execute(%d, "exec 'norm! %dG'")"""
          % (self._getInstance().getPopupWinId(), int(index))
        )
