...
class CocExplManager(Manager):
  ...
  def _defineMaps(self):
    lfCmd(f"call leaderf#Coc#Maps({id(self)})")
...
class CommonExplManager(CocExplManager):
  ...
  def _createPreviewWindow(self, config, source, line_num, jump_cmd):
    return super(CocExplManager, self)._createPreviewWindow(
      config, source, line_num, jump_cmd
    )

  def _useExistingWindow(self, title, source, line_num, jump_cmd):
    return super(CocExplManager, self)._useExistingWindow(
      title, source, line_num, jump_cmd
    )
