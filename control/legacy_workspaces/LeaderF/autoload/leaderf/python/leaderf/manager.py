...
class Manager:
  ...
  def _defineNormalCommandMaps(self):
    ...
    new_commands = set(
      i.lower() if i.startswith("<") else i for i in command_map.values()
    )
    map_rhs = {k: lfEval(f"maparg('{k}', 'n', 0, 1)") for k in command_map}
    for old, new in command_map.items():
      ...
      if maparg and maparg["buffer"] == "1":
        ...
        if old_cmd not in new_commands:
          lfCmd(f"silent! nunmap <buffer> {old}")
  ...
  def _defineCommonMaps(self):
    ...
    for [lhs, rhs] in normal_map["_"]:
      ...
      maparg = lfEval(f"maparg('{lhs}', 'n', 0, 1)")
      if maparg == {} or maparg.get("buffer", "0") == "0":
        lfCmd(f"nnoremap <buffer> <silent> {lhs} {rhs}")
  ...
  def _afterEnter(self):
    ...
    if "--nowrap" in self._arguments:
      if self._getInstance().getWinPos() == "popup":
        lfCmd(
          "call win_execute(%d, 'setlocal nowrap')"
          % self._getInstance().getPopupWinId()
        )
      elif self._getInstance().getWinPos() == "floatwin":
        lfCmd(
          "call nvim_win_set_option(%d, 'wrap', v:false)"
          % self._getInstance().getPopupWinId()
        )
      else:
      ...
      if self._getInstance().getWinPos() == "popup":
        lfCmd(
          "call win_execute(%d, 'setlocal wrap')" % self._getInstance().getPopupWinId()
        )
      elif self._getInstance().getWinPos() == "floatwin":
        lfCmd(
          "call nvim_win_set_option(%d, 'wrap', v:true)"
          % self._getInstance().getPopupWinId()
        )
      else:
    ...
    if self._getInstance().getWinPos() != "popup":
      ...
      lfCmd(
        rf"""call win_execute({self._getInstance().getPopupWinId()}, 'let matchid = matchadd(''Lf_hl_cursorline'', ''.*\%#.*'', -100)')"""
      )
  ...
  def _beforeExit(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      for i in self._match_ids:
        lfCmd(
          "silent! call matchdelete(%d, %d)" % (i, self._getInstance().getPopupWinId())
        )
    else:
      for i in self._match_ids:
  ...
  def _bangEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      ...
      if (
        lfEval(
          "exists('*leaderf#%s#NormalModeFilter')"
  ...
  def _closePreviewPopup(self):
    if lfEval("has('nvim')") == "1":
      ...
      if self._preview_winid:
        ...
        self._preview_winid = 0
        lfCmd(f"silent! noautocmd bwipe /Lf_preview_{id(self)}")

    self._preview_filetype = None

  def _previewResult(self, preview):
    preview_in_popup = lfEval(
      "get(g:, 'Lf_PreviewInPopup', 1)"
    ) == "1" or self._getInstance().getWinPos() in ("popup", "floatwin")
  ...
  def _setWinOptions(self, winid):
    if lfEval("has('nvim')") == "1":
      ...
      lfCmd(
        "call nvim_win_set_option(%d, 'winhighlight', 'Normal:Lf_hl_popup_window')"
        % winid
      )
    else:
      lfCmd(
        "noautocmd call win_execute(%d, 'setlocal number norelativenumber cursorline')"
        % winid
      )
      lfCmd(
        "noautocmd call win_execute(%d, 'setlocal nofoldenable foldmethod=manual')"
        % winid
      )
      if (
        lfEval("get(g:, 'Lf_PopupShowFoldcolumn', 1)") == "0"
        or lfEval("get(g:, 'Lf_PopupShowBorder', 1)") == "1"
      ):
    ...
    if lfEval("has('nvim')") == "1":
      if isinstance(source, int):
        ...
        self._preview_winid = int(
          lfEval("nvim_open_win(%d, 0, %s)" % (source, str(config)))
        )
      else:
        try:
        ...
        lfCmd(
          "noautocmd call nvim_buf_set_option(g:Lf_preview_scratch_buffer, 'bufhidden', 'wipe')"
        )
        lfCmd(
          "noautocmd call nvim_buf_set_option(g:Lf_preview_scratch_buffer, 'undolevels', -1)"
        )
        lfCmd(
          "noautocmd call nvim_buf_set_option(g:Lf_preview_scratch_buffer, 'modeline', v:true)"
        )

        self._preview_winid = int(
          lfEval("nvim_open_win(g:Lf_preview_scratch_buffer, 0, %s)" % str(config))
        )
      ...
      self._preview_filetype = lfEval(
        "getbufvar(winbufnr(%d), '&ft')" % self._preview_winid
      )
      ...
      if buffer_len >= line_num > 0:
        lfCmd(
          """call nvim_win_set_cursor(%d, [%d, 1])""" % (self._preview_winid, line_num)
        )
      lfCmd("norm! zz")
      lfCmd("noautocmd call win_gotoid(%s)" % cur_winid)
      ...
      if isinstance(source, int):
        lfCmd(
          "noautocmd silent! let winid = popup_create(%d, %s)"
        ...
        lfCmd(
          "noautocmd silent! let winid = popup_create(bufadd('/Lf_preview_%s'), %s)"
        ...
        lfCmd(
          "call win_execute(winid, 'silent! doautocmd filetypedetect BufNewFile %s')"
          % escQuote(filename)
        )
      ...
      if jump_cmd:
        lfCmd(
          """call win_execute(%d, '%s')""" % (self._preview_winid, escQuote(jump_cmd))
        )
        lfCmd("call win_execute(%d, 'norm! zz')" % self._preview_winid)
      elif line_num > 0:
        lfCmd(
          """call win_execute(%d, "call cursor(%d, 1)")"""
          % (self._preview_winid, line_num)
        )
        lfCmd("call win_execute(%d, 'norm! zz')" % self._preview_winid)
    else:
      if isinstance(source, int):
      ...
      lfCmd(
        "noautocmd silent! let winid = popup_create(content, %s)" % json.dumps(config)
      )
      lfCmd("call win_execute(winid, 'setlocal modeline')")
  ...
  def _createPopupModePreview(self, title, source, line_num, jump_cmd):
    ...
    if lfEval("has('nvim')") == "1":
      ...
      float_win_row = int(
        float(lfEval("nvim_win_get_config(%d).row" % float_window.id))
      )
      float_win_col = int(
        float(lfEval("nvim_win_get_config(%d).col" % float_window.id))
      )
      float_win_height = int(
        float(lfEval("nvim_win_get_config(%d).height" % float_window.id))
      )
      float_win_width = int(
        float(lfEval("nvim_win_get_config(%d).width" % float_window.id))
      )
      ...
      if preview_pos.lower() == "bottom":
        ...
        delta_height = lfActualLineCount(
          self._getInstance().buffer, start, end, col_width
        )
      ...
      if preview_pos.lower() == "bottom":
        ...
        lfCmd(
          """call win_execute(%d, "let numberwidth = &numberwidth")""" % popup_window.id
        )
        col = (
          int(popup_pos["core_col"])
          + int(lfEval("numberwidth"))
          + popup_window.cursor[1]
        )

        lfCmd(
          """call win_execute(%d, "let delta_height = line('.') - line('w0')")"""
          % popup_window.id
        )
      ...
      if preview_pos.lower() == "bottom":
        ...
        preview_pos.lower() == "cursor" and maxheight < int(lfEval("&lines")) // 2 - 2
      ):
  ...
  def isPreviewWindowOpen(self):
    return (
      self._preview_winid > 0
      and int(lfEval("winbufnr(%d)" % self._preview_winid)) != -1
    )
  ...
  def setOptionsForCursor(self):
    ...
    if preview_pos == "cursor" and self._getInstance().getWinPos() not in (
      "popup",
      "floatwin",
    ):
      show_borders = lfEval("get(g:, 'Lf_PopupShowBorder', 1)") == "1"
      ...
      if lfEval("has('nvim')") == "1":
        ...
        lfCmd(
          "call nvim_win_set_config(%d, %s)"
          % (self._preview_winid, str(self._preview_config))
        )
      else:
        lfCmd(
          "call popup_setoptions(%d, %s)"
          % (self._preview_winid, str(self._preview_config))
        )
  ...
  def _useExistingWindow(self, title, source, line_num, jump_cmd):
    ...
    if self._orig_source != source:
      ...
      if lfEval("has('nvim')") == "1":
        if isinstance(source, int):
          lfCmd(
            "silent noautocmd call nvim_win_set_buf(%d, %d)"
            % (self._preview_winid, source)
          )
          self._setWinOptions(self._preview_winid)
        else:
          try:
          ...
          if (
            lfEval(
              "!exists('g:Lf_preview_scratch_buffer') || !bufexists(g:Lf_preview_scratch_buffer)"
            )
            == "1"
          ):
            lfCmd("noautocmd let g:Lf_preview_scratch_buffer = nvim_create_buf(0, 1)")
          ...
          lfCmd(
            "noautocmd call nvim_buf_set_lines(g:Lf_preview_scratch_buffer, 0, -1, v:false, content)"
          )
          lfCmd(
            "silent noautocmd call nvim_win_set_buf(%d, g:Lf_preview_scratch_buffer)"
            % self._preview_winid
          )
          preview_filetype = lfEval(
            "getbufvar(winbufnr(%d), '&ft')" % self._preview_winid
          )
          ...
          if cur_filetype != preview_filetype:
            if cur_filetype is None:
              lfCmd(
                "call win_execute(%d, 'silent! doautocmd filetypedetect BufNewFile %s')"
                % (self._preview_winid, escQuote(source))
              )
            else:
              lfCmd(
                "call win_execute(%d, 'noautocmd set ft=%s')"
                % (self._preview_winid, cur_filetype)
              )
              lfCmd(
                "call win_execute(%d, 'set syntax=%s')"
                % (self._preview_winid, cur_filetype)
              )
      elif lfEval("exists('*popup_setbuf')") == "1":
        if isinstance(source, int):
          ...
          try:
            ...
            return
          lfCmd(
            "silent call popup_setbuf(%d, bufadd('/Lf_preview_%d'))"
        ...
        if isinstance(source, int):
          lfCmd(
            "noautocmd call popup_settext(%d, getbufline(%d, 1, 20480))"
            % (self._preview_winid, source)
          )
          filename = vim.buffers[source].name
        else:
  ...
  def _createPopupPreview(self, title, source, line_num, jump_cmd=""):
    ...
    if lfEval("has('nvim')") == "1":
      ...
      if win_pos == "bottom":
        ...
        line = self._getInstance().window.row + 1
        col = (
          self._getInstance().window.col
          + self._getInstance().window.width
          - maxwidth
          + 1
        )
  ...
  def _updateOptions(self, preview_pos, show_borders, options):
    if lfEval("has('nvim')") == "1":
      if preview_pos.lower() == "cursor":
        ...
        row = int(
          lfEval("screenpos(%d, line('.'), 1)" % self._getInstance().windowId)["row"]
        )
    ...
    if self._getInstance().isReverseOrder():
      ...
      if buffer_len < self._initial_count:
        if "--nowrap" not in self._arguments:
          self._getInstance().window.height = min(
            self._initial_count,
  ...
  def _hideHelp(self):
    ...
    if self._getInstance().isReverseOrder():
      ...
      if 0 < countdown < self._initial_count:
        self._getInstance().window.cursor = (
          len(self._getInstance().buffer) - countdown,
          0,
        )
      else:
      ...
      if self._help_length > 0 and self._getInstance().getWinPos() == "popup":
        lfCmd(
          "call win_execute(%d, 'norm! %dk')"
          % (self._getInstance().getPopupWinId(), self._help_length)
        )
  ...
  def _inHelpLines(self):
    if self._getInstance().isReverseOrder():
      if (
        self._getInstance().window.cursor[0]
        > len(self._getInstance().buffer) - self._help_length
      ):
        return True
  ...
  def _toUpInPopup(self):
    if self.isPreviewWindowOpen():
      ...
      if lfEval("has('nvim')") == "1":
        ...
        lfCmd(
          "call win_execute(%d, 'norm! %dk')" % (self._preview_winid, scroll_step_size)
        )
  ...
  def _toDownInPopup(self):
    if self.isPreviewWindowOpen():
      ...
      if lfEval("has('nvim')") == "1":
        ...
        lfCmd(
          "call win_execute(%d, 'norm! %dj')" % (self._preview_winid, scroll_step_size)
        )
  ...
  def move(self, direction):
    ...
    if (
      direction == "j"
      and self._getInstance().window.cursor[0] == len(self._getInstance().buffer)
      and self._circular_scroll
    ):
      lfCmd(
        "noautocmd call win_execute(%d, 'norm! gg')"
        % (self._getInstance().getPopupWinId())
      )
    elif (
      direction == "k"
      and self._getInstance().window.cursor[0] == 1
      and self._circular_scroll
    ):
      lfCmd(
        "noautocmd call win_execute(%d, 'norm! G')"
        % (self._getInstance().getPopupWinId())
      )
    else:
      lfCmd(
        "noautocmd call win_execute(%d, 'norm! %s')"
        % (self._getInstance().getPopupWinId(), direction)
      )
  ...
  def moveAndPreview(self, direction):
    ...
    if (
      ...
      if len(direction) > 1:
        lfCmd(rf'noautocmd exec "norm! {count}\<{direction}>"')
      else:
        lfCmd(f'noautocmd exec "norm! {count}{direction}"')
  ...
  def _toUp(self):
    if self._getInstance().getWinPos() == "popup":
      if self._getInstance().window.cursor[0] == 1 and self._circular_scroll:
        ...
        lfCmd(
          "noautocmd call win_execute(%d, 'norm! k')"
          % (self._getInstance().getPopupWinId())
        )
      self._getInstance().refreshPopupStatusline()
      return
    ...
    adjust = False
    if (
      self._getInstance().isReverseOrder()
      and self._getInstance().getCurrentPos()[0] == 1
    ):
      adjust = True
      self._setResultContent()
      if (
        self._cli.pattern
        and self._cli.isFuzzy
        and len(self._highlight_pos)
        < (len(self._getInstance().buffer) - self._help_length) // self._getUnit()
  ...
  def _toDown(self):
    if self._getInstance().getWinPos() == "popup":
      if (
        self._getInstance().window.cursor[0] == len(self._getInstance().buffer)
        and self._circular_scroll
      ):
        lfCmd(
          "noautocmd call win_execute(%d, 'norm! gg')"
        ...
        lfCmd(
          "noautocmd call win_execute(%d, 'norm! j')"
          % (self._getInstance().getPopupWinId())
        )
      self._getInstance().refreshPopupStatusline()
      return
    ...
    if (
      not self._getInstance().isReverseOrder()
  ...
  def _pageUp(self):
    if self._getInstance().getWinPos() == "popup":
      lfCmd(
        r"""call win_execute(%d, 'exec "norm! \<PageUp>"')"""
        % (self._getInstance().getPopupWinId())
      )
      self._getInstance().refreshPopupStatusline()
      return
  ...
  def _pageDown(self):
    if self._getInstance().getWinPos() == "popup":
      lfCmd(
        r"""call win_execute(%d, 'exec "norm! \<PageDown>"')"""
        % (self._getInstance().getPopupWinId())
      )
      self._getInstance().refreshPopupStatusline()
      return
    ...
    if self._cli.isAndMode:
      ...
      if is_continue:
        self._previous_result = (
          self._previous_result[0] + result[0],
  ...
  def _refineFilter(self, first_get_weight, get_weight, iterable):
    ...
    triples = (
      (first_get_weight(getDigest(line, 1)), get_weight(getDigest(line, 2)), line)
      for line in iterable
    )
    MIN_WEIGHT = fuzzyMatchC.MIN_WEIGHT if is_fuzzyMatch_C else FuzzyMatch.MIN_WEIGHT
    return (
      (i[0] + i[1], i[2]) for i in triples if i[0] > MIN_WEIGHT and i[1] > MIN_WEIGHT
    )
  ...
  def _andModeFilter(self, iterable):
    ...
    for p in self._cli.pattern:
      use_fuzzy_engine = False
      if (
        self._fuzzy_engine and isAscii(p) and self._getUnit() == 1
      ):  # currently, only BufTag's _getUnit() is 2
        ...
        if self._getExplorer().getStlCategory() == "File" and self._cli.isFullPath:
          filter_method = partial(
            fuzzyEngine.fuzzyMatchEx,
            engine=self._fuzzy_engine,
            pattern=pattern,
            is_name_only=False,
            sort_results=False,
            is_and_mode=True,
          ...
          filter_method = partial(
            ...
            engine=self._fuzzy_engine,
            pattern=pattern,
            is_name_only=not self._cli.isFullPath,
            sort_results=False,
            is_and_mode=True,
          )

        getHighlights = partial(
          fuzzyEngine.getHighlights,
          engine=self._fuzzy_engine,
          pattern=pattern,
          is_name_only=not self._cli.isFullPath,
        )
        highlight_method = partial(
          self._highlight, self._cli.isFullPath, getHighlights, True, clear=False
        )
      elif is_fuzzyMatch_C and isAscii(p):
        pattern = fuzzyMatchC.initPattern(p)
        if self._getExplorer().getStlCategory() == "File" and self._cli.isFullPath:
          getWeight = partial(
            fuzzyMatchC.getWeight, pattern=pattern, is_name_only=False
          )
          getHighlights = partial(
            fuzzyMatchC.getHighlights, pattern=pattern, is_name_only=False
          )
        else:
          getWeight = partial(fuzzyMatchC.getWeight, pattern=pattern, is_name_only=True)
          getHighlights = partial(
            fuzzyMatchC.getHighlights, pattern=pattern, is_name_only=True
          )

        filter_method = partial(self._fuzzyFilterEx, self._cli.isFullPath, getWeight)
        highlight_method = partial(
          self._highlight, self._cli.isFullPath, getHighlights, clear=False
        )
      else:
        fuzzy_match = FuzzyMatch(p, encoding)
        if self._getExplorer().getStlCategory() == "File" and self._cli.isFullPath:
          filter_method = partial(
            self._fuzzyFilterEx, self._cli.isFullPath, fuzzy_match.getWeight2
          )
        elif self._getExplorer().getStlCategory() in [
          "Self",
          "Buffer",
          ...
          filter_method = partial(
            self._fuzzyFilterEx, self._cli.isFullPath, fuzzy_match.getWeight3
          )
        else:
          filter_method = partial(
            self._fuzzyFilterEx, self._cli.isFullPath, fuzzy_match.getWeight
          )

        highlight_method = partial(
          self._highlight, self._cli.isFullPath, fuzzy_match.getHighlights, clear=False
        )
  ...
  def _fuzzySearch(self, content, is_continue, step):
    ...
    if self._cli.isAndMode:
      ...
      if self._cli.pattern[1] == "":  # e.g. abc;
        ...
        if is_fuzzyMatch_C and isAscii(self._cli.pattern[0]):
          ...
          getWeight_0 = partial(
            fuzzyMatchC.getWeight, pattern=pattern_0, is_name_only=True
          )
          getHighlights_0 = partial(
            fuzzyMatchC.getHighlights, pattern=pattern_0, is_name_only=True
          )
        else:
          is_ascii_0 = False
        ...
        if is_fuzzyMatch_C and isAscii(self._cli.pattern[1]):
          ...
          getWeight_1 = partial(
            fuzzyMatchC.getWeight, pattern=pattern_1, is_name_only=False
          )
          getHighlights_1 = partial(
            fuzzyMatchC.getHighlights, pattern=pattern_1, is_name_only=False
          )
        else:
          is_ascii_1 = False
        ...
        highlight_method = partial(
          self._highlightRefine, getHighlights_0, getHighlights_1
        )
    else:
      if (
        self._fuzzy_engine and isAscii(self._cli.pattern) and self._getUnit() == 1
      ):  # currently, only BufTag's _getUnit() is 2
        use_fuzzy_engine = True
        ...
        if self._getExplorer().getStlCategory() == "File":
          ...
          if self._cli.isFullPath:
            ...
            filter_method = partial(
              fuzzyEngine.fuzzyMatchPart,
              engine=self._fuzzy_engine,
              pattern=pattern,
              category=fuzzyEngine.Category_File,
          ...
          if self._cli.isFullPath or "--match-path" in self._arguments:
            ...
            filter_method = partial(
              ...
              engine=self._fuzzy_engine,
              pattern=pattern,
              category=fuzzyEngine.Category_Rg,
              param=fuzzyEngine.createRgParameter(
                self._getExplorer().displayMulti(),
                self._getExplorer().getContextSeparator(),
                self._has_column,
              ),
              is_name_only=True,
              sort_results=do_sort,
          ...
          filter_method = partial(
            ...
            engine=self._fuzzy_engine,
            pattern=pattern,
            category=fuzzyEngine.Category_Tag,
            param=fuzzyEngine.createParameter(mode),
            is_name_only=True,
            sort_results=do_sort,
          )
          ...
          filter_method = partial(
            ...
            engine=self._fuzzy_engine,
            pattern=pattern,
            category=fuzzyEngine.Category_Gtags,
          ...
          filter_method = partial(
            ...
            engine=self._fuzzy_engine,
            pattern=pattern,
            category=fuzzyEngine.Category_Line,
            param=fuzzyEngine.createParameter(1),
            is_name_only=True,
            sort_results=do_sort,
          )
          ...
          filter_method = partial(
            ...
            engine=self._fuzzy_engine,
            pattern=pattern,
            category=fuzzyEngine.Category_GitDiff,
            param=fuzzyEngine.createParameter(mode),
            is_name_only=False,
            sort_results=do_sort,
          )
        ...
        highlight_method = partial(
          self._highlight, self._cli.isFullPath, getHighlights, True
        )
        ...
        if "--no-sort" in self._arguments:
          filter_method = partial(
            self._fuzzyFilter, self._cli.isFullPath, fuzzy_match.getWeightNoSort
          )
        elif self._getExplorer().getStlCategory() == "File" and self._cli.isFullPath:
          filter_method = partial(
            self._fuzzyFilter, self._cli.isFullPath, fuzzy_match.getWeight2
          )
        elif self._getExplorer().getStlCategory() in [
          "Self",
          "Buffer",
          ...
          filter_method = partial(
            self._fuzzyFilter, self._cli.isFullPath, fuzzy_match.getWeight3
          )
        else:
          filter_method = partial(
            self._fuzzyFilter, self._cli.isFullPath, fuzzy_match.getWeight
          )

        highlight_method = partial(
          self._highlight, self._cli.isFullPath, fuzzy_match.getHighlights
        )
    ...
    if self._cli.isAndMode:
      ...
      _, self._result_content = self._filter(
        step, filter_method, content, is_continue, True, return_index
      )
    else:
      if step == 0:
  ...
  def _guessFilter(self, filename, suffix, dirname, icon, iterable):
    ...
    icon_len = len(icon)
    return (
      (FuzzyMatch.getPathWeight(filename, suffix, dirname, line[icon_len:]), line)
      for line in iterable
    )

  def _guessSearch(self, content, is_continue=False, step=0):
    if self._cur_buffer.name == "" or self._cur_buffer.options["buftype"] not in [
      b"",
      "",
    ]:
      self._getInstance().setBuffer(content[: self._initial_count])
    ...
    if self._fuzzy_engine:
      ...
      _, self._result_content = self._filter(
        step, filter_method, content, is_continue, True
      )
    else:
      step = len(content)
  ...
  def _resetHighlights(self):
    ...
    for n, highlight_pos in enumerate(highlight_pos_list):
      ...
      for i, pos in enumerate(highlight_pos):
        ...
        for j in range(0, len(pos), 8):
          if self._getInstance().getWinPos() == "popup":
            lfCmd(
              """call win_execute(%d, "let matchid = matchaddpos('%s', %s)")"""
    ...
    for i, pos in enumerate(self._highlight_refine_pos):
      ...
      for j in range(0, len(pos), 8):
        if self._getInstance().getWinPos() == "popup":
          lfCmd(
            """call win_execute(%d, "let matchid = matchaddpos('Lf_hl_matchRefine', %s)")"""
  ...
  def _highlight(
    ...
    use_fuzzy_engine=False,
    clear=True,
    hl_group="Lf_hl_match",
  ):
    # matchaddpos() is introduced by Patch 7.4.330
    if (
      lfEval("exists('*matchaddpos')") == "0"
      or lfEval("g:Lf_HighlightIndividual") == "0"
    ):
      return
    ...
    if use_fuzzy_engine:
      self._highlight_pos = get_highlights(
        source=[getDigest(line) for line in content[:highlight_number:unit]]
      )
    else:
      ...
      self._highlight_pos = [
        get_highlights(getDigest(line)) for line in content[:highlight_number:unit]
      ]
    if self._cli.isAndMode:
  ...
  def _highlightRefine(self, first_get_highlights, get_highlights):
    ...
    self._highlight_pos = [
      first_get_highlights(getDigest(line, 1))
    ...
    for i, pos in enumerate(self._highlight_pos):
      ...
      for j in range(0, len(pos), 8):
        if self._getInstance().getWinPos() == "popup":
          lfCmd(
            """call win_execute(%d, "let matchid = matchaddpos('Lf_hl_match', %s)")"""
            % (self._getInstance().getPopupWinId(), str(pos[j : j + 8]))
          )
          id = int(lfEval("matchid"))
        else:
    ...
    self._highlight_refine_pos = [
      get_highlights(getDigest(line, 2)) for line in content[:highlight_number:unit]
    ]
    for i, pos in enumerate(self._highlight_refine_pos):
  ...
  def _regexFilter(self, iterable):
    def noErrMatch(text, pattern):
      try:
        ...
        return "-1" != lfEval(
          "g:LfNoErrMsgMatch('%s', '%s')" % (text.replace("\x00", "\x01"), pattern)
        )
  ...
  def accept(self, mode=""):
    if self._getInstance().isReverseOrder():
      ...
      if self._getInstance().window.cursor[0] <= self._help_length:
        if self._getInstance().getWinPos() == "popup":
          lfCmd(f"call win_execute({self._getInstance().getPopupWinId()}, 'norm! j')")
        else:
          lfCmd("norm! j")
    ...
    if self._getExplorer().getStlCategory() == "Rg":
      if self._getInstance().currentLine == self._getExplorer().getContextSeparator():
        return
      if "--heading" in self._arguments and not re.match(
        r"^\d+[:-]", self._getInstance().currentLine
      ):
        return
    ...
    if len(self._selections) > 0:
      ...
      if "--stayOpen" in self._arguments:
        ...
        try:
          if self._preview_winid:
            self._closePreviewPopup()
          vim.current.tabpage, vim.current.window, vim.current.buffer = (
            self._getInstance().getOriginalPos()
          )
  ...
  def _jumpNext(self):
    ...
    if (
      instance.window is None
      or instance.empty()
      or len(instance.buffer) == self._help_length
    ):
      return False
    ...
    if instance.isReverseOrder():
      if instance.window.valid:
        ...
        line_num = self._getInstance().window.cursor[0]
        self._accept(
          instance.buffer[instance.window.cursor[0] - 1],
          "",
          self._getInstance().buffer,
          line_num,
        )
      else:
        ...
        self._accept(
          instance.buffer[instance.cursorRow - 1],
          "",
          self._getInstance().buffer,
          line_num,
        )
        lfCmd(
          "echohl WarningMsg | redraw | echo ' (%d of %d)' | echohl NONE"
          % (
      ...
      if instance.window.valid and self._getInstance().getWinPos() != "popup":
        ...
        lfCmd(
          "echohl WarningMsg | redraw | echo ' (%d of %d)' | echohl NONE"
          % (
            instance.cursorRow - instance.helpLength,
            len(instance.buffer) - instance.helpLength,
          )
        )
  ...
  def refresh(self, normal_mode=True):
    ...
    self._content = self._getInstance().initBuffer(
      content, self._getUnit(), self._getExplorer().setContent
    )
  ...
  def addSelections(self):
    ...
    if self._getInstance().getWinPos() != "popup":
      if int(lfEval("v:mouse_win")) != 0 and nr != int(lfEval("v:mouse_win")):
        return
    ...
    if line_num in self._selections:
      if self._getInstance().getWinPos() == "popup":
        lfCmd(
          "call matchdelete(%d, %d)"
          % (self._selections[line_num], self._getInstance().getPopupWinId())
        )
      else:
      ...
      if self._getInstance().getWinPos() == "popup":
        lfCmd(
          """call win_execute(%d, "let matchid = matchadd('Lf_hl_selection', '\\\\%%%dl.')")"""
          % (self._getInstance().getPopupWinId(), line_num)
        )
        id = int(lfEval("matchid"))
      else:
  ...
  def _gotoFirstLine(self):
    if self._getInstance().getWinPos() == "popup":
      lfCmd(f"call win_execute({self._getInstance().getPopupWinId()}, 'norm! gg')")
    else:
      lfCmd("normal! gg")
  ...
  def startExplorer(self, win_pos, *args, **kwargs):
    ...
    remember_last_status = (
      "--recall" in self._arguments
      or lfEval("g:Lf_RememberLastSearch") == "1"
      and self._cli.pattern
    )
    ...
    lfCmd(
      "call leaderf#colorscheme#popup#hiMode('%s', '%s')"
    ...
    self._getInstance().buffer.vars["Lf_category"] = (
      self._getExplorer().getStlCategory()
    )
    ...
    if isinstance(content, list):
      ...
      if not kwargs.get("bang", 0):
        ...
        elif remember_last_status and len(self._getInstance().buffer) < len(
          self._result_content
        ):
          self._getInstance().appendBuffer(self._result_content[self._initial_count :])
        ...
        if not remember_last_status and not self._cli.pattern and empty_query:
          ...
          if self._result_content:  # self._result_content is [] only if
            ...
            self._getInstance().appendBuffer(
              self._result_content[self._initial_count :]
            )
          else:
            self._getInstance().appendBuffer(self._content[self._initial_count :])
      ...
      if lfEval("get(g:, 'Lf_NoAsync', 0)") == "1":
        ...
        self._reader_thread = threading.Thread(
          target=self._readContent, args=(content,)
        )
  ...
  def _workInIdle(self, content=None, bang=False):
    ...
    if self._is_content_list:
      if self._cli.pattern and (
        self._index < len(self._content) or len(self._cb_content) > 0
      ):
        if self._fuzzy_engine:
    ...
    if self._read_finished > 0:
          ...
          if bang:
            if self._getInstance().empty():
              ...
              if cur_len > self._offset_in_content:
                self._getInstance().appendBuffer(
                  self._content[self._offset_in_content : cur_len]
                )
      ...
      if self._cli.pattern:
        ...
        if bang:
          ...
          if (
            self._getInstance().getWinPos() not in ("popup", "floatwin")
            and time.time() - self._bang_start_time > 0.5
          ):
            self._bang_start_time = time.time()
            lfCmd(
              "echohl WarningMsg | redraw | echo ' searching %s' | echohl NONE"
              % ("." * self._bang_count)
            )
  ...
  def input(self):
    ...
    if self._cli.pattern and "--live" not in self._arguments:
      ...
      self._empty_query
      and self._getExplorer().getStlCategory() in ["File"]
      ...
      if equal(cmd, "<Update>"):
        if self._getInstance().getWinPos() == "popup":
          if self._getInstance()._window_object.cursor[0] > 1:
            lfCmd(
              f"call win_execute({self._getInstance().getPopupWinId()}, 'norm! gg')"
            )
        ...
        selection = (
          int(vim.eval('confirm("Action?", "&Edit\n&Split\n&Vsplit\n&Tabnew\n&Drop")'))
          - 1
        )
