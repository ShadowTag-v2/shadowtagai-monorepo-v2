        ...
        if self._getInstance().getWinPos() == "popup":
            ...
            if lfEval("exists('*leaderf#%s#NormalModeFilter')" % self._getExplorer().getStlCategory()) == "1":
                lfCmd(
                    ...
                    % (self._getInstance().getPopupWinId(), "leaderf#%s#NormalModeFilter" % self._getExplorer().getStlCategory())
                )
            else:
                lfCmd(
    ...
    def _createPreviewWindow(self, config, source, line_num, jump_cmd):
        ...
        if lfEval("has('nvim')") == "1":
            ...
            if isinstance(source, int):
                ...
                lfCmd("noautocmd silent! let winid = popup_create(bufadd('/Lf_preview_%s'), %s)" % (id(self), json.dumps(config)))
