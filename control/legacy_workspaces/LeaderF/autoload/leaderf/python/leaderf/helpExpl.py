      ...
      try:
        with lfOpen(tags_file, "r", errors="ignore") as f:
          ...
          for line in lines:
            ...
            self._content.append(f"{tag:<40} {file} {file_id}")
      except OSError:
        pass
...
class HelpExplManager(Manager):
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      lfCmd(
        r"""call win_execute(%d, 'let matchid = matchadd(''Lf_hl_helpTagfile'', '' \zs.*$'')')"""
  ...
  def _previewInPopup(self, *args, **kwargs):
    ...
    jump_cmd = r"let pos = search('\m\*%s\*', 'w') | exec 'norm! '.pos.'G'" % escQuote(
      tagname
    )
