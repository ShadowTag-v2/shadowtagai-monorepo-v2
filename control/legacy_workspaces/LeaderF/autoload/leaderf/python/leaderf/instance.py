...
import time
from .utils import *
from .devicons import webDevIconsGetFileTypeSymbol, highlightDevIcons
...
class PopupWindow:
  ...
  def cursor(self):
    ...
    lfCmd(
      """call win_execute(%d, 'let cursor_pos = getpos(".")') | let cursor_pos[2] -= 1"""
      % self._winid
    )
...
class LfInstance:
  ...
  def __init__(
    ...
    self._ignore_cur_buffer_name = lfEval(
      "get(g:, 'Lf_IgnoreCurrentBufferName', 0)"
    ) == "1" and self._category in ["File"]
    self._popup_winid = 0
  ...
  def _initStlVar(self):
    if int(lfEval(f"!exists('g:Lf_{self._category}_StlCategory')")):
      lfCmd(f"let g:Lf_{self._category}_StlCategory = '-'")
      lfCmd(f"let g:Lf_{self._category}_StlMode = '-'")
      lfCmd(f"let g:Lf_{self._category}_StlCwd= '-'")
      lfCmd(f"let g:Lf_{self._category}_StlRunning = ''")
      lfCmd(f"let g:Lf_{self._category}_IsRunning = 0")
      lfCmd(f"let g:Lf_{self._category}_StlTotal = '0'")
      lfCmd(f"let g:Lf_{self._category}_StlLineNumber = '1'")
      lfCmd(f"let g:Lf_{self._category}_StlResultsCount = '0'")
  ...
  def _setStatusline(self):
    ...
    if not self._is_autocmd_set:
      ...
      lfCmd(f"augroup Lf_{self._category}_Colorscheme")
      lfCmd(
        f"autocmd ColorScheme * call leaderf#colorscheme#setStatusline({self.buffer.number}, '{self._stl}')"
      )
      lfCmd(
        f"autocmd WinEnter,FileType * call leaderf#colorscheme#setStatusline({self.buffer.number}, '{self._stl}')"
      )
      lfCmd("augroup END")
  ...
  def enlargePopupWindow(self):
    ...
    if preview_pos.lower() == "right":
      ...
      if lfEval("has('nvim')") == "1":
        ...
        lfCmd(
          f"call leaderf#ResetFloatwinOptions({self._popup_winid}, 'width', {width})"
        )
        lfCmd(
          f"call leaderf#ResetFloatwinOptions({self._popup_instance.input_win.id}, 'width', {width})"
        )
        if lfEval("get(g:, 'Lf_PopupShowStatusline', 1)") == "1":
          lfCmd(
            f"call leaderf#ResetFloatwinOptions({self._popup_instance.statusline_win.id}, 'width', {width})"
          )
      else:
        width = self._initial_maxwidth * 2 + 2

        lfCmd(
          f"call leaderf#ResetPopupOptions({self._popup_winid}, 'maxwidth', {width})"
        )
        lfCmd(
          f"call leaderf#ResetPopupOptions({self._popup_winid}, 'minwidth', {width})"
        )

        lfCmd(
          f"call leaderf#ResetPopupOptions({self._popup_instance.input_win.id}, 'maxwidth', {width})"
        )
        lfCmd(
          f"call leaderf#ResetPopupOptions({self._popup_instance.input_win.id}, 'minwidth', {width})"
        )

        if lfEval("get(g:, 'Lf_PopupShowStatusline', 1)") == "1":
          lfCmd(
            f"call leaderf#ResetPopupOptions({self._popup_instance.statusline_win.id}, 'maxwidth', {width})"
          )
          lfCmd(
            f"call leaderf#ResetPopupOptions({self._popup_instance.statusline_win.id}, 'minwidth', {width})"
          )
  ...
  def shrinkPopupWindow(self):
    ...
    if preview_pos.lower() == "right":
      if lfEval("has('nvim')") == "1":
        lfCmd(
          f"call leaderf#ResetFloatwinOptions({self._popup_winid}, 'width', {self._initial_maxwidth})"
        )
        lfCmd(
          f"call leaderf#ResetFloatwinOptions({self._popup_instance.input_win.id}, 'width', {self._initial_maxwidth})"
        )

        if lfEval("get(g:, 'Lf_PopupShowStatusline', 1)") == "1":
          lfCmd(
            f"call leaderf#ResetFloatwinOptions({self._popup_instance.statusline_win.id}, 'width', {self._initial_maxwidth})"
          )
      else:
        lfCmd(
          f"call leaderf#ResetPopupOptions({self._popup_winid}, 'maxwidth', {self._initial_maxwidth})"
        )
        lfCmd(
          f"call leaderf#ResetPopupOptions({self._popup_winid}, 'minwidth', {self._initial_maxwidth})"
        )

        lfCmd(
          f"call leaderf#ResetPopupOptions({self._popup_instance.input_win.id}, 'maxwidth', {self._initial_maxwidth})"
        )
        lfCmd(
          f"call leaderf#ResetPopupOptions({self._popup_instance.input_win.id}, 'minwidth', {self._initial_maxwidth})"
        )

        if lfEval("get(g:, 'Lf_PopupShowStatusline', 1)") == "1":
          lfCmd(
            f"call leaderf#ResetPopupOptions({self._popup_instance.statusline_win.id}, 'maxwidth', {self._initial_maxwidth})"
          )
          lfCmd(
            f"call leaderf#ResetPopupOptions({self._popup_instance.statusline_win.id}, 'minwidth', {self._initial_maxwidth})"
          )
  ...
  def _createPopupWindow(self):
    ...
    if lfEval("has('nvim')") == "0" and self._popup_instance.valid():
      if (
        self._popup_instance.tabpage == vim.current.tabpage
    ...
    buf_number = int(lfEval(f"bufadd('{escQuote(self._buffer_name)}')"))
    ...
    if lfEval("has('nvim')") == "1":
      ...
      lfCmd(
        "noautocmd silent noswapfile let winid = nvim_open_win(%d, 1, %s)"
        % (buf_number, str(config))
      )
      ...
      self._window_object = FloatWindow(
        self._popup_winid,
        vim.current.window,
        self._buffer_object,
        self._tabpage_object,
        config["row"],
      )
      ...
      lfCmd(
        "noautocmd silent let winid = nvim_open_win(%d, 0, %s)"
        % (buf_number, str(input_win_config))
      )
      winid = int(lfEval("winid"))
      self._nvim_set_options(buf_number, winid)
      lfCmd("silent! call nvim_win_set_option(%d, 'statuscolumn', '')" % winid)
      lfCmd(
        "call nvim_win_set_option(%d, 'winhighlight', 'Normal:Lf_hl_popup_inputText')"
        % winid
      )
      ...
      self._popup_instance.input_win = FloatWindow(
        winid,
      ...
      if lfEval("get(g:, 'Lf_PopupShowStatusline', 1)") == "1":
        ...
        lfCmd(
          "noautocmd silent let winid = nvim_open_win(%d, 0, %s)"
          % (buf_number, str(stl_win_config))
        )
        winid = int(lfEval("winid"))
        self._nvim_set_options(buf_number, winid)
        lfCmd("silent! call nvim_win_set_option(%d, 'statuscolumn', '')" % winid)
        lfCmd(
          "call nvim_win_set_option(%d, 'winhighlight', 'Normal:Lf_hl_popup_blank')"
          % winid
        )
        self._popup_instance.statusline_win = FloatWindow(
          winid,
          getWindow(int(lfEval("win_id2win(%d)" % winid))),
          vim.buffers[buf_number],
          vim.current.tabpage,
      ...
      lfCmd(
        "autocmd! WinEnter * call leaderf#closeAllFloatwin(%d, %d, %d, %d, %d)"
        % (
      ...
      lfCmd(
        "noautocmd silent noswapfile let winid = popup_create(%d, %s)"
        % (buf_number, str(options))
      )
      self._popup_winid = int(lfEval("winid"))
      self._window_id = self._popup_winid
      ...
      lfCmd(
        "call win_execute(%d, 'setlocal wincolor=Lf_hl_popup_window')"
        % self._popup_winid
      )
      lfCmd(
        "call win_execute(%d, 'silent! setlocal filetype=leaderf')" % self._popup_winid
      )
      lfCmd(
        "call win_execute(%d, 'norm! %dG')"
        % (self._popup_winid, self._popup_cursor_line)
      )

      self._tabpage_object = vim.current.tabpage
      self._buffer_object = vim.buffers[buf_number]
      self._window_object = PopupWindow(
        self._popup_winid, self._buffer_object, self._tabpage_object, options["line"]
      )
      self._popup_instance.content_win = self._window_object
      ...
      lfCmd(
        ...
        % (buf_number, str(input_win_options))
      )
      winid = int(lfEval("winid"))
      self._vim_set_options(winid)
      lfCmd("call win_execute(%d, 'setlocal wincolor=Lf_hl_popup_inputText')" % winid)

      self._popup_instance.input_win = PopupWindow(
        winid, vim.buffers[buf_number], vim.current.tabpage, line
      )
      ...
      if lfEval("get(g:, 'Lf_PopupShowStatusline', 1)") == "1":
        ...
        lfCmd(
          "noautocmd silent let winid = popup_create(%d, %s)"
          % (buf_number, str(statusline_win_options))
        )
        winid = int(lfEval("winid"))
        self._vim_set_options(winid)
        lfCmd("call win_execute(%d, 'setlocal wincolor=Lf_hl_popup_blank')" % winid)

        self._popup_instance.statusline_win = PopupWindow(
          winid,
          vim.buffers[buf_number],
          vim.current.tabpage,
      ...
      lfCmd(
        "call leaderf#ResetPopupOptions(%d, 'callback', function('leaderf#PopupClosed', [%s, %d]))"
        % (
          self._popup_winid,
          str(self._popup_instance.getWinIdList()),
          id(self._manager),
        )
      )
    ...
    if not self._is_popup_colorscheme_autocmd_set:
      ...
      lfCmd(
        "call leaderf#colorscheme#popup#load('{}', '{}')".format(
          self._category, lfEval("get(g:, 'Lf_PopupColorscheme', 'default')")
        )
      )
      lfCmd(f"augroup Lf_Popup_{self._category}_Colorscheme")
      lfCmd(
        "autocmd ColorScheme * call leaderf#colorscheme#popup#load('{}', '{}')".format(
          self._category, lfEval("get(g:, 'Lf_PopupColorscheme', 'default')")
        )
      )
  ...
  def _createBufWindow(self, win_pos):
    ...
    if (
      self._buffer_object is not None
      and self._buffer_object.valid
    ...
    if (
      self._reverse_order or lfEval("get(g:, 'Lf_AutoResize', 0)") == "1"
    ) and "--recall" not in self._arguments:
    ...
    if not self._is_colorscheme_autocmd_set:
      ...
      lfCmd(
        f"call leaderf#colorscheme#highlight('{self._category}', {self._buffer_object.number})"
      )
      lfCmd(f"augroup Lf_{self._category}_Colorscheme")
      lfCmd("autocmd!")
      lfCmd(
        f"autocmd ColorScheme * call leaderf#colorscheme#highlight('{self._category}', {self._buffer_object.number})"
      )
      lfCmd(
        f"autocmd ColorScheme * call leaderf#colorscheme#highlightMode('{self._category}', g:Lf_{self._category}_StlMode)"
      )
      lfCmd("autocmd ColorScheme <buffer> doautocmd syntax")
      lfCmd(
        f"autocmd CursorMoved <buffer> let g:Lf_{self._category}_StlLineNumber = 1 + line('$') - line('.')"
      )
      lfCmd("autocmd VimResized * let g:Lf_VimResized = 1")
      lfCmd(f"autocmd BufHidden <buffer> call leaderf#Quit('{str(id(self._manager))}')")
      lfCmd("augroup END")
  ...
  def _enterOpeningBuffer(self):
    if (
      self._tabpage_object
      and self._tabpage_object.valid
      and self._window_object
      and self._window_object.valid
      and self._window_object.number
      != 0  # the number may be 0 although PopupWindow is valid because of popup_hide()
  ...
  def setArguments(self, arguments):
    ...
    if self._arguments.get("--reverse", 1) is None:
      ...
      if (
        "--reverse" in self._arguments or lfEval("get(g:, 'Lf_ReverseOrder', 0)") == "1"
      ):
      ...
      if (
        "--popup" in self._arguments or lfEval("g:Lf_WindowPosition") == "popup"
  ...
  def mimicCursor(self):
    if self._win_pos == "popup" and self._manager._current_mode == "NORMAL":
      if self._popup_winid > 0 and self._window_object.valid:
        ...
        lfCmd(
          """silent! call win_execute(%d, 'let g:Lf_mimicedCursorId = matchaddpos("Cursor", [cursor_pos])')"""
          % (self._popup_winid)
        )
  ...
  def setPopupStl(self, current_mode):
    ...
    match_mode = lfEval(f"g:Lf_{self._category}_StlMode")
    cwd = lfEval(f"g:Lf_{self._category}_StlCwd")

    text = (
      f" {current_mode} {sep} {self._category} {sep} {match_mode} {sep} {cwd} {sep}"
    )
    ...
    if self._win_pos == "popup":
      ...
      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_%s_mode'})")"""
        % (statusline_win.id, self._category)
      )
      lfCmd(
        """call win_execute(%d, "call prop_add(1, 1, {'length': %d, 'type': 'Lf_hl_popup_%s_mode'})")"""
        % (statusline_win.id, lfBytesLen(current_mode) + 2, self._category)
      )

      if sep != "":
        lfCmd(
          """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_%s_sep0'})")"""
          % (statusline_win.id, self._category)
        )
        lfCmd(
          """call win_execute(%d, "call prop_add(1, %d, {'length': %d, 'type': 'Lf_hl_popup_%s_sep0'})")"""
          % (statusline_win.id, sep0_start, sep_len, self._category)
        )

      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_category'})")"""
        % (statusline_win.id)
      )
      lfCmd(
        """call win_execute(%d, "call prop_add(1, %d, {'length': %d, 'type': 'Lf_hl_popup_category'})")"""
        % (statusline_win.id, category_start, category_len)
      )

      if sep != "":
        lfCmd(
          """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_%s_sep1'})")"""
          % (statusline_win.id, self._category)
        )
        lfCmd(
          """call win_execute(%d, "call prop_add(1, %d, {'length': %d, 'type': 'Lf_hl_popup_%s_sep1'})")"""
          % (statusline_win.id, sep1_start, sep_len, self._category)
        )

      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_%s_matchMode'})")"""
        % (statusline_win.id, self._category)
      )
      lfCmd(
        """call win_execute(%d, "call prop_add(1, %d, {'length': %d, 'type': 'Lf_hl_popup_%s_matchMode'})")"""
        % (statusline_win.id, match_mode_start, match_mode_len, self._category)
      )

      if sep != "":
        lfCmd(
          """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_%s_sep2'})")"""
          % (statusline_win.id, self._category)
        )
        lfCmd(
          """call win_execute(%d, "call prop_add(1, %d, {'length': %d, 'type': 'Lf_hl_popup_%s_sep2'})")"""
          % (statusline_win.id, sep2_start, sep_len, self._category)
        )

      lfCmd(
        """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_cwd'})")"""
        % (statusline_win.id)
      )
      lfCmd(
        """call win_execute(%d, "call prop_add(1, %d, {'length': %d, 'type': 'Lf_hl_popup_cwd'})")"""
        % (statusline_win.id, cwd_start, cwd_len)
      )

      if sep != "":
        lfCmd(
          """call win_execute(%d, "call prop_remove({'type': 'Lf_hl_popup_%s_sep3'})")"""
          % (statusline_win.id, self._category)
        )
        lfCmd(
          """call win_execute(%d, "call prop_add(1, %d, {'length': %d, 'type': 'Lf_hl_popup_%s_sep3'})")"""
      ...
      if self._stl_buf_namespace is None:
        ...
        lfCmd(
          "call nvim_buf_clear_namespace(%d, %d, 0, -1)"
          % (statusline_win.buffer.number, self._stl_buf_namespace)
        )

      lfCmd(
        "call nvim_buf_add_highlight(%d, %d, 'Lf_hl_popup_%s_mode', 0, 0, %d)"
        % (
          statusline_win.buffer.number,
          self._stl_buf_namespace,
          self._category,
      ...
      if sep != "":
        lfCmd(
          "call nvim_buf_add_highlight(%d, %d, 'Lf_hl_popup_%s_sep0', 0, %d, %d)"
          % (
            statusline_win.buffer.number,
            self._stl_buf_namespace,
            self._category,
      ...
      lfCmd(
        "call nvim_buf_add_highlight(%d, %d, 'Lf_hl_popup_category', 0, %d, %d)"
        % (
          statusline_win.buffer.number,
          self._stl_buf_namespace,
          category_start,
          category_start + category_len,
        )
      )

      if sep != "":
        lfCmd(
          "call nvim_buf_add_highlight(%d, %d, 'Lf_hl_popup_%s_sep1', 0, %d, %d)"
          % (
            statusline_win.buffer.number,
            self._stl_buf_namespace,
            self._category,
      ...
      lfCmd(
        "call nvim_buf_add_highlight(%d, %d, 'Lf_hl_popup_%s_matchMode', 0, %d, %d)"
        % (
          statusline_win.buffer.number,
          self._stl_buf_namespace,
          self._category,
          match_mode_start,
          match_mode_start + match_mode_len,
        )
      )

      if sep != "":
        lfCmd(
          "call nvim_buf_add_highlight(%d, %d, 'Lf_hl_popup_%s_sep2', 0, %d, %d)"
          % (
            statusline_win.buffer.number,
            self._stl_buf_namespace,
            self._category,
      ...
      lfCmd(
        "call nvim_buf_add_highlight(%d, %d, 'Lf_hl_popup_cwd', 0, %d, %d)"
        % (
          statusline_win.buffer.number,
          self._stl_buf_namespace,
          cwd_start,
      ...
      if sep != "":
        lfCmd(
          "call nvim_buf_add_highlight(%d, %d, 'Lf_hl_popup_%s_sep3', 0, %d, %d)"
          % (
            statusline_win.buffer.number,
            self._stl_buf_namespace,
            self._category,
  ...
  def setStlCategory(self, category):
    lfCmd(f"let g:Lf_{self._category}_StlCategory = '{category}'")

  def setStlMode(self, mode):
    lfCmd(f"let g:Lf_{self._category}_StlMode = '{mode}'")
    if self._win_pos in ("popup", "floatwin"):
      lfCmd(
        f"call leaderf#colorscheme#popup#hiMatchMode('{self._category}', g:Lf_{self._category}_StlMode)"
      )
    else:
      lfCmd(
        f"call leaderf#colorscheme#highlightMode('{self._category}', g:Lf_{self._category}_StlMode)"
      )

  def setStlCwd(self, cwd):
    lfCmd(f"let g:Lf_{self._category}_StlCwd = '{cwd}'")

  def setStlTotal(self, total):
    lfCmd(f"let g:Lf_{self._category}_StlTotal = '{total}'")

  def setStlResultsCount(self, count, check_ignored=False):
    if check_ignored and self._cur_buffer_name_ignored:
      count -= 1
    lfCmd(f"let g:Lf_{self._category}_StlResultsCount = '{count}'")
  ...
  def setStlRunning(self, running):
    if self._win_pos in ("popup", "floatwin"):
      if running:
        lfCmd(f"let g:Lf_{self._category}_IsRunning = 1")
      else:
        lfCmd(f"let g:Lf_{self._category}_IsRunning = 0")
      return

    if running:
      spin = f"{self._cli._spin_symbols[self._running_status]} "
      self._running_status = (self._running_status + 1) % len(self._cli._spin_symbols)
      lfCmd(f"let g:Lf_{self._category}_StlRunning = '{spin}'")
    else:
      self._running_status = 0
      lfCmd(f"let g:Lf_{self._category}_StlRunning = ''")
  ...
  def enterBuffer(self, win_pos, clear):
    ...
    if not self._is_icon_colorscheme_autocmd_set:
      ...
      if (
        lfEval("get(g:, 'Lf_highlightDevIconsLoad', 0)") == "0"
        or lfEval("hlexists('Lf_hl_devIcons_c')") == "0"
      ):
        highlightDevIcons()
  ...
  def exitBuffer(self):
    ...
    if self._win_pos == "popup":
      ...
      if len(vim.windows) > 1:
        ...
        if lfEval("get(g:, 'Lf_VimResized', 0)") == "0" and self._orig_win_count == len(
          vim.windows
        ):
  ...
  def _actualLength(self, buffer):
    num = 0
    columns = (
      self._window_object.width
      - int(lfEval("&numberwidth"))
      - int(lfEval("&foldcolumn"))
    )
    for i in buffer:
      try:
        num += (
          int(lfEval("strdisplaywidth('%s')" % escQuote(i))) + columns - 1
        ) // columns
      except:
        num += (
          int(lfEval("strdisplaywidth('%s')" % escQuote(i).replace("\x00", "\x01")))
          + columns
          - 1
        ) // columns
    return num
      ...
      if self._reverse_order:
        ...
        if buffer_len < self._initial_win_height:
          if "--nowrap" not in self._arguments:
            self._window_object.height = min(
              self._initial_win_height, self._actualLength(self._buffer_object)
            )
          else:
        ...
        if self._win_pos == "popup":
          self._orig_pos[1].options["foldmethod"] = self._orig_pos[1].options[
            "foldmethod"
          ]
    finally:
      self.refreshPopupStatusline()
  ...
  def refreshPopupStatusline(self):
    ...
    if self._win_pos == "popup":
      if "--nowrap" not in self._arguments:
        lfCmd(
          "call leaderf#ResetPopupOptions(%d, 'minheight', %d)"
          % (
            self._popup_winid,
            min(
              self._popup_maxheight,
              self._actualLength(self._buffer_object[: self._popup_maxheight]),
            ),
          )
        )
      else:
        lfCmd(
          "call leaderf#ResetPopupOptions(%d, 'minheight', %d)"
          % (self._popup_winid, min(self._popup_maxheight, buffer_len))
        )
      ...
      if statusline_win:
        ...
        if expected_line != int(lfEval("popup_getpos(%d).line" % statusline_win.id)):
          lfCmd(
            "call leaderf#ResetPopupOptions(%d, 'line', %d)"
            % (statusline_win.id, expected_line)
          )
    elif self._win_pos == "floatwin":
      if buffer_len < self._popup_maxheight:
        if "--nowrap" not in self._arguments:
          self._window_object.height = min(
            self._popup_maxheight, self._actualLength(self._buffer_object)
          )
        else:
          self._window_object.height = buffer_len
        ...
        if statusline_win:
          expected_line = self._window_object.initialLine + self._window_object.height
          if expected_line != int(
            float(lfEval("nvim_win_get_config(%d).row" % statusline_win.id))
          ):
            lfCmd(
              "call leaderf#ResetFloatwinOptions(%d, 'row', %d)"
              % (statusline_win.id, expected_line)
            )
  ...
  def setLineNumber(self):
    if self._reverse_order:
      ...
      lfCmd(f"let g:Lf_{self._category}_StlLineNumber = '{line_num}'")
