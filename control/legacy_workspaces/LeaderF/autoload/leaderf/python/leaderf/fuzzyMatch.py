...
class FuzzyMatch:
  ...
  def evaluate(text, pattern, text_mask, j, pattern_mask, k, val):
    ...
    if j + i == 0 or text[i - 1] in "_- ":
      ...
      special = (
        2
        if not text[i - 1].isupper() or i + 1 < text_len and text[i + 1].islower()
        else 0
      )
    elif text[i - 1] == ".":
      special = 1.9
    ...
    while i < text_len:
      ...
      if c in pattern_mask:
        ...
        text[i - 1].isupper()
        and c.lower() in pattern_mask
        and (i + 1 == text_len or not text[i + 1].islower())
      ):
      ...
      if d >= last:
        ...
        if n == pattern_len:
          ...
          if (
            prefix_score > max_prefix_score
            or special
            and prefix_score == max_prefix_score
          ):
            max_prefix_score = prefix_score
            res = FuzzyMatch.evaluate(
              text[i:], pattern[n:], text_mask, j + i, pattern_mask, k + n, val
            )
    ...
    if is_pattern_lower:
      ...
      if beg != -1:
        if text[beg].isupper() or beg == 0 or text[beg - 1] in "_.- /\\":
          ...
          if second_beg != -1 and (
            text[second_beg].isupper() or text[second_beg - 1] in "_.- /\\"
          ):
            special = 2
            beg = second_beg
        ...
        if text[end].isupper() or end == 0 or text[end - 1] in "_.- /\\":
          ...
          if second_end != -1 and (
            text[second_end].isupper() or text[second_end - 1] in "_.- /\\"
          ):
            if special != 2:
      ...
      if pattern[1].isupper():  # e.g. pattern is 'AB'
        ...
        return (
          FuzzyMatch.MIN_WEIGHT if end == -1 else 1.0 / (beg + end) + 1.0 / len(text)
        )
  ...
  def getWeight(self, text):
    ...
    if self._is_pattern_lower:
      ...
      for i, c in enumerate(
        text_lower[first_char_pos : last_char_pos + 1], first_char_pos
      ):
        if c in text_mask:
          text_mask[c] |= 1 << i
      ...
      for i, c in enumerate(text[first_char_pos : last_char_pos + 1], first_char_pos):
        if c.isupper():
          ...
          if j < pattern_len and (
            c == self._pattern[j] or c.lower() == self._pattern[j]
          ):
            j += 1
        else:
          if c in text_mask:
    ...
    if j < pattern_len:
      return FuzzyMatch.MIN_WEIGHT
    val = {}
    score, beg, end = FuzzyMatch.evaluate(
      text, self._pattern, text_mask, 0, self._pattern_mask, 0, val
    )
      ...
      if d >= last:
        ...
        if n == pattern_len:
          ...
          if (
            ...
            max_prefix_score = prefix_score
            res = FuzzyMatch.evaluateHighlights(
              text[i:], pattern[n:], text_mask, j + i, pattern_mask, k + n, val
            )
            score = prefix_score + res[0] if res[0] else 0
      ...
      for i, c in enumerate(text[first_char_pos : last_char_pos + 1], first_char_pos):
        if c.isupper():
          ...
          if c in text_mask:
            text_mask[c] |= 1 << i
    val = {}
    score, highlights = FuzzyMatch.evaluateHighlights(
      text, self._pattern, text_mask, 0, self._pattern_mask, 0, val
    )
    ...
    if suffix != "":
      ...
      if filename_lcp > 0:
        if "a" <= p <= "z" or "0" <= p <= "9" or "a" <= p1 <= "z" or "0" <= p1 <= "9":
          ...
          "A" <= p <= "Z"
          and "A" <= p1 <= "Z"
          and "A" <= filename[filename_prefix - 1] <= "Z"
        ):
          filename_prefix -= 1
          while filename_prefix > 0:
            if "A" <= filename[filename_prefix] <= "Z":
      ...
      if filename_lcp > 0:
        ...
        if (
          is_suffix_diff == 0
          and filename_lcp == len(filename) == len(root) - filename_start
        ):
          is_basename_same = 1
    else:
