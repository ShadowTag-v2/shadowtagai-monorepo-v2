...
class TagExplorer(Explorer):
    ...
    def getFreshContent(self, *args, **kwargs):
        ...
        if not has_new_tagfile and not has_changed_tagfile:
            ...
            if len(res) > 1:
                ...
                if result:
                    ...
                    if result:
                        ...
                        name = tagfield[0]
                        value = tagfield[-1]
                        lfCmd(rf"call search('\m{name}\_s\+{value}\_[^;{{]*{{', 'w')")
            ...
            if len(res) > 1:
                ...
                if result:
                    ...
                    if result:
                        ...
                        name = tagfield[0]
                        value = tagfield[-1]
                        jump_cmd += rf" | call search('\m{name}\_s\+{value}\_[^;{{]*{{', 'w')"

                    pattern = r"\M" + tagaddress[1:-1]
                    jump_cmd += f" | call search('{escQuote(pattern)}', 'w')"
