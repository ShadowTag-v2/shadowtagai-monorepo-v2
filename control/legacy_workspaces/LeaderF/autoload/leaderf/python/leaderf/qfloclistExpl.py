...
class QfLocListExplManager(Manager):
    ...
    def _afterEnter(self):
        ...
        if self._getInstance().getWinPos() == "popup":
            ...
            id = int(lfEval(r"""matchadd('Lf_hl_%sFileName', '^.\{-}:\ze\d\+:')""" % list_type))
            self._match_ids.append(id)

            id = int(lfEval(r"""matchadd('Lf_hl_%sLineNumber', '^.\{-}:\zs\d\+:')""" % list_type))
            self._match_ids.append(id)

            id = int(lfEval(r"""matchadd('Lf_hl_%sColumnNumber', '^.\{-}:\d\+:\zs\d\+:')""" % list_type))
            self._match_ids.append(id)
