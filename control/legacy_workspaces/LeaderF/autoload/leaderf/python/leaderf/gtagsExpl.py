...
class GtagsExplorer(Explorer):
  def __init__(self):
    ...
    self._db_location = os.path.join(lfEval("g:Lf_CacheDirectory"), "LeaderF", "gtags")
  ...
  def getContent(self, *args, **kwargs):
    ...
    if self._gtagsconf == "" and os.name == "nt":
      self._gtagsconf = os.path.normpath(
        os.path.join(self._which("gtags.exe"), "..", "share", "gtags", "gtags.conf")
      ).join('""')

    if "--gtagslibpath" in arguments_dict:
      self._gtagslibpath = [
        os.path.abspath(os.path.expanduser(p)) for p in arguments_dict["--gtagslibpath"]
      ]
    ...
    if "--update" in arguments_dict:
      ...
      if "--skip-symlink" in arguments_dict and self._skip_symlink != "":
        ...
        self._skip_symlink = "--skip-symlink%s " % (
          "=" + skip_symlink[0] if skip_symlink else ""
        )
    ...
    if "-d" in arguments_dict:
      ...
      pattern_option = '--from-here "%d:%s" %s ' % (
      ...
      if (
        not self._isDBModified(os.path.join(dbpath, "GTAGS"))
        and self._content
        and self._cmd == cmd
      ):
    ...
    if len(pattern) > 1 and (
      pattern[0] == pattern[-1] == '"' or pattern[0] == pattern[-1] == "'"
    ):
      p = pattern[1:-1]
    else:
      p = pattern
  ...
  def _remove(self, filename):
    ...
    try:
      ...
      if self._store_in_project:
        if lfEval('input("Are you sure you want to remove GTAGS files?[Ny] ")') in [
          "Y",
          "y",
        ]:
        ...
        'input("Are you sure you want to remove directory `{}`?[Ny] ")'.format(
          lfEncode(dbpath.replace("\\", r"\\"))
        )
      ) in ["Y", "y"]:
  ...
  def _update(self, filename, single_update, auto):
    ...
    if single_update:
      if exists:
        cmd = (
          'cd {}"{}" && {} {}{}{}{}--gtagslabel {} --single-update "{}" "{}"'.format(
            self._cd_option,
            root,
            self._gtags,
            self._accept_dotfiles,
        ...
        proc = subprocess.Popen(
          cmd, shell=True, universal_newlines=True, stderr=subprocess.PIPE, env=env
        )
  ...
  def _updateLibGtags(self, root, dbpath):
    ...
    libpaths = [
      "%s\t%s\n" % (p, self._generateDbpath(p))
      for p in self._gtagslibpath
      if os.path.exists(p) and p != root
    ]
    if libpaths:
      libdb = os.path.join(dbpath, "GTAGSLIBPATH")
  ...
  def _evalVimVar(self):
    ...
    self._accept_dotfiles = (
      "--accept-dotfiles "
      if lfEval("get(g:, 'Lf_GtagsAcceptDotfiles', '0')") == "1"
      else ""
    )
    self._skip_unreadable = (
      "--skip-unreadable "
      if lfEval("get(g:, 'Lf_GtagsSkipUnreadable', '0')") == "1"
      else ""
    )
    self._skip_symlink = "--skip-symlink%s " % (
      "=" + lfEval("get(g:, 'Lf_GtagsSkipSymlink', '')")
      if lfEval("get(g:, 'Lf_GtagsSkipSymlink', '')") != ""
      else ""
    )
    ...
    if (
      self._Lf_GtagsSource != 1
    ):  # only using FileExplorer needs to evaluate the following variables
  ...
  def _buildCmd(self, dir, **kwargs):
    ...
    if self._Lf_UseVersionControlTool:
      if self._exists(dir, ".git"):
        ...
        cmd = 'git ls-files %s "%s" && git ls-files --others %s %s "%s"' % (
          recurse_submodules,
          dir,
          no_ignore,
          ignore,
    ...
    if default_tool["rg"] and self._is_rg_executable:
      ...
      default_tool["pt"] and self._is_pt_executable and os.name != "nt"
      ...
      default_tool["ag"] and self._is_ag_executable and os.name != "nt"
  ...
  def _file_list_cmd(self, root):
    if self._Lf_GtagsSource == 1:
      ...
      if os.path.exists(os.path.join(root, ".git")) and os.path.isdir(
        os.path.join(root, ".git")
      ):
        cmd = self._Lf_GtagsfilesCmd[".git"]
      elif os.path.exists(os.path.join(root, ".hg")) and os.path.isdir(
        os.path.join(root, ".hg")
      ):
...
class GtagsExplManager(Manager):
  ...
  def _acceptSelection(self, *args, **kwargs):
    ...
    try:
      ...
      if vim.current.window not in self._cursorline_dict:
        self._cursorline_dict[vim.current.window] = vim.current.window.options[
          "cursorline"
        ]
  ...
  def autoJump(self, content):
    if (
      "--auto-jump" in self._arguments
      and isinstance(content, list)
      and len(content) == 1
    ):
      mode = (
        self._arguments["--auto-jump"][0] if len(self._arguments["--auto-jump"]) else ""
      )
  ...
  def _afterEnter(self):
    ...
    if self._getInstance().getWinPos() == "popup":
      if self._getExplorer().getResultFormat() is None:
        ...
        lfCmd(
          """call win_execute(%d, "let matchid = matchadd('Lf_hl_gtagsFileName', '^.\\\\{-}\\\\ze\\\\t')")"""
          % self._getInstance().getPopupWinId()
        )
        id = int(lfEval("matchid"))
        self._match_ids.append(id)
        lfCmd(
          """call win_execute(%d, "let matchid = matchadd('Lf_hl_gtagsLineNumber', '\\\\t\\\\zs\\\\d\\\\+\\\\ze\\\\t')")"""
          % self._getInstance().getPopupWinId()
        )
        id = int(lfEval("matchid"))
        self._match_ids.append(id)
        ...
        lfCmd(
          """call win_execute(%d, "let matchid = matchadd('Lf_hl_gtagsFileName', '\\\\t\\\\zs.\\\\{-}\\\\ze\\\\t')")"""
          % self._getInstance().getPopupWinId()
        )
        id = int(lfEval("matchid"))
        self._match_ids.append(id)
        lfCmd(
          """call win_execute(%d, "let matchid = matchadd('Lf_hl_gtagsLineNumber', '\\\\t\\\\zs\\\\d\\\\+$')")"""
          % self._getInstance().getPopupWinId()
        )
        id = int(lfEval("matchid"))
        self._match_ids.append(id)
        ...
        lfCmd(
          """call win_execute(%d, "let matchid = matchadd('Lf_hl_gtagsFileName', '^\\\\S\\\\+\\\\s\\\\+\\\\d\\\\+\\\\s\\\\+\\\\zs\\\\S\\\\+')")"""
          % self._getInstance().getPopupWinId()
        )
        id = int(lfEval("matchid"))
        self._match_ids.append(id)
        lfCmd(
          """call win_execute(%d, "let matchid = matchadd('Lf_hl_gtagsLineNumber', '^\\\\S\\\\+\\\\s\\\\+\\\\zs\\\\d\\\\+')")"""
          % self._getInstance().getPopupWinId()
        )
        id = int(lfEval("matchid"))
        self._match_ids.append(id)
      ...
      try:
        for i in self._getExplorer().getPatternRegex():
          lfCmd(
            """call win_execute(%d, "let matchid = matchadd('Lf_hl_gtagsHighlight', '%s', 9)")"""
            % (
              self._getInstance().getPopupWinId(),
              re.sub(r'\\(?!")', r"\\\\", escQuote(i)),
            )
          )
          id = int(lfEval("matchid"))
          self._match_ids.append(id)
      ...
      if self._getExplorer().getResultFormat() is None:
        ...
        id = int(
          lfEval(r"""matchadd('Lf_hl_gtagsFileName', '^\S\+\s\+\d\+\s\+\zs\S\+')""")
        )
        self._match_ids.append(id)
    ...
    if "--recall" not in self._arguments:
      ...
      if self._read_finished < 2:
        self._timer_id = lfEval(
          "timer_start(1, 'leaderf#Gtags#TimerCallback', {'repeat': -1})"
        )
    else:
      ...
      if instance.getWinPos() == "popup":
        ...
        lfCmd(
          "call nvim_win_set_option(%d, 'cursorline', v:true)"
          % instance.getPopupWinId()
        )
      else:
  ...
  def getArguments(self):
    if (
      self._getExplorer().getLastResultFormat() is not None
      and "--append" in self._arguments
    ):
