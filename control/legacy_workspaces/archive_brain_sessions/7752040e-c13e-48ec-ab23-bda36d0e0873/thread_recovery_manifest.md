...
*   **The Misfire:** We spent hours configuring the `mcp-toolbox-for-databases` to talk to a Postgres instance (`ShadowTag_governance`). We treated our vector memory like a Web 2.0 application database.
...
*   **The Re-Alignment:** We must wire `LiteLLM` into the `ShadowTag-v2-fastapi-services` deployment. `LiteLLM` becomes the singular proxy. When the agent needs a fast query, LiteLLM routes to Ollama. When we run a batch job of 10,000 documents, LiteLLM routes to vLLM. Zero code changes required on the backend.
...
We will define `LiteLLM` inside the Monorepo as the master AI router, intercepting all calls from `ShadowTag-v2-fastapi-services` and intelligently distributing them to local Inference engines.
