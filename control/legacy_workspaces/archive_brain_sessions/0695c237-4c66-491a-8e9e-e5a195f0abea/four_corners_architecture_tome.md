...
```python
async def push_to_ShadowTag-v2(matter_id: str, email_payload: dict):
    """
...
async def receive_email_pubsub(request: Request, background_tasks: BackgroundTasks):
    ...
    if data_b64:
        ...
        background_tasks.add_task(push_to_ShadowTag-v2, matter_id, email_payload)
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(
                self.edgar_rss_url,
                headers={"User-Agent": "ShadowTag-v2_Internal_HFLT_Agent admin@ShadowTag-v2.tech"}
            )
