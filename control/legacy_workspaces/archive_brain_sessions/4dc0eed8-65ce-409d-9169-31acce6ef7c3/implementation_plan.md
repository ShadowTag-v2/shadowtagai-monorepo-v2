...
The Commander requested the deployment of the "Absolute Master Payload," a multi-layered overarching architecture encompassing 18 precise script refactors across the system. We are dropping `n-autoresearch/Kosmos/BioAgents` and `CavMTOE` entirely in favor of an exclusive `kosmos` base.

## Proposed Changes

### 1. Delete Legacy Components

- Completely obliterate references to `n-autoresearch/Kosmos/BioAgents` in `/src/antigravity/pipeline.py`
