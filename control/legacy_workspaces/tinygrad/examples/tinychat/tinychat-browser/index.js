...
const normalizedParams = Object.fromEntries(
  [...queryParams].map(([key, value]) => [key.toUpperCase(), value.toUpperCase()]),
);
  ...
  return new Promise((resolve, reject) => {
    ...
    if (message.header === "token") worker.postMessage(message.data);
      ...
      if (message.data === "done") worker.postMessage(message.data);
        ...
        worker.postMessage(
          message.data,
          message.data.map((file) => file.bytes.buffer),
        );
...
async function load_state_dict(data, device, progress) {
  ...
  const getPart = async (filename, hash) => {
    const part = await readTensorFromDb(db, hash);
  ...
  const loadFileToStateDict = async (file) => {
    if (window.BACKEND === "WebGPU") {
      for (const part of file.parts) {
        ...
        part.bytes =
          part.size === file.bytes.length
            ? file.bytes
            : file.bytes.slice(part.file_start_pos, part.file_start_pos + part.size);
  ...
  if (window.BACKEND === "WebGPU") {
    // contiguous loading not needed for WebGPU stability
  ...
  for (const group of data.tensor_file_groups) {
    ...
    while (completed < files.length) {
      ...
      if (elapsed < loadDelay)
        await new Promise((resolve) => setTimeout(resolve, loadDelay - elapsed));
    }
    if (contiguous) {
      const orderMap = tensor_file_indices.reduce((acc, id, index) => {
        acc[id] = index;
        return acc;
      }, {});
  ...
  Alpine.data("state", () => ({
    ...
    async init() {
      ...
      window.MODEL_BASE_URL =
        window.BACKEND === "WebGPU" && !window.isMobile
          ? window.PC_MODEL_BASE_URL
          : window.MOBILE_MODEL_BASE_URL;
      this.max_context =
        window.BACKEND === "WebGPU" && !window.isMobile
          ? window.PC_MAX_CONTEXT
          : window.MOBILE_MAX_CONTEXT;

      const kernelsReady = (async () => {
        if (window.BACKEND === "WASM") {
          var exports = await import(`./net_clang.js?version=${Date.now()}`);
        } else if (window.BACKEND === "WebGPU" && !window.isMobile) {
          var exports = await import(`${PC_WEBGPU_EXPORT}?version=${Date.now()}`);
        } else if (window.BACKEND === "WebGPU" && window.isMobile) {
          var exports = await import(`${MOBILE_WEBGPU_EXPORT}?version=${Date.now()}`);
        }
      ...
      */
      ...
      const sortedEntries = Object.entries(state_dict).sort(
      ...
      for (const [k, v] of sortedEntries) {
        ...
        for (const part of v.parts) {
          part.key = k;
          if (part.empty)
            state_dict[k].empty = true; // assumes no other parts of this weight exist and are non-empty
          else {
        ...
        if (files_in_tensor.length > 1)
          data.tensor_file_groups.push({ contiguous: true, files: files_in_tensor }); // [tensorN_file0, tensorN_file1, ...]
      ...
      try {
        ...
        const tokenizer_works =
          new TextDecoder().decode(this.tokenizer.decode(this.tokenizer.encode("hello world"))) ===
          "hello world";
        console.log("tokenizer works:", tokenizer_works);
        this.progress(0.01 * totalSize);
      } catch (error) {
        this.progress(-1, `Error launching tokenizer: ${error}`);
        console.log(error);
        return;
      }

      try {
        const loadModelMessage = webgpuErrorMessage
          ? webgpuErrorMessage
          : `Loading ${window.BACKEND} model:`;
        ...
        if (window.BACKEND === "WebGPU") {
          ...
          this.nets = {
            transformer: async (tok, start_pos) =>
              sendMessageToWorker(model, { header: "token", data: [tok, start_pos] }),
          };
        }
        ...
        this.progress(-1, `Error launching model: ${error}`);
        console.log(error);
        return;
      }
    },
    ...
    async handleSend() {
      ...
      try {
        ...
        if (this.cancelGeneration && !this.maxContextReached)
    ...
    async *openaiChatCompletion(messages) {
      ...
      this.loadingMessage =
        window.BACKEND === "WebGPU" ? "Reading input:" : "Loading (enable WebGPU for speed):";
      ...
      for (const tok of unprocessedPrefillToks) {
        ...
        if (window.BACKEND === "WebGPU") {
          ...
          await this.nets["transformer"](tok, startPos);
        }
      ...
      while (true) {
        if (window.BACKEND === "WebGPU") {
          var tok = await this.nets["transformer"](
            new Int32Array([lastTok]),
            new Int32Array([startPos]),
          );
          tok = tok[0][0];
        } else {
          var tok = await this.nets["transformer"](lastTok, startPos);
        }
...
async function createTokenizer(bpeUrl) {
  ...
  return {
    ...
    get stop_tokens() {
      return new Set([special_tokens["<|end_of_text|>"], special_tokens["<|eot_id|>"]]);
    },
