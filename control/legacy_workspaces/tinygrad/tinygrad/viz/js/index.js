...
const displaySelection = (sel) => {
  for (const e of document.getElementsByClassName("view"))
    e.style.display = e.matches(sel) ? "flex" : "none";
};
...
const parseColors = (name, defaultColor = "#ffffff") =>
  Array.from(
    name.matchAll(/(?:\u001b\[(\d+)m([\s\S]*?)\u001b\[0m)|([^\u001b]+)/g),
    ([_, code, colored_st, st]) => ({
      st: colored_st ?? st,
      color:
        code != null
          ? (code >= 90 ? ANSI_COLORS_LIGHT : ANSI_COLORS)[(parseInt(code) - 30 + 60) % 60]
          : defaultColor,
    }),
  );
...
const colored = (n) =>
  d3
    ...
    .call((s) =>
      s
        .selectAll("span")
        .data(typeof n === "string" ? parseColors(n) : n)
        .join("span")
...
const updateProgress = ({ start, err }) => {
  ...
  if (err) {
    ...
    d3.select("#custom")
      .append("div")
      .classed("raw-text", true)
      .call((s) => s.append(() => codeBlock(err, "txt")))
      .node();
  }
};
...
const drawGraph = (data) => {
  ...
  const nodes = d3
    ...
    .selectAll("g")
    .data(
      g.nodes().map((id) => g.node(id)),
      (d) => d,
    )
    .join("g")
    .attr("class", (d) => d.className ?? "node")
    .attr("transform", (d) => `translate(${d.x},${d.y})`)
    .classed("clickable", (d) => d.ref != null)
    .on("click", (e, d) => {
      ...
      nodes
        .classed("highlight", (n) => src.includes(n.id))
        .classed("child", (n) => children.includes(n.id));
      const matchEdge = (v, w) =>
        v === d.id && children.includes(w)
          ? "highlight child "
          : parents.includes(v) && w === d.id
            ? "highlight "
            : "";
      d3.select("#edges")
        .selectAll("path.edgePath")
        .attr("class", (e) => matchEdge(e.v, e.w) + "edgePath");
      d3.select("#edge-labels")
        .selectAll("g.port")
        .attr("class", (_, i, n) => matchEdge(...n[i].id.split("-")) + "port");
      e.stopPropagation();
    });
  nodes
    .selectAll("rect")
  ...
  if (hasLabelDims)
    labels.attr(
      "transform",
      (d) => `translate(-${d.labelWidth / 2}, -${d.labelHeight / 2 + STROKE_WIDTH * 2})`,
    );
  labels
    .selectAll("text")
    .data((d) => {
    ...
    .data((d) => d)
    .join("tspan")
    .attr("fill", (d) => darkenHex(d.color, 25))
    .text((d) => d.st)
...
async function initWorker() {
  const resp = await Promise.all(
    ["/assets/dagrejs.github.io/project/dagre/latest/dagre.min.js", "/js/worker.js"].map((u) =>
      fetch(u),
    ),
  );
  workerUrl = URL.createObjectURL(
    new Blob([(await Promise.all(resp.map((r) => r.text()))).join("\n")], {
  ...
  worker.onmessage = (e) => {
    ...
    addTags(
      d3
        ...
        .attr("class", (e) => e.value.label.type)
        .attr("id", (e) => `${e.v}-${e.w}`)
        .datum((e) => e.value.label.text),
    );
...
function tabulate(rows) {
  const root = d3
    .create("div")
    .style("display", "grid")
    .style("grid-template-columns", `${Math.max(...rows.map((x) => x[0].length), 0)}ch 1fr`)
    .style("gap", "0.2em");
  for (const [k, v] of rows) {
    root.append("div").text(k);
    root.append("div").node().append(v);
  }
  return root;
}
...
function getMetadata(key) {
  ...
  if (eventType === EventTypes.EXEC) {
    ...
    html.append(() =>
      tabulate([
        ["Name", d3.create("p").html(n).node()],
        ["Duration", formatTime(e.width)],
        ["Start Time", formatTime(e.x)],
      ]).node(),
    );
    ...
    for (const b of e.arg.bufs.sort((a, b) => a.num - b.num)) {
      group
        .append("p")
        .text(`${Modes[b.mode]}@data${b.num} ${formatUnit(b.nbytes, "B")}`)
        .style("cursor", "pointer")
        .on("click", () => {
          const row = document.getElementById(b.k);
          if (!isExpanded(row)) {
            row.click();
          }
          focusShape(b.key);
    ...
    if (e.arg.ctx != null) {
      ...
      html
        .append("a")
        .text(ctxs[i + 1].steps[s].name)
        .on("click", () => switchCtx(i, s));
      const prgSrc = ctxs[i + 1].steps.findIndex((s) => s.name === "View Program");
      if (prgSrc !== -1)
        html
          .append("a")
          .text("View program")
          .on("click", () => switchCtx(i, prgSrc));
    }
  }
  if (eventType === EventTypes.BUF) {
    ...
    for (let u = 0; u < e.arg.users?.length; u++) {
      ...
      const p = kernels
        .append("p")
        .append(() => colored(`[${u}] ${repr} ${Modes[mode]}@data${num}`));
...
async function renderProfiler(path, unit, opts) {
  ...
  let offset = 0;
  const u8 = () => {
    const ret = view.getUint8(offset);
    offset += 1;
    return ret;
  };
  const u32 = () => {
    const ret = view.getUint32(offset, true);
    offset += 4;
    return ret;
  };
  const u64 = () => {
    const ret = new Number(view.getBigUint64(offset, true));
    offset += 8;
    return ret;
  };
  const f32 = () => {
    const ret = view.getFloat32(offset, true);
    offset += 4;
    return ret;
  };
  ...
  const { strings, dtypeSize, markers } = JSON.parse(
    textDecoder.decode(new Uint8Array(buf, offset, indexLen)),
  );
  offset += indexLen;
  // place devices on the y axis and set vertical positions
  const [tickSize, padding, baseOffset] = [10, 8, markers.length ? 14 : 0];
  const deviceList = profiler
    .append("div")
    .attr("id", "device-list")
    .style("padding-top", tickSize + padding + baseOffset + "px");
  const canvas = profiler.append("canvas").attr("id", "timeline").node();
  // NOTE: scrolling via mouse can only zoom the graph
  canvas.addEventListener("wheel", (e) => (e.stopPropagation(), e.preventDefault()), {
    passive: false,
  });
  ...
  const heightScale = d3
    .scaleLinear()
    .domain([0, tracePeak])
    .range([4, (maxheight = 100)]);
  for (let i = 0; i < layoutsLen; i++) {
    const nameLen = view.getUint8(offset, true);
    offset += 1;
    const k = textDecoder.decode(new Uint8Array(buf, offset, nameLen));
    offset += nameLen;
    const div = deviceList
      .append("div")
      .attr("id", k)
      .text(k)
    ...
    if (eventType === EventTypes.EXEC) {
      ...
      for (let j = 0; j < eventsLen; j++) {
        const e = {
          ...
          st: u32(),
          dur: f32(),
          info: strings[u32()] || null,
        };
        // find a free level to put the event
        let depth = 0;
        if (opts.levelKey != null) {
          depth = opts.levelKey(e);
          levels[depth] = 0;
        } else {
        ...
        if (!colorMap.has(colorKey)) {
          const color =
            colors instanceof Map
              ? colors.get(colorKey) || colors.get("DEFAULT")
              : cycleColors(colors, colorMap.size);
          colorMap.set(colorKey, d3.rgb(color));
        }
        const fillColor = colorMap
          .get(colorKey)
          .brighter(0.3 * depth)
          .toString();
        const label = parseColors(e.name).map(({ color, st }) => ({
          color,
          st,
        ...
        let shapeRef = e.ref;
        if (shapeRef != null) {
          ref = { ctx: e.ref, step: 0 };
          shapeRef = ref;
        } else if (ref != null) {
          const start = ref.step > 0 ? ref.step + 1 : 0;
          const stepIdx = ctxs[ref.ctx + 1].steps.findIndex(
            (s, i) => i >= start && s.name == e.name,
          );
          if (stepIdx !== -1) {
            ref.step = stepIdx;
            shapeRef = ref;
          ...
          for (let i = state.currentStep + 1; i < steps.length; i++) {
            ...
            if (loc == null) break;
            if (loc === e.name) {
              shapeRef = { ctx: state.currentCtx - 1, step: i };
              break;
            }
          }
        }
        ...
        const labelHTML = label
          .map((l) => `<span style="color:${l.color}">${l.st}</span>`)
          .join("");
        const arg = {
          tooltipText: labelHTML + "\n" + formatTime(e.dur) + (e.info != null ? "\n" + e.info : ""),
          bufs: [],
          key,
          ctx: shapeRef?.ctx,
          step: shapeRef?.step,
        ...
        shapes.push({
          ...
          height: levelHeight,
          arg,
          label: opts.hideLabels ? null : label,
          fillColor,
        });
        if (j === 0) data.first = data.first == null ? e.st : Math.min(data.first, e.st);
      }
      div
        .style("height", levelHeight * levels.length + padding + "px")
        .style("pointerEvents", "none");
    } else {
      const peak = u64();
      let x = 0,
        y = 0;
      const buf_shapes = new Map(),
        temp = new Map();
      const timestamps = [],
        valueMap = new Map();
      for (let j = 0; j < eventsLen; j++) {
        const alloc = u8(),
          ...
          key = u32();
        if (alloc) {
          const dtype = strings[u32()],
            sz = u64(),
            nbytes = dtypeSize[dtype] * sz;
          const shape = { x: [x], y: [y], dtype, sz, nbytes, key };
          buf_shapes.set(key, shape);
          temp.set(key, shape);
          timestamps.push(ts);
          x += 1;
          y += nbytes;
          ...
          free.users = Array.from({ length: u32() }, () => ({
            shape: shapeMap.get(u32()),
            repr: strings[u32()],
            num: u32(),
            mode: u8(),
          }));
      ...
      for (const [num, { dtype, sz, nbytes, y, x: steps, users }] of buf_shapes) {
        ...
        shapes.push({
          x,
          y0: y.map(yscale),
          y1: y.map((y0) => yscale(y0 + nbytes)),
          arg,
          fillColor: cycleColors(colorScheme.BUFFER, shapes.length),
        });
        users?.forEach((u) =>
          selectShape(u.shape).e?.arg.bufs.push({
            key: arg.key,
            nbytes,
            num: u.num,
            mode: u.mode,
      ...
      for (const sh of shapes) {
        for (let i = 0; i < sh.x.length - 1; i++) {
          const startIdx = idxs.get(sh.x[i]),
            endIdx = idxs.get(sh.x[i + 1]);
          const shapeY = sh.y1[i];
          for (let k = startIdx; k < endIdx; k++) {
            const x = allX[k];
            maxY.set(x, Math.min(maxY.get(x), shapeY));
          }
        }
      }
      ...
      for (let i = 0; i < allX.length - 1; i++) {
        ...
        const y = maxY.get(allX[i]);
        sum.y1.push(y, y);
        sum.y0.push(base0, base0);
      }
      if (timestamps.length > 0)
        data.first = data.first == null ? timestamps[0] : Math.min(data.first, timestamps[0]);
      data.tracks.set(k, {
        shapes: [sum],
        eventType,
        visible,
        ...
        height,
        peak,
        scaleFactor: (maxheight * 4) / height,
        views: [[sum], shapes],
        valueMap,
      });
      div
        ...
        .on("click", (e) => {
          ...
          for (const [tid, track] of data.tracks) {
            ...
            if (tid === newFocus) {
              track.shapes = track.views[1];
              offset += rescaleTrack(track, tid, track.scaleFactor);
            } else if (tid === focusedDevice) {
              track.shapes = track.views[0];
              offset += rescaleTrack(track, tid, 1 / track.scaleFactor);
            }
          }
          data.axes.y =
            newFocus != null
              ? {
                  domain: [0, (t = data.tracks.get(newFocus)).peak],
                  range: [t.offsetY + t.height, t.offsetY],
                  fmt: "B",
                }
              : null;
          toggleCls(
            document.getElementById(focusedDevice),
            document.getElementById(newFocus),
            "expanded",
          );
  ...
  for (const m of markers)
    m.label = m.name
      .split(/(\s+)/)
      .map((st) => ({ st, color: m.color, width: ctx.measureText(st).width }));
  updateProgress({ start: false });
      ...
      for (const e of shapes) {
        ...
        if (eventType === EventTypes.BUF) {
          ...
          for (let i = 1; i < x.length; i++) {
            ...
            let arg = e.arg;
            if (arg == null && valueMap != null)
              arg = { tooltipText: `Total: ${formatUnit(valueMap.get(e.x[i - 1]), "B")}` };
            visible.push({
              x0: x[i - 1],
              x1: x[i],
              y0: offsetY + e.y1[i - 1],
        ...
        if (focusedShape != null && e.arg?.key === focusedShape) {
          paths.push([p, pcolor]);
        }
      }
    }
  ...
  function resize() {
    ...
    const width = profiler.clientWidth - (sideRect.width + padding),
  ...
  const clickShape = (e) => {
    ...
    if (foundRect?.step != null && (foundRect?.key == null || e.type == "dblclick")) {
      return switchCtx(foundRect.ctx, foundRect.step);
    }
    if (foundRect?.key != focusedShape) {
      focusShape(foundRect?.key);
    }
  };
...
const vizZoomFilter = (e) =>
  (!e.ctrlKey || e.type === "wheel" || e.type === "mousedown") &&
  !e.button &&
  e.type !== "dblclick";
const svgZoom = d3
  .zoom()
  .filter(vizZoomFilter)
  .on("zoom", (e) => d3.select("#render").attr("transform", e.transform));
...
document.getElementById("zoom-to-fit-btn").addEventListener("click", () => {
  ...
  const R = {
    x: x0 + pad,
    y: mainRect.top + pad,
    width: (x1 > 0 ? x1 - x0 : mainRect.width) - 2 * pad,
    height: mainRect.height - 2 * pad,
  };
  const r = rect("#render");
  if (r.width === 0) return;
  const scale = Math.min(R.width / r.width, R.height / r.height);
  const [tx, ty] = [
    R.x + (R.width - r.width * scale) / 2 - r.left * scale,
    R.y + (R.height - r.height * scale) / 2,
  ];
...
const pathLink = (fp, lineno) =>
  d3
    .create("a")
    .attr("href", "vscode://file/" + fp + ":" + lineno)
    .text(`${fp.split("/").at(-1)}:${lineno}`);
...
hljs.registerLanguage("cpp", (hljs) => ({
  ...
  contains: [
    { begin: "\\b(?:float|half)[0-9]+\\b", className: "type" },
    ...hljs.getLanguage("cpp").contains,
  ],
}));

async function fetchValue(path) {
  const res = await fetch(path);
  return await (res.headers.get("content-type") === "application/json"
    ? res.json()
    : res.arrayBuffer());
}
...
async function main() {
  ...
  if (ctxs == null) {
    ...
    for (const [i, { name, steps }] of ctxs.entries()) {
      ...
      p.onclick = () => {
        setState(
          i === state.currentCtx
            ? { expandSteps: !state.expandSteps }
            : { expandSteps: true, currentCtx: i, currentStep: 0, currentRewrite: 0 },
        );
      };
      const stack = [];
      const list = ul;
      ...
      for (const l of ul.querySelectorAll("ul > ul > p")) {
        ...
        if (subrewrites.length > 0) {
          l.appendChild(d3.create("span").text(` (${subrewrites.length})`).node());
          l.parentElement.classList.add("has-children");
        }
      }
    }
  ...
  if (ctx.name === "Profiler")
    return renderProfiler("/get_profile", "realtime", { width: "132px" });
  ...
  if (!ckey.startsWith("/rewrites")) {
    ...
    if (ret instanceof ArrayBuffer) {
      opts = {
        heightScale: 0.5,
        hideLabels: true,
        levelKey: (e) => parseInt(e.name.split(" ")[1].split(":")[1]),
      };
        ...
        for (const [i, value] of r.entries()) {
          ...
          if (value.cols != null) {
            ...
            tr.on("click", () => {
              ...
              if (el?.classList.contains("nested-row")) {
                tr.classed("expanded", false);
                return el.remove();
              }
              tr.classed("expanded", true);
              const td = table
                .insert("tr", () => tr.node().nextSibling)
                .classed("nested-row", true)
                .append("td");
          ...
          value.forEach(([k, v, width]) =>
            bar
              .append("div")
              .style("width", width + "%")
              .attr("title", `${ret.cols[i].labels[k]} ${v}`)
      ...
      metadata.appendChild(
        tabulate(
          ret.summary.map((s) => {
            const div = d3
              .create("div")
              .style("background", cycleColors(colorScheme.CATEGORICAL, s.idx))
              .style("width", "100%")
              .style("height", "100%");
  ...
  const render = (opts) =>
    renderDag(
      ret[currentRewrite].graph,
      ret[currentRewrite].changed_nodes ?? [],
      currentRewrite === 0,
      opts,
    );
  ...
  metadata.replaceChildren(
    toggleLabel,
    codeBlock(step.code_line, "python", { loc: step.loc, wrap: true }),
    codeElement,
  );
  if (step.trace) {
    ...
    for (let i = step.trace.length - 1; i >= 0; i--) {
      ...
      trace
        ...
        .style("display", "flex")
        .text(fn + " ")
        .append(() => pathLink(fp, lineno).node());
      trace
        .append("div")
  ...
  if (step.match_count >= 1) {
    ...
    for (let s = 0; s <= step.match_count; s++) {
      ...
      if (s > 0 && s === currentRewrite) {
        ...
        const diffCode = metadata
          .appendChild(document.createElement("pre"))
          .appendChild(document.createElement("code"));
        for (const line of diff) {
          diffCode.appendChild(
            colored([
              {
                st: line,
...
function appendResizer(element, { minWidth, maxWidth }, left = false) {
  const handle = Object.assign(document.createElement("div"), {
    ...
    style: left ? "right: 0" : "left: 0; margin-top: 0",
  });
  element.appendChild(handle);
  const resize = (e) => {
    const change = e.clientX - element.dataset.startX;
    const newWidth =
      ((Number(element.dataset.startWidth) + (left ? change : -change)) /
        Number(element.dataset.containerWidth)) *
      100;
...
appendResizer(
  document.querySelector(".ctx-list-parent"),
  { minWidth: 15, maxWidth: 50 },
  (left = true),
);
...
const select = (ctx, step) => ({
  ctx: document.getElementById(`ctx-${ctx}`),
  step: document.getElementById(`step-${ctx}-${step}`),
});
const deselect = (element) => {
  const parts = element?.id.split("-").map(Number);
  return element?.id.startsWith("ctx")
    ? { ctx: parts[1], step: null }
    : element?.id.startsWith("step")
      ? { ctx: parts[1], step: parts[2] }
      : {};
};
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key == "ArrowUp") {
    ...
    if (changeStep) {
      ...
      return (
        prev.step != null &&
        !isExpanded(step) &&
        setState({ currentRewrite: 0, currentStep: prev.step })
      );
    }
    return setState({
      currentStep: 0,
      currentRewrite: 0,
      currentCtx: Math.max(0, currentCtx - 1),
      expandSteps: false,
    });
  }
  if (event.key == "ArrowDown") {
    ...
    return setState({
      currentStep: 0,
      currentRewrite: 0,
      currentCtx: Math.min(ctxs.length - 1, currentCtx + 1),
      expandSteps: false,
    });
  }
