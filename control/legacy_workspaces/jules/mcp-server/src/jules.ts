...
import { type ExecFileException, exec, execFile } from "child_process";
import { z } from "zod";
...
export async function startNewJulesTask(
  ...
  return new Promise((resolve) => {
    dependencies.execFile(
      "jules",
      ["remote", "new", "--repo", repo_name, "--session", user_task_description],
      { encoding: "utf8" },
...
server.tool(
  ...
  (input: { repo_name: string; user_task_description: string }) =>
