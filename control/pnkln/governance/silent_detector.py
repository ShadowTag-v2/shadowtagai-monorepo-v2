...
_UNAUTHORIZED_PRACTICE_PATTERNS: list[re.Pattern[str]] = [
  ...
  re.compile(
    r"you\s+(?:have|suffer\s+from|are\s+diagnosed\s+with)\s+(?!a\s+(?:right|question))",
    re.I,
  ),
  re.compile(
    r"(?:prescribe|prescribed|prescribing)\s+(?:you\s+)?\w+\s+(?:mg|ml|tablets)", re.I
  ),
]

_RAISE_ACT_PATTERNS: list[re.Pattern[str]] = [
  re.compile(
    r"(?:bypass|disable|remove|circumvent)\s+(?:safety|guardrail|filter|alignment)",
    re.I,
  ),
  re.compile(
    r"(?:generate|create|produce)\s+(?:bioweapon|biological\s+weapon|chemical\s+weapon)",
    re.I,
  ),
]
...
class SilentDetector:
  ...
  def __init__(
    self, engine: Cor_Claude_Code_6Engine, agent_id: str = "silent-detector"
  ) -> None:
    self.engine = engine
  ...
  def _detect_violations(self, content: str) -> list[tuple[ViolationType, str]]:
    ...
    for p in _UNAUTHORIZED_PRACTICE_PATTERNS:
      if m := p.search(content):
        hits.append(
          (ViolationType.LEGAL_NY_S7263_UNAUTHORIZED_PRACTICE, m.group(0)[:80])
        )
