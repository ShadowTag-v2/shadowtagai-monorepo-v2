...
class LegalMitigation(BaseMitigation):
  ...
  def apply(
    ...
    return MitigationControl(
      ...
      framework_ref=", ".join(
        VIOLATION_FRAMEWORK_MAP.get(event.violation_type, {}).get("refs", [])
      ),
...
class CyberMitigation(BaseMitigation):
  ...
  def apply(
    ...
    return MitigationControl(
      ...
      enforcement_level=enforcement_level,
      action_taken=(
        "Cyber threat detected. Apply input sanitization, rate-limit session, log to SIEM with full request context."
      ),
      automated=True,
      ceo_notified=enforcement_level >= EnforcementLevel.L4_CONTAIN,
      user_locked_out=enforcement_level >= EnforcementLevel.L5_LOCKOUT,
...
class OperationalMitigation(BaseMitigation):
  ...
  def apply(
    ...
    return MitigationControl(
      ...
      enforcement_level=enforcement_level,
      action_taken=(
        "Operational anomaly detected. Apply output filtering, escalate to human review if repeated within session."
      ),
      automated=True,
      ceo_notified=False,
...
class Cor_Claude_Code_6Engine:
  ...
  def evaluate_chain(self, commands: list[str]) -> GovernanceDecision:
    ...
    encoding_detected = any(
      ...
      for cmd in self._session_chain
    ...
    if chain_exceeded or encoding_detected:
      ...
      event = RiskEvent(
        ...
        context={
          "chain_length": len(self._session_chain),
  ...
  def evaluate(self, event: RiskEvent) -> GovernanceDecision:
    ...
    mitigator = _FRAMEWORK_MITIGATIONS.get(
      primary_framework, _FRAMEWORK_MITIGATIONS["OPERATIONAL"]
    )
    ...
    return json.dumps(
      {
        ...
        "enforcement_level": decision.control.enforcement_level.value
        if decision.control
        else None,
