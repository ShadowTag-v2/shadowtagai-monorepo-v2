...
class WealthAnalysis:
  ...
  def generate_report(self) -> str:
    ...
    if self.redesigns:
      for redesign in sorted(
        self.redesigns, key=lambda r: r.projected_gain, reverse=True
      ):
...
class WealthOptimizer:
  ...
  def __init__(
    self, target_tier_1_ratio: float = 0.40, target_cost_per_item: float = 0.015
  ):
    """
    ...
    logger.info(
      f"WealthOptimizer initialized: target_tier_1={target_tier_1_ratio:.1%}, target_cost={target_cost_per_item:.3f}"
    )
  ...
  async def analyze(
    ...
    analysis.net_monthly_improvement = (
      analysis.total_projected_gain - analysis.total_leak_cost
    )
  ...
  async def _propose_redesigns(
    self, result: Any, leaks: list[Leak]
  ) -> list[RedesignProposal]:
