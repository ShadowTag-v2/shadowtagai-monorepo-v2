...
class CheatSheetFusion:
  ...
  def create_variant(
    ...
    logger.info(
      f"Created variant {variant_id}: gen={self.generation}, parent={parent_id}, mutations={mutations}"
    )
  ...
  async def test_variant(
    ...
    if self.baseline_accuracy is None:
      ...
      logger.info(
        f"Accuracy gain: {gain:+.1%} (total: {self.total_accuracy_gain:+.1%}, target: +3.7%)"
      )
  ...
  def evolve(self, direction: str = "improve") -> str:
    ...
    logger.info(
      f"Evolved from {parent_id} → {new_variant_id} (direction={direction}, gen={self.generation})"
    )
