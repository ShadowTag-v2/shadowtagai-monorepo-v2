...
class DeadlineMath:
  ...
  local_holiday_set: frozenset[datetime.date] = field(
    default_factory=lambda: _FEDERAL_HOLIDAYS
  )
...
class JurisdictionEngine:
  ...
  def calculate(
    ...
    if math.business_days_only:
      return self._add_business_days(
        trigger_date, math.add_days, math.local_holiday_set
      )
  ...
  def resolve_rule(
    ...
    for rule in FRCP_RULES:
      if (
        rule.trigger_event_type == trigger_event_type
