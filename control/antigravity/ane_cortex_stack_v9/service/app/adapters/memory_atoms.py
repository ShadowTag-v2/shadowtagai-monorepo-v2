...
def search_atoms(
  pg_dsn: str, repo_id: str, query: str, limit: int = 12
) -> list[dict[str, Any]]:
  ...
  for r in rows:
    hay = " ".join(
      [str(r[1]), str(r[2]), str(r[3]), str(r[4]), " ".join(r[7] or [])]
    ).lower()
...
def replace_authority_atoms(pg_dsn: str, repo_id: str, authority: dict):
  with pg_conn(pg_dsn) as conn:
    cur = conn.cursor()
    cur.execute(
      "DELETE FROM memory_atoms WHERE repo_id = %s AND source_type = 'authority'",
      (repo_id,),
    )
  inserted = 0
