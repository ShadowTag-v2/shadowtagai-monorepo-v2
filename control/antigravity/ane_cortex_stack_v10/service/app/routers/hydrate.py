...
def hydrate_pack():
    ...
    with pg_conn(s.postgres_dsn) as conn:
        ...
        cur.execute("SELECT rel_path, drift_kind, expected, observed, severity, suggested_fix FROM drift_reports WHERE repo_id = %s ORDER BY created_at DESC LIMIT 12", (s.repo_id,))
        drifts = [{
            "rel_path": r[0], "drift_kind": r[1], "expected": r[2], "observed": r[3], "severity": r[4], "suggested_fix": r[5]
        } for r in cur.fetchall()]
        cur.execute("SELECT observed_root, canonical_root, severity FROM repo_root_conflicts WHERE repo_id = %s ORDER BY created_at DESC LIMIT 12", (s.repo_id,))
        root_conflicts = [{"observed_root": r[0], "canonical_root": r[1], "severity": r[2]} for r in cur.fetchall()]
    atoms = search_atoms(s.postgres_dsn, s.repo_id, "formatter ane fallback startup memory authority current standards settings monorepo canonical roots", limit=24)
    return {
        "repo_id": s.repo_id,
        "authority": authority,
