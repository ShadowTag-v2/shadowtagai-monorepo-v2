...
def detect_repo_root_conflicts():
  ...
  for p in [Path("./apps/ShadowTag-v2_stack"), Path("./apps")]:
    if p.exists():
      for child in p.rglob("*"):
        if child.is_dir():
          rel = str(child).replace("\\", "/")
          if rel.startswith("apps/ShadowTag-v2_stack/"):
            observed.add(rel)

  conflicts = []
  for root in observed:
    if root.startswith("apps/ShadowTag-v2_stack/") and root not in canonical:
      conflicts.append(
        {
          "observed_root": root,
