...
def search_category_by_type(category_type: str) -> dict:
    ...
    try:
        ...
        if result:
            ...
            print(f"[INFO] Debug: Potential match '{best_match}' found with score {score}")

            # Compare the threshold
            if score >= FUZZY_SCORE_THRESHOLD:
                print(f"[INFO] Result: Closest category found: '{best_match}' (Score: {score})")
                return {
                    "status": "found",
                ...
                print(f"[INFO] Result: No similar category found for '{category_type}' (Highest score {score} < {FUZZY_SCORE_THRESHOLD})")
                return {
                    "status": "not_found",
                    "input_query": category_type,
            ...
            print(f"[INFO] Result: No similar category found for '{category_type}' (extractOne returned None)")
            return {
                "status": "not_found",
                "input_query": category_type,
        ...
        print(f"[ERROR] Exception during search_category_by_type execution - {e}\n{traceback.format_exc()}")
...
def search_category_by_type_all() -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Exception during search_category_by_type_all execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def search_ingredient_by_type_all() -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Exception during search_ingredient_by_type_all execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def search_ingredient_by_brand(product_type: str, brand: str = None) -> dict:
    ...
    try:
        ...
        if filtered_data.empty:
            ...
            result = process.extractOne(product_type_lower, [p.lower() for p in all_products])
            if result and result[1] >= FUZZY_SCORE_THRESHOLD:
                matched_product = all_products[[p.lower() for p in all_products].index(result[0])]
        ...
        if brand:
            ...
            if brand_filtered_data.empty:
                ...
                result = process.extractOne(brand_lower, [b.lower() for b in available_brands])
                if result:
                    best_match, score = result
                    if score >= FUZZY_SCORE_THRESHOLD:
                        matched_brand = available_brands[[b.lower() for b in available_brands].index(best_match)]
                        brand_filter = filtered_data["product_brand"] == matched_brand
                        brand_filtered_data = filtered_data[brand_filter]
                        print(f"[INFO] Using fuzzy matched brand: {matched_brand}")
                    else:
                        print(f"[INFO] Brand '{brand}' score too low, suggesting best match '{best_match}' (score {score})")
                        return {
                            "status": "not_found",
        ...
        print(f"[ERROR] Exception during search_ingredient_by_brand execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def search_ingredient_by_rating(product_type: str, min_rating: float = 0.0) -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Exception during search_ingredient_by_rating execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def search_ingredient_by_price(product_type: str, max_price: float = None) -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Exception during search_ingredient_by_price execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}


@tool
def search_ingredient_by_review(product_type: str = None, min_reviews: int = 0, category_type: str = None) -> dict:
    """
    ...
    try:
        ...
        if product_type:
            product_type_lower = product_type.strip().lower()
            product_filter = filtered_data["product_type"].str.lower() == product_type_lower
        ...
        if category_type:
            ...
            if category_type_lower not in [cat.lower() for cat in available_categories]:
                # Try fuzzy matching
                result = process.extractOne(category_type_lower, [cat.lower() for cat in available_categories])
                if result and result[1] >= FUZZY_SCORE_THRESHOLD:
            ...
            category_filter = filtered_data["category_type"].str.lower() == category_type_lower
        ...
        print(f"[ERROR] Exception during search_ingredient_by_review execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def compare_ingredient_by_rating(product_type: str) -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Exception during compare_ingredient_by_rating execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def compare_ingredient_by_price(product_type: str) -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Exception during compare_ingredient_by_price execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def compare_ingredient_by_review(product_type: str) -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Exception during compare_ingredient_by_review execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def add_to_cart(product_type: str, brand: str, quantity: int = 1) -> dict:
    ...
    try:
        ...
        print(f"[INFO] Added to cart: {quantity} {product['product_brand']} {product['product_type']}")
        return {
            "status": "success",
        ...
        print(f"[ERROR] Exception during add_to_cart execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def remove_from_cart(product_type: str, brand: str = None) -> dict:
    ...
    try:
        ...
        print(f"[INFO] Removed from cart: {product_type} (Brand: {brand if brand else 'Any'})")
        return {
            "status": "success",
        ...
        print(f"[ERROR] Exception during remove_from_cart execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def modify_cart(product_type: str, brand: str, quantity: int) -> dict:
    ...
    try:
        ...
        print(f"[INFO] Modified cart: {product_type} from {brand}, new quantity: {quantity}")
        return {
            "status": "success",
        ...
        print(f"[ERROR] Exception during modify_cart execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def clear_cart() -> dict:
    ...
    try:
        ...
        print(f"[ERROR] Exception during clear_cart execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def help() -> dict:
    ...
    try:
        ...
        print("[INFO] Help information provided")
        return {"status": "success", "help_info": help_info}

    except Exception as e:
        print(f"[ERROR] Exception during help execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def greeting() -> dict:
    ...
    try:
        ...
        if data is not None and not data.empty:
            ...
            for category in available_categories[:3]:  # Limit to 3 categories
                ...
                if not category_data.empty:
                    ...
                    top_product = category_data.sort_values(by="product_rating", ascending=False).iloc[0].to_dict()
        ...
        print(f"[INFO] Greeting provided with {len(featured_products)} featured products")
        return {"status": "success", "greeting_info": greeting_info}

    except Exception as e:
        print(f"[ERROR] Exception during greeting execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
...
def fallback() -> dict:
    ...
    try:
        ...
        print("[INFO] Fallback provided with enhanced English message and suggestions.")
        return {"status": "success", "fallback_info": fallback_info}

    except Exception as e:
        print(f"[ERROR] Exception during fallback execution - {e}\n{traceback.format_exc()}")
        return {"status": "error", "message": str(e)}
