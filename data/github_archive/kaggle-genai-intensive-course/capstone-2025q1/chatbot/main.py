...
try:
    ...
    print("[ERROR] Please ensure all chatbot modules (llm, tools, graph, etc.) are correctly initialized.")
...
def run_chat():
    ...
    while True:
        try:
            ...
            for event in app.stream(current_state):
                ...
                if "update_cart" in event:
                    ...
                    if "cart_items" in current_state:
                        print(f"[DEBUG] Cart items updated: {len(current_state['cart_items'])} items")
            ...
            if tool_calls_made:
                print(f"[DEBUG INFO] Tool Calls made during this turn: {tool_calls_made}")
                sys.stdout.flush()
