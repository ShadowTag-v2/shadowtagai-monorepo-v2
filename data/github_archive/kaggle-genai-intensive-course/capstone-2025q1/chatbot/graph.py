...
def agent_node(state: State):
    ...
    if not llm_with_tools:
        ...
        return {"messages": [AIMessage(content="Sorry, I cannot process your request right now due to an internal error.")]}
    # Call the LLM
...
def view_cart_node(state: State):
    ...
    try:
        ...
        if not state or "cart_items" not in state or not state["cart_items"]:
            ...
            print(f"[INFO] Found {len(cart_items)} unique item types ({item_count} total items) in cart, total: ${total_price:.2f}")
            cart_result = {
                "status": "success",
        ...
        print(f"[ERROR] Exception during view_cart_node execution - {e}\n{traceback.format_exc()}")
    ...
    if isinstance(last_message, AIMessage) and last_message.tool_calls:
        ...
        if view_cart_call:
            ...
            print(f"[WARNING] view_cart_node reached but no 'view_cart' tool call found in last AI message. Tool calls: {last_message.tool_calls}")
            # Use default ID or empty ID
            tool_call_id = last_message.tool_calls[0].get("id", "") if last_message.tool_calls else ""
    ...
    if cart_result.get("status") == "success":
        items_str = ", ".join([f"{item['quantity']}x {item['product']}" for item in cart_result.get("cart_items", [])])
        result_content += f"Items ({cart_result.get('item_count', 0)} total): [{items_str}]\n"
    ...
    return {"messages": [ToolMessage(content=result_content, tool_call_id=tool_call_id)]}
    ...
    if isinstance(last_message, AIMessage) and last_message.tool_calls:
        ...
        if last_message.tool_calls:
            ...
            if first_tool_call_name == "view_cart":
                print("[INFO] Decision: 'view_cart' call required -> Routing to view_cart_node")
...
def update_cart_node(state):
    ...
    if isinstance(last_message, ToolMessage):
        ...
        print(f"[DEBUG] update_cart_node: Last message is not ToolMessage ({type(last_message)}), skipping cart update based on tool output.")
    ...
    for msg in reversed(updated_state["messages"][:-1]):
        if isinstance(msg, AIMessage) and hasattr(msg, "tool_calls") and msg.tool_calls:
            for tc in msg.tool_calls:
                if tc.get("id") == tool_output_message.tool_call_id:
                    tool_call_info = tc
                    print(f"[DEBUG] Found corresponding tool call in history: {tool_call_info}")
                    break
            if tool_call_info:
                break
        ...
        if not tool_call_info:
            ...
            ai_messages = [msg for msg in updated_state["messages"][:-1] if isinstance(msg, AIMessage)]
            tool_messages = [msg for msg in updated_state["messages"][:-1] if isinstance(msg, ToolMessage)]
            if ai_messages and tool_messages:
                last_ai = ai_messages[-1]
                # Fallback: try to match ToolMessages and tool_calls by tool_call_id
                idx = None
                for i, msg in enumerate(tool_messages):
                    if getattr(msg, "tool_call_id", None) == getattr(tool_output_message, "tool_call_id", None):
                        idx = i
                        break
                if idx is not None and idx < len(last_ai.tool_calls):
                    tool_call_info = last_ai.tool_calls[idx]
                    print(f"[DEBUG] [Fallback order-matching] Using tool_call_info via order: {tool_call_info}")
                else:
                    print("[ERROR] Could not find matching tool_call_id in tool_messages during order-based fallback.")
    ...
    if "cart_items" not in updated_state or not isinstance(updated_state["cart_items"], list):
    ...
    try:
        ...
        try:
            ...
            print(f"[WARNING] Could not parse ToolMessage content as JSON. Content: {tool_output_message.content}")
            # Parse failure, use tool_name and tool_args for processing

        # --- Cart update logic ---
        if tool_name == "add_to_cart":
            item_to_add = tool_result.get("item") if tool_result.get("status") == "success" else None

            if item_to_add and isinstance(item_to_add, dict):
                # Check if the item is already in the cart
                existing_item_index = -1
                for i, item in enumerate(updated_state["cart_items"]):
                    if item.get("product_type") == item_to_add.get("product_type") and item.get("product_brand") == item_to_add.get("product_brand"):
                        existing_item_index = i
                        break

                if existing_item_index != -1:
                    # Update the quantity and total price of the existing product
                    current_quantity = updated_state["cart_items"][existing_item_index].get("quantity", 0)
                    added_quantity = item_to_add.get("quantity", 1)
                    new_quantity = current_quantity + added_quantity
                    price = updated_state["cart_items"][existing_item_index].get("price", 0)

                    updated_state["cart_items"][existing_item_index]["quantity"] = new_quantity
                    updated_state["cart_items"][existing_item_index]["item_total"] = price * new_quantity
                    print(f"[INFO] Updated cart item quantity: {updated_state['cart_items'][existing_item_index]}")
                else:
                    # Add new product
                ...
                print(f"[INFO] add_to_cart tool did not succeed. Status: {tool_result.get('status')}, Message: {tool_result.get('message')}")
            else:
                print("[WARNING] add_to_cart executed but no valid 'item' found in result.")
            ...
            if product_type:
                ...
                for item in updated_state["cart_items"]:
                    ...
                    if brand_lower:
                        if not (matches_product and item_brand_lower == brand_lower):
                            ...
                            removed_items_desc.append(f"{item.get('quantity')}x {item.get('product_brand')} {item.get('product_type')}")
                ...
                if len(new_cart_items) < original_count:
                    ...
                    print(f"[INFO] Item to remove not found in cart: {product_type} (Brand: {brand if brand else 'Any'})")
            ...
            if product_type and brand and quantity is not None:
                ...
                new_cart_items = []
                for i, item in enumerate(updated_state["cart_items"]):
                    if item.get("product_type", "").lower() == product_type_lower and item.get("product_brand", "").lower() == brand_lower:
                        found = True
                        if quantity > 0:
                            ...
                            print(f"[INFO] Removed item due to quantity 0: {item.get('product_brand')} {item.get('product_type')}")
                    else:
                        new_cart_items.append(item)

                if not found:
                    print(f"[INFO] Item to modify not found in cart: {brand} {product_type}")
            ...
            if updated_state["cart_items"]:
                print(f"[INFO] Cart cleared. Removed {len(updated_state['cart_items'])} item types.")
                updated_state["cart_items"] = []
            else:
                print("[INFO] Cart is already empty.")

        # after update, log cart status
        total_items = sum(item.get("quantity", 0) for item in updated_state["cart_items"])
        total_price = sum(item.get("item_total", 0) for item in updated_state["cart_items"])
        print(f"[DEBUG] Cart items after update: {len(updated_state['cart_items'])} types, {total_items} total items, total price ${total_price:.2f}")

    except Exception as e:
        print(f"[ERROR] Error during cart update logic in update_cart_node: {e}\n{traceback.format_exc()}")
