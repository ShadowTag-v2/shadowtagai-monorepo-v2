...
def view_cart_node(state: State):
  ...
  if isinstance(last_message, AIMessage) and last_message.tool_calls:
    ...
    if view_cart_call:
      ...
      tool_call_id = (
        last_message.tool_calls[0].get("id", "") if last_message.tool_calls else ""
      )
...
def update_cart_node(state):
  ...
  for msg in reversed(updated_state["messages"][:-1]):
    ...
    if not tool_call_info:
      ...
      ai_messages = [
        msg for msg in updated_state["messages"][:-1] if isinstance(msg, AIMessage)
      ]
      tool_messages = [
        msg for msg in updated_state["messages"][:-1] if isinstance(msg, ToolMessage)
      ]
  ...
  try:
    ...
    if tool_name == "add_to_cart":
      item_to_add = (
        tool_result.get("item") if tool_result.get("status") == "success" else None
      )
      ...
      if item_to_add and isinstance(item_to_add, dict):
        ...
        for i, item in enumerate(updated_state["cart_items"]):
          if item.get("product_type") == item_to_add.get("product_type") and item.get(
            "product_brand"
          ) == item_to_add.get("product_brand"):
            existing_item_index = i
            break

        if existing_item_index != -1:
          # Update the quantity and total price of the existing product
          current_quantity = updated_state["cart_items"][existing_item_index].get(
            "quantity", 0
          )
