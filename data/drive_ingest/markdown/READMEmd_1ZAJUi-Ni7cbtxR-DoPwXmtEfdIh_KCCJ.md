# ShadowTag GKE GPU Infrastructure - Terraform

This directory contains Terraform configurations for deploying ShadowTag's GPU infrastructure on Google Kubernetes Engine (GKE).
...
2. **GCP Authentication**:
   ...
   gcloud config set project ShadowTag-production
   ```
...
4. **Create GCS Bucket for Terraform State**:
   ```bash
   gsutil mb -p ShadowTag-production -l us-central1 gs://ShadowTag-terraform-state
   gsutil versioning set on gs://ShadowTag-terraform-state
   ```
...
gcloud billing projects describe ShadowTag-production
...
- GCP SA: `gke-training-sa@ShadowTag-production.iam.gserviceaccount.com`
- Permissions: `roles/storage.objectAdmin`

**Inference SA** (`inference-sa`):
- Namespace: `production`
- GCP SA: `gke-inference-sa@ShadowTag-production.iam.gserviceaccount.com`
...
gcloud container clusters upgrade ShadowTag-production-gpu \
  --region us-central1 \
...
gsutil ls gs://ShadowTag-terraform-state/gke-gpu-cluster/

# Download backup
gsutil cp gs://ShadowTag-terraform-state/gke-gpu-cluster/default.tfstate ./backup/
```
