...
// https://github.com/CommunityToolkit/WindowsCommunityToolkit/blob/main/CommunityToolkit.WinUI.UI/Helpers/ObjectStorage/ApplicationDataStorageHelper.cs
...
namespace CommunityToolkit.WinUI.Helpers
{
    ...
    public partial class BytesApplicationDataStorageHelper : IFileStorageHelper, ISettingsStorageHelper<string>
    {
        ...
        AsyncLock _fileReadLock = new AsyncLock();
        private async Task<T?> ReadFileAsync<T>(StorageFolder folder, string filePath, T? @default = default)
        {
            //using var _ = await _fileReadLock.LockAsync(default);
            try
            {
                var fileName = NormalizePath(filePath);
                if (await folder.FileExistsAsync(fileName))
                {
                    var file = await folder.GetFileAsync(NormalizePath(filePath));
                    byte[] value = await file.ReadBytesAsync();
                    return (value != null && value.Length != 0) ? Serializer.Deserialize<T>(value) : @default;
                }
                else
                {
                    return @default;
