...
namespace TsubameViewer.Presentation.ViewModels
{
    ...
    public sealed class ImageViewerPageViewModel : NavigationAwareViewModelBase, IDisposable
    {
            ...
            )
        {
            ...
            _appView = App.Current.AppWindow;
        ...
        public override void OnNavigatedFrom(INavigationParameters parameters)
        {
            ...
            if (Images?.Any() ?? false)
            {
                Array.ForEach(Images, (IImageSource x) => (x as IDisposable)?.Dispose());
        ...
        private async Task ReloadItemsAsync(IImageCollectionContext imageCollectionContext, CancellationToken ct)
        {
            if (Images != null)
            {
                Parallel.ForEach(Images.AsParallel().WithDegreeOfParallelism(4), x => (x as IDisposable)?.Dispose());
            }
