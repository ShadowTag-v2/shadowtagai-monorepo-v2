...
def demo_glicko_failover():
    ...
    for rank in stats["rankings"]:
        print(f"  {rank['provider']:8s}: {rank['rating']:4.0f} rating ({rank['allocation_pct']:4.1f}% allocation)")
    ...
    for i in range(1, 21):
        ...
        if i % 5 == 0:  # Print every 5th
            print(f"  Decision {i:2d}: {decision.decision:8s} by {decision.provider_used.value:8s} ({decision.latency_ms:5.1f}ms)")

    print("\nFinal provider rankings after 20 decisions:")
    stats = engine.get_provider_stats()
    for rank in stats["rankings"]:
        print(f"  {rank['provider']:8s}: {rank['rating']:4.0f} rating ({rank['allocation_pct']:4.1f}% allocation, RD={rank['rating_deviation']:.0f})")
...
def demo_integrated_judge():
    ...
    metrics = judge.decide({"user_request": "Archive old customer support tickets", "risk_level": "low"})
    ...
    metrics = judge.decide({"user_request": "Update pricing for premium tier from $99 to $149/month", "risk_level": "medium"})
    ...
    if "provider_stats" in status:
        ...
        for rank in status["provider_stats"]["rankings"]:
            print(f"    {rank['provider']:8s}: {rank['rating']:4.0f} rating ({rank['allocation_pct']:4.1f}%)")
...
def demo_dte_trainer():
    ...
    trainer = DTELocalModelTrainer(model_path="models/demo_judge_local.pt", target_accuracy=0.80)
    ...
    if summary["final_accuracy"] >= trainer.target_accuracy:
        ...
        print(f"  ⚠️  Continue training ({summary['final_accuracy']:.2%} < {trainer.target_accuracy:.0%})")
