...
def demo_jr_engine():
    ...
    from pnkln_agents import Purpose, Reason
...
def demo_revenue_model():
    ...
    print(f"  Operational cost: ${constraints_dict['operational_cost_range'][0]:,.2f}-${constraints_dict['operational_cost_range'][1]:,.2f}/mo")

    # Break-even analysis
    print("\nBreak-even Analysis:")
    min_customers, max_customers = DEFAULT_CONSTRAINTS.calculate_break_even_customers(DEFAULT_REVENUE_MODEL.base_tier.monthly_price_usd)
    print(f"  Base tier ($297/mo): {min_customers}-{max_customers} customers")

    min_leads, max_leads = DEFAULT_CONSTRAINTS.calculate_break_even_leads(DEFAULT_REVENUE_MODEL.price_per_validated_lead)
    print(f"  Usage tier ($0.10/lead): {min_leads:,}-{max_leads:,} leads/mo")


def main():
    """Run all demos"""
    print_separator("PNKLN AGENT PLATFORM DEMO")
