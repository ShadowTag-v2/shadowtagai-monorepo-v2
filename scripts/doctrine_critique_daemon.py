...
class DoctrineAuditResult:
  ...
  def to_dict(self) -> dict:
    return {
      ...
      + len(self.skill_issues)
      + len(self.contradictions)
      + len(self.staleness_flags),
    }
...
class DoctrineCritiqueEngine:
  ...
  def _audit_single_skill(self, path: Path, result: DoctrineAuditResult) -> None:
    ...
    if "---" in content:
      ...
      if (
        "name:" not in frontmatter.lower() and "description:" not in frontmatter.lower()
      ):
        result.skill_issues.append(
          DoctrineWarning(
...
def format_human_readable(result: DoctrineAuditResult) -> str:
  ...
  # Truth surfaces
  lines.append(
    f"  Truth Surfaces: {len(result.truth_surfaces_found)}/{len(TRUTH_SURFACES)} found"
  )
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="Doctrine Critique Daemon (ported from Claude Code doctorDiagnostic.ts)"
  )
  ...
  parser.add_argument(
    "--log", action="store_true", help="Write results to .beads/doctrine_audit.jsonl"
  )
  ...
  has_critical = any(
    w.severity in ("error", "critical")
    for w in (result.warnings + result.contradictions)
  )
