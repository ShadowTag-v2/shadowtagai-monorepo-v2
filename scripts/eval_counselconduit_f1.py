...
def evaluate_all(
  ...
  if predictions_path:
    ...
    predictions = {
      ex["id"]: ex["expected_citations"] for ex in ground_truth["ground_truth_examples"]
    }
  ...
  # Aggregate scores
  if summary.results:
    summary.mean_precision = sum(r.precision for r in summary.results) / len(
      summary.results
    )
    ...
    for r in summary.results:
      query_weighted = sum(
        weights.get(dim, 0.25) * score for dim, score in r.dimension_scores.items()
      )
