...
def run_nuclear_trim(root_path: Path):
  ...
  for git_dir in root_path.rglob(".git"):
    if git_dir.is_dir():
      try:
        ...
        pass
  print(
    f"[X] Obliterated {git_folders_deleted} embedded .git tracking folders and hooks."
  )
  ...
  print(
    f"[X] Stripped {deleted_binaries} binaries/databases and {massive_files} massive >25MB artifacts."
  )

  # 3. Replace 'https://github.com/karpathy/autoresearch' with 'https://github.com/karpathy/autoresearch'
  flying_monkey_pattern = re.compile(
    r"https://github.com/karpathy/autoresearch", re.IGNORECASE
  )
  replaced_monkeys = 0
      ...
      if filepath.suffix.lower() in text_extensions or filepath.suffix == "":
        try:
          ...
          if flying_monkey_pattern.search(content):
            new_content = flying_monkey_pattern.sub(
              "https://github.com/karpathy/autoresearch", content
            )
  ...
  print(
    f"[X] Re-routed {replaced_monkeys} 'https://github.com/karpathy/autoresearch' literals to 'karpathy/autoresearch'."
  )
  print("=============================================")
