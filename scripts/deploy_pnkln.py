...
WORKDIR /app
COPY . .
RUN npm ci && chmod +x scripts/pnkln-test.sh scripts/Claude_Code_6.sh scripts/cleanup-cinematic-videos.sh
...
write_file(
    f"{pack_dir}/scripts/Claude_Code_6.sh",
    """
#!/usr/bin/env bash
set -euo pipefail
PROJECT_ROOT="${PWD}"
REPORT_DIR="${PROJECT_ROOT}/docs/Claude_Code_6-reports"
mkdir -p "${REPORT_DIR}"
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
REPORT="${REPORT_DIR}/Claude_Code_6-${TIMESTAMP}.md"
...
if [[ -f "${VIDEO_FILE}" ]]; then
  ...
  PROMPT=$(cat "${PROJECT_ROOT}/prompts/Claude_Code_6-cinematic.md")
...
DAYS=7
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
LOG="docs/Claude_Code_6-cleanup-${TIMESTAMP}.log"
...
find docs/Claude_Code_6-reports -name "Claude_Code_6-*.md" -mtime +${DAYS} -delete 2>&1 | tee -a "${LOG}"
...
chmod +x scripts/Claude_Code_6.sh
./scripts/Claude_Code_6.sh --full-audit
""",
)

for f in ["scripts/Claude_Code_6.sh", "scripts/cleanup-cinematic-videos.sh", "scripts/pnkln-test.sh"]:
...
def run_meta_experiment():
    ...
    result = subprocess.run(["./scripts/Claude_Code_6.sh", "--full-audit"], capture_output=True, text=True)
...
mkdir -p apps/counselconduit labs/uphillsnowball docs/Claude_Code_6-reports scripts prompts .antigravity/rules
