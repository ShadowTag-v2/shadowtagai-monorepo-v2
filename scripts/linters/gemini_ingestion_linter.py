...
class GeminiIngestionLinter(ast.NodeVisitor):
  ...
  def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
    ...
    if any(
      ...
      for keyword in ["crawl", "fetch", "scrape", "collect"]
    ):
      has_rate_limit = any(
    ...
    if any(
      keyword in node.name.lower() for keyword in ["api_call", "call_api", "request"]
    ):
  ...
  def visit_ClassDef(self, node: ast.ClassDef) -> None:
    ...
    if any(
      keyword in node.name.lower() for keyword in ["crawler", "scraper", "spider"]
    ):
