...
class GodModeAdmin:
  ...
  def check_gcp_project(self) -> None:
    ...
    self._record(
      _check(
        "PROJECT_ID env var", os.getenv("GCP_PROJECT_ID") == PROJECT_ID, PROJECT_ID
      )
    )
  ...
  def check_gcloud_auth(self) -> None:
    ...
    self._record(
      _check("Active gcloud account", bool(accounts), ", ".join(accounts[:2]))
    )
  ...
  def check_firebase(self) -> None:
    ...
    if ok:
      try:
        ...
        found = any(
          p.get("projectId") == PROJECT_ID for p in projects.get("result", [])
        )
  ...
  def check_secrets(self) -> None:
    ...
    if ok:
      ...
      self._record(
        _check(f"Secrets accessible ({len(secrets)} found)", len(secrets) > 0)
      )
    else:
  ...
  def check_pem(self) -> None:
    ...
    self._record(
      _check(
        "$SHADOWTAG_PEM env var", bool(env_pem), env_pem[:50] if env_pem else "unset"
      )
    )
  ...
  def check_python(self) -> None:
    ...
    self._record(
      _check(
        "Virtual env",
        bool(venv) or Path(".venv").exists(),
        venv or (".venv exists" if Path(".venv").exists() else "none"),
      )
    )
  ...
  def check_firestore(self) -> None:
    ...
    for db_id in FIRESTORE_DBS:
      ...
      self._record(
        _check(f"Database '{db_id}'", ok, out.strip() if ok else "not found")
      )
  ...
  def check_dotenv(self) -> None:
    ...
    if env_path.exists():
      ...
      self._record(
        _check(".env kernel-locked (uchg)", locked, "chflags uchg .env to lock")
      )
