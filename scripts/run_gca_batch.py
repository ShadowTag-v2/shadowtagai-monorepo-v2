...
def review_pr(pr_num: int, token: str, dry_run: bool = False) -> dict:
  ...
  cmd = [
    sys.executable,
    os.path.join(SCRIPTS_DIR, "run_gca_local.py"),
    "--pr",
    str(pr_num),
  ]
  if dry_run:
  ...
  total_findings = sum(
    int(l.split(":")[1].strip().split()[0]) for l in findings if ":" in l
  )  # noqa: E741
...
def main() -> int:
  ...
  parser.add_argument(
    "--merge", action="store_true", help="Auto-merge PRs with 0 findings"
  )
  parser.add_argument(
    "--dry-run", action="store_true", help="Review only, no GitHub posting"
  )
  ...
  for pr_num in pr_nums:
    try:
      # Get PR metadata
      pr_data = _gh_api(
        "GET", f"/repos/ShadowTag-v2/Monorepo-Uphillsnowball/pulls/{pr_num}", token
      )
      ...
      if not head_ref:
        _gh_api(
          "PATCH",
          f"/repos/ShadowTag-v2/Monorepo-Uphillsnowball/pulls/{pr_num}",
          token,
          {"state": "closed"},
        )
        continue
      ...
      if args.merge and not args.dry_run and result["findings"] == 0:
        ...
        _gh_api(
          "PUT",
          f"/repos/ShadowTag-v2/Monorepo-Uphillsnowball/pulls/{pr_num}/update-branch",
          token,
          {"update_method": "merge"},
        )
        time.sleep(3)
