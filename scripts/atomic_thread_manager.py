...
class AtomicThreadManager:
  ...
  def create(
    ...
    template = template.replace(
      "parent: null | ATOMIC-XXX", f"parent: {parent or 'null'}"
    )
    template = template.replace(
    ...
    if mission:
      template = template.replace(
        "[One sentence: Action verb + object + outcome]", mission
      )
  ...
  def complete(
    self, thread_id: str, outcome: str, files_changed: list[str] | None = None
  ) -> None:
