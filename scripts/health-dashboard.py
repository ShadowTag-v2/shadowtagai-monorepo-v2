...
def check_ruff() -> dict:
  ...
  return {
    ...
    "detail": out or "All checks passed",
    "code": code,
  }
...
def check_ruff_dead_code() -> dict:
  ...
  code, out = run(
    "python3 -m ruff check --select F401,F841 scripts/ apps/counselconduit/ --statistics 2>/dev/null"
  )
  return {
    "status": "✅" if code == 0 else "⚠️",
    "detail": out or "No dead code",
    "code": code,
  }
...
def check_git_pack() -> dict:
  ...
  return {
    ...
    "status": f"{'⚠️' if size_gb > 5 else '✅'} {size_gb:.1f} GB",
  }
