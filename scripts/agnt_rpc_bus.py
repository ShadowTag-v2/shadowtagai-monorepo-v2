...
def format_status(storage: RPCStorage) -> str:
  ...
  lines.append(
    f"  Messages: {stats['total_messages']} total, {stats['pending_messages']} pending"
  )
  lines.append(
    f"  Services: {stats['active_services']}/{stats['total_services']} active"
  )
  lines.append("")

  if services:
    lines.append(
      "  | Service              | PID    | Status | Last Heartbeat       | Methods |"
    )
    lines.append(
      "  |----------------------|--------|--------|----------------------|---------|"
    )
    for svc in services:
      ...
      lines.append(
        f"  | {svc.name:<20s} | {svc.pid:<6d} | {status_emoji}     | {hb_time:<20s} | {methods_str:<7s} |"
      )
  else:
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="AGNT RPC Bus (ported from Claude Code bridge/ architecture)"
  )
  ...
  parser.add_argument(
    "--cleanup", action="store_true", help="Clean up expired messages"
  )
