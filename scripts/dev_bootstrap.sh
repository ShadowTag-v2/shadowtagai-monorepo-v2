...
setup_antigravity_proxy() {
  echo "[*] Setting up Antigravity proxy (mitmproxy)..."
  if ! command -v brew >/dev/null 2>&1; then
    echo "[-] Homebrew required for mitmproxy install."; return 1
  fi
  brew install mitmproxy || true
  python3 -m pip install --upgrade pip python-dotenv
  if [ ! -d "$HOME/antigravity-proxy" ]; then
    git clone https://github.com/elad12390/antigravity-proxy "$HOME/antigravity-proxy"
  fi
  ( cd "$HOME/antigravity-proxy" && python3 -m pip install -r requirements.txt || true )
  echo "[*] To trust cert: open ~/.mitmproxy/mitmproxy-ca-cert.pem and 'Always Trust'."
}
