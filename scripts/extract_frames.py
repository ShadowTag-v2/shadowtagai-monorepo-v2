...
def extract_frames(
  ...
  return len(
    [
      ...
      if f.startswith("frame_") and f.endswith(f".{frame_format}")
    ]
  )
...
def main() -> None:
  ...
  parser = argparse.ArgumentParser(
    description="Extract video frames or generate placeholders for cinematic scroll."
  )
  parser.add_argument("video", nargs="?", help="Input video file path")
  parser.add_argument(
    "output_dir", nargs="?", default="frames", help="Output directory"
  )
  parser.add_argument(
    "--fps", type=int, default=30, help="Frames per second (default: 30)"
  )
  parser.add_argument(
    "--format", choices=["png", "webp"], default="png", help="Frame format"
  )
  parser.add_argument("--quality", type=int, default=85, help="WebP quality (1-100)")
  parser.add_argument(
    "--generate-placeholder", action="store_true", help="Generate placeholder frames"
  )
  parser.add_argument(
    "--count", type=int, default=240, help="Number of placeholder frames"
  )
