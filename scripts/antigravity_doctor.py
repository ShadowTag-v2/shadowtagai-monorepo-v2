...
class AntigravityDoctor:
    ...
    def diagnose(self) -> DoctorReport:
        ...
        report.failed = sum(1 for c in report.checks if not c.passed and c.severity in ("error", "critical"))
        report.warnings = sum(1 for c in report.checks if not c.passed and c.severity == "warning")
            ...
            try:
                ...
                report.checks.append(
                    CheckResult(
                        name="repo/clean",
                        passed=changes == 0,
                        message=f"Working tree: {changes} uncommitted changes" if changes else "Working tree clean",
    ...
    def _check_auth(self, report: DoctorReport) -> None:
        ...
        report.checks.append(
            CheckResult(
                ...
                message="GitHub App PEM found" if pem_found or pem_env else "GitHub App PEM not found",
...
def format_report(report: DoctorReport) -> str:
    ...
    lines.append(f"  Platform: {report.platform_info.get('os', '?')} {report.platform_info.get('machine', '?')}")
    ...
    for cat, checks in categories.items():
        ...
        for check in checks:
            emoji = "✅" if check.passed else ("⚠️ " if check.severity == "warning" else ("ℹ️ " if check.severity == "info" else "❌"))
    ...
    # Summary
    health_emoji = {"healthy": "💚", "degraded": "💛", "unhealthy": "❤️"}.get(report.overall_health, "?")
    lines.append(f"  {health_emoji} Overall: {report.overall_health.upper()}")
    lines.append(f"     ✅ {report.passed} passed  ⚠️  {report.warnings} warnings  ❌ {report.failed} failed")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Antigravity Doctor (ported from Claude Code doctor diagnostic)")
    parser.add_argument("--json", action="store_true", help="Output as JSON")
    parser.add_argument("--verbose", action="store_true", help="Show detailed output")
    parser.add_argument("--log", action="store_true", help="Write results to .beads/doctor_report.jsonl")
    ...
    sys.exit(0 if report.overall_health == "healthy" else (1 if report.overall_health == "degraded" else 2))
