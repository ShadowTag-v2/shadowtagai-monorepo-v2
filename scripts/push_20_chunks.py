...
def push_commits_individually(
  ...
  for _j, sha in enumerate(commits):
    ...
    ok = push_chunk(
      sha, branch, current_token, force=force, timeout=300, post_buffer=52428800
    )
    if ok:
      pushed += 1
    else:
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="Chunked Git Push Daemon (Invariant #58)"
  )
  parser.add_argument(
    "--batch-size",
    type=int,
  ...
  for i, chunk_commits in enumerate(chunk_groups):
    ...
    # Try the batch push
    success = push_chunk(
      target_sha, args.branch, access_token, force=force, timeout=300
    )
    if success:
      pushed += 1
