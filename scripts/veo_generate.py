...
Examples:
  ...
  parser.add_argument(
    "--preset", choices=list(DEMO_PRESETS), help="Use a preset prompt"
  )
  parser.add_argument(
    "--mode",
  ...
  parser.add_argument(
    "--list-presets", action="store_true", help="List all preset prompts"
  )
  ...
  if args.mode == "vertex":
    ...
    result = generate_vertex_ai(
      ...
      image_gcs_uri=args.image
      if args.image and args.image.startswith("gs://")
      else None,
    )
  else:  # gemini
    result = generate_gemini_api(
      ...
      if args.image and not args.image.startswith("gs://")
      else None,
    )
