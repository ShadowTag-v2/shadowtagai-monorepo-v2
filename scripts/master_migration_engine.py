...
sys.path.insert(
    0,
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../apps/aiyou_stack/aiyou-fastapi-services")),
)
...
def process_single_file(filepath: str, size: int) -> tuple:
    ...
    try:
        with open(filepath, encoding="utf-8", errors="ignore") as f:
            content = f.read(5000)  # Read only first 5000 chars for semantic memory snapshot to prevent RAM overflow
...
def index_and_route(directory, conn):
    ...
    print(f"🚀 Discovered {len(target_files)} targeted text files for extraction in {directory}.")
    ...
    with ThreadPoolExecutor(max_workers=os.cpu_count() or 8) as executor:
        futures = {executor.submit(process_single_file, path, size): (path, size) for path, size in target_files}
        ...
        for future in as_completed(futures):
            ...
            if len(batch_results) >= batch_size or processed == total:
                ...
                conn.commit()
                print(f"✅ Ingested {processed}/{total} files synchronously into beads_index...")
                batch_results.clear()
