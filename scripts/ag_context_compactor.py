...
# 1. Gate Definition
FEATURE_CACHED_MICROCOMPACT = (
  os.getenv("AG_CACHED_MICROCOMPACT", "true").lower() == "true"
  or os.getenv("USER_TYPE") == "ant"
)
...
def perform_micro_compaction(memory_file=".memory/events.ndjson"):
  ...
  if file_size < (MAX_CAPACITY * 0.8):
    print(
      "📊 Memory at "
      + str(round((file_size / MAX_CAPACITY) * 100, 1))
      + "% capacity. No compaction needed."
    )
    return
  ...
  print(
    "✅ Micro-compaction complete. Saved approx " + str(tokens_saved) + " characters."
  )
  print(
    "🧠 Routing summary blocks through active KV-cache slab for zero-latency retrieval..."
  )
