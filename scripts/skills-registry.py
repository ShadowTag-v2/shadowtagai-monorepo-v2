    ...
    for child in sorted(directory.iterdir()):
        ...
        if skill_md.is_file():
            ...
            skills.append(
                {
                    "id": name,
                    "path": str(child.relative_to(REPO_ROOT) if str(child).startswith(str(REPO_ROOT)) else child),
...
def scan_archived(directory: Path) -> int:
    ...
    for child in directory.iterdir():
        if child.is_dir() and (child.name.startswith("_archive_") or child.name.startswith("_dedup_")):
...
def main() -> None:
    ...
    parser.add_argument("--refresh", action="store_true", help="Update registry + evidence")
    parser.add_argument("--check-upstream", action="store_true", help="Check for upstream updates")
    parser.add_argument("--install-missing", action="store_true", help="Install missing upstream skills")
