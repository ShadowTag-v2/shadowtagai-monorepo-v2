...
def render_dashboard(model: str = "gemini-3.1-flash-lite-preview-thinking") -> str:
  ...
  lines.append(
    f"  \033[1mContext Window:\033[0m  {_format_tokens(context_window)} tokens"
  )
  lines.append(
    f"  \033[1mAutocompact:\033[0m    triggers at {_format_tokens(context_window - autocompact_buffer)}"
  )
  lines.append(
    f"  \033[1mBuffer:\033[0m         {_format_tokens(autocompact_buffer)} reserved"
  )
  lines.append("")
  ...
  lines.append(f"    {'─' * 52}")
  lines.append(
    f"    {'Available for messages':<22} {_format_tokens(available):>8}  ({available / context_window * 100:4.1f}%)"
  )
  lines.append("")
  ...
  lines.append(
    f"    {_format_tokens(total_reserved)} / {_format_tokens(context_window)}"
  )
  lines.append("")
  ...
  if heartbeat:
    ...
    sleep_guard = (
      "active" if "sleep" not in str(status.get("health", "")).lower() else "N/A"
    )

    lines.append(
      f"    Prompt Assembly:  {prompt_assembly} ({prompt_sections_count} sections)"
    )
    lines.append(
      f"    Sleep Guard:     {'🟢 active' if sleep_guard == 'active' else '🔴 inactive'}"
    )
  ...
  if quarantine_dir.exists():
    quarantined = [
      f for f in quarantine_dir.iterdir() if f.is_file() and f.name != ".gitkeep"
    ]
