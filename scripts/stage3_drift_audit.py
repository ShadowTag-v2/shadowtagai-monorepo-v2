...
def audit_file(filepath):
    try:
        ...
        if filepath.name.endswith(".json") or filepath.name.endswith(".yaml") or filepath.name.endswith(".py"):
        ...
        for sm in stale_models:
            if sm in content:
                drift_results.append(f"[DRIFT DETECTED] Stale model '{sm}' found in {filepath} (Expected: {CANONICAL_MODEL})")

        # 3. Canonical Project
        for sp in stale_projects:
            if sp in content:
                drift_results.append(f"[DRIFT DETECTED] Stale project '{sp}' found in {filepath} (Expected: {CANONICAL_PROJECT})")
    except Exception:
        pass
...
for d in target_dirs:
    ...
    if not target.exists():
        drift_results.append(f"[RULE 3 WARNING] Expected path {d} is missing or incomplete.")
        continue
