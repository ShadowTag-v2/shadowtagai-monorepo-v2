...
def chunk_commit_push(token):
    ...
    subprocess.run(["git", "commit", "-m", "chore: canonical origin initialization"], check=False)
    subprocess.run(["git", "push", "origin", "main", "--force", "--no-verify"], check=False)
    ...
    for f in all_files:
        ...
        if current_size > MAX_BYTES or idx == total_files:
            ...
            subprocess.run(["git", "commit", "-m", f"chore: canonical monorepo payload chunk {BATCH}"], check=False)
