  ...
  lx.data.ExampleData(
    ...
    extractions=[
      lx.data.Extraction(
        ...
        extraction_text=(
          "Venture-backed startups with formal advisory boards had a 34% higher 5-year survival rate"
        ),
      ...
      lx.data.Extraction(
        ...
        extraction_text=(
          "Cox proportional hazards regression on Crunchbase longitudinal data"
        ),
...
class CheckpointDB:
  ...
  def stats(self) -> dict:
    row = self.conn.execute(
      "SELECT COUNT(*), SUM(count), SUM(status='ok'), SUM(status='failed') FROM processed"
    ).fetchone()
    return {
      "total": row[0],
...
class Extractor:
  def __init__(self, api_key: str) -> None:
    ...
    zero_cpu_path = os.path.join(
      PROJECT_ROOT, "apps/aiyou_stack/aiyou-fastapi-services"
    )
  ...
  def extract(self, text: str, doc_name: str) -> list[dict]:
    ...
    try:
      return zero_cpu_router.dispatch_compute(
        text=text,
  ...
  def __init__(self) -> None:
    ...
    if not creds or not creds.valid:
      if creds and creds.expired and creds.refresh_token:
        ...
        raise RuntimeError(
          "token.json missing or invalid. Please execute `python scripts/auth_setup.py` first to bind the Web Client ID!"
        )
