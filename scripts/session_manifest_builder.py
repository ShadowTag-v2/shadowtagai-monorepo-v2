...
def build_manifest(
  session_map: dict[str, str], raw_lines: int, duplicates: int
) -> dict:
...
def merge_with_existing(new_manifest: dict, existing_path: Path) -> dict:
  ...
  if old_judge:
    ...
    if "JUDGE_LEVEL" not in new_manifest["groups"]:
      new_manifest["groups"]["JUDGE_LEVEL"] = {
        "priority": 1,
        "count": 0,
        "sessions": [],
      }
...
def main() -> None:
  ...
  parser.add_argument(
    "--merge", action="store_true", help="Merge with existing manifest"
  )
  parser.add_argument(
    "--dry-run", action="store_true", help="Print manifest without writing"
  )
  parser.add_argument(
    "--out", type=Path, default=MANIFEST_PATH, help="Output manifest path"
  )
