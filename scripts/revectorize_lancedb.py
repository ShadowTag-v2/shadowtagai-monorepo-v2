...
JSONL_PATH = Path(__file__).parent.parent / "data" / "drive_ingest" / "extractions.jsonl"
...
def build_vectorized_table(db: lancedb.DBConnection, records: list[dict], embeddings: np.ndarray) -> None:
    ...
    logger.info("Writing %d vectorized records to table '%s'...", len(data), VECTORIZED_TABLE)
