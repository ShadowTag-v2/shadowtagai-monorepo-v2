...
try:
  ...
  subprocess.check_call(
    [sys.executable, "-m", "pip", "install", "PyJWT", "requests", "--quiet"]
  )
  import jwt
  import requests
...
def get_installation_token():
  ...
  resp = requests.get(
    f"https://api.github.com/repos/{REPO}/installation", headers=headers, timeout=15
  )
...
def create_pr(token, branch, title, body, base="main"):
  ...
  if resp.status_code == 201:
    ...
    print(
      f"  ⚠️  PR already exists or validation error: {resp.json().get('message', '')}"
    )
    return False
  else:
...
def main():
  ...
  if not token:
    print(
      "[!] Could not obtain GitHub App token. Branches are pushed but PRs not created."
    )
    print(
      "[!] Run manually: open https://github.com/ShadowTag-v2/Monorepo-Uphillsnowball/branches"
    )
    sys.exit(1)
