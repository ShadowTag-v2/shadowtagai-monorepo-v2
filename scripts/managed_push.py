...
class TokenManager:
  ...
  def _issue(self) -> str | None:
    ...
    if resp.status_code != 200:
      log.error(
        "App installations fetch failed: %s %s", resp.status_code, resp.text[:200]
      )
      return None
...
def main() -> int:  # returns exit code
  ...
  for i, chunk in enumerate(chunks):
    ...
    commit_msg = (
      f"chore(sync): monorepo bulk ingestion batch {i + 1} of {total} (25MB chunks)"
    )
