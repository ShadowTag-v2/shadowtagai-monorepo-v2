...
async function expandClineFleet() {
  ...
  if (existsSync(githubDist)) {
    ...
    console.warn("⚠️ anthropic-github dist not found — run 'bun install && bun run build' in src/github/");
  }
  ...
  if (existsSync(fetchDist)) {
    ...
    console.warn("⚠️ anthropic-fetch dist not found — run 'bun install && bun run build' in src/fetch/");
  }

  // ──────────────────────────────────────────────
  // Servers already present (preserve, don't overwrite)
  // ──────────────────────────────────────────────
  const preserved = ["cloud-run", "gcloud", "observability"].filter(
    (s) => s in config.mcpServers
  );
