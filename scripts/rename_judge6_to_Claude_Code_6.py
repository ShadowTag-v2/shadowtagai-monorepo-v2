...
def main():
  ...
  for dirpath, _dirnames, filenames in os.walk(root_dir, topdown=False):
    ...
    for filename in filenames:
      if (
        filename == "rename_judge6_to_Claude_Code_6.py"
        or filename == "rename_Claude_Code_6.py"
      ):
        continue
      ...
      new_filename = re.sub(
        r"judge6", "Claude_Code_6", new_filename, flags=re.IGNORECASE
      )
      new_filename = re.sub(
        r"judge_six", "Claude_Code_6", new_filename, flags=re.IGNORECASE
      )
    ...
    new_dir_name = re.sub(
      r"judge_six", "Claude_Code_6", new_dir_name, flags=re.IGNORECASE
    )
