...
def gather(entries: list[KIEntry], report: DreamReport) -> dict:
    ...
    relative_date_patterns = [
        ...
        "this morning", "earlier", "recently", "a few days ago",
        "last night", "this week", "next week",
    ]
    ...
    print(f"[GATHER] Found: {len(issues['relative_dates'])} relative dates, "
...
def consolidate(entries: list[KIEntry], issues: dict, report: DreamReport):
        ...
        for prev_name, prev_tokens in names_seen.items():
            ...
            if len(overlap) >= 3:  # Significant overlap
                ...
                report.actions.append(
                    f"POTENTIAL CONTRADICTION: '{entry.name}' may overlap with '{prev_name}' "
                    f"(shared tokens: {overlap})"
                )
    ...
    for rd in issues["relative_dates"]:
        report.actions.append(
            f"RELATIVE DATE: '{rd['pattern']}' found in '{rd['ki']}' — "
            f"convert to absolute ISO-8601 date"
        )
    
    print(f"[CONSOLIDATE] {report.contradictions_found} potential contradictions, "
          f"{len(issues['relative_dates'])} dates to normalize")
...
def prune(entries: list[KIEntry], issues: dict, report: DreamReport):
    ...
    for sv in issues["size_violations"]:
        report.actions.append(
            f"SIZE VIOLATION: '{sv['ki']}' is {sv['size_kb']}KB "
            f"(limit: {sv['limit_kb']}KB) — consider splitting"
        )
    ...
    if total_summary_kb > INDEX_MAX_KB:
        report.actions.append(
            f"INDEX SIZE: Total summaries are {total_summary_kb:.1f}KB "
            f"(limit: {INDEX_MAX_KB}KB) — prune oldest entries"
        )
    
    if len(entries) > MAX_INDEX_ENTRIES:
        report.actions.append(
            f"INDEX COUNT: {len(entries)} entries "
            f"(limit: {MAX_INDEX_ENTRIES}) — archive oldest"
        )
    
    print(f"[PRUNE] {report.artifacts_pruned} orphaned artifacts, "
          f"index size: {total_summary_kb:.1f}KB/{INDEX_MAX_KB}KB")
