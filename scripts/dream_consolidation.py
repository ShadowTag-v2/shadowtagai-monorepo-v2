...
def consolidate(entries: list[KIEntry], issues: dict, report: DreamReport) -> None:
    ...
    for prev_name, prev_tokens in names_seen.items():
      ...
      if len(overlap) >= 3:  # Significant overlap
        ...
        report.actions.append(
          f"POTENTIAL CONTRADICTION: '{entry.name}' may overlap with '{prev_name}' (shared tokens: {overlap})"
        )
  ...
  for rd in issues["relative_dates"]:
    report.actions.append(
      f"RELATIVE DATE: '{rd['pattern']}' found in '{rd['ki']}' — convert to absolute ISO-8601 date"
    )
...
def prune(entries: list[KIEntry], issues: dict, report: DreamReport) -> None:
  ...
  for sv in issues["size_violations"]:
    ...
    report.actions.append(
      f"SIZE VIOLATION: '{sv['ki']}/{artifact_name}' is {sv['size_kb']}KB (limit: {sv['limit_kb']}KB) — consider splitting"
    )
  ...
  if total_summary_kb > INDEX_MAX_KB:
    report.actions.append(
      f"INDEX SIZE: Total summaries are {total_summary_kb:.1f}KB (limit: {INDEX_MAX_KB}KB) — prune oldest entries"
    )

  if len(entries) > MAX_INDEX_ENTRIES:
    report.actions.append(
      f"INDEX COUNT: {len(entries)} entries (limit: {MAX_INDEX_ENTRIES}) — archive oldest"
    )
...
def promote_and_detect(
  entries: list[KIEntry], ki_dir: Path, report: DreamReport
) -> None:
...
def run_dream_cycle(ki_dir: Path) -> DreamReport:
  ...
  try:
    ...
    with guard:
      ...
      logger.info(
        "Phase 4/8: Consolidate — resolving contradictions and normalizing dates"
      )
      consolidate(entries, issues, report)
      logger.info(
        "Phase 4/8: Consolidate complete — %d contradictions found",
      ...
      logger.info(
        "Phase 6/8: Prune complete — %d orphaned artifacts found",
        report.artifacts_pruned,
      )
...
def _run_deep_research_sweep(report: DreamReport, ki_dir: Path) -> None:
  ...
  try:
    ...
    for action in report.actions:
      if action.startswith("POTENTIAL CONTRADICTION:") or action.startswith(
        "CONFLICT:"
      ):
        research_queries.append(
          f"Resolve this knowledge conflict in the ShadowTag monorepo: {action}"
        )
    ...
    for i, query in enumerate(research_queries[:3]):
      logger.info(
        "Deep Research sweep %d/%d: %s",
        i + 1,
        min(3, len(research_queries)),
        query[:80],
      )  # noqa: G004
      try:
        ...
        sweep_path.write_text(
          f"# Dream Research Sweep — {datetime.now(UTC).isoformat()}\n\n## Query\n{query}\n\n## Result\n{result}\n"
        )
...
if __name__ == "__main__":
  ...
  if guardian_script.exists():
    gl_report = (
      repo_root
      / ".beads"
      / f"gitleaks_nightly_{datetime.now(UTC).strftime('%Y%m%d')}.md"
    )
    cmd = [
      sys.executable,
      str(guardian_script),
      ...
      try:
        ...
        if result.returncode in (0, 1):
          ...
          if result.stdout:
            pass
          report.actions.append(
            f"GITLEAKS: Production scan found issues — review {gl_report}"
          )
  ...
  try:
    ...
    if not DRY_RUN:
      ...
      nlm.upload_source(
        title=f"Dream Report {datetime.now(UTC).strftime('%Y-%m-%d')}",
        content=report_text,
        source_type="text",
      )
