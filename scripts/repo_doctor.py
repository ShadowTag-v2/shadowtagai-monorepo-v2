...
def check_conflict_markers(report: HealthReport) -> None:
  ...
  if r.returncode == 0 and r.stdout.strip():
    ...
    if not real_conflicts:
      report.findings.append(
        Finding("conflicts", Severity.INFO, "No conflict markers found")
      )
  else:
    report.findings.append(
      Finding("conflicts", Severity.INFO, "No conflict markers found")
...
def check_large_files(report: HealthReport) -> None:
  ...
  if not any(f.gate == "large_files" for f in report.findings):
    report.findings.append(
      Finding("large_files", Severity.INFO, "No large files in staging")
    )
...
def check_drift(report: HealthReport) -> None:
  ...
  if drift_count == 0 and legacy_count == 0:
    ...
    report.findings.append(
      Finding("drift", Severity.INFO, "No unknown drift — only known legacy dirs")
    )
...
def check_manifest(report: HealthReport) -> None:
  ...
  try:
    ...
    report.findings.append(
      Finding("manifest", Severity.INFO, f"Manifest v{version} valid")
    )
  except ImportError:
    report.findings.append(
      Finding("manifest", Severity.INFO, "Manifest exists (YAML parser not available)")
    )
  except Exception as e:
    report.findings.append(
      Finding("manifest", Severity.ERROR, f"Manifest parse error: {e}")
    )
