...
def route_all(*, dry_run: bool = False) -> list[dict]:
    ...
    for path in sorted(SERVE_DIR.iterdir()):
        if path.is_file() and path.name != ".gitkeep" and not path.name.endswith("_obsidian.md"):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Intelligence Router — Vault downstream routing")
    parser.add_argument("input", nargs="?", help="File or directory to route")
    parser.add_argument("--route-all", action="store_true", help="Route all files in serve/")
