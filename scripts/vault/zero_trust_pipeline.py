...
IPI_PATTERNS: list[re.Pattern[str]] = [
  ...
  re.compile(
    r"<\s*img\b[^>]*\bwidth\s*=\s*[\"']?1\b", re.IGNORECASE
  ),  # tracking pixels
]
...
def strip_hostile_content(content: str) -> str:
  ...
  content = re.sub(
    r"<script\b[\s\S]*?</script>", "[SCRIPT REMOVED]", content, flags=re.IGNORECASE
  )

  # Remove HTML style tags
  content = re.sub(
    r"<style\b[\s\S]*?</style>", "[STYLE REMOVED]", content, flags=re.IGNORECASE
  )
  ...
  content = re.sub(
    r"data:[a-zA-Z]+/[a-zA-Z]+;base64,[A-Za-z0-9+/=]+", "[BASE64 REMOVED]", content
  )
...
def process_file(path: Path, *, dry_run: bool = False) -> dict:
  ...
  if ipi_findings:
    ...
    if not dry_run:
      dest = (
        QUARANTINE_DIR
        / f"{path.stem}_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}{path.suffix}"
      )  # noqa: UP017
...
def main() -> int:
  ...
  parser.add_argument(
    "--scan-dir", help="Directory to scan (alternative to positional arg)"
  )
