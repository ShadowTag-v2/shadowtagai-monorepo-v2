...
def write_beads_entry(lint_results: dict) -> None:
  ...
  tool_results = {
    k: v
    for k, v in lint_results.items()
    if k not in ("ruff_version", "ruff_statistics")
  }
...
def parse_args() -> argparse.Namespace:
  ...
  parser = argparse.ArgumentParser(
    description="Omni-Autolint Daemon — Secure lint + push"
  )
  parser.add_argument(
    "--yes", "-y", action="store_true", help="Auto-approve changes (headless mode)"
  )
  parser.add_argument(
    "--json", action="store_true", help="Write structured results to .lint-results/"
  )
  parser.add_argument(
    "--dry-run", action="store_true", help="Run linters but skip commit/push"
  )
  parser.add_argument(
    "--branch",
    type=str,
  ...
  parser.add_argument(
    "--notify", action="store_true", help="Send GWS notification on completion"
  )
  parser.add_argument(
    "--aggressive", action="store_true", help="Enable ruff --unsafe-fixes (item 9)"
  )
