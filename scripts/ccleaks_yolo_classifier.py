...
class YoloClassifier:
  def __init__(self):
    self.log_file = os.path.join(
      os.path.dirname(os.path.dirname(__file__)), ".beads", "yolo_decisions.jsonl"
    )
...
if __name__ == "__main__":
  ...
  print(
    "Action 'list_dir' risk:", classifier.classify_action("list_dir", {"path": "/tmp"})
  )
  print(
    "Action 'run_command' (rm -rf) risk:",
    classifier.classify_action("run_command", {"command": "rm -rf /"}),
  )
  print(
    "Action 'generate_image' risk:",
    classifier.classify_action("generate_image", {"prompt": "test"}),
  )
