...
def scan_manifest_drift() -> dict:
    ...
    global_skills = list(GLOBAL_SKILLS_DIR.rglob("SKILL.md")) if GLOBAL_SKILLS_DIR.exists() else []
...
def scan_stale_repos() -> dict:
    ...
    return {"total_repos": len(list(omni_dir.iterdir())), "stale_count": len(stale), "stale": stale[:10]}
...
def main():
    parser = argparse.ArgumentParser(description="pnkln-evolve: Recursive self-improvement daemon")
    parser.add_argument("--once", action="store_true", help="Single pass, exit with report")
    parser.add_argument("--daemon", action="store_true", help="Continuous 5-min jitter cycles")
    parser.add_argument("--interval", type=int, default=300, help="Base interval in seconds (default: 300)")
