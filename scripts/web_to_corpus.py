...
_STAT_RE = re.compile(
  r"\b\d+\.?\d*\s*%|\b\d{4,}\b|\bpercent\b|\bGDP\b|\bmillion\b|\bbillion\b",
  re.IGNORECASE,
)
_RESEARCH_RE = re.compile(
  r"\bstudy\b|\bpaper\b|\bresearch\b|\bjournal\b|\barXiv\b|\bpreprint\b", re.IGNORECASE
)
_SOCIAL_RE = re.compile(
  r"\bthink\b|\bfeel\b|\bIMO\b|\bLMK\b|\breddit\b|\b4chan\b|\bunpopular opinion\b",
  re.IGNORECASE,
)
...
def main() -> None:
  ap = argparse.ArgumentParser(
    description="Normalize web scrape items into corpus extractions"
  )
  ...
  ap.add_argument(
    "--stats", action="store_true", help="Show extraction counts by source"
  )
  args = ap.parse_args()
