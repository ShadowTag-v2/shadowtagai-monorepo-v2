...
class Finding:
  ...
  def _classify(self) -> None:
    ...
    if (
      re.search(r"\.env\.example|README|\.md$", self.file)
      and self.rule_id in LOW_SIGNAL_RULES
    ):
      self.verdict = "WARN"
...
"""
  ...
  if blocks:
    ...
    report += "> [!CAUTION]\n"
    report += (
      f"> {len(blocks)} credential(s) detected in production code. Pipeline HALTED.\n\n"
    )
...
def _generate_manifest(raw_findings: list[dict], output_dir: str | None = None) -> None:
  ...
  for f in raw_findings:
    ...
    short_path = (
      file_path.split("Monorepo-Uphillsnowball/")[-1]
      if "Monorepo-Uphillsnowball/" in file_path
      else file_path
    )
  ...
  for rule in sorted(by_rule):
    ...
    for idx, item in enumerate(items, 1):
      ...
      fc = len(item["files"])
      lines.append(
        f"- [ ] **#{idx}** `{redacted}` — {repos} ({fc} file{'s' if fc > 1 else ''})"
      )
