...
def get_token(client_id, pem_path, owner_name):
  print(
    f"Attempting to fetch installation token for {owner_name} using Client ID {client_id}..."
  )
  try:
    ...
    if resp.status_code != 200:
      print(
        f"Failed to get installations for {owner_name}: {resp.status_code} {resp.text}"
      )
      return None
    ...
    if resp.status_code == 201:
      ...
      print(
        f"Failed to create access token for {owner_name}: {resp.status_code} {resp.text}"
      )
...
if __name__ == "__main__":
  ...
  if not token_e and not token_s:
    print(
      "CRITICAL: Failed to acquire any GitHub App tokens. Check network and PEM files."
    )
    sys.exit(1)
  ...
  print(
    f"Targeting {len(repos)} internal repositories to clone into canonical roots..."
  )
