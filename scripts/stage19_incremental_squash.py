...
for idx, chunk in enumerate(chunks):
  print("\n==============================")
  print(
    f"🚀 EXECUTING BATCH {idx + 1}/{len(chunks)} ({len(chunk)} files, approx {sum(os.path.getsize(x) for x in chunk) / (1024 * 1024):.1f} MB)"
  )
  print("==============================")
  ...
  subprocess.run(
    [
      ...
      "--no-verify",
      "-m",
      f"chore(core): structural ingestion payload {idx + 1}",
    ]
  )
  ...
  if idx == 0:
    push_args.insert(
      2, "--force"
    )  # Force push the very first root commit to wipe GitHub history
