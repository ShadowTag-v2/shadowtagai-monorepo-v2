...
try:
  ...
  print(
    "FATAL: PyJWT not installed. Run: /opt/homebrew/bin/python3.14 -m pip install 'PyJWT[crypto]'"
  )
  sys.exit(1)
...
try:
  ...
  print(
    "FATAL: requests not installed. Run: /opt/homebrew/bin/python3.14 -m pip install requests"
  )
  sys.exit(1)
...
def main() -> None:
  ...
  guardian_result = subprocess.run(
    [
      sys.executable,
      os.path.join(os.path.dirname(__file__), "gitleaks_guardian.py"),
      "--mode",
      "gate",
    ],
    check=False,
