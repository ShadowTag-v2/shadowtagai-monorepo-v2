...
ENDPOINT = f"https://{REGION}-aiplatform.googleapis.com/v1/projects/{GCP_PROJECT_ID}/locations/{REGION}/publishers/google/models/{MODEL}:generateContent"
TEMPERATURE = 0.3
MAX_TOKENS = 800
...
def insert_domain(
  conn: sqlite3.Connection, doc_id: str, title: str, domain: str, method: str
) -> None:
  """Insert a domain classification."""
  conn.execute(
    "INSERT OR REPLACE INTO doc_domains (doc_id, title, domain, method, tagged_at) VALUES (?, ?, ?, ?, ?)",
    (doc_id, title, domain, method, datetime.now(timezone.utc).isoformat()),
  )
...
def run_domain_tagger(cfg=None) -> dict:
  ...
  conn.commit()
  logger.info(
    f"Pass 1 (heuristic): {heuristic_count} classified, {len(llm_queue)} remaining"
  )
  ...
  if llm_queue and not (cfg and cfg.dry_run):
    ...
    for i in range(0, len(llm_queue), batch_size):
      ...
      try:
        ...
        for doc in batch:
          insert_domain(
            conn, doc["id"], doc["title"], _fallback_classify(doc["title"]), "fallback"
          )
      conn.commit()
