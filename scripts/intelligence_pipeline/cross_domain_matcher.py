...
EMBED_ENDPOINT = f"https://{REGION}-aiplatform.googleapis.com/v1/projects/{GCP_PROJECT_ID}/locations/{REGION}/publishers/google/models/{EMBED_MODEL}:predict"
TOP_K = 3
BATCH_SIZE = 10
...
def cosine_sim(a: list[float], b: list[float]) -> float:
  ...
  b_arr = np.array(b)
  return float(
    np.dot(a_arr, b_arr) / (np.linalg.norm(a_arr) * np.linalg.norm(b_arr) + 1e-9)
  )
...
def run_cross_domain_matcher(cfg=None) -> dict:
  ...
  unmatched = [
    row for _, row in docs_df.iterrows() if str(row.get("id", row.name)) not in matched
  ]
