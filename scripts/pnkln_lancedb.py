  ...
  if not _vertex_inited:
    ...
    if os.path.exists("token.json"):
      creds = Credentials.from_authorized_user_file(
        "token.json", ["https://www.googleapis.com/auth/cloud-platform"]
      )
...
def ingest_jsonl(table: Any, jsonl_path: str) -> None:
  ...
  try:
    ...
    existing_ids = set(
      [r["id"] for r in table.search().limit(10000000).select(["id"]).to_list()]
    )
...
def main() -> int:
  parser = argparse.ArgumentParser(
    description="pnkln LanceDB + Vertex AI embeddings bootstrap"
  )
  ...
  parser.add_argument(
    "--ingest", type=str, default=None, help="Path to extractions.jsonl to batch ingest"
  )
  parser.add_argument(
    "--source", type=str, default="manual", help="Source for added text"
  )
  ...
  if args.query:
    ...
    for i, row in enumerate(results, start=1):
      print(
        f"{i}. id={row.get('id')} source={row.get('source')} text={row.get('text')}"
      )
    return 0
