...
async function expandFleet() {
    ...
    for (const target of targets) {
        ...
        if (existsSync(targetPath)) {
            await $`cd ${targetPath} && bun install && bun run build`.quiet()
    ...
    const newServers = {
        "gemini-github-mcp": {
            ...
            "env": {
                "GITHUB_PERSONAL_ACCESS_TOKEN": process.env.GITHUB_TOKEN || "PLACEHOLDER_TOKEN_REQUIRE_MANUAL_ENTRY"
            },
            "transport": "stdio"
        },
        "gemini-graph-memory": {
    ...
    console.log(`⚠️ NOTE: If using the GitHub MCP, you must replace PLACEHOLDER_TOKEN_REQUIRE_MANUAL_ENTRY in your Cline settings with a real Personal Access Token.`);
}
