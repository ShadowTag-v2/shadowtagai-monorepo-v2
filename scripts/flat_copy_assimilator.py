...
def main() -> None:
  ...
  run_cmd(
    "git pull origin main --no-rebase -s recursive -X ours || true", cwd=monorepo_root
  )
  ...
  for _i, repo in enumerate(repos):
    ...
    clone_url = repo["clone_url"].replace(
      "https://", f"https://x-access-token:{source_token}@"
    )
    ...
    try:
      ...
      if push_res.returncode == 0:
        ...
        run_cmd(
          "git pull origin main --no-rebase -s recursive -X ours", cwd=monorepo_root
        )
