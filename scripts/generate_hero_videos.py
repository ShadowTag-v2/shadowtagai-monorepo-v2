...
def main() -> int:
  ...
  if target in ("shadowtag", "both"):
    st_path = os.path.join(
      OUTPUT_DIR, "shadowtagai", "public", "fluid-kinetic-aura.mp4"
    )
    results["shadowtag"] = generate_video(
      SHADOWTAG_PROMPT, st_path, "ShadowTagAI — Fluid Kinetic Aura"
    )

  if target in ("kovelai", "both"):
    kv_path = os.path.join(OUTPUT_DIR, "kovelai", "public", "legal-data-arch.mp4")
    results["kovelai"] = generate_video(
      KOVELAI_PROMPT, kv_path, "KovelAI — Legal Data Architecture"
    )
