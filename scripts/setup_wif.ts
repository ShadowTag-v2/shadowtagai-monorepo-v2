...
async function setupWIF() {
  ...
  await $`gcloud iam service-accounts add-iam-policy-binding ${SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com --project=${PROJECT_ID} --role="roles/iam.workloadIdentityUser" --member="${member}" 2>&1`
    .catch(() => console.log("   Binding may already exist."));
...
`);
  ...
  console.log("✅ WIF Setup Complete. GitHub Actions can now authenticate to GCP without static keys.");
}
