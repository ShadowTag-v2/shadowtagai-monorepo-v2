...
res = subprocess.run(["git", "ls-files", "--others", "--exclude-standard", "--cached"], capture_output=True, text=True)
...
for batch_idx, batch in enumerate(batches):
    print("\n==============================")
    print(f"🚀 EXECUTING BATCH {batch_idx + 1}/{len(batches)} (Pushing {len(batch)} chunks)")
    print("==============================")
    ...
    for inner_chunk in batch:
        ...
        subprocess.run(["git", "commit", "--no-verify", "-m", f"chore(core): incremental 90MB chunk {chunk_index}"])
