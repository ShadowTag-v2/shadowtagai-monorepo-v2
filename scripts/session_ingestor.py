...
def insert_chunk(
  artifact_id: str, text: str, tags: dict[str, Any], embed: list[float]
) -> bool:
  payload = json.dumps(
    {
...
def main() -> None:
  ...
  parser.add_argument(
    "--all", action="store_true", help="Ingest all groups (priority order)"
  )
  parser.add_argument(
    "--local-only",
    action="store_true",
  ...
  parser.add_argument(
    "--dry-run", action="store_true", help="Print what would be ingested"
  )
  ...
  total_not_found = sum(
    1 for v in ingest_status.values() if v.get("status") == "not_found"
  )
