...
async function provisionDataPlane() {
  ...
  await $`gcloud services enable spanner.googleapis.com datastream.googleapis.com pubsub.googleapis.com run.googleapis.com --project=${PROJECT_ID} --quiet`.catch(
    () => console.log("   APIs already enabled or auth issue."),
  );

  // Step 2: Create Pub/Sub topic + subscription
  console.log("\n[2/5] Pub/Sub Topic + Subscription...");
  await $`gcloud pubsub topics create ${TOPIC} --project=${PROJECT_ID} 2>&1`.catch(() =>
    console.log(`   Topic ${TOPIC} already exists.`),
  );
  await $`gcloud pubsub subscriptions create ${SUBSCRIPTION} --topic=${TOPIC} --push-endpoint=${HANDLER_URL} --ack-deadline=60 --project=${PROJECT_ID} 2>&1`.catch(
    () => console.log(`   Subscription ${SUBSCRIPTION} already exists.`),
  );
  ...
  const handlerStatus =
    await $`gcloud run services describe database-events-handler --region=${REGION} --project=${PROJECT_ID} --format="value(status.url)" 2>&1`
      .text()
      .catch(() => "NOT DEPLOYED");
