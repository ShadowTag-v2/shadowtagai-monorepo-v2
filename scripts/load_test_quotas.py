...
async def main() -> None:
  parser = argparse.ArgumentParser(
    description="Load test for sandbox quota enforcement"
  )
  parser.add_argument("--url", default="http://localhost:8080", help="Base URL")
  parser.add_argument(
    "--tier", default="all", choices=["trial", "solo", "practice", "enterprise", "all"]
  )
  ...
  tiers = (
    ["trial", "solo", "practice", "enterprise"] if args.tier == "all" else [args.tier]
  )
  results = []
