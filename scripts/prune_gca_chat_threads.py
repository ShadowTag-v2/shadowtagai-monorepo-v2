...
def prune(db_path: Path | str, keep: int = 0) -> dict:
  ...
  try:
    ...
    threads_bytes = len(json.dumps(threads).encode("utf-8"))
    print(
      f"  📊 chatThreads payload: {threads_bytes / 1024:.1f} KB across {len(threads)} threads"
    )
    ...
    error_keys = [
      k for k in state_dict if k.startswith(("Error:", "TypeError:", "RangeError:"))
    ]
    for k in error_keys:
      del state_dict[k]
    if error_keys:
...
def monitor_mode(threshold_mb: float = 20.0) -> None:
  ...
  while True:
    ...
    if size_mb > threshold_mb:
      print(
        f"  🚨 [{ts}] BLOAT DETECTED: {size_mb:.1f} MB > {threshold_mb:.0f} MB threshold"
      )
...
def cli_write(keep: int = 0) -> None:
  ...
  print(
    f"  🔍 GCA state: {metrics['gca_total_bytes'] / 1024:.1f} KB total, {metrics['thread_count']} threads"
  )
  ...
  print(
    f"  ✂️  Trimmed {result['threads_before']} → {result['threads_after']} threads (freed {freed_kb:.1f} KB)"
  )
  ...
  if vac["success"]:
    ...
    print(
      f"  🗜️  VACUUM: {vac['before_size'] / 1024:.0f} KB → {vac['after_size'] / 1024:.0f} KB ({pct:.0f}% recovered)"
    )
  print("✅ Done.")
...
def cli_dry_run() -> None:
  ...
  print(
    f"  💬 chatThreads: {threads_kb:.1f} KB across {metrics['thread_count']} threads"
  )
...
if __name__ == "__main__":
  ...
  if "--monitor" in args:
    ...
    if db_path:
      ...
      if result["success"]:
        print(
          f"🗜️  VACUUM: {result['before_size'] / 1024:.0f} KB → {result['after_size'] / 1024:.0f} KB"
        )
      else:
