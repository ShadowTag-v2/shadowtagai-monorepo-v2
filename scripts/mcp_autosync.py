  ...
  def _detect_drift(self, result: MCPSyncResult) -> None:
    ...
    for server in result.servers:
      ...
      if prev_tools != server.expected_tools and prev_tools > 0:
        ...
        server.drift_details = (
          f"Tool count changed: {prev_tools} → {server.expected_tools}"
        )
    ...
    for server in result.servers:
      ...
      if not server.configured:
        result.actions_taken.append(
          f"SKIP: {server.name} not configured — add to {MCP_CONFIG_PATH}"
        )
        continue
      ...
      if server.last_error and "Missing env var" in server.last_error:
        result.actions_taken.append(
          f"HEAL: {server.name} — run 'source scripts/load_mcp_secrets.sh' to refresh env vars"
        )

      # Firebase-specific healing
      if server.name == "firebase-mcp-server" and not server.reachable:
        result.actions_taken.append(
          f"HEAL: {server.name} — run 'CI=true firebase login --reauth --no-localhost'"
        )
...
def format_fleet_table(result: MCPSyncResult) -> str:
  ...
  for i, server in enumerate(result.servers, 1):
    status_emoji = (
      "🟢" if server.health_check_passed else ("🟡" if server.configured else "🔴")
    )
    health = (
      "PASS"
      if server.health_check_passed
      else ("DRIFT" if server.drift_detected else "FAIL")
    )
    lines.append(
      f"  | {i} | {server.name:<29s} | {status_emoji}     | {server.expected_tools:>5} | {health:<6} |"
    )
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="MCP Auto-Sync (ported from Claude Code mcpValidation.ts)"
  )
  parser.add_argument(
    "--check", action="store_true", help="Check fleet health (default)"
  )
  parser.add_argument(
    "--heal", action="store_true", help="Attempt self-healing on failed servers"
  )
  parser.add_argument("--json", action="store_true", help="Output as JSON")
  parser.add_argument(
    "--log", action="store_true", help="Write results to .beads/mcp_sync.jsonl"
  )
