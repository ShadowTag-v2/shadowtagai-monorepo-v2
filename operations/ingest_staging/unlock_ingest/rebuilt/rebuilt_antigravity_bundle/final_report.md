...
- A list of 56 (not 57) repos, mostly prefixed with `aiyou-` plus `pnkln`, `erik-hancock-llm-memory`, and several public demos (e.g. `antigravity-go`, `judge6`).
...
4. **Classify and Fold Repos**: 
   ...
   - Prioritise `aiyou-*` repos, `pnkln`, `erik-hancock-llm-memory`, `cosmic-crab-payload`, `Pipeline`, and `nascent-apollo` for early canonicalization.  
