...
def read_manifest(path: str) -> dict[str, Any]:
    ...
    # fallback heuristic: look for apps/pnkln_stack/... paths in raw yaml
    if not canonical_roots:
        canonical_roots = re.findall(r"apps/pnkln_stack/[A-Za-z0-9_\-]+", raw)
...
def summarize_control_plane(path: str) -> dict[str, Any]:
    raw = _safe_read(path)
    roots = re.findall(r"apps/pnkln_stack/[A-Za-z0-9_\-]+", raw)
    return {
