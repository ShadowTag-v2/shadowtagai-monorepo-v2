...
class MCPClient:
    ...
    async def execute_code(self, code: str, user_id: str, session_id: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
        """Execute code via MCP server"""
        request = {"code": code, "user_id": user_id, "session_id": session_id, "context": context or {}}
...
class ValidationRunner:
    ...
    async def run_scenario(self, scenario: TestScenario, run_number: int) -> ValidationResult:
        """Run a single test scenario"""
        start_time = time.time()

        try:
            response = await self.client.execute_code(
                code=scenario.code, user_id=self.config.user_id, session_id=f"{self.config.session_id}-{run_number}"
            )
    ...
    async def run_all_scenarios(self):
        ...
        while len(run_distribution) < self.config.num_runs:
            run_distribution.append(TEST_SCENARIOS[len(run_distribution) % len(TEST_SCENARIOS)])
    ...
    def analyze_results(self) -> ValidationSummary:
        ...
        successful_runs = sum(1 for r in functional_results if r.success and r.result_matches)
        failed_runs = sum(1 for r in functional_results if not r.success and r.expected_success)
        blocked_runs = sum(1 for r in functional_results if not r.success and not r.expected_success)

        success_rate = successful_runs / len(functional_results) * 100 if functional_results else 0
        ...
        # Security test results
        security_tests_passed = sum(1 for r in security_results if not r.success)  # Should be blocked
        security_tests_failed = sum(1 for r in security_results if r.success)  # Should NOT succeed

        # GO/NO-GO decision
        go_decision, decision_rationale = self._make_go_decision(p99_latency, success_rate, security_tests_failed)
    ...
    def _make_go_decision(self, p99_latency: float, success_rate: float, security_failures: int) -> tuple[str, str]:
        ...
        # PIVOT criteria
        if p99_latency > 75:
            return "PIVOT", f"p99 latency ({p99_latency:.1f}ms) between 75-90ms, recommend AutoGen-only deployment"

        # GO criteria
        if p99_latency <= 75 and success_rate >= 99.9 and security_failures == 0:
            return "GO", f"All criteria met: p99={p99_latency:.1f}ms, success={success_rate:.1f}%, security=PASS"
    ...
    def print_summary(self, summary: ValidationSummary):
        ...
        print(f"  p99:  {summary.p99_latency_ms:7.2f} ms {'✓' if summary.p99_latency_ms <= 75 else '✗ EXCEEDS TARGET'}")
        ...
        print(f"  Success rate:    {summary.success_rate:.2f}% {'✓' if summary.success_rate >= 99.9 else '✗ BELOW TARGET'}")
        print()

        print("CATEGORY BREAKDOWN")
        print("-" * 80)
        for category, stats in summary.category_stats.items():
            print(f"  {category:20s}: {stats['count']:4d} runs, p99={stats['p99_latency_ms']:6.2f}ms, success={stats['success_rate']:5.1f}%")
        print()

        print("SECURITY TESTS")
        print("-" * 80)
        print(f"  Tests passed (blocked): {summary.security_tests_passed}")
        print(f"  Tests failed (allowed): {summary.security_tests_failed} {'✓' if summary.security_tests_failed == 0 else '✗ SECURITY RISK'}")
        print()
        ...
        with open(results_file, "w") as f:
            json.dump({"summary": asdict(summary), "detailed_results": [asdict(r) for r in self.results]}, f, indent=2)
...
async def main():
    ...
    parser.add_argument("--mcp-url", default="http://mcp-server.mcp-system", help="MCP server URL")
    parser.add_argument("--runs", type=int, default=1000, help="Number of validation runs")
