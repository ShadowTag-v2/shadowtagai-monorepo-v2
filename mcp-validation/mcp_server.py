...
metrics_sandbox_pool_size = Gauge(
  "mcp_sandbox_pool_size", "Number of warm sandboxes available"
)

metrics_audit_log_errors = Counter(
  "mcp_audit_log_errors_total", "Failed audit log writes"
)
...
class CodeExecutionRequest(BaseModel):
  ...
  code: str = Field(
    ..., description="Python code to execute", max_length=config.MAX_CODE_LENGTH
  )
  ...
  timeout_seconds: int | None = Field(
    default=config.SANDBOX_TIMEOUT_SECONDS, ge=1, le=60
  )
