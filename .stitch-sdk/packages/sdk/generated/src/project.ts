...
export class Project {
  ...
  async generate(
    ...
    try {
      const raw = await this.client.callTool<any>('generate_screen_from_text', {
  ...
  async screens(): Promise<Screen[]> {
    try {
      ...
      return (raw?.screens || []).map(
        (item: any) => new Screen(this.client, { ...item, projectId: this.projectId }),
      );
    } catch (error) {
      throw StitchError.fromUnknown(error);
    }
  ...
  async getScreen(screenId: string): Promise<Screen> {
    try {
      const raw = await this.client.callTool<any>('get_screen', {
        projectId: this.projectId,
        screenId,
  ...
  async createDesignSystem(designSystem: any): Promise<DesignSystem> {
    try {
      const raw = await this.client.callTool<any>('create_design_system', {
        projectId: this.projectId,
        designSystem,
      });
  ...
  async listDesignSystems(): Promise<DesignSystem[]> {
    try {
      const raw = await this.client.callTool<any>('list_design_systems', {
        projectId: this.projectId,
      });
      return (raw?.designSystems || []).map(
        (item: any) => new DesignSystem(this.client, { ...item, projectId: this.projectId }),
      );
    } catch (error) {
      throw StitchError.fromUnknown(error);
    }
