...
try {
  ...
  const createdId =
    (createResult as any).name?.replace('projects/', '') ?? (createResult as any).projectId;
  ...
  assert(typeof html === 'string' && html.length > 0, `Got HTML (${html.length} chars)`);
  ...
  if (err instanceof Error) process.exit(1);
}
