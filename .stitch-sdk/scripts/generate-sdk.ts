...
import { Scope, Project as TsProject } from 'ts-morph';
import { type ArgSpec, type Binding, DomainMap, type ProjectionStep } from './ir-schema.js';
...
function generateReturnExpression(
  ...
  if (binding.returns.class) {
    ...
    if (binding.returns.array) {
      const itemExpr = parentField ? `{ ...item, ${parentField}: this.${parentField} }` : 'item';
