...
Environment Variables Required:
  ...
  ANTHROPIC_API_KEY   - Anthropic API key for Claude
    ...
    parser.add_argument("--topics", nargs="+", help="GitHub topics to search for (overrides config)")

    parser.add_argument("--days-back", type=int, help="Number of days to look back for papers/repos (default: 7)")

    parser.add_argument("--download-pdfs", action="store_true", help="Download PDF files from arXiv (slower, uses more storage)")
    ...
    try:
        briefing_file = run_pipeline(github_topics=args.topics, arxiv_days_back=args.days_back, download_pdfs=args.download_pdfs)
        ...
        print(f"  2. Check the database: sqlite3 {Path(__file__).parent / 'storage' / 'intel_pipeline.db'}")
