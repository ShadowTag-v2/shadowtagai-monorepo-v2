...
class ArxivCrawler:
    ...
    def __init__(self):
        ...
        logger.info("arxiv_crawler_initialized", categories=self.config["categories"], storage_path=str(self.storage_path))
        ...
        for category in categories:
            for search_term in search_terms:
                try:
                    ...
                    logger.info("searching_arxiv", category=category, search_term=search_term, query=query)

                    # Search arXiv
                    search = arxiv.Search(
                        query=query, max_results=max_results, sort_by=arxiv.SortCriterion.SubmittedDate, sort_order=arxiv.SortOrder.Descending
                    )
                    ...
                    logger.error("arxiv_search_error", category=category, search_term=search_term, error=str(e))
                    continue

        logger.info("arxiv_search_complete", total_papers=len(all_papers), categories=categories, days_back=days_back)
...
"""
    ...
    def download_paper(self, paper: arxiv.Result, download_pdf: bool = True) -> str | None:
        ...
        try:
            ...
            logger.info("paper_metadata_saved", arxiv_id=arxiv_id, title=paper.title, file=str(metadata_file))
    ...
    def download_papers(self, papers: list[arxiv.Result] | None = None, download_pdfs: bool = False, **search_kwargs) -> list[str]:
        """
        ...
        logger.info("papers_download_complete", total_papers=len(papers), successful=len(downloaded_files))

        return downloaded_files

    def get_recent_papers(self, days_back: int | None = None, download_pdfs: bool = False) -> list[str]:
        """
    ...
    def search_by_keywords(self, keywords: list[str], max_results: int = 50, days_back: int = 30, download_pdfs: bool = False) -> list[str]:
        """
        ...
        Returns:
            List of paths to downloaded metadata files
        """
        return self.download_papers(search_terms=keywords, max_results=max_results, days_back=days_back, download_pdfs=download_pdfs)
    ...
    def generate_summary(self, papers: list[arxiv.Result]) -> str:
        ...
        for category, cat_papers in sorted(by_category.items()):
            ...
            for paper in cat_papers[:10]:  # Top 10 per category
                ...
                summary_parts.append(f"- **{paper.title}** - {authors} et al. ({paper.published.strftime('%Y-%m-%d')})")
