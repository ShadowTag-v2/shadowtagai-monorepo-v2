...
class GitHubFlattener:
  ...
  def flatten_repository(self, repo: Repository.Repository) -> str | None:
    ...
    try:
      logger.info(
        "flattening_repository", name=repo.full_name, stars=repo.stargazers_count
      )
            ...
            if content_file.type == "dir":
              ...
              if content_file.size > max_size_bytes:
                logger.warning(
                  ...
                  size_mb=content_file.size / (1024 * 1024),
                )
                continue
              ...
              if files_processed % 10 == 0:
                logger.debug(
                  "flattening_progress", repo=repo.full_name, files=files_processed
                )
      ...
      logger.error(
        "github_api_error", repo=repo.full_name, error=str(e), status=e.status
      )
      return None
    except Exception as e:
  ...
  def flatten_repositories(
    ...
    for i, repo in enumerate(repos, 1):
      logger.info(
        "processing_repository", index=i, total=len(repos), repo=repo.full_name
      )
    ...
    logger.info(
      "flattening_complete", total_repos=len(repos), successful=len(flattened_files)
    )
  ...
  def get_recent_updates(self, days_back: int = 7) -> list[Repository.Repository]:
    ...
    for topic in self.config["target_topics"]:
      try:
        query = (
          f"topic:{topic} stars:>={self.config['min_stars']} pushed:>={cutoff_str}"
        )
