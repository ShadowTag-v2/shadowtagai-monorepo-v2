...
class CircuitBreaker:
    ...
    def record_failure(self):
        ...
        if self.failures >= self.failure_threshold:
            ...
            logger.warning("circuit_breaker_opened", failures=self.failures, timeout=self.timeout_seconds)
...
class EthicalScraper:
    ...
    def __init__(self, config: dict | None = None):
        ...
        logger.info("ethical_scraper_initialized", user_agent=self.user_agent, rate_limits=self.rate_config)
    ...
    async def _fetch_robots_txt(self, url: str) -> RobotFileParser:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(robots_url, headers={"User-Agent": self.user_agent}, timeout=aiohttp.ClientTimeout(total=10)) as response:
        ...
        if domain in self.last_request:
            ...
            if remaining > 0:
                ...
                if self.rate_config["adaptive_throttling"]:
                    jitter = random.uniform(-delay * self.rate_config["jitter_factor"], delay * self.rate_config["jitter_factor"])
                    remaining = max(0.1, remaining + jitter)

                logger.debug("rate_limit_applied", domain=domain, delay=remaining, configured_delay=delay)
    ...
    def get_circuit_breaker(self, domain: str) -> CircuitBreaker:
        ...
        if domain not in self.circuit_breakers:
            self.circuit_breakers[domain] = CircuitBreaker(
                failure_threshold=self.circuit_config["failure_threshold"], timeout_seconds=self.circuit_config["timeout_seconds"]
            )
        return self.circuit_breakers[domain]

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=10), reraise=True)
    async def fetch_url(self, url: str, method: str = "GET", headers: dict | None = None, **kwargs) -> tuple[int, str]:
        """
        ...
        try:
            ...
            logger.error("url_fetch_error", url=url, domain=domain, error=str(e), circuit_failures=circuit.failures)
            raise

    async def fetch_multiple(self, urls: list[str], max_concurrent: int | None = None) -> dict[str, tuple[int, str]]:
        """
