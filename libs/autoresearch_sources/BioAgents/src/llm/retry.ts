 ...
 */
export function getFallbackConfig(provider: string): { provider: string; model: string } | null {
...
export async function withRetry<T>(
  ...
  if (enableFallback && lastError) {
    ...
    if (fallbackConfig) {
      ...
      const fallbackError = new Error(`PRIMARY_PROVIDER_FAILED:${fallbackConfig.provider}`);
