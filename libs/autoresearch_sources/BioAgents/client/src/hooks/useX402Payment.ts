...
export function useX402Payment(): UseX402PaymentReturn {
  ...
  const checkBalanceForAddress = useCallback(
  ...
  const canSign = Boolean(config?.enabled) && Boolean(walletAddress) && !isConnecting;
  ...
  const hasInsufficientBalance = usdcBalance !== null && parseFloat(usdcBalance) < 0.1;
