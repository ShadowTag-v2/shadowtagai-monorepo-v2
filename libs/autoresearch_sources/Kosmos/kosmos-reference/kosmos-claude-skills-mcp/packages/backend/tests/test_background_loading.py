...
import pytest
from claude_skills_mcp_backend.mcp_handlers import LoadingState, SkillsMCPServer
from claude_skills_mcp_backend.search_engine import SkillSearchEngine
from claude_skills_mcp_backend.skill_loader import Skill, load_skills_in_batches
...
def test_background_loading_with_server():
    ...
    def background_loader():
        ...
        skills = [
            Skill(f"Skill {i}", f"Description {i}", f"Content {i}", f"source{i}") for i in range(20)
        ]
