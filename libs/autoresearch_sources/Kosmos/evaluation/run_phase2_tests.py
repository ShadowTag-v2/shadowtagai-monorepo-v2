...
def test_hypothesis_generation(research_question=None, domain=None):
    ...
    response = agent.generate_hypotheses(
        research_question=rq, domain=dom, num_hypotheses=3, store_in_db=False
    )

    # Response is a HypothesisGenerationResponse
    hypotheses = (
        getattr(response, "hypotheses", response) if not isinstance(response, list) else response
    )
    ...
    result["novelty_checker_note"] = (
        "NoveltyChecker exists but never called from HypothesisGeneratorAgent"
    )
...
def test_experiment_design(research_question=None, domain=None):
    ...
    for p in protocols:
        ...
        print(
            f"    Steps: {p_data['step_count']}, Controls: {p_data['has_controls']}, Stats: {p_data['has_statistical_tests']}"
        )
    ...
    protocol = ExperimentProtocol(
        ...
        variables={
            ...
            "measurement": Variable(
                name="measurement", type=VariableType.DEPENDENT, description="Measurement variable"
            ),
        },
        statistical_tests=[
            StatisticalTestSpec(
        ...
        resource_requirements=ResourceRequirements(
            estimated_duration_minutes=5, estimated_cost_usd=0.01, compute_hours=0.1
        ),
    )
...
def test_data_analysis(research_question=None, domain=None):
    ...
    interpretation = agent.interpret_results(
        result=mock_result, hypothesis=hypothesis, literature_context=None
    )
