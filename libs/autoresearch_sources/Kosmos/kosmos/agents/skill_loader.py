...
class SkillLoader:
    ...
    def _find_skills_directory(self) -> str | None:
        ...
        # Try one level up
        skills_path = (
            kosmos_package_dir.parent / "kosmos-claude-scientific-skills" / "scientific-skills"
        )
        if skills_path.exists():
            logger.info(f"Found skills directory at {skills_path}")
            return str(skills_path)
    ...
    def load_skill_from_file(self, skill_path: Path, skill_name: str | None = None) -> dict:
