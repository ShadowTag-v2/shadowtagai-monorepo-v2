...
class FailureDetectionResult:
    ...
    def has_failures(self) -> bool:
        ...
        return (
            self.over_interpretation.detected
            or self.invented_metrics.detected
            or self.rabbit_hole.detected
        )
...
class FailureDetector:
    ...
    def detect_failures(
        ...
        recommendations = list(
            dict.fromkeys(
                over_interp.recommendations + invented.recommendations + rabbit_hole.recommendations
            )
        )
    ...
    def _measure_statistical_strength(self, statistics: dict[str, Any]) -> float:
        ...
        effect_size = (
            statistics.get("effect_size") or statistics.get("cohens_d") or statistics.get("d")
        )
    ...
    def _generate_over_interp_evidence(
        self, claim_strength: float, stat_strength: float, text: str, statistics: dict[str, Any]
    ) -> list[str]:
    ...
    def detect_invented_metrics(
        self, finding: dict[str, Any], dataset_schema: list[str] | None = None
    ) -> FailureModeScore:
        """
        ...
        if dataset_schema:
            valid_metrics.update({col.lower().replace(" ", "_") for col in dataset_schema})

        # Add metrics from statistics dict (they're at least defined somewhere)
        if statistics:
            valid_metrics.update({k.lower() for k in statistics.keys()})
    ...
    def detect_rabbit_hole(
        ...
        if generation_penalty > 0:
            evidence.append(
                f"Hypothesis generation: {generation} (penalty: {generation_penalty:.2f})"
            )
