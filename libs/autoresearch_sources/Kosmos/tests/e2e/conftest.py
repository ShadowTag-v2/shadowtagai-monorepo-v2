...
requires_llm = pytest.mark.skipif(
    not HAS_ANY_LLM_KEY, reason="Requires ANTHROPIC_API_KEY or OPENAI_API_KEY"
)

requires_anthropic = pytest.mark.skipif(not HAS_ANTHROPIC_KEY, reason="Requires ANTHROPIC_API_KEY")

requires_openai = pytest.mark.skipif(not HAS_OPENAI_KEY, reason="Requires OPENAI_API_KEY")

requires_neo4j = pytest.mark.skipif(not HAS_NEO4J, reason="Requires NEO4J_URI environment variable")

requires_docker = pytest.mark.skipif(not HAS_DOCKER, reason="Requires Docker to be running")
...
def configured_metrics_collector():
    ...
    from kosmos.core.metrics import BudgetPeriod, MetricsCollector

    collector = MetricsCollector()
    collector.configure_budget(
        limit_usd=1.00, period=BudgetPeriod.DAILY, alert_thresholds=[50.0, 75.0, 90.0, 100.0]
    )
    yield collector
    collector.reset()
...
def rate_limiter():
    ...
    return RateLimiter(max_requests_per_minute=60, max_concurrent=5)
...
def research_plan():
    ...
    return ResearchPlan(research_question="Test research question", max_iterations=5)
