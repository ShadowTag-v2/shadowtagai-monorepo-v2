...
def analyzer_with_embeddings_disabled():
    ...
    return ConvergenceAnalyzer(
        similarity_threshold=0.70, replication_threshold=0.6, use_embeddings=False
    )
...
class TestEnsembleFullPipeline:
    ...
    @pytest.mark.asyncio
    async def test_full_pipeline_with_mocked_workflows(
        self, realistic_multi_run_findings, run_configs_5
    ):
        """Test complete pipeline from raw findings to convergence report."""
        # Create analyzer
        analyzer = ConvergenceAnalyzer(similarity_threshold=0.70, use_embeddings=False)
    ...
    async def test_runner_integration_with_mocked_workflow(self):
        ...
        with (
            patch("kosmos.workflow.research_loop.ResearchWorkflow") as mock_class,
            ...
            def create_mock_workflow(*args, **kwargs):
                ...
                mock.state_manager.get_all_findings = Mock(
                    return_value=[Mock(to_dict=Mock(return_value=f)) for f in findings]
                )
                return mock
...
class TestFindingMatching:
    ...
    def test_similar_findings_cluster_together(self, analyzer_with_embeddings_disabled):
        ...
        findings = [
            ...
            [
                {
                    "summary": "KRAS mutations cause drug resistance",
                    "statistics": {"effect_size": 0.7},
                }
            ],
            # Run 1
            [
                {
                    "summary": "KRAS mutations lead to drug resistance",
                    "statistics": {"effect_size": 0.75},
                }
            ],
            # Run 2
            [
...
class TestConvergenceReport:
    ...
    def test_report_formats_statistics_correctly(self, run_configs_5):
        ...
        metrics = ConvergenceMetrics(
            total_runs=5, total_unique_findings=1, total_raw_findings=4, strong_convergence_count=1
        )
    ...
    def test_report_includes_seeds(self, run_configs_5):
        ...
        metrics = ConvergenceMetrics(total_runs=5, total_unique_findings=1, total_raw_findings=1)
