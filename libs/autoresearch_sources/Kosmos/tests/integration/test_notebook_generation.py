...
from kosmos.execution.jupyter_client import CellOutput, ExecutionResult, ExecutionStatus
...
"""
        metadata = generator.create_notebook(
            code=code, cycle=1, task_id=1, analysis_type="correlation", title="Correlation Analysis"
        )
    ...
    def test_notebook_has_correct_structure(self, generator):
        ...
        code = "print('hello')"
        metadata = generator.create_notebook(
            code=code, cycle=1, task_id=1, analysis_type="test", title="Test Notebook"
        )
    ...
    def test_notebook_validates(self, generator):
        ...
        code = "x = 1 + 2\nprint(x)"
        metadata = generator.create_notebook(code=code, cycle=1, task_id=1, analysis_type="test")
    ...
    def test_outputs_embedded_correctly(self, generator):
        ...
        mock_result = ExecutionResult(
            status=ExecutionStatus.COMPLETED, stdout="test output\n", stderr="", execution_time=0.5
        )

        metadata = generator.create_notebook(
            code=code, cycle=1, task_id=1, analysis_type="test", execution_result=mock_result
        )
    ...
    def test_figures_referenced(self, generator, tmp_path):
        ...
        metadata = generator.create_notebook(
            code=code, cycle=1, task_id=1, analysis_type="test", figure_paths=figure_paths
        )
    ...
    def test_hypothesis_in_header(self, generator):
        ...
        metadata = generator.create_notebook(
            code="print('test')", cycle=1, task_id=1, analysis_type="test", hypothesis=hypothesis
        )
    ...
    def test_multiple_notebooks_per_cycle(self, generator, tmp_path):
        ...
        for i in range(3):
            generator.create_notebook(
                code=f"print('task {i}')", cycle=1, task_id=i, analysis_type="test"
            )
...
class TestNotebookKernels:
    ...
    def test_python_kernel_notebook(self, generator):
        ...
        code = "import pandas as pd\ndf = pd.DataFrame()"
        metadata = generator.create_notebook(
            code=code, cycle=1, task_id=1, analysis_type="test", kernel="python3"
        )
...
"""
        metadata = generator.create_notebook(
            code=r_code, cycle=1, task_id=1, analysis_type="r_analysis", kernel="ir"
        )
...
class TestFindingIntegration:
    ...
    def test_finding_with_notebook_metadata(self):
        ...
        finding = Finding(
            ...
            notebook_metadata={"kernel": "python3", "line_count": 50, "cell_count": 5},
        )
...
class TestConvenienceFunction:
    ...
    def test_create_notebook_from_code_validates(self, tmp_path):
        ...
        create_notebook_from_code(
            code="print('hello')", output_path=output_path, title="Test Notebook"
        )
...
class TestExecutionResultIntegration:
    ...
    def test_with_cell_outputs(self, generator):
        ...
        cell_output = CellOutput(
            output_type="stream", content="Hello from cell output", mime_type="text/plain"
        )

        result = ExecutionResult(
            status=ExecutionStatus.COMPLETED, outputs=[cell_output], execution_time=0.5
        )

        metadata = generator.create_notebook(
            code="print('hello')", cycle=1, task_id=1, analysis_type="test", execution_result=result
        )
