...
from kosmos.agents.base import BaseAgent, MessageType
...
class TestAsyncMessagePassing:
    ...
    async def test_async_broadcast_message(self, registry):
        ...
        messages = await registry.broadcast_message(
            from_agent_id="broadcaster", content={"broadcast": "test"}, target_types=["Receiver"]
        )
...
class TestAsyncResearchDirector:
    ...
    def research_director(self):
        ...
        director = ResearchDirectorAgent(
            ...
            domain="biology",
            config={"max_iterations": 2, "enable_concurrent_operations": False},
        )
    ...
    async def test_async_send_to_hypothesis_generator(self, research_director):
        ...
        message = await research_director._send_to_hypothesis_generator(
            action="generate", context={"max_hypotheses": 3}
        )
    ...
    async def test_async_send_to_experiment_designer(self, research_director):
        ...
        message = await research_director._send_to_experiment_designer(hypothesis_id="test-hyp-001")
...
class TestAsyncCLIIntegration:
    ...
    async def test_director_registration_in_cli_flow(self):
        ...
        director = ResearchDirectorAgent(
            research_question="Test question", domain="biology", config={"max_iterations": 1}
        )
...
class TestAsyncErrorHandling:
    ...
    async def test_async_execute_unknown_action_raises(self):
        ...
        director = ResearchDirectorAgent(research_question="Test", domain="biology")
    ...
    async def test_async_message_to_unregistered_agent(self):
        ...
        message = await sender.send_message(to_agent="nonexistent", content={"test": "data"})
...
class TestAsyncPerformance:
    ...
    async def test_concurrent_message_sending(self):
        ...
        receivers = [
            BaseAgent(agent_id=f"perf_receiver_{i}", agent_type="Receiver") for i in range(5)
        ]
        ...
        tasks = [
            sender.send_message(to_agent=f"perf_receiver_{i}", content={"index": i})
            for i in range(5)
        ]
