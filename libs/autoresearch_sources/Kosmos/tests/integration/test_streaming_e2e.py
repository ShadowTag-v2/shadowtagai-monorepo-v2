...
class TestStageTrackerEventBusIntegration:
    ...
    def test_stage_tracker_emits_to_event_bus(self):
        ...
        event_bus.subscribe(
            callback, event_types=[EventType.STAGE_STARTED, EventType.STAGE_COMPLETED]
        )

        # Create tracker with event bus integration enabled
        tracker = StageTracker(process_id="test_proc", output_file=None, emit_to_event_bus=True)
    ...
    def test_stage_tracker_with_callback(self):
        ...
        def my_callback(event):
            ...
            callback_events.append({"stage": event.stage, "status": event.status})

        # Create tracker with callback
        tracker = StageTracker(process_id="test_proc", output_file=None, emit_to_event_bus=False)
    ...
    def test_stage_tracker_disabled_no_events(self):
        ...
        tracker = StageTracker(process_id="test_proc", enabled=False)
...
class TestEventBusFiltering:
    ...
    async def test_filter_by_process_id(self):
        ...
        await event_bus.publish(WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="proc_1"))
        await event_bus.publish(WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="proc_2"))
...
class TestWorkflowEventFlow:
    ...
    async def test_complete_workflow_event_sequence(self):
        ...
        for cycle in range(1, 4):
            await event_bus.publish(
                CycleEvent(
                    type=EventType.CYCLE_STARTED, process_id="workflow_1", cycle=cycle, max_cycles=3
                )
            )
...
class TestLLMEventFlow:
    ...
    def test_llm_token_streaming_sync(self):
        ...
        event_bus.publish_sync(
            LLMEvent(
                type=EventType.LLM_CALL_STARTED, model="claude-3-5-sonnet", provider="anthropic"
            )
        )

        for word in ["Hello", " ", "world", "!"]:
            event_bus.publish_sync(
                LLMEvent(type=EventType.LLM_TOKEN, model="claude-3-5-sonnet", token=word)
            )
...
class TestCLIStreamingDisplay:
    ...
    def test_streaming_display_handles_events(self):
        ...
        try:
            ...
            event_bus.publish_sync(
                WorkflowEvent(
                    type=EventType.WORKFLOW_STARTED, research_question="Test question", max_cycles=5
                )
            )
    ...
    async def test_event_generator_yields_events(self):
        ...
        try:
            ...
            async def event_producer():
                ...
                await event_bus.publish(
                    WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="test_proc")
                )
