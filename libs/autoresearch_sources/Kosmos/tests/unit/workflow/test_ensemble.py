...
def run_configs():
    ...
    return [
        RunConfig(run_id=f"run_{i}", seed=42 + i, temperature=0.7, run_index=i) for i in range(5)
    ]
...
class TestRunConfig:
    ...
    def test_runconfig_to_dict(self):
        ...
        assert d == {"run_id": "run_0", "seed": 42, "temperature": 0.7, "run_index": 0}
...
class TestConvergenceMetrics:
    ...
    def test_convergencemetrics_creation(self):
        ...
        metrics = ConvergenceMetrics(total_runs=5, total_unique_findings=10, total_raw_findings=25)
...
class TestEnsembleResult:
    ...
    def test_ensembleresult_creation(self, run_configs):
        ...
        result = EnsembleResult(
            research_objective="Test objective", n_runs=5, run_configs=run_configs
        )
    ...
    def test_ensembleresult_to_dict(self, run_configs):
        ...
        metrics = ConvergenceMetrics(total_runs=5, total_unique_findings=3, total_raw_findings=10)
...
class TestConvergenceAnalyzer:
    ...
    def test_analyzer_initialization(self):
        ...
        analyzer = ConvergenceAnalyzer(
            similarity_threshold=0.85, replication_threshold=0.7, use_embeddings=False
        )
    ...
    def test_compute_text_similarity_identical(self, analyzer):
        ...
        sim = analyzer._compute_text_similarity(
            "KRAS mutations cause resistance", "KRAS mutations cause resistance"
        )
        assert sim == 1.0
    ...
    def test_compute_text_similarity_different(self, analyzer):
        ...
        sim = analyzer._compute_text_similarity(
            "KRAS mutations in lung cancer", "Weather patterns in Antarctica"
        )
    ...
    def test_compute_finding_similarity(self, analyzer, sample_findings):
        ...
        # Similar findings
        sim = analyzer._compute_finding_similarity(sample_findings[0], sample_findings[1])
        assert sim > 0.5  # Should be similar (both about KRAS)

        # Different findings
        sim = analyzer._compute_finding_similarity(sample_findings[0], sample_findings[2])
...
class TestEnsembleRunner:
    ...
    async def test_execute_single_run(self):
        ...
        with (
            patch("kosmos.workflow.research_loop.ResearchWorkflow") as mock_workflow_class,
            ...
            mock_workflow.state_manager.get_all_findings = Mock(
                return_value=[Mock(to_dict=Mock(return_value={"summary": "test"}))]
            )
            ...
            result, findings = await runner._execute_single_run(config, "Test objective", 1, 5)
...
class TestConvergenceReporter:
    ...
    def test_generate_json_report(self, run_configs):
        ...
        metrics = ConvergenceMetrics(total_runs=5, total_unique_findings=5, total_raw_findings=10)
    ...
    def test_report_includes_non_replicating(self, run_configs):
        ...
        metrics = ConvergenceMetrics(
            total_runs=5, total_unique_findings=1, total_raw_findings=1, findings_replicated_1=1
        )
    ...
    def test_report_methodology_section(self, run_configs):
        ...
        metrics = ConvergenceMetrics(total_runs=5, total_unique_findings=0, total_raw_findings=0)
