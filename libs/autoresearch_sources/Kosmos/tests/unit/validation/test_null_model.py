...
from kosmos.validation.null_model import NullModelResult, NullModelValidator
...
class TestNullModelResult:
    ...
    def test_create_null_model_result(self):
        ...
        assert result.passes_null_test
        assert not result.persists_in_noise
    ...
    def test_is_valid_property(self):
        ...
        assert not result_fail_null.is_valid
        ...
        assert not result_persists.is_valid
...
class TestNullModelValidatorInit:
    ...
    def test_custom_init(self):
        ...
        validator = NullModelValidator(
            n_permutations=500, alpha=0.01, random_seed=42, persistence_threshold=0.6
        )
...
class TestShuffleMethods:
    ...
    def test_shuffle_columns(self):
        ...
        assert not (
            list(shuffled["a"].values) == list(df["a"].values)
            and list(shuffled["b"].values) == list(df["b"].values)
        )
...
class TestValidateFinding:
    ...
    def test_weak_finding_fails(self):
        ...
        assert result.permutation_p_value > 0.05 or result.persists_in_noise
    ...
    def test_computation_time_tracked(self):
        ...
        finding = {
            "statistics": {"test_type": "t_test", "statistic": 2.5, "degrees_of_freedom": 30}
        }
