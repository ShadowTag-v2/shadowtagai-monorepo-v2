...
class TestBatchResponse:
    ...
    def test_error_response(self):
        """Test error response."""
        resp = BatchResponse(id="test-1", success=False, error="Rate limit exceeded")
...
class TestAsyncClaudeClient:
    ...
    async def test_batch_generate(self, real_async_client):
        ...
        requests = [
            BatchRequest(id="1", prompt=unique_prompt("Say 'one'"), temperature=0.0, max_tokens=20),
            BatchRequest(id="2", prompt=unique_prompt("Say 'two'"), temperature=0.0, max_tokens=20),
        ]
    ...
    async def test_concurrency_limiting(self, real_async_client):
        ...
        requests = [
            BatchRequest(
                id=str(i), prompt=unique_prompt(f"Count: {i}"), temperature=0.0, max_tokens=10
            )
            for i in range(5)
        ]

        responses = await asyncio.wait_for(real_async_client.batch_generate(requests), timeout=90.0)
    ...
    async def test_token_counting(self, real_async_client):
        ...
        requests = [
            BatchRequest(id="1", prompt=unique_prompt("Hi"), temperature=0.0, max_tokens=10),
    ...
    async def test_execution_time_tracking(self, real_async_client):
        ...
        requests = [
            BatchRequest(id="1", prompt=unique_prompt("Test"), temperature=0.0, max_tokens=10)
        ]

        responses = await asyncio.wait_for(real_async_client.batch_generate(requests), timeout=30.0)
...
class TestErrorHandling:
    ...
    async def test_empty_prompt(self, real_async_client):
        ...
        try:
            await asyncio.wait_for(real_async_client.async_generate(prompt=""), timeout=30.0)
