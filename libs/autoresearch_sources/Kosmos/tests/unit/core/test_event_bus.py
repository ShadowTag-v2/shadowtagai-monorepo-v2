...
class TestEventTypeFiltering:
    ...
    def test_subscribe_to_multiple_types(self, event_bus):
        ...
        callback = Mock()
        event_bus.subscribe(
            callback, event_types=[EventType.LLM_TOKEN, EventType.LLM_CALL_COMPLETED]
        )
...
class TestProcessIdFiltering:
    ...
    def test_filter_by_process_id(self, event_bus):
        ...
        event1 = WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="proc_123")
        event_bus.publish_sync(event1)
        assert callback.call_count == 1

        event2 = WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="proc_456")
        event_bus.publish_sync(event2)
        assert callback.call_count == 1  # Still 1
    ...
    def test_filter_by_multiple_process_ids(self, event_bus):
        ...
        event1 = WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="proc_123")
        event2 = WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="proc_456")
        event3 = WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="proc_789")
    ...
    def test_combined_type_and_process_filter(self, event_bus):
        ...
        callback = Mock()
        event_bus.subscribe(callback, event_types=[EventType.LLM_TOKEN], process_ids=["proc_123"])

        # Match both - should receive
        event1 = LLMEvent(type=EventType.LLM_TOKEN, process_id="proc_123", token="Hi")
        event_bus.publish_sync(event1)
        assert callback.call_count == 1

        # Match type but not process - should NOT receive
        event2 = LLMEvent(type=EventType.LLM_TOKEN, process_id="proc_456", token="Hi")
        event_bus.publish_sync(event2)
        assert callback.call_count == 1

        # Match process but not type - should NOT receive
        event3 = WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id="proc_123")
        event_bus.publish_sync(event3)
        assert callback.call_count == 1
...
class TestEventWithoutProcessId:
    ...
    def test_event_with_none_process_id(self, event_bus):
        ...
        event = WorkflowEvent(type=EventType.WORKFLOW_STARTED, process_id=None)
        event_bus.publish_sync(event)
