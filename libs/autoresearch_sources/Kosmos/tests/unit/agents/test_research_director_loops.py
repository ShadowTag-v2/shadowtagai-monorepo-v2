...
from kosmos.agents.research_director import MAX_ACTIONS_PER_ITERATION, ResearchDirectorAgent
...
def mock_director():
    ...
    with (
        patch("kosmos.agents.research_director.get_client") as mock_client,
...
class TestBugADesigningStateFix:
    ...
    def test_designing_empty_converges(self, mock_director):
        ...
        assert action == NextAction.CONVERGE, "Should converge when nothing left to do"
...
class TestBugBAnalyzingStateFix:
    ...
    def test_analyzing_with_results_analyzes(self, mock_director):
        ...
        assert action == NextAction.ANALYZE_RESULT, "Should analyze when results exist"
...
class TestBugDExecutingStateFix:
    ...
    def test_executing_empty_queue_with_results_analyzes(self, mock_director):
        ...
        assert action == NextAction.ANALYZE_RESULT, "Should analyze results when queue empty"
    ...
    def test_executing_empty_everything_refines(self, mock_director):
        ...
        assert action == NextAction.REFINE_HYPOTHESIS, "Should refine to recover from stuck state"
    ...
    def test_executing_with_queue_executes(self, mock_director):
        ...
        assert action == NextAction.EXECUTE_EXPERIMENT, "Should execute when queue has items"
...
class TestMaxActionsPerIterationSafety:
    ...
    def test_action_counter_forces_convergence(self, mock_director):
        ...
        assert action == NextAction.CONVERGE, "Should force convergence when action limit exceeded"
    ...
    def test_action_counter_increments(self, mock_director):
        ...
        assert mock_director._actions_this_iteration == 1, "Action counter should start at 1"

        mock_director.decide_next_action()

        assert mock_director._actions_this_iteration == 2, "Action counter should increment"

    def test_max_actions_constant_defined(self):
        """MAX_ACTIONS_PER_ITERATION should be defined and reasonable."""
        assert MAX_ACTIONS_PER_ITERATION > 0, "MAX_ACTIONS should be positive"
        assert MAX_ACTIONS_PER_ITERATION >= 10, "MAX_ACTIONS should allow reasonable workflow"
        assert MAX_ACTIONS_PER_ITERATION <= 100, "MAX_ACTIONS should prevent runaway loops"
