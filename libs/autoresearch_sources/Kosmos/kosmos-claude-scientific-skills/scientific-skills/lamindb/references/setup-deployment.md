...
# S3 + PostgreSQL
lamin init --storage s3://my-bucket/path \
  --db postgresql://REDACTED_USER:REDACTED_PASS@hostname:5432/dbname \
  --modules bionty
...
lamin init --storage s3://my-bucket/project-data \
  --db postgresql://REDACTED_USER:REDACTED_PASS@host:5432/db
```
...
lamin init --storage s3://bucket/path \
  --db postgresql://REDACTED_USER:REDACTED_PASS@hostname:5432/database

# With SSL
lamin init --storage s3://bucket/path \
  --db "postgresql://REDACTED_USER:REDACTED_PASS@host:5432/db?sslmode=require"
```
...
lamin init --storage s3://prod-bucket/data \
  --db postgresql://REDACTED_USER:REDACTED_PASS@db-host:5432/prod-db \
  --modules bionty \
  --name production
```
...
lamin init --storage s3://us-bucket/data \
  --db postgresql://REDACTED_USER:REDACTED_PASS@us-db:5432/db \
  --name us-production

# EU instance
lamin init --storage s3://eu-bucket/data \
  --db postgresql://REDACTED_USER:REDACTED_PASS@eu-db:5432/db \
  --name eu-production
```
...
lamin init --storage s3://shared-bucket/data \
  --db postgresql://REDACTED_USER:REDACTED_PASS@host:5432/user1_db \
  --name user1-workspace

lamin init --storage s3://shared-bucket/data \
  --db postgresql://REDACTED_USER:REDACTED_PASS@host:5432/user2_db \
...
psql postgresql://REDACTED_USER:REDACTED_PASS@host:5432/db
