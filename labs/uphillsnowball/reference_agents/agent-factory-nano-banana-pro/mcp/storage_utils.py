...
ai_bucket_name = os.environ.get("AI_ASSETS_BUCKET", f"{project_id}-adk-video-agent")
...
def download_data_from_gcs(url: str) -> types.Blob:
    ...
    mime_type = mimetypes.guess_type(file_name)[0] or blob.content_type or "application/octet-stream"
    if ";" in mime_type:
        mime_type = mime_type.split(";")[0]
    return types.Blob(display_name=file_name, data=blob_data, mime_type=mime_type.strip())
