...
class AgentMemory:
    ...
    async def save_agent_state(self, state: AgentState) -> None:
        ...
        await db.collection("agent_state").document(state.agent_id).set({
            "role": state.role,
            "status": state.status,
    ...
    async def record_task(self, record: TaskRecord) -> None:
        ...
        await db.collection("task_history").document(record.task_id).set({
    ...
    async def save_learned_pattern(self, pattern: LearnedPattern) -> None:
        ...
        await db.collection("learned").document(pattern.pattern_id).set({
