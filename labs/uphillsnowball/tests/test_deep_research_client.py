...
class TestDeepResearchClient:
    ...
    def test_init_missing_key_raises(self):
        ...
        with patch.dict("os.environ", {}, clear=True), pytest.raises(ValueError, match="GEMINI_API_KEY not set"):
            DeepResearchClient()
    ...
    def test_resolve_agent_standard(self, mock_genai):
        ...
        with patch.dict("os.environ", {"GEMINI_API_KEY": "test-key"}):
            ...
            assert client._resolve_agent(ResearchTier.STANDARD) == "deep-research-preview-04-2026"

    def test_resolve_agent_max(self, mock_genai):
        """Max tier should use the max agent."""
        with patch.dict("os.environ", {"GEMINI_API_KEY": "test-key"}):
            client = DeepResearchClient()
            assert client._resolve_agent(ResearchTier.MAX) == "deep-research-max-preview-04-2026"
