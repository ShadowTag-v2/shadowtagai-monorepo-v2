...
class SecureBlastOrchestrator:
  ...
  async def extract_intelligence_safely(self, target_source: str):
    ...
    async with (
      stdio_client(switchboard_params) as (r_sw, w_sw),
      ClientSession(r_sw, w_sw) as sw_session,
    ):
    ...
    async with (
      stdio_client(notebooklm_params) as (r_nl, w_nl),
      ClientSession(r_nl, w_nl) as nl_session,
    ):
