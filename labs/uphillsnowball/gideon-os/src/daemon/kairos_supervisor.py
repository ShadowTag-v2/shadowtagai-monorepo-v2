...
class KairosSessionSupervisor:
    ...
    async def execute_user_command(self, command: str, context: dict):
        ...
        if estimated_time > 15:
            logger.info(
                "⏳ [KAIROS] Budget >15s. Farming to ULTRAPLAN remote instance."
            )
    ...
    async def _dispatch_ultraplan(self, objective: str, context: dict):
        ...
        parent = self.task_client.queue_path(
            self.project, "us-central1", "ultraplan-queue"
        )
    ...
    async def nightly_auto_dream(self):
        ...
        logger.info(
            "🌙 [KAIROS] Midnight boundary reached. Initiating Auto-Dream..."
        )
        ...
        LOG: {yesterday_log}
        """
        consolidation = await self.client.models.generate_content_async(
    ...
    def _append_daily_log(self, cmd: str, result: str):
        ...
        path = os.path.expanduser(
            f"~/.claude/projects/gideon/memory/logs/{today[:4]}/{today[5:7]}/{today}.md"
        )
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "a") as f:
            f.write(
                f"- [{datetime.utcnow().isoformat()}] CMD: {cmd}"
                f" | RES: {result[:50]}...\n"
            )
    ...
    def _estimate_complexity(self, cmd):
        return (
            30
            if "research" in cmd.lower() or "build" in cmd.lower()
            else 5
        )
