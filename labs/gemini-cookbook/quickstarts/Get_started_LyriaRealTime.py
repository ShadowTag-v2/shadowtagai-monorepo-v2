...
async def main():
    ...
    async with client.aio.live.music.connect(model=MODEL) as session:
        ...
        async def receive():
            chunks_count = 0
            output_stream = p.open(format=FORMAT, channels=CHANNELS, rate=OUTPUT_RATE, output=True, frames_per_buffer=CHUNK)
        ...
        async def send():
            ...
            while True:
                ...
                await session.set_weighted_prompts(prompts=[types.WeightedPrompt(text=prompt_str, weight=1.0)])

        print("Starting with some piano")
        await session.set_weighted_prompts(prompts=[types.WeightedPrompt(text="Piano", weight=1.0)])
