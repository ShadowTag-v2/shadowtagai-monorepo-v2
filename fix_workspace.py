...
paths = [".vscode/settings.json", "pnkln.code-workspace", "Monorepo-Uphillsnowball.code-workspace"]
...
for p in paths:
    if os.path.exists(p):
        ...
        if "json.schemas" in data:
            data["json.schemas"] = [s for s in data["json.schemas"] if "ty" not in str(s.get("url", ""))]
