...
def run_report(
  property_id, metrics, dimensions, start_date, end_date, limit, filter_expr=None
):
...
def main():
  ...
  parser.add_argument(
    "--property", "-p", help="GA4 Property ID (or set GA4_PROPERTY_ID env var)"
  )
  parser.add_argument(
    "--metrics",
    "-m",
  ...
  parser.add_argument(
    "--start", "-s", help="Start date (YYYY-MM-DD, default: 30 days ago)"
  )
  parser.add_argument("--end", "-e", help="End date (YYYY-MM-DD, default: today)")
  parser.add_argument(
    "--limit", "-l", type=int, default=25, help="Max rows (default: 25)"
  )
  parser.add_argument(
    "--filter", "-f", help="Filter expression (e.g., 'pagePath=~/blog/')"
  )
  ...
  if not property_id:
    print(
      "Error: GA4 Property ID required. Use --property or set GA4_PROPERTY_ID env var."
    )
