...
class PromptTraceStore:
    ...
    def get_traces_by_prompt(self, prompt_id: str, limit: int = 100) -> list[PromptExecutionRecord]:
        with self.Session() as session:
            rows = session.query(PromptTraceRecord).filter_by(prompt_id=prompt_id).limit(limit).all()
            return [
