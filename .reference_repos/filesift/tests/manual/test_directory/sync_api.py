...
class SyncLMStudioWebsocket(LMStudioWebsocket[SyncToAsyncWebsocketBridge]):
  ...
  def remote_call(
    ...
    with ws.start_call() as (call_id, getter):
      rpc = SyncRemoteCall(call_id, getter, self._logger.event_context, notice_prefix)
...
class _SyncSession(ClientSession["Client", SyncLMStudioWebsocket]):
  ...
  def connect(self) -> SyncLMStudioWebsocket:
    ...
    if namespace is None:
      raise LMStudioClientError(f"No API namespace defined for {type(self).__name__}")
...
class DownloadedEmbeddingModel(
  ...
  def __init__(self, model_info: DictObject, session: "_SyncSessionEmbedding") -> None:
...
class _SyncSessionSystem(_SyncSession):
  ...
  def _process_download_listing(self, model_info: DictObject) -> AnyDownloadedModel:
    ...
    if model_type is None:
      raise LMStudioClientError(f"No 'type' field in download listing: {model_info}")
    match model_type:
      case "embedding":
...
class LLM(SyncModelHandle[_SyncSessionLlm]):
  ...
  def act(
    ...
    *,
    ...
    config: LlmPredictionConfig | LlmPredictionConfigDict | None = None,
    preset: str | None = None,
    on_message: Callable[[AssistantResponse | ToolResultMessage], Any] | None = None,
    on_first_token: Callable[[int], Any] | None = None,
    on_prediction_fragment: Callable[[LlmPredictionFragment, int], Any] | None = None,
    ...
    with ThreadPoolExecutor(max_parallel_tool_calls) as pool:
      for round_index in round_counter:
        self._logger.debug("Starting .act() prediction round", round_index=round_index)
        ...
        self._logger.debug("Completed .act() prediction round", round_index=round_index)
        if on_prediction_completed:
          round_result = PredictionRoundResult.from_result(prediction, round_index)
        ...
        if pending_tool_calls:
          ...
          def _finish_tool_call(fut: SyncFuture[Any]) -> Any:
            ...
            if exc is not None:
              ...
              return endpoint._handle_failed_tool_request(exc, failed_request)
            return fut.result()

          tool_results = [
            _finish_tool_call(fut) for fut in as_completed(pending_tool_calls)
          ]
...
class Client(ClientBase):
  ...
  def __init__(self, api_host: str | None = None, api_token: str | None = None) -> None:
  ...
  @sdk_public_api()
  def list_loaded_models(self, namespace: str | None = None) -> Sequence[AnySyncModel]:
