...
def _normalize_scope(scope: str | None, *, default: str = "project") -> str:
  ...
  if normalized not in {"project", "workspace", "global"}:
    raise ValueError(
      f"Invalid scope {scope!r}; expected one of project, workspace, global."
    )
  return normalized
...
def _scope_filter_for_query(query: BenchmarkQuery) -> Callable[[MemoryChunk], bool]:
  def _matches(chunk: MemoryChunk) -> bool:
    chunk_scope = _normalize_scope(
      str(chunk.metadata.get("scope", "global")), default="global"
    )
...
def _coerce_document(item: dict[str, Any], index: int) -> BenchmarkDocument:
  ...
  for query_item in raw_queries:
    if isinstance(query_item, dict):
      ...
      parsed_scope_id = (
        str(parsed_scope_id_raw) if parsed_scope_id_raw is not None else None
      )
    else:
  ...
  scope_id = _normalize_scope_id(
    scope,
    str(item.get("scope_id")) if item.get("scope_id") is not None else parsed_scope_id,
  )
...
def _load_queries(
  dataset_path: Path, documents: list[BenchmarkDocument]
) -> list[BenchmarkQuery]:
    ...
    for line in text.splitlines():
      ...
      for raw_query in raw_queries:
        if isinstance(raw_query, dict):
          ...
          relevant_ids = {
            str(value) for value in relevant_ids_raw if str(value).strip()
          }
          if not relevant_ids:
            relevant_ids = {doc.id}
          current_scope = _normalize_scope(
            str(raw_query.get("current_scope", doc.scope))
          )
          ...
          if allowed_scopes_raw is not None and not isinstance(
            allowed_scopes_raw, list
          ):
            raise ValueError(
...
def evaluate_production_replacement_gate(
  ...
  *,
  ...
  for comp in comparisons:
    ...
    latency_ratio = (
      engine_latency / latency_denominator if latency_denominator > 0 else 0.0
    )
    ...
    passed = (
      mrr_lift_ratio >= min_mrr_lift and latency_ratio <= applied_max_latency_ratio
    )
    details.append(
      {
...
def main() -> None:
  ...
  parser.add_argument(
    "--top-k", type=int, default=5, help="k for Recall@k/MRR (default: 5)."
  )
  parser.add_argument(
  ...
  store_types = [
    item.strip().lower() for item in args.stores.split(",") if item.strip()
  ]
  if not store_types:
    raise ValueError("No store types provided.")
  retrievers = [
    item.strip().lower() for item in args.retrievers.split(",") if item.strip()
  ]
  if not retrievers:
