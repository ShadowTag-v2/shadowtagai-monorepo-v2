...
class OpenAIEmbeddingProvider(EmbeddingProvider):
  ...
  def _embed_with_key(
    self, text: str, *, api_key: str, provider_name: str
  ) -> list[float]:
    payload = call_with_retry(
  ...
  def embed(self, text: str) -> list[float]:
    ...
    try:
      return self._embed_with_key(
        text, api_key=self.api_key, provider_name=provider_name
      )
...
async def embed_batch_async(
  embedder: EmbeddingProvider, texts: list[str]
) -> list[list[float]]:
