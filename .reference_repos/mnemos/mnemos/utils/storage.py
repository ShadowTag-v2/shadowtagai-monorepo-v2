  ...
  def touch(
    ...
    *,
    ...
    if chunk is None:
      return False
    chunk.access_count = (
      chunk.access_count + 1 if access_count is None else access_count
    )
...
class SQLiteStore(MemoryStore):
  ...
  def __init__(self, db_path: str = "mnemos_memory.db", name: str = "sqlite") -> None:
    ...
    )  # Lower fsync cost for high-read local use
  ...
  def _meta_value(self, key: str) -> str | None:
    row = self._conn.execute(
      "SELECT value FROM mnemos_meta WHERE key = ?", (key,)
    ).fetchone()
  ...
  def update(self, chunk_id: str, chunk: MemoryChunk) -> bool:
    ...
    with self._lock:
      cursor = self._conn.execute(
        "SELECT id FROM memory_chunks WHERE id = ?", (chunk_id,)
      )
  ...
  def get_graph_edges(
    ...
    if chunk_ids:
      ...
      query += f" AND source_id IN ({placeholders}) AND target_id IN ({placeholders})"
  ...
  def get_stats(self) -> dict[str, Any]:
    ...
    with self._lock:
      ...
      avg_salience = (
        self._conn.execute("SELECT AVG(salience) FROM memory_chunks").fetchone()[0]
        or 0.0
      )
      avg_access = (
