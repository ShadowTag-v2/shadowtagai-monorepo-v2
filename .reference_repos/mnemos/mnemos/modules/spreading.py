...
class SpreadingActivation:
  ...
  def remove_edge(
    ...
    *,
    ...
    if bidirectional:
      removed = (
        self._nodes[node_b_id].neighbors.pop(node_a_id, None) is not None or removed
      )
    return removed
  ...
  def connect_node(self, node_id: str, threshold: float | None = None) -> int:
    ...
    edges_added = 0
    for similarity, other_id in sorted(
      candidates, key=lambda item: item[0], reverse=True
    ):
      node = self._nodes.get(node_id)
  ...
  def activate(self, node_id: str, energy: float | None = None) -> dict[str, float]:
    ...
    while queue:
      ...
      for neighbor_id, edge_weight in current_node.neighbors.items():
        ...
        transferred_energy = (
          current_energy * (1.0 - self._config.decay_rate) * edge_weight
        )
