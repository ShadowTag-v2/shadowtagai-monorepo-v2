...
class SleepDaemon:
  ...
  def __init__(
    ...
    self._episodic_buffer: dict[tuple[str, str | None], list[Interaction]] = (
      defaultdict(list)
    )
  ...
  def should_consolidate(self) -> bool:
    ...
    time_threshold = (
      time.time() - self._last_consolidation
      >= self._config.consolidation_interval_seconds
    )
  ...
  async def _measure_recall_support(
    ...
    """
    episode_texts = [
      episode.content.strip() for episode in episodes if episode.content.strip()
    ]
  ...
  async def consolidate(
    ...
    for partition_key, buffered_episodes in list(self._episodic_buffer.items()):
      ...
      consolidation_prompt = self._config.consolidation_prompt.format(
