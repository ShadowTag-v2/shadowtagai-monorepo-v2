  ...
  *,
  ...
  if llm_provider == "mock":
    ...
    if ollama_url:
      ...
      add_check(
        "llm.ollama", "fail", "MNEMOS_OLLAMA_URL is required for ollama provider."
      )
    ...
    add_check(
      "llm.provider", "fail", f"Unsupported MNEMOS_LLM_PROVIDER: {llm_provider!r}"
    )
  ...
  if embedding_provider == "simple":
    ...
    if ollama_url:
      add_check(
        "embedding.ollama",
        "pass",
        f"Ollama embedding provider configured at {ollama_url}.",
      )
    else:
      add_check(
        "embedding.ollama",
    ...
    if api_key:
      add_check(
        "embedding.openai", "pass", "OpenAI embedding provider API key configured."
      )
    else:
      add_check(
        "embedding.openai",
  ...
  latency_threshold_exceeded = (
    observed_latency_p95_ms is not None
    and observed_latency_p95_ms >= latency_p95_threshold_ms
  )
