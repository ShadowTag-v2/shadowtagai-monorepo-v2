...
def _normalize_scope(scope: str | None, *, default: str = _DEFAULT_SCOPE) -> str:
  ...
  if normalized not in _VALID_SCOPES:
    raise ValueError(
      f"Invalid scope {scope!r}; expected one of {', '.join(_VALID_SCOPES)}."
    )
  return normalized
...
class MnemosEngine:
  ...
  def _hydrate_spreading_graph_from_store(self) -> None:
    ...
    persisted_edges = self._store.get_graph_edges(
      [chunk.id for chunk in chunks_to_load]
    )
    if persisted_edges:
  ...
  async def process(
    ...
    *,
    ...
    if safety.content != result.chunk.content:
      ...
      result.chunk.metadata["safety_redactions"] = [
        match.label for match in safety.matches
      ]
