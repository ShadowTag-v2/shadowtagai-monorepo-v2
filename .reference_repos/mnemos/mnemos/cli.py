...
def _build_profile_env(
  *,
  ...
  env: dict[str, str] = {
    ...
    "MNEMOS_EMBEDDING_PROVIDER": embedding_provider
    or _infer_embedding_provider(llm_provider),
  }
  if model:
...
def _migrate_chunks(
  *, source_store: Any, target_store: Any, dry_run: bool
) -> dict[str, Any]:
...
async def _cmd_doctor(args: argparse.Namespace) -> None:
  ...
  doctor_env["MNEMOS_DOCTOR_LATENCY_P95_THRESHOLD_MS"] = str(
    args.latency_p95_threshold_ms
  )
...
def main() -> None:
  ...
  sp_store = subparsers.add_parser(
    "store", help="Store a memory through the full pipeline"
  )
  ...
  sp_retrieve.add_argument(
    "--top-k", type=int, default=5, help="Max results (default: 5)"
  )
  sp_retrieve.add_argument(
    "--current-scope",
    choices=VALID_SCOPES,
  ...
  sp_feedback.add_argument(
    "event_type", choices=("helpful", "not_helpful", "missed_memory")
  )
  sp_feedback.add_argument(
    "--query", required=True, help="Query or task the feedback refers to."
  )
  ...
  sp_feedback.add_argument(
    "--notes", default="", help="Optional explanation or annotation."
  )
  ...
  sp_feedback_list.add_argument(
    "--event-type", choices=FEEDBACK_EVENT_CHOICES, default=None
  )
  ...
  sp_feedback_export.add_argument(
    "--event-type", choices=FEEDBACK_EVENT_CHOICES, default=None
  )
  ...
  sp_feedback_export.add_argument(
    "--limit", type=int, default=0, help="0 means no limit."
  )
  ...
  sp_inspect.add_argument(
    "--query",
    default="",
    help="Optional query to explain why this memory would be retrieved.",
  )
  ...
  sp_doctor = subparsers.add_parser(
    "doctor", help="Run readiness and dependency checks"
  )
  ...
  sp_migrate.add_argument(
    "--source-store", required=True, choices=MIGRATION_SOURCE_STORE_CHOICES
  )
  sp_migrate.add_argument(
    "--target-store", required=True, choices=MIGRATION_TARGET_CHOICES
  )
