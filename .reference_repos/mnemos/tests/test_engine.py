...
class TestEngineProcess:
  ...
  async def test_process_tags_chunk_with_cognitive_state(self, engine):
    ...
    result = await engine.process(
      make_interaction("I'm really stressed about this bug!")
    )
    assert result.stored is True
    assert result.chunk is not None
  ...
  async def test_process_adds_to_spreading_activation_graph(self, engine):
    ...
    result = await engine.process(
      make_interaction("Docker containerization and Kubernetes")
    )
    if result.stored:
  ...
  async def test_curator_process_skips_repetitive_status_noise(self, engine):
    result = await engine.process(
      make_interaction(
        "still debugging still debugging still debugging still debugging"
      )
    )
    assert result.stored is False
...
class TestEngineRetrieve:
  ...
  async def test_retrieve_reuses_single_store_candidate_pool(self):
    ...
    results = await engine.retrieve(
      "python deploy tooling", top_k=2, reconsolidate=False
    )
...
class TestEngineScopedMemory:
  ...
  async def test_consolidate_preserves_scope_and_isolates_projects(self) -> None:
    ...
    assert consolidated
    assert {
      (chunk.metadata.get("scope"), chunk.metadata.get("scope_id"))
    ...
    assert not any(
      chunk.metadata.get("scope_id") == "repo-beta" for chunk in alpha_results
    )
    ...
    assert not any(
      chunk.metadata.get("scope_id") == "repo-alpha" for chunk in beta_results
    )
...
class TestEngineGovernance:
  ...
  async def test_max_chunks_per_scope_cap_prunes_oldest(self) -> None:
    ...
    await engine.process(
      make_interaction("fact one"), scope="project", scope_id="alpha"
    )
    await engine.process(
      make_interaction("fact two"), scope="project", scope_id="alpha"
    )
    await engine.process(
      make_interaction("fact three"), scope="project", scope_id="alpha"
    )
  ...
  async def test_retention_ttl_prunes_old_chunks(self) -> None:
    ...
    await engine.process(
      make_interaction("fresh memory"), scope="project", scope_id="alpha"
    )
...
class TestEngineFullPipeline:
  ...
  async def test_full_encode_retrieve_cycle(self, engine):
    ...
    session = [
      ...
      (
        "user",
        "My team of 8 engineers uses GitHub and follows trunk-based development",
      ),
    ]
