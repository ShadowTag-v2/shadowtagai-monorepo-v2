...
class TestSurprisalGate:
  ...
  async def test_low_surprisal_not_stored(self, mock_llm, embedder, store):
    ...
    identical_llm = MockLLMProvider(
      responses={"predict": "routine greeting, nothing new"}
    )
    config = SurprisalConfig(threshold=0.3)
    gate = SurprisalGate(
      llm=identical_llm, embedder=embedder, store=store, config=config
    )
...
class TestMutableRAG:
  ...
  async def test_reconsolidate_changed(self, embedder, store):
    ...
    updated_chunk, changed = await rag.reconsolidate(
      chunk, "User mentioned migrating to Rust"
    )
...
class TestAffectiveRouter:
  ...
  @pytest.mark.asyncio
  async def test_retrieve_uses_supplied_candidates_without_store_lookup(
    self, mock_llm, embedder
  ):
