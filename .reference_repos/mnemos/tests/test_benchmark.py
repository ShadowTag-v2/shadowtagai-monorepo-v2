...
def test_evaluate_production_replacement_gate_uses_store_specific_latency_defaults() -> (
  None
):
  comparisons = [
    {
...
def test_sqlite_path_for_run_adds_unique_suffixes(tmp_path: Path) -> None:
  ...
  first = _sqlite_path_for_run(
    base, repetition=1, store_type="sqlite", retriever="baseline"
  )
  second = _sqlite_path_for_run(
    base, repetition=2, store_type="sqlite", retriever="engine"
  )
