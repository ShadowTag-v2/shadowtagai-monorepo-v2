...
from scripts.core.config import ensure_gemini_cli, SwarmConfig, ensure_dirs, STATE_DIR
from scripts.core.types import assign_color
...
def _save_config_and_team(yaml_content, plan_content, mission):
  ...
  print("[Planner] Created 'task_plan.md', 'findings.md', 'progress.md'.")
...
def main():
  ...
  try:
    ...
    if yaml_match:
      ...
      if (
        "gemini-2.0" in yaml_content
        or "gemini-1.5" in yaml_content
        or "gemini-3-flash" in yaml_content
      ):
        print(
          "[Planner] [WARN] Validating model availability. Switching to 'auto-gemini-3' (system default)..."
        )
