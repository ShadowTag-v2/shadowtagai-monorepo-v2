...
class TmuxBackend(SpawnBackend):
  ...
  def _ensure_session(self):
    ...
    self._run_tmux(
      "new-session", "-d", "-s", self.SESSION_NAME, "-x", "220", "-y", "50"
    )

    # Enable pane-border-status
    self._run_tmux("set-option", "-t", self.SESSION_NAME, "pane-border-status", "top")
    self._run_tmux(
      "set-option", "-t", self.SESSION_NAME, "pane-border-format", " #{pane_title} "
    )
  ...
  def spawn(self, agent_name: str, command: List[str], color: str = "white") -> str:
    ...
    if self._panes:
      ...
      pane_id = self._run_tmux(
        "list-panes", "-t", self.SESSION_NAME, "-F", "#{pane_id}", capture=True
      )
    ...
    if pane_id:
      ...
      self._run_tmux("select-pane", "-t", pane_id, "-P", f"fg={tmux_color}")
