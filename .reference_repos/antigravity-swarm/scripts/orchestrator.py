...
from scripts.core.config import SwarmConfig, ensure_dirs, STATE_DIR
...
def render_dashboard(runners, spinner_idx, selected_idx, backend, show_help):
  ...
  for idx, runner in enumerate(runners):
    ...
    if idx == selected_idx:
      agent_name = (
        f"[{name_style}]{agent_name}[/{name_style}]" if name_style else agent_name
      )
  ...
  footer_text = Text.assemble(
    (" [Tab] ", "bold white on dark_blue"),
    ("View  ", "dim"),
    (" [w,s] ", "bold white on dark_blue"),
    ("Select  ", "dim"),
    (" [Enter] ", "bold white on dark_blue"),
    ("Detail  ", "dim"),
    (" [k] ", "bold white on dark_blue"),
    ("Kill  ", "dim"),
    (" [x] ", "bold white on dark_blue"),
    ("Shutdown  ", "dim"),
    (" [q] ", "bold white on dark_blue"),
    ("Quit  ", "dim"),
...
def render_detail_view(runner, all_msgs=None, log_tail=None):
  ...
  content.append(
    f"  Messages: {runner.msg_count['sent']} sent, "
  ...
  agent_msgs = [
    m for m in all_msgs if m.sender == runner.name or m.recipient == runner.name
  ]
  if not agent_msgs:
...
def main():
  ...
  mission = None
  try:
    resume_stale_seconds = float(
      os.environ.get("AG_SWARM_RESUME_STALE_SECONDS", "1800")
    )
  except ValueError:
  ...
  if resume_mode:
    mission = (
      MissionState.load(mission_id_override)
      if mission_id_override
      else MissionState.latest()
    )
  ...
  if not resume_mode or mission is None:
    mission = MissionState.create(config_data.get("mission", "Swarm Mission"))
  ...
  def spawn_runner(r):
    ...
    r.backend_info = f"{backend.get_type()} {handle}" if handle else backend.get_type()
    r.is_running = True
  ...
  try:
    with Live(console=console, refresh_per_second=config.tui_refresh_rate) as live:
          ...
          if entry and not entry["alive"]:
            ...
            if rc == 0:
              ...
              if r.saw_completion_signal:
                ...
                r.retryable = r.stop_mode not in (
                  "force_kill",
                  "graceful_shutdown",
      ...
      while not quit_requested:
        ...
        if demo_mode:
          all_done = not any(r.is_running for r in parallel_runners)
        else:
    ...
    failed_runners = [r for r in runners if r.status != AgentStatus.COMPLETED.value]
