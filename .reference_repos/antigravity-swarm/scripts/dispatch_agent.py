...
class AgentLifecycle:
  ...
  def run(
    ...
    try:
      idle_timeout_seconds = float(
        os.environ.get("AG_SWARM_AGENT_IDLE_TIMEOUT_SECONDS", "120")
      )
    except ValueError:
    ...
    while self.status == AgentStatus.IDLE:
      ...
      if (
        ...
        and (time.time() - idle_started_at) >= idle_timeout_seconds
      ):
        self.status = AgentStatus.COMPLETED
        self.audit.record(
          self.identity.name,
  ...
  def _execute_task(
    self, task: str, gemini_path: str, model: str, log_file: Optional[str] = None
  ):
    ...
    try:
      task_timeout_seconds = float(
        os.environ.get("AG_SWARM_TASK_TIMEOUT_SECONDS", "240")
      )
    except ValueError:
    ...
    try:
      while True:
        if (
          ...
          and (time.time() - task_started_at) >= task_timeout_seconds
        ):
          process.terminate()
          try:
          ...
          self.audit.record(
            ...
            {
              "timeout_seconds": round(task_timeout_seconds, 2),
...
def main():
  ...
  def _signal_handler(signum, _frame):
    ...
    try:
      mailbox.send(
        "leader", MessageType.STATUS_UPDATE, f"Interrupted by signal {signum}."
      )
    except Exception:
      pass
    raise SystemExit(1)
