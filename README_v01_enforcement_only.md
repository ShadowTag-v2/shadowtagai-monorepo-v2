...
Traditional agent frameworks use "start simple, add enforcement later" approach, creating liability in regulated verticals. Pnkln provides **enforcement-first architecture** where every agent action is validated before execution and verified after completion.
...
All Pnkln agents inherit from `PnklnAgent` base class:

```python
from pnkln_agents import PnklnAgent, AgentTask

class MyCustomAgent(PnklnAgent):
...
```python
from pnkln_agents import DEFAULT_CONSTRAINTS
...
```python
from pnkln_agents import DEFAULT_REVENUE_MODEL, PricingTier
...
```bash
# Build Docker image
docker build -t pnkln-agents .

# Run container
docker run -p 8000:8000 pnkln-agents
...
**Architecture:** Pnkln Engineering Team
