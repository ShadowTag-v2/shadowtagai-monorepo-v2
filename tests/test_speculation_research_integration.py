...
class TestOrchestratorLifecycle:
    ...
    def test_reset_clears_state(self, workspace: str) -> None:
        ...
        orch._speculation_results.append(SpeculativePhaseResult(phase="researching", speculation_active=True))
        orch.reset()
...
class TestPhaseHandlerWiring:
    ...
    async def test_speculative_handler_returns_inner_result(self, workspace: str) -> None:
        ...
        handlers = orch.create_phase_handlers(inner_handlers={ResearchPhase.RESEARCHING: inner_research})
...
class TestTrustLevelGate:
    ...
    def test_trust_level_0_denies_writes(self, workspace: str) -> None:
        ...
        engine.start()
        allowed, boundary = engine.can_use_tool("write_to_file", file_path=f"{workspace}/test.py")
        assert not allowed
    ...
    def test_trust_level_2_denies_outside_cwd(self, workspace: str, beads_dir: str) -> None:
        engine = SpeculationEngine.create(cwd=workspace, trust_level=2)
        engine.start()
...
class TestTelemetryCorrelation:
    ...
    def test_phase_change_captures_session_id(self, workspace: str, beads_dir: str) -> None:
        orch = SpeculativeResearchOrchestrator(workspace=workspace)
    ...
    def test_telemetry_events_written(self, workspace: str, beads_dir: str) -> None:
        engine = SpeculationEngine.create(cwd=workspace, trust_level=0, session_id="tel-001")
        engine.start()
