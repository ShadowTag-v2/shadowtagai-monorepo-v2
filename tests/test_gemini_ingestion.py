...
class TestGeminiIngestionLayer:
  ...
  async def test_runtime_under_45_minutes(self):
    ...
    assert result.runtime_minutes <= 45, (
      f"Runtime {result.runtime_minutes:.1f} min exceeds 45 min target"
    )
  ...
  async def test_tier_classification(self):
    ...
    result = await self.ingestion.run_nightly_job(
      job_id="test_tiers", max_items_per_source=100
    )
  ...
  async def test_tier_1_ratio_target(self):
    ...
    assert result.tier_1_ratio >= 0.35, (
      f"Tier 1 ratio {result.tier_1_ratio:.1%} below target (≥40%)"
    )
  ...
  async def test_quality_gates_evaluation(self):
    ...
    result = await self.ingestion.run_nightly_job(
      job_id="test_gates", max_items_per_source=200
    )
  ...
  async def test_cost_tracking(self):
    ...
    result = await self.ingestion.run_nightly_job(
      job_id="test_cost", max_items_per_source=100
    )
...
class TestQualityGates:
  ...
  async def test_cost_per_item_gate(self):
    ...
    result = await ingestion.run_nightly_job(
      job_id="test_cost_gate", max_items_per_source=100
    )
