...
class TestContextCollapser:
    ...
    def test_collapse_above_threshold(self):
        ...
        c = ContextCollapser(min_consecutive=3)
        results = [
            ToolResult(tool_name="grep_search", arguments={"Query": "foo"}, output="file1:1:match"),
            ToolResult(tool_name="grep_search", arguments={"Query": "bar"}, output="file2:1:match"),
            ToolResult(tool_name="grep_search", arguments={"Query": "baz"}, output="file3:1:match"),
        ]
        result = c.collapse(results)
