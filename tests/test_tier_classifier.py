...
async def test_tier_1_domain_classification():
    ...
    content = {"source": "news", "domain": "reuters.com", "title": "Breaking News", "text": "Important development...", "author": "Staff Reporter"}
...
async def test_tier_distribution_targets():
    ...
    for i in range(30):
        await classifier.classify(
            {"source": "news", "domain": tier_1_domains[i % len(tier_1_domains)], "title": f"Article {i}", "text": "Content", "author": "Author"}
        )

    # 50 Tier 2
    for i in range(50):
        await classifier.classify(
            {"source": "news", "domain": tier_2_domains[i % len(tier_2_domains)], "title": f"Article {i}", "text": "Content", "author": "Author"}
        )

    # 20 Tier 3
    for i in range(20):
        await classifier.classify(
            {"source": "social", "domain": other_domains[i % len(other_domains)], "title": f"Post {i}", "text": "Content", "author": "User"}
        )
...
async def test_confidence_threshold_60_percent():
    ...
    content = {"source": "news", "domain": "reuters.com", "title": "News Article", "text": "Content", "author": "Reporter"}
