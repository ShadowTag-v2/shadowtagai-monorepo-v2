...
pos_float = st.floats(min_value=0.0, max_value=1e15, allow_nan=False, allow_infinity=False)
pos_int = st.integers(min_value=0, max_value=86400_000)
task_id_st = st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=("L", "N")))
...
class TestRecurringTaskAged:
    ...
    def test_non_recurring_never_ages(self, now_ms, max_age, created):
        ...
        task = CronTask(id="t", cron="* * * * *", prompt="p", created_at=created, recurring=False)
    ...
    def test_permanent_never_ages(self, now_ms, max_age, created):
        ...
        task = CronTask(id="t", cron="* * * * *", prompt="p", created_at=created, recurring=True, permanent=True)
        assert not is_recurring_task_aged(task, now_ms, max_age)
    ...
    def test_zero_max_age_never_ages(self, now_ms, created):
        ...
        task = CronTask(id="t", cron="* * * * *", prompt="p", created_at=created, recurring=True)
...
class TestSchedulerAddRemove:
    ...
    def test_add_grows_tasks(self, n):
        ...
        for i in range(n):
            sched.add_task(CronTask(id=f"t{i}", cron="* * * * *", prompt=f"p{i}", created_at=0.0))
...
class TestSchedulerCheck:
    def test_one_shot_fires_and_removes(self):
        ...
        task = CronTask(id="oneshot", cron="* * * * *", prompt="do it", created_at=now_ms - 120_000, recurring=False)
        sched.add_task(task)
    ...
    def test_recurring_reschedules(self):
        ...
        sched = CronScheduler(opts)
        now_ms = time.time() * 1000
        task = CronTask(id="rec", cron="* * * * *", prompt="repeat", created_at=now_ms - 120_000, recurring=True)
        sched.add_task(task)
...
class TestBuildMissedNotification:
    ...
    def test_plural_tasks(self, n):
        tasks = [CronTask(id=f"m{i}", cron="* * * * *", prompt=f"p{i}", created_at=0.0) for i in range(n)]
