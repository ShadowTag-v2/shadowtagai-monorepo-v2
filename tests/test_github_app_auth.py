...
_auth_path = os.path.abspath(
  os.path.join(
    os.path.dirname(__file__), "..", "scripts", "utils", "github_app_auth.py"
  )
)
...
class TestGitHubAppAuth(unittest.TestCase):
  ...
  @patch(
    "builtins.open", new_callable=unittest.mock.mock_open, read_data=b"test_pem_data"
  )
  def test_get_github_app_token_success(
    self, mock_open, mock_get_session, mock_jwt_encode
  ):
  ...
  def test_get_github_app_token_installations_request_fails(
    self, mock_open, mock_get_session, mock_jwt_encode
  ):
    mock_session = MagicMock()
    mock_get_session.return_value = mock_session
