...
def _make_conversation(n: int = 8) -> list[Message]:
  ...
  msgs = [
    _make_message(
      role="system", content="You are a helpful assistant.", token_count=50
    ),
  ]
  for i in range(1, n):
    if i % 2 == 1:
      msgs.append(
        _make_message(role="user", content=f"User message {i}", token_count=20)
      )
    else:
      msgs.append(
        _make_message(
          role="assistant", content=f"Assistant response {i}", token_count=30
        )
      )
  return msgs
...
class TestPostVerify:
  ...
  def test_system_prompt_change_detected(self) -> None:
    ...
    msgs[0] = _make_message(
      role="system", content="MODIFIED system prompt", token_count=50
    )
  ...
  def test_tool_result_modification_detected(self) -> None:
    ...
    msgs[2] = _make_message(
      role="tool", content="tool output v2 MODIFIED", token_count=15
    )
...
class TestCacheAnchor:
  ...
  def test_equality(self) -> None:
    ...
    a = CacheAnchor(
      index=0, content_hash="abc", role="system", is_system=True, token_count=50
    )
    b = CacheAnchor(
      index=0, content_hash="abc", role="system", is_system=True, token_count=50
    )
