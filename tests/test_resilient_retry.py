...
class TestWithRetry:
  ...
  async def test_529_circuit_breaker_no_fallback(self):
    ...
    with pytest.raises(CannotRetryError):
      async for _outcome, _value in with_retry(
        op, cfg, on_retry=lambda e: events.append(e)
      ):
        pass
...
class TestRateLimitResetDelay:
  ...
  def test_invalid_value(self):
    err = FakeAPIError(
      429, headers={"anthropic-ratelimit-unified-reset": "not-a-number"}
    )
    assert _get_rate_limit_reset_delay_ms(err) is None

  def test_infinity_returns_none(self):
    err = FakeAPIError(
      429, headers={"anthropic-ratelimit-unified-reset": str(math.inf)}
    )
    assert _get_rate_limit_reset_delay_ms(err) is None
