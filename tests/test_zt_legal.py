...
Gate 1: Mock Filing Ingestion          — aiyou_ingest MCP simulation
...
class TestGate1MockIngestion(unittest.TestCase):
  ...
  @patch("control.pnkln.pnkln_core.agents.legal.dispatch_compute")
  def test_ingestion_passes_filing_text_in_prompt(
...
class TestGate3HallucinationTrap(unittest.TestCase):
  ...
  @patch("control.pnkln.pnkln_core.agents.legal.dispatch_compute")
  def test_no_deadline_document_returns_empty_list(
    self, mock_dispatch: MagicMock
  ) -> None:
    mock_dispatch.return_value = [{"text": _MOCK_AI_RESPONSE_EMPTY}]
    results = extract_deadlines_from_filing(
  ...
  @patch("control.pnkln.pnkln_core.agents.legal.dispatch_compute")
  def test_empty_model_output_returns_empty_list(
    self, mock_dispatch: MagicMock
  ) -> None:
