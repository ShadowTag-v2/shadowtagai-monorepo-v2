    ...
    def test_secrets_redacted_in_cassette(self, method, secret_key, secret_value, vcr_dir, monkeypatch):
        ...
        for fname in os.listdir(vcr_dir):
            ...
            if secret_key in req_kwargs:
                assert req_kwargs[secret_key] == "[REDACTED]", f"Secret key {secret_key!r} was not redacted: got {req_kwargs[secret_key]!r}"
...
class TestCassetteStats:
    ...
    def test_stats_arithmetic(self, n_fresh, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            assert stats["total_cassettes"] == stats["fresh_cassettes"] + stats["stale_cassettes"]
