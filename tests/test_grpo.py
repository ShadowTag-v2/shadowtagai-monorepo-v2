...
class TestGRPOConfig:
  ...
  def test_default_config(self):
    ...
    assert config.normalize_advantages == True
...
class TestGRPOTrainer:
  ...
  def test_compute_advantages_not_normalized(self):
    ...
    config = GRPOConfig(
      num_groups=8, responses_per_prompt=4, normalize_advantages=False
    )
    trainer = GRPOTrainer(config)
...
class TestGRPOScenarios:
  ...
  def test_all_same_rewards(self):
    ...
    config = GRPOConfig(
      num_groups=1, responses_per_prompt=4, normalize_advantages=False
    )
    trainer = GRPOTrainer(config)
  ...
  def test_multiple_groups_different_difficulties(self):
    ...
    config = GRPOConfig(
      num_groups=3, responses_per_prompt=2, normalize_advantages=False
    )
    trainer = GRPOTrainer(config)
