...
class TestTelemetryEmitter:
  ...
  def test_event_count_filters_by_type(self):
    emitter = TelemetryEmitter(buffer_events=True)
    emitter.emit(
      TelemetryPayload(event=GatewayEvent.BLOCK_ALLOW_BLOCKED.value, tool_id="a")
    )
    emitter.emit(
      TelemetryPayload(event=GatewayEvent.BLOCK_ALLOW_ALLOWED.value, tool_id="b")
    )
    emitter.emit(
      TelemetryPayload(event=GatewayEvent.BLOCK_ALLOW_BLOCKED.value, tool_id="c")
    )
...
class TestClassifiedGatewaySandbox:
  ...
  def test_sandbox_blocks_outside_workspace(self, workspace, sandbox):
    ...
    assert (
      "NEVER_ACCESS" in result.deny_reason
      or "outside sandbox boundary" in result.deny_reason
    )
