...
class TestComputerUseTool:
  ...
  def test_custom(self):
    tool = computer_use_tool(
      environment="desktop", display_width_px=1920, display_height_px=1080
    )
...
class TestToolDefinition:
  ...
  def test_to_dict_with_config(self):
    d = ToolDefinition(
      type="mcp_server", config={"name": "s1", "url": "https://x.com"}
    ).to_dict()
...
class TestStreamAccumulator:
  def test_text_reconstruction(self):
    ...
    acc.feed(
      StreamEvent(event_type=EventType.CONTENT_START, index=0, content_type="text")
    )
    acc.feed(
      StreamEvent(
        event_type=EventType.CONTENT_DELTA, index=0, delta_type="text", text="Hello "
      )
    )
    acc.feed(
      StreamEvent(
        event_type=EventType.CONTENT_DELTA, index=0, delta_type="text", text="world"
      )
    )
  ...
  def test_mixed_thought_and_text(self):
    ...
    acc = StreamAccumulator()
    acc.feed(
      StreamEvent(event_type=EventType.CONTENT_START, index=0, content_type="thought")
    )
    acc.feed(
      StreamEvent(
    ...
    acc.feed(
      StreamEvent(event_type=EventType.CONTENT_START, index=1, content_type="text")
    )
    acc.feed(
      StreamEvent(
        event_type=EventType.CONTENT_DELTA, index=1, delta_type="text", text="Answer"
      )
    )
    outputs = acc.outputs
  ...
  def test_function_call_reconstruction(self):
    ...
    acc.feed(
      StreamEvent(
        event_type=EventType.CONTENT_START, index=0, content_type="function_call"
      )
    )
    acc.feed(
      StreamEvent(
  ...
  def test_interaction_id_and_usage(self):
    acc = StreamAccumulator()
    acc.feed(
      StreamEvent(event_type=EventType.INTERACTION_START, interaction_id="int-abc")
    )
    acc.feed(
      StreamEvent(
...
class TestNewSchemaStreamAccumulator:
  ...
  def test_step_based_text_reconstruction(self):
    ...
    acc = StreamAccumulator()
    acc.feed(
      StreamEvent(event_type=EventType.STEP_START, index=0, content_type="model_output")
    )
    acc.feed(
      StreamEvent(
        event_type=EventType.STEP_DELTA, index=0, delta_type="text", text="Hello "
      )
    )
    acc.feed(
      StreamEvent(
        event_type=EventType.STEP_DELTA, index=0, delta_type="text", text="step world"
      )
    )
  ...
  def test_step_based_interaction_lifecycle(self):
    ...
    acc = StreamAccumulator()
    acc.feed(
      StreamEvent(event_type=EventType.INTERACTION_CREATED, interaction_id="new-int-1")
    )
    acc.feed(
      StreamEvent(event_type=EventType.STEP_START, index=0, content_type="model_output")
    )
    acc.feed(
      StreamEvent(
        event_type=EventType.STEP_DELTA, index=0, delta_type="text", text="Response"
      )
    )
    acc.feed(StreamEvent(event_type=EventType.STEP_STOP, index=0))
    acc.feed(
  ...
  def test_step_based_function_call(self):
    ...
    acc.feed(
      StreamEvent(
        event_type=EventType.STEP_START, index=0, content_type="function_call"
      )
    )
    acc.feed(
      StreamEvent(
...
class TestPhase2StepSchema:
  ...
  def test_step_text_accumulation(self):
    ...
    acc.feed(
      StreamEvent(event_type="step.delta", index=0, delta_type="text", text="Hello")
    )
    acc.feed(
      StreamEvent(event_type="step.delta", index=0, delta_type="text", text=", ")
    )
    acc.feed(
      StreamEvent(event_type="step.delta", index=0, delta_type="text", text="world!")
    )
  ...
  def test_step_function_call_delta(self):
    ...
    acc = StreamAccumulator()
    acc.feed(
      StreamEvent(event_type="step.start", index=0, content_type="function_call")
    )
    acc.feed(
      StreamEvent(
  ...
  def test_multi_block_step_indexing(self):
    ...
    acc.feed(
      StreamEvent(event_type="step.delta", index=1, delta_type="text", text="Answer")
    )
  ...
  def test_mixed_legacy_new_events(self):
    ...
    acc.feed(StreamEvent(event_type="step.start", index=0, content_type="model_output"))
    acc.feed(
      StreamEvent(event_type="step.delta", index=0, delta_type="text", text="Mixed")
    )
    acc.feed(StreamEvent(event_type="step.stop", index=0))
    # Legacy completion
    acc.feed(
...
class TestConversationSessionFunctionCallLoop:
  ...
  def test_function_call_loop_missing_handler(self):
    ...
    assert any(
      "No handler" in str(item.get("result", "")) for item in second_call_input
    )
