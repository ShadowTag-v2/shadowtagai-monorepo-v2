...
from app.models import User, Project, Memory
...
async def test_create_memory(db_session, test_user, test_project):
    ...
    memory_data = MemoryCreate(
        title="Test Memory",
        content="This is a test memory about FastAPI",
        memory_type="fact",
        project_id=test_project.id
    )

    memory = await memory_service.create_memory(
        db_session, test_user.id, memory_data
    )
...
async def test_get_memories(db_session, test_user, test_project):
    ...
    memories = await memory_service.get_memories(
        db_session, test_user.id, project_id=test_project.id
    )
...
async def test_update_memory(db_session, test_user, test_project):
    ...
    updated_memory = await memory_service.update_memory(
        db_session, memory.id, test_user.id, update_data
    )
...
async def test_delete_memory(db_session, test_user, test_project):
    ...
    success = await memory_service.delete_memory(
        db_session, memory.id, test_user.id
    )
