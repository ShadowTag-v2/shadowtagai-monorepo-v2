...
def basic_context() -> list[dict]:
  ...
  return [
    ...
    {
      "role": "assistant",
      "content": "I've created the ConfigTool at packages/agnt_tools/config_tool.py",
    },
  ]
...
class TestSpeculate:
  ...
  def test_returns_speculation_result(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
  ...
  def test_status_is_ready_on_success(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
  ...
  def test_suggestion_has_context_hash(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
  ...
  def test_generation_time_tracked(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
...
class TestTaskPhases:
  ...
  def test_start_phase_generates_prompts(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
    """'start' phase should generate session-start prompts."""
    result = engine.speculate(
      basic_context, task_phase="start", complexity="architecture"
    )
    assert result.suggestion is not None

  def test_error_phase_generates_prompts(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
  ...
  def test_complete_phase_generates_prompts(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
...
class TestAccept:
  ...
  def test_accept_returns_time_saved(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
  ...
  def test_accept_increments_metric(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
  ...
  def test_accept_sets_accepted_at(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
...
class TestCaching:
  ...
  def test_cache_hit_on_same_context(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
  ...
  def test_cache_miss_on_different_context(
    self, engine: PromptSpeculationEngine
  ) -> None:
...
class TestSuppression:
  ...
  def test_suppressed_in_planning_phase(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
...
class TestSuggestionFilters:
  ...
  def test_filter_rules(self, text: str, should_filter: bool) -> None:
    ...
    assert result is should_filter, (
      f"Expected filter={should_filter} for '{text}', got {result}"
    )
...
class TestReset:
  ...
  def test_reset_clears_abort_flag(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
...
class TestFormatMetrics:
  ...
  def test_format_metrics_after_activity(
    self, engine: PromptSpeculationEngine, basic_context: list
  ) -> None:
