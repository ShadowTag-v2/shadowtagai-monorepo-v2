...
class TestCronSchedulerHypothesis(unittest.TestCase):
    ...
    @given(
        now_ms=st.floats(min_value=0.0, max_value=1e15, allow_nan=False, allow_infinity=False),
        max_age_ms=st.integers(min_value=0, max_value=86400_000),
        created_at=st.floats(min_value=0.0, max_value=1e15, allow_nan=False, allow_infinity=False),
    )
    @settings(max_examples=200)
    def test_recurring_task_aged_deterministic(self, now_ms: float, max_age_ms: int, created_at: float) -> None:
        ...
        task = CronTask(id="test", cron="0 * * * *", prompt="test", created_at=created_at, recurring=True)
    ...
    @settings(max_examples=100)
    def test_jitter_config_construction(self, base_jitter: int, max_jitter: int, max_age: int) -> None:
...
class TestConversationRecoveryHypothesis(unittest.TestCase):
    ...
    @given(
        session_id=st.text(min_size=1, max_size=64, alphabet=st.characters(whitelist_categories=("L", "N"))),
    ...
    @given(
        session_ids=st.lists(
            st.text(min_size=1, max_size=32, alphabet=st.characters(whitelist_categories=("L", "N"))),
            min_size=1,
            max_size=10,
            unique=True,
        ),
