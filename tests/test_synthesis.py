...
class TestSynthesizeFindings:
    ...
    def test_confidence_capped_at_one(self) -> None:
        findings = [Finding(content="x", source=f"s{i}", confidence=0.99) for i in range(10)]
        result = synthesize_findings(findings, objective="Cap test")
        assert result.confidence <= 1.0

    def test_recommendations_from_high_confidence(self) -> None:
        findings = [Finding(content="Use Cloud Run for deployment", source="docs", confidence=0.95)]
    ...
    def test_recommendations_capped_at_ten(self) -> None:
        findings = [Finding(content=f"Must do thing {i} immediately.", source="sec", confidence=0.95) for i in range(20)]
        result = synthesize_findings(findings, objective="Cap")
        assert len(result.recommendations) <= 10

    def test_dedup_by_prefix(self) -> None:
        findings = [
            Finding(content="You should implement caching for performance. Details follow.", source="a"),
            Finding(content="You should implement caching for performance. More info.", source="b"),
        ]
