...
def violations_mixed():
  ...
  return ViolationsScanOutput(
    violations=[
      ...
      Violation(
        rule_id="ATP-5-19-3.2", description="Incomplete record", severity="moderate"
      ),
      Violation(
        rule_id="ATP-5-19-4.1", description="Outside timeframe", severity="major"
      ),
    ],
    scan_metadata={"model": "test"},
  )
...
@pytest.mark.skipif(
  JudgeSixModel is None, reason="JudgeSixModel removed in pipeline refactor"
)
...
class TestKernelChainIntegration:
  ...
  @pytest.mark.asyncio
  async def test_full_chain_no_violations(
    self, judge_kernel, audit_kernel, kernel_input_empty
  ):
  ...
  @pytest.mark.asyncio
  async def test_full_chain_critical(
    self, judge_kernel, audit_kernel, kernel_input_critical
  ):
