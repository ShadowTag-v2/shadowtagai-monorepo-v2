...
class TestFinJudge:
  ...
  def test_approved_vendor_with_po_allows(self):
    ...
    assert (
      response.approval_gate.value == "auto"
      or response.approval_gate.value == "finance_director"
    )
  ...
  def test_high_risk_destination_blocks(self):
    ...
    assert response.decision == JudgeDecision.BLOCK
    assert (
      "destination" in response.reasoning.lower()
      or "country" in response.reasoning.lower()
    )
...
class TestCaseJudge:
  ...
  def test_low_value_case_blocks(self):
    ...
    assert response.decision == JudgeDecision.BLOCK
    assert (
      "value" in response.reasoning.lower() or "threshold" in response.reasoning.lower()
    )
  ...
  def test_low_success_probability_blocks(self):
    ...
    assert response.decision == JudgeDecision.BLOCK
    assert (
      "probability" in response.reasoning.lower()
      or "success" in response.reasoning.lower()
    )
...
class TestLawJudge:
  ...
  def test_high_risk_ai_without_legal_review_blocks(self):
    ...
    assert response.decision == JudgeDecision.BLOCK
    assert (
      "high-risk" in response.reasoning.lower()
      or "legal review" in response.reasoning.lower()
    )
  ...
  def test_export_control_restricted_country_blocks(self):
    ...
    assert response.decision == JudgeDecision.BLOCK
    assert (
      response.risk_assessment.severity.value == "I"
    )  # Catastrophic (criminal penalties)
...
class TestFraudJudge:
  ...
  def test_multiple_fraud_indicators_blocks(self):
    ...
    assert response.decision == JudgeDecision.BLOCK
    assert (
      "indicator" in response.reasoning.lower()
      or "multiple" in response.reasoning.lower()
    )
...
class TestCrossJudgeConsistency:
  ...
  def test_all_judges_produce_semantic_trails(self):
    ...
    test_requests = [
      (
        JudgeType.FIN,
        {"amount_usd": 50000, "vendor_status": "new", "purchase_order": None},
      ),
      (
        JudgeType.CASE,
        {
          "case_value_usd": 100000,
          "conflict_check_passed": True,
          "probability_of_success": 0.6,
        },
      ),
      ...
      (
        JudgeType.FRAUD,
        {"fraud_score": 0.8, "identity_verified": False, "amount_usd": 5000},
      ),
    ]
