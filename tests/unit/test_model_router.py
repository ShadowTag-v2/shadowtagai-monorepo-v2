...
class TestClassifyPrompt:
  ...
  def test_complex_query(self):
    ...
    assert (
      classify_prompt("why does this contract clause matter?") == DispatchTier.COMPLEX
    )
  ...
  def test_agentic_query(self):
    ...
    assert (
      classify_prompt("analyze this contract and draft a response memo")
      == DispatchTier.AGENTIC
    )
...
class TestSessionPinning:
  ...
  def test_expired_pin_returns_none(self):
    ...
    _session_pins["old_sess"] = (
      "gemini-pro",
      time.time() - SESSION_PIN_TTL_SECONDS - 1,
    )
  ...
  def test_chain_completeness(self):
    ...
    for model, fallback in FALLBACK_CHAIN.items():
      assert fallback in AVAILABLE_MODELS, (
        f"Fallback {fallback} for {model} not in registry"
      )
