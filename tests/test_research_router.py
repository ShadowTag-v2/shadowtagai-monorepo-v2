...
class TestClassifyQuery:
    ...
    def test_google_cloud(self) -> None:
        assert classify_query("Google Cloud Run best practices") == QuerySource.GOOGLE_DEVELOPER_KNOWLEDGE

    def test_vertex_ai(self) -> None:
        assert classify_query("Vertex AI pipeline") == QuerySource.GOOGLE_DEVELOPER_KNOWLEDGE
    ...
    def test_local_existing(self) -> None:
        assert classify_query("Show me existing implementation") == QuerySource.LOCAL_CODEBASE

    def test_thinking_tradeoff(self) -> None:
        assert classify_query("Compare tradeoffs, break down pros and cons") == QuerySource.SEQUENTIAL_THINKING

    def test_thinking_hypothesis(self) -> None:
        assert classify_query("Verify the hypothesis about caching") == QuerySource.SEQUENTIAL_THINKING
