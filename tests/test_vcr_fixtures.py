...
class TestRateLimitCassetteValidation:
  ...
  def rate_limit_cassette(self) -> dict:
    ...
    fixture_path = (
      Path(__file__).parent / "fixtures" / "vcr" / "bash-rate-limit-10calls.json"
    )
  ...
  def test_all_interactions_have_scrubbed_auth(self, rate_limit_cassette: dict):
    ...
    for interaction in rate_limit_cassette["interactions"]:
      ...
      assert "Bearer [SCRUBBED]" in auth, (
        f"Unscrubbed auth in interaction: {interaction['request'].get('url')}"
      )
  ...
  def test_rate_limit_headers_progressive_decrease(self, rate_limit_cassette: dict):
    ...
    for i in range(6):
      ...
      if i > 0:
        prev_remaining = int(
          rate_limit_cassette["interactions"][i - 1]["response"]["headers"][
            "X-RateLimit-Remaining"
          ]
        )
        assert remaining < prev_remaining, (
          f"Rate limit not decreasing at interaction {i + 1}"
        )
...
class TestCassette:
  ...
  def test_new_episodes_records_new(self, tmp_path: Path):
    ...
    fixture = tmp_path / "test.json"
    cassette = Cassette(
      name="test", fixture_path=fixture, mode=CassetteMode.NEW_EPISODES
    )
  ...
  def test_has_recording(self, tmp_path: Path):
    ...
    assert not cassette.has_recording(
      method="POST", url="https://api.example.com/v1/gen"
    )
    ...
    assert cassette.has_recording(
      method="POST", url="https://api.example.com/v1/gen", body="{}"
    )
...
class TestFixtureManager:
  ...
  def test_test_gate_blocks_outside_test(
  ...
  def test_test_gate_allows_in_pytest(
    self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
  ):
  ...
  def test_test_gate_allows_force_enable(
    self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
  ):
