...
class TestToolPermissions:
    ...
    def test_write_tools_blocked_without_bypass(self, engine: SpeculationEngine, tool_name: str) -> None:
...
class TestSessionSuppression:
    ...
    def test_pending_permission(self) -> None:
        ...
        config = SuggestionConfig()
        assert get_suggestion_suppress_reason(state, config) == SuppressReason.PENDING_PERMISSION

    def test_elicitation_active(self) -> None:
        state = SessionState(elicitation_active=True, assistant_turn_count=3)
        config = SuggestionConfig()
        assert get_suggestion_suppress_reason(state, config) == SuppressReason.ELICITATION_ACTIVE
...
class TestTelemetry:
    def test_log_suggestion_event_no_crash(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_log_speculation_event_no_crash(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("BEADS_DIR", str(tmp_path / ".beads"))
