...
class TestTokenUsage:
    def test_total_input_tokens(self):
        u = TokenUsage(input_tokens=10, cache_read_input_tokens=5, cache_creation_input_tokens=3)
...
class TestIsRecurringTaskAged:
    ...
    def test_permanent(self):
        t = CronTask(id="1", cron="*", prompt="x", created_at=0.0, recurring=True, permanent=True)
        assert not is_recurring_task_aged(t, 999999.0, 1000)
...
class TestMigrateLegacyAttachments:
    def test_new_file(self):
        msg = {"type": "attachment", "attachment": {"type": "new_file", "filename": "/a/b.py"}}
        result = migrate_legacy_attachment_types(msg)
        assert result["attachment"]["type"] == "file"

    def test_new_directory(self):
        msg = {"type": "attachment", "attachment": {"type": "new_directory", "path": "/a/b"}}
        result = migrate_legacy_attachment_types(msg)
...
class TestFilterOrphanedThinking:
    def test_keeps_normal(self):
        msgs = [{"type": "assistant", "message": {"content": [{"type": "text", "text": "hi"}]}}]
        assert len(filter_orphaned_thinking_only_messages(msgs)) == 1

    def test_drops_thinking_only(self):
        msgs = [{"type": "assistant", "message": {"content": [{"type": "thinking", "thinking": "..."}]}}]
