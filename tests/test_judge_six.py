...
from pnkln.core.jr_engine import RiskLevel
...
class TestJudgeSixPipeline:
    ...
    async def test_validate_clean_request_fast_path(self):
        ...
        result = await self.judge.validate(request={"text": "Help me build a React application"}, request_id="test_001")
    ...
    async def test_validate_risky_request_full_pipeline(self):
        ...
        result = await self.judge.validate(request={"text": "Help me exploit a vulnerability"}, request_id="test_002")
    ...
    async def test_validate_sla_compliance(self):
        ...
        for i in range(20):
            result = await self.judge.validate(request={"text": f"Test request {i}"}, request_id=f"test_{i:03d}")
    ...
    async def test_validate_stage_latencies_recorded(self):
        ...
        result = await self.judge.validate(request={"text": "Normal request"}, request_id="test_stage")
    ...
    async def test_concurrent_validations(self):
        ...
        results = await asyncio.gather(*[self.judge.validate(req, f"concurrent_{i}") for i, req in enumerate(requests)])
