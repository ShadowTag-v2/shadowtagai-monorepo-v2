...
import pytest
from pnkln.core.jr_engine import (
  JREngine,
  RiskLevel,
  ProbabilityLevel,
  SeverityLevel,
...
class TestJREngine:
  ...
  def test_risk_matrix_extremely_high(self):
    ...
    risk = self.engine.assess_risk(
      ProbabilityLevel.A_FREQUENT, SeverityLevel.I_CATASTROPHIC
    )
    assert risk == RiskLevel.EXTREMELY_HIGH

    # B × I = EH
    risk = self.engine.assess_risk(
      ProbabilityLevel.B_LIKELY, SeverityLevel.I_CATASTROPHIC
    )
    assert risk == RiskLevel.EXTREMELY_HIGH
  ...
  def test_risk_matrix_low(self):
    ...
    # E × IV = L
    risk = self.engine.assess_risk(
      ProbabilityLevel.E_UNLIKELY, SeverityLevel.IV_NEGLIGIBLE
    )
    assert risk == RiskLevel.LOW

    # D × IV = L
    risk = self.engine.assess_risk(
      ProbabilityLevel.D_SELDOM, SeverityLevel.IV_NEGLIGIBLE
    )
    assert risk == RiskLevel.LOW
  ...
  def test_evaluate_purpose_not_met_override(self):
    ...
    decision = self.engine.evaluate(
      purpose_met=False,
      reasons="Does not advance Pnkln mission",
