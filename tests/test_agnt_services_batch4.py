...
class TestRoughTokenEstimateForBlock:
  def test_string_block(self):
    assert rough_token_estimate_for_block("hello world") == rough_token_estimate(
      "hello world"
    )
  ...
  def test_thinking_block(self):
    ...
    assert rough_token_estimate_for_block(block) == rough_token_estimate(
      "let me reason..."
    )
...
class TestRoughTokenEstimateForMessages:
  def test_assistant_message(self):
    ...
    assert rough_token_estimate_for_messages(msgs) == rough_token_estimate(
      "hello world"
    )
...
class TestShouldEnableThinkingByDefault:
  ...
  def test_override_disabled(self):
    with mock.patch.dict(
      os.environ, {"AGNT_FC_OVERRIDES": "thinking_enabled=false"}, clear=True
    ):
      assert should_enable_thinking_by_default() is False
...
class TestPolicyLimitsService:
  ...
  def test_file_cache_roundtrip(self):
    with tempfile.TemporaryDirectory() as tmp:
      ...
      restrictions = {
        "test_policy": PolicyRestriction(
          allowed=False, reason="blocked", metadata={"org": "acme"}
        ),
      }
