...
from gemini_ingestion_layer.quality import QualityGates, GateStatus
...
def test_relevance_score_threshold_60_percent():
  ...
  relevance_gate_fail = next(
    g for g in result_fail.gates if g.name == "Relevance Score"
  )
...
def test_runtime_efficiency():
  ...
  runtime_gate_warn = next(
    g for g in result_warn.gates if g.name == "Runtime Efficiency"
  )
  ...
  result_fail = gates.evaluate(stats_fail)
  runtime_gate_fail = next(
    g for g in result_fail.gates if g.name == "Runtime Efficiency"
  )
...
def test_source_diversity():
  ...
  result_fail = gates.evaluate(stats_fail)
  diversity_gate_fail = next(
    g for g in result_fail.gates if g.name == "Source Diversity"
  )
