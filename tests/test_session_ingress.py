...
class TestCompaction:
  ...
  async def test_compaction_marker_written(self, tmp_sessions: Path):
    ...
    result = await write_compaction_marker(
      tmp_sessions, "sess-compact", "Summary of compacted content"
    )
    assert result is True
    ...
    after_compact = await get_session_logs(
      tmp_sessions, "sess-compact", after_last_compact=True
    )
...
class TestConcurrency:
  ...
  async def test_concurrent_appends_same_session(self, tmp_sessions: Path):
    ...
    results = await asyncio.gather(
      *[append_session_log(tmp_sessions, "sess-concurrent", entry) for entry in entries]
    )
