...
class MockDocSnapshot:
  ...
  def __init__(
    self, doc_id: str, data: dict[str, Any] | None = None, exists: bool = True
  ):
    self.id = doc_id
    self._data = data or {}
    self.exists = exists
...
class TestCreateSession:
  ...
  async def test_create_persists_session(
    ...
    doc_ref = store.db.collection("sandbox_sessions").document(
...
class TestDecisionAuditTrail:
  ...
  async def test_record_decision_includes_metadata(self):
    ...
    decisions_col.add = AsyncMock(
      return_value=(datetime.now(UTC), MagicMock(id="dec-002"))
    )
