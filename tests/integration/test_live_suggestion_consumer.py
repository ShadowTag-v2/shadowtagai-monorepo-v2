...
class TestLiveGenerationCycle:
    ...
    def _live_generate_fn(msgs, prompt):
      ...
      response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
          system_instruction=(
            "You predict the developer's next action. STRICT FORMAT: Reply with ONLY a single short phrase, 2-12 words."
          ),
          temperature=0.3,
  ...
  def test_tier2_quality_filter_on_live_response(self):
    ...
    assert (
      should_filter_suggestion("Let me help you with that") is not None
    )  # Claude voice
    assert (
      should_filter_suggestion("I'll run the tests for you") is not None
    )  # Claude voice
    ...
    assert (
      should_filter_suggestion("Run tests. Then deploy.") is not None
    )  # Multi-sentence
  ...
  def test_accept_emits_telemetry(self, beads_dir, cache_file, consumer):
    ...
    try:
      ...
      events = [
        json.loads(line) for line in telemetry_file.read_text().strip().split("\n")
      ]
