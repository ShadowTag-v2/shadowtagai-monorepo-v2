...
class TestConnectionManager:
    ...
    @pytest.mark.asyncio()
    async def test_connect_registers(self, manager: ConnectionManager, mock_ws: MagicMock) -> None:
        await manager.connect("session-1", mock_ws)
        assert manager.active_session_count == 1
        mock_ws.accept.assert_awaited_once()

    @pytest.mark.asyncio()
    async def test_disconnect_removes(self, manager: ConnectionManager, mock_ws: MagicMock) -> None:
        await manager.connect("session-1", mock_ws)
        await manager.disconnect("session-1", mock_ws)
        assert manager.active_session_count == 0

    @pytest.mark.asyncio()
    async def test_disconnect_unknown_noop(self, manager: ConnectionManager, mock_ws: MagicMock) -> None:
