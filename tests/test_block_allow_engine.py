...
class TestChainDepth:
  ...
  def test_c3_base64_encoding(self, engine: BlockAllowRuleEngine) -> None:
    ...
    assert any(
      r.verdict == Verdict.BLOCK for r in result.matched_rules if r.rule_id == "C3"
    )
...
class TestAllowExceptions:
  ...
  def test_a1_test_environment_overrides_block(
  ...
  def test_chain_depth_never_overridden_by_allows(
    self, engine: BlockAllowRuleEngine
  ) -> None:
