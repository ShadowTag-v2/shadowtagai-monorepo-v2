...
def dream_workspace(tmp_path: pathlib.Path):
  ...
  for i in range(10):
    (transcript_dir / f"session_{i}.jsonl").write_text(
      json.dumps({"role": "user", "content": f"test message {i}"}) + "\n"
    )
...
class TestDreamGateCascade:
  ...
  def test_fresh_workspace_fires_immediately(
    self, memory_dir: str, transcript_dir: str
  ):
    ...
    result = check_dream_gates(
      memory_dir, transcript_dir, config=AutoDreamConfig(min_hours=0, min_sessions=0)
    )
  ...
  def test_recent_consolidation_blocks(self, memory_dir: str, transcript_dir: str):
    ...
    result = check_dream_gates(
      memory_dir, transcript_dir, config=AutoDreamConfig(min_hours=24.0, min_sessions=0)
    )
  ...
  def test_config_min_sessions_respected(self, memory_dir: str, transcript_dir: str):
    ...
    result = check_dream_gates(
      memory_dir, transcript_dir, config=AutoDreamConfig(min_hours=0, min_sessions=999)
    )
