...
def _mock_live_call(request_body: dict) -> dict:
  ...
  return {
    "candidates": [
      {
        "content": {
          "parts": [{"text": f"Response to: {request_body.get('contents', '')}"}]
        },
        "finishReason": "STOP",
      }
    ],
...
class TestCassette:
  ...
  def test_record_and_lookup(self, tmp_path: Path) -> None:
    ...
    entry = cassette.record(
      request_body=req, response_body=resp, model="gemini-3.1-flash"
    )
...
class TestVCRRecorderReplay:
  ...
  def test_replay_returns_recorded(self, tmp_path: Path) -> None:
    ...
    replayed_response = replay_recorder.intercept(
      req, lambda r: pytest.fail("should not call live")
    )
  ...
  def test_sequential_fallback(self, tmp_path: Path) -> None:
    ...
    response = replay.intercept(
      _sample_request("different"), lambda r: pytest.fail("should not call")
    )
