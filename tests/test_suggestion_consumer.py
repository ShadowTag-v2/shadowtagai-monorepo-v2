...
class TestGetSuggestion:
  ...
  def test_returns_suggestion_when_fresh(
    self, consumer: SuggestionConsumer, cache_dir
  ) -> None:
    _write_cache(
      cache_dir,
      {
  ...
  def test_returns_none_when_filtered(
    self, consumer: SuggestionConsumer, cache_dir
  ) -> None:
    _write_cache(
      cache_dir,
  ...
  def test_returns_none_when_suggestion_null(
    self, consumer: SuggestionConsumer, cache_dir
  ) -> None:
    _write_cache(
      cache_dir,
...
class TestQualityScore:
  ...
  def test_imperative_verb_boost(self) -> None:
    ...
    without_verb = SuggestionEntry(
      text="the failing test needs fixing", timestamp=time.time()
    )
