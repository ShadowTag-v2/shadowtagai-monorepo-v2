...
async def test_dte_baseline_accuracy():
    ...
    # Baseline should be ~60% (simulated)
    assert 0.55 <= accuracy <= 0.65, f"Baseline accuracy {accuracy:.1%} out of expected range (55-65%)"
...
async def test_dte_evolution_convergence():
    ...
    for cycle in range(20):  # More cycles to test stability
        prompt = fusion.generate_prompt(variant_id)
