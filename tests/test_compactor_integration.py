...
def _layer_messages(n: int = 5) -> list[LayerMessage]:
    ...
    msgs = []
    for i in range(n):
        msgs.append(LayerMessage(role="user", content=f"Q{i}: " + "x" * 200, timestamp=time.time() - 600, token_count=60))
        msgs.append(LayerMessage(role="assistant", content=f"A{i}: " + "y" * 200, timestamp=time.time() - 500, token_count=60))
    return msgs


def _mock_summarize(summary: str = "This is a comprehensive summary of the conversation."):
        ...
        def _failing_then_ok(messages, prompt):
            ...
            if call_count == 1:
                ...
                raise PromptTooLongError(
                    "too long",
                    response={"role": "assistant", "content": "Prompt exceeded limit by 5000 tokens"},
                )
...
class TestCrossModuleIntegration:
    ...
    def test_full_pipeline_flow(self, tmp_path):
        ...
        assert cc_result.tokens_after < cc_result.tokens_before or cc_result.tokens_after > 0
