...
import pytest
import asyncio
from pnkln.core.monte_carlo_risk import MonteCarloRiskAssessment, MonteCarloResult
from pnkln.core.jr_engine import RiskLevel, ProbabilityLevel, SeverityLevel
...
class TestMonteCarloRiskAssessment:
  ...
  async def test_evaluate_scenarios_performance(self):
    """Test that evaluation meets <500μs target."""
    decision = {"text": "Performance test decision"}
    result = await self.assessor.evaluate_scenarios(decision)

    assert result.execution_time_us < 10000, (
      f"Execution took {result.execution_time_us:.1f}μs (target <10000μs)"
    )
  ...
  async def test_batch_evaluations(self):
    ...
    results = await asyncio.gather(
      *[self.assessor.evaluate_scenarios(d) for d in decisions]
    )
