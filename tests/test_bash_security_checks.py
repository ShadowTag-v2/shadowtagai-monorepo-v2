...
class TestCommandSubstitutionPatterns:
  ...
  def test_pattern_detects_attack(
    self, attack_input: str, expected_msg_fragment: str
  ) -> None:
    ...
    for pattern, msg in COMMAND_SUBSTITUTION_PATTERNS:
      if pattern.search(attack_input):
        if expected_msg_fragment in msg:
          ...
          break
    assert matched, (
      f"No pattern matched input={attack_input!r} with fragment={expected_msg_fragment!r}"
    )
...
class TestBashTelemetryTrackerSecurity:
  ...
  def test_track_security_check_failed_emits_event(
    self, tracker: BashTelemetryTracker
  ) -> None:
  ...
  def test_track_security_check_failed_all_ids(
    self, tracker: BashTelemetryTracker
  ) -> None:
  ...
  def test_track_security_validated_emits_event(
    self, tracker: BashTelemetryTracker
  ) -> None:
