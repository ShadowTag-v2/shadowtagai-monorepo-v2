...
class JudgeRequest(BaseModel):
    ...
    context: dict[str, Any] | None = Field(None, description="Additional context for assessment")
...
class JudgeResponse(BaseModel):
    ...
    risk_level: str = Field(..., description="catastrophic, critical, moderate, low, negligible")
    confidence: float = Field(..., ge=0.0, le=1.0, description="Confidence score 0-1")
    reasoning: str = Field(..., description="Human-readable explanation")
    violated_rules: list[str] = Field(default_factory=list, description="List of violated rules")
...
class PolicyCreate(BaseModel):
    ...
    brakes: list[str] = Field(..., description="ATP 5-19 Brakes: What are the hard stops?")
