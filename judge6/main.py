...
async def get_usage(
  ...
  return {
    ...
    "percentage_used": (user.current_month_usage / user.monthly_request_limit * 100)
    if user.monthly_request_limit > 0
    else 0,
  }
...
async def create_checkout_session(
  ...
  if tier not in ["starter", "professional"]:
    raise HTTPException(
      status_code=400, detail="Invalid tier. Must be 'starter' or 'professional'"
    )
  ...
  if not price_id:
    raise HTTPException(
      status_code=500, detail=f"Price ID not configured for {price_key}"
    )
