...
class Export(base.Command):
  ...
  def Args(parser):
    resource_args.AddEnvironmentResourceArg(
      parser, "from whose Cloud Storage bucket to export data.", positional=False
    )
  ...
  def Run(self, args):
    ...
    source_path = posixpath.join(
      Export.SUBDIR_BASE, (args.source or "*").strip(posixpath.sep)
    )
    return storage_util.Export(
      env_ref, source_path, args.destination, release_track=self.ReleaseTrack()
    )
