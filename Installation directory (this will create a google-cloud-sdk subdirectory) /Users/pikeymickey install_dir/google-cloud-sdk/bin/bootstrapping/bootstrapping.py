...
if "." in __name__:
  ...
  pass  # pylint:disable=g-import-not-at-top
else:
...
def ExecuteJavaClass(
  java_bin, jar_dir, main_jar, main_class, java_flags=None, main_args=None
):
...
def ExecuteShellTool(tool_dir, exec_name, *args):
  ...
  """
  _ExecuteTool(
    execution_utils.ArgsForExecutableTool(_FullPath(tool_dir, exec_name), *args)
  )
...
def WarnAndExitOnBlockedCommand(args, blocked_commands):
  ...
  if blocked:
    sys.stderr.write(
      'It looks like you are trying to run "%s %s".\n' % (args[0], bad_arg)
    )
    sys.stderr.write(
      'The "%s" command is no longer needed with Google Cloud CLI.\n' % bad_arg
    )
