...
def _MaybeAddBotoOption(args, section, name, value):
  ...
  args.append("-o")
  args.append(
    "{section}:{name}={value}".format(section=section, name=name, value=value)
  )
...
def _GetCertProviderCommand(context_config):
  ...
  try:
    ...
    log.debug(
      "context aware settings discovery file %s - %s", context_config.config_path, e
    )
...
def _AddContextAwareOptions(args):
  ...
  if (
    context_config
    and context_config.config_type == context_aware.ConfigType.ENTERPRISE_CERTIFICATE
  ):
    return
  ...
  _MaybeAddBotoOption(
    args, "Credentials", "use_client_certificate", use_client_certificate
  )
  if context_config:
    ...
    _MaybeAddBotoOption(
      args, "Credentials", "cert_provider_command", cert_provider_command
    )
...
def main():
  ...
  encoding.SetEncodedValue(os.environ, "GA_CID", metrics.GetCIDIfMetricsEnabled())
