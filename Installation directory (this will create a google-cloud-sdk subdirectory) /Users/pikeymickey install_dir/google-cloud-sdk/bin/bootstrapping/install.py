...
def ParseArgs():
  ...
  parser.add_argument(
    ...
    "-q",
    default=None,
    action=actions.StoreConstProperty(properties.VALUES.core.disable_prompts, True),
    help=(
  ...
  parser.add_argument(
    "--no-compile-python",
    action="store_false",
    help=("False. If set, skips python compilation after component installation."),
  )
...
def Prompts(usage_reporting, universe_domain):
  ...
  if usage_reporting is None:
    if (
      encoding.GetEncodedValue(os.environ, "CLOUDSDK_CORE_DISABLE_USAGE_REPORTING")
      is not None
    ):
...
""")
  ...
  properties.PersistProperty(
    properties.VALUES.core.disable_usage_reporting,
    not usage_reporting,
...
def Install(
  ...
  # components.
  if update_installed_components and (installed_components := GetInstalledComponents()):
    to_install.extend(installed_components)

  if additional_components:
    to_install.extend([c for c in additional_components if c not in REMOVED_COMPONENTS])
...
def main():
  ...
  if pargs.screen_reader is not None:
    properties.PersistProperty(
      properties.VALUES.accessibility.screen_reader,
      pargs.screen_reader,
      scope=properties.Scope.INSTALLATION,
    )
