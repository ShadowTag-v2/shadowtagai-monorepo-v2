...
# Should respond with Pnkln architecture details
```
...
# 3. Test in notebook
print(pnkln_memory['version'])
print(pnkln_memory['pnkln_architecture']['judge_6'])
...
**Auto-load**: IPython startup script at `~/.ipython/profile_default/startup/00-load-pnkln-memory.py`
...
kubectl logs pnkln-worker-example -c sync-memory
...
### Vertex AI Workbench
```bash
export GOOGLE_CLOUD_PROJECT=pnkln-prod  # Your GCP project
...
- [ ] File contains Pnkln architecture sections
...
- [ ] Variable `pnkln_memory` available
...
### "Vertex Workbench: pnkln_memory not found"
**Cause**: IPython startup script not executed
**Fix**:
```bash
# Check startup script
cat ~/.ipython/profile_default/startup/00-load-pnkln-memory.py
...
4. **Test**: Ask Claude Code about Pnkln architecture
...
**Contact**: Erik Hancock (Pnkln CEO)
