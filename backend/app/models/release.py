...
class Release(Base):
  ...
  deployments = relationship(
    "Deployment", back_populates="release", cascade="all, delete-orphan"
  )
...
class Deployment(Base):
  ...
  rollback_deployment = relationship(
    "Deployment", remote_side=[id], foreign_keys=[rollback_deployment_id]
  )
  ...
  def __repr__(self) -> str:
    return (
      f"<Deployment(id={self.id}, release_id={self.release_id}, status={self.status})>"
    )
...
class FeatureFlag(Base):
  ...
  # Status
  status = Column(
    Enum(FeatureFlagStatus), default=FeatureFlagStatus.DISABLED, nullable=False
  )
