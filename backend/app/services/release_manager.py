...
class ReleaseManagerService:
  ...
  async def list_releases(
    ...
    count_result = await db.execute(
      select(Release).where(*query.whereclause.clauses if query.whereclause else [])
    )
  ...
  async def create_deployment(
    ...
    logger.info(
      f"Created deployment: {deployment.id} for release: {deployment.release_id}"
    )
    return deployment
  ...
  async def list_deployments(
    ...
    # Get total count
    count_result = await db.execute(
      select(Deployment).where(*query.whereclause.clauses if query.whereclause else [])
    )
    total = len(count_result.all())
  ...
  async def get_latest_deployment(
    ...
    query = (
      ...
      .order_by(desc(Deployment.started_at))
      .limit(1)
    )
  ...
  async def _deploy_blue_green(
    ...
    deployment.logs += (
      "Traffic switched successfully. Blue-Green deployment complete.\n"
    )
    await db.commit()
  ...
  async def _deploy_canary(
    ...
    deployment.logs = (
      f"Starting canary deployment with {canary_percentage}% traffic...\n"
    )
    await db.commit()
  ...
  async def rollback_deployment(
    ...
    try:
      ...
      rollback_deployment.logs = (
        f"Rollback initiated. Reason: {reason or 'Not specified'}\n"
      )
      rollback_deployment.logs += (
        f"Rolling back to release {previous_deployment.release_id}...\n"
      )
      await db.commit()
