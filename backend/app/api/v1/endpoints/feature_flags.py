...
async def create_feature_flag(
  ...
  if existing:
    raise HTTPException(
      status_code=400, detail=f"Feature flag with key '{flag_data.key}' already exists"
    )
...
async def enable_feature_flag(
  flag_key: str,
  percentage: int | None = Query(
    None, ge=0, le=100, description="Percentage rollout (0-100)"
  ),
  db: AsyncSession = Depends(get_db),
) -> FeatureFlagResponse:
  """
