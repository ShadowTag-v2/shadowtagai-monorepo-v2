...
async def create_release(
  ...
  existing = await release_manager_service.get_release_by_version(
    db, release_data.version
  )
  if existing:
    raise HTTPException(
      status_code=400, detail=f"Release version {release_data.version} already exists"
    )
