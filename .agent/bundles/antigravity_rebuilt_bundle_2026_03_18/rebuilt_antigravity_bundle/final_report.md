...
- A list of 56 (not 57) repos, mostly prefixed with `pnkln-` plus `pnkln`, `erik-hancock-llm-memory`, and several public demos (e.g. `antigravity-go`, `Cor.Claude_Code_6`).
...
4. **Classify and Fold Repos**:
   ...
   - Prioritise `pnkln-*` repos, `pnkln`, `erik-hancock-llm-memory`, `cosmic-crab-payload`, `Pipeline`, and `nascent-apollo` for early canonicalization.
