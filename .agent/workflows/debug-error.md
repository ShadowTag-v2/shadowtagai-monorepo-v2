...
- Use the `systematic-debugging` skill for complex bugs (3+ failed fixes = architectural problem)
...
| ImportError (Python) | Virtual env mismatch, circular imports |
| OOMError | Context window overflow, memory leak |

### 4. Investigate the Code
// turbo
Look at the relevant code:
- Read the file and line mentioned in the error
- Check recent changes if it's a regression (`git log -5 -- <file>`)
...
**Verification is mandatory** — no fix is done until fresh evidence confirms it works (Rule 01).
...
- After 3 failed fix attempts, escalate to architectural review
