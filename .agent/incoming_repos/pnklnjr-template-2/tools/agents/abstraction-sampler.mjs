...
async function loadTasks(path='ops/tasks.jsonl'){
  try { const text = await fs.readFile(path,'utf8'); return text.trim().split('\n').map(JSON.parse) } catch { return [] }
}
...
function scoreAbstraction(abs){
  ...
  const weights = { 'functional-decomp': 0.9, 'data-flow': 0.8, 'property-based': 0.85, 'spec-first': 0.75 }
  return { ...abs, score: weights[abs.name] || 0.7 }
}

function sampleSolutions(abs, count){
  const sols = []
  for (let i=0;i<count;i++) sols.push({ abstraction: abs.name, rank: i+1, hint: abs.hint, text: `Solution ${i+1} via ${abs.name}` })
  return sols
}
...
async function main(){
  ...
  for (const t of tasks){
    const abs = generateAbstractions(t).map(scoreAbstraction).sort((a,b)=>b.score-a.score).slice(0, topK)
    for (const a of abs){ results.push(...sampleSolutions(a, perAbstraction)) }
  }
  await fs.mkdir('ops/.telemetry', { recursive: true })
  await fs.writeFile('ops/.telemetry/abstraction-samples.json', JSON.stringify({ budget, topK, results }, null, 2))
