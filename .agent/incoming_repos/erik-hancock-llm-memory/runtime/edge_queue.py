  ...
  def wait(self, value: int, timeout_ms: int = 30000):
    ...
    while self.value < value:
      ...
      if elapsed_ms > timeout_ms:
        raise TimeoutError(
          f"Signal wait timeout: {timeout_ms}ms exceeded. Expected value={value}, got value={self.value}"
        )
...
class EdgeQueue:
  ...
  def timestamp(self, signal: EdgeSignal) -> "EdgeQueue":
    ...
    self.commands.append(
      EdgeCommand(type="timestamp", args={"signal_id": signal.do_id})
    )
    return self
  ...
  def submit(self, worker_url: str) -> dict[str, Any]:
    ...
    payload = {
      "commands": [{"type": cmd.type, "args": cmd.args} for cmd in self.commands]
    }
