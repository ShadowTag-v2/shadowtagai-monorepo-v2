...
**Lesson:** pnkln Core Stack components (Judge #6, JR Engine, Cor, ShadowTagAI) should be feature-gated for deployment flexibility.
...
### ShadowTagAI 2.0 Watermark Viewer
```rust
// TUI for inspecting ShadowTagAI watermarks in video streams

struct ShadowTagAIViewer {
    decoder: ShadowTagAIDecoder,
    frames: VecDeque<WatermarkFrame>,
}

impl ShadowTagAIViewer {
    fn render(&self, frame: &mut Frame, area: Rect) {
...
- ⚠️ **Consider egui** if GPU acceleration required (e.g., ShadowTagAI video preview)
