...
"""
Claude_Code_6 Real Engine Integration - Connect TUI to Actual Governance

This module bridges the mock engine to your real Claude_Code_6 stack:
- Cor_Claude_Code_6/renderer/jr_engine.py (PolicyUOp → WASM)
- Cor_Claude_Code_6/runtime/* (WASM execution)

USAGE:
    python3 tui/Cor_Claude_Code_6_monitor.py --mode=live
"""
...
sys.path.insert(0, "/Users/pikeymickey/Documents/Claude Code/Code/Claude Demo/pnkln-stack-fastapi-services/erik-hancock-llm-memory")

from Cor_Claude_Code_6.renderer.jr_engine import ATP519Patterns, JREngineRenderer
from Cor_Claude_Code_6.uop import PolicyOps, PolicyUOp

# Import from Cor_Claude_Code_6_monitor.py
from tui.Cor_Claude_Code_6_monitor import DecisionMetric


class RealCor_Claude_Code_6Engine:
    """Production Claude_Code_6 engine with actual policy evaluation
    ...
    def __init__(self):
        ...
        print(f"[Claude_Code_6] Policy compiled to WASM:\n{self.wasm_text[:200]}...")
    ...
    async def make_decision(self, context: dict | None = None) -> DecisionMetric:
        ...
        try:
            ...
            return DecisionMetric(timestamp=time.time(), latency_us=latency_us, result="ERROR", violation_type=f"Exception: {str(e)}")
...
def get_engine(mode: str = "mock"):
    ...
    Returns:
        Engine instance (MockCor_Claude_Code_6Engine or RealCor_Claude_Code_6Engine)
    """
    if mode == "live":
        return RealCor_Claude_Code_6Engine()
    else:
        from tui.Cor_Claude_Code_6_monitor import MockCor_Claude_Code_6Engine

        return MockCor_Claude_Code_6Engine()


if __name__ == "__main__":
    # Test the real engine
    async def test():
        engine = RealCor_Claude_Code_6Engine()

        print("\n🧪 Testing RealCor_Claude_Code_6Engine with 10 decisions...\n")
