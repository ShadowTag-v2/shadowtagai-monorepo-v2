export async function hello(event) {
  const message = 'pnkln-stack serverless starter online';
  return {
    statusCode: 200,
...
export async function insertThought(event) {
  ...
  try {
    ...
    if (!Array.isArray(embedding) || embedding.length !== dim || typeof label !== 'string') {
      return { statusCode: 400, body: JSON.stringify({ error: 'Invalid input: label and embedding[dim] required' }) };
    }
    ...
    await client.end();
    return { statusCode: 201, body: JSON.stringify({ id: res.rows[0]?.id, createdAt: res.rows[0]?.created_at }) };
  } catch (err) {
