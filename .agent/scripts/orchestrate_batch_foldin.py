...
REPOS = [
  ...
  (
    "ehanc69/pnkln-stack-objections-decisions",
    "governance/pnkln-stack-objections-decisions",
  ),
...
for git_source, dest_path in REPOS:
  ...
  if status == "blocked":
    markdown_table += (
      f"| {repo_name} | BLOCKED | {dest_path} | - | {blockers} | Fail |\n"
    )
    continue
  ...
  try:
    ...
    insert_idx = next(
      (
        i + 1 for i, line in enumerate(manifest_lines) if line.startswith("repo_roots:")
      ),
      -1,
    )
  ...
  markdown_table += (
    f"| {repo_name} | canonical | {dest_path} | resolved | None | Success |\n"
  )
