...
CORPUS_DBS: list[Path] = [
    ...
    REPO_ROOT / "data/alphaxiv/ingest.db",  # live — arXiv research via alphaXiv MCP (mcp SDK)
]
...
_PUBLIC_DOMAIN_PATTERNS = re.compile(
    r"NIST|ATP_|ARN\d|HIPAA|PCI.DSS|IRS.Pub|IRC.\d|ISO_IEC|SSUG|" r"Open.Source.Initiative|auditing_standards|army\.mil",
    re.IGNORECASE,
)
...
def _ensure_fts5(conn: sqlite3.Connection) -> bool:
    ...
    c = conn.cursor()
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='extractions_fts'")
    if c.fetchone():
        return True
    try:
        c.execute("CREATE VIRTUAL TABLE extractions_fts USING fts5(text, name, class, content='extractions', content_rowid='id')")
...
def format_context(extractions: list[dict]) -> str:
    ...
    for e in extractions:
        tag = "[SYNTH-ONLY — do not quote]" if e["copyrighted"] else "[PUBLIC DOMAIN — citable]"
...
def check_copyright_verbatim(output: str, extractions: list[dict]) -> list[str]:
    ...
    for e in extractions:
        ...
        if src_ngrams and len(overlap) / len(src_ngrams) > NGRAM_THRESHOLD:
            warnings.append(f"Verbatim overlap with copyrighted source: {e['name']} ({len(overlap)} n-gram matches)")
    return warnings
...
def _classify_changed_docs(diff: str) -> list[str]:
    ...
    patterns = re.compile(
        r"(\.md$|bizplan|PNKLN|AGENTS|REVIEW|icloud_notes|docs/|" r"legaltrack|cosmic-crab|uphillsnowball)",
        re.IGNORECASE,
    )
...
def run_post_ingest(source_label: str) -> None:
    ...
    new_rows = sum(current_counts.get(k, 0) - last_counts.get(k, 0) for k in current_counts)
...
def main() -> None:
    ...
    parser.add_argument("--gatekeeper", action="store_true", help="Judge 6 gate on git diff")
    parser.add_argument("--analyze", action="store_true", help="Bulk biz plan x corpus analysis")
    parser.add_argument("--loop", action="store_true", help="Daily delta loop mode")
    parser.add_argument("--post-ingest", metavar="SOURCE", help="Post-ingest trigger (drive|web|downloads)")
    parser.add_argument("--evolve", type=str, metavar="FILE", help="Evolve a single document")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite instead of _evolved suffix")
