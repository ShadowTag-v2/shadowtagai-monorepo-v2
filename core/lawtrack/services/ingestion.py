...
async def ingest_email_event(payload: EmailWebhookPayload, background_tasks: BackgroundTasks):
    """
    ...
    if not payload.matter_id or not payload.tenant_id:
        raise HTTPException(status_code=400, detail="Matter and Tenant ID required for routing.")

    background_tasks.add_task(parse_and_route_email, payload)

    return {"status": "queued", "message": "Email payload accepted for parsing and timeline generation."}
