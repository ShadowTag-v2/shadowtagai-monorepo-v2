...
def build_cache(
  model: str = DEFAULT_MODEL, ttl_hours: int = DEFAULT_TTL_HOURS
) -> dict | None:
  ...
  logger.info(
    "Building cache slab (%d chars) for model %s...", len(slab_content), model
  )
...
def main() -> None:
  ...
  parser.add_argument(
    "--ttl", type=int, default=DEFAULT_TTL_HOURS, help="Cache TTL in hours"
  )
