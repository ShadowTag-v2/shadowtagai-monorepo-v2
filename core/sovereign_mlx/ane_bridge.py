    ...
    def _infer(self, task: ANETask, slab_path: Path) -> ANEResult:
        if not shutil.which(LLAMA_CLI):
            return ANEResult(
                ...
                tier=task.tier,
                text="",
                error=(f"llama-cli not found at '{LLAMA_CLI}'. Install llama.cpp: cmake -DLLAMA_METAL=on .."),
            )
        ...
        try:
            ...
            logger.debug("[%s/%s] %.0fms → %d chars", task.tier, task.name, latency_ms, len(output))
            return ANEResult(
                task_name=task.name,
                tier=task.tier,
                text=output,
