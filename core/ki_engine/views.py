...
def render_decisions(kis: list[KIMetadata]) -> str:
  ...
  decisions = [
    ki for ki in kis if ki.ki_type == KIType.DECISION and ki.status == KIStatus.ACTIVE
  ]
...
def render_constraints(kis: list[KIMetadata]) -> str:
  ...
  constraints = [
    ki for ki in kis if ki.ki_type == KIType.CONSTRAINT and ki.status == KIStatus.ACTIVE
  ]
...
def render_conflicts(kis: list[KIMetadata]) -> str:
  ...
  conflicts = [
    ki for ki in kis if ki.ki_type == KIType.CONFLICT and ki.status == KIStatus.ACTIVE
  ]
...
def render_handoff(
  ...
  if critical:
    ...
    for rki in critical:
      ki_type = (
        rki.ki.ki_type.value
        if hasattr(rki.ki.ki_type, "value")
        else str(rki.ki.ki_type)
      )
  ...
  other = [
    r
    for r in ranked
    if r.ki.ki_type not in (KIType.CONFLICT, KIType.DECISION, KIType.CONSTRAINT)
  ]
  if other:
    ...
    for rki in other:
      ...
      lines.append(
        f"- [{ki_type}] **{rki.ki.name}** (score: {rki.score:.2f}) — {rki.ki.summary}"
      )
    lines.append("")
