...
def infer_ki_type(ki: dict) -> KIType:
  ...
  # Decision indicators
  if any(
    w in name_lower for w in ["decision", "chose", "selected", "verdict", "approved"]
  ):
  ...
  if any(
    w in name_lower
    for w in ["constraint", "rule", "forbidden", "must", "never", "always"]
  ):
    return KIType.CONSTRAINT

  # Procedure indicators
  if any(
    w in name_lower for w in ["how to", "procedure", "workflow", "steps", "guide"]
  ):
  ...
  if any(
    w in name_lower for w in ["question", "todo", "investigate", "unclear", "unknown"]
  ):
  ...
  # Belief indicators
  if any(
    w in name_lower for w in ["believe", "hypothesis", "suspect", "think", "seems"]
  ):
    return KIType.BELIEF

  # Conflict indicators
  if any(
    w in name_lower for w in ["conflict", "contradiction", "inconsistent", "mismatch"]
  ):
