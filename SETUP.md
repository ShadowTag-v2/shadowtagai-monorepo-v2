# PNKLN Intelligence Pipeline - Setup Guide

This guide walks you through setting up the PNKLN Intelligence Pipeline from scratch.
...
gcloud iam service-accounts create pnkln-pipeline \
    --display-name="PNKLN Intelligence Pipeline"

# Grant necessary permissions
gcloud projects add-iam-policy-binding YOUR_PROJECT_ID \
    --member="serviceAccount:pnkln-pipeline@YOUR_PROJECT_ID.iam.gserviceaccount.com" \
...
gcloud iam service-accounts keys create ~/pnkln-sa-key.json \
    --iam-account=pnkln-pipeline@YOUR_PROJECT_ID.iam.gserviceaccount.com

# Set environment variable
export GOOGLE_APPLICATION_CREDENTIALS=~/pnkln-sa-key.json
```
...
EMBEDDING_OPENAI_API_KEY=sk-your-actual-openai-key
...
```bash
python -m pnkln_intelligence.scripts.init_infrastructure
```
...
INFO: Creating GCS bucket: pnkln-code-storage-prod
...
```bash
# List buckets
gsutil ls gs://pnkln-code-storage-prod/

# Should show directory structure:
# gs://pnkln-code-storage-prod/raw/
# gs://pnkln-code-storage-prod/processed/
# gs://pnkln-code-storage-prod/models/
# gs://pnkln-code-storage-prod/indexes/
# gs://pnkln-code-storage-prod/logs/
```
...
python -m pnkln_intelligence.scripts.ingest_repositories --priority critical --limit 5
```
...
python -c "
import asyncio
from pnkln_intelligence.aggregators import HackerNewsAggregator
...
async def create():
    ...
    index = await manager.create_index(
        display_name='pnkln-code-search-v1',
    ...
    endpoint = await manager.create_index_endpoint(
        display_name='pnkln-code-endpoint-v1'
    )
...
gcloud projects get-iam-policy YOUR_PROJECT_ID \
    ...
    --filter="bindings.members:pnkln-pipeline@YOUR_PROJECT_ID.iam.gserviceaccount.com"
```
...
```bash
GCP_GCS_BUCKET_RAW=pnkln-code-storage-prod-YOUR_UNIQUE_ID
```
...
1. **Scale Ingestion**: Ingest all 70 repositories
   ```bash
   python -m pnkln_intelligence.scripts.ingest_repositories
   ```
